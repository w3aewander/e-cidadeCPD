...
$oAnexoSumario->setEsfera($oGet->esfera);
...
if (isRioGrandeDoNorte()) {
    $complemento = "- ESFERA: ".($oGet->esfera == 1 ? "FISCAL" : ($oGet->esfera == 2 ? "SEGURIDADE" : "TOTAL"));
}
$head2 = "SUMÁRIO GERAL DA RECEITA POR FONTES E DESPESA POR FUNÇÕES DO GOVERNO {$complemento}";
...
$oPdf = new PDF((isset($oGet->orientacao_pagina)?$oGet->orientacao_pagina:"P"));
$oPdf->addTitulo($head1);
$oPdf->addTitulo($head2);
$oPdf->addTitulo($head3);
$oPdf->addTitulo($head4);
$oPdf->addTitulo($head5);
$oPdf->addTitulo($head6);
$oPdf->AliasNbPages();
$oPdf->SetAutoPageBreak(true);
$oPdf->setfillcolor(235);
$oPdf->setfont('arial', 'b', 7);
$oPdf->init(false, true, ((isset($mostrar_rodape) && $mostrar_rodape == "N")?false:true));
...
$oPdf->setfont('arial', 'B', $iFonte);
$oPdf->cell($iColunaDescr, $iAlt, setIdentacao($aDadosSumarioGeral[26]->nivellinha).$aDadosSumarioGeral[26]->descricao, 0, 0, "L", 0);
$oPdf->cell($iColunaValor, $iAlt, db_formatar($aDadosSumarioGeral[26]->total, 'f'), 0, 1, "R", 0);
