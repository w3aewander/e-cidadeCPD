...
class M25070NovaRegraTrienioQuinquenioProgressao extends Migration
{
    /**
     * Run the migrations.
     *
...
DECLARE
    ...
    TOTAL_DIAS_POSTERGA_TRIENIO    INTEGER := 0;
    TOTAL_DIAS_AVERBA_TRIENIO      INTEGER := 0;
    DIAS_DIREITO_TRIENIO           INTEGER := 0;
    DIAS_DIREITO_AVERBA_TRIENIO    INTEGER := 0;
    DIAS_DIREITO_POSTERGA_TRIENIO  INTEGER := 0;

    TOTAL_DIAS_POSTERGA_PROGRESS   INTEGER := 0;
    TOTAL_DIAS_AVERBA_PROGRESS     INTEGER := 0;
    DIAS_DIREITO_PROGRESS          INTEGER := 0;
    DIAS_DIREITO_AVERBA_PROGRESS   INTEGER := 0;
    DIAS_DIREITO_POSTERGA_PROGRESS INTEGER := 0;
    ...
    R_DEPEND          RECORD;
    R_FERIAS          RECORD;
    R_DATA_TRIENIO    RECORD;
    R_DATA_PROGRESSAO RECORD;
...
FOR R_DATA_TRIENIO IN
          SELECT H19_TIPO, H19_DIA01, H19_OP01, H19_PER01, H16_QUANT, RH01_ADMISS, RH01_TRIENIO
          FROM PROTELAC
          ...
          INNER JOIN RHPESSOAL ON RH01_REGIST = H16_REGIST
           WHERE H16_REGIST = REGISTRO 
           AND H19_TIPO = 'T'
           AND H19_INSTIT = INSTIT
   LOOP
          -- Cálculos da diferença
          TOTAL_DIAS_POSTERGA_TRIENIO := CASE WHEN R_DATA_TRIENIO.H19_OP01 = '+' THEN R_DATA_TRIENIO.H16_QUANT ELSE 0 END;
          TOTAL_DIAS_AVERBA_TRIENIO := CASE WHEN R_DATA_TRIENIO.H19_OP01 = '-' THEN R_DATA_TRIENIO.H16_QUANT ELSE 0 END;
  
          DIAS_DIREITO_AVERBA_TRIENIO := DIAS_DIREITO_AVERBA_TRIENIO - TOTAL_DIAS_AVERBA_TRIENIO;
          DIAS_DIREITO_POSTERGA_TRIENIO := DIAS_DIREITO_POSTERGA_TRIENIO + TOTAL_DIAS_POSTERGA_TRIENIO;
  
          DIAS_DIREITO_TRIENIO := DIAS_DIREITO_AVERBA_TRIENIO + DIAS_DIREITO_POSTERGA_TRIENIO;
          
          -- Calcular a nova data de triênio
          R_PES.R01_TRIEN := R_DATA_TRIENIO.RH01_TRIENIO + DIAS_DIREITO_TRIENIO;

          -- Exibir os valores calculados
          --raise notice 'Soma Quantidades: %', R_DATA_TRIENIO.H16_QUANT;
          --raise notice 'Total Dias Posterga: %', TOTAL_DIAS_POSTERGA_TRIENIO;
          --raise notice 'Total Dias Averba: %', TOTAL_DIAS_AVERBA_TRIENIO;
          --raise notice 'Data de Admissão: %', R_DATA_TRIENIO.RH01_ADMISS;
          --raise notice 'Data de Triênio: %', R_DATA_TRIENIO.RH01_TRIENIO;
          --raise notice 'Dias de Direito Averba: %', DIAS_DIREITO_AVERBA_TRIENIO;
          --raise notice 'Dias de Direito Posterga: %', DIAS_DIREITO_POSTERGA_TRIENIO;
          --raise notice 'Dias de Direito Triênio: %', DIAS_DIREITO_TRIENIO;
          --raise notice 'Nova Data de Triênio: %', R_PES.R01_TRIEN;
  
END LOOP;

-- Atualizar o campo rh01_trienio na tabela rhpessoal com a nova data
UPDATE RHPESSOAL SET RH01_TRIENIO = R_PES.R01_TRIEN WHERE RH01_REGIST = REGISTRO;
  
FOR R_DATA_PROGRESSAO IN
            SELECT H19_TIPO, H19_DIA01, H19_OP01, H19_PER01, H16_QUANT, RH01_ADMISS, RH01_PROGRES
            FROM PROTELAC
            INNER JOIN TIPOASSE ON CAST(H12_CODIGO AS VARCHAR) = H19_ASSENT
            INNER JOIN ASSENTA ON H12_CODIGO = H16_ASSENT
            INNER JOIN RHPESSOAL ON RH01_REGIST = H16_REGIST
   ...
   LOOP
         -- Cálculos da diferença
         TOTAL_DIAS_POSTERGA_PROGRESS := CASE WHEN R_DATA_PROGRESSAO.H19_OP01 = '+' THEN R_DATA_PROGRESSAO.H16_QUANT ELSE 0 END;
         TOTAL_DIAS_AVERBA_PROGRESS := CASE WHEN R_DATA_PROGRESSAO.H19_OP01 = '-' THEN R_DATA_PROGRESSAO.H16_QUANT ELSE 0 END;

         DIAS_DIREITO_AVERBA_PROGRESS := DIAS_DIREITO_AVERBA_PROGRESS - TOTAL_DIAS_AVERBA_PROGRESS;
         DIAS_DIREITO_POSTERGA_PROGRESS := DIAS_DIREITO_POSTERGA_PROGRESS + TOTAL_DIAS_POSTERGA_PROGRESS;

         DIAS_DIREITO_PROGRESS := DIAS_DIREITO_AVERBA_PROGRESS + DIAS_DIREITO_POSTERGA_PROGRESS;

         -- Calcular a nova data de triênio
         R_PES.R01_ANTER := R_DATA_PROGRESSAO.RH01_PROGRES + DIAS_DIREITO_PROGRESS;

         -- Exibir os valores calculados
         --raise notice 'Soma Quantidades: %', R_DATA_PROGRESSAO.H16_QUANT;
         --raise notice 'Total Dias Posterga: %', TOTAL_DIAS_POSTERGA_PROGRESS;
         --raise notice 'Total Dias Averba: %', TOTAL_DIAS_AVERBA_PROGRESS;
         --raise notice 'Data de Admissão: %', R_DATA_PROGRESSAO.RH01_ADMISS;
         --raise notice 'Data de Progressão: %', R_DATA_PROGRESSAO.RH01_PROGRES;
         --raise notice 'Dias de Direito Averba: %', DIAS_DIREITO_AVERBA_PROGRESS;
         --raise notice 'Dias de Direito Posterga: %', DIAS_DIREITO_POSTERGA_PROGRESS;
         --raise notice 'Dias de Direito: %', DIAS_DIREITO_PROGRESS;
         --raise notice 'Nova Data de Progressão: %', R_PES.R01_ANTER;
      
END LOOP;

-- Atualizar o campo rh01_progres na tabela rhpessoal com a nova data
UPDATE RHPESSOAL SET RH01_PROGRES = R_PES.R01_ANTER WHERE RH01_REGIST = REGISTRO;
