...
if(pg_num_rows($rsSql) > 0){
    ...
    $oDado->complemento = "{$complemento->getCodigo()} - {$complemento->getDescricao()}";
  ...
  if(pg_num_rows($rsSql) > 0){
		$oRelatorio->historico = db_utils::getCollectionByRecord($rsSql,false,false,false);
 	}
}else{
...
$pdf->SetFont('Arial','',8);
$pdf->Cell(20,5,$oRelatorio->dados[0]->fonteRecurso    ,0,0,"R",0);
$pdf->Cell(100,5,"{$oRelatorio->dados[0]->o15_descr} - {$oRelatorio->dados[0]->complemento}"   ,0,1,"L",0);
$pdf->SetFont('Arial','b',8);
