...
abstract class DiarioDisciplina implements iDiarioDisciplina {
  ...
  /**
   * Retorna os aproveitamentos do periodo, pela sequencial da tabela
   * Esse método ? usado na rotina Notas Parciais
   *
   * @param $iOrdem
   * @return AvaliacaoAproveitamento
   */
  public function getAvaliacoesPorOrdemSequencial($iOrdem)
  {
    foreach ($this->getAvaliacoes() as $oAvaliacao) {
      if (method_exists($oAvaliacao->getElementoAvaliacao(), 'getPeriodoAvaliacao')) {
        if ($oAvaliacao->getElementoAvaliacao()->getPeriodoAvaliacao()->getOrdemPeriodo() == $iOrdem) {
          return $oAvaliacao;
        }
      }
    }
   ...
   * Os periodos que sao levadas em conta sao apenas os periodos somados no periodo
            ...
            foreach ($oResultadoFinal->getElementosCalculoFaltas() as $oElementoFalta) {
                ...
                foreach ($aAvaliacoesAproveitamento as $oAvaliacao) {
                    ...
                    if ($oElementoFalta->getOrdemSequencia() == $oAvaliacao->getOrdemSequencia()) {
                        $iTotalFaltas += (int)$oAvaliacao->getNumeroFaltas();
                    }
                }
            }
        } else {
  ...
  /**
   * Retorna o total de faltas do periodo de avaliacao informado
      ...
      foreach ($aAvaliacoes as $oElementoFalta) {
        ...
        if ($oElementoFalta->getElementoAvaliacao() instanceof AvaliacaoPeriodica) {
          ...
          if ($oPeriodoAvaliacao->getCodigo() == $oElementoFalta->getElementoAvaliacao()->getPeriodoAvaliacao()->getCodigo()) {
            ...
            $iTotalFaltasPeriodo += intval($oElementoFalta->getNumeroFaltas());
          }
        }
      }
    }
   ...
   * Data: 23/12/2025
   * Demanda: 18059
   * Razao: Para Anos Finais, a frequencia deve ser calculada usando 1000 horas/aula fixas
   *        (conforme tabela de frequencia oficial), ao inves do total real de aulas dadas.
   *        O calculo anterior usava o total real de aulas, causando frequencia incorreta.
   *        Exemplo: Aluno com 312 faltas = floor((1000-312)/1000*100) = 68% (correto),
   *        mas o sistema calculava usando ~1549 aulas reais = 79% (incorreto).
   * 
   * @return float
   */
  protected function calculoDeFrequenciaGlobal() {
      ...
      // Autor: Uemerson Santana | Data: 23/12/2025 | Demanda: 18059
      // Razao: Verificar se eh Anos Finais ou EJA Finais para aplicar calculo correto com horas/aula fixas
      $lEhAnosFinais = false;
      $lEhEjaFinais = false;
      $sTurno = '';
      try {
        $sNomeCalendario = $this->oDiario->getTurma()->getCalendario()->getDescricao();
        if (strpos($sNomeCalendario, 'ANOS FINAIS') !== false) {
          $lEhAnosFinais = true;
          // Obter turno para Anos Finais (INTEGRAL = 1522, outros = 1000)
          /**
           * Autor: Uemerson Santana
           * Data: 22/01/2026
           * Demanda: 18059
           * Razao: Corrigido metodo getNome() para getDescricao(). A classe Turno nao possui getNome().
           */
          try {
            $oTurma = $this->oDiario->getTurma();
            if ($oTurma && $oTurma->getTurno()) {
              $sTurno = trim($oTurma->getTurno()->getDescricao());
            }
          } catch (Exception $e) {
            // Se houver erro ao acessar turno, continua com padrao
          }
        } elseif (strpos($sNomeCalendario, 'EJA FINAIS') !== false || strpos($sNomeCalendario, 'EJA ANOS FINAIS') !== false) {
          $lEhEjaFinais = true;
          // Obter turno para EJA Finais (NOITE = 1000, outros = 1200)
          try {
            $oTurma = $this->oDiario->getTurma();
            if ($oTurma && $oTurma->getTurno()) {
              $sTurno = substr(trim($oTurma->getTurno()->getDescricao()), 0, 5);
            }
          } catch (Exception $e) {
            // Se houver erro ao acessar turno, continua com padrao
          }
        ...
        // Se houver erro ao acessar calendario, continua com calculo padrao
      }
      ...
      // Autor: Uemerson Santana | Data: 23/12/2025 | Demanda: 18059
      // Razao: Para Anos Finais usar horas/aula fixas baseado no turno (INTEGRAL = 1522, outros = 1000)
      //        Para EJA Finais: NOITE = 1000, outros turnos = 1200
      if ($lEhAnosFinais) {
        // Anos Finais: INTEGRAL = 1522 horas/aula, outros turnos = 1000 horas/aula
        if (strtoupper($sTurno) === 'INTEGRAL') {
          $iTotalAulas = 1522;
        } else {
          $iTotalAulas = 1000;
        }
        ...
        if ($iTotalAulas > 0) {
          $this->nPercentualGlobal = floor(($iTotalAulasPresentes * 100) / $iTotalAulas);
        }
      } elseif ($lEhEjaFinais) {
        // EJA Finais: NOITE = 1000, outros turnos = 1200
        if (strtoupper($sTurno) === 'NOITE') {
          $iTotalAulas = 1000;
        } else {
          $iTotalAulas = 1200;
        }
        ...
        // Calculo padrao: usar total real de aulas
        $iTotalAulasPresentes = $iTotalAulas - $nTotalFaltasSemAbono;
        if ($iTotalAulas > 0) {
  ...
  /**
   * Retorna as faltas abonadas no periodo de avaliacao
   * @param PeriodoAvaliacao $oPeriodoAvaliacao
   * @return int
   */
  ...
  /**
   * Verifica se o diario possui aproveitamento lancado para algum periodo da disciplina
   * @return boolean
   */
   ...
   * Verifica os periodos que estao sendo utilizados para avaliacao do aluno
   * Este metodo verifica se o aluno esta utilizando proporcionalidade
   * Neste metodo, nao sao contabilizadas aulas nos periodos em que o aluno esta amparado.
   * @return integer
   */
   ...
   * @return boolean true para reclassificado, false para nao reclassificado
   */
  public function reclassificadoPorBaixaFrequencia() {
    ...
    /**
   * Adiciona amparo para os periodos informados
  ...
  public function salvarAmparo (array $aPeriodosAvaliacao, $oTipoAmparo, $lAproveitaCargaHoraria) {
    ...
    if ($oTipoAmparo instanceof Justificativa) {
      $oAmparo->setJustificativa($oTipoAmparo);
        if (is_null($oTipoAmparo->getCodigo())) {
            throw new Exception("O codigo da Justificativa nao eh valido.");
        }
    } else if ($oTipoAmparo instanceof Convencao) {
        if (is_null($oTipoAmparo->getCodigo())) {
            throw new Exception("O codigo da Convencao nao eh valido.");
        }
      $oAmparo->setConvencao($oTipoAmparo);
    } else {
      throw new BusinessException("Nao foi informado o tipo de justificativa");
    }
