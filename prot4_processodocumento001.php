...
if (!empty($oGet->iCodigoProcesso)) {
    ...
    $sSqlNumeroProcesso = $oDaoProtprocesso->sql_query_file($oGet->iCodigoProcesso, 'p58_numero, p58_ano, p58_requer');
    ...
    if ($oDaoProtprocesso->numrows > 0) {
        ...
        $z01_nome = $oDaoProcesso->p58_requer;
    }
...
$codigoProcesso = !empty($oGet->iCodProcesso) ? $oGet->iCodProcesso : '';

if (!empty($oGet->volumes)) {
  $labelAnexar = 'Volume';
} else {
  $labelAnexar = 'Processo';
}
?>
<html>
<head>
...
<div class="container" style="width:650px;">
  ...
  <fieldset>
    <legend>Anexar Documentos ao <?php echo $labelAnexar ?></legend>
      ...
      <table class="form-container">
        ...
        <tr>
          <td nowrap title="<?php echo $Tp01_protprocesso; ?>">
              <?php db_ancora($Lp58_numero, "js_pesquisarProcesso(true, true);", $iOpcaoProcesso); ?>
          </td>
          <td>
              <?php
              // Alteracao Plugin TaxonomiaDeProcessosDoMinisterioPublico - prot4_processodocumento001.php #2
              db_input('p58_numero', 12, $Ip58_numero, true, 'text', 3,
                " onChange='js_pesquisarProcesso(false);'");
...
<script type="text/javascript">
  ...
  var flagVolume = get.volumes;
  <?php
    if (!empty($oGet->iCodigoProcesso)) {?>
      $('p58_codproc').value = <?php echo $oGet->iCodigoProcesso ?>;
  <?php
    }?>
  ...
  if(!empty($('p58_numero').value)) {
    ...
    js_pesquisarProcesso(true, true);
  }
  ...
  function js_buscarDocumentos() {
    ...
    oParametros.storage = $('storage').value;
   ...
   * @return boolean
   */
  function js_pesquisarProcesso(lMostra, anexarDocumento = false) {
    var parametroExtra = '';
    var sArquivo = 'func_protprocesso_protocolo.php?funcao_js=parent.';

    if(lMostra) {
      if (flagVolume == '') {
        parametroExtra = '&apenas_processopai=1';
      }
      if (anexarDocumento == true) {
        parametroExtra += '&anexar_documento=1';
      }
      sArquivo += 'js_mostraProcesso|dl_codigo_do_processo|p58_numero|dl_titular';
      sArquivo += parametroExtra;
      js_OpenJanelaIframe('', 'db_iframe_proc', sArquivo, 'Pesquisa de Processos', true);
    } else {

      /**
       * Alterado lógica de buscar o processo quando o mesmo já vem na url.
       * A forma como esta pesquisando o processo gera um bug quando existe dois processos com o mesmo número (1/2021)
       * por exemplo. Tem que ser revisado os outros programas que realizam essa busca, pois provavelmente possuem o
       * mesmo erro.
       */
      if (get.iCodigoProcesso) {
        $('p58_codproc').value = get.iCodigoProcesso;
        $('p01_descricao').value = '';
        $('uploadfile').disabled = false;
        js_buscarDocumentos();
      } else {
        ...
        sArquivo += 'js_mostraProcesso&pesquisa_chave=' + iNumeroProcesso + '&sCampoRetorno=p58_codproc&anexardocumento=1';
        sArquivo += parametroExtra;

        js_OpenJanelaIframe('', 'db_iframe_proc', sArquivo, 'Pesquisa de Processos', false);
      }
    }
  }
  ...
  function js_mostraProcesso(iCodigoProcesso, iNumeroProcesso, sNome) {
    ...
    var codigoProcesso = $('codigoProcesso').value;

    if(codigoProcesso != "") {
      $('p58_codproc').value = codigoProcesso;
    }
  ...
  function js_salvar() {
    ...
    oParametros.iCodigoProcesso = iCodigoProcesso;
    oParametros.sDescricaoDocumento = sDescricaoDocumento;
    oParametros.sCaminhoArquivo = encodeURIComponent(sCaminhoArquivo);
    oParametros.storage = storage;
