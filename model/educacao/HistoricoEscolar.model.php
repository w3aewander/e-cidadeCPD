...
class HistoricoEscolar
{
     ...
     * @throws DBException
     */
    public static function temInconsistenciaEtapaSelecionada(
        Etapa $oEtapa,
        Aluno $oAluno,
        $iAnoEtapa,
        $sResultado,
    ...
    )
    {
        ...
        if ($oMatricula != null && $oMatricula instanceof Matricula) {
            if ($iAnoEtapa == $oMatricula->getTurma()->getCalendario()->getAnoExecucao()) {
                if($oMatricula->getTurma()->getCalendario()->getPeriodo() != 0) {
                    if (!empty($periodo) && $oMatricula->getTurma()->getCalendario()->getPeriodo() != null && $periodo >= $oMatricula->getTurma()->getCalendario()->getPeriodo()) {
                        db_msgbox(_M(MENSAGENS_HISTORICOESCOLAR . 'ano_invalido'));
                        return true;
                    }

                    if (empty($periodo) || $oMatricula->getTurma()->getCalendario()->getPeriodo() == null) {
            ...
            /**
             * Compara se o ano é maior que o da matrícula
             */
            if ($iAnoEtapa > $oMatricula->getTurma()->getCalendario()->getAnoExecucao()) {  
                db_msgbox(_M(MENSAGENS_HISTORICOESCOLAR . 'ano_invalido'));
                return true;
            ...
            if ($oEtapa->getEnsino()->getCodigo() == $oMatricula->getEtapaDeOrigem()->getEnsino()->getCodigo()
                ...
                if ($oEtapa->getOrdem() > $oMatricula->getEtapaDeOrigem()->getOrdem()) {
                    db_msgbox(_M(MENSAGENS_HISTORICOESCOLAR . 'etapa_invalida'));
                    return true;
                }

                if ($oEtapa->getOrdem() == $oMatricula->getEtapaDeOrigem()->getOrdem() && $sResultado == 'A') {
                    db_msgbox(_M(MENSAGENS_HISTORICOESCOLAR . 'etapa_invalida'));
                    return true;
                }
            }
    ...
    public function permiteManutencaoHistorico(Aluno $oAluno, Escola $oEscola)
    {
        ...
        if (empty($iDiasManutencaoHistorico)) {
            foreach ($oAluno->getMatriculas() as $oMatricula) {
                if ($oMatricula->getTurma()->getEscola()->getCodigo() == $oEscola->getCodigo()) {
                    return self::PERMITE_MANUTENCAO;
                }
            }
        ...
        if (in_array($oUltimaMatricula->getSituacao(), $aSituacoes)) {
            ...
            /**
             * Se aluno foi transferido fora não valida os dias
             */
            if ($oUltimaMatricula->getSituacao() == 'TRANSFERIDO FORA') {
        ...
        if ($oUltimaMatricula->getSituacao() == 'MATRICULADO') {
            ...
            if (count($aMatriculas) > 1) {
                ...
                if ($oPenultimaMatricula->getSituacao() == 'MATRICULADO' && !$oPenultimaMatricula->isConcluida()) {
                    if ($oUltimaMatricula->getTurma()->getEscola()->getCodigo() != $oEscola->getCodigo()) {
                        return self::NAO_PERMITE_MANUTENCAO;
                    }
                    return self::PERMITE_MANUTENCAO;
                ...
                if ($oUltimaMatricula->getTurma()->getEscola()->getCodigo() != $oEscola->getCodigo()) {
                    $iNumeroDiasUltimaMatricula = DBDate::getIntervaloEntreDatas($oDataAtual, $oUltimaMatricula->getDataMatricula())->days;
                    if ($iNumeroDiasUltimaMatricula <= $iDiasManutencaoHistorico) {
                        return self::PERMITE_MANUTENCAO_ETAPAS_MENORES;
                    }
                }
                ...
                if (in_array($oPenultimaMatricula->getSituacao(), $aSituacoes)) {
                    ...
                    if ($oPenultimaMatricula->getSituacao() == 'TRANSFERIDO FORA'
                        ...
                        if ($oPenultimaMatricula->getTurma()->getEscola()->getCodigo() == $oEscola->getCodigo() && $iNumeroDiasTransferencia <= $iDiasManutencaoHistorico) {
                            return self::PERMITE_MANUTENCAO;
                        } elseif ($oUltimaMatricula->getTurma()->getEscola()->getCodigo() == $oEscola->getCodigo()) {
                            if ($iNumeroDiasTransferencia <= $iDiasManutencaoHistorico) {
