...
if(isset($opcao) && $opcao == "alterar" && !isset($pesqandam)){
  echo "<script>parent.iframe_fiscais.location.href='fis3_fis_fandamusu002.php?chavepesquisa=$y40_codandam&chavepesquisa1=$y40_id_usuario'</script>";}
if(isset($opcao) && $opcao == "excluir" && !isset($pesqandam)){
  echo "<script>parent.iframe_fiscais.location.href='fis3_fis_fandamusu003.php?chavepesquisa=$y40_codandam&chavepesquisa1=$y40_id_usuario'</script>";
}
if(isset($opcao) && $opcao == "alterar" && isset($pesqandam)){
  echo "<script>parent.iframe_fiscais.location.href='fis3_fis_fandamnotiusu002.php?chavepesquisa=$y40_codandam&chavepesquisa1=$y40_id_usuario'</script>";
}
if(isset($opcao) && $opcao == "excluir" && isset($pesqandam)){
  echo "<script>parent.iframe_fiscais.location.href='fis3_fis_fandamnotiusu003.php?chavepesquisa=$y40_codandam&chavepesquisa1=$y40_id_usuario'</script>";
}
$db_opcao =3;
?>
...
?>
       <?php 
       if(isset($y39_data) && $y39_data != ""){
        $sData = explode("-", $y39_data);
        $y39_data_ano = $sData[0];
        $y39_data_mes = $sData[1];
        $y39_data_dia = $sData[2];
        // $y39_data = db_formatar($y39_data,"d");
       }
...
function js_pesquisay40_codandam(mostra){
  if(mostra==true){
    js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_fandam','func_fis_fandam.php?funcao_js=parent.js_mostrafandam1|y39_codandam|y39_data','Pesquisa',true);
  }else{
    js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_fandam','func_fis_fandam.php?pesquisa_chave='+document.form1.y40_codandam.value+'&funcao_js=parent.js_mostrafandam','Pesquisa',false);
  }
}
