...
if ($ordem == 'e') {
    ...
    $xordem = ' desdobramento, c70_codlan';
    $head7 = 'ORDEM : DESDOBRAMENTO, TIPO:'.$tipocompra_descr;
}
...
$where_recursos = '';
if (!empty($oPost->recursos_selecionados)) {
    $where_recursos = " and  orctiporec.o15_codigo in ({$oPost->recursos_selecionados}) ";
}
...
$sqlperiodo .= ' 	       (select o56_elemento||\'-\'||o56_descr
	                               from orcelemento where o56_codele = c67_codele and
                                                              o56_anousu = c70_anousu) as desdobramento, ';
$sqlperiodo .= ' 	       c72_complem,                                                                              ';
$sqlperiodo .= " 	       {$sCampos}                                                                                ";
...
$sqlperiodo .= ' INNER JOIN origemcomplementorecurso ON origemcomplementorecurso.o206_numero = empempenho.e60_numemp ';
$sqlperiodo .= ' AND origemcomplementorecurso.o206_origem = 1                                                        ';
$sqlperiodo .= ' INNER JOIN orctiporec ON orctiporec.o15_codigo = origemcomplementorecurso.o206_recurso              ';
...
$sqlperiodo .= '   left join conlancamcompl	on c72_codlan                 = c70_codlan                             ';
$sqlperiodo .= '   inner join conlancamele on c67_codlan = c70_codlan ';
$sqlperiodo .= "  where $xtipo $where_credor                                                                       ";
$sqlperiodo .= "    and c70_data between '$dataini' and '$datafin'                                                 ";
$sqlperiodo .= "    and $sele_work                                                                                 ";
$sqlperiodo .= "    $where_recursos                                                                                ";
$sqlperiodo .= "    $instits                                                                                       ";
...
if ($rows == 0) {
    $sErro = 'Verifique os dados escolhidos! Não foi retornado nenhum resultado.';
    db_redireciona('db_erros.php?fechar=true&db_erro='.$sErro);
}
//////////////////////////////////////////////////////////////////////
$resultinst = db_query('select codigo,
                               nomeinst
                          from db_config
...
$pdf->addTitulo('PERÍODO : '.db_formatar($dataini, 'd').' A '.db_formatar($datafin, 'd'));
$pdf->addTitulo('');
$pdf->addTitulo($head7);
...
for ($x = 0; $x < $rows; ++$x) {
    ...
    if (($pdf->gety() > $pdf->getH() - 40) || $x == 0) {
        ...
        $pdf->Cell(60, $tam, 'DOTAÇÃO', 1, 0, 'C', 1);
    ...
    $pdf->Cell(60, $tam, $dl_estrutural, 0, 0, 'L', $pre);
    $pdf->Cell(15, $tam, db_formatar($c70_data, 'd'), 0, 0, 'C', $pre);
    $pdf->Cell(40, $tam, db_formatar($c70_valor, 'f'), 0, 0, 'C', $pre);
    ...
    if ($ordem == 's') {
	$pdf->Cell(85, $tam, 'Desdobramento: '.$desdobramento, 0, 1, 'L', $pre);
    ...
    if ($com_mov == 's') {
        ...
        if ($c72_complem != '') {
            $pdf->multiCell(0, $tam, 'MOTIVO : '.$c72_complem, 0, 'L', $pre);
        }
    }
...
//        $pdf->Cell(20,$tam,db_formatar($e62_vltot,'f'),0,0,"R",$preenche);
    ...
    if (!array_key_exists($desdobramento, $total_desdobramento)) {
        $total_desdobramento[$desdobramento] = 0;
    }
    if ($c53_tipo == 10 || $c53_tipo == 11) {
        if ($c53_tipo == 10) {
            $total_emp += $c70_valor;
            ++$iContEmp;
            $total_desdobramento[$desdobramento] = $total_desdobramento[$desdobramento] + $c70_valor;
        } else {
...
if( count($total_desdobramento) > 0 ){
  $pdf->addpage();
  $pdf->SetFont('Arial', '', 7);

  $pdf->Cell(80, $tam, 'Desdobramento', 0, 0, 'L', 1);
  $pdf->Cell(30, $tam, 'Valor', 0, 1, 'R', 1);
  $total_desdobramento_g = 0;
  for($i=0;$i<count($total_desdobramento);$i++){

    $pdf->Cell(80, $tam, key($total_desdobramento), 0, 0, 'L', 0);
    $pdf->Cell(30, $tam, db_formatar($total_desdobramento[key($total_desdobramento)], 'f'), 0, 1, 'R', 0);
    $total_desdobramento_g += $total_desdobramento[key($total_desdobramento)];
    next($total_desdobramento);
  }

  $pdf->Cell(80, $tam, 'Total:', 0, 0, 'L', 1);
  $pdf->Cell(30, $tam, db_formatar($total_desdobramento_g, 'f'), 0, 0, 'R', 1);
