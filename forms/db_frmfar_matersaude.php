...
<form name="form1" method="post" action="">
  <fieldset>
    ...
    <table border="0">
      ...
      <tr>
        ...
        <td>
          <?php
          // muda a cor do background para dizer ao usuário que o campo é obrigátorio, quando o parametro for verdadeiro
          $bgcolor = $config->fa02_informa_receita_medicamento == 't' && in_array($db_opcao, [1, 2]) ? 'white': '';
          db_input('fa01_i_prescricaomed',10,@$Ifa01_i_prescricaomed,true,'text',$db_opcao," onchange='js_pesquisafa01_i_prescricaomed(false);'", '', $bgcolor);
  ...
  <input name="<?=($db_opcao==1?"incluir":($db_opcao==2||$db_opcao==22?"alterar":"excluir"))?>" 
    ...
    <?= $config->fa02_informa_receita_medicamento == 't' ? "onClick='validaCampos(event);'" : ''; ?>>
...
function pesquisaMedicamentoFarmaciaBasica(lMostra) {
  ...
  if (lMostra) {
    ...
    js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_medicamentos', sUrl, sTitulo, true);
  } else if ( $F('fa01_medicamentos') != '' ) {

    sUrl += '?funcao_js=parent.medicamentoFarmaciaBasica';
    sUrl += '&pesquisa_chave=' + $F('fa01_medicamentos');
    js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_medicamentos', sUrl, sTitulo, false);
  } else {
...
/**
 * função para validar o campo prescricaomed caso esteja setado por parametro a opçção fa02_informa_receita_medica
 */
