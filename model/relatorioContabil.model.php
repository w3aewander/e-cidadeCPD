...
class relatorioContabil {
  ...
  */
   ...
   */
  function getParametrosUsuario($iUsuario = null) {

    $oDaoLinhas = new  cl_orcparamseqfiltrousuario;
    $sWhere = "o72_orcparamrel = {$this->codigo}";
    $order = " 1 desc limit 1";
    $sSqlFiltroPadrao  = $oDaoLinhas->sql_query_file(null, "*", $order, $sWhere);
    ...
    if ($oDaoLinhas->numrows > 0) {
      ...
      if ($oElemento->item(0)) {
        ...
        if ($fonteRecurso->length > 0) {
            ...
            $valor = $fonteRecurso->item(0)->getAttribute("valor");
            $oFiltro->fonterecurso->valor = explode(",", $valor);
        }
        ...
        if ($complemento->length > 0) {
            ...
            $valor = $complemento->item(0)->getAttribute("valor");
            $oFiltro->complemento->valor = explode(",", $valor);
        }
    }
    return $oFiltro;
  }
