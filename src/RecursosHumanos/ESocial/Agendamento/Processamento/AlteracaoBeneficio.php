 ...
 */
class AlteracaoBeneficio extends ProcessamentoAbstract implements ProcessamentoInterface
{
    /**
     * @var
     */
    ...
    public function processar()
    {
        ...
        $dados->ano = $this->ano;
        $dados->mes = $this->mes;
        $dados->beneficios = CadastroBeneficioRepository::buscarBeneficios(
            $competencia,
            $this->servidores,
            $this->selecao,
            true
        );
        $formatter = FormatterFactory::get(Tipo::S2416);
        $formatter->setEmpregador(CgmRepository::getByCodigo($this->cgm));
        $alteracaoBeneficio = $formatter->formatar($dados);

        $validaMd5 = true;
        if ($this->envioForcado) {
            $validaMd5 = false;
        }
        foreach ($alteracaoBeneficio as $iIndice => $oDados) {
            $oEvento = new Evento(Tipo::S2416, $this->cgm, $oDados->referencia, $oDados);
