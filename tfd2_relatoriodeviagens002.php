...
if( isset( $oGet->iEspecialidade ) && !empty( $oGet->iEspecialidade ) ) {
  $aWhere[] = " rh70_estrutural = '{$oGet->iEspecialidade}' ";
}
...
$oConfig->iColunaEspecializacao = 85;

$head1 = 'RELATÓRIO DE VIAGENS';
$head2 = 'Período: ' . $oDataInicial->convertTo(DBDate::DATA_PTBR) . 
          ' até ' . $oDataFinal->convertTo(DBDate::DATA_PTBR);
...
foreach ($aPedidos as $oDestino) {
  ...
  $oPdf->cell(278, 4, "Destino: {$oDestino->sDestino}", 'B', 1);

  foreach ($oDestino->aSituacao as $oSituacao) {

    $oPdf->SetFont('arial', 'B', 8);
    $oPdf->ln(2.5);
    $oPdf->cell(278, 4, $oSituacao->sSituacao, 'B', 1);
    $oPdf->ln(0.4);
    ...
    foreach ($oSituacao->aPedidos as $oPedido) {
      ...
      $oPdf->cell($oConfig->iColunaEspecializacao, 4, $oPedido->rh70_descr,  1, 0, "L");
...
function imprimeCabecalhoPedidos(FPDF $oPdf, $oConfig) {
  ...
  $oPdf->cell($oConfig->iColunaEspecializacao, 4, "Especialidade", 1, 0, "C", 1);
