...
for ($iContLinha = 0; $iContLinha < $iQtdLinhasPassageiros; $iContLinha++) {
    ...
    imprimePassageiro($oPdf, $iContLinha, 1, $sNome, $sRg, $sTelefone, $sDestino, $sHora, $lFica);
}
...
if ($iQtdLinhasPassageiros < $iCapacidadeVeiculo) {
    ...
    for ($iCont = 0; $iCont < $iTotalEspacosBranco; $iCont++) {
        imprimePassageiro($oPdf, $iProximoCont++, 1);
    }
}
...
for ($iCont = 0; $iCont < $iContLinhaColo; $iCont++) {
    ...
    imprimePassageiro($oPdf, $iCont, 2, $sNome, $sRg, $sTelefone);
}
...
if ($iQtdLinhasPassageirosColo < 10) {
    ...
    for ($iCont = 0; $iCont < $iTotalEspacosBranco; $iCont++) {
        imprimePassageiro($oPdf, $iProximoCont++);
    }
}
 ...
 */
function imprimeCabecalho(Pdf $oPdf, $iTipo = 2)
{
...
function imprimeTitulo(Pdf $oPdf, $sTipo)
{
    $iTam = 8;
    $lCor = false;
