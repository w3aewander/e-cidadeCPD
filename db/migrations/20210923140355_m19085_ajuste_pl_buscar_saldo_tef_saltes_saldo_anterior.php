...
class M19085AjustePlBuscarSaldoTefSaltesSaldoAnterior extends PostgresMigration
{
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tp_saltessaldo') THEN


create  type tp_saltessaldo  as (

                                riconta         integer,
                                riestatus       integer,
                                rnsaldoanterior numeric,
                                rnvalorcredito  numeric,
...
END
$$;
 create or replace function fc_saltessaldoanterior(integer,date,date,varchar(20),integer)
 ...
 begin
     ...
     dataini := datainiparam-1;

     select k13_dtimplantacao
       into dtimplantacao
       from saltes
           ...
           from (
                 select 'r' as recebe,round(c.k12_valor,2) as k12_valor
                  ...
                  where e.k12_conta = conta
                    and c.k12_data between dtimplantacao and dataini
                   ...
                   select recebe,k12_valor
                     from (
                           ...
                           union all
                            ...
                            select distinct c.k12_id,
                                            c.k12_data,
                                            c.k12_autent,'r'::char(1) as recebe,
                                            round(c.k12_valor,2) as k12_valor
                              from corrente c
                        inner join corlanc e on c.k12_id = e.k12_id
                                            and c.k12_data   = e.k12_data
                        ...
                        inner join conlancamdoc on c71_codlan = c86_conlancam
                             ...
                             where c.k12_conta = conta
                               ...
                               and c.k12_valor >= valorzero
                               and case
                                     when id_term is not null then c.k12_id = id_term else true end
                               and c.k12_instit = instit
