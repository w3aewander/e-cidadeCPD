  ...
  switch ($oParam->exec) {
    ...
    case 'processar':
      ...
      /**
    	 ...
    	 */
    	$oLancamentoAuxiliar = null;
        $aDocumentosCeitos = [300, 301, 302];
        $iDocumento = $oParam->iCodigoDocumento;

    	if ( !in_array($iDocumento, $aDocumentosCeitos) ) {

    		$sMsgErro  = 'O documento informado não é um documento de provisão de férias / 13º salário.\n.';
    		$sMsgErro .= "Código do documento informado: {$iDocumento}";
    		throw new BusinessException($sMsgErro);
    	}
        // caso o valor seja negativo, vamos alterar o documento para que seja feito um lançamento reverso
        if ($oParam->nValor < 0) {

            $iDocumento = 301;
            $oParam->nValor = ($oParam->nValor * -1);
            $oParam->iCodigoDocumento = "301";
        }

    	$oLancamentoAuxiliar = buscaLancamentoAuxiliar($iDocumento);
    	processarLancamentosContabeis($oLancamentoAuxiliar, $oParam);
		db_fim_transacao(false);
