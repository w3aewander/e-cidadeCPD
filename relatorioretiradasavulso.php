...
function retornaIdVeiculo($sql){
  $consulta = pg_query($sql);
  $resultado = pg_fetch_all($consulta);
  return $resultado[0]["ve01_codigo"];
}
...
$pinicio = implode("-", array_reverse(explode("/", $_GET["inicio"])));
$pfim = implode("-", array_reverse(explode("/", $_GET["fim"])));
$placa = $_GET["placa"];
$vr = $_GET["vr"];
$rawhere = "";
if(!empty($placa) && !empty($vr)){
  $rawhere .= "ve01_placa = '{$placa}' AND vr = '{$vr}'";
}elseif(!empty($placa)){
  $rawhere .= "ve01_placa = '{$placa}'";
}
elseif(!empty($vr)){
  $rawhere .= "vr = '{$vr}'";
}

$sql = "SELECT ve01_codigo FROM veiculos WHERE {$rawhere}";

$veiculo = retornaIdVeiculo($sql);
$lRetirada = true;
  ...
  if (isset($lRetirada) ) {
    ...
    if ($numrows_ret > 0) {
      ...
      for ($w = 0; $w < $numrows_ret; $w++) {        
        ...
        if($ve60_datasaida >= $pinicio && $ve61_datadevol <= $pfim){
          $pdf->setfont('arial','',7);
