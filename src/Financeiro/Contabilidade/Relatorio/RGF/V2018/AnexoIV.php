...
class AnexoIV extends ProcessamentoRelatorioLegal
{
    ...
    public function __construct($iAno, \Periodo $oPeriodo)
    {
        ...
        $this->oRCL = new ReceitaCorrenteLiquida($iAno, $aInstituicoes, 178);
    }
    ...
    public function processar()
    {
        ...
        if (empty($this->aLinhas)) {
            ...
            if ((int)$this->iAno === 2018) {
                $descricao = "Operações de crédito previstas no art. 7º § 3º da RSF nº 43/2001 1 (I)";
                $this->aLinhasConsistencia[10]->descricao = $descricao;
                $descricao = 'Operações de crédito previstas no art. 7º § 3º da RSF nº 43/2001 1 (II)';
                $this->aLinhasConsistencia[16]->descricao = $descricao;
            }
     ...
     *   18 - RECEITA CORRENTE LÍQUIDA - RCL (IV)
     *   19 - OPERAÇÕES VEDADAS (V)
     *   20 - TOTAL CONSIDERADO PARA FINS DA APURAÇÃO DO CUMPRIMENTO DO LIMITE (VI)= (IIIa + V - Ia - IIa)
     *   21 - LIMITE GERAL DEFINIDO POR RESOLUÇÃO DO SENADO FEDERAL PARA AS OPERAÇÕES DE CRÉDITO INTERNAS E EXTERNAS
     *   22 - LIMITE DE ALERTA (inciso III do §1º do art. 59 da LRF) - <%>
     *        verificado na linha abaixo a % e de 14.4%
     */
    protected function processaLinhasQuadro2()
    {
        ...
        $this->aLinhasConsistencia[20]->valor      = round(
            ($this->aLinhasConsistencia[17]->ateperiodo + $this->aLinhasConsistencia[19]->valor -
                $this->aLinhasConsistencia[10]->ateperiodo - $this->aLinhasConsistencia[16]->ateperiodo),
            2
        );
        $this->aLinhasConsistencia[20]->percentual = round(
            (( $this->aLinhasConsistencia[20]->valor / $nRCL ) * 100),
            2
        );

        $this->aLinhasConsistencia[21]->valor      = round(($this->aLinhasConsistencia[18.2]->valor * 0.16), 2);
        $this->aLinhasConsistencia[21]->percentual = 16;

        $this->aLinhasConsistencia[22]->valor      = round(($this->aLinhasConsistencia[18.2]->valor * 14.4) / 100, 2);
        $this->aLinhasConsistencia[22]->percentual = 14.4;

        //$this->aLinhasConsistencia[23]->percentual =
          //round((($this->aLinhasConsistencia[23]->valor) / $nRCL) * 100, 2);
        $this->aLinhasConsistencia[23]->percentual =
          round((($this->aLinhasConsistencia[23]->valor) / $this->aLinhasConsistencia[18.2]->valor) * 100, 2);

        $this->aLinhasConsistencia[24]->valor      = round($this->aLinhasConsistencia[18.2]->valor * 0.07, 2);
