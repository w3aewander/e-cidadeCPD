...
<script type="text/javascript">
  ...
  function js_setCodigo(oAjax) {
    ...
    iCodigo = JSON.parse(oAjax.responseText);
...
<div class="container">
  <form name="form1" method="post" action="">
    <fieldset>
      ...
      <table>
        ...
        <tr id="PJ" style="display: none;">
          <td><?php db_ancora(@$Lq71_estrutural, "js_pesquisa_cnae(true);", $db_opcaoselect); ?></td>
          <td nowrap="nowrap">
            <?php
        ...
        <tr id="PF" style="display: none">
          <td>
            <?php db_ancora(@$Lrh70_estrutural, "js_pesquisa_cbo(true);", $db_opcaoselect); ?>
          </td>
          <td nowrap="nowrap">
            <?php
        ...
        <tr>
          <td align="left" nowrap title="<?php echo $Tq12_classe ?>">
            <strong><?php db_ancora("Código da Classe:", "js_pesquisaq12_classe(true);", $db_opcao); ?></strong>
          </td>
        ...
        <tr id='servico' style="display: <?php echo $sServicoStyle ?> ">
          ...
          <td align="left" nowrap>
            ...
            ?>
            <input name="lanca" type="button" value="Lançar" onclick="js_insSelect()">
          </td>
        </tr>
        <tr id="servico-2" style="display:<?php echo $sServicoStyle ?>">
        <td></td>
          <td style="width: 100px;">
            <select name="campos[]" id="campos" size="7" multiple style="width:530px">
            </select>
          </td>
          <td align="left" valign="middle" width="20%" id="comandos">
        ...
        <tr id='deducao' style="display: <?php echo $sServicoStyle ?> ">
          ...
          <td colspan="2">
            <?php db_input('q03_deducao', 10, $Iq03_deducao, TRUE, 'checkbox', $db_opcao); ?>
          </td>
        </tr>
        <tr id='tributacao_municipio' style="display: <?php echo $sServicoStyle ?> ">
          ...
          <td colspan="2">
            <?php db_input('q03_tributacao_municipio', 10, $Iq03_tributacao_municipio, TRUE, 'checkbox', $db_opcao); ?>
          </td>
        </tr>
      </table>
    ...
    <?php db_input("alteracbocnae", 5, '', TRUE, "hidden", 3); ?>
  </form>
...
<script type="text/javascript">
    ...
    if (lCalCiss) {
      ...
      if($('campos').options.length > 0) {
        ...
        for(var i = 0; i < $('campos').options.length; i++) {
          ...
          sItemServicos += sVirgula + $('campos').options[i].value;
          sVirgula      = ',';
        }
      }

      oObject.sServico = sItemServicos;
   }
  ...
  function js_mostraclasse(chave_classe, chave1_classe, juridica, calciss) {
    ...
    if(chave_classe != 5 && chave_classe != 10 && chave_classe != 11){
  ...
  function js_pesquisa_tipo_fator_risco(mostra) {
    if (mostra == true) {
      js_OpenJanelaIframe("", 'db_iframe_cadcalc', "func_tle_fator_risco_issqn.php?funcao_js=parent.js_mostraFatorRisco|dl_codigo|dl_descricao", 'Pesquisa', true);
    } else {
      js_OpenJanelaIframe("", 'db_iframe_cadcalc', "func_tle_fator_risco_issqn.php?pesquisa_chave=" + document.form1.tle03_fator_risco.value + "&funcao_js=parent.js_mostraFatorRisco1", 'Pesquisa', false);
    }
  }

  function js_mostraFatorRisco1(chave, erro) {
    document.form1.tle03_fator_risco_descr.value = chave;
    if (erro == true) {
      document.form1.tle03_fator_risco.focus();
      document.form1.tle03_fator_risco.value = '';
    }
  }

  function js_mostraFatorRisco(chave1, chave2) {
    document.form1.tle03_fator_risco.value = chave1;
    document.form1.tle03_fator_risco_descr.value = chave2;
  ...
  function js_preenche(){

    var url = "iss1_ativid.RPC.php";
    var oObject = new Object();

    oObject.codAtivid = document.form1.q03_ativ.value;
    oObject.exec      = 'preencheDadosServico';

    var objAjax = new Ajax.Request(url, {
      method: "post",
      parameters: 'json=' + Object.toJSON(oObject),
      onComplete: js_preencheDadosServico
    });
  }

  function js_preencheDadosServico(oAjax){
    var oRetorno = eval("(" + oAjax.responseText + ")");
    oRetorno.message = oRetorno.message.urlDecode().replace(/\\n/g, "\n");
    var F     = document.getElementById("campos");

    oRetorno.dados.each(function(oData,iContador){
      var id = oData.q127_issgruposerviso;
      var valor = oData.db121_estrutural;
      var texto = oData.db121_descricao;
      var valorConcat = valor + " - " + texto;
      F.options[F.length] = new Option(valorConcat,id);
      js_trocacordeselect();
    });
  }

  function js_preenchepesquisa(chave){
    location.href = "<?=basename($_SERVER["PHP_SELF"])?>"+"?chavepesquisa="+chave;
  }

  function js_insSelect(){
  var valor = document.form1.q127_sequencial.value;

  var url = "iss1_ativid.RPC.php";
  var oObject = new Object();

  oObject.sequencial = valor;
  oObject.exec      = 'insereDadosServico';

  var objAjax = new Ajax.Request(url, {
    method: "post",
    parameters: 'json=' + Object.toJSON(oObject),
    onComplete: js_insereDadosServico
  });
}

function js_insereDadosServico(oAjax){
  ...
  oRetorno.dados.each(function(oData,iContador){
    ...
    if(texto != "" && id != "") {
      ...
      for(var x = 0; x < F.length; x++) {
        ...
        if(F.options[x].value == id || F.options[x].text == texto) {
