...
<body>
    ...
    <form method="POST" action="">
      ...
      <?php endif; ?>
        ...
        <div class="col-md-12">
            <input
                type="checkbox"
                name="visulizarEmOutraJanela"
                id="visulizarEmOutraJanela"
                class="form-check-inline"
                <?=$this->visulizarEmOutraJanela =="true" ? "checked" : ""?>
            >
            <label for="visulizarEmOutraJanela" >Visualizar documentos em outra janela</label>
        </div>

        <?php if (db_getsession("DB_administrador") == 1) : ?>  
          <div class="input">
            <div class="buttons" style="position:relative;top:1px;bottom:2px;">
              <button type="button" id="processos-sistema" class="btn btn-small btn-link" 
              value="">Processos Ativos no Sistema
              </button>
            </div>
          </div>
  ...
  <script type="text/javascript">
    ...
    ;(function($, global) {
      ...
      $('#processos-sistema').on('click', function(event) {
            js_OpenJanelaIframe('CurrentWindow.corpo',
                                'db_iframe_processo',
                                'con4_dbProcesso.RPC.php',
                                'Pesquisar Processos',
                                true); 
            CurrentWindow.close();                   
      });
