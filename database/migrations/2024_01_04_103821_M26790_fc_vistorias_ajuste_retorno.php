...
class M26790FcVistoriasAjusteRetorno extends Migration
{
    /**
     * Run the migrations.
     *
...
DECLARE
  ...
  nValorVistoriaComDesconto   float8;
  nValorDescontoVistoria      float8;
  sSql                        text default '';
  nValorExcedente             float8;
  iQuantidadeAtividades       integer default 0;

  iFormulaCalculoVistoria     integer;
  iPercentualDescontoVistoria integer;
  ...
  BEGIN
    ...
    if iFormulaCalculoVistoria =  1 and v_tipo_quant = 1 then /* Normal / por área */
       lPorArea = true;
    end if;
    ...
    if V_Y74_CODSANI = 0 OR V_Y74_CODSANI is null then
      ...
      if  V_Q138_CARACTERISTICA = 33 then
        return '33 - INSCRIÇÃO COM CARACTERÍSTICA VISTORIA: ISENTO';
      end if;
    ...
    if iFormulaCalculoVistoria = 1 then
      ...
      with isencaoinscricoes as (
          ...
          from
              isencaoinscr
          where
              isencaoinscr.v16_inscr = V_Y71_INSCR

          union all

          select
               v12_isencao as sequencialisencao
          from
              isencaocgm
          where
              isencaocgm.v12_numcgm = V_Q02_NUMCGM
      ),
      isencoes as (
          select
              v10_sequencial,
      ...
      select percentual
      from percentualisencaocalculos
      into iPercentualDescontoVistoria 
      where (lIsSanitario = true and sequencialcalculo = 8) OR (V_INSCR = true and sequencialcalculo = 5)
      limit 1;
        ...
        FOR rSaniAtividade IN
          ...
          if calc_baixo_risco is false then
            ...
            if V_INSCR_BAIXO_RISCO != 0 OR V_INSCR_BAIXO_RISCO is not null then
              ...
              return '26 - INSCRIÇÃO COM GRAU DE RISCO BAIXO, SE DESEJA CALCULAR ALTERE PARA SIM O PARÂMETRO GERAR CÁLCULO PARA BAIXO RISCO';
            end if;
          end if;
          ...
          if iCodigoReceitaExercicio is not null then
            ...
            /**
             * Aplicacao de desconto de vistoria
             */
            if iPercentualDescontoVistoria is not null and iPercentualDescontoVistoria > 0 then
              nValorDescontoVistoria    = cast(nValorVistoria as float) * ((cast(iPercentualDescontoVistoria as float) / cast(100 as float)));
              nValorVistoriaComDesconto = cast(nValorVistoria as float) - cast(nValorDescontoVistoria as float);
              nValorVistoria            = cast(round(nValorVistoriaComDesconto, 2) as float);
            end if;

            if round(nValorVistoria, 2) <= 0 then
              continue;
            end if;
...
DECLARE
  ...
  BEGIN
    ...
    if V_Y74_CODSANI = 0 OR V_Y74_CODSANI is null then
      ...
      if  V_Q138_CARACTERISTICA = 33 then
        return 'INSCRIÇÃO COM CARACTERÍSTICA VISTORIA: ISENTO';
      end if;
