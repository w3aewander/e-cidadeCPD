...
$anoSesao = db_getsession("DB_anousu");
if (strlen($data1) < 7) {
    $data1 = $anoSesao . "-01-31";
}
if (strlen($data2) < 7) {
    $data2 = $anoSesao . "-12-31";
}
 //---------
if (isset($lista)) {
    $w = "(";
...
if ($listarec != "") {
    ...
    if (isset($verrec) and $verrec == "com") {
        ...
        $txt_where = $txt_where . " and orctiporec.o15_recurso not in  ($recursos)";
    }
}
 ...
 $sql_analitico= "
    select o56_codele,
      ...
      inner join conplanoreduz aa on aa.c61_reduz = c69_credito and aa.c61_anousu = {$anoSesao}
      inner join conplano bb on bb.c60_codcon  = aa.c61_codcon and  bb.c60_anousu  = aa.c61_anousu
      inner join conplanoreduz cc on cc.c61_reduz= c69_debito and cc.c61_anousu = {$anoSesao}
...
if ($quebra == "g" and $tipo == "a") {
    ...
    for ($x = 0; $x < $rows; $x++) {
        ...
        if ($repete != $o56_codele) {
            ...
            if ($x > 1) {
                $sql_resumido = "
                    select c53_coddoc as doc, c53_descr as descr, sum(c70_valor) as total
                      from conlancamcgm
                     inner join conlancam on c70_codlan=c76_codlan
                     inner join conlancamdot on conlancamdot.c73_codlan = conlancam.c70_codlan
                     inner join orcdotacao on o58_coddot=c73_coddot and o58_anousu=c73_anousu
                     inner join orctiporec on orctiporec.o15_codigo = orcdotacao.o58_codigo
                     inner join conlancamdoc on c71_codlan=conlancam.c70_codlan
                     inner join conhistdoc on c53_coddoc=conlancamdoc.c71_coddoc
    ...
    for ($x = 0; $x < $rows; $x++) {
        ...
        if ($repete != $o56_codele) {
            ...
            if ($x > 1) {
                ...
                //-- //classe conlancamcgm
                $sql01 = "
                    select c53_coddoc as doc,c53_descr as descr,sum(conlancam.c70_valor) as total
                      from conlancamdot
                     inner join orcdotacao  on o58_coddot=c73_coddot and o58_anousu=c73_anousu
                     inner join orctiporec on orctiporec.o15_codigo = orcdotacao.o58_codigo
                     inner join orcelemento on o56_codele=orcdotacao.o58_codele and o56_anousu = orcdotacao.o58_anousu
