...
<script>
  ...
  function js_retornoGetDados(oAjax){
    ...
    oJson  = JSON.parse(oAjax.responseText);
    if (oJson.status == 2){ //erro na consulta
      ...
      if (oJson.totalItens > 0){//3
        ...
        processoEntradaNota = oJson.processoEntradaNota;
  ...
  function js_anularEntrada() {
    ...
    if (lEmpenhoMaterialPermanente || processoEntradaNota == 3) {
  ...
  function js_retorno(oAjax){
    ...
    if (oJson.status == 2) { //erro na consulta
      ...
      jan = window.open('rel_entradanotaavulsa.php?id='+oJson.idnrm,'','width='+(screen.availWidth-5)+',height='+(screen.availHeight-40)+',scrollbars=1,location=0 ');
      jan.moveTo(0,0);
    }
  }
  function js_reset(){
