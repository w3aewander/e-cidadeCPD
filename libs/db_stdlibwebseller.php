...
function db_inputdatasaude( $intEspecmed,
  ...
  //#20#//Nome Secund‡rio : Nome do input que ser‡ gerado, assumindo somente as caracterÏsticas do campo Nome
  ...
  //#99#//
  //#99#//O sistema ger‡ para a primeira data incluÏda um formul‡rio, um objeto de JanelaIframe do nosso
...
//fun„·o para mostrar mensagens de aviso ao usu‡rio
...
function DiasLetivos($data_inicio,$data_fim,$sabado,$calendario,$retorno) {
  ...
  $mi = date('m', $data_in);
  $semanas = 1;
  #se o dia da entrada + total de dias for menor que total de dias do mes, ou seja, se n·o passar do mesmo m∂s.
  if ($dias+$d <= $days_month) {
    for ($i = 0; $i < $dias+1; $i++) {
      $letivo = true;
      $day++;
      #checa o dia da semana para cada dia do m∂s, se for igual a 0 (domingo) ou 6 (sabado) ele adiciona 1 no dia n·o …til
      if ($sabado=="N") {
    ...
    #se o dia da entrada + total de dias for maior que total de dias do mes, ou seja, se passar do mesmo m∂s.
  }else{
    #enquanto o m∂s de entrada for diferente do m∂s de saida ou ano de entrada for diferente do ano de saida.
    while($m != $m2 || $y != $y2) {
      ...
      #se o mes for igual a 12 (dezembro), mes recebe 1 (janeiro) e ano recebe +1 (pr◊ximo ano)
      if ($m == 12) {
        $m = 1;
        $y++;
        #m∂s recebe mais 1 para fazer o mesmo processo do pr◊ximo m∂s
      }else{
        $m++;
      }
      $d = 1;
...
function ResultadoFinal($ed60_i_codigo,$ed60_i_aluno,$ed60_i_turma,$ed60_c_situacao,$ed60_c_concluida, $iCodigoEnsino = null) {
  ...
  if ( in_array( trim($ed60_c_situacao), array("CLASSIFICADO", "AVAN∫ADO", "RECLASSIFICADO") ) ) {
      ...
      if (strstr($res_final," ")) {
        ...
        if ($ed60_c_concluida=="S") {
          ...
          if (strstr($res_final,"R")) {
            ...
            $resultado = "APROVADO COM PROGRESSAO PARCIAL /DEPEND NCIA";
          } else {
            $resultado = Situacao($ed60_c_situacao,$ed60_i_codigo);
          }
        } else {
...
function data_farmacia($ano,$tipo='1T') {
  ...
  if ($tipo=='1T') { ////COME∫A TRIMESTRE
    ...
    echo "Datas inv‡lidas tipo=$tipo ano=$ano ";
    exit;
  }
...
/**
 * 01/03/2013 - Adicionado o par¿metro $sGeraResultado, que controla o jon com a tabela procresultado
...
function GradeAproveitamentoSQL($matricula,$relatorio="N", $iAno, $sGeraResultado = "S") {
  ...
  ## 01 - Descri„·o da Disciplina
  ## 02 - Array contendo informa„Ëes de cada elemento(periodo de avalia„·o e/ou resultado)
  ##      do procedimento de avalia„·o, separados por "|"
  ##      [0] - sequencial
  ##      [1] - abreviatura da descri„·o do elemento
  ##      [2] - forma de avalia„·o do elemento
  ##      [3] - aproveitamento do aluno no elemento
  ##      [4] - n? de faltas do aluno no elemento
  ##      [5] - se o elemento ÷ periodo de avalia„·o ou resultado: AVA->Periodo de Avalia„·o | RES->Resultado
  ##      [6] - ordena„·o do elemento no procedimento de avalia„·o
  ##      [7] - se aluno atingiu aproveitamento minimo no elemento: S->Sim | N->N·o
  ##      [8] - descri„·o do elemento
  ##      [9] - se o elemento est‡ amparado: S->Sim | N->N·o
  ## 03 - Nota Parcial
  ## 04 - Total de Aulas Dadas ou Dias Letivos
  ## 05 - Percentual de Frequ∂ncia
  ## 06 - Aproveitamento do aluno no Resultado Final
  ## 07 - Resultado Final da matrÏcula: APR->Aprovado | REP->Reprovado
  ## 08 - Total de Faltas
  ## 09 - Total de Faltas Abonadas
  ## 10 - MÏnimo para Aprova„·o do Resultado que gera o Resultado Final
  ## 11 - Descri„·o dos NÏveis da Forma de Avalia„·o, caso a turma tenha forma de avalia„·o NIVEL
  ## 12 - Se matricula est‡ concluida: S->Sim | N->N·o
  ## 13 - Situa„·o da MatrÏcula
  ## 14 - C◊digo do Aluno
  ## 15 - C◊digo da Turma
  ## 16 - Descri„·o da Etapa
  ## 17 - Se par¿metro Calcular M÷dia Final est‡ habilitado: S->Sim | N->N·o
  ## 18 - Se par¿metro Casas Decimais est‡ habilitado: S->Sim | N->N·o
  ## 19 - Medidor da Frequ∂ncia: PERIODOS ou DIAS LETIVOS
  ## 20 - Forma de Avalia„·o do Resultado que gera o Resultado Final
  ## 21 - Convencoes de Amparo
  ## 22 - Forma de Obten„·o do Resultado que gera o Resultado Final
  if ($relatorio=="S") {
...
function GradeAproveitamentoSQL2($matricula,$relatorio="N", $iAno, $sGeraResultado = "S",$coddisciplina) {
 ...
 regencia.ed59_i_codigo as codigo_disciplina,
       ...
       WHERE matricula.ed60_i_codigo = $matricula
       AND matriculaserie.ed221_c_origem = 'S'
       AND ed232_i_codigo = $coddisciplina
       ORDER BY regencia.ed59_i_ordenacao
       ";
  return $sql;
}
  ...
  if (pg_numrows($result)>0) {
    ...
    for($y=0;$y<pg_num_rows($result);$y++) {
      ...
      if ($oDisciplinasDiario) {
        ...
        * Demanda: 17633
        */
        $sNomeCalendario = $oDisciplinasDiario->getDiario()->getTurma()->getCalendario()->getDescricao();
        if (strpos($sNomeCalendario, 'ANOS FINAIS') !== false) {
            $iEscola = db_getsession("DB_coddepto");
            $iCalendario = $oDisciplinasDiario->getDiario()->getTurma()->getCalendario()->getCodigo();
            $objFreq = calcularFrequenciaAnosFinaisLibWebSeller($aluno, $iEscola, $iCalendario);
            $frequencia = $objFreq->nPercentualFrequencia . '%';
        } else {
      ...
      if ($y==0) {
        ...
        if (trim($periodos)=="") {
          ...
          echo '<td class="cabec1" colspan="2">Frequ∂ncia</td>';
        ...
        if (trim($periodos)=="") {
          ...
          echo '<td class="cabec1">'.(trim($medfrequencia)=="PERÕODOS"?"Aulas":"Dias").'</td>';
      ...
      if (trim($periodos)=="") {
        ...
        for($x=0;$x<count($array_peraval);$x++) {
          ...
          if ($arr_explode[5] == "RES") {
            $arr_explode[3] = ArredondamentoNota::formatar($notavirgula, $iAno);
          ...
          echo '<td class="aluno1">'.(trim($arr_explode[3])==""?"&nbsp;":trim($notavirgula)).'</td>';
    ...
    if (trim($periodos) != '' || trim($situacao) == 'AVAN∫ADO' || trim($situacao) == 'CLASSIFICADO') {
...
function GradeAproveitamentoHTML2($matricula, $mostraresfinal="S", $iAno, $coddisciplina) {
  ...
  //$sql = GradeAproveitamentoSQL($matricula, "N", $iAno);
  $sql = GradeAproveitamentoSQL2($matricula, "N", $iAno, "S", $coddisciplina);
    ...
    for($y=0;$y<pg_num_rows($result);$y++) {
      ...
      if (trim($periodos)=="") {
        ...
        for($x=0;$x<count($array_peraval);$x++) {
          ...
          $arr_explode[3] = str_replace(".", ",", $arr_explode[3]);
...
function GradeAproveitamentoPDF( $matricula, $largura, $pdf, $mostraresfinal = "S", $iAno ) {
  ...
  if (pg_numrows($result) > 0) {
    ...
    /**
     * Percorre os dados retornados primeiramente para montar o cabe„alho correto, de acordo com o procedimento de
     * avalia„·o da turma
     */
    for ($y = 0; $y < pg_num_rows($result); $y++) {
      ...
      /**
       * Caso o procedimento de avalia„·o da disciplina seja o mesmo da turma, imprime o cabe„alho com as colunas
       * corretas
       */
      if(    $oDisciplinasDiario->getRegencia()->getProcedimentoAvaliacao()->getFormaAvaliacao()->getTipo() ==
        ...
        if( trim( $oDadosGrade->periodos ) != "" && trim( $oDadosGrade->periodos ) != "{}" ) {
          ...
          $pdf->cell( $largura_rf / 2, 4, "Frequ∂ncia",      1, 0, "C", 0 );
        ...
        if( trim( $oDadosGrade->periodos ) == "" || trim( $oDadosGrade->periodos ) == "{}" ) {
          ...
          $pdf->cell( $largura_meio + $largura_rf, 4, "Observa„Ëes", 1, 1, "C", 1 );
        } else {
          ...
          $sFrequencia = trim( $oDadosGrade->medfrequencia ) == "PERÕODOS" ? "Aulas" : "Dias";
    ...
    /**
     * Ap◊s montado o cabe„alho, percorre novamente os dados, desta vez para montagem dos dados de cada disciplina
     */
    for ($y = 0; $y < pg_num_rows($result); $y++) {
...
/**
 * 01/03/2013 - Adicionado o par¿metro $sGeraResultado, que ÷ levado para a fun„·o GradeAproveitamentoSQL
 *
 * @param unknown_type $matricula
...
function GradeAproveitamentoBOLETIM($matricula, $largura, $pdf, $niveis, $orientacao, $seqatual, $ficha = "N", $sGeraResultado = "S") {
  ...
  if ($orientacao == "P") {
    ...
    $largura_disc = $largura*20/100;//coluna da descri„ao da disciplina 23%
    $largura_meio = $largura*53/100;//coluna dos elementos da avalia„ao 50%

  } else {

    $largura_disc = $largura*18/100;//coluna da descri„ao da disciplina 18%
    $largura_meio = $largura*55/100;//coluna dos elementos da avalia„ao 55%
  ...
  if (pg_numrows($result) > 0) {
    ...
    /**
     * Monta o cabe„alho da grid e define o tamanho das colunas a serem impressas de acordo com os perÏdos de avalia„·o.
     */
    for ($y = 0; $y < pg_num_rows($result); $y++) {
      ...
      if ( count($aPeriodosCabecalho) > 0 && $lImprimeCabecalho) {
        ...
        if (trim($periodos) == "") {
          ...
          $pdf->cell(($largura_rf*52/100), 4, "Frequ∂ncia", 1, 0, "C", 0);
        ...
        if (trim($periodos) == "") {
          ...
          $pdf->cell($largura_dias, 4, (trim($medfrequencia) == "PERÕODOS" ? "AD" : "DL"), 1, 0, "C", 0);
    ...
    /**
     * Percorre as disciplinas e as imprime de acordo com o tamanho das colunas calculadas ao montar o cabe„alho
     */
    for ($y = 0; $y < pg_num_rows($result); $y++) {
      ...
      if (trim($periodos) == "") {
        ...
        for ($x = 0; $x < count($array_peraval); $x++) {
          ...
          if (isset($arr_explode[5]) && trim(@$arr_explode[5]) == "AVA" && @$arr_explode[10] == "S") {
            ...
            /**
             * Alterado valor da avalia„·o quando = "Amparo" para informar somente "Amp"
             */
    ...
    if ($formafinal != "PARECER") {
      ...
      $pdf->cell(43, 4, "MÏnimo para Aprova„·o Anual:", "LT", 0, "L", 0);
    ...
    if ($niveis == "yes") {
      ...
      $pdf->setfont('arial', 'b', 8);
      $pdf->cell($largura, 4, "NÏveis:", 1, 1, "L", 1);
      $pdf->setfont('arial', '', 7);
...
/**
 * Fun„·o para montar um INPUT do tipo COMBOBOX com os DIRETORES e SECRET?RIOS.
 *
  ...
  if (pg_num_rows($result_hist)>0) {
    ...
    if ($ult_resfinal=="R") {
      ...
      if ($ult_cod!=$etapaindicada) {
        ...
        $temequiv  = false;
        $msgequiv .= "\\nATEN∫?O: Aluno(a) $aluno tem a Etapa $ult_descr ($ult_descrensino - $ult_abrevensino) como a …ltima cursada, na situa„·o de $descr_situacao. Selecione uma turma que contenha alguma das etapas abaixo relacionadas:\\n";
      ...
      //$restricao = true;
      //$msgequiv .= "\\nATEN∫?O: Aluno(a) $aluno tem a Etapa $ult_descr ($ult_descrensino - $ult_abrevensino) como a …ltima cursada, na situa„·o de $descr_situacao. N·o existem etapas ap◊s esta descrita acima.\\n";
      //}
...
/**
 * Fun„·o que gerar arquivo do BPA
 * @param  object   $oDados
 * @param  resource $rsCabecalho
 * @param  resource $rsProducao
 * @param  booleano $lValidaCid
 * @param  string   $sArquivo
 * @return integer  -1 conven„·o para falha durante a gera„·o, numeros maiore que 0(zero) representan numero de BPA's
 */
function geraArquivoBPA($oDados, $rsCabecalho, $rsProducao, $lValidaCid = true, $sArquivo = "/tmp/arquivobpa.txt") {
  ...
  /* Escreve no arquivo o cabe„alho */
  $cldb_layouttxt->setByLineOfDBUtils($oCabecalho, 1);

  /* Percorre todas as linhas da produ„·o */
  for ($iIndice = 0; $iIndice < $oDados->iLinhas; $iIndice ++) {
    ...
    if ($oDados->sTipo == "01") {
      ...
      /* Valida informa„Ëes antes de gerar o arquivo */
  ...
  if (($lErro == true) && ($oDados->sTipo == "02")) {
    ...
    db_msgbox("Arquivo possui inconsist∂ncias. Verifique!");
...
/*
 * Fun„·o que soma dias, messes e ou anos a uma determinada data
 * @descr fun„·o que soma uma quantidade de dias, meses e ou anos œ data passada nos par¿metros $iDia, $iMes e $iAno.
 * @param int $iDia dia da data base para o c‡lculo
 * @param int $iMes mes da data base para o c‡lculo
 * @param int $iAno ano da data base para o c‡lculo
 * @param int $iDiaSomar quantidade de dias a serem somados œ data passada
 * @param int $iMesSomar quantidade de meses a serem somados œ data passada
 * @param int $iAnoSomar quantidade de anos a serem somados œ data passada
...
function geraArquivoHiperdia($iTipo, $sArquivo, $rsRegistros, $iLinhas) {
  ...
  if ($iTipo == 0 || $iTipo == 4) { // $rsRegistros j‡ vem com os dados prontos
  ...
  for ($iIndice = 0; $iIndice < $iLinhas; $iIndice++) {
    ...
    or die ('Erro na gera„·o do Arquivo. Contate o administrador do sistema.');
    ...
    if ($iTipo == 2) {
      ...
      if (empty($oRegistro->sexo)) {
        ...
        $aCgs[$oRegistro->z01_i_cgsund]['sexo'] = 'Sexo n·o informado';
        $lErro = true;
      ...
      if (empty($oRegistro->data_nascimento)) {
        ...
        $aCgs[$oRegistro->z01_i_cgsund]['datanasc'] = 'Data de nascimento n·o informada';
        $lErro = true;
      ...
      if (empty($oRegistro->nome_mae)) {
        ...
        $aCgs[$oRegistro->z01_i_cgsund]['datanasc'] = 'Nome da m·e n·o informado.';
        $lErro = true;
...
/**
 * Carrega a tabela de configura„·o de qualquer modulo
 * @param  string $sTabela - Nome da tabela no banco
 * @param  string $sCondicao - Condi„·o (where) do select
...
/*
 * fun„·o par somar minutos numa hora (tem especificidades para ser utilizada nos hor‡rios do m◊dulo agendamento
...
/*
 * fun„·o par somar minutos numa hora
...
/*
 * fun„·o retorno minutos  entre duas horas
 * @author Cristian Tales
 * @revision Tony F. B. M. Ribeiro
 ...
 * @descr Fun„·o que verifica se algum dos agendamentos passados no array $aAgendamentos est‡ alocado
          para o hor‡rio da grade indicado nos par¿metros iIdGrade, $sTipoGrade e $iIdFicha
 ...
 * @param array $aAgendamentos -> vetor com as informacoes dos agendamentos.
                                  Nem todas as informa„Ëes s·o necess‡rias
 * @return iIdAgendamento -> indice do vetor de agendamentos que est‡ alocado para o hor‡rio da grade
...
function verificaAgendamentoHorarioByArray($iIdGrade, $sTipoGrade, $iNumFicha, $aAgendamentos) {
  ...
  if ($sTipoGrade == 'I') { // Pra grade do tipo intervalo, basta verificar se o n…mero da ficha ÷ igual
  ...
  } else { // Pra grade do tipo perÏodo, basta verificar se o c◊digo da grade ÷ o mesmo
  ...
  // Nenhum agendamento est‡ alocado para o hor‡rio da grade
  return -1;
...
/*
 * @author Tony Farney B. M. Ribeiro
 * @descr Fun„·o que verifica se existe algum dos agendamento para o horario
          de $sHoraIni a $sHoraFim nas datas de $dIni a $dFim para a especmedico $iEspecMed
 * @param string $sHoraIni Hor‡rio de inÏcio para verificar (formato HH:MM)
 * @param string $sHoraFim Hor‡rio de fim para verificar (formato HH:MM)
 * @param string $sHoraIni Data de inÏcio para verificar (formato YYYY-MM-DD)
 * @param string $sHoraIni Data de fim para verificar (formato YYYY-MM-DD)
 * @param int $iEspecMedico Codigo da tabela especmedico
 * @return boolean Indica se existe ou n·o hor‡rio marcado para o hor‡rio indicado no perÏodo indicado
 */
function verificaAgendamentoHorario($sHoraIni, $sHoraFim, $dIni, $dFim, $iEspecMed) {
  ...
  if ($iTam > 0) {
    ...
    for ($iCont = 0; $iCont < $iTam; $iCont++) {
      ...
      /* Hor‡rio final que j‡ estaria agendado (mas como pode haver ausencias neste perÏodo,
         os agendamentos podem acabar sendo movidos para hor‡rios mais adiante */
      ...
      for ($iCont2 = 0; $iCont2 < $oDaoAusencias->numrows; $iCont2++) {
        ...
        // Se a ausencia come„ar no perÏodo j‡ agendado, tenho que aumentar o tempo de aus∂ncia ao perÏodo j‡ ocupado
        if ($oAusencia->sd06_c_horainicio >= $aGradesPeriodo[$iCont]->sd30_c_horaini
          ...
          /* Se a aus∂ncia terminar no perÏodo j‡ agendado, mas come„ar antes dele,
             tenho que aumentar o tempo do inicio da grade at÷ o fim da aus∂ncia ao perÏodo j‡ ocupado */
      ...
      // Se tiver mais de uma aus∂ncia, somo um para compensar a imprecis·o
  ...
  // Ainda n·o encontrou nenhuma interse„·o entre o hor‡rio da aus∂ncia e o dos agendamentos
  if ($iTam > 0) {
...
/*
 * @author Tony Farney B. M. Ribeiro
 * @descr Fun„·o que calcula o IMC (?ndice de Massa Corporal)
 * @param float $nPeso Peso em quilos
 * @param int $iAltura Altura em centÏmetros
 * @return array $aRetorono Primeira posi„·o: float IMC; Segunda posi„·o: Descri„·o do IMC segundo tabela.
    Retorna vazio se o c‡lculo n·o for possÏvel de ser realizado.
 */
...
/*
 * @author Tony Farney B. M. Ribeiro
 * @descr Fun„·o que gera o select dos modelos de FAA disponÏveis.
...
/*
 * @author Adriano Quili·o de Oliveira
 * @descr Fun„·o que retorna a quantidade de cotas para uma UPS solicitante e se o controle ÷ feito por cotas ou n·o
 * @param int $iUpssolicitante    Unidade portadora das cotas
 * @param int $iUpsprstadora      unidade que est‡ fornecendo as cotas
 * @param int $sRh70_estrutural   C◊digo estrutural da especialidade
 ...
 * @param int $iUndmedhorario     C◊digo referente ao dia da grade de hor‡rio
 * @param int $iMedico            M÷dico a ter as cotas validadas
...
function getCotasAgendamento(
  ...
  $oResult->lStatus           = 0;
  $oResult->sMensagem         = "N·o encontrado o controle por cotas para a UPS solicitante.";
...
/**
 * Soma os valores dos elementos do vetor em cada posi„·o
...
/**
 * Fun„·o que verifica a quantidade de matriculados em uma determinada turma.
 *
 * @param Integer $iTurma         -> C◊digo da turma que se deseja verificar
 * @param Integer $iEscola        -> C◊digo da escola da turma, caso seja null busca-se o departamento logado
 * @param Integer $iTipoRetorno   -> Caso o tipo seja 1 -> Retorna TRUE ou FALSE (Turma tem ou n·o vagas disponÏveis)
 ...
 *                 FALSE          -> Caso a turma n·o possua mais vagas para matrÏcula
 * if tipoRetorno == 2 (Obs.: Usado quando existe chave_pesquisa)
 * @return Integer $iMatriculados -> N…mero de alunos matrÏculados na turma
 *
...
function verificaMatriculados($iTurma, $iEscola = null, $iTipoRetorno = 1) {
  ...
  if ($iLinhasTurma > 0) {
    ...
    /* Busco a quantidade de alunos que j‡ est·o matrÏculados na turma */
    $oDaoMatricula = db_utils::getdao('matricula');
    ...
    db_msgbox("N·o foi possÏvel localizar a turma informada.");
    return false;
...
function trocaTurma($iSeqMatricula, $iCodigoTurmaDestino, $lAproveitamento = false, $iMatriculaOrigem = null, $sTurno = null, $lValidaEtapataOrigem = true) {
  ...
  if (!db_utils::inTransaction()) {
    throw new Exception('N·o existe transa„·o ativa com o banco de dados.');
  }
  ...
  if ($oDaoTurma->numrows == 0) {
    throw new Exception("Turma de destino sem s÷ries vinculadas");
  }
  ...
  if ($oDaoMatricula->numrows == 0) {
    throw new Exception("Matricula {$iSeqMatricula} inv‡lida.");
  }
  ...
  if ($oMatriculaAtual->ed60_i_turma == $iCodigoTurmaDestino) {
    throw new Exception("Turma de destino n·o pode ser a mesma de origem.");
  }
  ...
  /**
   * Alterado l◊gica que identifica a etapa de origem para validar se etapa da turma de destino = etapa turma de origem
   * Como essa fun„·o ÷ chamada somente quando ensino turma origem = ensino turma de destino o c◊digo das etapas ser·o iguais
   */
  ...
  for ($i = 0; $i < $iTotalSeriesTurma; $i++) {
    ...
    /**
     * L◊gica aplicada quando fun„·o chamada da rotina de Alterar Situa„·o da Matricula e selecionado a
     * op„·o: MATRICULA INDEVIDA ÷ possÏvel selecionar uma turma de qualquer etapa do ensino (maior ou menor que etapa atual)
   ...
   * devemos atualizar todos os dados da turma, para garantir que n·o havera
   * informa„Ëes incorretas, pois pode haver variaves com o nome do campo no escopo global.
  ...
  for ($iDiario = 0; $iDiario < $iTotalDiarios; $iDiario++) {
    ...
    if ($oDaoDiarioClasse->erro_status == 0) {
      throw new Exception("Erro ao cancelar diarios de avalia„·o do aluno\n{$oDaoDiarioClasse->erro_msg}");
    }
  }
  ...
  if ( !empty( $sTurno ) ) {
    ...
    /**
     * Busca os registros da turma na tabela turmaturnoreferente para inserir a matrÏcula em matriculaturnoreferente
     */
...
/**
 * Verifica qual ÷ a forma de calculo de frequencia. Se for globalizada, ent·o verifica se h‡ ao menos uma disciplina
 * com reclassifica„·o por baixa frequ∂ncia, se n·o valida por disciplina.
...
*/
function calcularFrequenciaAnosFinaisLibWebSeller($iCodigoAluno, $iCodigoEscola, $iCodigoCalendario) {
    ...
    // Regra 75% mÏnimo para reclassificados
    if ($nFrequencia < 75) {
        $nFrequencia = 75;
    }
