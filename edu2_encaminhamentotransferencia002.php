...
// 2018-06-18 Wallace Esta página era referente Atestado de Frequencia e se tranformou em Declaração de Matrícula como uma nova  página php
...
foreach ($oParametros->aMatriculas as $oMat) {
  ...
  $oParagrafo                         = new libdocumento(5027);
  ...
  $oParagrafo->municipio_naturalidade         = $naturalidade; 
  $oParagrafo->estado_naturalidade  = ""; 
  $oParagrafo->uf_naturalidade      = "";     

  if ( !empty($oParagrafo->municipio_naturalidade) ) {
  ...
  $oParagrafo->nome_aluno             = $oMatricula->getAluno()->getNome();
...
$head1 = "ENCAMINHAMENTO DE TRANSFERÊNCIA";
...
foreach ($aDadosAlunos as $oDadosAlunos) {
  ...
  $oPdf->Cell(192, $oParametros->iAlturaLinha, "Encaminhamento de Transferência", 0, 1, "C"); 
  ...
  if ($oPdf->GetY() + 40 > $oPdf->h - 15) {
    ...
    $oPdf->setfont('arial','',$tamanhoFonte-3);
    $oPdf->Cell("192",320-$oPdf->GetY(), 'Este documento só é válido para as transferências realizadas entre as Unidades Escolares da Rede Municipal de Ensino de Petrópolis.', 0, 1, "C");
}
