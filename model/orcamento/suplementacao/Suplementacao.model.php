...
class Suplementacao
{
    ...
    public function getvalorSuplementacao()
    {
        ...
        if ( $this->getTipo() == 1050 ) {

            $sSqlTotalSuplementacao .= "
               union all
                     SELECT sum(o85_valor) as soma_suplem
                       FROM ORCSUPLEM
                       join orcsuplemrec on o85_codsup = o46_codsup
                      where O46_CODSUP = {$this->iCodigo} and  o85_valor > 0
            ";
        }
        ...
        foreach ($aSuplementacoes as $oSuplementacao) {
            $nValorSuplementacoes += (float)$oSuplementacao->soma_suplem;
        }
    ...
    public function getValorReceita()
    {
        ...
        $nValorReceitas = 0;
        $aReceitas = db_utils::getCollectionByRecord($rsTotalReceita);
        foreach ($aReceitas as $oReceita) {
            $nValorReceitas += (float)$oReceita->soma_suplem;
        }
        unset($aReceitas);
    ...
    public function processarDadosPPA()
    {
        ...
        $aDotacoesPPA = db_utils::getCollectionByRecord($rsDotacoes);
        foreach ($aDotacoesPPA as $oDotacaoPPA) {
            ...
            if (pg_num_rows($rsVerificaDotacao) == 1) {
                ...
                if ($oDaoOrcDotacao->numrows == 1) {
                    ...
                    $sSqlCodDot
                        = "update orcparametro set o50_coddot = o50_coddot + 1 where o50_anousu = {$iAnoIntegracao}";
                    $rsCodDot = db_query($sSqlCodDot);
                    if (!$rsCodDot) {

                        $sErroMsg
                            = "Erro ao gerar número da dotação. Verifique o cadastro dos paramêtros do orçamento para {$iAnoIntegracao}.\n";
        ...
        $aReceitasPPA = db_utils::getCollectionByRecord($rsReceitas);
    ...
    public function processar($dtData)
    {
        ...
        $sSqlValidaSuplementacao
            = "select fc_lancam_suplementacao({$this->iCodigo}, '{$dtData}', {$iUsuario}) as retorno";
         ...
         */
        $sSqlLancamentos = " select  codsup, o48_tiposup,tipo,dot,valor,o48_coddocsup, linhapacto,o48_arrecadmaior ";
        /**
         * suplementacoes
         */
        $sSqlLancamentos .= "   from (select o47_codsup as codsup, ";
        $sSqlLancamentos .= "                's'::char(1) as tipo, ";
        $sSqlLancamentos .= "                o47_coddot as dot,  ";
        $sSqlLancamentos .= "                o47_valor as valor, ";
        $sSqlLancamentos .= "                 o47_planoorcamentariolinhapacto  as linhapacto ";
        ...
        $sSqlLancamentos .= "                  0 as linhapacto ";
        ...
        //$sSqlLancamentos = " select * from ($sSqlLancamentos) as ss where tipo = 'rec'";


        $rsVerificaLancamentos = db_query($sSqlLancamentos);

        //db_criatabela($rsVerificaLancamentos);

        if (pg_num_rows($rsVerificaLancamentos) == 0) {
            throw new Exception("Erro ao consultar dados para iniciar lançamentos contábeis.");
        }
        $aLancamentos = db_utils::getCollectionByRecord($rsVerificaLancamentos);
        ...
        foreach ($aLancamentos as $oLancamentoSuplementacao) {;
            ...
            if ($oLancamentoSuplementacao->tipo == 'rec') {
                ...
                if ($oLancamentoSuplementacao->o48_tiposup == 1050) {
                ...
                //dd("DOC: $iCodigoDocumento" ,$oTransacao );
                ...
                db_putsession('DB_instit', $oDotacao->o58_instit);
                ...
                if ($oLancamentoSuplementacao->tipo == "s") { //suplementacao
                    ...
                    if (!empty($oLancamentoSuplementacao->linhapacto)) {

                        $movimentacaoPlano = db_query("
                            insert into linhapactosaldomovimentacao
                                 values (
                                    nextval('linhapactosaldomovimentacao_o162_sequencial_seq'),
                                    {$oLancamentoSuplementacao->linhapacto},
                                    2,
                                    '{$dtData}',
                                    {$oLancamentoSuplementacao->valor}
                                 );
                        ");
                        if (!$movimentacaoPlano) {
                            throw new Exception("Ocorreu um erro ao executar a movimentação para a linha de pacto.");
                        }
                    }
           ...
           // dump("DOCUMENTO: " . $iCodigoDocumento);
