...
class M11790Recibopagaold extends PostgresMigration
{
    public function up()
    {
        $this->dicionarioUp();
        $this->reciboPagaOldUp();
        $this->baixaBancoUp();
    }
    ...
    {
        $this->execute("
insert into db_sysarquivo values (1010324, 'recibopagaold', 'Controle dos recibo baixados na tesouraria. Essa tabela serve para guardar o log da recibopaga antes de alterar a composição de tuplas do recibo.', 'k00', '2018-09-24', 'Pagamento Recibo Histórico', 0, 'f', 'f', 'f', 'f' );
insert into db_sysarqmod values (5,1010324);
insert into db_syscampo values(1009985,'k00_numpreold','int4','Código auxiliar de arrecadacao, gerado na emissão de recibo na consulta geral financeira. Para recibopagaold, se tiver valor diferente de recibopaga.k00_numnov tem o valor referente ao novo recibo gerado a partir da alteração.','0', 'Codigo Auxiliar Historico',8,'f','f','f',0,'text','Codigo Auxiliar Historico');
delete from db_sysarqarq where codarq = 1010324;
delete from db_sysarqcamp where codarq = 1010324;
insert into db_sysarqcamp values(1010324,372,1,0);
insert into db_sysarqcamp values(1010324,373,2,0);
insert into db_sysarqcamp values(1010324,374,3,0);
insert into db_sysarqcamp values(1010324,375,4,0);
insert into db_sysarqcamp values(1010324,376,5,0);
insert into db_sysarqcamp values(1010324,377,6,0);
insert into db_sysarqcamp values(1010324,361,7,0);
insert into db_sysarqcamp values(1010324,362,8,0);
insert into db_sysarqcamp values(1010324,378,9,0);
insert into db_sysarqcamp values(1010324,379,10,0);
insert into db_sysarqcamp values(1010324,736,11,0);
insert into db_sysarqcamp values(1010324,737,12,0);
insert into db_sysarqcamp values(1010324,2129,13,0);
insert into db_sysarqcamp values(1010324,1009985,14,0);
");
    }
...
delete from db_sysarqarq where codarq = 1010324;
delete from db_sysarqcamp where codarq = 1010324;
delete from db_syscampo where codcam = 1009985;
delete from db_sysarqmod where codarq = 1010324;
delete from db_sysarquivo where codarq = 1010324;
");
    }

    private function reciboPagaOldUp()
    {
        $this->execute("
create table recibopagaold(like recibopaga);
alter table recibopagaold add column k00_numpreold integer not null;
");
    }

    private function reciboPagaOldDown()
    {
        $this->execute("drop table recibopagaold;");
    }

    private function baixaBancoUp()
    {
        $this->execute(
<<<SQL
...
declare
  ...
  lPossuiCustas                    boolean default false;
  ...
  if lAtivaPgtoParcial is true then
    ...
    loop
       ...
       */
        ...
        -- @TODO - verificar essas obs, v35_tipolancamento / v35_dtpagamento
        sObservacao := 'Lista de custas pagas \n';

        nVlrTotalCustasCredito := 0;
        lPossuiCustas := false;

        for rRecordCustas in select ar36_descricao as descricao,
                                    v77_valor as valor
                               from processoforopartilhacusta
                                    inner join taxa on taxa.ar36_sequencial = v77_taxa
                              ...
                              union all
                             select ar36_descricao as descricao,
                                    v36_valor as valor
                               from inicialpartilhacustas
                                    inner join taxa on taxa.ar36_sequencial = v36_taxa
        ...
        loop
          ...
          if lRaise is true then
            perform fc_debug('  <Custas>  - Possui custas. Descricao: '||rRecordCustas.descricao || ': R$ ' || rRecordCustas.valor,lRaise,false,false);
          end if;

          lPossuiCustas := true;

          sObservacao := sObservacao || rRecordCustas.descricao || ': R$ ' || rRecordCustas.valor || '\n';

        end loop;

        if lPossuiCustas is false then
          sObservacao := '';
        end if;
        ...
        if iTipoAbatimento = 3 then
          ...
          loop
            ...
            if found and nVlrCalculado = 0 then
              ...
              update recibopaga 
                 set k00_numpre = (select k00_numpre from arrecad where k00_numpre in (select k00_numpre from recibopaga where k00_numnov = r_idret.numpre) limit 1)
               where k00_numnov = r_idret.numpre
                 and k00_hist = 11403;
...
SQL
    ...
    private function baixaBancoDown()
    {
        $this->execute(
<<<SQL
 ...
 */
  if lAtivaPgtoParcial is true then
    ...
    /*********************************************************************************
     *  Regra 0.5.6 verifica se houve um  pagamento de uma parcela supeior a que deveria ser pago
     *  caso seja isso e atualizado o numpar para parcela correta.
     ********************************************************************************/

    if lRaise then
      perform fc_debug('  <PgtoParcial> Regra 0.5.6 - VERIFICA PAGAMENTOS DUPLICADOS',lRaise,false,false);
    end if;
    ...
    if lPlugin then
         ...
         -- Adicionado somente para alguns cadtipo, segundo regra passada
         for rPagamentoRecibo  in  
                     select      
                        min(recibopaga.k00_numpar) as k00_numpar,
                        recibopaga.k00_numpre, 
                        disbanco.dtpago
                        from  
                          disbanco
         ...
         loop               
               select k00_numpre,
                      k00_numpar, 
                      k00_dtvenc into rArrecadRecibo 
                     from  arrecad 
                     inner join arretipo on  arretipo.k00_tipo = arrecad.k00_tipo 
                     where k00_numpre = rPagamentoRecibo.k00_numpre 
                     and   k00_dtvenc >= rPagamentoRecibo.dtpago  
                     and   k00_numpar = (rPagamentoRecibo.k00_numpar - 1) and k03_tipo not in (15, 13, 18);
                    
               if found then
                    update recibopaga  set k00_numpar = (rArrecadRecibo.k00_numpar) 
                     where k00_numpre = rArrecadRecibo.k00_numpre and k00_numpar = rPagamentoRecibo.k00_numpar;     
               end if;   
         end loop;                                    
    end if; 
