...
function bb_condicaosubpesatual($prefixo=null){
 $submes = DBPessoal::getMesFolha();
 $subano = DBPessoal::getAnoFolha();
...
function bb_condicaosubpesanterior($prefixo=null){
 $submes = DBPessoal::getMesFolha()-1;
 $subano = DBPessoal::getAnoFolha();
...
function db_selectmaxlinha($linha,$matriz=null,$query=null, $tabela = "", $campos=" * ", $order = "", $where = "", $group = ""){
...
function db_deletelinha($linha,$tabela=null,$condicao_deleta=null,$sem_condicao=null){
...
function db_insertlinha($linha,$tabela,$mat_campos,$mat_valores,$execdie=true){
  $linha2  = "";
  $linha3  = "";
  $linha4  = "";
...
function situacao_funcionario ($registro=null,$datafim=null) {
  ...
  /*
   Busca o nome do munícipio para compor o IF
  */
  $oDaoConfig = new cl_db_config;
  $rsConfig = $oDaoConfig->sql_record($oDaoConfig->sql_query_file(null, "munic", null, "prefeitura is true"));
  $sMunicipio = db_utils::fieldsMemory($rsConfig, 0)->munic;
  $sMunicipio = strtoupper($sMunicipio);
  ...
  if ($sMunicipio == 'PORTO VELHO') {
    $condicaoaux = " and r45_regist =". db_sqlformat( $registro )." and r45_dtafas <= '$dtfim'  order by r45_regist, r45_dtafas desc"  ;
  } else {
...
function salario_base($pessoal,$Ipessoal,$cfuncao = "") {
  ...
  global $F007, $F008, $F010, $F024, $F030, $F031, $cfpess;
