...
/**
 * Classe de captura e processamento da requisição de debitos da CGF
 */
final class GeralFinanceiraDebitosRequest {
  ...
  /**
   * Processa array em sessão para propriedades utilizadas
    ...
    foreach($aDebitos as $i => $oDebito){
      ...
      if($oDebito->lChecked){
        ...
        foreach($aNumpres as $j => $sNumpre){
          ...
          $aNumpre    = explode("P", $sNumpre);
          $sNumpreAux = $aNumpre[0];

          $aNumpar    = explode("P", strstr($sNumpre, "P"));

          if($lInicial == false){

            $aNumpar    = explode("R", $aNumpar[1]);
