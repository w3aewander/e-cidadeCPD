...
class SigfisArquivoPagamento extends SigfisArquivoBase implements iPadArquivoTXTBase {
  ...
  */
  public function gerarDados() {
    ...
    $sCampos .= "round(sum(case c53_tipo when 30 then conlancam.c70_valor when 31 then (conlancam.c70_valor * -1) end),2) as valor_pago, empempenho.e60_numemp as numemp    ";   
      ...
      for ($i = 0; $i < $clConLanCamEmp->numrows; $i++) {
        ...
        if ($oDadosQuery->valor_pago == 0 ){
           continue;
			...
			$numemp = $oDadosQuery->numemp;
		   $data_pag = $oDadosQuery->c70_data;
    //$sErroLog  = "$data_pag
    //$this->addLog($sErroLog);
			...
			$sSqlTotalRetencao .= "WHERE  e50_numemp = {$numemp}";
			$sSqlTotalRetencao .= "       AND e50_data = {$data_pag} ";
...
//          $sUnidadeOrcamentaria  = str_pad($oDadosQuery->o58_orgao, 2, '0', STR_PAD_LEFT);
          ...
          //$vlrpag_liquido = $valorretido;
          
//     $sErroLog  = "ORDEM: $codordem - VALOR PAGO:$oDadosQuery->valor_pago - RETIDO:$vlrretencao - VALOR LIQUIDO: $vlrpag_liquido ----- \r";
//    $this->addLog($sErroLog);          
...
//          $oDados->vl_Pagamento           = str_pad($vlrpag_liquido, 16, ' ', STR_PAD_LEFT);
