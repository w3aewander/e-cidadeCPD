...
for ($iCopia = 0; $iCopia < $iTotalCopiasVia; $iCopia++) {
  ...
  for ($iVia = 0;$iVia < $iNumerovias;$iVia++) {
      ...
      $this->objpdf->text($xcol + 2, $xlin + 6, 'Depósito Origem');

      $this->objpdf->Setfont('Arial', '', 7);
      $this->objpdf->text($xcol + 25, $xlin + 6, ':  ' . $this->RdepartCod . ' - ' . $this->Rdepart);
      $this->objpdf->Setfont('Arial', 'b', 8);
      $this->objpdf->text($xcol + 2, $xlin + 10, 'Usuário');
      $this->objpdf->Setfont('Arial', '', 7);
      $this->objpdf->text($xcol + 25, $xlin + 10, ':  ' . $this->Rnomeus);
      $this->objpdf->Setfont('Arial', 'b', 8);
      $this->objpdf->text($xcol + 105, $xlin + 10, 'Depósito Destino');
      $this->objpdf->Setfont('Arial', '', 7);

      $this->objpdf->text($xcol + 128, $xlin + 10, ':  ' . $this->RdepartdestCod . ' - ' . $this->Rdepartdest);
