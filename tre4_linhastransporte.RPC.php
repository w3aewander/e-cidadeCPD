...
try {
  ...
  switch ($oParam->sExecucao) {
    ...
    case 'salvarKm':   
      ...
      $oLinhaItinerario = new LinhaItinerario();
      $oLinhaItinerario->setLinhaTransporte($oLinhaTransporte);
      $oLinhaItinerario->setTipo($oParam->tipoItinerario);
      $oLinhaItinerario->salvar();
      
      $itinerarioKm = new LinhaItinerarioKm(); 
      $itinerarioKm->setLinhaItinerario($oLinhaItinerario);
      $itinerarioKm->setKm($oParam->km);
      $itinerarioKm->salvar();
      $oRetorno->sMensagem = urlencode("A Km do itinerário foi salvo com sucesso!");
      break;
    
    case 'getKmItinerario':
      $itinerarioKm = new LinhaItinerarioKm();
      $oRetorno->aKm = [];
      ...
      if (isset($oParam->iCodigoLinha)) {
        $oLinhaTransporte       = new LinhaTransporte($oParam->iCodigoLinha);
        foreach ($oLinhaTransporte->getItinerarios() as $oLinhaItinerario) {
          if (empty($oLinhaItinerario->getKm())) {
            continue;
          }          
          $oRetorno->aKm[] = [
            'km' => $oLinhaItinerario->getKm(),
            'codigoItinerario' => $oLinhaItinerario->getCodigo(),
            'tipoItinerario' => $oLinhaItinerario->getTipo()                        
          ];           
        }            
      }  
      break; 
    ...
    case 'removerKm':
      $itinerarioKm = new LinhaItinerarioKm(); 
      $itinerarioKm->setItinerario($oParam->codigoItinerario);      
      $itinerarioKm->excluir();     
      $oRetorno->sMensagem = urlencode('Km do itinerário excluído com sucesso');
      break;

    case 'salvarMeioTransporteDificuldades':   
      $oLinhaTransporte = new LinhaTransporte($oParam->iCodigoLinha);
      $oLinhaItinerario = new LinhaItinerario();
      $oLinhaItinerario->setLinhaTransporte($oLinhaTransporte);
      $oLinhaItinerario->setTipo($oParam->tipoItinerario);
      ...
      $itinerarioMeioTransporte = new LinhaItinerarioMeioTransporte(); 
      $itinerarioMeioTransporte->setLinhaItinerario($oLinhaItinerario);
      $itinerarioMeioTransporte->setMeioTransporte($oParam->meioTransporte);
      $itinerarioMeioTransporte->salvar();

      $itinerarioDificuldadesAcesso = new LinhaItinerarioDificuldadesAcesso(); 
      $itinerarioDificuldadesAcesso->setLinhaItinerario($oLinhaItinerario);
      $dificuldadesAcesso = json_decode($oParam->dificuldadesAcesso);
      $itinerarioDificuldadesAcesso->setDificuldadesAcesso($dificuldadesAcesso);
      $itinerarioDificuldadesAcesso->salvar();

      $msg = "O meio de transporte e as dificuldades de acesso";
      $msg .= " do itinerário foram salvos com sucesso!";
      $oRetorno->sMensagem = urlencode($msg);
      break;   
      
    case 'getMeioTranporteDificuldades':
      $itinerarioKm = new LinhaItinerarioKm();
      $oRetorno->meioTransporteDificuldades = [];

      if (isset($oParam->iCodigoLinha)) {
        $oLinhaTransporte = new LinhaTransporte($oParam->iCodigoLinha);
        foreach ($oLinhaTransporte->getItinerarios() as $oLinhaItinerario) {

          $dificuldadesAcesso = $oLinhaItinerario->getDificuldadesAcesso();
          $meioTransporte = $oLinhaItinerario->getMeioTransporte();
          
          if(!empty($dificuldadesAcesso)){
            $oRetorno->meioTransporteDificuldades[] = [
              'porteira' =>urlencode($dificuldadesAcesso['porteira']),
              'mataburro' => urlencode($dificuldadesAcesso['mataburro']),
              'colchete' => urlencode($dificuldadesAcesso['colchete']),
              'atoleiro' => urlencode($dificuldadesAcesso['atoleiro']),
              'ponterustica' => urlencode($dificuldadesAcesso['ponterustica']),
              'meioTransporte' => !empty($meioTransporte) ? $meioTransporte : null,
              'codigoItinerario' => $oLinhaItinerario->getCodigo(),        
              'tipoItinerario' => $oLinhaItinerario->getTipo()                        
            ];
          } 
    ...
    case 'removerMeioTranporteDificuldades':
      $itinerarioKm = new LinhaItinerarioDificuldadesAcesso(); 
      $itinerarioKm->setItinerario($oParam->codigoItinerario);      
      $itinerarioKm->excluir();    

      $itinerarioKm = new LinhaItinerarioMeioTransporte(); 
      $itinerarioKm->setItinerario($oParam->codigoItinerario);      
      $itinerarioKm->excluir();

      $msg = "O meio de transporte e as dificuldades de acesso";
      $msg .= " do itinerário foram excluídos com sucesso!";
      $oRetorno->sMensagem = urlencode($msg);
      break;      
  }
