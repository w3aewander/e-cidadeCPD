      ...
      switch ($oArquivo->codigo_registro) {
        ...
        case '3' :
          ...
          if ($oArquivo->codigo_segmento == "A") {
            ...
            $oRegistro->linhaarquivo     = $oArquivo->getLinha();
          ...
          /**
           * Segmento K: Informações sobre o pagamento de registro de boletos e convênios com código de barras.
           */
          if ($oArquivo->codigo_segmento == "K") {

            $sValorEfetivado  = substr($oArquivo->oValorEfetivacao,  0, 13).".";
            $sValorEfetivado .= substr($oArquivo->oValorEfetivacao,  13,2);
            $sDataEfetivacao  = substr($oArquivo->oDtEfetivacao,     4, 4);
            $sDataEfetivacao .= "-".substr($oArquivo->oDtEfetivacao, 2, 2);
            $sDataEfetivacao .= "-".substr($oArquivo->oDtEfetivacao, 0, 2);
            if ($sDataEfetivacao == '0000-00-00' ||$sDataEfetivacao == '00000000') {
              $sDataEfetivacao  = date("Y-m-d", db_getsession("DB_datausu"));
            }

            $oRegistro = new stdClass();
            $oRegistro->codigo_movimento = trim($oArquivo->oDocumentoEmpresa);        // e76_codmov
            $oRegistro->numero_lote      = trim($oArquivo->oLoteServico);             // e76_lote
            $oRegistro->mov_lote         = "0";                                       // e76_movlote
            $oRegistro->numero_banco     = trim($oArquivo->oNumeroDocumentoBanco);    // e76_numbanco
            $oRegistro->valor_efetivado  = (float)$sValorEfetivado;                   // e76_valorefet
            $oRegistro->data_efetivacao  = $sDataEfetivacao;                          // e76_dataefet
             ...
             * Para saber se o registro pode ser baixado no sistema.
             */
            $oArquivo->ocorrencias       = trim($oArquivo->oOcorrencia);
            $oRegistro->retorno_banco    = $oArquivo->oOcorrencia;
            $oRegistro->codigo_retorno   = $this->getCodigoErro($oRegistro->retorno_banco); // e76_errobanco
            $oDadosRetorno->registros[]  = $oRegistro;
          }
          break;
