...
class cl_iptuconstr_extends extends cl_iptuconstr
{
  ...
  function incluir ($j39_matric,$j39_idcons,$valida_data = true)
  {
    ...
    if ($this->validar_j39_ano and (empty($this->j39_ano) || strlen($this->j39_ano) != 4)) {
      $this->erro_sql    = "Campo Ano da Construção não Informado.";
      $this->erro_campo  = "j39_ano";
      $this->erro_banco  = "";
   ...
   function alterar ($j39_matric=null,$j39_idcons=null,$valida_data = true) {
     ...
     if (trim($this->j39_ano) != "" || isset($GLOBALS["HTTP_POST_VARS"]["j39_ano"])){
      if (empty($this->j39_ano)) {
        $this->j39_ano = $GLOBALS["HTTP_POST_VARS"]["j39_ano"];
      }
    ...
    function alterar_idprinc($j39_matric = null, $j39_idcons = null, $j39_idprinc)
    {
    $this->atualizacampos();
    $sql = " update iptuconstr set j39_idprinc = $j39_idprinc";

    $sql .= " where ";
    if ($j39_matric != null) {
      $sql .= " j39_matric = $j39_matric";
    }
    if ($j39_idcons != null) {
      $sql .= " and  j39_idcons = $j39_idcons";
    }
    ...
    if ($this->numrows > 0) {
      for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
        ...
        $acount = pg_result($resac, 0, 0);
        $resac = db_query("insert into db_acountacesso values($acount," . db_getsession("DB_acessado") . ")");
        $resac = db_query("insert into db_acountkey values($acount,150,'$j39_matric','A')");
        if ($j39_idcons != null)
          $resac = db_query("insert into db_acountkey values($acount,151,'$j39_idcons','A')");

        if ($j39_idprinc != "")
          $resac = db_query("insert into db_acount values($acount,30,2532,'" . AddSlashes(pg_result($resaco, $conresaco, 'j39_idprinc')) . "','$j39_idprinc'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
      }
    }
    $result = db_query($sql);
    if ($result == false) {
      $this->erro_banco = str_replace("\n", "", @pg_last_error());
      $this->erro_sql   = " Não Alterado. Alteracao Abortada.\\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = " Não foi Alterado. Alteracao Executada.\\n";
        $this->erro_sql .= "Valores : " . $this->j39_matric . "-" . $this->j39_idcons;
        $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
