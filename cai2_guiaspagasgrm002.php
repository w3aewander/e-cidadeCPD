...
foreach ($aDados as $oTipoRecolhimento) {
  ...
  foreach ($oTipoRecolhimento->guias as $oGuia) {
    ...
    if ($lAdicionaPagina || $pdf->GetY() + (20 + $iTotalAlturaCamposDinamicos) > $pdf->h) {
      ...
      $pdf->Cell(35, $iAlturalinha, 'Num. Ref', '1', 0, 'C');
      $pdf->Cell(18, $iAlturalinha, 'Competência', '1', 0, 'C');
      $pdf->Cell(15, $iAlturalinha, 'Valor', '1', 0, 'C');
      $pdf->Cell(15, $iAlturalinha, 'Desconto', '1', 0, 'C');
      $pdf->Cell(15, $iAlturalinha, 'Multa', '1', 0, 'C');
      $pdf->Cell(15, $iAlturalinha, 'Juros', '1', 0, 'C');
      $pdf->Cell(18, $iAlturalinha, 'Outros Acres.', '1', 0, 'C');
    ...
    $pdf->Cell(15, $iAlturalinha, db_formatar($oGuia->valor, 'f'), 1, 0, 'R');
    $pdf->Cell(15, $iAlturalinha, db_formatar($oGuia->desconto,'f'), 1, 0, 'R');
    $pdf->Cell(15, $iAlturalinha, db_formatar($oGuia->multa,'f'), 1, 0, 'R');
    $pdf->Cell(15, $iAlturalinha, db_formatar($oGuia->juros,'f'), 1, 0, 'R');
    $pdf->Cell(18, $iAlturalinha, db_formatar($oGuia->outros_acrescimos,'f'), 1, 0, 'R');
