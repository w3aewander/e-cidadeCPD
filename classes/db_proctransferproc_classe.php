...
class cl_proctransferproc {
   ...
   function sql_query_transferencia ( $p63_codtran=null,$p63_codproc=null,$campos="*",$ordem=null,$dbwhere=""){
     ...
     $sql .= "
      LEFT JOIN db_depart
        ON coddepto = p62_coddepto
      LEFT JOIN db_departorg
        ON db01_coddepto = coddepto AND db01_anousu = ". db_getsession('DB_anousu') ."
      LEFT JOIN orcorgao
  ...
  function sql_query_andamento_processo($p63_codtran=null,$p63_codproc=null,$campos="*",$ordem=null,$dbwhere="") {
    ...
    public function sql_query_processos_por_transferencia($codigoTransferencia, $campos = '*')
    {
        $sql = "select {$campos}
        ...
        inner join proctransfer on p63_codtran = p62_codtran
         ...
         left join arqproc      on p68_codproc = p63_codproc
                 where p63_codtran = {$codigoTransferencia}
