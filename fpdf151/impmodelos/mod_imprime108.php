<?php
    ...
    $this->cepmunic = $oDadosInstit->cep;
    ...
    for ($iInd = 0; $iInd < $iNroRec; $iInd++) {
        ...
        if ($this->arraycodhist[$iInd] == 918) {
            $this->totaldesc += $this->arrayvalreceitas[$iInd];
        } else {
            if ($this->arraycodtipo[$iInd] == 't' and $this->arrayvalreceitas[$iInd] > 0) {
    ...
    $this->valtotal = !empty($this->valtotal) ? $this->valtotal : db_formatar(($this->totalrec), 'f');
    ...
    $this->objpdf->cell(30, $iAlt + 1, 'RECIBO DO PAGADOR', 'LRB', 1, 'C', 0);
    ...
    $this->objpdf->SetFont('Arial', 'b', $nFonteLabel);
    $this->objpdf->cell(18, $iAltLabel, $sReciboSacadoContribuinteTitulo, 'TLR', 0, "L");
    $this->objpdf->cell(12, $iAltLabel, 'Exercicio', 'TLR', 1, "L");

    $this->objpdf->SetFont('Arial', '', $nFonte);
    $this->objpdf->cell(18, $iAlt, $iReciboSacadoContribuinteCodigo, 'BLR', 0, "L");
    $this->objpdf->cell(12, $iAlt, $this->exerc_debito, 'BLR', 1, "L");
    ...
    if (!empty($this->cepmunic)) {
        $sMunicPref .= " - CEP: " . $this->cepmunic;
    }
    ...
    //valor venal iptu
    $nFonteLabelInfoImovel = 6;
    $nFonteInfoImovel = 5;
    $iBordaInfoImovel = 0;

    $this->objpdf->setX($iXPosPagina);
    $this->objpdf->SetFont('Arial', 'B', $nFonteLabelInfoImovel);
    $this->objpdf->cell(32, $iAltLabel, 'VALOR VENAL DO TERRENO', $iBordaInfoImovel, 0, 'L', 0);
    
    $this->objpdf->setX($iXPosPagina + 32);
    $this->objpdf->SetFont('Arial', 'B', $nFonteLabelInfoImovel);
    $this->objpdf->cell(35, $iAltLabel, 'VALOR VENAL DA EDIFICAÇÃO', $iBordaInfoImovel, 0, 'L', 0);
    
    $this->objpdf->setX($iXPosPagina + 67);
    $this->objpdf->SetFont('Arial', 'B', $nFonteLabelInfoImovel);
    $this->objpdf->cell(30, $iAltLabel, 'VALOR VENAL DO IMÓVEL', $iBordaInfoImovel, 0, 'L', 0);
    
    $this->objpdf->setX($iXPosPagina + 97);
    $this->objpdf->SetFont('Arial', 'B', $nFonteLabelInfoImovel);
    $this->objpdf->cell(25, $iAltLabel, 'ALIQUOTA', $iBordaInfoImovel, 2, 'L', 0);
    
    //VALOR VENAL DO TERRENO
    $this->objpdf->setX($iXPosPagina);
    $this->objpdf->SetFont('Arial', '', $nFonteInfoImovel);
    $this->objpdf->cell(32, $iAltLabel, trim($this->iptAreaTer)."m² = ".trim($this->iptVlrTer), $iBordaInfoImovel, 0, 'L', 0);

    //VALOR VENAL DA EDIFICAÇÃO
    $this->objpdf->setX($iXPosPagina + 32);
    $this->objpdf->SetFont('Arial', '', $nFonteInfoImovel);
    $this->objpdf->cell(35, $iAltLabel, trim($this->iptAreaEdi)."m² = ".trim($this->iptVlrEdi), $iBordaInfoImovel, 0, 'L', 0);
    
    //VALOR VENAL DO IMÓVEL
    $this->objpdf->setX($iXPosPagina + 67);
    $this->objpdf->SetFont('Arial', '', $nFonteInfoImovel);
    $this->objpdf->cell(30, $iAltLabel, trim($this->iptVlrImovel), $iBordaInfoImovel, 0, 'L', 0);

    //ALIQUOTA
    $this->objpdf->setX($iXPosPagina + 97);
    $this->objpdf->SetFont('Arial', '', $nFonteInfoImovel);
    $this->objpdf->cell(25, $iAltLabel, "{$this->iptj23_aliq}%", $iBordaInfoImovel, 0, 'L', 0);

    $this->objpdf->setY($this->objpdf->getY() - 3);
    ...
    if (isset($this->descr12_1)) {
        $aDescr12_1 = explode("\n", $this->descr12_1, 3);
        $aDescr12_1[1] = str_replace($this->descr17, "  Cadastro imobiliário: ".$this->refant, $aDescr12_1[1]);
    ...
    if (!empty($this->taxaExpediente)) {
        $sInstrucao .= " {$this->taxaExpediente}";
    }

    $this->predescr12_1 = "";
    $iYPos = $this->objpdf->getY();
    $this->objpdf->setY($iYPos + 6);
    $this->objpdf->setX($iXPosPagina);
    $this->objpdf->SetFont('Arial', 'b', $nFonteLabel);
    $this->objpdf->cell(122, $iAltLabel, 'Instruções', 'TLR', 1, 'L', 0);
    $this->objpdf->SetFont('Arial', '', $nFonte);
    $this->objpdf->setX($iXPosPagina);
    $this->objpdf->multicell(122, 2.2, $sInstrucao);
    ...
    if ($this->banco == "CAIXA" || $this->banco == "CEF") {
        ...
        $this->objpdf->setX($iXPosPagina);
        $this->objpdf->cell(122, $iAltLabel, "SAC CAIXA: 0800 726 0101 (informações, reclamações, sugestões e elogios)",
            'LR', 1, "L");
        $this->objpdf->setX($iXPosPagina);
        $this->objpdf->cell(122, $iAltLabel - 1,
            "Para pessoas com deficiência auditiva ou de fala: 0800 726 2492 OUVIDORIA: 0800 725 7474", 'LR', 1, "L");
        $this->objpdf->setX($iXPosPagina);
        $this->objpdf->cell(122, $iAltLabel - 1, "caixa.gov.br", 'LR', 0, "L");

    } else {
        if ($this->banco == "BANRISUL") {

            $this->objpdf->setY($iYPos + 22);
            $this->objpdf->setX($iXPosPagina);
            $this->objpdf->cell(122, $iAltLabel + 2, "", 'LR', 1, "L");
            $this->objpdf->setX($iXPosPagina);
            $this->objpdf->cell(122, $iAltLabel - 1, "SAC BANRISUL: 0800 646 1515  -  OUVIDORIA BANRISUL: 0800 644 2200",
                'LR', 0, "L");
        }
    }
    /* FIM SAC BANCO */
    ...
    $this->objpdf->setX($iXPos);
    $this->objpdf->SetFont('Arial', '', $nFonte);
    $this->objpdf->cell(50, $iAlt + 2.7, '', 1, 1, "L");
