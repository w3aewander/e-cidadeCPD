...
final class PadArquivoSigapBalanceteReceita extends PadArquivoSigap
{
    ...
    public function gerarDados()
    {
        ...
        if (EMENTARIO_RECEITA) {
            $sSqlDadosBalancete = ReceitaSaldo(11, 1, 3, false, $sWhere, $iAno, $this->sDataInicial, $this->sDataFinal, true);
        } else {
            $sSqlDadosBalancete = db_receitasaldo(11, 1, 3, false, $sWhere, $iAno, $this->sDataInicial, $this->sDataFinal, true);
        }
        $sSqlBalancete = "select case  when {$iAno} <= 2007 then ";
        $sSqlBalancete .= "         substr(o57_fonte,2,14) ";
        $sSqlBalancete .= "       else  ";
        $sSqlBalancete .= "         case when substr(o57_fonte,1,1) <> '9' then ";
        $sSqlBalancete .= "           substr(o57_fonte, 2, 15) ";
        $sSqlBalancete .= "         else  ";
        $sSqlBalancete .= "          substr(o57_fonte,1,15) ";
        $sSqlBalancete .= "         end end as o57_fonte, ";
        $sSqlBalancete .= "       ";
        ...
        $sSqlBalancete .= "       substr(x.o57_fonte, 1, 1) = '9' as deducao, ";
        $sSqlBalancete .= "        nivel, ";
        ...
        for ($i = 1; $i < $iTotalLinhas; $i++) {
            ...
            $rsCaracteristica = @db_query($sSqlCaracteristica);
            $iCaracteristicaPeculiar = db_utils::fieldsMemory($rsCaracteristica, 0)->o70_concarpeculiar;

            if ($oReceita->o57_fonte == "00000000000000") {
                continue;
            }
            /**
             * a deducao esta com os niveis complettos. na receita normal, tiramos o primeiro digito,
             * por isso é deduzido um nivel.
             */
            if ($oReceita->deducao == 'f') {
             ...
             */
            if ($nSaldoInicial < 0) {
                $nSaldoInicial = number_format(($nSaldoInicial), 2, ".", "");
            } else {
                $nSaldoInicial = number_format(abs($nSaldoInicial), 2, ".", "");
            }
            ...
            if ($oReceita->saldo_arrecadado_acumulado < 0) {
                $nArrecadadoAcumulado = number_format(($oReceita->saldo_arrecadado_acumulado), 2, ".", "");
            } else {
