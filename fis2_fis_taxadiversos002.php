...
$aGrupos = $oGet->grupo == 'T' ? buscaTodosGrupos() : array(FisGrupoTaxaDiversosRepository::getInstanciaPorCodigo($oGet->grupo));
 ...
 * @param FisGrupoTaxaDiversos $oGrupo
 */
function imprimeConteudo(PDF $oPdf, $oConfig, FisGrupoTaxaDiversos $oGrupo) {
 ...
 *
 * @return FisGrupoTaxaDiversos[]
 * @throws DBException
 */
function buscaTodosGrupos() {

  $oDaoGrupos = new cl_fis_grupotaxadiversos();
  ...
  return \db_utils::makeCollectionFromRecord($rsGrupos, function($oRetorno) {
    return FisGrupoTaxaDiversosRepository::getInstanciaPorCodigo($oRetorno->y118_sequencial);
  });
}
 ...
 *
 * @param FisLancamentoTaxaDiversos[] $aLancamentos
 * @param stdClass $oConfig
 * @return array
 */
function organizaLancamentos($aLancamentos, $oConfig) {
  ...
  foreach($aLancamentos as $oLancamento) {
    ...
    $sUnidade = FisLancamentoTaxaDiversos::getDescricaoUnidade($oLancamento->getNaturezaTaxa()->getUnidade());
 ...
 * @return bool
 */
function validaImpressaoGrupo($oConfig, FisGrupoTaxaDiversos $oGrupo) {
