...
if ($oParam->exec == 'duplicarControleFisFin') {
    ...
    $permiteDuplicar = true;
    $tipoControle = ControleFisicoFinanceiroHelper::getTipoControleAtual();
    switch ($tipoControle) {
        case ControleFisicoFinanceiroHelper::CONTROLE_DEPARTAMENTO:
            $sWhere .= ' and (la56_i_tipocontrole BETWEEN 1 AND 3 or la56_i_tipocontrole = 9) ';
            break;
        case ControleFisicoFinanceiroHelper::CONTROLE_LABORATORIO:
            $sWhere .= ' and (la56_i_tipocontrole BETWEEN 4 AND 6) ';
            break;
        case ControleFisicoFinanceiroHelper::CONTROLE_GRUPO_EXAME:
            $sWhere .= ' and la56_i_tipocontrole = 7 ';
            break;
        case ControleFisicoFinanceiroHelper::CONTROLE_EXAME:
            $sWhere .= ' and la56_i_tipocontrole = 8 ';
            break;
        default:
            $permiteDuplicar = false;
    }

    $sSql = $oDaolab_ctrlfisfin->sql_query_file("", "lab_controlefisicofinanceiro.*", "", $sWhere);
    $rsResult = $oDaolab_ctrlfisfin->sql_record($sSql);
    if ($oDaolab_ctrlfisfin->numrows > 0 && $permiteDuplicar) {
    ...
    if ($oDaoLabControleFisicoFinanceiro->numrows > 0) {
        $oRetorno->aControles = db_utils::getCollectionByRecord($rs, false, false, true);
    } else {
        $oRetorno->iStatus = 0;
    ...
    for ($i = 0; $i < count($oParam->iSetorExame); $i++) {
        ...
        if ($oControleExames->getTipoControle() != 0) { // Possui um tipo de controle
            ...
            if ($oInfoControle != null) { // Possui informação de controle
                ...
                $lNovo = true;
                // Verifica se já existe algum exame que se enquadrou no mesmo controle
                foreach ($aInfoReq as $oInfoReq) {
                    // Mesmas info de controle
                    if ($oInfoReq->oInfoControle->la56_i_codigo == $oInfoControle->la56_i_codigo) {
                        /**
                         * Agora tenho que verificar se o saldo vai ser descontado do mesmo saldo do que já existe
                         */
                        if ($oInfoControle->la56_i_periodo == 2) { // Mensal
                            /**
                             * Em controle mensal, a data do exame no objeto oInfoReq será somente uma também,
                             * pois para agrupar,
                             *  basta que a data da nova requisição sendo avaliada esteja no mesmo mês de requisições
                             * (a contar da data de início do controle)
                             */
                            ...
                            // Se for diário, só desconta do mesmo saldo se for no mesmo dia
                            if ($dData == $oInfoReq->dDataExame) {
                                $oInfoReq->nQtdeReq += $nQtdeReq;
                                $lNovo = false;
                                break;
                            }
                ...
                // Não foi agrupado, ou seja a quantidade da requisicao não vai descontar do mesmo saldo de outra
                if ($lNovo) {
                    $iInd = count($aInfoReq);
                ...
                // Existe controle físico financeiro, mas nenhum se encaixa para o exame em questão. Bloqueia a req.
    ...
    /**
     * Verifico se vai ter saldo suficiente para todas as requisições.
     * Se o saldo for insuficiente para alguma requisição
