...
class licitacao
{
    ...
    public function getProcessoProtocolo()
    {
        ...
        $sSqlProcesso = $oDaoLicLicitaProc->sql_query(
            null,
            "l34_protprocesso",
            null,
        ...
        if ($oDaoLicLicitaProc->numrows > 0) {
            ...
            $this->oProcessoProtocolo = new processoProtocolo($iProcessoProtocolo);
    ...
    public function alterarSituacao($iCodigoSituacao, $sObservacao = "", DBDate $oData = null)
    {
        ...
        if (!in_array(
            ...
            $sSqlReserva = $oDaoLiclicitem->sql_query_dotacao_reserva(
                null,
                "orcreservasol.*",
                null,
                "l21_codliclicita = {$this->iCodLicitacao}"
    ...
    public function getCompilacaoRegistroPreco()
    {
        if ($this->oRegistroPreco == null) {
            ...
            if (pg_num_rows($rsRegistroPreco) == 1) {
                $this->oRegistroPreco = new compilacaoRegistroPreco(db_utils::fieldsMemory(
                    $rsRegistroPreco,
                    0
                )->pc10_numero);
            } else {
    ...
    static function getItensPorFornecedor(
        ...
        $sCampos = "distinct l21_codigo as codigo, pc01_codmater as codigomaterial,";
        ...
        $sCampos .= "m61_descr AS unidade, pc01_complmater AS descr_compl,l21_ordem AS ordem,";
        $sCampos .= "pc23_obs AS observacao";
        ...
        $aItens = db_utils::getCollectionByRecord($rsLicitacoes, false, false, $lEncode);

        if ($flagCredenciamento) {
            $acordo = new Acordo($acordo);
            $aAcordoItemQuantidade = [];
            if (!empty($acordo->getItens())) {
                foreach ($acordo->getItens() as $item) {
                    $aAcordoItemQuantidade[$item->getCodigoItemLicitacao()] = $item->getQuantidade();
                }

                foreach ($aItens as $k => $item) {
                    if (array_key_exists($item->codigo, $aAcordoItemQuantidade)) {
                        unset($aItens[$k]);
                    }
                }
            }
    ...
    static function getLicitacoesByFornecedor($iFornecedor, $lValidaAutorizadas = false, $iInstituicao = null)
    {
        ...
        $sValidaTipoLicitacao .= "   when l03_pctipocompratribunal not in (54 , 61, 80, 96) and not exists (select 1 from acordoliclicitem where ac24_liclicitem = l21_codigo ) ";
        ...
        $sValidaTipoLicitacao .= "   when l03_pctipocompratribunal in (54 , 61, 80, 96) and not exists( ";
        ...
        $sWhere = implode(" and ", $aWhere);
        $sCampos = "distinct l20_codigo as licitacao, l20_objeto as objeto, l20_numero as numero, l03_descr as modalidade, l44_descricao as tipo_compra";
        $sCampos .= ", pc21_numcgm as cgm, pc11_numero as numero_exercicio, l20_datacria as data, l20_usaregistropreco";
    ...
    public function getValoresParciais($iCodigoItemProcesso, $iCodigoDotacao, $iOrcTipoRec = null, $cgmFornecedor = null)
    {
        ...
        $sWhereFornecedor = '';
        if (!empty($cgmFornecedor)) {
            $sWhereFornecedor = "AND e54_numcgm = '${cgmFornecedor}'";
        }
    ...
    public function getItensParaAutorizacao()
    {
        ...
        $sCampos .= "pc10_solicitacaotipo as tiposolicitacao,";
        ...
        $sWhere .= "
            and l21_codigo not in (
                select acordoliclicitem.ac24_liclicitem from acordoliclicitem
                join acordoitem ON acordoitem.ac20_sequencial = acordoliclicitem.ac24_acordoitem
                join acordoposicao ON acordoposicao.ac26_sequencial = acordoitem.ac20_acordoposicao
                join acordo ON acordo.ac16_sequencial = acordoposicao.ac26_acordo
                where acordo.ac16_origem != 3
            )";
        ...
        $sSqlPeriodoParametro = $this->oDaoParametros->sql_query_file(
            null,
            "e30_numdec",
            null,
        ...
        if ($iRowPcOrcamJulg > 0) {
            for ($i = 0; $i < $iRowPcOrcamJulg; $i++) {
                ...
                // Quando a licitação é PNAE, precisamos filtrar
                // pelo CGM do fornecedor também
                // Para que possamos filtrar pelo CGM em todos os casos,
                // o filtro de CGM dentro do getValoresParciais tem que
                // estar ciente das trocas de fornecedor, registradas em
                // cl_pcorcamtroca
                if ($this->isPNAE()) {
                 ...
                 */
                $oDados->saldoquantidade = $oDados->quantfornecedor - $oDados->quantidadeautorizada;
                ...
                // PLUGIN LICITACAOCOMPARTILHADA - Disabilita linha instituicao diferente

                $oDados->autorizacaogeradas = $this->getAutorizacoes(
                    $oDados->codigoitemprocesso,
                    $oDados->codigodotacao,
                    null,
                    $oDados->tiposolicitacao
                );
     ...
     */
    public function getAutorizacoes($iCodigoItemProcesso, $iCodigoDotacao, $iOrcTipoRec = null, $tipoSolicitacao = null)
    {
        if (empty($iCodigoItemProcesso)) {
            throw new Exception("Código do item do processo não informado!");
        }
    ...
    {
        $comissao = new cl_liccomissao();
        $sqlComissao = $comissao->sql_query($this->iComissao);
        $resultadoComissao = $comissao->sql_record($sqlComissao);
        $dadosComissao = db_utils::fieldsMemory($resultadoComissao, 0);

        if (empty($dadosComissao)) {
            return false;
        }

        $comissaoCgm = new cl_liccomissaocgm();
        $sqlComissaoCgm = $comissaoCgm->sql_query(
            null,
            '*',
            null,
            "l31_liccomissao = {$dadosComissao->l30_codigo}"
        );
        $resultadoComissaoCgm = $comissaoCgm->sql_record($sqlComissaoCgm);
        $dadosComissaoCgm = db_utils::getCollectionByRecord($resultadoComissaoCgm);

        $dados = $dadosComissao;
        $dados->participantes = $dadosComissaoCgm;
        return $dados;
    }
    ...
    public function gerarAutorizacoes($aDadosAutorizacao)
    {
        ...
        foreach ($aDadosAutorizacao as $oDados) {
            ...
            foreach ($oDados->itens as $oItem) {
                 ...
                 * caso exista, devemos calcular a diferença entre o que deve ser gerado para a autorizacao e solictacao
                 */
                $aReservas = itemSolicitacao::getReservasSaldoDotacao($oItem->pcdotac, $oItem->solicitem);
                ...
                $numrows = pg_num_rows(
                    db_query(
                        "
                            select coddepto from db_depart
                            where
                                descrdepto like '%SEDUC%'
                                and coddepto = {$iUnidade};
                        "
                    )
                );
                ...
                if ($nNovoValorReserva > 0 && $oSaldo->nValorAutorizacao + $oItem->valortotal < $oSaldo->nValorItemJulgado) {
                    ...
                    $oDaoOrcReservaSol->o82_pcdotac = $oItem->pcdotac;
            ...
            $oAutorizacao->setInstituicaoLicitacao($oDados->institLic);
            ...
            $oAutorizacao->setComplemento($oDados->complemento);

            if (isset($oDados->iMetaHistoricos)) {
                $oAutorizacao->setMetaHistorico($oDados->iMetaHistoricos);
            }
            $oAutorizacao->salvar();
            ...
            /**
             * Buscar o código do processo da tabela solicitaprotprocesso e
             * incluir na empautorizaprotprocesso caso tenha
             */
            ...
            if (!empty($oDadoSolicitaProtProcesso->pc90_numeroprocesso)) {
                $oDaoEmpAutorizaProcesso = new cl_empautorizaprocesso;
                ...
                if ($oDaoEmpAutorizaProcesso->erro_status == 0) {
                    $sMensagemProcessoAdministrativo = "
                        Ocorreu um erro para incluir o número do processo administrativo
                    ";
                    $sMensagemProcessoAdministrativo .= "
                        na autorização de empenho.\n\n{$oDaoEmpAutorizaProcesso->erro_msg}
                    ";
    ...
    public function getFornecedor()
    {
        if (count($this->aFornecedores) == 0) {
            ...
            $sSqlBuscaFornecedor = $oDaoOrcamentoItemLicitacao->sql_query_fornecedores(
                null,
                $sCampos,
                null,
                $sWhereFornecedor
    ...
    public static function verificaSaldoModalidade($iModalidade, $iItem, $dtJulgamento)
    {
        ...
        $sSqlMateriaisComprados = $oDaoPcOrcamItem->sql_query_saldoModalidade(
            null,
            $sCamposMateriaisComprados,
            null,
    ...
    public function getFase()
    {
        if (empty($this->iFaseLicitacon)) {
            ...
            if (
                $this->hasFornecedor()
                || EventoLicitacao::possuiFaseEvento(
                    $this,
    ...
    /**
     * Retorna Dados do registro de preço
     * @return object
     * @throws BusinessException
     */
    public function getRegistroDePreco()
    {
        if (empty($this->iCodLicitacao)) {
            throw new BusinessException("Licitação não carregada.");
        }

        $sSqlRegistroPreco = "select distinct pc10_numero, pc54_datainicio, pc54_datatermino                                                    ";
        $sSqlRegistroPreco .= "  from liclicitem                                                                                                ";
        $sSqlRegistroPreco .= "       inner join pcprocitem            on liclicitem.l21_codpcprocitem = pcprocitem.pc81_codprocitem            ";
        $sSqlRegistroPreco .= "       inner join solicitem             on pcprocitem.pc81_solicitem    = solicitem.pc11_codigo                  ";
        $sSqlRegistroPreco .= "       inner join solicita              on solicitem.pc11_numero        = solicita.pc10_numero                   ";
        $sSqlRegistroPreco .= "       inner join solicitaregistropreco on solicita.pc10_numero         = solicitaregistropreco.pc54_solicita    ";
        $sSqlRegistroPreco .= " where l21_codliclicita = {$this->iCodLicitacao}                                                                 ";
        $sSqlRegistroPreco .= "   and pc10_solicitacaotipo = 6                                                                                  ";
        ...
        if (pg_num_rows($rsRegistroPreco) == 1) {
            $result = db_utils::fieldsMemory($rsRegistroPreco, 0);
        }
    ...
    {
        return $this->obterAtributoDinamico('permitesubcontratacao');
    }
    ...
    /**
     * Diz se a modalidade dessa licitação é do PNAE ou não
     *
     * @return bool
     */
    public function isPNAE()
    {
        return $this->getModalidade()->isPNAE();
    }

    /**
     * Diz se uma modalidade de licitação é Chamamento Público ou não
     *
     * @return bool
     */
    public function isCHP()
    {
        return $this->getModalidade()->isCHP();
    }
}
