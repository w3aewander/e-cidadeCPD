...
if(isset($opcao) && $opcao == "alterar"){
  echo "<script>parent.iframe_fiscais.location.href='fis1_fis_fiscalusuario002.php?chavepesquisa=$y38_codnoti&chavepesquisa1=$y38_id_usuario&y39_codandam=$y39_codandam".$getIntimacao."'</script>";}
if(isset($opcao) && $opcao == "excluir"){
  echo "<script>parent.iframe_fiscais.location.href='fis1_fis_fiscalusuario003.php?chavepesquisa=$y38_codnoti&chavepesquisa1=$y38_id_usuario&y39_codandam=$y39_codandam".$getIntimacao."'</script>";
}

// Ticket 108335
if(isset($intimacao) && $intimacao == 1){
  $Ty38_codnoti = 'Código da Intimação
...
Campo:y30_data                             ';
  ...
  $Ly38_codnoti = '<strong>Código da Intimação:</strong>';
  $Ly30_data    = '<strong>Data da Intimação:</strong>';

}
// -------------
$rsConsultaProc = db_query(" select y110_procfiscal as procfiscal from fiscalizacao.fis_procfiscalnotificacao where y110_notificacaofiscal = $y38_codnoti");
...
?>
  ...
  <tr>
    <td align="center" colspan="2">
      ...
      ?>
        <input name="novo" type="button" id="novo" value="Novo" onclick="location.href='fis1_fis_fiscalusuario001.php?y38_codnoti=<?=$y38_codnoti?><?=$getIntimacao?>'">
      <?php 
      }
      ?>
    </td>
  ...
  <tr>
   ...
   <?php 
    ...
    $cliframe_alterar_excluir->campos="y38_codnoti,y38_id_usuario,nome,y38_obs";
   ...
   ?>
   <p align="center"><small>*A <?=$getLabel?> deve ter no mínimo um fiscal cadastrado!</small></p>
   <?php 
   }
   ?>
 </tr>  
...
<?php }?> 
  if(mostra==true){
    js_OpenJanelaIframe('','db_iframe_db_usuarios','func_fis_cadfiscaisdepto.php?fiscal=1&funcao_js=parent.js_mostradb_usuarios1|id_usuario|nome<?=$getIntimacao?>&y38_codnoti=<?=$y38_codnoti?>'+sUrl,'Pesquisa',true);
  }else{
    js_OpenJanelaIframe('','db_iframe_db_usuarios','func_fis_cadfiscaisdepto.php?fiscal=1&pesquisa_chave='+document.form1.y38_id_usuario.value+'&funcao_js=parent.js_mostradb_usuarios<?=$getIntimacao?>&y38_codnoti=<?=$y38_codnoti?>'+sUrl,'Pesquisa',false);
  }
}
function js_mostradb_usuarios(chave,chave2,erro){
...
function js_pesquisa(){
  js_OpenJanelaIframe('','db_iframe_fiscalusuario','func_fis_fiscalusuario.php?funcao_js=parent.js_preenchepesquisa|y38_codnoti|1<?=$getIntimacao?>','Pesquisa',true);
}
