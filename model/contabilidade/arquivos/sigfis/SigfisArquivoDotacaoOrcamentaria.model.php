    ...
    public function gerarDados()
    {
        ...
        if ($clOrcDotacao->numrows > 0) {
            ...
            for ($i = 0; $i < $clOrcDotacao->numrows; $i++) {
                ...
                $repositorySiconfi->scopeCodigoRecurso($oDadosQuery->o58_codigo);
                $repositorySiconfi->scopeExercicio($this->iAnoUso);

                $novoRecurso = $repositorySiconfi->first();

                if (empty($novoRecurso)) {
                    $sErroLog = "Recurso {$oDadosQuery->o58_codigo} - {$oDadosQuery->o15_descr} ";
                    $sErroLog .= "Sem Vinculo com Dotao {$oDadosQuery->o58_coddot} do SIGFIS.\n";
                ...
                if ($oDados->Dt_Ano > 2018) {
                    $oDados->Nm_FonteRecurso = str_pad($novoRecurso->getCodigoSiconfi(), 8, ' ', STR_PAD_LEFT);
                } else {
