...
function DBGrid(sName)
{
    ...
    /**
     * define se a grid tera uma celula com checkbox title
     * @type bool
     * @private
     */
    this.titleCheckbox = '';
    ...
    /**
     * define se a grid tera uma celula com checkbox width
     * @type bool
     * @private
     */
    this.widthCheckbox = '21px';
     ...
     * @param {String} classe para adicionar ao table row
     * @return void
     * @see tableRow
     */
    this.addRow = function (aRow, lRender, lDisabled, lChecked, classe) {
        ...
        if ((aRow instanceof Array)) {
            ...
            oRow.lDisabled = lDisabled;

            if (classe != null) {
                oRow.addClassName(classe);
            }
            ...
            if (this.hasCheckbox) {
                ...
                oRow.addCell(new tableCell(sCheckbox, this.sName + 'row' + iRowCount + 'checkbox', 21, 'checkbox text-center'));
                iNumCol++;
            }
            ...
            for (var iLength = 0; iLength < aRow.length; iLength++) {
                ...
                if (!this.aHeaders[iNumCol] || !this.aHeaders[iNumCol].lDisplayed) {
    ...
    this.setHeader = function (aHeader) {
        ...
        if (this.hasCheckbox) {
            if (this.hasSelectAll) {
                ...
                sHeader += `style="cursor:pointer;">${this.titleCheckbox !== '' ? this.titleCheckbox : 'M'}</a></b>`;
            } else {
                sHeader += "&nbsp;";
            }
            iNumCells++;
    ...
    /**
     * Reseta o array dos headers
     * Deve ser usada quando se tem a necessidade de chamar mais de uma vez o metodo show.
     * @example
     *   grid.resetHeader()
     *   grid.show(element)
     */
    ...
    this.show = function (oNode) {
        ...
        sGrid += "<div class='grid-container'>";
        ...
        sGrid += "<span id='spanPersonalizado_" + this.sName + "'>&nbsp;</span>&nbsp;";
        sGrid += "</div></td></tr>";
     ...
     * @param widthCheckbox
     */
    this.setCheckbox = function (iCell, titleCheckbox = '',widthCheckbox = '21px') {
    ...
    /**
     * retorna Todas as linhas da grid
     * @param {string} sTipoRetorno define como sera o retorno do metodo. caso object retorna os objetos tableRow selecionados
     * caso array, retornara um array com os valores das linhas selecionadas.
     *
     * @return array
     */
    this.getAll = function (sTipoRetorno) {
    ...
    this.checkAllRows = () => {
        const checkboxPrincipal = document.querySelector(`input[id^="mtodositens${this.sName}"]`);
        checkboxPrincipal.checked = true;
        this.aRows.map(row => row.select(true));
    };
                ...
                if (lInSelection) {
                    if (aSelection[i][iCol]) {
                        if (aSelection[i][iCol].indexOf(",") > 0) {
                            ...
                            nSomaTotal += Number(aSelection[i][iCol]);
                        }
                    }
                } else {
                    if (aSelection[i].aCells[iCol].getValue().indexOf(",") > 0) {
                        ...
                        nSomaTotal += Number(aSelection[i].aCells[iCol].getValue());
                    }
                }
            }
        }
    ...
    this.renderHeader = function () {
        ...
        for (var i = 0; i < this.aHeaders.length; i++) {
            ...
            if (this.hasCheckbox && i == 0) {
                this.aHeaders[i].setWidth(this.widthCheckbox);
    ...
    this.setNumRows = function (iNumRows) {
        $(this.sName + "numrows").innerHTML = Number(iNumRows).valueOf();
    };
    ...
    this.getNumRows = function () {
        return Number($(this.sName + "numrows").innerHTML).valueOf();
    };
