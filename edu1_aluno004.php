...
if (isset($alunouf)) {
    ...
    $sSqlMunic = $clcensomunic->sql_query_file("","ed261_i_codigo,ed261_c_nome", "ed261_c_nome","ed261_i_censouf = $alunouf");
    $rsResultMunic = $clcensomunic->sql_record($sSqlMunic);
    $numLinhas = $clcensomunic->numrows;

    if ($numLinhas > 0) {
        for ($i = 0; $i < $numLinhas; $i++) {
            db_fieldsmemory($rsResultMunic, $i);
            $arrayMunic[$i] = [
                'nome' => $ed261_c_nome,
                'codigo' => $ed261_i_codigo
            ];
        }
    }

    $jsonResponse = json_encode($arrayMunic);
