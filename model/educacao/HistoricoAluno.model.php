...
class HistoricoAluno
{
    ...
    public function __construct($iCodigoHistorico = null)
    {
        if (!empty($iCodigoHistorico)) {
            $sCampos = "historico.*, ";          
            $sCampos .= "(select ed55_c_descr from duracaocal where ed55_i_codigo = ed52_i_duracaocal) as periodo";
            $oDaoHistorico = new cl_historico;
            $sSqlDadosHistorico = $oDaoHistorico->sql_query_aluno($iCodigoHistorico, $sCampos);
            ...
            if ($oDaoHistorico->numrows > 0) {
                ...
                $this->setCodigoCurso($oDadosHistorico->ed61_i_curso);
                $this->setPeriodo($oDadosHistorico->periodo);
     ...
     */
    public function setCodigoCurso($codigo)
    {
    ...
    public function salvar()
    {
        ...
        $oDaoHistorico->ed61_i_curso = $this->getCodigoCurso();
     ...
     */
    public function getEtapas($iAnoLimite = '')
    {
        $where = [];
        if ($iAnoLimite != '') {
            $where['anolimite'] = "ano <= $iAnoLimite";
        }

        if (count($this->aEtapasCursadas) == 0) {
            $oDaoHistorico = new cl_historico;
            $sSqlEtapas = $oDaoHistorico->sql_query_etapas_historico(
    ...
    {
        if (empty($this->oCurso) && !empty($this->codigoCurso)) {
            $this->oCurso = CursoRepository::getByCodigo($this->codigoCurso);
        }
