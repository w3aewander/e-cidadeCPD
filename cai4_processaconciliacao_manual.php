    ...
    try {
        ...
        if ($lJustificativaCaixa == "true") {
            ...
            $sSqlCodigo = $oDaoConciliaPendenteCorrente->sql_query_file(null, 
                                                                        ...
                                                                        and k89_data     = '{$sDataAutenticacao}' ");
            ...
            if ($oDaoConciliaPendenteCorrente->erro_status == "0") {
                ...
                $clconciliapendcorrente->k89_data = substr($sDataAutenticacao, 0, 4) . "-" . substr($sDataAutenticacao, 5, 2) . "-" . substr($sDataAutenticacao, 8, 2);
                $clconciliapendcorrente->k89_autent = $iCodigoAutenticacao;
                $clconciliapendcorrente->k89_justificativa = $sJustificativa;
            ...
            if ($oDaoConciliaPendenteCorrente->erro_status == '0') {
                throw new Exception("Erro ao gravar a justificativa\n{$oDaoConciliaPendenteCorrente->erro_msg}\n".pg_last_error());
            }
        } else {

            $oDaoConciliaPendenteExtrato = new cl_conciliapendextrato();
            $sSqlCodigo = $oDaoConciliaPendenteExtrato->sql_query_file(null, 
                                                                       "k88_sequencial", 
                                                                       null, 
            ...
            if ($oDaoConciliaPendenteExtrato->erro_status == "0") {
                throw new Exception("Erro ao Buscar código para Justificativa: " . pg_last_error());
            }
            ...
            $_POST["k88_justificativa"] = $sJustificativa;
        ...
        foreach ($arrayObjAutent as $i => $objAutent) {
            if (is_object($objAutent)) {
                ...
                if ($clconciliacor->numrows > 0) {
                    ...
                    $sqlerro = true;
                    $erromsg  = "Não foi possível realizar a conciliação da autenticação do caixa {$objAutent->caixa} "; 
                    $erromsg .= "na data {$objAutent->data} com o autent. {$objAutent->autent} \n";
            ...
            foreach ($arrayObjAutent as $i => $objAutent) {
                ...
                if (is_object($objAutent)) {
                    ...
                    if ($clconciliapendcorrente->erro_status == "0") {
                        ...
                        $sqlerro = true;
                        $erromsg = "conciliapendcorrente - " . $clconciliapendcorrente->erro_msg . "\n";
                        $erromsg .= "$objAutent->classe Concilia: $concilia - Caixa: $objAutent->caixa - Autent: $objAutent->autent - Data: $clconciliapendcorrente->k89_data";
                        break;
                    }
                }
            }
    ...
    if ($strJSONAutent != '') {
        ...
        foreach ($arrayObjAutent as $i => $objAutent) {
            if (is_object($objAutent)) {
                ...
                $sWhere  = "     k84_id = " . $objAutent->caixa;
                $sWhere .= " and k84_data = '" . substr($objAutent->data, 6, 4) . "-" . substr($objAutent->data, 3, 2) . "-" . substr($objAutent->data, 0, 2) . "'";
                $sWhere .= " and k84_autent = " . $objAutent->autent;
                $sWhere .= " and k84_conciliaitem = " . $objAutent->itemconciliacao;
                $clconciliacor->excluir(null, $sWhere);
                if ($clconciliacor->erro_status == 0) {
                    $sqlerro = true;
        ...
        if ($strJSONAutent != '') {
            ...
            foreach ($arrayObjAutent as $i => $objAutent) {
                ...
                if (is_object($objAutent)) {
                     ...
                     * Caso seja encontrado registro o mesmo nao sera incluido
                     */
                    $sWhereValidaExistenciaConciliaPendCorrente  = " k89_concilia = {$k68_sequencial} ";
                    $sWhereValidaExistenciaConciliaPendCorrente .= " and k89_id = {$objAutent->caixa} ";
                    $sWhereValidaExistenciaConciliaPendCorrente .= " and k89_data = '{$dDataConcilia}' ";
                    $sWhereValidaExistenciaConciliaPendCorrente .= " and k89_autent = {$objAutent->autent}";
