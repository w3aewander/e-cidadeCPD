...
use App\Domain\Configuracao\DocumentosTemplate\Reports\Cadastro\CertidaoLancamentoMatricula;

$matricula = $_GET['matricula'];

$oDaoCertLancImov = new cl_certlancimov();

$sSqlUltimaCertidaoGerada = $oDaoCertLancImov->sql_ultimaCertidao($matricula);
...
try {
    $oTemplateCertidaoLancamento = new CertidaoLancamentoMatricula(
        $aUltimaCertidaoGerada['j176_sequencial']
    );

    $oTemplateCertidaoLancamento->configuraDadosVariaveis();
    $pathDocumento = $oTemplateCertidaoLancamento->processaTemplate(false);
