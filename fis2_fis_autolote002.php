...
$clautonumpre  = db_utils::getDao("fis_autonumpre");
...
for($abc = 0; $abc < count($procFiscal); $abc++){
  ...
  $sSqlFis   =  " select distinct y50_codauto, fd01_doc from fiscalizacao.fis_auto ";
  $sSqlFis  .=  " inner join fiscalizacao.fis_tipofiscaliza on fis_tipofiscaliza.y27_codtipo = fis_auto.y50_codtipo "; 
  ...
  if (isset($iCodigoProcessoFiscal) and $iCodigoProcessoFiscal != '') {
    ...
    $sSqlFis .= " where fd01_instit = ".db_getsession("DB_instit")." and y100_sequencial IN ( {$iCodigoProc} ) ORDER BY y50_codauto limit 1";
  }
  ...
  if(pg_num_rows($ResulFis) > 0){
    ...
    $iTemplateAutoinfracao = $fd01_doc;
  } else {
  ...
  if ($iTemplateNotiLancamento != "") {
    ...
    $sDescrDoc        = "AU_{$iCodigoLote}_{$iCodigoProc}";
    $sImprFinal       = "tmp/AU_{$iCodigoLote}_".date("Ymd").".pdf";
    ...
    $sArquivoAgt      = "fiscal/fis_auto_de_infracao_lote.agt";
    ...
    $oDocumentoTemplate = new documentoTemplate(51, $iTemplateAutoinfracao);
