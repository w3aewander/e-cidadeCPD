    ...
    public function gerarDados()
    {
        ...
        for ($iSuplementacao = 0; $iSuplementacao < pg_num_rows($rsDecretos); $iSuplementacao++) {
            ...
            if ($iTipoSuplementacao != '0') {
                ...
                $repositorySiconfi->scopeCodigoRecurso($oDadosSuplementacao->recurso);
                $repositorySiconfi->scopeExercicio(db_getsession('DB_anousu'));

                $novoRecurso = $repositorySiconfi->first();

                if (empty($novoRecurso)) {
                    $sErroLog = "Recurso {$oDadosSuplementacao->recurso} - nao encontrado";
                ...
                $oSuplementacao->cd_Elemento = str_pad(
                    substr($oDadosSuplementacao->elemento, 1, 8),
                    8,
                    ' ',
                    STR_PAD_LEFT
                );

                if ($this->iAnoUso > 2018) {
                    $oDadosSuplementacao->recurso = '';
                }
                ...
                $oSuplementacao->de_LeisAutorizativas = str_pad(
                    substr($oDadosSuplementacao->descricao_lei, 0, 100),
                    100,
                    ' ',
                    STR_PAD_RIGHT
                );
                ...
                if ($this->iAnoUso > 2018) {
                    $oSuplementacao->Nm_FonteRecurso = str_pad($novoRecurso->getCodigoSiconfi(), 8, ' ', STR_PAD_LEFT);
                } else {
                    $oSuplementacao->Nm_FonteRecurso = str_pad(" ", 8, " ", STR_PAD_LEFT);
                }
