<template>
    <Dialog header="Consulta Tipos Ordenação CGF" :maximizable="true" :modal="true" :style="{ width: '1000px' }"
        ...
        <section style="width: 100%" class="m-auto">
            <DataTable :value="data.data" responsiveLayout="scroll" :rowHover="true" :rows="8" showGridlines
                ...
                <Column field="sequencial" :header="data.labels.k240_sequencial" />
                <Column field="descricao" :header="data.labels.k240_descricao" />
            </DataTable>
        </section>
    </Dialog>
</template>
...
async function pesquisaLabels() {
    const labels = await window.axios.get(
        "v4/api/tributario/arrecadacao/tipos-ordenacao-cgf/rotulos"
    );
    data.labels = labels.data.data;
}

async function pesquisaGeralTiposOrdenacao() {
    if (data.data.length > 0) {
        return data.data;
    }
    const response = await window.axios.get(
        "v4/api/tributario/arrecadacao/tipos-ordenacao-cgf/buscar"
    );
