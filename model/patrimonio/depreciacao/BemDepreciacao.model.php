...
class BemDepreciacao {
   ...
   * @param float
   */
  public function getValorSaldoDepreciar() {

    return $this->nValorAtual - $this->nValorResidual;
  }
  /**
  ...
  public function executarLancamentosDepreciacao($sObservacao , $iDocumento, $lEstorno) {
    ...
    foreach ($aDadosSintetico as $oStdSintetico) {
      ...
      $oClassificacao = new BemClassificacao($oStdSintetico->iClassificacao);
      ...
      $oLancamentoAuxiliarDepreciacao->setValorTotal(round($oStdSintetico->nValorDepreciacao,2));
      ...
      $oLancamentoAuxiliarDepreciacao->setClassificacao($oClassificacao);
  ...
  public static function getCompetenciaDisponivel($iInstituicao, $lEstorno = false) {
  	...
  	$oDaoBensDepreciacaoLancamento = new cl_bensdepreciacaolancamento;
  ...
  public function getDadosSintetico() {
        ...
        $aWhereDadosPlanilha = [];
        $aWhereDadosPlanilha[] = "t57_ano = {$this->iAno}";
        $aWhereDadosPlanilha[] = "t57_mes = {$this->iMes}";
        $aWhereDadosPlanilha[] = "t57_instituicao = {$this->iInstituicao}";
        $aWhereDadosPlanilha[] = "t57_processado is true";
        $aWhereDadosPlanilha[] = "t57_ativo is true";
        $aWhereDadosPlanilha[] = "t57_tipocalculo = 1";
        $sWhereDadosPlanilha = implode(" and ",  $aWhereDadosPlanilha);
        ...
        if ($iCodigoConta == null) {
            ...
            $erro = " Existe Classificação de Bem sem associação no plano de contas Classificação:{$iClassificacao}";
  ...
  public static function getInstance(Bem $oBem, DBDate $oDate = null) {
    ...
    if ($oDaoBemDepreciacao->numrows == 1) {
      ...
      $oBemDepreciacao->setTipoCalculo($oStdDadosDepreciacao->t57_tipocalculo);
    }
