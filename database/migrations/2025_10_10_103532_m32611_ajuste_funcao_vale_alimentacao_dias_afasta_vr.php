...
class M32611AjusteFuncaoValeAlimentacaoDiasAfastaVr extends Migration
{
    public function up()
    {
        DB::connection()->getPdo()->exec(
            <<<SQL
      alter function fc_valealimentacao_dias_afasta_vr(integer,integer,integer) set schema pessoal;
      create or replace function pessoal.fc_valealimentacao_dias_afasta_vr(integer,integer,integer) returns setof tp_dias_afastamento as
$$
declare
...
-- Adicione isso antes do SELECT principal
SELECT rh01_admiss INTO _data_admissao
FROM rhpessoal
WHERE rh01_regist = _registro;
...
IF _data_admissao > to_date(ano_ant_||'-'||mes_ant_||'-'||ndias(ano_ant_,mes_ant_)::char(2),'YYYY-mm-dd') THEN
    -- Admitido DEPOIS do mês anterior: não recebe VA
    dias_mes_ant_ := 0;
ELSE
    -- Admitido ANTES ou DURANTE o mês anterior: faz o cálculo normal
    SELECT
        CASE
            WHEN COUNT(*) = 0 THEN
                -- Sem afastamentos
                CASE
                    WHEN _data_admissao > to_date(ano_ant_||'-'||mes_ant_||'-01','YYYY-mm-dd') THEN
                        -- Admitido DURANTE o mês: calcula dias trabalhados
                        ndias(ano_ant_, mes_ant_) - EXTRACT(DAY FROM _data_admissao) + 1
                    ELSE
                        -- Admitido ANTES do mês: 30 dias
                        30
                END
            ELSE
                -- Com afastamentos: 30 - dias_afastados
                (30 - COUNT(*))
        END INTO dias_mes_ant_
FROM
(
    SELECT DISTINCT
        h16_regist,
    ...
    (
        SELECT
            ...
            h16_dtconc,
            h16_dtterm,
            GREATEST(h16_dtconc, to_date(ano_ant_||'-'||mes_ant_||'-01','YYYY-mm-dd')) as dtconc,
            LEAST(COALESCE(h16_dtterm, to_date(ano_ant_||'-'||mes_ant_||'-'||ndias(ano_ant_,mes_ant_)::char(2),'YYYY-mm-dd')),
                  to_date(ano_ant_||'-'||mes_ant_||'-'||ndias(ano_ant_,mes_ant_)::char(2),'YYYY-mm-dd')) as dtterm
        FROM assenta
        INNER JOIN tipoasse ON h12_codigo = h16_assent
        WHERE h12_assent in ('001', '003', '004', '006', '025', '032', '044', '049', '053', '100', '118', '127', '130', '153')
...
WHERE data_dia BETWEEN to_date(ano_ant_||'-'||mes_ant_||'-01','YYYY-mm-dd')
                   AND to_date(ano_ant_||'-'||mes_ant_||'-'||ndias(ano_ant_,mes_ant_)::char(2),'YYYY-mm-dd');
END IF;
