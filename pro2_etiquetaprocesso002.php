...
$sSqlProcessos = $oDaoProtProcesso->sql_query( null,
                                               "*",
                                               "p58_numero::integer",
                                               "p58_numero::integer between {$oGet->numero_inicial} and {$oGet->numero_final} and p58_instit = ".db_getsession("DB_instit")." AND p58_ano = " . db_getsession("DB_anousu"));
...
//var_dump($sSqlProcessos); die("Confere");

//$x = pg_fetch_all($rsProcessos);
//echo "<pre>";
//print_r($x);
//echo "</pre>";
//die("Confere campos");
...
for ($iRow = 0; $iRow < pg_num_rows($rsProcessos); $iRow++) {
  ...
  $oPdf->multicell(98, $nLineHeight, $oProcesso->p51_descr, 0, 'L');

  $oPdf->setY($iY);
  $oPdf->setX(99.1 + $oPdf->getX() + $nDistX + 10);
