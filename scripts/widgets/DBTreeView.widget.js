...
DBTreeView = function (sName) {
   ...
   * @return DBNodeTree
   */
  this.addNode = function (sValue, sLabel, sParentNode, lExpanded, iType, checkbox, action, aParams, iTabIndex = 0, ) {
    ...
    if (typeof(action) == "function") {
        oNodeLabel.observe('keypress', function(event) {
          if (event.keyCode == 13) {
           action(oNode, event);
          }
      });
    ...
    oNodeConector.observe("keypress", function (event) {
      if (event.keyCode == 13) {
        me.showChildren(event, $(oNodeElement.id));
      }
    });
    ...
    if(iTabIndex > 0){
      oNodeLabel.setAttribute('tabindex', iTabIndex);
    }
