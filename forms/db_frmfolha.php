    ...
    <center>
        <table border="0">
            <tr>
                <td nowrap colspan="2">
                    ...
                    try {
                        ...
                        /**
                         * Valida se a variável está ativa no db_conn, estando ativa, valida-se se a folha está aberta,
                         * caso esteja aberta, não é possível fazer a geração para aquela folha, entao retiramos ela
                         * do multi-select. E verifica-se também se existe uma folha, caso não exista, ele retira
                         * também do multi-select a folha.
                         */
            ...
            if (isset($arr_pontosgerfs_final[5])) {
                $sSqlComplementar = $clgerfcom->sql_query_file(
                    $anofolha,
                    $mesfolha,
                    null,
                    null,
                    "distinct r48_semest as comp1,r48_semest as comp2"
                );
                ...
                if (DBPessoal::verificarUtilizacaoEstruturaSuplementar()) {
                    ...
                    $sWhereFolhaPagamento = "rh141_anousu = {$anofolha} "
                        . " and rh141_mesusu = {$mesfolha} and rh141_tipofolha = 3"
                        . " and rh141_instit = ".db_getsession('DB_instit');
                    $sSqlComplementar = $oDaoRhFolhaPagamento->sql_query_file(
                        null,
                        'rh141_codigo,
                        rh141_codigo',
                        'rh141_codigo asc',
                        $sWhereFolhaPagamento
                    );
                    $result_gerfcom = db_query($sSqlComplementar);
            ...
            if (isset($arr_pontosgerfs_final[6]) && DBPessoal::verificarUtilizacaoEstruturaSuplementar()) {
                $oDaoRhFolhaPagamento = new cl_rhfolhapagamento();
                $sWhereFolhaPagamento = "rh141_anousu = {$anofolha} and rh141_mesusu "
                    . "= {$mesfolha} and rh141_tipofolha = 6";
                $sSqlComplementar = $oDaoRhFolhaPagamento->sql_query_file(
                    null,
                    'rh141_codigo, rh141_codigo',
                    null,
                    $sWhereFolhaPagamento
                );
...
function js_complementar(opcao) {
    ...
    if ((erro == 0 && x.complementares) || (erro == 0 && x.suplementares)
        || (erro > 0 && !x.complementares) || (erro > 0 && !x.suplementares) || opcao == 'am') {
...
function js_pesquisa() {
    js_OpenJanelaIframe(
        'CurrentWindow.corpo',
        'db_iframe_folha',
        'func_folha.php?funcao_js=parent.js_preenchepesquisa|r38_regist',
        'Pesquisa',
        true
    );
}
...
function js_enviardados() {
    ...
    if (stringretorno == "") {
        ...
        return false;
    } else if(document.form1.pagtosaldo.selectedIndex == 0 && (document.form1.pagarliq.value == ""
        && document.form1.pagarperc.value == "")) {
