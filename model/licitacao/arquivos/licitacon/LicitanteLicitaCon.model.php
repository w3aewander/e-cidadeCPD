...
class LicitanteLicitaCon extends ArquivoLicitaCon
{
    ...
    private function getAutoresEvento(stdClass $oDadosLicitacao)
    {
        ...
        for ($iRowAutor = 0; $iRowAutor < $iTotalRegistros; $iRowAutor++) {
            ...
            if ($this->isFornecedor($iCodigoAutor,  $oLicitacao->getCodigo())) {
                continue;
            }
    ...
    /**
     * verifica se existe fornecedor na licitacao
     *
     * @param string $cgm
     * @param string $codLicitacao
     * @return bool
     *
     */
    private function isFornecedor($cgm , $codLicitacao)
    {

        $sSql = "SELECT * FROM liclicita
                ...
                INNER JOIN pcorcam ON pcorcamitem.pc22_codorc = pcorcam.pc20_codorc
                  INNER JOIN pcorcamforne ON pcorcamforne.pc21_codorc = pcorcam.pc20_codorc
                WHERE liclicita.l20_codigo = {$cgm}  and  pcorcamforne.pc21_numcgm = {$codLicitacao} ;";
    ...
    private function prepararLicitantesVencedores(array $aCampos)
    {
        ...
        $sSqlBuscaLicitacoes = $oDaoLicitacao->sql_query_licitantes(
            'distinct ' . implode(',', $aCampos),
            implode(' and ', $aWhere),
            true
        );
     ...
     * @throws DBException
     */
    static public function getTipoCondicaoFornecedor($iCodigoFornecedor)
    {
     ...
     * @throws Exception
     */
    public static function getTipoPessoaPorCGM($iNumeroCGM)
    {
        if (empty($iNumeroCGM)) {
            return '';
        }
