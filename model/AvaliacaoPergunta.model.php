...
class AvaliacaoPergunta
{
    ...
    /**
     * AvaliacaoPergunta constructor.
     * @param null|int $iPergunta
     * @throws DBException
     */
    public function __construct($iPergunta = null)
    {
        ...
        $this->setSomenteLeitura($oDadosPergunta->db103_somenteleitura === 't');
    }
     ...
     * @param array|null $aRespostasDissertativas
    ...
    /**
     * @param stdClass $oDadosPergunta
     * @return AvaliacaoPergunta
     * @throws DBException
     */
    public static function make(stdClass $oDadosPergunta)
    {
        $avaliacaoPergunta = new AvaliacaoPergunta();
        $avaliacaoPergunta->setCodigo($oDadosPergunta->db103_sequencial);
        $avaliacaoPergunta->setAtivo($oDadosPergunta->db103_ativo == 't');
        $avaliacaoPergunta->setObrigatoria($oDadosPergunta->db103_obrigatoria == 't');
        $avaliacaoPergunta->setDescricao($oDadosPergunta->db103_descricao);
        $avaliacaoPergunta->setGrupo($oDadosPergunta->db103_avaliacaogrupopergunta);
        $avaliacaoPergunta->setTipo($oDadosPergunta->db103_avaliacaotiporesposta);
        $avaliacaoPergunta->setIdentificador($oDadosPergunta->db103_identificador);
        $avaliacaoPergunta->setTipoComponente($oDadosPergunta->db103_tipo);
        $avaliacaoPergunta->setMascara($oDadosPergunta->db103_mascara);
        $avaliacaoPergunta->setCampoCarga($oDadosPergunta->db103_camposql);
        $avaliacaoPergunta->setPerguntaIdentificadora($oDadosPergunta->db103_perguntaidentificadora == 't');
        $avaliacaoPergunta->setOrdem($oDadosPergunta->db103_ordem);
        $avaliacaoPergunta->setLayoutCampo($oDadosPergunta->db103_dblayoutcampo);
        $avaliacaoPergunta->setIdentificadorCampo($oDadosPergunta->db103_identificadorcampo);
        $avaliacaoPergunta->setSomenteLeitura($oDadosPergunta->db103_somenteleitura === 't');
     ...
     * @return bool
     */
    public function somenteLeitura()
    {
    ...
    public function getRespostas()
    {
        if (count($this->aOpcoesResposta) == 0) {
            $oDaoRespostas = new cl_avaliacaoperguntaopcao();
            ...
            if ($oDaoRespostas->numrows > 0) {
                for ($i = 0; $i < $oDaoRespostas->numrows; $i++) {
                    ...
                    $oResposta->valorresposta = $oRespostaTemp->db104_valorresposta;
                    ...
                    if (!empty($this->iAvaliacao)) {
                        ...
                        $sSqlAvaliacao = $oDaoAvaliacaoResposta->sql_query_pergunta(null, "avaliacaoresposta.*", null,
                          $sWhere);
    ...
    public function getRespostaAvaliacao()
    {
        if (count($this->aResposta) == 0) {
            $oDaoAvaliacaoGrupo = new cl_avaliacaogrupoperguntaresposta();
            $sSqlAvaliacoes = $oDaoAvaliacaoGrupo->sql_query_pergunta(null, "*", null,
     ...
     *
     * @param DBLayoutLinha $oLinhaLayout Linha do layout
     * @param null $iAno
     */
    ...
    {
        $oDaoVinculos = new cl_avaliacaoperguntaopcaolayoutcampo();
    ...
    public function getFormulaVinculada()
    {
        ...
        $sSqlFormula = $oDaoPergunta->sql_query(null, "db148_nome", null,
          "eso01_avaliacaopergunta = {$this->iPergunta}");
    ...
    public function salvar()
    {
        ...
        $oDao->db103_somenteleitura = $this->somenteLeitura === true ? 'true' : 'false';
     ...
     *
     * @return array|AvaliacaoPerguntaOpcao[]
     * @throws DBException
     */
    ...
    /**
     * Método que valida se a pergunta foi respondida
     * @return bool
     */
    public function isRespondida()
    {
        if ($this->iTipo == self::TIPO_RESPOSTA_OBJETIVA || $this->iTipo == self::TIPO_RESPOSTA_MULTIPLA) {

            if (!empty($this->aResposta)) {
                return true;
            }
        }

        if ($this->iTipo == self::TIPO_RESPOSTA_DISSERTATIVA) {
            if (!empty($this->aResposta[0]->textoresposta)) {
                return true;
            }
        }
     ...
     * @param $oResposta
     */
    public function addResposta($oResposta)
    {
        $this->aResposta[] = $oResposta;
    }
}
