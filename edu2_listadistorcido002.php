<?php
/*
 *     E-cidade Software Publico para Gestao Municipal
 *  Copyright (C) 2009  DBSeller Servicos de Informatica
 *                            www.dbseller.com.br
 *                         e-cidade@dbseller.com.br
 *
 *  Este programa e software livre; voce pode redistribui-lo e/ou
 *  modifica-lo sob os termos da Licenca Publica Geral GNU, conforme
 *  publicada pela Free Software Foundation; tanto a versao 2 da
 *  Licenca como (a seu criterio) qualquer versao mais nova.
 *
 *  Este programa e distribuido na expectativa de ser util, mas SEM
 *  QUALQUER GARANTIA; sem mesmo a garantia implicita de
 *  COMERCIALIZACAO ou de ADEQUACAO A QUALQUER PROPOSITO EM
 *  PARTICULAR. Consulte a Licenca Publica Geral GNU para obter mais
 *  detalhes.
 *
 *  Voce deve ter recebido uma copia da Licenca Publica Geral GNU
 *  junto com este programa; se nao, escreva para a Free Software
 *  Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA
 *  02111-1307, USA.
 *
 *  Copia da licenca no diretorio licenca/licenca_en.txt
 *                                licenca/licenca_pt.txt
 */

require_once(modification("libs/db_utils.php"));
require_once(modification("libs/db_app.utils.php"));
require_once(modification("libs/db_stdlib.php"));
require_once(modification("libs/db_conecta.php"));
require_once(modification("libs/db_sessoes.php"));
require_once(modification("dbforms/db_funcoes.php"));
require_once(modification("std/DBDate.php"));
require_once(modification("fpdf151/pdf.php"));
require_once(modification("classes/db_edu_parametros_classe.php"));
require_once(modification("model/educacao/avaliacao/iFormaObtencao.interface.php"));
require_once(modification("model/educacao/avaliacao/iElementoAvaliacao.interface.php"));

$oGet = db_utils::postMemory($_GET);

db_app::import("CgmFactory");
db_app::import("educacao.progressaoparcial.*");
db_app::import("educacao.avaliacao.*");
db_app::import("educacao.*");
db_app::import("exceptions.*");

$aTurmaProcessada = array();
$aTurmas          = array();
$iTotalAlunosEscola = 0;
$iIdade = $oGet->iIdade;


foreach (explode(",", $oGet->aTurmas) as $iCodigoTurma) {

  // Busca a turma pelo código e adiciona ao array $aTurmas
  $aTurmas[] = TurmaRepository::getTurmaByCodigo($iCodigoTurma);

}


/**
 * Percorremos as e filtra os dados necessarios para o relatorio
 */
foreach ($aTurmas as $oTurma) {

  $aEtapas                      = $oTurma->getEtapas();
  $sNomeEscola       = $oTurma->getEscola()->getNome();
  $iCodigoReferencia = $oTurma->getEscola()->getCodigoReferencia();

  if ( $iCodigoReferencia != null ) {
    $sNomeEscola = "{$iCodigoReferencia} - {$sNomeEscola}";
  }

  $oDadosTurma                  = new stdClass();
  $oDadosTurma->aAlunos         = array();
  $oDadosTurma->iCodigoTurma    = $oTurma->getCodigo();
  $oDadosTurma->sDescricaoTurma = $oTurma->getDescricao();
  $oDadosTurma->sCalendario     = $oTurma->getCalendario()->getDescricao();
  $oDadosTurma->iAno            = $oTurma->getCalendario()->getAnoExecucao();
  $oDadosTurma->sEscola         = $sNomeEscola;
  $oDadosTurma->sCurso          = $aEtapas[0]->getEtapa()->getEnsino()->getNome();
  $oDadosTurma->sEtapa          = $aEtapas[0]->getEtapa()->getNome();
  $oDadosTurma->sTurma          = $oTurma->getDescricao();
  // $oDadosTurma->iTotalAulas     = 0;
  // $oDadosTurma->iTotalHorasAula = 0;


  // foreach ($oTurma->getDisciplinas() as $oRegencia) {
  //   $oPeriodo = PeriodoAvaliacaoRepository::getPeriodoAvaliacaoByCodigo($oGet->iPeriodo);
  //   $oDadosTurma->iTotalAulas += $oRegencia->getTotalDeAulasNoPeriodo($oPeriodo);
  //   $oDadosTurma->iTotalHorasAula += $oRegencia->getTotalHorasAula($oPeriodo);
  // }

  /**
   * Buscamos todos os alunos matriculados na turma
   */
  foreach ($oTurma->getAlunosMatriculados(true) as $oMatricula) {

    db_inicio_transacao();
    $oDiario    = $oMatricula->getDiarioDeClasse();
    db_fim_transacao();

    $oAluno                   = new stdClass();
    $oAluno->sNome            = $oMatricula->getAluno()->getNome();
    $oAluno->iCodigoAluno     = $oMatricula->getAluno()->getCodigoAluno();
    $oAluno->sSituacao        = $oMatricula->getSituacao();
    $oAluno->dtDataNascimento = $oMatricula->getAluno()->getDataNascimento();
    $oAluno->iIdade           = date_diff(new DateTime($oMatricula->getAluno()->getDataNascimento()), new DateTime())->y;
    $oAluno->iTotalFaltas     = 0;
    $oAluno->iTotalAbonadas   = 0;


    // foreach ($oDiario->getDisciplinas() as $oDiarioAvaliacaoDisciplina) {

    //   $oPeriodo = PeriodoAvaliacaoRepository::getPeriodoAvaliacaoByCodigo($oGet->iPeriodo);
    //   $oAluno->iTotalFaltas   += $oDiarioAvaliacaoDisciplina->getTotalFaltasPorPeriodo($oPeriodo);
    //   $oAluno->iTotalAbonadas += $oDiarioAvaliacaoDisciplina->getTotalFaltasAbonadasPorPeriodo($oPeriodo);
    // }

    // if ($oDadosTurma->iTotalAulas == 0) {

    //   $nPercentualFaltas     = "";
    //   $nPercentualFrequencia = "";
    // } else {

    //   $iTotalFaltas                    = $oAluno->iTotalFaltas - $oAluno->iTotalAbonadas;
    //   $nPercentualFaltasNaoArredondado = ($iTotalFaltas * 100) / $oDadosTurma->iTotalAulas;
    //   $nPercentualFrequencia           = ArredondamentoFrequencia::formatar(100 - $nPercentualFaltasNaoArredondado, $oDadosTurma->iAno);
    //   $nPercentualFaltas               = 100 - $nPercentualFrequencia;
    // }
    // $oAluno->nPercentualFrequencia = "{$nPercentualFrequencia}";
    // $oAluno->nPercentualFaltas     = "{$nPercentualFaltas}";



    if ($oAluno->sSituacao == "MATRICULADO") {
      if($oAluno->dtDataNascimento < $dtDataInicial || $oAluno->dtDataNascimento > $dtDataFinal) {
        $sCalendarioBase = preg_replace('/ \d+$/', '', $oDadosTurma->sCalendario);
        $oDadosTurma->aAlunos[] = $oAluno;
      }

    //   if ($sCalendarioBase == "ANOS INICIAIS" && $oAluno->iTotalFaltas >= 10) {
    //       $oDadosTurma->aAlunos[] = $oAluno;
    //   } elseif ($sCalendarioBase == "ANOS FINAIS" && $oAluno->iTotalFaltas >= 50) {
    //       $oDadosTurma->aAlunos[] = $oAluno;
    //   } elseif ($sCalendarioBase == "ED. INFANTIL" && $oAluno->iTotalFaltas >= 10) {
    //       $oDadosTurma->aAlunos[] = $oAluno;
    //   } elseif ($sCalendarioBase == "EJA FINAIS" && $oAluno->iTotalFaltas >= 50) {
    //       $oDadosTurma->aAlunos[] = $oAluno;
    //   } elseif ($sCalendarioBase == "EJA INICIAIS" && $oAluno->iTotalFaltas >= 10) {
    //       $oDadosTurma->aAlunos[] = $oAluno;
      }
    }
    $aTurmaProcessada[$oTurma->getCodigo()] = $oDadosTurma;
  }






$oPdf = new PDF();
$oPdf->Open();
$oPdf->AliasNbPages();
$oPdf->SetFillColor(235);
$oPdf->SetAutoPageBreak(false);

$lPrimeiraVolta = true;
$iHeigth        = 4;

foreach ($aTurmaProcessada as $oTurma) {
  // Soma o total de alunos na escola
  $iTotalAlunosEscola += count($oTurma->aAlunos);
}


// foreach ($aTurmaProcessada as $oTurma ) {
//   // Soma o total de alunos na escola
 
// }

$iTotalAlunosEscolaHead = $iTotalAlunosEscola;
$iTotalAlunosFaltasHead =  $iTotalAlunosFaltas;
/**
 * Processando impressao do relatorio
 */
foreach ($aTurmaProcessada as $oTurma) {

  

  $head1 = "Escola : {$oTurma->sEscola}";
  $head2 = "Calendario : {$oTurma->sCalendario}";
  $head3 = "Curso: {$oTurma->sCurso}";
  $head4 = "Etapa: {$oTurma->sEtapa}";
  $head5 = "Turma: {$oTurma->sTurma}";
  $head6 = "Total de Alunos : {$iTotalAlunosEscolaHead}";
  $head7 = "Intervalo de Idade: ". $iIdadeInicial . ' - ' . $iIdadeFinal;


  foreach ($oTurma->aAlunos as $oAluno) {



    if (($oPdf->GetY() > $oPdf->h -10) || $lPrimeiraVolta) {

      $lPrimeiraVolta = false;
      imprimeHeader($oPdf, $iHeigth);
    }

    $oPdf->SetFont("arial", "", 6);
    $oPdf->Cell(12,  $iHeigth, $oAluno->iCodigoAluno,      "TBLR", 0, "R");
    $oPdf->Cell(100, $iHeigth, $oAluno->sNome,                 1, 0, "L");
    $oPdf->Cell(30,  $iHeigth, $oAluno->sSituacao,      "TB",0, "L");
    $oPdf->Cell(30,  $iHeigth, $oAluno->dtDataNascimento,     "TBL", 0, "L");
    $oPdf->Cell(20,  $iHeigth, $oAluno->iIdade,     "TBLR", 1, "L");
  }

  if(count($oTurma->aAlunos) > 0 && !empty($oTurma->aAlunos)){
    $oPdf->ln();
    $oPdf->SetFont("arial", "b", 7);
    $oPdf->Cell(35,  $iHeigth, "Total de Alunos na Turma: ",  0, 0, "L");
    $oPdf->Cell(80,  $iHeigth, count($oTurma->aAlunos),       0, 0, "L");

    $oPdf->Cell(115,  $iHeigth, "Data: _____/_____/__________",                  0, 1, "L");
    $oPdf->Cell(80,  $iHeigth, "Assinatura: ________________________________",  0, 1, "L");
  }
  else{
    $oPdf->AddPage();
    $oPdf->SetFont("arial", "b", 20);
    $oPdf->Cell(35,  $iHeigth, "A turma não possui alunos com o intervalo de idade selecionado.",  0, 0, "L");
  }



  $lPrimeiraVolta = true; 

}

$oPdf->Output();

/**
 * Cria o cabeçalho da truma
 * @param PDF $oPdf
 * @param integer $iHeigth altura da linha
 */
function imprimeHeader($oPdf, $iHeigth) {

  $oPdf->AddPage();
  $oPdf->SetFont("arial", "b", 7);
  $oPdf->Cell(12,  $iHeigth, "Código",             "TBRL", 0, "C", 1);
  $oPdf->Cell(100, $iHeigth, "Nome",                       1, 0, "C", 1);
  $oPdf->Cell(30,  $iHeigth, "Situação",                   1, 0, "C", 1);
  $oPdf->Cell(30,  $iHeigth, "Data de Nascimento",         1, 0, "C", 1);
  $oPdf->Cell(20,  $iHeigth, "Idade",              "TBLR", 1, "C", 1);
}