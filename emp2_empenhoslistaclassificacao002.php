...
try {
  ...
  if (!empty($oGet->data_inicial_nota)) {
    $oRelatorioEmpenhoClassificao->setDataInicialNota(new DBDate($oGet->data_inicial_nota));
  }

  if (!empty($oGet->data_final_nota)) {
    $oRelatorioEmpenhoClassificao->setDataFinalNota(new DBDate($oGet->data_final_nota));
  }

  if (!empty($oGet->data_inicial_liquidacao)) {
    $oRelatorioEmpenhoClassificao->setDataLiquidacaoInicial(new DBDate($oGet->data_inicial_liquidacao));
  }

  if (!empty($oGet->data_final_liquidacao)) {
     $oRelatorioEmpenhoClassificao->setDataLiquidacaoFinal(new DBDate($oGet->data_final_liquidacao));
   }

  $oRelatorioEmpenhoClassificao->setOrdenacao($oGet->ordenacao);
