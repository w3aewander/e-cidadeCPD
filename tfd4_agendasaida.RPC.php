  ...
  switch($oParam->sExecucao) {
    ...
    case 'getDadosPedido':
      ...
      $sCamposPedido .= ", CASE
                            WHEN exists
                                    (select 1
                                    from cancelamentoviagem where cancelamentoviagem.tf41_veiculodestino = tfd_veiculodestino.tf18_i_codigo) THEN 'true'
                            ELSE 'false'
                        END as cancelado";
      ...
      $oDadosPedido      = db_utils::getCollectionByRecord($rsPedido);
      $oDadosPedido = $oDadosPedido[0];
      ...
      if ($oDadosPedido->cancelado === 'true') {
          $oPedido->iVeiculo         = '';
          $oPedido->iLotacaoVeiculo  = '';
          $oPedido->sHoraSaida       = '';
      }
      ...
      $oPedido->cancelado        = $oDadosPedido->cancelado;
    ...
    case 'getParametros':
      ...
      $oRetorno->lUtilizaGradeHorario = $config->tf11_i_utilizagradehorario == 1;
      $oRetorno->obrigaHoraSaida = $config->tf11_obriga_hora_saida == 't';
    ...
    case 'getVagasVeiculo':
      ...
      $aWhere[] = " not exists (select 1 from cancelamentoviagem where tf41_veiculodestino = tf18_i_codigo) ";
    ...
    case 'salvar' :
      ...
      */
      ...
      if(empty($oParam->horaSaida) && ($config->tf11_obriga_hora_saida === 't' || !empty($oParam->iCodigoVeiculo))) {
    ...
    case "removerPassageirosVeiculo":
      ...
      case 'getEnderecoPaciente':
        $oRetorno->endereco = '';
        $dao = new cl_cgs_und();
        $sql = $dao->sql_query_file($oParam->idCgs, 'z01_v_ender, z01_i_numero, z01_v_bairro, z01_v_compl');
        $rs = $dao->sql_record($sql);
        if ($dao->numrows > 0) {
          $endereco = db_utils::fieldsMemory($rs, 0);
          $bairro = $endereco->z01_v_bairro ?: 'N/A';
          $enderecos = [];
          $enderecos[] = $endereco->z01_v_ender ?: 'N/A';
          $enderecos[] = $endereco->z01_i_numero ?: 'N/A';
          if (!empty($endereco->z01_v_compl)) {
                $enderecos[] = $endereco->z01_v_compl;
          }
          $endereco = implode(', ', $enderecos);
          $oRetorno->endereco = "{$endereco} - {$bairro}";
        }
      break;
  }
...
function removePassageirosVeiculo($iPedido, $aPassageiros = array()) {
  ...
  if ( !empty($iVeiculoDestino) ) {
    ...
    if ($rsTemPassageiro && pg_num_rows($rsTemPassageiro) == 0) {
      removePassageirosRetorno($iVeiculoDestino);
...
function removePassageirosRetorno($veiculoDestino) {
    $dao = new cl_tfd_passageiroretorno();
    $dao->excluir(null,"tf31_i_veiculodestino = {$veiculoDestino}");

    if($dao->erro_status == 0) {
      throw new Exception("Erro ao remover passageiros que iriam retornar no veículo. \n{$dao->erro_msg};");
    }
}
