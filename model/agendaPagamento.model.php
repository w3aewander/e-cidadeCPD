...
class agendaPagamento
{
     ...
     */
    public function setFormaPagamentoCGM($iNumCgm, $iFormaPagamento)
    {

        if ($iFormaPagamento != 0) {

            $oDaoEmpAgeCgm = new cl_empageformacgm();
     ...
     * @return integer
     */
    public function getformaPagamentoCGM($iNumCgm)
    {
     ...
     */
    public function setContaPagadoraCgm($iNumCgm, $iCodTipo)
    {
     ...
     * @return array
     */
    public function getNotasLiquidacao($sWhere = null, $lRetornaContasVinculadas = false)
    {
    ...
    /**
     * @param integer $iCodigoCP código da conta pagadora
     *
     *
     * @return string
     * @throws \Exception
     */
    public function getContasParametrosCod($iCodigoOrdem, $lRetornaContasVinculadas = true)
    {
        ...
        $buscaContasPagadoras = $daoParametro->sql_query_file($iInstituicaoSessao, "k29_fr_contapagadora");

        $iCodigoCP = db_utils::fieldsMemory($daoParametro->sql_record($buscaContasPagadoras), 0)->k29_fr_contapagadora;

        $sCampos = " distinct e83_conta, e83_descr,e83_codtipo,c61_codigo, gestao as o15_recurso ";
        $sWhere = " e50_codord = {$iCodigoOrdem} AND (k13_limite is null or k13_limite >= '{$sDataAtual}') ";

        if ($iCodigoCP == 1) {
            $sWhere = " pagordem.e50_codord  = {$iCodigoOrdem} AND (saltes.k13_limite IS NULL OR saltes.k13_limite >= '{$sDataAtual}' ";
            $sSqlContas = $oDaoEmpAgeTipo->sql_query_contas_vinculadas_fr(null, $sCampos, "e83_conta", $sWhere, $lRetornaContasVinculadas);
        } else {
     ...
     * @throws \Exception
     * comitando para versionar fonte
     */
    public function getContasRecurso($iCodigoOrdem, $lRetornaContasVinculadas = true)
    {
        ...
        $sDataAtual = date('Y-m-d', db_getsession('DB_datausu'));
        $oDaoEmpAgeTipo = new cl_empagetipo;
        $sSqlContas = $this->getContasParametrosCod($iCodigoOrdem, $lRetornaContasVinculadas);

        /* [Extensão] - Filtro da Despesa - getCopntasRecurso */
     ...
     */
    public function getVersoCheque($aMovimentos = "", $sInformacaoExtra = '')
    {
     ...
     * @return float
     */
    public function getSaldoConta($iCodTipoConta, $dtBase, $movimento = null)
    {
     ...
     * @return float
     */
    public function getTotalCheques($iCodTipoConta)
    {
    ...
    public function setCodigoOrdemAuxiliar()
    {
     ...
     * @return integer
     */
    public function verificaAgendaNota($iCodNota)
    {

        $oAgenda = new stdClass;
        $oAgenda->iCodAgenda = null;
        $oAgenda->iCodMov = null;
        $oDaoAgenda = new cl_empage();
     ...
     * @return integer
     */
    public function addMovimentoAgenda($iTipo, $oObjeto)
    {
        ...
        if ($iTipo == 1) {
            ...
            if (strpos($oObjeto->iNumEmp, "/", 0)) {
                ...
                $sSqlEmpenho = $oDaoEmpenho->sql_query_file(
                    null,
                    "e60_numemp",
                    null,
                    " e60_codemp='{$aEmpenho[0]}' and e60_anousu = '{$aEmpenho[1]}'"
                );
            ...
            $oDaoEmpAgeMov->e81_valor  = (float)"".$oObjeto->nValor."";
            ...
            if (isset($oObjeto->iConcarPeculiar) && !empty($oObjeto->iConcarPeculiar)) {
                ...
                $oDaoEmpageConcar = new cl_empageconcarpeculiar();
            ...
            $empenho = EmpenhoFinanceiroRepository::getEmpenhoFinanceiroPorNumero($oObjeto->iNumEmp);
            $dadosPrestacaoContas = $empenho->getDadosPrestacaoContas();
            if ($dadosPrestacaoContas !== false) {
                $daoEmpPresta = new cl_emppresta;
                if (empty($dadosPrestacaoContas->e45_codmov)) {
                    $daoEmpPresta->e45_sequencial = $dadosPrestacaoContas->e45_sequencial;
                    $daoEmpPresta->e45_codmov = $iCodMovimento;
                    $daoEmpPresta->alterar($daoEmpPresta->e45_sequencial);
                } else {
                    $daoEmpPresta->e45_numemp  = $dadosPrestacaoContas->e45_numemp;
                    $daoEmpPresta->e45_data    = date('Y-m-d');
                    $daoEmpPresta->e45_tipo    = $dadosPrestacaoContas->e45_tipo;
                    $daoEmpPresta->e45_codmov  = $iCodMovimento;
                    $daoEmpPresta->incluir(null);
                }

                if ($daoEmpPresta->erro_status === '0') {
                    throw new Exception("Ocorreu um erro ao atualizar o movimento da prestação de contas.");
                }
            }
     ...
     */
    public function setCodigoAgenda($iCodigoAgenda)
    {
     ...
     */
    public function emiteCheque($aNotasLiquidacao, $dtData, $sCredor = '', $aCheques = null)
    {
     ...
     *
     */
    public function cancelarCheque($iCodMov)
    {
     ...
     */
    public function autorizarPagamento($dtPagamento, $aMovimentos = null, $iTipoOperacao = 1, $iCodigoOPauxiliar = null)
    {
     ...
     * @throws Exception
     */
    public function getMovimentosAgenda($sWhere = null, $sJoin, $lTrazfornecedor = true, $lTrazContaPagadora = true, $sCamposAdicionais = '', $lVinculadas = true, $sCredorCgm = null)
    {
      ...
      e60_numemp,
      e60_codemp,
      (case when e91_recurso is null then  o15_codigo else e91_recurso end) as o15_codigo,
      e86_data,
      e50_codord,
      e53_valor,
      o58_codigo,
      ...
      (case when e85_codmov is null then
           (select e28_empagetipo
             from empageformacgm
             join empagetipo on e83_codtipo = e28_empagetipo
             join conplanoreduz on c61_reduz = e83_conta
                  and c61_anousu = e60_anousu
      ...
      e42_dtpagamento,
      e53_vlrpag,
      round(e81_valor + (select coalesce(sum(e34_valordesconto), 0)
                           from pagordemdesconto
                           where e34_codord = e50_codord
                        ), 2) as e81_valor,
      e86_codmov,
      e43_sequencial,
      e42_sequencial,
    ...
    public function getMovimentosAgendaPagamento($sWhere = null, $sJoin, $lTrazfornecedor = true, $lTrazContaPagadora = true, $sCamposAdicionais = '', $lVinculadas = true, $sCredorCgm = null)
    {
        $sOrderBy = "e81_codmov, e50_codord";
        if ($this->orderBy != null) {
            $sOrderBy = $this->orderBy;
        ...
        /**
         * Com apropriacao ativada sistema gera o movimento com o valor liquidado. ja aplicando o desconto.
         * tendo que aplicar o desconto
         *
         * Sem apropriacao o sistema gera o movimento com o valor da OP sem o desconto
         */

        $sCampoValor = "
               round(e81_valor - (select coalesce(sum(e34_valordesconto), 0)
          from pagordemdesconto
          left join pagordemdescontoempanulado on e06_pagordemdesconto = e34_sequencial
            where e34_codord = e50_codord
           /* and e34_sequencial is null */
           ), 2) as e81_valor
         ";

        if ($lApropriacao) {

            $sCampoValor = " e81_valor ";
        }

        /**
         * essa regra de desconto foi desconsiderada, pois na agenda deve vir o valor liquidado,
         * que ja está sendo aplicado o desconto ao criar a empagemov e o desconto ele ajusta o e81_valor
         */
    ...
    public function getMovimentosPagos($sWhere, $lTrazContaPagadora = true)
    {
        if (!empty($sWhere)) {
            $sWhere = " and {$sWhere} ";
        }
     ...
     */
    public function getContasFornecedor($iNumCgm)
    {
        $oDaoPcForne = new cl_pcfornecon();
    ...
    public function configurarPagamentos($dtPagamento, stdClass $oMovimento, $iCodigoOrdemAuxiliar = null, $lGerarOpAuxiliar = true)
    {
        ...
        if ($oMovimento->iCodForma == 0) {
            ...
            if (round(($oMovimento->nValor + $oMovimento->nValorRetencao), 2) < $oMovimentoOriginal->e81_valor) {
                 ...
                 */
                $oDaoEmpageNotas = new cl_empagenotasordem();
        ...
        if ($isPB && isset($oMovimento->sHistorico)) {

            if (!empty($oMovimento->sHistorico)) {

                $sHistorico = $oMovimento->sHistorico;
                $daoHistorico = new cl_empagemovhistorico;
                $daoHistorico->excluir(null, "e141_empagemov = {$iCodigoMovimento}");

                $daoHistorico->e141_empagemov = $iCodigoMovimento;
                $daoHistorico->e141_historico = db_stdClass::normalizeStringJsonEscapeString($sHistorico);
                $daoHistorico->incluir(null);
                if ($daoHistorico->erro_status == "0") {
                    throw new Exception("Erro [8] - Erro ao configurar dados de historico do movimento.");
                }
            }
                                      ...
                                      $lComSlip = false, $iCtaCredito = null, $iCtaDebito = null, $sWhere = null)
    {
        ...
        $oDaoEmpagemovslip = new cl_empagemovslips();
     ...
     * @return array
     */
    public function getSlips($sWhere, $lTrazContaFornecedor = false)
    {
        ...
        $sCampos .= "e97_codmov,                                             ";
        $sCampos .= "e83_conta,                                              ";
        $sCampos .= "case when sliprecursocontas.k181_recursocredito is not null ";
        $sCampos .= "       then sliprecursocontas.k181_recursocredito ";
        $sCampos .= "     else ctapag.c61_codigo ";
        $sCampos .= "end as c61_codigo, ";
        ...
        $sCampos .= " case ";
        $sCampos .= "   when ";
        $sCampos .= "     retencaotiporec.e21_envioremessabancaria is false ";
        $sCampos .= "     and not exists (select 1 ";
        $sCampos .= "                       from retencaotiporecorcunidadeliberaremessa ";
        $sCampos .= "                            inner join db_departorg on db01_orgao = e287_orgao ";
        $sCampos .= "                                                   and db01_unidade = e287_unidade ";
        $sCampos .= "                                                   and db01_anousu = " . db_getsession("DB_anousu");
        $sCampos .= "                      where db01_coddepto = " . db_getsession("DB_coddepto") . ") ";
        $sCampos .= "     then false ";
        $sCampos .= "   else true ";
        $sCampos .= " end AS permite_pagamento_remessa";
    ...
    public function getMovimentosCheque($sWhereOrdens, $sWhereSlips, $sJoin = '')
    {
        $oInstit = db_stdClass::getDadosInstit();
        $oDaoSlip = new cl_empageslip();
     ...
     */
    public function setOrdemConsultas($sOrdem)
    {
        $this->orderBy = $sOrdem;
    }
     ...
     * @return void
     */
    public function cancelaMovimentoOrdemAuxiliar($iOrdemAuxiliar, $iMovimento)
    {
     ...
     * @return array
     */
    public function getTotaisAgenda($sWhere = null)
    {
        ...
        $sSqlTotais .= "               join fonterecurso on fonterecurso.orctiporec_id = orctiporec.o15_codigo
     ...
     */
    public function agruparMovimentos($aMovimentos)
    {
    ...
    /**
     * Retorna o recurso vinculado ao movimento
     * @param int $movimento Código do movimento da agenda de pagamentos
     * @return Recurso Instancia do recurso
     * @throws Exception
     */
    public static function getRecursoDoMovimento($movimento)
    {
        $daoEmpAgemov = new cl_empagemov();
        $sqlDados = $daoEmpAgemov->sql_query_dados_recurso($movimento);
    ...
    /**
     * @param $movimentoEstornado
     * @param Silex\Exception\
     *
     * @return bool
     * @throws Exception
     */
    public static function criarMovimentoAgendaParaSlip($movimentoEstornado, $codigoSlip)
    {

        $transferencia = TransferenciaFactory::getInstance(null, $codigoSlip);
        $buscaCodigoAgenda = db_query('
            select e81_codage,
                   e85_codtipo
              from empagemov
                   join empagepag on empagepag.e85_codmov = empagemov.e81_codmov
             where e81_codmov = ' . $movimentoEstornado);
        if (!$buscaCodigoAgenda) {
            throw new Exception("Não foi possível buscar o código da agenda.");
        }

        $dadosAgenda = db_utils::fieldsMemory($buscaCodigoAgenda, 0);
        $daoEmpAgeMov = new cl_empagemov();
        $daoEmpAgeMov->e81_codmov = null;
        $daoEmpAgeMov->e81_codage = $dadosAgenda->e81_codage;
        $daoEmpAgeMov->e81_numemp = 'null';
        $daoEmpAgeMov->e81_valor = $transferencia->getValor();
        $daoEmpAgeMov->e81_cancelado = '';
        $daoEmpAgeMov->incluir(null);
        if ($daoEmpAgeMov->erro_status === '0') {
            throw new Exception("Não foi possível incluir o movimento na agenda.");
        }

        $daoEmpageSlip = new cl_empageslip();
        $daoEmpageSlip->e89_codigo = $codigoSlip;
        $daoEmpageSlip->e89_codmov = $daoEmpAgeMov->e81_codmov;
        $daoEmpageSlip->incluir($daoEmpAgeMov->e81_codmov, $codigoSlip);
        if ($daoEmpageSlip->erro_status === '0') {
            throw new Exception("Ocorreu um erro ao vincular o slip ao novo movimento.");
        }

        $daoEmpagePag = new cl_empagepag();
        $daoEmpagePag->e85_codmov = $daoEmpAgeMov->e81_codmov;
        $daoEmpagePag->e85_codtipo = $dadosAgenda->e85_codtipo;
        $daoEmpagePag->incluir($daoEmpagePag->e85_codmov, $daoEmpagePag->e85_codtipo);
        if ($daoEmpagePag->erro_status === '0') {
            throw new Exception("Erro ao incluir a conta pagadora.");
        }

        unset($daoEmpAgeMov);
        $dataAtual = new DBDate(date('Y-m-d', db_getsession('DB_datausu')));
        $daoEmpAgeMov = new cl_empagemov();
        $daoEmpAgeMov->e81_codmov = $movimentoEstornado;
        $daoEmpAgeMov->e81_cancelado = $dataAtual->getDate();
        $daoEmpAgeMov->alterar($movimentoEstornado);
        if ($daoEmpAgeMov->erro_status === '0') {
            throw new Exception("Erro ao cancelar o movimento anterior.");
        }

        $transferencia->alterarSituacao(Transferencia::SITUACAO_NAO_AUTENTICADO);
        return true;
    }
}
