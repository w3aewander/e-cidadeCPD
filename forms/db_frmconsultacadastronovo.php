...
function mostraJanelaPesquisa() {
	...
	if (F.j01_matric.value.length > 0) {
	...
	} else if(F.j108_condominio.value.length > 0) {
        VisualizacaoSetorQuadraLoteLoc.jan.location.href = `func_iptubase.php?funcao_js=parent.mostraTodasMatriculasSetorLoc|j01_matric&j108_condominio=${F.j108_condominio.value}&j107_nome=${F.j107_nome.value}`;
        VisualizacaoSetorQuadraLoteLoc.mostraMsg();
        VisualizacaoSetorQuadraLoteLoc.show();
        VisualizacaoSetorQuadraLoteLoc.focus();
	}else if(F.j34_loteam.value.length > 0) {
        VisualizacaoSetorQuadraLoteLoc.jan.location.href = `func_iptubase.php?funcao_js=parent.mostraTodasMatriculasSetorLoc|j01_matric&j34_loteam=${F.j34_loteam.value}&j34_descr=${F.j34_descr.value}`;
        VisualizacaoSetorQuadraLoteLoc.mostraMsg();
        VisualizacaoSetorQuadraLoteLoc.show();
        VisualizacaoSetorQuadraLoteLoc.focus();
        ...
        $parametro = F.j108_condominio.value != "" ? "&j108_condominio=" + encodeURIComponent(F.j108_condominio.value) : "";
        ...
        if (F.j34_setor.value.length > 0){
            $parametro = $parametro + "setor=" + encodeURIComponent(F.j34_setor.value);
        }
		...
		VisualizacaoMatricula.jan.location.href = 'func_iptubaseregimovel.php?funcao_js=parent.mostraTodasMatriculas_PesquisaMatricregimo|2&matricregimo='
            + F.j04_matricregimo.value;
	    VisualizacaoMatricula.mostraMsg();
	    VisualizacaoMatricula.show();
        VisualizacaoMatricula.focus();
	    ...
	    if (F.j04_setorregimovel.value.length > 0){
            $parametro = $parametro + "setor=" + F.j04_setorregimovel.value;
        }
...
function js_pesquisaMatricula(lMostra) {
  var sQueryString = 'func_iptubase.php?tipoImovel=<?=$tipoImovel?>&';
  if(lMostra) {
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<table border="0">
                    ...
                    <tr>
                        <td nowrap title="<?=@$Tj108_condominio?>">
                            <?php
                            db_ancora(@$Lj108_condominio,"js_pesquisaj108_condominio(true);",1);
                            ?>
                        </td>
                        <td>
                            <?php
                            db_input('j108_condominio',10,$Ij108_condominio,true,'text',1," onchange='js_pesquisaj108_condominio(false);'")
                            ?>
                            <?php
                    ...
                    <tr>
                        <td nowrap title="<?=@$Tj34_loteam?>">
                            <?php
                            db_ancora(@$Lj34_loteam,"js_pesquisaj34_loteam(true);",1);
                            ?>
                        </td>
                        <td>
                            <?php
                            db_input('j34_loteam',10,$Ij34_loteam,true,'text',1," onchange='js_pesquisaj34_loteam(false);'")
                            ?>
                            <?php
...
function js_pesquisaj108_condominio(mostra){
    if(mostra==true){
        ...
        if(document.form1.j108_condominio.value != ''){
            js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_condominio','func_condominio.php?pesquisa_chave='+document.form1.j108_condominio.value+'&funcao_js=parent.js_mostracondominio&tipo=1','Pesquisa',false);
        }else{
...
function js_mostracondominio(chave,erro){
    ...
    if(erro==true){
        document.form1.j108_condominio.focus();
        document.form1.j108_condominio.value = '';
    }
}
function js_mostracondominio1(chave1,chave2){
    document.form1.j108_condominio.value = chave1;
...
function js_pesquisaj34_loteam(mostra){
    if(mostra==true){
        js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_loteamento','func_loteam.php?funcao_js=parent.js_mostraloteamento1|j34_loteam|j34_descr','Pesquisa',true);
    }else{
        if(document.form1.j34_loteam.value != ''){
            js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_loteamento','func_loteam.php?pesquisa_chave='+document.form1.j34_loteam.value+'&funcao_js=parent.js_mostraloteamento','Pesquisa',false);
        }else{
            document.form1.j34_descr.value = '';
        }
    }
}
function js_mostraloteamento(chave,erro){
