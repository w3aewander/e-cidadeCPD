...
class cancelamentoDebitos {
   ...
   */
  public $sHistoricoProcessamento = "Cancelamento gerado automaticamente pelo Cancelamento da Inscrição em Dívida.";
  ...
  function geraCancelamento($aDebitos=null){
    ...
    $oDaoCancDebitos->k20_depart          = db_getsession("DB_coddepto");
  ...
  function cancelaDebitos($aDebitos=null,$iCodCancDebitos){
     ...
     */
     ...
     if ( $rsRegistrosExistentes && pg_num_rows($rsRegistrosExistentes) > 0 ) {
      ...
      $oDaoCancDebitosReg->k21_obs    = "Cancelamento gerado automaticamente pelo Cancelamento da Inscrição em Dívida.";
  ...
  public function cancelaReciboRetencao(array $aDebitos)
  {

    foreach( $aDebitos as $debito ){

      $numpre = $debito['Numpre'];
      // verifica se é de retencao:
      $campos = "e50_codord, arrecad.*, cornump.* ";
      $where = " k00_numpre = {$numpre} and arrecad.k00_numtot = 1";
      $daoCornump = new cl_cornump;
      $sqlRetencao = $daoCornump->sql_query_debitoRetencao($campos, $where);

      $daoCornump->sql_record($sqlRetencao);

      if ( $daoCornump->numrows <= 0) {
          return;
      }
      $this->geraCancelamento($aDebitos);
    }
