  ...
  function getRegistroDePreco() {
    ...
    new AjaxRequest(
      ...
      function (oRetorno, lErro) {
        ...
        if (oRetorno.pc10_numero) {
          if (confirm(`Verificamos que o processo licitatório usa registro de preço, deseja atualizar a vigência ${js_formatar(oRetorno.pc54_datainicio, 'd')} a ${js_formatar(oRetorno.pc54_datatermino, 'd')} informada durante a inclusão de compilação?`)) {
            location.href = `com4_vigenciaregistropreco001.php?pc54_solicita=${oRetorno.pc10_numero}&redireciona=lic4_adjudicacaohomologacao001.php?situacao=7`; 
          }
        }
      }
    ).execute();
