...
for ($i = 0;$i < 2;$i++){
  ...
  for($ii = 0;$ii < $this->linhasdadospagto;$ii++) {
    ...
    $this->objpdf->setx($xcol+3+$maiscol);
    $this->objpdf->cell(7,3,trim(pg_result($this->recorddadospagto,$ii,$this->receita)),0,0,"R",0);
    $this->objpdf->cell(9,3,"(".trim(pg_result($this->recorddadospagto,$ii,$this->receitared)).")",0,0,"R",0);
    ...
    if ($valor < 0 || $valor == null){
         $this->totaldesc += pg_result($this->recorddadospagto,$ii,$this->valor) + $this->descontoLICBomPagador->j21_valor;
    }else if ($codtipo == 't' and $valor > 0){
         $this->totalacres  += pg_result($this->recorddadospagto,$ii,$this->valor);
    }else{
      if(!$somouLICBomPagador) {
        $this->totalrec  += pg_result($this->recorddadospagto,$ii,$this->valor) + abs($this->descontoLICBomPagador->j21_valor);
      } else {
        $this->totalrec  += pg_result($this->recorddadospagto,$ii,$this->valor);
      }
    }
    if(!$somouLICBomPagador && pg_result($this->recorddadospagto,$ii,"k00_hist") != 918) {
      $valores = pg_result($this->recorddadospagto,$ii,$this->valor) + abs($this->descontoLICBomPagador->j21_valor);
      $somouLICBomPagador = true;
    } else if(!$existeLinhaDesconto && pg_result($this->recorddadospagto,$ii,"k00_hist") == 918) {
      $valores = pg_result($this->recorddadospagto,$ii,$this->valor) + $this->descontoLICBomPagador->j21_valor;

      $existeLinhaDesconto = true;
    } else {
      $valores = pg_result($this->recorddadospagto,$ii,$this->valor);
    }
  ...
  //IF QUE ESCREVE ÚLTIMA LINHA DE DESCONTO CASO NÃO TENHA SIDO REGISTRADO EM OUTRA LINHA
    if(!$existeLinhaDesconto && ($ii+1 == $this->linhasdadospagto)) {
      if($this->descontoLICBomPagador) {
        ...
        $this->objpdf->cell(9,3,"(".trim(pg_result($this->recorddadospagto,0,$this->receitared)).")",0,0,"R",0);
        $this->objpdf->cell(63,3, trim(pg_result($this->recorddadospagto,0,$this->ddreceita)." (desconto)"),0,0,"L",0);
