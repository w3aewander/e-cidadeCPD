		...
		case 'gerarRelatorio' :
			...
			foreach ( $aDadosBanco as $oDadosRegistro ) {
			   ...
			   * inicia contagem de funcioarios por faixa
				...
				if ( !isset($aTotalizadores[$oDadosRegistro->codigo_agrupador ]['iTotalFuncionarios']) ) {
					$aTotalizadores[$oDadosRegistro->codigo_agrupador ]['iTotalFuncionarios']  = 1;
				} else {
					$aTotalizadores[$oDadosRegistro->codigo_agrupador ]['iTotalFuncionarios'] += 1;
				}
      ...
      $lValorDentroFaixa = false;
             ...
             */
						...
						if ( $lValorDentroFaixa ) {
							...
							$aDadosRelatorio[$iCodigoAgrupador]->aFaixas[$iFaixa]['iFuncionarios'] += 1;
				...
				for ( $iFaixa = 0; $iFaixa < $iQuantidadeFaixas; $iFaixa++ ) {
					...
					$nPercentualFunc = ($aDadosRelatorio[$iCodigoAgrupador]->aFaixas[$iFaixa]['iFuncionarios'] * 100 / $aTotalizadores[$iCodigoAgrupador]['iTotalFuncionarios']);
					...
					$aDadosRelatorio[$iCodigoAgrupador]->aFaixas[$iFaixa]['nPercentualFunc'] = round( $nPercentualFunc, 2 );
				}
			}
