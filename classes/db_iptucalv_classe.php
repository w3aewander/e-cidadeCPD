...
class cl_iptucalv
{
    ...
    function sql_queryValoresCalculoIptu($iAnoCalculo)
    {
        ...
        $sSql .= "                                              and iptucalv.j21_anousu = {$iAnoCalculo}          ";
        ...
        $sSql .= "                                 and arrepaga.k00_receit = arrecant.k00_receit) ";
        $sSql .= "                  and arrecant.k00_numpre = iptunump.j20_numpre ";
        $sSql .= "                  and arrecant.k00_receit = tabrec.k02_codigo";
        ...
        $sSql .= "                        WHERE dv13_numpre = iptunump.j20_numpre ";
        $sSql .= "                          AND dv13_receita = tabrec.k02_codigo ";
        $sSql .= "                        GROUP BY dv13_numpre, dv05_vlrhis) AS x ";
        $sSql .= " ";
        $sSql .= "                UNION ALL                           					             ";
        $sSql .= " ";
        $sSql .= "               select sum(k00_valor) as valor                                      ";
        ...
        $sSql .= "                                       from issvar                                 ";
        $sSql .= "                                      where q05_numpre = iptunump.j20_numpre)		 ";
        $sSql .= "                  and k00_receit =  tabrec.k02_codigo                           ";
    ...
    function sql_query_taxa($j21_anousu, $j21_matric, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
            $campos_sql = explode("#", $campos);
        ...
        $sql .= "         iptucalv";
        $sql .= "         where";
        $sql .= "           j21_anousu=$j21_anousu";
        $sql .= "           and j21_matric=$j21_matric";
        $sql .= "       UNION";
        $sql .= "       select";
        $sql .= "       j152_valor as j21_valor";
        $sql .= "       from";
        $sql .= "         iptutaxacalv";
        $sql .= "         inner join iptutaxanump on j151_codigo = j152_iptutaxanump";
        $sql .= "         inner join iptucadtaxaexe on j08_iptucadtaxaexe = j151_iptucadtaxaexe";
        $sql .= "         where";
        $sql .= "           j08_anousu = $j21_anousu";
        $sql .= "           and j151_matric = $j21_matric";
        $sql .= "    ) calv";
        $sql2 = "";
        if ($dbwhere == "") {
        } else {
