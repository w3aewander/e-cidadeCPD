...
class SigfisArquivoEmpenho extends SigfisArquivoBase implements iPadArquivoTXTBase {
    ...
    $sCampos .= " ( select distinct l20_numero || '/' || l20_anousu from liclicita inner join liclicitem on l21_codliclicita = l20_codigo inner join empautitempcprocitem on empautitempcprocitem.e73_pcprocitem = liclicitem.l21_codpcprocitem inner join empautitem on empautitem.e55_autori = empautitempcprocitem.e73_autori and empautitem.e55_sequen = empautitempcprocitem.e73_sequen inner join empempaut on empempaut.e61_autori = empautitem.e55_autori where e61_numemp = empempenho.e60_numemp ) as procadm_licitacao, ";
    $sCampos .= " (  select e150_numeroprocesso from empempaut inner join empautoriza on e61_autori = e54_autori inner join empautorizaprocesso on e150_empautoriza = e54_autori where e61_numemp = empempenho.e60_numemp ) as procadm ";
   ...
   WHERE e61_numemp = empempenho.e60_numemp ) AS processoadministrativo ";
    ...
    if (count($oEmpenhos) > 0) {
      ...
      if (empty($this->sCodigoTribunal)) {
        ...
        if($rsProc == '')
            $rsProcessoadministrativo =  $oEmpenho->processoadministrativo;
        else
            $rsProcessoadministrativo =  '';
...
//    		$oDadosLinha->nu_ProcessoLicitatorio     = str_pad( str_repeat(' ', 36), 36, ' ', STR_PAD_RIGHT );
    	...
    	//	$oDadosLinha->cd_Dispensa                = str_pad($rsProcessoadministrativo,                                 16, ' ', STR_PAD_RIGHT); // ?...
		...
		$oDadosJustificativaEmpenho = getJustificativaByEmpenho( $numemp );
            	$oDadosLinha->TPJUSTIFICATIVACONTRATO = str_pad($oDadosJustificativaEmpenho->tpjustificativacontrato,                                 2, '0', STR_PAD_LEFT);
            	$oDadosLinha->DEJUSTIFICATIVACONTRATO = str_pad($oDadosJustificativaEmpenho->dejustificativacontrato,                                 255, ' ', STR_PAD_RIGHT);
            	$oDadosLinha->TPJUSTIFICATIVALICITA   = str_pad($oDadosJustificativaEmpenho->tpjustificativalicita,                                 2, '0', STR_PAD_LEFT);
            	$oDadosLinha->DEJUSTIFICATIVALICITA   = str_pad($oDadosJustificativaEmpenho->dejustificativalicita,                                 255, ' ', STR_PAD_RIGHT);
