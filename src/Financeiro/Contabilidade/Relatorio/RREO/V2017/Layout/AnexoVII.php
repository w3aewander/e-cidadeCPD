...
class AnexoVII
{
    ...
    public function emitir()
    {
        ...
        $this->oPdf->addHeaderDescription('JANEIRO A ' . $sMesFim . '/' . $this->oAnexo->getAno() . ' - BIMESTRE '
            . $sMesInicio . '-' . $sMesFim);
        $this->oPdf->open();
        $this->oPdf->addPage();
        ...
        $oRelatorio->notaExplicativa(
            $this->oPdf,
            $this->oAnexo->getPeriodo()->getCodigo(),
    ...
    private function imprimeLinha(LinhaAnexoVII $oLinha, $iNivel = 0, $lNegrito = false, $lTotalizar = false)
    {
        ...
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorProcessadoEmExerciciosAnteriores(), 'f'),
            $sBordaFim,
            0,
        ...
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorProcessadoNoExercicioAnterior(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorPagoProcessado(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorCanceladoProcessado(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            19,
            self::ALTURA,
            db_formatar($oLinha->getSaldoProcessado(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorNaoProcessadoEmExerciciosAnteriores(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorNaoProcessadoNoExercicioAnterior(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorLiquidadoNaoProcessado(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorPagoNaoProcessado(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getValorCanceladoNaoProcessado(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getSaldoNaoProcessado(), 'f'),
            $sBordaFim,
            0,
            \PDFDocument::ALIGN_RIGHT
        );
        $this->oPdf->cell(
            18,
            self::ALTURA,
            db_formatar($oLinha->getSaldoTotal(), 'f'),
            $sBordaFim,
            1,
            \PDFDocument::ALIGN_RIGHT
        );
    ...
    {
        if ($this->oAnexo->getAno() >= 2020) {
            $this->cabecalho2020();
        }
        if ($this->oAnexo->getAno() <= 2019) {
            $this->cabecalho2017();
        }
    }

    private function cabecalho2017()
    {
        ...
        $label = 'RESTOS A PAGAR PROCESSADOS E NÃO PROCESSADOS LIQUIDADOS EM EXERCÍCIOS ANTERIORES';
        $this->oPdf->cell(91, self::ALTURA, $label, 1, 0, \PDFDocument::ALIGN_CENTER);
    ...
    /**
     * Cabeçalho do ano de 2020
     */
    private function cabecalho2020()
    {
        $iAltura = $this->oPdf->getY();
        $iWidthPoder = 60;
        $this->oPdf->cell($iWidthPoder, 20, 'PODER / ÓRGÃO', "TRB", 0, \PDFDocument::ALIGN_CENTER);
        $this->oPdf->setXY(($iWidthPoder + 10), $iAltura);
        $label = 'RESTOS A PAGAR PROCESSADOS';
        $this->oPdf->cell(91, self::ALTURA, $label, 1, 0, \PDFDocument::ALIGN_CENTER);
        $this->oPdf->cell(126, self::ALTURA, 'RESTOS A PAGAR NÃO PROCESSADOS', 'TLB', 1, \PDFDocument::ALIGN_CENTER);
        $this->oPdf->setX($iWidthPoder + 10);
        $this->oPdf->cell(36, self::ALTURA, 'Inscritos', 1, 0, \PDFDocument::ALIGN_CENTER);
        ...
        $this->oPdf->setX($iWidthPoder + 10);
        $iAlturaCelula = $this->oPdf->getY();
        $nHeight = $this->oPdf->getMultiCellHeight(18, self::ALTURA - 2, "Em Exercícios Anteriores");
        $this->oPdf->MultiCell(18, $nHeight, "Em Exercícios Anteriores\n(a)", 1, 'C');

        $this->oPdf->setXY($iWidthPoder + 28, $iAlturaCelula);
        $sDescricao = "Em 31 de Dezembro de " . ($this->oAnexo->getAno() - 1);
        $nHeight = $this->oPdf->getMultiCellHeight(18, 2, $sDescricao);
        $this->oPdf->MultiCell(18, $nHeight, "{$sDescricao}\n(b)", 1, 'C');

        $this->oPdf->setXY($iWidthPoder + 46, $iAlturaCelula);
        $this->oPdf->setAutoNewLineMulticell(false);
        $this->oPdf->MultiCell(18, 6, "Pagos\n(c)", 'LRB', \PDFDocument::ALIGN_CENTER);
        $this->oPdf->MultiCell(18, 6, "Cancelados\n(d)", 'LRB', \PDFDocument::ALIGN_CENTER);
        $this->oPdf->MultiCell(19, 6, "Saldo\ne = (a+b) - (c+d)", 'LRB', \PDFDocument::ALIGN_CENTER);
        ...
        $this->oPdf->setXY($iWidthPoder + 137, $iAlturaCelula);
        $this->oPdf->setAutoNewLineMulticell(false);
        $this->oPdf->MultiCell(18, 6, "Liquidados\n(h)", "LRB", \PDFDocument::ALIGN_CENTER);
        $this->oPdf->MultiCell(18, 6, "Pagos\n(i)", "LRB", \PDFDocument::ALIGN_CENTER);
        $this->oPdf->MultiCell(18, 6, "Cancelados\n(j)", "LRB", \PDFDocument::ALIGN_CENTER);
        $this->oPdf->MultiCell(18, 6, "Saldo\nk = (f+g) - (i+j)", "BLR", \PDFDocument::ALIGN_CENTER);
        $this->oPdf->setAutoNewLineMulticell(true);
        $this->oPdf->SetXY($iWidthPoder + 209, $iAlturaCelula);
        $this->oPdf->MultiCell(18, 6, "Saldo Total\nl = (e) + (k)", "BL", \PDFDocument::ALIGN_CENTER);
    }
}
