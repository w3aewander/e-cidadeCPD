...
class cl_calendario {
   ...
   public function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->ed52_t_obs = ($this->ed52_t_obs == ""?@$GLOBALS["HTTP_POST_VARS"]["ed52_t_obs"]:$this->ed52_t_obs);
     }else{
       $this->ed52_i_codigo = ($this->ed52_i_codigo == ""?@$GLOBALS["HTTP_POST_VARS"]["ed52_i_codigo"]:$this->ed52_i_codigo);
     }
   }
   ...
   public function incluir ($ed52_i_codigo){
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,1010083,1008495,'','".AddSlashes(pg_result($resaco,0,'ed52_t_obs'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
    }
     return true;
   }
   ...
   public function alterar ($ed52_i_codigo=null) {
     ...
     if(trim($this->ed52_c_passivo) != null){
      $sql  .= $virgula." ed52_t_obs = '$this->ed52_t_obs' ";
      $virgula = ",";
    }
     $sql .= " where ";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if (isset($GLOBALS["HTTP_POST_VARS"]["ed52_t_obs"]) || $this->ed52_t_obs != "")
           $resac = db_query("insert into db_acount values($acount,1010083,1008495,'".AddSlashes(pg_result($resaco,$conresaco,'ed52_t_obs'))."','$this->ed52_t_obs',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){
   ...
   public function excluir ($ed52_i_codigo=null,$dbwhere=null) {
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac  = db_query("insert into db_acount values($acount,1010083,1008495,'','".AddSlashes(pg_result($resaco,$iresaco,'ed52_t_obs'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
