    ...
    <form name="form1" method="post" action="">
        <fieldset>
            ...
            <table border="0">
                ...
                <tr>
                    <td nowrap title="<?= @$Tpc12_tipo ?>" <?=isset($pc30_seltipo) && $pc30_seltipo == "t" ? '' : 'hidden'?>>
                        <b>Tipo de Compra:</b>
                    </td>
                    <td colspan="3" <?=isset($pc30_seltipo) && $pc30_seltipo == "t" ? '' : 'hidden'?>>
                        <?php
                        $parampesquisa = true;
                        if (isset($pc30_seltipo) && $pc30_seltipo == "t" && (!isset($pc12_tipo) || $pc12_tipo == '')) {
                            $sql = $clpcparam->sql_query_file(db_getsession("DB_instit"), "pc30_tipcom as pc12_tipo");
                            $rs = $clpcparam->sql_record($sql);
                        ...
                        $sqlParam = $clpcparam->sql_record($clpcparam->sql_query_file(db_getsession("DB_instit"), "pc30_tipcom"));
                        $paramSolicita = \db_utils::fieldsMemory($sqlParam, 0)->pc30_tipcom;
                        $where = "pc50_ativo = True";
                        $result_tipocompra = $clpctipocompra->sql_record(
                            $clpctipocompra->sql_query_file(
                                null,
                                "pc50_codcom as pc12_tipo,pc50_descr",
                                'pc50_codcom',
                                $where
                            )
                        );
                ...
                if (isset($param) && trim($param) != "") {
                    ...
                    if ($flag_liclicita == false) {
                        ...
                        <tr>
                            ...
                            <td nowrap colspan="3">
                                <?php
                                db_input('codproc', 10, $Ipc80_codproc, true, "text", $pc,
                                    "OnChange='js_pesquisapc80_codproc(false);'");
                                ?>
                            </td>
                        </tr>
                        <?php
                ...
                if (isset($aParametrosOrcamento[0]->o50_utilizapacto) && $aParametrosOrcamento[0]->o50_utilizapacto == "t") {
                    ...
                    <tr id='planopacto' style='display: none'>
                        ...
                        <td colspan="3">
                            <?php
                            db_input('o74_sequencial', 10, $Io74_sequencial, true, 'text', $db_opcao,
                                " onchange='js_pesquisapactoplano(false);'");
                ...
                <td colspan="3">
                    ...
                    ?>
                    <input type="hidden" id="tipoCompraRegistroPreco" >
                    <input type="hidden" id="tipoCompraAnterior" >
                </td>
                </tr>
                <tr>
        ...
        <div class="container">
            ...
            <input name="pesquisar" type="button" id="pesquisar" value="Pesquisar"
                   onclick="js_pesquisa();" <?= ($parampesquisa == false ? "disabled" : "") ?> >
            ...
            if (isset($pc10_numero) || isset($chavepesquisa)) {
                ...
                $result_itens = $clsolicitem->sql_record($clsolicitem->sql_query_file(null, "pc11_codigo", "",
...
<script>
    ...
    function js_importa() {
        ...
        js_OpenJanelaIframe('CurrentWindow.corpo.iframe_solicita',
            'db_iframe_solicita',
            'func_solicita.php?funcao_js=parent.js_preencheimporta|pc10_numero|pc52_sequencial&nada=true&param_mostra_regprecos=nao&tiposolicitacao=1',
            'Pesquisa', true, '0');
    }
    ...
    function js_pesquisa() {
        ...
        ?>
        js_OpenJanelaIframe(
            'CurrentWindow.corpo.iframe_solicita',
            'db_iframe_solicita',
            'func_solicitaalt.php?<?=$db_proc?>'
            + '&funcao_js=parent.js_preenchepesquisa|pc10_numero'
            +'&departamento=<?=db_getsession("DB_coddepto")?><?=$parametro?>',
            'Pesquisa',
            true,
            '0'
        );
    ...
    function js_pesquisarcompilacao(lMostrarRegistro) {
        ...
        if (lMostrarRegistro == true) {
            ...
            js_OpenJanelaIframe('',
                ...
                'func_solicitacompilacao.php?funcao_js=parent.js_mostracompilacao|pc10_numero|dl_codigo' +
    ...
    function js_mostracompilacao(iSolicitacao, codigo) {
        if ($('pc12_tipo')) {
            $('pc12_tipo').value = codigo;
            $('pc12_tipo').setAttribute('readonly', '');
            $('pc12_tipo').setAttribute('tabindex', '-1');
            $('pc12_tipodescr').setAttribute('readonly', '');
            $('pc12_tipodescr').setAttribute('tabindex', '-1');
            $('pc12_tipo').dispatchEvent(new Event('change'));
        }
        $('pc54_solicita').value = iSolicitacao;
        db_iframe_estimativaregistropreco.hide();
        $("tipoCompraRegistroPreco").value = codigo;
    }

    function getDbOpcao() {
        /**
         * Caso for solicitação registro de preço e parametro ativo
         */
        if (($F('pc10_solicitacaotipo') == 5) && $('pc12_tipo')) {
            $('pc12_tipo').setAttribute('readonly', '');
            $('pc12_tipo').setAttribute('tabindex', '-1');
            $('pc12_tipodescr').setAttribute('readonly', '');
            $('pc12_tipodescr').setAttribute('tabindex', '-1');
            $('tipoCompraAnterior').setValue($F('pc12_tipo'));
            $('pc12_tipo').setValue($F("tipoCompraRegistroPreco"));
            $('pc12_tipo').dispatchEvent(new Event('change'));
            console.log(2);
        }

        /**
         * Caso for solicitação normal e parametro ativo
         */

        if (($F('pc10_solicitacaotipo') == 1) && $('pc12_tipo')) {
            $('pc12_tipo').setValue($F('tipoCompraAnterior'));
            $("pc12_tipo").dispatchEvent(new Event("change"));
            $("pc12_tipo").removeAttribute("readonly");
            $("pc12_tipodescr").removeAttribute("readonly");
        }
    }

    js_showCamposRegistro();
    getDbOpcao();
    document.getElementById("pc90_numeroprocesso").maxLength = 30;
</script>
