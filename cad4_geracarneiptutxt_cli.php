...
$DB_SERVIDOR               = "dev17";
$DB_BASE                   = "t66020_araruama_2012116_v2_3_2_v1";
$DB_PORTA                  = "5433";
$DB_USUARIO                = "dbportal";
$DB_SENHA                  = "";
...
$HTTP_SESSION_VARS['DB_uol_hora'         ] =  '1353668279';
...
$HTTP_SESSION_VARS['DB_nome_modulo'      ] =  'Cadastro';
$HTTP_SESSION_VARS['DB_anousu'           ] =  '2013';
$HTTP_SESSION_VARS['DB_datausu'          ] =  '1357048800';
...
$HTTP_SERVER_VARS['SCRIPT_FILENAME']   ='/var/www/dbportal_prj/cad4_geracarneiptutxt.php';
$HTTP_SERVER_VARS['PHP_SELF']          ='/dbportal_prj/cad4_geracarneiptutxt.php';
...
// setando variaveis INI

$iTamanho                  = 50000;
$iInicio                   = 0;

$sLimitacao                = is_null($iTamanho) ?  "" : " limit{$iTamanho} offset {$iInicio} ";
...
$quantidadeparcelas        = '';
$listamatrics              = '';
$unica                     = '2013-01-23=2013-01-01=20U2013-02-23=2013-01-01=10';
$anousu                    = '2013';
$quantidade                = '';
$quantidade_registros_real = '';
$processarmovimentacao     = '';
$mensagemanosanteriores    = 'n';
$ordem                     = 'endereco';
$especie                   = 'todos';
$imobiliaria               = 'todos';
$loteamento                = 'todos';
...
db_putsession('REQUEST_URI'         , '');
db_putsession('DB_configuracao_ok'  , '');
db_putsession('DB_acessado'         , '1325613');
db_putsession('DB_login'            , 'dbseller');
db_putsession('DB_id_usuario'       , '1');
db_putsession('DB_administrador'    , '1');
db_putsession('DB_ip'               , '127.0.0.1');
...
db_putsession('DB_uol_hora'         , '1353668279');
db_putsession('DB_instit'           , '1');
db_putsession('DB_totalmodulos'     , '55');
db_putsession('DB_use_pcasp'        , 'f');
db_putsession('DB_Area'             , '1');
db_putsession('DB_modulo'           , '578');
db_putsession('DB_nome_modulo'      , 'Cadastro');
db_putsession('DB_anousu'           , '2013');
db_putsession('DB_datausu'          , '1357048800');
db_putsession('DB_coddepto'         , '1');
db_putsession('DB_nomedepto'        , 'COINF');
db_putsession('DB_itemmenu_acessado', '1576');

$HTTP_SERVER_VARS['DB_id_usuario']    = '1';
$HTTP_SERVER_VARS['DB_administrador'] = '1';
...
$temporizador           = date("His", mktime(date("H"), date("i"), date("s"), 0, 0, 0));
  ...
  /*
 * fun√ß√£o que retorna ultima sexta do mes
...
$sqlprinc .= " order by {$sOrder} {$sLimitacao}";
    ...
    for ($i = 0; $i < $quantidade; $i ++) {
      ...
      if ($debugar_passou == 1) {
        echo $j23_matric . "<br>";
      	...
      	$nProgresso = round( ( ( ($i+1) * 100 ) / $quantidade ) ,2) . " % ";
      	echo "Processando registro {$i} de {$quantidade} Progresso : {$nProgresso} \r";
  ...
  if(isset($tx_banc) && $tx_banc != ""){
    ...
    $taxa_bancaria = 0;
        ...
        if ($cliptunump->numrows > 0) {
          ...
          if ($gerar == "dados") {
            ...
            if ($tipo == "txt") {
              ...
              fputs($clabre_arquivo->arquivo, db_contador("EXERCICIO","EXERC√çCIO DO CALCULO",$contador, 4));
          ...
          if ( $db21_codcli == 19985 ) {
            $mensagemdebitosanosanteriores = "ParabÈns! Contribuinte que paga os tributos colabora para a construÁ„o da Nova Maric·.";
          }
          ...
          if ($mensagemanosanteriores == "s") {
            ...
            if (pg_numrows($result_debitos) > 0) {
              ...
              if ($fc_tipocertidao == "positiva") {
                $mensagemdebitosanosanteriores = "EXISTEM D…BITOS EM ABERTO PARA ESTA MATRÕCULA AT… A DATA " . db_formatar(date("Y-m-d",db_getsession("DB_datausu")),'d');
              }
            }
          }
        ...
        /* PARA PADRAO COBRANCA */
          ...
          // Demais Conv√™nios ARRECADA√á√ÉO, CAIXA PADR√ÉO etc.
          } else {
          ...
          if ( $gerar == "dados" ) {
            ...
            if ($tipo == "txt") {
              for ( $iParcela=0; $iParcela <= $iParcelaMaxima; $iParcela++ ) {
...
########################################################################################################################
// Melhoria na emissao para gerar parcelas e unicas opcionais de vencimentos, aplicando corre√ß√£o
...
########################################################################################################################
      ...
      if ($gerar == "layout" and $quantos >= 1) {
        if ($tipo == "txt") {
          ...
          fputs($clabre_arquivo->arquivo, "ORDEM: " . ($ordem == "endereco"?"Endereco de entrega":($ordem == "alfabetica"?"AlfabÈtica":"Zona de entrega")) . "{$sQuebraLinha}");
...
echo "<br><br><b>".date("H:i:s", mktime(date("H"), date("i"), date("s"), 0, 0, 0))."</b><br>";
echo "<b>Diferen√ßa: ".((((int)date("His", mktime(date("H"), date("i"), date("s"), 0, 0, 0)) - (int)$temporizador)) / 60)."</b>";
