...
(function(exports) {
  ...
  DBViewResposta.makeFromObject = function(object) {
    ...
    resposta.set("identificador", object.pergunta.identificador_campo);
    resposta.set("identificador_campo", object.identificador_campo);
    resposta.set("peso", object.peso);
    resposta.set("valor", object.valor);
    resposta.set("data", object);
    if (object.options) {
  ...
  DBViewResposta.prototype = {
    ...
    'setElementoAuxiliar' : function(element) {
      ...
      if (this.pergunta.tipo === '10') {

          this.elementoAuxiliar =  document.createElement('div');
          this.elementoAuxiliar.id          = this.id+"_auxiliar_div";


          this.elementoAuxiliar.innerHTML     = "";

          var cols = eval(this.data.valorresposta);
          var tabela = document.createElement('table');
          tabela.className = 'datagrid';
          tabela.style.width = '100%';
          var linha = document.createElement('tr');
          for (var coluna of cols) {
              var colunaTD  = document.createElement('td');
              colunaTD.className = 'table_header';
              colunaTD.innerHTML = coluna.label;

              linha.appendChild(colunaTD);
          }
          tabela.appendChild(linha);
          this.elementoAuxiliar.appendChild(tabela);
          if (this.valor !== '') {

            var respostas = eval(this.valor);
            for (resposta of respostas) {
                var linha = document.createElement('tr');
                for (var coluna of cols) {
                    var colunaTD = document.createElement('td');
                    colunaTD.className = 'linhagrid';
                    var textoResposta = resposta[coluna.campo];
                    colunaTD.innerHTML = textoResposta;
    ...
    'getDados' : function () {
      ...
      resposta = {
        ...
        identificador_opcao : this.identificador_campo,
    ...
    'aplicarAtributos' : function() {
      ...
      this.elemento.identificador = this.identificador;
      this.elemento.setAttribute("identificador", this.identificador);
      this.elemento.setAttribute("identificador_campo", this.identificador_campo);

      this.elemento.id = this.id;
      if(this.elemento.type == 'radio') {
        this.elemento.setAttribute("codigo", this.codigo);
      ...
      if(this.pergunta.somenteLeitura === true) {
        this.elemento.setAttribute('disabled', 'disabled');
        this.elemento.addClassName('readonly');
      }
    },
    ...
    'renderizarElemento' : function(tipo) {
      ...
      switch (tipo + '') {
        ...
        case "5":// 5=>'Data'
          ...
          if(this.pergunta.somenteLeitura === true) {
            this.elemento.setReadOnly(true);
          }
