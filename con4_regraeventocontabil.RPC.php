...
switch ($oParam->exec) {
    ...
    case "getTipoBem" :
        ...
        if ($oDaoBemTipos->numrows > 0) {
            ...
            $oValores->t24_sequencial = "";
            $oValores->t24_descricao = "Selecione...";
            $aTipoBem[0] = $oValores;

            if (isset($oParam->iAlteracao) && $oParam->iAlteracao != 'novo') {

                $oDaoAlteracao = new cl_bemtipos;
            ...
            for ($iIndice = 0; $iIndice < $oDaoBemTipos->numrows; $iIndice++) {
    ...
    case "getTipoAquisicao" :
        ...
        if ($oDaoBensTipoAquisicao->numrows > 0) {
            ...
            if (isset($oParam->iAlteracao) && $oParam->iAlteracao != 'novo') {
                ...
                $oDaoAlteracao = new cl_benstipoaquisicao;
                $SqlAlteracao = $oDaoAlteracao->sql_query_file($oParam->iAlteracao);
                $rsAlteracao = $oDaoAlteracao->sql_record($SqlAlteracao);
    ...
    case "getTipoBaixa" :
        ...
        if ($oDaoBensTipoBaixa->numrows > 0) {
            ...
            if (isset($oParam->iAlteracao) && $oParam->iAlteracao != 'novo') {
                ...
                $oDaoAlteracao = new cl_bensmotbaixa;
                $SqlAlteracao = $oDaoAlteracao->sql_query_file($oParam->iAlteracao);
                $rsAlteracao = $oDaoAlteracao->sql_record($SqlAlteracao);
    ...
    case "getTiposEventoEmpenho" :
        ...
        if ($oDaoEmpPrestaTip->numrows > 0) {
            ...
            if (isset($oParam->iAlteracao) && $oParam->iAlteracao != 'novo') {
                ...
                $oDaoAlteracao = new cl_empprestatip;
                $SqlAlteracao = $oDaoAlteracao->sql_query_file($oParam->iAlteracao);
                $rsAlteracao = $oDaoAlteracao->sql_record($SqlAlteracao);
            ...
            if (!empty($oParam->sElemento) && in_array($oParam->c47_compara, array(1, 2, 3, 4))) {
                ...
                if (!empty($oParam->sElemento) && in_array($oParam->c47_compara, array(
                    ...
                    foreach ($aRegrasCadastradas as $oRegra) {
                        ...
                        $comparaRetencao = (int)$oParam->c47_compara === RegraLancamentoContabil::COMPARA_TIPO_RETENCAO;
                        $comparaTipoRetencao = (int)$oRegra->getReferencia() === (int)$oParam->c47_ref;
                        $elementoIgual = $oRegra->getElemento() == str_pad($oParam->sElemento, 15, "0", STR_PAD_RIGHT);
                        if ($comparaRetencao && $comparaTipoRetencao && $elementoIgual) {
                            throw new Exception("Tipo de retenção já cadastrada com esse elemento.");
                        }

                        $anoIgual = $oParam->c47_anousu == $oRegra->getAnoUso();
                        $regraDiferente = $oRegra->getSequencialRegra() != $oRegraLancamentoContabil->getSequencialRegra();
                        if ($elementoIgual && $anoIgual && $regraDiferente && !$comparaRetencao) {
                ...
                if (empty($oParam->sElemento)) {
                    $oRegraLancamentoContabil->vincularElemento(str_repeat('0', 15));
                }
            }
