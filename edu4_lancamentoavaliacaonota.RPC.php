...
try {
  ...
  switch($oParam->exec) {
      ...
      if (isset($oParam->iTurma) && isset($oParam->iEtapa) && !empty($oParam->iTurma) && !empty($oParam->iEtapa)) {
        ...
        foreach ($oProcedimentoAvaliacao->getElementos() as $oAvaliacao) {
         ...
         if (count($aPeriodosAvaliacaoRetorno) == 0) {
           ...
           $sMsgErro  = "Nao foi possivel buscar os periodos de avaliacao da turma.\n";
           $sMsgErro .= "Acessar Cadastro > Procedimentos de Avaliacao > Alteracao e verificar a configuracao do ";
    ...
    /**
     * Retorna todos os alunos de uma turma e as avaliacoes obtidas em uma regencia
     * Apos processamento serializa a turma na sessao do PHP
    ...
    case 'getAlunosAvaliacaoRegencia' :
      ...
      if (!isset($oParam->iTurma) || !isset($oParam->iEtapa)) {
        throw new BusinessException('Turma ou regencia nao informada.');
      }
      ...
      foreach ($aMatriculas as $oMatricula) {
        ...
        */
        if ($oMatricula->getAluno()->getNomeSocial() != '') {
          $oDadosAluno->sNomeAluno     = urlencode($oMatricula->getAluno()->getNomeSocial());
        } else {
        ...
        /**
         * Buscamos as avaliacoes da regencia selecionada
         */
        $oRegencia = RegenciaRepository::getRegenciaByCodigo($oParam->iRegencia);
        ...
        // $oDadosAproveitamento ? o DiarioAvaliacaoDisciplina
        ...
        foreach ($oDadosAproveitamento->getAvaliacoes() as $oAvaliacao) {
          ...
          /**
           * Esta validacao eh para quando um aluno especifico eh avaliado por parecer
           */
          ...
          /**
           * Proporcionalidade, validacoes para bloqueio dos campos
           * - quando turma possui proporcionalidade e aluno tem proporcionalidade configurada
           * - valida bloqueio dos periodos que nao fazem parte do calculo do resultado final
           */
          $oDadosAvaliacao->lAlunoComProporcionalidade = false;
          $oDadosAvaliacao->lBloqueiaPeriodo           = false; // inicializa como false para nao quebrar outros procedimentos
          ...
          if ( $lTurmaUtilizaProporciolalidade && count($aOrdemPeriodoAplicaProporcionalidade) > 0 ) {
            ...
            /**
             * So bloqueia o periodo se o periodo de avaliacao, faz parte do calculo de soma
             */
            if ( !in_array($oDadosAvaliacao->iPeriodo, $aOrdemPeriodoAplicaProporcionalidade) &&
              ...
              $oDadosAvaliacao->lBloqueiaPeriodo = true; // caso nao esteja configurado para aplicar proporcionalidade deve bloquear
            }
          }

          /**
           * Identifica que existe uma avaliacao alternativa configurada para o periodo e que este nao pode receber avaliacao
           */
          ...
          if( $oAvaliacao->getElementoAvaliacao() instanceof AvaliacaoPeriodica ) {
            ...
            /**
             * Em caso de Forma de Obtencao igual a Aprovacao por Periodo nao exibe a nota final
             */
          ...
          /**
           * Quando possuir avaliacao externa, buscamos a origem para verificar se ja foi migrado
           */
          ...
          /**
           * Quando nao for Resultado Final
           */
          if (!$oAvaliacao->getElementoAvaliacao()->isResultado()) {
            ...
            if (trim($oDadosAvaliacao->nNota) != '' && $lBloqueiaAlteracaoAvaliacao && $lProfessorLogado &&
          ...
          if ($oAvaliacao->getElementoAvaliacao()->isResultado() &&
            ...
            /**
             * Adicionado validacao para so calcular nota parcial se resultado nao possui avaliacao lancada/calculada
             */
          ...
          if (!empty($oResultadoFinalRegencia)) {
            ...
            if ($oAprovadoConselho != null) {
              ...
              /**
               * Reclassificacao por baixa frequencia nao aprova o aluno, pois o mesmo pode estar com o aproveitamento
               * abaixo da media, e a reclassificacao aprova apenas no quesito frequencia
               */
          ...
          /**
           * Pega os elementos de recuperacao
           */
          ...
          /**
           * Pega os elementos que geraram uma recuperacao
           */
        ...
        /**
         * Percorre os elementos de recuperacao, para validar se o mesmo nao encontra-se amparado e se o elemento
         * vinculado a ele, eh o mesmo elemento que gerou uma recuperacao
         */
        ...
        foreach ($oRetorno->aAlunos as $oAluno) {
            // Verifica se os elementos do array aAproveitamentos estao ordenados por iPeriodo
            $aproveitamentosOrdenados = true;
            $periodoAnterior = null;
            foreach ($oAluno->oDisciplina->aAproveitamentos as $aproveitamento) {
                if ($periodoAnterior !== null && $aproveitamento->iPeriodo < $periodoAnterior) {
                    $aproveitamentosOrdenados = false;
                    break; // Interrompe o loop se encontrar um elemento fora de ordem
                }
                $periodoAnterior = $aproveitamento->iPeriodo;
            }

            // Se os elementos nao estiverem ordenados, ordena o array com base em iPeriodo
            if (!$aproveitamentosOrdenados) {
                usort($oAluno->oDisciplina->aAproveitamentos, function($a, $b) {
                    if ($a->iPeriodo == $b->iPeriodo) {
                        return 0;
                    }
                    return ($a->iPeriodo < $b->iPeriodo) ? -1 : 1;
                });
            }
        }
    ...
    case 'salvaAvaliacaoAluno':
      ...
      if(isset($oParam->gambiarra) && $oParam->gambiarra == "sim"){
        $oTurma = EducacaoSessionManager::carregarTurma($oParam->iTurma);
        $oEtapa = EducacaoSessionManager::carregarEtapa($oParam->iEtapa);
      }else{
        $oTurma = EducacaoSessionManager::carregarTurma();
      ...
      foreach ($oTurma->getAlunosMatriculadosNaTurmaPorSerie($oEtapa) as $oMatricula) {
         ...
         * Data: 22/01/2026
         * Demanda: 18059
         * Razao: Removido codigo que passava parametro $encerrar para salvaAvaliacaoAluno().
         *        O metodo LancamentoAvaliacaoAluno::salvaAvaliacaoAluno() aceita apenas 1 parametro (Matricula).
         *        O codigo anterior causava erro 500 por incompatibilidade de assinatura.
         *        Alem disso, a funcionalidade de "encerrar" e um processo separado (EncerramentoAvaliacao),
         *        nao devendo ser misturada com o salvamento de avaliacoes em andamento.
         */
      ...
      unset($_SESSION["DB_desativar_account"]);
      $oRetorno->status   = 1;
      $oRetorno->message  = urlencode("Dados de avaliacao salvos com sucesso.");
      break;
...
/**
 * Retorna a ordem dos periodos que compoem o Resultado
