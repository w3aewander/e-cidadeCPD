...
$clrotulo->label("pa01_autodvenc");
$clrotulo->label("pa01_autodprazo");
$clrotulo->label("instit");
  ...
  <fieldset>
    ...
    <table>
      ...
      <tr>
	    ...
	    <tr>
        <td><b>Verifica tipo:</b></td>
        <td>
          <?php
            $arrayVerifica = array('t' => 'Sim', 'f' => 'Não');
            db_input('sequencial', 10, $sequencial, true, 'hidden',$db_opcao);
            db_select('verificatipo',$arrayVerifica,true,$db_opcao);
          ?>
        </td>
      </tr>
      <tr>
    ...
    <fieldset>
      ...
      <tr>
        <td><strong>Quantidade de dias para vencimento:</strong>
        </td>
        <td align="left">
          <?php
            db_input('pa01_autodvenc',5,$Ipa01_autodvenc,true,'text',$db_opcao,'');
          ?>
        <td>
       </tr>

       <tr>
        <td><strong>Quantidade de dias para recurso:</strong>
        </td>
        <td align="left">
          <?php
            db_input('pa01_autodprazo',5,$Ipa01_autodprazo,true,'text',$db_opcao,'');
          ?>
        <td>
       </tr>
      </table>
    ...
    <!-- COMEÇO NOTIFICAÇÂO DE LANÇAMENTO -->
    <fieldset>
      ...
      <table>
        <tr>
          <td nowrap title="<?php echo @$Ty32_tipo; ?>">
            <?php
              db_ancora(@$Ly32_tipo, "js_pesquisanl27_tipo(true);", $db_opcao);
            ?>
          </td>
          <td>
            <?php
              db_input('nl27_instit',50,$Iinstit,true,'hidden',3,'');
              db_input('nl27_tipo', 10, $Inl27_tipo, true, 'text', $db_opcao, " onchange='js_pesquisanl27_tipo(false);'");
              db_input('nl27_tipo_descr',50,$Inl27_tipo_descr,true,'text',3,'');
            ?>
          </td>
        </tr>
        ...
        <tr>
          <td nowrap title="<?php echo @$Ty32_hist; ?>">
            <?php
              db_ancora(@$Ly32_hist,"js_pesquisanl27_hist(true);",$db_opcao);
            ?>
          </td>
          <td>
            <?php
              db_input('nl27_historico',10,$Inl27_historico,true,'text',$db_opcao," onchange='js_pesquisanl27_hist(false);'");
              db_input('nl27_hist_descr',50,$Inl27_hist_descr,true,'text',3,'');
            ?>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <?php
              $aProcProtBaixaAuto = array('t'=>'Sim','f'=>'Não');
              db_select('nl27_procbaixaauto',$aProcProtBaixaAuto,true,$db_opcao,"");
            ?>
          </td>
        </tr>
       ...
       <tr>
        ...
        <td align="left">
          <?php
            ...
            $aOpcoes2 = array( "t" => "Sim", "f" =>"Não");

            db_select("nl27_utilizadocpadrao",$aOpcoes2,true,$db_opcao, "onchange='js_templateAutodeInfracao2(this.value);'");
          ?>
        <td>
       </tr>

         <tr id="linhaTemplate2" style="display:none;">
           <td nowrap="nowrap" title="<?=@$Tp90_db_documentotemplate?>">
             <?php
               db_ancora("<strong>Documento Template:</strong>","js_pesquisaDocumentoAutodeInfracao2(true);",$db_opcao);
             ?>
           </td>
           <td nowrap="nowrap">
             <?php
               db_input('nl27_templateautoinfracao',10,@$Inl27_templateautoinfracao,true,'text',$db_opcao,'onchange="js_pesquisaDocumentoAutodeInfracao2(false);"');
               db_input('nl27_descricaoautodeinfracao',50,$Idb82_descricao,true,'text',3,'','nl27_descricaoautodeinfracao');
             ?>
           </td>
        </tr>
        ...
        <tr>
          ...
          <td align="left">
            <?php
              db_input('nl27_autodvenc',5,$Inl27_autodvenc,true,'text',$db_opcao,'');
            ?>
          <td>
         </tr>
       ...
       <tr>
        ...
        <td align="left">
          <?php
            db_input('nl27_autodprazo',5,$Inl27_autodprazo,true,'text',$db_opcao,'');
          ?>
        <td>
       </tr>
      </table>
    </fieldset>
    <!-- FIM NOTIFICAÇÂO DE LANÇAMENTO -->
    ...
    <fieldset>
      ...
      <table>
        ...
        <tr>
          <td>
            <label><strong>Calcula Vistoria de Alvará sanitário provisório:</strong></label>
          </td>
          <td>
            <?php
              $aOpcoes = array('f'=>'Não', 't'=>'Sim');
              db_select('y32_calcprovisorio',$aOpcoes,true,$db_opcao,"");
            ?>
          </td>
        </tr>
...
<script type="text/javascript">
   ...
   * Template Auto de Infrção (hide/show)
   */
  function js_verificaTemplateAuto2() {

    if( !empty($F('nl27_templateautoinfracao')) ){
      $('nl27_utilizadocpadrao').options[0].selected = true;
    }

    var oTrAutodeInfracao = $('nl27_templateautoinfracao').up('tr');
    oTrAutodeInfracao.hide();
    if ($F('nl27_utilizadocpadrao') == 'f') {
      oTrAutodeInfracao.show();
    }else{
      oTrAutodeInfracao.hide();
    }
  ...
  function js_verificaTemplateAuto() {
    ...
    if( !empty($F('y32_templateautoinfracao')) ){
      $('utilizadocpadrao').options[0].selected = true;
    }
    ...
    if ($F('utilizadocpadrao') == '0') {
      oTrAutodeInfracao.show();
    }
  }


    function js_pesquisanl27_tipo(mostra){
    if(mostra==true){
      js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_arretipo','func_arretipo.php?funcao_js=parent.js_mostraarretipo3|k00_tipo|k00_descr','Pesquisa',true);
    }else{
       if(document.form1.nl27_tipo.value != ''){
          js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_arretipo','func_arretipo.php?pesquisa_chave='+document.form1.nl27_tipo.value+'&funcao_js=parent.js_mostraarretipo2','Pesquisa',false);
       }else{
         document.form1.nl27_tipo_descr.value = '';
       }
    }
  }
  function js_mostraarretipo2(chave,erro){
    document.form1.nl27_tipo_descr.value = chave;
    if(erro==true){
      document.form1.nl27_tipo.focus();
      document.form1.nl27_tipo.value = '';
    }
  }
  function js_mostraarretipo3(chave1,chave2){
    document.form1.nl27_tipo.value = chave1;
    document.form1.nl27_tipo_descr.value = chave2;
    db_iframe_arretipo.hide();
  }

  function js_pesquisanl27_hist(mostra){
    if(mostra==true){
      js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_histcalc','func_histcalc.php?funcao_js=parent.js_mostrahistcalc3|k01_codigo|k01_descr','Pesquisa',true);
    }else{
       if(document.form1.y32_hist.value != ''){
          js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_histcalc','func_histcalc.php?pesquisa_chave='+document.form1.y32_hist.value+'&funcao_js=parent.js_mostrahistcalc2','Pesquisa',false);
       }else{
         document.form1.k01_descr.value = '';
       }
    }
  }
  function js_mostrahistcalc2(chave,erro){
    document.form1.nl27_hist_descr.value = chave;
    if(erro==true){
      document.form1.nl27_historico.focus();
      document.form1.nl27_historico.value = '';
    }
  }
  function js_mostrahistcalc3(chave1,chave2){
    document.form1.nl27_historico.value = chave1;
    document.form1.nl27_hist_descr.value = chave2;
    db_iframe_histcalc.hide();
  }

  function js_pesquisaDocumentoAutodeInfracao2(lMostra) {
   if (lMostra) {
     sArquivoPesquisa = 'func_db_documentotemplate.php?funcao_js=parent.js_mostraDocumentoLookUpAutodeInfracao2|db82_sequencial|db82_descricao&tipo=51';
   } else {
     sArquivoPesquisa = 'func_db_documentotemplate.php?pesquisa_chave=' + $F('nl27_templateautoinfracao') + '&funcao_js=parent.js_mostraDocumentoDigitacaoAutodeInfracao3&tipo=51';
   }

    js_OpenJanelaIframe('CurrentWindow.corpo', 'db_iframe_db_documentotemplate', sArquivoPesquisa, 'Pesquisa', lMostra);
  }

  function js_mostraDocumentoDigitacaoAutodeInfracao3(sRetorno, lErro){

    $('nl27_descricaoautodeinfracao').value = sRetorno;

    if (lErro) {
      $('nl27_descricaoautodeinfracao').focus();
      $('nl27_descricaoautodeinfracao').value = '';
    }
  }

  function js_mostraDocumentoLookUpAutodeInfracao2(iCodigo, sRetorno) {

      $('nl27_templateautoinfracao').value     = iCodigo;
      $('nl27_descricaoautodeinfracao').value = sRetorno;
      db_iframe_db_documentotemplate.hide();
  }
   ...
   * Template Auto de Infraçao
   */
  function js_templateAutodeInfracao2(iOpcao) {

    if( typeof(iOpcao) === 'undefined' ) {
      iOpcao = $F('nl27_utilizadocpadrao');
    }

    if( iOpcao == 't' ) {

      $('linhaTemplate2').style.display        = 'none';
      $('nl27_templateautoinfracao').value     = '';
      $('db14_descricaoautodeinfracao').value = '';
    }else{
    $('linhaTemplate2').style.display = '';
    }
  }
  ...
  function js_pesquisa(){
    js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_parfiscal','func_fis_parfiscal.php?funcao_js=parent.js_preenchepesquisa|y32_instit','Pesquisa',true);
  }
