 ...
 * Data: 25/03/2025
 * Correção: Tratamento do valor utf8 do campo observação.
 */
...
//$head1 = "DECLARAÇÃO DE ESCOLARIDADE";
...
foreach ($aDadosAlunos as $oDadosAlunos) {  
  ...
  $sTexto = explode("nesta Unidade Escolar.", $sTexto);
  $complemento = ", no {$oDadosAlunos->xetapa}, turma {$oDadosAlunos->xurma} do(a) {$oDadosAlunos->xurso}, turno(s) {$oDadosAlunos->xurno} nesta escola.";
  
  $sTexto = $sTexto[0] . $complemento;
  ...
  $oPdf->multicell(180, $oParametros->iAlturaLinha + 4, $sTexto, 0, "J", 0, 0);
  $oPdf->Ln($oParametros->iAlturaLinha * 2);
  $oPdf->setXY(16, $oPdf->GetY());
