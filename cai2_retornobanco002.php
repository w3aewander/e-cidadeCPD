...
if (!empty($codgera)) {
    $dbwhere .= " and e87_codgera = {$codgera} ";
}
...
for ($i=0;$i<$numrows_retorno;$i++) {
  ...
  if(!empty($e60_anousu)){
    $pdf->cell(20,$alt,$e60_codemp."/".$e60_anousu,"T",0,"C",0);
  }
  else{
  ...
  if ($cl_empagedadosretornodetalhe->numrows > 0) {
   $pdf->setfont('arial', 'b', 8);
   $pdf->cell(100, $alt, "Dados Boleto(s): ",                     0, 1, "L", 0);
  }

  for ($x=0; $x < $cl_empagedadosretornodetalhe->numrows; $x++) {
    db_fieldsmemory($resultadoboletos,$x);
    $pdf->setfont('arial', 'b', 8);
    $pdf->cell(30, $alt, "Cod. Autenticação: ",         0, 0, "L", 0);
    $pdf->setfont('arial', '', 8);
    $pdf->cell(40, $alt, $e140_numeroautenticacao,      0, 0, "L", 0);
    $pdf->setfont('arial', 'b', 8);
    $pdf->cell(10, $alt, "Valor: ",                     0, 0, "L", 0);
    $pdf->setfont('arial', '', 8);
    $pdf->cell(15, $alt, db_formatar($e140_valor,"f"),  0, 0, "R", 0);
    $pdf->setfont('arial', 'b', 8);
    $pdf->cell(20, $alt, "Cod. Barras: ",               0, 0, "L", 0);
    $pdf->setfont('arial', '', 8);
    $pdf->cell(150, $alt, $codbarras,                   0, 1, "L", 0);
  }
  $total++;
}
