...
class dadosEmpenhoFolha
{
    ...
    public function getEstrututal($iAnoUsu, $iLotacao, $sVinculo, $iElemento)
    {
        ...
        $codigosRecursos = implode(",", $this->getRecursosCompativeis($iRecurso, $iAnoUsu));

        $whereRecurso = "and o58_codigo in ({$codigosRecursos}) ";
        $sWhereDotacao .= $whereRecurso;

        $sWhereDotacao .= $sWhereParam;

        $sSqlDotacao = $oDaoOrcDotacao->sql_query_ele(null, null, $sCamposDotacao, null, $sWhereDotacao, true);
    ...
    public function getRecursosCompativeis($codigo, $exercicio)
    {
        $sql = "
        with base as
                (
                SELECT
                    codigo_siconfi as base_siconfi,
                    o15_recurso as base_recurso
                FROM
                    orctiporec
                    JOIN fonterecurso ON orctiporec_id =o15_codigo
                WHERE
                    exercicio = {$exercicio}
                    AND o15_codigo = {$codigo}
            )
            SELECT
                o15_codigo
            FROM
                base
                join orctiporec on o15_recurso = base_recurso
                JOIN fonterecurso ON orctiporec_id = o15_codigo
                    and codigo_siconfi = base_siconfi
        ...
        if (!$rs) {
            throw new DBException("Não foi localizado o recurso.");
        }

        $count = pg_num_rows($rs);
        $dados = [];
        for ($i = 0; $i < $count; $i++) {
            $dados[] = db_utils::fieldsMemory($rs, $i)->o15_codigo;
        }
    ...
    public function geraDadosEmpenhosPrev($sTipo = '', $iAnoUsu = '', $iMesUsu = '', $sListaPrev = '', $iInstit = '')
    {
        ...
        $sCampos  = "r33_rubmat as rubmater, '2'||substr(r33_rubmat,2) as rubmaterfer, ";
        $sCampos .= "'4'||substr(r33_rubmat,2) as rubmater13";
        $sWhere   = "r33_anousu = {$iAnoUsu} and ";
        $sWhere  .= "r33_mesusu = {$iMesUsu} and ";
        $sWhere  .= "r33_codtab = 3 and ";
        $sWhere  .= "r33_instit = {$iInstit}";

        $clinssirf = new cl_inssirf();
        $sql_inss = $clinssirf->sql_query_file(null,
            null,
            $sCampos,
            "r33_nome",
            $sWhere,
        ...
        $rsInss = $clinssirf->sql_record($sql_inss);
        $maternidade = db_utils::fieldsMemory($rsInss, 0);
        ...
        foreach ($aSiglas as $iInd => $sSigla) {
            ...
            $sCampos = "distinct rh30_vinculo     as vinculo,                                              ";
            $sCampos .= "        rh02_lota        as lotacao,                                              ";
            $sCampos .= "        case when (select distinct                                                ";
            $sCampos .= "                     r33_codele                                                   ";
            ...
            $sCampos .= "                      and r33_mesusu = {$iMesUsu}                                 ";
            $sCampos .= "                      and r33_instit = {$iInstit}                                 ";
            $sCampos .= "                      and (cast((r33_codtab) as integer)-2) = rh02_tbprev)        ";
            $sCampos .= "        end as elemento,                                                          ";
            $sCampos .= "        {$sSigla}_rubric as rubric,                                               ";
            $sCampos .= "        {$sSigla}_regist as regist,                                               ";
            $sCampos .= "        rh02_seqpes      as pessoalmov,                                           ";
            $sCampos .= "        {$sSigla}_pd     as pd,                                                   ";
        ...
        if ($rsGerador) {
            ...
            if ($iLinhasGerador > 0) {
                for ($iInd = 0; $iInd < $iLinhasGerador; $iInd++) {
                    ...
                    /*
                     * Busca o valor do salario maternidade para abater do valor patronal
                     * */
                    $sWhere = "{$oGerador->sigla}_rubric in ('{$maternidade->rubmater}', '{$maternidade->rubmaterfer}', ";
                    $sWhere .= "'{$maternidade->rubmater13}')";
                    $sCampos = "round(coalesce(sum({$oGerador->sigla}_valor),0),2) as vl_maternidade";

                    $sSqlTotalMaternidade = $clgeradorsql->gerador_sql($oGerador->sigla,
                        $iAnoUsu,
                        $iMesUsu,
                        $oGerador->regist,
                        "",
                    ...
                    $rsTotalMaternidade = db_query($sSqlTotalMaternidade);
                    $nValorMaternidade = db_utils::fieldsMemory($rsTotalMaternidade, 0)->vl_maternidade;

                    /* Busca informação do percentual do patronal */
                    ...
                    $nValorPatronal = round((($oGerador->valor - $nValorMaternidade) * $nPercPatronal), 2);

                    $oDaorhEmpenhoFolhaRubrica->rh73_rubric = $oGerador->rubric;
                    $oDaorhEmpenhoFolhaRubrica->rh73_seqpes = $oGerador->pessoalmov;
                    $oDaorhEmpenhoFolhaRubrica->rh73_instit = $iInstit;
                    $oDaorhEmpenhoFolhaRubrica->rh73_valor = db_formatar($nValorPatronal, 'p');
                    $oDaorhEmpenhoFolhaRubrica->rh73_pd = 1;
                    $oDaorhEmpenhoFolhaRubrica->rh73_tiporubrica = 1;
    ...
    function getDotacaoByFiltro($iOrgao, $iUnidade, $iProjAtiv, $iRecurso, $iElemento, $iAnoUsu, $iFuncao = null, $iSubFuncao = null, $iPrograma = null)
    {
        ...
        /**
         * aqui devemos trocar o codigo do recurso, pesquisar pela especificação
         */
        ...
        $filtroRecurso = " and o58_codigo = {$iRecurso}";
        if (!FONTE_RECURSO_UNIAO) {
            $recurso = \ECidade\Financeiro\Orcamento\Repository\RecursoRepository::getByCodigo($iRecurso);
            $especificacao = $recurso->getEspecificacaoLOA();
            $filtroRecurso = "  and o15_loaespecificacao = '{$especificacao}'";
        }
        $sSql .= "   {$sWhereParam}";
        $sSql .= $filtroRecurso;
    ...
    public function geraPlanilhaDevolucao($sSigla = '', $iAnoUsu = '', $iMesUsu = '', $iInstit = '', $sSemestre = '')
    {
        ...
        if ($iLinhasDevolucao > 0) {
            ...
            for ($iInd = 0; $iInd < $iLinhasDevolucao; $iInd++) {
                ...
                $sSqlContaextra = $oDaoSaltesExtra->sql_query_extra(
                    null,
                    "k109_contaextra",
                    null,
    ...
    public function geraPlanilhaEmpenhoFolha($sSigla, $iAnoUsu, $iMesUsu, $sSemestre, $iCgm = '', $iInstit)
    {
        ...
        if ($iNroRubricas > 0) {
            ...
            for ($iInd = 0; $iInd < $iNroRubricas; $iInd++) {
                ...
                if ($oDaoPlaCaixaRec->erro_status == '0') {
                    throw new DBException($sMsgErro . "\n2. " . $oDaoPlaCaixaRec->erro_msg . " , Recurso Codigo: ".  $oDadosRubricas->recurso);
                }
