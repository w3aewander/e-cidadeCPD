...
//funções de interface da classe
     ...
     * @return boolean|stdClass Objeto com com parametros codigo,unidade,alfanumerino,numerico,etc...
     */
    function dadosRef($iAtributo_codigo)
    {
        ...
        $sCampos .= ", la30_c_calculavel, la13_c_descr, la29_i_codigo, la29_v_fixo";

        $daoLabValorReferencia = new cl_lab_valorreferencia();
        $sSql = $daoLabValorReferencia->sql_query_tipos("", $sCampos, "", "la27_i_atributo = {$iAtributo_codigo} ");
        $rResult = $daoLabValorReferencia->sql_record($sSql);
        ...
        if ($rResult != false) {
            ...
            $oResultado->fixo = $aResultado[0]->la29_v_fixo;
    ...
    function loadRef($iAtributo_codigo, $iTipo = 1)
    {
        ...
        $daoLabResultado = new cl_lab_resultado();
        $sSql = $daoLabResultado->sql_query_item("", $sCampos, "", $sWhere);
        $rResult = $daoLabResultado->sql_record($sSql);
     ...
     * @param $iAtributo_codigo
     * @return boolean|stdClass Objeto com com parametros codigo,nome,estrutural,tipo,nivel
     */
    function dadosAtributo($iatributo_codigo)
    {
        ...
        $daoLabAtributo = new cl_lab_atributo();
        $sSql = $daoLabAtributo->sql_query($iatributo_codigo, $sCampos);
        $rResult = $daoLabAtributo->sql_record($sSql);
    ...
    function levantamentoFilhos($iPai_atributo)
    {
        ...
        $sSql = $daoLabExameAtributoLigacao->sql_query_file("", $sCampos, " la26_i_exameatributofilho ", $sWhere);
    ...
    function montaSintetico($iPai_atributo, $cPai_estrutural, $cPai_nome, $cPai_nivel)
    {
        if ($this->showAtributo($iPai_atributo)) {
            echo "<tr><td colspan=3><b>" . $this->nivel($cPai_nivel,
                5) . $this->removeCasas($cPai_estrutural) . " - $cPai_nome </b></td></tr>";
        }
        ...
        for ($x = 0; $x < count($oFilhos); $x++) {
            if ($oFilhos[$x]['tipo'] == 1) {
                $this->montaSintetico($oFilhos[$x]['codigo'], $oFilhos[$x]['estrutural'], $oFilhos[$x]['nome'],
                  $oFilhos[$x]['nivel']);
            } else {
                if ($this->showAtributo($oFilhos[$x]['codigo'])) {
                    ...
                    echo "<tr><td>" . $this->nivel($oFilhos[$x]['nivel'],
                        5) . $this->removeCasas($oFilhos[$x]['estrutural']) . " - " . $oFilhos[$x]['nome'] . "</td>
    ...
    {
        echo "<tr><td colspan=2><b>" . $this->nivel($cPai_nivel,
            5) . $this->marca($iPai_atributo) . $this->removeCasas($cPai_estrutural) . " - $cPai_nome </b></td></tr>";
        $oFilhos = $this->levantamentoFilhos($iPai_atributo);

        for ($x = 0; $x < count($oFilhos); $x++) {
            if ($oFilhos[$x]['tipo'] == 1) {
                $this->montaSinteticocheck($oFilhos[$x]['codigo'], $oFilhos[$x]['estrutural'], $oFilhos[$x]['nome'],
                  $oFilhos[$x]['nivel']);
            } else {
                echo "<tr><td>" . $this->nivel($oFilhos[$x]['nivel'],
                    5) . $this->marca($oFilhos[$x]['codigo']) . $this->removeCasas($oFilhos[$x]['estrutural']) . " - " . $oFilhos[$x]['nome'] . "</td></tr>";
            }
        }
    }
    ...
    function montaSinteticoPDF($iPai_atributo, $cPai_estrutural, $cPai_nome, $cPai_nivel)
    {
        if ($this->showAtributo($iPai_atributo)) {
            ...
            $this->oPDF->cell(190, 5,
              $this->nivel($cPai_nivel, 5, 2) . $this->removeCasas($cPai_estrutural) . " - {$cPai_nome} ",
              $this->iBordas, 1, "L", 0);
        }
        ...
        for ($x = 0; $x < count($oFilhos); $x++) {
            if ($oFilhos[$x]['tipo'] == 1) {
                $this->montaSinteticoPDF($oFilhos[$x]['codigo'], $oFilhos[$x]['estrutural'], $oFilhos[$x]['nome'],
                  $oFilhos[$x]['nivel']);
            } else {
                if ($this->showAtributo($oFilhos[$x]['codigo'])) {
                    $this->oPDF->setfont('arial', '', 8);
                    $this->oPDF->cell(120, 5, $this->nivel($oFilhos[$x]['nivel'], 5,
                        2) . $this->removeCasas($oFilhos[$x]['estrutural']) . " - " . $oFilhos[$x]['nome'],
                      $this->iBordas, 0, "L", 0);
                    $this->oPDF->cell(70, 5, "" . $this->valorRefPDF($oFilhos[$x]['codigo']), $this->iBordas, 1, "R",
                      0);
                }
            }
        }
    ...
    function montaSinteticoMapaPDF($iPai_atributo, $cPai_estrutural, $cPai_nome, $cPai_nivel)
    {
        ...
        for ($x = 0; $x < count($oFilhos); $x++) {
            if ($oFilhos[$x]['tipo'] == 1) {
                $this->montaSinteticoMapaPDF($oFilhos[$x]['codigo'], $oFilhos[$x]['estrutural'], $oFilhos[$x]['nome'],
                  $oFilhos[$x]['nivel']);
            } else {
                if ($this->showAtributo($oFilhos[$x]['codigo'])) {
                    $this->oPDF->setfont('arial', '', 8);
                    ...
                    $this->oPDF->cell(120, 5, $this->nivel($oFilhos[$x]['nivel'], 5,
                        2) . $this->removeCasas($oFilhos[$x]['estrutural']) . " - " . $oFilhos[$x]['nome'], $borda2, 0,
                      "L", 0);
                    $this->oPDF->multicell(70, 5, "" . $this->valorRefMapaPDF($oFilhos[$x]['codigo']), $this->iBordas,
                      1, "R", 0);
                }
            }
        }
