...
if(empty($argv[2])){
	db_logduplos("Necessário passar o parametro do codigo da unificacao");
	exit;
}

if(empty($argv[3])){
	db_logduplos("Necessário passar o parametro do login do processamento");
	exit;
}

$codUnificacao = $argv[2];
$loginProcessamento = $argv[3];
...
$sql_correto = "select * from sau_cgscorreto where s127_b_proc is false and s127_i_codigo = $codUnificacao order by s127_d_data, s127_c_hora, s127_i_codigo";
...
for($record_correto = 0; $record_correto < pg_numrows ( $result_correto ); $record_correto ++) {
	...
	$daoCgs = new cl_cgs_und;
	$camposFaltantes= "z01_v_mae,z01_v_pai,z01_orgaoemissoridentidade,z01_v_ident,z01_v_cgccpf";
	$sqlCgs = $daoCgs->sql_query_file($s127_i_numcgs,$camposFaltantes);
	$rsCgs = db_query($sqlCgs);

	$cgs = db_utils::fieldsMemory($rsCgs,0);

	if((!empty($cgs->z01_v_mae) && in_array($cgs->z01_v_mae,['SEM INFORMAÇÃO','SEM INFORMACAO']))
	    || empty($cgs->z01_v_mae)
	){
		$dadosFaltantes['mae'] = true;
	}

	if( (!empty($cgs->z01_v_pai) && in_array($cgs->z01_v_pai,['SEM INFORMAÇÃO','SEM INFORMACAO']))
	    || empty($cgs->z01_v_pai)){
		$dadosFaltantes['pai'] = true;
	}

	if(empty($cgs->z01_orgaoemissoridentidade)){
		$dadosFaltantes['orgao_rg'] = true;
	}

	if(empty($cgs->z01_v_ident)){
		$dadosFaltantes['rg'] = true;
	}

	if(empty($cgs->z01_v_cgccpf)){
		$dadosFaltantes['cpf'] = true;
	}
	...
	for($record_errado = 0; $record_errado < pg_numrows ( $result_errado ); $record_errado ++) {
		...
		if ($v_cgscerto > 0) {
		    ...
		    /**
		     * Verifica se há dados faltantes no cgs_correto e se há dados
		     * relevantes no cgs_errado. Se houverem dados, preencher o cgs_correto
		     * de acordo com o cgs_errado. Se houver mais de um cgs_errado, qualquer
		     * um deles servirá para preenchimento da informação.
		     *
		     */

		    $daoCgsErrado = new cl_cgs_und;
		    $camposFaltantes= "z01_v_mae,z01_v_pai,z01_orgaoemissoridentidade,z01_v_ident,z01_v_cgccpf";
		    $sqlCgsErrado = $daoCgsErrado->sql_query_file($v_sau_cgserrado,$camposFaltantes);
		    $rsCgsErrado = db_query($sqlCgsErrado);
		    $cgsErrado = db_utils::fieldsMemory($rsCgsErrado,0);

		    $atualizacaoCgsCorreto = [];

		    if(!empty($cgsErrado->z01_v_mae)
				&& !in_array($cgsErrado->z01_v_mae,['SEM INFORMAÇÃO','SEM INFORMACAO'])
			    && $dadosFaltantes['mae'] == true
			){
				$maeCgsErrado = $cgsErrado->z01_v_mae;
		    	$atualizacaoCgsCorreto[] = " z01_v_mae = '$maeCgsErrado' ";
		    	$dadosFaltantes['mae'] = false;
		    }

		    if(!empty($cgsErrado->z01_v_pai)
			    && !in_array($cgsErrado->z01_v_pai,['SEM INFORMAÇÃO','SEM INFORMACAO'])
			    && $dadosFaltantes['pai'] == true
			){
				$paiCgsErrado = $cgsErrado->z01_v_pai;
			    $atualizacaoCgsCorreto[] = " z01_v_pai = '$paiCgsErrado' ";
			    $dadosFaltantes['pai'] = false;
		    }

		    if(!empty($cgsErrado->z01_orgaoemissoridentidade) && $dadosFaltantes['orgao_rg'] == true){
				$orgaoRgCgsErrado = $cgsErrado->z01_orgaoemissoridentidade;
			    $atualizacaoCgsCorreto[] = " z01_orgaoemissoridentidade = '$orgaoRgCgsErrado' ";
			    $dadosFaltantes['orgao_rg'] = false;
		    }

		    if(!empty($cgsErrado->z01_v_ident) && $dadosFaltantes['rg'] == true){
				$rgCgsErrado = $cgsErrado->z01_v_ident;
			    $atualizacaoCgsCorreto[] = " z01_v_ident = '$rgCgsErrado' ";
			    $dadosFaltantes['rg'] = false;
		    }

		    if(!empty($cgsErrado->z01_v_cgccpf) && $dadosFaltantes['cpf'] == true){
				$cpfCgsErrado = $cgsErrado->z01_v_cgccpf;
		    	$atualizacaoCgsCorreto[] = " z01_v_cgccpf = '$cpfCgsErrado' ";
		    	$dadosFaltantes['cpf'] = false;
		    }

		    if(count($atualizacaoCgsCorreto) > 0){
			    $sqlUpdateCgsCorreto = " update cgs_und set ";
			    $sqlUpdateCgsCorreto .= implode(",",$atualizacaoCgsCorreto);
			    $sqlUpdateCgsCorreto .= " where z01_i_cgsund = $v_cgscerto ";
				db_logduplos ( "     Atualiza Dados Faltantes - " . $sqlUpdateCgsCorreto );
			    db_query ( $sqlUpdateCgsCorreto ) or die(
				    db_logduplos ( "sql: $sqlUpdateCgsCorreto\n" . pg_ErrorMessage () )
			    );
		    }
       ...
       */
		...
		$sql7 = "insert into sau_cgserradolog values ($s127_i_codigo, $s128_i_numcgs, '$v_log');";
		$result = db_query ( $sql7 ) or die ( db_logduplos ( $sql7 . "---- sql: " . pg_ErrorMessage () ) );
	}

	$dataProcessamento = strval(date("d/m/Y"));
    $horaProcessamento = strval(date("H:i"));

	$sql8 = "update sau_cgscorreto set s127_b_proc = true,s127_data_proc = '$dataProcessamento',s127_hora_proc = '$horaProcessamento',s127_login_proc=$loginProcessamento where s127_i_codigo = $s127_i_codigo";
