...
require_once(modification("libs/db_mail_class.php"));
...
if ( $oPost->sMethod == 'consultaProcessos') {
  ...
  echo $oJson->encode($aRetorno);
    ...
    if (isset($oPost->iTipoRetorno) == 3){

      $oMail->setsClass(9);
      $oMail->setsEmailTo($oPost->sContribuinteEmail);
      $oMail->setsSubject('RESPOSTA CAC 156 - pedido ' . $oPost->iCodAtendimento);
      $oMail->setsEmailFrom(array('cac.156@epdvr.com.br', 'CAC 156'));
      //$oPost->sRequerente;
      //$oPost->sSolicitacao;
      $message = "<!DOCTYPE html PUBLIC '-//W3C//DTD XHTML 1.0 Transitional//EN' 'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'>
        <html xmlns='http://www.w3.org/1999/xhtml'>
          ...
          <body>
            <b>Prezado Senhor(a): {$oPost->sRequerente}</b>
            <p>Em resposta a sua solicitação realizada pela Central de Atendimento ao Cidadão e encaminhada a devida Secretaria:</p>

            <p>Depto.: {$oPost->sDepartamento}</p>

            <p>Resposta:</p>
            <p>{$oPost->sResposta}</p>

            <b>A PMVR agradece pelo seu contato.</b>

          </body>
        </html>";

      $oMail->setsMsg($message);
      //$oMail->setsMsg(utf8_decode($oPost->sInformacao . ' - ' .utf8_decode($oPost->sResposta)));
      $oMail->Send();
    }
  }
