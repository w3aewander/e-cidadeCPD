    ...
    switch ($oParam->exec) {
        case "salvar":
            ...
            foreach ($oParam->aBens as $oBemParametro) {
                if (empty($oBemParametro->nValorAquisicao) || !is_numeric($oBemParametro->nValorAquisicao)) {
                    throw new ParameterException(_M(CAMINHO_MENSAGENS . "valor_aquisicao_obrigatorio"));
                }

                if (empty($oBemParametro->nValorResidual) || !is_numeric($oBemParametro->nValorResidual)) {
                    throw new ParameterException(_M(CAMINHO_MENSAGENS . "valor_residual_obrigatorio"));
                }

                if (empty($oBemParametro->iQuantidade) ||
                    !is_numeric($oBemParametro->iQuantidade)) {
                ...
                if (empty($oBemParametro->iSeqPlaca)) {
                ...
                if (!empty($oBemParametro->iDivisao)) {
                ...
                if (!empty($oBemParametro->iConvenio)) {
                ...
                foreach ($aBens as $bem) {
                    $bem->setFoto('null');
                    $bem->setEstorage('f');

                    // Salva imagem no banco
                    if (!empty($oBemParametro->foto)) {
                        $id = StorageHelper::uploadArquivo($oBemParametro->foto, [], true, null, null, $oBemParametro->nameImage);
                        $bem->setFoto($id);
                        $bem->setEstorage('t');
                    }
                }
            ...
            if (round($nTotalNotas, 2) != round($nTotalBens, 2) && $oParam->recalcularCentavoBem === false) {
                throw new BusinessException(_M(CAMINHO_MENSAGENS . "valor_bem_nota"));
            }
            try {
                if ($oParam->recalcularCentavoBem === true && count($aBens) > 1) {
                    $calculoBem = new CalculoBem();
                    $calculoBem->setBem($aBens[0]);
                    $calculoBem->atualizarCentavoValorBem();
                ...
                /**
                 * Para o caso de n bens para n notas, processará os lançamentos separamento,
                 * realizando um lançamento por nota
                ...
                foreach ($aBens as $bem) {
                    if (!empty($bem->getFoto()) && $bem->getFoto() != "null") {
                        StorageHelper::deleteArquivo($bem->getFoto());
                    }
                }
