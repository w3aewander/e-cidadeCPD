...
if ($db_opcao == 1) {
    ...
    $sSql = " select numpref.k03_recjur as k02_recjur,
                   ...
                   credito.k02_descr   as descr_credito,
...
<form name="form1" method="post" action="">
    <fieldset>
        <legend>Manutenção de Receitas</legend>
        <table class="form-container">
            ...
            <tr id="linha_complemento">
                ...
                <td>
                    <?php
                    $itens = Complemento::getAll();
                    db_select('k02_complemento', $itens, true, $db_opcao);
                    ?>
                </td>
            </tr>
            <tr>
            ...
            <tr id="receitacredito">
                <td nowrap title="<?= @$Tk02_reccredito ?>">
                    <?php
                    db_ancora(@$Lk02_reccredito, "js_pesquisak02_reccredito(true);", $db_opcao);
                    ?>
                </td>
                <td>
                    <?php
                    db_input('k02_reccredito', 10, @$Ik02_reccredito, true, 'text', $db_opcao, " onchange='js_pesquisak02_reccredito(false);'");
                    db_input('k02_descr', 57, @$Ik02_descr, true, 'text', 3, "", "descr_credito");
                    ?>
                </td>
            </tr>
    ...
    <input id="novareceita" type="button" value="Nova Receita"/>
</form>
<script>

    document.getElementById('k02_complemento').className = 'field-size8';
    ...
    function js_pesquisak02_recjur(mostra) {
        ...
        var sUrl1 = 'func_tabrec_todas.php?' + sFuncao + '.js_mostrak02_recjur1|k02_codigo|k02_descr&k02_tabrectipo=2,5';
    ...
    function js_mostrak02_reccredito(chave, erro) {
        if (erro == true) {
            document.form1.k02_reccredito.focus();
            document.form1.k02_reccredito.value = '';
            document.form1.descr_jur.value = '';
            alert(chave);
        } else {
            document.form1.descr_credito.value = chave;
        }
    }

    function js_mostrak02_reccredito1(chave1, chave2) {
        document.form1.k02_reccredito.value = chave1;
        document.form1.descr_credito.value = chave2;
        (window.CurrentWindow || parent.CurrentWindow).corpo.iframe_tabrec.db_iframe_tabreccredito.hide();
    }

    function js_pesquisak02_recmul(mostra) {
        var sFuncao = 'funcao_js=parent.CurrentWindow.corpo.iframe_tabrec';
        var k02_recmul = document.form1.k02_recmul.value;
        var sUrl1 = 'func_tabrec_todas.php?' + sFuncao + '.js_mostrak02_recmul1|k02_codigo|k02_descr&k02_tabrectipo=3,5';
    ...
    function js_pesquisak02_reccredito(mostra) {
        var sFuncao = 'funcao_js=parent.CurrentWindow.corpo.iframe_tabrec';
        var k02_reccredito = document.form1.k02_reccredito.value;
        var sUrl1 = 'func_tabrec_todas.php?' + sFuncao + '.js_mostrak02_reccredito1|k02_codigo|k02_descr&k02_tabrectipo=1';
        var sUrl2 = 'func_tabrec_todas.php?pesquisa_chave=' + k02_reccredito + '&k02_tabrectipo=1&' + sFuncao + '.js_mostrak02_reccredito';

        if (mostra == true) {
            js_OpenJanelaIframe('CurrentWindow.corpo.iframe_tabrec', 'db_iframe_tabreccredito', sUrl1, 'Pesquisa', true, '0');
        } else {
            if (k02_reccredito != '') {
                js_OpenJanelaIframe('CurrentWindow.corpo.iframe_tabrec', 'db_iframe_tabreccredito', sUrl2, 'Pesquisa', false, '0');
            } else {
                document.form1.k02_reccredito.value = '';
                document.form1.descr_credito.value = '';
            }
        }
    }
    ...
    function js_param_tipgruprec(chave) {
        ...
        mostrarComplemento(document.getElementById('k02_tipo').value);
    }
    ...
    document.getElementById('descr_credito').style.width = 424;

    function mostrarComplemento(tipoReceita) {

        let display = 'none';
        if (tipoReceita === 'O') {
            display = '';
        }
        document.getElementById('linha_complemento').style.display = display;
    }

    document.getElementById('novareceita').addEventListener('click', () => {
        location.href = 'cai1_receita004.php';
    });
</script>
