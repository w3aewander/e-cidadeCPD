...
/**
 *  Consulta os dados referente a competência passada por parâmetro
 */
...
switch ($oGet->iTipoReg) {
	...
	break;
  case 2 :
    $sCabTipoReg = " Somente Registros com Inconsistência";
  break;
  case 3 :
  	$sCabTipoReg = " Somente Registros sem Inconsistência";
  break;
...
function imprimeCab($oPdf,$iAlt,$iFonte,$lImprime=false){
  ...
  if ($oPdf->gety() > $oPdf->h - 30 || $lImprime ){
		...
		$oPdf->Cell(25 ,$iAlt,"Código Evento"      ,1,0,'C',1);
		$oPdf->Cell(95 ,$iAlt,"Descrição Evento"   ,1,0,'C',1);
		$oPdf->Cell(25 ,$iAlt,"Data Evento"        ,1,0,'C',1);
		$oPdf->Cell(30 ,$iAlt,"Tipo Registro"      ,1,0,'C',1);
		$oPdf->Cell(30 ,$iAlt,"Situação"           ,1,0,'C',1);
		$oPdf->Cell(30 ,$iAlt,"Posição Atual"      ,1,1,'C',1);
