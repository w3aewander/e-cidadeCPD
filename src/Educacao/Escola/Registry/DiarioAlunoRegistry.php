...
class DiarioAlunoRegistry
{
    ...
    /**
     * @param DiarioAluno $diarioArea
     */
    public static function set(DiarioAluno $diarioArea)
    {
        self::$storage[$diarioArea->getCodigo()] = $diarioArea;
    }
    ...
    public static function get($key)
    {
        if (!array_key_exists($key, self::$storage)) {
            $diarioAluno = DiarioAlunoRepository::find($key);
            if (is_null($diarioAluno)) {
                return null;
            }

            self::set($diarioAluno);
        }
