...
DBViewHelpInline.prototype.startElement = function(element, field) {
  ...
  this.icon = new DBViewHelpInline.Icon(element);
  this.icon.registerEvents();
  this.icon.initTooltip();

  return element;
};

/**
 * Remove os eventos do icone, fazendo com que ele não seja exibido
 */
DBViewHelpInline.prototype.removeElement = function() {
  if (this.icon == null) {
    return;
  }
  this.icon.hide();
  this.icon.element.replaceWith(this.icon.element.cloneNode(true));
}
