...
class M23525AjusteBaixaBancoPercentualPagamentoCustas extends Migration
{
    /**
     * Run the migrations.
     *
  ...
  if lAtivaPgtoParcial is true then
    ...
    for rRecordDisbanco in select distinct
                            ...
                            where disbanco.classi is false
                              and disbanco.codret = cod_ret
                              and disbanco.instit = iInstitSessao
                              and recibopaga.k00_numnov is null 
    ...
    loop
        ...
        from (select (substr(fc_calcula, 15, 13)::float8 +
                ...
                from (select fc_calcula(x.k00_numpre,
                        ...
                        from (select distinct
                                     ...
                                     recibopaga.k00_numpar,
                                     case 
                                     when disbanco.dtpago < recibopaga.k00_dtpaga
                               ...
                               inner join arrecad 
                                  on arrecad.k00_numpre = recibopaga.k00_numpre
                                 and arrecad.k00_numpar = recibopaga.k00_numpar
      ...
      if (
        ...
        insert into disbanco (k00_numbco,
                ...
                for rPercentualCustas in (
                  with dadoscustasrecibo as (
                   select k00_numnov          as numnovt,
                          sum(k00_valor)      as valortotalrecibo
                     from recibopaga
                    where k00_numnov = rRecordDisbanco.numpre
                      and k00_hist  <> 11403
                    group by 1   
                 ), dadoscustasnumprerecibo as (
                 select k00_numnov         as numnovn,
                        k00_numpre         as numpre,
                        sum(k00_valor)     as valornumpre
                  from dadoscustasrecibo   
                 inner join recibopaga 
                    on k00_numnov = numnovt
                 where k00_hist  <> 11403   
                 group by 1, 2  
              ), dadosmatricinscr as (
              select numnovn                 as numnovmi,
                     numpre                  as numpremi, 
                     'm'                     as tipo,
                     m.k00_matric            as numero,
                     (valornumpre*k00_perc)  as valorperc,
                     valortotalrecibo
                from dadoscustasnumprerecibo
               inner join dadoscustasrecibo
                  on numnovt = numnovn
               inner join arrematric m
                  on m.k00_numpre = numpre
               union all
               select numnovn                as numnovmi,
                      numpre                 as numpremi,  
                      'i'                    as tipo,
                      i.k00_inscr            as numero,
                      (valornumpre*k00_perc) as valorperc,
                      valortotalrecibo                  
                from dadoscustasnumprerecibo     
               inner join dadoscustasrecibo
                  on numnovt = numnovn
               inner join arreinscr i
                  on i.k00_numpre = numpre
              )
              select numnovmi,
                     tipo,
                     numero,
                     valortotalrecibo,
                     sum(valorperc)/valortotalrecibo as perc
                from dadosmatricinscr
              group by 1,2,3,4)
                  loop

                    if  rPercentualCustas.tipo = 'm'
                    then
                      insert into arrematric(k00_numpre, k00_matric, k00_perc) 
                      values(iNumpreReciboAvulso, rPercentualCustas.numero, rPercentualCustas.perc);
                    end if;

                    if  rPercentualCustas.tipo = 'i'
                    then
          ...
          from (select (substr(fc_calcula, 15, 13)::float8 +
                  ...
                  from (select fc_calcula(x.k00_numpre,
                                          ...
                                          coalesce((select sum(k00_valor) 
                                                      from recibopaga
                                                     where k00_numnov = x.k00_numnov
                                                       and k00_numpre = x.k00_numpre
                                                       and k00_numpar = x.k00_numpar
                                                       and k00_hist   = 918
                                                       and k00_dtpaga <= x.dtpago
                                                   ), 0)  as valor_desc
                          from (select distinct
