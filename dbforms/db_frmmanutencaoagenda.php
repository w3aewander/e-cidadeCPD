                  ...
                  <fieldset class='filtros'>
                    ...
                    <table border="0" align="left" >
                      ...
                      <tr>
                        ...
                        <td colspan='2'>
                          <?php /* ?><input type='checkbox' id='efetuarpagamento' onclick="js_showAutenticar(this)" /><?php */ ?>
                      ...
                      <table id="showrDadosExtras" style='display:none'>
                          <!-- CRIAÇÃO DE NOVOS CAMPOS NO FORMULÁRIO -->
                          <tr>
                              <td valign="top"><b>Data da publicação justificando a quebra do contrato</b></td>
                              <td nowrap>
				                  <?php db_inputdata("e50_dataquebraordem", null, null, null, true, "text", 1); ?>
                              </td>

                              <td valign="center"><b>Nº da edição:</b></td>
                              <td nowrap>
                                  <input name="e50_numedi" id='e50_numedi'
                                         title='<?=$Te50_numedi?>' size="15" type='text'  onKeyPress="return js_mascara(event);" >
                              </td>
                          </tr>
                          <tr>
                              <td valign="top"><b>Justificativa da Quebra de ordem</b></td>
                              <td colspan="3" nowrap>
				                  <?php db_input('e50_justificativaquebraordem',60,null,true,"text",3);?>
                              </td>
                          </tr>
                      </table>
                  </fieldset>
...
<script type="text/javascript">
  ...
  document.getElementById("e50_numedi").addEventListener("blur", mpreencherjustificativa);
  document.getElementById("e50_dataquebraordem").addEventListener("blur", mpreencherjustificativa);
  document.getElementById("e50_dataquebraordem").addEventListener("change", mpreencherjustificativa);
  //document.getElementById("Janiframe_data_e50_dataquebraordem").addEventListener("click", mpreencherjustificativa);


  function mpreencherjustificativa() {
      var dataquebra = $('e50_dataquebraordem').value;
      if(dataquebra.length > 0) {
          ...
          var numedi = $('e50_numedi').value;
          if(numedi.length > 0)
            $('e50_justificativaquebraordem').value = "http://new.voltaredonda.rj.gov.br/images/Documentos/VRDestaques/" + ano + "/" + dataquebra + "_" + numedi + ".pdf";
      }
  }
  ...
  function js_init() {
      //document.getElementById("e50_justificativaquebraordem").disabled = true;
  ...
  function js_configurar() {
    ...
    /** Novos Campos Bruno Mattos **/
    var dtDataQuebraOrdem = $('e50_dataquebraordem').value;
      var numediQuebraOrdem = $('e50_numedi').value;
    var sJustificativaQuebraOrdem = $('e50_justificativaquebraordem').value;
    /** Novos campos fim **/
    var lSemErro = true;
    var sAviso = ''
      //alert (dtDataQuebraOrdem.trim().length);
    //alert(numediQuebraOrdem.trim().length);

    if(dtDataQuebraOrdem.trim().length > 0 && numediQuebraOrdem.trim().length == 0)
    {
        alert('Data da quebra de ordem e número da edição devem ser informados juntos.');
        return false;
    }
    else if(dtDataQuebraOrdem.trim().length == 0 && numediQuebraOrdem.trim().length > 0)
      {
          alert('Data da quebra de ordem e número da edição devem ser informados juntos.');
          return false;
      }
    ...
    if (dtDataQuebraOrdem.trim().length == 0 && numediQuebraOrdem.trim().length == 0 && $F('e42_dtpagamento') == "") {
    ...
    /** Novos campos **/
    oEnvioMovimento.dtDataQuebraOrdem = dtDataQuebraOrdem;
    oEnvioMovimento.sJustificativaQuebraOrdem = sJustificativaQuebraOrdem;
    oEnvioMovimento.numediQuebraOrdem =  numediQuebraOrdem;
    /** Novos campos fim **/
    ...
    if ($('emitirordemauxiliar').checked) {
      ...
      if (dtDataQuebraOrdem.trim().length > 0 && numediQuebraOrdem.trim().length > 0)
      {
          ...
          for (var iMov = 0; iMov < aMovimentos.length; iMov++) {
              if(iMov == 0)
                  strnumemp = aMovimentos[iMov][5];
              else
                  strnumemp = strnumemp + "," + aMovimentos[iMov][5];

          }

          if(strnumemp.length > 0) {
              strnumemp = "( " + strnumemp + " )";
              var url = 'emp4_agendaPagamentoRPC.php';
              var sJson = '{"exec":"quebraordem","params":[{"strnumemp":"' + strnumemp + '","e50_dataquebraordem":"'+dtDataQuebraOrdem+'","e50_justificativaquebraordem":"'+sJustificativaQuebraOrdem+'","e50_numedi":"'+numediQuebraOrdem+'"}]}';
              var oAjax = new Ajax.Request(
                  url,
                  {
                      method: 'post',
                      parameters: 'json=' + sJson,
                      onComplete: js_retornoQuebraOrdem
                  }
                  );
          }
          return true;
  ...
  function js_retornoQuebraOrdem(oAjax){
  ...
  function js_showAutenticar(obj) {
    if (obj.checked) {
      ...
      $('showreemissao').style.visibility = 'visible';
        ...
        $('showrDadosExtras').style.visibility = 'visible';
        $('showrDadosExtras').style.display = 'block';
        //$('e50_justificativaquebraordem').style.disable = true;
      ...
      $('showreemissao').style.visibility  = 'hidden';
        ...
        $('showrDadosExtras').style.visibility = 'hidden';
        $('showrDadosExtras').style.display = 'none';
