...
for ($c = 0; $c < $linhas; $c++) {
    ...
    if ($pdf->gety() > $pdf->h - 30 || $troca != 0) {
        ...
        for ($w = 0; $w < $linhas_disc; $w++) {
            ...
            $pdf->cell($tamanhoDisciplinas, 4, $ed232_c_abrev, 1, 0, "C", 1);
        }
        ...
        $pdf->cell(2, 12, "", 1, 0, "C", 1);
        $pdf->SetXY(171, $posy);
        $pdf->cell(117, 8, $titulo_rendimento, 1, 1, "C", 1);
        $pdf->SetXY(10, $posy + 12);
        $troca = 0;
    }

    if ($pri_base != $ed31_i_codigo && $c > 0) {
        $pdf->cell(278, 1, "", 1, 1, "L", 0);
        $pdf->setfont('arial', 'b', 8);
        ...
        for ($w = 0; $w < $linhas_disc; $w++) {
            ...
            $sql5 .= " AND not exists (select 1 from turmaserieregimemat
                         join areaprocedimento ON ed157_procedimento = ed220_i_procedimento
                         join serieregimemat ON ed223_i_codigo = ed220_i_serieregimemat
                   where ed59_i_turma = turmaserieregimemat.ed220_i_turma
                     and ed59_i_serie = ed223_i_serie); ";
            $result5 = db_query($sql5);
            db_fieldsmemory($result5, 0);
            $pdf->cell($tamanhoDisciplinas, 4, $reprov_disc == "" || $reprov_disc == 0 ? "" : $reprov_disc, 1, 0, "C", 0);
        }
    ...
    if ($pri_ensino != $ed11_i_ensino) {
        $pdf->cell(278, 4, $ed10_c_descr . " - " . $ed10_c_abrev, 1, 1, "L", 1);
        $pri_ensino = $ed11_i_ensino;
    }

    $pdf->setfont('arial', '', 8);
    if ($pri_base != $ed31_i_codigo) {
        $pdf->cell(278, 4, $ed31_c_descr, 1, 1, "L", 0);
    ...
    $sql1 .= "          and (ed60_d_datasaida >= '$data_censo' OR ed60_d_datasaida IS NULL) ";
    $sql1 .= "        ) as antecenso, ";
    $sql1 .= "      (select count(*)  ";
    $sql1 .= "        from matricula ";
    $sql1 .= "	         inner join turma on ed57_i_codigo = ed60_i_turma ";
    ...
    $sql3 = " SELECT ed165_resultado_final as rendimentofinal ";
    $sql3 .= " FROM matricula ";
    ...
    $sql3 .= "  join diarioaluno ON diarioaluno.ed161_turma = turma.ed57_i_codigo and ed161_aluno = matricula.ed60_i_aluno and ed161_serie = matriculaserie.ed221_i_serie ";
    $sql3 .= "  join diarioalunoresultadofinal ON diarioalunoresultadofinal.ed165_diarioaluno = diarioaluno.ed161_codigo ";
