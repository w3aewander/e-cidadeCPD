...
class cl_tabrec
{
    ...
    var $campos = "
                 ...
                 k02_reccredito = int4 = Receita de Crédito
                 ";
    ...
    function atualizacampos($exclusao = false)
    {
        if ($exclusao == false) {
            ...
            $this->k02_reccredito = ($this->k02_reccredito == "" ? @$GLOBALS["HTTP_POST_VARS"]["k02_reccredito"] : $this->k02_reccredito);
        } else {
            $this->k02_codigo = ($this->k02_codigo == "" ? @$GLOBALS["HTTP_POST_VARS"]["k02_codigo"] : $this->k02_codigo);
        }
    }
    ...
    function sql_query_instComplementoFonteRecurso($k02_codigo = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
            $campos_sql = split("#", $campos);
        ...
        $sql .= "             inner join orcreceita on taborc.k02_codrec = orcreceita.o70_codrec
                                                 ...
                                                 " and orcreceita.o70_instit = " . db_getsession("DB_instit");
        $sql .= "             inner join orctiporec on  o70_codigo = o15_codigo ";
        $sql .= "             inner join fonterecurso on  orctiporec_id = o15_codigo ";
        $sql .= "                        and exercicio = o70_anousu ";
        if (USE_PCASP) {
            ...
            $sql .= "  inner join conplano on orcreceita.o70_anousu=conplano.c60_anousu
                                          and orcreceita.o70_codfon = conplano.c60_codcon ";
        }
        $sql .= "        where k02_anousu = " . db_getsession("DB_anousu");
        $sql .= "        union";
        $sql .= "        select k02_estpla,
                                tabrec.k02_codigo,
                                tabrec.k02_tipo,
                                tabrec.k02_descr,
                                tabrec.k02_drecei,
        ...
        $sql .= "             inner join conplanoreduz on conplanoreduz.c61_anousu = tabplan.k02_anousu
                                                      and conplanoreduz.c61_reduz = tabplan.k02_reduz
                                                      and conplanoreduz.c61_instit = " . db_getsession("DB_instit");
        $sql .= "             inner join conplano on conplanoreduz.c61_anousu = conplano.c60_anousu
                                                 and conplanoreduz.c61_codcon = conplano.c60_codcon ";
        $sql .= "             inner join orctiporec on  c61_codigo = o15_codigo ";
        $sql .= "             inner join fonterecurso on  orctiporec_id = o15_codigo ";
        $sql .= "                        and exercicio = c61_anousu ";
        $sql .= "        where k02_anousu = " . db_getsession("DB_anousu");
        $sql .= "      ) as tabrec";
        $sql2 = "";
        if ($dbwhere == "") {
        ...
        $sql2 .= ($sql2 != "" ? " and " : " where ") . " (k02_limite is null or k02_limite >= '" . date('Y-m-d',
    ...
    function sql_query_inst_depto_instituicao(
        $k02_codigo = null,
        $campos = "*",
        $ordem = null,
        $dbwhere = "",
        ...
        $sql .= " from ( select k02_estorc,tabrec.*,o70_codrec,0 as c61_reduz,o70_codigo as recurso, ";
        $sql .= "               case when k79_arretipo is not null then k79_arretipo ";
        $sql .= "                    else (select k03_reciboprot from numpref where k03_anousu = " . db_getsession("DB_anousu") . " and k03_instit = " . $iInstituicao . " )";
        $sql .= "               end as arretipo ";
        $sql .= "        from tabrec ";
        $sql .= "             inner join taborc        on tabrec.k02_codigo      = taborc.k02_codigo and taborc.k02_anousu = " . db_getsession("DB_anousu");
        $sql .= "             inner join orcreceita    on taborc.k02_codrec      = orcreceita.o70_codrec and orcreceita.o70_anousu = " . db_getsession("DB_anousu") . " and orcreceita.o70_instit = " . $iInstituicao;
        $sql .= "             left join tabrecdepto    on tabrecdepto.k98_receit = tabrec.k02_codigo ";
        $sql .= "             left join tabrecarretipo on k79_receit             = tabrec.k02_codigo ";
        $sql .= "        where k02_anousu = " . db_getsession("DB_anousu");
        $sql .= "        union";
        ...
        $sql .= "        from tabrec ";
        $sql .= "             inner join tabplan       on tabrec.k02_codigo         = tabplan.k02_codigo and tabplan.k02_anousu = " . db_getsession("DB_anousu");
        $sql .= "             inner join conplanoreduz on   conplanoreduz.c61_anousu= tabplan.k02_anousu and conplanoreduz.c61_reduz = tabplan.k02_reduz and conplanoreduz.c61_instit = " . $iInstituicao;
        $sql .= "             left join tabrecdepto    on tabrecdepto.k98_receit    = tabrec.k02_codigo ";
        $sql .= "             left join tabrecarretipo on k79_receit                = tabrec.k02_codigo ";
        $sql .= "        where k02_anousu = " . db_getsession("DB_anousu");
        $sql .= "      ) as tabrec";
    ...
    function sql_query_conta_contabil($ano, $instituicao, $campos = "*", $where = "", $orderby = '')
    {

        $sql = "select ";
        $sql .= $campos;
        $sql .= "        from tabrec ";
        $sql .= "             inner join taborc on tabrec.k02_codigo = taborc.k02_codigo and taborc.k02_anousu = {$ano}";
        $sql .= "             inner join orcreceita on taborc.k02_codrec = orcreceita.o70_codrec and orcreceita.o70_anousu = {$ano} ";
        $sql .= "                     and orcreceita.o70_instit = {$instituicao}";

        $sql .= "  inner join conplanoorcamento on orcreceita.o70_anousu = conplanoorcamento.c60_anousu ";
        $sql .= "                              and orcreceita.o70_codfon = conplanoorcamento.c60_codcon ";

        $sql .= "  inner join conplanoconplanoorcamento on c72_anousu = conplanoorcamento.c60_anousu ";
        $sql .= "                                      and c72_conplanoorcamento = conplanoorcamento.c60_codcon ";
        $sql .= "  inner join conplano                 on  c72_anousu = conplano.c60_anousu ";
        $sql .= "                                      and c72_conplano = conplano.c60_codcon ";
        $sql .= "  inner join conplanoreduz             on conplano.c60_anousu  = conplanoreduz.c61_anousu";
        $sql .= "                                      and conplano.c60_codcon  = conplanoreduz.c61_codcon";
        $sql .= "                                      and conplanoreduz.c61_instit = {$instituicao} ";
    ...
    public function sql_filtroReceitas($campos = ['*'], $where = [], $order = ['k02_estorc'])
    {
        ...
        select {$campos}
          from (
                ...
                select k02_estorc,
                       ...
                       o15_recurso ,
                       case
                         when k79_arretipo is not null
                           then k79_arretipo
                         else (select k03_reciboprot
                                 from numpref
                                where k03_anousu = {$ano}
                                  and k03_instit = {$instituicao}
                              )
                       end as dl_Tipo_Debito
                 from tabrec
                 ...
                 join orcreceita on taborc.k02_codrec = orcreceita.o70_codrec
                                and orcreceita.o70_anousu = {$ano}
                                and orcreceita.o70_instit = {$instituicao}
                 ...
                 join fonterecurso on fonterecurso.orctiporec_id = orctiporec.o15_codigo
                      and fonterecurso.exercicio = taborc.k02_anousu
                 left join tabrecdepto on tabrecdepto.k98_receit = tabrec.k02_codigo
                 left join tabrecarretipo on k79_receit = tabrec.k02_codigo
                where k02_anousu = {$ano}
                  and (k98_coddepto = $depto or k98_receit is null )
                ...
                select k02_estpla,
                   ...
                   join conplanoreduz on conplanoreduz.c61_anousu= tabplan.k02_anousu
                              and conplanoreduz.c61_reduz = tabplan.k02_reduz
                              and conplanoreduz.c61_instit = {$instituicao}
                   join orctiporec on orctiporec.o15_codigo = conplanoreduz.c61_codigo
                   join fonterecurso on fonterecurso.orctiporec_id = orctiporec.o15_codigo
                      and fonterecurso.exercicio = tabplan.k02_anousu
                   left join tabrecdepto    on tabrecdepto.k98_receit    = tabrec.k02_codigo
                   left join tabrecarretipo on k79_receit                = tabrec.k02_codigo
                   where k02_anousu = {$ano}
                     and (k98_coddepto = $depto or k98_receit is null )
             ) as dados
           left join arretipo on  arretipo.k00_tipo = dados.dl_Tipo_Debito
        ";
