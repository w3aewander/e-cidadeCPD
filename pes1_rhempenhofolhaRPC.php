...
try {
    ...
    switch ($oPost->sMethod) {
        ...
        case 'consultarDadosGeracaoSLIP':
            ...
            $sCamposSlip = "
                distinct rh79_recurso as recurso,
                gestao as fonte_recurso,
                o15_recurso as subrecurso,
                descricao as descricao_recurso,
                o200_descricao as complemento,
                c61_reduz as conta_debito,
                rh41_conta as conta_credito,
                sum(rh73_valor ) as valor,
                rh79_sequencial as folhaslip,
                e48_cgm as cgm,
                e21_sequencial as retencao,
                rh79_concarpeculiar as concarpeculiar
            ";

            $sGroupSlip = "
                group by rh79_recurso,
                gestao,
                o15_recurso,
                descricao,
                o200_descricao,
            ...
            $sql = $clrhslipfolha->sql_query_slip(null, $sCamposSlip, null, $sWhereSlip);

            $rsSlips = $clrhslipfolha->sql_record($sql);
            $aListaSlips = db_utils::getCollectionByRecord($rsSlips, false, false, true);

            if (SaltesHelper::usaContaExtraPlanilhaSlipFolha(db_getsession("DB_instit"))) {
                foreach ($aListaSlips as $lista) {
                    $lista->conta_credito = SaltesHelper::getContaPlanilhaSlipFolha(
                        $lista->conta_credito,
                        db_getsession("DB_instit")
                    );
                }
            ...
            $lLiberada = $oDadosEmpenhoFolha->isLiberada(
                $oPost->sSigla,
                1,
                $oPost->iAnoFolha,
                $oPost->iMesFolha,
        ...
        case 'geraSLIP':
            ...
            foreach ($aSlips as $slip) {
                $oSlip = new stdClass();
                $oSlip->iRecurso = $slip->recurso;
                $oSlip->iContaDebito = $slip->conta_debito;
                $oSlip->iContaCredito = $slip->conta_credito;
                $oSlip->nValor = $slip->valor;
                $oSlip->iCodFolhaSlip = $slip->folhaslip;
                $oSlip->iNumCgm = $slip->cgm;
                $oSlip->iRetencao = $slip->retencao;

                $sRecursoFolha = sprintf(
                    "%s - %s - %s",
                    $slip->fonte_recurso,
                    db_stdClass::normalizeStringJsonEscapeString($slip->descricao_recurso),
        ...
        case 'anularEmpenho':
            ...
            foreach ($aSiglas as $sSigla) {
                ...
                if ($oPost->sSigla == 'sup' && $oPost->iTipo == 1) {
                    $sSqlEmpenhos .= "                                        and rh72_seqcompl = {$oPost->sSemestre_} ";
                }

                if ($oPost->sSigla == 'r48' && $oPost->iTipo == 1) {
                    $sSqlEmpenhos .= "                                      and rh72_seqcompl = {$oPost->sSemestre_} ";
                }
