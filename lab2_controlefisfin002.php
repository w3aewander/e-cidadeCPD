...
if ($iTpcontrole == ControleFisicoFinanceiroHelper::CONTROLE_DEPARTAMENTO) {
...
} elseif ($iTpcontrole == ControleFisicoFinanceiroHelper::CONTROLE_LABORATORIO) {
...
} elseif ($iTpcontrole == ControleFisicoFinanceiroHelper::CONTROLE_GRUPO_EXAME) {
...
} elseif ($iTpcontrole == ControleFisicoFinanceiroHelper::CONTROLE_EXAME) {
...
try {
    $sSql = $oDaoLabControleFisFin->sql_query_controle(null, $sCampos, "{$sOrder}, dtinicio desc", $sWhere);
    $rs = db_query($sSql);
...
function valorUtilizado($oDados, $periodoInicial, $periodoFinal)
{
    if ($oDados->iteto == 1) {
        $sSql = 'SELECT count(*) AS valor FROM lab_autoriza AS lat ';
    } else {
        $sum = "sum((prc.sd63_f_sh+prc.sd63_f_sa+prc.sd63_f_sp+la53_n_acrescimo)* lri.la21_i_quantidade)";
        $sSql = "SELECT coalesce({$sum}, 0) AS valor FROM lab_autoriza AS lat ";
    }
    ...
    $sWhere = ' where ';
    $sWhere .= " lat.la48_d_data between '{$periodoInicial}' and '{$periodoFinal}' ";
...
for ($iInd = 0; $iInd < $iLinhas; $iInd++) {
    ...
    if (($oPdf->GetY() > $oPdf->getH() - 25) || $lQuebra == true) {
    ...
    $valor = valorUtilizado($oDados, $dDataIniDB, $dDataFimDB);
    $fTotal += $valor;
    ...
    $oPdf->cell(20, $iTam, number_format($valor, 2, ',', '.'), 1, 1, 'R', $lCor);
}
