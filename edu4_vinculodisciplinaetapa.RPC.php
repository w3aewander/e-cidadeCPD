...
$oParam             = isset($_POST['json']) ? $oJson->decode(str_replace("\\", "", $_POST["json"])) : (object)$_POST;
...
try {
    switch ($oParam->exec) {
        ...
        case 'getDisciplinasVinculadasEtapaBase':
            ...
            $sCampos .= " ed34_lancarhistorico, ed34_disiciplinaglobalizada, ed34_caracterreprobatorio, ed34_basecomum, ed34_tipobase, ";
            ...
            $sSqlBaseMps = $oDaoBaseMps->sql_query_areaconhecimento(null, $sCampos, $sOrdem, $sWhere);
            ...
            for ($i = 0; $i < $iLinhas; $i++) {
                ...
                if (!is_null($oDados->ed34_tipobase) && !empty($oDados->ed34_tipobase)) {
                    $sql = "select ed182_id, ed182_descricao from tipobase where ed182_id = {$oDados->ed34_tipobase}";
                ...
                $oDisciplina->codigoAreaConhecimento = $oDados->ed293_sequencial;
                $oDisciplina->descricaoAreaConhecimento = urlencode($oDados->ed293_descr);
                $oDisciplina->iTipoBase = isset($tipoBase) ? (object)[
                    'ed182_descricao' => urlencode($tipoBase['ed182_descricao']),
                    'ed182_id'=> urlencode($tipoBase['ed182_id'])
                ] : (object)['ed182_descricao' => urlencode('Não Cadastrado')];
        ...
        case 'getDisciplinasVinculadasEtapaTurma':
            ...
            {
                if (!is_null($oRegencia->getTipoBase()) && !empty($oRegencia->getTipoBase())) {
                    $sql = "select ed182_id, ed182_descricao from tipobase where ed182_id = {$oRegencia->getTipoBase()}";
                    $resource = db_query($sql);
                    $tipoBase = pg_fetch_assoc($resource);
                }
                ...
                $oDisciplina->descricaoAreaConhecimento = '';
                if (!is_null($areaConhecimento)) {
                    $oDisciplina->descricaoAreaConhecimento = urlencode($areaConhecimento->getDescricao());
                    $oDisciplina->codigoAreaConhecimento =  $areaConhecimento->getCodigo();
                }
        ...
        case 'salvarDisciplinaTurma':
            ...
            if (salvarDisciplinaTurma($oParam)) {
                ...
                $mensagem .= "Atualize também o Diário de Classe desta turma em:\n";
                $mensagem .= "Procedimetos>Diário de Classe>Lançamento por turma";
                $oRetorno->sMessage = urlencode($mensagem);
            }
            excluirDiarioareaSemRegencia();
            db_fim_transacao();
            break;
        ...
        case 'atualizarBase':
            ...
            $sCampos = "basemps.*, ed232_c_descr, ed31_c_contrfreq";
         ...
         */
            foreach ($aDisciplinasBase as $oDadosDisciplinaBase) {
                ...
                // Atualiza regências
                    for ($i = 0; $i < $iLinhasRegencia; $i++) {
                        ...
                        $oRegencia->setTipoBase($oDadosDisciplinaBase->ed34_tipobase);
                        $oRegencia->setUnidadeCurricular($oDadosDisciplinaBase->ed34_unidade_crurricular);
                        ...
                        if ($oRegencia->getProcedimentoAvaliacao()->getCodigo() != $oProcedimentoAvalicao->getCodigo()) {
                            $oProcedimentoAvalicao = $oRegencia->getProcedimentoAvaliacao();
                        }

                        $oRegencia->setProcedimentoAvaliacao($oProcedimentoAvalicao);
                        if (!empty($oDadosDisciplinaBase->ed34_procedimento) &&
                        $oDadosDisciplinaBase->ed34_procedimento != $oProcedimentoAvalicao->getCodigo()) {
                            $oRegencia->excluirVinculoDiario();
                            $oProcedimentoAvaliacao = ProcedimentoAvaliacaoRepository::getProcedimentoByCodigo(
                                $oDadosDisciplinaBase->ed34_procedimento
                            );
                        ...
                        $oRegencia->setAreaConhecimento(null);
                        if (!empty($oDadosDisciplinaBase->ed34_areaconhecimento)) {
                            $oRegencia->setAreaConhecimento(
                                AreaConhecimentoRegistry::get($oDadosDisciplinaBase->ed34_areaconhecimento)
                            );
                        }
                        $oRegencia->salvar();
              ...
              // Insere nova regencia
                if (!$lEncontrouDisciplina) {
                    ...
                    $oNovaRegencia->setAreaConhecimento(null);
                    if (!empty($oDadosDisciplinaBase->ed34_areaconhecimento)) {
                        $oNovaRegencia->setAreaConhecimento(
                            AreaConhecimentoRegistry::get($oDadosDisciplinaBase->ed34_areaconhecimento)
                        );
                    }
            ...
            db_fim_transacao();
            $mensagem = "Disciplina atualizada pela base.\n";
            $mensagem .= "Atualize também o Diário de Classe desta turma em:\n";
            $mensagem .= "Procedimetos>Diário de Classe>Lançamento por turma";
            $oRetorno->sMessage = urlencode($mensagem);
            break;
        ...
        case 'reordenarDisciplinasTurma':
            ...
            foreach ($oParam->aDisciplinasComumOrdenada as $oOrdem) {
                unset($_POST["ed59_lancarhistorico"]) ;
                unset($_POST["ed59_caracterreprobatorio"]);
        ...
        case 'reordenarDisciplinasBase':
            ...
            $oDaoBaseMps = new cl_basemps();
            foreach ($oParam->aDisciplinasComumOrdenada as $oOrdem) {
                unset($_POST["ed34_caracterreprobatorio"]);
                unset($_POST["ed34_lancarhistorico"]);
...
function salvarDisciplinaTurma($oParam)
{
    ...
    $base = $oTurma->getBaseCurricular();
    $disicpinasBase = $base->getDisciplina($oEtapa);

    $unidadeCurricular = null;
    foreach ($disicpinasBase as $disciplinaBase) {
        if ($disciplinaBase->getCodigoDisciplina() === $oParam->iDisicplina) {
            $unidadeCurricular = $disciplinaBase->unidadeCurricular;
        }
    }
    ...
    $oRegencia->setBaseComum($oParam->lTipoBase == 1);
    $oRegencia->setTipoBase($oParam->lTipoBase);
    $oRegencia->setControleFrequencia($oParam->sTipoControleFrequencia);
    $oRegencia->setDataAtualizacao(new DBDate(date(DBDate::DATA_EN)));
    $oRegencia->setUnidadeCurricular($unidadeCurricular);

    if (!empty($oParam->areaConhecimento)) {
        $oRegencia->setAreaConhecimento(AreaConhecimentoRegistry::get($oParam->areaConhecimento));
    }
...
function salvarDisciplinaBase($oParam)
{
    ...
    $oDaoBaseMps->ed34_basecomum              = $oParam->lTipoBase == 1 ? 'true' : 'false';
    $oDaoBaseMps->ed34_areaconhecimento       = $oParam->areaConhecimento;
    $oDaoBaseMps->ed34_tipobase              = $oParam->lTipoBase;

    db_inicio_transacao();
    if (empty($oParam->iCodigoVinculo)) {
        $rs = db_query("select count(*) from basemps where ed34_i_base = {$oParam->iBase} and ed34_i_serie = {$oParam->iEtapa}");
        $ordem = db_utils::fieldsMemory($rs, 0)->count + 1;
        $oDaoBaseMps->ed34_i_ordenacao = $ordem;
...
function excluirDiarioareaSemRegencia()
{
    //corrigir inconsistencs
    $sqlDeletarDiarioarea = "
      with deletar as (select ed162_codigo
              from diarioaluno
                         join diarioarea on ed162_diarioaluno = ed161_codigo
                where not exists (select 1
                        from regencia
                        where ed161_turma = ed59_i_turma
                          and ed161_serie = ed59_i_serie
                          and ed59_areaconhecimento = ed162_areaconhecimento
                    )) delete from diarioarea using deletar where diarioarea.ed162_codigo = deletar.ed162_codigo;
                    ";
    $rs = db_query($sqlDeletarDiarioarea);
    if (!$rs) {
        throw new Exception("Erro ao excluir areas de Conhecimento não utilizadas.");
    }
}
