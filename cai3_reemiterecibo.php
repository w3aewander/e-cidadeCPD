...
$DadosInstit = db_query("select cgc,
                                db12_uf,
                                db12_extenso,
                                nomeinst,
                                ender,
...
$oDadosInstit = db_utils::fieldsMemory($DadosInstit, 0);

//cria codigo de barras e linha digitável
$NumBanco     = $oDadosInstit->numbanco;
$taxabancaria = $oDadosInstit->tx_banc;
$src          = $oDadosInstit->logo;
$db_nomeinst  = $oDadosInstit->nomeinst;
$db_ender     = $oDadosInstit->ender;
$db_munic     = $oDadosInstit->munic;
$db_uf        = $oDadosInstit->uf;
$db_telef     = $oDadosInstit->telef;
$db_email     = $oDadosInstit->email;
$db_logo      = $oDadosInstit->logo;
$cgc_pref     = $oDadosInstit->cgc;
...
if(!empty($_POST["ver_matric"]) || $matricularecibo > 0 ) {
  ...
  $Identificacao = db_query("select z01_numcgm,
                                    z01_nome,
                                    z01_ender,
                                    z01_numero,
                                    z01_compl,
    ...
    $sqlhist = "select *
                 from (
                        select distinct v59_inicial,
                               case
                                 when divida.v01_exerc is null then
                                 case
                                   when divida2.v01_exerc is null then 0
