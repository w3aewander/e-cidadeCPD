...
if (empty($this->cep_munic)) {
    $this->cep_munic = $oDadosInstit->cep;
}
...
$this->objpdf->text($xcol + 90, $xlin + 19,
  db_formatar(@$this->cgccpf, (strlen(@$this->cgccpf) < 12 ? 'cpf' : 'cnpj')));
$this->objpdf->Setfont('Arial', '', 6);
...
$this->objpdf->text($xcol + 143, $xlin + 15,
  $this->nrpri . (isset($this->complpri) && $this->complpri != "" ? " / " . $this->complpri : ""));
...
$this->objpdf->setx($xcol + 4 + $maiscol);
$this->objpdf->cell(8, 3, "Rec", 0, 0, "L", 0);
$this->objpdf->cell(7, 3, "Reduz", 0, 0, "L", 0);
$this->objpdf->cell(64, 3, "Descrição", 0, 0, "L", 0);
$this->objpdf->cell(12, 3, "Valor", 0, 1, "R", 0);
...
for ($x = 0; $x < $intnumrows; $x++) {
    ...
    $this->objpdf->SetY($yline + 28);
    $this->objpdf->Text($reccol, $bklin, $this->arraycodreceitas[$x]);
    $this->objpdf->Text($reccol + 8, $bklin, "(" . $this->arrayreduzreceitas[$x] . ")");
    if (@$this->arraycodhist[$x] == 918) {
        $this->obsdescr = " (desconto)";
    }
    $this->objpdf->Text($reccol + 15, $bklin, $this->arraydescrreceitas[$x] . $this->obsdescr);
    ...
    if ($iFormaCorrecao == 1) {
        ...
        } elseif (@$this->arraycodtipo[$x] == 't' and $this->arrayvalreceitas[$x] > 0 and @$this->arraycodhist[$x] != 918) {
        ...
        if ($this->arraycodhist[$x] == 918) {
            $this->totaldesc += $this->arrayvalreceitas[$x];
        } elseif (@$this->arraycodtipo[$x] == 't' and $this->arrayvalreceitas[$x] > 0) {
            $this->totalacres += $this->arrayvalreceitas[$x];
        } else {
            $this->totalrec += $this->arrayvalreceitas[$x];
        }
...
$sInstrucao = $this->descr12_1;
$this->objpdf->multicell(155, 4, 'HISTÓRICO :   ' . "\n" . $sInstrucao);
if(!empty($this->refantmatric)) {
    $this->objpdf->ln();
    $this->objpdf->cell(40, 3, "Referência anterior: ".$this->refantmatric, 0, 1, "C", 0);
}
if(!empty($this->refantinscr)) {
    $this->objpdf->ln();
    $this->objpdf->cell(22, 3, "Referência anterior: ".$this->refantinscr, 0, 1, "C", 0);
}
