...
switch ($oParam->sExec) {
    ...
    case "incluirInconsistencia" :
        ...
        try {
            ...
            /**
             * Buscamos o ano das matriculas dos alunos selecionados, se o aluno marcado como 'Errado', tiver uma matricula
             * no ano atual, ele não inclui as inconsistencias.
             */
            $oDaoAluno = db_utils::getDao('aluno');
            $sWhere    = "ed47_i_codigo = $oCampo->iSequencialCampo";
            $sCampos   = "MAX(extract(year from ed60_d_datamatricula)) as ano_matricula";
            $sSqlVerificaMatriculaAnoAtual = $oDaoAluno->sql_query_matricula_ano(null, $sCampos, null, null, $sWhere);
            $rsVerificaMatriculaAnoAtual   = $oDaoAluno->sql_record($sSqlVerificaMatriculaAnoAtual);
            $iLinhas   = $oDaoAluno->numrows;
            $anoMatricula = '';
            for ($i = 0; $i < $iLinhas; $i++) {
                $anoMatricula = db_utils::fieldsMemory($rsVerificaMatriculaAnoAtual, $i, false, false, true);
            }
            $anoAtual = date('Y');
            if ($anoAtual == $anoMatricula->ano_matricula) {
                throw new Exception("O Cadastro selecionado como 'Errado' tem matrícula no calendário atual, selecione o outro.");
            }
