...
class cl_matricula
{
    ...
    function alterar($ed60_i_codigo = null)
    {
        ...
        if (trim($this->ed60_d_datamodif) != ""
            || !empty($GLOBALS["HTTP_POST_VARS"]["ed60_d_datamodif_dia"])) {

            if ($this->ed60_d_datamodif == "null" || $this->ed60_d_datamodif == null) {
        ...
        if (trim($this->ed60_d_datamodifant) != ""
            || isset($GLOBALS["HTTP_POST_VARS"]["ed60_d_datamodifant_dia"])
            && ($GLOBALS["HTTP_POST_VARS"]["ed60_d_datamodifant_dia"] != "")) {
            if ($this->ed60_d_datamodifant == "null" || $this->ed60_d_datamodifant == null) {
    ...
    function sql_query_naturalidade_aluno($ed60_i_codigo = null, $campos = "*", $ordem = null, $dbwhere = "", $group = null)
    {
        $sql = "select ";
        if ($campos != "*") {
            $campos_sql = explode("#", $campos);
        ...
        $sql .= " from matricula ";
        $sql .= "      inner join aluno  on  aluno.ed47_i_codigo = matricula.ed60_i_aluno";
        $sql .= "      left JOIN alunocidadao ON aluno.ed47_i_codigo = alunocidadao.ed330_aluno";
        $sql .= "      left join alunocidadaoresponsavel on alunocidadaoresponsavel.ed331_aluno = aluno.ed47_i_codigo";
        $sql .= "      left join cidadao ON cidadao.ov02_sequencial = alunocidadaoresponsavel.ed331_cidadao";
        $sql .="       left JOIN cidadaofiliacao ON alunocidadao.ed330_cidadao = cidadaofiliacao.ov29_cidadao";
        $sql .="       left JOIN cidadao cidadao2 ON cidadao2.ov02_sequencial = cidadaofiliacao.ov29_cidadaovinculo";
        $sql .= "      left join pais   on pais.ed228_i_codigo = aluno.ed47_i_pais";
        $sql .= "      inner join turma  on  turma.ed57_i_codigo = matricula.ed60_i_turma";
        $sql .= "      inner join escola  on  escola.ed18_i_codigo = turma.ed57_i_escola";
        $sql .= "      left join censouf  on  censouf.ed260_i_codigo = escola.ed18_i_censouf";
    ...
    public function sql_query_excportacao_alunos_censo($where = array(), $ordem = array())
    {
        $where = !empty($where) ? " where " . implode(' and ', $where) : null;
        $ordem = !empty($ordem) ? " order by " . implode(' and ', $ordem) : null;

        $sql = "
        with matricula_enviada_censo as (
          select min(ed60_i_codigo) as matricula, ed60_i_aluno
            from matricula
            JOIN aluno ON aluno.ed47_i_codigo = matricula.ed60_i_aluno
            JOIN turma ON turma.ed57_i_codigo = matricula.ed60_i_turma
            JOIN escola ON escola.ed18_i_codigo = turma.ed57_i_escola
          ...
          select  distinct
                  ed18_c_codigoinep               as codigo_escola_inep
                 ,ed57_i_codigo                   as codigo_turma_escola
                 ,ed57_i_codigoinep               as codigo_turma_inep
                 ,ed47_c_codigoinep               as codigo_aluno_inep
                 ,ed47_i_codigo                   as codigo_aluno_escola
                 ,ed280_i_matcenso                as codigo_matricula_inep
                 ,ed10_mediacaodidaticopedagogica as mediacao_didatico_pedagogico
                 ,ed10_i_tipoensino               as modalidade
                 ,ed133_censoetapa                as etapa
                 ,ed60_i_codigo                   as codigo_matricula
                 ,ed132_censoetapa                as etapa_turma
                 ,ed60_c_situacao                 as situacao_matricula
            from matricula_enviada_censo
            join matricula on matricula.ed60_i_codigo =  matricula_enviada_censo.matricula
            join aluno on aluno.ed47_i_codigo = matricula.ed60_i_aluno
            join turma on turma.ed57_i_codigo = matricula.ed60_i_turma
            join escola on escola.ed18_i_codigo = turma.ed57_i_escola
            join calendario on calendario.ed52_i_codigo = turma.ed57_i_calendario
            join base on base.ed31_i_codigo = turma.ed57_i_base
            join cursoedu on cursoedu.ed29_i_codigo = base.ed31_i_curso
            join ensino on ensino.ed10_i_codigo = cursoedu.ed29_i_ensino
            join matriculaserie on matriculaserie.ed221_i_matricula = matricula.ed60_i_codigo
                               and ed221_c_origem = 'S'
            join serie on serie.ed11_i_codigo = matriculaserie.ed221_i_serie
            join turmacensoetapa on turmacensoetapa.ed132_turma = turma.ed57_i_codigo
            left join alunomatcenso on alunomatcenso.ed280_i_aluno = aluno.ed47_i_codigo
                                   and alunomatcenso.ed280_i_ano = calendario.ed52_i_ano
                                   and alunomatcenso.ed280_i_turmacenso = turma.ed57_i_codigoinep
            join seriecensoetapa on seriecensoetapa.ed133_serie = serie.ed11_i_codigo
                                and seriecensoetapa.ed133_ano = calendario.ed52_i_ano
        ) select * from dados_censo {$ordem}
      ";
        return $sql;
    }
