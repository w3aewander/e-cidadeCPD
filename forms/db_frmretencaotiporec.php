...
<fieldset>
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                ...
                db_selectrecord(
                    ...
                    '',
                    '',
                    "js_mostraCodigoIRRF(this.value)"
                );
                ?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                ...
                db_input(
                    ...
                    'text',
                    $db_opcao,
                    " onchange='js_pesquisae21_receita(false);'"
                );
        ...
        <tr>
            ...
            <td>
                <?php
                db_input('e21_descricao', 44, $Ie21_descricao, true, 'text', $db_opcao, "")
                ?>
            </td>
        </tr>
        <tr>
        ...
        <tr id='codigoirrf'>
            ...
            <td>
                ...
                db_input(
                    ...
                    'text',
                    $db_opcao,
                    "onchange='js_pesquisae21_retencaonatureza(false);'"
                );
        ...
        <tr>
            ...
            <td>
                ...
                db_input(
                    ...
                    'text',
                    $db_opcao,
                    "onchange='js_pesquisae48_cgm(false);'"
                );
        ...
        <tr>
            <td nowrap title="<?=@$Te21_enterecebedor ?>">
                <?=$Le21_enterecebedor?>
            </td>
            <td>
                <?php
                ...
                for ($i= 0; $i < $totalInstituicoes; $i++) {
                    $dadosInstituicao = db_utils::fieldsMemory($rsInstituicoes, $i);
                    $instituicoes[$dadosInstituicao->codigo] = $dadosInstituicao->nomeinst;
                }
                 db_select('e21_enterecebedor', $instituicoes, true, $db_opcao, "style='width:320' onchange='mostrarReceitaEnte();'")
                ?>
            </td>
        </tr>
        <tr id="trReceitaEnteRecebedor" style="display: none">
            <td nowrap title="<?=@$Te21_receitaenterecebedor ?>">
                <?php
                db_ancora(@$Le21_receitaenterecebedor, "js_pesquisae21_receitaenterecebedor(true);", $db_opcao);
                ?>
            </td>
            <td>
                <?php
                db_input(
                    ...
                    10,
                    $Ie21_receitaenterecebedor,
                    true,
                    'text',
                    $db_opcao,
                    " onchange='js_pesquisae21_receitaenterecebedor(false);'"
                );
                db_input('k02_descrrecebedora', 30, $Ik02_descr, true, 'text', 3, '');
                ?>
            </td>
        </tr>
        <tr>
            <td nowrap title="<?=@$Te21_ativo ?>">
                <?=$Le21_ativo?>
            </td>
            <td>
                <?php
                 db_select('e21_ativo', ["t"=>"SIM", "f"=>"NÃO"], true, $db_opcao, "style='width:320'");
                ?>
            </td>
        </tr>        
        ...
        <tr>
            ...
            <td>
                <select id="tipor" name="tipor">
                    ...
                    <option value="1" <?=($e21_tiporet == 1) ? 'selected' : ''?>>Pensão</option>
                    <option value="2" <?=($e21_tiporet == 2) ? 'selected' : ''?>>Empréstimos</option>
                    <option value="3" <?=($e21_tiporet == 3) ? 'selected' : ''?>>Contribuições Previdenciárias</option>
                    <option value="4" <?=($e21_tiporet == 4) ? 'selected' : ''?>>Outras Contribuições</option>
                    <option value="5" <?=($e21_tiporet == 5) ? 'selected' : ''?>>Outras Consignações</option>
                    <option value="6" <?=($e21_tiporet == 6) ? 'selected' : ''?>>Imposto de Renda Retido na Fonte (IRRF)</option>
                    <option value="7" <?=($e21_tiporet == 7) ? 'selected' : ''?>>Imposto Sobre Serviços Retido na Fonte (ISS)</option>
                    <option value="8" <?=($e21_tiporet == 8) ? 'selected' : ''?>>Outras Retenções</option>
                    <option value="9" <?=($e21_tiporet == 9) ? 'selected' : ''?>>Rúbricas Folha</option>
                </select>
            </td>
        </tr>
    </table>
    ...
    <input name="<?=($db_opcao == 1 ? "incluir" : ($db_opcao == 2 || $db_opcao == 22 ? "alterar" : "excluir")) ?>"
           onclick="return js_validaCadastro()"
           type="submit" id="db_opcao"
           value="<?=($db_opcao == 1 ? "Incluir" : ($db_opcao == 2 || $db_opcao == 22 ? "Alterar" : "Excluir")) ?>" <?=($db_botao == false ? "disabled" : "")?> >
    <input name="pesquisar" type="button" id="pesquisar" value="Pesquisar" onclick="js_pesquisa();">
</form>
<script>
var enteRecebedor = document.getElementById('e21_enterecebedor');
var receitaEnte   = document.getElementById('e21_receitaenterecebedor');
var tipoRetencao   = document.getElementById('tipor');
function js_validaCadastro() {
    ...
    if (enteRecebedor.value != '' && receitaEnte.value == '') {

        alert('Para retenções que possuem Ente Recebedor, é obrigatório informar qual receita de destino da retenção.');
        return false;
    }

    if (tipoRetencao.value == 0 || tipoRetencao.value == '') {
        alert('É obrigatório informar o tipo da retenção.');
        return false;
    }
...
function mostrarReceitaEnte()
{
    document.getElementById('trReceitaEnteRecebedor').style.display='none';
    if (enteRecebedor.value != '') {
        document.getElementById('trReceitaEnteRecebedor').style.display='';
    }
}
...
function js_pesquisae21_receitaenterecebedor(mostra) {

    var tipo_calculo = $F('e21_retencaotipocalc');

    if (mostra == true) {
        js_OpenJanelaIframe('CurrentWindow.corpo', 'db_iframe_tabrec', 'func_tabrecinstituicao.php?funcao_js=parent.js_mostratabrecrecebedora1|k02_codigo|k02_descr&instituicao='+$F('e21_enterecebedor')+'&tipocalculo='+tipo_calculo, 'Pesquisa', true);
    } else {
        if (document.form1.e21_receita.value != '') {
            js_OpenJanelaIframe('CurrentWindow.corpo', 'db_iframe_tabrec', 'func_tabrecinstituicao.php?pesquisa_chave=' + document.form1.e21_receita.value + '&funcao_js=parent.js_mostratabrecrecebedora&instituicao='+$F('e21_enterecebedor')+'&tipocalculo='+tipo_calculo, 'Pesquisa', false);
        } else {
            document.form1.k02_descr.value = '';
        }
    }
...
function js_mostratabrecrecebedora(chave, erro) {
    document.form1.k02_descrrecebedora.value = chave;
    if (erro == true) {
        document.form1.e21_receitaenterecebedor.focus();
        document.form1.e21_receitaenterecebedor.value = '';
    }
}

function js_mostratabrecrecebedora1(chave1, chave2) {
    document.form1.e21_receitaenterecebedor.value = chave1;
    document.form1.k02_descrrecebedora.value = chave2;
    db_iframe_tabrec.hide();
}
