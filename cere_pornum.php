...
function busca($sequencial){
  $sql = pg_query("SELECT id, e60_numemp, noempenho, nocertificado, e50_codord, CASE WHEN cseq2 = 0 THEN 'Não' ELSE 'Sim' END as retiradooc, e60_instit, CASE WHEN excluido = 1 THEN 'Sim' ELSE 'Não' END as excluido FROM certificacaoconformidade WHERE noempenho = '{$sequencial}' AND e50_codord is not null AND cseq2 = 0 ORDER BY e60_instit, id");
