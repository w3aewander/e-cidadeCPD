...
class M7987PlParcelamento extends PostgresMigration
{
    public function up()
    {
        $this->execute(<<<SQL
...
SQL
      ...
      $this->execute(<<<SQL
             drop function fc_parc_criatemptable(boolean);
create or replace function fc_parc_criatemptable(boolean) returns boolean as
$$
declare
  bRaise alias for $1;

  lTabelasCriadas boolean default true;
  nome name;
begin
  ...
  if bRaise then
    raise notice 'CRIANDO TABELAS TEMPORARIAS ...';
  end if;

  begin

  /*
   * NAO REMOVER CAMPOS DESSAS TABELAS, ESSA ALTERACAO PODE CAUSAR PROBLEMAS NA FUNCAO DE PARCELAMENTO
   * QUANDO USAR AS TABELAS TEMPORARIAS NAO USE SELECT * INTO VAI1, VAR2,VAR3 FROM XXX. USE: SELECT CAMPO1,CAMPO2,CAMPO3 INTO  VAR1, VAR2,VAR3 FROM XXXX.
   */
  create temporary table vcto ("data" date);

  create temporary table parcelas ( "parcela"    integer,
                                    "receit"     integer,
                                    "receitaori" integer,
                                    "hist"       integer,
                                    "valor"      float8 default 0,
                                    "valprop"    float8 default 0,
                                    "valhis"     float8 default 0,
                                    "valcor"     float8 default 0,
                                    "valjur"     float8 default 0,
                                    "valmul"     float8 default 0,
                                    "descor"     float8 default 0,
                                    "descjur"    float8 default 0,
                                    "descmul"    float8 default 0
                                  );

  create temporary table totrec ( "receit"  integer,
                                  "parcela" integer,
                                  "valor"   float8
                                );

  create temporary table arrecad_parc_rec ( "numpre"     integer,
                                            "numpar"     integer,
                                            "receit"     integer,
                                            "tipo"       integer,
  ...
  create index arrecad_parc_rec_npr_receit_in on arrecad_parc_rec using btree (numpre, numpar, receit);

  create temporary table arrecad_parc_rec_perc ( "numpre"     integer,
                                                 "numpar"     integer,
                                                 "receit"     integer,
                                                 "matric"     integer,
                                                 "percmatric" float8 default 0,
  ...
  exception
    when duplicate_table then
      ...
      truncate parcelas;
      truncate totrec;
      truncate arrecad_parc_rec;
      truncate arrecad_parc_rec_perc;
  end;

  return lTabelasCriadas;
...
-- 6 = tentando parcelar mais de um numpre (debito)
    ...
    lRaise  := ( case when fc_getsession('DB_debugon') is null or fc_getsession('DB_debugon') = '' then false else true end );
    if lRaise is true then
      perform fc_debug('Processando parcelamento dos d\E9bitos...',lRaise,true,false);
    end if;
    ...
    iInstit := cast(fc_getsession('DB_instit') as integer);
    if iInstit is null then
       raise exception 'Variavel de sess\E3o [DB_instit] n\E3o encontrada.';
    end if;

    iAnousu := cast(fc_getsession('DB_anousu') as integer);
    if iAnousu is null then
       raise exception 'Variavel de sess\E3o [DB_anousu] n\E3o encontrada.';
    end if;

    dDataUsu := cast(fc_getsession('DB_datausu') as date);
    if dDataUsu is null then
       raise exception 'Variavel de sess\E3o [DB_datausu] n\E3o encontrada.';
    end if;
    ...
    -- se houver d\E9bitos do tipo Inicial verificamos se o par\E2metro do M\F3d. Juridio PARTILHA est\E1 ativo "SIM" e
    -- caso exista mais de um processo para as iniciais bloqueamos o parcelamento.
    if lValidaParcInicial is true then
       ...
       if found then
          ...
          if found then
            return '[2] - N\E3o \E9 poss\EDvel parcelar iniciais com processos do foro diferentes para um mesmo parcelamento! [Utiliza\E7\E3o de Partilha Ativada]';
          end if;
    ...
    -- Desativado par\E2metro para que n\E3o seja gerado registros na incorpora\E7\E3o tribut\E1ria
    perform fc_putsession('DB_utiliza_incorporacao','false');

    -- funcao que corrige o arrecad no caso de encontrar registros duplicados(numpre,numpar,receit)
    perform fc_corrigeparcelamento();

    -- Ativado par\E2metro para que continue sendo gerado registros na incorpora\E7\E3o tribut\E1ria
    perform fc_putsession('DB_utiliza_incorporacao','true');
    ...
    loop
      ...
      if lRaise is true then
        perform fc_debug('Verificando quantidades de parcelaas marcadas com a quantidade de parcelas do d\E9bito: v_somar1: '||v_somar1||' - v_somar2: '||v_somar2,lRaise,false,false);
      end if;
            -- compara
      if v_somar1 <> v_somar2 then
        return '[5] - Todas as parcelas do parcelamento ' || v_record_origem.v07_parcel || ' devem ser marcadas!';
    ...
    if v_parcinicial is true then
      ...
      if lRaise is true then
        perform fc_debug(' ',lRaise,false,false);
        perform fc_debug(' ',lRaise,false,false);
        perform fc_debug('Buscando CGM princial por matricula ou inscri\E7\E3o',lRaise,false,false);
        perform fc_debug(' ',lRaise,false,false);
      end if;
    ...
    loop
      ...
      if v_record_recpar.tipo_origem is null then
         return '[6] - N\E3o encontrados registros na tabela Divida para um dos debitos que esta sendo parcelado.';
      end if;
    ...
    loop
      ...
      if v_valparc > 0 then
        ...
        if lSeparaJuroMulta = 1 then
          ...
          --
          --
          -- Caso seja a ultima parcela do parcelamento realizamos a verifica\E7\E3o nos valores gerados para a composi\E7\E3o das parcelas
          -- Se encontrar alguma diferenca \E9 realizado o processamento do ajuste da composicao
          --
          if v_record_recpar.parcela = v_totparcdestarec then
             ...
             --
             -- Verificamos os valores j\E1 gerados para a composicao do d\E9bito somando com o valor que ser\E1 gerado para esta parcela e receita
             --
             select sum(k00_vlrhist)+v_historico_compos,
                    sum(k00_correcao)+v_correcao_compos,
                    sum(k00_juros)+v_juros_compos,
              ...
              --
              -- Verificamos o total do valor de origem do parcelamento sem altera\E7\F5es e aplica\E7\F5es de regra.
              --
              select sum(k00_vlrhis),
                     sum(k00_vlrcor-k00_vlrhis),
                     sum(k00_juros),
              ...
              --
              -- Caso seja encontrada diferenca na composicao do d\E9bito com o total parcelado
              -- Realizamos os ajustes necessarios nos valores onde existem diferenca, se o valor da diferenca existir e n\E3o for maior que 1.
              --
              if abs(nVlrDiferencaComposicaoTotal) between 0.01 and 1.00 then
                 ...
                 --
                 --
                 -- Se a vari\E1vel de sess\E3o db_debugon estiver setada, verificamos os valores finais gerados para a composi\E7\E3o do d\E9bito
                 -- Essa verifica\E7\E3o \E9 realizada buscando os valores j\E1 gerados somando com o valor da receita que ser\E1 cadastrado j\E1 com
                 -- os ajustes de valores.
                 --
                 if lRaise is true then
...
--    raise notice 'valor: % - nValorTotalOrigem: % - PercCalcComRound: %', rPercOrigem.valor, nValorTotalOrigem, nPercCalc ;
    ...
    */
    ...
    if nPercCalc < 0.5 then
       ...
       --
       -- Jogamos a diferen\E7a do percentual na Arreinscr quando:
       --  - N\E3o existir vinculo com matricula
       --  - O percentual da inscri\E7\E3o for menor que o percentual da matricula
       --
       -- Jogamos a difen\E7a do percentual na Arrematric quando:
       -- - N\E3o existir vinculo com a inscri\E7\E3o
       -- - O Percentual da matricula for menor que o percentual da inscri\E7\E3o
       --
      if lRaise then
        perform fc_debug('nPercCalc < 0.5 --------------------- nSomaPercInscr ...: '||nSomaPercInscr ,lRaise,false,false);
        perform fc_debug('nPercCalc < 0.5 --------------------- nSomaPercMatric...: '||nSomaPercMatric,lRaise,false,false);
...
-- return 'final percentual 2';
    ...
    if round( nValidacaoPerc, 2) <> 100.00 then
        ...
        --
        -- Verificamos se o problema \E9 devido estar no parcelamento d\E9bitos que pertencem a matricula/inscri\E7\E3o e d\E9bitos sem vinculo com matricula/inscricao
        -- Se for encontrado um numpre que n\E3o esteja vinculado a matricula e a inscri\E7\E3o na origem, mostramos uma mensagem de erro diferenciada para facilitar a
        -- corre\E7\E3o do caso. Geralmente a corre\E7\E3o \E9 realizada vinculando o numpre a uma matricula ou inscri\E7\E3o.
        --
        select array_to_string(array_accum( distinct arrecad_parc_rec.numpre),',')
          into sNumpreSemVinculoMatricInsc
          from arrecad_parc_rec
         ...
         where arrematric.k00_numpre is null
           and arreinscr.k00_numpre  is null;
        if sNumpreSemVinculoMatricInsc <> '' then
          return '[15] - Inconsistencia no percentual do d\E9bito gerado ap\F3s o processamento do parcelamento - numpre: '||v_numpre||'. - Encontrados numpres que n\E3o possuem vinculo com Matricula/Inscri\E7\E3o. Numpres ['||sNumpreSemVinculoMatricInsc||']';
        else
          return '[15] - Inconsistencia no percentual do d\E9bito gerado ap\F3s o processamento do parcelamento - numpre: '||v_numpre;
        end if;
