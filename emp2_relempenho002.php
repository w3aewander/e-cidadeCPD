...
$sele_work = $clselorcdotacao->getDadosComplemento(false);
...
$campos .= ", o15_codigo, descricao as o15_descr, fc_estruturaldotacao(e60_anousu,e60_coddot) as dl_estrutural, e60_codcom";
...
if ($agrupar == "oo") {
    $ordem = "to_number(e60_codemp::text,'9999999999') , e60_numemp, e60_emiss, z01_nome, e60_anousu";
}
if ($processar == "a") {
    ...
    if ($agrupar == "d" or 1 == 1) {
        $sqlrelemp = "
            select  x.e60_resumo,
               ...
               from ($sqlrelemp) as x
			   join empelemento on x.e60_numemp = e64_numemp  " . $sele_desdobramentos . "
			   join orcelemento on o56_codele = e64_codele and o56_anousu = x.e60_anousu
               group by x.e60_resumo,
                       x.e60_numemp,
                       x.e60_codemp,
                       x.e60_emiss,
                       x.e60_numcgm,
    ...
    if ($clempempenho->numrows > 0 or isParaiba()) {
    ...
    $sqlperiodo = "
				  ...
				  select e60_numemp,
				    ...
				    from (
				       select e60_numemp,
					          ...
					          e60_coddot
				         from empempenho
				        where e60_instit in ($instits) and e60_emiss between '$datacredor' and '$datacredor1'
				      ) as xxx
        ...
        inner join origemcomplementorecurso on origemcomplementorecurso.o206_numero = empempenho.e60_numemp
                   and  origemcomplementorecurso.o206_origem = 1
        inner join orctiporec on orctiporec.o15_codigo = origemcomplementorecurso.o206_recurso
    ...
    if ($listaitem != "") {
        $sqlperiodo .= "
        group by empempenho.e60_numemp,  empempenho.e60_resumo, empempenho.e60_codemp,
                 empempenho.e60_emiss,   empempenho.e60_numcgm, cgm.z01_nome, cgm.z01_cgccpf, cgm.z01_munic,
                 yyy.e60_vlremp, yyy.e60_vlranu, yyy.e60_vlrliq, empemphist.e63_codhist, emphist.e40_descr,
                 yyy.e60_vlrpag, empempenho.e60_anousu, empempenho.e60_coddot, orcdotacao.o58_coddot,
                 orcdotacao.o58_orgao, orcorgao.o40_orgao, orcorgao.o40_descr, orcdotacao.o58_unidade,
                 orcunidade.o41_descr, orctiporec.o15_codigo, orctiporec.o15_descr, empempenho.e60_codcom,
                 pctipocompra.pc50_descr,empempenho.e60_concarpeculiar ";
    }
    ...
    if ($agrupar == "d" || $sele_desdobramentos != "") {
        $sqlperiodo = "
          ...
          group by e60_numemp, e60_resumo, e60_codemp, e60_emiss, e60_numcgm, z01_nome, z01_cgccpf, z01_munic,
          e60_vlremp, e60_vlranu, e60_vlrliq, e63_codhist, e40_descr, e60_vlrpag, e60_anousu, e60_coddot, o58_coddot,
          o58_orgao, o40_orgao, o40_descr, o58_unidade, o41_descr, o15_codigo, o15_descr, dl_estrutural, e60_codcom,
          pc50_descr, empelemento.e64_codele, orcelemento.o56_descr, e60_concarpeculiar";
        if ($agrupar == "d") {
            $sqlperiodo .= " order by  empelemento.e64_codele ";
        } else {
            $sqlperiodo .= " order by  $ordem, empelemento.e64_codele ";
        }
    }
...
if (isset($tipoemp) && $tipoemp != "") {
    if ($tipoemp == "todos") {
        $cabecalho = "Todos os empenhos";
    } elseif ($tipoemp == "saldo") {
        $cabecalho = "Com saldo a pagar geral";
    } elseif ($tipoemp == "saldoliq") {
        $cabecalho = "Com saldo a pagar liquidados";
    } elseif ($tipoemp == "saldonaoliq") {
        $cabecalho = "Com saldo a pagar nao liquidados";
    } elseif ($tipoemp == "anul") {
        $cabecalho = "Com anulação";
    } elseif ($tipoemp == "anultot") {
        $cabecalho = "Apenas os totalmente anulados";
    } elseif ($tipoemp == "anulparc") {
        $cabecalho = "Apenas os anulados parcialmente";
    } elseif ($tipoemp == "anulsem") {
        $cabecalho = "Apenas os sem anulação";
    } elseif ($tipoemp == "liq") {
        $cabecalho = "Com liquidação";
    } elseif ($tipoemp == "liqtot") {
        $cabecalho = "Apenas os liquidados totalmente";
    } elseif ($tipoemp == "liqparc") {
        $cabecalho = "Apenas os liquidados parcialmente";
    } elseif ($tipoemp == "liqsem") {
        $cabecalho = "Apenas os sem liquidação";
    } elseif ($tipoemp == "pag") {
        $cabecalho = "Com pagamentos";
    } elseif ($tipoemp == "pagtot") {
        $cabecalho = "Apenas os pagos totalmente";
    } elseif ($tipoemp == "pagparc") {
        $cabecalho = "Apenas os pagos parcialmente";
    } elseif ($tipoemp == "pagsem") {
        $cabecalho = "Apenas os sem pagamento";
    } elseif ($tipoemp == "pagsemsemliq") {
        $cabecalho = "Apenas os sem pagamento e sem liquidação";
    } elseif ($tipoemp == "pagsemcomliq") {
        $cabecalho = "Apenas os sem pagamento e com liquidação";
    }
}

$pdf->addTitulo($cabecalho);
$pdf->addTitulo($info);

if ($processar == "a") {
    $pdf->addTitulo("Posição atual");
} else {
    $pdf->addTitulo("Periodo especificado: " . db_formatar($dataesp11, "d") . " a " . db_formatar($dataesp22, "d"));
}
$cabecalho = '';
if ($chk_ordem == "C") {
    $cabecalho = "Ordenado crescente por valor ";
    if ($chk_valor == "E") {
        $cabecalho .= "empenhado";
    } elseif ($chk_valor == "L") {
        $cabecalho .= "liquidado";
...
if ($chk_ordem == "D") {
    $cabecalho = "Ordenado decrescente por valor ";
    if ($chk_valor == "E") {
        $cabecalho .= "empenhado";
    } elseif ($chk_valor == "L") {
        $cabecalho .= "liquidado";
...
if ($tipo == "a" or 1 == 1) {
    ...
    for ($x = 0; $x < $rows; $x++) {
        ...
        if (!array_key_exists($o15_codigo, $listaRecursos)) {
            $listaRecursos[$o15_codigo] = \App\Domain\Financeiro\Orcamento\Models\FonteRecurso::with('recurso')
                ->where('exercicio', $e60_anousu)
                ->where('orctiporec_id', $o15_codigo)
                ->first();
        }

        if (db_getsession("DB_anousu") >= 2019) {
            $recurso = $listaRecursos[$o15_codigo];
            $codigo_recurso = $recurso->gestao;

            $o15_descr .= sprintf(
                " | %s - %s ",
                $recurso->recurso->o15_complemento,
                $recurso->recurso->complemento->o200_descricao
            );
        }
        // testa novapagina
        ...
        if ($imprime_header == true) {
            ...
            if ($agrupar == "a") {
                if ($sememp == "n") {
                    ...
                    $pdf->Cell(120, $tam, strtoupper($RLz01_nome), 1, 0, "C", 1);
            ...
            if ($agrupar == "orgao") {
                if ($sememp == "n") {
                    $pdf->Cell(45, $tam, strtoupper($RLo58_orgao), 1, 0, "C", 1);
            ...
            if ($tipo == "a" and $sememp == "n") {
                if ($agrupar == "oo") {
                    if ($mostrar == "r") {
                        ...
                        $pdf->Cell(45, $tam, strtoupper($RLo15_codigo), 1, 0, "C", 1);
                        $pdf->Cell(120, $tam, strtoupper('Tipo de Compra'), 1, 0, "C", 1);
                    } elseif ($mostrar == "f") {
                        $pdf->Cell(45, $tam, "", 1, 0, "C", 1);
                        $pdf->Cell(120, $tam, "", 1, 0, "C", 1);
                    }
                ...
                if ($agrupar == "a") {
                    /**
                     *    Quando agrupar  por fornecedor , não mostrar desnecessariamente o fornecedor
                     */
                    if ($mostrar == "r" || $mostrar == "f") {
                        $pdf->Cell(40, $tam, strtoupper($RLo15_recurso), 1, 0, "C", 1); // recurso
                    } elseif ($mostrar == "t") {
                ...
                if ($agrupar == "d") {
                    if ($mostrar == "r") {
                        ...
                        $pdf->Cell(40, $tam, strtoupper($RLz01_nome), 1, 0, "C", 1); // Fornecedor
                    }
                }

                if ($agrupar == "r") {
                    /**
                     *    Quando agrupar  por recurso , não mostrar desnecessariamente o recurso
                     */
                    if ($mostrar == "r" || $mostrar == "f") {
                        $pdf->Cell(40, $tam, strtoupper($RLz01_nome), 1, 0, "C", 1); // fornecedor
                    } elseif ($mostrar == "t") {
            ...
            } elseif ($tipo == "a" and $sememp == "s" and $agrupar == "oo") {
        ...
        if ($repete != $e60_numcgm and $agrupar == "a") {
            ...
            if ($agrupar == "r") {
                $pdf->Cell(45, $tam, "$codigo_recurso", $iBorda, 0, "C", 0);
        ...
        if ($agrupar == "oo") {
            /**
             * Quando nao ha agrupamento, o usuario pode ver o recurso/tipo de compra a cada dotacao.
             * Quando ele quiser ver o fornecedor,o relatorio continua com o seu comportamento original.
             */
            if ($mostrar == "r") {
                $pdf->SetFont('Arial', 'b', 7);
                $pdf->Cell(45, $tam, "$codigo_recurso", $iBorda, 0, "C", 0);
                ...
                $pdf->SetFont('Arial', 'b', 7);
                $pdf->Cell(45, $tam, "$e60_codcom", $iBorda, 0, "C", 0);
                $pdf->Cell(105, $tam, "$pc50_descr", $iBorda, 1, "L", 0);
                $pdf->SetFont('Arial', '', 7);
            }
        }
        ...
        if ($tipo == "a" and $sememp == "n") {
            ...
            if ($agrupar == "a") {
                 ...
                 */
                if ($mostrar == "r" || $mostrar == "f") {
                    $recurso = db_formatar($codigo_recurso, 'recurso') . " - " . substr($o15_descr, 0, 20);
                    $pdf->Cell(40, $tam, $recurso, $iBorda, 0, "L", $preenche); // recurso
                } elseif ($mostrar == "t") {
            ...
            if ($agrupar == "d") {
                if ($mostrar == "r") {
                    ...
                    $pdf->Cell(40, $tam, substr($z01_nome, 0, 25), $iBorda, 0, "L", $preenche); // fornecedor
                }
            }
            ...
            if (1 == 1) {
                ...
                for ($lancemp = 0; $lancemp < $rows_lancamemp; $lancemp++) {
                    ...
                    if ($mostralan == "m") {
                        ...
                        $pdf->cellAdapt(7, 25, $tam, $c53_descr, $iBorda, 0, "L", $preenche);
...
if ($hist == "h") {
    if ($processar == "a") {
        $sql = "
            select case when e63_codhist is null then 0 else e63_codhist end as e63_codhist,
        ...
        $sql = "
            select case when x.e63_codhist is null then 0 else x.e63_codhist end as e63_codhist,
                   case when x.e40_descr   is null then 'SEM HISTORICO' else x.e40_descr end as e40_descr,
                   sum(e60_vlremp) as e60_vlremp, sum(e60_vlranu) as e60_vlranu, sum(e60_vlrliq) as e60_vlrliq,
                   sum(e60_vlrpag) as e60_vlrpag
              from ($sqlperiodo) as x
...
if ($hist == "h") {
    if ($processar == "a") {
        $sql = "
            select case when o58_codigo is null then 0 else o58_codigo end as o58_codigo,
        ...
        $sql = "
          select case when x.o58_codigo is null then 0 else x.o58_codigo end as o58_codigo,
...
if ($hist == "h") {
    if ($processar == "a") {
        ...
        $sql = "
          select x.e60_codcom, x.pc50_descr,
      ...
      ";
        ...
        $sql = "
          select x.o58_orgao,
            ...
            inner join orcunidade  on o41_orgao = orcorgao.o40_orgao and o41_unidade = orcdotacao.o58_unidade
                       and o41_anousu = orcorgao.o40_anousu
...
if (isParaiba()) {
    if ($clempempenho->numrows == 0) {
        ...
        $pdf->cell($pdf->w - 15, $pdf->getH() / 2, "NADA A REGISTRAR", 0, 0, "C", 0);
        $pdf->setfont('arial', '', 7);
        $pdf->sety($pdf->getH() - 50);
    }
