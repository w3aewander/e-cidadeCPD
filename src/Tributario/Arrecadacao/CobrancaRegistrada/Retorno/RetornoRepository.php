 ...
 *
 * @author Rafael Macedo <macedo.dbseller.com.br>
 */
class RetornoRepository
{
    /**
    ...
    public static function findAllByRequestFilters(RetornoRequestFilters $oRetornoRequestFilters)
    {
        ...
        $sSqlUnion1  = "select rpad(lpad(recibopagaboleto.k138_numnov::text, 8, '0'), 11, '0') as codigo_arrecadacao,
                               (select recibopaga.k00_numcgm                                                   
                                  from recibopaga                                                              
                                 where recibopaga.k00_numnov = recibopagaboleto.k138_numnov limit 1) as cgm,   
                               array_to_string((                                                               
                                select array_agg(distinct                                                      
                                       arretipo.k00_descr)                                                     
                                  from (select k00_tipo                                                        
                                          from arrecad                                                         
                                         where k00_numpre in (select k00_numpre                                
                                                                from recibopaga                                
                                                               where k00_numnov = recibopagaboleto.k138_numnov)
                                         union all                                                             
                                        select k00_tipo                                                        
                                          from arrecant                                                        
                                         where k00_numpre in (select k00_numpre                                
                                       ...
                                       inner join arretipo on arretipo.k00_tipo = arrec.k00_tipo               
                               ), '#') as tipo,                                                                
                               recibopagaboleto.k138_numnov as numnov,                                         
                               cadtipoconvenio.ar12_nome as convenio,                                          
                               recibopagaboleto.k138_data as data_emissao,                                     
                               ocorrenciacobrancaregistrada.k149_descricao,                                    
                               ocorrenciaretornocobrancaregistrada.k170_sequencial,                            
                               movimentoocorrenciacobrancaregistrada.k169_sequencial                           
                          from recibopagaboleto                                                                
                         inner join remessacobrancaregistradarecibo 
                            on remessacobrancaregistradarecibo.k148_numpre = recibopagaboleto.k138_numnov
                         inner join remessacobrancaregistrada 
                            on remessacobrancaregistrada.k147_sequencial = 
                               remessacobrancaregistradarecibo.k148_remessacobrancaregistrada              
                         inner join retornocobrancaregistrada 
                            on retornocobrancaregistrada.k168_numpre = remessacobrancaregistradarecibo.k148_numpre
                         inner join ocorrenciaretornocobrancaregistrada 
                            on ocorrenciaretornocobrancaregistrada.k170_retornocobrancaregistrada = 
                               retornocobrancaregistrada.k168_sequencial
                         inner join ocorrenciacobrancaregistrada 
                            on ocorrenciacobrancaregistrada.k149_sequencial = 
                               ocorrenciaretornocobrancaregistrada.k170_ocorrenciacobrancaregistrada 
                         inner join movimentoocorrenciacobrancaregistrada 
                            on movimentoocorrenciacobrancaregistrada.k169_sequencial = 
                               ocorrenciacobrancaregistrada.k149_movimento         
                         inner join cadconvenio 
                            on cadconvenio.ar11_sequencial = remessacobrancaregistrada.k147_convenio
                         inner join cadtipoconvenio 
                            on cadtipoconvenio.ar12_sequencial = cadconvenio.ar11_cadtipoconvenio";
        $sSqlUnion2  = "select rpad(lpad(recibo.k00_numpre::text, 8, '0'), 11, '0') as codigo_arrecadacao,
                               recibo.k00_numcgm as cgm, 
                               arretipo.k00_descr as tipo, 
                               recibo.k00_numpre as numnov,
                               cadtipoconvenio.ar12_nome as convenio,
                               recibo.k00_dtoper as data_emissao,    
                               ocorrenciacobrancaregistrada.k149_descricao,
                               ocorrenciaretornocobrancaregistrada.k170_sequencial,                  
                               movimentoocorrenciacobrancaregistrada.k169_sequencial                 
                          from recibo
                         inner join remessacobrancaregistradarecibo 
                            on remessacobrancaregistradarecibo.k148_numpre = recibo.k00_numpre
                         inner join remessacobrancaregistrada 
                            on remessacobrancaregistrada.k147_sequencial =
                               remessacobrancaregistradarecibo.k148_remessacobrancaregistrada
                         inner join retornocobrancaregistrada 
                         ...
                         inner join cadtipoconvenio 
                            on cadtipoconvenio.ar12_sequencial = cadconvenio.ar11_cadtipoconvenio 
                         inner join arretipo 
                            on arretipo.k00_tipo = recibo.k00_tipo";
        ...
        if (!empty($iCodigoArrecadacao)) {
            $sSqlWhereUnion1 .= " and rpad(lpad(recibopagaboleto.k138_numnov::text, 8, '0'), 11, '0') like 
                                 '{$iCodigoArrecadacao}' ";
            $sSqlWhereUnion2 .= " and rpad(lpad(recibo.k00_numpre::text, 8, '0'), 11, '0') like 
                                 '{$iCodigoArrecadacao}'            ";
        }
        ...
        if (!empty($iCodigoTipoDebito)) {
            ...
            $sSqlWhereUnion2 .= " and recibo.k00_tipo = {$iCodigoTipoDebito} ";
        }
    ...
    private function incluirOcorrencia($iRetornoCobrancaRegistrada, $iCodigoMovimento, $iOcorrencia)
    {
        ...
        $sSql  = "insert into ocorrenciaretornocobrancaregistrada
                  (k170_retornocobrancaregistrada, k170_ocorrenciacobrancaregistrada)
                  (select {$iRetornoCobrancaRegistrada},              
                          ocorrenciacobrancaregistrada.k149_sequencial
                     from ocorrenciacobrancaregistrada
                    inner join bancoagencia 
                       on bancoagencia.db89_db_bancos = ocorrenciacobrancaregistrada.k149_banco
                    inner join conveniocobranca 
                       on conveniocobranca.ar13_bancoagencia = bancoagencia.db89_sequencial
                    inner join cadconvenio 
                       on cadconvenio.ar11_sequencial = conveniocobranca.ar13_cadconvenio
                    inner join remessacobrancaregistrada 
                       on remessacobrancaregistrada.k147_convenio = cadconvenio.ar11_sequencial
                    inner join remessacobrancaregistradarecibo 
                       on remessacobrancaregistradarecibo.k148_remessacobrancaregistrada = 
                          remessacobrancaregistrada.k147_sequencial
                    inner join retornocobrancaregistrada 
                       on retornocobrancaregistrada.k168_numpre = remessacobrancaregistradarecibo.k148_numpre
                    inner join movimentoocorrenciacobrancaregistrada 
                       on movimentoocorrenciacobrancaregistrada.k169_sequencial = 
                          ocorrenciacobrancaregistrada.k149_movimento
             where ocorrenciacobrancaregistrada.k149_codigo = '{$iOcorrencia}'              
               and movimentoocorrenciacobrancaregistrada.k169_codigo = '{$iCodigoMovimento}'
               and retornocobrancaregistrada.k168_sequencial = {$iRetornoCobrancaRegistrada})";
