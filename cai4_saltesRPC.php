...
require_once(modification("model/contaTesouraria.model.php"));
...
switch ($oParam->exec) {
  ...
  case "getDepartamentosContaBancaria":

    $clSaltesDepartamento = new cl_saltesdepartamento();
    $sSqlDepartamentos = $clSaltesDepartamento->sql_query(null,
                                                          "k212_departamento, db_depart.descrdepto, k212_principal",
                                                          "k212_departamento",
                                                          "k212_saltes = {$oParam->k13_conta}");
    $rsDepartamentos = db_query($sSqlDepartamentos);
    $oRetorno->departamentos = array();
    if (pg_num_rows($rsDepartamentos) > 0) {
      $oRetorno->departamentos = db_utils::getCollectionByRecord($rsDepartamentos, false, false, true);
    }
    break;
}
