...
class cl_rhferiasconfiguracao { 
   ...
   var $rh168_tipoassentamentopecunia = 0; 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->rh168_tipoassentamentopecunia = ($this->rh168_tipoassentamentopecunia == ""?@$GLOBALS["HTTP_POST_VARS"]["rh168_tipoassentamentopecunia"]:$this->rh168_tipoassentamentopecunia);
     }else{
       $this->rh168_sequencial = ($this->rh168_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["rh168_sequencial"]:$this->rh168_sequencial);
     }
   }
   ...
   function incluir ($rh168_sequencial){ 
     ...
     $sql = "insert into rhferiasconfiguracao(
                ...
                values (
                               ...
                               ,$this->rh168_tipoassentamentopecunia 
                      )";
     $result = db_query($sql); 
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,3872,1009749,'','".AddSlashes(pg_result($resaco,0,'rh168_tipoassentamentopecunia'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 
   ...
   public function alterar ($rh168_sequencial=null) { 
     ...
     if(trim($this->rh168_tipoassentamentopecunia)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh168_tipoassentamentopecunia"])){ 
        if(trim($this->rh168_tipoassentamentopecunia)=="" && isset($GLOBALS["HTTP_POST_VARS"]["rh168_tipoassentamentopecunia"])){ 
           $this->rh168_tipoassentamentopecunia = "0" ; 
        } 
       $sql  .= $virgula." rh168_tipoassentamentopecunia = $this->rh168_tipoassentamentopecunia ";
       $virgula = ",";
     }
     $sql .= " where ";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["rh168_tipoassentamentopecunia"]) || $this->rh168_tipoassentamentopecunia != "")
             $resac = db_query("insert into db_acount values($acount,3872,1009749,'".AddSlashes(pg_result($resaco,$conresaco,'rh168_tipoassentamentopecunia'))."','$this->rh168_tipoassentamentopecunia',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
   ...
   public function excluir ($rh168_sequencial=null,$dbwhere=null) { 
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,3872,22179,'','".AddSlashes(pg_result($resaco,$iresaco,'rh168_ultimoperiodoaquisitivo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,3872,1009749,'','".AddSlashes(pg_result($resaco,$iresaco,'rh168_tipoassentamentopecunia'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     ...
     if ($result == false) { 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Tabela de configuração para Férias não Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Tabela de configuração para Férias não Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$rh168_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
  ...
  public function sql_query_tipos ($rh168_sequencial = null,$campos = "*", $ordem = null, $dbwhere = "") {
    ...
    $sql .= "      left  join tipoasse  as assentamento_pecunia  on  assentamento_pecunia.h12_codigo = rhferiasconfiguracao.rh168_tipoassentamentopecunia";
    $sql2 = "";
    if (empty($dbwhere)) {
      if (!empty($rh168_sequencial)) {
        $sql2 .= " where rhferiasconfiguracao.rh168_sequencial = $rh168_sequencial ";
