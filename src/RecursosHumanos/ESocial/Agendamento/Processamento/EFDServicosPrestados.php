...
class EFDServicosPrestados extends ProcessamentoAbstract implements ProcessamentoInterface
{
    ...
    public function processar()
    {
        ...
        $dadosPreenchimento = $dadosEsocial->getPorTipo(Tipo::EFD_SERVICOS_PRESTADOS);

        $formatter = FormatterFactory::get(Tipo::R2020);
        $dadosFormatados = $formatter->formatar($dadosPreenchimento);

        foreach ($dadosFormatados as $dados) {
            $eventoFila = new Evento(Tipo::R2020, $cgm, $dados['referencia'], $dados);
