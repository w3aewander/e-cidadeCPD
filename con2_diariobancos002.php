...
for ($x = 0; $x < pg_numrows($result);$x++) {
  db_fieldsmemory($result,$x);
   ...
   $sSqlContaBancaria = <<<SQL
                         select *
                           from conplanoconta
                          where c63_anousu = $iAnousu
                            and c63_codcon = $c61_codcon
                            and c63_reduz  = $c61_reduz
SQL;


   $resconta = db_query( $sSqlContaBancaria ) or die ($sSqlContaBancaria);
   if($resconta and pg_num_rows($resconta) > 0) {
     db_fieldsmemory($resconta,0);
   }

   if ($somentediferenca == "S" && $saldo_anterior <= 0) {
      continue;
   }
   ...
   if ( $c61_reduz!= 0) {
      ...
      $sql  = "select substr(fc_saltessaldo,2,13)::float8 as anterior,    \n";
		  $sql .= "       substr(fc_saltessaldo,15,13)::float8 as debitado ,  \n";
		  $sql .= "       substr(fc_saltessaldo,28,13)::float8 as creditado,  \n";
		  $sql .= "       substr(fc_saltessaldo,41,13)::float8 as atual       \n";
      $sql .= "  from ( select fc_saltessaldo($c61_reduz,'$perini','$perfin',null,$c61_instit) ) as x";
