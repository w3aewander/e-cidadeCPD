...
if(isset($opcao) && $opcao == "alterar"){
  echo "<script>parent.iframe_fiscalprocrec.location.href='fis1_fis_fiscalprocrec002.php?chavepesquisa=$y45_codtipo&chavepesquisa1=$y45_receit'</script>";
}
if(isset($opcao) && $opcao == "excluir"){
  echo "<script>parent.iframe_fiscalprocrec.location.href='fis1_fis_fiscalprocrec003.php?chavepesquisa=$y45_codtipo&chavepesquisa1=$y45_receit'</script>";
}
?>
...
<form name="form1" method="post" action="">
  <fieldset>
    ...
    <table border="0">
      ...
      <tr>
        ...
        <td>
          ...
          $aFormaCalculo = array(  "1" => "Valor Fixo",
                                   ...
                                   "5" => "Percentual de Outra Procedência Fixo",
                                   "6" => "Percentual de Outra Procedência Variável"
                                 );
     ...
     <!--  <tr id="sreceita" style="display:none;" >
        <td> 
          <?php
           db_ancora(@$Ly45_receit,"js_pesquisay45_receit2(true);",$db_opcao);
          ?>
        </td>
        <td>
          <?php 

            db_input('y45_receit2',10,$Iy45_receit,true,'text',$db_opcao," onchange='js_pesquisay45_receit2(false);'");
            db_input('k02_descr2',50,$Ik02_descr,true,'text',3,'')
          ?>
        </td>
      </tr> -->
      <tr>
...
function js_validaValor(){
  ...
  if( iFormaCalculo == 2 || iFormaCalculo == 4 || iFormaCalculo == 6){

    $('y45_valor').disabled = 'true';
    $('y45_valor').style    = 'background-color:#DEB887;';
  }

  // if( iFormaCalculo == 5 || iFormaCalculo == 6 ){
  //   $('sreceita').style.display = '';
  // }
}
function js_pesquisay45_codtipo(mostra){

  if(mostra==true){
    js_OpenJanelaIframe('','db_iframe_fiscalproc','func_fis_fiscalproc.php?funcao_js=parent.js_mostrafiscalproc1|y29_codtipo|y29_descr','Pesquisa',true);
  }else{
    js_OpenJanelaIframe('','db_iframe_fiscalproc','func_fis_fiscalproc.php?pesquisa_chave='+document.form1.y45_codtipo.value+'&funcao_js=parent.js_mostrafiscalproc','Pesquisa',false);
  }
}
...
function js_pesquisay45_receit2(mostra){
  if(mostra==true){
    js_OpenJanelaIframe('','db_iframe_tabrec','func_tabrec.php?funcao_js=parent.js_mostratabrec12|k02_codigo|k02_descr','Pesquisa',true);
  }else{
    js_OpenJanelaIframe('','db_iframe_tabrec','func_tabrec.php?pesquisa_chave='+document.form1.y45_receit.value+'&funcao_js=parent.js_mostratabrec2','Pesquisa',false);
  }
}
function js_mostratabrec2(chave,erro){
  document.form1.k02_descr2.value = chave;
  if(erro==true){
    document.form1.y45_receit2.focus();
    document.form1.y45_receit2.value = '';
  }
}
function js_mostratabrec12(chave1,chave2){
  document.form1.y45_receit2.value = chave1;
  document.form1.k02_descr2.value = chave2;
  db_iframe_tabrec.hide();
}

function js_pesquisa(){
  js_OpenJanelaIframe('','db_iframe_fiscalprocrec','func_fis_fiscalprocrec.php?funcao_js=parent.js_preenchepesquisa|y45_codtipo|1','Pesquisa',true);
}
...
if(isset($y45_codtipo) && $y45_codtipo != ""){
  echo "<script>js_OpenJanelaIframe('','db_iframe_fiscalproc','func_fis_fiscalproc.php?pesquisa_chave=$y45_codtipo&funcao_js=parent.js_mostrafiscalproc','Pesquisa',false);</script>";
}
?>