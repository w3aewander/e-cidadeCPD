...
$where = " o58_coddot = {$coddot} and o58_anousu = {$anousu} ";
$result = db_dotacaosaldo($nivel, 2, 2, true, $where, $anousu, $dPeriodoIni, $dPeriodoFim);
...
$fonteRecurso = wordwrap(descricaoCompletaRecurso($o58_codigo, $anousu, 2), 95, "\n");
$fontes = explode("\n", $fonteRecurso);
if (count($fontes) > 1) {
    $fonteRecurso = "{$fontes[0]}...";
}
...
$pdf->init(false);
$pdf->addTitulo("SALDO DESPESA");
$pdf->addTitulo('');
$pdf->addTitulo("REDUZIDO DA DOTAÇÃO: {$o58_coddot}");
$pdf->addTitulo("MÊS/ANO: {$x[$mesusu]}/{$anousu}");
...
$pdf->setfont('arial', '', 8);
$pdf->cell(128, $alt, $fonteRecurso, 1, 0, "L", 0);
$pdf->cell(1.5, $alt, "", 0, 0, "C", 0);
$pdf->setfont('arial', 'b', 8);
...
if (isset($reservado) && ($reservado > 0)) {
  ...
  //---------------  reserva de automática
    $res=$clorcreserva->sql_record(
        $clorcreserva->sql_query_reservas(
            null,
            "*",
            "o80_codres",
