...
$oConfig->iAlturaLimiteAlunos    = 220; // altura limite do eixo y que devemos quebrar a pagina dos alunos
$oConfig->iAlturaRetanguloAlunos = 180; // Height para desenho dos retangulos
$oConfig->iLarguraRetangulo      = 194; // Width para desenho dos retangulos
$oConfig->iAlturaRodape          = 225; // eixo y que devemos escrever rodapé
...
try {
  ...
  foreach ($oGet->aTurmas as $oStdTurmaEtapa) {
    ...
    $oDadosTurma->iEtapa             = $oEtapa->getCodigo();
    ...
    foreach ($oDadosTurma->aRegencias as $oDadosRegenciaComAlunos) {
      ...
      foreach ($aProgressoes as $oProgressaoParcial) {
        ...
        db_inicio_transacao();
        $oDiarioProgressaoParcial = new DiarioProgressaoParcial( $oProgressaoParcial );
        ...
        if ( $oDiarioProgressaoParcial instanceof DiarioProgressaoParcial ) {
          ...
          if ( $oConfig->sTipoFrequencia == 'P' ) {
            $iTotalFalta = $oResultadoDiario->getPercentualFrequencia();
          }
          ...
          $oAvaliadoConcelho = $oResultadoDiario->getFormaAprovacaoConselho();
          if(    $oAvaliadoConcelho instanceof AprovacaoConselhoProgressao
              && $oAvaliadoConcelho->getFormaAprovacao() == 1
              && $oAvaliadoConcelho->getAlterarNotaFinal() == 2
            ) {

            $mAvaliacao = $oAvaliadoConcelho->getAvaliacaoConselho();
          }

          if (!empty($oAvaliadoConcelho)) {

            switch ($oAvaliadoConcelho->getFormaAprovacao()) {
...
foreach ($aDadosRelatorio as $oDadosRelatorio) {
  ...
  $oConfig->iTurma = $oDadosRelatorio->iTurma;
  $oConfig->sTurma = $oDadosRelatorio->sTurma;
  $oConfig->iEtapa = $oDadosRelatorio->iEtapa;
...
function imprimirRodape($oPdf, $oConfig, $aTermoEncerramento) {
  ...
  $iAltura        = 60;            // altura do quadro das assinaturas
  $iMeiaPagina    = $iLargura / 2; // metade da pagina contando as bordas
  ...
  $oPdf->Rect($oPdf->GetX(), $oPdf->GetY(), $iLargura, $iAltura);
  $oPdf->Line($iMeiaPagina, $oPdf->GetY(), $iMeiaPagina, $oPdf->GetY() + $iAltura  );
  ...
  foreach ($aTermoEncerramento as $oTermo) {
    $aLegendas[] = "{$oTermo->sAbreviatura} - {$oTermo->sDescricao}";
  }

  $sLegendaFaltas = $oConfig->sTipoFrequencia == 'T' ? 'FT - Faltas' : 'F - Frequência (%)';
  $aLegendas[] = "Eva - EVADIDO";
  $aLegendas[] = "AVAL - Avaliação | {$sLegendaFaltas}";
  $aLegendas[] = "RF - Resultado Final";
  $oPdf->MultiCell( $iMeiaPagina -10, 4, "Legendas: " . implode(" | ", $aLegendas));

  $iAlturaAposImprimirLegenda = $oPdf->getY();

  /**
   * Busca os resultados alterados, se houver
   */
  $sCampos  = " trim(ed47_v_nome) as ed47_v_nome, ed996_aprovconselhotipo, ed996_alterarnotafinal, ";
  $sCampos .= " ed996_observacao, trim(ed232_c_descr) as ed232_c_descr, ";
  $sCampos .= " trim(ed11_c_descr) as ed11_c_descr, ed52_i_ano , ed996_avaliacaoconselho ";
  $sWhere   =  " ed57_i_codigo = {$oConfig->iTurma} ";
  $sWhere  .=  " and ed11_i_codigo = {$oConfig->iEtapa} ";

  $oDao = new cl_aprovadoconselhoprogressao();
  $sSql = $oDao->sql_aprovadoconselhoprogressao(null, $sCampos, null, $sWhere);

  $rsAprovConselho = db_query($sSql);

  if ( !$rsAprovConselho ) {

    $sMsg = urlencode("Erro ao buscar os dados dos alunos.");
    db_redireciona('db_erros.php?fechar=true&db_erro=' . $sMsg);
    die();
  }
  ...
  if ( is_resource( $rsAprovConselho ) && pg_num_rows( $rsAprovConselho ) > 0) {
    ...
    for ($iContObs = 0; $iContObs < $iLinhasAprovCons; $iContObs++) {
      ...
      switch ($oAprovConselho->ed996_aprovconselhotipo) {
        ...
        case 1:
          ...
          $oDocumento->justificativa = $oAprovConselho->ed996_observacao;
          $oDocumento->nota          = $oAprovConselho->ed996_avaliacaoconselho;
  ...
  if (count($aAlunosReclaBaixaFrequencia) > 0) {
    ...
    $oDocumento->nome_turma   = $oConfig->sTurma;
  ...
  $iWLinhaObservacao = $iMeiaPagina - 10;
  $iLinhas = $oPdf->NbLines($iWLinhaObservacao, $sObservacoesTurma);

  /* ************************************************************************************** *
   * ******************************* QUADRO DAS ASSINATURAS ******************************* *
   **************************************************************************************** */
  $oPdf->SetXY($iMeiaPagina, $oConfig->iAlturaRodape);
  $oPdf->SetFont('Arial', '', 6);
  $sTermoFinal = "E, para constar, foi lavrada esta ata.    {$oConfig->sMunicipio}, {$oConfig->sData}.";
  $oPdf->MultiCell($iMeiaPagina, 4, $sTermoFinal, 0, 'C');
  ...
  $iInicioAssinatura = $iMeiaPagina + 20;
  $iFimAssinatura    = $iMeiaPagina + 85;

  if ( empty($oConfig->sAssinaturaAdicicional) ) {

    $oPdf->Line($iInicioAssinatura , $oPdf->GetY(),      $iFimAssinatura, $oPdf->GetY() );
    $oPdf->Line($iInicioAssinatura , $oPdf->GetY() + 20, $iFimAssinatura, $oPdf->GetY() +20 );
    $oPdf->ln(0.5);

    $iAlturaAssinatura = $oPdf->getY();
    $oPdf->SetXY($iInicioAssinatura, $iAlturaAssinatura );
    $oPdf->MultiCell(65, 3, "{$sNomeDiretor}", 0, 'C');
    $oPdf->SetXY($iInicioAssinatura, $iAlturaAssinatura +20);
    $oPdf->MultiCell(65, 3, "{$sNomeSecretario}", 0, 'C');

  } else {

    $oPdf->Line($iInicioAssinatura, $oPdf->GetY(), $iFimAssinatura, $oPdf->GetY() );
    $oPdf->Line($iInicioAssinatura, $oPdf->GetY() + 15 , $iFimAssinatura, $oPdf->GetY() + 15);
    $oPdf->Line($iInicioAssinatura, $oPdf->GetY() + 30, $iFimAssinatura, $oPdf->GetY() + 30);
    $oPdf->ln(0.5);

    $iAlturaAssinatura = $oPdf->getY();
    $oPdf->SetXY($iInicioAssinatura, $iAlturaAssinatura);
    $oPdf->MultiCell(65, 3, "{$sNomeDiretor}", 0, 'C');
    $oPdf->SetXY($iInicioAssinatura, $iAlturaAssinatura + 15);
    $oPdf->MultiCell(65, 3, "{$sNomeSecretario}", 0, 'C');
    $oPdf->SetXY($iInicioAssinatura, $iAlturaAssinatura + 30);

    $sAssinaturaAdicional = "{$oConfig->sAssinaturaAdicicional}\n{$oConfig->sCargoAdicional}";
    $oPdf->MultiCell(65, 3, "{$sAssinaturaAdicional}", 0, 'C');
  }
  ...
  if ( $iLinhas > 13 ) {
    ...
    $oPdf->SetFont('Arial', 'B', 7);
    $oPdf->Cell( 194, 4, "Legenda das alterações de resultados finais:", "LRT", 1);
    $oPdf->SetFont('Arial', '', 7);
    $oPdf->MultiCell( 194, 4, $sObservacoesTurma, "BLR");

  } else {

    $oPdf->setY( $iAlturaAposImprimirLegenda );
    $oPdf->MultiCell( $iMeiaPagina - 10, 4, $sObservacoesTurma);
  }
}
