 ...
 class PeriodoAquisitivoFerias
{
    ...
    {
        $iSaldoDiasDireito = $this->getDiasDireito() - ($this->getDiasAbonados() + $this->getDiasGozados() + $this->getDiasPecunia());
    ...
    public function salvar()
    {
        ...
        $oDaoRhFerias->rh109_perdeudireitoferias = $this->getPerdeuDireitoFerias();
        ...
        if (empty($this->iCodigo)) {
            ...
            if ($oDaoRhFerias->rh109_perdeudireitoferias === null) {
                $oDaoRhFerias->rh109_perdeudireitoferias = 'false';
            }
    ...
    public function getPeriodosGozo()
    {
        ...
        $sSqlPeriodosAquisitivos = $oDaoRhFeriasPeriodo->sql_query_file(null, "rh110_sequencial", 'rh110_sequencial',
    ...
    {
        foreach ($this->getPeriodosGozo() as $oPeriodoGozo) {
            if ($oPeriodoGozo->isPagaTerco() && $oPeriodoGozo->isPrimeiroPeriodo()) {
    ...
    /**
     * Retorna o número de dias a pagar disponível para o período aquisitivo
     * @return int
     * @throws DBException
     */
    public function getDiasAPagar()
    {
        $iDiasAPagar = $this->getDiasDireito();

        foreach ($this->getPeriodosGozo() as $oPeriodoGozo) {
            $iDiasAPagar -= $oPeriodoGozo->getDiasAPagar();
        }
     ...
     * @throws DBException
     */
    public function getTotalDiasPagos()
    {
        $totalDiasPagos = 0;

        foreach ($this->getPeriodosGozo() as $oPeriodoGozo) {
            $totalDiasPagos += $oPeriodoGozo->getDiasAPagar();
        }

        return $totalDiasPagos;
    }

    /**
     * Retorna o total de dias a Pagar
     * @return int
     */
    public function getSaldoDiasPagar()
    {

        $iSaldoDiasDireito = $this->getDiasAPagar() - $this->getDiasAbonados();
        if ($iSaldoDiasDireito < 0) {
            $iSaldoDiasDireito = 0;
        }
        return $iSaldoDiasDireito;
    }
    ...
    public function getDiasPecunia()
    {
        ...
        foreach ($this->getPeriodosGozo() as $oPeriodoGozo) {
            $iDiasPecunia += $oPeriodoGozo->getDiasPecunia();
        }
