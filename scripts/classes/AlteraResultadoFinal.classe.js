...
DBViewAvaliacao.AlteraResultadoFinal.prototype.retornoAlunosReprovados = function (oAjax) {
  ...
  oRetorno.aAlunos.each( function (oAluno) {
    ...
    // Data: 17/12/2025
    // Demanda: 18029
    // Razao: Propagar para o select se o aluno é avaliado por parecer (NEE),
    //        permitindo restringir as formas de aprovação disponíveis na tela.
    oOption.setAttribute('lAvaliadoParecer', oAluno.lAvaliadoParecer);
...
DBViewAvaliacao.AlteraResultadoFinal.prototype.validaBloqueioOptionFormaAprovacao = function () {
   ...
   * Data: 17/12/2025
   * Demanda: 18029
   * Razao: Impedir que alunos avaliados por parecer sejam aprovados pelo conselho (formas 1 e 3),
   *        permitindo apenas reclassificação por baixa frequência (forma 2) quando houver reprovação
   *        por frequência.
   *
   * Regra específica para alunos avaliados por parecer (NEE):
   * - Somente é permitida a opção "Reclassificação por baixa frequência" (forma 2);
   * - Opções 1 e 3 devem ser bloqueadas.
   */
  if (oOptionSelecionado.value != '' && oOptionSelecionado.getAttribute('lavaliadoparecer') == 'true') {

    // Bloqueia "Aprovado pelo conselho" e "Aprovado conforme regimento escolar"
    $('formaAprovacao').options[1].setAttribute('disabled', 'disabled');
    $('formaAprovacao').options[3].setAttribute('disabled', 'disabled');

    // Se não estiver reprovado por frequência, também não faz sentido liberar a forma 2
    if (oOptionSelecionado.getAttribute('lreprovadofrequencia') == 'true') {
      $('formaAprovacao').options[2].removeAttribute('disabled');
      $('formaAprovacao').value = '2';
    } else {
      $('formaAprovacao').options[2].setAttribute('disabled', 'disabled');
      $('formaAprovacao').value = '';
    }
