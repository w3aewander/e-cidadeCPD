...
if (isset($oGet->ordem)) {
    if ($oGet->ordem == 'rc') {
        $sOrdem = 'Recurso';
        $sOrder = ' order by o15_recurso,rh82_slip';
    } else {
...
$sSqlConferencia = "
  select distinct
         ...
         rh82_slip,
         case rh79_siglaarq
              when 'r14' then 'salario'
              when 'r20' then 'rescisao'
              when 'r48' then 'complementar'
              when 'r35' then '13. salario'
              when 'r22' then 'adiantamento'
              else 'nao cadastrada'
         end as rh79_siglaarq,
         rh79_seqcompl,
         rh80_rhslipfolha,
         k17_valor,
         k17_dtaut,
         k17_dtanu
  from pessoal.rhslipfolha
       join orctiporec on orctiporec.o15_codigo = rhslipfolha.rh79_recurso
       join pessoal.rhslipfolhaslip          on rh82_rhslipfolha           = rh79_sequencial
       join pessoal.rhslipfolharhemprubrica  on rh79_sequencial            = rh80_rhslipfolha
       join pessoal.rhempenhofolharubrica    on rh80_rhempenhofolharubrica = rh73_sequencial
       join caixa.slip                       on k17_codigo                 = rh82_slip
  {$sWhere}
  {$sOrder}
";
...
for ($iInd = 0; $iInd < $iNumrows; $iInd++) {
    ...
    $pdf->cell(22, $iAlt, $oConferencia->o15_recurso, 0, 0, "C", $iCorFundo);
