...
$sql = "
    ...
    where slip.k17_instit = " . db_getsession("DB_instit") . "
      and corlanc.k12_data between '{$get->datai}'  and '{$get->dataf}'
      and rdeb.o15_codigo <> rcred.o15_codigo
";
if (!empty($filtroRecurso)) {
    $sql .= " and {$filtroRecurso}";
}
...
$head4 = "PERÍODO : " . db_formatar($get->datai, "d") . " A " . db_formatar($get->dataf, "d");
...
for ($linha = 0; $linha < $rows; $linha++) {
    ...
    if ($quebra_recurso == '' || ($quebra_recurso != $debito_recurso && $quebra_recurso != $credito_recurso)) {
        ...
        $recurso = \ECidade\Financeiro\Orcamento\Repository\RecursoRepository::getByCodigo($quebra_recurso);
        $texto = sprintf(
            '%s - %s - %s',
        ...
        $pdf->Ln();
        $pdf->Cell(120, $alt, "RECURSO: {$texto}", '0', 1, "L", 0);//<BR>
    ...
    $recursoContra = \ECidade\Financeiro\Orcamento\Repository\RecursoRepository::getByCodigo($contra_partida);
    $textoContra = sprintf(
        '%s - %s - %s',
        $recursoContra->getFonteDeRecurso(),
        $recursoContra->getDescricao(),
        $recursoContra->getComplementoRecursoVinculado()->getDescricao()
    );

    $pdf->Cell(30, $alt, $k17_codigo, '0', 'L', "C", 0);
    $pdf->Cell(20, $alt, db_formatar($k12_data, 'd'), '0', 0, "C", 0);
    $pdf->Cell(80, $alt, $textoContra, '0', 0, "L", 0);
