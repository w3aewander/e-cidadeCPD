        ...
        <table class="form-container">
          ...
          <tr>
            ...
            <td>
              <div style="display: flex; flex-direction: row; align-items: center;">
                  ...
                  $sScript .= "onchange='$(\"saida-endereco-paciente\").checked = false'";
                  db_input('sLocalSaida', 55, $Itf17_c_localsaida, true, 'text', 1, $sScript, "", "", "", 50);
                  ?>
                  <div style="display: flex; flex-direction: row; width: 100px;">
                      <input id="saida-endereco-paciente" type="checkbox" onchange="handleChkResidenciaPaciente(this)">
                      <label for="saida-endereco-paciente" style="white-space: normal;">
          ...
          <tr>
            ...
            <td class="field-size4">
              <select id="tipoSaida" style="width: 200px;">
        ...
        <table class="form-container">
          ...
          <tr>
            ...
            <td nowrap="nowrap">
              ...
              db_input('sPlacaVeiculo',  56, '',               true, 'text', 1, " onchange='limparVeiculo();' onclick='verificarLocalSaida();'");
              ?>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td nowrap="nowrap">
              ...
              db_input('sNomeMotorista',   56, '' ,                true, 'text', 1,"onchange='limparMotorista();'");
              ?>
            </td>
          </tr>
...
/**
 * Autocomplete dos veiculos
 */
let oAutoComplete = new dbAutoComplete($('sPlacaVeiculo'), 'tfd4_informacoesgerais.RPC.php?veiculos');
oAutoComplete.setHeightList(390);
oAutoComplete.show();
oAutoComplete.setCallBackFunction(function (codigoVeiculo, nomeVeiculo) {
  $('iCodigoVeiculo').value = codigoVeiculo;
  $('sPlacaVeiculo').value = nomeVeiculo;        
});

/**
 * Autocomplete dos motoristas
 */
let oAutoComplete2 = new dbAutoComplete($('sNomeMotorista'), 'tfd4_informacoesgerais.RPC.php?motoristas');
oAutoComplete2.setHeightList(390);
oAutoComplete2.show();
oAutoComplete2.setCallBackFunction(function (codigoMotorista, nomeMotorista) {
    $('iCodigoMotorista').value = codigoMotorista;
    $('sNomeMotorista').value = nomeMotorista;        
});
  ...
  oObjeto.onComplete   = function(oAjax) {
    ...
    if ( lUsaGradeHorario ) {
      ...
      $('selectHoraSaida').style.display = '';
        ...
        obrigaHoraSaida = oRetorno.obrigaHoraSaida;
        $('horaSaida').style.backgroundColor = obrigaHoraSaida ? '' : '#E6E4F1';
    }
  };
...
function js_atualizaDadosPedido(oDadosPedido) {
  ...
  $('dtSaida').value                   = oPedido.dtSaida === '' ? oPedido.dtAgendamento : oPedido.dtSaida;
  ...
  if ($('dtSaida').value  != '' && lUsaGradeHorario) {
    ...
    for (var i = 1; i < iLinhas; i++) {
      ...
      oCheckGrid.observe('click', function() {
        ...
        if( $F('tipoSaida') == 1 && $F('iCodigoVeiculo') == '' ) {
          ...
          this.checked = false;
          // dispara o evento de clique colocado pelo datagrid para evitar inconsistências na hora de pegar as seleções
...
function js_validaDadosAgendamento() {
  ...
  if ( ($F('iCodigoVeiculo') !== '' || obrigaHoraSaida) && $F('horaSaida') == '' ) {
...
function handleChkResidenciaPaciente(chk) {
    if (chk.checked) {
        buscaEnderecoPaciente();
    }
}

function buscaEnderecoPaciente() {
    const parametros = { sExecucao: 'getEnderecoPaciente', idCgs: $F('iCgsPaciente') };

    js_divCarregando('Buscando endereço do paciente...' , 'msgBox');
    new Ajax.Request(sRPC, {
        method: 'post',
        parameters: 'json=' + Object.toJSON(parametros),
        onComplete: (ajax) => {
            js_removeObj('msgBox');
            const retorno = JSON.parse(ajax.responseText);
            if (retorno.erro) {
                alert(retorno.sMensagem.urlDecode());
                return;
            }

            $('sLocalSaida').value = retorno.endereco;
        }
    });
}

function limparVeiculo(){  
  if($('sPlacaVeiculo').value.length == 0){
    $('iCodigoVeiculo').value = '';
  }  
}

function limparMotorista(){  
  if($('sNomeMotorista').value.length == 0){
    $('iCodigoMotorista').value = '';
  }  
}

function verificarLocalSaida(){
