...
DBViewRemocaoPeriodoGradeHorario = function (iTurma, iEtapa, iRegenciaHorario, iRecHumano, iRegencia) {
...
DBViewRemocaoPeriodoGradeHorario.prototype.criaWindow = function () {
  ...
  var sConteudo  = "<div class='container'>                                                         ";
      ...
      sConteudo += "              <option value='1'>Vigência do período até a data</option>         ";
      sConteudo += "              <option value='2'>Remoção permanente</option>                     ";
      sConteudo += "            </select>                                                           ";
      sConteudo += "          </td>                                                                 ";
      sConteudo += "        </tr>                                                                   ";
      sConteudo += "        <tr id='linhaData' style='display:table-row'>                           ";
      sConteudo += "          <td>                                                                  ";
...
DBViewRemocaoPeriodoGradeHorario.prototype.confirmarRemocao = function ( ) {
  ...
  var oParamentros = {exec: 'removerPeriodo', iTipo: $F('tipoRemocao'), sData : '',
    iTurma : this.iTurma, iEtapa : this.iEtapa, iRegenciaHorario : this.iRegenciaHorario, iRecHumano : this.iRecHumano,
      iRegencia : this.iRegencia
  };

  if ( oParamentros.iTipo == 2 && !confirm( _M(this.sFonteMsg + 'confirmar_remocao_permanente') ) ) {
    return false;
  }

  if ( oParamentros.iTipo == 1 && empty($F('dataFinalPeriodo'))) {

    alert ( _M(this.sFonteMsg + 'informe_data') );
    return;
  }

  if ( oParamentros.iTipo == 1 ) {
