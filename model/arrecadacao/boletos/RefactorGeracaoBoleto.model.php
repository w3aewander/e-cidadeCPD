...
class RefactorGeracaoBoleto
{
    ...
    private $k00_dtoper;
    private $geraTaxaExpediente = false;
    private $sHistorico = null;

    private $oConvenio;

    /**
     * Seta o texto do hist�rico
     */
    ...
    /**
     * @param mixed $oConvenio
     */
    public function setConvenio($oConvenio)
    {
    ...
    public function set($sVariavel, $valor)
    {
        if (!property_exists($this, $sVariavel)) {
            throw new Exception(__CLASS__ . ": Propriedade {$sVariavel} n�o encontrada.");
        }
    ...
    public function processar()
    {
        ...
        /**
         * Busca par�metros do tributario
        ...
        if ($rsSqlParametros && pg_num_rows($rsSqlParametros)) {
            ...
            throw new Exception('Erro ao selecionar par�metros do tribut�rio.');
        }
        ...
        if ($rsParJuridico && pg_num_rows($rsParJuridico) > 0) {
            ...
            throw new Exception('Erro ao selecionar par�metros do jur�dico.');
        }
        ...
        if ($lConfReemissaoRecibo) {
            foreach ($oDebitosFormulario->aValidaNumpre as $iNumpre) {
                ...
                if (count($aProcessosForo) == 0) {
                    ...
                    $sSqlPartilha = $oDaoProcessoForoPartilha->sql_query_file(null, "*", null,
                      "v76_processoforo = {$iProcessoForo}");
                    ...
                    throw new Exception("Existe mais de um processo do foro encontrado para os d�bitos selecionados");
                }
            }
        } else {
            $iTipoModeloRecibo = 2;
        ...
        /*
         * valida��es para emiss�o de guia atravvez da nota
         * cadtipmod 21
         */
        ...
        if ($oParJuridico->v19_partilha == "t" && $lEmissaoCarne && $oArretipo->k03_tipo == 18) {
            ...
            throw new Exception("Tipo de regra de emiss�o n�o encontrado");
        }
        ...
        if ($iRowsRegraEmissao > 0) {
            ...
            /**
             * Separa as regras de emissao se s�o regras gerais e regras especificas para tipo de d�bito
             */
            ...
            foreach ($aRegrasEmissao as $iIndiceRegra => $oRegraEmissao) {
                /**
                 * valida se tipo de convenio � refente cobran�a
                 */
                ...
                /**
                 * Instancia novo recibo para ser gerado ou n�o
                 */
                ...
                if (!empty($this->sHistorico)) {
                    $oRecibo->setHistorico($this->sHistorico);
                }
                ...
                foreach ($aChecks as $iIndice => $aValores) {
                    ...
                    if (($aValores["Numpar"] >= $oRegraEmissao->k48_parcini) && ($aValores["Numpar"] <= $oRegraEmissao->k48_parcfim)) {
                        /**
                         * Se for emissao de recibo, adiciona numpre ao recibo caso contr�rio apenas seta par�metro para emissao de carne
                         */
                        if (($lCobrancaRegistrada || empty($sGeraCarne)) && $oRegraEmissao->k48_cadtipomod == $iTipoModeloRecibo) {
                             ...
                             * lista os recibos v�lidos emtidos
                             */
                            if ($lConfReemissaoRecibo) {
                        ...
                        /**
                         * Cria array de dados que ser�o gravados na sessao
                ...
                if ($sNumNov != "") {
                    ...
                    foreach ($aNumpreNumpar as $oNumpreNumpar) {
                         ...
                         * Caso exista caso para compara��o apenas faz reemissao do recibo
                ...
                if ((!in_array(0,
                      $iDiferenca) && ($lCobrancaRegistrada || $sGeraCarne == "") && $this->lNovoRecibo) || ($sGeraCarne == "" && !$lConfReemissaoRecibo)) {
                    /**
                    ...
                    if (count($oRecibo->getDebitosRecibo()) > 0) {
                        try {
                            ...
                            if (!$lForcaVencimento) {
                                /**
                                 *
                                 * Caso a data do sistema foi maior que as datas de vencimento, significa que parcelas est�o vencidas e
                                ...
                                // @note: n�o usado pelo nfse (e-cidadeonline2)
                            ...
                            //@note: se vencimento � no ano seguinte, vencimento � ultimo dia do ano
                            ...
                            */
                            ...
                            if (in_array($oRegraEmissao->k03_tipo,
                                $aTipoInicial) && $oRegraEmissao->ar11_cadtipoconvenio == regraEmissao::getConveioCustaBoleto()) {
                                /**
                                 * Valida se existe custas para o recibo gerado
                                 */
                                $sSqlValidaCustas = $oDaoProcessoForoPartilhaCusta->sql_query_file("", "*", "",
                                  "v77_numnov ={$oRecibo->getNumpreRecibo()} ");
                                ...
                                /**
                                 * Busca o processo do foro conforme tipo de d�bito
                                 */
                                if ($oRegraEmissao->k03_tipo == 13) {
                                    ...
                                    $sSqlProcessosForo .= " where v59_inicial in (" . implode(", ",
                                        $aIniciais) . ")               ";
                                }
                                ...
                                /**
                                 * Valida se existem processos do foro e se n�o existem custas vinculadas a ele.
                                 */
                                if ($iProcessosForo > 0 || $iNumRowsValidaCustas == 0) {
                                    ...
                                    foreach ($aProcessosForo as $oProcessoForo) {
                                        /**
                                         * Verifica se existe lan�amento manual ou isen��o
                                        ...
                                        if ($rsSqlPartilhaProcesso && $iNumRowsPartilhaProcesso == 0) {
                                            ...
                                            /**
                                             * Calcula o valor total de d�bitos no arrecad referentes ao processo do foro
                                             */
                                            ...
                                            foreach ($aTaxas as $oTaxa) {
                                                if ($oTaxa->ar36_perc == 0) {
                                                    ...
                                                    $oValor = new DBNumber();
                                                    $nVlrPercentualDebito = $oValor->truncate($nTotalDebito * ($oTaxa->ar36_perc / 100),
                                                      2);

                                                    /**
                                                     * Verifica se valor do percentual do d�bito � maior que maximo ou minimo permitido
                                                     * caso ele ultrapasse um dos limites o valor da taxa ser� o limite
                                                     * caso contrario sera o resultado da opera�ao
                                                     */
                                ...
                                }//Fim valida��o Processo Foro

                                /**
                                 *  Gera��o do Convenio de Cobran�a
                                 */
                                db_app::import('convenio');
                            }//Valida��o regra de emissao com as parcelas do numpre e numpar
                            ...
                            $sSqlTxBancaria .= "       inner join arrecad on arretipo.k00_tipo = arrecad.k00_tipo ";
                            $sSqlTxBancaria .= " where arretipo.k00_instit = " . db_getsession("DB_instit");
                            $sSqlTxBancaria .= "   and arrecad.k00_numpre in ({$iNumpre})";

                            $rsSqlTxBancaria = db_query($sSqlTxBancaria);

                            $fValorTaxaBancaria = 0.00;

                            if (pg_num_rows($rsSqlTxBancaria) > 0) {
                                $fValorTaxaBancaria = db_utils::fieldsMemory($rsSqlTxBancaria, 0)->tx_banc;
                            }

                            $nValorRecibo = $oRecibo->getTotalRecibo() + $nValorTotalCustas + $fValorTaxaBancaria;
                            $sValorCodigoBarras = str_pad(number_format($nValorRecibo, 2, "", ""), 11, "0",
                              STR_PAD_LEFT);
                            ...
                            if ($lConvenioCobrancaValido) {
                                if (CobrancaRegistrada::utilizaIntegracaoWebService($oRegraEmissao->k48_cadconvenio)) {
                                    CobrancaRegistrada::registrarReciboWebservice($oRecibo->getNumpreRecibo(),
                                      $oRegraEmissao->k48_cadconvenio, $oRecibo->getTotalRecibo(), true);
                                } else {
            ...
            /**
             * @todo manter?
             * Integra��o Webservice com TJ RJ
             */
            if (count($aRecibosComCustasEmitidos) > 0) {
                include(modification('cai3_integracaotjemissao001.php'));
            }
        } else {
            throw new Exception("Erro: Nenhuma regra cadastrada para este tipo de d�bito ({$this->tipo_debito}). \nVerifique Par�metros.");
        }
    ...
    /**
     * Retorna os D�bitos  selecionados no formul�rio da CGF
     * Com minimo e maximo de parcelas array com combina��o de numpre, numpar e receita e string de retorno
     *
     * @param object $oFormulario - Objeto contentdo os dados do Formul�rio da CGF
     * @param integer $iTipoAgrupamento - Tipo de Agrupamento do Tipo de D�bito
     * @return stdClass
     */
    public function retornaDebitosSelecionados($oFormulario, $iTipoAgrupamento = null)
    {
        ...
        if (isset($oFormulario->inicial)) {
            foreach ($oFormulario->aDadosNumpre as $sChave => $sValor) {
                if (stripos(" " . $sChave, "CHECK")) {
                    ...
                    $sSqlInicial .= "  where v59_inicial in (" . implode(", ",
                        $aInicial) . ");                          ";
                    ...
                    foreach ($aIniciais as $oInicial) {
                        ...
                        /**
                         * Cria array com os numpres e numpar dos d�bitos
                         */
        ...
        /**
         * Caso o agrupamento do d�bito seja do tipo 2 (Parcial)
         * Adiciona os d�bitos do arrecad ao recibo.
         */
        ...
        /**
         * Valida loteador quando matricula n�o estiver setada
         */
        $lLoteador = false;
        ...
        /**
         * Variaveis para chamada da fun��o
         * recibodesconto
         */
