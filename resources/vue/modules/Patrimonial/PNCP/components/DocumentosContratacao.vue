<template>
    ...
    >
        <Column field="statusAtivo" header="Compra" class="card flex justify-content-center">
            <template #body="data">
                <Tag :value="ativo" :severity="getStatusLabel(data)"/>
            </template>
        </Column>
        ...
        <Column field="tipoDocumentoDescricao" header="Tipo Documento"></Column>
    ...
    <Dialog v-model:visible="deleteDocumentoDialog" :style="{width: '450px'}" header="Confirmar" :modal="true">
        ...
        <div class="p-fluid grid formgrid">
            <div class="field col-12 md:col-12 mt-5">
                ...
                <p style="color: cornflowerblue">Justificativa não obrigatoria.</p>
            </div>
        </div>
        <template #footer>
...
watch(() => props.sequencialCompra, async (newValue, oldValue) => {
    loading.value = true;
    const formData = new FormData;
    formData.append('ano', props.anoCompra);
    formData.append('sequencial', props.sequencialCompra);
    try {
        const response = await axios.post('v4/api/patrimonial/pncp/compraEditalAviso/buscar-documentos', formData);
        loading.value = false;
...
const props = defineProps(['sequencialCompra', 'anoCompra']);
...
const deleteDocumento = async () => {
    ...
    try {
        await axios.post('v4/api/patrimonial/pncp/compraEditalAviso/excluir-documento', documento.value)
        loading.value = false;
        justificativa.value = '';
        documento.value = {};
