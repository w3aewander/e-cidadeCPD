...
try {
  ...
  $str_where = "sd63_c_procedimento is not null ";
  ...
  for ($i = 0; $i < pg_num_rows($res_prontproced); $i++) {
    ...
    if ( !array_key_exists($sd03_i_cgm, $aProfissionais) ) {
      ...
      for ( $iX = 0; $iX < pg_num_rows($rsMatriculas); $iX++ ) {
        ...
        $aProfissionais[$sd03_i_cgm]['matriculas'][] = $sMatricula;
      }
    }
    ...
    if ( !array_key_exists($sd63_c_procedimento, $aProfissionais[$sd03_i_cgm]['ups'][$sd02_i_codigo]['atendimentos'][$sd24_i_codigo]['procedimentos']) ) {
      $aProfissionais[$sd03_i_cgm]['ups'][$sd02_i_codigo]['atendimentos'][$sd24_i_codigo]['procedimentos'][$sd63_c_procedimento] = $sd63_c_nome;
    }
  }
