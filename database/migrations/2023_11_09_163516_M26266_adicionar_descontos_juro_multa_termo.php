...
class M26266AdicionarDescontosJuroMultaTermo extends Migration
{
    public function up()
    {
        $sql = <<<SQL
...
declare
  ...
  v_cadtipoparcdepoislancamento    integer default 0;
  v_descontojurdepoislancamento    float8 default 0;
  v_descontomuldepoislancamento    float8 default 0;
  ...
  v_entradaminima                  numeric default 0;
  ...
  begin
    ...
    -- senao for inicial do foro
      ...
      loop
        ...
        if lRaise is true then
          perform fc_debug('Processando... matricula: '||v_record_origem.k00_matric, lRaise, false, false);
          perform fc_debug('Processando... inscricao: '||v_record_origem.k00_inscr, lRaise, false, false);
        end if;
    ...
    select cadtipoparc.k40_codigo, k42_minentrada
      into v_cadtipoparc, v_entradaminima
      from tipoparc
    ...
    -- caso nao encontre uma regra de parcerlamento, busca regra de desconto para aplicar depois do lancamento
    -- desse modo o percentual de descontos no juro e multa podera ser inserido na tabela termo
    if v_cadtipoparc is null then
      select cadtipoparc.k40_codigo
       into v_cadtipoparcdepoislancamento
       from tipoparc
            inner join cadtipoparc on cadtipoparc = k40_codigo
      where maxparc       > 1
        and dDataUsu     >= cadtipoparc.k40_dtini
        and dDataUsu     <= cadtipoparc.k40_dtfim
        and k40_codigo    = v_desconto
        and k40_aplicacao = 2 -- Aplicar Depois do Lancamento
      order by maxparc
      limit 1;
      ...
      for v_record_desconto in select *
                             from tipoparc
                            where maxparc     > 1
                              and cadtipoparc = v_cadtipoparcdepoislancamento
                              and cadtipoparc = v_desconto
                              and dDataUsu     >= tipoparc.dtini
                              and dDataUsu     <= tipoparc.dtfim
                            order by maxparc
      loop

        if v_totalparcelas >= v_ultparc and v_totalparcelas <= v_record_desconto.maxparc then
          v_descontomuldepoislancamento = v_record_desconto.descmul;
          v_descontojurdepoislancamento = v_record_desconto.descjur;
          exit;
    ...
    loop
      ...
      perform j150_receita
        ...
        where j150_receita = v_record_recpar.receit
          and v59_numpre   = v_record_recpar.numpre;
    ...
    if lRaise is true then
      perform fc_debug('v_totalliquido: '||v_totalliquido,lRaise,false,false);
    end if;
    ...
    insert into termo ( v07_parcel,
               ...
               values ( v_termo,
                        ...
                        nVlrMul,
                        nVlrJur,
                        v_descontojurdepoislancamento,
                        v_descontomuldepoislancamento,
                        v_login,
                        v_cgmresp,
                        sObservacao,
    ...
    if abs(v_teste) between nFaixaInicial and nFaixaFinal and v_juronaultima is false then
      ...
      ultimaparcelareceita := v_totalparcelas;

      if v_entradaminima = 100 then
        ultimaparcelareceita := 1;
      end if;
      ...
      select k00_receit
        ...
        from arrecad
       where k00_numpre = v_numpre
         and k00_numpar = ultimaparcelareceita
      ...
      perform fc_debug('alterar diferenca: v_numpre: '||v_numpre||' v_entradaminima:'||v_entradaminima,lRaise,false,false);
