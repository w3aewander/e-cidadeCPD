...
// Tipos de turma (ed57_i_tipoturma)
//   1 - NORMAL
//   2 - EJA
//   3 - MULTIETAPA
...
try {
  ...
  $aCursosMinistradosNoAno = buscaEtapasCursosMinistrados($aWhere, $oDtInicio);

  $aCursosErEJA = montaEstruturaCursoEtapas($aCursosMinistradosNoAno, false);
  foreach($aCursosErEJA  as $oCurso) {
    $iNumeroColunasPrimeiroQuadro += $oCurso->iNumeroColunasEtapas +1;
    $lRelatorioSemDadosParaPeriodoInformado = false;
  }

  calculaAlunosMatriculadosEscola($aEscolasPrimeiroQuadro, $aCursosErEJA, $oDtInicio, $oDtFim);
  ...
  $aCursosMinistradosNoAno = buscaEtapasCursosMinistrados($aWhere, $oDtInicio);
  $aCursosSegundoQuadro    = montaEstruturaCursoEtapas($aCursosMinistradosNoAno, true);
  foreach($aCursosSegundoQuadro  as $oCurso) {
    $iNumeroColunasSegundoQuadro += $oCurso->iNumeroColunasEtapas +1;
  }

  calculaAlunosMatriculadosEscola($aEscolasSegundoQuadro, $aCursosSegundoQuadro, $oDtInicio, $oDtFim);
  /**
  ...
  foreach ($aEscolasTerceiroQuadro as $oEscola) {
    ...
    foreach ($aAlunoMatriculadosSexo as $oAlunosSexoTurno) {
      if(isset($oEscola->aTurnos[$oAlunosSexoTurno->turnoreferente])){
  ...
  /** ******************************************************************************************************************
   ******************************* INICIO da busca dos dados do QUARTO QUADRO ******************************************
   ****************************************************************************************************************** **/
...
/**
 * Monta a estrutura dos cursos e suas etapas para os quadros 1 e 2
 * @param  array   $aCursosMinistradosNoAno array com os cursos ministrados
 * @param  boolean $lCorrecaoFluxo          se é cursos com turmas de correção
 * @return array
 */
function montaEstruturaCursoEtapas($aCursosMinistradosNoAno, $lCorrecaoFluxo) {
  ...
  foreach ($aCursosMinistradosNoAno as $oDadosCurso) {
    ...
    $iCodigoCurso = $oDadosCurso->cod_curso;

    // cria o curso no array
    if ( !array_key_exists($oDadosCurso->cod_curso, $aCursos) ) {

      $oCurso              = new stdClass();
      $oCurso->iCurso      = $oDadosCurso->cod_curso;
      ...
      $aCursos[$iCodigoCurso]  = $oCurso;
    }

    $oEtapa              = new stdClass();
    $oEtapa->iEtapa      = $oDadosCurso->cod_etapa;
    $oEtapa->sEtapa      = $oDadosCurso->etapa;
    $oEtapa->sAbrev      = $oDadosCurso->etapa_abrev;
    $oEtapa->iFill       = 0;
    $oEtapa->iTotalEtapa = 0;

    $oCurso->iNumeroColunasEtapas ++;
    $aCursos[$iCodigoCurso]->aEtapas[$oDadosCurso->tipo][] = $oEtapa;
  }

  /**
   * Adiciona o totalizador por base (Inicial e Final)
   */
  foreach ($aCursos as $oDadosCurso) {

    if ( count($oDadosCurso->aEtapas) == 2 ) {

      $oEtapa                    = new stdClass();
      $oEtapa->iEtapa            = null;
      $oEtapa->sEtapa            = 'T. Inicial';
      $oEtapa->sAbrev            = 'T. Inicial';
      $oEtapa->iFill             = 1;
      $oEtapa->iTotalEtapa       = 0;
      $oDadosCurso->aEtapas[1][] = $oEtapa;
      $oDadosCurso->iNumeroColunasEtapas ++;

      $oEtapa                    = new stdClass();
      $oEtapa->iEtapa            = null;
      $oEtapa->sEtapa            = 'T. Final';
      $oEtapa->sAbrev            = 'T. Final';
      $oEtapa->iFill             = 1;
      $oEtapa->iTotalEtapa       = 0;
      $oDadosCurso->aEtapas[2][] = $oEtapa;
      $oDadosCurso->iNumeroColunasEtapas ++;
    }
  }
...
/**
 * Calcula os alunos matrículados nas escolas e totaliza as etapas e cursos.
 * @param  array  $aEscolas  array com todas escolas ativas
 * @param  array  $aCursos   array com os cursos onde tem alunos matrículados
 * @param  DBDate $oDtInicio
 * @param  DBDate $oDtFim
 */
function calculaAlunosMatriculadosEscola($aEscolas, $aCursos, $oDtInicio, $oDtFim) {
  ...
  foreach ($aEscolas as $oEscola) {
    ...
    foreach ($aCursos as $oCurso) {
      ...
      $iTotalCurso = 0;
      foreach ($oCurso->aEtapas as $aEtapa) {
        ...
        foreach ($aEtapa as $oEtapa) {
          ...
          if (is_null($oEtapa->iEtapa)) {
            ...
            // Cria o totalizador por base da etapa
            $sHash                         = "{$oCurso->iCurso}#$oEtapa->sAbrev";
            $oEtapa->iTotalEtapa          += $iTotalEtapaBase;
            $oEscola->aAlunosEtapa[$sHash] = $iTotalEtapaBase;
            continue;
          }

          $sHash                = "{$oCurso->iCurso}#{$oEtapa->iEtapa}";
          $iTotalAlunosEtapa    = buscaAlunosMatriculados($oEscola->codigo, $oEtapa->iEtapa, $oDtInicio, $oDtFim, $oCurso->lCorrecaoFluxo);
          // echo "Escola: $oEscola->codigo, Etapa: $oEtapa->iEtapa - $oEtapa->sEtapa  , Total alunos: {$iTotalAlunosEtapa} <br>";
          $iTotalCurso         += $iTotalAlunosEtapa;
          $oEtapa->iTotalEtapa += $iTotalAlunosEtapa;
          $iTotalEtapaBase     += $iTotalAlunosEtapa;

          $oEscola->aAlunosEtapa[$sHash] = $iTotalAlunosEtapa;
        }
      }
      $sHash                         = "{$oCurso->iCurso}#T";
      $oEscola->aAlunosEtapa[$sHash] = $iTotalCurso;
      $oCurso->iTotalCurso          += $iTotalCurso;
      $oEscola->iTotalEscola        += $iTotalCurso;
    }
  }
}
...
/**
 * Busca os alunos matrículados em turmas de mais educação
 * @param  integer $iEscola   código da escola
 * @param  DBDate  $oDtInicio período da matricula
 * @param  DBDate  $oDtFim    período da matricula
 * @return integer            Número de alunos matriculados
...
/**
 * Abrevia uma string removendo as preposições retornando a primeira letra de cada palavra presente na $string
...
function abreviaNomeCurso($sString) {
  ...
  return implode('', $aAbrev);
}

/**
 * Busca os cursos onde foi criado turmas
 * @param  array  $aFiltros  array com os filtros usados na busca
 * @param  DBDate $oDtInicio
 * @return array
 */
function buscaEtapasCursosMinistrados($aFiltros, $oDtInicio) {

  $sWhere   = implode(" and ", $aFiltros);

  $sCampos  = " distinct ed141_sequencial as cod_base, ed141_tipo as tipo, ed141_cursoedu as cod_curso, ";
  $sCampos .= " ed29_c_descr as curso, ed10_i_codigo as cod_ensino, trim(ed10_c_descr) as ensino, ed10_c_abrev as ensino_abrev,";
  $sCampos .= " ed11_i_codigo as cod_etapa, trim(ed11_c_descr) as etapa, trim(ed11_c_abrev) as etapa_abrev, ";
  $sCampos .= " ed10_ordem, ed11_i_sequencia";

  $sSqlCurso  = " select {$sCampos}";
  $sSqlCurso .= "   from basecurricular ";
  $sSqlCurso .= "   join basecurricularserie on ed142_basecurricular = ed141_sequencial ";
  $sSqlCurso .= "   join cursoedu            on ed29_i_codigo = ed141_cursoedu ";
  $sSqlCurso .= "   join serie               on ed11_i_codigo = ed142_serie ";
  $sSqlCurso .= "   join ensino              on ed10_i_codigo = ed11_i_ensino";
  $sSqlCurso .= "   join base ON base.ed31_i_curso = cursoedu.ed29_i_codigo";
  $sSqlCurso .= "   join turma ON turma.ed57_i_base = base.ed31_i_codigo";
  $sSqlCurso .= "   join calendario ON calendario.ed52_i_codigo = ed57_i_calendario";
  $sSqlCurso .= "  where {$sWhere}";
  $sSqlCurso .= "  order by ed10_ordem, ed11_i_sequencia; ";
  $rsCurso    = db_query($sSqlCurso);
  if (!$rsCurso) {
    throw new Exception("Falha ao buscar Níveis de Ensino.\n" . pg_last_error());
  }

  return db_utils::getCollectionByRecord($rsCurso);
}
...
function buscaEscolas() {
  ...
  $sWhere  = "ed18_i_funcionamento = 1 and ed18_i_tipoescola = 1";
  ...
  for ($i = 0; $i < $iEscolas; $i++) {
    ...
    $oEscola->iTotalEscola = 0;
    $oEscola->iTotalMatriculasMaisEducacao = 0;
    $aEscolas[] = $oEscola;
  }
  return $aEscolas;
}
...
/**
 * Quantifica os alunos por turno e  sexo
 * @param  integer $iEscola   código da escola
 * @param  DBDate  $oDtInicio período da matricula
 * @param  DBDate  $oDtFim    período da matricula
 * @return integer            Número de alunos matriculados
...
if (!empty($iNumeroColunasPrimeiroQuadro)) {
  $oConfig->iXColunasEtapasPrimeriro = $oConfig->iXMaximoEnsino / $iNumeroColunasPrimeiroQuadro;
}

// SEGUNDO QUADRO
$oConfig->iXColunasEtapasSegundo   = ($oConfig->iXMaximoEnsino - $oConfig->iOutrasColunas);
if ( !empty($iNumeroColunasSegundoQuadro) ) {
  $oConfig->iXColunasEtapasSegundo   = ($oConfig->iXMaximoEnsino - $oConfig->iOutrasColunas) / $iNumeroColunasSegundoQuadro;
}
...
$oPdf->addTitulo("Mapa Estatístico", 1);
$oPdf->addTitulo("Período: {$oGet->dtInicio} até {$oGet->dtFim}", 2);
...
/**
 * Imprime os Alunos matriculados nos ensinos de Níveis ER e EJA de todas as escolas
 * Este bloco abrange todos os alunos matriculados no período informado em turmas do tipo:
 * 1 - NORMAL
 * 2 - EJA
 * 3 - MULTIETAPA
 */
if (count($aCursosErEJA) > 0) {

  imprimePrimeiroCabecalho($oPdf, $oConfig, $aCursosErEJA);
  $iLinhasImpressas = 0;
  ...
  foreach ($aCursosErEJA as $oCurso) {
    ...
    $iTotalGeralPrimeiroQuadro += $oCurso->iTotalCurso;
    foreach ($oCurso->aEtapas as $aEtapa) {
    ...
    $oPdf->Cell($oConfig->iXColunasEtapasPrimeriro,  5, $oCurso->iTotalCurso, 1, 0, 'C', 1);
  }
 ...
 */
imprimeSegundoCabecalho($oPdf, $oConfig, $aCursosSegundoQuadro);
$iLinhasImpressas             = 0;
...
foreach ($aEscolasSegundoQuadro as $oEscola) {
  ...
  if (count($aCursosSegundoQuadro) == 0) {
  ...
  $oPdf->Line($oPdf->GetLeftMargin(), $oPdf->GetY() + 5, $oConfig->iXMaxima, $oPdf->GetY() + 5);
  $oPdf->ln();
...
foreach ($aCursosSegundoQuadro as $oEnsino) {

  $iTotalGeralSegundoQuadro += $oEnsino->iTotalCurso;
  foreach ($oEnsino->aEtapas as $aEtapa) {

    foreach ($aEtapa as $oEtapa) {
      $oPdf->Cell($oConfig->iXColunasEtapasSegundo,  5, $oEtapa->iTotalEtapa, 1, 0, 'C', 1);
    }
  }
  $oPdf->Cell($oConfig->iXColunasEtapasSegundo,  5, $oEnsino->iTotalCurso, 1, 0, 'C', 1);
}

$oPdf->Line($oPdf->GetLeftMargin(), $oPdf->GetY(), $oConfig->iXMaxima, $oPdf->GetY());
...
foreach ($aCursosErEJA as $oCurso) {
  ...
  $oPdf->Cell(200, 5, " ({$oCurso->sCursoAbrev}) - {$oCurso->sCurso}",      1, 0, 'L', 0);
  $oPdf->Cell(81,  5, $oCurso->iTotalCurso, 1, 1, 'R', 0);
}

foreach ($aCursosSegundoQuadro as $oCurso) {

  $oPdf->Cell(200, 5, " ({$oCurso->sCursoAbrev}) - {$oCurso->sCurso}  - CORREÇÃO DE FLUXO",     1, 0, 'L', 0);
  $oPdf->Cell(81,  5, $oCurso->iTotalCurso, 1, 1, 'R', 0);
}
 ...
 * @param ECidade\Pdf\Pdf $oPdf    Instancia de FPDF
 * @param StdClass           $oConfig  Configuração do relatorio
 */
function addPage(ECidade\Pdf\Pdf $oPdf, $oConfig, $lImprimeLinhaMovimentacao = true) {
 ...
 */
function cortaString(ECidade\Pdf\Pdf $oPdf, $sNome, $sNomeAbreviado, $iTamanhoCampo) {
 ...
 * @param  ECidade\Pdf\Pdf     $oPdf        Instancia de FPDF
 * @param  StdClass            $oConfig     Configuração do relatorio
 * @param  array               $aCursosErEJA Array com os ensinos e as etapas
 * @return void
 */
function imprimePrimeiroCabecalho(ECidade\Pdf\Pdf $oPdf, $oConfig, $aCursosErEJA) {
  addPage($oPdf, $oConfig);
  ...
  $oPdf->SetY($iYInicial);
  $oPdf->SetX($oPdf->GetLeftMargin() + $oConfig->iXEscola);
  ...
  foreach ($aCursosErEJA as $oCurso) {
    ...
    // iNumeroColunasEtapas = numero de etapas e a coluna de totalizador por base. O +1
    $iXEnsino = ($oCurso->iNumeroColunasEtapas + 1) * $oConfig->iXColunasEtapasPrimeriro;

    $sEnsino = cortaString($oPdf, $oCurso->sCurso, $oCurso->sCursoAbrev, $iXEnsino);
  ...
  foreach ($aCursosErEJA as $oCurso) {
    ...
    foreach ($oCurso->aEtapas as $aEtapa) {
      ...
      foreach ($aEtapa as $oEtapa) {
        $oPdf->vCell($oConfig->iXColunasEtapasPrimeriro, $iAlturaColunaEtapa, $oEtapa->sAbrev, 1, 0, 'C', $oEtapa->iFill);
      }
    }
    // coluna de totalizador
    $oPdf->vCell($oConfig->iXColunasEtapasPrimeriro, $iAlturaColunaEtapa, 'Total', 1, 0, 'C', 1);
  }
...
function imprimeSegundoCabecalho($oPdf, $oConfig, $aEtapasSegundoQuadro) {
  ...
  $iXCorrecaoFluxo = 0;
  foreach ($aEtapasSegundoQuadro as $oCurso) {
    $iXCorrecaoFluxo += ($oCurso->iNumeroColunasEtapas + 1 ) * $oConfig->iXColunasEtapasSegundo;
  }
  ...
  foreach ($aEtapasSegundoQuadro as $oCurso) {
    ...
    $iXCurso = ($oCurso->iNumeroColunasEtapas + 1) * $oConfig->iXColunasEtapasSegundo;
    $sCurso  = cortaString($oPdf, $oCurso->sCurso, $oCurso->sCursoAbrev, $iXCurso);
    $oPdf->Cell($iXCurso, $iAlturaLinha, $sCurso, 1, 0, 'C');
  }
  ...
  foreach ($aEtapasSegundoQuadro as $oCurso) {
    ...
    foreach ($oCurso->aEtapas as $aEtapa) {
      ...
      foreach ($aEtapa as $oEtapa) {
        $oPdf->vCell($oConfig->iXColunasEtapasSegundo, $iAlturaColunaEtapa, $oEtapa->sAbrev, 1, 0, 'C', $oEtapa->iFill);
      }
    }
    $oPdf->vCell($oConfig->iXColunasEtapasSegundo, $iAlturaColunaEtapa, 'Total', 1, 0, 'C', $oEtapa->iFill);
  }
...
function imprimeTerceiroCabecalho($oPdf, $oConfig) {
  ...
  $iAlturaLinha = 5;
  $iXmargim     = $oPdf->GetLeftMargin() + $oConfig->iXEscola;
  $iYInicial    = $oPdf->GetY();
