...
try {
    ...
    switch ($parametros->exec) {
        case "getDadosParcelamento":
            ...
            $parcel = $parametros->parcel;

            $retorno->aListaSimulacao = "";
            $retorno->iTipoAnulacao = "";
            $retorno->oTotal = "";
            $retorno->iStatus = "1";
            $retorno->sMensagem = "";
            ...
            $codigoRegraParcelamento = $oTermo->k40_codigo;
            ...
            foreach ($aSimulacao as $stdLinha) {
                ...
                ");
                if (!$updateSimulacao) {
                	throw new Exception("Não foi possível atualizar os valorde de Juro e Multa.");
				}
            }

            $_SESSION["aListaSimulacao"] = serialize($aSimulacao);
            $retorno->aListaSimulacao = $aSimulacao;
            $retorno->iTipoAnulacao = $oTermo->k40_tipoanulacao;
            $retorno->oTotal = $oTotal;
        break;

    case "setConfirmaExclusao" :

            if (empty($parametros->parcel)) {
                throw new Exception('É necessário selecionar um parcelamento para anular.');
            }
        ...
        if (db_utils::ativaParcelamentoCustas()) {
            // coloca custas e honorarios do arrecad para o arreold
            $parcelamentoCustas = new ParcelamentoCustasProcessoService();
            $parcelamentoCustas->inserirTaxasArreold($parametros->parcel);
            
            // vincula o estado atual do parcelamento de custas ao parcelamento que esta sendo anulado,
            // desse modo a liberacao do parcelamento de custas manual devera ser refeita para um proximo parcelamento
            $parcelamentoCustas->vincularParcelamento($parametros->parcel);
        }

            $termoRepository = TermoRepository::getInstance();
            $parcelamento = $termoRepository->getByCode($parametros->parcel);
            ...
            $sqlAnulacao = "SELECT fc_excluiparcelamento({$parametros->v21_sequencial}, {$parametros->usuario}, '{$parametros->motivo}', {$processo})";
            $rsAnulacao = db_query($sqlAnulacao);

            if (!$rsAnulacao) {
                throw new Exception('Não foi possível anular o parcelamento.');
        }
            ...
            $retorno->mensagem = "Parcelamento anulado com sucesso!";
