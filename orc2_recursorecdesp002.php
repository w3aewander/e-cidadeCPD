...
select *,
 ...
 from (select o15_recurso,
         ...
         from (select 0::int as tipo,o70_codigo,
                      sum(substr(fc_receitasaldo({$ano}, o70_codrec, 1, '{$dataInicio}', '{$dataInicio}'), 2, 15)::float8)
                 from orcreceita
                where o70_anousu = {$ano}
                  and o70_instit in ($instit)
               ...
               select 1::int as tipo,
                      o58_codigo,
          	          sum(substr(fc_dotacaosaldo({$ano}, o58_coddot, 1, '{$dataInicio}', '{$dataInicio}'), 2, 15)::float8)
                 from orcdotacao
      ...
      join fonterecurso on orctiporec_id = o15_codigo
           and exercicio = {$ano}
      group by o15_recurso, gestao, o15_complemento, o15_descr
     ) as x
