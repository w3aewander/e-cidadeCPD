...
if(isset($z01_numcgm) && $z01_numcgm != ""){
  ...
  if($clissruas->numrows > 0){
    ...
    if($clcgm->numrows > 0 && $rua == ""){
     ...
     $result    = $clissbase->sql_record($clissbase->sql_query_file($q02_inscr));
    include(modification("classes/db_cgm_classe.php"));
    db_fieldsmemory($result,0);
    $clcgm = new cl_cgm;
    $result = $clcgm->sql_record($clcgm->sql_query($q02_numcgm));
  ...
  include_once(modification("classes/db_fis_fiscalcgm_classe.php"));
  $clfiscalcgm = new cl_fis_fiscalcgm;
  $result = $clfiscalcgm->sql_record($clfiscalcgm->sql_query(null,"*",null," y36_codnoti = {$y30_codnoti} and y30_instit = ".$instituicao ));
  if($clfiscalcgm->numrows > 0){
    db_fieldsmemory($result,0);
  }
  include_once(modification("classes/db_fis_fiscalinscr_classe.php"));
  $clfiscalinscr = new cl_fis_fiscalinscr;
  $result = $clfiscalinscr->sql_record($clfiscalinscr->sql_query(null,"*",null," y34_codnoti = {$y30_codnoti} and y30_instit = ".$instituicao));
  if($clfiscalinscr->numrows > 0){
    db_fieldsmemory($result,0);
  }
  include_once(modification("classes/db_fis_fiscalmatric_classe.php"));
  $clfiscalmatric = new cl_fis_fiscalmatric;
  $result = $clfiscalmatric->sql_record($clfiscalmatric->sql_query(null,"*",null," y35_codnoti = {$y30_codnoti} and y30_instit = ".$instituicao));
  if($clfiscalmatric->numrows > 0){
    db_fieldsmemory($result,0);
  }
  include_once(modification("classes/db_fis_fiscalsanitario_classe.php"));
  $clfiscalsanitario = new cl_fis_fiscalsanitario;
  $result = $clfiscalsanitario->sql_record($clfiscalsanitario->sql_query(null,"*",null," y37_codnoti = {$y30_codnoti} and y30_instit = ".$instituicao));
  ...
  $dados = "<a onClick=\"js_abre('fis3_fis_fiscal006.php?y30_codnoti=$y30_codnoti');return false;\" href=''>notificação: ".$y30_codnoti." &nbsp;|&nbsp;".@$z01_nome."</a>";
}
...
<table>
  ...
  <tr>
    ...
    <?php
      ...
      $sqlTipoFiscaliza = <<<SQL
        select
          *
        from
          fiscalizacao.fis_tipofiscaliza
          left join fiscalizacao.fis_fisdocdep on y27_codtipo = fd02_codtipo and fd02_instit = y27_instit
        where
          y27_instit = $instituicao
          and case when exists (select 1 from fiscalizacao.fis_fisdocdep a where y27_instit = a.fd02_instit) then
            fd02_coddep = $departamento
          else true end
SQL;

      $result_tipofisc = db_query($sqlTipoFiscaliza);
      ...
      ?>
                ...
                <tr id="listaParagrafo">
                    ...
                    <td>
                      <select name="paragrafos" id="paragrafos">
                        <option value=""></option>
                        <?php
                            $pSql  = 'select distinct pl09_paragrafo, pl09_descr from fiscalizacao.fis_paragrafo where pl09_status = true and pl09_tipo = 1  and pl09_coddepto = '.$departamento;
                            $pRs   = db_query($pSql);
                ...
                <?php
                    ...
                    if (isset($y50_codauto)){
                       ...
                       $paRs   = db_query($paSql);
                       if (is_resource($paRs) && pg_num_rows($paRs) > 0) {
                          ...
                          for ($i=0; $i < $paRows; $i++) {
                              ...
                              } else if($pl10_paragrafo == 2){
                                  $textoInfringencia = $pl10_texto;
                                  $codInfringencia   = $pl10_codigo;

                              } else if($pl10_paragrafo == 3){
                                  $textoSancao = $pl10_texto;
                                  $codSancao   = $pl10_codigo;

                              } else if($pl10_paragrafo == 4) {
                                  $textoBaseLegal = $pl10_texto;
                                  $codBaseLegal   = $pl10_codigo;
  ...
  <!-- <tr>
    ...
    <td>
      ...
      //}
      db_inputdata('y50_dtvenc',@$y50_dtvenc_dia,@$y50_dtvenc_mes,@$y50_dtvenc_ano,true,'hidden',$db_opcao,"");
      ?>
      <strong>Prazo p/ Recurso:</strong>
      <?php
      db_inputdata('y50_prazorec',@$y50_prazorec_dia,@$y50_prazorec_mes,@$y50_prazorec_ano,true,'hidden',$db_opcao,"");
      ?>
    </td>
  </tr>
 -->
  ...
  <tr>
    ...
    <td>
      <?php
      $j14_nome = (@$y14_codigo=="" or @$y14_codigo==null)?@$z01_ender:@$j14_nome;
      if ($j14_nome == '' or $j14_nome == null) $j14_nome = $j14_nome_exec;
  ...
  <tr>
    ...
    <td>
      <?php
      if(!isset($y14_numero) or empty($y14_numero)){
       $y14_numero = 0;
      }
  ...
  <tr>
    ...
    <td nowrap>
      <?php
        $j13_descr = (@$y14_codi=="" or @$y14_codi==null)?@$z01_bairro:@$j13_descr;
	if ($j13_descr == '' or $j13_descr == null) $j13_descr = $j13_descr_exec;
...
?>
<input name="novo" type="button" id="novo" value="Incluir Novo" onclick="parent.location.href='fis1_fis_auto005.php';" >
<?php
}
if($db_opcao==1||$db_opcao==2){
...
function js_pesquisa_tipofisc(mostra){
  if(mostra==true){
    ...
    js_OpenJanelaIframe('','db_iframe_tipo','func_fis_tipofiscaliza.php?pesquisa_chave='+document.form1.y50_codtipo.value+'&funcao_js=parent.js_mostratipo','Pesquisa',false);
  }
}
...
function js_pesquisa()
{
    sUrl = 'func_fis_auto_alteracao.php?db_opcao=<?=$db_opcao?>&funcao_js=parent.js_preenchepesquisa|dl_auto&fisauto=1';
...
function js_pesquisare(num,origem){
   js_OpenJanelaIframe('','db_iframe_auto','func_fis_auto001.php?db_opcao=<?=$db_opcao?>&funcao_js=parent.js_preenchepesquisa|dl_auto&origem='+origem+'&num='+num,'Pesquisa',true);
}
...
function js_preenchepesquisa(chave){
  ...
  <?php
    if($db_opcao == 2 || $db_opcao == 22){
      echo " location.href = 'fis1_fis_auto002.php?abas=1&chavepesquisa='+chave;";
    }elseif($db_opcao == 33 || $db_opcao == 3){
      echo " location.href = 'fis1_fis_auto003.php?abas=1&chavepesquisa='+chave;";
    }
  ?>
}
