...
try {
    ...
    if (count($aEnsinosNoAno) > 0) {
        ...
        calculaAlunosMatriculadosEscola($aEscolasPrimeiroQuadro, $aEtapasErEp, $oDtInicio, $oDtFim, $oNomeCal);
    ...
    if (count($aEnsinosCorrecaoFluxo) > 0) {
        ...
        calculaAlunosMatriculadosEscola($aEscolasSegundoQuadro, $aEtapasSegundoQuadro, $oDtInicio, $oDtFim, $oNomeCal);
    }
    ...
    foreach ($aEscolasSegundoQuadro as $oEscola) {
        $oEscola->iTotalMatriculasProfissional = buscaAlunosTipoEnsinoDataCorte($oEscola->codigo, 4, $oDtInicio, $oDtFim, $oNomeCal);
     ...
     */
    foreach ($aEscolasSegundoQuadro as $oEscola) {
        $oEscola->iTotalMatriculasEspecial = buscaAlunosTipoEnsinoDataCorte($oEscola->codigo, 2, $oDtInicio, $oDtFim, $oNomeCal);
    ...
    foreach ($aEscolasSegundoQuadro as $oEscola) {
        ...
        $oEscola->iTotalMatriculasAee = buscaAlunosMatriculadosAEE($oEscola->codigo, $oDtInicio, $oDtFim, $oNomeCal);
    ...
    foreach ($aEscolasSegundoQuadro as $oEscola) {
        ...
        $oEscola->iTotalMatriculasComplementar = buscaAlunosMatriculadosAtivComplementar($oEscola->codigo, $oDtInicio, $oDtFim, $oNomeCal);
     ...
     */
    foreach ($aEscolasSegundoQuadro as $oEscola) {
        $oEscola->iTotalMatriculasMaisEducacao = buscaAlunosDataCorteMaisEducacao($oEscola->codigo, $oDtInicio, $oDtFim, $oNomeCal);
    }
    ...
    foreach ($aEscolasTerceiroQuadro as $oEscola) {
        ...
        $aAlunoMatriculadosSexo = quantificarAlunosDataCortePorSexo($oEscola->codigo, $oDtInicio, $oDtFim, $oNomeCal);
        ...
        $oEscola->iTotalTransfFora = quantificarAlunosPorSituacao($oEscola->codigo, $oDtInicio, $oDtFim, "= 'TRANSFERIDO FORA'",$oNomeCal);
        $oEscola->iTotalTransfRede = quantificarAlunosPorSituacao($oEscola->codigo, $oDtInicio, $oDtFim, "= 'TRANSFERIDO REDE'",$oNomeCal);
        $oEscola->iTotalEvadido = quantificarAlunosPorSituacao($oEscola->codigo, $oDtInicio, $oDtFim, "='EVADIDO'",$oNomeCal);

        $sSituacao = " not in ('TRANSFERIDO FORA', 'TRANSFERIDO REDE', 'EVADIDO', 'MATRICULADO') ";
        $oEscola->iTotalOutrasSituacoes = quantificarAlunosPorSituacao($oEscola->codigo, $oDtInicio, $oDtFim, $sSituacao,$oNomeCal);
    ...
    foreach ($aEscolasQuartoQuadro as $oEscola) {
        ...
        $aTurmas = quantificarTurmasTurno($oEscola->codigo, $oDtInicio, $oDtFim,$oNomeCal);
...
$oConfig->iMargem = 3;
$oConfig->iXMaxima = 285;
$oConfig->iYMaximo = 195;
$oConfig->iHLinha = 4;
$oConfig->iXNumero = 10;
$oConfig->iXEscola = 50;
$oConfig->iXSubTotal = 10;
$oConfig->iXMaximoEnsino = $oConfig->iXMaxima - $oConfig->iXEscola - $oConfig->iXSubTotal - $oConfig->iXNumero;
...
$head1 = "Mapa Estatístico por Calendário";
...
if (count($aEtapasErEp) > 0) {
    ...
    foreach ($aEscolasPrimeiroQuadro as $oEscola) {
        ...
        $oPdf->SetX($oConfig->iMargem);
        $oPdf->Cell($oConfig->iXNumero, 5, $oEscola->escola_referencia, 1, 0,"C");
    ...
    $oPdf->Cell($oConfig->iXEscola+$oConfig->iXNumero, 5, "Total", 1, 0, 'L', 1);
...
foreach ($aEtapasErEp as $oEnsino) {
    ...
    $oPdf->Cell(85, 5, $oEnsino->iTotalEnsino, 1, 1, 'R', 0);
}
...
$oPdf->Cell(85, 5, $iTotalMatriculasProfissional, 1, 1, 'R');
$oPdf->SetX($oConfig->iMargem);
$oPdf->Cell(200, 5, "EDUCAÇÃO ESPECIAL", 1, 0, 'L');
$oPdf->Cell(85, 5, $iTotalMatriculasEspecial, 1, 1, 'R');
$oPdf->SetX($oConfig->iMargem);
$oPdf->Cell(200, 5, "ATENDIMENTO EDUCACIONAL ESPECIAL - AEE", 1, 0, 'L');
$oPdf->Cell(85, 5, $iTotalMatriculasAee, 1, 1, 'R');
$oPdf->SetX($oConfig->iMargem);
$oPdf->Cell(200, 5, "ATIVIDADE COMPLEMENTAR", 1, 0, 'L');
$oPdf->Cell(85, 5, $iTotalMatriculasComplementar, 1, 1, 'R');

$oPdf->SetX($oConfig->iMargem);
$oPdf->Cell(200, 5, "MAIS EDUCAÇÃO", 1, 0, 'L');
$oPdf->Cell(85, 5, $iTotalGeralMaisEducacao, 1, 1, 'R');
...
foreach ($aEscolasTerceiroQuadro as $oDadosEscola) {
    ...
    $oPdf->SetX($oConfig->iMargem);
    $oPdf->Cell($oConfig->iXNumero, 5, $oDadosEscola->escola_referencia, 1, 0,"C");
...
function imprimePrimeiroCabecalho(FpdfMultiCellBorder $oPdf, $oConfig, $aEtapasErEp)
{
    ...
    $iYInicial = $oPdf->GetY();
    $oPdf->Rect($oConfig->iMargem, $iYInicial, $oConfig->iXNumero, $oConfig->iAlturaCabecalho);
    $oPdf->Rect($oConfig->iMargem, $oPdf->GetY(), $oConfig->iXMaxima, $oConfig->iAlturaCabecalho);
    $oPdf->ln();

    $oPdf->SetX($oConfig->iMargem);
    $oPdf->SetFont('arial', 'B', 9);
    $oPdf->MultiCell($oConfig->iXNumero, 5, "REF.", 0, 'C');
    $oPdf->MultiCell($oConfig->iXEscola, 5, "Unidade\nEscolar", 0, 'C');

    $oPdf->SetY($iYInicial);

    $oPdf->SetX($oConfig->iMargem + $oConfig->iXEscola+$oConfig->iXNumero);
 ...
 * @throws Exception
 */
function buscaAlunosDataCorte($iEscola, $iEtapa, DBDate $oDtInicio, DBDate $oDtFim, $lCorrecaoFluxo, $oNomeCal)
{
    ...
    $sWhere .= " and trim(ed52_c_descr) IN ('$oNomeCal') ";
    if ($lCorrecaoFluxo) {
 ...
 * @throws Exception
 */
function buscaAlunosMatriculados($iEscola, $iEtapa, DBDate $oDtInicio, DBDate $oDtFim, $lCorrecaoFluxo, $oNomeCal)
{
...
function calculaAlunosMatriculadosEscola($aEscolas, $aEnsinos, $oDtInicio, $oDtFim, $oNomeCal)
{
    /**
    ...
    foreach ($aEscolas as $oEscola) {
        ...
        foreach ($aEnsinos as $oEnsino) {
            ...
            foreach ($oEnsino->aEtapas as $oEtapa) {
                ...
                $iTotalAlunosEtapa = buscaAlunosDataCorte($oEscola->codigo, $oEtapa->iEtapa, $oDtInicio, $oDtFim, $lCorrecaoFluxo, $oNomeCal);
...
function imprimeSegundoCabecalho($oPdf, $oConfig, $aEtapasSegundoQuadro)
{
    ...
    $oPdf->Rect($oConfig->iMargem, $oPdf->GetY(), $oConfig->iXMaxima, $iAlturaCabecalho);
    $oPdf->Rect($oConfig->iMargem, $iYInicial, $oConfig->iXNumero, $iAlturaCabecalho);
    $oPdf->Rect(13, $oPdf->GetY(), $oConfig->iXEscola, $iAlturaCabecalho);
    $oPdf->ln();
    ...
    $oPdf->SetX($oConfig->iMargem+ $oConfig->iXEscola);
    $oPdf->SetFont('arial', '', 6);
    //imprime os níveis de ensino
    foreach ($aEtapasSegundoQuadro as $oEnsino) {
 ...
 */
function buscaAlunosMatriculadosAEE($iEscola, $oDtInicio, $oDtFim, $oNomeCal)
{
 ...
 * @throws Exception
 */
function buscaAlunosTipoEnsinoDataCorte($iEscola, $sTipoEnsino, $oDtInicio, $oDtFim, $oNomeCal)
{
    $sSql = "SELECT count(*)
             FROM matricula
       INNER JOIN turma ON ed57_i_codigo = matricula.ed60_i_turma
       INNER JOIN escola ON ed18_i_codigo = ed57_i_escola
       INNER JOIN calendario ON ed52_i_codigo = ed57_i_calendario
       INNER JOIN matriculaserie ON ed221_i_matricula = matricula.ed60_i_codigo
       INNER JOIN serie ON ed11_i_codigo = matriculaserie.ed221_i_serie
       INNER JOIN turno ON ed15_i_codigo = turma.ed57_i_turno
       INNER JOIN ensino ON ed10_i_codigo = serie.ed11_i_ensino
       INNER JOIN aluno ON ed47_i_codigo = matricula.ed60_i_aluno
            WHERE matriculaserie.ed221_c_origem = 'S' ";
    $sWhere = " and ed60_d_datamatricula <= '" . $oDtFim->getDate() . "' ";
    $sWhere .= " and extract(year FROM ed60_d_datamatricula) = " . $oDtFim->getAno();
    $sWhere .= " and ( ed60_d_datasaida is null or ";
    $sWhere .= "       ed60_d_datasaida not between '" . $oDtInicio->getDate() . "' and '" . $oDtFim->getDate() . "') ";
 ...
 * @throws Exception
 */
function buscaAlunosMatriculadosTipoEnsino($iEscola, $sTipoEnsino, $oDtInicio, $oDtFim, $oNomeCal)
{
    $sSql = "SELECT count(*)
             FROM matricula
       INNER JOIN turma ON ed57_i_codigo = matricula.ed60_i_turma
 ...
 * @return integer            Número de alunos matriculados
 */
function buscaAlunosMatriculadosAtivComplementar($iEscola, $oDtInicio, $oDtFim, $oNomeCal)
{
 ...
 * @return integer            Número de alunos matriculados
 */
function buscaAlunosDataCorteMaisEducacao($iEscola, $oDtInicio, $oDtFim, $oNomeCal)
{
    $sSql = "SELECT count(*)
             FROM matricula
       INNER JOIN turma ON ed57_i_codigo = matricula.ed60_i_turma
    ...
    $sWhereAc .= " and trim(ed52_c_descr) IN ('$oNomeCal') ";
...
function buscaAlunosMatriculadosMaisEducacao($iEscola, $oDtInicio, $oDtFim, $oNomeCal)
{
    $sSql = "SELECT count(*)
             FROM matricula
       INNER JOIN turma ON ed57_i_codigo = matricula.ed60_i_turma
...
function buscaEscolas()
{
    ...
    $sCampos = " ed18_i_codigo as codigo, trim(ed18_c_nome) as escola, trim(ed18_c_abrev) as escola_abrev, ed18_codigoreferencia as escola_referencia ";
    $sWhere = "ed18_i_funcionamento = 1 and ed18_i_tipoescola = 1";

    $oDaoEscola = new cl_escola();
    $rsEscolas = db_query($oDaoEscola->sql_query_file(null, $sCampos, 'escola_referencia', $sWhere));
 ...
 * @return integer            Número de alunos matriculados
 */
function quantificarAlunosDataCortePorSexo($iEscola, $oDtInicio, $oDtFim, $oNomeCal)
{
    ...
    $sCampos .= "     then 4 ";
    $sCampos .= "   else (select distinct ed336_turnoreferente from turmaturnoreferente where ed336_turma = ed57_i_codigo) ";
...
function quantificarAlunosMatriculadoPorSexo($iEscola, $oDtInicio, $oDtFim, $oNomeCal)
{
 ...
 */
function quantificarAlunosPorSituacao($iEscola, $oDtInicio, $oDtFim, $sSituacao,$oNomeCal)
{
...
function quantificarTurmasTurno($iEscola, $oDtInicio, $oDtFim,$oNomeCal)
{
    ...
    $sSql .= " and trim(ed52_c_descr) IN ('$oNomeCal') ";
...
function imprimeTerceiroCabecalho($oPdf, $oConfig)
{
    ...
    $iAlturaLinha = 5;
    $iXmargim = 3 + $oConfig->iXEscola + $oConfig->iXNumero;
    $iYInicial = $oPdf->GetY();
    $iAlturaCabecalho = $oConfig->iAlturaCabecalho + $iAlturaLinha;
