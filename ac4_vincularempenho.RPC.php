...
try {
  ...
  switch ($oParam->exec) {
    ...
    case "getEmpenhos":
      ...
      $sSql              = $oDaoAcordoEmpenho->sql_query(null, 
                                                         $sCampos, 
                                                         "e60_numemp", 
                                                         "ac54_acordo = {$iCodigoAcordo}");
    ...
    case "salvar":
      ...
      $oEmpenhoFinanceiro  = EmpenhoFinanceiroRepository::getEmpenhoFinanceiroPorCodigoAno($iNumemp, 
                                                                                           $iAnoEmpenho, 
                                                                                           $oInstituicao);
      ...
      /**
       * vamos verificar se o empenho escolhido ja possui vinculo
       * se possuir emitimos um alerta informando o acordo que o empenho
       * selecionado esta vinculado.
       * Regra:
       * 1 Empenho p/  1 Contrato
       * 1 Contrato p/ N empenhos
       */
      $oDaoVerificaVinculo = new cl_acordoempempenho;
      $sSqlVerificaVinculo = $oDaoVerificaVinculo->sql_query (null,
                                                              "*", 
                                                              null, 
                                                              "ac54_empempenho = {$oEmpenhoFinanceiro->getNumero()}");
      $rsVinculos = $oDaoVerificaVinculo->sql_record($sSqlVerificaVinculo);

      if ($oDaoVerificaVinculo->numrows > 0 ) {
        
        $oDadosVinculo = db_utils::fieldsMemory($rsVinculos, 0);
        $oAcordo = new Acordo($oDadosVinculo->ac54_acordo);
        
        $sAcordo = "{$oAcordo->getNumero()}/{$oAcordo->getAno()}";
        throw new Exception("Este empenho já encontra-se vinculado ao contrato: {$oAcordo->getCodigo()}");
      }
      
      $oAcordo = new Acordo($iCodigoAcordo);
      if($oEmpenhoFinanceiro->getCgm()->getCodigo() !== $oAcordo->getContratado()->getCodigo()) {
      
        throw new Exception("Credor do empenho diferente do contratado para o acordo {$iCodigoAcordo}");        
      }
      $oDaoAcordoEmpenho                       = new cl_acordoempempenho;
      ...
      $sSql = $oDaoEmpenhoAcordo->sql_query_file(null, '*', null, ' e100_numemp = ' . $oEmpenhoFinanceiro->getNumero() . 
                                                                  ' AND e100_acordo = ' . $iCodigoAcordo);

      db_utils::makeFromRecord(db_query($sSql), function($linha) use ($oDaoEmpenhoAcordo, 
                                                                      $oEmpenhoFinanceiro, 
                                                                      $iCodigoAcordo) {
        if (empty($linha)) {
    ...
    case 'excluirVinculo':
      ...
      $oDaoEmpenhoAcordo = new cl_empempenhocontrato();
      $oDaoEmpenhoAcordo->excluir(null, "    e100_numemp = {$oParam->iNumeroEmpenho} 
                                         and e100_acordo = {$oParam->iCodigoAcordo}");
