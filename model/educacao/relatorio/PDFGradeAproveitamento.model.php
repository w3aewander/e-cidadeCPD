...
abstract class PDFGradeAproveitamento {
  ...
  protected $iTamanhoDisciplina = 55;
  protected $iTamanhoFrequencia = 30;
  ...
  // Se houver, esse ? o tamanho de um elemento com recuperacao
  protected $iTamanhoElementosRecuperacao = 10; // se tiver apenas um assume este tamanho

  // Colunas que comp?e o elemento de avaliacao
  ...
  protected function calculaTamanhoColunas($aElementos) {
    ...
    if ( $iNumeroElementosRecuperacao > 0 ) {
      ...
      if ($iNumeroElementosRecuperacao > 1) {
        $this->iTamanhoElementosRecuperacao = 8; // Tamanho Mínimo para coluna de Recuperação
      }
    ...
    // Se a coluna de avaliação ficou menor que 8mm ? trocado o tamanho com a coluna de falta
    if ($this->iColunaAvaliacao < 8 ) {

      $this->iColunaAvaliacao = 8; // Tamanho M?nimo para coluna de Avaliação
    ...
    foreach ($aElementos as $oElemento) {
       ...
       */
        if ( $oElemento instanceof AvaliacaoPeriodica )
		{
			...
			//{
            if( $DescricaoAbreviada == '1º TRIM' or $DescricaoAbreviada == '2º TRIM' or $DescricaoAbreviada == '3º TRIM' or
			   $DescricaoAbreviada == '1º BIM'  or $DescricaoAbreviada == '2º BIM'  or $DescricaoAbreviada == '3º BIM' or $DescricaoAbreviada == '4º BIM')
			{
				...
				if ( $oElemento instanceof AvaliacaoPeriodica ) {
					...
					if ( !is_null($oElementoDependente) ) {
						...
						}elseif( $oElemento->getDescricaoAbreviada() == 'AVA FINAL'){
							$this->oPdf->Cell($this->iTamanhoElementosRecuperacao, 4, 'AV FIN', 1, 0, "C");
						}
					}
				}
				...
				if( $DescricaoAbreviada <> '2º BIM' and $DescricaoAbreviada <> '4º BIM' )
				{
					...
					if($DescricaoAbreviada == '2º BIM' and $imp2 == true)
					{
						$this->oPdf->Cell($this->iColunaAvaliacao, 4, $DescricaoAbreviada, 1, 0, "C");
						$this->oPdf->SetFont("Arial", "B", 7);
						$this->oPdf->Cell($this->iColunaFalta,     4, "FT",    1, 0, "C");
					...
					if($DescricaoAbreviada == '4º BIM' and $imp4 == true)
					{
						$this->oPdf->Cell($this->iColunaAvaliacao, 4, $DescricaoAbreviada, 1, 0, "C");
						$this->oPdf->SetFont("Arial", "B", 7);
						$this->oPdf->Cell($this->iColunaFalta,     4, "FT",    1, 0, "C");
