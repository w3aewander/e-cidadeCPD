  ...
  <form name="form1" method="POST" action="" onSubmit="return validarFormulario();">
    ...
    <fieldset>
      ...
      <table class="form-container">
        ...
        <tr>
          <td>
            <b><a href="#" onclick="js_pesquisa(true)"> Buscar Itens da NRM</a></b>
          </td>
          
          <td>
            <input type="text" name="sequencial" id="sequencial" readonly style="background-color:#DEB887">
            <input type="submit" name="incluir" id="enviaitens" value="Importar Itens da NRM" disabled>
...
$quantot = $oValorMatEstoque->quantidade_total;
          ...
          <td>
            <?php
              ...
              if (isset($quantot)&&($quantot!="")){
                $quant_disp=$quantot;
                //$quant_disp=$quantidade_total;
              }
              db_input('quant_disp',10,$Im41_quant,true,'text',3,"");
              //db_input('quant_disp',10,$quantidade_total,true,'text',3,"");
            ?>
          </td>
        </tr>
...
function validarFormulario() {
  ...
  var verifica = document.getElementById("idtabela").value;
  if(verifica != ""){
    return true;
  }
...
function js_pesquisa(mostra){
    ...
    if(mostra==true){
      ...
      js_OpenJanelaIframe('','db_iframe_ieoc','func_eoc.php?funcao_js=parent.js_mostranrm|id|anousuario|sequencial','Pesquisa',true);
    }
    

  }
  function js_mostranrm(chave1,chave2, chave3){    
    //js_reset();
    db_iframe_ieoc.hide();
    //js_getDadosNota(chave2,chave1);
    var idtabela  = chave1;
    document.getElementById("sequencial").value = chave3 +"/"+chave2;
    document.getElementById("idtabela").value = idtabela;

    document.getElementById("enviaitens").disabled = false;    
