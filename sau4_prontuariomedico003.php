...
$sCamposProntuarios  = "
  CASE
    WHEN sd01_descricao is null THEN 'Desistiu do atendimento'
    ELSE
      sd01_descricao
  END alta,
";
...
$sCamposProntuarios .= ' s152_i_login,                                                                     ';
$sCamposProntuarios .= ' m.z01_nome as profissional                                                       ';

$sWhereProntuarios   = " {$sWhereProntProced}";

$sSqlProntuarios = $oDaoProntproced->sql_query_prontuario('', $sCamposProntuarios, 'sd29_d_data DESC , sd29_c_hora DESC, sd29_i_codigo', $sWhereProntuarios);
$rsProntuarios   = db_query($sSqlProntuarios);

$linhas1   = pg_num_rows($rsProntuarios);
...
$p = 0;
$x = 0;
$prontuarios = db_utils::getCollectionByRecord($rsProntuarios);
$atendimentos = db_utils::getCollectionByRecord($query);

while ($p < $linhas1 || $x < $linhas) {
  //Percorre todos os registroa na 'prontproced' para o CGS escolhido
  for ($p=$p; $p < $linhas1; $p++) {
    if ($linhas > 0 && $x < $linhas) {
      $comparaData = !comparaData(
        $prontuarios[$p]->sd29_d_data, 
        $atendimentos[$x]->sd32_d_atendimento, 
        $prontuarios[$p]->sd24_c_cadastro, 
        $atendimentos[$x]->sd32_c_horaatend
      );
    ...
    // PLUGIN ESF: PROFISSIONAL TRIAGEM
    $xlin = $pdf->Gety();
    ...
    if ($retorna_obs == 0) {
      ...
      $query3       = db_query($sSql);
      $iQuanttCid   = pg_num_rows($query3);
      ...
      // Plugin ESF: substitui esse método INÍCIO
      ...
      for($iCont = 0; $iCont < pg_num_rows($rs); $iCont++){
        // PLUGIN ESF - SUBSTITUI ESSE BLOCO DE CÓDIGO - INÍCIO (NÃO MEXER)
        ...
        // PLUGIN ESF - SUBSTITUI ESSE BLOCO DE CÓDIGO - FIM

        $sConteudo .= "\n  Evolução......: {$s152_evolucao}";
        $sConteudo .= "\n  Prescrição....: {$sConteudoPrescricao}\n";
      }

      // PLUGIN ESF: CONCATENA CONTEÚDO
      ...
      if (pg_num_rows($rs) > 0) {
        ...
        for( $iContador = 0; $iContador < pg_num_rows($rs); $iContador++ ) {
...
function cabecalho($pdf, $result, $pri) {
  ...
  global $z01_i_numcgs,$z01_c_raca,$z01_v_nome, $z01_v_sexo, $z01_d_nasc, $z01_v_pai, $z01_v_mae,
         $z01_v_ender, $z01_i_numero, $z01_v_compl,$sd114_unidadesorigem,$sd113_descricao,$z01_v_municnasc;
  global $z01_v_telef, $z01_v_bairro, $ec, $z01_v_ident, $s115_c_cartaosus, $ec_cgs, $z01_i_estciv, $sexo_cgs, $z01_v_sexo;
  ...
  if (  ($pdf->gety() > $pdf->h -30) || $pri) {
    ...
    $pdf->roundedrect( 10, 43, 192, 32, 2, 'DF', '1234' );

    $pdf->cell( 192, 8, "PRONTUÁRIO PACIENTE", 0, 1, "C", 0 );
    ...
    $pdf->cell( 66, 4, @$sexo_cgs[$z01_v_sexo], 0, 0, "L", 0 );

    $pdf->cell( 30, 4, "Endereço : ",                                             0, 0, "R", 0 );
    $pdf->cell( 66, 4, $z01_v_ender . ", " . $z01_i_numero . ", " . $z01_v_compl, 0, 1, "L", 0 );

    $pdf->cell( 30, 4, "Data de Nasc : ",               0, 0, "R", 0 );
    $pdf->cell( 66, 4, db_formatar( $z01_d_nasc, 'd' ), 0, 0, "T", 0 );

    $pdf->cell( 30, 4, "Bairro : ",   0, 0, "R", 0 );
    $pdf->cell( 66, 4, $z01_v_bairro, 0, 1, "L", 0 );    

    $pdf->cell( 30, 4, "Raça/Cor : ",               0, 0, "R", 0 );
    $pdf->cell( 66, 4, $z01_c_raca, 0, 0, "T", 0 );

    $pdf->cell( 30, 4, "Unidade de Origem : ",    0, 0, "R", 0 );
    $pdf->cell( 66, 4, $sd113_descricao, 0, 1, "L", 0 );    

    $pdf->cell( 30, 4, "Munic. Nasc : ",    0, 0, "R", 0 );
    $pdf->cell( 66, 4, $z01_v_municnasc, 0, 0, "L", 0 );

    $pdf->cell( 30, 4, "Nome da Mãe : ", 0, 0, "R", 0 );
    $pdf->cell( 66, 4, $z01_v_mae,       0, 1, "L", 0 );

    $pdf->cell( 30, 4, "RG : ", 0, 0, "R", 0 );
    $pdf->cell( 66, 4, $z01_v_ident, 0, 0, "L", 0 );    

    $pdf->cell( 30, 4, "Nome do Pai : ", 0, 0, "R", 0 );
    $pdf->cell( 66, 4, $z01_v_pai,       0, 1, "L", 0 );

    $pdf->cell( 30, 4, "Cartão SUS : ",  0, 0, "R", 0 );
    $pdf->cell( 66, 4, $s115_c_cartaosus, 0, 0, "L", 0 );
    ...
    $pdf->cell(  30, 5, "IMUNIZAÇÃO: ", 0, 0, "L", 0 );
    $pdf->cell( 162, 5, "C.N.S.: ",     0, 1, "L", 0 );
    ...
    $xcol = 10;
    $xlin = 21;
    $pdf->rect( $xcol,     $xlin +56,  20, 210, 2, 'DF', '12' );
    $pdf->rect( $xcol +20, $xlin +56,  45, 210, 2, 'DF', '12' );
    $pdf->rect( $xcol +65, $xlin +56, 127, 210, 2, 'DF', '12' );
...
{
  $data1 = new DateTime ("{$data1} {$hora1}");
  $data2 = new DateTime ("{$data2} {$hora2}");
