...
$aDadosRetorno      = array();
  ...
  switch ($oParam->exec) {
    ...
    /**
     * case para geração do empenho passivo
     */
    ...
    case 'gerarEmpenhoPassivo':
      try {
        ...
        /**
         * Incluimos a autorização de empenho pois não podemos gerar um empenho sem antes autorizá-lo
         */
        ...
        /**
         * Salvamos a autorização e os itens
         */
        ...
        if ($oDaoVinculoAutorizaInscricao->erro_status == 0) {
          ...
          $sMsgErro  = "Impossível criar o vínculo entre a autorização de empenho e a inscrição.\n\n";
          $sMsgErro .= "Erro Técnico: {$oDaoVinculoAutorizaInscricao->erro_msg}";
        ...
        $oEmpenho = new EmpenhoFinanceiro();
          ...
          $oEmpenhoItem->setValorUnitario($oItemAutorizacao->valorunitario);
  		    ...
  		    /*
  		     * Verifico se é necessário liquidar. Caso seja, alimento um array contendo os itens que devemos
  		     * liquidar. Fiz a alimentação do array aqui para poupar um foreach mais a baixo após salvar o empenho
  		     */
  		    if ($lLiquidar) {
        ...
        //Credor, Despesa, Recurso, Dotação, Despesa
        ...
        $oContaCorrenteDetalhe->setRecurso($oEmpenho->getDotacao()->getDadosRecurso());
  		  ...
  		  /**
  		   * Inicio dos Lançamentos contábeis
  		   */
        ...
        $oLancamentoAuxiliarEmpenho->setContaCorrenteDetalhe($oContaCorrenteDetalhe);
  			...
  			/**
  			 * Executamos os lançamentos contábeis para o EMPENHO
  			 */
  			...
  			/**
  			 * Verifico se o usuário selecionou para liquidar o empenho.
  			 */
  			$sMsgLiquidar = "";
  			if ($lLiquidar) {
