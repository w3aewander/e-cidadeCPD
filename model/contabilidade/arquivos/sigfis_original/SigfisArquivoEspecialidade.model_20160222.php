...
//        $UnidadeOrcamentaria  = str_pad($oDadosQuery->o58_orgao, 2, '0', STR_PAD_LEFT);
        ...
        if ($oDadosQuery->ano <= 2013 and $UnidadeOrcamentaria == 1) $UnidadeOrcamentaria = 0; 

	$orgao = $oDadosQuery->o58_orgao;
	$ano_rp = '';
	if ($oDadosQuery->ano = 2014) 
        { 
                switch ( $orgao)
                 {
                    case 70;
                          $orgao = 12;
                          $UnidadeOrcamentaria = 70; 
			  $ano_rp = '4';
                 }
         }
...
//$this->addLog($oDadosQuery->ano);
	if ($oDadosQuery->ano < 2015) {
	    $oDados->Nu_Empenho  = str_pad($UnidadeOrcamentaria.$oDadosQuery->e60_codemp.$ano_rp, 10, ' ', STR_PAD_RIGHT);
        }
        ELSE {
	    $oDados->Nu_Empenho  = str_pad($DadosQuery->e60_codemp, 10, ' ', STR_PAD_RIGHT);
	}
//$oDados->Nu_Empenho              = str_pad("20".$oDadosQuery->e60_codemp, 10, ' ', STR_PAD_RIGHT);

        $oDados->cd_UnidadeOrcamentaria  = str_pad($UnidadeOrcamentaria,      4, ' ', STR_PAD_LEFT);
        $oDados->cd_Orgao                = str_pad($orgao,   4, ' ', STR_PAD_LEFT);
