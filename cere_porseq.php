...
function busca($sequencial){
  $sql = pg_query("SELECT id, e60_numemp, noempenho, nocertificado, e50_codord, CASE WHEN cseq2 = 0 THEN 'Não' ELSE 'Sim' END as retiradooc, e60_instit, CASE WHEN excluido = 1 THEN 'Sim' ELSE 'Não' END as excluido FROM certificacaoconformidade WHERE e60_numemp = {$sequencial} AND e50_codord is not null AND cseq2 = 0 ORDER BY e60_instit, id");
...
<p>
  ...
  <input type="radio" id="g11" name="g11" value="1">Sim
  <input type="radio" id="g11" name="g11" value="2" checked>Não  
   Folha: <input type="text" name="g11f" id="g11f" size="30" maxlength="30"><br>
</p>
...
<p>
  ...
  <input type="radio" id="g22" name="g22" value="1">Sim
  <input type="radio" id="g22" name="g22" value="2" checked>Não  
   Folha: <input type="text" name="g22f" id="g22f" size="30" maxlength="30"><br>
</p>

<p>
  <label>Publicação da quebra de ordem cronológica</label><br>
  <input type="radio" id="g33" name="g33" value="1">Sim
  <input type="radio" id="g33" name="g33" value="2" checked>Não  
   Folha: <input type="text" name="g33f" id="g33f" size="30" maxlength="30"><br>
</p>
</div>

<input type="button" value="Gerar Certificado e Retirar da Ordem Cronológica" id="botaozao" onclick="imprime()">
...
<?php else : ?>
  <h4>Não há OPs para o sequencial do empenho buscado</h4>
<?php endif; ?>