...
if ($sqlerro==false) {
  ...
  if($linhasdestino>0) {
    ...
    // Apaga contranslrvinculo
    $sqldelete  = "delete ";
    $sqldelete .= "  from contranslrvinculo  ";
    $sqldelete .= " using contranslr, ";
    $sqldelete .= "       contranslan, ";
    $sqldelete .= "       contrans, ";
    ...
    if ($resultdelete==true) {
      ...
      // Apaga contranslrelemento
      $sqldelete  = "delete ";
      $sqldelete .= "  from contranslrelemento  ";
      $sqldelete .= " using contranslr, ";
      $sqldelete .= "       contranslan, ";
      $sqldelete .= "       contrans, ";
      $sqldelete .= "       conhistdoc ";
      $sqldelete .= " where c47_seqtranslr = c114_contranslr";
      $sqldelete .= "   and c47_seqtranslan = c46_seqtranslan ";
      $sqldelete .= "   and c46_seqtrans    = c45_seqtrans ";
      $sqldelete .= "   and c45_coddoc      = c53_coddoc ";
      $sqldelete .= "   and c45_anousu      = $anodestino ";
      $sqldelete .= "   and c53_tipo        in ($_tipodoc)";
  ...
  if($sqlerro==false) {
    ...
    if (($linhasorigem > 0) && ($linhasdestino == 0 )) {
        ...
        for ($c=0; $c<$linhascontranslan; $c++) {
          ...
          if ($linhascontranslr > 0) {
            for ($h=0; $h < $linhascontranslr; $h++) {
              ...
              if ($c45_coddoc!=31 && $c45_coddoc!=32 && $c45_coddoc!=33 && $c45_coddoc!=34 && $c45_coddoc!=35 && $c45_coddoc!=36 && $c45_coddoc!=37) {
      ...
      if ($cldb_viradaitemlog->erro_status==0) {
        $sqlerro   = true;
        $erro_msg .= $cldb_viradaitemlog->erro_msg.pg_last_error();
        return;
      }
    }
  }
  
  // Solicitado pelo Leandro a virada das regras
  $sSqlDocumentoContabilRegra       = "select * 
  ...
  for ($iDocumento=0; $iDocumento<$iRegistrosDocumentoContabilRegra; $iDocumento++) {
    ...
    try {
      ...
      $oDadosDocumento         = db_utils::fieldsMemory($rsDocumentoContabilRegra, $iDocumento);
      $oDocumentoContabilRegra = new DocumentoContabilRegra();
