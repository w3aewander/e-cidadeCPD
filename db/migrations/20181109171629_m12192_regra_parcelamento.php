...
class M12192RegraParcelamento extends PostgresMigration
{
    public function up()
    {
        $this->dicionarioDadosUp();
        $this->alterTableValorMaxUp();
        $this->alterTableTipoPessoaUp();
        $this->reciboUp();
        $this->calculaTotalArrecadUp();
        $this->verificaDescontoFaixaValorUp();
    }

    public function down()
    {
        $this->dicionarioDadosDown();
        $this->alterTableValorMaxDown();
        $this->alterTableTipoPessoaDown();
        $this->reciboDown();
        $this->calculaTotalArrecadDown();
        $this->verificaDescontoFaixaValorDown();
    }

    private function dicionarioDadosUp()
    {        
        $sql = "insert into db_syscampo values(1010077,'vlrmax','float8','Valor máximo da parcela','0', 'Valor Máximo',15,'f','f','f',0,'text','Valor Máximo');";
        $sql .= "insert into db_sysarqcamp values(95,1010077,15,0);";        

        $sql .= "insert into db_syscampo values(1010087,'tipopessoa','bool','Fisica ou Jurídica','f', 'Tipo de Pessoa',1,'t','f','f',5,'text','Tipo de Pessoa');";
        $sql .= "insert into db_sysarqcamp values(95,1010087,16,0);";
        $sql .= "update db_syscampo set nomecam = 'tipopessoa', conteudo = 'int4', descricao = 'Física ou Jurídica', valorinicial = '0', rotulo = 'Tipo de Pessoa', nulo = 't', tamanho = 1, maiusculo = 'f', autocompl = 'f', aceitatipo = 1, tipoobj = 'text', rotulorel = 'Tipo de Pessoa' where codcam = 1010087;";
        $sql .= 'delete from db_syscampodep where codcam = 1010087;';
        $sql .= 'delete from db_syscampodef where codcam = 1010087;';
    ...
    {
        $sql = "delete from db_sysarqcamp where codcam = 1010077;";
        $sql .= "delete from db_syscampo where codcam = 1010077;";

        $sql .= "delete from db_sysarqcamp where codcam = 1010087;";
        $sql .= "delete from db_syscampo where codcam = 1010087;";

        $this->execute($sql);
    }

    private function alterTableValorMaxUp()
    {        
        $this->execute("alter table tipoparc add column vlrmax double precision default 0;");
    }

    private function alterTableValorMaxDown()
    {
        $this->execute("alter table tipoparc drop column vlrmax;");
    }

    private function alterTableTipoPessoaUp()
    {
        $this->execute("alter table tipoparc add column tipopessoa smallint default 0");
    }

    private function alterTableTipoPessoaDown()
    {
        $this->execute("alter table tipoparc drop column tipopessoa");
    }
        ...
        LOOP
          ...
          IF ( VALOR_RECEITA <> 0 OR RECORD_NUMPRE.K99_TIPO = 6 ) THEN
            ...
            if TEM_DESCONTO > 0 then
              ...
              select sum(fc_calcula_total_arrecad(db_reciboweb.k99_numpre, DTVENC))
                into numpreValorTotal
                from db_reciboweb 
               where db_reciboweb.k99_numpre_n = NUMPRE;
...
SQL;
    ...
    private function calculaTotalArrecadUp()
    {
        $sql = <<<SQL
create or replace function fc_calcula_total_arrecad(numpre integer, data date) returns numeric
as $$ 
declare
    ...
    from (select (substr(fc_calcula, 15, 13)::float8 +
            ...
            from (select fc_calcula(x.k00_numpre,
                    ...
                    from (select distinct 
                                 ...
                                 arrecad.k00_numpar
                            from arrecad
                           where arrecad.k00_numpre = numpre) as x
                 ) as y
         ) as z;
...
SQL;
    ...
    private function calculaTotalArrecadDown()
    {
        $sql = <<<SQL
drop function fc_calcula_total_arrecad;
SQL;

        $this->execute($sql);
    }

    private function verificaDescontoFaixaValorUp()
    {
        $sql = <<<SQL
        
create or replace function fc_verifica_desconto_faixa_valor(valor numeric, valor_minimo numeric, valor_maximo numeric) returns boolean
as $$ 
begin

  perform *
     from db_plugin
    where db145_nome = 'desconto-debito-faixa-valor'
      and db145_situacao is true;
  if found then

    if valor_maximo = 0 or (valor >= valor_minimo and valor <= valor_maximo) then
...
SQL;
    ...
    private function verificaDescontoFaixaValorDown()
    {
        $sql = <<<SQL
drop function fc_verifica_desconto_faixa_valor;
SQL;
