...
class cl_db_projetosclientes { 
   ...
   function incluir ($at64_sequencial){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,1414,8359,'','".AddSlashes(pg_result($resaco,0,'at64_projeto'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,1414,8360,'','".AddSlashes(pg_result($resaco,0,'at64_cliente'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["at64_projeto"]))
           $resac = db_query("insert into db_acount values($acount,1414,8359,'".AddSlashes(pg_result($resaco,$conresaco,'at64_projeto'))."','$this->at64_projeto',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["at64_cliente"]))
           $resac = db_query("insert into db_acount values($acount,1414,8360,'".AddSlashes(pg_result($resaco,$conresaco,'at64_cliente'))."','$this->at64_cliente',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = @db_query($sql);
     if($result==false){ 
   ...
   function excluir ($at64_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,1414,8359,'','".AddSlashes(pg_result($resaco,$iresaco,'at64_projeto'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,1414,8360,'','".AddSlashes(pg_result($resaco,$iresaco,'at64_cliente'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
