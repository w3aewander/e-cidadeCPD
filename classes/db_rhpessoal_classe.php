...
class cl_rhpessoal
{
   // cria variaveis de erro
    ...
    public $rh01_registrapontoeletronico = 'f';
    public $rh01_matriculaanterior = null;
   // cria propriedade com as variaveis do arquivo
    public $campos = "
                 ...
                 rh01_reajusteparidade = int4 = Tipo de Reajuste
                 rh01_registrapontoeletronico = bool = Registra Ponto Eletrônico
                 rh01_matriculaanterior = varchar(30) = Matrícula Anterior eSocial
                 ";

    /**
     * cl_rhpessoal constructor.
     */
    ...
    function atualizacampos($exclusao = false)
    {
     if($exclusao==false){
       ...
       $this->rh01_registrapontoeletronico = ($this->rh01_registrapontoeletronico == "f"?@$GLOBALS["HTTP_POST_VARS"]["rh01_registrapontoeletronico"]:$this->rh01_registrapontoeletronico);
       $this->rh01_matriculaanterior = ($this->rh01_matriculaanterior == ""?@$GLOBALS["HTTP_POST_VARS"]["rh01_matriculaanterior"]:$this->rh01_matriculaanterior);
     }else{
       $this->rh01_regist = ($this->rh01_regist == ""?@$GLOBALS["HTTP_POST_VARS"]["rh01_regist"]:$this->rh01_regist);
     }
   }
    ...
    public function incluir($rh01_regist)
    {
     ...
     if($this->rh01_reajusteparidade == null ){
       $this->erro_sql = " Campo Tipo de Reajuste não informado.";
     ...
     if($this->rh01_matriculaanterior == null ){
       $this->rh01_matriculaanterior = null;
     }
       $this->rh01_regist = $rh01_regist;
     if(($this->rh01_regist == null) || ($this->rh01_regist == "") ){
       $this->erro_sql = " Campo rh01_regist não declarado.";
     ...
     $sql = "insert into rhpessoal(
                ...
                values (
                               ...
                               ,'$this->rh01_matriculaanterior'
                      )";
     $result = db_query($sql);
     if($result==false){
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Cadastro de funcionários. ($this->rh01_regist) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,1153,1015074,'','".AddSlashes(pg_result($resaco,0,'rh01_matriculaanterior'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   }
    ...
    public function alterar($rh01_regist=null)
    {
     ...
     if(trim($this->rh01_matriculaanterior)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh01_matriculaanterior"])){
        if(trim($this->rh01_matriculaanterior)=="" && isset($GLOBALS["HTTP_POST_VARS"]["rh01_matriculaanterior"])){
           $this->rh01_matriculaanterior = null;
        }
       $sql  .= $virgula." rh01_matriculaanterior = '$this->rh01_matriculaanterior'";
       $virgula = ",";
     }
     $sql .= " where ";
     if($rh01_regist!=null){
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["rh01_matriculaanterior"]) || $this->rh01_matriculaanterior != "")
             $resac = db_query("insert into db_acount values($acount,1153,1015074,'".AddSlashes(pg_result($resaco,$conresaco,'rh01_matriculaanterior'))."','$this->rh01_matriculaanterior',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     if (!$result) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Cadastro de funcionários. não Alterado. Alteração Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->rh01_regist;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Cadastro de funcionários. não foi Alterado. Alteração Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->rh01_regist;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
...
function sql_query_cgm($rh01_regist = null, $campos = "*", $ordem = null, $dbwhere = "")
    ...
    function sql_query_cgm_instituicoes($rh01_regist = null, $campos = "*", $ordem = null, $dbwhere = "", $instituicoes = '')
    {
        ...
        $sql .= "      inner join rhpessoalmov   on  rhpessoalmov.rh02_regist = rhpessoal.rh01_regist
                                           and  rh02_mesusu = " . db_mesfolha() . "
                                           and  rh02_anousu = " . db_anofolha() . "
                                           and  rh02_instit IN (" . $instituicoes . ") ";
        $sql .= "      inner join rhlota  on  rhlota.r70_codigo = rhpessoalmov.rh02_lota
                                           and  rhlota.r70_instit = rhpessoalmov.rh02_instit ";
        $sql .= "      inner join cgm  on  cgm.z01_numcgm = rhpessoal.rh01_numcgm ";
        $sql .= "      inner join db_config on  codigo = rhpessoal.rh01_instit";
        $sql2 = "";
        if ($dbwhere == "") {
            if ($rh01_regist != null) {
                $sql2 .= " where rhpessoal.rh01_regist = $rh01_regist ";
...
// So retornar matriculas ativas
    function sql_query_matriculas_ativas($rh01_regist, $ano = null ,$mes = null)
    {
      if($ano == null) {
        $ano = db_anofolha();
      }
      ...
      $sql  = "with cgmMatricula as (";
      $sql .= " select distinct rh01_numcgm FROM pessoal.rhpessoal ";
      $sql .= "   where rh01_regist = $rh01_regist),";
      $sql .= " listaMatricula as (";
      $sql .= "   select distinct rh01_regist from pessoal.rhpessoal p";
      $sql .= "                               inner join cgmMatricula c on c.rh01_numcgm = p.rh01_numcgm),";
      $sql .= " listaServidorAtivo as (";
      $sql .= " select distinct
                  	rh02_regist,
                    rh30_vinculo,
                    rh05_recis,
                    ...
                    rh05_codigorescisao,
                    r59_motivoesocial,
                    r59_cessacaobeneficios AS cessacaobeneficios,
                    r59_demissaovoluntaria AS demissaovoluntaria from pessoal.rhpessoal p";
      $sql .= "               inner join listaMatricula li on li.rh01_regist = p.rh01_regist
                              inner join pessoal.rhpessoalmov mo on mo.rh02_regist = p.rh01_regist
                                                and mo.rh02_anousu =  $ano 
                                                and mo.rh02_mesusu =  $mes
                                                and mo.rh02_instit = " . db_getsession("DB_instit") . " ";
      $sql .= "               inner join pessoal.rhregime re on mo.rh02_codreg = re.rh30_codreg
	                                              and trim(upper(re.rh30_vinculo)) != 'I'  
                              left join pessoal.rhpesrescisao ps on ps.rh05_seqpes = mo.rh02_seqpes
                              left join pessoal.rescisao rs on 	rs.r59_anousu = mo.rh02_anousu
                                                and rs.r59_mesusu = mo.rh02_mesusu
                                                and rs.r59_regime = re.rh30_regime
                                                and rs.r59_causa = ps.rh05_causa
                                                and rs.r59_caub = ps.rh05_caub::char(2)) ";
      $sql .= "select rh02_regist,rh30_vinculo from listaServidorAtivo where rh05_recis is null";
        return $sql;
    }

    function sql_query_cgm_todas_instituicoes($rh01_regist = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
    ...
    function sql_query_cedencia($rh01_regist = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
            $campos_sql = explode("#", $campos);
        ...
        $sql .= " from rhpessoal ";
        $sql .= "      inner join rhcedencia  on rhcedencia.rh261_regist  = rhpessoal.rh01_regist";
        $sql2 = "";
        if ($dbwhere == "") {
            if ($rh01_regist != null) {
                $sql2 .= " where rhpessoal.rh01_regist = $rh01_regist ";
    ...
    function sql_query_baseRelatorios($sTabela, $sSigla, $iAno = null, $iMes = null, $iInstituicao = null, $sCampos = "*", $sWhere = "", $sOrderBy = "", $sCadAtual = null)
    {
        ...
        $sSQLBase .= "       case when {$sSigla}_pd not in (3,4,5)                                                                   \n";
        $sSQLBase .= "            then 'PD'                                                                                          \n";
        $sSQLBase .= "            else 'N'                                                                                           \n";
        ...
        $sSQLBase.= "                                       and rh55_instit = {$iInstituicao}                                       \n";
        ...
        $sSQLBase .= "       left  join rhtipoapos           on rhtipoapos.rh88_sequencial    = rh02_rhtipoapos                      \n";
    ...
    function sql_queryInsntituicoesServidoresVinculo(
        $iMesCompetencia = null,
        $iAnoCompetencia = null,
        $campos = " codigo , codigo  || ' - ' || nomeinst as nomeinst, rh30_vinculo, (rh30_vinculo = 'A') as ativo ",
        $where = " db_config.db21_ativo = 1 ",
        $groupby = " codigo, rh30_vinculo ",
        $orderby = " codigo ",
        $getCgm = false
    )
    {
    ...
    function sql_verificaSituacaoServidor($iMatriculaServidor, $iAnoCompetencia, $iMesCompetencia)
    {
        ...
        select
            ...
            r59_motivoesocial,
            r59_cessacaobeneficios as cessacaobeneficios,
            r59_demissaovoluntaria as demissaovoluntaria
        from
            rhpessoal
            inner join rhpessoalmov on rhpessoalmov.rh02_regist = rhpessoal.rh01_regist
                and rhpessoalmov.rh02_anousu = {$iAnoCompetencia}
                and rhpessoalmov.rh02_mesusu = {$iMesCompetencia}
...
SQL;
    ...
    function sql_query_com_temporarios($rh01_regist = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        ...
        $sql .= "      left  join rhlota       on  rhlota.r70_codigo = rhpessoalmov.rh02_lota
                                           and  rhlota.r70_instit = rhpessoalmov.rh02_instit ";
        $sql .= "      inner join cgm  on  cgm.z01_numcgm = rhpessoal.rh01_numcgm";
        $sql .= "      left join cgmfisico  on  cgmfisico.z04_numcgm = cgm.z01_numcgm";
        $sql .= "      inner join db_config    on  db_config.codigo = rhpessoal.rh01_instit";
        $sql .= "      inner join rhestcivil  on  rhestcivil.rh08_estciv = rhpessoal.rh01_estciv";
        $sql .= "      inner join rhraca  on  rhraca.rh18_raca = rhpessoal.rh01_raca";
        $sql .= "      left  join rhfuncao     on  rhfuncao.rh37_funcao = rhpessoalmov.rh02_funcao
        ...
        $sql .= "      left  join pessoal.rhimigrante on rh252_matricula = rh01_regist and rh252_instituicao = rh01_instit";
        $sql2 = "";
        if ($dbwhere == "") {
            if ($rh01_regist != null) {
                $sql2 .= " where rhpessoal.rh01_regist = $rh01_regist ";
    ...
    public function sqlServidorSemVinculoESocial($matricula)
    {
        $sql = "
            select
                z01_cgccpf as cpfTrab
               ,rh16_pis as nisTrab
               ,z01_nomecomple as nmTrab
               ,z01_sexo as sexo
              ...
              ,z01_email as emailPrinc
                -- cargoFuncao
              ,rh02_funcao as cargo
              ,rh20_cargo as funcao
                -- fgts
              ,case when rh15_data is null then 2
                    else 1
              end as opcFGTS
              ,to_char(rh15_data, 'DD/MM/YYYY') as dtOpcFGTS
                -- remuneracao
              ,(select f010 from fc_variaveis_matricula(rh01_regist, fc_anofolha(rh01_instit), fc_mesfolha(rh01_instit), rh01_instit)) as vrSalFx
              ,rh02_tipsal as undSalFixo

                -- infoTrabCedido
              ,case when rh30_regime = 1 then 2
                 when rh30_regime = 2 then 1
                else null
              end tpRegTrab
              ,case when rh02_tbprev = 1 then 1
                 else 2
              end as tpRegPrev
              ,rh02_cedencia
            ...
            join configuracoes.cadenderbairrocadenderrua ON cadenderbairrocadenderrua.db87_sequencial = cadenderlocal.db75_cadenderbairrocadenderrua
            join configuracoes.cadenderrua ON cadenderrua.db74_sequencial = cadenderbairrocadenderrua.db87_cadenderrua
            join configuracoes.cadenderbairro ON cadenderbairro.db73_sequencial = cadenderbairrocadenderrua.db87_cadenderbairro
            join configuracoes.cadendermunicipio ON cadendermunicipio.db72_sequencial = cadenderrua.db74_cadendermunicipio
            left join configuracoes.cadendermunicipiosistema ON cadendermunicipiosistema.db125_cadendermunicipio = cadendermunicipio.db72_sequencial
            left join configuracoes.db_sistemaexterno ON db_sistemaexterno.db124_sequencial = cadendermunicipiosistema.db125_db_sistemaexterno
    ...
    public function sqlDependentesServidorESocial($matricula)
    {
        $sql = "
            select
               rh31_nome as nmDep
               ,to_char(rh31_dtnasc, 'DD/MM/YYYY') as dtNascto
               ,rh31_gparen as tpDep
               ,dp01_cpf as cpfDep
              from rhdepend
              join rhdependeplug on rhdependeplug.dp01_rhdepend = rhdepend.rh31_codigo
            where rh31_regist = {$matricula};
        ";
        return $sql;
    }

    /**
     * Busca somente servidores com vínculo (rh30_vinculoemprego = true)
     * @param string $campos
     * @param array $where
     * @param string $order
     * @return string
     */
    function sql_servidores_com_vinculo($campos = "*", $where = array(), $order = '')
    {
        $where[] = "rh30_vinculoemprego is true";
    ...
    public function sqlDadosServidor($matricula, $campos = array('*'))
    {
     ...
     * @throws Exception
     */
    public function sqlLotacaoOrgao($matricula, $campos = '*')
    {
        return "select {$campos}
                from rhpessoal
                     join rhpessoalmov on rhpessoalmov.rh02_regist = rhpessoal.rh01_regist
                                            ...
                                            and rh02_instit = " . db_getsession("DB_instit") . "
                     join rhlotaexe ON rhpessoalmov.rh02_lota = rhlotaexe.rh26_codigo
                                            and rhpessoalmov.rh02_anousu = rhlotaexe.rh26_anousu
