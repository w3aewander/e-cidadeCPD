...
define( 'MENSAGEM_MATRICULA_REPOSITORY', 'educacao.escola.MatriculaRepositoryVR.' );
...
class MatriculaRepositoryVR {
  ...
  protected static function getInstance() {
    ...
    if(self::$oInstance == null) {
      self::$oInstance = new MatriculaRepositoryVR();
    }
  ...
  public static function getMatriculaByCodigo($iMatricula) {
    ...
    if (!array_key_exists($iMatricula, MatriculaRepositoryVR::getInstance()->aMatricula)) {
      MatriculaRepositoryVR::getInstance()->aMatricula[$iMatricula] = new Matricula($iMatricula);
    }

    return MatriculaRepositoryVR::getInstance()->aMatricula[$iMatricula];
  }
  ...
  public static function getAlunoByMatricula($iMatricula) {
    ...
    if( pg_num_rows( $rsMatricula ) > 0 ) {
      return MatriculaRepositoryVR::getInstance()->getMatriculaByCodigo(db_utils::fieldsMemory($rsMatricula, 0)->ed60_i_codigo);
    }
    return false;
  }
  ...
  public static function getMatriculasByTurma(Turma $oTurma) {
    ...
    if ($iTotalLinhas > 0) {
      ...
      for ($i = 0; $i < $iTotalLinhas; $i++) {
       ...
       $aMatriculasTurma[] = MatriculaRepositoryVR::getMatriculaByCodigo($iCodigoMatricula);
      }
    }
  ...
  public static function adicionarMatricula(Matricula $oMatricula) {
    ...
    if (!array_key_exists($oMatricula->getCodigo(), MatriculaRepositoryVR::getInstance()->aMatricula)) {
      MatriculaRepositoryVR::getInstance()->aMatricula[$oMatricula->getCodigo()] = $oMatricula;
    }
    return true;
  }
  ...
  public static function removerMatricula(Matricula $oMatricula) {
    ...
    if (array_key_exists($oMatricula->getCodigo(), MatriculaRepositoryVR::getInstance()->aMatricula)) {
      unset(MatriculaRepositoryVR::getInstance()->aMatricula[$oMatricula->getCodigo()]);
    }
    return true;
  }
  ...
  public static function removeAll() {
    ...
    unset(MatriculaRepositoryVR::getInstance()->aMatricula);
    MatriculaRepositoryVR::getInstance()->aMatricula = array();
    return true;
  }
  ...
  public static function getTodasMatriculasAluno( Aluno $oAluno, $lSomenteSituacaoMatriculado = true, $oTurma = null, $sOrdem = null ) {
    ...
    for( $iContador = 0; $iContador < $iTotalMatriculas; $iContador++ ) {
      ...
      $aMatriculas[]    = MatriculaRepositoryVR::getInstance()->getMatriculaByCodigo( $iCodigoMatricula );
    }
  ...
  public static function getMatriculaAtivaTurma( Turma $oTurma, Aluno $oAluno ) {
    ...
    $aMatriculas = MatriculaRepositoryVR::getTodasMatriculasAluno( $oAluno, true, $oTurma );
...
// Divaldo 14/06/2024
		...
		if( pg_num_rows($rs) == 0 )
		{
			$ed60_i_codigo = substr($where,17,10);
		}else{
			$ed60_i_codigo = db_utils::fieldsMemory($rs, 0)->ed60_i_codigo;
		}
...
//		if( pg_num_rows($rs) == 0) {
        ...
        return MatriculaRepositoryVR::getInstance()->getMatriculaByCodigo($ed60_i_codigo);
    }
    ...
    public static function getMatriculaByCodigoMatricula($codigoMatricula)
    {
        ...
        return MatriculaRepositoryVR::getInstance()->getMatriculaByCodigo(db_utils::fieldsMemory($rs, 0)->codigo);
    }
}
