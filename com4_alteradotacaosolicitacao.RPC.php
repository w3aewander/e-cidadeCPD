    ...
    case "getDotacoes":
        ...
        if ($oDadosSolicitacao->numrows == 0) {
            ...
            for ($i = 0; $i < $iNumRows; $i++) {
                ...
                /**
                 *  Foi adicionada uma regra para exibir itens que foram parcialmente autorizados em outros processos
                 *  Anteriormente somente verificava se o item possui autorização, agora também é verificado o saldo do item
                 */
                $iQuantidadeAutorizada = AutorizacaoEmpenho::quantidadeAutorizada($oItensDotacao->pc11_codigo,
                  $oItensDotacao->pc13_coddot, $oParam->iCodigoSolicitacao);
                $iQuantidadeDisponivel = ($oItensDotacao->pc13_quant - $iQuantidadeAutorizada);
                $oItensDotacao->pc13_quant = $iQuantidadeDisponivel;

                if ($iQuantidadeDisponivel >= 0 || !AutorizacaoEmpenho::verificaItemAutorizado($oItensDotacao->pc11_codigo,
                    $oItensDotacao->pc13_coddot,
                    $oParam->iCodigoSolicitacao)) {
                    /*
    ...
    case "alteraDotacoes":
        try {
            ...
            foreach ($oParam->aItens as $oItem) {
                ...
                $oItemSolicitacao->alterarDotacao($oItem->iCodigoDotacaoItem, $oItem->iCodigoDotacao,
                  $oItem->iAnoDotacao);
            }
