...
<fieldset >
  ...
  <form name="form1" method="post" action="">
    ...
    <fieldset id="alfa" style='display:none;'>
      ...
      <label id='labelAlfa'> <?=$Lla29_v_fixo?> </label>
      <?php
        db_input('marcado',1,@$Imarcado,true,'checkbox',$db_opcao,"onclick='js_bloqueia()'");
        db_input('la29_i_codigo',10,$Ila29_i_codigo,true,'hidden',$db_opcao,"");
        db_input('la29_v_fixo',30,$Ila29_v_fixo, true, 'text', 1);
      ?>
      </div>
...
function js_valida(){
	...
	if((F.la29_v_fixo.value=='')||(F.la29_v_fixo.value=='0')){
...
function js_bloqueia(){
  ...
  if (document.getElementById("marcado").checked==true) {
	document.getElementById("la29_v_fixo").readOnly = false;
    document.getElementById("la29_v_fixo").style.cssText = 'background-color: #E6E4F1;';

  } else {
	document.getElementById("la29_v_fixo").value='';
	readonlyValorFixo();
  }
}
  ...
  var oAjax  = new Ajax.Request(sUrlRPC,
    {
      ...
      onComplete: function(oResponse) {
        ...
        $('la30_casasdecimaisapresentacao').value = response.oValorReferencia.casas_decimais;
      }
    }
  )
}

function valorFixoMarcado() {
...
function readonlyValorFixo() {
    let inputValorFixo = document.getElementById('la29_v_fixo');
    inputValorFixo.readOnly = true;
    inputValorFixo.style.cssText = 'background-color: #DEB887;';
}
