...
class CompraEditalAvisoService
{
    ...
    public function incluirCompra(InclusaoCEARequest $request)
    {
        ...
        try {
            ...
            $linkCompra = env('PNCP_LINK_COMPRA') . "{$request->cnpj}/{$compra[0]}/$compra[1]";
            $dadosCompra = $this->http->buscarCompra($request, $compra[0], $compra[1]);
    ...
    public function montarDadosCompra($request)
    {
        ...
        if (!empty($request->justificativaPresencial)) {
            $justificativa = $request->justificativaPresencial;

            // Se não estiver em UTF-8, converte
            if (!mb_check_encoding($justificativa, 'UTF-8')) {
                $justificativa = mb_convert_encoding($justificativa, 'UTF-8', 'ISO-8859-1');
            }

            $compra['justificativaPresencial'] = $justificativa;
        }
