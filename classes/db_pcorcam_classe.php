...
class cl_pcorcam
{
    ...
    public function buscaItensOrcamentoPorFornecedor($codigoOrcamento, $numeroCgm)
    {
        $sql = "
            SELECT
                pc11_seq,
                ...
                m61_usaquant,
                pc01_servico,
                pc21_prazoent as pc11_prazo,
                pc23_validmin as pc01_validademinima
            FROM   pcorcam
                INNER JOIN pcorcamitem
                        ON pcorcamitem.pc22_codorc = pcorcam.pc20_codorc
                INNER JOIN pcorcamforne
                    ON pcorcamforne.pc21_codorc = pcorcam.pc20_codorc
                INNER JOIN cgm
                    ON cgm.z01_numcgm = pcorcamforne.pc21_numcgm
                INNER JOIN pcorcamitemproc
                        ON pcorcamitemproc.pc31_orcamitem = pcorcamitem.pc22_orcamitem
                ...
                INNER JOIN pcdotac
                        ON pc13_codigo = solicitem.pc11_codigo
                LEFT JOIN solicitempcmater
                    ON solicitempcmater.pc16_solicitem = solicitem.pc11_codigo
                LEFT JOIN pcmater
                    ON pcmater.pc01_codmater = solicitempcmater.pc16_codmater
                LEFT JOIN pcsubgrupo
                    ON pcsubgrupo.pc04_codsubgrupo = pcmater.pc01_codsubgrupo
                LEFT JOIN pctipo
                    ON pctipo.pc05_codtipo = pcsubgrupo.pc04_codtipo
                LEFT JOIN solicitemele
                    ON solicitemele.pc18_solicitem = solicitem.pc11_codigo
                LEFT JOIN solicitemunid
                    ON solicitemunid.pc17_codigo = solicitem.pc11_codigo
                LEFT JOIN matunid
                    ON matunid.m61_codmatunid = solicitemunid.pc17_unid
                LEFT JOIN pcorcamjulg
                    ON pcorcamjulg.pc24_orcamforne = pcorcamforne.pc21_orcamforne
                        AND pcorcamjulg.pc24_orcamitem = pcorcamitem.pc22_orcamitem
                LEFT JOIN pcorcamval
                    ON pcorcamval.pc23_orcamforne = pcorcamforne.pc21_orcamforne
                        AND pcorcamval.pc23_orcamitem = pcorcamitem.pc22_orcamitem
            WHERE  pc22_codorc = {$codigoOrcamento} AND pc21_numcgm = {$numeroCgm}
            ORDER  BY pc20_codorc,
                pc21_orcamforne,
                pc22_orcamitem;
        ";
    ...
    public function buscaOrcamentosSolicitacao($idSolicitacao, $campos = array('*'), $filtros = array())
    {
        ...
        $sql = "
            ...
            FROM
                pcorcam
            LEFT JOIN pcorcamitem
                ON pcorcamitem.pc22_codorc = pcorcam.pc20_codorc
            LEFT JOIN pcorcamitemproc
                ON pcorcamitemproc.pc31_orcamitem = pcorcamitem.pc22_orcamitem
            LEFT JOIN pcprocitem
                ON pcprocitem.pc81_codprocitem = pcorcamitemproc.pc31_pcprocitem
            LEFT JOIN pcproc
                ON pcproc.pc80_codproc = pcprocitem.pc81_codproc
            LEFT JOIN pcorcamitemsol
                ON pcorcamitemsol.pc29_orcamitem = pcorcamitem.pc22_orcamitem
            LEFT JOIN solicitem
                ON pc11_codigo = pcorcamitemsol.pc29_solicitem
                OR pc11_codigo = pcprocitem.pc81_solicitem
            LEFT JOIN solicita
                ON solicita.pc10_numero = solicitem.pc11_numero
            WHERE
                solicita.pc10_numero = {$idSolicitacao}
                {$filtros}
            ORDER BY
                pc20_codorc DESC
        ";
