...
class AjustePrevidencia
{
    ...
    static function gravarModificacoes()
    {
        ...
        if ($pessoal[$Ipessoal]["r01_tbprev"] > 0) {
            ...
            $condicaoaux .= " and  extract(month from rh05_recis) >= " . db_sqlformat(substr("#" . $subpes, 6,
                2)) . "))";
        ...
        " . bb_condicaosubpes("rh02_") . $condicaoaux);
            ...
            if (count($pes_prev) > 1) {
                ...
                db_selectmax("transacao",
                  "select * from " . $chamada_geral_arquivo . " " . bb_condicaosubpes($siglag) . $condicaoaux);
                ...
                for ($Itransacao = 0; $Itransacao < count($transacao); $Itransacao++) {
                    ...
                    if ($opcao_geral == 1 && $chamada_geral_arquivo == "gerfsal") {
                        ...
                        if (db_selectmax("transacao",
                          "select * from gerfcom " . bb_condicaosubpes("r48_") . $condicaoaux)) {
                ...
                db_selectmax("transacao",
                  "select * from " . $chamada_geral_arquivo . " " . bb_condicaosubpes($siglag) . $condicaoaux);
                $xy = (db_val(substr("#" . $transacao[0][$siglag . "rubric"], 2,
                      3)) - db_val(substr("#" . $rubrica_base, 2, 3))) + 1;
                ...
                for ($nosx = 1; $nosx < 4; $nosx++) {
                    ...
                    if ($matrizr[$nosx] != "") {
                        ...
                        if (db_selectmax("transacao",
                          "select * from previden " . bb_condicaosubpes("r60_") . $condicaoaux)) {
                            LogCalculoFolha::write("Alterando os valores da tabela previdencia." . bb_condicaosubpes("r60_") . $condicaoaux . "\n\n" . print_r(array_combine($matriz1,
                                $matriz2), true));
                            db_update("previden", $matriz1, $matriz2, bb_condicaosubpes("r60_") . $condicaoaux);
                        } else {
                            LogCalculoFolha::write("Inserindo os valores da tabela previdencia." . print_r(array_combine($matriz1,
                                $matriz2), true));
    ...
    public static function ajustar($arquivo, $rubrica_base, $sequencia, $sigla_ajuste)
    {
        ...
        if (db_selectmax("previden_",
          "select * from previden " . $condicaoAjustePreviden . " " . bb_condicaosubpes("r60_"))) {
            ...
            db_selectmax("previden_",
              "select * from previden " . $condicaoAjustePreviden . " " . bb_condicaosubpes("r60_") . $condicaoaux);
          ...
          left join rhinssoutros on rh51_seqpes = rhpessoalmov.rh02_seqpes " . bb_condicaosubpes("rh02_") . $condicaoaux);
                ...
                db_selectmax("pessoal_2", "select h13_tpcont as r01_tpcont,
          ...
          " . bb_condicaosubpes("rh02_") . " and rh02_regist = {$registro}");
                ...
                if ($desc_prev_ext > 0) {
                    $valor_desconto_ext = teto_tabprev($soma_base1, db_str($tbprev + 2, 1),
                      $pessoal_2[0]["r01_tpcont"]);
                ...
                if ($arquivo == 'gerffer') {
                    ...
                    if ($soma_base_F != 0) {
                        ...
                        $valor_desc_total_F = teto_tabprev($soma_base_F, db_str($tbprev + 2, 1),
                          $pessoal_2[0]["r01_tpcont"]);

                        if ($valor_desc_total_F <= 0) {
                            LogCalculoFolha::write("Chamando função que calcula o valor da previdencia");
                            $valor_desc_total_F = calc_tabprev($soma_base_F, db_str($tbprev + 2, 1),
                              $pessoal_2[0]["r01_tpcont"]);
                        }
                    ...
                    if ($soma_base_D != 0) {
                        $valor_desc_total_D = teto_tabprev($soma_base_D, db_str($tbprev + 2, 1),
                          $pessoal_2[0]["r01_tpcont"]);
                        if ($valor_desc_total_D <= 0) {

                            LogCalculoFolha::write("Chamando função que calcula o valor da previdencia");
                            $valor_desc_total_D = calc_tabprev($soma_base_D, db_str($tbprev + 2, 1),
                              $pessoal_2[0]["r01_tpcont"]);
                        }
                    ...
                    if ($rubrica_base == 'R985' && $arquivo == 'gerfsal' && isset($soma_base_teto) && $soma_base_teto > 0) {
                        ...
                        $valor_desconto_total = teto_tabprev($soma_base_teto, db_str($tbprev + 2, 1),
                          $pessoal_2[0]["r01_tpcont"]);
                    ...
                    $valor_desconto_total = calc_tabprev($soma_base1, db_str($tbprev + 2, 1),
                      $pessoal_2[0]["r01_tpcont"]);
          ...
          left join rhregime     on rh30_codreg = rh02_codreg " . bb_condicaosubpes("r60_") . $condicaoaux;
                LogCalculoFolha::write('SQL previden ...: ' . $sql);
                    ...
                    for ($Ipreviden_ = 0; $Ipreviden_ < count($previden_); $Ipreviden_++) {
                        ...
                        if ($arquivo == 'gerffer') {
                            ...
                            if ($soma_base_F != 0) {
                                ...
                                $novo_desc_F = round($previden_[$Ipreviden_]["r60_basef"] / $soma_base_F * $valor_a_ratear_F,
                                  2);
                                ...
                                if (round($soma_desc_F, 2) > round($valor_a_ratear_F, 2)) {
                                    $matriz2[1] = abs($novo_desc_F - (round($soma_desc_F - ($soma_desc_F - $valor_a_ratear_F),
                                        2)));
                                }
                                ...
                                $retornar = db_update($arquivo, $matriz1, $matriz2,
                                  bb_condicaosubpes($sigla_ajuste) . $condicaoaux . " and r31_tpp = 'F'");
                            ...
                            if ($soma_base_D != 0) {
                                ...
                                $novo_desc_D = round(($previden_[$Ipreviden_]["r60_base"] - $previden_[$Ipreviden_]["r60_basef"]) / $soma_base_D * $valor_a_ratear_D,
                                  2);
                                $matriz2[1] = $novo_desc_D;
                                $matriz2[2] = $perc_inss_D;
                                $retornar = db_update($arquivo, $matriz1, $matriz2,
                                  bb_condicaosubpes($sigla_ajuste) . $condicaoaux . " and r31_tpp = 'D'");
                                LogCalculoFolha::write('Update: ' . bb_condicaosubpes($sigla_ajuste) . $condicaoaux . "\n\n" . print_r(array_combine($matriz1,
                                    $matriz2), true));;
                                ...
                                if (round($soma_desc_D, 2) > round($valor_a_ratear_D, 2)) {
                                    ...
                                    $matriz2[1] = abs($novo_desc_D - (round($soma_desc_D - ($soma_desc_D - $valor_a_ratear_D),
                                        2)));
                                }
                            ...
                            if ($valor_a_ratear > 0) {
                                ...
                                LogCalculoFolha::write("Novo Desconto...: ({$previden_[$Ipreviden_]["r60_base"]} / {$soma_base}  * {$valor_a_ratear})");
                                $novo_desconto = round($previden_[$Ipreviden_]["r60_base"] / $soma_base * $valor_a_ratear,
                                  2);
                  ...
                  and " . $sigla_ajuste . "rubric in ('R903','R906','R909','R912') and rh01_numcgm = $numcgm and rh01_regist = $registrop";
                    ...
                    for ($nro = 0; $nro < count($mat_r60_numcgm); $nro++) {
                        ...
                        if ($opcao_geral != 3) {
                            ...
                            $rubrica_desconto = "R9" . db_str(((3 * $mat_r60_tbprev[$nro]) - 2) - 1 + $sequencia, 2, 0,
                                "0");
                              ...
                              "select * from " . $arquivo . " " . bb_condicaosubpes($sigla_ajuste) . $condicaoaux)) {
                                ...
                                if (($ajusta_dif <= 0 && $atingiu_o_teto_tbprev == false) || $atingiu_o_teto_tbprev == true) {
                                    ...
                                    if ($arquivo == 'gerfsal') {
                                        ...
                                        if (db_selectmax("transacao1",
                                          "select sum(r14_valor) as descontod from gerfsal " . bb_condicaosubpes("r14_") . $condicaoaux)) {
                                    ...
                                    if ($arquivo != "gerffer") {
                                        ...
                                        $retornar = db_update($arquivo, $matriz1, $matriz2,
                                          bb_condicaosubpes($sigla_ajuste) . $condicaoaux);
                            ...
                            if (db_selectmax("transacao1",
                              "select sum(" . $sigla_ajuste . "valor) as descontod from " . $arquivo . " " . bb_condicaosubpes($sigla_ajuste) . $condicaoaux)) {
                                $mat_r60_novods[$nro] = $transacao1[0]["descontod"];
                            }
                            ...
                            if (db_selectmax($arquivo,
                              "select * from " . $arquivo . " " . bb_condicaosubpes($sigla_ajuste) . $condicaoaux)) {
                                ...
                                if ($mat_r60_novods[$nro] > 0) {
                                    ...
                                    $retornar = db_insert($arquivo, $matriz1, $matriz2);
                                    LogCalculoFolha::write('Insertchê: ' . print_r(array_combine($matriz1, $matriz2),
                                        true));
                                }
                            }
                            ...
                            if (substr("#" . $arq_[$Iarquivo][$sigla_ajuste . "rubric"], 1, 1) != "R") {
                                if ($db21_codcli == "999999999" && $arq_[$Iarquivo][$sigla_ajuste . "rubric"] == "0045") {
                                    ...
                                    if ($arq_[$Iarquivo][$sigla_ajuste . "pd"] == 1) {
                                        ...
                                        if ($arq_[$Iarquivo][$sigla_ajuste . "pd"] == 2) {
                                            $tot_desc += $arq_[$Iarquivo][$sigla_ajuste . "valor"];
                                            if ($db_debug == true) {
                                                echo "[ajusta_previdencia] 18 - tot_desc: $tot_desc<br>";
                                            }
                                        }
                                    }
                                }
                                ...
                                if ($arq_[$Iarquivo][$sigla_ajuste . "rubric"] == "R927" || $arq_[$Iarquivo][$sigla_ajuste . "rubric"] == "R929") {
                                    ...
                                    if ($db_debug == true) {
                                        echo "[ajusta_previdencia] 19 - tot_desc: $tot_desc<br>";
                                    }
                                    ...
                                    if ($nro1 >= 17 && $nro1 <= 22) {
                                        ...
                                        if ($arq_[$Iarquivo][$sigla_ajuste . "pd"] == 1) {
                                            ...
                                            if ($arq_[$Iarquivo][$sigla_ajuste . "pd"] == 2) {
                                                $tot_desc += $arq_[$Iarquivo][$sigla_ajuste . "valor"];
                                                if ($db_debug == true) {
                                                    echo "[ajusta_previdencia] 20 - tot_desc: $tot_desc<br>";
                                                }
                                            }
                                        }
                                    }
    ...
    static function gravarDadosCalculados(Servidor $oServidorPrincipal, FolhaPagamento $oFolha)
    {
        ...
        foreach ($aServidores as $oServidor) {
            ...
            $aEventosSalario = $oCalculoSalario->getEventosFinanceiros(null,
              $sRubricaBasePrevidencia);//R985 - Folha Salario
            $aEventosComplementar = $oCalculoComplementar->getEventosFinanceiros(null,
              $sRubricaBasePrevidencia);//R985 - Folha Complementar
