...
//var_dump($this->e60seqemp); die("Confere");
//var_dump($this->ordpag); die("Confere OP");
...
for ($xxx = 0; $xxx < $this->nvias; $xxx++) {
  ...
  $this->objpdf->text($xcol+2,$xlin+30,'Recurso');
  //$this->objpdf->text($xcol+2,$xlin+38,'Processo');
  $this->objpdf->text($xcol+2,$xlin+34,'Processo');
  //$this->objpdf->text($xcol+2,$xlin+42,'Processo Licitatório');
  $this->objpdf->text($xcol+2,$xlin+38,'Processo Licitatório');
  ...
  $this->objpdf->text($xcol+17,$xlin+27,':  '.db_formatar($this->elemento,'elemento') . ' '. $this->descr_elemento);
  

  $this->objpdf->text($xcol+17,$xlin+30,':  '.$this->recurso.' - '.$this->descr_recurso);
  $this->objpdf->text($xcol+17,$xlin+34,':  '.$this->processo);

  
  $numprocessolicitatorio = "";

  $numerodanota = (int)$this->numeronota;

//var_dump($numerodanota); die("Verifica");
//123321
if($this->numeronota != 0 ){
  $sqlds = pg_query("SELECT * FROM pagordemprocesso WHERE e03_numnota = '{$numerodanota}' AND e03_pagordem = {$this->ordpag}");
  $resultadods = pg_fetch_all($sqlds);  
  if(!empty($resultadods)){    
    $numprocessolicitatorio = $resultadods[0]['e03_numprocessolicit'];
  }

  $sqlds2 = pg_query("SELECT * FROM notafiscalcoc WHERE nota = '{$numerodanota}'");  
  $resultadods2 = pg_fetch_all($sqlds2);
  
  if(!empty($resultadods2)){    
    $numprocessolicitatorio = $resultadods2[0]['numprocessolicit'];        
  }
  ...
  $this->objpdf->text($xcol+30,$xlin+38,':  '."{$numprocessolicitatorio}");

  if ($ano < db_getsession("DB_anousu")) {
//	$this->objpdf->text($xcol+2,$xlin+38,'RESTOS A PAGAR ');
/* linha modificada para o texto 'RESTOS À PAGAR' ser acertado que está subindo em PROCESSO por Flávio Henrique em 08/01/2016 && */
	$this->objpdf->text($xcol+76,$xlin+38,'RESTOS A PAGAR ');
  }
  ...
  //LINHA SETADA PARA O ELEMENTO SUBIR A GRADE. por Flávio Henrique em 08/01/2016 && */
  $this->objpdf->sety($xlin+49);
  ...
  $rsGuias = db_query("SELECT e03_numguia, e03_descguia FROM pagordemprocesso WHERE e03_numguia IS NOT NULL AND e03_pagordem = ". $this->ordpag);
  ...
  if( pg_numrows($rsNotas) > 0 ){
   ...
   if(pg_numrows($rsGuias) > 0 ){
    $nugu = pg_result($rsGuias, 0, 0);
    $degu = pg_result($rsGuias, 0, 1);
    $this->objpdf->cell(102,4,"Número Guia: {$nugu}  | Descrição: {$degu}",0,1,"L");
   }
...
if($this->numeronota != 0 ){
  ...
  $resultadods = pg_fetch_all($sqlds);
  if(!empty($resultadods)){
    $serienota = $resultadods[0]['e03_serienota'];
    $subserienota = $resultadods[0]['e03_subserienota'];
  }else{
    $sqlds2 = pg_query("SELECT * FROM notafiscalcoc WHERE nota = '{$numerodanota}'");  
    $resultadods2 = pg_fetch_all($sqlds2);
    if(!empty($resultadods2)){
      $serienota = $resultadods2[0]['serienf'];
      $subserienota = $resultadods2[0]['subserienf'];
    }
  }
...
if(db_getsession("DB_instit") == 45){
  ...
  if (is_array($this->aRetencoes) && count($this->aRetencoes) > 0 ) {
    ...
    for ($ii = 0; $ii < count($this->aRetencoes); $ii++) {
      ...
      if ($ii < 9) {
      //SCRIPT REMONTADO PARA QUE AS RETENÇÕES SAIAM CORRETAMENTE NO RETANGULO DAS RETENÇÕES.por Flávio Henrique em 08/01/2016 &&
         $codigo  = $this->aRetencoes[$ii]->k02_codigo;
         $receita = substr($this->aRetencoes[$ii]->k02_drecei,0,40);
         $valorret=db_formatar($this->aRetencoes[$ii]->e23_valorretencao,'f'); 
         $jj = $ii*3;
         $this->objpdf->text($xcol+104,$xlin+139+$jj,$codigo);
         $this->objpdf->text($xcol+113,$xlin+139+$jj,$receita);
         $this->objpdf->text($xcol+185,$xlin+139+$jj,$valorret);

       /* CÓDIGO OFICIAL DA RELEASE 44 COMENTADO. Pof Flávio Henrique em 08/01/2016 &&
   ...
   if ($ii > 9) {
      $this->objpdf->sety($xlin+139+$jj);
      $this->objpdf->setx(113);
      $this->objpdf->cell(25,4,"Outras Retenções",0,0,"R");
