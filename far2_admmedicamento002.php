...
function getDataProx($iCgs, $iMedicamento, $isPeriodoFixo)
{
    global $dDataAtual;
    ...
    return $infoContinuado->proximaRetirada->format('d/m/Y');
}
...
function novoTotal($oPdf, $iPacientes, $iMedicamentos)
{
    ...
    $texto = "Total de Pacientes: $iPacientes             Total de Medicamentos: $iMedicamentos ";
    $oPdf->cell(190, 5, $texto, 0, 1, 'C', $lCor);
}
...
if (!empty($medicamentos)) {
    ...
    if ($iExato != 2) {
        ...
        $sWhereMedicamentos = "{$sSeparador} {$sInMedicamentos} AND ({$sExatoMedicamento} and {$sInMedicamentos}) >= 1";
        $sSeparador = ' and ';
    } else {
...
$sCampos = ' far_retiradaItensDepartamento.coddepto,far_retiradaItensDepartamento.fa06_f_quant,';
$sCampos .=' far_retiradaItensDepartamento.fa04_d_data,';
...
for ($iCount = 0; $iCount < $iLinhas; $iCount++) {
    ...
    if ($iMedicamento != $oDados->fa01_i_codigo) {
        ...
        $dDataProx = getDataProx($oDados->z01_i_cgsund, $iMedicamento, $oDadosFarParametros->fixo == 1);
