...
function buscaAutorizacao($codaluno){  
  $sql = pg_query("SELECT aimagem, acoluna FROM aluno WHERE ed47_i_codigo = {$codaluno}");
  $resultado = pg_fetch_all($sql);
  return $resultado[0];
}
...
$oFiltros->iTamanhoEtapa         = 20;
$oFiltros->iTamanhoDataMatricula = 24;
$oFiltros->iTamanhoTurno = 26;
$oFiltros->iTamanhoTurma = 17;
$oFiltros->iTamanhoImagem = 18;
$oFiltros->iTamanhoColuna = 22;
...
if (in_array('xaimagem', $oFiltros->aCampos)) {
  $iPosicaoArray = array_search('xaimagem', $oFiltros->aCampos);  
  $oFiltros->oImagem                = new stdClass();
  $oFiltros->oImagem->sDescricao    = 'Aut. Imagem';
  $oFiltros->oImagem->iTamanho      = $oFiltros->iTamanhoImagem;
  $oFiltros->oImagem->sAlinhamento  = $oFiltros->aAlinhamento[$iPosicaoArray];
  $oFiltros->oImagem->iTipo         = 8;
  $oFiltros->oNomeAluno->iTamanho         -= $oFiltros->oImagem->iTamanho;
  $oFiltros->aCabecalho[$iPosicaoArray]    = $oFiltros->oImagem;
}

if (in_array('xacoluna', $oFiltros->aCampos)) {
  $iPosicaoArray = array_search('xacoluna', $oFiltros->aCampos);  
  $oFiltros->oColuna                = new stdClass();
  $oFiltros->oColuna->sDescricao    = 'Proj. Coluna Reta';
  $oFiltros->oColuna->iTamanho      = $oFiltros->iTamanhoColuna;
  $oFiltros->oColuna->sAlinhamento  = $oFiltros->aAlinhamento[$iPosicaoArray];
  $oFiltros->oColuna->iTipo         = 9;
  $oFiltros->oNomeAluno->iTamanho         -= $oFiltros->oColuna->iTamanho;
  $oFiltros->aCabecalho[$iPosicaoArray]    = $oFiltros->oColuna;
}
...
foreach ($oFiltros->aQuebras as $iEscola => $aEscola) {
  ...
  $cabecalho = cabecalhoRelatorio($oPdf, $oFiltros);  
  $corpanzil = corpoRelatorio($oPdf, $oFiltros, $aEscola);

  header("Content-Type:   application/vnd.ms-excel; charset=utf-8");
  header("Content-type:   application/x-msexcel; charset=utf-8");
  header("Content-Disposition: attachment; filename=relatorio_autorizacoes.xls");
  header("Expires: 0");
  ...
  $xcab = explode(";", $cabecalho);
  print "<table>";
  print "<thead>";
  print "<tr>";
  foreach($xcab as $linha){
    print "<th>".$linha."</th>";
  }
  print "<th>Escola</th>";
  print '</tr>';

  foreach($corpanzil as $linha){
    $laluno = explode(";", $linha);
    print "<tr>";
    foreach($laluno as $lf){
      print "<td>".$lf."</td>";
    }
    print "<td>" . $aEscolas[$iEscola] . "</td>";
    print "</tr>";    
  }
  print "</table>";
...
function cabecalhoRelatorio(PDF $oPdf, $oFiltros) {
  ...
  foreach ($oFiltros->aCabecalho as $oCabecalho) {
    ...
    $cabeca1 .= $oCabecalho->sDescricao . ";";
  }
  $oPdf->Ln();
  $cabeca1 = substr($cabeca1, 0, -1);
  return $cabeca1;
}
    ...
    foreach ($aEnsino as $iTurma => $aTurma) {
      ...
      foreach ($aTurma as $iEtapa => $sEtapa) {
        ...
        foreach ($aAlunos as $oMatricula ) {          
          $autorizacoes = buscaAutorizacao($oMatricula->getAluno()->getCodigoAluno());
          $autimagem = $autorizacoes["aimagem"];
          if($autimagem == 1){
            $autimagem = "S";
            $autimagemescreve = "SIM";
          }elseif($autimagem == 2){
            $autimagem = "N";
            $autimagemescreve = "NÃO";
          }
          $autcoluna = $autorizacoes["acoluna"];
          if($autcoluna == 1){
            $autcoluna = "S";
            $autcolunaescreve = "SIM";
          }elseif($autcoluna == 2){
            $autcoluna = "N";
            $autcolunaescreve = "NÃO";
          }
          
          foreach ($oFiltros->aCabecalho as $confirma) {
            if($confirma->iTipo == 8){
              $mostraimagem = $confirma->sAlinhamento;
            }
            if($confirma->iTipo == 9){
              $mostracoluna = $confirma->sAlinhamento;
            }
          }
          //var_dump($autimagem); var_dump($autcoluna); echo "<br>";
          //var_dump($mostraimagem); var_dump($mostracoluna);
          
          //var_dump(!($autimagem == $mostraimagem));
          //var_dump(!($autcoluna == $mostracoluna));
          //die("B");
          
          if(($autimagem == $mostraimagem || $mostraimagem == null) && ($autcoluna == $mostracoluna || $mostracoluna == null)){
            //Mostra
          }else{
            continue;
          }
          
          //var_dump($mostraimagem); var_dump($autimagem); echo "<br>";
          //var_dump($mostracoluna); var_dump($autcoluna);  echo "<br>";          
          //die("Confere");
          ...
          foreach ($oFiltros->aCabecalho as $oCabecalho) {
            ...
            switch ($oCabecalho->iTipo) {
              ...
              case 1:
                ...
                $corpo1 .= $oMatricula->getAluno()->getCodigoAluno() . ";";
                break;
              ...
              case 2:
                ...
                $corpo1 .= $oMatricula->getAluno()->getNome() . ";";
                break;
              ...
              case 8:
                $oPdf->Cell($oCabecalho->iTamanho, 4, $autimagemescreve, 1, 0, "C");
                $corpo1 .= $autimagemescreve . ";";
                break;

              case 9:
                $oPdf->Cell($oCabecalho->iTamanho, 4, $autcolunaescreve, 1, 0, "C");
                $corpo1 .= $autcolunaescreve . ";";
                break;
            }
          }
          $corpo1 = substr($corpo1, 0, -1);
          array_push($corpao, $corpo1);
