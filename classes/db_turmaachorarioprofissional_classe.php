...
class cl_turmaachorarioprofissional
{
    ...
    public function incluir($ed346_sequencial)
    {
        $this->atualizacampos();
    ...
    public function alterar($ed346_sequencial = null)
    {
        $this->atualizacampos();
    ...
    public function excluir($ed346_sequencial = null, $dbwhere = null)
    {
    ...
    // funcao do sql
    public function sql_query($ed346_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
    ...
    // funcao do sql
    public function sql_query_file($ed346_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
            $campos_sql = split("#", $campos);
    ...
    public function sql_query_vinculo_profissional($ed346_sequencial = null, $sCampos = "*", $sOrdem = null, $sWhere = "")
    {
        $sSql = "select ";
        if ($sCampos != "*") {
        ...
        $sSql .= "       left join cgmfisico on cgmfisico.z04_numcgm = cgmcgm.z01_numcgm or cgmfisico.z04_numcgm = cgmrh.z01_numcgm";
        $sSql2 = "";
    ...
    public function sql_query_vinculo_profissional_union_sem_profissional($ed346_sequencial = null, $sCampos = "*", $sOrdem = null, $sWhere = "", $sWhereUnion = "", $sOrdemUnion = null)
    {
        ...
        $sSql .= " UNION ";
        $sSql .= "  SELECT DISTINCT ed176_sequencial,";
        $sSql .= "  ed176_rechumano,";
        $sSql .= "  ed119_descricao as profissional,";
        $sSql .= "  ed119_sequencial,";
        $sSql .= "  ed119_descricao,";
        $sSql .= "  ed32_i_codigo,";
        $sSql .= "  ed32_c_descr,";
        $sSql .= "  ed176_horainicial,";
        $sSql .= "  ed176_horafinal";
        $sSql .= "  FROM turmaachorarioprofissionalsemrec";
        $sSql .= "  INNER JOIN turmaac ON turmaac.ed268_i_codigo = turmaachorarioprofissionalsemrec.ed176_turmaac";
        $sSql .= "  LEFT JOIN funcaoatividade ON funcaoatividade.ed119_sequencial = turmaachorarioprofissionalsemrec.ed176_funcaoatividade";
        $sSql .= "  INNER JOIN diasemana ON diasemana.ed32_i_codigo = turmaachorarioprofissionalsemrec.ed176_diasemana";
        $sSql .= "  INNER JOIN escola ON escola.ed18_i_codigo = turmaac.ed268_i_escola";
        $sSql .= "  INNER JOIN turno ON turno.ed15_i_codigo = turmaac.ed268_i_turno";
        $sSql .= "  LEFT JOIN sala ON sala.ed16_i_codigo = turmaac.ed268_i_sala";
        $sSql .= "  INNER JOIN calendario ON calendario.ed52_i_codigo = turmaac.ed268_i_calendario";
        ...
        if ($sWhereUnion == "") {
            if ($ed176_sequencial != null) {
                $sSql2 .= " where turmaachorarioprofissionalsemrec.ed176_sequencial = $ed176_sequencial ";
            }
        } else if ($sWhereUnion != "") {
            $sSql2 = " where $sWhereUnion";
        }
        $sSql .= $sSql2;
        if ($sOrdemUnion != null) {

            $sSql .= " order by ";
            $campos_sql = split("#", $sOrdemUnion);
            $virgula = "";
            for ($i = 0; $i < sizeof($campos_sql); $i++) {
                $sSql .= $virgula . $campos_sql[$i];
                $virgula = ",";
    ...
    public function sqlDadosCenso($where = array()) {
        ...
        $sql = "
        select distinct ed75_i_codigo as vinculo_escola, ed346_funcaoatividade as funcao
          from turmaachorarioprofissional
          join turmaac on  turmaac.ed268_i_codigo = turmaachorarioprofissional.ed346_turmaac
          join funcaoatividade on  funcaoatividade.ed119_sequencial = turmaachorarioprofissional.ed346_funcaoatividade
          join rechumano on rechumano.ed20_i_codigo = turmaachorarioprofissional.ed346_rechumano
