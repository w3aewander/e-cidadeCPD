...
if (!empty($recusos_selecionados)) {
    $sele_work .= " and orctiporec.o15_codigo in ($recusos_selecionados) ";
}
$dataini = $DBtxt21_ano . '-' . $DBtxt21_mes . '-' . $DBtxt21_dia;
$datafin = $DBtxt22_ano . '-' . $DBtxt22_mes . '-' . $DBtxt22_dia;
...
$pdf->init(false);
$pdf->addTitulo("BALANCETE DA DESPESA");
$pdf->addTitulo('');
$pdf->addTitulo("EXERCÍCIO: " . db_getsession("DB_anousu"));
$pdf->addTitulo("PERÍODO: $dt1 à $dt2 ");
...
for ($i = 0; $i < count($arr_niveis); $i++) {
    ...
    if ($nivel == 1) {
        ...
        // Recurso
        if (trim($selecao) != "") {
            $selecao .= ",o58_codigo, gestao, descricao";
        } else {
            $selecao = "select o58_codigo, gestao, descricao";
        }

        if (trim($agrupar) != "") {
            $agrupar .= ",o58_codigo ,gestao, descricao";
        } else {
            $agrupar = "group by o58_codigo, gestao, descricao";
        }

        if (trim($ordem) != "") {
            $ordem .= ",o58_codigo, gestao, descricao";
        } else {
            $ordem = "order by o58_codigo, gestao, descricao";
        }
    }
}
...
for ($i = 0; $i < pg_num_rows($result); $i++) {
    ...
    */
    ...
    if (trim(@$o58_codigo) != "") {
        ...
        $descricao = "RECURSO: ";
        $recurso = apresentacaoRecurso($apresentarRecurso, $o58_codigo, $anousu);
        if ($codigo != $o58_codigo) {
            if ($flag_nivel == true) {
                $pdf->cell(($col * $mult_nivel), $alt, '', 0, 0, "L", 0, '.');
            }

            $pdf->cell(10, $alt, "{$descricao} {$recurso}", 0, 1, "L", 0);
            $pdf->ln(3);
            $codigo = $o58_codigo;
            if ($flag_imp == false) {
                $flag_lib8 = true;
