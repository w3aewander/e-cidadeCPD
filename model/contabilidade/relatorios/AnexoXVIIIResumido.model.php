...
class AnexoXVIIIResumido extends RelatoriosLegaisBase {
   ...
   * @return stdClass
   */
  public function getDadosBalancoOrcamentario() {
  ...
  private function getValorReceitaCorrenteLiquida() {
    ...
    {
        $anexoIV = AnexoIV::getInstance($this->iAnoUsu, $this->oPeriodo->getCodigo());

        return $anexoIV->getDadosSimplificado();
    }
  ...
  private function getDadosRestosPagar() {
    ...
    {
        $anexoVIII = FactoryAnexoVIII::getInstance($this->iAnoUsu, $this->oPeriodo->getCodigo());
        $instituicao = InstituicaoRepository::getInstituicaoPrefeitura();
        $anexoVIII->setInstituicoes($instituicao->getCodigo());

        return $anexoVIII->getDadosSimplificado();
    }
