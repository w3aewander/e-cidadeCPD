...
class M20865EstornoPagamentoEmpenho extends Migration
{
    /**
     * Run the migrations.
     *
...
SQL
        ...
        DROP SEQUENCE IF EXISTS corempagemovestorno_k12_sequencial_seq cascade;
        CREATE SEQUENCE caixa.corempagemovestorno_k12_sequencial_seq
          INCREMENT 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1;

        CREATE TABLE IF NOT EXISTS caixa.corempagemovestorno (
       
           k12_sequencial integer NOT NULL DEFAULT nextval('caixa.corempagemovestorno_k12_sequencial_seq'),
           ...
           CONSTRAINT corempagemovestorno_sequ_pk PRIMARY KEY (k12_sequencial)
        );

        CREATE INDEX IF NOT EXISTS corempagemovestorno_codmov_in ON caixa.corempagemovestorno USING btree (k12_codmov);
        CREATE INDEX IF NOT EXISTS corempagemovestorno_id_data_autent_in
                  ON caixa.corempagemovestorno USING btree (k12_id, k12_data, k12_autent);

        ALTER TABLE ONLY caixa.corempagemovestorno
           ADD CONSTRAINT corempagemovestorno_codmov_fk FOREIGN KEY (k12_codmov)
           REFERENCES empenho.empagemov(e81_codmov) MATCH FULL DEFERRABLE;

        ALTER TABLE ONLY caixa.corempagemovestorno
           ADD CONSTRAINT corempagemovestorno_id_data_autent_fk FOREIGN KEY (k12_id, k12_data, k12_autent)
           REFERENCES caixa.corrente(k12_id, k12_data, k12_autent) MATCH FULL DEFERRABLE;

        COMMENT ON TABLE caixa.corempagemovestorno
           IS '{"k12_sequencial": "Tabela que liga uma autenticacao (corrente) de estorno a um movimento da agenda de pagamentos (empagemov)",
                "sigla": "k12",
                "dataincl": "2022-05-20",
                "rotulo": "corempagemovestorno",
                "tipotabela": "0",
                "naolibclass": "false",
                "naolibfunc": "false",
                "naolibprog": "false",
        ...
        COMMENT ON COLUMN caixa.corempagemovestorno.k12_sequencial
        ...
        COMMENT ON COLUMN caixa.corempagemovestorno.k12_id
        ...
        COMMENT ON COLUMN caixa.corempagemovestorno.k12_data
        ...
        COMMENT ON COLUMN caixa.corempagemovestorno.k12_autent
        ...
        COMMENT ON COLUMN caixa.corempagemovestorno.k12_codmov
        ...
        SELECT fc_gera_dicionario_apartir_tabela('caixa', 'corempagemovestorno');
...
SQL
    ...
    public function down()
    {
        ...
        SELECT fc_remove_dicionario_tabela('caixa', 'corempagemovestorno');
        DROP TABLE IF EXISTS caixa.corempagemovestorno;
...
begin
   ...
   if  iCodCorGrupo > 0 then
    ...
    if codmov is not null and codmov > 0 and valor > 0 then
      ...
      select k12_codmov 
        into codmovimento
        from coremp
             inner join corempagemov on corempagemov.k12_id = coremp.k12_id 
                                    and corempagemov.k12_data   = coremp.k12_data 
                                    and corempagemov.k12_autent = coremp.k12_autent
       where coremp.k12_codord = codord;
   ...
   */
   insert 
     into corautent (k12_id, k12_data, k12_autent, k12_codautent)
   values (idterm,dtemite,codaut, sAutenticacao);
         ...
         if nValorRetencao = abs(valor) then
           ...
           from coremp
                inner join corempagemov     on corempagemov.k12_id     = coremp.k12_id 
                                           and corempagemov.k12_data   = coremp.k12_data 
                                           and corempagemov.k12_autent = coremp.k12_autent
                inner join corgrupocorrente on corempagemov.k12_id     = k105_id 
                                           and corempagemov.k12_data   = k105_data 
                                           and corempagemov.k12_autent = k105_autent
          where coremp.k12_codord = codord
            and corempagemov.k12_codmov = codmov
