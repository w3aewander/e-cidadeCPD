...
class cl_acordo {
   ...
   function sql_query ( $ac16_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){
     ...
     $sql .= "      left join contratospncp on ac16_sequencial = pn04_acordo";
     $sql2 = "";
     if($dbwhere==""){
       if($ac16_sequencial!=null ){
  ...
  public function sql_query_troca_fornecedor($sCampos = "*", $sWhere = null, $sOrderBy = null, $sLimit = null) {
   ...
   function sql_query_completo_posicao( $ac16_sequencial=null,$campos="*",$ordem=null,$dbwhere="",$flagBuscaLicitacao=false){
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = explode("#",$campos);
       $virgula = "";
  ...
  public function sqlBuscaEventosAcordoPorTipo($idAcordo, $tipoEvento)
  {
    $sql = "
      SELECT
        acordoevento.*
      FROM
        acordo
      INNER JOIN acordoevento
        ON ac55_acordo = ac16_sequencial
      WHERE
        ac16_sequencial = {$idAcordo}
        AND ac55_tipoevento = {$tipoEvento}
    ";
