...
class M26160AjusteFcFichacompensacaoFatorvenc extends Migration
{
    /**
     * Run the migrations.
     *
...
create or replace function fc_fichacompensacao(integer, integer, integer, date, float8) returns tp_fichacompensacao 
LANGUAGE 'plpgsql'
AS
\$function$
declare
...
begin
  ...
  -- Formatacoes
  sFatorVencAux := dVencimento - dDataBase;

  /* Caso passe de 9999 precisa dar reset para 1000 cfe manual */
  if sFatorVencAux > 9999 then
     sFatorVencAux = sFatorVencAux - 9000;
  end if;

  sFatorVenc = trim(to_char(sFatorVencAux, '0000'));
...
SQL
    ...
    public function down()
    {
        ...
        DB::connection()->getPdo()->exec(<<<SQLDOWN
drop function fc_fichacompensacao(integer, integer, integer, date, float8);
