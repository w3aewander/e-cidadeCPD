...
$sWhereAlunos = " ed57_i_codigo = {$oGet->iTurma} and ed221_i_serie in ({$oGet->iEtapa}) ";
...
if($iTotalLinhas > 0) {
  for($iIndiceAlunos = 0; $iIndiceAlunos < $iTotalLinhas; $iIndiceAlunos++) {
    ...
    $oPdf->oDadosRelatorio->etapa = $oRetornoAlunos->ed11_c_descr;
    ...
    $oDadosAlunos->etapa              = $oRetornoAlunos->ed11_c_descr;
...
for($iIndiceTurma = 0; $iIndiceTurma < count($aDisciplinas); $iIndiceTurma++) {
  ...
  $sWhereTurma   = "ed57_i_codigo = {$oGet->iTurma} AND regencia.ed59_i_codigo  = {$aDisciplinas[$iIndiceTurma]}";
  ...
  if($oDaoTurma->numrows > 0) {
    ...
    $regencia = RegenciaRepository::getRegenciaByCodigo($oRetornoTurma->ed59_i_codigo);
    $professores = $regencia->getDocentes();
    
    if (count($professores) > 0) {
      foreach ($professores as $prof) {
        $nome        = $prof->getNome();
        $nome_social = $prof->getNomeSocial();

        $oDadosTurma->professor = $nome;
        $oDadosTurma->professorSocial = $nome_social;
      }
    }
 ...
 */
$oPdf->oCalculosRelatorio->iLarguraColunaAluno = 88;
...
for($iIndiceDisciplinas = 0; $iIndiceDisciplinas < count($oPdf->oDadosRelatorio->aDadosDisciplinas); $iIndiceDisciplinas++) {
  ...
  for($iIndiceAvaliacao = 0; $iIndiceAvaliacao < $oPdf->oCalculosRelatorio->iQuantidadeRegistrosPorPagina; $iIndiceAvaliacao++) {
    ...
    //valor da coluna etapa
    $oPdf->setfont('times', '', 8);
    $oPdf->Cell(14, $iAlturaLinha,  "$iEtapa",    1, 0, "C", 0);
...
function headerRelatorio($oPdf, $iContador) {
  ...
  $oDados = $oPdf->oDadosRelatorio->aDadosTurma[$iContador];
    ...
    $oPdf->Cell(80, $oPdf->oDadosRelatorio->iAlturaLinha,  "Disciplina (Área): {$sDisciplina}",  0, 0, "L", 0);
    $oPdf->Cell(70, $oPdf->oDadosRelatorio->iAlturaLinha,  mb_strimwidth("Professor: {$oDados->professor} ", 0, 72), 0, 1, "L", 0);
    if(isset($oDados->professorSocial) && !empty($oDados->professorSocial)){
      "\n";
      $oPdf->setX(90);
      $oPdf->Cell(70, $oPdf->oDadosRelatorio->iAlturaLinha,  mb_strimwidth("Nome Social: {$oDados->professorSocial} ", 0, 70), 0, 1, "L", 0);
    }
  ...
  $oPdf->Cell(14, $oPdf->oDadosRelatorio->iAlturaLinha*2,  "ETAPA",     1, 0, "C", 0);
  ...
  /**
    ...
    */
    ...
    $oPdf->Cell(14,$oPdf->oDadosRelatorio->iAlturaLinha,'', 0, 0, "C", 0);
...
/**
 * Buscar o professor regente da disciplina 
 * pelo codigo da regencia
 * @param int
 * @return string
 */
function getProfessorByRegencia($codigoregencia = 0)
{ 
  
  if (empty($codigoregencia)) {
    return false;
  }
 
  $query = <<<SQL
select
    distinct z01_nome, z04_nomesocial
from
    regenciahorario
join rechumano on
    ed20_i_codigo = ed58_i_rechumano
join rechumanopessoal on
    ed284_i_rechumano = ed20_i_codigo
join rhpessoal on
    rh01_regist = ed284_i_rhpessoal
join cgm on
    rh01_numcgm = z01_numcgm
join cgmfisico on 
    cgmfisico.z04_numcgm = cgm.z01_numcgm
where
    ed58_i_regencia = {$codigoregencia}
SQL;
  
  $result = db_query($query);
 
  if ($result && pg_num_rows($result) > 0) {
    return db_utils::fieldsMemory($result, 0)->z01_nome;
  }
