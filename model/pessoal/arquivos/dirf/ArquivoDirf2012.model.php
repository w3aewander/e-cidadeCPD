 ...
 *  @version $Id: ArquivoDirf2012.model.php,v 1.17 2019/02/27 18:59:05 dbrenan.silva Exp $
 */
class ArquivoDirf2012
{
   ...
   */
    public function escreverLinhaRRA()
    {
        ...
        $oPessoaFisica->valor_pago_advogado               = '';
        $oPessoaFisica->pipe                              = '';
   ...
   */
     ...
     * @param $sNome
     * @param $sCpf
     * @param null $lPossuiPensionistas
     * @return void
     */
   ...
   */
    public function escreverLinhaValoresMensais($sIdentificador, $aValores)
    {
        ...
        $aDados['janeiro']               = self::converterValor($aValores[1], $iTamanho, $sIdentificador);
        $aDados['fevereiro']             = self::converterValor($aValores[2], $iTamanho, $sIdentificador);
        $aDados['marco']                 = self::converterValor($aValores[3], $iTamanho, $sIdentificador);
        $aDados['abril']                 = self::converterValor($aValores[4], $iTamanho, $sIdentificador);
        $aDados['maio']                  = self::converterValor($aValores[5], $iTamanho, $sIdentificador);
        $aDados['junho']                 = self::converterValor($aValores[6], $iTamanho, $sIdentificador);
        $aDados['julho']                 = self::converterValor($aValores[7], $iTamanho, $sIdentificador);
        $aDados['agosto']                = self::converterValor($aValores[8], $iTamanho, $sIdentificador);
        $aDados['setembro']              = self::converterValor($aValores[9], $iTamanho, $sIdentificador);
        $aDados['outubro']               = self::converterValor($aValores[10], $iTamanho, $sIdentificador);
        $aDados['novembro']              = self::converterValor($aValores[11], $iTamanho, $sIdentificador);
        $aDados['dezembro']              = self::converterValor($aValores[12], $iTamanho, $sIdentificador);
        if ($sIdentificador != "QTMESES") {
            $aDados['decimo_terceiro']       = self::converterValor($aValores[13]);
        }
        $aDados['Pipe']                  = "";

        $this->oLayout->setByLineOfDBUtils((object)$aDados, 3, $sIdentificador);
    }
   ...
   *  @return String
   */
    protected static function converterValor($nValor, $iTamanho = 8, $identificador = null)
    {
        ...
        if ($identificador == 'QTMESES') {
            $nValor = db_formatar($nValor, 'f', ' ', $iTamanho, 'e', 1);
        } else {
