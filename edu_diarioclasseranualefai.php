...
function voltaFaltas($matricula){
  ...
  $sql2 = pg_query("select ed95_i_codigo, ed59_i_codigo from diario inner join aluno on ed47_i_codigo = ed95_i_aluno inner join matricula on ed60_i_aluno = ed47_i_codigo inner join matriculaserie on ed60_i_codigo = ed221_i_matricula inner join regencia on ed59_i_codigo = ed95_i_regencia and ed59_i_serie = ed221_i_serie where ed60_i_codigo = {$matricula} and ed95_i_regencia = ed59_i_codigo and ed95_i_serie = {$serie} and ed59_i_turma = {$turma} order by ed95_i_codigo");
  $r2 = pg_fetch_all($sql2);
  $grade = array();

  foreach ($r2 as $valor) {
    $sql3 = pg_query("select ed72_i_numfaltas, ed72_c_aprovmin from diarioavaliacao inner join procavaliacao on ed41_i_codigo = ed72_i_procavaliacao left join pareceraval on ed93_i_diarioavaliacao = ed72_i_codigo left join abonofalta on ed80_i_diarioavaliacao = ed72_i_codigo where ed72_i_diario = {$valor["ed95_i_codigo"]} ORDER BY ed72_i_procavaliacao");
    $res = pg_fetch_all($sql3);
    
    $grade[$valor["ed59_i_codigo"]][0] = ($res[0]["ed72_i_numfaltas"]) ? $res[0]["ed72_i_numfaltas"] : "0";
    $grade[$valor["ed59_i_codigo"]][1] = ($res[1]["ed72_i_numfaltas"]) ? $res[1]["ed72_i_numfaltas"] : "0";
    $grade[$valor["ed59_i_codigo"]][2] = ($res[2]["ed72_i_numfaltas"]) ? $res[2]["ed72_i_numfaltas"] : "0";
    $grade[$valor["ed59_i_codigo"]]["aprovado"] = $res[2]["ed72_c_aprovmin"];
  }
...
//$matx = 26;
//$tudox = voltaFaltas($matx);
//testa($tudox);
//die("Busca faltas e aprovação");
...
foreach ($aTurmasSelecionadas as $oTurmaSelecionada) {
  ...
  foreach ($oTurma->getDisciplinasPorEtapa($oEtapa) as $oRegencia) {
    //testa($oRegencia->getDisciplina()->getNomeDisciplina()); die("Confere");
    
    array_push($xdisciplinas, $oRegencia->getDisciplina()->getNomeDisciplina());
    //echo $oRegencia->getDisciplina()->getNomeDisciplina(); echo "<br>";
    $iContDisciplinas ++;    
  ...
  $oTurmaEtapa->xdisciplinas = $xdisciplinas;
...
foreach ($aTurmas as $oTurmaEtapa) {
  ...
  for ($iPagina = 0; $iPagina < $iPaginas; $iPagina ++) {
    ...
    foreach ($oTurmaEtapa->aAlunos as $oAluno) {      
      ...
      $oPdf->Cell(50, $oConfigRelatorio->iAlturaLinha, $oAluno->sNome, 1);

      //testa($oAluno->aAvaliacao[0]); die("Faltas e aprovação");
      //testa($xfaltas); die("a");
      //foreach ($oTurmaEtapa->xdisciplinas as $disciplina) {
      $totalfaltas1 = 0;
      $totalfaltas2 = 0;
      $totalfaltas3 = 0;
      $rfinal = "APROVADO";
      foreach ($oAluno->aAvaliacao[0] as $disciplina) {        
        $codigodisciplina = $disciplina->iRegencia;        
        $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, $xfaltas[$codigodisciplina][0], 1, 0, 'C');
        $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, $xfaltas[$codigodisciplina][1], 1, 0, 'C');
        $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, $xfaltas[$codigodisciplina][2], 1, 0, 'C');
        
        $cfreq1 = (($diasletivos - ($xfaltas[$codigodisciplina][0] + $xfaltas[$codigodisciplina][1] + $xfaltas[$codigodisciplina][2])) / $diasletivos) * 100;      
        $cfreq1 = (int)$cfreq1;
        //$cfreq1 = ($cfreq1) ? $cfreq1 : "0";

        $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, $cfreq1, 1, 0, 'C');  
        //$xfaltas[$codigodisciplina]["aprovado"] -- resultado final "S"
        $totalfaltas1 += $xfaltas[$codigodisciplina][0];
        $totalfaltas2 += $xfaltas[$codigodisciplina][1];
        $totalfaltas3 += $xfaltas[$codigodisciplina][2];
        if($xfaltas[$codigodisciplina]["aprovado"] == "N"){
          $rfinal = "REPROVADO";
        }
      }
      $totalfaltas1 = ($totalfaltas1) ? $totalfaltas1 : "0";
      $totalfaltas2 = ($totalfaltas2) ? $totalfaltas2 : "0";
      $totalfaltas3 = ($totalfaltas3) ? $totalfaltas3 : "0";

      $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, $totalfaltas1, 1, 0, 'C');
      $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, $totalfaltas2, 1, 0, 'C');
      $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, $totalfaltas3, 1, 0, 'C');
      $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, ($totalfaltas1 + $totalfaltas2 + $totalfaltas3), 1, 0, 'C');  
      //testa($oTurmaEtapa->xdisciplinas);
      ...
      $cfreq = (($diasletivos - ($totalfaltas1 + $totalfaltas2 + $totalfaltas3)) / $diasletivos) * 100;      
      $cfreq = (int)$cfreq;
      $oPdf->Cell(20, $oConfigRelatorio->iAlturaLinha, $cfreq, 1, 0, 'C');
      //$cfreq = number_format($cfreq, 0, ",", ",");
      //$cfreq = round($cfreq);

      $oPdf->Cell(25, $oConfigRelatorio->iAlturaLinha, $rfinal, 1, 1, 'C');
...
function adicionaHeader(FPDF $oPdf, $oTurmaEtapa, $oConfigRelatorio, $iPagina) {
  ...
  $materias = $oTurmaEtapa->xdisciplinas;
  $oPdf->AddPage();
  ...
  $oPdf->Cell(55, $oConfigRelatorio->iAlturaLinha, '', 0, 0, "C");
  foreach ($materias as $disciplina){
    $oPdf->Cell(30, $oConfigRelatorio->iAlturaLinha, $disciplina, 1, 0, "C");
  }
  $oPdf->Cell(30, $oConfigRelatorio->iAlturaLinha, "TOTAL DE FALTAS", 1, 0, "C");
  ...
  $oPdf->Cell(55, $oConfigRelatorio->iAlturaLinha, '', 0, 0, "C");
  foreach ($materias as $disciplina){
    $oPdf->Cell(30, $oConfigRelatorio->iAlturaLinha, 'TRIMESTRE', 1, 0, "C");
  }
  $oPdf->Cell(30, $oConfigRelatorio->iAlturaLinha, 'TRIMESTRE', 1, 0, "C");
  //$oPdf->Cell(20, $oConfigRelatorio->iAlturaLinha, "FREQUÊNCIA", 1, 0, "C");
  //$oPdf->Cell(25, $oConfigRelatorio->iAlturaLinha, "RESULTADO FINAL", 1, 0, "C");
  ...
  $oPdf->Cell(50, $oConfigRelatorio->iAlturaLinha, 'Nome do Aluno', 1);

  foreach ($materias as $disciplina){
    $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '1º', 1, 0, "C");
    $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '2º', 1, 0, "C");
    $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '3º', 1, 0, "C");
    $oPdf->SetFont("arial", 'b', 6);
    $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, 'MÉDIA', 1, 0, "C");
    $oPdf->SetFont("arial", 'b', 7);
  }
  $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '1º', 1, 0, "C");
  $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '2º', 1, 0, "C");
  $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '3º', 1, 0, "C");
  $oPdf->SetFont("arial", 'b', 6);
  $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, 'TOTAL', 1, 0, "C");
  $oPdf->SetFont("arial", 'b', 7);
...
function imprimeLinhaEmBranco(FPDF $oPdf, $oConfigRelatorio, $oTurmaEtapa) {
  ...
  $oPdf->Cell(50, $oConfigRelatorio->iAlturaLinha, '', 1);

  foreach ($materias as $disciplina){
    $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '', 1, 0, "C");
    $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '', 1, 0, "C");
    $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '', 1, 0, "C");    
    $oPdf->Cell(7.5, $oConfigRelatorio->iAlturaLinha, '', 1, 0, "C");
  }
  ...
  $oPdf->Cell(25, $oConfigRelatorio->iAlturaLinha, '', 1, 1, "C");
