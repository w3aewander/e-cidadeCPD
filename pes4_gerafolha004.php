...
function gerfadi($opcao_geral=null,$opcao_tipo=1){
    ...
    for ($Ipessoal = 0; $Ipessoal < $iTotalLinhasPessoal; $Ipessoal++) {
        ...
        echo "<script>
             oProgressBar.setTotal({$iTotalLinhasPessoal}).setLinha({$Ipessoal}).calculaPercentual();
          </script>";
...
function gerffer($opcao_geral=null,$opcao_tipo=1){
    ...
    for ($Ipessoal =0; $Ipessoal < $iTotalPessoalRescisao; $Ipessoal++) {
        ...
        echo "<script>
             oProgressBar.setTotal({$iTotalPessoalRescisao}).setLinha({$Ipessoal}).calculaPercentual();
          </script>";
        ...
        // ver se pagamento e de diferença ou não, se já pago e qual o mês
...
function gerfres($opcao_geral=null,$opcao_tipo=1){
    ...
    global $F001, $F002, $F004, $F005, $F006,
           ...
           $F012, $F013, $F014, $F015, $F016,
           $F017, $F018, $F019, $F020, $F021,
           $F022, $F023, $F006_clt, $F024, $F003, $F025, $F026, $F027, $F028, $F030, $F031, $F032, $F033, $F050;
...
function gerfs13($opcao_geral=null,$opcao_tipo=1) {
    ...
    global $campos_pessoal, $r110_regisi, $subpes,$chamada_geral,$chamada_geral_arquivo,$pessoal,$Ipessoal,$transacao,$cfpess;
    global $situacao_funcionario,$naoencontroupontosalario,$db21_codcli,$carregarubricas,$diversos,$Iinssirf,$inssirf,$subpes,$cadferia;
    global $dias_pagamento,$mes,$ano,$dias_do_mes,$vlr_sal_saude_ou_acidente,$valor_salario_maternidade,$valor_licenca_premio;
    global $rubricas,$prev_desc,$func_em_ferias,$ajusta,$sigla_ajuste,$inssirf_base_ferias;
...
function gerfsal( $opcao_geral=null, $opcao_tipo = TIPO_CALCULO_PARCIAL ) {
    ...
    // globais de outras funcoes
    global $rubrica_maternidade, $rubrica_licenca_saude, $rubrica_acidente, $rubrica_licenca_premio;
    global $anousu, $mesusu, $DB_instit, $db_debug;
    ...
    global $F001, $F002, $F004, $F005, $F006, $F007, $F008, $F009, $F010, $F011, $F012, $F013, $F014, $F015, $F016, $F017, $F018, $F019,
           $F020, $F021, $F022, $F023, $F006_clt, $F024, $F003, $F025, $F026, $F027, $F028, $F030, $F031, $F032, $F033, $F050;
    global $quais_diversos;
    eval($quais_diversos);
    global $opcao_filtro,$opcao_gml,$r110_regisf,$r110_lotaci, $r110_lotacf,$faixa_regis,$faixa_lotac;
    global $dtot_vpass,$dperc_pass,$dquant_pass,$quant_pass,$contador_registros;
    ...
    for ($Ipessoal = 0; $Ipessoal < $iTotalLinhasPessoal; $Ipessoal++) {
        ...
        $rubrica_licenca_premio          = trim($inssirf_[0]["r33_rublicencapremio"]);
        $inssirf_base_ferias   = "B002";
        if (!db_empty($inssirf_[0]["r33_basfer"] )) {
            $inssirf_base_ferias = $inssirf_[0]["r33_basfer"];
        }
        ...
        for ($Iponto=0; $Iponto<$quant_pontofs; $Iponto++) {
            ...
            // Fim --> Rubricas que serao calculadas por ultimo em calculos especificos
        ...
        $aRubricaDeCadaTipo[Afastamento::LICENCA_PREMIO]                          = $rubrica_licenca_premio;
        ...
        if (isset($SituacoesFuncionario)) {
            ...
            if (!DBPessoal::verificarUtilizacaoEstruturaSuplementar()) {
                if (!empty($aAfastamentosServidor)) {
                    $rubricasPermitidas = [];
                    ...
                    foreach ($aAfastamentosServidor as $oAfastamento) {
                        ...
                        $situacao = $oAfastamento->r45_situac;

                        if (!empty($controleAfastamento[$DB_instit][$tabela][$situacao])) {
                            foreach ($controleAfastamento[$DB_instit][$tabela][$situacao] as $afastamentoPermitido) {
                                $rubricasPermitidas[] = $afastamentoPermitido->getRubrica()->getCodigo();
                            }
                        }
                    }

                    // Elimina duplicados
                    $rubricasPermitidas = array_unique($rubricasPermitidas);

                    // Filtra o array $aValoresFolhaSalario
                    $aValoresFolhaSalario = array_filter($aValoresFolhaSalario, function ($linha) use ($rubricasPermitidas) {
                        return in_array($linha['r53_rubric'], $rubricasPermitidas);
                    });
                }
            }
                ...
                if ($iTotalLinhasFolhaSalario > 0) {
                    for ($Itransacao = 0; $Itransacao < $iTotalLinhasFolhaSalario; $Itransacao++) {
                        ...
                        if (substr("#" . $aValoresFolhaSalario[$Itransacao]["r53_rubric"], 1, 1) != "R") {
                             ...
                             */
                            if (
                                in_array($oAfastamento->r45_situac, $aAfastamentosLicencaSaude) && !db_empty($rubrica_licenca_saude)
                            ...
                            if ((($oAfastamento->r45_situac == Afastamento::LICENCA_PREMIO)) && !db_empty($rubrica_licenca_premio)) {

                                if ($aValoresFolhaSalario[$Itransacao]["rh27_pd"] == 2) {
                                    if ($proporcionalizaRubricaControleAfastamento) {
                                        $valor_licenca_premio -= $nValorProporcionalAfastamentoSalario;
                                        $nValorAfastamento    -= $nValorProporcionalAfastamentoSalario;
                                    }
                                } else {
                                    if ($proporcionalizaRubricaControleAfastamento) {
                            ...
                            if ($dias_pagamento - $iDiasFerias == 0) {
                                ...
                                if ((30 - (30 - $dias_pagamento) - $iDiasFerias) > 0) {
                                    ...
                                    if (isSapiranga()) {
                                        ...
                                        $rubricasComCalculoEspecial = ['0001', '0002', '0004', '0005', '0006', '0008'];

                                        if (in_array($sRubrica, $rubricasComCalculoEspecial)) {
                                            $iDiasTrabalhados = ($nQuantidadeRubrica - $iDiasFerias - $iDiasDoAfastamento);
                                            $valor = abs(($aValoresFolhaSalario[$Itransacao]["r53_valor"] / 30) * $iDiasTrabalhados);
                                        } else {
                                    ...
                                    if (!db_empty($nQuantidadeRubrica) && ('t' == $aValoresFolhaSalario[$Itransacao]["rh27_propq"])) {
                                        ...
                                        if (isSapiranga()) {
                                            $quantidade = abs($nQuantidadeRubrica - $iDiasFerias - $iDiasDoAfastamento);
                                        } else {
        ...
        if ($vlr_sal_saude_ou_acidente > 0) {
          ...
          ";
            ...
            if (!db_query($sqlDeleteGerfsal)) {
                throw new Exception('Não foi possível excluir o salário.');
            }
          ...
          ";
            ...
            if (!db_query($sqlInsertGerfsal)) {
                throw new Exception('Não foi possível inserir o salário.');
            }
        }
    }
...
function carrega_fxxx ($codigo, $carrega, $cfuncao=null ) {
    ...
    RH30_NATUREZAREGIME as r01_natreg,";
...
RH01_CLAS2 AS r01_clas2, ";
  ...
  ".bb_condicaosubpes("rh02_" ).$condicaoaux );
    ...
    $F050 = DBPessoal::getVariaveisCalculo($oServidor, 'f050');
...
function carrega_r9xx($area, $sigla, $sigla2, $nro_do_registro,$opcao_tipo) {
    ...
    global $anousu, $mesusu, $DB_instit,$n,$vlr_sal_saude_ou_acidente,$valor_salario_maternidade,$valor_licenca_premio,$SituacoesFuncionario;
    ...
    if ($area != "pontof13" && $area != "pontocom") {
        ...
        if ( ( $valor_base_sal_familia > 0 && $gerar_salario_familia ) ) {
            ...
            $aMatriculasServidor = "";
            $sqlSalario = "
          SELECT coalesce(SUM(valor_provento - valor_desconto), 0) AS valor
          FROM
              (SELECT CASE
                          WHEN r14_pd = 1 THEN coalesce(sum(r14_valor), 0)
                          ELSE 0
                      END AS valor_provento,
                      CASE
                          WHEN r14_pd = 2 THEN coalesce(sum(r14_valor), 0)
                          ELSE 0
          ...
          FROM
              (SELECT CASE
                          WHEN r48_pd = 1 THEN coalesce(sum(r48_valor), 0)
                          ELSE 0
                      END AS valor_provento,
                      CASE
                          WHEN r48_pd = 2 THEN coalesce(sum(r48_valor), 0)
                          ELSE 0
                      END AS valor_desconto
               FROM gerfcom
               WHERE r48_rubric IN
    ...
    if ($db_debug == true) {
        echo "[carrega_r9xx] 22 - chamando a função calc_rubrica() <br>";
    }
    ...
    if ((($pessoal[$Ipessoal]["r01_regime"] == 2) || ($pessoal[$Ipessoal]["r01_regime"] == 4))
        ...
        if ($db_debug == true) {
            echo "[carrega_r9xx] 57 - r14_valor = $r14_valor  <br>";
        }
        $r20_rubr = "R991";
        if ($dias_pagamento > 0  && (servidorPossuiSituacao($pessoal[$Ipessoal]["r01_regist"], Afastamento::AFASTADO_LICENCA_GESTANTE) ||
            ...
            if ($db_debug == true) {
                echo "[carrega_r9xx] 58 - r14_valor = $r14_valor  <br>";
            }
        }

        if ($db_debug == true) {
            echo "[carrega_r9xx] 12 - Chamando a função grava_gerf() <br>";
        }
        ...
        // R961 BASE F.G.T.S. Menor Aprendiz
        if (($pessoal[$Ipessoal]["r01_natreg"] == 7)
        ) {

            $r07_valor = calc_rubrica("R961", $area, $sigla, $sigla2, $nro_do_registro, true);
            $r14_valor = round($r07_valor, 2);
                    ...
                    for ($ix = 0; $ix <$iTotalArq_; $ix++) {
                        ...
                        if ($achou && $campo_rubrica != $rubrica) {
                            ...
                            if ($base == "B004" && ($area0 == "pontofs" || $area0 == "pontocom")) {
                                if (( isset($cadferia) && strtolower($cfpess[0]["r11_fersal"]) == "f" && ('t' == $cadferia[0]["r30_paga13"]) && $cadferia[0][$r30_proc] ==  $mes_gozo )) {
...
function le_tbprev($r20_rubr=null, $area=null, $sigla=null, $sigla2=null, $nro_do_registro=null, $operacao=null) {
    ...
    global $rubrica_maternidade, $rubrica_licenca_saude, $rubrica_acidente, $rubrica_licenca_premio, $db_debug;
    ...
    global $naoencontroupontosalario,$dias_pagamento,$valor_salario_maternidade,$vlr_sal_saude_ou_acidente,$valor_licenca_premio;
    ...
    for ($n=1; $n < 4; $n++) {
        ...
        if ($n==1) { // <-- R985 - BASE PREVIDENCIA
            ...
            if ( (in_array(11, $SituacoesFuncionario)) && !db_empty($rubrica_licenca_premio) ) {

                if ($dias_pagamento > 0 ) {

                    $r07_valor += $valor_licenca_premio;
                    if($db_debug == true) { echo "[le_tbprev] Acrescenta o Valor da Licenca Premio a Base da Previdencia --> $r07_valor += $valor_licenca_premio <br>"; }
        ...
        global $gerfcom_;
        if (($opcao_geral == 1 && !empty(trim($cfpess[0]["r11_pagarferias"])) && !$naoencontroupontosalario) ||
            $opcao_geral == 4) {
            // Salario ou Rescisao
            if ($n == 1) {
        ...
        if ($r07_valor + $base_folha_complementar > 0) {
            if ($n == 2 ) {
                ...
                if ($db_debug == true) { echo "[le_tbprev] 2 - Valor do Desconto Previdenciario --> $r07_valor <br>"; }
                ...
                LogCalculoFolha::write("Chamando função que calcula o valor da previdencia");
                $r07_valor = calc_tabprev($r07_valor + $pessoal[$Ipessoal]["r01_basefo"] + $base_folha_complementar,db_str($pessoal[$Ipessoal]["r01_tbprev"]+2,1),$pessoal[$Ipessoal]["r01_tpcont"],$pessoal[$Ipessoal]["rh02_codreg"]);

                if ($db_debug == true) { echo "[le_tbprev] {$n} - Valor do Desconto Previdenciario --> $r07_valor <br>"; }
        ...
        // verifica o parametro da cfpess, só entra se estiver selecionado a opção diferente de Nenhum
        if (!empty(trim($cfpess[0]["r11_pagarferias"]))) {
        ...
        if(isNiteroi()){
            if (db_getsession('DB_instit') == 1) {
                if ($r20_rubr == 'R904' && $area == 'pontof13') {
                    continue;
                }
            }
        }
        if( $area == "pontofs" || $area == "pontocom" || $area == "pontofe" ){
            ...
            if ( $n == 1 ){
                ...
                if((   't' == $cadferia[0]["r30_paga13"]
                    ...
                    $condicaoaux = " and r31_rubric in ( 'R903','R906','R909','R912' ) and r31_tpp = 'F' and ";
                    $condicaoaux .= "r31_regist = ".db_sqlformat($pessoal[$Ipessoal]["r01_regist"] );
                    $sqlfer = "select r31_anousu,r31_mesusu,r31_regist,r31_rubric,r31_valor,r31_quant,r31_lotac ";
                    $sqlfer .= "from gerffer " . bb_condicaosubpes("R31_" ).$condicaoaux;
...
function grava_base_prev($area_grava) {
    ...
    global $vlr_sal_saude_ou_acidente,$valor_licenca_premio,$pessoal,$Ipessoal;
    global $teto_prev;
    ...
    if ( (in_array(11, $SituacoesFuncionario)) && !db_empty($rubrica_licenca_premio) ) {
        if ($dias_pagamento > 0) {
            $r14_valor += $valor_licenca_premio;
            if ($db_debug == true) { echo "[grava_base_prev] 82 - licenca premio - r14_valor = $r14_valor  <br>"; }
        }
    }
...
function gravb_base_irf ($area_grava,$r20_rubrp){
    global $r20_rubr, $cfpess, $subpes, $naoencontroupontosalario, $base_folha_complementar, $r14_valor,
           $db_debug, $base_irfb, $situacao_funcionario, $r110_lotac, $r14_quant, $dias_pagamento,
           $rubrica_maternidade, $valor_salario_maternidade, $vlr_sal_saude_ou_acidente, $rubrica_acidente,
           $rubrica_licenca_saude, $rubrica_licenca_premio, $valor_licenca_premio;
    ...
    if( $r20_rubr  == "R981" || $r20_rubr  == "R982" || $r20_rubr  == "R983"){
        ...
        if ($r20_rubr == "R981") {
            if (
                in_array(Afastamento::AFASTADO_LICENCA_GESTANTE, $aSituacoesFuncionario) ||
                ...
                if (!db_empty($rubrica_maternidade)) {
                    if ($dias_pagamento > 0) {
                        $r14_valor += $valor_salario_maternidade;
                        if ($db_debug == true) {
                            echo "[gravb_base_irf] 90 - r14_valor = $r14_valor  <br>";
                        }
                    }
                }
            }
        ...
        if ($r20_rubr == "R981") {
            if (
                in_array(Afastamento::LICENCA_PREMIO, $aSituacoesFuncionario)
            ) {
                if (!db_empty($rubrica_licenca_premio)) {
                    if ($dias_pagamento > 0) {
                        $r14_valor += $valor_licenca_premio;
                        if ($db_debug == true) {
                            echo "[gravb_base_irf] 90 - licenca premio - r14_valor = $r14_valor  <br>";
                        }
                    }
                }
            }
...
function grava_gerf($area_grava,$grava_tpp=" ") {
    ...
    global $anousu, $mesusu, $DB_instit,$subpes, $db_debug, $pontofs, $codreg;
    ...
    $iMatricula = $pessoal[$Ipessoal]["r01_regist"];
    $codreg = $pessoal[$Ipessoal]["rh02_codreg"];
    ...
    if ($r14_valor > 0.01 || $lPermiteInclusaoZerada ) {
        ...
        if ($area_grava == "pontofs") {
            if ((DBPessoal::verificarUtilizacaoEstruturaSuplementar())) {
                if (!$proporcionalizaRubricaControleAfastamento && $dias_pagamento === 0) {
                    if (
                        ($r20_rubr == "R991" || $r20_rubr == "R985" || $r20_rubr == "R992" || $r20_rubr == "R993" || $r20_rubr == "R901") &&
                        (
                            servidorPossuiSituacao($iMatricula, Afastamento::AFASTADO_DOENCA_MAIS_15_DIAS) ||
                            servidorPossuiSituacao($iMatricula, Afastamento::AFASTADO_DOENCA_MAIS_30_DIAS)
                        ) && $pessoal[$Ipessoal]["r01_regime"] == 2
                    ) {
                        $condicaoaux = " and r14_regist = " . db_sqlformat($iMatricula) . " and r14_rubric = " . db_sqlformat($r20_rubr);
                        db_delete("gerfsal", bb_condicaosubpes("r14_") . $condicaoaux);

                        if ($db_debug == true) {
                            echo "[grava_gerf] Gravação cancelada por ser do regime CLT ele não pode ter os descontos das 'R' e existir um afastamento AFASTADO_DOENCA_MAIS_15_DIAS OU AFASTADO_DOENCA_MAIS_30_DIAS<br>";
                        }
                        return;
                    }
                }
            ...
            if (( (!db_empty($dias_pagamento) || $dias_pagamento > 0 ) || $r20_rubr == "R991" || $r20_rubr == "R985" )
                ...
                $matriz2[10] = $DB_instit;
                if(isNiteroi()){
                    if ($r20_rubr == 'R901' && $codreg == "9" && $DB_instit == '1') {
                        $matriz2[5] = "11";
                    }
                }
                if ($acao=="insere") {
...
// CALCULOS SOBRE O LÍQUIDO ( BRUTO - OBRIGATÓRIOS )
 ...
 */
function calc_tabprev ($base_inss=null,$codigo=null, $tpcont=null, $codreg=null)
{
    LogCalculoFolha::write();
    ...
    global $perc_inss,$r14_quant,$r20_quant,$r22_quant,$inssirf,$Iinssirf, $F010;
    $calculo     = 0;
    ...
    for( $Iinssirf = 0; $Iinssirf < $iTotalLinhas; $Iinssirf++ ) {
        ...
        if ($base_inss >= $oDadosPrevidencia->r33_inic && $base_inss <= $oDadosPrevidencia->r33_fim) {
            ...
            if(isNiteroi()){
                if($codreg == "9" && db_getsession("DB_instit") == "1") {
                    $valorBase = $base_inss;
                    $calculo = floor((($valorBase) / 100) * 11 * 100) / 100;
                }
            }
            LogCalculoFolha::write("Valor Calculado : $calculo");
            return $calculo;
    ...
    if ($base_inss > $oDadosPrevidencia->r33_fim && $oDadosPrevidencia->r33_codtab == $codigo) {
        ...
        if(isNiteroi()){
            if($codreg == "9" && db_getsession("DB_instit") == "1") {
                $valorBase = (round($base_inss, 2) > round($oDadosPrevidencia->r33_tinati, 2) ? $oDadosPrevidencia->r33_tinati : $base_inss);
                $calculo = floor((($valorBase) / 100) * 11 * 100) / 100;
            }
        }
    } else {
...
function calc_irf($r20_rubr_=null, $area=null, $sigla=null, $sigla2=null, $nro_do_registro=null, $operacao=null) {
    ...
    global $vlr_desc_prev_ferias_D,$diferencaDeducao;
    global $F001, $F002, $F004, $F005, $F006,
           $F007, $F008, $F009, $F010, $F011,
           $F012, $F013, $F014, $F015, $F016,
           $F017, $F018, $F019, $F020, $F021,
    ...
    if ($r20_rubr == "R915" && $opcao_geral == 3 && $cadferia[0][$r30_proc] < $subpes ) {
        if (('t' == $cadferia[0]["r30_paga13"])
            ...
            $base_irf       -= $nValorCalculado;
            $r07_valor      -= $nValorCalculado;
      ...
      ".bb_condicaosubpes("rh02_" ).$condicaoaux );
        ...
        if(count($transacao) < 2){
            ...
            if (db_selectmax("transacao", "select sum(r31_valor) as descontod from gerffer ".bb_condicaosubpes("r31_" ).$condicaoaux )) {
                 ...
                 *  A R983 - Base de irrf de ferias estava descontando a R903 - INSS S/ FÉRIAS
                 *  gerando valor da base incorreto no calculo
                 * Redmine - 24549
                 */
                LogCalculoFolha::write("Modificando valor(Desconto férias mes anterior(R979)(Codigo Comentado!)......: -{$transacao[0]["descontod"]}" );
            }
        }
    }
    ...
    if($db_debug == true) { echo "[calc_irf] --> Inicio do levantamento do Desconto da Previdencia Complementar e a Base da Previdencia Complementar <br>"; }

    $base_desconto_prev_complementar = 0  ;
    $base_folha_complementar = 0;
    // verifica o parametro da cfpess, só entra se estiver selecionado a opção diferente de Nenhum
    if (($opcao_geral == 1 && !empty(trim($cfpess[0]["r11_pagarferias"]))) || $opcao_geral == 4) {
        if ($r20_rubr == "R913" ) {
            // R913 % IRRF S/SALARIO
            $condicaoaux  = " and r48_regist = ".db_sqlformat($pessoal[$Ipessoal]["r01_regist"] );
        ...
        if ($r20_rubr == "R914" ) {
            ...
            if (db_selectmax("gerfcom_", "select * from gerfcom ".bb_condicaosubpes("r48_" ).$condicaoaux )) {
                ...
                // R905 % Funpas S/13o SALARIO
                // R908 % F Inativos S/13o SALARIO
                // R911 % Previdencia 4 S/13o SALARIO
                if (db_selectmax("gerfcom_", "select * from gerfcom ".bb_condicaosubpes("r48_" ).$condicaoaux )) {
                    $base_desconto_prev_complementar  = $gerfcom_[0]["r48_valor"];
    ...
    if ( $r20_rubr == "R913" || $r20_rubr == "R915" ) {
        ...
        if ($opcao_geral == 1) {
            ...
            if (db_selectmax("transacao", "select * from gerffer ".bb_condicaosubpes("r31_" ).$condicaoaux )) {
                ...
                LogCalculoFolha::write("Modificando valor (Pensões Alimentícias).................: -{$transacao[0]["r31_valor"]}");
            }
        }
        ...
        if ($r20_rubr == "R913") {
            if (
                ...
                if ($dias_pagamento > 0 && (!db_empty($rubrica_licenca_saude) || !db_empty($rubrica_acidente))) {
                    $base_irf += $vlr_sal_saude_ou_acidente;
                    $r07_valor += $vlr_sal_saude_ou_acidente;
                    LogCalculoFolha::write("Modificando valor (Afastamento saude/acidente)...........: +{$vlr_sal_saude_ou_acidente}");
                }
    ...
    if(isNiteroi()){
        ...
        if (isNiteroi() && $instituicao == 18) {
            $temExcecoes = $pessoal[$Ipessoal]['r01_tbprev'] == 3 && $pessoal[$Ipessoal]['r01_regime'] == 1;
        }
    }
    ...
    if ($r07_valor > 0.009) {
        ...
        if ($db_debug == true) { echo "[calc_irf] 98 - Valor do IRRF r14_valor = $r14_valor  <br>"; }
    ...
    for ($Iinssirf = 0; $Iinssirf < $iTotalLinhasIrrf; $Iinssirf++) {
        ...
        if( !db_empty($inssirf[$Iinssirf]["r33_fim"]) ) {
            if( $base_irf >= $inssirf[$Iinssirf]["r33_inic"] && $base_irf <= $inssirf[$Iinssirf]["r33_fim"]){
                $r14_quant        = (float)$inssirf[$Iinssirf]["r33_perc"];
                $r20_quant        = $r14_quant;
                $r22_quant        = $r14_quant;
                $inssirf_r33_perc = $r14_quant;
                $inssirf[$Iinssirf]["r33_deduzi"] = (float)$inssirf[$Iinssirf]["r33_deduzi"];

                $calculo          = ($base_irf/100)*($inssirf_r33_perc)-($inssirf[$Iinssirf]["r33_deduzi"]);
                LogCalculoFolha::write('Cálculo: ($base_irf/100)*($inssirf_r33_perc)-($inssirf[$Iinssirf]["r33_deduzi"])');
                LogCalculoFolha::write("Cálculo: ({$base_irf}/100)*({$inssirf_r33_perc})-({$inssirf[$Iinssirf]["r33_deduzi"]})");
...
/**
 * Verifica se o servidor possui a situação informada na competencia
 * @param $sSituacao
 * @param $iMatricula
 * @return bool
 */
...
/**
 * Função utilizada na fórmula da rubrica R932. Verifica se tem direito a receber 1/3 do abono, quando houverem dias
 * a serem abonados.
 * Fórmula: (PAGATERCO() > 0 ?  (((B030/(F019+F020+F023))*(F020))  / 3): 0)
 * @return int
...
function pagaTerco()
{
    ...
    if (!$rs) {
        throw new  BusinessException("Erro ao pesquisar férias");
    }
