...
for ($x = 0; $x < $clmatricula->numrows; $x++) {
  ...
  $ed47_v_nome = empty($ed47_v_nomesocial) || is_null($ed47_v_nomesocial) ? $ed47_v_nome : $ed47_v_nomesocial;
 ...
 if ($clregenteconselho->numrows > 0) {
  ...
  if ( $oLibDocumento->lErro ) {
    ...
    if(trim($sLogoEscola)!=""){
        $pdf->Image('imagens/'.$sLogoEscola,$iPosXLogoEscola,$pdf->GetY(),20);
    }
    $pdf->Text(33,$m0,$nome);
    ...
    $oGrade = GradeAproveitamentoFactory::get($matricula, $pdf, 280);
  if ($grade=="yes") {
  ...
  if ($grade == "yes" ) {
    ...
    if ($oGrade instanceof  RelatorioGradeAproveitamento) {
        ...
        $aPeriodosApresentados = $oGrade->getElementosApresentados();
    }
    ...
    if ( count($aPeriodosApresentados) > 5 ) {
      ...
      $pdf->multicell( $oDadosGrade->nLarguraGrade / 2, 4,"LOL" .$sPeriodos, 0, "L", 0, 0 );
    } else {
