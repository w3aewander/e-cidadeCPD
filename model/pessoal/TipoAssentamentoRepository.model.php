...
class TipoAssentamentoRepository
{
     ...
     * @throws Exception
     */
    public static function tiposPermitemDuplicata()
    {
        $oRepository = self::getInstance();

        $dao = new cl_tipoasse();
        $sql = $dao->sql_query_file(null, "h12_codigo", null, "h12_permiteduplicar = true");
        $rs = db_query($sql);
        ...
        while ($retorno = pg_fetch_object($rs)) {
            if (!isset(self::$aColecao[$retorno->h12_codigo])) {
                $tipo = $oRepository->make($retorno->h12_codigo);
                self::add($tipo);
                $tipos[] = $tipo;
            }
        }
