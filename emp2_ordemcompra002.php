...
for ($i = 0; $i < $num; $i++) {
    ...
    $campos = "distinct m52_codordem, m52_sequen, m52_quant, m52_numemp, m52_vlruni, m52_valor, pcmater.pc01_descrmater";
    $campos .= ", pc01_codmater, e62_descr, empempenho.e60_codemp, empempenho.e60_anousu, e62_vltot, e62_quant";
    $campos .= ", e54_conpag, e54_destin";
    $campos .= ", case when rp.pc81_codproc is not null then rp.pc81_codproc else  pcprocitem.pc81_codproc end as pc81_codproc";
    $campos .= ", case when solrp.pc11_numero is not null then solrp.pc11_numero else  solicitem.pc11_numero end as pc11_numero";
    $campos .= ", case when pc10_solicitacaotipo = 5 then coalesce(trim(pcitemvalrp.pc23_obs), '') else  coalesce(trim(pcorcamval.pc23_obs), '') end as pc23_obs";
    $campos .= ", pc50_descr, coalesce(matunid.m61_descr, matunidautorizacao.m61_descr) as unidade, o41_cnpj";
    $campos .= ", o41_descr";

    $ordem = "m52_numemp, m52_sequen";
    $where = "m52_codordem = $m51_codordem";
    $sqlItem = $clmatordemitem->sql_query_emiteordem(null, $campos, $ordem, $where, true);
    ...
    if ($numrows == 0) {
        ...
        $o41_cnpj = db_utils::fieldsMemory($resultitem, 0)->o41_cnpj;
        $o41_descr = db_utils::fieldsMemory($resultitem, 0)->o41_descr;
    }
