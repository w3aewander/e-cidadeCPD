...
use ECidade\Saude\Tfd\Service\IndiqueVeiculosService;
...
try {
    if (isset($confirmar)) {
        ...
        $service->salvarHoraSaida((object)[
            'passageiros' => $aPassageirosCGS,
            'data' => $tf18_d_datasaida,
            'hora' => $tf18_c_horasaida,
            'idVeiculo' => $tf18_i_veiculo,
            'idMotorista' => $tf18_i_motorista
        ]);
        ...
        if ($oDaoTfdVeiculoDestino->erro_status == '0') {
            throw new Exception($oDaoTfdVeiculoDestino->erro_msg);
        }
        ...
        for ($iCont = 0; $iCont < count($aPassageirosSelecionados); $iCont++) {
            /*
             * Cada posição do vetor $aPassageirosSelecionados possui
             * as seguintes informações dispostas da seguinte forma:
            ...
            if ($oDaoTfdPassageiroVeiculo->erro_status == '0') {
                throw new Exception($oDaoTfdPassageiroVeiculo->erro_msg);
            }
        }
        ...
        if (empty($tf18_i_motorista)) {
            // deleto pq o default é 0 e isso viola restrição de chave estrangeira
        ...
        $passageiros = $service->salvarHoraSaida((object)[
            ...
            'idVeiculo' => $tf18_i_veiculo,
            'idMotorista' => $tf18_i_motorista,
            'idVeiculoDestino' => $tf18_i_codigo,
        ]);
         ...
         */
        $sSql = $oDaoTfdPassageiroVeiculo->sql_query(
            null,
            'tf19_i_cgsund, tf19_i_tipopassageiro, tf19_i_codigo, tf19_i_pedidotfd',
            '',
    ...
    $oDaoTfdVeiculoDestino->erro_msg = $e->getMessage();
}
...
if ((isset($tf18_i_codigo)) && ($tf18_i_codigo != '')) {
    ...
    if ($oDaoTfdVeiculoDestino->numrows > 0) {
        ...
        $sSql = $oDaoTfdPassageiroVeiculo->sql_query(
            '',
            ' tf19_i_cgsund,tf19_i_tipopassageiro ',
            '',
            " tf19_i_veiculodestino = $tf18_i_codigo and tf19_i_valido = 1 "
        );
