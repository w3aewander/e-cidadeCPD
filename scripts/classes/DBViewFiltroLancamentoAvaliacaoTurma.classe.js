...
DBViewFiltroLancamentoAvaliacaoTurma.prototype.retornoDadosDaTurma = function(oAjax) {
   ...
   */
  var oSelf    = this;
  var oRetorno = JSON.parse(oAjax.responseText);  // no turma2  esta eval('('+oAjax.responseText+')');
  //var oRetorno = eval('('+oAjax.responseText+')');
  ...
  this.iMaiorPermissao          = oRetorno.iMaiorPermissao;  //acrescentado, não existe em Turma2
  ...
  oRetorno.aDisciplinas.each(function(oDisciplina, iSeq) {
    ...
    oDisciplina.aPeriodos.each(function(oPeriodo, iPeriodo) {
      ...
      var sId                   = oDisciplina.iCodigo + oDisciplina.sAbrev.urlDecode() + iPeriodo; // Turma2 oDisciplina.sAbrev.urlDecode() + iPeriodo;
      ...
      aIdDadosEventosDisciplina.push(oDadosPeriodo);
        let disabledAulasDadas = oRetorno.habilitaAulasDadas ? '' : ' disabled ' // não existe em Turma2
      var sInput = " <input type='text' id='"+sId+"' name='"+sId+"' value='"+oPeriodo.iAulas+"' ";
      sInput    += "        onkeyPress='return js_mask(event, \"0-9\");' maxlength = '4'";
      sInput += disabledAulasDadas;  // não existe em Turma2
...
DBViewFiltroLancamentoAvaliacaoTurma.prototype.retornoSalvarAulasDadas = function(oAjax) {
  ...
  js_removeObj("msgBox");
  var oRetorno = JSON.parse(oAjax.responseText); // diferente
  //var oRetorno = eval('('+oAjax.responseText+')');
