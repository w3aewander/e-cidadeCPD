...
class M21134AjusteTabelaBnccrefrencial extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::table('bnccreferencial')->where('ed168_objeto_conhecimento', '')->update([
            'ed168_objeto_conhecimento' => null
        ]);
        $ocorrencias = 0;
        ...
        foreach ($objetosDeConhecimentoPorCodigo as $codigoHabilidade => $objetosConhecimento) { 
            $ocorrencias = DB::table('bnccreferencial')
                ...
                ->get()->map( function ($objeto, $key) use($objetosConhecimento, $codigoHabilidade) {
