...
class RelatorioFolhaSinteticoAnalitico extends RelatorioFolhaPagamento
{
    ...
    public function getDadosBase()
    {
        ...
        $sWhereAfastamentos .= " and (
                                           ( extract(month from r45_dtafas) = {$this->iMes} 
                                             and extract(year from r45_dtafas) = {$this->iAno} 
                                            ) 
                                        or ( r45_dtreto is null or r45_dtreto >= '{$this->iAno}-{$this->iMes}-01' )
                                     )";
        ...
        foreach ($aSQLBase as $sTabelaPonto => $sSqlBase) {
            ...
            if ($rsDados && pg_num_rows($rsDados) > 0) {
                while ($oDados = pg_fetch_object($rsDados)) {
                     ...
                     */
                    $sSqlAfastamentos = $oDaoAfasta->sql_query_file(
                        null,
                    ...
                    if ($oDaoAfasta->numrows > 0) {
                      ...
                      $oDadosServidor->tabela_previdencia                  = $oDados->tabela_previdencia;
                      $oDadosServidor->data_admissao                       = $oDados->data_admissao     ;
                      $oDadosServidor->descr_cargo                         = $oDados->descr_cargo       ;
                      $oDadosServidor->codigo_localtrab                    = $oDados->codigo_localtrab  ;
                      $oDadosServidor->estrutural_localtrab                = $oDados->estrutural_localtrab;
                      $oDadosServidor->descr_localtrab                     = $oDados->descr_localtrab   ;
