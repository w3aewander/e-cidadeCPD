...
// Módulo: fiscal
// Classe da entidade usuariosparametrosandamento
class cl_fis_usuariosparametrosandamento {
// cria variaveis de erro
    ...
    public $db_usuarios = 1;

    //funcao construtor da classe
    public function __construct() {
    //classes dos rotulos dos campos
        $this->rotulo = new rotulo("fis_usuariosparametrosandamento");
    ...
    public function atualizacampos($exclusao=false) {
        if($exclusao==false){
            ...
            $this->db_usuarios = ($this->db_usuarios == ""?@$GLOBALS["HTTP_POST_VARS"]["db_usuarios"]:$this->db_usuarios);
        }else{
            $this->sequencial = ($this->sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["sequencial"]:$this->sequencial);
        }
    }
    ...
    public function incluir (){
        ...
        if($this->db_usuarios == null ){
            $this->erro_sql = " Campo usuário não informado.";
            $this->erro_campo = "db_usuarios";
            $this->erro_banco = "";
            $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
            $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
            $this->erro_status = "0";
        ...
        $sql = " INSERT INTO fiscalizacao.fis_usuariosparametrosandamento (parametrosandamento, db_usuarios) ";
        $sql .= " VALUES ($this->parametrosandamento, $this->db_usuarios) ";
        $result = db_query($sql);
        if($result==false){
            $this->erro_banco = str_replace("\n","",@pg_last_error());
            if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
                $this->erro_sql   = " Usuários parâmetros ($this->sequencial) não incluídos. Inclusão Abortada.";
                $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
    ...
    public function alterar ($sequencial=null) {
        $this->atualizacampos();
        $sql = " UPDATE fiscalizacao.fis_usuariosparametrosandamento SET ";
        $virgula = "";
        ...
        if(trim($this->db_usuarios)!="" || isset($GLOBALS["HTTP_POST_VARS"]["db_usuarios"])){
            $sql  .= $virgula." db_usuarios = $this->db_usuarios ";
            $virgula = ",";
            if(trim($this->db_usuarios) == null ){
                $this->erro_sql = " Campo usuários não informado.";
                $this->erro_campo = "db_usuarios";
                $this->erro_banco = "";
                $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
                $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        ...
        if($result==false){
            $this->erro_banco = str_replace("\n","",@pg_last_error());
            $this->erro_sql   = "Usuários parâmetros não alterados. Alteracão abortada.\\n";
            $this->erro_sql .= "Valores : ".$this->sequencial;
            $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
            $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
            $this->erro_status = "0";
            ...
            if(pg_affected_rows($result)==0){
                $this->erro_banco = "";
                $this->erro_sql = "Usuários parâmetros não alterados. Alteracão executada.\\n";
                $this->erro_sql .= "Valores : ".$this->sequencial;
                $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
                $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
                $this->erro_status = "1";
    ...
    public function excluir ($sequencial=null,$dbwhere=null) {
        $sql = " DELETE FROM fiscalizacao.fis_usuariosparametrosandamento
        WHERE ";
        $sql2 = "";
    ...
    public function sql_query ( $sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
        ...
        $sql .= " FROM fiscalizacao.fis_usuariosparametrosandamento ";
        $sql2 = "";
        if($dbwhere==""){
            if($sequencial!=null ){
                $sql2 .= " WHERE fis_usuariosparametrosandamento.sequencial = $sequencial ";
            }
        }else if($dbwhere != ""){
            $sql2 = " WHERE $dbwhere ";
        }
