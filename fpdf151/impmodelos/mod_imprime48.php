...
if (isset($this->sCedenteBoleto) && trim($this->sCedenteBoleto) != '') {
    ...
    if (!empty($cnpjBeneficiario) && !empty($this->enderecoInstituicao)) {
...
foreach ($this->aTaxas2 as $oTaxas) {
    $valor = trim(db_formatar($oTaxas->valor,"f"));
    $instrucao .= " {$oTaxas->descricao} = {$valor} ";
}
...
if(!empty($this->aTaxas2) && $this->impmodelo == '102'){
    $this->objpdf->Text($x+93,  $y+80,"SERVIDOR(A): {$this->usuarioNomeIncluido}"); // CPF/CNPJ
    $this->objpdf->Text($x+93,  $y+83,"INSCRIÇÃO: {$this->pretitulo8}"); // CPF/CNPJ

    $this->objpdf->Line($x+136, $y+9,  $x+136, $y+73); //vert 2
} else {
