...
try {
    ...
    $nextValCertValVen               = $oDaoCertValVen->nextValTabela();
    $oDaoCertValVen->j177_sequencial = $nextValCertValVen;
    $oDaoCertValVen->j177_usuario    = $idUsuario;
    $oDaoCertValVen->j177_matricula  = $matricula;
    $oDaoCertValVen->j177_emissao    = $dataEmissao;
    $oDaoCertValVen->j177_observacao = addslashes($observacao);
    ...
    switch ($processoDoSistema) {
        case (1):
            $oDaoCertValVen->j177_processo = $p58_codproc;
            $oDaoCertValVen->j177_titular  = addslashes(strtoupper($z01_nomeprocesso));
            $oDaoCertValVen->incluir($nextValCertValVen);
            break;

        case (2):
            $oDaoCertValVen->j177_processo = $numeroProcesso;
            $oDaoCertValVen->j177_titular  = addslashes(strtoupper($titularProcesso));
            $oDaoCertValVen->incluir($nextValCertValVen);
            break;
    ...
    if ($oDaoCertValVen->erro_status && $oDaoCertValVen->erro_status == '0') {
        echo "<script>alert('" . $oDaoCertValVen->erro_msg . "')</script>";
    ...
    echo "<script>location.href = 'cad3_conscertidoes001.php?abaSelecionada=cad3_conscertvalven001.php&matricula=" . $matricula . "';</script>";
}

echo "<script>location.href = 'cad3_conscertvalven003.php?matricula=" . $matricula . "';</script>";
