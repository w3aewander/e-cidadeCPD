 ...
 */
if (isset($oGet->depositos) && !empty($oGet->depositos)) {
    $sDepartamento = " m91_codigo in  ($oGet->depositos) ";
    if (isset($oGet->veralmoxarifados) && $oGet->veralmoxarifados == "false") {
        $sDepartamento = " m91_codigo not in  ($oGet->depositos) ";
    }
...
$sCampos .= " m60_codmater, m60_descr, m91_codigo as codigo_almoxarifado,  m77_lote, ";
$sCampos .= " dpartestoque.descrdepto as descricao_deposito, ";
...
if ($iTotalItensEstoque > 0) {
    ...
    for ($iEstoque = 0; $iEstoque < $iTotalItensEstoque; $iEstoque++) {
        ...
        $oItemEstoque->sDescricaoAlmoxarifado = $oDadosItemEstoque->descricao_deposito;
        ...
        $sCamposSaidaItem = " coalesce(m82_quant, 0) as quantidade,   ";
    ...
    foreach ($aItensEstoque as $oItemEstoque) {
        ...
        $nQuatidadeEstoque = intval($oItemEstoque->iQuantidadeEstoque) - intval($oItemEstoque->iQuantidadeAtendida);
    ...
    switch ($oGet->tipoimpressao) {
        ...
        case "S": // Sintético
            ...
            foreach ($aEstoques as $oMovimentos) {
                ...
                $lImprime = getDadosSintetico($oPdf, $oMovimentos, $oTotalEstoqueSintetico, $oGet, true);
                if ($lImprime) {
                    $iNumeroLancamentos++;
                }
                $lPrimeiraCelula = false;
            }
            $oTotalEstoqueSintetico->iNumeroLancamentos = $iNumeroLancamentos;
            getTotalizadorSintetico($oPdf, $oTotalEstoqueSintetico, true);
            break;
        case "A": // Analítica
...
function setHeaderSintetico($oPdf, $lSintetico = true) {
    ...
    $oPdf->cell(70 - $iLarguraLinha,  $iAlturaLinha, "Depósito",       1, 0, "C", 1);
...
function getDadosSintetico($oPdf, $oMovimentos, $oTotalEstoqueSintetico, $oGet, $lSintetico = true, $iPreenche = 0, $lAnalitico = false) {
    ...
    $quantidadeEstoque = (string)Material::arredondarQuantidade($oMovimentos->nQuantidadeEstoque);
    $oPdf->cell(40 - $iLarguraLinha, $iAlturaLinha, $quantidadeEstoque, $iBorda, 0, "R", $iPreenche);
    if (!$lSintetico) {
        $oPdf->cell(40 - $iLarguraLinha, $iAlturaLinha, "", $iBorda, 0, "R", $iPreenche);
    }

    $oPdf->cell(20, $iAlturaLinha, 'R$ ', $iBorda, 0, "R", $iPreenche);
    $oPdf->cell(20 - $iLarguraLinha, $iAlturaLinha, db_formatar($nValorEstoque, 'f'), $iBorda, 1, "R", $iPreenche);
...
function getTotalizadorSintetico($oPdf, $oTotalEstoqueSintetico, $lSintetico = true)
{
    ...
    $totalRegistros = 'Total de Registros: ' . $oTotalEstoqueSintetico->iNumeroLancamentos;
    $oPdf->cell(200 - $iLarguraLinha, 4, $totalRegistros, 'TB', 0, "L", 0);
    $quantidadeTotal = (string)Material::arredondarQuantidade($oTotalEstoqueSintetico->iTotalEstoque);
    $oPdf->cell(40 - $iLarguraLinha, 4, $quantidadeTotal, 1, 0, "R", 0);
    if (!$lSintetico) {
        $oPdf->cell(40 - $iLarguraLinha, 4, "", 'TB', 0, "R", 0);
    }
    $oPdf->cell(40 - $iLarguraLinha, 4, 'R$ ' . trim(db_formatar($oTotalEstoqueSintetico->iTotal, 'f')), 1, 1, "R", 0);
