...
<form name="form1" method="post" action="">
    <input type="hidden" name="r33_anousu" value="<?php echo !empty($r33_anousu) ? $r33_anousu : ''?>">
    <input type="hidden" name="r33_mesusu" value="<?php echo !empty($r33_mesusu) ? $r33_mesusu : ''?>">
    <input type="hidden" name="codigoTabela" value="<?php echo !empty($codigo_tab) ? $codigo_tab : ''?>">
  ...
  <div class="container">
          ...
          <fieldset>
            ...
            <table width="100%">
              ...
              if($alteraform == true) {
                ...
                $sCampos  = "r33_nome, r33_tipo, r33_rubmat, a.rh27_descr as rh27_descrmat, r33_rubsau, r33_tiposegregacao";
                $sCampos .= ", b.rh27_descr as rh27_descrsau, r33_rubaci, c.rh27_descr as rh27_descraci, r33_basfer, r33_rubprorrogacaomaternidade, f.rh27_descr as rh27_descrprorrogacaomaternidade";
                $sCampos .= ", d.r08_descr as rh32_descrfer, r33_basfet, e.r08_descr as rh32_descrfet, r33_ppatro";
                $sCampos .= ", r33_tinati, r33_codele, o56_descr, r33_rubfamiliar, r33_rublicencapremio, g.rh27_descr as rh27_descrfamiliar, h.rh27_descr as rh27_descrlicencapremio";
      ...
      <tr>
        <td>
          <fieldset>
            <table>
                <tr>
                    <td nowrap title="<?=$Tr33_rubprorrogacaomaternidade?>">
                        <label for="r33_rubprorrogacaomaternidade">
                            <?php
                            db_ancora($Lr33_rubprorrogacaomaternidade, "js_pesquisarubricas(true,'prorrogacaomaternidade');", $db_opcao);
                            ?>
                        </label>
                    </td>
                    <td nowrap>
                        <?php
                        $sEvento = "onchange='js_pesquisarubricas(false,\"prorrogacaomaternidade\");'";
                        db_input('r33_rubprorrogacaomaternidade',  5, $Ir33_rubprorrogacaomaternidade, true, 'text', $db_opcao, $sEvento);
                        db_input('rh27_descr', 30, $Irh27_descr, true, 'text', 3, "", "rh27_descrprorrogacaomaternidade");
                        ?>
                    </td>
                </tr>
              <tr>
              ...
              <tr>
                <td nowrap title="<?=$Tr33_rubfamiliar?>" >
                  <label for="r33_rubfamiliar">
                    <?php
                    db_ancora($Lr33_rubfamiliar, "js_pesquisarubricas(true,'familiar');", $db_opcao);
                    ?>
                  </label>
                </td>
                <td nowrap>
                  <?php
                  $sEvento = "onchange='js_pesquisarubricas(false,\"familiar\");'";
                  db_input('r33_rubfamiliar',  5, $Ir33_rubfamiliar, true, 'text', $db_opcao, $sEvento);
                  db_input('rh27_descr', 30, $Irh27_descr, true, 'text', 3, "", "rh27_descrfamiliar");
                  ?>
                </td>
              </tr>
              <tr>
                  <td nowrap title="<?=$Tr33_rublicencapremio?>" >
                      <label for="r33_rublicencapremio">
                          <?php
                          db_ancora($Lr33_rublicencapremio, "js_pesquisarubricas(true,'licencapremio');", $db_opcao);
                          ?>
                      </label>
                  </td>
                  <td nowrap>
                      <?php
                      $sEvento = "onchange='js_pesquisarubricas(false,\"licencapremio\");'";
                      db_input('r33_rublicencapremio',  5, $Ir33_rublicencapremio, true, 'text', $db_opcao, $sEvento);
                      db_input('rh27_descr', 30, $Irh27_descr, true, 'text', 3, "", "rh27_descrlicencapremio");
                      ?>
                  </td>
                </tr>
              <tr>
    ...
    <div id="container-faixas" style="width: 700px; margin-top: 25px;"></div>
...
<script>
    ...
    const codigoFaixa = document.querySelector('[name=r33_codigo]');
    const anoUsuario = document.querySelector('[name=r33_anousu]');
    const mesUsuario = document.querySelector('[name=r33_mesusu]');
    const codigoTabela = document.querySelector('[name=codigoTabela]');
    const containerFaixas = document.getElementById('container-faixas');
    const collectionFaixas = new Collection().setId('sequencial');
    const gridFaixas = DatagridCollection.create(collectionFaixas).configure({'order': false, height: 150});

    gridFaixas.addColumn('valorInicial', {label: 'Valor Inicial da Faixa', align: 'center', width: '20%'});
    gridFaixas.addColumn('valorFinal', {label: 'Valor Final da Faixa', align: 'center', width: '20%'});
    gridFaixas.addColumn('percentual', {label: 'Percentual', align: 'center', width: '20%'});
    gridFaixas.addColumn('deduzir', {label: 'Deduzir', align: 'center', width: '20%'});

    const atualizaOpcao = (linha, opcao) => {
        codigoFaixa.value = linha.sequencial;
        anoUsuario.value = linha.ano;
        mesUsuario.value = linha.mes;

        const opcaoInput = parent.document.createElement('input');
        opcaoInput.setAttribute('type', 'hidden');
        opcaoInput.setAttribute('name', 'opcao');
        opcaoInput.setAttribute('value', opcao);
    ...
    gridFaixas.addAction('Alterar', 'Alterar', (event, linha) => atualizaOpcao(linha, 'alterar'), true, 'fa-edit');

    gridFaixas.addAction('Excluir', 'Excluir', (event, linha) => atualizaOpcao(linha, 'excluir'), true, 'fa-trash');

    gridFaixas.show(containerFaixas);
  ...
  function js_mostrarubricasprorrogacaomaternidade(chave,erro){
      document.form1.rh27_descrprorrogacaomaternidade.value  = chave;
    if(erro==true){
          document.form1.r33_rubprorrogacaomaternidade.value = '';
          document.form1.r33_rubprorrogacaomaternidade.focus();
    }
  }

  function js_mostrarubricasprorrogacaomaternidade1(chave1,chave2){
      document.form1.r33_rubprorrogacaomaternidade.value  = chave1;
      document.form1.rh27_descrprorrogacaomaternidade.value  = chave2;
    db_iframe_rhrubricas.hide();
  }
  ...
  function js_verificar_campos(){
    ...
    if (!empty($F('r33_rubmat')) || !empty($F('r33_rubprorrogacaomaternidade')) || !empty($F('r33_rubsau')) || !empty($F('r33_rubaci')) ) {

      var sMsg  = "Não é possível repetir uma mesma rubrica nos campos: Rubrica salário maternidade, Rubrica prorrogação maternidade,";
          sMsg += "Rubrica Licença Saúde e Rubrica Acidente de Trabalho.";

      if ( (!empty($F('r33_rubprorrogacaomaternidade')) && !empty($F('r33_rubprorrogacaomaternidade'))) && $F('r33_rubmat') == $F('r33_rubprorrogacaomaternidade') ) {
            alert( sMsg );
            return false;
      }
      if ( (!empty($F('r33_rubprorrogacaomaternidade')) && !empty($F('r33_rubaci'))) && $F('r33_rubprorrogacaomaternidade') == $F('r33_rubaci') ) {
            alert( sMsg );
            return false;
      }
      if ( (!empty($F('r33_rubprorrogacaomaternidade')) && !empty($F('r33_rubsau'))) && $F('r33_rubprorrogacaomaternidade') == $F('r33_rubsau') ) {
            alert( sMsg );
            return false;
      }
  ...
  function js_mostrarubricasfamiliar1(chave1,chave2){
      document.form1.r33_rubfamiliar.value = chave1;
      document.form1.rh27_descrfamiliar.value = chave2;
      db_iframe_rhrubricas.hide();
  }
  function js_mostrarubricasfamiliar(chave,erro){
      document.form1.rh27_descrfamiliar.value = chave;
      if(erro==true){
          document.form1.r33_rubfamiliar.value = '';
          document.form1.r33_rubfamiliar.focus();
      }
  }

    function js_mostrarubricaslicencapremio1(chave1,chave2){
        document.form1.r33_rublicencapremio.value = chave1;
        document.form1.rh27_descrlicencapremio.value = chave2;
        db_iframe_rhrubricas.hide();
    }
    function js_mostrarubricaslicencapremio(chave,erro){
        document.form1.rh27_descrlicencapremio.value = chave;
        if(erro==true){
            document.form1.r33_rublicencapremio.value = '';
            document.form1.r33_rublicencapremio.focus();
        }
    }

  const buscaFaixas = () => {
      const data = new FormData();
      data.append('codigoTabela', codigoTabela.value);
      data.append('acao', 'buscarFaixas');
      ...
      HttpClient.post('pes1_inssirf002.RPC.php', { body: data}).then(response => {
          ...
          response.faixas.map(faixa => {
              collectionFaixas.add(faixa)
          });
