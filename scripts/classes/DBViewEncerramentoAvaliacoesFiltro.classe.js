DBViewEncerramentoAvaliacoesFiltro = function (sInstancia, iEncerra) {
  ...
  this.validarProfessorLogado = true;
  /**
   * iEncerra = 1 Encerrar
   * iEncerra = 2 Cancelar
   */
  ...
  this.criarTreeView = function () {
    ...
    oParametro.lValidarProfessorLogado = me.validarProfessorLogado;
  ...
  js_montaGridAlunos = function(oResponse) {
    ...
    oRetorno.aDadosAlunos.each(function(oAluno, iSeq) {
      ...
      if (me.lProgressaoParcial) {
        ...
        aLinha[5]  = (oAluno.lEvadido == true) ? "EVADIDO" : oAluno.sResultadoFinal.urlDecode();
    ...
    oRetorno.aDadosAlunos.each(function(oAluno, iSeq) {
      ...
      $(me.oDataGridAlunos.aRows[iSeq].sId).setAttribute('evadido', oAluno.lEvadido);
      if (!me.lProgressaoParcial) {
  ...
  *
  * @default true
  * @param {Boolean} validarProfessorLogado
  */
  this.setValidarProfessorLogado = function(validarProfessorLogado) {
    me.validarProfessorLogado = validarProfessorLogado;
  };
  ...
  this.buscaAlunosSelecionados = function() {
    ...
    aAlunosSelecionados.each(function (oAlunoLinha, iSeq) {
      ...
      oAluno.sNome = oAlunoLinha.aCells[3].getValue(); 
      oAluno.lEvadido = $(oAlunoLinha.sId).readAttribute('evadido');
      aAlunos.push(oAluno);
    });
  ...
  this.encerraProgressaoParcial = function () {
    ...
    for (var oAluno of aAlunos) {

      if ( oAluno.lEvadido == 'true' && !aAlunosEvadidos.in_array(oAluno.sNome) ) {
        aAlunosEvadidos.push( oAluno.sNome );
      }
    }

    if ( aAlunosEvadidos.length > 0 ) {

      sMensagem = 'Os seguintes alunos estão evadidos e serão reprovados no ano letivo atual: ';
      for (var sNome of aAlunosEvadidos) {
        sMensagem += "\n - " + sNome;
      }

      sMensagem += '\n\nConfirmar o encerramento das Progressões selecionadas?';
    }
