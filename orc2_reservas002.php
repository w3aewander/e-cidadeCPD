...
$sCampos .= "w.o58_codigo, w.o58_coddot, gestao, o15_complemento,descricao";
$sCampos .= "";
...
select distinct
 ...
 inner join fonterecurso          on orctiporec_id   = r.o15_codigo
                                 and exercicio       = w.o58_anousu
...
for ($iInd=0; $iInd < $iNroReserva; $iInd++) {
    ...
    //if($oReserva->o80_coddot != 677299){continue;}
    
    $verificaatual = buscaReservaAtual($oReserva->o80_anousu, $oReserva->o58_projativ, $oReserva->o58_codigo,  $oReserva->o80_coddot, $oReserva->o56_elemento);
    if($verificaatual){
        $sqlpg = pg_query("SELECT substr(fc_dotacaosaldo($oReserva->o80_anousu, $oReserva->o80_coddot, 2, '{$sDataFinal}', '{$sDataFinal}'), 137, 12)::float8");
        $vatual = pg_fetch_all($sqlpg);
        $vatual = $vatual[0]["substr"];
        $xresultado = $vatual * ($verificaatual / 100);
        $oReserva->o80_valor += $xresultado;        
    }
    ...
    $aReserva['CodDot']  = $oReserva->o58_coddot;
    $aReserva['Descricao']  = $oReserva->o56_descr;
    ...
    $sDescrRecurso = sprintf('%s - %s %s', $oReserva->gestao, $oReserva->o15_complemento, $oReserva->descricao);

    $sDescrDesdobramento = db_formatar($oReserva->coddesdobramento, 'elemento').' - '.$oReserva->descrdesdobramento;

    $aNivel[$oReserva->tiporeserva]
           [$sDescrOrgao]
           [$sDescrUnidade]
           [$sDescrFuncao]
           [$sDescrSubFuncao]
...
$pdf->init(false);
$pdf->addTitulo('RELATORIO DE RESERVA DE SALDO');
$pdf->addTitulo('');

$pdf->addTitulo("EXERCÍCIO : ".db_getsession("DB_anousu"));
$pdf->addTitulo("INSTITUIÇÕES : ".$descr_inst);
$pdf->addTitulo("POSIÇÃO ATÉ : ".$data_fin);
$pdf->addTitulo("NIVEL : ".($nivel{1}=="A"?"Até ":"").$sDescricaoNivel);
...
if ($nivel{1} == 'A') {
                                ...
                                foreach ($aDadosProjAtiv as $sElemento => $aDadosElemento) {
                                    ...
                                    foreach ($aDadosElemento as $sRecurso => $aDadosRecursos) {
                                        ...
                                        foreach ($aDadosRecursos['aReservas'] as $iCodReserva => $aReserva) {
                                            ...
                                            $codDot        = $aReserva['CodDot'];
                                            $descricao     = $aReserva['Descricao'];
                                            ...
                                            if ($forma_impressao == 'a') {
                                                ...
                                                $pdf->multicell(80, $alt, $sDescrReserva, 'TB', 'L');
                                                $pdf->setfont('arial', 'b', 8);
                                            }
    ...
    foreach ($aNivelUnico[$nivel{0}] as $sTipoReserva => $aDadosNivel) {
        ...
        $pdf->cell(90, $alt, 'TOTAL RESERVA', 'TBR', 0, 'L');
...
function imprimeTotal($pdf, $iAlt, $nValorTotal, $sDescr, $iNivel, $iNivelTotal)
{
    ...
    if ($iNivel == $iNivelTotal) {
        $pdf->cell(90, $iAlt, $sDescr, 'TBR', 0, 'L');
