...
class M10320EstornoPagamentoArrecadacaoReceita extends PostgresMigration
{
    public function up()
    {
        $funcao = <<<FUNCAO
    ...
    if valor is null then
      ...
      select sum(k00_valor),
             case when
               ...
               ( select sum(1)
                   from recibopaga rp
                        inner join cornump on k00_numpre = k12_numpre
                                          and k00_numpar = k12_numpar
                                          and k00_receit = k12_receit
                  where rp.k00_numnov = NUMPRE
                    and k12_autent is not null
               ) > 0
               then 1
              else null
             end as autenticado
