...
// if($this->labtitulo == "Solicitação"){
//   $sSqlAdmi = db_query("select pc90_numeroprocesso from  solicitaprotprocesso  where pc90_solicita =$this->Snumero");
// }else{
//   $sSqlAdmi = db_query("select pc90_numeroprocesso from pcprocitem left join solicitem on pc11_codigo = pc81_solicitem left join solicitaprotprocesso  on pc90_solicita = pc11_numero where pc81_codproc =  $this->Snumero");
// }
// die("select pc90_numeroprocesso from solicitaprotprocesso where pc90_solicita =  $this->Snumero");
// $sProcAdmin = pg_result($sSqlAdmi,0,'pc90_numeroprocesso');
$this->objpdf->text(130,$xlin-14,$this->modalidade.": "); 
$this->objpdf->text(130,$xlin-11,"Processo: "); 
$this->objpdf->text(130,$xlin-7,"Data:");
$this->objpdf->text(130,$xlin-3,"Solic. de Compra:");
$this->objpdf->text(130,$xlin+1,"Nº da compra:");

$this->objpdf->Setfont('Arial','',8);
$this->objpdf->text(160,$xlin-14,$this->pregao); 
$this->objpdf->text(160,$xlin-11,$this->processo); 
$this->objpdf->text(160,$xlin-7,db_formatar($this->data,"d"). " às ".$this->hora ); 
$this->objpdf->text(160,$xlin-3,$this->solicita.'/'.date('Y')); 
$this->objpdf->text(160,$xlin+1,$this->pcproc.'/'.date('Y')); 
...
// $this->objpdf->text($xcol+2,$xlin+10,'Dados do Orçamento/'.$this->labtitulo);
$this->objpdf->Setfont('Arial','B',8);
// $this->objpdf->text($xcol+  2,$xlin+ 13,'Orçamento');
// $this->objpdf->text($xcol+109,$xlin+ 13,'Data Limite');
// $this->objpdf->text($xcol+150,$xlin+ 13,'Hora Limite');
// $this->objpdf->text($xcol+  2,$xlin+18,$this->labtitulo);
...
if($this->anexo2 == false ){
  ...
  $this->objpdf->setxy($xcol,$xlin+12);
  $this->objpdf->multicell(136,4,"$texto",0,"L",0);
  $this->objpdf->setxy($xcol+24.5,$xlin+23);





   $sLabel = ($this->temlote==true ? " - ".$this->descrlote : '');
  $this->objpdf->rect($xcol,$xlin+10,$xcol+130,$this->objpdf->gety()-($xlin+3)+20,2,'DF','1234');
  $this->objpdf->rect($xcol+134,$xlin+10,$xcol+64,$this->objpdf->gety()-($xlin+3)+20,2,'DF','1234');
  $this->objpdf->rect($xcol+135,$xlin+11,$xcol+62,$this->objpdf->gety()-($xlin+3)+18,2,'DF','1234');

  $this->objpdf->Setfont('Arial','b',7);
  $this->objpdf->SetTextColor(205,205,205);
  $this->objpdf->text($xcol+158,$xlin+30,'Carimbo CNPJ');
  $this->objpdf->SetTextColor(0,0,0);

  $this->objpdf->Setfont('Arial','',6);
  $this->objpdf->text($xcol+2,$xlin+30,'Dados do Fornecedor');
  $this->objpdf->Setfont('Arial','B',8);
  $this->objpdf->text($xcol+  2,$xlin+35,'Firma');
  $this->objpdf->text($xcol+  2,$xlin+45,'Endereço');

  $this->objpdf->Setfont('Arial','',8);

  // Imprime dados dos fornecedores
  $this->objpdf->text($xcol+ 18,$xlin+35,':  '.$this->nome);
  // $this->objpdf->text($xcol+122,$xlin+43,':  '.$this->numcgm);
  // $this->objpdf->text($xcol+163,$xlin+43,':  '.(trim($this->cnpj)!=""?(strlen($this->cnpj) == 11?db_formatar($this->cnpj,'cpf'):db_formatar($this->cnpj,'cnpj')):""));
  $this->objpdf->text($xcol+ 18,$xlin+ 45,':  '.$this->ender);
}
$this->objpdf->rect($xcol,$xlin+2,$xcol+198,5,2,'DF','1234');
$this->objpdf->Setfont('Arial','b',11);
if(isset($this->anexo2) and $this->anexo2 == true) {
  $this->objpdf->text($xcol+80,$xlin+6,'PLANILHA DE VALORES '.$sLabel);
} else {
...
// $this->objpdf->rect($xcol,$xlin+37,$xcol+198,16,2,'DF','1234');

// $this->objpdf->text($xcol+109,$xlin+43,'Numcgm');
// $this->objpdf->text($xcol+150,$xlin+43,(strlen($this->cnpj) == 11?'CPF':'CNPJ'));

// $this->objpdf->text($xcol+102,$xlin+47,'Complemento');
// $this->objpdf->text($xcol+  2,$xlin+51,'Município');
// $this->objpdf->text($xcol+115,$xlin+51,'CEP');
// $this->objpdf->text($xcol+150,$xlin+47,'Contato');
// $this->objpdf->text($xcol+150,$xlin+51,'Fone/Fax');
...
// $this->objpdf->text($xcol+163,$xlin+51,':  '.$this->telefone.$this->fax);

// Caixa com dados da entrega
/*$this->objpdf->rect($xcol,$xlin+55,$xcol+198,10,2,'DF','1234');
$this->objpdf->Setfont('Arial','',6);
$this->objpdf->text($xcol+2,$xlin+57,'Dados dos Produtos');
$this->objpdf->Setfont('Arial','B',8);
$this->objpdf->text($xcol+2,$xlin+60,'Prazo de entrega ');
$this->objpdf->text($xcol+2,$xlin+64,'Validade do orçamento ');*/
...
// Recria cabecalho do relatorio antes de imprimir restante de itens  
     if($this->objpdf->gety() > $this->objpdf->h - 80){
         $this->objpdf->AddPage();
         $this->objpdf->settopmargin(1);
...
// Caixa externa
         ...
         $this->objpdf->Setfont('Arial','B',9);
         $this->objpdf->Image('imagens/files/'.$sLogo,15,$xlin-17,12); //.$this->logo
         $this->objpdf->text(130,$xlin-14,"PREGÃO: "); 
         $this->objpdf->text(130,$xlin-11,"Processo: "); 
         $this->objpdf->text(130,$xlin-7,"Data:");
         $this->objpdf->text(130,$xlin-3,"Solic. de Compra:");
         $this->objpdf->text(130,$xlin+1,"Nº da compra:");
     ...
     if($volta_impressao == 0){
       ...
       if(isset($this->anexo2) and $this->anexo2 == true) {
        if($codgrupo == 11 && isset($this->casadec_medicamentos)){
         ...
         // echo pg_result($this->recorddositens,$ii,$this->vlrmedia)."<>".pg_result($this->recorddositens,$ii ,$this->quantitem);
         // echo "<br>";
         // echo $nTotalGeral;
         // echo "<br>";
       }else{
...
if (strlen(trim($this->declaracao)) > 0) {
    ...
    $this->objpdf->text($xcol+193,($linha+26.5),'Página: '.$pagina);    
if($final == false){
   /* $this->objpdf->Setfont('Arial','',7);
    $this->objpdf->text($xcol+100,($linha+15),'Em         /        / '.date('Y').'      _______________________________________________');
    $this->objpdf->text($xcol+133,($linha+20),'Firma Proponente - Assinatura do Responsável');
*/
    
}else if($mostra_rodape == true && $this->anexo2){
/*  $mostra_rodape = false;
  $this->objpdf->rect($xcol,$linha-10, 104,32,2,'DF','34');
  $this->objpdf->text($xcol+40,($linha-5),'OBSERVAÇÕES');
  $this->objpdf->rect($xcol+104,$linha-10, 98,32,2,'DF','34');

  $this->objpdf->text($xcol+107,($linha-5),'Prazo de Entrega dos Produtos: '.$this->prazo_entrega);
  $this->objpdf->text($xcol+107,($linha),'Local de Entrega dos Produtos:'.substr($this->local_entrega,0,40));
  $this->objpdf->text($xcol+193,($linha+26.5),'Página: '.$pagina);   */
}
   if ($muda_pagina == true){
       $linha = $xlin+10;
...
$linha += 240;
    ...
    $this->objpdf->Setfont('Arial','B',7);
    $this->objpdf->text($xcol+4,($linha),'1 - A proposta deverá :');
    $this->objpdf->Setfont('Arial','',7);
    $this->objpdf->text($xcol+4,($linha+5),'- Ser preenchhida sem emendas e rasuras');
    $this->objpdf->text($xcol+4,($linha+8),'- já incluídas as despesas de fretes, impostos federais, ou estaduais e descontos especiais');
    $this->objpdf->text($xcol+4,($linha+11),'- ser datadas e assinadas pelo responsável');
    $this->objpdf->Setfont('Arial','B',7);
    $this->objpdf->text($xcol+4,($linha+15),'2 - O proponente:');
    $this->objpdf->Setfont('Arial','',7);
    $this->objpdf->text($xcol+4,($linha+18),' - Se obrigará, mediante a devolução da Proposta, a cumprir todos os  termos nela contida');
    $this->objpdf->Setfont('Arial','B',7);
    $this->objpdf->text($xcol+4,($linha+21),'3 - Validade da Proposta: 60 dias ');
    $this->objpdf->Setfont('Arial','',7);
    ...
    $this->objpdf->Setfont('Arial','',8);
    $this->objpdf->text($xcol+104,($linha+5),'______________________________________________________________');
    $this->objpdf->text($xcol+107,($linha+8),'Declaramos inteira submissão ao presente termo e legislação vigente');
    $this->objpdf->Setfont('Arial','',7);
    $this->objpdf->text($xcol+107,($linha+15),'Em         /        /'.date('Y').'      _______________________________________________');
    $this->objpdf->text($xcol+143,($linha+20),'Firma Proponente - Assinatura do Responsável');

    $this->objpdf->text($xcol+107,($linha+20),'CNPJ: ');
