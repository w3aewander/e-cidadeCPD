...
try {
    ...
    $nextValCertForLaud               = $oDaoCertForLaud->nextValTabela();
    $oDaoCertForLaud->j178_sequencial = $nextValCertForLaud;
    $oDaoCertForLaud->j178_usuario    = $idUsuario;
    $oDaoCertForLaud->j178_matricula  = $matricula;
    $oDaoCertForLaud->j178_emissao    = $dataEmissao;
    $oDaoCertForLaud->j178_observacao = addslashes($observacao);
    ...
    switch ($processoDoSistema) {
        case (1):
            $oDaoCertForLaud->j178_processo = $p58_codproc;
            $oDaoCertForLaud->j178_titular  = addslashes(strtoupper($z01_nomeprocesso));
            $oDaoCertForLaud->incluir($nextValCertForLaud);
            break;

        case (2):
            $oDaoCertForLaud->j178_processo = $numeroProcesso;
            $oDaoCertForLaud->j178_titular  = addslashes(strtoupper($titularProcesso));
            $oDaoCertForLaud->incluir($nextValCertForLaud);
            break;
    ...
    if ($oDaoCertForLaud->erro_status && $oDaoCertForLaud->erro_status == '0') {
        echo "<script>alert('" . $oDaoCertForLaud->erro_msg . "')</script>";
...
echo "<script>location.href = 'cad3_conscertforlaud003.php?matricula=" . $matricula . "';</script>";
