 ...
 */
$sWhereElemento = '';
if (!is_null($sElementos)) {
    $aElementos = explode(",", trim($sElementos));
    $whereElemento = [];

    foreach ($aElementos as $elemento) {
        $whereElemento[] = "o56_elemento like '" . trim($elemento) . "%'";
    }
    ...
    foreach ($aLinhasRelatorio as $oLinhaRelatorio) {
        ...
        if ($nTotal > 0) {
            ...
            foreach ($oLinhaRelatorio->elementos as $oElemento) {
                ...
                $pdf->SetFont($sFonte, "", 5);
                $pdf->cell(65, $iAlt, "{$oElemento->o56_descr} - Rec: {$oElemento->o15_recurso}", 0, 0, "L");
                $nTotal = 0;
                $pdf->SetFont($sFonte, "", 7);
                for ($iAno = $oPost->o01_anoinicio; $iAno <= $oPost->o01_anofinal; $iAno++) {
