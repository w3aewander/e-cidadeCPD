...
$HEAD3 = "RELATÓRIO DE AUTORIZAÇÃO DE PAGTO";
$HEAD5 = @$e87_codgera;
$HEAD6 = @$e87_descgera;
...
$sqlOrdem = "
 ...
 where e80_instit = " . db_getsession("DB_instit") . " and  $db_where 
  ...
  from empageconfgera
       ...
       left join orctiporec                on cre.c61_codigo     = o15_codigo 
...
//      $pdf->cell(125,$alt,'',1,1,"C",1);
    ...
    // seleciona o conta e agencia bancaria do banco
    $sql_conta = "select case when trim(db83_codigooperacao) = '' or db83_codigooperacao is null then '' 
                              else db83_codigooperacao||'.' end ||db83_conta||'-'||db83_dvconta as conta,
                         db89_db_bancos as banco,
                         db89_codagencia||case when trim(db89_digito) = '' or db89_digito is null then '' else '-'||db89_digito end as agencia
            from saltes
                 inner join conplanoreduz         on k13_reduz = c61_reduz
                 inner join conplanocontabancaria on c61_codcon = c56_codcon
            ...
            where k13_reduz = $e83_conta
              and c56_anousu = ".db_getsession("DB_anousu");
    $rconta = db_query($sql_conta);
    if (pg_numrows($rconta) > 0 ){
      db_fieldsmemory($rconta,0);
    }

    $pdf->ln(3);
    $pdf->cell(15,$alt,$e85_codtipo,1,0,"C",1);
    $pdf->cell(220,$alt,$e83_descr.'     AGENCIA : '.$agencia.'     CONTA : '.$conta,"LTB",0,"L",1);
 //   $pdf->cell(42,$alt,"COVÊNIO - $e83_convenio","RTB",1,"L",1);
    $pdf->cell(42,$alt,"","RTB",1,"L",1);
    $xtotal = 0;
  ...
  if($o15_codigo != 0){
     $pdf->cell(45,$alt,$o15_codigo.'-'.substr($o15_descr,0,20),1,0,"L",0);
  }else{
...
$pdf->ln(2);
$pdf->cell(254,$alt,'Total a Pagar',1,0,"R",1);
$pdf->cell(23,$alt,db_formatar($xvaltotal,'f'),1,1,"R",1);
//$pdf->cell(125,$alt,'',1,1,"C",1);
...
//$pdf->text(40,$pdf->h - 17,'______________________________',0,4);
//$pdf->text(45,$pdf->h - 14,'Cleide C. da S. Ferreira',0,4);
//$pdf->text(46,$pdf->h - 11,'Tesoureira DF/SMF',0,4);

$pdf->text(40,$pdf->h - 17,'______________________________',0,4);
$pdf->text(45,$pdf->h - 14,'  Claudionor de Souza',0,4);
$pdf->text(46,$pdf->h - 11,'DIRETOR DE DF/SMF',0,4);

$pdf->text(210,$pdf->h - 17,'______________________________',0,4);
$pdf->text(220,$pdf->h - 14,'Fabiano Vieira de A. de Souza',0,4);
$pdf->text(220,$pdf->h - 11,'SECRETÁRIO DE FAZENDA',0,4);
