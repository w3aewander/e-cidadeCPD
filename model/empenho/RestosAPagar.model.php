...
class RestosAPagar extends EmpenhoFinanceiro {
   ...
   * @throws Exception
   */
  public static function getValorProcessado($iAno, $iInstituicao, $lExerciciosAnteriores = false) {
    ...
    $rsSqlEmpresto = db_query($sSqlEmpresto);
    if ($rsSqlEmpresto == false) {
     ...
     * @param bool $lExerciciosAnteriores
     * @return stdClass[]
     * @throws Exception
     */
  public static function getEmpenhosProcessados($iAno, $iInstituicao, $lExerciciosAnteriores = false) {
      ...
      $sCampos       = "e60_numemp, round(coalesce(sum(e91_vlrliq - e91_vlrpag), 0), 2) as valor";
      $sSqlEmpresto  = $oDaoEmpresto->sql_query_empenho(null, null, $sCampos, null, $WhereEmpresto);
      $sSqlEmpresto  = "select * from ($sSqlEmpresto group by e60_numemp) as x where x.valor > 0;";
      $rsSqlEmpresto = db_query($sSqlEmpresto);
  ...
  public static function getValorNaoProcessadoAno($iAno, $iInstituicao, $lExerciciosAnteriores = false) {
     ...
     * @return stdClass[]
     */
  public static function getEmpenhosNaoProcessadosAno($iAno, $iInstituicao, $lExerciciosAnteriores = false) {
      ...
      $sCampos       = "e60_numemp, round(coalesce(sum(e91_vlremp - e91_vlranu - e91_vlrliq), 0), 2) as valor";
      $sSqlEmpresto  = $oDaoEmpresto->sql_query_empenho(null, null, $sCampos, null, $WhereEmpresto);
      $sSqlEmpresto  = "select * from ({$sSqlEmpresto} group by e60_numemp) as x where x.valor > 0 order by 1;";
      $rsSqlEmpresto = db_query($sSqlEmpresto);
     ...
     * @param $iAno
     * @param $iInstituicao
     * @param $lExerciciosAnteriores
     * @return stdClass[]
     * @throws Exception
     */
  public static function getEmpenhos($iTipoProcessamento, $iAno, $iInstituicao, $lExerciciosAnteriores)
  {
      if ((int)$iTipoProcessamento === 1) {
          return self::getEmpenhosNaoProcessadosAno($iAno, $iInstituicao, $lExerciciosAnteriores);
      }

      if ((int)$iTipoProcessamento === 2) {
          return self::getEmpenhosProcessados($iAno, $iInstituicao, $lExerciciosAnteriores);
      }
  }
