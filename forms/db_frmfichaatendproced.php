...
$clrotulo->label("sd24_idadegestacional");
$clrotulo->label("sd24_dum");
      ...
      <td>
        <fieldset>
          ...
          <table class="form-container">
            <tr>
              ...
              <td style="display: flex;">
                ...
                ?>
                <div style="display: flex; align-items: center; height: 18px; padding-left: 280px" id="linhaSd29icodigo"></div>
              </td>
            </tr>
            <tr>
              ...
              <td>
                ...
                ?>
                  <!-- plugin ESF operation#12 - adicionando campo hidden tipoAtendimento -->
              </td>
            </tr>
          ...
          <div class="subcontainer">
            <button type="button" id="btnProblemasPaciente">
        ...
        <!-- Este dado é o crm do funcionario -->
        <input type="hidden" id="sd04_v_registroconselho" value="<?= (!empty($sd04_v_registroconselho) ? $sd04_v_registroconselho : '') ?>">
          ...
          <legend>Outros Achados</legend>
            <table>
                <tr>
                    <td>
                        <?php
                        $sd29_t_achado = !isset( $sd29_t_achado ) ? ' ' : $sd29_t_achado;
                        db_textarea( 'sd29_t_achado', 4, 110, @$sd29_t_achado, true, 'text', $db_opcao );
                        ?>
                    </td>
                </tr>
            </table>
        ...
        <fieldset>
          ...
          <table>
            <tr>
              <td>
                <button name="<?=($db_opcao==1?"incluir":($db_opcao==2||$db_opcao==22?"alterar":"excluir"))?>"
                        type="submit"
                        id="db_opcao"
                        <?=($db_botao==false?"disabled":"")?>
                        style="width:150px;">
                  <?=($db_opcao==1?"<i class='fas fa-plus'></i> Incluir":($db_opcao==2||$db_opcao==22?"<i class='fas fa-save'></i> Alterar":"<i class='fas fa-trash'></i> Excluir"))?>
                </button>
              </td>
              <td>
              ...
              <td>
                <button name="novo" type="button" id="novafaa" <?=($db_botao1==true?"disabled":"")?> style="width:150px;">
                 <i class="fas fa-file-medical"></i>
                  Nova FAA
                </button>
                <button name="emitir" id="emitir" type="button" onclick="js_emitirFaa();" style="width:150px;">
            ...
            <tr>
              <td>
                <button name="emitir" type="button" id="emitirfaa" <?=($db_botao1==true?"disabled":"")?> style="width:150px;">
              ...
              <td>
                <button name="fatoresderisco" type="button" <?=($db_botao1==true?"disabled":"")?> onclick="js_fatoresderisco()" style="width:150px;">
              ...
              <td>
                ...
                ?>
                  <button type="button" name="consulta" id="consulta" onclick="js_consulta();" style="width:150px;">
            ...
            <tr>
              <td>
                <button id="btnFinalizar" type="button" style="width: 150px;">
                  <i class="fas fa-check-double"></i>
                  Finalizar Atendimento
                </button>
              </td>
              <td>
                <button id="btnEncaminhar" type="button" style="width: 170px; visibility:hidden;" >
                  <i class="fas fa-forward"></i>
                  Encaminhar
                </button>
              </td>
              <td>
                <button id="btnAtestado" type="button" style="width: 150px;" onclick="emitirAtestado();">
                  <i class="fas fa-stamp"></i>
                  Atestado
                </button>
              </td>
              <td>
                <button id="btnCadastroGeral" type="button" style="width: 150px;" onclick="cadastroGeralCGS();">
        ...
        <!-- PLUGIN ESF: CAMPOS DE HISTÓRICO SUBJETIVO/EVOLUÇÃO -->
      </td>
    </tr>
  </table>
</form>
<!-- plugin ESF operation#16 - adicionando html da janela tipoAtendimento -->
<!-- Modal problemas/condições do paciente -->
<div id="divProblemas" class="subcontainer"></div>
...
<!-- plugin ESF operation#15 - criando janela tipoAtendimento -->


const inputFAA = document.getElementById('sd24_i_codigo');
...
const divProblemas = document.getElementById('divProblemas');
const wndProblemasPaciente = new ViewProblemasPaciente(divProblemas, true);
...
window.onload = () => {
  if (inputFAA.value == '') {
    js_pesquisaprontuarios();
  }
  validaCgs.cadastroMicroarea(inputCgs, divAlert);
    <!-- plugin ESF operation#14 - invocando função js -->
}
...
if( a1.toLowerCase() == 'procedimentos' ) { // Consulta médica
  ...
  document.getElementById('voltar').innerHTML      = '<i class="fas fa-search"></i> Consultar Paciente';
  document.getElementById('voltar').onclick        = function(){ js_pesquisaprontuarios() };
  document.getElementById('emitirfaa').innerHTML   = '<i class="fas fa-notes-medical"></i> Prontuário Paciente';
  ...
  document.getElementById('voltar').innerHTML     = '<i class="fas fa-reply"></i> Voltar';
  document.getElementById('voltar').onclick       = function(){ parent.mo_camada('a2') };
  document.getElementById('emitirfaa').innerHTML  = '<i class="fas fa-print"></i> Emitir FAA';
  document.getElementById('emitirfaa').onclick    = function(){ js_emitirFaa() };
  document.getElementById('novafaa').innerHTML    = '<i class="fas fa-file-medical"></i> Nova FAA';
  document.getElementById('cancelar').onclick     = function(){ js_cancelaAlteracao('sau4_fichaatendabas003.php') };
}

document.getElementById('btnProblemasPaciente').addEventListener('click', () => {
  if (inputFAA.value == '') {
    alert('FAA não informada!');
    return;
  }
  wndProblemasPaciente.open(inputCgs.id.value, inputCgs.nome.value, inputFAA.value);
});
...
function js_prontuarioMedico() {
  ...
  if (document.form1.sd24_i_codigo.value == '') {
    ...
    js_OpenJanelaIframe('', 'db_iframe_relatorioprontuario', sUrl, 'Prontuário Paciente', true, '20', iLeft, 700, 320);
  }
}
    ...
    ?>
      ...
      if (parent.document.formaba.a4 != undefined) {
        ...
        parent.iframe_a3.location.href = 'sau4_sau_receitamedica001.php?s158_i_profissional=' +
                                         ...
                                         '&sd03_i_crm='+document.getElementById('sd04_v_registroconselho').value;
        ...
        var sUrlAbaRequisicao  = 'sau4_requisicaoexame001.php?';
            ...
            sUrlAbaRequisicao += '&numCgs= ' + $F('z01_i_cgsund');
...
$('btnFinalizar').onclick = function() {
  ...
  var fRedireciona = async function() {
    parent.document.formaba.a2.disabled = true;
    parent.document.formaba.a3.disabled = true;
...
function retornaIdadePaciente(oRetorno, lErro) {
    ...
    $('linhaSd29icodigo').innerHTML  = `<p style="display: inline-block">Idade: <span style="color: red;">${idadeCompleta}</span></p>`;
}
...
function emAtendimento() {
    let prontuario = document.getElementById('sd24_i_codigo').value;
    if (!empty(prontuario)) {
        let medico = document.getElementById('sd03_i_codigo');

        let diretrizes = {}
        diretrizes.sExecucao = 'criaSituacaoEmAtendimento';
        diretrizes.iProntuario = prontuario;
        diretrizes.iTelaOrigem = DBViewEncaminhamento.RECEPCAO;
        diretrizes.medicoAtendendo = medico.value;

        let oAjaxRequest  = new AjaxRequest('sau4_fichaatendimento.RPC.php', diretrizes, function() {
            return;
        });
        oAjaxRequest.execute();
    }
...
<!-- plugin ESF operation#13 - adicionando função js -->
</script>
