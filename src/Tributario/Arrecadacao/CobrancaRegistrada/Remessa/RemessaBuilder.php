...
class RemessaBuilder
{
    ...
    public function processaArquivoRemessa($lCallBack, $lQuebraLinha = false)
    {
        ...
        while ($iQuantidadeAtual < $iQuantidadeRegistros) {
            ...
            $sSqlInstituicao = $this->oRemessaTemporaryService->getSqlInstituicao();

            $oCollection = new RegistroCollection($sSql, $sSqlInstituicao);

            $this->oModeloArquivo->setCallback(function ($iQuantidade) use (
              $iQuantidadeAtual,
    ...
    public function salvaArquivoRemessa($conn, $aRecibosGerados, $lCallBack)
    {
        ...
        foreach ($aRecibosGerados as $iRemessa => $aGeracao) {
            ...
            $sTiposDebito = $this->oRemessaTemporaryService->getTiposDebito();
            ...
            $oRemessa = new Remessa($iInstit, $iConvenio, $iSequencialRemessa, $oDataEmissao, $iOid, $sTiposDebito);
