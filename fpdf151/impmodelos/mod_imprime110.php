...
$pix = $pixRepository->getByCodBar($this->codigobarras);
...
for ($i = 0; $i < 2; $i++) {
  ...
  //Via
    ...
    $this->objpdf->text(
        $xcol+128,
        $xlin,
        'Data : '. date("d/m/Y", db_getsession("DB_datausu")). ' Hora: '.date("H:i:s")
    );
    ...
    $somouLICBomPagador = false;
    $existeLinhaDesconto = false;
    ...
    for ($ii = 0; $ii < $this->linhasdadospagto; $ii++) {
        ...
        if ($ii==0 || $ii == 14) {
            ...
            $this->objpdf->cell(7, 3, "Rec", 0, 0, "L", 0);
            $this->objpdf->cell(9, 3, "Reduz", 0, 0, "L", 0);
        ...
        $this->objpdf->setx($xcol+3+$maiscol);
        $this->objpdf->cell(7, 3, trim(pg_fetch_result($this->recorddadospagto, $ii, $this->receita)), 0, 0, "R", 0);
        $this->objpdf->cell(
            9,
            3,
            "(".trim(pg_fetch_result($this->recorddadospagto, $ii, $this->receitared)).")",
            0,
            0,
            "R",
            0
        ...
        if (trim(pg_fetch_result($this->recorddadospagto, $ii, $this->ddreceita)) == '') {
            $this->objpdf->cell(
                63,
                3,
                trim(pg_fetch_result($this->recorddadospagto, $ii, $this->dreceita)." ".$this->obsdescr),
                0,
                0,
                "L",
                0
            ...
            $this->objpdf->cell(
                63,
                3,
                trim(pg_fetch_result($this->recorddadospagto, $ii, $this->ddreceita)." ".$this->obsdescr),
                0,
                0,
                "L",
                0
        ...
        $nDescontoLICBomPagadorValor = 0;
        if (isset($this->descontoLICBomPagador->j21_valor)) {
            $nDescontoLICBomPagadorValor = $this->descontoLICBomPagador->j21_valor;
        }

        if ($valor < 0 || $valor == null) {
             $this->totaldesc += pg_fetch_result(
                 $this->recorddadospagto,
                 $ii,
                 $this->valor
             ) + $nDescontoLICBomPagadorValor;
        } elseif ($codtipo == 't' and $valor > 0) {
             $this->totalacres  += pg_fetch_result($this->recorddadospagto, $ii, $this->valor);
        } else {
            if (!$somouLICBomPagador) {
                $this->totalrec  += pg_fetch_result(
                    $this->recorddadospagto,
                    $ii,
                    $this->valor
                ) + abs($nDescontoLICBomPagadorValor);
            } else {
                $this->totalrec  += pg_fetch_result($this->recorddadospagto, $ii, $this->valor);
            }
        }
        if (!$somouLICBomPagador && pg_fetch_result($this->recorddadospagto, $ii, "k00_hist") != 918) {
            $valores = pg_fetch_result(
                $this->recorddadospagto,
                $ii,
                $this->valor
            ) + abs($nDescontoLICBomPagadorValor);
            $somouLICBomPagador = true;
        } elseif (!$existeLinhaDesconto && pg_fetch_result($this->recorddadospagto, $ii, "k00_hist") == 918) {
            $valores = pg_fetch_result(
                $this->recorddadospagto,
                $ii,
                $this->valor
            ...
            $existeLinhaDesconto = true;
        } else {
            $valores = pg_fetch_result($this->recorddadospagto, $ii, $this->valor);
        }

        $this->objpdf->cell(15, 3, db_formatar($valores, 'f'), 0, 1, "R", 0);

    //IF QUE ESCREVE ÚLTIMA LINHA DE DESCONTO CASO NÃO TENHA SIDO REGISTRADO EM OUTRA LINHA
        if (!$existeLinhaDesconto && ($ii+1 == $this->linhasdadospagto)) {
            if (isset($this->descontoLICBomPagador)) {
                $this->totaldesc += $this->descontoLICBomPagador->j21_valor;

                $this->objpdf->setx($xcol+3+$maiscol);
                $this->objpdf->cell(7, 3, trim($this->descontoLICBomPagador->j21_receit), 0, 0, "R", 0);
                $this->objpdf->cell(
                    9,
                    3,
                    "(".trim(pg_fetch_result($this->recorddadospagto, 0, $this->receitared)).")",
                    0,
                    0,
                    "R",
                    0
                ...
                $this->objpdf->cell(
                    63,
                    3,
                    trim(pg_fetch_result($this->recorddadospagto, 0, $this->ddreceita)." (desconto)"),
                    0,
                    0,
                    "L",
                    0
                );
                $this->objpdf->cell(15, 3, db_formatar($this->descontoLICBomPagador->j21_valor, 'f'), 0, 1, "R", 0);
            }
        }
    }
    $this->objpdf->Roundedrect($xcol, $xlin+71, 140, 30, 2, 'DF', '1234');
    $this->objpdf->SetY($xlin+72);
    $this->objpdf->SetX($xcol+3);

    $this->objpdf->Setfont('Arial', '', 6);
    $this->objpdf->multicell(130, 4, 'HISTÓRICO :    '.$this->historico);
    $this->objpdf->SetX($xcol+3);

    $this->objpdf->Roundedrect($xcol+143.5, $xlin+71, 30, 30, 2, 'DF', '1234');

    if ($pix) {
        $imagemPng = "tmp/pix_arrecadacao_{$this->numpre}".time().".png";
        $url = $pix->k00_qrcode;
        \PHPQRCode\QRcode::png($url, $imagemPng, 'L', 4, 2);
        $this->objpdf->Image($imagemPng, $xcol+144, $xlin+71.5, 29);
    }
...
if ($this->loteamento) {
  ...
  x.k99_desconto,
  x.k00_ano,
  fc_calcula(x.k99_numpre,x.k99_numpar,x.k00_receit,'" . date("Y-m-d", db_getsession("DB_datausu")) . "',
             '" . date("Y-m-d", db_getsession("DB_datausu")) . "', " . db_getsession("DB_anousu") . ")
  ...
  ";
    ...
    global $k00_matric, $j14_nome, $j34_setor, $j34_quadra, $j34_lote, $k99_desconto, $descmul, $descjur, $descvlr;
    global $vlrcor, $vlrjuros, $vlrmulta, $descontovlr, $descontojur, $descontomul, $k00_ano, $fc_calcula;
