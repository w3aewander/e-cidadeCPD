...
ImportacaoDiversos.prototype = {
  _prepararParametros: function () {
    ...
    if (unificarDebitos) {
      ...
      dataVencimento = $F('campoDataVencimento');
    }

    var listaDebitos = $F('k60_codigo');
    var codigoProcesso = '';
    var titularProcesso = '';
    var dataProcesso    = '';
  ...
  processar: function() {
    ...
    if (oParametros) {
      ...
      if (this.importaDividaAtiva === true) {
        ...
        oParametros.aDados.forEach( function (oDados) {
          parametrosDados += virgula + oDados.iCodigoProcedencia + ":" + oDados.iCodigoReceita;
          virgula = ",";
        });

        delete oParametros.aDados;
        delete oParametros.sExec;

        var parametros = "dados="+parametrosDados;

        for(var parametro in oParametros) {
          parametros += "&" + parametro + "=" + oParametros[parametro];
      }
        ...
        js_OpenJanelaIframe(
          'CurrentWindow.corpo',
          'db_iframe_carne',
          'arr4_importacaodebitoscobrancaadministrativageralprocesso.php?'+parametros,
  ...
  pesquisar: function () {
    ...
    var oParametros = {
      ...
      'listaDebitos' : this.iListaDebitos,
  ...
  setListaDebitos: function (listaDebitos) {
