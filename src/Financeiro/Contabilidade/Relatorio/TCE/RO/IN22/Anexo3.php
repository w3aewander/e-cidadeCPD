 ...
 */
class Anexo3 extends Base implements In22
{

    const CODIGO_RELATORIO = 208;
    const TEMPLATE = 'config/templates/IN22/anexo3.xlsx';
    ...
    public function processar()
    {
        ...
        foreach ($dados as $dado) {
            ...
            $this->formataValor($dado->valorpago);
        }
        $parser->addVariable('total', $this->formataValor($total));
        $parser->addCollection('dados', $dados);
        $parser->parse();
        $path = 'tmp/anexo3.xlsx';
        $parser->save($path);
        return $path;
    }
