...
class ObjetivoEstrategicoController extends Controller
{
    ...
    {
        $objetivos = ObjetivoEstrategico::orderBy('pl5_titulo')
        ->when($request->has('area'), function ($query) use ($request) {
            $query->where('pl5_arearesultado', '=', $request->get('area'));
        })
        ->get();

        return new DBJsonResponse($objetivos, 'Objetivos Estratégicos encontrados.');
    }
}
