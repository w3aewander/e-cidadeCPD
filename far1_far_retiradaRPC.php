...
try {
    if ($objParam->exec == 'ConsultaSaldo') {
        ...
        if ($oDadosFar_parametros->fa02_i_numdiasmedcontinativo > 0) {
            ...
            $sSubMovimentacao .= 'join far_retirada on far_retirada.fa04_i_codigo=far_retiradaitens.fa06_i_retirada';
        ...
        if ($clfar_controlemed->numrows > 0) {
            ...
            $validadeContinuado = '';
            if ($oContinuado->fa10_d_datafim !== '') {
                $dFim = explode("-", $oContinuado->fa10_d_datafim);
                $validadeContinuado = $dFim[2] . '/' . $dFim[1] . '/' . $dFim[0];
            }
            $objRetorno->fa10_d_datafim = $validadeContinuado;

            $infoContinuado = MedicamentoContinuadoService::getInfoContinuado(
                $objParam->fa04_i_cgsund,
                $objParam->fa06_i_matersaude,
                new DateTime($hoje2),
                $oDadosFar_parametros->fa02_i_acumularsaldocontinuado == 1,
                $oDadosFar_parametros->fa02_i_tipoperiodocontinuado == 1
            );

            $proximaRetirada = '';
            if ($infoContinuado->proximaRetirada !== null) {
                $proximaRetirada = $infoContinuado->proximaRetirada->format('d/m/Y');
            }
            $objRetorno->prox_data = $proximaRetirada;
            $objRetorno->saldo_atual = $infoContinuado->saldo;
        } else {
        ...
        $sSubQuantidadeSaldo = "(select quantidade_estoque -
...
from (
         ...
         group by m70_codigo) as x) as m70_quant";
        ...
        $sSql = $clmatestoque->sql_query(
            null,
            " $sSubQuantidadeSaldo , $sSubPontoPedido",
            null,
        ...
        if ($clmatestoque->numrows > 0) {
            ...
            $objRetorno->quant_disp = number_format($m70_quant, 0, '', '');
        ...
        $objRetorno->localizacao = [];

        $daoEstoque = new cl_matmaterestoque;
        $where = "m64_matmater = {$fa01_i_codmater}";
        $sql = $daoEstoque->sql_query('', 'm64_localizacao', '', $where);
    ...
    if ($objParam->exec == 'getGridRemedioscont') {
        ...
        if ($objRetorno->status == 2 && $objParam->prescricao != 0) {
            //Obtem todos os medicamentos continuados do paciente
            $strSQL = $clfar_controlemed->sql_query(
                "",
                $strCampos,
                "",
        ...
        if ($clfar_controlemed->numrows > 0) {
            ...
            for ($x = 0; $x < $clfar_controlemed->numrows; $x++) {
                ...
                $objRetorno->itens[$x]->prox_data = $proximaRetirada;
                $saldo_atual = $infoContinuado->saldo;
                ...
                try {
                    ...
                    $oItensMaterial = $oMaterialEstoque->ratearLotes($saldo_atual, null, $departamento);
                    if (count($oItensMaterial) > 0) {
                        $oItemMaterial  = $oItensMaterial[0];

                        $objRetorno->itens[$x]->lote = $oItemMaterial->m71_codlanc;
                        $objRetorno->itens[$x]->label_lote = $oItemMaterial->m77_lote;
                    ...
                    $objRetorno->message = $e->getMessage();
                }
                ...
                $camposSaldo = 'round(coalesce((m70_quant)::numeric, 5), 0) as saldo';
                $whereSaldo = "m70_codmatmater= {$m60_codmater} and m70_coddepto = {$departamento}";
                $saldo = "(select {$camposSaldo} from matestoque  where {$whereSaldo}) as quantidade";

                $sSql = $clmatestoque->sql_query_file("", "{$saldo}, $sSubPontoPedido", "", $sWhere);
                $result_saldo = $clmatestoque->sql_record($sSql);

                if (pg_num_rows($result_saldo) > 0) {
                    $retornoSaldo = db_utils::fieldsMemory($result_saldo, 0);
                    $objRetorno->itens[$x]->saldo_estoque = $retornoSaldo->quantidade;
                    $objRetorno->itens[$x]->m64_pontopedido = (string)$retornoSaldo->m64_pontopedido;
                } else {
                ...
                $objRetorno->itens[$x]->margem = $infoContinuado->margem;

                // Localização no estoque
                $objRetorno->itens[$x]->localizacao = [];

                $daoEstoque = new cl_matmaterestoque;
                $where = "m64_matmater = {$m60_codmater}";
                $sql = $daoEstoque->sql_query('', 'm64_localizacao', '', $where);
                $rs = $daoEstoque->sql_record($sql);
    ...
    if ($objParam->exec == 'Confirma_Remedio') {
        ...
        if(isset($objParam->nomeSocial)){
            $clcgs_und = new cl_cgs_und;
            $clcgs_und->z01_i_cgsund = $cgs;
            $clcgs_und->z01_nome_social = $objParam->nomeSocial;
        ...
        for ($iX = 0; $iX < $iTam; $iX++) {
            ...
            $nome[] = $aItemMed[2];
            $quant[] = $aItemMed[12];
            $posologia[] = $aItemMed[13];

            if ($objParam->iTiporetirada == 1) {
                $lote[] = $aItemMed[14];
                $validade[] = $aItemMed[10];
            }
            ...
            if ($aItemMed[0] == 'A') {
                ...
                if ($clfar_controle->numrows == 0) {
                    ...
                    $clfar_controle->fa11_t_obs = $aItemMed[13];
                ...
                $clfar_controlemed->fa10_i_programa = $aItemMed[18];
        ...
        if ($sqlerro == "N") {
            ...
            $clfar_retirada->fa04_data_prescricao = $objParam->dataPrescricao;
        ...
        if ($sqlerro == "N") {
            ...
            if ($sqlerro == "N" && isset($objParam->sOrigemReceita) && !empty($objParam->sOrigemReceita)) {
                ...
                $oDaofar_origemreceitaretirada = new cl_far_origemreceitaretirada();
                $sSqlOrig = $oDaofar_origemreceita->sql_query(
                    null,
                    'fa40_i_codigo',
                    null,
...
// Obtém o cartão sus a partir de um CGS
    if ($objParam->exec == 'getCgsCns') {
        $oDaocgs_cartaosus = new cl_cgs_cartaosus();

        $sSql = $oDaocgs_cartaosus->sql_query(
            null,
...
function convetremedio($remedio, $material = 0)
{
    ...
    if ($clfar_matersaude->numrows > 0) {
        return db_utils::fieldsMemory($result, 0)->resultado;
    }
    return null;
}
