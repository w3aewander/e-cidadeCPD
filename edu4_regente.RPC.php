  ...
  switch ($oParam->exec) {
    ...
    case 'vincularRegenteDisciplina':
      ...
      db_inicio_transacao();
        ...
        $oGradeHorario->setTipoGrade(PeriodoAula::VINCULAR_PROFESSOR_DISCISPLINA);

        $codigo = $oTurma->getEscola()->getCodigo();
        $sql = "
            select ed17_i_codigo as periodo, ed04_i_diasemana as dia from periodoescola, dialetivo
            where ed17_i_turno = {$oTurma->getTurno()->getCodigoTurno()}
              and ed17_i_escola = {$codigo} and ed04_c_letivo = 'S' and ed04_i_escola = {$codigo}
        ";
        $rs = db_query($sql);

        if (!$rs) {
            throw new Exception("Erro ao buscar os períodos de aula.");
        }

        $periodos = [];
        while ($dados = pg_fetch_array($rs)) {
            foreach ($oParam->aRegencias as $codigoRegencia) {
                $oPeriodoAula = new PeriodoAula();

                $oPeriodoAula->setRegencia(RegenciaRepository::getRegenciaByCodigo($codigoRegencia));
                $oPeriodoAula->setDiaSemana($dados['dia'] - 1);
                $oPeriodoAula->setPeriodoEscola(PeriodoEscolaRepository::getByCodigo($dados['periodo']));
                $oPeriodoAula->setRegente($oParam->iRecHumano);

                $oPeriodoAula->setDataInicio(new DBDate($oParam->dataInicio));
                $oPeriodoAula->setDataFim(new DBDate($oParam->dataFim));
                $oPeriodoAula->setAtivo(true);
                $oPeriodoAula->setTipoVinculo(PeriodoAula::VINCULAR_PROFESSOR_DISCISPLINA);

                $periodos[] = $oPeriodoAula;
            }
        }
        $oGradeHorario->setPeriodosAula($periodos);
    ...
    case 'excluiVinculos':
        ...
        $oDaoRegenciaHorario = new cl_regenciahorario();
        $campos = "ed58_i_codigo as codigo, ed58_i_rechumano as rechumano";
        $where = [
            "ed59_i_turma = {$oParam->iTurma}",
            "ed59_i_serie = {$oParam->iEtapa}",
        ];
        $where = implode(' and ', $where);
        $sql = $oDaoRegenciaHorario->sql_query(null, $campos, null, $where);
        $rs = db_query($sql);

        $turma = new Turma($oParam->iTurma);
        $etapa = new Etapa($oParam->iEtapa);
        if ($rs && pg_num_rows($rs) > 0) {
            while ($oPeriodoAula = pg_fetch_object($rs)) {
                GradeHorarioService::removerRegentePermanente(
                    $turma,
                    $etapa,
                    $oPeriodoAula->codigo,
                    $oPeriodoAula->rechumano
                );
            }
        }
    ...
    case 'getProfessorEscola' :
      ...
      if ( isset($oParam->sNomeProfessorSocial) && !empty($oParam->sNomeProfessorSocial) ) {

        $sProfessoresSocial = db_stdClass::normalizeStringJsonEscapeString($oParam->sNomeProfessorSocial);
        $aWhere[] = " cgmfisico.z04_nomesocial ilike '{$sProfessoresSocial}%' ";
      }
      ...
      $sCampos .= " cgmfisico.z04_nomesocial as z04_nomesocial, ";
