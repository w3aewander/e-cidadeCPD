...
class RequisicaoMaterial
{
    ...
    /*
     * Array que acumula os dados das requisições a terem lancamento contabil processado
     */
    private $envioLancamentosContabeis = [];

    /**
     * Metodo Construtor
     */
    public function __construct($iCodReq)
    {
    ...
    {
        $oDaoMatRequisicao = new cl_matrequi;
    ...
    {
        $sWhere = "m41_codmatrequi = {$this->getIcodReq()}";
        if ($iCodItem != '') {
            $sWhere .= " and m41_codigo = {$iCodItem}";
        } elseif ($iCodigoMaterial != '') {
        ...
        $sSqlreqItens = $oDaoRequisicaoItens->sql_query_estoque_atend_requi(null, $sCampos, 'm60_descr', $sWhere);
        ...
        for ($iInd = 0; $iInd < $oDaoRequisicaoItens->numrows; $iInd++) {
            ...
            $oItem->avisarpontopedido = ControleEstoque::itemEstaNoPontoPedido(
                $oMaterialAlmoxarifado,
                $oAlmoxarifado
            );
    ...
    public function getEnvioLancamentosContabeis()
    {
        return $this->envioLancamentosContabeis;
    }
     ...
     * @throws Exception
     */
    public function atenderRequisicao($iCodtipo, $aItensRequisicao, $iCodAlmox, $iCodAtend = null, $dadosBnafar = null)
    {
        /**
        ...
        $lPossuiIntegracao = ParametroIntegracaoPatrimonial::possuiIntegracaoMaterial(
            $oDataImplantacao,
            $oInstituicao
        );
        ...
        if ($lPossuiIntegracao) {
            ...
            $where .= " and c99_data >= '" . $dData . "'";
            $dao = new cl_condataconf();
            $sql = $dao->sql_query_file(null, null, '*', '', $where);
            $rs = db_query($sql);
            if (pg_num_rows($rs) > 0) {
                $date = date("d/m/Y", db_getsession("DB_datausu"));

                $msg = "Você não pode executar esse procedimento em {$date} pois a contabilidade já executou o ";
                $msg .= "Encerramento do Período Contábil.";
        ...
        if ($dData == $dDataRequi && $tHora < $tHoraRequi) {
        ...
        if (!is_null($dadosBnafar)) {
            EstoqueMovimentacaoBnafarService::salvar($dadosBnafar, $oDaoMatestoqueIni->m80_codigo);
        }

        $sessaoEnvioLancamentosContabeis = [
            'DB_login' => $_SESSION['DB_login'],
            'DB_instit' => $_SESSION['DB_instit'],
            'DB_coddepto' => $_SESSION['DB_coddepto'],
            'DB_id_usuario' => $_SESSION['DB_id_usuario'],
            'DB_Area' => $_SESSION['DB_Area'],
            'DB_modulo' => $_SESSION['DB_modulo'],
            'DB_acessado' => $_SESSION['DB_acessado'],
            'DB_itemmenu_acessado' => $_SESSION['DB_itemmenu_acessado'],
            'DB_ano_pcasp' => $_SESSION['DB_ano_pcasp'],
            'DB_datausu' => $_SESSION['DB_datausu'],
            'DB_uol_hora' => $_SESSION['DB_uol_hora'],
            'DB_anousu' => $_SESSION['DB_anousu'],
            'DB_ip' => $_SESSION['DB_ip'],
            'DB_administrador' => $_SESSION['DB_administrador'],
        ];
        ...
        for ($iInd = 0; $iInd < $iTotItens; $iInd++) {
            ...
            $idMaterial = $oItemAtual->m41_codmatmater;

            if ($aItensRequisicao [$iInd]->nQtde < 0) {
                $sMsgErro = "Erro[-1] - Não Foi possível efetuar o atendimento do material({$idMaterial}).";
            ...
            if ($oDaoAtendrequiItem->erro_status == 0) {
                $sMsgErro = "Erro[3] - Não Foi possível efetuar o atendimento do material({$idMaterial}).";
            ...
            for ($iIndEst = 0; $iIndEst < count($aItemsEstoque); $iIndEst++) {
                ...
                if ($oDaoMatestoqueItem->erro_status == 0) {
                    $sMsgErro = "Erro[5] - Não Foi possível atualizar saldo do estoque do material({$idMaterial}).";
                ...
                if ($oDaoAtendRequiItemMei->erro_status == 0) {
                    $sMsgErro = "Erro[6] - Não Foi possível atualizar saldo do estoque do material({$idMaterial}).";
                ...
                if ($oDaoMatEstoqueIniMei->erro_status == '0') {
                    $sMsgErro = "Erro[7] - Não Foi possível atualizar saldo do estoque do material({$idMaterial}).";
                ...
                if (isset($aItensRequisicao[$iInd]->iCentroDeCusto) && $aItensRequisicao[$iInd]->iCentroDeCusto != "") {
                    ...
                    if ($oDaoCustoApropria->erro_status == 0) {
                        $sMsgErro = "Erro[8] - Não Foi possível apropriar custos do material({$idMaterial}).";
                        throw new Exception($sMsgErro);
                    }
                }
                ...
                if ($oMatEstoqueIniMeiARI->erro_status == 0) {
                    $sMsgErro = "Erro[9] - Não Foi possível atualizar saldo do estoque do material({$idMaterial}).";
                ...
                if (USE_PCASP && $lPossuiIntegracao) {
                    $nValorAtendimentoItem = round(
                        $oMaterialEstoque->getPrecoMedio() * $oDaoMatEstoqueIniMei->m82_quant,
                        2
                    );
                    ...
                    if ($nValorAtendimentoItem >= 0.01) {
                        $this->envioLancamentosContabeis[] = (object) [
                            "requisicaoMaterial" => $this->getIcodReq(),
                            "codigoMaterial" => $oMaterialEstoque->getcodMater(),
                            "codigoLancamentoMaterial" => $oDaoMatEstoqueIniMei->m82_codigo,
                            "valorAtendido" => $nValorAtendimentoItem,
                            "session" => $sessaoEnvioLancamentosContabeis
                        ];
                    }
                }
            ...
            if (round($nTotalAtendido, 2) != round($nTotalIniMei, 2)) {
                ...
                $sMsgErro = "Não existe saldo para atender o item da Requisição: ";
                $sMsgErro .= "{$aItensRequisicao[$iInd]->iCodItemReq}.\n";
        ...
        foreach ($this->envioLancamentosContabeis as $dadosRequisicao) {
            $this->enviaProcessamentoLancamentoContabil($dadosRequisicao);
        }

        return true;
    }

    private function enviaProcessamentoLancamentoContabil($dadosRequisicao)
    {
        ...
        $job = new ProcessarLancamentoContabilRequisicaoJob($dadosRequisicao);
     ...
     * @return bool
     */
    public function getDadosPedidoRequisicao()
    {
        $oDaoMatRequisicao = new cl_matrequi;
        $sSqlMatRequi = $oDaoMatRequisicao->sql_query_almox($this->icodReq);
        $rsMatRequi = $oDaoMatRequisicao->sql_record($sSqlMatRequi);
    ...
    public function getItensPedidoRequisicao($iCodItem = null)
    {
        ...
        $sSqlreqItensanulacao = $oDaoRequisicaoItens->sql_query_estoque_anul(
            null,
            $sCampos,
            null,
            " m70_coddepto = m91_depto and m41_codmatrequi = " . $this->getIcodReq() . " {$sWhere} $sGroupBy"
        );
     ...
     * @throws Exception
     */
    public function processarLancamento(
        MaterialEstoque $oMaterial,
        $iCodigoMovimentacao,
        $nValorLancamento,
        $dtLancamento = null
    ) {
     ...
     * @throws Exception
     */
    public function estornarLancamento(
        MaterialEstoque $oMaterial,
        $iCodigoMovimentacao,
        $nValorLancamento,
        $dtLancamento = null
    ...
    public function anularItemRequisicao($iCodigoItemRequisicao, $nQuantidadeAnulada, $sMotivo = null)
    {
        ...
        $oDaoMatAnulItemRequi = new cl_matanulitemrequi;
