...
class AnexoIV extends \RelatoriosLegaisBase implements InterfaceRelatorioLegal
{
     ...
     * @var \DBDate
     */
    protected $oDataInicialExercicioAnterior;

    /**
     * @var \DBDate
     */
    protected $oDataFinalExercicioAnterior;

    /**
     * @var integer
     */
    protected $iAnoExercicioAnterior;
    ...
    {
        $this->aLinhasConsistencia[51]->dot_ini =
            ($this->aLinhasConsistencia[34]->prev_ini - $this->aLinhasConsistencia[50]->dot_ini);
        $this->aLinhasConsistencia[51]->dot_atual =
            ($this->aLinhasConsistencia[34]->prev_atual - $this->aLinhasConsistencia[50]->dot_atual);
        $this->aLinhasConsistencia[51]->liq_atebim =
            ($this->aLinhasConsistencia[34]->rec_atebim - $this->aLinhasConsistencia[50]->liq_atebim);
        $this->aLinhasConsistencia[51]->liq_atebimexant =
            ($this->aLinhasConsistencia[34]->recbiexant - $this->aLinhasConsistencia[50]->liq_atebimexant);

        $this->aLinhasConsistencia[110]->dot_ini =
            ($this->aLinhasConsistencia[93]->prev_ini - $this->aLinhasConsistencia[109]->dot_ini);
        $this->aLinhasConsistencia[110]->dot_atual =
            ($this->aLinhasConsistencia[93]->prev_atual - $this->aLinhasConsistencia[109]->dot_atual);
        $this->aLinhasConsistencia[110]->liq_atebim =
            ($this->aLinhasConsistencia[93]->rec_atebim - $this->aLinhasConsistencia[109]->liq_atebim);
        $this->aLinhasConsistencia[110]->liq_atebimexant =
            ($this->aLinhasConsistencia[93]->recbiexant - $this->aLinhasConsistencia[109]->liq_atebimexant);
    }

    /**
     * Executa o balancete da receita para o exercicio anterior
     */
    protected function processaBalanceteReceita()
    {
        $sWhereReceita = "o70_instit in ({$this->getInstituicoes()})";
        if ($this->iAnoExercicioAnterior >= 2018) {
        ...
        foreach ($this->aLinhasProcessarReceita as $iLinha) {
             ...
             */
            if (in_array($iLinha, static::LINHAS_IGNORAR_RECEITA)) {
                continue;
            }
     ...
     */
    protected function processaBalanceteDespesa()
    {
        $rsBalanceteDespesa = db_dotacaosaldo(
            ...
            2,
            true,
            " o58_instit in({$this->getInstituicoes()}) ",
            $this->iAnoExercicioAnterior,
            $this->oDataInicialExercicioAnterior->getDate(),
            $this->oDataFinalExercicioAnterior->getDate()
        );
        ...
        foreach ($this->aLinhasProcessarDespesa as $iLinha) {
             ...
             * Neste caso, não deve calcular nada
             */
            if (in_array($iLinha, static::LINHAS_IGNORAR_DESPESA)) {
                continue;
            }
