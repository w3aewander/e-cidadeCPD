...
use ECidade\RecursosHumanos\Pessoal\Factory\DirfArquivoFactory;
 ...
 *  @version $Id: GeracaoArquivoTXTDirf.model.php,v 1.13 2019/04/16 17:24:22 dbjeferson Exp $
 */
  ...
  */
   ...
   */
    public function processar()
    {
        ...
        $sSqlDadosInstituicao = " select distinct z01_cgccpf as cgc, " . PHP_EOL;
        ...
        $sSqlDadosInstituicao.= "                            and rh26_anousu  = o41_anousu  " . PHP_EOL;
        $sSqlDadosInstituicao.= "        inner join rhlota    on r70_codigo   = rh26_codigo " . PHP_EOL;
        $sSqlDadosInstituicao.= "        inner join cgm       on z01_numcgm   = r70_numcgm  " . PHP_EOL;
        $sSqlDadosInstituicao.= "  where o41_anousu = '{$this->iAno} '" . PHP_EOL;
        $sSqlDadosInstituicao.= "    and o41_cnpj   = '{$this->sCnpj}'" . PHP_EOL;
   ...
   */
    public function getRegistros()
    {
        ...
        for ($i = 0; $i < $iTotalLinhas; $i++) {
            ...
            $oPessoa->indicador_identificacao_previdencia_complementar = "N";
            if ($oPessoa->previdencia_privada) {
                $oPessoa->indicador_identificacao_previdencia_complementar = "S";
            }

            $oPessoa->pensionistas            = $this->oDirf->getDadosPensionista($oPessoa, $oTipoReceita->rh98_tipoirrf);
            $oPessoa->indicador_identificacao_alimentando = "N";
            if ($oPessoa->pensionistas) {
                $oPessoa->indicador_identificacao_alimentando = "S";
            }

            $oPessoa->pensionistas_rra        = $this->oDirf->getDadosPensionista($oPessoa, '1889');
            if ($oPessoa->pensionistas_rra) {
            ...
            if ($oTipoReceita->tipopessoa == 11) {
                if (!isset($aLinhasDirf[$oTipoReceita->rh98_tipoirrf]->fisica[$oTipoReceita->rh96_numcgm])) {
                    ...
                    $sSqlMolestias .= "   AND rh02_portadormolestia IS TRUE";
   ...
   */
    ...
    {
        return DirfArquivoFactory::create($iAno, $oLayout);
    }
