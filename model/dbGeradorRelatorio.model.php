...
class dbGeradorRelatorio
{
     ...
     * @throws Exception
     */
    public function __construct($iCodRelatorio = "")
    {
    ...
    private function loadRelatorio($iCodRelatorio)
    {
        ...
        if ($cldb_relatorio->numrows > 0) {
            ...
            foreach ($aPropriedades as $oXMLPropriedades) {
                ...
                $oPropriedades->setVersao(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('versao'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setFormato(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('formato'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setLayout(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('layout'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setMargemDir(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('margemdir'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setMargemEsq(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('margemesq'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setMargemInf(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('margeminf'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setMargemSup(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('margemsup'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setNome(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('nome'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setOrientacao(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('orientacao'), 'ISO-8859-1', 'UTF-8')
                );
                $oPropriedades->setTipoSaida(
                    mb_convert_encoding($oXMLPropriedades->getAttribute('tiposaida'), 'ISO-8859-1', 'UTF-8')
                );
                $delimitarTexto = true;
                if ($oXMLPropriedades->hasAttribute('delimitartexto')) {
                    $delimitarTexto = $oXMLPropriedades->getAttribute('delimitartexto');
                }
                $oPropriedades->setDelimitarTexto($delimitarTexto);

                $imprimirCabecalho = true;
                if ($oXMLPropriedades->hasAttribute('imprimircabecalho')) {
                    $imprimirCabecalho = $oXMLPropriedades->getAttribute('imprimircabecalho');
                }
                $oPropriedades->setImprimirCabecalho($imprimirCabecalho);
            ...
            if (!empty($aVariavel)) {
                foreach ($aVariavel as $oXMLVariavel) {
                    ...
                    $oVariavel->setNome(
                        mb_convert_encoding($oXMLVariavel->getAttribute('nome'), 'ISO-8859-1', 'UTF-8')
                    );
                    $oVariavel->setLabel(
                        mb_convert_encoding($oXMLVariavel->getAttribute('label'), 'ISO-8859-1', 'UTF-8')
                    );
                    $oVariavel->setValor(
                        mb_convert_encoding($oXMLVariavel->getAttribute('valor'), 'ISO-8859-1', 'UTF-8')
                    );
                    $oVariavel->setTipoDado(
                        mb_convert_encoding($oXMLVariavel->getAttribute('tipodado'), 'ISO-8859-1', 'UTF-8')
                    );
                    $oVariavel->setSql(
                        mb_convert_encoding($oXMLVariavel->getAttribute('sql'), 'ISO-8859-1', 'UTF-8')
                    );
                    $this->addVariavel(
                        mb_convert_encoding($oXMLVariavel->getAttribute('nome'), 'ISO-8859-1', 'UTF-8'),
                        $oVariavel
            ...
            foreach ($aConsulta as $oXMLConsulta) {
                        ...
                        foreach ($aCampos as $oXMLCampos) {
                            ...
                            foreach ($aCampo as $oXMLCampo) {
                                if ($oXMLCampo->getAttribute('id') == $oXMLCampoSelect->getAttribute('id')) {
                                    ...
                                    $oCampo->setNome(
                                        mb_convert_encoding($oXMLCampo->getAttribute('nome'), 'ISO-8859-1', 'UTF-8')
                                    );
                                    $oCampo->setAlias(
                                        mb_convert_encoding($oXMLCampo->getAttribute('alias'), 'ISO-8859-1', 'UTF-8')
                                    );
                                    $oCampo->setAlinhamento(
                                        mb_convert_encoding(
                                            $oXMLCampo->getAttribute('alinhamento'),
                                            'ISO-8859-1',
                                            'UTF-8'
                                        )
                                    );
                                    $oCampo->setAlinhamentoCab(
                                        mb_convert_encoding(
                                            $oXMLCampo->getAttribute('alinhamentocab'),
                                            'ISO-8859-1',
                                            'UTF-8'
                                        )
                                    );
                                    $oCampo->setLargura(
                                        mb_convert_encoding($oXMLCampo->getAttribute('largura'), 'ISO-8859-1', 'UTF-8')
                                    );
                                    $oCampo->setMascara(
                                        mb_convert_encoding($oXMLCampo->getAttribute('mascara'), 'ISO-8859-1', 'UTF-8')
                                    );
                                    $oCampo->setTotalizar(
                                        mb_convert_encoding(
                                            $oXMLCampo->getAttribute('totalizar'),
                                            'ISO-8859-1',
                                            'UTF-8'
                                        )
                                    );
                ...
                foreach ($aWhere as $oXMLWhere) {
                    ...
                    $oFiltro->setOperador(
                        mb_convert_encoding($oXMLWhere->getAttribute('operador'), 'ISO-8859-1', 'UTF-8')
                    );
                    $oFiltro->setCampo(
                        mb_convert_encoding($oXMLWhere->getAttribute('campo'), 'ISO-8859-1', 'UTF-8')
                    );
                    $oFiltro->setCondicao(
                        mb_convert_encoding($oXMLWhere->getAttribute('condicao'), 'ISO-8859-1', 'UTF-8')
                    );
                    $oFiltro->setValor(
                        mb_convert_encoding($oXMLWhere->getAttribute('valor'), 'ISO-8859-1', 'UTF-8')
                    );
                ...
                foreach ($aOrder as $oXMLOrder) {
                    ...
                    $oOrdem->setNome(mb_convert_encoding($oXMLOrder->getAttribute('nome'), 'ISO-8859-1', 'UTF-8'));
                    $oOrdem->setAscDesc(
                        mb_convert_encoding($oXMLOrder->getAttribute('ascdesc'), 'ISO-8859-1', 'UTF-8')
                    );
                    $oOrdem->setAlias(mb_convert_encoding($oXMLOrder->getAttribute('alias'), 'ISO-8859-1', 'UTF-8'));
            ...
            if ($iNroLinhas > 0) {
                ...
                for ($iInd = 0; $iInd < $iNroLinhas; $iInd++) {
                    ...
                    if ($oCampos->conteudo == "float4" || $oCampos->conteudo == "float8") {
                        $sAlinhamento = "r";
                    } elseif (substr(trim($oCampos->conteudo), 0, 7) == "varchar") {
                        $sAlinhamento = "l";
                    } else {
                        $sAlinhamento = "c";
                    }
    ...
    /**
     * @param dbPropriedadeRelatorio $oPropriedade
     * @return void
     * @throws Exception
     */
    ...
    /**
     * @param $sNome
     * @param dbVariaveisRelatorio $oVariavel
     * @return void
     * @throws Exception
     */
    ...
    /**
     * @param dbColunaRelatorio $oColuna
     * @param $sTipoRel
     * @return void
     * @throws Exception
     */
    ...
    /**
     * @param dbFiltroRelatorio $oFiltro
     * @param $sTipoRel
     * @return void
     * @throws Exception
     */
    ...
    public function converteAgt($sXml)
    {
        ...
        foreach ($aPropriedades as $oPropriedades) {
            ...
            $oXmlWriter->writeElement(
                "Title",
                mb_convert_encoding($oPropriedades->getAttribute('nome'), 'ISO-8859-1', 'UTF-8')
            );
            ...
            $oXmlWriter->writeElement(
                "Layout",
                mb_convert_encoding($oPropriedades->getAttribute('layout'), 'ISO-8859-1', 'UTF-8')
            );
        ...
        if (!empty($aVariavel)) {
            ...
            foreach ($aVariavel as $oVariavel) {
                $oXmlWriter->startElement(
                    str_replace("$", "", mb_convert_encoding($oVariavel->getAttribute('nome'), 'ISO-8859-1', 'UTF-8'))
                );
            ...
            if ($oConsulta->getAttribute('tipo') == "Principal") {
                ...
                foreach ($aCampo as $oCampo) {
                    ...
                    if ($oCampo->getAttribute('totalizar') != "n") {
                        ...
                        } elseif ($oCampo->getAttribute('totalizar') == "q") {
                            $aTotalizador[] = "count({$iIndiceCampo})";
                        }
                    }

                    if ($oCampo->getAttribute('quebra') == "1" || $oCampo->getAttribute('quebra') == "true") {
                ...
                $oXmlWriter->writeElement("Select", mb_convert_encoding(implode(",", $sSelect), 'ISO-8859-1', 'UTF-8'));
                $oXmlWriter->writeElement("From", mb_convert_encoding($sFrom, 'ISO-8859-1', 'UTF-8'));
                $oXmlWriter->writeElement("Where", mb_convert_encoding(implode(" ", $sWhere), 'ISO-8859-1', 'UTF-8'));
                $oXmlWriter->writeElement("GroupBy", mb_convert_encoding(implode(",", $sGroup), 'ISO-8859-1', 'UTF-8'));
                $oXmlWriter->writeElement("OrderBy", mb_convert_encoding(implode(",", $sOrder), 'ISO-8859-1', 'UTF-8'));
                ...
                $oXmlWriter->writeElement("ShowNumber", "");
    ...
    public static function importar(DOMDocument $oDomDocument, $iGrupoRelatorio, $iTipoRelatorio)
    {
        if (empty($iGrupoRelatorio)) {
            throw new ParameterException(
                _M('configuracao.configuracao.dbGeradorRelatorio.erro_codigo_grupo_relatorio')
            );
        }
    ...
    /**
     * @param DOMElement $oDomElementoRelatorio
     * @return true
        ...
        foreach ($aCamposXML as $oCampo) {
            if (!$oCampo->hasAttribute('id')) {
                throw new FileException(
                    _M('configuracao.configuracao.dbGeradorRelatorio.erro_xml_node_sem_atributo_id')
                );
            }
    ...
    public function gerarRelatorio($sNomeArquivo = null, $aParametros = array())
    {
            ...
            foreach ($aVariaveisGerador as $sIndVar => $oVariavelGerador) {
                foreach ($aParametros as $iInd => $oParamentro) {
                    if ($oParamentro->sNome == $oVariavelGerador->getNome()) {
                        ...
                        $oAgataApi->setParameter(
                            $oParamentro->sNome,
                            mb_convert_encoding($sValor, 'ISO-8859-1', 'UTF-8')
                        );
                    }
                }
            }
        ...
        $sNomeRelatorio = "";
        // Verifica o tipo de relatório 1-Relatório,
        //2-Documento Template e utiliza o método da API do Agata referente ao tipo
        if ($this->iTipoRelatorio == 2) {
            ...
            $sNomeRelatorio = sprintf(
                "tmp/geraRelatorio%s%s.%s",
                date("YmdHis"),
