...
class AutenticacaoBaixaBanco
{
    ...
    public function __construct($iCodigoClassificacao)
    {
        ...
        $this->use_pcasp = ($_SESSION["DB_use_pcasp"] == "t" ? true : false);
    }
    ...
    protected function executarLancamentoContabeis($lDesconto = false, $lArrecadaDesconto = false)
    {
        ...
        $sSqlDisrec = $oDaoDisrec->$sNomeMetodo(
            null,
        ...
        for ($rec = 0; $rec < $iReceitas; $rec++) {
            ...
            $oReceitaContabil->setTef($this->getTef()); // para pagamentos via TEF deverá executar o doc 165
            if ($lArrecadaDesconto) {
                ...
                $receitaContabilRetorno = $oContaPlano->getReceitaContabil();
                $oReceitaContabil = $receitaContabilRetorno instanceof ReceitaContabil ? $receitaContabilRetorno : $oReceitaContabil;
                $lDesconto = false;
            }
            $oReceitaContabil->processaLancamentosReceita(
                abs($oDadoSqlGeral->total_receita),
    ...
    public function processaArrecadacaoReceitaExtraOrcamentaria()
    {
        ...
        $oDaoDisrec = db_utils::getDao("disrec");
        $sSqlBuscaReceita = $oDaoDisrec->sql_query_receita_extra(
            null,
    ...
    public function processaArrecadacaoReceitaExtraOrcamentariaPrestacaoContas()
    {
        ...
        $oDaoDisrec = db_utils::getDao("disrec");
        $sSqlBuscaReceita = $oDaoDisrec->sql_query_receita_extra_prestacao_conta(
            null,
            "corrente.k12_id,
                                                            corrente.k12_data,
                                                            corrente.k12_autent,
