...
class Menu extends Model
{
    ...
    public function getInstituicoes()
    {
        ...
        $sSqlInstituicoes  = " select c.codigo as id, c.codigo||' - '||c.nomeinst as nome, db21_tipoinstit as tipo_instit ";
        ...
        if ($_SESSION["DB_id_usuario"] == "1" || $_SESSION['DB_administrador'] == "1") {
            $sSqlInstituicoes  = "select codigo as id, codigo||' - '||nomeinst as nome, db21_tipoinstit as tipo_instit                    ";
   ...
   */
    public function getMenuArquivo(Request $request, $path)
    {
        ...
        $file = str_contains($path, 'web/') ? trim($path) : trim(addslashes(basename($path)));
        $where = null;
        ...
        $sql = "select db_itensmenu.id_item
              ...
              inner join db_menu  on  db_menu.id_item_filho = db_itensmenu.id_item
                    where trim(funcao) = '".$file."' and libcliente = true
              $where limit 1 ";
