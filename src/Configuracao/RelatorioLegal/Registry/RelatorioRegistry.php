...
class RelatorioRegistry
{
    ...
    public static function get($key)
    {
        if (!array_key_exists($key, self::$storage)) {
            $resultado = RelatorioRepositorio::find($key);
            if ($resultado) {
                self::set($resultado);
            } else {
                return null;
     ...
     */
    public static function set(Relatorio $relatorio)
    {
        self::$storage[$relatorio->getSequencial()] = $relatorio;
    }
}
