...
switch ($oParam->exec) {
    ...
    case "getCompilacao":
        ...
        $sCamposSolicita .= "l20_numero || '/' || l20_anousu AS numero_ano,";
        $sCamposSolicita .= '(SELECT l03_descr FROM cflicita WHERE l03_codigo = l20_codtipocom) AS l03_descr';
        $sSqlSolicitaCompilacao = $oDaoSolicita->sql_query_compilacao(
            null,
            $sCamposSolicita,
            "pc10_numero desc",
            $sWhere,
    ...
    case "getDepartamento":
        ...
        $oRetorno->compilacao = $oParam->iCompilacao;
    ...
    case "getItens":
        ...
        foreach ($aItensEstimativa as $oItem) {
            ...
            $oDadoItem->fornecedor = '';

            $itemFornecedores = $oItem->getFornecedorItem($oParam->iCodCompilacao, $oItem->getCodigoMaterial());
            if (array_key_exists((int)$oItem->getOrdem(), $itemFornecedores)) {
                $oDadoItem->fornecedor = $itemFornecedores[(int)$oItem->getOrdem()]->codigofornecedor;
            }
