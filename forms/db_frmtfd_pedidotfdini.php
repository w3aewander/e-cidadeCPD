        ...
        <table class="form-container">
          ...
          <tr>
            ...
            <td>
              ...
              db_input('z01_v_nome', 72, $Iz01_v_nome, true, 'text', 1, ' onchange="verificaNomePaciente();" ');
              ?>
            </td>
          </tr>
            ...
            <fieldset class="separator">
              ...
              <table class="form-container">
                ...
                <tr>
                  ...
                  <td>
                    <?php
                    db_input('z01_v_ident', 12, $Iz01_v_ident, true, 'text', $db_opcao, 'onchange="js_change();"', '', '', '', 11);
                    ?>
                  </td>
                </tr>
...
// Autocomplete do CGS
let oAutoComplete = new dbAutoComplete($('z01_v_nome'), 'sau4_pesquisanome.RPC.php?tipo=1');
oAutoComplete.setHeightList(390);
oAutoComplete.show();
oAutoComplete.setCallBackFunction(function (cgsUnd, nomePaciente, resposta) {
    $('tf01_i_cgsund').value = cgsUnd;
    $('z01_v_nome').value = nomePaciente.urlDecode();
    if(cgsUnd.length > 0){
      js_pesquisatf01_i_cgsund(false);
    }
});
...
function js_cria_datagrid() {
  ...
  oDBGrid.setCellWidth(new Array('4%', '7%', '3%', '6%', '21%', '30%', '15%', '7%', '4%'));
  oDBGrid.setHeight(150);
  oDBGrid.allowSelectColumns(false);

  var aHeader = new Array();
  aHeader[0]  = 'Pedido';
  aHeader[1]  = 'Data Cons/Ex';
  aHeader[2]  = 'Hora';
  aHeader[3]  = 'Data Saída';
  aHeader[4]  = 'Tratamento';
...
function js_retornogetPedidosTfdCgs( oRetorno ) {
  ...
  if ( oRetorno.iStatus != 1 ) {
    ...
    oRetorno.oPedidos.each(
      ...
      function ( oPedidosTfd ) {
        ...
        aLinha[2] = oPedidosTfd.tf16_c_horaagendamento;
        aLinha[3] = js_formataData(oPedidosTfd.tf17_d_datasaida);
        aLinha[4] = `${oPedidosTfd.tf04_c_descr} - ${oPedidosTfd.sd63_c_nome}`;
        aLinha[5] = oPedidosTfd.z01_nomeprestadora.urlDecode();
        aLinha[6] = oPedidosTfd.tf03_c_descr.urlDecode();
        aLinha[7] = oPedidosTfd.tf26_c_descr.urlDecode();
        aLinha[8] = sAcao;
  ...
  oRetorno.oPedidos.each(function(oPedido, iLinha) {
    ...
    oDBGridPedidostfd.setHint(iLinha, 4, `${oPedido.tf04_c_descr} - ${oPedido.sd63_c_nome}`, oParametros);
  });
}
...
function js_retornogetInfoCgs( oRetorno ) {
  ...
  let msgConfirmacaoMunicipio = "O paciente reside em outro município. ";
  msgConfirmacaoMunicipio += "Cadastrado em: "+oRetorno.municipioPaciente+". ";
  msgConfirmacaoMunicipio += "Deseja realmente continuar o pedido?";
  if(
    oRetorno.residenciaMunicipio == false && 
    !confirm(msgConfirmacaoMunicipio)
  ){    
    js_limpaInfoCgs();          
...
function validaRg() {
    let campoRg = document.getElementById('z01_v_ident');
...
function verificaNomePaciente(){
  if($('z01_v_nome').value.length == 0){
    js_limpaInfoCgs();
  }
}
</script>
