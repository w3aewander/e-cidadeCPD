 ...
 *
 * Adicionado parâmetro para selecionar após pesquisar...
 *    oLancador.selecionarAposPesquisar = true;
 * @param {String} Nome
 * @constructor
 */
DBLancador = function( sName ) {
  ...
  this.lAdicionarItensPrimeiraPosicao = false;
  this.lConfirmarRemocaoRegistro = false;
  this.sMensagemConfirmacao = "";
  this.withIcon = false;
  this.selecionarAposPesquisar = false;
  ...
  /**
   * Callback para executar depois de selecionado os registros da ancora
   * @param fCallbackSelecao 
   */
  this.setCallbackSelecao = function(fCallbackSelecao) {
    this.fCallbackSelecao = fCallbackSelecao;
  }
   ...
   * @return {boolean}
   */
  this.adicionarRegistro = function ( sCodigo, sDescricao, sClasse, desabilitarBotaoRemover) {
    ...
    oRegistro.desabilitarBotaoRemover = (typeof desabilitarBotaoRemover === "undefined") ? false : desabilitarBotaoRemover;
  ...
  this.removerRegistro = function ( sCodigo ) {
    ...
    var mensagemConfirmacao = 'Confirma a remoção do registro ' + sCodigo + ' ?';
    if (this.sMensagemConfirmacao !== "") {
        mensagemConfirmacao = this.sMensagemConfirmacao;
    }

    if(oSelf.lConfirmarRemocaoRegistro && !confirm(mensagemConfirmacao)) {
      return false;
    }
...
DBLancador.prototype.criarElementosBasicos = function () {
  ...
  oElementos.oFieldSet.id        = "flsd"+me.sName;
  oElementos.oLegend             = document.createElement('legend');
  oElementos.oLegend.innerHTML   = this.sTextoFieldset;
...
DBLancador.prototype.adicionarComponentes = function() {
  ...
  oAncora.onClick(function () {
    ...
    me.getParametros().each(function(oParametro) {
      ...
      const janela = js_OpenJanelaIframe('', sIframe, sQuery, me.sTituloJanela,
          true);
      janela.setAltura('calc(100% - 10px)');
      janela.setLargura('calc(100% - 10px)');
      janela.setPosY('5%');
      janela.focus();
  });
  ...
  oBotao.onclick = function() {
    ...
    const codigo    = me.oElementos.oInputCodigo.value;
    const descricao = me.oElementos.oInputDescricao.value;

    me.oElementos.oInputCodigo.value    = "";
    me.oElementos.oInputDescricao.value = "";

    if (me.fCallbackBotao) {
      me.fCallbackBotao(codigo, descricao);
    }
  };
...
DBLancador.prototype.criaGridLancador = function() {
    ...
    var aLargura = ['70px', '330px', '75px'];
    var aAlinhamento = ['center', 'left', 'center'];
    var aDescricao = ['Código', 'Descrição', 'Ação'];
...
DBLancador.prototype.renderizarRegistros = function() {
  ...
  for ( var sIdRegistro in oRegistros ) {
    ...
    if (this.withIcon) {
        oBotaoRemover = document.createElement('button');
        oBotaoRemover.classList.add('button');

        const icon = document.createElement('i');
        icon.classList.add('fas');
        icon.classList.add('fa-trash');
        oBotaoRemover.appendChild(icon);
    }

    if (!me.lHabilitado || oRegistro.desabilitarBotaoRemover) {
  ...
  if(me.lAdicionarItensPrimeiraPosicao) {
    oDataGrid.aRows.reverse();
  }
...
DBLancador.prototype.retornoPesquisaLookUp = function(sCodigo, sDescricao) {
  ...
  if (this.selecionarAposPesquisar) {
      document.getElementById(this.sIdBotaoAdicionar).dispatchEvent(new Event('click'));
  }

  if (this.fCallbackSelecao) {
    this.fCallbackSelecao();
  }
};
...
/**
 * Por padrão, os itens na grid são adicionados um abaixo do outro. Setando true, os itens adicionados, serão os primeiros
 * na grid
 * @param lAdicionarItensPrimeiraPosicao
 */
DBLancador.prototype.adicionarItensPrimeiraPosicao = function(lAdicionarItensPrimeiraPosicao) {
  this.lAdicionarItensPrimeiraPosicao = lAdicionarItensPrimeiraPosicao;
};

/**
 * Adicionar uma confirmação do registro na grid
 * @param lConfirmarRemocaoRegistro
 */
DBLancador.prototype.confirmarRemocaoRegistro = function(lConfirmarRemocaoRegistro) {
  this.lConfirmarRemocaoRegistro = lConfirmarRemocaoRegistro;
};

/**
 * Set uma mensagem de confirmação de exclusão do registro na grid
 * @param sMensagemConfirmacao
 */
DBLancador.prototype.setMensagemConfirmacao = function(sMensagemConfirmacao) {
    this.sMensagemConfirmacao = sMensagemConfirmacao;
};
