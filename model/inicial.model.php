...
class inicial
{
    ...
    public function excluiCertidaoInicial($iCertidao = "", $iInicial = "")
    {
        ...
        if ($oDaoInicialCert->numrows > 1) {
            ...
            for ($iInd = 0; $iInd < $iLinhasNumpres; $iInd++) {
                ...
                $oDaoInicialNumpre->excluir(null,
                  " v59_inicial = {$iInicial} and v59_numpre = {$oNumpresInicial->v01_numpre} ");
    ...
    public function anulaInicial($iInicial = "", $iCodSituacao = "", $sObservacao = null)
    {
        ...
        if (!empty($sObservacao)) {
            $this->setObservacaoMovimentacao($sObservacao);
        }
        ...
        $sSqlTermoini = $oDaoTermoini->sql_query(null, null, "*, v07_situacao", null,
          "inicial = {$iInicial} and v07_situacao in (1, 3)");
        ...
        $rsForo = $oDaoProcessoForoInicial->sql_record($oDaoProcessoForoInicial->sql_query_file("", "*", "",
          " v71_inicial = {$iInicial}"));
        ...
        /* Implementado na classe, pois estava fazendo 2 updates
        ...
        */
        ...
        $oDaoInicialmov->atuinicialmov($iInicial, $iCodSituacao, 2);
        ...
        $rsProcuraCertidao = $oDaoInicialCert->sql_record($oDaoInicialCert->sql_query_file(null, null, "v51_certidao",
          null, " v51_inicial = {$iInicial} "));
    ...
    public function salvar()
    {
        ...
        $this->adicionarHonorarioParcelamento();
    ...
    public function adicionarHonorarioParcelamento()
    {
        ...
        $honorarioParcelamentoRepository = HonorarioParcelamentoRepository::getInstance();

        $inicialModel = new InicialModel();
        $inicialModel->setCodigo($this->iCodigoInicial);;

        if ($honorarioParcelamentoRepository->getByInicial($inicialModel) !== null) {
            return false;
        }

        $honorarioParcelamentoModel = new HonorarioParcelamentoModel();
        $honorarioParcelamentoModel->setInicial($inicialModel);
        $honorarioParcelamentoModel->setNumeroParcelas(1);

        $honorarioParcelamentoRepository->persist($honorarioParcelamentoModel);
    ...
    public function excluirMovimentacao($iCodigoMovimentacao)
    {
        ...
        $sSqlInicialMov = $oDaoInicialMov->sql_query_file(null, "coalesce(max(v56_codmov), 0) as codigo_movimentacao",
          null, "v56_inicial = {$this->getCodigoInicial()}");
