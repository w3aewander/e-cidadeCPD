...
class endereco
{
    ...
    function __construct($iCodigoEndereco = null)
    {
        ...
        if ($iCodigoEndereco != null) {
            ...
            $this->setSiglaRua($oDadosEndereco->j88_sigla);
    ...
    /**
     * Retorna a sigla da rua
     * @return string
     */
    public function getSiglaRua()
    {
        return $this->rua_sigla;
    }

    /**
     * Seta a sigla da rua referente ao endereço
     * @param string $rua_sigla
     */
    public function setSiglaRua($rua_sigla)
    {
        $this->rua_sigla = $rua_sigla;
    }
    ...
    static function findCep($sCep, $sNomeBairro = null)
    {
        ...
        if (!empty($sCep)) {
            ...
            if ($sNomeBairro != null) {
                ...
                $sWhere .= " and Translate(cadenderbairro.db73_descricao, 'áéíóúàèìòùãõâêîôôäëïöüçÁÉÍÓÚÀÈÌÒÙÃÕÂÊÎÔÛÄËÏÖÜÇ', 'aeiouaeiouaoaeiooaeioucAEIOUAEIOUAOAEIOOAEIOUC') = Translate($2,'áéíóúàèìòùãõâêîôôäëïöüçÁÉÍÓÚÀÈÌÒÙÃÕÂÊÎÔÛÄËÏÖÜÇ','aeiouaeiouaoaeiooaeioucAEIOUAEIOUAOAEIOOAEIOUC')";
            }
    ...
    static function findMunicipioByCodigo($iCodigoMunicipio, $iCodigoEstado)
    {
        ...
        if (!empty($iCodigoMunicipio) && !empty($iCodigoEstado)) {
            ...
            $sQueryMunicipio = $oDaoMunicipio->sql_query(null, "db72_descricao", null, $sWhere);
    ...
    static function findEnderecoByCep($cep, $condicoes = [], $limit = 0)
    {
        $condicoes[] = "db86_cep = '{$cep}'";

        $cadenderruacep = new cl_cadenderruacep;
        $sql = $cadenderruacep->sql_query_endereco_by_cep("*", $condicoes);
        if (!empty($limit)) {
            $sql .= "LIMIT {$limit}";
        }
        $result = $cadenderruacep->sql_record($sql);

        if (!empty($result)) {
            $result = db_utils::getCollectionByRecord($result);
        }
    ...
    static function findMunicipioByEstado($iCodigoEstado)
    {
        ...
        if (!empty($iCodigoEstado)) {
            ...
            $oDaoMunicipio = db_utils::getDao('cadendermunicipio');
            $sCampos = "select max(db72_sequencial) as codigo, db72_descricao as descricao ";
            $sGroupOrder = "group by db72_descricao ";
            $sGroupOrder .= "order by db72_descricao ";
            $sWhere = "db72_cadenderestado = " . $iCodigoEstado . " ";
            $sWhere .= "and db72_descricao != ''";

            $sSqlMunicipio = $oDaoMunicipio->sql_query(null, "db72_sequencial, upper(trim(db72_descricao)) as db72_descricao", null, $sWhere);

            $sSqlMunicipio2 = $sCampos . " from ({$sSqlMunicipio}) as x " . $sGroupOrder;
            $rsMunicipio = $oDaoMunicipio->sql_record($sSqlMunicipio2);
    ...
    static function findMunicipioByName($sName, $iCodigoEstado, $sAlias = false)
    {
        ...
        if (!empty($sName) && !empty($iCodigoEstado)) {
            ...
            $sCampos = "max(db72_sequencial) as db72_sequencial, db72_descricao";
            if ($sAlias) {
                $sCampos = "max(db72_sequencial) as cod, db72_descricao as label";
            }

            $sQueryMunicipio2 = "select {$sCampos} from ({$sQueryMunicipio}) as x ";
            $sQueryMunicipio2 .= "group by db72_descricao";
            $sQueryMunicipio2 .= "order by db72_descricao";

            $rsQueryMunicipio = $oDaoMunicipio->sql_record($sQueryMunicipio2);
    ...
    static function findBairroByName($sName, $iCodigoEstado, $iCodigoMunicipio, $sAlias = false)
    {
        ...
        if (!empty($sName)) {
            ...
            $sCampos = "to_ascii(upper(trim( replace(replace(replace(db73_descricao,'.',' '), ',',' '), '''', ' ') ))) AS db73_descricao, ";
            $sCampos .= "db73_sequencial, upper(trim(db72_descricao)) as db72_descricao ";

            $sQueryBairro = $oDaoBairro->sql_query(null, $sCampos, null, $sWhere);

            $sQueryBairro2 = "select max(x.db73_sequencial) AS db73_sequencial ";
            if ($sAlias) {
                $sQueryBairro2 .= ", db72_descricao ";
            }
            $sQueryBairro2 .= "from ({$sQueryBairro}) as x ";

            if ($sAlias) {
                $sQueryBairro2 .= "group by db73_descricao, db72_descricao ";
            } else {
                $sQueryBairro2 .= "group by db73_descricao ";
            }

            $sQueryBairro3 = "select z.db73_sequencial, cadenderbairro.db73_descricao ";
            if ($sAlias) {
                $sQueryBairro3 = "select z.db73_sequencial as cod, cadenderbairro.db73_descricao||' - '||db72_descricao as label ";
            }
            $sQueryBairro3 .= "from ($sQueryBairro2) as z inner join cadenderbairro ON cadenderbairro.db73_sequencial = z.db73_sequencial ";
            $sQueryBairro3 .= "order by cadenderbairro.db73_descricao ";

            $rsQueryBairro = $oDaoBairro->sql_record($sQueryBairro3);
    ...
    static function findCgmEnderecoByCgm($iNumCgm)
    {
        ...
        $sQueryCgmEndereco = $oDaoCgmEndereco->sql_query_cgmendereco(null, $sCampos, 'z07_tipo desc, z07_sequencial desc', $sWhere);
     ...
     * @param cep string
     */
    public static function getCodigoExternoSistemaByCep($cep, $matricula='')
    {
        if (empty($cep) || $cep == "") {
            return "";
        }
        ...
        $sql = "
            ...
            from
                ...
                inner join configuracoes.cadendermunicipiosistema
            ...
            where
                {$cep}::int between
                REGEXP_REPLACE(db72_cepinicial,'[^a-zA-Y0-9 ]','','g')::int and
                REGEXP_REPLACE(db72_cepfinal,'[^a-zA-Y0-9 ]','','g')::int
                limit 1";
        $rs = db_query($sql);
        
        if (!$rs) {
            throw new DBException("Ocorrêu um erro ao procurar o cep {$cep}.");
        }
        ...
        if (pg_num_rows($rs) == 0) { 
            ...
            $msg = "O CEP:<b>($cep)</b> ";
            if (!empty($matricula)) {
                $msg .= "da matrícula<b>($matricula)</b> ";
            }

            throw new DBException($msg."não foi localizado.");
        }

        if (pg_num_rows($rs) > 0) {
            return db_utils::fieldsmemory($rs, 0)->db125_codigosistema;
        }
        return "";
    }
}
