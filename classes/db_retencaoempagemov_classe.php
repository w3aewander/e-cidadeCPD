...
class cl_retencaoempagemov
{
    ...
    public $e27_sequencial = 0;
    public $e27_retencaoreceitas = 0;
    public $e27_empagemov = 0;
    public $e27_principal = 'f';
    ...
    public function incluir($e27_sequencial)
    {
        $this->atualizacampos();
        if ($this->e27_retencaoreceitas == null) {
            $this->erro_sql = " Campo Código da Retencao não informado.";
        ...
        if ($this->e27_principal == null) {
            $this->erro_sql = " Campo Retenção Principal não informado.";
        ...
        if (($this->e27_sequencial == null) || ($this->e27_sequencial == "")) {
            $this->erro_sql = " Campo e27_sequencial não declarado.";
        ...
        if ($result == false) {
            ...
            if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
                $this->erro_sql = "Movimento da retencao ($this->e27_sequencial) não Incluído. Inclusão Abortada.";
                $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
    ...
    public function alterar($e27_sequencial = null, $where = null)
    {
        $this->atualizacampos();
        ...
        if ($e27_sequencial != null) {
            $filtro = " e27_sequencial = $this->e27_sequencial";
        } elseif (!empty($where)) {
            $filtro = " $where";
        }
        ...
        $lSessaoDesativarAccount = db_getsession("DB_desativar_account", false);
        if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
                && ($lSessaoDesativarAccount === false)) && !empty($filtro)) {

            $resaco = $this->sql_record($this->sql_query_file(null, '*', null, $filtro));
            ...
            if ($this->numrows > 0) {
                ...
                for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
                    ...
                    if (isset($GLOBALS["HTTP_POST_VARS"]["e27_sequencial"]) || $this->e27_sequencial != "")
                    ...
                    if (isset($GLOBALS["HTTP_POST_VARS"]["e27_retencaoreceitas"]) || $this->e27_retencaoreceitas != "")
                    ...
                    if (isset($GLOBALS["HTTP_POST_VARS"]["e27_empagemov"]) || $this->e27_empagemov != "")
                    ...
                    if (isset($GLOBALS["HTTP_POST_VARS"]["e27_principal"]) || $this->e27_principal != "")
        ...
        if (!$result) {
            $this->erro_banco = str_replace("\n", "", @pg_last_error());
            $this->erro_sql = "Movimento da retencao não Alterado. Alteração Abortada.\\n";
            $this->erro_sql .= "Valores : " . $this->e27_sequencial;
            $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
            $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
            $this->erro_status = "0";
            ...
            if (pg_affected_rows($result) == 0) {
                $this->erro_banco = "";
                $this->erro_sql = "Movimento da retencao não foi Alterado. Alteração Executada.\\n";
                $this->erro_sql .= "Valores : " . $this->e27_sequencial;
                $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
                $this->erro_status = "1";
    ...
    public function excluir($e27_sequencial = null, $dbwhere = null)
    {
        $lSessaoDesativarAccount = db_getsession("DB_desativar_account", false);
        if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
                && ($lSessaoDesativarAccount === false))) {
        ...
        if ($result == false) {
            $this->erro_banco = str_replace("\n", "", @pg_last_error());
            $this->erro_sql = "Movimento da retencao não Excluído. Exclusão Abortada.\\n";
            ...
            if (pg_affected_rows($result) == 0) {
                $this->erro_banco = "";
                $this->erro_sql = "Movimento da retencao não Encontrado. Exclusão não Efetuada.\\n";
                $this->erro_sql .= "Valores : " . $e27_sequencial;
                $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
                $this->erro_status = "1";
    ...
    public function sql_query($e27_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
    ...
    public function sql_query_file($e27_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
