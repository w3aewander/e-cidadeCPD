...
//    echo "<pre>";
//    print_r($oDiarioAvaliacaoDisciplina);
//    echo "</pre>";
        ...
        if ($oDadosRelatorio->iTipoPeriodo == 2) {
           ...
           * Data: 26/11/2025
           * Demanda: 18003
           * Razao: Ajustar a exibição do resultado quando houver progressão parcial / dependência.
           *        Nesses casos, o resultado global do diário permanece 'R' (reprovado no ensino
           *        regular), mas o aluno está pedagogicamente APROVADO COM DEPENDÊNCIA.
           *        Para o Livro de Notas, passamos a buscar o termo de APROVAÇÃO (como na Ficha/ATA),
           *        evitando que o aluno apareça visualmente como reprovado no quadro final.
           *        Regra: se aprovadoComProgressaoParcial() for true, tratar como 'A' na busca do termo.
           */
		...
		if($oDadosRelatorio->sPeriodo <> 'RESULTADO RECUPERAÇÃO SEMESTRAL' )
		{
            $oPdf->Cell($oDadosRelatorio->iTamanhoColunasDisciplinas, $oDadosRelatorio->iAltura, "{$sAproveitamento}", 1, 0, "C");
		}else{
// por algum motivo o resultado do RSS estava saindo com a soma dos dois bimestres mais a recuperação
// tive que criar a rotina abaixo para resolver isso
//
// função getAluno fica em Matricula.model.php na linha 499
// a função getAluno chama a funcao getAlunoByCodigo em AlunoRepositório.model.php linha 63
// função getCodigoAluno fica em Aluno.model.php na linha 481
//
// funcaoo getDisciplina fica em Regencia.model.php linha 261
// funcao getCodigoDisciplina fica em Disciplina.model.php na linha 116
            $rss = buscaRSS($oMatricula->getAluno()->getCodigoAluno(),$oRegencia->getDisciplina()->getCodigoDisciplina());
			$oPdf->Cell($oDadosRelatorio->iTamanhoColunasDisciplinas, $oDadosRelatorio->iAltura, "{$rss}", 1, 0, "C");
		}
        $oPdf->SetFont('arial', '', 7);
...
function buscaRSS($CodAluno,$CodDisciplina)
{
	$sql = pg_query("
			 select
			 ed59_i_disciplina,
			 ...
			 where
			 ed47_i_codigo  = '".$CodAluno."'
			 and
			 ed59_i_disciplina = '".$CodDisciplina."'
			 and
			 ed09_i_codigo in (1,2)
			 group by ed59_i_disciplina,ed232_c_descr,ed47_i_codigo,ed47_v_nome");
// ed09_i_codigo in (1,2) é o codigo do primeiro e segundo bimestre
