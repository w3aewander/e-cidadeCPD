...
$arquivoJsonVinculoCaminho = 'arquivos/esocial/tabelas/vinculo_descricao.json';
...
try {
    ...
    switch ($oParam->exec) {
        case 'getDadosFormulario':
            ...
            if(!empty($arquivoJsonVinculo)) {
                foreach ($arquivoJsonVinculo as $codigoResposta => $item) {
                    if ($codigoResposta == $oParam->codigo_resposta) {
                        $oRetorno->aVinculos[] = $item;
                    }
                }
            }
            break;
        ...
        case 'salvar':
             ...
             * para evitar de colocar descrição diferente quando houver códigos iguais no .json
             */
            if (!empty($oParam->camposDescricao)) {
                $arquivoJsonVinculo = \JSON::create()->parse(file_get_contents($arquivoJsonVinculoCaminho));

                $aCamposDescricao = $oParam->camposDescricao;
                foreach ($aCamposDescricao as $indice => $campo) {
                    $arquivoJsonVinculo->{$campo->identificadorResposta} = $campo;
                }

                file_put_contents($arquivoJsonVinculoCaminho, \JSON::create()->stringify($arquivoJsonVinculo));
            }
            ...
            */
                            ...
                            case AvaliacaoPergunta::TIPO_RESPOSTA_OBJETIVA:
                                foreach ($perguntasRespondidas->respostas as $resposta) {
                                    if ($resposta->valor) {
                                        ...
                                        $buscarRespostaVazia = false;
                                        if ($resposta->codigo === "null") {
                                            $buscarRespostaVazia = true;
                                        }
                                        $oResposta->adicionarRespostaParaPergunta($pergunta, $resposta->codigo, $buscarRespostaVazia);
                                    }
                                }
                                break;
                        }
            ...
            /**
             * Adicionado para possibilitar a exclusão de respostas desse formulário sem impactar em outros formulários do sistema
             */
            foreach ($oResposta->getRespostas() as $pergunta) {
                if ($pergunta instanceof \ECidade\Configuracao\Formulario\Resposta\Model\Valor) {
                    if ($pergunta->getValor() === "" || $pergunta->getValor() === "null") {
                        $pergunta->setValor("null");
                    }
                }
            }

            Resposta::persist($oResposta, true);
            if (!empty($oParam->iCodigoGrupoPerguntas)) {
                $iCodigoGrupoPerguntas = $oParam->iCodigoGrupoPerguntas;
            }

            $oRetorno->iCodigoResposta = $oResposta->getCodigo();
