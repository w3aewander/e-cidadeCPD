...
function js_validaTamanhoCampo(){
  ...
  var municipio = $F('j43_munic');
  var logradouro = $F('j43_ender');
  ...
  if(logradouro.length > 40) {
      var mensagem  = 'O logradouro informado possui descrição maior que 40 caracteres. ';
          mensagem += 'O endereço será salvo com logradouro até a quantidade de caracteres permitida. ';
          mensagem += 'Deseja prosseguir?';

      var salvaLogradouro = confirm(mensagem);

      if(salvaLogradouro) {
          $('j43_ender').value = logradouro.substr(0,40);
      } else {
          return false;
      }
  }
