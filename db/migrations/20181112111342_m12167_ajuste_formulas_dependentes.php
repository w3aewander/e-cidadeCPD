...
class M12167AjusteFormulasDependentes extends PostgresMigration
{
    public function up()
    {
        $this->atualizaFormulaRemuneracao();
    ...
    {
        $this->reverteFormulaRemuneracao();
...
WHERE rh05_seqpes IS NULL AND rh01_numcgm = [CODIGO_CGM])) AS x
    ...
    private function atualizaFormulaRemuneracao()
    {
        $this->execute("UPDATE db_formulas SET db148_formula = 'select f010 as remuneracao from fc_variaveis_matricula([ESOCIAL_MATRICULA_SERVIDOR], fc_anofolha([ESOCIAL_INSTITUICAO]), fc_mesfolha([ESOCIAL_INSTITUICAO]), [ESOCIAL_INSTITUICAO])'
                        WHERE db148_nome = 'ESOCIAL_REMUNERACAO'");
    }

    private function reverteFormulaRemuneracao()
    {
        $this->execute("UPDATE db_formulas SET db148_formula = 'select f010 as remuneracao from fc_variaveis_matricula([ESOCIAL_MATRICULA_SERVIDOR], fc_anofolha([ESOCIAL_INSTITUICAO]), fc_mesfolha([ESOCIAL_INSTITUICAO]), [ESOCIAL_INSTITUICAO])'
                        WHERE db148_nome = 'ESOCIAL_REMUNERACAO'");
    }
