...
$planoorcamento = '';
if (!empty($o80_codres) && !empty($o80_planoorcamentariolinhapacto)) {

    $daoPlanoOrcamentarioLinha = new cl_planoorcamentariolinhapacto();

    $dadosPlanoOrcamento = $daoPlanoOrcamentarioLinha->findBydId($o80_planoorcamentariolinhapacto);
...
<form name="form1" method="post" action="">
    ...
    <fieldset style="width: 700px">
      ...
      <table border="0"> <!---  3 colunas --->
        ...
        if (isset($o80_coddot) and !$o80_coddot== "")
        {
          ...
          {
            $resdotsql = db_dotacaosaldo(8,2,2,true," o58_coddot = $o80_coddot and o58_anousu = $anousu",$anousu,$DBtxtperiodoini,$DBtxtperiodofim,8,0,true);
            $result = db_query($resdotsql);
            if (!$result || pg_num_rows($result) == 0) {
              echo "<script> \nalert(' Dotação $o80_coddot  não encontrada '); location.href = 'orc1_orcreserva001.php'; \n </script>";
            } else {
              db_fieldsmemory($result, 0);

                $recurso = RecursoRepository::getRecursoPorCodigo($o58_codigo);
                $fonteRecurso = $recurso->getFonteDeRecurso();
                $o58_codigo = $fonteRecurso;
            }
          } else {
            echo "<script> \nalert(' Dotação $o80_coddot  não encontrada '); location.href = 'orc1_orcreserva001.php'; \n </script>";
          }
        ...
        <tr>
          ...
          <td> <?php db_input('descricao',40,"",true,'text',3,"");  ?> </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <tr style="<?=$mostrarLinhaPacto;?>">
              ...
              <td colspan="2">
                  <select id="o80_planoorcamentariolinhapacto" name="o80_planoorcamentariolinhapacto" style="width: 100%">
                  </select>
              </td>
          </tr>
        <tr>
  ...
  <input name="db_opcao" type="submit" id="db_opcao"
    ...
    <?=($db_botao==false?"disabled":"")?>  onclick="return critica_form();" >
...
<script>
   ...
   function critica_form(){
       ...
       var linhaPacto = $('o80_planoorcamentariolinhapacto');
       ...
       if (new Number($F('o80_valor')) > saldoFinal) {
           alert('o Saldo reservado é maior que o saldo da Linha de pacto selecionado');
           return false;
       }
       return true;
   }
  ...
  function js_pesquisaLinhaPactos(planoOrcamentario, valorSelecionado ) {
      ...
      $('o80_planoorcamentariolinhapacto').options.length = 0;
      new AjaxRequest('orc4_dotacao.RPC.php', parametro, function (response, erro)
      {

          var elementoPlano = $('o80_planoorcamentariolinhapacto');
  ...
  <?php
  if (!empty($o80_planoorcamentariolinhapacto) && FONTE_RECURSO_UNIAO) {
      echo "js_pesquisaLinhaPactos(\$F('planoorcamento'), {$o80_planoorcamentariolinhapacto});\n";
  }
  ?>
</script>
