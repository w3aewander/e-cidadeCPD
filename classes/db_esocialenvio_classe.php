...
class cl_esocialenvio {
   ...
   var $campos = "
                 ...
                 rh213_evento = varchar(5) = Código do Evento 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->rh213_data = ($this->rh213_data == ""?@$GLOBALS["HTTP_POST_VARS"]["rh213_data"]:$this->rh213_data);
     }else{
       $this->rh213_sequencial = ($this->rh213_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["rh213_sequencial"]:$this->rh213_sequencial);
     }
   }
   ...
   function incluir ($rh213_sequencial){
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,1010244,1009604,'','".AddSlashes(pg_result($resaco,0,'rh213_data'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   }
   ...
   public function alterar ($rh213_sequencial=null) {
     ...
     if(trim($this->rh213_data)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh213_data"])){
       $sql  .= $virgula." rh213_data = '$this->rh213_data' ";
       $virgula = ",";
     }
     $sql .= " where ";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["rh213_data"]) || $this->rh213_data != "")
             $resac = db_query("insert into db_acount values($acount,1010244,1009604,'".AddSlashes(pg_result($resaco,$conresaco,'rh213_data'))."','$this->rh213_data',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,1010244,1009604,'','".AddSlashes(pg_result($resaco,$iresaco,'rh213_data'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
   ...
   // funcao do sql
   public function sql_query_file ($rh213_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "", $join = '') {
  ...
  public function sql_query_status($rh213_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "") {

      $sql  = "SELECT {$campos} ";
      $sql .= "  FROM esocialenvio ";
      $sql .= "  LEFT JOIN esocialenviostatus on esocialenvio.rh213_sequencial = esocialenviostatus.rh214_esocialenvio";
      $sql2 = "";
      if (empty($dbwhere)) {
          if (!empty($rh213_sequencial)){
              $sql2 .= " WHERE esocialenvio.rh213_sequencial = $rh213_sequencial ";
