...
abstract class DAOBasica {
  ...
  protected function formatarAtributo($sNomeCampo, $sValor) {
    ...
    $aValoresFormatados ['date']    = (empty($sValor) || $sValor == "null" ? "null" : "'" . implode("-", array_reverse(explode("/",$sValor))) . "'");
  ...
  private function salvarAccount($sTipoAccount) {
    ...
    if (!$this->lSalvarAccount || !$this->gerarDadosDeAcount()) {
      return true;
    }
  ...
  public function findBydId($value) {
    ...
    **/
    ...
    /**
     * Verifica se foi setado a nao geração de acount na sessão atual
     * @return bool
     */
  public function gerarDadosDeAcount() {

      $lSessaoDesativarAccount = (bool)db_getsession("DB_desativar_account", false);
      return empty($lSessaoDesativarAccount);
  }
}
