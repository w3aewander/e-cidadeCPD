...
try {
    ...
    switch ($oParam->exec) {
        ...
        case "exemplaresByAcervo":
            ...
            $sCampos .= " ,bi23_anoedicao as ano_edicao ";
            $sCampos .= " ,bi23_edicao as edicao ";
        ...
        case 'quantidadeItensColecao':
            if(empty($oParam->colecao)) {
                throw new ParameterException(_M(MSG_BIB4_ACERVORPC . "informe_colecao"));
            }

            $daoColecaoAcervo = new cl_colecaoacervo();
            $whereColecaoAcervo = "bi29_sequencial = {$oParam->colecao}";
            $sqlColecaoAcervo = $daoColecaoAcervo->sql_query_file(null, 'bi29_quantidade', null, $whereColecaoAcervo);
            $rsColecaoAcervo = db_query($sqlColecaoAcervo);

            if(!$rsColecaoAcervo) {
                throw new DBException(_M(MSG_BIB4_ACERVORPC . "erro_buscar_quantidade"));
            }

            if(pg_num_rows($rsColecaoAcervo) === 0) {
                throw new BusinessException(_M(MSG_BIB4_ACERVORPC . "colecao_sem_quantidade"));
            }

            $oRetorno->quantidadeItens = db_utils::fieldsMemory($rsColecaoAcervo, 0)->bi29_quantidade;
