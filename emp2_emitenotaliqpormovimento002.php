...
for ($i = 0; $i < $clpagordem->numrows; $i++) {
    ...
    $sSqlDadosOrdem .= "              																	 and conplanoreduz.c61_anousu = "
    ...
    $sSqlDadosOrdem .= "                                       			   and conplanoconta.c63_reduz = conplanoreduz.c61_reduz           ";
    ...
    $rsDadosOrdem = db_query($sSqlDadosOrdem);

    /* Autenticação */
    $sSqlAutenticacoa = " select k12_valor, k12_codautent, c72_complem  ";
    ...
    $sSqlAutenticacoa .= "      left join conlancamcorgrupocorrente on c23_corgrupocorrente = k105_sequencial ";
    $sSqlAutenticacoa .= "      left join conlancamcompl on c72_codlan = c23_conlancam";
    $sSqlAutenticacoa .= " where corempagemov.k12_codmov = {$e81_codmov} ";

    $rsDadosAutenticacoa = db_query($sSqlAutenticacoa);

    $k12_valor = null;
    $k12_codautent = null;
    if ($rsDadosAutenticacoa && pg_num_rows($rsDadosAutenticacoa) > 0) {
    ...
    $rsItem = db_query($sSqlItem);

    $sSqlOutrasOrdens
        = " select sum(saldo) as outrasordens                                                                    ";
    ...
    $rsOutrasOrdens = db_query($sSqlOutrasOrdens);
    ...
    $pdf1->contarec = null;
    $sSqlConta = " select c61_codcon, c61_codigo, c61_reduz  ";
    ...
    $sSqlConta .= " 						                         and c63_reduz = c61_reduz  ";
    ...
    if ($rsConta != false && (pg_numrows($rsConta) == 1)) {
        ...
        $sSqlConplanoConta
            = "select * from conplanoconta where c63_codcon = $c61_codcon and c63_anousu = {$iAnoUso} and c63_reduz = {$c61_reduz} ";
        $rsConta = db_query($sSqlConplanoConta);
    ...
    $pdf1->obs = !empty($c72_complem) ? $c72_complem : $e50_obs;
