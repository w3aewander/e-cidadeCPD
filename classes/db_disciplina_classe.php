...
class cl_disciplina
{
    ...
    var $campos = "
                 ...
                 ed12_matrizcurricular = boolean = Matriz Curricular
                 ";
    ...
    function atualizacampos($exclusao = false)
    {
        if ($exclusao == false) {
            ...
            $this->ed12_matrizcurricular = ($this->ed12_matrizcurricular == "" ? @$GLOBALS["HTTP_POST_VARS"]["ed12_matrizcurricular"] : $this->ed12_matrizcurricular);
        } else {
            $this->ed12_i_codigo = ($this->ed12_i_codigo == "" ? @$GLOBALS["HTTP_POST_VARS"]["ed12_i_codigo"] : $this->ed12_i_codigo);
        }
    }
    ...
    function incluir($ed12_i_codigo)
    {
        ...
        if (($resaco != false) || ($this->numrows != 0)) {
            ...
            $resac = db_query("insert into db_acount values($acount,1010046,1008267,'','" . AddSlashes(pg_result($resaco,
                    0,
                    'ed12_i_codigo')) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
            $resac = db_query("insert into db_acount values($acount,1010046,1008268,'','" . AddSlashes(pg_result($resaco,
                    0,
                    'ed12_i_ensino')) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
            $resac = db_query("insert into db_acount values($acount,1010046,11712,'','" . AddSlashes(pg_result($resaco,
                    0,
                    'ed12_i_caddisciplina')) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
            $resac = db_query("insert into db_acount values($acount,1010046,1013641,'','" . AddSlashes(pg_result($resaco,
                    0,
                    'ed12_matrizcurricular')) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
        }
        return true;
    }

    // funcao para alteracao
    function alterar($ed12_i_codigo = null, $sWhere = null)
    {
        $this->atualizacampos();
        ...
        if (trim($this->ed12_matrizcurricular) != "" || isset($GLOBALS["HTTP_POST_VARS"]["ed12_matrizcurricular"])) {
            $sql .= $virgula . " ed12_matrizcurricular = $this->ed12_matrizcurricular ";
            $virgula = ",";
        }
        ...
        if ($this->numrows > 0) {
            for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
                ...
                if (isset($GLOBALS["HTTP_POST_VARS"]["ed12_i_codigo"])) {
                    $resac = db_query("insert into db_acount values($acount,1010046,1008267,'" . AddSlashes(pg_result($resaco,
                            $conresaco,
                            'ed12_i_codigo')) . "','$this->ed12_i_codigo'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                }
                if (isset($GLOBALS["HTTP_POST_VARS"]["ed12_i_ensino"])) {
                    $resac = db_query("insert into db_acount values($acount,1010046,1008268,'" . AddSlashes(pg_result($resaco,
                            $conresaco,
                            'ed12_i_ensino')) . "','$this->ed12_i_ensino'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                }
                if (isset($GLOBALS["HTTP_POST_VARS"]["ed12_i_caddisciplina"])) {
                    $resac = db_query("insert into db_acount values($acount,1010046,11712,'" . AddSlashes(pg_result($resaco,
                            $conresaco,
                            'ed12_i_caddisciplina')) . "','$this->ed12_i_caddisciplina'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                }
                if (isset($GLOBALS["HTTP_POST_VARS"]["ed12_matrizcurricular"])) {
                    $resac = db_query("insert into db_acount values($acount,1010046,1013641,'" . AddSlashes(pg_result($resaco,
                             $conresaco,
                             'ed12_matrizcurricular')) . "','$this->ed12_matrizcurricular'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                }
            }
        }
        $result = db_query($sql);
    ...
    function sql_disciplinas_ensino($ensino) {
        $sql = "
        
        select ed12_i_codigo as id_vinculo, ed232_i_codigo as codigo_disciplina, ed232_c_descr as disciplina, ed12_matrizcurricular,
               (select distinct 1
                from (select ed335_disciplina from avaliacaoclassificacao where ed335_disciplina = ed12_i_codigo
                      union all
                      select ed89_i_disciplina from basediscglob where ed89_i_disciplina = ed12_i_codigo
                      union all
                      select ed35_i_disciplina from basempd where ed35_i_disciplina = ed12_i_codigo
                      union all
                      select ed34_i_disciplina from basemps where ed34_i_disciplina = ed12_i_codigo
                      union all
                      select ed339_disciplina from criterioavaliacaodisciplina where ed339_disciplina = ed12_i_codigo
                      union all
                      select ed70_i_disciplina from histmpdaprov where ed70_i_disciplina = ed12_i_codigo
                      union all
                      select ed66_i_disciplina from histmpsadap where ed66_i_disciplina = ed12_i_codigo
                      union all
                      select ed28_i_disciplina from histmpsaprov where ed28_i_disciplina = ed12_i_codigo
                      union all
                      select ed65_i_disciplina from histmpsdisc where ed65_i_disciplina = ed12_i_codigo
                      union all
                      select ed100_i_disciplina from histmpsdiscfora where ed100_i_disciplina = ed12_i_codigo
                      union all
                      select ed64_i_disciplina from historicompd where ed64_i_disciplina = ed12_i_codigo
                      union all
                      select ed106_disciplina from parecerdisciplina where ed106_disciplina = ed12_i_codigo
                      union all
                      select ed45_i_disciplina from procdiscfreqindiv where ed45_i_disciplina = ed12_i_codigo
                      union all
                      select ed114_disciplina from progressaoparcialaluno where ed114_disciplina = ed12_i_codigo
                      union all
                      select ed59_i_disciplina from regencia where ed59_i_disciplina = ed12_i_codigo
                      union all
                      select ed23_i_disciplina from relacaotrabalho where ed23_i_disciplina = ed12_i_codigo) as x
                      ) as com_vinculo
            from disciplina
