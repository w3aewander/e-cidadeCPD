...
class AnexoSumarioGeralReceita extends RelatoriosLegaisBase
{
  ...
  /**
   * Setter de iEsfera
   * @param string $iEsfera
   */
    public function setEsfera($sEsfera2)
    {
        $this->sEsfera = $sEsfera2;
    }
   ...
   */
    public function getDados()
    {
        ...
        if ($this->iOrigemFase == 1 && !empty($this->sEsfera)) {
            $sWhereDespesa .= " and o58_esferaorcamentaria = {$this->sEsfera}";
            $sWhereReceita .= " and o70_esferaorcamentaria = {$this->sEsfera}";
        }

        $sWherePlano      = " c61_instit in ({$this->getInstituicoes()})";
        $rsDespesa        = db_dotacaosaldo(
            3,
            3,
            2,
            true,
        ...
        $rsPlano          = db_planocontassaldo_matriz(
            $this->iAnoUsu,
            $sDataInicial,
            $sDataFinal,
            false,
       ...
       */
        ...
        for ($i = 0; $i < $iTotalLinhasDespesa; $i++) {
           ...
           */
            ...
            switch ($this->iOrigemFase) {
               ...
               */
                case 1:
                    $valorTotalDespesa += $oDespesa->dot_ini;
                    break;
               ...
               */
                case 2:
                    $valorTotalDespesa += ($oDespesa->empenhado_acumulado - $oDespesa->anulado_acumulado);
                    break;
               ...
               */
                case 3:
                    $valorTotalDespesa += $oDespesa->liquidado_acumulado;
                    break;
               ...
               */
                case 4:
                    $valorTotalDespesa += $oDespesa->pago_acumulado;
                    break;
            }
           ...
           */
            if (isset($aFuncoes[$oDespesa->o58_funcao])) {
                $aFuncoes[$oDespesa->total]->total += $valorTotalDespesa;
            } else {
                $oFuncao = new stdClass();
                $oFuncao->descricao              = $oDespesa->o52_descr;
                $oFuncao->total                  = $valorTotalDespesa;
        ...
        for ($iLinha = 1; $iLinha <= count($aLinhasRelatorio); $iLinha++) {
            ...
            if (!$aLinhasRelatorio[$iLinha]->isTotalizador()) {
                $aValoresColunasLinhas = $aLinhasRelatorio[$iLinha]->getValoresColunas(
                    null,
                    null,
               ...
               */
                if ($this->iOrigemFase != 1 && ($iLinha == 23 || $iLinha == 26)) {
       ...
       */
        foreach ($aLinhas[25]->funcoes as $oFuncao) {
            $aLinhas[25]->total += $oFuncao->total;
        }
