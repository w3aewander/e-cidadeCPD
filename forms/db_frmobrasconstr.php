...
$oRotulo->label("ob07_areacoberta");
$oRotulo->label("ob07_areadescoberta");

$rsObras = false;
$rsObrasAlvara = false;
$rsObrasConstr = false;
$rsObrasEnder = false;
$rsObrasIPTUBase = false;
$rsParProjetos = false;

$rsCaracterOcupacao = false;
$rsCaracterConstrucao = false;
$rsCaracterLancamento = false;
...
$ob07_fim_mes = null;
$ob07_fim_ano = null;
$ob07_areacoberta = null;
$ob07_areadescoberta = null;
$j14_nome = null;
$j13_descr = null;

if (!empty($oGet->ob08_codobra)) {
  ...
  $sSqlObrasIPTUBase = $oDaoObrasIPTUBase->sql_query_file(null, "ob24_iptubase", null,
    " ob24_obras   = {$oGet->ob08_codobra}");
  ...
  if ($oDaoObrasConstr->numrows > 0 && $oDaoObrasEnder->numrows > 0) {
      ...
      $ob07_areacoberta = $oObrasEnder->ob07_areacoberta;
  ...
  <center>
    <fieldset style="width: 600px">
      ...
      <table id="tabela_construcoes">
        ...
        <tr>
          <td nowrap title="Área Coberta">
              <?= $Lob07_areacoberta ?>
          </td>
          <td>
              <?php
              db_input('ob07_areacoberta', 10, $Iob07_areacoberta, true, 'text', $iDBOpcao);
              ?>
          </td>
        </tr>

        <tr>
          <td nowrap title="Área Descoberta">
              <?= $Lob07_areadescoberta ?>
          </td>
          <td>
              <?php
              db_input('ob07_areadescoberta', 10, $Iob07_areadescoberta, true, 'text', $iDBOpcao);
              ?>
          </td>
        </tr>
        <tr>
    ...
    <input name="excluir" type="button" id="excluir" onClick="return js_excluirConstrucao()"
           value="Excluir" <?php echo $lPermiteExclusao ? "" : "disabled"; ?> >
  </center>
</form>
<script>
  ...
  function js_salvaDados() {
    ...
    oDados.ob07_areaatual = $F('ob07_areaatual') !== '' ? $F('ob07_areaatual') : 0;
    ...
    oDados.ob07_areacoberta = $F('ob07_areacoberta') !== '' ? $F('ob07_areacoberta') : 0;
    oDados.ob07_areadescoberta = $F('ob07_areadescoberta') !== '' ? $F('ob07_areadescoberta') : 0;
  ...
  $('ob07_areacoberta').observe('change', function() {
    let areaCoberta = new Number($F('ob07_areacoberta'));
    let areaDescoberta = new Number($F('ob07_areadescoberta'));

    $('ob07_areaatual').value = areaCoberta + areaDescoberta;
  });

  $('ob07_areadescoberta').observe('change', function() {
    let areaCoberta = new Number($F('ob07_areacoberta'));
    let areaDescoberta = new Number($F('ob07_areadescoberta'));
