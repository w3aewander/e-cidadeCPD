...
try {
  ...
  switch($oParam->exec) {
    ...
    case 'pesquisaDadosAluno':
      ...
      $sCamposMatricula = "ed60_i_codigo, ed60_c_situacao, turma.ed57_i_codigo as ed57_i_codigo, ed221_i_serie";
      $sWhereMatricula  = "ed60_matricula = {$oMatricula->getMatricula()} AND (ed60_c_situacao = 'TROCA DE TURMA' or ed60_c_situacao = 'TROCA DE MODALIDADE')";
      ...
      if (pg_num_rows($rsMatricula) > 0) {
        ...
        $oRetorno->sSituacaoOrigem  = $oDadosMatricula->ed60_c_situacao;
...
//              }
    ...
    case 'pesquisaPeriodosTurma':
      ...
      if (!empty($oParam->iTurma) && !empty($oParam->iEtapa)) {
        ...
        foreach ($aElementos as $oElemento) {
          ...
          if($oParam->iTipoBusca == 99 && substr($oElemento->getDescricao(), 0, 9) == "RESULTADO"){
            continue;
            //$oDadosPeriodo->sDescricao      = substr(urlencode($oElemento->getDescricao()), 10);
          }else{
          ...
          if(  $oParam->iTipoBusca == 99 && $oElemento->getDescricao() == "MÉDIA FINAL"
            || $oParam->iTipoBusca == 99 && $oElemento->getDescricao() == "MÉDIA ANUAL"
            || $oParam->iTipoBusca == 99 && $oElemento->getDescricao() == "NOTA FINAL"
           ){
            continue;
            //echo $oElemento->getDescricao(); echo "<br>";
          }
          $oRetorno->aPeriodos[]          = $oDadosPeriodo;
          //echo $oDadosPeriodo->sDescricao;
        }
      }
    ...
    // Pega os Periodos do Calendario
      case 'getPeriodoCalendario':
          ...
          //iCodigo é o proc, espaço da avaliação na grade
          $oRetorno->aPeriodos = array();
