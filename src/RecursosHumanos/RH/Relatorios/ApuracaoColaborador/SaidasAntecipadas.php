 ...
 */
class SaidasAntecipadas extends Layout
{
    public function __construct()
    {
        $this->titulo2Relatorio = 'Saídas Antecipadas';
        parent::__construct();
    }
    ...
    public function imprimirDados()
    {
        ...
        foreach ($this->getDadosRelatorio() as $servidor) {
            ...
            foreach ($servidor->jornadas as $jornada) {
                ...
                foreach ($jornada->dadosApurados as $saidasAntecipadas) {
                    $this->linhaDados($saidasAntecipadas);
                }
            }
    ...
    public function montarConteudo()
    {
        ...
        foreach ($this->servidores as $servidor) {
            ...
            foreach ($this->getDatasIntervalo() as $dataAtual) {
                ...
                if ($diaTrabalhoModel->getHorasSaidaAntecipada() === '') {
                    continue;
                }

                if ($diaTrabalhoModel->getHorasSaidaAntecipada() === '00:00') {
                    continue;
                }
                ...
                $saidaAntecipada = new \stdClass();
                $saidaAntecipada->data = $diaTrabalhoModel->getData()->getDate(\DBDate::DATA_PTBR);
                $saidaAntecipada->dia = \DBDate::getDiasSemanaAbreviado(date('w', $diaTrabalhoModel->getData()->getTimeStamp()));
                $saidaAntecipada->horas = $diaTrabalhoModel->getHorasSaidaAntecipada();
                $saidaAntecipada->marcacoes = $this->getMarcacoes($diaTrabalhoModel->getMarcacoes());
                $saidaAntecipada->tipo = 'Saída Antecipada';

                $dadosRelatorio[$matricula]->jornadas[$codigoJornada]->dadosApurados[] = $saidaAntecipada;
            }
        }
