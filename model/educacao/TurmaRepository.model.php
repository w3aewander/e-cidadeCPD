...
class TurmaRepository
{
    ...
    public static function getTurmaByEscola($iCodigoEscola)
    {
        ...
        $sSqlTurma = $oDaoTurma->sql_query_turma(
            null,
    ...
    public static function getTurmaPorDisciplinasEscola($iCodigoDisciplina, $iCodigoEscola)
    {

        $aTurmas = [];
        $aFiltros = [];

        if (isset($iCodigoDisciplina) && !empty($iCodigoDisciplina)) {
            $aFiltros[] = "ed59_i_disciplina = {$iCodigoDisciplina}";
        }
        if (isset($iCodigoEscola) && !empty($iCodigoEscola)) {
            $aFiltros[] = "ed57_i_escola = {$iCodigoEscola}";
        }

        $oDaoTurma = new cl_turma();
        $sWhereTurma = implode(" and ", $aFiltros);
        $sCamposTurma = "ed57_i_codigo";
    ...
    /**
     * Busca as turmas de uma escola por etapa
    ...
    public static function getTurmaByEscolaEtapaDisciplina($iCodigoEscola, $iCodigoEtapa, $iCodigoDisciplina)
    {
        ...
        $sSqlTurma = $oDaoTurma->sql_query_turmaetapadisciplinaescola(
            null,
            'ed57_i_codigo, ed11_i_sequencia, ed57_c_descr',
            'ed52_i_ano,ed31_i_codigo, ed11_i_sequencia, ed57_c_descr',
            $sWhere
        ...
        if ($iTotalLinhas > 0) {
            for ($i = 0; $i < $iTotalLinhas; $i++) {
                $oDadosTurma = db_utils::fieldsMemory($rsTurma, $i);
                $aTurmasEscolaEtapa[] = TurmaRepository::getInstance()->getTurmaByCodigo($oDadosTurma->ed57_i_codigo);
            }
        }

        return $aTurmasEscolaEtapa;
    }
    ...
    public static function getTurmasByAnoAtual($iEscola = null)
    {
        ...
        if (!is_null($iEscola)) {
            $where[] = "ed57_i_escola = {$iEscola}";
        }

        $where[] = "ed52_i_ano = {$anoAtual}";
