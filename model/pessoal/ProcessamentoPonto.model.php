...
class ProcessamentoPonto{
  ...
  public function processaDadosPonto(){
    ...
    $oDadosPonto = $this->getDadosPontoCompetencia();
  ...
  /**
   * Retorna os dados para o processamento do ponto da competencia ATUAL.
   * Irá buscar todos os dados da tabela rhpreponto que estejam vinculados a instituição atual.
   * @return Object Objeto com os dados para processamento do ponto da competencia,
   */
  private function getDadosPontoCompetencia(){

    $sSqlRhPrePonto = "
      SELECT rhpreponto.*
      FROM
          rhprepontoloteregistroponto
      INNER JOIN
          loteregistroponto
      ON
          rh156_loteregistroponto = rh155_sequencial

      INNER JOIN rhpreponto
      ON
          rh156_rhpreponto = rh149_sequencial

      WHERE
          rh149_instit = {$this->oInstituicao->getCodigo()}
          AND rh155_ano = {$this->oCompetencia->getAno()}
          AND rh155_mes = {$this->oCompetencia->getMes()}
    ";
    ...
    $sSqlRhPrePonto .= "
      ...
      WHERE
          rh149_instit = {$this->oInstituicao->getCodigo()}
          AND rh149_sequencial not in (
              SELECT rh156_rhpreponto FROM rhprepontoloteregistroponto
      )
    ";
