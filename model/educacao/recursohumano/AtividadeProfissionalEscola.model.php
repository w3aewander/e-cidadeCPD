...
class AtividadeProfissionalEscola
{
    ...
    public function __construct($iCodigo = null)
    {
        ...
        $this->setTurno($oDados->ed22_turno);
        $this->dataInicio = DBDate::create($oDados->ed22_datainicio);
        if (!empty($oDados->ed22_datafim)) {
            $this->dataFim = DBDate::create($oDados->ed22_datafim);
        }
    ...
    public function salvar()
    {
        ...
        $oDaoRecHumanoAtiv->ed22_datainicio = $this->getDataInicio()->getDate();
        $oDaoRecHumanoAtiv->ed22_datafim = null;
        $oDaoRecHumanoAtiv->ed22_turno = $this->getTurno();
        if (!is_null($this->getDataFim())) {
            $oDaoRecHumanoAtiv->ed22_datafim = $this->getDataFim()->getDate();
        }
