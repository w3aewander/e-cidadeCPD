...
//*************************************************************************************************************************
  ...
  protected function validarDiarioAluno($oDiario, $oErroMensagem) {
	...
	$sqlt = "
		...
		where 
		ed57_i_codigo = ".$oDiario->getMatricula()->getTurma()->getCodigo();
    ...
    // aqui quando o aluno é avaliado por relatorio o sistema apenas exigirá o lançamento de presenças para o encerramento
        $sqlf ="
			    ...
			    where
				ed47_i_codigo = ".$oDiario->getMatricula()->getCodigo()."
				and
...
//******************************************************************************************************
    ...
    $sql = "
			...
			edu_parametros
			where
			ed233_i_escola = ".$oDiario->getTurma()->getEscola()->getCodigo();
    $result   = db_query($sql);
	$encerrar = db_utils::fieldsMemory($result,0);
    ...
    if (count($aPendenciasDiario > 0)) {
      ...
      foreach ($aPendenciasDiario as $sPendencia) {
        ...
        if( substr($sPendencia,1,25) == 'Aluno esta em recuperação')
		{
			if( $encerrar->ed233_encerrarecvazia )
			{	
			    $lDiarioSemPendencia = true;
			}else{
				$lDiarioSemPendencia = false;
...
// ***********************************************************************************************************
		...
		if( substr(trim($oalusit->ed56_c_situacao),0,11) <> 'TRANSFERIDO')
		{	
			$sWhereAlunoPossib  = " ed56_i_aluno = {$iCodigoAluno}";
		}	
		$sSqlAlunoPossib    = $oDaoAlunoPossib->sql_query("", $sCamposAlunoPossib, "", $sWhereAlunoPossib);
		$rsResultPossib     = $oDaoAlunoPossib->sql_record($sSqlAlunoPossib);
		$iLinhasAlunoPoss   = $oDaoAlunoPossib->numrows;
