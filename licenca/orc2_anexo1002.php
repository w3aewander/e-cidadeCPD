...
//echo "<pre>";
//$x = pg_fetch_all($rsSqlWork1);
//print_r($x);
//echo "</pre>";
//die("Procura 4160");

//$guardaelemento = array();
$guarda = 1;
for ($i = 0; $i < $iNumRowsWork1; $i++) {
  ...
  if ($elemento != "920000000000000" && $elemento != "980000000000000" ) {
  	...
  	/*if(in_array($elemento, $guardaelemento)){  		
  		continue;
  	}*/
    //if($elemento == "416000000000000"){continue;}

    if($elemento == "416000000000000"){
      if($guarda == 1){
        $descr = "SERVIOS ADMINISTRATIVOS E COMERCIAIS GERAIS";
        $guarda++;  
      } else {
        continue;
      }
    ...
    //RECEITA DE SERVIÇOS
    if($elemento == "416100000000000"){continue;}
    ...
    if($elemento == "470000000000000"){
	  ...
	  $a         += 1;
	  $valora    += $valor;
	  array_push($guardaelemento, $elemento);
...
if($descra[7] == "RECEITAS CORRENTES INTRA-ORÇAMENTARIAS"){
  $vlra[7] = -$vlra[7];
}
...
for($i = 0; $i < $numreg; $i++) {
  ...
  $pdf->setfont('arial','',7);
  if (isset($vlra[$i])) {
    if($descra[$i] == "RECEITAS CORRENTES INTRA-ORÇAMENTARIAS"){
...
for ($i = 0; $i < $iNumRowsWork1; $i++) {
  ...
  if ((substr($elemento,3,12) == "000000000000"
    ...
    if (substr($elemento,1,1) == "2" || substr($elemento,1,1) == "8") {
      ...
      if($elemento == $guardaels){continue;}
      $guardaels = $elemento;
      //var_dump($elemento); echo "<br>";
      //var_dump($descr); echo "<br>";
