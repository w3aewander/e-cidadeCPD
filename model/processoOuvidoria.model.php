...
require_once(modification('model/ouvidoria/AtendimentoOuvidoria.model.php'));
require_once(modification('model/ouvidoria/AtendimentoProcessoEletronico.model.php'));
require_once(modification('model/ouvidoria/AtendimentoOuvidoriaCidadao.model.php'));
...
class processoOuvidoria
{
     ...
     * @var integer
     */
    private $atendimentoOuvidoriaID;
     ...
     * @return int
     */
    public function getAtendimentoOuvidoriaID()
    {
        return $this->atendimentoOuvidoriaID;
    }

    /**
     * @param int $atendimentoOuvidoriaID
     * @return $this
     */
    public function setAtendimentoOuvidoriaID($atendimentoOuvidoriaID)
    {
        $this->atendimentoOuvidoriaID = $atendimentoOuvidoriaID;
        return $this;
    }
    ...
    {
        return AtendimentoOuvidoria::find($this->getAtendimentoOuvidoriaID());
    }

    /**
     * @return AtendimentoProcessoEletronico|false
     */
    public function getAtendimentoProcessoEletronico()
    {
        return AtendimentoProcessoEletronico::findByAtendimento($this->getAtendimentoOuvidoriaID());
    }

    /**
     * @return AtendimentoOuvidoriaCidadao|false
     */
    public function getAtendimentoCidadao(){
        return AtendimentoOuvidoriaCidadao::findByAtendimento($this->getAtendimentoOuvidoriaID());
    }

    public function getProcesso()
    {
        return new  processoProtocolo($this->getProcessoId());
    }


    /**
     * @param $id
     * @return false|ProcessoOuvidoria
     */
    public static function find($id)
    {
        ...
        $processoouvidoriaDao = new cl_processoouvidoria();
        $sql = $processoouvidoriaDao->sql_query_file($id);
        $rs = $processoouvidoriaDao->sql_record($sql);

        if ($processoouvidoriaDao->numrows < 1) {
            return false;
        }

        $objProcessoOuvidoria = pg_fetch_object($rs);
        return self::fromDao($objProcessoOuvidoria);
    }
     ...
     * @return false|ProcessoOuvidoria
     */
    public static function findByProcesso($idProcesso)
    {
        if (empty($idProcesso)) {
            return false;
        }
        ...
        $processoouvidoriaDao = new cl_processoouvidoria();
        $sql = $processoouvidoriaDao->sql_query_file(null, "*", null, "ov09_protprocesso = {$idProcesso}");
        $rs = $processoouvidoriaDao->sql_record($sql);
    ...
    public static function findByAtendimento($idAtendimento)
    {
        ...
        $processoouvidoriaDao = new cl_processoouvidoria();
        $sql = $processoouvidoriaDao->sql_query_file(null, "*", null, "ov09_ouvidoriaatendimento = {$idAtendimento}");
        $rs = $processoouvidoriaDao->sql_record($sql);
    ...
    {
        $processo = new ProcessoOuvidoria();
        return $processo
            ->setId($resultDao->ov09_sequencial)
            ->setProcessoId($resultDao->ov09_protprocesso)
            ->setAtendimentoOuvidoriaID($resultDao->ov09_ouvidoriaatendimento)
            ->setPrincipal($resultDao->ov09_principal == 'f' ? false : true);
    }
    ...
    public function incluirRecebimento(
        $iCodProc = '',
        $sDespacho = '',
        $iCodTran = '',
        $iIdUsuario = '',
        ...
        $clProcAndamInt->p78_publico = $lPublico;
        $clProcAndamInt->p78_data = date('Y-m-d', db_getsession('DB_datausu'));
        $clProcAndamInt->p78_despacho = $sDespacho;
        $clProcAndamInt->p78_hora = db_hora();
        $clProcAndamInt->p78_usuario = $iIdUsuario;
        $clProcAndamInt->p78_codandam = $clProcAndam->p61_codandam;
        $clProcAndamInt->p78_transint = "false";
        $clProcAndamInt->incluir(null);

        if ($clProcAndamInt->erro_status == 0) {
            throw new Exception("{$sMsgErro}\n{$clProcAndamInt->erro_msg}");
        }
...
//          echo "Depto: $clProcessoOuvidoriaProrrogacao->ov15_coddepto Sequencial: $clProcessoOuvidoriaProrrogacao->ov15_sequencial DataIni:{$clProcessoOuvidoriaProrrogacao->ov15_dtini} DataFin:{$clProcessoOuvidoriaProrrogacao->ov15_dtfim}\n";
    ...
    function incluiNovoDeptoProrrogacao(
        $iCodProc = '',
        $iCodDepto = '',
        $sMotivo = '',
        $iOrdem = '',
...
//          echo "Depto: $clProcessoOuvidoriaProrrogacao->ov15_coddepto Sequencial: $clProcessoOuvidoriaProrrogacao->ov15_sequencial DataIni:{$clProcessoOuvidoriaProrrogacao->ov15_dtini} DataFin:{$clProcessoOuvidoriaProrrogacao->ov15_dtfim}\n";
    ...
    function getProximoDepto($iCodProc = '', $iDeptoAtual = '')
    {
        ...
        $sSqlPrazoPrevisto = $clProcessoOuvidoriaProrrogacao->sql_query_file(null, "ov15_coddepto", "ov15_dtfim",
            $sWherePrazo);
    ...
    function getPosicaoAtualProrrogacao($iCodProc = '')
    {
        ...
        $rsDadosAndam = $clProcTransferProc->sql_record($clProcTransferProc->sql_query_andam(null, null, "count(*)",
            null, $sWhereAndam));
    ...
    function arquivarProcesso($iCodProc = '', $sHistorico = '', $iIdUsuario = '', $iCodDepto = '')
    {
        ...
        $sSqlAtendimento = $clOuvidoriaAtendimento->sql_query_proc(null, "distinct ov01_sequencial", null,
            $sWhereAtendimento);
