...
try {
    switch ($oParam->sExec) {
        ...
        case "operacoesExtraOrcamentaria" :
            ...
            $sWhereConlancam .= " and conlancaminstit.c02_instit = {$iInstituicaoSessao} ";
            ...
            $sCamposConLancam .= " k17_codigo ";

            $sSqlConlancam = $oDaoConlancam->sql_query_reprocessaExtraOrcamentario(
                null,
                $sCamposConLancam,
                "c70_codlan",
                $sWhereConlancam
            ...
            for ($iResultLancamento = 0; $iResultLancamento < $oDaoConlancam->numrows; $iResultLancamento++) {
                ...
                $possuiSlip = !empty($oDadosConlancam->k17_codigo);
                ...
                $oLancamentoAuxiliar = LancamentoAuxiliarFactory::getInstance($iDocumento,
                    $oDadosConlancam->c70_codlan, $possuiSlip);
                $oLancamentoAuxiliar->setHistorico($iHistorico);

                $daoConlancamRecurso = new cl_conlancamrecurso();
                $daoConlancamRecurso->excluir($oDadosConlancam->c70_codlan);

                $oEventoContabil->reprocessaLancamentos(
                    $oDadosConlancam->c70_codlan,
                    $oLancamentoAuxiliar,
                    $oDadosConlancam->c70_data);

                $dadosRecurso = new \ECidade\Financeiro\Contabilidade\LancamentoContabil\Recurso();
                $dadosRecurso->processar($oDadosConlancam->c70_codlan);
        ...
        case "recriarLancamentosMovimentacaoPatrimonial":
            ...
            $sSqlConlancam = $oDaoConlancam->sql_query_reprocessaMovimentacaoPatrimonial(null, $sCamposConLancam, 3,
                $sWhereConlancam);
            $rsConlancam = $oDaoConlancam->sql_record($sSqlConlancam);
            ...
            $sSqlRemover = $oDaoConlancam->sql_query_reprocessaMovimentacaoPatrimonial(null, 'DISTINCT c70_codlan',
                null, $sWhereConlancam);
            ...
            for ($iRowLancamento = 0; $iRowLancamento < $oDaoConlancam->numrows; $iRowLancamento++) {
                ...
                for ($iRowItem = 0; $iRowItem < $oDaoEmpNotaItem->numrows; $iRowItem++) {
                    ...
                    if (empty($aItens[$oMaterialAlmoxarifado->getGrupo()->getCodigo()])) {
                        ...
                        $sSqlBuscaOrdem = $oDaoOrdemLancamento->sql_query_nota('c03_ordem', 'c03_ordem',
                            "c66_codnota = {$oStdBuscaLancamento->m72_codnota} and c71_coddoc = {$iDocumento}");
                        ...
                        $oStdItemOrdem->iOrdemLancamento = db_utils::fieldsMemory($rsBuscaOrdem,
                            $iRowResource)->c03_ordem;
                ...
                foreach ($aItens as $iGrupo => $oDadosItem) {
                    ...
                    $oLancamentoAuxiliarEmLiquidacao->setObservacaoHistorico('Lançamento em liquidação da ordem de compra '
                        . $oDadosItem->iOrdemCompra);
        ...
        case "reprocessarLancamentosMovimentacaoPatrimonial" :
            ...
            if (in_array($iDocumento, array(700, 701, 702, 703, 704))) {
                ...
                $sSqlConlancam = $oDaoConlancam->sql_query_reprocessaMovimentacaoPatrimonial(null, $sCamposConLancam,
                    'c70_codlan', $sWhereConlancam);
                $rsConlancam = $oDaoConlancam->sql_record($sSqlConlancam);
                if ($oDaoConlancam->numrows == 0) {
                    $sSqlConlancam = $oDaoConlancam->sql_query_reprocessaMovimentacaoPatrimonial(null,
                        $sCamposConLancam, 'c70_codlan', $sWhereConlancam, false);
                    $rsConlancam = $oDaoConlancam->sql_record($sSqlConlancam);
                }
            }
            ...
            for ($iResultLancamento = 0; $iResultLancamento < $oDaoConlancam->numrows; $iResultLancamento++) {
                ...
                $oEventoContabil->reprocessaLancamentos($oDadosConlancam->c70_codlan, $oLancamentoAuxiliar,
                    $oDadosConlancam->c70_data);
                $iTotalRegistrosProcessados++;
            }
        ...
        case "reprocessarLancamentos":
            ...
            $sSqlConlancam = $oDaoConlancam->sql_query_reprocessamento(null, $sCamposConLancam, 'c70_codlan',
                $sWhereConlancam);
            $rsConlancam = $oDaoConlancam->sql_record($sSqlConlancam);
            ...
            for ($iResultLancamento = 0; $iResultLancamento < $oDaoConlancam->numrows; $iResultLancamento++) {
                ...
                $oLancamentoAuxiliar = LancamentoAuxiliarFactory::getInstance($iDocumento,
                    $oDadosConlancam->c70_codlan);
                $oEventoContabil = new EventoContabil($iDocumento, $oDadosConlancam->anolancamento, $iInstituicaoSessao);
                $oEventoContabil->reprocessaLancamentos($oDadosConlancam->c70_codlan, $oLancamentoAuxiliar,
                    $oDadosConlancam->c70_data);
                $iTotalRegistrosProcessados++;
            }
...
function excluirLancamentos($iCodigoLancamento)
{
    ...
    /**
     * Removemos os dados do processamento da MSC
     */
