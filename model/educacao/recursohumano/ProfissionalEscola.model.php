...
class ProfissionalEscola
{
   ...
   * @throws Exception
   */
    public function getHorariosRegencia($dataFim = null)
    {
   ...
   * - Nome
   * - Identifica??o( Matr?cula / CGM )
   * @throws DBException
   */
    private function getDadosPessoais()
    {
        $sCampos  = "ed20_i_rhregime as tiporegime,";
        ...
        if (pg_num_rows($rsRecursoHumano) > 0) {
            ...
            $this->setTipoRegime($oDados->tiporegime);
        }
    }

  /**
   * Retorna o c?digo de identifica??o do profissional na escola( Matr?cula / CGM )
   * @return integer
   */
   ...
   */
    ...
    public function getMaiorPermissaoDiario()
    {
        $maiorPermissao = 0; 
        foreach ($this->getAtividades() as $atividade) {
            if($atividade->getAtividadeEscolar()->getPermissaoDiario() > $maiorPermissao
                && $atividade->getDataFim() == null) {
                $maiorPermissao = $atividade->getAtividadeEscolar()->getPermissaoDiario();
            }
        }
