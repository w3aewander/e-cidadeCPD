...
$sql =  "
    ...
    from (
        select j01_matric, 
            valor_imposto_1, valor_imposto_2, 
            case when valor_imposto_1 = 0 
                or valor_imposto_2 = 0 
                then 0 
                else round(100 - (valor_imposto_2 / valor_imposto_1 * 100),5) * -1 
            end as percentual_imposto, 
            valor_taxas_1, valor_taxas_2, 
            case when valor_taxas_1 = 0 
                or valor_taxas_2 = 0 
                then 0 
                else round(100 - (valor_taxas_2 / valor_taxas_1 * 100),5) * -1 
            end as percentual_taxas
        from (
            select * from ( 
                select j01_matric, sum_imposto_1 as valor_imposto_1, 
                                   sum_imposto_2 as valor_imposto_2, 
                                   sum_taxas_1 as valor_taxas_1, 
                                   sum_taxas_2 as valor_taxas_2 
                from (
                    select j01_matric, sum_imposto_1, sum_imposto_2, sum_taxas_1, sum_taxas_2
                    from (
                        select j01_matric, 
                        coalesce(x.vlr,0) as sum_imposto_1, 
                        coalesce(y.vlr,0) as sum_imposto_2, 
                        coalesce(zx.vlr, 0) as sum_taxas_3,
                        coalesce(zzx.vlr, 0) as sum_taxas_4,
                        coalesce(xx.vlr, 0) + coalesce(zx.vlr, 0) as sum_taxas_1,
                        coalesce(yy.vlr, 0) + coalesce(zzx.vlr, 0) as sum_taxas_2

                        from iptubase
                        left join	(	
                            select j21_matric, 
                            sum(j21_valor) as vlr 
                            from iptucalv 
                            where j21_anousu = $ano1 and j21_codhis in (1,5)
                            group by iptucalv.j21_matric
                        ) as x on x.j21_matric = iptubase.j01_matric

                        left join (	
                            select j21_matric, sum(j21_valor) as vlr 
                            from iptucalv 
                            where j21_anousu = $ano2 and j21_codhis in (1,5)
                            group by iptucalv.j21_matric
                        ...
                        (
                             select j151_matric,
                                    coalesce(sum(j152_valor),0) as vlr
                             from iptutaxacalv
                             ...
                             where j08_anousu = $ano1
                               and j152_codhis in (2)
                             group by j151_matric
                        ) as zx on zx.j151_matric = iptubase.j01_matric
                        ...
                        (
                              ...
                              from iptutaxacalv
                                       inner join iptutaxanump on j152_iptutaxanump = j151_codigo
                                       inner join iptucadtaxaexe on j08_iptucadtaxaexe = j151_iptucadtaxaexe
                              where j08_anousu = $ano2
                                and j152_codhis in (2)
                              group by j151_matric
                        ) as zzx on zzx.j151_matric = iptubase.j01_matric

                        left join
                        (
                            select j21_matric,
                                   coalesce(sum(j21_valor),0)  as vlr
                              from iptucalv
                             where j21_anousu = $ano1
                               and j21_codhis in (2)
                            group by iptucalv.j21_matric
                        ) as xx on xx.j21_matric = iptubase.j01_matric
                        ...
                        (
                             ...
                             where j21_anousu = $ano2
                               and j21_codhis in (2)
                             group by iptucalv.j21_matric
                        ) as yy on yy.j21_matric = iptubase.j01_matric
