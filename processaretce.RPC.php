...
switch ($oParam->exec) {
  ...
  case "processarSigfis":
    ...
    try {
      //var_dump($oParam->aArquivos); die("Confere arquivo"); 

      //$oEscritor     = new PadArquivoEscritorTXT();
      $oEscritor     = new Xmltce();      
      ...
      $otxtLogger    = fopen("tmp/eTCE.log", "w");

      if (count($oParam->aArquivos) > 0) {

        foreach ($oParam->aArquivos as $sArquivo){            
            //var_dump("etceArquivo{$sArquivo}"); die("Confere");

          if (file_exists("model/contabilidade/arquivos/etcerj/etceArquivo{$sArquivo}.model.php")) {
            require_once("model/contabilidade/arquivos/etcerj/etceArquivo{$sArquivo}.model.php");

            $sNomeClasse = "etceArquivo{$sArquivo}";
            ...
            $oEscritor->adicionarArquivo($oEscritor->criarArquivo($oArquivo), $oArquivo->nomexml());

          }
        }
        $oEscritor->zip("eTCE");
        $oEscritor->adicionarArquivo("tmp/eTCE.zip", "eTCE.zip");
        $oEscritor->adicionarArquivo("tmp/eTCE.log", "eTCE.LOG");
