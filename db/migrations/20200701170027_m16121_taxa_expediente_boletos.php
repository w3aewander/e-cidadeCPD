...
class M16121TaxaExpedienteBoletos extends PostgresMigration
{
    ...
    private function downBaixaBanco()
    {
        $this->execute(<<<SQL
drop function if exists fc_baixabanco(integer, date);
create or replace function fc_baixabanco(cod_ret integer, datausu date, OUT codigo_retorno integer, OUT descricao text)
returns record as
$$
declare
  ...
  lCreditoPartilha                 boolean default false;
  ...
  lTrocouNumpre                    boolean default false;
  ...
  iNumpreReciboAvulsoCustas        integer;
  iNumpreReciboAvulsoCreditoCustas integer;
  iNovoNumpreReciboAvulso          integer;
  ...
  nVlrDiferencaPgtoCusta           numeric(15,2) default 0;
  nVlrCustaRecibo                  numeric(15,2) default 0;
  ...
  nVlrTotalCustasCredito           numeric(15,2) default 0;
  nVlrTotalReciboAvulso            numeric(15,2) default 0;
  nVlrDesconto                     numeric(15,2) default 0;
  ...
  iCountReceitas                   integer default 0;
  iCountCustas                     integer default 0;
  ...
  rCreditoPartilha                 record;
  rPagamentoRecibo                 record;
  rArrecadRecibo                   record;
...
--   lRaise := true;
  ...
  /**
   * Verifica configuração das receitas de custas.
   */
  perform taxa.ar36_receita
     from processoforopartilhacusta
          inner join taxa on taxa.ar36_sequencial = processoforopartilhacusta.v77_taxa
          inner join tabrec on tabrec.k02_codigo = taxa.ar36_receita
    where processoforopartilhacusta.v77_numnov in (select distinct k00_numpre
                                                     from disbanco
                                                    where disbanco.codret = cod_ret)
      and (tabrec.k02_reccredito is null or tabrec.k02_reccredito = 0)
    union all
   select taxa.ar36_receita
     from inicialpartilhacustas
          inner join taxa on taxa.ar36_sequencial = inicialpartilhacustas.v36_taxa
          inner join tabrec on tabrec.k02_codigo = taxa.ar36_receita
    where inicialpartilhacustas.v36_numnov in (select distinct k00_numpre
                                                 from disbanco
                                                where disbanco.codret = cod_ret)
      and (tabrec.k02_reccredito is null or tabrec.k02_reccredito = 0);
  ...
  if found then
    ...
    perform fc_debug('  <Custas>  - ## INCONSISTENCIA: Existe taxa sem receita de credito configurada na tabrec.',lRaise,false,false);

    codigo_retorno := 2;
    descricao := 'Nao foi possivel gerar credito. Existe taxa sem receita de credito configurada na tabrec.';
    return;
 ...
 */
  if lAtivaPgtoParcial is true then
    ...
    /*********************************************************************************
     *  Regra 1.5 verifica se houve um pagamento de uma parcela supeior a que deveria ser pago
     *  caso seja isso e atualizado o numpar para parcela correta.
     ********************************************************************************/
    if lRaise then
      perform fc_debug('  <PgtoParcial> Regra 1.5 - VERIFICA PAGAMENTO DE PARCELA', lRaise, false, false);
    end if;

    select true
      into lPlugin
      from db_plugin
     where db145_nome = 'pagamentoDuplicidateRecibo'
       and db145_situacao = true;

    if lPlugin then

      if lRaise then
        perform fc_debug('<PagamentoDuplicado> Plugin de pagamentoDuplicadoReceita ...: ATIVO!', lRaise, false, false);
      end if;

      for rPagamentoRecibo in
        select min(recibopaga.k00_numpar) as k00_numpar,
      ...
      loop
        ...
        select k00_numpre,
         ...
         where k00_numpre = rPagamentoRecibo.k00_numpre
           and k00_dtvenc >= rPagamentoRecibo.dtpago
           and k00_numpar = (rPagamentoRecibo.k00_numpar - 1)
           and k03_tipo not in (15, 13, 18);

        if found then
          update recibopaga
             set k00_numpar = rArrecadRecibo.k00_numpar
           where k00_numpre = rArrecadRecibo.k00_numpre
             and k00_numpar = rPagamentoRecibo.k00_numpar;
        end if;
      end loop;
    end if;
    ...
    loop
      ...
      if r_idret.arrecad is false and r_idret.arrecant is false and r_idret.arreold is false then
        ...
        -- @TODO - corrigir esse comentario, so é deletado agora se não for recibo de custas. Regra 5.5 depende dos registros
        --
    ...
    if lRaise then
      raise notice '----------------------> REGRA 5 <--------- END --------';
    end if;

    /*******************************************************************************/
    /********************************* REGRA 5.5 ***********************************/
    /****************** CUSTAS RECIBO AVULSO DEBITOS PARCELADOS ********************/
    /*******************************************************************************/
    /* Processa recibo avulso de credito para custas de debitos parcelados         */
    /*******************************************************************************/

    if lRaise then
      raise notice '---------------------> REGRA 5.5 <------- BEGIN -------';
    end if;
    ...
    for rRecordDisbanco in
      ...
      select distinct
       ...
       where disbanco.codret = cod_ret
         and disbanco.classi is false
         and disbanco.instit = iInstitSessao
         and recibopaga.k00_hist = 11403
         and numprebloqpag.ar22_numpre is null
         and case
              when iNumprePagamentoParcial = 0 then true
              else disbanco.k00_numpre > iNumprePagamentoParcial
             end
         and not exists (select 1
                           from tmpnaoprocessar
    ...
    loop
      ...
      if lRaise then
        raise notice ' _____________________________________________________ ';
        raise notice '|                                                     |';
        raise notice '|-> RECIBO ...........................................: %', rRecordDisbanco.k00_numpre;
      end if;

      iNumpreReciboAvulso := null;
      nVlrTotalReciboAvulso := 0;

      for rRecord in

        select recibopaga.k00_numcgm,
               recibopaga.k00_dtoper,
               recibo_partilhas_credito.receita,
               recibo_partilhas_credito.receita_credito,
               recibopaga.k00_hist,
               recibo_partilhas_credito.valor,
               ...
               (select arrecad.k00_tipo
                ...
                select arreold.k00_tipo
                 ...
                 limit 1) as k00_tipo
          from (select tabrec.k02_codigo as receita,
                       tabrec.k02_reccredito as receita_credito,
                       sum(partilhas.partilha_taxa_valor) as valor,
                       partilhas.recibo_numpre
                  from (select distinct
                               processoforopartilhacusta.v77_sequencial as partilha_sequencial,
                               processoforopartilhacusta.v77_taxa as partilha_taxa_sequencial,
                               processoforopartilhacusta.v77_valor as partilha_taxa_valor,
                               recibopaga.k00_numnov as recibo_numpre
                          from recibopaga
                               ...
                               inner join processoforopartilha on processoforopartilha.v76_processoforo = processoforo.v70_sequencial
                               inner join processoforopartilhacusta on processoforopartilhacusta.v77_processoforopartilha = processoforopartilha.v76_sequencial
                               left join arrecad on arrecad.k00_numpre = recibopaga.k00_numpre
                                                 and arrecad.k00_numpar = recibopaga.k00_numpar
                         where recibopaga.k00_numnov = rRecordDisbanco.k00_numpre
                           and processoforopartilhacusta.v77_numnov = rRecordDisbanco.k00_numpre
                           and arrecad.k00_numpre is null
                         union
                        select distinct
                               inicialpartilhacustas.v36_sequencial as partilha_sequencial,
                               inicialpartilhacustas.v36_taxa as partilha_taxa_sequencial,
                               inicialpartilhacustas.v36_valor as partilha_taxa_valor,
                               recibopaga.k00_numnov as recibo_numpre
                          from recibopaga
                               inner join inicialnumpre on inicialnumpre.v59_numpre = recibopaga.k00_numpre
                               inner join inicial on inicial.v50_inicial = inicialnumpre.v59_inicial
                               inner join inicialpartilha on inicialpartilha.v35_inicial = inicial.v50_inicial
                               inner join inicialpartilhacustas on inicialpartilhacustas.v36_inicialpartilha = inicialpartilha.v35_sequencial
                               left join arrecad on arrecad.k00_numpre = recibopaga.k00_numpre
                                                 and arrecad.k00_numpar = recibopaga.k00_numpar
                         where recibopaga.k00_numnov = rRecordDisbanco.k00_numpre
                           and inicialpartilhacustas.v36_numnov = rRecordDisbanco.k00_numpre
                           and arrecad.k00_numpre is null
                       ) as partilhas
                       inner join taxa on taxa.ar36_sequencial = partilhas.partilha_taxa_sequencial
                       inner join tabrec on tabrec.k02_codigo = taxa.ar36_receita
                 group by tabrec.k02_codigo,
                          tabrec.k02_reccredito,
                          partilhas.recibo_numpre
               ) as recibo_partilhas_credito
               inner join recibopaga on recibopaga.k00_numnov = recibo_partilhas_credito.recibo_numpre
                                    and recibopaga.k00_receit = recibo_partilhas_credito.receita

      loop

        if iNumpreReciboAvulso is null then

          select nextval('numpref_k03_numpre_seq')
            into iNumpreReciboAvulso;

          insert into recibopagaold (k00_numcgm,
                                     k00_dtoper,
                                     k00_receit,
                                     k00_hist,
                                     k00_valor,
                                     ...
                                     k00_dtpaga,
                                     k00_numnov,
                                     k00_numpreold) select k00_numcgm,
                                                           k00_dtoper,
                                                           k00_receit,
                                                           k00_hist,
                                                           k00_valor,
          ...
          select string_agg(origem.info::text, ', ')
            into sObservacao
            from (select distinct 'Processo do Foro: [' || processoforo.v70_sequencial || ']' as info
                    from recibopaga
                         inner join inicialnumpre on inicialnumpre.v59_numpre = recibopaga.k00_numpre
                         inner join inicial on inicial.v50_inicial = inicialnumpre.v59_inicial
                         inner join processoforoinicial on processoforoinicial.v71_inicial = inicial.v50_inicial
                   ...
                   where recibopaga.k00_numnov = rRecordDisbanco.k00_numpre
                     ...
                     and arrecad.k00_numpre is null
                   union
                  select distinct 'Inicial do Foro: [' || inicial.v50_inicial || ']' as info
                    from recibopaga
                         inner join inicialnumpre on inicialnumpre.v59_numpre = recibopaga.k00_numpre
                         inner join inicial on inicial.v50_inicial = inicialnumpre.v59_inicial
                         inner join inicialpartilha on inicialpartilha.v35_inicial = inicial.v50_inicial
                   ...
                   where recibopaga.k00_numnov = rRecordDisbanco.k00_numpre
                     and inicialpartilhacustas.v36_numnov = rRecordDisbanco.k00_numpre
                     and arrecad.k00_numpre is null) as origem;
          ...
          insert into arrehist (k00_numpre,
                                ...
                                k00_histtxt,
                                k00_limithist,
                                k00_idhist) values (iNumpreReciboAvulso,
                                                    1,
                                                    rRecord.k00_hist,
                                                    datausu,
                                                    '00:00',
                                                    1,
                                                    'Recibo avulso referente ao credito de pagamento de custas. ( ' || sObservacao || ' )',
                                                    null,
                                                    nextval('arrehist_k00_idhist_seq')
                                                  );
        ...
        if lRaise then
          raise notice '|.....................................................|';
          raise notice '|-> rRecord.valor ....................................: %', rRecord.valor;
          raise notice '|-> rRecord.receita ..................................: %', rRecord.receita;
          raise notice '|-> rRecord.receita_credito ..........................: %', rRecord.receita_credito;
          raise notice '|.....................................................|';
        end if;

        update recibopaga set k00_valor = (k00_valor - rRecord.valor)
         where k00_numnov = rRecordDisbanco.k00_numpre
           and k00_receit = rRecord.receita
           and k00_hist = 11403;
        ...
        insert into recibo (k00_numcgm,
                            ...
                            k00_tipojm,
                            k00_codsubrec,
                            k00_numnov) values (rRecord.k00_numcgm,
                                                rRecord.k00_dtoper,
                                                rRecord.receita_credito,
                                                rRecord.k00_hist,
                                                rRecord.valor,
                                                rRecord.k00_dtvenc,
                                                iNumpreReciboAvulso,
        ...
        nVlrTotalReciboAvulso := nVlrTotalReciboAvulso + rRecord.valor;
      ...
      if iNumpreReciboAvulso is not null then
        ...
        insert into disbanco (k00_numbco,
                              ...
                              k00_numpar,
                              convenio,
                              instit) select k00_numbco,
                                             k15_codbco,
                                             k15_codage,
                                             codret,
                                             dtarq,
                                             dtpago,
                                             round(nVlrTotalReciboAvulso, 2),
                                             0,
                                             0,
                                             0,
                                             0,
        ...
        insert into tmpantesprocessar (idret,
                                       vlrpago,
                                       v01_seq) values (iSeqIdRet,
                                                        nVlrTotalReciboAvulso,
                                                        (select nextval('w_divold_seq')));
        update disbanco
           set vlrpago = (vlrpago - nVlrTotalReciboAvulso),
               vlrtot = (vlrtot - nVlrTotalReciboAvulso)
         where idret = rRecordDisbanco.idret;

        update tmpantesprocessar
           set vlrpago = round(vlrpago - nVlrTotalReciboAvulso, 2)
         where idret = rRecordDisbanco.idret;
        ...
        update inicialpartilhacustas
           set v36_numnov = iNumpreReciboAvulso
         where v36_sequencial in (select distinct inicialpartilhacustas.v36_sequencial
                                    from recibopaga
                                         inner join inicialnumpre on inicialnumpre.v59_numpre = recibopaga.k00_numpre
                                         inner join inicial on inicial.v50_inicial = inicialnumpre.v59_inicial
                                         inner join inicialpartilha on inicialpartilha.v35_inicial = inicial.v50_inicial
        ...
        update processoforopartilhacusta
           set v77_numnov = iNumpreReciboAvulso
         where v77_sequencial in (select distinct processoforopartilhacusta.v77_sequencial
                                    from recibopaga
                                         inner join inicialnumpre on inicialnumpre.v59_numpre = recibopaga.k00_numpre
                                         inner join inicial on inicial.v50_inicial = inicialnumpre.v59_inicial
                                         inner join processoforoinicial on processoforoinicial.v71_inicial = inicial.v50_inicial
      ...
      if lRaise then
        raise notice '|-> nVlrTotalReciboAvulso ............................: %', nVlrTotalReciboAvulso;
        raise notice '|_____________________________________________________|';
      end if;
    ...
    /*******************************************************************************/
    /*************************** VERIFICA PGTO. PARCIAL ****************************/
    /*******************************************************************************/
    /* Gera recibo avulso para custas de recibos que vão receber pagamento parcial */
    /*******************************************************************************/

    if lRaise then
      raise notice '---------------------> REGRA 6.5 <------- BEGIN -------';
      perform fc_debug('  <PgtoParcial> ---------------------> REGRA 6.5 <------- BEGIN -------',lRaise);
    end if;
         ...
         where disbanco.codret = cod_ret
           ...
           and exists (select true
                        ...
                        where k00_hist = 11403
                          and recibopaga.k00_numnov = disbanco.k00_numpre
                        )
    ...
    loop
      ...
      -- @TODO - condicionar recibos que deve receber o reajuste, fc_calcula em toda a galera fica lerto a baixa.
      select coalesce(round(sum(total), 2), 0)::numeric(15, 2)
        into nVlrCalculado
        from (select (substr(fc_calcula, 15, 13)::float8 +
                ...
                from (select fc_calcula(x.k00_numpre,
                        ...
                        from (select distinct
                                     recibopaga.k00_numpre,
                                     recibopaga.k00_numpar,
                                     case when disbanco.dtpago < recibopaga.k00_dtpaga
                                      then recibopaga.k00_dtpaga
                                ...
                                from recibopaga
                                     ...
                                     inner join arrecad on arrecad.k00_numpre = recibopaga.k00_numpre
                                                       and arrecad.k00_numpar = recibopaga.k00_numpar
                               where recibopaga.k00_numnov = rRecordDisbanco.numpre
                                 and disbanco.idret = rRecordDisbanco.idret) as x
                     ) as y
             ) as z;

      select coalesce(sum(k00_valor), 0)::numeric(15, 2)
        into nVlrCustaRecibo
        from recibopaga
       where k00_numnov = rRecordDisbanco.numpre
         and k00_hist = 11403;

      nVlrPgto := (rRecordDisbanco.vlrpago)::numeric(15, 2);
      nVlrDiferencaPgto := (nVlrCalculado - nVlrPgto)::numeric(15, 2);
      nVlrDiferencaPgtoCusta := (nVlrCalculado - (nVlrPgto - nVlrCustaRecibo))::numeric(15, 2);

      if lRaise then
        raise notice ' _____________________________________________________ ';
        raise notice '|                                                     |';
        raise notice '|-> RECIBO ...........................................: %', rRecordDisbanco.numpre;
        raise notice '|-> nVlrCalculado ....................................: %', nVlrCalculado;
        raise notice '|-> nVlrPgto .........................................: %', nVlrPgto;
        raise notice '|-> nVlrCustaRecibo ..................................: %', nVlrCustaRecibo;
        raise notice '|-> nVlrDiferencaPgto ................................: %', nVlrDiferencaPgto;
        raise notice '|-> nVlrDiferencaPgtoCusta ...........................: %', nVlrDiferencaPgtoCusta;

        perform fc_debug('  <PgtoParcial>  _____________________________________________________ ',lRaise);
        perform fc_debug('  <PgtoParcial> |                                                     |',lRaise);
        perform fc_debug('  <PgtoParcial> |-> RECIBO ...........................................: '|| rRecordDisbanco.numpre,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrCalculado ....................................: '|| nVlrCalculado,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrPgto .........................................: '|| nVlrPgto,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrCustaRecibo ..................................: '|| nVlrCustaRecibo,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrDiferencaPgto ................................: '|| nVlrDiferencaPgto,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrDiferencaPgtoCusta ...........................: '|| nVlrDiferencaPgtoCusta,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrToleranciaPgtoParcial ........................: '|| nVlrToleranciaPgtoParcial,lRaise);

      end if;

      if (

        (nVlrPgto > nVlrCustaRecibo) and
        (nVlrDiferencaPgto != 0) and
        (nVlrDiferencaPgtoCusta != 0) and
        (nVlrDiferencaPgtoCusta != nVlrToleranciaPgtoParcial)
        ...
        if lRaise then
          raise notice '|.....................................................|';
          raise notice '|------------------> PGTO. PARCIAL <------------------|';
          raise notice '|.....................................................|';
          raise notice '|-> RECIBO AVULSO ....................................: %', iNumpreReciboAvulso;

          perform fc_debug('  <PgtoParcial> |.....................................................|', lRaise);
          perform fc_debug('  <PgtoParcial> |------------------> PGTO. PARCIAL <------------------|', lRaise);
          perform fc_debug('  <PgtoParcial> |.....................................................|', lRaise);
          perform fc_debug('  <PgtoParcial> |-> RECIBO AVULSO ....................................: '|| iNumpreReciboAvulso, lRaise);
        end if;
        ...
        for rRecord in
            ...
            select recibopaga.k00_numcgm,
                   ...
                   tabrec.k02_codigo,
                   recibopaga.k00_hist,
                   sum(recibopaga.k00_valor) as k00_valor,
                   recibopaga.k00_dtvenc,
                   recibopaga.k00_tipo
              from (select recibopaga.k00_numcgm,
                   ...
                   inner join tabrec on tabrec.k02_codigo = taxa.ar36_receita
          group by recibopaga.k00_numcgm,
                   recibopaga.k00_dtoper,
                   recibopaga.k00_receit,
                   tabrec.k02_codigo,
                   recibopaga.k00_hist,
        ...
        loop
          ...
          nVlrTotalReciboAvulso := nVlrTotalReciboAvulso + rRecord.k00_valor;
        ...
        if lRaise then
          raise notice '|-> nVlrTotalReciboAvulso ............................: %', nVlrTotalReciboAvulso;
          perform fc_debug('  <PgtoParcial> |-> nVlrTotalReciboAvulso ............................: '|| nVlrTotalReciboAvulso, lRaise);
        end if;
        ...
        select string_agg(origem.info::text, ', ')
          ...
          from (select distinct 'Processo do Foro: [' || processoforo.v70_sequencial || ']' as info
                  from processoforo
                       inner join processoforopartilha on processoforopartilha.v76_processoforo = processoforo.v70_sequencial
                       inner join processoforopartilhacusta on processoforopartilhacusta.v77_processoforopartilha = processoforopartilha.v76_sequencial
                 where processoforopartilhacusta.v77_numnov = rRecordDisbanco.numpre
                union
                select distinct 'Inicial do Foro: [' || inicial.v50_inicial || ']' as info
                  from inicial
                       inner join inicialpartilha on inicialpartilha.v35_inicial = inicial.v50_inicial
                       inner join inicialpartilhacustas on inicialpartilhacustas.v36_inicialpartilha = inicialpartilha.v35_sequencial
                 where inicialpartilhacustas.v36_numnov = rRecordDisbanco.numpre) as origem;
        ...
        insert into arrehist (k00_numpre,
                              ...
                              k00_idhist) values (iNumpreReciboAvulso,
                                                  ...
                                                  '00:00',
                                                  1,
                                                  'Recibo avulso referente ao pagamento de custas de recibo que recebeu pagamento parcial. ( ' || sObservacao || ')',
                                                  null,
                                                  nextval('arrehist_k00_idhist_seq'));
        ...
        update inicialpartilhacustas
           set v36_numnov = iNumpreReciboAvulso
         where v36_numnov = rRecordDisbanco.numpre;

        update inicialpartilha
           set v35_dtpagamento = datausu
         where v35_sequencial in (select v36_inicialpartilha
                                    from inicialpartilhacustas
                                   where v36_numnov = iNumpreReciboAvulso);

        update processoforopartilhacusta
           set v77_numnov = iNumpreReciboAvulso
         where v77_numnov = rRecordDisbanco.numpre;

        update processoforopartilha
           set v76_dtpagamento = datausu
         where v76_sequencial in (select v77_processoforopartilha
                                    from processoforopartilhacusta
                                   where v77_numnov = iNumpreReciboAvulso);
    ...
    if lRaise then
      raise notice '---------------------> REGRA 6.5 <-------- END --------';
      perform fc_debug('  <PgtoParcial> ---------------------> REGRA 6.5 <-------- END --------', lRaise);
    end if;
    ...
    loop
      ...
      /**
       * REGRA 7 CALCULO RECIBO
       */
      if lRaise is true then

        raise notice '-> fc_proximo_dia_util(r_idret.k00_dtpaga) - %', fc_proximo_dia_util(r_idret.k00_dtpaga);
        raise notice '-> r_idret.dtpago - %', r_idret.dtpago;
        raise notice '-> lReciboPossuiPgtoParcial - %', lReciboPossuiPgtoParcial;
        raise notice '-> lReciboInvalidoPorTrocaDeReceita - %', lReciboInvalidoPorTrocaDeReceita;
      end if;
      ...
      then
        ...
        if lRaise is true then
          ...
          raise notice '-> nVlrCalculado CALCULO 1 - % - %', fc_proximo_dia_util(r_idret.k00_dtpaga), r_idret.dtpago;
        end if;
        ...
        if lRaise is true then
          ...
          raise notice '-> nVlrCalculado CALCULO 2';
        end if;
        ...
        perform fc_debug('  <PgtoParcial>  - nVlrCalculado '|| nVlrCalculado, lRaise, false, false);

        /**
         * Grava log do recibo antes de receber o delete.
         */
        insert into recibopagaold (k00_numcgm,
                                   k00_dtoper,
                                   k00_receit,
                                   ...
                                   k00_numpreold) select k00_numcgm,
                                                         ...
                                                         r_idret.numpre
                                                    from recibopaga
                                                   where k00_numnov = (select recibopaga.k00_numnov
                                                                         from recibopaga
                                                                              inner join arrecad on arrecad.k00_numpre = recibopaga.k00_numpre
                                                                                                and arrecad.k00_numpar = recibopaga.k00_numpar
                                                                        where recibopaga.k00_numnov = r_idret.numpre
                                                                          ...
                                                                          and exists (select 1
                                                                                        from recibounica
                                                                                       where recibounica.k00_numpre = recibopaga.k00_numpre)
                                                                        limit 1);
         ...
         * Using no arrecad existe para que em casos de pagamento em duplicidade indiferente da data de pagamento, não remova o desconto.
         * e creditos sejam gerados com o valor total do documento.
         *
         * @TODO - Sistema não da suporte a recibos de cota unica com bom pagador pagos atrasados.
         * @TODO - Sistema não da suporte a recibos de cota unica e bom pagador pago duas vezes.
         */
        delete from recibopaga
         using arrecad
         where recibopaga.k00_numnov = r_idret.numpre
           ...
           and not exists(select 1
                            from vistorianumpre
                           where vistorianumpre.y69_numpre = recibopaga.k00_numpre)
           and recibopaga.k00_numpre = arrecad.k00_numpre
           and recibopaga.k00_numpar = arrecad.k00_numpar;

        /**
         * Busca o que há de desconto referente a r_idret.numpre
         * Aqui, considera-se somente Alvará, e que possui compensação anterior ao pagamento do recibo
         */
        select coalesce(sum(recibopaga.k00_valor), 0)
          into nVlrDesconto
          from recibopaga
               inner join vistorianumpre on vistorianumpre.y69_numpre = recibopaga.k00_numpre
         where recibopaga.k00_numnov = r_idret.numpre
           and recibopaga.k00_hist = 918
           and exists (
               select true
                ...
                where arreckey.k00_numpre = recibopaga.k00_numpre
                  and abatimento.k125_tipoabatimento = 4
                  and abatimento.k125_datalanc <= r_idret.k00_dtpaga);

        if nVlrDesconto <> 0 then
          nVlrCalculado := nVlrCalculado + nVlrDesconto;
          perform fc_debug('  <PgtoParcial>  - Total de desconto de unica: ' || nVlrDesconto,lRaise,false,false);
          perform fc_debug('  <PgtoParcial>  - Valor calculado corrigido: ' || nVlrCalculado,lRaise,false,false);
        end if;
      ...
      nVlrCalculado := (nVlrCalculado)::numeric(15, 2);
      ...
      if lRaise is true then
        ...
        raise notice '-> nVlrDiferencaPgto .................................: %', nVlrDiferencaPgto;
        raise notice '-> nVlrDiferencaPgtoCusta ............................: %', nVlrDiferencaPgtoCusta;
        raise notice '-> nVlrToleranciaPgtoParcial .........................: %', nVlrToleranciaPgtoParcial;
        raise notice '-> nVlrToleranciaCredito .............................: %', nVlrToleranciaCredito;
        raise notice '-> nVlrCustaRecibo ...................................: %', nVlrCustaRecibo;
        raise notice '-------------------------------------------------------';
        ...
        perform fc_debug('  <PgtoParcial> -> nVlrDiferencaPgtoCusta ............................: '|| nVlrDiferencaPgtoCusta, lRaise);
        perform fc_debug('  <PgtoParcial> -> nVlrToleranciaPgtoParcial .........................: '|| nVlrToleranciaPgtoParcial, lRaise);
        perform fc_debug('  <PgtoParcial> -> nVlrToleranciaCredito .............................: '|| nVlrToleranciaCredito, lRaise);
        perform fc_debug('  <PgtoParcial> -> nVlrCustaRecibo ...................................: '|| nVlrCustaRecibo, lRaise);
        perform fc_debug('  <PgtoParcial> -------------------------------------------------------', lRaise);
        ...
        if (
          ...
          (
            (nVlrDiferencaPgto > 0) and 
            (nVlrDiferencaPgto > nVlrToleranciaPgtoParcial)
          ) or (
            (nVlrDiferencaPgto != 0) and 
            (nVlrDiferencaPgtoCusta > 0) and
            (nVlrDiferencaPgtoCusta > nVlrToleranciaPgtoParcial)
          ...
          perform fc_debug('  <PgtoParcial> ----------------------> PARCIAL <----------------------', lRaise);

        elsif (

          (nVlrDiferencaPgto != 0) and
          (nVlrDiferencaPgto > 0 or ((nVlrDiferencaPgto < 0 and abs(nVlrDiferencaPgto) > nVlrToleranciaCredito) and
                                     (nVlrDiferencaPgtoCusta < 0 and abs(nVlrDiferencaPgtoCusta) > nVlrToleranciaCredito)))

        ) then
          
          raise notice '------------------> CREDITO/DESCONTO <-----------------';
          perform fc_debug('  <PgtoParcial> ------------------> CREDITO/DESCONTO <-----------------', lRaise);

          if nVlrDiferencaPgto > 0 then
            raise notice '----------------------> DESCONTO <---------------------';
            perform fc_debug('  <PgtoParcial> ----------------------> DESCONTO <---------------------', lRaise);
          else
            raise notice '----------------------> CREDITO <----------------------';
            perform fc_debug('  <PgtoParcial> ----------------------> CREDITO <----------------------', lRaise);
          end if;
      ...
      if (
        ...
        if found then
          ...
          if lRaise is true then
            perform fc_debug('  <PgtoParcial>  - RECIBO COM TAXA! NAO PROCESSADO! ', lRaise, false, false);
            raise notice '-----------> RECIBO COM TAXA! NAO PROCESSADO <---------';
          end if;
       ...
       */
        ...
        if lRaise is true then
          ...
          raise notice '-> nVlrDiferencaPgto .................................: %', nVlrDiferencaPgto;
          raise notice '-> nVlrDiferencaPgtoCusta ............................: %', nVlrDiferencaPgtoCusta;
          raise notice '-> iReceitaPadraoCredito .............................: %', iReceitaPadraoCredito;

          perform fc_debug('  <PgtoParcial> -> nVlrCustaRecibo ...................................: '|| nVlrCustaRecibo, lRaise);
          perform fc_debug('  <PgtoParcial> -> nVlrCalculado .....................................: '|| nVlrCalculado, lRaise);
          perform fc_debug('  <PgtoParcial> -> nVlrDiferencaPgto .................................: '|| nVlrDiferencaPgto, lRaise);
          perform fc_debug('  <PgtoParcial> -> nVlrDiferencaPgtoCusta ............................: '|| nVlrDiferencaPgtoCusta, lRaise);
          perform fc_debug('  <PgtoParcial> -> iReceitaPadraoCredito .............................: '|| iReceitaPadraoCredito, lRaise);
        end if;

        if nVlrCalculado > 0 and abs(nVlrDiferencaPgtoCusta) > 0 then 
          
          nVlrDiferencaPgto := abs(nVlrDiferencaPgtoCusta);
          
          if lRaise is true then

            raise notice '-----------> CREDITO DE RECIBO COM CUSTAS <------------';
            raise notice '-> nVlrDiferencaPgto .................................: %', nVlrDiferencaPgto;
            raise notice '-> nVlrDiferencaPgtoCusta ............................: %', nVlrDiferencaPgtoCusta;

            perform fc_debug('  <PgtoParcial> -----------> CREDITO DE RECIBO COM CUSTAS <------------',lRaise);
            perform fc_debug('  <PgtoParcial> -> nVlrDiferencaPgto .................................: '|| nVlrDiferencaPgto,lRaise);
            perform fc_debug('  <PgtoParcial> -> nVlrDiferencaPgtoCusta ............................: '|| nVlrDiferencaPgtoCusta,lRaise);
          end if;
        ...
        select string_agg(k.descricao, '\n')
          into sObservacao
          from (select string_agg('Processo do Foro: ' || processoforo || ' -> ' || array_to_string(taxas, ', '), '\n') as descricao
                  from (select distinct v76_processoforo as processoforo,
                               array_agg((taxa.ar36_descricao || ': ' || (cast(processoforopartilhacusta.v77_valor::numeric as money)))) as taxas
                          from processoforopartilhacusta 
                               ...
                               inner join taxa on taxa.ar36_sequencial = processoforopartilhacusta.v77_taxa
                         where v77_numnov = r_idret.numpre
                         group by v76_processoforo
                       ) as i
                 union all
                select string_agg('Inicial do Foro: ' || inicial || ' -> ' || array_to_string(taxas, ', '), '\n') as descricao
                  from (select distinct v35_inicial as inicial,
                               array_agg((taxa.ar36_descricao || ': ' || (cast(inicialpartilhacustas.v36_valor::numeric as money)))) as taxas
                          from inicialpartilhacustas 
                               inner join inicialpartilha on inicialpartilha.v35_sequencial = inicialpartilhacustas.v36_inicialpartilha
                               inner join taxa on taxa.ar36_sequencial = inicialpartilhacustas.v36_taxa
                         where v36_numnov = r_idret.numpre
                         group by v35_inicial
                       ) as j
               ) as k;

        if sObservacao != '' then
          sObservacao := 'Lista de custas pagas:\n' || sObservacao;
        end if;
        ...
        if iTipoAbatimento = 3 then
          ...
          -- @TODO - dar next so se tiver custas
          select nextval('numpref_k03_numpre_seq')
            into iNumpreReciboAvulsoCustas;
          ...
          for rRecord in 
            select (case
                      when k02_reccredito is not null then k02_reccredito
                      when k00_receitacredito is not null then k00_receitacredito
                      else null
                    end) as receita_credito,
                   k00_receitacredito as receita_credito_tipo_debito,
                   round(sum(k00_valor), 2) as k00_valor,
                   k00_numcgm,
                   array_accum(k00_receit) as receitas
              from (select k00_numcgm,
                     ...
                     group by k00_numcgm,
                              x.k00_receit,
                              k00_tipo
                   ) as agrupamento_receita_credito
                   inner join tabrec on tabrec.k02_codigo = agrupamento_receita_credito.k00_receit
                   left join arretipo on arretipo.k00_tipo = agrupamento_receita_credito.k00_tipo
             group by receita_credito,
                      k00_receitacredito,
                      k00_numcgm
          loop

            -- @TODO verificar receitas certas
            nVlrReceita := (rRecord.k00_valor * (nPercPgto / 100))::numeric(15, 2);
            iReceitaCredito := rRecord.receita_credito;

            --
            -- if nVlrCalculado > 0 then              
              -- iReceitaCredito := rRecord.receita_credito_tipo_debito;
            -- end if;

            --Caso não tenha receita de crédito configurada para o tipo de débito, usamos a receita padrão dos parâmetros
            if iReceitaCredito is null or iReceitaCredito = 0 then
            ...
            if iReceitaCredito is null then
              ...
              codigo_retorno := 8;
              descricao := 'Receita de Crédito não configurada nos parâmetros, na configuração do tipo de débito ou na receita para o numpre: ' || r_idret.numpre;
              return;
            end if;

            if lRaise is true then
              perform fc_debug('  <PgtoParcial>  - ## lancando o recibo ref ao credito. ReceitaCredito: ' || iReceitaCredito || ' ValorReceita: ' || nVlrReceita, lRaise, false, false);
            end if;

            iNovoNumpreReciboAvulso := iNumpreReciboAvulso;

            perform 1
               from (select ar36_receita
                       from processoforopartilhacusta
                            inner join taxa on ar36_sequencial = v77_taxa
                            inner join disbanco on v77_numnov = k00_numpre
                            inner join processoforopartilha on processoforopartilha.v76_sequencial = processoforopartilhacusta.v77_processoforopartilha
                      where disbanco.idret = r_idret.idret
                        and ar36_receita = ANY(rRecord.receitas)
                      union all
                     select ar36_receita
                       from inicialpartilhacustas
                            inner join taxa on ar36_sequencial = v36_taxa
                            inner join disbanco on v36_numnov = k00_numpre
                            inner join inicialpartilha on inicialpartilha.v35_sequencial = inicialpartilhacustas.v36_inicialpartilha
                      where disbanco.idret = r_idret.idret
                        and ar36_receita = ANY(rRecord.receitas)
                 ) as x;

            if found and nVlrCalculado = 0 then

              lTrocouNumpre := true;
              iNovoNumpreReciboAvulso := iNumpreReciboAvulsoCustas;
              nVlrTotalCustasCredito := nVlrTotalCustasCredito + nVlrReceita;

              -- @TODO - testar caso do hist 11403 com numpre que não ta no arrecad

            end if;

            -- Altera o numpre do historico de custas para um numpre que esta em aberto no sistema
            update recibopaga 
               set k00_numpre = (select k00_numpre 
                                   from arrecad 
                                  where k00_numpre in (select k00_numpre 
                                                         from recibopaga 
                                                        where k00_numnov = r_idret.numpre)
                                  limit 1)
             where k00_numnov = r_idret.numpre
            ...
            -- @TODO - mudar o sql do loop para remover esse if insert update
            perform 1
               from recibo 
              where k00_numpre = iNovoNumpreReciboAvulso
                and k00_numpar = 1
                and k00_receit = iReceitaCredito
                and k00_tipo = iTipoReciboAvulso
                and k00_hist = 505;
            ...
            if not found then
              ...
              update recibo set k00_valor = (k00_valor + nVlrReceita)
               where k00_numpre = iNovoNumpreReciboAvulso
                 and k00_numpar = 1
                 and k00_receit = iReceitaCredito
                 and k00_tipo = iTipoReciboAvulso
            ...
            if not found then
              ...
              insert into arrenumcgm (k00_numcgm, k00_numpre) values (rRecord.k00_numcgm, iNovoNumpreReciboAvulso);
              
            end if;

          end loop;

          if lTrocouNumpre is true then
            ...
            select string_agg(origem.info::text, ', ')
              ...
              from (select distinct 'Processo do Foro: [' || processoforo.v70_sequencial || ']' as info
                      from processoforo 
                           inner join processoforopartilha on processoforopartilha.v76_processoforo = processoforo.v70_sequencial
                           inner join processoforopartilhacusta on processoforopartilhacusta.v77_processoforopartilha = processoforopartilha.v76_sequencial
                     where processoforopartilhacusta.v77_numnov = r_idret.numpre
                    union
                    select distinct 'Inicial do Foro: [' || inicial.v50_inicial || ']' as info
                      from inicial 
                           inner join inicialpartilha on inicialpartilha.v35_inicial = inicial.v50_inicial
                           inner join inicialpartilhacustas on inicialpartilhacustas.v36_inicialpartilha = inicialpartilha.v35_sequencial
                     where inicialpartilhacustas.v36_numnov = r_idret.numpre) as origem;
            ...
            insert into disbanco (k00_numbco,
                                  ...
                                  instit) select k00_numbco,
                                                 ...
                                                 dtarq,
                                                 dtpago,
                                                 round(nVlrTotalCustasCredito, 2),
                                                 0,
                                                 0,
                                                 0,
                                                 0,
            ...
            update abatimento 
               set k125_valor = (k125_valor - nVlrTotalCustasCredito), 
                   k125_valordisponivel = (k125_valordisponivel - nVlrTotalCustasCredito)
             where k125_sequencial = iAbatimento;
          ...
          if nVlrCalculado = 0 then
            ...
            if lTrocouNumpre then
              update disbanco
                 set vlrtot = (vlrtot - nVlrTotalCustasCredito),
                     vlrpago = (vlrpago - nVlrTotalCustasCredito)
               where idret = r_idret.idret;
            end if;
            ...
            -- @TODO - revisar esse update em credito das custas
            update tmpantesprocessar
               set vlrpago = round(vlrpago - nVlrDiferencaPgto, 2)
             where idret = r_idret.idret;
  ...
  loop
    ...
    loop
      ...
      -- PROCESSA DATA DE PAGAMENTO EM INICIALPARTILHA E PROCESSOFOROPARTILHA
      perform inicialpartilhacustas.v36_sequencial
         from inicialpartilhacustas
              inner join inicialpartilha on inicialpartilha.v35_sequencial = inicialpartilhacustas.v36_inicialpartilha
        where inicialpartilhacustas.v36_numnov = r_idret.numpre
          and inicialpartilhacustas.v36_dispensalancamentorecibo = false
      ...
      if found then
        ...
        update inicialpartilha
           set v35_dtpagamento = datausu
         where v35_tipolancamento = 1
           and v35_sequencial in (select v36_inicialpartilha
                                   from inicialpartilhacustas
                                  where v36_numnov = r_idret.numpre);

      end if;

      perform processoforopartilhacusta.v77_sequencial
         from processoforopartilhacusta
              inner join processoforopartilha on processoforopartilha.v76_sequencial = processoforopartilhacusta.v77_processoforopartilha
        where processoforopartilhacusta.v77_numnov = r_idret.numpre
          and processoforopartilhacusta.v77_dispensalancamentorecibo = false
      ...
      if found then
        ...
        update processoforopartilha
           set v76_dtpagamento = datausu
         where v76_tipolancamento = 1
           and v76_sequencial in (select v77_processoforopartilha
                                    from processoforopartilhacusta
                                   where v77_numnov = r_idret.numpre);
...
-- TESTA SE EXISTE NUMPRE E NUMPAR NO ARREIDRET, NAO EXISTINDO INSERE O IDRET DO PAGAMENTO
          ...
          loop
            ...
            loop
              ...
              --
               -- Modificaçao realizada devido ao erro gerado na tarefa 32607
              -- Motivo do erro:
...
SQL
    ...
    {
        $this->execute(<<<SQL
drop index arretipo_taxaespecifica_in;
alter table arretipo drop constraint arretipo_taxaespecifica_fk;
alter table arretipo drop column k00_taxaespecifica;

delete from histcalc where k01_codigo in(11505, 11605);
SQL
        );
    }
    ...
    {
        $this->execute(<<<SQL
delete from db_syscadind where codind = 1008585;
delete from db_sysindices where codind = 1008585;
delete from db_sysforkey where codcam = 1011642;
delete from db_sysarqcamp where codcam = 1011642;
delete from db_syscampo where codcam = 1011642;
SQL
);
    }

    private function downLayout()
    {
        $this->execute(<<<SQL
delete from db_layoutcampos where db52_codigo in(171800, 171801);
SQL
);
    }
...
declare
  ...
  nVlrTotalReciboAvulso            numeric(15,2) default 0;
  nVlrDesconto                     numeric(15,2) default 0;
  nVlrTaxaExpediente               numeric(15,2) default 0;
   ...
   */
  iNumprePagamentoParcial          integer default 0;
  historicoTaxaExpediente          integer;
  historicoPagamentoTaxaExpediente integer;
...
begin
  ...
  historicoTaxaExpediente := 11505;
  historicoPagamentoTaxaExpediente := 11605;
 ...
 */
  if lAtivaPgtoParcial is true then
    ...
    /****************************************************************************/
    /*************** REGRA PAGAMENTO PARCIAL COM TAXA DE EXPEDIENTE *************/
    /****************************************************************************/
    if lRaise then
      raise notice '---------------------> REGRA 6.6 <------- BEGIN -------';
      perform fc_debug('  <PgtoParcial> ---------------------> REGRA 6.6 <------- BEGIN -------',lRaise);
    end if;
    ...
    for rRecordDisbanco in
        ...
        select distinct
         ...
         where disbanco.codret = cod_ret
           ...
           and exists (select true
                         from recibopaga
                        where k00_hist = historicoTaxaExpediente
                          and recibopaga.k00_numnov = disbanco.k00_numpre
                        )
      group by disbanco.k00_numpre,
               disbanco.k00_numpar,
    ...
    loop
      ...
      -- Calculo da taxa de expediente
      select coalesce(sum(k00_valor), 0)::numeric(15, 2)
        into nVlrTaxaExpediente
        from recibopaga
       where k00_numnov = rRecordDisbanco.numpre
         and k00_hist = historicoTaxaExpediente;

      nVlrCalculado := nVlrCalculado + nVlrTaxaExpediente;
      nVlrPgto := (rRecordDisbanco.vlrpago)::numeric(15, 2);
      nVlrDiferencaPgto := (nVlrCalculado - nVlrPgto)::numeric(15, 2);

      if lRaise then
        raise notice '|-> Disbanco Numpre ..................................: %', rRecordDisbanco.numpre;
        raise notice '|-> nVlrPgto .........................................: %', nVlrPgto;
        raise notice '|-> nVlrCalculado ....................................: %', nVlrCalculado;
        raise notice '|-> nVlrDiferencaPgto ................................: %', nVlrDiferencaPgto;
        raise notice '|-> nVlrTaxaExpediente ...............................: %', nVlrTaxaExpediente;

        perform fc_debug('  <PgtoParcial> |-> Disbanco Numpre ..................................: '|| rRecordDisbanco.numpre,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrPgto .........................................: '|| nVlrPgto,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrCalculado ....................................: '|| nVlrCalculado,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrDiferencaPgto ................................: '|| nVlrDiferencaPgto,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrToleranciaPgtoParcial ........................: '|| nVlrToleranciaPgtoParcial,lRaise);
        perform fc_debug('  <PgtoParcial> |-> nVlrTaxaExpediente ...............................: '|| nVlrTaxaExpediente,lRaise);

      end if;

      if (
        (nVlrDiferencaPgto != 0) and
        (nVlrTaxaExpediente != 0) and
        (nVlrTaxaExpediente != nVlrToleranciaPgtoParcial)
      ) then
        ...
        for rRecord in
            ...
            select recibopaga.k00_numcgm,
              ...
              from (select recibopaga.k00_numcgm,
                      ...
                      from recibopaga
                     where recibopaga.k00_numnov = rRecordDisbanco.numpre
                       and recibopaga.k00_hist = historicoTaxaExpediente
        ...
        insert into arrehist (k00_numpre,
                              ...
                              k00_idhist) values (iNumpreReciboAvulso,
                                                  1,
                                                  historicoTaxaExpediente,
                                                  datausu,
                                                  '00:00',
                                                  1,
                                                  'Recibo avulso referente ao pagamento da taxa de expediente de recibo que recebeu pagamento parcial.',
                                                  null,
                                                  nextval('arrehist_k00_idhist_seq'));
        ...
        update disbanco
           set vlrpago = (vlrpago - nVlrTotalReciboAvulso),
               vlrtot = (vlrtot - nVlrTotalReciboAvulso),
               vlrcalc = (vlrcalc - nVlrTotalReciboAvulso)
         where idret = rRecordDisbanco.idret;
    ...
    if lRaise then
      raise notice '---------------------> REGRA 6.6 <-------- END --------';
      perform fc_debug('  <PgtoParcial> ---------------------> REGRA 6.6 <-------- END --------', lRaise);
    end if;
    ...
    loop
      ...
      nVlrPgto := (r_idret.vlrpago)::numeric(15, 2) + nVlrTaxaExpediente;
      nVlrCalculado := (nVlrCalculado)::numeric(15, 2);
                               ...
                               from ( select k00_numcgm,
                                             ...
                                             case
                                               when exists ( select 1
                                                               from tmplista_unica
                                                              where idret = r_idret.idret ) then 990
                                               when k00_hist = historicoTaxaExpediente then historicoPagamentoTaxaExpediente
                                               else k00_hist
                                             end as k00_hist,
                                             round((k00_valor / r_idret.k00_valor) * r_idret.vlrpago, 2) as k00_valor,
                                             k00_numpre,
        ...
        */
        insert into arrepaga (
          ...
          k00_dtpaga
                 ...
                 min(datausu),
                  recibo.k00_receit,
                  case when k00_hist = historicoTaxaExpediente then historicoPagamentoTaxaExpediente else k00_hist end as k00_hist,
                 sum(round((recibo.k00_valor / valorrecibo) * r_idret.vlrpago, 2)),
                 min(datausu),
                  recibo.k00_numpre,
                  recibo.k00_numpar,
...
SQL
    ...
    {
        $this->execute(<<<SQL
alter table arretipo add column k00_taxaespecifica int4;
alter table arretipo add constraint arretipo_taxaespecifica_fk foreign key (k00_taxaespecifica) references tabdesc;
create index arretipo_taxaespecifica_in on arretipo(k00_taxaespecifica);

insert into histcalc (k01_codigo, k01_descr, k01_tipo) values(11505, 'TAXA DE EXPEDIENTE', '');
insert into histcalc (k01_codigo, k01_descr, k01_tipo) values(11605, 'PGTO TAXA EXPEDIENTE', '');
SQL
);
    }
    ...
    {
        $this->execute(<<<SQL
insert into db_syscampo values(1011642,'k00_taxaespecifica','int4','Taxa Específica a ser aplicada no tipo de débito.','0', 'Taxa Específica',10,'t','f','f',1,'text','Taxa Específica');
insert into db_sysarqcamp values(82,1011642,45,0);
insert into db_sysforkey values(82,1011642,1,79,0);
insert into db_sysindices values(1008585,'arretipo_taxaespecifica_in',82,'0');
insert into db_syscadind values(1008585,1011642,1);
SQL
);
    }

    private function upLayout()
    {
        $this->execute(<<<SQL
insert into db_layoutcampos( db52_codigo ,db52_layoutlinha ,db52_nome ,db52_descr ,db52_layoutformat ,db52_posicao ,db52_default ,db52_tamanho ,db52_ident ,db52_imprimir ,db52_alinha ,db52_obs ,db52_quebraapos ) values ( 171800 ,281 ,'valor_taxa_expediente' ,'VALOR DA TAXA DE EXPEDIENTE' ,3 ,421 ,'' ,10 ,'f' ,'t' ,'e' ,'' ,0 );
insert into db_layoutcampos( db52_codigo ,db52_layoutlinha ,db52_nome ,db52_descr ,db52_layoutformat ,db52_posicao ,db52_default ,db52_tamanho ,db52_ident ,db52_imprimir ,db52_alinha ,db52_obs ,db52_quebraapos ) values ( 171801 ,115 ,'valor_taxa_expediente' ,'VALOR DA TAXA DE EXPEDIENTE' ,1 ,1808 ,'0' ,10 ,'f' ,'t' ,'d' ,'' ,0 );
SQL
);
    }
}
