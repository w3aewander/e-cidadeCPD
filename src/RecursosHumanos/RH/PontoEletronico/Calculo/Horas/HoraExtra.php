...
class HoraExtra extends BaseHora
{
    ...
    protected function verificaHorasExtrasIntervalo()
    {
        ...
        $aMarcacoes = $this->getDiaTrabalho()->getMarcacoesSemAlteracao();
        ...
        if (!$aMarcacoes->getMarcacaoSaida1()->hasMarcacaoLancada() && !$aMarcacoes->getMarcacaoEntrada2()->hasMarcacaoLancada()) {
            ...
            if (!$oIntervaloMarcacoes->diff($oIntervaloJornada)->invert) {
                ...
                $horaExtraIntervalo = clone $oIntervaloJornada;
                $horas              = $horaExtraIntervalo->diff($oIntervaloMarcacoes);
                $minutosHoraExtra   = BaseHora::converterIntervaloEmMinutos($horas);
                if ($minutosHoraExtra > $minutosTolerancia) {
    ...
    {
        $this->logger->debug('-- Entrou no metodo totalHorasExtras() com parametro: ' . ($lCalcula ? 'TRUE' : 'FALSE'));
        ...
        if ($lCalcula) {
            ...
            $ultimaHoraCalcular = $this->getUltimasHoras();
            $this->oHoraExtraFinal = $this->verificaHorasExtras($ultimaHoraCalcular, false);
    ...
    private function calcularHoraExtraNoturnaIntervalo()
    {
        ...
        if ($aMarcacoes->getMarcacaoSaida1() != null && $aMarcacoes->getMarcacaoEntrada2() != null) {
            ...
            if (!$aMarcacoes->getMarcacaoSaida1()->hasMarcacaoLancada() && !$aMarcacoes->getMarcacaoEntrada2()->hasMarcacaoLancada()) {
                ...
                if (!empty($aHorasJornada[1]->oHora) && !empty($aHorasJornada[2]->oHora)) {

                    $diferencaIntervalo = $aHorasJornada[1]->oHora->diff($aHorasJornada[2]->oHora);
                    $oIntervaloJornada->add($diferencaIntervalo);
                }
    ...
    public function calcularHorasExtrasFolgaFeriadoNoturna()
    {
            ...
            if ($this->horaEstaNoIntervalo($marcacao->getMarcacao(), $oInicioPeriodoNoturno, $oFimPeriodoNoturno) || $oFimPeriodoNoturnoAnterior->diff($marcacao->getMarcacao())->invert) {
                ...
                switch ($marcacao->getTipo()) {
                    ...
                    default:
                        ...
                        $proximaMarcacao = $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacao($marcacao->getTipo() + 1);
        ...
        if ($this->getDiaTrabalho()->getJornada()->isFolga()) {
            ...
            $marcacoesSaida = $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacoesSaida();
    ...
    public function atualizaTotalizadoresEMarcacoes()
    {
        ...
        if ($this->getDiaTrabalho()->getMarcacoesSemAlteracao()->isEmpty()) {
            return;
        }
        ...
        $oPrimeiraMarcacao = clone $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacaoEntrada1();
        $oUltimaMarcacao = clone $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getUltimaMarcacaoComRegistro();

        $oHoraExtraInicial = $this->getHoraZerada();

        if ($oJornadaEntrada->oHora->getTimestamp() > $oPrimeiraMarcacao->getMarcacao()->getTimestamp()) {

            $horaLimite = $oJornadaEntrada->oHora;

            if($this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacaoSaida1()) {
                if($this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacaoSaida1()->getMarcacao()) {
                    if($oJornadaEntrada->oHora->getTimestamp() > $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacaoSaida1()->getMarcacao()) {
                        $horaLimite = $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacaoSaida1()->getMarcacao();
                    }
                }
            }

            $intervaloHoraExtraInicio = $this->getDiferencaHoras($oPrimeiraMarcacao->getMarcacao(), $horaLimite);
        ...
        if ($oJornadaSaida->oHora->getTimestamp() < $oUltimaMarcacao->getMarcacao()->getTimestamp()) {
            ...
            $this->logger->debug('-- IntervaloHoraExtraFinal...........:'. $intervaloHoraExtraFinal);
        ...
        if ($oSomaExtraInicialFinal->getTimestamp() > $oHorasAutorizadas->getTimestamp()) {
            ...
            if (!$this->isZero($oHoraExtraInicial)) {
                ...
                if ($oHoraExtraInicial->getTimestamp() > $oHorasExtrasSobrando->getTimestamp()) {
                    ...
                    $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->atualizaMarcacao($oPrimeiraMarcacao);
            ...
            if (!$this->isZero($oHoraExtraIntervalo)) {
                ...
                if ($oHoraExtraIntervalo->getTimestamp() > $oHorasExtrasSobrando->getTimestamp()) {
                    ...
                    $oSegundaMarcacao = clone $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacaoSaida1();
                    $oTerceiraMarcacao = clone $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->getMarcacaoEntrada2();
                    ...
                    $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->atualizaMarcacao($oTerceiraMarcacao);
            ...
            if (!$this->isZero($oHoraExtraFinal)) {
                ...
                if ($oHoraExtraFinal->getTimestamp() > $oHorasExtrasSobrando->getTimestamp()) {
                    ...
                    $this->getDiaTrabalho()->getMarcacoesSemAlteracao()->atualizaMarcacao($oUltimaMarcacao);
                }
            }
        }
    }
