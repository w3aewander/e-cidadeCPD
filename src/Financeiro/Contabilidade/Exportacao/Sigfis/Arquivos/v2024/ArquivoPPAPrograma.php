            ...
            foreach ($aProgramas as $oPrograma) {
                ...
                for ($iIndicador = 0; $iIndicador < pg_num_rows($rsIndicadoresPrograma); $iIndicador++) {
                    ...
                    for ($j=0; $j<$numRows; $j++) {
                        ...
                        $oDadosPrograma->Identificador = $identificador;
                        $nomeIndicador =  Helper::convertAndLimit($oDadosIndicador->o10_descr, 200);
                        $oDadosPrograma->Nome = $nomeIndicador;
                        $oDadosPrograma->ValorModificado =  $dadosEsperadoIndicador->o25_valor;
                        $oDadosPrograma->ValorFinal = $valorFinal;
                        $oDadosPrograma->CodigoPPAPrograma = $oPrograma->codigo;
                        $oDadosPrograma->Ano = $dadosEsperadoIndicador->o25_anousu;
                        $oDadosPrograma->CodigoUnidadeGestora = $this->sCodigoTribunal;
                        
                        $programa[] = (object) ['PPAPrograma' => $oDadosPrograma];
                    }
                }
            }
        }
        
        $RemessaPPAPrograma->Indicadores = $programa;
