...
class LotePropLicitaCon extends ArquivoLicitaCon
{
    ...
    public function getDados()
    {
        ...
        $sSqlLotes = $oDaoLicitacao->sqlPropostasLoteLicitacon(implode(', ', $aCampos), implode(' AND ', $aWhere), $sGroupBy);
        ...
        for ($iLinha = 0; $iLinha < $iTotalLotes; $iLinha++) {
            ...
            $sSqlNrLote = "select min(l04_codigo) as nr_lote from liclicita
                                   ...
                                   where liclicitemlote.l04_descricao ilike '{$sAgrupador}' and liclicita.l20_codigo = {$oLinha->l20_codigo}";
            ...
            if (!isset($aLicitacoes[$oLinha->l20_codigo][$oLinha->z01_numcgm][$sAgrupador])) {
                ...
                $oStdLote->NR_LOTE = $oLinha->nr_lote != '1' ? $nrLote->nr_lote : $oLinha->nr_lote;
                $oStdLote->PC_DESCONTO = null;
                $oStdLote->VL_TOTAL_LOTE = $oLinha->vl_total_item ?: 0;

                if (empty($oStdLote->VL_TOTAL_LOTE)) {
                    continue;
                }
                ...
                if ($oHabilitacao->situacao == 2) {
                    $oStdLote->TP_RESULTADO_HABILITACAO = '';
                    ...
                    if ($lJulgamentoLote) {
                        $oStdLote->TP_RESULTADO_HABILITACAO = 'I';
                    }
                }
