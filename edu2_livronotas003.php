...
// a impressão das notas são na linha abaixo e a variavel $nAproveitamento esta recebendo os valores

			if( $contaNota < 2 )
			{
				
			   $oPdf->Cell($iLarguraQuadroAvaliacao * 2, $oControle->iAlturaLinha, $nAproveitamento, 1, 0, "C");
			   $notaRRS += $nAproveitamento;
			   $contaNota++;
			}else{
				
			   $oPdf->Cell($iLarguraQuadroAvaliacao * 2, $oControle->iAlturaLinha, $notaRRS, 1, 0, "C");
			   $contaNota = 0;
			   $notaRRS   = 0;
			}
...
//$arq = fopen("/dados/www/homologacao.epdvr.com.br/disciplina2.txt","a+");
//fwrite($arq, $sNomeDisciplina);
//fwrite($arq,"\r\n");
//fclose($arq); 		
  ...
  foreach ($aPeriodo as $oPeriodo) {
    ...
    if( $sDescricaoPeriodo == 'RESULTADO RECUPERAÇÃO SEMESTRAL' )
    {
		$sDescricaoPeriodo = 'RRS';
	}	
    if( $sDescricaoPeriodo == 'RECUPERAÇÃO SEMESTRAL' )
    {
		$sDescricaoPeriodo = 'RS';
	}	
  ...
  if (count($aRegente) > 0) {
      ...
      $sProfessorSocial = $oRegente->getNomeSocial();
	  	break;
  	}
  }

  $sDados  = "Escola: {$sNomeEscola}\n";
  if(!empty($sProfessorSocial) && $sProfessorSocial != ""){
    $sDados .= "Profº: {$sProfessor}" . "\nNome Social: " . $sProfessorSocial . PHP_EOL;
  } else {
    $sDados .= "Profº: {$sProfessor}\n";
  }
  ...
  $GLOBAL['materia'] = $sNomeDisciplina;
  $oPdf->SetXY(10, 14);
  $oPdf->SetFont('arial', '', 7);
