    ...
    <fieldset >
      ...
      <table class="form-container" >
        ...
        <tr>
          ...
          <tr>
              <td><label for="data-inicio">Data Início Exercício: </label></td>
              <td><input type="text" id="data-inicio"></td>
              <td><label for="data-fim">Até: </label></td>
              <td><input type="text" id="data-fim"></td>
          </tr>
      </table>
      ...
      <button type="button" class="btn btn-light" name="salvar" id="salvarAtividade">
          <i class="fa fa-save"></i>
          Salvar
      </button>
      <button type="button" class="btn btn-light" name="excluir" id="excluirAtividade" disabled="disabled">
          <i class="fa fa-trash-alt"></i>
          Excluir
      </button>
      <button type="button" class="btn btn-light" name="cancelar" id="cancelarAcao" >
          <i class="fa fa-ban"></i>
          Cancelar
      </button>
  </form>
...
const dataInicio = new DBInputDate(inputDataInicio);
const dataFim = new DBInputDate(inputDataFim);

$('ed05_c_finalidade').addClassName('readonly');
$('ed05_c_finalidade').setAttribute('disabled', 'disabled');
...
var aHeaders   = ['Função/Atividade', 'Turno', 'Tipo de Hora', 'Data Início', 'Data Fim', 'Ação', 'codigo_atividade', 'codigos_agendas'];
var aCellWidth = ['30%', '15%', '20%', '15%', '10%', '10%'];
var aCellAlign = ['left', 'center', 'left', 'center', 'center', 'center'];

oGridAtividades.setCellWidth(aCellWidth);
oGridAtividades.setCellAlign(aCellAlign);
oGridAtividades.setHeader(aHeaders);
oGridAtividades.aHeaders[6].lDisplayed=false;
...
function js_buscaTipoHora() {
  ...
  var oParamentros = {exec: 'buscaTipoHoraPorAtividade', iAtividade : $F('ed22_i_atividade')};
...
function js_retornoAtividadesProfissional ( oRetorno, lErro) {
  ...
  oRetorno.aAtividades.each( function (oAtividade) {
    ...
    aLinha.push('');
    aLinha.push('');
    aLinha.push(oAtividade.dataInicio);
    aLinha.push(oAtividade.dataFim);
    ...
    if ( !empty(oAtividade.aResumoTurno) ) {
      for (var iIndice in oAtividade.aResumoTurno ) {
        ...
        aCloneLinha[5]  = oBtnAlterar.outerHTML + ' ' + oBtnExcluir.outerHTML;
...
function js_CarregaDadosSemAgenda(oAtividade) {
  ...
  $('data-inicio').value = oAtividade.dataInicio;
  $('data-fim').value = oAtividade.dataFim;
  $('atolegal').style.display = 'none';
...
function js_AlterarAtividadeSemAgenda(iCodigoAgenda) {
  ...
  $('identificacao').setAttribute('disabled', 'disabled');
  $('z01_nome').addClassName('readonly');
  $('z01_nome').setAttribute('disabled', 'disabled');
  $('ed01_c_descr').addClassName('readonly');
  $('ed01_c_descr').setAttribute('disabled', 'disabled');
...
function js_validaDadosSalvar() {
    ...
    if (empty(dataInicio.__toLocaleDateString())) {
        alert("Data de Início do Exercício deve ser preenchido.");
        return false;
    }

    if (js_formatar(dataInicio.__toLocaleDateString(), "d") > js_formatar(dataFim.__toLocaleDateString(), "d")) {
...
$('salvarAtividade').observe('click', function () {
  ...
  oParamentros.dataInicio     = dataInicio.__toLocaleDateString();
  oParamentros.dataFim        = dataFim.__toLocaleDateString();
...
function js_reloadAbas() {
  ...
  (window.CurrentWindow || parent.CurrentWindow).corpo.iframe_a5.location.href = (window.CurrentWindow || parent.CurrentWindow).corpo.iframe_a5.location.href;
  (window.CurrentWindow || parent.CurrentWindow).corpo.iframe_a6.location.href = (window.CurrentWindow || parent.CurrentWindow).corpo.iframe_a6.location.href;
}
