...
<form class="container" name="form1" method="post" action="dvr3_diversos00<?=$p?>.php">
  ...
  if(!empty($selectImportarPlanilha)) {
    ...
    $dadosValidos = str_replace("\\","",$_POST["dadosValidos"]);
    $dadosValidos = str_replace("[","",$dadosValidos);
    $dadosValidos = str_replace("]","",$dadosValidos);
    $dadosValidos = str_replace("\"","",$dadosValidos);

    $dadosInvalidos = str_replace("\\","",$_POST["dadosInvalidos"]);
    $dadosInvalidos = str_replace("[","",$dadosInvalidos);
    $dadosInvalidos = str_replace("]","",$dadosInvalidos);
    $dadosInvalidos = str_replace("\"","",$dadosInvalidos);

    if ($selectImportarPlanilha == 'cgm') {
      $tituloCadastro = 'CGM';
    } else if ($selectImportarPlanilha == 'matric') {
      $tituloCadastro = 'Matrícula';
    } else if ($selectImportarPlanilha == 'inscr') {
      $tituloCadastro = 'Inscrição';
    }

  ?>
  <input type="hidden" name="importarplanilha" value="<?=$importarplanilha['name']?>">
  <input type="hidden" name="selectImportarPlanilha" value="<?=$selectImportarPlanilha?>">
  <input type="hidden" name="dadosValidos" value="<?=$dadosValidos?>">
  <input type="hidden" name="dadosInvalidos" value="<?=$dadosInvalidos?>">
  ...
  <fieldset>
    <legend>Cadastro de diversos <?php echo !empty($selectImportarPlanilha) ? "Em Lote (" . $tituloCadastro .")": ""; ?></legend>
...
function js_volta() {
  location.href = "dvr3_diversos00<?=($db_opcao == 2 ? 6 : 7) ?>.php";
}
<?
echo "js_trocatotal();";
