...
$clrotulo->label("q02_dtcada");
$clrotulo->label("q30_areapublicidade");
$clrotulo->label("q30_tempofuncionamento");
...
global $cepIssRuas;
if (@$q02_inscr!="") {
    $sSqlIss  = "select issbase.*, issruas.z01_cep as cepissruas, escrito.q10_numcgm, cgm.z01_nome as z01_nome_escrito, q177_setorfiscal";
    ...
    $sSqlIss .= "   left join isssetorfiscal on q177_issbase  = issbase.q02_inscr ";
  ...
  ********************************************************************************************************************
  ...
  <fieldset class='separator'>
    ...
    ?>
     ...
     <tr>
       <td><strong>Controle Local Atividade</strong></td>
       <td >
  ...
  ********************************************************************************************************************
  ...
  <fieldset class ='separator'>
    ...
    <table border="0" width="100%" >
      ...
      <tr>
        <td nowrap title="<?=@$Tq177_setorfiscal?>">
          <?php
            db_ancora($Lq177_setorfiscal,"js_pesquisaq177_setorfiscal(true);",$db_opcao);
          ?>
        </td>
        <td colspan="2">
          <?php
            $j90_descr = "";
            if (!empty($q177_setorfiscal)) {
              // Busca descrição do distrito na tabela setorfiscal
              $sql = "SELECT j90_descr FROM setorfiscal WHERE j90_codigo = $q177_setorfiscal";
              $result = db_query($sql);
              db_fieldsmemory($result, 0);
            }
            db_input('q177_setorfiscal',10,$Iq177_setorfiscal,true,'text',$db_opcao," onchange='js_pesquisaq177_setorfiscal(false);'");
            db_input('j90_descr',53,$j90_descr,true,'text',3,'');
          ?>
        </td>
      </tr>
      ...
      <tr>
        ...
        <td >
            ...
            if (!isset($q30_quant) && $db_opcao!=33 && $db_opcao!=3 && $db_opcao!=2 && $db_opcao!=22) {
                $q30_quant='1';
            } elseif (isset($q05_matric) && $q05_matric!="" && isset($j39_area)) {
              if(!isset($q30_area) || $q30_area == ''){
                $q30_area = $j39_area;
              }
            }
      ...
      <tr>
        ...
        <td align="right" nowrap>
            <?=@$Lq30_tempofuncionamento?>
            <?php
            db_input('q30_tempofuncionamento', 18, $Iq30_tempofuncionamento, true, 'text', $db_opcao, "")
            ?>
        </td>
      </tr>
      <tr>
          <td nowrap title="<?=@$Tq30_areapublicidade?>">
              <?=@$Lq30_areapublicidade?>
          </td>
          <td colspan="2">
              <?php
                  db_input('q30_areapublicidade', 10, $Iq30_areapublicidade, true, 'text', $db_opcao, "")
              ?>
              m²
          </td>
      </tr>
        <tr>
            <td nowrap title="<?=@$Tq30_graurisco?>">
                <?=@$Lq30_graurisco?>
            </td>
            <td colspan="2">
                <?php
                    $aGrausRisco = ["" => "", "A" => "ALTO", "M" => "MÉDIO", "B" => "BAIXO"];
	                db_select("q30_graurisco", $aGrausRisco, true, $db_opcao);
                ?>
            </td>
        </tr>
      ...
      <tr>
        ...
        <td colspan="2">
            <?php
            db_input('q14_proces', 10, $Iq14_proces, true, 'hidden', 3, '');
            db_input('p58_numero', 10, $Ip58_numero, true, 'text', $db_opcao, " onchange='js_pesquisaq14_proces(false);'");
            db_input('p58_requer', 43, $Ip58_requer, true, 'text', 3, '');
            ?>
        <td>
      </tr>
      ...
      <tr style=''>
        ...
        <td align='right'>
          <?php
            if ($tam_cgccpf==14) {
                ?>
                <?=$Lq02_protocolojuntacomercial?>
                <?php
                db_input('q02_protocolojuntacomercial', 18, $Iq02_protocolojuntacomercial, true, 'text', $db_opcao, "");
            } else {
                $q02_protocolojuntacomercial="";
                db_input('q02_protocolojuntacomercial', 18, $Iq02_protocolojuntacomercial, true, 'hidden', 3, "");
            }
            db_input('q02_formalocalvara', 18, '', true, 'hidden', 3, "");
          ?>
        </td>
      </tr>
...
// Busca na tabela issruas de acordo com a inscrição atual
if(!empty($q02_inscr)){
  $rsClissruas = $clissruas->sql_record($clissruas->sql_query($q02_inscr,"*",null,""));
  $numrowsClissruas = $clissruas->numrows;
  db_fieldsmemory($rsClissruas,0);
}
...
function js_getDescricaoLocalAlvara(oRetorno)
{
  ...
  js_alteraselectformalocalvara();

}

function js_alteraselectformalocalvara()
{
  var
    obj = document.form1,
    formalocalAlvara = obj.q02_formalocalvara.value;

  if (obj.q02_inscr.value > 0) {
    obj.selectformalocalvara.value = formalocalAlvara;
  }
}
...
function js_testaproc(opc) {
  ...
  if(+document.form1.q30_tempofuncionamento.value > 24){
    alert("Horas de funcionamento diárias não podem ultrapassar 24!");
    document.form1.q30_tempofuncionamento.focus();
    return false;
  }
...
function js_pesquisaq14_proces(mostra) {
  ...
  if (mostra==true) {
    ...
    js_OpenJanelaIframe('CurrentWindow.corpo.iframe_issbase','db_iframe_proc','func_protprocesso.php?funcao_js=parent.js_mostraprocesso1|p58_codproc|p58_numero|p58_requer','Pesquisa',true);
  } else {
    if (document.form1.p58_numero.value != '') {

      js_OpenJanelaIframe('CurrentWindow.corpo.iframe_issbase','db_iframe_proc','func_protprocesso.php?chave_p58_numero='+document.form1.p58_numero.value+'&funcao_js=parent.js_mostraprocesso|p58_codproc|p58_requer','Pesquisa',false);
    } else {
...
function js_pesquisaq177_setorfiscal(mostra){
  if(mostra==true){
    ...
    if(document.form1.q177_setorfiscal.value != ''){
      js_OpenJanelaIframe('','db_iframe_setorfiscal','func_setorfiscal.php?pesquisa_chave='+document.form1.q177_setorfiscal.value+'&funcao_js=parent.js_mostrasetorfiscal','Pesquisa',false);
    }else{
...
function js_mostrasetorfiscal(chave,erro){
  ...
  if(erro==true){
    document.form1.q177_setorfiscal.focus();
    document.form1.q177_setorfiscal.value = '';
  }
}
function js_mostrasetorfiscal1(chave1,chave2){
  document.form1.q177_setorfiscal.value = chave1;
...
function js_mostraprocesso(chave,nome,erro) {

  document.form1.q14_proces.value = chave;
  document.form1.p58_requer.value = nome;
  if (erro==true) {
    document.form1.p58_numero.focus();
    document.form1.q14_proces.value = '';
  }
}
  function js_mostraprocesso1(chave,numero,nome,erro) {

    document.form1.q14_proces.value = chave;
    document.form1.p58_numero.value = numero;
    document.form1.p58_requer.value = nome;
    if (erro==true) {
      document.form1.p58_numero.focus();
      document.form1.q14_proces.value = '';
