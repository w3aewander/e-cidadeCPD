...
for ($i = 0; $i < count($arr_niveis); $i++) {
    ...
    if ($nivel == 1) {
        ...
        // Recurso
        if (trim($selecao) != "") {
            $selecao .= ",gestao, descricao";
        } else {
            $selecao = "select gestao, descricao";
        }

        if (trim($agrupar) != "") {
            $agrupar .= ",gestao,descricao";
        } else {
            $agrupar = "group by gestao,descricao";
        }

        if (trim($ordem) != "") {
            $ordem .= ",gestao,descricao";
        } else {
            $ordem = "order by gestao,descricao";
        }
    }
}
...
for ($i = 0; $i < pg_num_rows($result); $i++) {
    ...
    if (trim(@$o58_orgao) != "") {
        ...
        if ($codigo != "" && $codigo != $gestao && $nivelb != 8 && $i > 0) {
    ...
    */
    ...
    if (trim(@$gestao) != "") {
        ...
        if ($codigo != $gestao) {
            ...
            $sRotulo = $sEspaco . $descricao . $gestao . '  -  ' . $descricao;
            $codigo = $gestao;
            if ($flag_imp == false) {
                $flag_lib8 = true;
            }
