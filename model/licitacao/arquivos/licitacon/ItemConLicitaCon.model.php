...
class ItemConLicitaCon extends ArquivoLicitaCon
{
    ...
    public function carregarItensOrigemLicitacao()
    {
        ...
        for ($iIndice = 0; $iIndice < $iTotalItens; $iIndice++) {
            ...
            $objetoImpressao = self::criarObjetoImpressao($oItem);

            if ($objetoImpressao !== null) {
                $this->adicionarItem($objetoImpressao);
            }
        }
    ...
    {
        $valorUnitario = number_format($oStdRecordSet->ac20_valorunitario, 4, ",", '');
        ...
        $oStdItem->VL_ITEM = $valorUnitario;
    ...
    {
        // Realizado mapeamento para evitar que os itens repitam
        $sMap = "{$oStdItem->NR_LICITACAO}|{$oStdItem->ANO_LICITACAO}|{$oStdItem->CD_TIPO_MODALIDADE}";
        $sMap .= "|{$oStdItem->NR_CONTRATO}|{$oStdItem->ANO_CONTRATO}|{$oStdItem->TP_INSTRUMENTO}";
        $sMap .= "|{$oStdItem->NR_LOTE}|{$oStdItem->NR_ITEM}";

        $index = $this->mapItens[$sMap];
        if ($index) {
            $quantidadeA = (float) str_replace(',', '.', $this->aItens[$index]->QT_ITENS);
            $quantidadeB = (float) str_replace(',', '.', $oStdItem->QT_ITENS);

            $quantidade = $quantidadeA + $quantidadeB;
            $this->aItens[$index]->QT_ITENS = number_format($quantidade, 3, ",", "");

            $valorTotalA = (float) str_replace(',', '.', $this->aItens[$index]->VL_TOTAL_ITEM);
            $valorTotalB = (float) str_replace(',', '.', $oStdItem->VL_TOTAL_ITEM);

            $valorTotal = $valorTotalA + $valorTotalB;
            
            $this->aItens[$index]->VL_TOTAL_ITEM = number_format($valorTotal, 2, ",", "");
        } else {
            $this->mapItens[$sMap] = count($this->aItens);
