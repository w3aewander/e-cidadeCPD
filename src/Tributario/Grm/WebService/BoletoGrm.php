...
class BoletoGrm
{
   ...
   * @var boolean
   */
    protected $taxa = false;

    /**
     * @var integer
     */
    protected $tipoInscrMunic;
  
    /**
     * @var integer
     */
    protected $codigoInscrMunic;
   ...
   */
     ...
     * @return integer
     */
    public function getTipoInscrMunic()
    {
        return $this->tipoInscrMunic;
    }

    /**
     * @param integer
     */
    public function setTipoInscrMunic($tipoInscrMunic)
    {
        $this->tipoInscrMunic = $tipoInscrMunic;
     ...
     * @return integer
     */
    public function getCodigoInscrMunic()
    {
        return $this->codigoInscrMunic;
    }

    /**
     * @param integer
     */
    public function setCodigoInscrMunic($codigoInscrMunic)
    {
        $this->codigoInscrMunic = $codigoInscrMunic;
   ...
   */
    public function gerar()
    {
        ...
        if ($this->calcularValorTotal() != $this->getValorTotal()) {
            throw new \BusinessException("Valor total  {$this->calcularValorTotal()} 
            está inconsistente do informado {$this->getValorTotal()}. Verifique!");
        }

        if ($this->isTaxa() == "true") {
            db_inicio_transacao();
            $retorno = $this->geraReciboTaxa();
            db_fim_transacao(false);

            return $retorno;
        }

        if (empty($this->getCompetencia())) {
            throw new \BusinessException("Informe a competência.");
        }

        if (empty($this->getUnidadeGestora())) {
            throw new \BusinessException("Informe a unidade gestora.");
        }
        ...
        if (empty($oReceita)) {
            throw new \BusinessException("Receita não encontrada para o 
            recolhimento {$oRecolhimento->getCodigoRecolhimento()}");
        }
        ...
        $valorCodigoBarras = str_replace(
            '.',
            '',
            str_pad(number_format($reciboProtocolo->getTotalRecibo(), 2, "", "."), 11, "0", STR_PAD_LEFT)
        );
        ...
        if ($lConvenioCobrancaValido
                &&
            !CobrancaRegistrada::utilizaIntegracaoWebService($regraEmissao->getConvenio())
        ) {
            CobrancaRegistrada::adicionarRecibo($reciboProtocolo, $regraEmissao->getConvenio());
        }
        ...
        if ($cadastrarCidadao) {
           ...
           *
           * Em caso de não encontrar o cidadão com mesmo nome e documento,
           * ele buscará o cidadão somente pelo documento.
           * Se ele achar o cidadão por documento,
           * cria um novo registro com um sequencial interno diferente do existente
           *
    ...
    public function validarDadosPorTipoDeRecolhimento(TipoRecolhimento $tipoRecolhimento)
    {
        ...
        switch ($tipoRecolhimento->getTipoPessoa()) {
            case TipoRecolhimento::TIPO_EMISSAO_FISICA:
                if (strlen($this->cpfcnpj) != 11) {
                    $aErros['recolhedor'] = "Recolhimento {$tipoRecolhimento->getCodigoRecolhimento()} 
                    - {$tipoRecolhimento->getNome()} é permitido para pessoa física.";
                }
                break;
            ...
            case TipoRecolhimento::TIPO_EMISSAO_JURIDICA:
                if (strlen($this->cpfcnpj) != 14) {
                    $aErros['recolhedor'] =
                    "Recolhimento {$tipoRecolhimento->getCodigoRecolhimento()} 
                    - {$tipoRecolhimento->getNome()} é permitido para pessoas jurídica.";
                }
                break;
        }
   ...
   */
    ...
    private function geraReciboTaxa()
    {
        $ip          = db_getsession("DB_ip");
        $dataEmissao = new \DBDate(date('Y-m-d'));
        ...
        if (!$this->cgm) {
            throw new \BusinessException("Contribuinte não cadastrado, 
            dirija-se a prefeitura para realizar o cadastro.");
        }

        if ($this->getUnidadeGestora() != "null") {
            $sHistorico = "GERAL";

            if ($this->getUnidadeGestora() != 0) {
                $rsUnidadeGestoraTaxa = $taxasLancadasRepository->getUgTaxasGrm($this->getUnidadeGestora());

                if (!$rsUnidadeGestoraTaxa) {
                    throw new \BusinessException("Erro ao buscar a Unidade gestora da taxa.");
                }

                $oDadosUnidade = \db_utils::fieldsMemory($rsUnidadeGestoraTaxa, 0);

                $sHistorico = "{$oDadosUnidade->codigo} - {$oDadosUnidade->ug}";
            }
        }

        if ($this->getUnidadeGestora() != "null") {
            $rsTaxaUg = $taxasLancadasRepository->getTaxasUgGrm(
                $this->getUnidadeGestora(),
                $this->getTipoRecolhimento(),
                $this->isTaxa()
            );

            $oDados = \db_utils::fieldsMemory($rsTaxaUg, 0);
        } else {
            $oDados = $taxasLancadasRepository->getTaxa($this->getTipoRecolhimento());
        }
        ...
        $reciboProtocolo->setDataVencimentoRecibo($dataVencimento->getDate());
        $reciboProtocolo->setVinculoCgm($this->cgm->getCodigo());
        $reciboProtocolo->adicionarReceita($oDados->ar44_receita, $this->getValor());
        $reciboProtocolo->setHistorico($sHistorico);

        if (!empty($this->getTipoInscrMunic())) {
            if ($this->getTipoInscrMunic() == 1) {
                $reciboProtocolo->setMatricula($this->getCodigoInscrMunic());
            } elseif ($this->getTipoInscrMunic() == 2) {
                $reciboProtocolo->setInscricao($this->getCodigoInscrMunic());
            }
        }

        $reciboProtocolo->emiteRecibo($regraEmissao->isCobranca(), true, $regraEmissao->getConvenio());

        $taxasLancadasRecibo->setSequencial(null);
        $taxasLancadasRecibo->setTaxaslancadas($this->getTipoRecolhimento());
        $taxasLancadasRecibo->setNumnov($reciboProtocolo->getNumpreRecibo());
        $taxasLancadasRecibo->setTipoemissao(1);
        ...
        $oEmissaoDocumentoGrm->setAnousu($anoSessao);
        $oEmissaoDocumentoGrm->setHistorico($sHistorico);
        $arquivo = $oEmissaoDocumentoGrm->gerarPdfNoLocal($localArquivo, $this->isTaxa());
