...
class pontoFolha
{
    ...
    public function incluiRubricaPonto($sTipoPonto = '', $aObjDadosPonto = null)
    {
        ...
        foreach ($aObjPonto as $iInd => $oDadosPonto) {
            ...
            $this->verificaHorasExtras($sTipoPonto, $oDadosPonto);
            ...
            try {
                $dados = (object) [
                    'matricula'  => $oDadosPonto->r90_regist,
                    'rubrica'    => $oDadosPonto->r90_rubric,
                    'anousu'     => $oDadosPonto->r90_anousu,
                    'mesusu'     => $oDadosPonto->r90_mesusu,
                    'observacao' => $oDadosPonto->observacao,
                    'instit'     => db_getsession('DB_instit'),
                    'ponto'      => $sTipoPonto
                ];
                $service->saveFromLegacy($dados);
    ...
    public function alteraRubricaPonto($sTipoPonto = '', $aObjDadosPonto = null, $lSomaValores = false)
    {
        ...
        foreach ($aObjPonto as $iInd => $oDadosPonto) {
            ...
            $this->verificaHorasExtras($sTipoPonto, $oDadosPonto, $lSomaValores ? false : true);
    ...
    public function excluiRubricaPonto($sTipoPonto = '', $aObjDadosPonto = null)
    {
        ...
        foreach ($aObjDadosPonto as $iInd => $oDadosPonto) {
            ...
            try {
                ...
                $service = new ObservacaoRubricaService();
                $service->deleteFromLegacy($dados);
            } catch (Exception $erro) {
                throw new Exception($erro->getMessage());
            }
        }
        ...
        if ($rsPonto) {
            ...
            if ($iLinhasPonto > 0) {
                ...
                for ($iInd = 0; $iInd < $iLinhasPonto; $iInd++) {
                    ...
                    $dados = (object) [
                        'matricula'  => $oRubrica->r90_regist,
                        'rubrica'    => $oRubrica->r90_rubric,
                        'anousu'     => $oRubrica->r90_anousu,
                        'mesusu'     => $oRubrica->r90_mesusu,
                        'instit'     => db_getsession('DB_instit'),
                        'ponto'      => $sTipoPonto
                      ];

                    $oRubrica->observacao = '';
                    $dadosObservacao = $service->getObservacaoRubricaServidor($dados);
                    if ($dadosObservacao->observacao) {
                        $oRubrica->observacao = urlencode($dadosObservacao->observacao);
                    }
...
//          $aListaRubricas[] = $oRetornoRubrica;
    ...
    public function repassarValoresFixoSalario($sTipoPonto = '', $aObjDadosPonto = null, $dtDataAdmissao = '')
    {
        ...
        foreach ($aObjPonto as $iInd => $oDadosPonto) {
            ...
            $this->verificaHorasExtras($sTipoPontoDestino, $oDadosPonto);
     ...
     * @throws Exception
     */
    public function verificaHorasExtras($sTipoPonto, $oDadosPonto, $isAlteracao = false)
    {
        if ($sTipoPonto === 'fx') {
            return;
        }
        ...
        $sigla = $this->buscaSiglaPorTipo('fx');
        $campoMatricula = "{$sigla}_regist";
        $campoRubrica = "{$sigla}_rubric";
        $campoQuantidade = "{$sigla}_quant";
        $campoAno = "{$sigla}_anousu";
        $campoMes = "{$sigla}_mesusu";

        $matricula = $oDadosPonto->{$campoMatricula};
        $rubrica = $oDadosPonto->{$campoRubrica};
        $quantidade = $oDadosPonto->{$campoQuantidade};
        $ano = $oDadosPonto->{$campoAno};
        $mes = $oDadosPonto->{$campoMes};

        /**
         * Faz a verificação se permite a inclusão de horas extras
         * para o servidor
         */
        ...
        $calculoParametros = new ControleRubricasCalculoParametros(
            $instituicao,
            new DBCompetencia($ano, $mes),
            ServidorRepository::getInstanciaByCodigo($matricula),
            RubricaRepository::getInstanciaByCodigo($rubrica),
            $quantidade,
            $isAlteracao,
            Ponto::buscaTabelaPorSigla($sTipoPonto)
        );
