...
foreach ($this->aLinhasRelatorio as $iInd => $oLinhaRelatorio) {
    $hash = "{$oLinhaRelatorio->gestao}#{$oLinhaRelatorio->o15_recurso}";
    if ($iRecurso != $hash) {
        if ($iInd != 0) {
            $this->objpdf->ln();
        ...
        $recurso = "{$oLinhaRelatorio->gestao} - {$oLinhaRelatorio->o15_recurso} - {$oLinhaRelatorio->o15_descr}";
        $this->objpdf->cell(50, $iAlt, $recurso, 0, 1, "L");
        $this->objpdf->ln();
    ...
    $sLocalizacao .= "." . str_pad($oLinhaRelatorio->gestao, 4, "0", STR_PAD_LEFT);
    ...
    $this->objpdf->cell(27, $iAlt, $sLocalizacao, 1, 0, "R");
    $this->objpdf->cell(20, $iAlt, $oLinhaRelatorio->o56_elemento, 1, 0, "L");
    $this->objpdf->cell(65, $iAlt, substr($oLinhaRelatorio->o56_descr, 0, 43), 1, 0, "L");
...
function cabecalhoRelatorio(&$oPdf, $sFonte, $iAlt, $lRetencoes)
{
    ...
    $oPdf->cell(27, $iAlt, "Localização", 1, 0, "C", 1);
    $oPdf->cell(20, $iAlt, "Estrutural", 1, 0, "C", 1);
    $oPdf->cell(65, $iAlt, "Descrição", 1, 1, "C", 1);
    if ($lRetencoes == "t") {
