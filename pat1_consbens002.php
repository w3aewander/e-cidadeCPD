...
if (isset($oGet->t52_ident) && trim($oGet->t52_ident != "")) {
  ...
  $sWhereBuscaBemPorPlaca  = "     t52_ident  ilike '{$oGet->t52_ident}' ";
...
if (isset($oGet->t52_bem) && trim($oGet->t52_bem != "")) {    
  $consulta = pg_query("SELECT num_processo_adm, t52_processo from bens where t52_bem={$oGet->t52_bem}");
  $resultado = pg_fetch_array($consulta, 0);    
  $num_processo_adm = $resultado["t52_processo"];
...
<fieldset>
  ...
  <table style="display:inline;">
    ...
    <tr>
      ...
      <td>
        <strong>Valor Histórico Aquisição:</strong>
      </td>
      
      <td class="valores" colspan="2" align="right">
        <?php echo db_formatar($oBem->getValorHistorico(), "f");?>
      </td>
...
$oTabDetalhes->add('dadosAgregacao', 'Dados Agregação', "func_dadosagregacao.php{$sGetUrl}");
