...
class VeiculoDocumentoAutorizacaoCirculacao {
  ...
  private function emitirCorpo($iNumeroVia) {
    ...
    $this->oPdf->setFontSize(13);
    $this->oPdf->setBold(true);
    $this->MultiCell($this->iLargura, $this->iAltura, "AUTORIZAÇÃO DE CIRCULAÇÃO Nº {$this->oAutorizacao->getCodigo()}", 0, "C");
    //$this->MultiCell($this->iLargura, $this->iAltura, "Autorização Nº. {$this->oAutorizacao->getCodigo()}");
    $this->oPdf->setBold(false);
    $this->oPdf->setFontSize(11);

    $this->Ln($this->iAltura + 10);
    $this->MultiCell($this->iLargura, $this->iAltura + 2, $sMensagem);
    if (!empty($sObservacao)) {
      $sObservacao = substr($sObservacao, 0, 200);
      $this->Ln($this->iAltura);
      $this->MultiCell($this->iLargura, $this->iAltura, "Observação: {$sObservacao}");
    }

    if($iNumeroVia == 1){
      $this->Ln($this->iAltura + 5);
      $this->MultiCell($this->iLargura, $this->iAltura + 2, "- Em caso de dano ou avarias no veículo, decorrentes de negligência ou má utilização, bem como o recebimento de multas por infração de trânsito, o motorista autoriza a PMVR proceder o desconto em folha de pagamento do valor da reparação do dano ou avarias e/ou da referida infração de trânsito. Em caso de acidentes, o motorista responderá civil e criminalmente, pelos danos causados a terceiros, excetuando-se os casos de comprovada responsabilidade de terceiros.

- O MOTORISTAS SE COMPROMETE A:
A - Zelar pela conservação do Veículo;
B - Comunicar a Chefia, quanto da necessidade de conserto ou manutenção do Veículo; 
C - Comunicar de imediato a chefia, qualquer ocorrência relacionada ao veículo, tais como: danos, avarias, roubo, furto e recebimento de multas por infração de trânsito; 
D - Ser indicado como real infrator e pagar multas decorrentes de infração de 
trânsito de sua responsabilidade;
E - Cumprir às leis e regulamentos de trânsito."); 
    }

    $this->Ln(2);
    $this->oPdf->setFont("arial", "", 8);      

    $this->oPdf->Cell(50,8,"ITINERÁRIO",'LTBR',0,'C');
    $this->oPdf->Cell(32,4,"SAÍDA",'LTBR',0,'C');
    $this->oPdf->Cell(32,4,"CHEGADA",'LTBR',0,'C');
    $this->oPdf->Cell(20,8,"KM RODADOS",'LTBR',0,'C');
    $this->oPdf->Cell(60,8,"OBSERVAÇÃO",'LTBR',1,'C');
    $this->oPdf->Cell(50,8,"",'',0,'C');
    $this->oPdf->setFont("arial", "", 6);
    $this->oPdf->Cell(16,-4,"HORÁRIO",'LTBR',0,'C');
    $this->oPdf->Cell(16,-4,"KM",'LTBR',0,'C');
    $this->oPdf->Cell(16,-4,"HORÁRIO",'LTBR',0,'C');
    $this->oPdf->Cell(16,-4,"KM",'LTBR',0,'C');
    $this->Ln();
    ...
    //$this->MultiCell($this->iLargura, $this->iAltura + 1, "{$iNumeroVia}ª via", 'B', 'R');
  }

  public function quadro(){
    $this->oPdf->Cell(50,6,"",'LTBR',0,'C');
    //$this->oPdf->Cell(32,6,"",'LTBR',0,'C');
    //$this->oPdf->Cell(32,6,"",'LTBR',0,'C');
    $this->oPdf->Cell(16,6,"",'LTBR',0,'C');
    $this->oPdf->Cell(16,6,"",'LTBR',0,'C');
    $this->oPdf->Cell(16,6,"",'LTBR',0,'C');
    $this->oPdf->Cell(16,6,"",'LTBR',0,'C');
    $this->oPdf->Cell(20,6,"",'LTBR',0,'C');
    $this->oPdf->Cell(60,6,"",'LTBR',0,'C');
    $this->Ln();    
  }
  ...
  private function configurarPdf() {
    ...
    $this->oPdf->addHeaderDescription("Autorização de Circulação");
