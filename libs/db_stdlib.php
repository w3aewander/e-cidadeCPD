 ...
 * @deprecated recomendado a utilização do db_query_params
 */
...
/**
 * Executa pg_query_params com TraceLog.
 * Aceita tanto sequencial quanto associativo.
 * Suporta WHERE IN (:ids) com array ['ids' => [1,2,3]].
 * O $param  deve seguir o formato usado no $sql.
 * @param string $sql   SQL com placeholders posicionais ($1, $2...) ou nomeados (:id, :nome...)
 * @param array  $params Valores sequenciais [1,'João'] ou associativos ['id'=>1,'nome'=>'João']
 * @param resource|null $conn Conexão PG (opcional)
 * @return false|\PgSql\Result
 */
function db_query_params($sql, $params = [], $conn = null)
{
    global $conn;
    ...
    if ($params === array_values($params)) {
        ...
        $finalSql = preg_replace_callback('/:([a-zA-Z_][a-zA-Z0-9_]*)/', function ($m) use (&$params, &$orderedVals, &$offset) {
            $key = $m[1];

            if (!array_key_exists($key, $params)) {
                trigger_error("Parâmetro ':$key' não encontrado no array de parâmetros.", E_USER_ERROR);
                return false;
            }
            ...
            if (is_array($val)) {
                if (empty($val)) {
                    trigger_error("Array vazio para ':$key' em WHERE IN não é permitido.", E_USER_ERROR);
                    return false;
                }
                $placeholders = [];
                foreach ($val as $v) {
        ...
        $finalVals = $orderedVals;
    }

    // Monta TraceLog
    $sqlDebug = $finalSql;
    foreach ($finalVals as $i => $val) {
        if (is_null($val)) {
            $display = 'NULL';
        } elseif (is_numeric($val)) {
            $display = $val;
        } else {
            $display = "'" . addslashes($val) . "'";
        }
        $sqlDebug = preg_replace('/\$' . ($i+1) . '\b/', $display, $sqlDebug, 1);
    }

    if (db_getsession("DB_traceLog", false) != null) {
        $oTraceLog = TraceLog::getInstance();
        $oTraceLog->makeMessage($sqlDebug, false);
    }

    $dbresult = pg_query_params($conn, $finalSql, $finalVals);

    if (!$dbresult && db_getsession("DB_traceLog", false) != null) {
        $oTraceLog = TraceLog::getInstance();
        $oTraceLog->makeMessage($sqlDebug, true);
    }

    if (db_getsession("DB_premenus", false) != null) {
        $oPreMenus = PreMenus::getInstance();
        $oPreMenus->verificaInstrucaoSql($sqlDebug);
    }
...
{
    public function __construct($clboletim)
    {
        global $k11_numbol;
        $data = date("Y-m-d", db_getsession("DB_datausu"));
        $instit = db_getsession("DB_instit");
        $result = $clboletim->sql_record($clboletim->sql_query_file(
            null,
            null,
            "k11_numbol",
            "",
...
function db_verifica_ip_banco($idUsuario)
{
    ...
    global $db48_ip, $db47_id_usuario;
...
class janela
{
    ...
    function mostrar()
    {
        ...
        if ($this->iniciarVisivel) {
            $this->iniciarVisivel = "block";
        } else {
            $this->iniciarVisivel = "none";
        }
        ?>
        <div id="Jan<?php echo $this->nome ?>"
             style=" background-color: #c0c0c0;border: 0px outset #666666;position:absolute; left:<?php echo $this->posX ?>px; top:<?php echo $this->posY ?>px; width:<?php echo $this->largura ?>; height:<?php echo $this->altura ?>; z-index:1; display: <?php echo $this->iniciarVisivel ?>;">
            <table width="100%"
                   height="100%"
                   style="border-color: #f0f0f0 #606060 #404040 #d0d0d0;border-style: solid;  border-width: 2px;"
                   border="0"
                   cellspacing="0"
                   cellpadding="2">
                <tr>
                    <td>
                        <table width="100%"
                               ...
                               cellspacing="0"
                               cellpadding="0">
                            <tr id="CF<?php echo $this->nome ?>"
                                style="white-space: nowrap;background-color:<?php echo $this->corFundoTitulo ?>">
                                <td nowrap
                                    onmousedown="js_engage(document.getElementById('Jan<?php echo $this->nome ?>'),
                                            event)"
                                    onmouseup="js_release(document.getElementById('Jan<?php echo $this->nome ?>'),
                                            event)"
                                    onmousemove="js_dragIt(document.getElementById('Jan<?php echo $this->nome ?>'),
                                            event)"
                                    onmouseout="js_release(document.getElementById('Jan<?php echo $this->nome ?>'),
                                            event)"
                                    width="80%"
                                    style="cursor:hand;font-weight: bold;color: <?php echo $this->corTitulo ?>;font-family: <?php echo $this->fonteTitulo ?>;font-size: <?php echo $this->tamTitulo ?>px">
                                    &nbsp;<?php echo $this->titulo ?></td>
                                <td width="20%"
                                    align="right"
                                    valign="middle"
                                    nowrap><?php $kp = 0x4;
                                    $m = $kp & $this->janBotoes;
                                    $kp >>= 1; ?>
                                    <img <?php echo $m ? 'style="cursor:hand"' : "" ?>
                                            src=<?php echo $m ? "skins/img.php?file=Controles/jan_mini_on.png" :
                                                "skins/img.php?file=Controles/jan_mini_off.png" ?> title="Minimizar"
                                            border="0"
                                            onClick="js_MinimizarJan(this,'<?php echo $this->nome ?>')"><?php $m = $kp & $this->janBotoes;
                                    $kp >>= 1; ?><?php $m = $kp & $this->janBotoes;
                                    $kp >>= 1; ?>
                                    <img <?php echo $m ? 'style="cursor:hand"' : "" ?>
                                            src=<?php echo $m ? "skins/img.php?file=Controles/jan_fechar_on.png" :
                                                "skins/img.php?file=Controles/jan_fechar_off.png" ?> title="Fechar"
                                            border="0"
                                            onClick="js_FecharJan(this,'<?php echo $this->nome ?>')">
                                </td>
                            </tr>
                        </table>
                    </td>
                ...
                <tr>
                    <td width="100%"
                        ...
                        <iframe frameborder="1"
                                style="border-color:#C0C0F0"
                                height="100%"
                                width="100%"
                                id="IF<?php echo $this->nome ?>"
                                name="IF<?php echo $this->nome ?>"
                                scrolling="<?php echo $this->scrollbar ?>"
                                src="<?php echo $this->arquivo ?>"></iframe>
                    </td>
                </tr>
            </table>
        </div>
        <script><?php echo $this->nome ?> = new janela(document.getElementById('Jan<?php echo $this->nome ?>'),
                document.getElementById('CF<?php echo $this->nome ?>'), IF<?php echo $this->nome ?>);</script>
        <?php
    }
//|XX|//
}
...
if (!empty($HTTP_SERVER_VARS)) {
    ...
    $DB_URL_ABS = "http://" . $HTTP_SERVER_VARS['HTTP_HOST'] . substr(
...
function db_verfPostGet($post)
{
    ...
    for ($i = 0; $i < $tam_vetor; $i++) {
        ...
        if (key($post) != 'triggerfuncao' && key($post) != 'corpofuncao' && key($post) != 'eventotrigger' &&
            key($post) != 'codigoclass' && key($post) != 'db33_obs' && key($post) != 'db33_obscpd' &&
            key($post) != 'descricao' && key($post) != 'descr') {
            $dbarraypost = (gettype($post[key($post)]) != "array" ? $post[key($post)] : "");
        } else {
            $dbarraypost = "";
        }
        if (findword(strtoupper($dbarraypost), "INSERT") || findword(
            strtoupper($dbarraypost),
            "UPDATE"
        ) || findword(strtoupper($dbarraypost), "DELETE")  && key($post)!= '_method'|| db_indexOf(
            strtoupper($dbarraypost),
            "EXEC("
        ) > 0 || db_indexOf(
            strtoupper($dbarraypost),
            ...
            strtoupper($dbarraypost),
            "<SCRIPT>"
        ) > 0 || db_indexOf(strtoupper($dbarraypost), "PASSTHRU(") > 0) {
...
function db_formatar($str, $tipo, $caracter = " ", $quantidade = 0, $TipoDePreenchimento = "e", $casasdecimais = 2)
{
    ...
    switch ($tipo) {
        case "sistema":
            return substr($str, 0, 1) . "." . substr($str, 1, 1) . "." . substr($str, 2, 1) . "." . substr(
                $str,
                3,
                1
            ) . "." . substr($str, 4, 1) . "." . substr($str, 5, 2) . "." . substr(
                $str,
                7,
                2
        ...
        case "receita":
            return substr($str, 0, 1) . "." . substr($str, 1, 1) . "." . substr($str, 2, 1) . "." . substr(
                ...
                7,
                2
            ) . "." . substr($str, 9, 2) . "." . substr($str, 11, 2) . "." . substr($str, 13, 2);
        case "receita_int":
            return substr($str, 0, 1) . "." . substr($str, 1, 1) . "." . substr($str, 2, 1) . "." . substr(
                $str,
                3,
        ...
        case "elemento":
            return substr($str, 1, 1) . "." . substr($str, 2, 1) . "." . substr($str, 3, 1) . "." . substr(
                $str,
                4,
                1
            ) . "." . substr($str, 5, 2) . "." . substr($str, 7, 2) . "." . substr(
                $str,
                9,
                2
            ) . "." . substr($str, 11, 2);
        ...
        case "cnpj":
            return substr($str, 0, 2) . "." . substr($str, 2, 3) . "." . substr($str, 5, 3) . "/" . substr(
                $str,
                8,
                4
            ) . "-" . substr($str, 12, 2);
        ...
        case "v":
            ...
            if ($quantidade == 0) {
                if ($str == 0) {
                    return "   " . str_pad(
                        number_format($str, $casasdecimais, ",", "."),
                        15,
                        "$caracter",
                        STR_PAD_LEFT
                    );
...
function db_fieldsmemory($recordset, $indice, $formatar = "", $mostravar = false, $bTrim = true)
{
    ...
    //    echo pg_field_name($recordset,$i)."<br>";
    // }
    // exit;
    // }
    for ($i = 0; $i < $fm_numfields; $i++) {
...
function verifica_data($dia, $mes, $ano)
{
    ...
    while ((checkdate($mes, $dia, $ano) == false) or ((date(
        "w",
 ...
 *                                    </pre>
 * @param bool|array $pdf          Libera o botão e adiciona a estrutura para geração do PDF
 *
...
function db_lovrot(
    ...
    if ($origem === 'func_empnota001.php') {
        while ($row = pg_fetch_assoc($result)) {
            $codNotas[] = $row["e69_codnota"];
        }
    }
    $NumRows = pg_num_rows($result);
    ...
    } elseif (isset($_POST["totalizacao_repas"])) {
    ...
    for ($i = 0; $i < $NumRows; $i++) {
        ...
        if ($arquivo == "()") {
            ...
            if ($quantidadeItemsArrayFuncao > 1) {
                for ($cont = 1; $cont < $quantidadeItemsArrayFuncao; $cont++) {
                    ...
                    $loop .= $caracter . "'";
                    $loop .= addslashes(str_replace('"', '', @pg_fetch_result(
                        $result,
                        $i,
        ...
        for ($j = 0; $j < $NumFields; $j++) {
            ...
            if (strlen(strstr(pg_field_name($result, $j), "db_")) == 0
                ...
                if (pg_field_type($result, $j) == "timestamp") {
                    ...
                    if (pg_field_type($result, $j) == "date") {
                        ...
                        if (pg_field_type($result, $j) == "float8" || pg_field_type(
                            $result,
                            $j
                        ) == "float4" || pg_field_type($result, $j) == "numeric") {
                            $var_data = db_formatar(pg_fetch_result($result, $i, $j), 'f', ' ');
                        } else {
                            if (pg_field_type($result, $j) == "bool") {
                                $var_data = (pg_fetch_result($result, $i, $j) == 'f' || pg_fetch_result(
                                    $result,
                                    $i,
                                    $j
                                ) == '' ? 'Não' : 'Sim');
                                ...
                                if (pg_field_type($result, $j) == "text") {
                                    ...
                                    } else if (pg_field_name($result, $j) == 'q02_obs') {
                                        $var_data = substr(pg_fetch_result($result, $i, $j), 0, 100) . "...";
                                    } else {
                                    ...
                                    if ($lEncontrouResultado) {
                                        $sHtmlCampos = "<td id=\"I" . $i . $j . "\" class='DBLovrotRegistrosRetornados' bgcolor=\"$cor\"><a title='" . $sTitulo . "' onclick=\"js_JanelaAutomatica('" . $sLabel . "','" . (trim(pg_fetch_result(
                                            $result,
                                            $i,
                                            $j
                                        ))) . "');return false;\">&nbsp;Inf->&nbsp;</a>" . ($arquivo != "" ? "<a title=\"$mensagem\" class='DBLovrotRegistrosRetornados'  " . ($arquivo == "()" ? "OnClick=\"" . $resultadoRetorno . ";return false\">" : "onclick=\"JanBrowse = window.open('" . $arquivo . "?" . base64_encode("retorno=" . ($BrowSe == 1 ? $i : trim(pg_fetch_result(
                                            $result,
                                            $i,
                                            0
                                        )))) . "','$aonde','width=800,height=600');return false\">") .trim(pg_fetch_result(
                                            $result,
                                            $i,
                                            $j
                                        )) . "</a>" : (trim(pg_fetch_result(
                                            $result,
                                            $i,
                                            $j
                                        )))) . "&nbsp;</td>\n";
                                    } else {
                                        $sHtmlCampos = "<td id=\"I" . $i . $j . "\" class='DBLovrotRegistrosRetornados' bgcolor=\"$cor\">" . ($arquivo != "" ? "<a title=\"$mensagem\" class='DBLovrotRegistrosRetornados'  " . ($arquivo == "()" ? "OnClick=\"" . $resultadoRetorno . ";return false\">" : "onclick=\"JanBrowse = window.open('" . $arquivo . "?" . base64_encode("retorno=" . ($BrowSe == 1 ? $i : trim(pg_fetch_result(
                                            $result,
                                            $i,
                                            0
                                        )))) . "','$aonde','width=800,height=600');return false\">") .
                                                trim(pg_fetch_result($result, $i, $j))
                                                . "</a>" : (trim(pg_fetch_result(
                                                    $result,
                                                    $i,
                                                    $j
            ...
            if ($arquivo != "") {
                ...
                $sHtml .= "   " . ($arquivo == "()" ? "OnClick=\"" . $resultadoRetorno . ";return false\">" : "onclick=\"JanBrowse = window.open('" . $arquivo . "?" . base64_encode("retorno=" . ($BrowSe == 1 ? $i : trim(pg_fetch_result(
                    $result,
                    $i,
                    0
                )))) . "','$aonde','width=800,height=600');return false\">") .addslashes(trim($var_data));
                $sHtml .= "</a>";
            } else {
                $sHtml .= trim($var_data);
            }
        ...
        if ($origem === 'func_empnota001.php') {
            $sHtml .= "<td style='background-color: white; text-align: center;'>";
            $sHtml .= "<i class='fa fa-eye' title='Visualizar Nota' onclick='setArquivos($codNotas[$i])'></i>";
            $sHtml .= "</td>";
        }
        $sHtml .= "</tr>";
    }
    ...
    if ($NumRows > 0) {
        ...
        if ($pdf !== false) {
            $sHtml .= '<input style="margin-left: 10px;"
                onclick="js_pdf_lovrot(event)"
                class="DBLovrotInputRodape"
                type="button" title="Imprimir em PDF"
                value="PDF">';

            $sHtml .= '<input type="hidden" value="' . ECIDADE_REQUEST_PATH . '" id="ecidade_request_path" >';

            $sHtml .= file_get_contents(__DIR__ . DIRECTORY_SEPARATOR . 'db_pdf_lovrot.php');

            if (is_array($pdf)) {
                $sHtml .= '<input
                    id="DBLovrotInputHeaderPDF"
                    name="header_pdf"
                    type="hidden"
                    value="' . base64_encode(json_encode($pdf)) . '">';
            }
        }
...
function db_lov(
    ...
    if ($NumRows > 0) {
        ...
        for ($i = 0; $i < $NumFields; $i++) {
            if (strlen(strstr(pg_field_name($result, $i), "db")) == 0) {
                echo "<td nowrap bgcolor=\"$db_corcabec\"  style=\"font-size:13px\" align=\"center\"><b><u>" . ucfirst(pg_field_name(
                    $result,
                    $i
                )) . "</u></b></td>\n";
            }
    ...
    $i,
    $j
)) == "" ? "&nbsp;" : trim(pg_fetch_result($result, $i, $j)))) . "</td>\n";
            }
        }
        echo "</tr>\n";
    }
...
function db_menu($usuario = null, $modulo = null, $anousu = null, $instit = null)
{
    ...
    $sHtmlTooltipAviso2 = DBTooltipAvisoQuestionario::getInstanceQuestionario(
        'Questionário',
        'modalPreenchimentoQuestionario()',
        $idItem,
        $modulo
    )->renderQuestionario($idItem, $modulo, $iTop);
    ...
    $DBMenu->setAdministrador(in_array(db_getsession("DB_administrador"), [1,2]));
    ...
    if (!empty($sMenu)) {
        ...
        if (isset($_SESSION["DB_coddepto"])) {
            ...
            if ($result != false && pg_num_rows($result) > 0) {
                $descrdep = "[<strong>" . db_getsession("DB_coddepto") . "-" . substr(pg_fetch_result(
                    $result,
                    0,
                    'descrdepto'
                ), 0, 40) . "</strong>]";
...
function db_permissaomenu($ano, $modulo, $item)
{
    ...
    if (db_getsession("DB_id_usuario") == 1 || in_array(db_getsession("DB_administrador"), [1,2])) {
        return "true";
    } else {
        $sql = "select id_item
                                                                from db_permissao
...
function monta_menu($item_modulo, $id_modulo, $espacos, $lista, $iUsuario = false)
{
    ...
    if ($iUsuario and ($iUsuario != 1 || in_array(db_getsession("DB_administrador"), [1,2]))) {
        $sql = "  select id_item_filho,descricao ,funcao                                                  ";
        $sql .= "    from db_menu m                                                                        ";
...
function db_getnomelogo()
{
    ...
    if (!$rsLogo || pg_num_rows($rsLogo) == 0) {
        return false;
    } else {
...
function db_dataextenso($timestamp = null, $sMunic = null)
{
    ...
    $sData = ($sMunic == "" ? "" : ucfirst(strtolower($sMunic)) . ", ") . date('d', $timestamp) . " de " . $aMeses[date('m', $timestamp)] . " de " . date(
        'Y',
        $timestamp
    ) . ".";
    return $sData;
