...
if ( empty($valida_dados) && isset($y08_inscr) && $y08_inscr != "" && $db_opcao == 1) {
  ...
  if ($debito || $lTipo){
    ...
    if ($lTipo) {
      db_redireciona('fis4_fis_aidof004.php');
    }
  }
}
...
<fieldset>
  ...
  <iframe  name="itens" id="itens" src="forms/db_frm_fis_aidofant.php?inscr=<?=@$y08_inscr?>" width="600" height="130" marginwidth="0" marginheight="0" frameborder="1"></iframe>
</fieldset>
</div>
...
function js_pesquisay08_numcgm(mostra){
  if(mostra==true){
    js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe','func_fis_graficas.php?funcao_js=parent.js_mostragraficas1|0|1&aidof=1','Pesquisa',true);
  }else{
    js_OpenJanelaIframe('','db_iframe','func_fis_graficas.php?pesquisa_chave='+document.form1.y08_numcgm.value+'&funcao_js=parent.js_mostragraficas&aidof=1','Pesquisa',false);
  }
}
...
function js_pesquisa(){
  js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_aidof','func_fis_aidofalt.php?funcao_js=parent.js_preenchepesquisa|0','Pesquisa',true);
}
...
function js_imprimir( iCodigo, iIncricao ) {
  ...
  if (confirm('Deseja imprimir AIDOF ?')) {
    ...
    jan = window.open('fis2_fis_emiteaidof002.php?codaidof='+iCodigo,'','width='+(screen.availWidth-5)+',height='+(screen.availHeight-40)+',scrollbars=1,location=0 ');
    jan.moveTo(0,0);
  }

  if ( iOpcao == 1 ) {
    location.href = "fis4_fis_aidof004.php?y08_inscr="+iIncricao;
  }
}

function js_voltar() {
  window.location.href = "fis4_fis_aidof004.php";
}
</script>
