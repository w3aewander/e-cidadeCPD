...
if (substr($nivel, 1, 1) == 'A') {
    ...
    if ($formato_arq == 'C') {
        ...
        while ($ln = pg_fetch_array($result)) {
            ...
            fputs($fp, $ln["o58_coddot"] . ";" . $ln["gestao"] . " - " . $ln["o15_complemento"] . " - " . $ln["descricao"] . ";" . $ln["liquidado_acumulado"] . "; {$ln["reservado"]} ; "
    ...
    for ($i = 0; $i < pg_num_rows($result); $i++) {
        ...
        if ($gestao > 0) {
            ...
            $pdf->cell(15, $alt, "$gestao - $o15_complemento", 0, 0, "L", 0);
            $descricao = substr($descricao, 0, 35);
            $pdf->cell(45, $alt, "$descricao", 0, 0, "L", 0);
            ...
            if ($lista_subeleme == 'S') {
                $sql = "
                  select *
                    from orcelemento
                   where substr(o56_elemento,1,7) = '" . str_replace('.', '', substr($o58_elemento, 0, 7)) . "'
                     and substr(o56_elemento,8,5) != '00000' and o56_liberado is true";
                $res = db_query($sql);
    ...
    $sql = "
        select {$campos}
         ...
         {$whereFiltroRecurso}
        group by {$group}
order by gestao, o15_complemento
    ";
    $result = db_query($sql);
    ...
    for ($k = 0; $k < pg_num_rows($result); $k++) {
        ...
        if ($pdf->gety() > $pdf->getH() - 15 || $pagina == 1) {
            $pagina = 0;
            $qualou = $o58_orgao . $o58_unidade;
            $pdf->addpage("L");
            $pdf->setfont('arial', 'b', 7);
            ...
            if ($nivela == 1) {
                $pdf->cell(150, $alt, "ÓRGÃO", 0, 0, "L", 0);
            }
            if ($nivela == 2) {
                $pdf->cell(100, $alt, "UNIDADE", 0, 0, "L", 0);
            }
            if ($nivela == 3) {
                $pdf->cell(150, $alt, "FUNÇÃO", 0, 0, "L", 0);
            }
            if ($nivela == 4) {
                $pdf->cell(150, $alt, "SUBFUNÇÃO", 0, 0, "L", 0);
            }
            if ($nivela == 5) {
                $pdf->cell(150, $alt, "PROGRAMA", 0, 0, "L", 0);
            }
            if ($nivela == 6) {
                $pdf->cell(150, $alt, "PROJ/ATIV", 0, 0, "L", 0);
            }
            if ($nivela == 7) {
                $pdf->cell(150, $alt, "ELEMENTO", 0, 0, "L", 0);
            }
            if ($nivela == 8) {
                $pdf->cell(150, $alt, "RECURSO", 0, 0, "L", 0);
            }
            $pdf->cell(25, $alt, "SALDO INICIAL", 0, 0, "R", 0);
            $pdf->cell(25, $alt, "SALDO ANTERIOR", 0, 0, "R", 0);
            $pdf->cell(25, $alt, "RESERVADO", 0, 0, "R", 0);
            $pdf->cell(25, $alt, "SALDO ATUAL", 0, 1, "R", 0);
            $pdf->cell(0, $alt, '', "T", 1, "C", 0);
            $pdf->setfont('arial', '', 7);
        ...
        if ($nivela == 8) {
            $descricao = substr($descricao, 0, 125);
            $pdf->cell(25, $alt, "$gestao - $o15_complemento", 0, 0, "L", 0);
            $pdf->cell(150, $alt, $descricao, '1', 0, "L", 0);
            $pdf->cell(25, $alt, db_formatar($dot_ini, 'f'), 0, 0, "R", 0);
            $pdf->cell(25, $alt, db_formatar($atual, 'f'), 0, 0, "R", 0);
            $pdf->cell(25, $alt, db_formatar($reservado, 'f'), 0, 0, "R", 0);
            $pdf->cell(25, $alt, db_formatar($atual_menos_reservado, 'f'), 0, 1, "R", 0);
