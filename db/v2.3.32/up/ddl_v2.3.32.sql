...
insert into procedimento select 0, 1, 'MIGRAÇÃO', 0, 'M', 0 where exists (select 1 from regencia where ed59_procedimento is null);

update regencia
   set ed59_procedimento = 0
 where ed59_procedimento is null;
...
----------------------------------------------------
-------------------------------------
-- Migração Folha de Adiantamento
-------------------------------------
create table w_migracao_adiantamento as select distinct r21_anousu,
                                                        r21_mesusu,
                                                        r21_instit
                                          from pontofa
                                          inner join gerfadi on r21_anousu = r22_anousu
                                                            and r21_mesusu = r22_mesusu
                                                            and r21_instit = r22_instit;
...
select nextval('rhfolhapagamento_rh141_sequencial_seq'),
       ...
       4,
       false,
       'Folha adiantamento número: 0 da competência: ' || r21_anousu || '/' || r21_mesusu || ' gerada automaticamente.'
  from w_migracao_adiantamento
  order by r21_anousu asc,
           r21_mesusu asc;
...
-------------------------------------
-- Migração Folha de Rescisão
-------------------------------------
create table w_migracao_rescisao as select distinct r19_anousu,
                                                         r19_mesusu,
                                                         r19_instit
                                           from pontofr
                                           inner join gerfres on r19_anousu = r20_anousu
                                                             and r19_mesusu = r20_mesusu
                                                             and r19_instit = r20_instit;
...
select nextval('rhfolhapagamento_rh141_sequencial_seq'),
       ...
       2,
       false,
       'Folha adiantamento número: 0 da rescisão: ' || r19_anousu || '/' || r19_mesusu || ' gerada automaticamente.'
  from w_migracao_rescisao
  order by r19_anousu asc,
           r19_mesusu asc;

-------------------------------------
-- Migração Folha de 13º Salário
-------------------------------------
create table w_migracao_13salario as select distinct r34_anousu,
                                                         r34_mesusu,
                                                         r34_instit
                                           from pontof13
                                           inner join gerfs13 on r34_anousu = r35_anousu
                                                             and r34_mesusu = r35_mesusu
                                                             and r34_instit = r35_instit;
...
select nextval('rhfolhapagamento_rh141_sequencial_seq'),
       ...
       5,
       false,
       'Folha adiantamento número: 0 da rescisão: ' || r34_anousu || '/' || r34_mesusu || ' gerada automaticamente.'
  from w_migracao_13salario
  order by r34_anousu asc,
           r34_mesusu asc;

drop table w_migracao_adiantamento;
drop table w_migracao_rescisao;
drop table w_migracao_13salario;
