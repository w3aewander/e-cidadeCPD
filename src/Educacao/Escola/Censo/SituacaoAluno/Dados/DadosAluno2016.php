...
class DadosAluno2016
{
    ...
    {
        //Regra 1
        $campo = 'O campo "Código de identificação única do aluno - INEP"';
        if (empty($this->iAlunoInep)) {
            $this->addErro(sprintf('%s é uma informação obrigatória.', $campo));
        }

        //Regra 2
        if (strlen($this->iAlunoInep) != 12) {
            $this->addErro(sprintf('%s está com o tamanho diferente do especificado.', $campo));
        }
    ...
    /**
     * Realiza as validações do campo 7 - Matrícula (INEP)
     */
    protected function validarINEPMatricula()
    {
        //Regra 1
        if (empty($this->iMatriculaInep)) {
            $this->addErro('O campo "Código da matrícula (INEP)" é uma informação obrigatória.');
        }

        //Regra 2
        if (strlen($this->iMatriculaInep) > 12) {
            $this->addErro('O campo "Código da matríula (INEP)" está maior do que o especificado.');
        }

        //Regra 3
        if (!\DBString::isSomenteNumero($this->iMatriculaInep)) {
            $msg = 'O campo "Código da matrícula (INEP)" foi preenchido com valor inválido. ->' . $this->iMatriculaInep;
            $this->addErro($msg);
        }
    }
    ...
    protected function validarSituacao()
    {
        ...
        $aEtapas = [27, 28, 29, 32, 33, 34, 37, 38, 39, 40, 41, 67, 68, 70, 71, 73, 74];

        if ($this->iSituacaoAluno == 6 && !in_array($iEtapaCenso, $aEtapas)) {
            $sErro = 'O campo "Situação do aluno" não pode ser preenchido com 6 (Aprovado concluinte) quando a ';
            $sErro .= 'etapa da matrícula for diferente de ';
            $sErro .= '27, 28, 29, 32, 33, 34, 37, 38, 39, 40, 41, 67, 68, 70, 71, 73 e 74';
            $this->addErro($sErro);
        }

        //Regra 6
        $aEtapas = [1, 2, 39, 40, 65, 67, 68, 69, 70, 71, 73, 74];
