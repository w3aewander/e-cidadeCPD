...
class MetasVersusCotasService extends BaseRelatoriosCronograma
{
    /**
     * @var MetasArrecadacaoService
     */
    private $metasArrecadacaoService;
    /**
     * @var integer[]
     */
    private $recursos = [];
    ...
    {
        $relatorio = new MetasVersusCotasPdf();
    ...
    {
        $estimativas = $this->metasArrecadacaoService->getEstimativas();
        $this->metasReceita = $this->metasArrecadacaoService->agruparPorRecurso($estimativas);
    }
     ...
     * @throws \Exception
     */
    private function getCotas()
    {
        $cotas = [];
        $service = new ProgramaEstrategicoService();
        $programas = $service->buscar(['planejamento' => $this->planejamento->pl2_codigo]);
        $programas->each(function (ProgramaEstrategico $programaEstrategico) use (&$cotas) {
            $iniciativas = $programaEstrategico->iniciativas;
            foreach ($iniciativas as $iniciativa) {
                /**
                 * @var Iniciativa $iniciativa
                 */
                foreach ($iniciativa->detalhamentoDespesa as $detalhamentoDespesa) {
                    if (!empty($this->recursos) && !in_array($detalhamentoDespesa->pl20_recurso, $this->recursos)) {
                        continue;
                    }

                    if (!in_array($detalhamentoDespesa->pl20_instituicao, $this->instituicoes)) {
                        continue;
                    }
                    ...
                    $data->recurso = (object)[
                        'recurso' => $recurso->o15_codigo,
                        'fonte_recurso' => $fonte->codigo_siconfi,
                        'descricao' => $fonte->descricao,
                        'complemento' => (object)$recurso->complemento->toArray(),
                    ];

                    $data->cp = (object)$detalhamentoDespesa->caracteristicaPeculiar->toArray();
                    $data->instituicao = (object)[
                        'descricao' => $instituicao->nomeinst, 'codigo' => $instituicao->codigo
                    ];
                    $cotas[$data->id] = $data;
                }
            }
    ...
    private function agruparDespesa(array $cotas)
    {
        ...
        foreach ($cotas as $cota) {
            $recurso = $cota->recurso;
            $agrupar = $recurso->fonte_recurso;
            if ($this->agruparPor === 'recurso') {
                $agrupar = sprintf('%s#%s', $recurso->fonte_recurso, $recurso->complemento->codigo);
            }

            if (!array_key_exists($agrupar, $dadosAgrupados)) {
                $descricao = sprintf(
                    '%s - %s | %s - %s',
                    $recurso->fonte_recurso,
                    $recurso->descricao,
                    str_pad($recurso->complemento->codigo, 4, '0', STR_PAD_LEFT),
                    $recurso->complemento->descricao
                );

                if ($this->agruparPor === 'fonte_recurso') {
                    $descricao = sprintf('%s - %s', $recurso->fonte_recurso, $recurso->descricao);
                }
                $dadosAgrupados[$agrupar] = $this->criaObjeto();
                $dadosAgrupados[$agrupar]->codigo = $agrupar;
                $dadosAgrupados[$agrupar]->descricao = $descricao;
            }

            $dadosAgrupados[$agrupar]->valor += $cota->valor_base;
