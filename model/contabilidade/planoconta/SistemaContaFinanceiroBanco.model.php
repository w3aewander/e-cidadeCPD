...
class SistemaContaFinanceiroBanco implements ISistemaConta{
    ...
    private function persisteSaltes(ContaPlano $oContaPlano) {
        ...
        if (isParaiba()) {
            $outrosdados = new stdClass();
            $outrosdados->conta_ativa = "true";
            $outrosdados->enviada_sagres = "false";
            $oDaoSaltes->k13_outrosdados   = json_encode($outrosdados);
        }else{
            $oDaoSaltes->k13_outrosdados   = 'null';
        }
    ...
    public function excluirDadosIntegrados(ContaPlano $oContaPlano) {
        ...
        if ($this->hasContaTesouraria($oContaPlano)) {
            ...
            if ($oDaoSaltes->numrows > 0) {
                ...
                $daoVinculoContaBancaria = new cl_conplanocontabancaria();
                $daoVinculoContaBancaria->excluir(null, "c56_reduz = {$oContaPlano->getReduzido()}");
                if ($daoVinculoContaBancaria->erro_status === '0') {
                    throw new DBException("Ocorreu um erro ao excluir o vínculo da conta bancária com o reduzido.");
                }
            }
        }
    }
