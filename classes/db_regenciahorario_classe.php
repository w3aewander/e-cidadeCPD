...
class cl_regenciahorario
{
    ...
    public $ed58_i_codigo = 0;
    public $ed58_i_regencia = 0;
    public $ed58_i_diasemana = 0;
    public $ed58_i_periodo = 0;
    public $ed58_i_rechumano = 0;
    public $ed58_ativo = 'f';
    public $ed58_tipovinculo = 0;
    public $ed58_datainicio_dia = null;
    public $ed58_datainicio_mes = null;
    public $ed58_datainicio_ano = null;
    public $ed58_datainicio = null;
    public $ed58_datafim_dia = null;
    public $ed58_datafim_mes = null;
    public $ed58_datafim_ano = null;
    public $ed58_datafim = null;
    ...
    public function incluir($ed58_i_codigo)
    {
        $this->atualizacampos();
    ...
    public function sql_query_grade_horario_professor($campos = "*", $dbwhere = "", $ordem = null)
    {
        $sql = "SELECT {$campos}";
        ...
        $sql .= " INNER JOIN dialetivo ON dialetivo.ed04_i_diasemana = diasemana.ed32_i_codigo AND ed04_i_escola = ed57_i_escola";
    ...
    public function sql_query($ed58_i_codigo = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        ...
        $sql .= "      left join cgmfisico on cgmfisico.z04_numcgm = rhpessoal.rh01_numcgm";
        $sql2 = "";
        if (empty($dbwhere)) {
            if (!empty($ed58_i_codigo)) {
    ...
    public function sql_query_union_discsemreg(
        $ed58_i_codigo = null,
        $campos = "*",
        $camposUnion = "*",
        $ordem = null,
    ...
    )
    {
        $sql = "
            ...
            INNER JOIN relacaotrabalho ON relacaotrabalho.ed23_i_rechumanoescola = rechumanoescola.ed75_i_codigo AND disciplina.ed12_i_codigo = relacaotrabalho.ed23_i_disciplina
        ...
        /* Faz UNION com a tabela de disciplina sem regente */
        if ($lFazUnion) {
            $sql .= "
            UNION
            SELECT
        ...
        ";
            ...
            if (empty($dbwhereUnion)) {
                if (!empty($ed175_codigo)) {
                    $sql2 .= " where regenciahorariodiscsemreg.ed58_i_codigo = $ed175_codigo ";
                }
            } else {
                $sql2 = " where $dbwhereUnion";
            }
    ...
    function sql_query_todosProfessoresDaTurma($iCodigo = null, $sCampos = '*', $sOrdem = null, $sDbWhere = '')
    {
        $sSql = 'select ';
        if ($sCampos != '*') {
            $sCamposSql = explode('#', $sCampos);
        ...
        $sSql .= ' FROM regenciahorario ';
        $sSql .= ' JOIN regencia ON ed58_i_regencia = ed59_i_codigo ';
        $sSql .= ' JOIN disciplina ON ed59_i_disciplina = ed12_i_codigo ';
        $sSql .= ' JOIN caddisciplina ON ed12_i_caddisciplina = ed232_i_codigo ';
        $sSql .= ' JOIN rechumano ON ed58_i_rechumano = ed20_i_codigo ';
        $sSql .= ' JOIN periodoescola ON ed58_i_periodo = ed17_i_codigo ';
        $sSql .= ' JOIN periodoaula ON ed17_i_periodoaula = ed08_i_codigo ';
        $sSql .= ' JOIN diasemana ON diasemana.ed32_i_codigo = ed58_i_diasemana ';
    ...
    function sql_query_regencia_dia_semana_union_semreg($iCodigo = null, $sCampos = '*', $sOrdem = null, $sDbWhere = '', $sOrdemUnion = null, $sDbWhereUnion = '',$disciplina)
    {

        $sSql  = 'select  ';
//		$sSql .= 'distinct on (ed58_i_regencia,ed58_i_diasemana,ed58_datafim) ';
        if ($sCampos != '*') {
            $sCamposSql = explode('#', $sCampos);
            $sVirgula = '';
            for ($iCont = 0; $iCont < sizeof($sCamposSql); $iCont++) {
        ...
        $sSql .= "  UNION ";
        $sSql .= "  SELECT "; 
//		$sSql .= "  distinct on (ed175_regencia,ed175_diasemana,ed175_datafim) ";
		$sSql .= "  regenciahorariodiscsemreg.*";
        $sSql .= "  FROM regenciahorariodiscsemreg";
        $sSql .= "  INNER JOIN periodoescola ON periodoescola.ed17_i_codigo = regenciahorariodiscsemreg.ed175_periodo";
        $sSql .= "  INNER JOIN periodoaula ON periodoaula.ed08_i_codigo = periodoescola.ed17_i_periodoaula";
        $sSql .= "  INNER JOIN regencia ON regencia.ed59_i_codigo = regenciahorariodiscsemreg.ed175_regencia";
        $sSql .= "  INNER JOIN turma ON ed59_i_turma = ed57_i_codigo";
        $sSql .= "  INNER JOIN diasemana ON diasemana.ed32_i_codigo = regenciahorariodiscsemreg.ed175_diasemana";
		
        if ($sDbWhereUnion == '') {
            if ($iCodigo != null) {
                $sSql .= " where regenciahorario.ed175_codigo = $iCodigo ";
            }
        } elseif ($sDbWhereUnion != '') {
            $sSql .= " where $sDbWhereUnion";
        }

        if ($sOrdemUnion != null) {
            $sSql .= ' order by ';
            $sCamposSql = explode('#', $sOrdemUnion);
            $sVirgula = '';
            for ($iCont = 0; $iCont < sizeof($sCamposSql); $iCont++) {
                $sSql .= $sVirgula . $sCamposSql[$iCont];
                $sVirgula = ',';
    ...
    function sql_query_regencia_dia_semana_union_semregEJA($iCodigo = null, $sCampos = '*', $sOrdem = null, $sDbWhere = '', $sOrdemUnion = null, $sDbWhereUnion = '')
    {

        $sSql  = 'select ';
		$sSql .= 'distinct on (ed58_i_regencia,ed58_i_diasemana,ed58_ativo) ';
        if ($sCampos != '*') {
            $sCamposSql = explode('#', $sCampos);
            $sVirgula = '';
            for ($iCont = 0; $iCont < sizeof($sCamposSql); $iCont++) {
        ...
        $sSql .= "  UNION ";
        $sSql .= "  SELECT "; 
		$sSql .= "  distinct on (ed175_regencia,ed175_diasemana) ";
		$sSql .= "  regenciahorariodiscsemreg.*";
        $sSql .= "  FROM regenciahorariodiscsemreg";
        $sSql .= "  INNER JOIN periodoescola ON periodoescola.ed17_i_codigo = regenciahorariodiscsemreg.ed175_periodo";
        $sSql .= "  INNER JOIN periodoaula ON periodoaula.ed08_i_codigo = periodoescola.ed17_i_periodoaula";
    ...
    function sql_query_disciplinas_cgm(
        $sCampos = "*",
        $sOrdem = null,
        $sWhereFuncionario = null,
        $sWhereNaoFuncionario = null,
        $sWhereRegenciaHorario = null,
        $sWhereSubstituto = null,
        $sWhere = null
    )
    {
        ...
        $sql .= " inner join turma         on turma.ed57_i_codigo             = regencia.ed59_i_turma ";
        $sql .= " INNER JOIN calendario ON turma.ed57_i_calendario = calendario.ed52_i_codigo ";
        $sql .= " inner join disciplina    on disciplina.ed12_i_codigo        = regencia.ed59_i_disciplina          ";
        $sql .= " inner join caddisciplina on caddisciplina.ed232_i_codigo    = disciplina.ed12_i_caddisciplina     ";
    ...
    function sql_query_diario_classe_periodo_substituicao(
        $sCampos = "*",
        $sOrdem = null,
        $sWhereRegencia = null,
        $sWhereSubstituicao
    ...
    /**
     * Retorna os dados da recuperação de dias letivos
     * @param string $campos
     * @param string|null $where
     * @return string
     */
    public function sql_query_recuperacao_dias_letivos($campos = "*", $where = null)
    {
        ...
        $sql .= "  INNER JOIN periodoaula ON periodoaula.ed08_i_codigo = periodoescola.ed17_i_periodoaula ";
        $sql .= "  INNER JOIN turno ON turno.ed15_i_codigo = periodoescola.ed17_i_turno ";
        $sql .= "   LEFT JOIN rechumanocgm ON rechumanocgm.ed285_i_rechumano = regenciahorario.ed58_i_rechumano ";
        $sql .= "   LEFT JOIN cgm ON cgm.z01_numcgm = rechumanocgm.ed285_i_cgm ";
        $sql .= "   LEFT JOIN rechumanopessoal ON rechumanopessoal.ed284_i_rechumano = regenciahorario.ed58_i_rechumano ";
        $sql .= "   LEFT JOIN rhpessoal ON rhpessoal.rh01_regist = rechumanopessoal.ed284_i_rhpessoal ";
        $sql .= "   LEFT JOIN cgm cgmpessoal ON cgmpessoal.z01_numcgm = rhpessoal.rh01_numcgm ";
        $sql .= "   LEFT JOIN diarioclasseregenciahorario ON diarioclasseregenciahorario.ed302_regenciahorario = regenciahorario.ed58_i_codigo ";
        $sql .= "   LEFT JOIN diarioclassealunofalta on diarioclassealunofalta.ed301_diarioclasseregenciahorario = diarioclasseregenciahorario.ed302_sequencial";
        $sql .= "  WHERE {$where} ";
        return $sql;
    }

    public function sql_query_turnoreferente($ed58_i_codigo = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
    ...
    public function sql_query_union_discsemreg_turnoreferente(
        $ed58_i_codigo = null,
        $campos = "*",
        $camposUnion = "*",
        $ordem = null,
    ...
    /**
     * Traz os dados que populam a tabela do Quadro Geral de Horários
     * @param null $ed58_i_codigo
     * @param string $campos
     * @param string $camposUnion
     * @param null $ordem
     * @param string $dbwhere
     * @param string $dbwhereUnion
     * @param bool $lFazUnion
     * @return string
     */
    public function sql_query_quadrogeraldehorarios(
        $ed58_i_codigo = null,
        $campos = "*",
        $camposUnion = "*",
        $ordem = null,
    ...
    )
    {
        ...
        $sql .= $sql2;
        $sql3 = "SELECT * FROM ({$sql}) AS K {$aCodigoTurno} ";
        ...
        ";
            ...
            if (empty($dbwhereUnion)) {
                if (!empty($ed175_codigo)) {
                    $sqlUnion2 .= " where regenciahorariodiscsemreg.ed58_i_codigo = $ed175_codigo ";
                }
            } else {
                $sqlUnion2 = " where $dbwhereUnion";
            }

            $sqlUnion .= $sqlUnion2;
            $sqlUnion3 = "UNION SELECT * FROM ({$sqlUnion}) AS K {$aCodigoTurnoUnion} ";
        }

        if (!empty($ordem)) {
            $sqlUnion3 .= " order by {$ordem}";
        }

        return $sql3 .= $sqlUnion3;
    }

    public function sql_query_regenciahorario_periodo_aula($pk = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "
            SELECT
                {$campos}
