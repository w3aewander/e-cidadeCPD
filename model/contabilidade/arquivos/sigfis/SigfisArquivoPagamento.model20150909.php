...
class SigfisArquivoPagamento extends SigfisArquivoBase implements iPadArquivoTXTBase {
  ...
  */
  public function gerarDados() {
    ...
    $sCampos  = "array_to_string(array_accum(distinct c60_estrut),',') as c60_estrut, array_to_string(array_accum(distinct c60_codcon),',') as c60_codcon, empempenho.e60_codemp, empempenho.e60_anousu, conlancam.c70_data, orcdotacao.o58_orgao,          ";
    $sCampos .= "min(pagordem.e50_codord), to_char(max(conlancam.c70_data),'YYYYmm') as competencia, orcdotacao.o58_unidade,   	         ";
    $sCampos .= "sum(case c53_tipo when 30 then conlancam.c70_valor when 31 then (conlancam.c70_valor * -1) end)-coalesce((select sum(k12_valor) from coremp ";
    $sCampos .= "inner join corrente on corrente.k12_id = coremp.k12_id and corrente.k12_data = coremp.k12_data and corrente.k12_autent = coremp.k12_autent ";
    $sCampos .= "left  join corgrupocorrente  on k105_id = corrente.k12_id and k105_data = corrente.k12_data and k105_autent = corrente.k12_autent ";
    $sCampos .= "left  join corgrupotipo on k106_sequencial = k105_corgrupotipo	where k12_empen = empempenho.e60_numemp and k106_sequencial IN (2,5) and k12_codord = min(pagordem.e50_codord)),0) as valor_pago      ";
