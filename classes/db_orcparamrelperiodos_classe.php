...
class cl_orcparamrelperiodos
{
    ...
    public $o113_sequencial = 0;
    public $o113_periodo = 0;
    public $o113_orcparamrel = 0;
    ...
    /**
     * cl_orcparamrelperiodos constructor.
     */
    ...
    public function incluir($o113_sequencial)
    {
        $this->atualizacampos();
        ...
        if (($resaco != false) || ($this->numrows != 0)) {
            ...
            $resac = db_query("insert into db_acount values($acount,2481,14110,'','" . AddSlashes(pg_result(
                    $resaco,
                    0,
                    'o113_sequencial'
                )) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
            $resac = db_query("insert into db_acount values($acount,2481,14111,'','" . AddSlashes(pg_result(
                    $resaco,
                    0,
                    'o113_periodo'
                )) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
            $resac = db_query("insert into db_acount values($acount,2481,14114,'','" . AddSlashes(pg_result(
                    $resaco,
                    0,
                    'o113_orcparamrel'
                )) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
        ...
        if (!empty($o113_sequencial)) {
            db_query("SELECT setval('orcparamrelperiodos_o113_sequencial_seq', (SELECT max(o113_sequencial) FROM orcparamrelperiodos))");
        }
        return true;
    }

    // funcao para alteracao
    public function alterar($o113_sequencial = null)
    {
        $this->atualizacampos();
            ...
            for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
                ...
                if (isset($GLOBALS["HTTP_POST_VARS"]["o113_sequencial"]) || $this->o113_sequencial != "") {
                    $resac = db_query("insert into db_acount values($acount,2481,14110,'" . AddSlashes(pg_result(
                            ...
                            $conresaco,
                            'o113_sequencial'
                        )) . "','$this->o113_sequencial'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                }
                if (isset($GLOBALS["HTTP_POST_VARS"]["o113_periodo"]) || $this->o113_periodo != "") {
                    $resac = db_query("insert into db_acount values($acount,2481,14111,'" . AddSlashes(pg_result(
                            ...
                            $conresaco,
                            'o113_periodo'
                        )) . "','$this->o113_periodo'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                }
                if (isset($GLOBALS["HTTP_POST_VARS"]["o113_orcparamrel"]) || $this->o113_orcparamrel != "") {
                    $resac = db_query("insert into db_acount values($acount,2481,14114,'" . AddSlashes(pg_result(
                            ...
                            $conresaco,
                            'o113_orcparamrel'
                        )) . "','$this->o113_orcparamrel'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                }
            }
        }
        $result = db_query($sql);
    ...
    public function excluir($o113_sequencial = null, $dbwhere = null)
    {
    ...
    // funcao do sql
    public function sql_query($o113_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
    ...
    // funcao do sql
    public function sql_query_file($o113_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
            $campos_sql = split("#", $campos);
