...
class cl_assenta {
   ...
   function incluir ($h16_codigo){
     ...
     if($result==false){
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Código de assentamento ($this->h16_codigo) do funcionario ($this->h16_regist) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "Assentamento do funcionario já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "Código de assentamento ($this->h16_codigo) do funcionario ($this->h16_regist) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
  ...
  function sql_query ( $h16_codigo=null,$campos="*",$ordem=null,$dbwhere=""){
    ...
    $sql .= "       left join portariatipo     on  portariatipo.h30_tipoasse = tipoasse.h12_codigo";
     $sql .= "      left join assentadb_cadattdinamicovalorgrupo ON assentadb_cadattdinamicovalorgrupo.h80_assenta = assenta.h16_codigo ";
    $sql2 = "";
    if($dbwhere==""){
      if($h16_codigo!=null ){
...
function sql_query_perdasMatricula($iMatricula, $dtInicio, $dtFim = null, $sCampos = "*", $sGroupBy) {
  ...
  function sql_query_funcional ( $h16_codigo=null,$campos="*",$ordem=null,$dbwhere="") {
    ...
    $sql .= "      inner join rhfuncao               on rhfuncao.rh37_funcao    = rh02_funcao
                                                    and rh37_instit             = ".db_getsession('DB_instit');
  ...
  function sql_query_join_somente_assentamentofuncional_tipoasse ( $h16_codigo=null,$campos="*",$ordem=null,$dbwhere="") {
  ...
  public function sql_dados_afastamento_temporario($assenta)
  {
      $sql = "
            select
                   -- ideVinculo
                   ...
                   ,h16_dtterm
                   -- infoRetif
                   ,rh220_origemretificacao
                   ,rh220_tipoprocesso
                   ,rh220_numeroprocesso

               from assenta
               join rhpessoal on rh01_regist = h16_regist
               join cgm on z01_numcgm = rh01_numcgm
               left join retificacaoafastamento on rh220_assenta = h16_codigo
              where assenta.h16_codigo = {$assenta}
      ";
  ...
  function sql_dados_afastamento_esocial($h16_regist=null,$campos="*",$dbwhere="",$ordem=null) {

    $sql = "
        with assentamento_esocial as (
          select
            distinct on
            (h12_codigo)
                h12_codigo,
                ...
                db109_valordefault
          from
                esocial.afastamentosesocial
          inner join recursoshumanos.tipoasse on
                eso08_tipoasse = h12_codigo
          inner join esocial.grupomotivoafastamentoesocial on
                eso10_sequencial = eso08_grupomotivoafastamentoesocial
          inner join configuracoes.db_cadattdinamico on
                eso10_db_cadattdinamico = db118_sequencial
          inner join configuracoes.db_cadattdinamicoatributos on
                db109_db_cadattdinamico = db118_sequencial
          inner join configuracoes.db_cadattdinamicoatributosopcoes on
                db18_cadattdinamicoatributos = db109_sequencial
          where
    ...
    $sql .= " from assenta a ";
    $sql .= "      inner join assentamento_esocial b on b.h12_codigo = a.h16_assent";
    $sql .= "      inner join recursoshumanos.assentadb_cadattdinamicovalorgrupo on h80_assenta = h16_codigo";
    $sql .= "      inner join configuracoes.db_cadattdinamicovalorgrupo on db120_sequencial = h80_db_cadattdinamicovalorgrupo";
    $sql .= "      inner join configuracoes.db_cadattdinamicoatributosvalor on db110_cadattdinamicovalorgrupo = db120_sequencial";
    $sql .= "      inner join configuracoes.db_cadattdinamicoatributos on db110_db_cadattdinamicoatributos = db109_sequencial";
    $sql2 = "";

    if($dbwhere==""){
      if($h16_regist!=null ){
        $sql2 .= " where db109_nome = 'motivo_esocial' and a.h16_regist = $h16_regist ";
      }
    }else if($dbwhere != "") {
      if($h16_regist!=null ) {
        $sql2 = " where db109_nome = 'motivo_esocial' and a.h16_regist = $h16_regist ";
