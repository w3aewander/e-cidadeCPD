...
var DBAbas = function ( oContainerDestino ) {
  ...
  this.mostraFilho = function( oItemFilho ) {
    ...
    if (!oContainerDestino.closest('body').classList.contains('no-menu')) {
        if (oContainerDestino.style.marginTop === '') {
            oContainerDestino.style.marginTop = '20px';
        }
    }
  ...
  this.oContainerConteudo .appendChild( oObjetoConteudo );
    ...
    const handler = {
        set: (object, property, value) => {
            if (property === 'lBloqueada') {
                oObjetoSeletor.style.color = value ? 'gray' : 'black';
                oObjetoSeletor.style.backgroundColor = value
                    ? '#b4b4b4'
                    : '#cccccc';
            } else if (property === 'id'){
                oObjetoSeletor.id = value;
            }
