...
final class BalancoOrcamentarioDCASP2017 extends RelatoriosLegaisBase
{
        ...
        switch ($iQuadro) {
            ...
            case self::QUADRO_RESTOS_PROCESSADOS:
                if ($this->iAnoUsu >= 2018) {
                    $this->oPdf->addHeaderDescription("QUADRO DA EXECUÇÃO DE RESTOS A PAGAR PROCESSADOS");
                } else {
                    $smg = "QUADRO DA EXECUÇÃO DE RESTOS A PAGAR PROCESSADOS E NÃO PROCESSADOS LIQUIDADOS";
                    $this->oPdf->addHeaderDescription($smg);
                }
                break;
        }
     ...
     */
    private function escreveAssinatura($iQuadro = null)
    {
    ...
    private function prepararCabecalhos()
    {
        ...
        if (count($aListaInstituicoes) > 1) {
            ...
            $descr = "INSTITUIÇÃO : {$oPrefeitura->getDescricao()} - {$oPrefeitura->getUf()} - CONSOLIDAÇÃO";
            $this->sDescricaoInstituicao = $descr;
        } else {
    ...
    private function processarQuadros()
    {
        ...
        $rpNprocF = self::QUADRO_RESTOS_NAO_PROCESSADOS_FINAL;
    ...
    private function escreverCabecalhoPrincipal()
    {
        ...
        } elseif ($this->valorNoIntervalo(
            $this->oLinha->ordem,
            self::QUADRO_PRINCIPAL_DESPESAS_INICIO,
            self::QUADRO_PRINCIPAL_DESPESAS_FIM
        )) {
...
//                $aLinhasConsistencia[49]->despliq = $nDespLiq;
//                $aLinhasConsistencia[49]->desppag = $nDespPago;
    ...
    public function getDados()
    {
        ...
        $nDespLiq = $aLinhasConsistencia[24]->recrealiza - $aLinhasConsistencia[48]->despliq;
        $nDespPago = $aLinhasConsistencia[24]->recrealiza - $aLinhasConsistencia[48]->desppag;
