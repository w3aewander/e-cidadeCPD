...
$displayBotoes = $db_opcao == 3 || empty($ob09_codhab) ? "display: none;" : '';
$valueCancelarReativar = isset($ob09_ativo) && $ob09_ativo === 't' ? 'Cancelar' : 'Reativar';
...
<form class="container" name="form1" method="post" action="">
  <fieldset>
    ...
    <table class="form-container">
      ...
      <tr>
        <td nowrap title="<?= $Tob09_engprefeitura ?>">
            <?php
            db_ancora($Lob09_engprefeitura, "js_pesquisaob09_engprefeitura(true);", $db_opcao);
            ?>
        </td>
        <td>
            <?php
            db_input('ob09_engprefeitura', 15, $Iob09_engprefeitura, true, 'text', $db_opcao,
              "onChange='js_pesquisaob09_engprefeitura(false)'");
      ...
      <tr>
        <td nowrap title="<?= $Tob09_codconstr ?>">
            <?php
            db_ancora($Lob09_codconstr, "js_pesquisaob09_codconstr(true);", $db_opcao);
            ?>
        </td>
        <td>
            <?php
            db_input('ob09_codconstr', 15, $Iob09_codconstr, true, 'text', $db_opcao,
              " onchange='js_pesquisaob09_codconstr(false);'");
        ...
        <?php } else {
          ...
          <tr>
            ...
            <td>
                <?php
                db_input('ob19_codproc', 15, $Iob19_codproc, true, 'text', $db_opcao,
                  " onchange='js_pesquisaob19_codproc(false);'");
    ...
    <table class="form-container">
            ...
            if (isset($ob22_codhab) && trim($ob22_codhab) != "") {
                ...
                if (!empty($p58_dtproc)) {
                    ...
                    if (count($dataFinal) === 3) {
                        ...
                        $p58_dtproc_mes = $dataFinal[1];
                        $p58_dtproc_ano = $dataFinal[0];
                    }
                }

                db_inputdata('p58_dtproc', $p58_dtproc_dia, $p58_dtproc_mes, $p58_dtproc_ano, true, 'text', 3);
            }
        ...
        <td align="right">
            ...
            if (!empty($ob09_data)) {
                ...
                if (count($data) === 3) {
                    ...
                    $ob09_data_mes = $data[1];
                    $ob09_data_ano = $data[0];
                }
            }
      ...
      <tr>
        <td width="131px;" nowrap title="<?= $Tob09_datafinalobra ?>">
            <?= $Lob09_datafinalobra ?>
        </td>
        <td>
            <?php
            ...
            if (!empty($ob09_datafinalobra)) {
                ...
                if (count($dataFinal) === 3) {
                    ...
                    $ob09_data_mes = $dataFinal[1];
                    $ob09_data_ano = $dataFinal[0];
                }
            }

            db_inputdata('ob09_datafinalobra', $ob09_data_dia, $ob09_data_mes, $ob09_data_ano, true, 'text', $db_opcao);
            ?>
        </td>
      </tr>
  ...
  <input name="db_opcao" type="submit" id="db_opcao"
    ...
    <?= ($db_botao == false ? "disabled" : "") ?>
         onClick='return js_testaarea()'><input name="pesquisar" type="button" id="pesquisar" value="Pesquisar"
                                                onclick="js_pesquisa();">

  <input id="imprimirCarta" name="imprimirCarta" type="button" value="Imprimir Carta de Habite-se"/>
  <input id="cancelarReativar" name="cancelarReativar" type="submit" value="<?= $valueCancelarReativar; ?>"
         style="<?= $displayBotoes; ?>"/>
</form>
<script>
...
//    alert(objJ[5].ob09_codhab+" -- "+objJ[2].z01_nome);
  ...
  $('imprimirCarta').observe('click', function() {

    if($F("ob09_codhab") === '') {

      alert('Selecione um Habite-se.');
      return false;
    }

    let tipoRelatorio = <?=$tipoRelatorio?>;
    let url = tipoRelatorio === 0 ? "pro2_cartahabite002.php" : "pro2_cartahabite003.php";
    let emissaoRelatorio = new EmissaoRelatorio(url, {'codigo': $F("ob09_codhab")});
  ...
  $("ob09_datafinalobra").addClassName("field-size2");
