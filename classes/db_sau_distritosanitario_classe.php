...
class cl_sau_distritosanitario
{
   // cria variaveis de erro 
    ...
    public $s153_i_codigo = 0; 
    public $s153_c_codigo = null; 
    public $s153_c_descr = null; 
    public $s153_ativo = 'f'; 
   // cria propriedade com as variaveis do arquivo 
    ...
    public function atualizacampos($exclusao = false)
    {
     if($exclusao==false){
       ...
       $this->s153_ativo = ($this->s153_ativo == ""?@$GLOBALS["HTTP_POST_VARS"]["s153_ativo"]:$this->s153_ativo);
     }else{
       $this->s153_i_codigo = ($this->s153_i_codigo == ""?@$GLOBALS["HTTP_POST_VARS"]["s153_i_codigo"]:$this->s153_i_codigo);
     }
   }

    public function incluir($s153_i_codigo)
    {
      $this->atualizacampos();
     if($this->s153_c_codigo == null ){ 
       $this->erro_sql = " Campo Código Distrito não informado.";
     ...
     if($this->s153_ativo == null ){ 
       $this->erro_sql = " Campo Ativo não informado.";
       $this->erro_campo = "s153_ativo";
       $this->erro_banco = "";
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
     ...
     if(($this->s153_i_codigo == null) || ($this->s153_i_codigo == "") ){ 
       $this->erro_sql = " Campo s153_i_codigo não declarado.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "sau_distritosanitario ($this->s153_i_codigo) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,3044,1015649,'','".AddSlashes(pg_result($resaco,0,'s153_ativo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 

    public function alterar($s153_i_codigo=null)
    {
      $this->atualizacampos();
     ...
     if(trim($this->s153_ativo)!="" || isset($GLOBALS["HTTP_POST_VARS"]["s153_ativo"])){ 
       $sql  .= $virgula." s153_ativo = '$this->s153_ativo' ";
       $virgula = ",";
       if(trim($this->s153_ativo) == null ){ 
         $this->erro_sql = " Campo Ativo não informado.";
         $this->erro_campo = "s153_ativo";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["s153_ativo"]) || $this->s153_ativo != "")
             $resac = db_query("insert into db_acount values($acount,3044,1015649,'".AddSlashes(pg_result($resaco,$conresaco,'s153_ativo'))."','$this->s153_ativo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     if (!$result) { 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "sau_distritosanitario não Alterado. Alteração Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->s153_i_codigo;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "sau_distritosanitario não foi Alterado. Alteração Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->s153_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
    ...
    public function excluir($s153_i_codigo=null, $dbwhere = null)
    {
     $lSessaoDesativarAccount = db_getsession("DB_desativar_account", false);
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       && ($lSessaoDesativarAccount === false))) {
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,3044,1015649,'','".AddSlashes(pg_result($resaco,$iresaco,'s153_ativo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     ...
     if ($result == false) { 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "sau_distritosanitario não Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "sau_distritosanitario não Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$s153_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
    ...
    public function sql_query($s153_i_codigo = null,$campos = "*", $ordem = null, $dbwhere = "") { 
    ...
    public function sql_query_file($s153_i_codigo = null, $campos = "*", $ordem = null, $dbwhere = "") {
