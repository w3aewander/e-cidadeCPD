 ...
 */
class Cargo extends ProcessamentoAbstract implements ProcessamentoInterface
{
    /**
     * @var
     */
    ...
    public function processar()
    {
        ...
        $oDadosPreenchimento = $oDadosESocial->getPorTipo(Tipo::CARGO);

        $oFormatter = FormatterFactory::get(Tipo::S1030);
        $aDadosPreenchimentoEmpregador = $oFormatter->formatar($oDadosPreenchimento);


        foreach ($aDadosPreenchimentoEmpregador as $oDados) {
            $oEvento = new Evento(TIPO::S1030, $this->cgm, $oDados->referencia, $oDados);
