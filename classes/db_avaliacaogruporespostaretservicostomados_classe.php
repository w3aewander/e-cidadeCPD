...
class cl_avaliacaogruporespostaretservicostomados
{
   ...
   // cria propriedade com as variaveis do arquivo 
    ...
    public function incluir($efd04_sequencial)
    {
     ...
     if($this->efd04_mes == null ){ 
       ...
       $this->efd04_sequencial = $efd04_sequencial; 
     if(($this->efd04_sequencial == null) || ($this->efd04_sequencial == "") ){ 
       $this->erro_sql = " Campo efd04_sequencial não declarado.";
     ...
     $sql = "insert into avaliacaogruporespostaretservicostomados(
                                       efd04_sequencial 
    ...
    public function excluir($efd04_sequencial=null, $dbwhere = null)
    {
     ...
     $sql = " delete from avaliacaogruporespostaretservicostomados
                    where ";
     $sql2 = "";
     if (empty($dbwhere)) {
    ...
    public function sql_record($sql)
    {
     ...
     $this->numrows = pg_num_rows($result);
      if ($this->numrows == 0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:avaliacaogruporespostaretservicostomados";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
    ...
    public function sql_query($efd04_sequencial = null,$campos = "*", $ordem = null, $dbwhere = "") { 
     ...
     $sql  = "select {$campos}";
     $sql .= "  from avaliacaogruporespostaretservicostomados ";
     $sql2 = "";
     if (empty($dbwhere)) {
       if (!empty($efd04_sequencial)) {
         $sql2 .= " where avaliacaogruporespostaretservicostomados.efd04_sequencial = $efd04_sequencial "; 
       } 
    ...
    public function buscarRespostasPreenchimento($campos = array('*'), $where = array(), $outrosComandos = "")
    {
        ...
        $sql .= "  INNER JOIN avaliacaogruporesposta ON db107_sequencial = efd04_avaliacaogruporesposta ";
        ...
        $sql .= "  INNER JOIN cgm cgmcontribuinte ON cgmcontribuinte.z01_numcgm = efd04_cgmcontribuinte";
        $sql .= "  INNER JOIN cgm cgmprestador ON cgmprestador.z01_numcgm = efd04_cgmprestador";
