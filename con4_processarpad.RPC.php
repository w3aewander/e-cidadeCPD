...
use ECidade\RecursosHumanos\Pessoal\Factory\PadEscritorXMLFactory;
...
require_once(modification("model/padArquivoEscritorXML.model.php"));
  ...
  case "processarSigap":
    ...
    if (count($oParam->aArquivos) > 0) {
      ...
      foreach ($oParam->aArquivos as $sArquivo) {
          ...
          $sArquivo = $sArquivo === 'FolhaPessoal' ? 'FolhaPessoalAtivo' : $sArquivo;
          
          if (file_exists("model/PadArquivoSigap{$sArquivo}.model.php")) {

          require_once(modification("model/PadArquivoSigap{$sArquivo}.model.php"));
