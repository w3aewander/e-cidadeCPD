...
for ($i = 0;$i < $iVias;$i++){
    ...
    if ($this->Ratendrequi != null){
      ...
      if (!empty($this->logo) && file_exists("imagens/files/{$this->logo}")) {
	  ...
	  $this->objpdf->Setfont('Arial','b',8);
	  $this->objpdf->text($xcol+132,$xlin+10,'Hora ');
	  $this->objpdf->Setfont('Arial','',8);
	  $this->objpdf->text($xcol+142,$xlin+10,':  '.$this->Rhora);
	  $this->objpdf->Setfont('Arial','b',8);
	  $this->objpdf->text($xcol+132,$xlin+14,'Data ');
	  $this->objpdf->Setfont('Arial','',8);
	  $this->objpdf->text($xcol+142,$xlin+14,':  '.db_formatar($this->Rdata,"d"));
