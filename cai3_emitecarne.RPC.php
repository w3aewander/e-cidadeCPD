...
switch ($oParam->exec) {
     ...
     */
        ...
        if ($lConfReemissaoRecibo) {
           ...
           */
                ...
                if ($lEncontrouIgual) {
                    $oRetorno->iConfirm = 0;
                    $oRetorno->message  = "Encontrou recibo com mesmo NUMPRE, NUMPAR, e DATA VENCIMENTO iguais.";// Sem confirmação
                }
            }
        } else {
            $oRetorno->iConfirm = 0;
    ...
    case "geraRecibo_Carne":
        try {
          ...
          /**
            *
            * Adcionado logica abaixo, pois quando a grid dos debitos abre nas receitas,
            * duplica a geração na recibopaga, quebrando a lógica da emissão
            */

            $aChecks = [];
            foreach ($oDebitosFormulario->aDadosChecks as $chaveNumpreNumpar => $aNumpreNumparReceita) {
                if (count($aChecks) == 0 || substr($chaveNumpreNumpar, 0, 6) == "CHECKU") {
                    if (substr($chaveNumpreNumpar, 0, 6) != "CHECKU") {
                        $parcela = explode("R", $aNumpreNumparReceita["valor"]);
                        array_pop($parcela);
                        $valor = implode("", $parcela)."R0";
                        $aNumpreNumparReceita["valor"]   = $valor;
                    }

                    $aChecks[$chaveNumpreNumpar] = $aNumpreNumparReceita;
                    continue;
                }

                $lAdiciona = true;
                foreach ($aChecks as $aNovoNumpreNumpar) {
                    if ($aNumpreNumparReceita["Numpre"] == $aNovoNumpreNumpar["Numpre"] && $aNumpreNumparReceita["Numpar"] == $aNovoNumpreNumpar["Numpar"]) {
                        $lAdiciona = false;
                        break;
                    }
                }
                ...
                if ($lAdiciona) {
                    ...
                    $valor                             = implode("", $parcela)."R0";
                    $aNumpreNumparReceita["valor"]     = $valor;
                    $aNumpreNumparReceita["Receita"]   = 0;
                    $aChecks[$chaveNumpreNumpar] = $aNumpreNumparReceita;
                }
            }
            ...
            $aDebitosFormularioIniciais = $oDebitosFormulario->aIniciais;

          /**
           * Validacao se vai existir emissao de custas para o recibo
           */
            $reciboCustasService = new ReciboCustasService($sDebitosFormularioTipo);
            $reciboCustasService->setIniciais($aDebitosFormularioIniciais);
            $reciboTemCustas = $reciboCustasService->validaUsoDeCustas();

            $processoForo = ProcessoForo::getInstance();

            if ($processoForo->verificaProcessoMigracaoPago($aIniciais)) {
                $reciboTemCustas = false;
            }


            if ($reciboTemCustas) {
                $iTipoModeloRecibo = TipoModeloCustas::RECIBO;
                $iTipoModeloCarne  = TipoModeloCustas::CARNE;
            }

            if ($oParam->isTef) {
                $iTipoModeloRecibo = 31;
            }
          ...
          /*********************************/
            ...
            } elseif ($lEmissaoRecibo) {
                $iTipoModeloReciboQuery = "{$iTipoModeloRecibo}";
            } elseif ($lEmissaoCarne) {
            ...
            $sSqlRegraEmissao .= "               ar11_cadtipoconvenio, k03_tipo, ar12_cadconveniomodalidade                                                     \n ";
            ...
            $sSqlRegraEmissao .= "               inner join cadtipoconvenio    on cadconvenio.ar11_cadtipoconvenio      = cadtipoconvenio.ar12_sequencial       \n ";
            ...
            $sSqlRegraEmissao .= "         group by k49_tipo, k36_ip, k48_parcini, k48_parcfim, k48_cadconvenio, ar11_cadtipoconvenio, k03_tipo,                \n ";
            $sSqlRegraEmissao .= "                  k48_cadtipomod, ar12_cadconveniomodalidade                                                                  \n ";
           ...
           */
            if ($iRowsRegraEmissao > 0) {
               ...
               */
                ...
                foreach ($aRegrasEmissao as $iIndiceRegra => $oRegraEmissao) {
                   ...
                   * busca do parametro
                   */
                    if ($oRegraEmissao->ar11_cadtipoconvenio == regraEmissao::getConveioCustaBoleto()) {
                   ...
                   */
                    if (($lCobrancaRegistrada && $lEmissaoRecibo) || $sGeraCarne == "" || $lEmissaoCarne) {
                   ...
                   */
                    ...
                    $unica = false;
                    if (!empty($aDadosForm['numpre_unica'])) {
                        $unica = true;
                    }

                    $k03_numnov = 0;
                    $numeroTotalParcelas = count($aChecks);
                    ...
                    foreach ($aChecks as $iIndice => $aValores) {
                       ...
                       */
                        ...
                        if (($aValores["Numpar"] >= $oRegraEmissao->k48_parcini) && ($aValores["Numpar"] <= $oRegraEmissao->k48_parcfim)) {
                            ...
                            * forcaEmissao criado quando modalidade do convenio for arrecadacao, cadtipoconvenio = 3
                            */
                            //$forcaEmissao = (($oRegraEmissao->ar11_cadtipoconvenio == 3) && $custasHonorarios);


                            //if (($lCobrancaRegistrada || empty($sGeraCarne)) &&
                            //    ($oRegraEmissao->k48_cadtipomod == $iTipoModeloRecibo ||
                            //    ($oRegraEmissao->k48_cadtipomod == $iTipoModeloCarne &&
                            //     substr($iIndice, 0, 6) != "CHECKU" && $forcaEmissao))) {

	                          //
                            // Nova Regra cfe orientacao do Dal Pozzo:
                            // - Modalidade de convenio <> 1 (ar12_cadconveniomodalidade) e custasHonorarios
                            // - Se for Recibo e custasHonorarios
                    				//

                            if ( (($oRegraEmissao->ar12_cadconveniomodalidade != 1) && ($custasHonorarios)) ||
                                 ($lEmissaoRecibo) ) {
                               /**
                                ...
                                */
                                if ($oParam->oDadosForm->processarDescontoRecibo == 'true') {
                                    ...
                                    $aNumpres = explode(',', $aValores["Numpre"]);

                                    foreach ($aNumpres as $indexNumpre => $iNumpreUnica) {
                                        $nValorDesconto += recibodesconto1(
                                            $iNumpreUnica,
                                            $aValores["Numpar"],
                                ...
                                */
                                ...
                                if (substr($iIndice, 0, 6) != "CHECKU") {

                                    $dataPagamentoSetada = property_exists($oParam,'oDadosForm')
                                        && property_exists($oParam->oDadosForm, 'k00_dtoper')
                                        && isset($oParam->oDadosForm->k00_dtoper)
                                        && !empty($oParam->oDadosForm->k00_dtoper);

                                    if ($lEmissaoCarne) {
                                        $oRecibo = new recibo(2, null, 1);

                                        $sSqlVenc  = "select case when k00_dtvenc <= '{$sDataHoje}'::date   ";
                                        $sSqlVenc .= "            then '{$sDataHoje}'::date                 ";
                                        $sSqlVenc .= "            else k00_dtvenc                           ";
                                        $sSqlVenc .= "       end as k00_dtvenc                              ";
                                        $sSqlVenc .= "from arrecad                                          ";
                                        $sSqlVenc .= "where k00_numpre = " . $aValores["Numpre"]             ;
                                        $sSqlVenc .= "  and k00_numpar = " . $aValores["Numpar"] . " limit 1";

                                        $rsVencimento = db_query($sSqlVenc);
                                        if (!$dataPagamentoSetada) {
                                            $dVencimento  = db_utils::fieldsMemory($rsVencimento, 0)->k00_dtvenc;
                                        }
                                    }
                                 ...
                                 */
                                if ($lConfReemissaoRecibo && $lEmissaoRecibo) {
                                ...
                                if ($lEmissaoCarne) {
                                   ...
                                   */
                                    if (count((array)$oRecibo->getDebitosRecibo()) > 0) {
                                        try {
                                              db_inicio_transacao();
                                          ...
                                          */
                                            if (!$lForcaVencimento) {
                                               /**
                                                *
                                                * Caso a data do sistema for maior que as datas de vencimento,
                                                * significa que parcelas estão vencidas e a data de vencimento será a
                                                * data do sistema. Caso a data do sistema for menor que a data
                                                * de vencimento a data de vencimento será a menor dentre as das
                                            ...
                                            $oRecibo->emiteRecibo($lConvenioCobrancaValido, true, $oRegraEmissao->k48_cadconvenio, $lEmissaoCarne);

                                            $reciboCustasService->setRecibo($oRecibo);
                                            $reciboCustasService->setRegraEmissao($oRegraEmissao);
                                            $reciboCustasService->setTotalParcelas($numeroTotalParcelas);

                                            if(property_exists($oParam, 'semCalculoTaxas')
                                              && isset($oParam->semCalculoTaxas)
                                              && $oParam->semCalculoTaxas === true
                                              && db_utils::ativaParcelamentoCustas()
                                              ) {
                                            ...
                                            if (in_array($oRegraEmissao->k03_tipo, $aTipoInicial) && $oRegraEmissao->ar11_cadtipoconvenio == regraEmissao::getConveioCustaBoleto()) {
                             ...
                             * Tambem cria um array das parcelas para serem utilizadas valores maximos e minimos
                             * da regra de emissao
                             */
                    ...
                    /**
                     *
                     * Caso a data do sistema foi maior que as datas de vencimento, significa que
                     * parcelas estão vencidas e a data de vencimento será a data do sistema. Caso a
                     * data do sistema for menor que a data de vencimento a data de vencimento será a
                     * menor dentre as das parcelas selecionadas
                     */
                    ...
                    if ((!in_array(0, $iDiferenca) && ($lCobrancaRegistrada || $sGeraCarne == "") &&
                        $oParam->lNovoRecibo && $lEmissaoRecibo) || ($sGeraCarne == "" && !$lConfReemissaoRecibo )) {
                        ...
                        if (count((array)$oRecibo->getDebitosRecibo()) > 0) {
                            try {
                                ...
                                if (!$lForcaVencimento) {
                                    ...
                                    if (( $oParam->oDadosForm->k03_tipo == 5 or
                                          $oParam->oDadosForm->k03_tipo == 18 ) and $oConfig->db21_codcli == 19985) {
                                        $db_datausu = date("Y-m-d", db_getsession("DB_datausu"));
                                ...
                                $sSqlVencBloqueio  = "select k00_dtvenc                                             ";
                                $sSqlVencBloqueio .= "from arrecad                                                  ";
                                $sSqlVencBloqueio .= "where k00_numpre in (".implode(", ", $aNumpresRecibo).")      ";
                                $sSqlVencBloqueio .= "  and k00_numpar in (".implode(", ", $aParcelasRecibo).")     ";

                                $rsVencimentoBloqueio = db_query($sSqlVencBloqueio);
                                $dtVencimentoBloqueio   = db_utils::fieldsMemory($rsVencimentoBloqueio, 0)->k00_dtvenc;

                                if (db_strtotime($dtVencimentoBloqueio) < db_strtotime($sDataHoje) && $oArretipo->k00_liberacarnesis != 't') {
                                    $temVencido = true;
                                }
                 ...
                 */
                    ...
                    } elseif ($lCobrancaRegistrada && $sGeraCarne != "") {
                        $oRetorno->bloqueioDebitoVencido = $temVencido;
            ...
            $oRetorno->dtvencimento = $dVencimento;
...
function retornaDebitosSelecionados($oFormulario, $iTipoAgrupamento = null)
{
   ...
   */
    ...
    if ($iTipoAgrupamento == 2 && empty($oFormulario->oDadosForm->geracarne)) {
        ...
        $aDebitosAgrupados = retornaDebitosAgrupados(
            $aObjDebitos,
...
function reciboDesconto2($numpre, $numpar, $tipo, $tipo_debito, $whereloteador, $totalregistrospassados, $totregistros)
{
  ...
  // desconto
    ...
    if (pg_num_rows($resulttipoparc) == 0 or ($k40_todasmarc == 't'?$totalregistrospassados <> $totregistros:false) or
        $passar == false) {
        $desconto = 0;
    } else {
        $desconto = $k40_codigo;
...
function recibodesconto1(
   ...
   */
    ...
    $sqltipoparc = "select k40_codigo,
                         k40_todasmarc,
                         cadtipoparc
                  from arrecad inner join cadtipoparcdeb on k41_arretipo = k00_tipo
                               inner join cadtipoparc    on k40_codigo   = k41_cadtipoparc
    ...
    if (pg_num_rows($resulttipoparcdeb) == 0) {
        ...
        $sqltipoparcdeb = "select k40_codigo,
                       ...
                       where k00_numpre = $numpre
                         and k00_numpar = $numpar
                         and cadtipoparcdeb.k41_cadtipoparc = $cadtipoparc
                         and cadtipoparcdeb.k41_arretipo = $tipo_debito
                             {$whereloteador}
                         and k00_dtvenc >= k41_vencini
                         and k00_dtvenc <= k41_vencfim
                      ";
