...
try {
  ...
  if ($oGet->lTriado == 'true' && $oGet->unidadeDispensada == 'false') {

    $aFiltroSituacao[] = " la21_c_situacao ='" . RequisicaoExame::EM_PROCESSAMENTO ."'";
    $aSituacoes[]      = "Triado";
  }
  ...
  $sCampos   .= " cast(lab_triagemitem.created_at as date) as data_triagem, ";
  ...
  for ($i = 0; $i < $iLinha; $i++) {
    ...
    if($oGet->unidadeDispensada == 'true'){
      $oCabecalhoMaterial->iLargura = 61;
    } else {
      $oCabecalhoMaterial->iLargura = 51;
    }
     ...
     */
    $oCabecalhoTriagem = new stdClass();
    $oCabecalhoTriagem->iLargura = 14;
    $oCabecalhoTriagem->sTitulo = "Triagem";
    $oCabecalhoTriagem->iSubStr = 10;
    $oCabecalhoTriagem->sBorda = "R";
    ...
    if($oGet->unidadeDispensada == 'true'){
      ...
      $oCabecalhoResultado->iLargura = 14;
    }
    ...
    if($oGet->unidadeDispensada == 'false'){
      $aExamesRequisicao['triagem'] = null;
    }
    ...
    if ( !empty($oExame->data_triagem) && $oGet->unidadeDispensada == 'false' ) {

      $oDataTriagem = new DBDate($oExame->data_triagem);
      $aExamesRequisicao['triagem'] = $oDataTriagem->getDate( DBDate::DATA_PTBR );
    }
    ...
    if($oGet->unidadeDispensada == 'false'){
      $aCabecalho["triagem"]    = $oCabecalhoTriagem;
    }
