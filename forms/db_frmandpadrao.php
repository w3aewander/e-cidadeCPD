...
?>
<script language="JavaScript" type="text/javascript" src="scripts/classes/patrimonio/ouvidoria/processoeletronico/DBViewCamposDinamicosAndamentoPadrao.js"></script>
<script type="text/javascript" src="scripts/classes/http/http.js"></script>
<script type="text/javascript">

var p53_codigo = null, p53_ordem = null;

</script>

<?php
if (!empty($p53_codigo) && !empty($p53_ordem)) {
  echo "
  <script>
    p53_codigo={$p53_codigo}
    p53_ordem={$p53_ordem}
  </script>";
}
?>
      ...
      <table>
        <tr>
          <td align="center" colspan="2">
            ...
            <input type="button" id="btnCamposDinamicosAndamentoPadrao" value="Campos" disabled />
          </td>
        </tr>
        <tr>
...
//  db_iframe.jan.location.href = 'func_db_depart.php?pesquisa_chave='+document.form1.p53_coddepto.value+'&funcao_js=parent.js_mostradb_depart';
        js_OpenJanelaIframe('','db_iframe_db_depart','func_db_depart_transferencias.php?pesquisa_chave='+document.form1.p53_coddepto.value+'&funcao_js=parent.js_mostradb_depart','Pesquisa',false);
      }
    }
...
//  db_iframe.focus();
    ...
    const
        viewCamposAndamentoPadrao = new DBViewCamposDinamicosAndamentoPadrao,
        btnCamposDinamicosAndamentoPadrao = document.querySelector('#btnCamposDinamicosAndamentoPadrao')
        btn_db_opcao = document.querySelector('#db_opcao')
    ;

    btnCamposDinamicosAndamentoPadrao.addEventListener('click', () => viewCamposAndamentoPadrao.show(p53_codigo, p53_ordem));
    btn_db_opcao.addEventListener('click', (ev) => {

        if (ev.target.value == 'Excluir') {

          ev.preventDefault();

          const
            formAndamentoPadrao = ev.target.form,
            parametros = `?ordem=${$F('p53_ordem')}`
          ;

          HttpClient.delete(
              `${apiUrl}${camposDinamicosRoutePrefix}${$F('p53_codigo')}/andamento-padrao/campos-dinamicos${parametros}`,
              {
                reportMessage: 'Excluindo campos dinamicos vinculados...',
                reportProgress: true,
              }
          ).then(response => {
              ...
              formAndamentoPadrao.db_opcao.type = 'text';
              formAndamentoPadrao.submit();
...
if (!empty($p53_codigo) && !empty($p53_ordem)) {
  echo "
    <script>
      btnCamposDinamicosAndamentoPadrao.removeAttribute('disabled');
    </script>
  ";
}
?>
