...
class Repository
{
   ...
   */
    public function add($oRegistro)
    {
        ...
        $this->oDaoPadrao->tr02_emissaogeralparcelaunica = $oRegistro->getParcelaUnica();
   ...
   *
   * O campo ordem esta configurado para trazer os registros na mesma ordem escolhida no momento da geração
   * desta emissão geral
   *
    ...
    public function getCollection(EmissaoGeral $oEmissao)
    {
        $sSqlRegistros = $this->oDaoPadrao->sql_query_registros(
            "emissaogeralregistro.*,
                     emissaogeralmatricula.*,
                     emissaogeralinscricao.*,
                     coalesce((SELECT ocorrenciacobrancaregistrada.k149_movimento
          FROM   retornocobrancaregistrada
                 INNER JOIN ocorrenciaretornocobrancaregistrada
                         ON k170_retornocobrancaregistrada = k168_sequencial
                 INNER JOIN ocorrenciacobrancaregistrada
                       ON k149_sequencial = k170_ocorrenciacobrancaregistrada
                 WHERE k168_numpre = tr02_numpre
          ORDER BY 1 DESC
          LIMIT 1), 0) as ocorrenciaretornobanco",
            "tr03_sequencial, tr04_sequencial, tr02_numcgm, tr02_parcela",
            "tr02_emissaogeral = {$oEmissao->getCodigo()}"
        );

        $sSql2 = "select *
                    from ($sSqlRegistros) as x
                   where x.ocorrenciaretornobanco <> 3
                     and x.ocorrenciaretornobanco <> 2";

        $rsRegistros = \db_query($sSql2);
