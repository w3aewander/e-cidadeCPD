...
foreach($aAlmoxarifados as $iIndice => $iAlmoxarifado){
    ...
    $sSqlAlmox = $oDaoAlmoxarifado->sql_query ( $iAlmoxarifado , $sCampos, "m91_codigo");
    $rsAlmox  = $oDaoAlmoxarifado->sql_record($sSqlAlmox);
    if ($oDaoAlmoxarifado->numrows > 0) {

        $oDadosAlmox = db_utils::fieldsMemory($rsAlmox, 0);
        $aListaAlmoxarifados[] = $oDadosAlmox;
    }
}
...
if (count($aListaAlmoxarifados) == 1) {

    $iCodAlmox = $aListaAlmoxarifados[0]->m91_codigo;
    $sDescrAlmox = $aListaAlmoxarifados[0]->descrdepto;
    $head6 = "DEPÓSITO: {$iCodAlmox} - {$sDescrAlmox}";
}
/**
...
function buscaMateriais($dtInicial, $dtFinal, $iInstituicao, $sOrder, $sAlmoxarifados)
{
    ...
    $sSql .= "                    sum(coalesce( case when m81_tipo = 1                                                ";
    $sSql .= "                                 then m89_valorunitario*m82_quant when m81_tipo = 2                     ";
    $sSql .= "                                 then m89_valorunitario*m82_quant *-1 end, 0) ) as saldo_inicial           ";
    $sSql .= "               from matestoqueini                                                                       ";
    $sSql .= "     inner join matestoquetipo on m80_codtipo = m81_codtipo and m81_tipo <> 4                          ";
    $sSql .= "     inner join matestoqueinimei on m82_matestoqueini = m80_codigo                                     ";
    $sSql .= "     left join matestoqueinimeipm on m82_codigo = m89_matestoqueinimei                                 ";
    ...
    if (!empty($sAlmoxarifados)) {
        ...
        $sSql .= " group by m70_codmatmater, m70_coddepto                                                            ";
    } else {
...
//    db_query($sSqlAcerto);
    ...
    $sSql .= "       ( select sum( coalesce((m89_valorunitario*m82_quant)::numeric, 0) )                                                                                   ";
    ...
    $sSql .= "       ( select sum( coalesce((m89_precomedio*m82_quant)::numeric,0) )                                                                           ";
    $sSql .= "           from matestoqueinimei                                                                                                          ";
    $sSql .= "                inner join matestoqueini      on m80_codigo = m82_matestoqueini                                                           ";
    $sSql .= "                                             and m80_data  >= '{$dtInicial}'                                                              ";
    $sSql .= "                                             and m80_data  <= '{$dtFinal}'                                                                ";
...
function contabilizaMateriais($rsMateriais)
{
    ...
    for ($iMaterial = 0; $iMaterial < $iNumeroMateriais; $iMaterial++) {
        ...
        $oItem->nSaldoAnterior = round((float)$oMaterial->saldo_inicial, 2);
        ...
        $oItem->nSaldoFinal          =  round((float)$oMaterial->saldo_inicial + ($oItem->nTotalEntradas - $oItem->nTotalSaidas), 2);
...
function agruparPorConta(&$aItem, $lPatrimonial = false, $iAno, $sWhere)
{
    ...
    foreach ($aItem as $oItem) {
        ...
        $aContas[$sEstrutural]->nSaldoFinal += $oItem->nSaldoFinal;
        ...
        $oAgrupamento->nSaldoFinal += $oItem->nSaldoFinal;
...
function imprimirRodapeConta(PDF $oPdf, $iAlturaLinha, $oConta, $lAnalitica, $oConfigItem)
{
    ...
    $oPdf->Cell($oConfigItem->iTamanhoSaldoInicial, $iAlturaLinha, 'R$ ' . trim(db_formatar(($oConta->nSaldoAnterior), "f")),
        "LTBR", 0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoQuantidadeInicial, $iAlturaLinha, $oConta->iQuantidadeInicial, "LTBR", 0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoEntradas, $iAlturaLinha, 'R$ ' . trim(db_formatar(($oConta->nTotalEntradas), "f")), "LTBR",
        0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoQuantidadeEntradas, $iAlturaLinha, $oConta->iQuantidadeEntrada, "LTBR", 0, "R",
        1);
    $oPdf->Cell($oConfigItem->iTamanhoSaidas, $iAlturaLinha, 'R$ ' . trim(db_formatar(($oConta->nTotalSaidas), "f")), "LTBR", 0,
        "R", 1);
    ...
    $oPdf->Cell($oConfigItem->iTamanhoSaldoFinal, $iAlturaLinha, 'R$ ' .db_formatar(($oConta->nSaldoFinal), "f"), "LTBR", 0,
        "R", 1);
...
function imprimirItem($oPdf, $iAlturaLinha, $oItem, $oConfigItem)
{
    ...
    $iQuantidadeInicial = (string)Material::arredondarQuantidade($oItem->iQuantidadeInicial);
    $iQuantidadeEntrada = (string)Material::arredondarQuantidade($oItem->iQuantidadeEntrada);
    $iQuantidadeSaida = (string)Material::arredondarQuantidade($oItem->iQuantidadeSaida);
    ...
    $oPdf->Cell($oConfigItem->iTamanhoEntradas, $iAlturaLinha, "R$ " . trim(db_formatar(($oItem->nTotalEntradas), "f")), "TBR", 0,
        "R", 0);
    $oPdf->Cell($oConfigItem->iTamanhoQuantidadeEntradas, $iAlturaLinha, $iQuantidadeEntrada, "TBR", 0, "R", 0);
    $oPdf->Cell($oConfigItem->iTamanhoSaidas, $iAlturaLinha, "R$ " . trim(db_formatar(($oItem->nTotalSaidas), "f")), "TBR", 0, "R",
        0);
    ...
    $oPdf->Cell($oConfigItem->iTamanhoSaldoFinal, $iAlturaLinha, "R$ ".trim(db_formatar(($oItem->nSaldoFinal), "f")), "TBR", 0,
        "R", 0);
...
function imprimirTotal($oPdf, $iAlturaLinha, $oTotal, $oConfigItem)
{
    ...
    $iQuantidadeInicial = (string)Material::arredondarQuantidade($oTotal->iQuantidadeInicial);
    $iQuantidadeEntrada = (string)Material::arredondarQuantidade($oTotal->iQuantidadeEntrada);
    $iQuantidadeSaida = (string)Material::arredondarQuantidade($oTotal->iQuantidadeSaida);
    $iQuantidadeEmEstoque = (string)Material::arredondarQuantidade($oTotal->iQuantidadeEmEstoque);

    $oPdf->SetFont('arial', 'b', 6);
    $oPdf->Ln(2);
    $oPdf->Cell($iTotalAcumulado, $iAlturaLinha, "TOTAL ACUMULADO", "TBR", 0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoSaldoInicial, $iAlturaLinha, 'R$ ' . trim(db_formatar(($oTotal->nSaldoAnterior), "f")),
        "LTBR", 0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoQuantidadeInicial, $iAlturaLinha, $iQuantidadeInicial, "LTBR", 0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoEntradas, $iAlturaLinha, 'R$ ' . trim(db_formatar(($oTotal->nTotalEntradas), "f")), "LTBR",
        0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoQuantidadeEntradas, $iAlturaLinha, $iQuantidadeEntrada, "LTBR", 0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoSaidas, $iAlturaLinha, 'R$ ' . trim(db_formatar(($oTotal->nTotalSaidas), "f")), "LTBR", 0,
        "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoQuantidadeSaidas, $iAlturaLinha, $iQuantidadeSaida, "LTBR", 0, "R", 1);
    $oPdf->Cell($oConfigItem->iTamanhoSaldoFinal, $iAlturaLinha, 'R$ ' . trim(db_formatar(($oTotal->nSaldoFinal), "f")), "LTBR", 0,
        "R", 1);
...
if ( count($aListaAlmoxarifados) > 1 ) {
    ...
    $oPdf->ln();
    $oPdf->Cell(80, $iAlturaLinha, "Depósitos", "TB", 1, "C", 1);
    foreach( $aListaAlmoxarifados as $oDadosAlmoxarifado ){

        $oPdf->Cell(10, $iAlturaLinha, $oDadosAlmoxarifado->m91_codigo, "TB", 0, "R", 0);
        $oPdf->Cell(70, $iAlturaLinha, $oDadosAlmoxarifado->descrdepto, "TB", 1, "L", 0);
    }
}
