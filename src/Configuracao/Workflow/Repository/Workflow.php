...
class Workflow
{
   ...
   */
    public function getPorCodigoTipoProcesso($codigoTipoProcesso)
    {
    
        if (empty(self::$itens[$codigo])) {
            $oDao = new \cl_workflow();
            $sSqlWorkFlow = $oDao->sql_query_tipoproc(null, "*", null, "p51_codigo = {$codigoTipoProcesso}");
   ...
   */
    public function getAtividadesDoWorkflow(WorkflowModel $workflow)
    {
        ...
        $sSqlAtividades   = $oDaoWorkFlowAtiv->sql_query_atributos(
            null,
            "*",
            'db114_ordem',
            'db114_workflow ='.$workflow->getCodigo()
        );
