...
$sql = "select l20_codigo, l31_tipo, case when l31_tipo = 'P' then 'PRESIDENTE' when l31_tipo = 'M' then 'MEMBRO' when l31_tipo = '2' then 'PREGOEIRO' when l31_tipo = '3' then 'SUPLENTE' end as descricao_tipo, l31_liccomissao, l31_numcgm, z01_nome, z01_cgccpf
from liclicita 
inner join liccomissao on liccomissao.l30_codigo = liclicita.l20_liccomissao
inner join liccomissaocgm on l30_codigo = l31_liccomissao
inner join cgm on z01_numcgm = l31_numcgm
where l20_codigo = $l20_codigo";
$result_forne = $clliclicitem->sql_record($sql);
...
for($i=0;$i<pg_num_rows($result_forne);$i++){
  ...
  if ( $i == 0 ){
    fputs($clabre_arquivo->arquivo, lic2_relitenhtml002_formatar("06", $vir, $del));
    for($ii=0;$ii<pg_num_fields($result_forne);$ii++){
  ...
  fputs($clabre_arquivo->arquivo, lic2_relitenhtml002_formatar("07", $vir, $del));
  for($ii=0;$ii<pg_num_fields($result_forne);$ii++){
