...
/*
 *  Este relatorio sera inativado por enquanto conforme solicitacao do analista Sandro
 */
...
function afastamentos_abertos($tipos, $regime, $datai)
{
    ...
    $iUsuario = (int)db_getsession("DB_id_usuario");
    ...
    for ($Itipoasse=0; $Itipoasse< count($tipoasse); $Itipoasse++) {
        ...
        $sWhereVerificaLotacao .= " where rh02_anousu = {$iAnoUsu}";
        $sWhereVerificaLotacao .= "   and rh02_mesusu = {$iMesUsu}";
        $sWhereVerificaLotacao .= "   and rh02_lota in (select rh157_lotacao       ";
        $sWhereVerificaLotacao .= "                       from db_usuariosrhlota   ";
        $sWhereVerificaLotacao .= "                      where rh157_usuario = {$iUsuario})";

        if (db_selectmax("assenta", "select * from assenta inner join assentamentofuncional on rh193_assentamento_funcional = h16_codigo where h16_regist in (".$sWhereVerificaLotacao.") and h16_assent = ".$tipoasse[$Itipoasse]["h12_codigo"]." limit 10")) {
            $mat_campos  = array();
            $mat_valores = array();
            ...
            for ($Iassenta=0; $Iassenta< count($assenta); $Iassenta++) {
                if (db_empty($assenta[$Iassenta]["h16_dtterm"]) ||
                    db_mktime($assenta[$Iassenta]["h16_dtterm"]) >= db_mktime($datai)) {
                    ...
                    db_selectmax(
                        "cgm",
                        "select z01_nome from cgm where z01_numcgm = ".db_sqlformat($pessoal[0]["r01_numcgm"])
                    );
    ...
    if (count($work) > 0) {
        ...
        if ($ordem == "l") {
            $pdf->cell(
                0,
                $alt,
                "Total de Funcionarios Listados : $quantidade  Total Geral : $quantidade",
                0,
                0,
                "C",
                1
