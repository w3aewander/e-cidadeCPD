...
/**
 * Percorremos e filtramos os dados necessarios para o relatorio
 */
foreach ($aTurmas as $oTurma) {
  ...
  foreach ($oTurma->getDisciplinas() as $oRegencia) {
    $oPeriodo = PeriodoAvaliacaoRepository::getPeriodoAvaliacaoByCodigo($oGet->iPeriodo);
    $oDadosTurma->iTotalAulas += $oRegencia->getTotalDeAulasNoPeriodo($oPeriodo);
    $oDadosTurma->iTotalHorasAula += $oRegencia->getTotalHorasAula($oPeriodo);
  }
  ...
  foreach ($oTurma->getAlunosMatriculados(true) as $oMatricula) {
    ...
    foreach ($oDiario->getDisciplinas() as $oDiarioAvaliacaoDisciplina) {
      $oPeriodo = PeriodoAvaliacaoRepository::getPeriodoAvaliacaoByCodigo($oGet->iPeriodo);
      $oAluno->iTotalFaltas   += $oDiarioAvaliacaoDisciplina->getTotalFaltasPorPeriodo($oPeriodo);
      $oAluno->iTotalAbonadas += $oDiarioAvaliacaoDisciplina->getTotalFaltasAbonadasPorPeriodo($oPeriodo);
    }
    ...
    if ($oDadosTurma->iTotalAulas == 0) {
      ...
      $nPercentualFaltasNaoArredondado = ($iTotalFaltas * 100) / $oDadosTurma->iTotalAulas;
      $nPercentualFrequencia           = ArredondamentoFrequencia::formatar(100 - $nPercentualFaltasNaoArredondado, $oDadosTurma->iAno);
      $nPercentualFaltas               = 100 - $nPercentualFrequencia;
    }
    $oAluno->nPercentualFrequencia = "{$nPercentualFrequencia}";
    $oAluno->nPercentualFaltas     = "{$nPercentualFaltas}";

    if ($oAluno->sSituacao == "MATRICULADO") {
        
        $sCalendarioBase = preg_replace('/ \d+$/', '', $oDadosTurma->sCalendario);
        $lAlunoAdicionado = false;

        if ($sCalendarioBase == "ANOS INICIAIS" && $oAluno->iTotalFaltas >= 20) {
            $oDadosTurma->aAlunos[] = $oAluno;
            $lAlunoAdicionado = true;
        } elseif ($sCalendarioBase == "ANOS FINAIS" && $oAluno->iTotalFaltas >= 70) {
            $oDadosTurma->aAlunos[] = $oAluno;
            $lAlunoAdicionado = true;
        } elseif ($sCalendarioBase == "ED. INFANTIL" && $oAluno->iTotalFaltas >= 10) {
            $oDadosTurma->aAlunos[] = $oAluno;
            $lAlunoAdicionado = true;
        } elseif ($sCalendarioBase == "EJA FINAIS" && $oAluno->iTotalFaltas >= 50) {
            $oDadosTurma->aAlunos[] = $oAluno;
            $lAlunoAdicionado = true;
        } elseif ($sCalendarioBase == "EJA INICIAIS" && $oAluno->iTotalFaltas >= 10) {
            $oDadosTurma->aAlunos[] = $oAluno;
            $lAlunoAdicionado = true;
        }

        if ($lAlunoAdicionado) {
            if (!isset($aTotalPorEtapa[$oDadosTurma->sEtapa])) {
                $aTotalPorEtapa[$oDadosTurma->sEtapa] = 0;
            }
            $aTotalPorEtapa[$oDadosTurma->sEtapa]++;
        }
    }
  }
...
//Inicializa o contador do total de alunos do relatório.
$iTotalAlunosRelatorio = 0;
...
foreach ($aTurmaProcessada as $oTurma) {
  ...
  $head6 = "Período: ".($oGet->oPeriodo);
  $head7 = "Aulas Dadas / Dias Letivos: {$oTurma->iTotalAulas}";
  ...
  $iTotalAlunosNestaTurma = count($oTurma->aAlunos);
  if($iTotalAlunosNestaTurma > 0){
    
    //Incrementa o total de alunos do relatório com o total da turma atual.
    $iTotalAlunosRelatorio += $iTotalAlunosNestaTurma;

    $oPdf->ln();
    $oPdf->SetFont("arial", "b", 7);
    $oPdf->Cell(35,  $iHeigth, "Total de Alunos na Turma: ",  0, 0, "L");
    $oPdf->Cell(80,  $iHeigth, $iTotalAlunosNestaTurma,       0, 0, "L");
    $oPdf->Cell(115,  $iHeigth, "Data: _____/_____/__________",                         0, 1, "L");
...
imprimeTotalizadorPorEtapa($oPdf, $aTotalPorEtapa);
imprimeTotalGeralAlunos($oPdf, $iTotalAlunosRelatorio);
...
/**
 * Imprime o totalizador de alunos por etapa no final do relatório.
 */
function imprimeTotalizadorPorEtapa($oPdf, $aTotais) {
    if (empty($aTotais)) {
        return;
    }
    $oPdf->ln(8);
    $oPdf->SetFont("arial", "b", 8);
    $oPdf->Cell(0, 5, "Total de Alunos (com faltas) por Etapa:", 0, 1, 'L');
    $oPdf->SetFont("arial", "", 8);
    ksort($aTotais);
    foreach ($aTotais as $sEtapa => $iTotal) {
        $oPdf->Cell(0, 5, "{$sEtapa}: {$iTotal}", 0, 1, 'L');
    }
}


//Imprime o total geral de alunos que aparecem no relatório.
function imprimeTotalGeralAlunos($oPdf, $iTotal) {
    if ($iTotal > 0) {
        $oPdf->ln(2);
        $oPdf->SetFont("arial", "b", 8);
        $oPdf->Cell(0, 5, "Total Geral de Alunos no Relatório: {$iTotal}", 0, 1, 'L');
    }
}