...
<form name="form1" method="post" action="">
    <div class="container" style="width: 65%;">
        <fieldset>
            ...
            <table>
                <tr>
                    <td nowrap title="<?=@$Ttf11_i_utilizagradehorario?>" style="width: 100px;">
                ...
                <tr>
                    <td nowrap title="<?=@$Ttf11_obriga_hora_saida?>" style="width: 100px;">
                        <?=@$Ltf11_obriga_hora_saida?>
                    </td>
                    <td>
                        <?php
                        db_select('tf11_obriga_hora_saida', ['f' => 'NÃO', 't' => 'SIM'], true, $db_opcao);
                        ?>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td nowrap title="<?=@$Ttf11_alertamunicipio?>"><b>Exibir alerta de paciente não morador do município:</b></td>
                    <td>
                        <?php
                        $alertaMunicipio = array('f' => 'NÃO','t' => 'SIM');
                        db_select('tf11_alertamunicipio', $alertaMunicipio, true, $db_opcao);
                        ?>
                    </td>
                </tr>               
                <tr>
...
<script>
  ...
  var input = {
    'tf11_especmedico' : $('$tf11_especmedico'),
    'z01_nome'         : $('z01_nome'),
    'rh70_estrutural'  : $('rh70_estrutural'),
    'rh70_descr'       : $('rh70_descr'),
    'sd02_i_codigo'    : $('sd02_i_codigo'),
    'descrdepto'       : $('descrdepto')
  };

  $('tf11_i_utilizagradehorario').addEventListener('change', () => {
      $('tf11_obriga_hora_saida').title = '';
      $('tf11_obriga_hora_saida').disabled = false;
      $('tf11_obriga_hora_saida').style.backgroundColor = '';
      if ($('tf11_i_utilizagradehorario').value === '1') {
          $('tf11_obriga_hora_saida').title = 'Só é habilitado quando o parâmetro "Utiliza grade de horário" estiver marcado como NÃO.';
          $('tf11_obriga_hora_saida').value = 't';
          $('tf11_obriga_hora_saida').disabled = true;
          $('tf11_obriga_hora_saida').style.backgroundColor = '#DEB887';
      }
  });
  $('tf11_i_utilizagradehorario').dispatchEvent(new Event('change'));
  
  function js_pesquisa_medico(lMostra) {

    input.z01_nome.value = '';
    input.rh70_estrutural.value = '';
    input.rh70_descr.value = '';
    input.sd02_i_codigo.value = '';
    input.descrdepto.value = '';
