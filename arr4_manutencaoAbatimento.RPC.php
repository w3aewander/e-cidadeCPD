...
switch ($oParam->exec) {
  ...
  case "exluirCredito":
    ...
    try {
      ...
      $daoAtimentoProcessoExterno = new cl_abatimentoprocessoexterno();
      $daoAtimentoProcessoExterno->excluir(null, "k160_abatimento = {$oParam->iAbatimento}");
      if ($daoAtimentoProcessoExterno->erro_status == "0") {
        $sMsg  = "Erro ao excluir registros da abatimento processo externo\n";
        $sMsg .= "Erro da classe : {$daoAtimentoProcessoExterno->erro_msg}\n";
        $sMsg .= "Erro do banco  : ".pg_last_error();
        throw new Exception($sMsg);
      }
  ...
  case "compensacaoCredito":
    ...
    case "getValorNumpreCalculado":

        $result_valinicial = debitos_numpre($oParam->iNumpre, 0,$oParam->iTipoDebito, db_getsession("DB_datausu"), db_getsession("DB_anousu"), $oParam->iNumpar, true, "", " and y.k00_hist <> 918");

        echo $oJson->encode(pg_fetch_object($result_valinicial));
  ...
  case "exluirPagamentoParcial":
    ...
    try {
       ...
       */
      if (!empty($sequencialArreCadCompos)) {
          $oDaoAbatimentoArreckeyArrecadCompos->excluir(null, 'k129_abatimentoarreckey IN(' . implode(',', $sequencialAbatimentoArreckey) . ')');
          if ($oDaoAbatimentoArreckeyArrecadCompos->erro_status == "0") {
              $sMsg  = "Erro ao excluir registros da abatimentoarreckeyarrecadcompos\n";
              $sMsg .= "Erro da classe : {$oDaoAbatimentoArreckeyArrecadCompos->erro_msg}\n";
              $sMsg .= "Erro do banco  : ".pg_last_error();
              throw new Exception($sMsg);
          }

          $oDaoArreCadCompos->excluir($sequencialArreCadCompos);
          if ($oDaoArreCadCompos->erro_status == "0") {
              $sMsg  = "Erro ao excluir registros da arrecadcompos\n";
              $sMsg .= "Erro da classe : {$oDaoArreCadCompos->erro_msg}\n";
              $sMsg .= "Erro do banco  : ".pg_last_error();
              throw new Exception($sMsg);
          }
      }
