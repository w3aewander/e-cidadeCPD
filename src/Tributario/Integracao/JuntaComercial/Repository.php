...
class Repository
{
    ...
    {
        $sLogradouroCgm = $cgm->getLogradouro();
        if (empty($sLogradouroCgm)) {
            $cgm->setLogradouro("LOGRADOURO NAO CADASTRADO");
        }
    ...
    public function persistEmpresa(\Empresa $oEmpresa, Protocolo $oProtocolo)
    {
        EmpresaRepository::persist($oEmpresa, $oProtocolo);
    }
    ...
    public function persistAlvara(\Alvara $oAlvara)
    {
        ...
        $sqlVerificaSanitario = $oDAoSanitario->sql_query_file(
            null,
            '*',
            null,
            "y80_codsani = {$oAlvara->getEmpresa()->getInscricao()}"
        );
        ...
        $sqlVerificaSanitarioInscricao = $oDaoSanitarioinscr->sql_query_file(
            $oDAoSanitario->y80_codsani,
            $oAlvara->getEmpresa()->getInscricao()
        );
        ...
        foreach ($oAlvara->getEmpresa()->getAtividades() as $indice => $oAtividade) {
            ...
            $oDaoSaniatividade->y83_databx    = null;
            ...
            $sqlVerificaSanitarioAtividade = $oDaoSaniatividade->sql_query_file(
                $oDAoSanitario->y80_codsani,
                $oDaoSaniatividade->y83_seq
            );
            $oDaoSaniatividade->sql_record($sqlVerificaSanitarioAtividade);

            if ($oDaoSaniatividade->numrows > 0) {
                $oDaoSaniatividade->alterar(
                    $oDAoSanitario->y80_codsani,
                    $oDaoSaniatividade->y83_seq
                );
            }
            ...
            if ($oDaoSaniatividade->numrows == 0) {
                $oDaoSaniatividade->incluir(
                    $oDaoSaniatividade->y83_codsani,
                    $oDaoSaniatividade->y83_seq
                );
            }

            if ($oDaoSaniatividade->erro_status==0) {
                $sMsg = "Não foi possível vincular o alvará sanitário com a atividade {$oAtividade->getCodigo()}";
                throw new \DBException($sMsg);
            }
        }

        $aLicencas[] = new Licenca(
            0,
            Licenca::LICENCA_ALVARA,
            Licenca::TIPO_LICENCA_DEFINITIVA,
            $oAlvara->getEmpresa()->getInscricao()
        );
        $aLicencas[] = new Licenca(
            0,
            Licenca::LICENCA_MOBILIARIO,
            Licenca::TIPO_LICENCA_DEFINITIVA,
            $oAlvara->getEmpresa()->getInscricao()
        );
    ...
    public function persistRetorno(Protocolo $oProtocolo, Retorno $oRetorno)
    {
        $oDaoRetorno = new \cl_juntacomercialprotocoloretorno();
        $oDaoRetorno->q149_xml = $oRetorno->getXmlRetorno()->asXML();
        $oDaoRetorno->q149_juntacomercialprotocolo = $oProtocolo->getCodigo();

        $oDaoRetorno->incluir(null);

        if ($oDaoRetorno->erro_status==0) {
            throw new \DBException("Não foi possível salvar o XML de retorno da requisição.");
        }
    }
}
