...
class CalculoPensao
{
    ...
    public static function calcular($icalc, $opcao_geral, $opcao_tipo, $chamada_geral_arquivo = null)
    {
        ...
        if ($opcao_tipo == 2) {
            ...
            if ($opcao_geral == 1) {
                ...
                $condicaoaux .= " and ( r52_limite is null or ";
                $condicaoaux .= "     (extract(year from r52_limite)=".db_sqlformat(substr("#".$subpes, 1, 4));
                $condicaoaux .= "      and extract(month from r52_limite)>=".db_sqlformat(substr("#".$subpes, 6, 2));
                $condicaoaux .= " ) or extract(year from r52_limite)>".db_sqlformat(substr("#".$subpes, 1, 4)).")";
                ...
                $sTrataData = substr("#".$subpes, 6, 2)."/".substr("#".$subpes, 1, 4);
                $condicaoaux  = " and ( rh05_recis is null or ";
                $condicaoaux .= "       rh05_recis >= ".db_sqlformat(db_ctod("01/".$sTrataData)).")";
                $condicaoaux .= " and ( r52_limite is null or ";
                $condicaoaux .= "     (extract(year from r52_limite)=".db_sqlformat(substr("#".$subpes, 1, 4));
                $condicaoaux .= "      and extract(month from r52_limite)>=".db_sqlformat(substr("#".$subpes, 6, 2));
                $condicaoaux .= " ) or extract(year from r52_limite)>".db_sqlformat(substr("#".$subpes, 1, 4)).")";
        ...
        for ($Ipensao=0; $Ipensao<count($pensao); $Ipensao++) {
            ...
            LogCalculoFolha::write("Valor Líquido.....................:" . $pvalor_liquido);
            ...
            if ($chamada_geral_arquivo == "gerfs13") {
                 ...
                 */
            } elseif ($chamada_geral_arquivo == "gerfcom") {
                 ...
                 */
            } elseif ($chamada_geral_arquivo == "gerffer") {
                 ...
                 */
            } elseif ($chamada_geral_arquivo == "gerfres") {
            ...
            $cgmalimentadop      = $pensao[$Ipensao]["r52_cgmalimentado"];
            $condicaoaux  = " and r52_regist = ".db_sqlformat($registrop);
            $condicaoaux .= " and r52_numcgm = ".db_sqlformat($numcgmp);
            $condicaoaux .= " and r52_cgmalimentado = ".db_sqlformat($cgmalimentadop);
                ...
                for ($Igeral=0; $Igeral<count($chamada_geral_); $Igeral++) {
                    ...
                    if (substr("#".$chamada_geral_[$Igeral][$qual_rub], 1, 1) != "R"
                        ...
                        if ($chamada_geral_arquivo == "gerfres") {
                            ...
                            } elseif (substr("#".$chamada_geral_[$Igeral][$qual_rub], 1, 1) == "R"
            ...
            if (!db_empty($formula_pensao)) {
                if ($tem_calculo) {
                    ...
                    $sSqlRubricas .= "from rhrubricas ";
                    $sSqlRubricas .= "where rh27_instit = {$DB_instit} ";
                    $sSqlRubricas .= "  and position(rh27_rubric in '{$formula_pensao}') > 0 ";
                    $sSqlRubricas .= "order by rh27_rubric";

                    $rsRubricas = db_query($sSqlRubricas);

                    while ($oRubricaPensao = pg_fetch_object($rsRubricas)) {
                        $condicaoaux  = " and ".$siglag."regist = ".db_sqlformat($pensao[$Ipensao]["r52_regist"]);
                        $condicaoaux .= " and ".$siglag."rubric = ".db_sqlformat($oRubricaPensao->rh27_rubric);

                        $sSql = "select * from ".$chamada_geral_arquivo." ".bb_condicaosubpes($siglag).$condicaoaux;
                        if (db_selectmax($chamada_geral_arquivo, $sSql)) {
                            $arq_ = $$chamada_geral_arquivo;
                            $vararq = $arq_[0][$sigla1."valor"] ;
                            $formpensao = db_strtran(
                                $formpensao,
                                $oRubricaPensao->rh27_rubric,
                                db_strtran(db_str($vararq, 15, 2), ",", ".")
                            );
                            //echo "<BR> formula da pensao 3 ->  $formpensao";
                        } else {
                            $formpensao = db_strtran($formpensao, $oRubricaPensao->rh27_rubric, "0");
                    ...
                    if (db_at("7777", $formpensao) > 0) {
                        $formpensao = db_strtran(
                            $formpensao,
                            "7777",
                            db_strtran(db_str($pvalor_liquido, 15, 2), ",", ".")
                        );
                    ...
                    if (db_at("8888", $formpensao) > 0) {
                        $formpensao = db_strtran(
                            $formpensao,
                            "8888",
                            db_strtran(db_str($pvalor_obriga, 15, 2), ",", ".")
                        );
                    ...
                    if (db_at("9999", $formpensao) > 0) {
                        $formpensao = db_strtran(
                            $formpensao,
                            "9999",
                            db_strtran(db_str($pvalor_bruto, 15, 2), ",", ".")
                        );
                    ...
                    $sMsgErroCalcPensao .= " Ano/Mes: ".$pensao[$Ipensao]["r52_anousu"]."/";
                    $sMsgErroCalcPensao .= $pensao[$Ipensao]["r52_mesusu"];
            ...
            if ($valor_pensao >= 0) {
                ...
                if ($valor_pensao > 0) {
                    ...
                    if ($opcao_geral == PONTO_SALARIO) {
                    ...
                    } elseif ($opcao_geral == PONTO_COMPLEMENTAR) {
                        $matriz1 = array();
                        $matriz2 = array();
                        ...
                        $matriz2[7] = $mesusu;
                        $matriz2[8] = $DB_instit;
                    } elseif ($opcao_geral == PONTO_RESCISAO) {
                        $matriz1 = array();
                        $matriz2 = array();
                        ...
                        $matriz2[8] = $mesusu;
                        $matriz2[9] = $DB_instit;
                    } elseif ($opcao_geral == PONTO_13_SALARIO) {
                        $matriz1 = array();
                        $matriz2 = array();
                    ...
                    if (DBPessoal::verificarUtilizacaoEstruturaSuplementar() && ($chamada_geral_arquivo == "gerfsal" || $chamada_geral_arquivo == "gerfcom")) {
                        ...
                        $sSqlValorPensao .= "     join rhfolhapagamento on rh145_rhfolhapagamento = rh141_sequencial ";
    ...
    public static function removerHistorico($iMatricula, $iCGMBeneficiario)
    {
        ...
        $oDaoRHHistoricoPensao->excluir(
            null,
            "rh145_pensao = {$iCodigoPensao} and rh145_rhfolhapagamento = {$iCodigoFolha}"
        );
