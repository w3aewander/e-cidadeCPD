...
use ECidade\Tributario\Integracao\JuntaComercial\Envio\Inscricao;
...
class InstituicaoService
{
    ...
    public function recebeDados(
      ...
      $xml,
      $data,
      $cnpjInstituicaoEnvia,
      $cnpjInstituicaoRecebe,
      $cpfCnpjProcesso
    ) {
        try {
            ...
            db_inicio_transacao();
            $this->oProtocolo = $this->salvaProtocolo($servico, $funcao, $protocolo, $xml, $data, $cnpjInstituicaoEnvia,$cnpjInstituicaoRecebe, $cpfCnpjProcesso);
            $validador = new Validador($this->oRegin);
            $validador->validar();
            ...
            $this->oRepository->persistRetorno($this->oProtocolo, $this->oRetorno);
        ...
        $caminhoArquivo = "tmp/juntaComercial_" . $protocolo . "_" . time() . ".xml";
        file_put_contents($caminhoArquivo, $this->oRetorno->getXmlRetorno()->asXML());

        Inscricao::criarTarefa($protocolo, $servico, $caminhoArquivo, $cnpjInstituicaoRecebe);
    ...
    private function salvaCgm()
    {
        ...
        $processaCgm->setGrupoPrincipal(Regin::SOCIO);
        $processaCgm->addGrupo(Regin::SOCIO);
        $processaCgm->addGrupo(Regin::ENDERECO_SOCIO);
        $processaCgm->addGrupo(Regin::ENDERECO_INSCRICAO);
        $processaCgm->addGrupo(Regin::INFO_EXTRA);
     ...
     */
    private function salvaProtocolo(
      $servico,
      $funcao,
      $protocolo,
      $xml,
