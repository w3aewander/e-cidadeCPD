...
if (!isset($arqinclude)){
  ...
  $lc = "RREO - Anexo III (LRF, Art. 53, inciso I)";
  $lc1 = "R$ 1,00";
  ...
  array_push($guardadados, ".");
  //array_push($guardalinhacabeca, ".");
  ...
  if ($iMesFinalAnterior !=13) {
    ...
    for ($x= $iMesInicialAnterior; $x <= 12; $x++) {
      ...
      $linha0 = $mes_dresc[$x]."/".$ano;
      array_push($guardadados, $linha0);
      //array_push($guardalinhacabeca, $linha0);
 	    if ($mes_dresc[$x] == "Dez" ) {
	  	  $ano++;
	    }
    }
  ...
  $dlinha01 = "ULT 12 MESES";
  $dlinha02 = "ATUAL EXERC";

  array_push($guardadados, $dlinha01);
  array_push($guardadados, $dlinha02);
  //array_push($guardalinhacabeca, $dlinha01);
  //array_push($guardalinhacabeca, $dlinha02);
  $pdf->ln();
  ...
  for ($x = 1; $x <= 22; $x++) { // 18 é a qtd de parametros existentes
    ...
    if ($x == 1) {
      ...
      $linha1 = "RECEITA CORRENTES(I)";
      array_push($guardadados, $linha1);
      ...
      for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
        ...
        if (isset($Trec[0][$y])) {
          ...
          $dado1 = db_formatar($Trec[0][$y], 'f');
          $total += $Trec[0][$y];
        } else {
          $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
          $dado1 = db_formatar(0, 'f');
        }
        array_push($guardadados, $dado1);
      }
      ...
      for ($y = 1; $y <= $iMesFinalAtual; $y++) {
        ...
        if (isset($TrecB[0][$y])) {
          ...
          $dado2 = db_formatar($TrecB[0][$y], 'f');
          $total += $TrecB[0][$y];
        }else {
          $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
          $dado2 = db_formatar(0, 'f');
        }
        array_push($guardadados, $dado2);
      }
      $pdf->cell($cl ,$alt, db_formatar($total, 'f'), 'R', 0, "R", 0);
      array_push($guardadados, db_formatar($total, 'f'));
      
      if (isset($TrecB[0][13])) {
        $pdf->cell($cl, $alt, db_formatar($TrecB[0][13], 'f'), 0, 0, "R", 0);
        $dado3 = db_formatar($TrecB[0][13], 'f');
      }else {
        $pdf->cell($cl, $alt, db_formatar(0, 'f'), 0, 0, "R", 0);
        $dado3 = db_formatar(0, 'f');
      }
      array_push($guardadados, $dado3);
      $pdf->ln();
      //-------------------------------
      $pdf->setfont('arial', '', 6);
      $pdf->cell(60, $alt, " Receita Tributária", 'R', 0, "L", 0);
      $linha2 = "Receita Tributária";
      array_push($guardadados, $linha2);
      $total = 0;
      for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
        if (isset($tot_rec_trib[0][$y])) {
          $pdf->cell($cl, $alt, db_formatar($tot_rec_trib[0][$y], 'f'), 'R', 0, "R", 0);
          $dado4 = db_formatar($tot_rec_trib[0][$y], 'f');
          $total += $tot_rec_trib[0][$y];
        } else {
          $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
          $dado4 = db_formatar(0, 'f');
        }
        array_push($guardadados, $dado4);
      }

      for ($y = 1; $y <= $iMesFinalAtual; $y++) {
        if (isset($tot_rec_trib[0][$y])) {
          $pdf->cell($cl, $alt, db_formatar($tot_rec_trib[0][$y], 'f'), 'R', 0, "R", 0);
          $dado5 = db_formatar($tot_rec_trib[0][$y], 'f');
          $total += $tot_rec_trib[0][$y];
        } else {
          $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
          $dado5 = db_formatar(0, 'f');
        }
        array_push($guardadados, $dado5);
      }
      ...
      if (isset($tot_rec_trib[0][13])) {
        ...
        $dado6 = db_formatar($tot_rec_trib[0][13],'f');
      } else {
        $pdf->cell($cl, $alt, db_formatar(0, 'f'), 0, 0, "R", 0);
        $dado6 = db_formatar(0, 'f');
      }
      array_push($guardadados, $dado6);
      $pdf->ln();
    ...
    if ($x == 11) {
      ...
      $linha3 = "Transferências Correntes";
      array_push($guardadados, $linha3);
      $total = 0;
      
      for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
        if (isset($tot_transf[0][$y])) {
          $pdf->cell($cl, $alt, db_formatar($tot_transf[0][$y], 'f'), 'R', 0, "R", 0);
          $dado7 = db_formatar($tot_transf[0][$y], 'f');
          $total += $tot_transf[0][$y];
        } else {          
          $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
          $dado7 = db_formatar(0, 'f');
        }
        array_push($guardadados, $dado7);
      }

      for ($y = 1; $y <= $iMesFinalAtual; $y++) {
        if (isset($tot_transf[0][$y])) {
          $pdf->cell($cl, $alt, db_formatar($tot_transf[0][$y], 'f'), 'R', 0, "R", 0);
          $dado8 = db_formatar($tot_transf[0][$y], 'f');
          $total += $tot_transf[0][$y];
        } else {
          $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
          $dado8 = db_formatar(0, 'f');
        }
        array_push($guardadados, $dado8);
      }
      // listamos o total das 12 colunas
      $pdf->cell($cl, $alt, db_formatar($total, 'f'), 'R', 0, "R", 0);
      array_push($guardadados, db_formatar($total, 'f'));
      ...
      if (isset($tot_transf[0][13])) {
        ...
        $dado9 = db_formatar($tot_transf[0][13], 'f');
      } else {
        $pdf->cell($cl, $alt, db_formatar(0, 'f'), 0, 0, "R", 0);
        $dado9 = db_formatar(0, 'f');
      }
      array_push($guardadados, $dado9);
      $pdf->ln();
    }
    // ----------------------------------
    if ($x == 20) {
      ...
      $linha4 = "DEDUÇÕES(II)";
      array_push($guardadados, $linha4);
      $total = 0;
      
      for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
        if (isset($Trec[1][$y])) {
          $pdf->cell($cl, $alt, db_formatar($Trec[1][$y], 'f'), 'R', 0, "R", 0);
          $dado10 = db_formatar($Trec[1][$y], 'f');
          $total += $Trec[1][$y];
        } else {
          $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
          $dado10 = db_formatar(0, 'f');
        }
        array_push($guardadados, $dado10);
      }

      for ($y = 1;$y <= $iMesFinalAtual; $y++) {
        if (isset($TrecB[1][$y])) {
          $pdf->cell($cl, $alt, db_formatar(($TrecB[1][$y]), 'f'), 'R', 0, "R", 0);
          $dado11 = db_formatar(($TrecB[1][$y]), 'f');
          $total += $TrecB[1][$y];
        } else {
          $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
          $dado11 = db_formatar(0, 'f');
        }
        array_push($guardadados, $dado11);
      }
      $pdf->cell($cl, $alt, db_formatar(($total), 'f'), 'R', 0, "R", 0);
      array_push($guardadados, db_formatar(($total), 'f'));
      ...
      if (isset($TrecB[1][13])) {
        ...
        $dado12 = db_formatar(($TrecB[1][13]), 'f');
      } else {
        $pdf->cell($cl, $alt, db_formatar(0, 'f'), 0, 0, "R", 0);
        $dado12 = db_formatar(0, 'f');
      }
      array_push($guardadados, $dado12);
      $pdf->ln();
    }
    ...
    array_push($guardadados, $rec[$x][0]);
    ...
    for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
      ...
      if (isset($rec[$x][$y])) {
        ...
        $dado13 = db_formatar($rec[$x][$y], 'f');
        $total += $rec[$x][$y];
      } else {
        $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
        $dado13 = db_formatar(0, 'f');
      }
      array_push($guardadados, $dado13);
    }
    // procura valores do exercicio atual
    for ($y = 1; $y <= $iMesFinalAtual; $y++) {
      if (isset($recB[$x][$y])) {
        ...
        $dado14 = db_formatar($recB[$x][$y], 'f');
        $total += $recB[$x][$y];
      } else {
        $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
        $dado14 = db_formatar(0, 'f');
      }
      array_push($guardadados, $dado14);
    }
    // listamos o total das 12 colunas
    $pdf->cell($cl, $alt, db_formatar($total, 'f'), 'R', 0, "R", 0);
    array_push($guardadados, db_formatar($total, 'f'));
    ...
    if (isset($recB[$x][13])) {
      ...
      $dado15 = db_formatar($recB[$x][13], 'f');
    } else {
      $pdf->cell($cl, $alt, db_formatar(0, 'f'), 0, 0, "R", 0);
      $dado15 = db_formatar(0, 'f');
    }
    array_push($guardadados, $dado15);
    $pdf->ln();
  ...
  $linha5 = "RECEITA CORRENTE LÍQUIDA (III) = (I-II)";
  array_push($guardadados, $linha5);
  $total = 0;
  ...
  for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
    ...
    if (isset($Trec[0][$y])) {
      if(isset($Trec[1][$y])) {
        ...
        $dado16 = db_formatar($Trec["0"]["$y"] - $Trec["1"]["$y"], 'f');
        $total += $Trec["0"]["$y"] - $Trec["1"]["$y"];
      } else {
        $pdf->cell($cl, $alt, db_formatar($Trec[0][$y], 'f'), 'RBT', 0, "R", 0);
        $dado16 = db_formatar($Trec[0][$y], 'f');
        $total += $Trec[0][$y];
      }
    } else {
      $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'RBT', 0, "R", 0);
      $dado16 = db_formatar(0, 'f');
    }
    array_push($guardadados, $dado16);
  }
  ...
  for ($y = 1; $y <= $iMesFinalAtual; $y++) {
    if (isset($TrecB[0][$y])) {
      if (isset($TrecB[1][$y])) {
        ...
        $dado17 = db_formatar($TrecB[0][$y] - $TrecB[1][$y], 'f');
        $total += $TrecB[0][$y] - $TrecB[1][$y];
      }else {
        $pdf->cell($cl, $alt, db_formatar($TrecB[0][$y], 'f'), 'RTB', 0, "R", 0);
        $dado17 = db_formatar($TrecB[0][$y], 'f');
        $total += $TrecB[0][$y];
      }
    } else {
      $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'TRB', 0, "R", 0);
      $dado17 = db_formatar(0, 'f');
    }
    array_push($guardadados, $dado17);
    //		echo "$y: " . $TrecB[0][$y] . "<br>";
  }
  ...
  if (isset($TrecB[0][13])) {
    if (isset($TrecB[1][13])) {
      ...
      $dado18 = db_formatar($TrecB[0][13] - $TrecB[1][13], 'f');
    } else {
      $pdf->cell($cl, $alt, db_formatar($TrecB[0][13], 'f'), 'TB', 0, "R", 0);
      $dado18 = db_formatar($TrecB[0][13], 'f');
    }
  }else {
    $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'TB', 0, "R", 0);
    $dado18 = db_formatar(0, 'f');
  }
  array_push($guardadados, $dado18);
...
/*
  ...
  foreach ($guardadados as $linha){
    ...
    echo $c; echo " - "; echo $linha; echo "<br>";
    $c++;
  }
  die("Confere dados Finais");
*/

  header("Content-Type:   application/vnd.ms-excel; charset=utf-8");
  header("Content-type:   application/x-msexcel; charset=utf-8");
  header("Content-Disposition: attachment; filename=dem_rec_corrente_líquida.xls");
  header("Expires: 0");
...
print "<thead>";
print "<tr>";
print "<th colspan='2'>ESPECIFICAÇÃO</th>";
print "<th colspan='24'>EVOLUÇÃO DA RECEITA REALIZADA NOS ÚLTIMOS 12 MESES</th>";
print "<th colspan='2'>TOTAL</th>";
print "<th colspan='2'>PREVISÃO</th>";
print "</tr>";
//print "</thead>";
...
print "<tr>";
print "<td colspan='10'>" . $head2 . "</td>";
print "</tr>";

print "<tr>";
print "<td colspan='10'>" . $head3 . "</td>";
print "</tr>";

print "<tr>";
print "<td colspan='10'>" . $head4 . "</td>";
print "</tr>";

print "<tr>";
print "<td colspan='10'>" . $head5 . "</td>";
print "</tr>";

print "<tr>";
print "<td colspan='10'>" . $head6 . "</td>";
print "</tr>";
