...
class ReceitaContabil
{
    ...
    /**
     * TEF se o pagamento foi via TEF
     * @var bool
     */
    private $lTef = false;

    public function setTef($lTef)
    {
        $this->lTef = $lTef;
    }
    ...
    public function processaLancamentosReceita(
        $nValorArrecadacao = null,
        $iId = null,
        $dtAutenticacao = null,
        $iAutent = null,
        $lEstorno = false,
        $iContaDebito = null,
        $sObservacaoHistorico = null,
        $iCodigoPlanilha = null,
        $iCodigoNumPre = null,
        $iCodigoNumPar = null,
        $lDesconto = false,
        $iCodigoRecurso = null,
        ...
        $retencao = null,
        $empenho = null,
        $lancamentoOutraInstuicao = false
    ) {
        ...
        if (count($aDesdobramentos) > 0 && $oDadoSqlGeral->arrecada != 0.01) {
            $nValorSoma = 0;
            $lMultiplica = false;
            ...
            foreach ($aDesdobramentos as $oDadosDesdobramento) {
                ...
                $nValorPercentual = db_formatar(round($oDadoSqlGeral->arrecada * ($oDadosDesdobramento->percentual_receita / 100),
                        2), 'p') + 0;
        ...
        if (count($aReceitas) > 0) {
            ...
            foreach ($aReceitas as $iCodigoReceita => $oDadosReceita) {
                ...
                $sCaracteristicaPeculiarPlanilhaRecibo = $oDadosReceita->caracteristica_peculiar;
                ...
                if (empty($iCodigoContaCreditoPcasp)) {
                    ...
                    if (pg_num_rows($rsBuscaConta) == 0) {
                        ...
                        $sErroMensagem = "Impossível localizar o vínculo da conta orçamentaria de ";
                        $sErroMensagem .= "código {$iCodigoContaCredito} ";
                ...
                if ($iId != 0 && $iAutent != 0) {
                    ...
                    if (!empty($iCodigoNumPre) && !empty($iCodigoNumPar)) {
                        ...
                        if ($oDaoReciboCP->numrows > 0) {
                            $sCaracteristicaPeculiarPlanilhaRecibo = db_utils::fieldsMemory($rsBuscaCP,
                                0)->k130_concarpeculiar;
                        }
                    }
                }

                if (empty($sCaracteristaPeculiarRecibo)) {
                    $sCaracteristaPeculiarRecibo = $sCaracteristicaPeculiarPlanilhaRecibo;
                }
                ...
                /**
                 * verifica se é TEF para executar o lancamento do doc 165
                 */
                if ($this->getTef()) {
                    $iCodigoDocumentoExecutar = 165;
                }
                 ...
                 */
                $oContaPlano = new ContaPlanoPCASP(null, db_getsession('DB_anousu'), $iContaDebito,
                ...
                if ($lDesconto) {
                    ...
                    $oLancamentoContabil = EventoContabilRepository::getEventoContabilByCodigo(
                        $iCodigoDocumentoExecutar,
                        $iAnoAutenticacao);
                    ...
                    if (!empty($iCodigoNumPre) && !empty($iCodigoNumPar)) {
                        ...
                        if ($oDaoEmpPrestaRecibo->numrows > 0) {
                            ...
                            $oLancamentoAuxiliar->setNumeroEmpenho(db_utils::fieldsMemory($rsBuscaPrestacaoConta,
                                0)->e60_numemp);
                    ...
                    /**
                     * Quando pagamos uma retenção apropriamos nas contas do documetno 6000
                     */
                    if (!empty($retencao)) {

                        $oDaoRetencaoTipoRec = new cl_retencaotiporec;
                        $sqlRetencao = $oDaoRetencaoTipoRec->sql_query_file($retencao, "*", null, null);
                        $rsRetencao = db_query($sqlRetencao);
                        $dadosRetencao = db_utils::fieldsMemory($rsRetencao, 0);
                        $iCodigoDocumentoExecutar = 6000;

                        if ($lancamentoOutraInstuicao) {
                            $iCodigoDocumentoExecutar = 6006;
                        }
                        $oLancamentoAuxiliar->setContaCredito($iCodigoContaCreditoPcasp);
                        if ($lEstorno) {

                            $iCodigoDocumentoExecutar = 6001;
                            $oLancamentoAuxiliar->setContaDebito($iCodigoContaCreditoPcasp);
                            if ($lancamentoOutraInstuicao) {
                                $iCodigoDocumentoExecutar = 6007;
                            }
                        }
                        $oLancamentoAuxiliar->setTipoCalculoRetencao($dadosRetencao->e21_retencaotipocalc);
                        $oLancamentoAuxiliar->setRetencao($dadosRetencao->e21_sequencial);
                        $oLancamentoAuxiliar->setNumeroEmpenho($empenho);
    ...
    public function getContaOrcamento()
    {
        ...
        if (empty($this->oContaOrcamento)) {
            $this->oContaOrcamento = ContaOrcamentoRepository::getContaByCodigo($this->iCodigoContaContabil,
                $this->getAno());
        }
     ...
     */
    public static function getReceitasNoAno($iAno, $iInstituicao, $sWhereAdicional = null)
    {
    ...
    public function getDesdobramentos()
    {
        ...
        if (count($this->aDesdobramentos) == 0) {
            ...
            */
            ...
            $sql = $oDaoOrcReceita->sql_query_fonte_desdobramento(
                null,
                null,
                "o70_codrec",
                "o57_fonte",
            ...
            $rsBuscaDesdobramento = $oDaoOrcReceita->sql_record($sql);
            if ($oDaoOrcReceita->numrows > 0) {
