...
$origem = \ECidade\Financeiro\Orcamento\Recurso\Origem::getAutorizacao($this->numaut);
$complemento = '';
if (!empty($origem)) {
    $complemento = "{$origem->o200_sequencial} - {$origem->o200_descricao}";
}
...
if (!empty($this->numero_licitacao)) {
...
//  $this->objpdf->text($xcol + 108, $xlin + 32, 'Destino');
  $this->objpdf->text($xcol + 108, $xlin + 32, 'Complemento');
...
//Quebrar o nome da unidade para 48 caracteres (3 parametro)
  ...
  $this->objpdf->text($xcol + 122, $xlin + 11.5, ' :  ' . substr(db_formatar($this->orgao, 'orgao') . ' - ' . $this->descr_orgao, 0, 46) );
...
//  $this->objpdf->text($xcol + 122, $xlin + 32, ' :  ' . stripslashes($this->destino));
  $this->objpdf->text($xcol + 126, $xlin + 32, ' :  ' . $complemento);
  ...
  $this->objpdf->rect($xcol, $xlin + 64, 8, 159, 2, 'DF', '34');
  $this->objpdf->rect($xcol + 8, $xlin + 64, 12, 159, 2, 'DF', '34');
  $this->objpdf->rect($xcol + 20, $xlin + 64, 15, 159, 2, 'DF', '34');
  $this->objpdf->rect($xcol + 35, $xlin + 64, 107, 159, 2, 'DF', '34');
  $this->objpdf->rect($xcol + 142, $xlin + 64, 30, 182, 2, 'DF', '');
  $this->objpdf->rect($xcol + 172, $xlin + 64, 30, 182, 2, 'DF', '34');

  $this->objpdf->rect($xcol, $xlin + 246, 142, 10, 2, 'DF', '34');
  $this->objpdf->rect($xcol + 142, $xlin + 246, 30, 10, 2, 'DF', '34');
  $this->objpdf->rect($xcol + 172, $xlin + 246, 30, 10, 2, 'DF', '34');

  $this->objpdf->rect($xcol, $xlin + 223, 142, 23, 2, 'DF', '');
    ...
    if (($this->objpdf->gety() > $this->objpdf->h - 110 && $pagina == 1) || ($this->objpdf->gety() > $this->objpdf->h - 40 && $pagina != 1)) {
      if (($proximo + 1) < $this->linhasdositens) { // Alterado para controle de itens nao imprimir paginas sem itens(branco)
          if ($pagina == 1) {
            ...
            $this->objpdf->Setfont('Arial', '', 7);
            $this->objpdf->multicell(140, 3, $this->resumo . $destino);
            $this->objpdf->Setfont('Arial', 'B', 8);
          }
  ...
  if ($pagina == 1) {
    ...
    $this->objpdf->setxy($xcol +1, $xlin +181.5);


    $sResumo =  substr($this->resumo . $destino, 0, 730) ;


      $this->objpdf->SetXY($xcol, $xlin + 223);
     $this->objpdf->multicell(140, 4, "RESUMO: " . $sResumo);
