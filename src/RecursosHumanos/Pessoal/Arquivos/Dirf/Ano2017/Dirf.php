...
class Dirf extends \Dirf2015
{
    ...
    protected function processarDadosPensaoAlimenticia($mrubr, $oPessoa, $sigla, $Iarq, $arq)
    {
        ...
        $sSqlPensionistas = $oDaoDirfPensionista->sql_query_valor_pensao(
            implode(", ", $aCampos),
            implode(" and ", $aWhere),
            implode(", ", $aGroup),
            implode(", ", $aGroup)
        ...
        for ($iPensao = 0; $iPensao < $iTotalPensionistas; $iPensao++) {
            ...
            if (empty($this->aListaPessoasPensao[$iCodigoRegistro]
                             [$oDadosPensao->rh96_sequencial][$oDadosPensao->r52_numcgm])) {
                ...
                $this->aListaPessoasPensao[$iCodigoRegistro][$oDadosPensao->rh96_sequencial]
                                          [$oDadosPensao->r52_numcgm] = $oDadosPensionista;
            }
            $oPensionista = $this->aListaPessoasPensao[$iCodigoRegistro][$oDadosPensao->rh96_sequencial]
                                                      [$oDadosPensao->r52_numcgm];
     ...
     */
    protected function processarDadosPrevidenciaPrivada(
        $oPessoa,
        $sRubrica,
        $iLinhaProcessamento,
        $sSiglaTabela,
        ...
        if (empty($oEmpresa)) {
            $sMensagem = "Rubrica {$sRubrica} está na base B910, mas não foram informados os dados da previdência ";
            $sMensagem .= "complementar cadastro da rubrica. Para realizar o ajuste, acessa a rotina ";
            $sMensagem .= "'Pessoal > Cadastro > Tabelas > Rubricas/Códigos'  para ajustar.";
        ...
        $oDadosPrevidencia->previdencia[$oEmpresa->getCodigo()][$iMes] +=
                            $aRegistros[$iLinhaProcessamento][$sSiglaTabela."valor"];
    }
            ...
            foreach ($oPessoa->previdencia as $iCgmEmpresa => $aMeses) {
                foreach ($aMeses as $iMes => $valor) {
                    ...
                    if ($oDaoDirfValor->erro_status == 0) {
                        $sMsg = "Erro ao incluir valores da previdencia privada.\n";
                        throw new \DBException($sMsg . $oDaoDirfValor->erro_msg);
                    }
                    ...
                    if ($oDaoDirfValorPrevidencia->erro_status == 0) {
                        $sMsg = "Erro ao incluir vinculo das previdencia privada com valor da dirf.\n";
                        throw new \DBException($sMsg . $oDaoDirfValorPrevidencia->erro_msg);
                    }
                    ...
                    $oDaoRhDirfGeracaoPessoalMatricula->rh99_rhdirfgeracaodadospessoalvalor =
                                                             $oDaoDirfValor->rh98_sequencial;
    ...
    public function processarRRA(\Servidor $oServidor, $lPortadorMolestia, $oPessoa)
    {
        ...
        $sSqlPensionistasRRA = $oDaoLancamentoRRAPensionista->sql_query_dirf(
    ...
    private function salvarDadosPensao($aDadosPensionistas, $tipoDirf = '0561', $tipoRegistro = Dirf::REGISTRO_PENSAO)
    {
        ...
        foreach ($aDadosPensionistas as $iCodigoDirfServidor => $aPensionistas) {
            foreach ($aPensionistas as $oPensionista) {
                $sWhere = "rh202_rhdirfgeracaopessoal = {$iCodigoDirfServidor} and ";
                $sWhere .= "rh202_numcgm = {$oPensionista->numcgm}";
                ...
                if (pg_num_rows($rsPensionista) == 0) {
                    ...
                    if ($oDaoDirfPensionista->erro_status == 0) {
                        $sMsg = "Erro ao incluir dados do pensionista.\n";
                        throw new \DBException($sMsg . $oDaoDirfPensionista->erro_msg);
                    }
                ...
                foreach ($oPensionista->valores as $iMes => $valor) {
                    ...
                    if ($oDaoPensionistaValor->erro_status == 0) {
                        $sMsg = "Erro ao incluir vinculo do pensionista com valor da dirf.\n";
                        throw new \DBException($sMsg . $oDaoPensionistaValor->erro_msg);
                    }
                    /**
                     * vincula as matriculas ao valor calculado para o cpf.
                     */
