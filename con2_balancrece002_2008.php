...
for ($i = 0; $i < $totalRecursos; $i++) {
    ...
    $recursos[$dadosRecurso->o15_codigo] = $dadosRecurso;
}
...
$resultinst = db_query("select codigo,nomeinst,nomeinstabrev from db_config where codigo in ($listaInstituicoes) ");
$descr_inst = '';
$xvirg = '';
$flag_abrev = false;
...
if ($origem == "O") {
    ...
    if ($opcao == 3) {
        ...
        $mesFim = strtoupper(db_mes(substr($perfin, 5, 2)));
        $head6 = "PERÍODO : " . strtoupper(db_mes(substr($perini, 5, 2))) . " A " . $mesFim;
    }
}
$db_filtro = " o70_instit in ($listaInstituicoes)";
...
if (!isset($tipo)) {
    ...
    $head1 = "DEMONSTRATIVO DA RECEITA";
...
if (! empty($oGet->recursos_selecionados)) {
    $db_filtro .= " and o70_codigo in ({$oGet->recursos_selecionados})";
}
...
if ($estrut_inicial != '') {
    ...
    foreach ($aEstrutural as $sEstrutural) {
        ...
        $aWhereEstrutural[] = " o57_fonte like '{$sEstrutural}%' ";
    }
    if ($aWhereEstrutural) {
        $db_filtro .= " and (" . implode(" OR ", $aWhereEstrutural) . ") ";
    }
}

$filtroPrevisao = "
 WHERE     CAST(COALESCE(NULLIF(substr(fc_receitasaldo, 81, 12), ''), '0') AS float8) <> '0'
        OR CAST(COALESCE(NULLIF(substr(fc_receitasaldo, 15, 12), ''), '0') AS float8) <> '0'
        OR CAST(COALESCE(nullif(substr(fc_receitasaldo, 68, 12), ''), '0') AS float8) <> '0'
        OR CAST(COALESCE(nullif(substr(fc_receitasaldo,55,12),''),'0') AS float8) <> '0'
";
if ($previsao=='nao' || $opcao == 1) {
    $filtroPrevisao = "";
}

if ($nivel_agrupar == 0) {    
    if (EMENTARIO_RECEITA) {        
        $result = ReceitaSaldo(11, 1, $opcao, true, $db_filtro, $anousu, $dataini, $datafin, false, ' * ', true, $nivel_agrupar, $filtroPrevisao);
    } else {
    ...
    $sql .= "select o57_anousu,
                     ...
                     saldo_anterior,
                     saldo_arrecadado,
                     ((saldo_inicial + saldo_prevadic_acum) - saldo_arrecadado_acumulado) as saldo_a_arrecadar,
              ...
              from orcfontes
               ...
               (
                ...
                left join orctiporec on o70_codigo = orctiporec.o15_codigo
                left join fonterecurso on orctiporec.o15_codigo = fonterecurso.orctiporec_id and exercicio = o70_anousu
...
$total_saldo_inicial26 = 0;
$total_saldo_prevadic_acum26 = 0;

foreach ($yyy as $linha){
    if($linha["o57_fonte"] == 470000000000000){
        $guarda004700["saldo_inicial"] = $linha["saldo_inicial"];
        $guarda004700["saldo_prevadic_acum"] = $linha["saldo_prevadic_acum"];
        $guarda004700["saldo_arrecadado"] = $linha["saldo_arrecadado"];
        $guarda004700["saldo_arrecadado_acumulado"] = $linha["saldo_arrecadado_acumulado"];
        $guarda004700["saldo_a_arrecadar"] = $linha["saldo_a_arrecadar"];
    }


    if($anousu == 2026){
        if( $linha["o57_fonte"] == 411000000000000 ||
            $linha["o57_fonte"] == 412000000000000 || 
            $linha["o57_fonte"] == 413000000000000 || 
            $linha["o57_fonte"] == 415000000000000 || 
            $linha["o57_fonte"] == 416000000000000 || 
            $linha["o57_fonte"] == 417000000000000 || 
            $linha["o57_fonte"] == 419000000000000 || 
            $linha["o57_fonte"] == 422000000000000 || 
            $linha["o57_fonte"] == 424000000000000            
        ){  
            $somaa += $linha["saldo_inicial"];
            $somab += $linha["saldo_prevadic_acum"];
        }elseif($linha["o57_fonte"] == 917000000000000 ){
            $subtraia += $linha["saldo_inicial"];
            $subtraib += $linha["saldo_prevadic_acum"];
        }
    }
}

if($anousu == 2026){
    $total_saldo_inicial26 = $somaa - $subtraia;
    $total_saldo_prevadic_acum26 = $somab - $subtraib;
...
for ($i = 0; $i < pg_num_rows($result); $i++) {
    ...
    if ($pdf->gety() > $pdf->getH() - 30 || $pagina == 1) {
        ...
        $pdf->cell($tm_recurso, $alt, "REC", 0, 0, "L", 0);
        if ($origem == "O") {
        ...
        $pdf->cell($tm_perc, $alt, "Perc", 0, 1, "R", 0);
        $pdf->ln(3);
    }
    /**
    ...
    if ($descr == "") {
        ...
        $pdf->cell($tm_descr, $alt, substr($descr, 0, ($impressao == 'paisagem'?80:43)), 0, 0, "L", $fundo);
    }

    if (isset($o70_concarpeculiar)) {
        $res_orcreceita = $clorcreceita->sql_record($clorcreceita->sql_query(
            null,
            null,
            "c58_descr",
            null,
            "o70_anousu = $anousu and o57_fonte = '$o57_fonte'"
        ));
    ...
    if (isset($o70_codigo) && $o70_codigo != 0) {
        $recurso = descricaoCompletaRecurso($o70_codigo, $anousu, $apresentar);
        $pdf->cell($tm_recurso, $alt, $recurso, 0, 0, "L", $fundo);
    } else {
        $pdf->cell($tm_recurso, $alt, '', 0, 0, "L", $fundo);
    }
    ...
    if ($origem == "O") {
        if ($nivel_agrupar == 0 || ($nivel_agrupar != 0 and $tipo_conta == 'A')) {
            ...
            $pdf->cell(
                $tm_valor,
                $alt,
                db_formatar($matriz_sintetico[$i]['total_saldo_inicial'], 'f'),
                0,
                0,
                "R",
                $fundo
            ...
            $pdf->cell(
                $tm_valor,
                $alt,
                db_formatar($matriz_sintetico[$i]['total_saldo_prevadic_acum'], 'f'),
                0,
                0,
                "R",
                $fundo
        ...
        if($elemento == "400000000000000"){

            $saldo_inicial -= $guarda004700["saldo_inicial"];
            $saldo_prevadic_acum -= $guarda004700["saldo_prevadic_acum"];
            $saldo_arrecadado -= $guarda004700["saldo_arrecadado"];
            $saldo_arrecadado_acumulado -= $guarda004700["saldo_arrecadado_acumulado"];
            $saldo_a_arrecadar -= $guarda004700["saldo_a_arrecadar"];
        }
        if ($nivel_agrupar == 0 || ($nivel_agrupar != 0 and $tipo_conta == 'A')) {
            $pdf->cell($tm_valor, $alt, db_formatar($saldo_inicial, 'f'), 0, 0, "R", $fundo);
            $pdf->cell($tm_valor, $alt, db_formatar($saldo_prevadic_acum, 'f'), 0, 0, "R", $fundo);
            ...
            $pdf->cell(
                $tm_valor,
                $alt,
                db_formatar($matriz_sintetico[$i]['total_saldo_arrecadado'], 'f'),
                0,
                0,
                "R",
                $fundo
            ...
            $pdf->cell(
                $tm_valor,
                $alt,
                db_formatar($matriz_sintetico[$i]['total_saldo_arrecadado_acumulado'], 'f'),
                0,
                0,
                "R",
                $fundo
            ...
            $pdf->cell(
                $tm_valor,
                $alt,
                db_formatar($matriz_sintetico[$i]['total_saldo_a_arrecadar'], 'f'),
                0,
                0,
                "R",
                $fundo
    ...
    if ($saldo_inicial + $saldo_prevadic_acum != 0) {
        $pdf->cell(
            $tm_perc,
            $alt,
            db_formatar($saldo_arrecadado_acumulado / ($saldo_inicial + $saldo_prevadic_acum) * 100, 'f'),
            0,
            1,
            "R",
            $fundo
        ...
        $pdf->cell($tm_perc, $alt, db_formatar(0, 'f'), 0, 1, "R", $fundo);
    }
    if($elemento == 470000000000000){
        $guarda4700["saldo_inicial"] = $saldo_inicial;
        $guarda4700["saldo_prevadic_acum"] = $saldo_prevadic_acum;
        $guarda4700["saldo_arrecadado"] = $saldo_arrecadado;
        $guarda4700["saldo_arrecadado_acumulado"] = $saldo_arrecadado_acumulado;
        $guarda4700["saldo_a_arrecadar"] = $saldo_a_arrecadar;
        $guarda4700["porcentual"] = db_formatar($saldo_arrecadado_acumulado / ($saldo_inicial + $saldo_prevadic_acum) * 100, 'f');
    }
}
$pdf->setfont('arial', 'B', $tm_fonte);
$pdf->cell($tm_estrut, $alt, '', 0, 0, "L", 0);
$pdf->cell($tm_descr + ($tm_reduz + $tm_recurso + $tm_concarpeculiar), $alt, 'TOTAL ', 0, 0, "L", 0);
if ($origem == "O") {    
    if($anousu == 2026){
        $pdf->cell($tm_valor, $alt, db_formatar($total_saldo_inicial26, 'f'), 0, 0, "R", 0);
        $pdf->cell($tm_valor, $alt, db_formatar($total_saldo_prevadic_acum26, 'f'), 0, 1, "R", 0);
    ...
    $total_saldo_inicial -= $guarda4700["saldo_inicial"];
    $total_saldo_prevadic_acum -= $guarda4700["saldo_prevadic_acum"];
    $total_saldo_arrecadado -= $guarda4700["saldo_arrecadado"];
    $total_saldo_arrecadado_acumulado -= $guarda4700["saldo_arrecadado_acumulado"];
    $total_saldo_a_arrecadar -= $guarda4700["saldo_a_arrecadar"];
    ...
    if ($total_saldo_inicial + $total_saldo_prevadic_acum != 0) {
        $pdf->cell(
            ...
            $alt,
            db_formatar(
                $total_saldo_arrecadado_acumulado / ($total_saldo_inicial + $total_saldo_prevadic_acum) * 100,
                'f'
            ),
            0,
            1,
        ...
        $pdf->cell($tm_prec, $alt, db_formatar(0, 'f'), 0, 1, "R", 0);
    }
}
...
if ($pdf->getAvailHeight() > 270 and isParaiba()) {
    $pdf->setfont('arial', '', 7);
    $pdf->cell($pdf->getW(), $pdf->getH(), "NADA A REGISTRAR", 0, 0, "C", 0);
    $pdf->sety($pdf->getH()-60);
}
