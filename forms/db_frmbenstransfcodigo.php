...
//MODULO: patrim
include(modification("classes/db_situabens_classe.php"));
...
<script>
  ...
  function js_pesquisat95_codbem(mostra){
    if(mostra == true){
      js_OpenJanelaIframe('CurrentWindow.corpo.iframe_benstransfcodigo','db_iframe_bensconfirmacao','<?=($func_conf)?>?funcao_js=parent.js_mostrabens1|t52_bem|t52_descr|t52_ident&chave_id_usuario=<?=$idus?>&chave_t93_depart=<?=$iddepart?>&db_param=<?=($db_param)?>','Pesquisa',true);
    }else{
      if(document.form1.t95_codbem.value != ''){
        js_OpenJanelaIframe('CurrentWindow.corpo.iframe_benstransfcodigo','db_iframe_bensconfirmacao','<?=($func_conf)?>?pesquisa_chave='+document.form1.t95_codbem.value+'&funcao_js=parent.js_mostrabens&chave_id_usuario=<?=$idus?>&chave_t93_depart=<?=$iddepart?>&db_param=<?=($db_param)?>','Pesquisa',false);
      }else{
  ...
  function js_pesquisat52_placa(mostra){
    if(mostra == true){
      ...
      if(document.form1.t52_ident.value != ''){
        ...
        js_OpenJanelaIframe('CurrentWindow.corpo.iframe_benstransfcodigo','db_iframe_bensconfirmacao','<?=($func_conf)?>?pesquisa_chave_placa='+document.form1.t52_ident.value+'&funcao_js=parent.js_mostrabens&chave_id_usuario=<?=$idus?>&chave_t93_depart=<?=$iddepart?>&db_param=<?=($db_param)?>','Pesquisa',false);
      }else{
        sPlacaParametro = '';
