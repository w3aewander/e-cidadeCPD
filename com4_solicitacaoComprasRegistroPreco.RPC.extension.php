...
switch ($oParam->exec) {
  ...
  case "salvarItensAbertura":
    try {
      ...
      foreach ($oParam->aItens as $oItemSolicitacao) {
       ...
       /**
       * Utilizamos este mesmo para salvar codigo da compilação na solicitação web
       * Facilitando a consulta e inserção dos itens necessários
       */

        @pg_query("update
          plugins.presolicitacao
        set
          pc10_compilacao = {$oSolicita->getCodigoSolicitacao()}
        where
          pc10_numero = {$oSolicita->getCodigoAbertura()}
          and datacancelamento is null
          and pc10_compilacao is null");

        $oRetorno->message = $oItemSolicitacao;
  ...
  case 'geraProcessoCompras' :
    ...
    try {
      ...
      db_fim_transacao(false);
       ...
       /**
       * Utilizamos este mesmo para salvar codigo do processo na solicitação web
       * Facilitando a consulta e inserção dos itens necessários
       */
      @pg_query("update
        plugins.presolicitacao
      set
        pc10_processo = {$oSolicita->getProcessodeCompras()}
      where
        pc10_numero = {$oSolicita->getCodigoAbertura()}
        and datacancelamento is null
        and pc10_processo is null");
  ...
  case 'cancelaProcessoCompras' :
    ...
    try {
      ...
      db_fim_transacao(false);
       ...
       /**
       * Utilizamos esta query para remover pc10_processo
       * Facilitando a consulta e inserção dos itens necessários
       */
      @pg_query("update
        plugins.presolicitacao
  ...
  case 'anularEstimativaRegistroPreco' :
    ...
    try {
      ...
      $oSql = "select ssv.pc53_solicitapai, ssv.pc53_solicitafilho from solicitavinculo ssv
        inner join solicita ss on (ssv.pc53_solicitafilho = ss.pc10_numero)
        left join solicitaanulada ssa on (ssa.pc67_solicita = ssv.pc53_solicitafilho)
        where ssv.pc53_solicitapai in (select solicitavinculo.pc53_solicitapai from solicitavinculo
        where solicitavinculo.pc53_solicitafilho = {$oParam->iCodigoSolicitacao})
        and ss.pc10_solicitacaotipo = 4
        and ssa.pc67_sequencial is null";

        $oSql1 = pg_fetch_all(db_query($oSql));

        foreach ($oSql1 as $i => $v) {
          $oSolicita = new estimativaRegistroPreco($v['pc53_solicitafilho']);
          $oSolicita->anular($sMotivo);
        }

        @pg_query("update plugins.presolicitacao set pc10_estimativa=null where pc10_numero = " . $oSql1[0]['pc53_solicitapai']);
  ...
  case 'anularCompilacaoRegistroPreco' :
    ...
    try {
      ...
      $oSql = "select solicitavinculo.pc53_solicitapai from solicitavinculo
      where solicitavinculo.pc53_solicitafilho = {$oParam->iCodigoSolicitacao}";

      $oSql1 = pg_fetch_all(db_query($oSql));

      @pg_query("update plugins.presolicitacao set pc10_compilacao=null where pc10_numero = " . $oSql1[0]['pc53_solicitapai']);
