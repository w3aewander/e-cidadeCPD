...
use ECidade\Patrimonial\Compras\RegistroPreco\Repository\Estimativa as EstimativaRegistroPreco;
use ECidade\Patrimonial\Compras\RegistroPreco\Repository\Compilacao as CompilacaoRegistroPreco;
...
for ( $iInd = 0; $iInd  < $iRsSql; $iInd++ ) {
    ...
    $oEstimativa = EstimativaRegistroPreco::getInstanciaPorCodigo($iCodigoEstimativa);

    $iDepartamentoEstimativa          = $oEstimativa->getCodigoDepartamento();
    $iInstituicaoEstimativa           = $oEstimativa->getCodigoInstituicao();

    $oCompilacao = CompilacaoRegistroPreco::getInstanciaPorCodigo($iCodigoCompilacao);

    $oSolicita->oDadosFornecedor      = $oCompilacao->getFornecedorItem($oSolicita->pc01_codmater, $oSolicita->pc11_codigo);
    $oSolicita->empenhada             = $oCompilacao->getValorEmpenhadoItem($oSolicita->pc11_codigo, $iDepartamentoEstimativa);
    $oSolicita->empenhadaAnulada      = $oCompilacao->getValorEmpenhadoAnuladoItem($oSolicita->pc11_codigo, $iDepartamentoEstimativa);
    $oSolicita->solicitada            = $oCompilacao->getValorSolicitadoItem($oSolicita->pc11_codigo, $iDepartamentoEstimativa);
    ...
    $oMovimentacao = $oItemEstimativa->getMovimentacao();

    $oItemCompilacao = new itemCompilacao($oSolicita->pc11_codigo);
    $oSaldosItemCompilacao = $oItemCompilacao->getSaldo();

    $saldo = 0;
    if ($oMovimentacao->saldo <= $oSaldosItemCompilacao || !$oItemCompilacao->temReservaDeCota()) {
        $saldo = $oMovimentacao->saldo;
    } else {
        $saldo = $oSaldosItemCompilacao;
    }
    ...
    $oStdItemEstimativa->iEmpenhada   = $oSolicita->empenhada - $oSolicita->empenhadaAnulada ?: '0';
    $oStdItemEstimativa->iLiquidada   = buscaQuantidadeLiquidada($oSolicita->pc11_codigo);
    $oStdItemEstimativa->iSolicitada  = $oSolicita->solicitada - $oSolicita->empenhadaAnulada ?: '0';
    $oStdItemEstimativa->iReservada   = buscaQuantidadeReservada($oSolicita->pc11_codigo);
    $oStdItemEstimativa->nQuantMax    = (empty($oSolicita->pc57_quantmax)                   ? '0' : $oSolicita->pc57_quantmax . '/' .$oEstimativa->getItemByCodigo($iCodigoItemEstimativa)->getQuantidade());
    $oStdItemEstimativa->iSolicitar   = $saldo ?: '0';
    ...
    $oStdItemEstimativa->cedidas = $oMovimentacao->cedidas;
    $oStdItemEstimativa->recebidas = $oMovimentacao->recebidas;
    ...
    if (empty($oDadosPosRegPreco->aSolicitacoes[$iCodigoCompilacao]->aInstituicoesEstimativas[$iInstituicaoEstimativa])) {
        ...
        $oInstituicao                    = InstituicaoRepository::getInstituicaoByCodigo($iInstituicaoEstimativa);
    ...
    if (empty($oDadosPosRegPreco->aSolicitacoes[$iCodigoCompilacao]->aInstituicoesEstimativas[$iInstituicaoEstimativa]->aDepartamentos[$iDepartamentoEstimativa])) {
        ...
        $oDepartamento                  = DBDepartamentoRepository::getDBDepartamentoByCodigo($iDepartamentoEstimativa);
...
foreach ($oDadosPosRegPreco->aSolicitacoes as $iCodigoCompilacao => $oCompilacao ) {
    ...
    foreach ($oCompilacao->aInstituicoesEstimativas as $iCodigoInstituicao => $oInstituicao) {
        ...
        foreach ($oInstituicao->aDepartamentos as $iCodigoDepartamento => $oDepartamento) {
            ...
            $oPdf->setfont('arial', '', 6);
            $oPdf->cell( 75, $iAlt, $iCodigoEstimativa  , "TB", 0, "L", 1 );
            $oPdf->cell(12, $iAlt, 'Recebida'  , "TBLR", 0, "L", 1);
            $oPdf->cell(11, $iAlt, 'Cedida'  , "TBLR", 0, "C", 1);
            $oPdf->cell(64, $iAlt, ''  , "TBL", 1, "L", 1);
            ...
            foreach ($oDepartamento->aEstimativas as $iCodigoEstimativa => $oEstimativa) {
                ...
                foreach($oEstimativa->aItens as $oItemEstimativa) {
                    ...
                    $oPdf->cell(28, $iAlt, str_replace("\\n", "\n",substr(trim($oItemEstimativa->sCompl), 0, 20)) , 0, 0, "L", $lPreenchimento);
                    $oPdf->cell(13, $iAlt, $oItemEstimativa->sUnidade    , 0, 0, "C", $lPreenchimento);
                    $oPdf->cell(16, $iAlt, db_formatar($oItemEstimativa->nVlrUnitario, 'v', " ", $casadec), 0, 0, "R", $lPreenchimento);
                    $oPdf->cell(($oCompilacao->lControlaValor ? 50 : 25), $iAlt, substr($oItemEstimativa->sFornecedor, 0, ($oCompilacao->lControlaValor ? 35 : 20)), 0, 0, "L", $lPreenchimento);

                    if (!$oCompilacao->lControlaValor) {
                        $oPdf->cell(22, $iAlt, $oItemEstimativa->nQuantMax   , 0, 0, "R", $lPreenchimento);
                    }

                    $oPdf->cell(30, $iAlt, ($oCompilacao->lControlaValor ? db_formatar($oItemEstimativa->iSolicitada, 'v', " ", $casadec) : $oItemEstimativa->iSolicitada) , 0, 0, "R", $lPreenchimento);
                    $oPdf->cell(12, $iAlt, $oItemEstimativa->recebidas, 0,0, "C", $lPreenchimento);
                    $oPdf->cell(11, $iAlt, $oItemEstimativa->cedidas, 0,0, "C", $lPreenchimento);
                    $oPdf->cell(23, $iAlt, ($oCompilacao->lControlaValor ? db_formatar($oItemEstimativa->iEmpenhada, 'v', " ", $casadec) : $oItemEstimativa->iEmpenhada), 0,0, "R", $lPreenchimento);
                    $oPdf->cell(22, $iAlt, ($oCompilacao->lControlaValor ? db_formatar($oItemEstimativa->iSolicitar, 'v', " ", $casadec) : $oItemEstimativa->iSolicitar), 0,0, "R", $lPreenchimento);
                    $oPdf->cell(19, $iAlt, ($oCompilacao->lControlaValor ? db_formatar($oItemEstimativa->iEmpenhar, 'v', " ", $casadec) : ($oItemEstimativa->iEmpenhar ? $oItemEstimativa->iEmpenhar : '0')), 0, 1, "R", $lPreenchimento);
                }
            }
            ...
            $oPdf->cell(3,  $iAlt, ($oCompilacao->lControlaValor ? db_formatar($oDepartamento->iSolicitada, 'v', " ", $casadec) : $oDepartamento->iSolicitada)  ,   0, 0, "R", 0);
            $oPdf->cell(47,  $iAlt, ($oCompilacao->lControlaValor ? db_formatar($oDepartamento->iEmpenhada, 'v', " ", $casadec) : $oDepartamento->iEmpenhada)  ,   0, 0, "R", 0);
        ...
        $oPdf->cell(18,  $iAlt, ''                          ,   0, 0, "R", 0);
        $oPdf->cell(3,  $iAlt, ($oCompilacao->lControlaValor ? db_formatar($oInstituicao->iSolicitada, 'v', " ", $casadec) : $oInstituicao->iSolicitada)  ,   0, 0, "R", 0);
        $oPdf->cell(47,  $iAlt, ($oCompilacao->lControlaValor ? db_formatar($oInstituicao->iEmpenhada, 'v', " ", $casadec) : $oInstituicao->iEmpenhada)   ,   0, 0, "R", 0);
...
foreach ($oDadosPosRegPreco->aSolicitacoes as $iCodigoCompilacao => $oCompilacao ) {
    ...
    foreach ($oCompilacao->aInstituicoesEstimativas as $iCodigoInstituicao => $oInstituicao) {
        ...
        foreach ($oInstituicao->aDepartamentos as $iCodigoDepartamento => $oDepartamento) {
            ...
            $posicaoY = $oPdf->GetY();
            $totalLinhas = $oPdf->NbLines(115, $sSolicitacoes);

            $oPdf->MultiCell(115, $iAlt, $sSolicitacoes, 0, 'L');

            $oPdf->SetY($posicaoY + ($totalLinhas * $iAlt));
...
function imprimeCabecalho(&$oPdf, $iAlt, $iAbertura, $iCompilacao, $lQuebraPagina = false, $lControlaValor) {
    ...
    $oPdf->setfont('arial', 'b', 8);
    $oPdf->cell(6, $iAlt, "Compilação:", 'TB', 0, "C", 1);

    $oPdf->setfont('arial', '', 6);
    $oPdf->cell(($lControlaValor ? 111 : 93), $iAlt, $iCompilacao , 'RTB', 0, "L", 1);

    $oPdf->setfont('arial', 'b', 8);
    $oPdf->cell(91, $iAlt, ($lControlaValor ? "Valor" : "Quantidade") , 1, 0, "C", 1);
    $oPdf->cell(41, $iAlt, "Saldos"     , "TB", 1, "C", 1);

    $oPdf->cell(15, $iAlt, "Seq."       , "TB", 0, "C", 1);
    $oPdf->cell(15, $iAlt, "Item"       , 1, 0, "C", 1);
    $oPdf->cell(28, $iAlt, "Descrição"  , 1, 0, "C", 1);
    $oPdf->cell(25, $iAlt, "Complemento", 1, 0, "C", 1);
    ...
    if (!$lControlaValor) {
        $oPdf->cell(22, $iAlt, "Máx/Estimado"        , 1, 0, "C", 1);
    }

    $oPdf->cell(23, $iAlt, ($lControlaValor ? "Solicitado" : "Solicitada") , 1, 0, "C", 1);
    $oPdf->cell(23, $iAlt, ($lControlaValor ? "Cedência" : "Cedência")  , 1, 0, "C", 1);
    $oPdf->cell(23, $iAlt, ($lControlaValor ? "Empenhado" : "Empenhada")  , 1, 0, "C", 1);
    $oPdf->cell(22, $iAlt, "Solicitar"  , 1, 0, "C", 1);
    $oPdf->cell(19, $iAlt, "Empenhar"   , "TB", 1, "C", 1);
