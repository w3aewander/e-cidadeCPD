...
for( $iX = 0; $iX < $iTam; $iX++ ) {
  ...
  for( $y = 1; $y <= 1; $y++ ) {
    ...
    $pdf->rect( $pdf->getX() + 58, $pdf->getY(), 46, 16, "D" );

    $pdf->setfont( 'arial', 'b', 7 );
    $pdf->text( $pdf->getX() + 63, $pdf->getY() + 5, "ATENDIMENTO Nro :   " . $sd24_i_codigo );
    $t1 = str_pad( $sd24_i_codigo, 10, 0, STR_PAD_LEFT ); //numero codigo barras

    $pdf->setfont( 'arial', 'b', 8 );
    $pdf->SetFillColor(000); //fundo codbarras
    $pdf->int25( $pdf->getX() + 66,$pdf->getY() + 7, $t1, 5, 0.341 ); //codbarras
    $pdf->setfont( 'arial', 'b', 7 );

    //Linha 1 - 3° Retangulo
    $pdf->rect( $pdf->getX() + 106, $pdf->getY(), 104, 7, "D" );

    $pdf->setfont( 'arial', 'b', 10 );
    $pdf->text( $pdf->getX() + 116 + 10, $pdf->getY() + 5, "FICHA DE ATENDIMENTO AMBULATORIAL" );
    
    $pdf->rect( $pdf->getX() + 106,      $pdf->getY() + 9, 104, 7, "D" );
    $pdf->setfont( 'arial', 'b', 6 );
    $pdf->text( $pdf->getX() + 107, $pdf->getY() + 12, "MOTIVO DO ATENDIMENTO" );

    $sMotivo = empty($s144_c_descr) ? $sd24_v_motivo : $s144_c_descr;
    $pdf->text( $pdf->getX() + 107, $pdf->getY() + 14, $sMotivo );
    ...
    $pdf->rect( $pdf->getX() + 79, $pdf->getY() + 18, 25, 99, "D") ;
    $pdf->text( $pdf->getX() + 80, $pdf->getY() + 21, "PROCEDIMENTO" );
    $pdf->text( $pdf->getX() + 80, $pdf->getY() + 23, "SIA/SUS" );
    $pdf->line( $pdf->getX() + 79, $pdf->getY() + 24, $pdf->getX() + 74 + 30, $pdf->getY() + 24 );

    //Linha 2 - 4ª Retangulo Procedimentos
    $pdf->rect( $pdf->getX() + 106, $pdf->getY() + 18, 104, 99, "D" );
    $pdf->text( $pdf->getX() + 107, $pdf->getY() + 21, "PROFISSIONAL" );
    $pdf->text( $pdf->getX() + 107, $pdf->getY() + 23, "SETOR" );
    $pdf->line( $pdf->getX() + 106, $pdf->getY() + 24, $pdf->getX() + 210, $pdf->getY() + 24 );

    //Linha 2 - 5ª Retangulo Procedimentos
    $pdf->rect( $pdf->getX() + 124, $pdf->getY() + 18, 63, 99, "D" );
    $pdf->text( $pdf->getX() + 125, $pdf->getY() + 21, "TRATAMENTO DESCRIÇÃO" );
    $pdf->text( $pdf->getX() + 125, $pdf->getY() + 23, "DOS PROCEDIMENTOS" );
    ...
    $saturacao = '';
    if (isset($s152_saturacao) && !empty($s152_saturacao)) {
      $saturacao = $s152_saturacao;
    }

    $frequenciaRespiratoria = '';
    if (isset($s152_frequenciarespiratoria) && !empty($s152_frequenciarespiratoria)) {
      $frequenciaRespiratoria = $s152_frequenciarespiratoria;
    }

    $frequenciaCardiaca = '';
    if (isset($s152_frequenciacardiaca) && !empty($s152_frequenciacardiaca)) {
      $frequenciaCardiaca = $s152_frequenciacardiaca;
    }
    $glicemia = '';
    if (isset($s152_i_glicemia) && !empty($s152_i_glicemia)) {
      $momentoColeta = 'Não Específicado';
      if ($s152_i_alimentacaoexameglicemia == 1) {
        $momentoColeta = 'Jejum';
      }
      if ($s152_i_alimentacaoexameglicemia == 2) {
        $momentoColeta = 'Pos-prandial';
      }
      if ($s152_i_alimentacaoexameglicemia == 3) {
        $momentoColeta = 'Pré-prandial';
      }
      $glicemia = "{$s152_i_glicemia} - {$momentoColeta}";
    }
    ...
    if( empty( $aImc ) ) {
      ...
      $detalheImc = ucfirst(strtolower($aImc[1]));
      $sImc = "{$aImc[0]} - {$detalheImc}";
    }
    ...
    $pdf->SetWidths( array( 48, 19, 38, 20, 62, 24 ) );
    $pdf->SetAligns( array( "C", "C", "C", "L", "L", "L" ) );

    $pdf->Row(array('', '', '', $proftriagemavulsa, 
                    "PA.: $sPressao   TEMP.: $sTemperatura   FR.: $frequenciaRespiratoria  FC.: $frequenciaCardiaca  SaO2.: $saturacao  GLICEMIA: $glicemia  PESO: $sPeso   IMC: $sImc ", ''
                   ), 3, false, 3 );
    $pdf->setX( $iXTmp );
    $pdf->line( $pdf->getX() + 58,  $pdf->getY(), $pdf->getX() + 58 + 19,  $pdf->getY() );
    $pdf->line( $pdf->getX() + 79,  $pdf->getY(), $pdf->getX() + 74 + 30,  $pdf->getY() );
    $pdf->line( $pdf->getX() + 106, $pdf->getY(), $pdf->getX() + 210, $pdf->getY() );
    ...
    for( $iCont = 0; $iCont < $iLinhasAgenda[$iX]; $iCont++ ) {
      ...
      $pdf->line( $lar + 106, $pdf->getY(), $lar + 210, $pdf->getY() );
    }
    ...
    for( $x = 0; $x < $linhas_proc[$iX]; $x++ ) {
      ...
      $pdf->setfont( 'arial', '', 7 );
      $pdf->SetWidths( array( 19, 28, 20, 53, 24 ) );
      $pdf->SetAligns( array( "C", "C", "L", "J", "L" ) );
      ...
      $pdf->line( $lar + 106, $pdf->getY(), $lar + 116 + 94, $pdf->getY() );
    }
    ...
    $pdf->setY( $alt + 76 );
    $pdf->setX( $lar );
    $pdf->text( $pdf->getX() + 1, $alt + 76, "CNS: {$s115_c_cartaosus}");
    $pdf->text( $pdf->getX() + 1, $alt + 80, "CPF: {$z01_v_cgccpf}");
    $pdf->setfont( 'arial', 'b', 8 );
    $pdf->setY( $alt + 82 );
    $pdf->setX( $lar );
    $pdf->MultiCell( 48, 3, "Nome: " . $z01_i_numcgs . " - " . $z01_v_nome, 0, "L" );
    $pdf->setY( $alt );
    $pdf->setX( $lar );
    ...
    $pdf->text( $pdf->getX() + 1, $alt + 37, "Endereço: " . substr($z01_v_ender,0,28));
    $pdf->text( $pdf->getX() + 1, $alt + 40, "Número: ".$z01_i_numero);
    $pdf->text( $pdf->getX() + 16, $alt + 40, substr($complemento, 0, 30));
    $pdf->text( $pdf->getX() + 1, $alt + 43, "Bairro: ".substr($z01_v_bairro, 0, 35));
    $pdf->text( $pdf->getX() + 1, $alt + 46, "Município: ".substr($z01_v_munic,0,40));
    $pdf->text( $pdf->getX() + 1, $alt + 49, "UF: ".$est_uf);
    $pdf->setfont( 'arial', 'b', 8 );
    $pdf->text( $pdf->getX() + 1, $alt + 52, "Sexo: ".$sexo[$z01_v_sexo]);
    $pdf->text( $pdf->getX() + 1, $alt + 55, "Idade: ". $idade);
    $pdf->text( $pdf->getX() + 1, $alt + 58, "Data Nasc.: ".$dia."/".$mes."/".$ano );
    $pdf->text( $pdf->getX() + 1, $alt + 61, "Telefone: ".$z01_v_telef);
    $pdf->text( $pdf->getX() + 1, $alt + 64, "Celular: ".$z01_v_telcel);
    $pdf->text( $pdf->getX() + 1 ,$alt + 67, "Família: ".$sd33_v_descricao);
    $pdf->text( $pdf->getX() + 1 ,$alt + 70, "Microárea: ".$sd34_v_descricao);
    ...
    $pdf->setY( $alt + 120 );
    $pdf->setX( $lar + 76 );
    $pdf->MultiCell( $lar + 110, $alt + 3, substr($sd24_t_diagnostico, 0, 120));
