...
{
    if (trim($valores) == '') {
        db_msgbox('Nenhum item informado para devolução.');
        return true;
    }
    ...
    for ($y = 1; $y < count($dados); $y++) {
        ...
        if ($utilizaBnafar && !$hasMaterialFarmacia) {
            $hasMaterialFarmacia = FarmaciaHelper::isMaterialFarmacia($codmatmater);
        }
    ...
    // só faz a movimentação do bnafar se possuir material da farmacia,
    // pois essa função também está sendo usada para vacinas.
