...
class processoProtocolo
{
    ...
    /**
     * forma que o tipo de processo foi incluido
     *
     * @var integer
     */
    private $iTipoInclusaoProcesso;
    ...
    function __construct($iCodProcesso = null)
    {
        if (!empty($iCodProcesso)) {
            ...
            if ($oDaoProtProcesso->numrows > 0) {
                ...
                $this->setCodigoAndamento($oDadosProtProcesso->p58_codandam);
                $this->setTipoInclusaoProcesso($oDadosProtProcesso->p58_tipoprocesso);
                $this->setOrgao($oDadosProtProcesso->p58_orgao);
                $this->setInstituicaoCodigo($oDadosProtProcesso->p58_instit);
                $this->setHora($oDadosProtProcesso->p58_hora);
                $this->setCodigoUsuario($oDadosProtProcesso->p58_id_usuario);
    ...
    /**
     * retorna o tipo de inclusao do processo
     * @return integer $iTipoInclusaoProcesso
     */
    public function getTipoInclusaoProcesso()
    {
        return $this->iTipoInclusaoProcesso;
    }

    /**
     * define o tipo de inclusao do processo
     * @param string $iTipoInclusaoProcesso
     */
    public function setTipoInclusaoProcesso($iTipoInclusaoProcesso)
    {
        $this->iTipoInclusaoProcesso = $iTipoInclusaoProcesso;
    }
    ...
    public function receber($iCodTran = '', $iCodDepto = '', $iIdUsuario = '', $sDespacho = '', $lAlteraProcesso = true)
    {
        ...
        if (trim($iCodTran) == '') {
            ...
            $rsDadosProc = $clProtProcesso->sql_record($clProtProcesso->sql_query_despachos($this->getCodProcesso(),
            ...
            $rsDadosProc = $clProtProcesso->sql_record($clProtProcesso->sql_query_file($this->getCodProcesso(),
                "p58_publico"));
            $oDadosProc = db_utils::fieldsMemory($rsDadosProc, 0);
        }
        ...
        if ($lAlteraProcesso) {
            ...
            $this->setCodigoAndamento($clProcAndam->p61_codandam);
    ...
    public function salvar()
    {
        ...
        if ($this->getCodProcesso() == '') {
            ...
            $oDaoProtProcesso->p58_tipoprocesso = $this->getTipoInclusaoProcesso();
            $oDaoProtProcesso->incluir(null);

            $this->setCodProcesso($oDaoProtProcesso->p58_codproc);
            $this->setNumeroProcesso($oDaoProtProcesso->p58_numero);
        } else {
    ...
    public function getPosicaoAtualAndamentoPadrao()
    {
        ...
        $rsDadosAndam = $clProcTransferProc->sql_record($clProcTransferProc->sql_query_andam(null, null, "*", null,
            $sWhereAndam));
    ...
    public function getProximoDeptoAndamentoPadrao()
    {
        ...
        $sSqlAndamentoPadrao = $oDaoAndamentoPadrao->sql_query_file(null, null, "p53_coddepto", null,
            $sWhereAndamentoPadrao);
    ...
    public function getDepartamentoAtual()
    {
        ...
        $sSqlBuscaAndamento = $oDaoProtProcesso->sql_query_andamento($this->iCodProcesso, $sCampo,
            "p61_codandam desc limit 1");
    ...
    public function getLicitacao()
    {
        ...
        $sSqlBuscaLicitacao = $oDaoProcesso->sql_query_file(null, "l34_liclicita", null,
            "l34_protprocesso = {$this->iCodProcesso}");
    ...
    {
        return UsuarioSistemaRepository::getPorCodigo($this->codigoUsuario);
    }


    public function getProcessoAtendimento()
    {
        return ProcessoOuvidoria::findByProcesso($this->getCodProcesso());
    }

    public function isEletronico()
    {
        return $this->getTipoInclusaoProcesso() == self::ELETRONICO;
    }
}
