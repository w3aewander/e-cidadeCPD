...
switch ($nivelele) {
    ...
    case '5':
        ...
        $cabec = "Programa";
        $ccampod = " o54_descr ";
        $relac = "  inner join orcprograma on orcdotacao.o58_programa = orcprograma.o54_programa and orcprograma.o54_anousu = " . $anousu;
        break;
    ...
    case '6':
        ...
        $cabec = "Proj/Ativ";
        $ccampod = " o55_descr ";
        $relac = " inner join orcprojativ on orcdotacao.o58_projativ = orcprojativ.o55_projativ and orcprojativ.o55_anousu = " . $anousu;
        break;
...
if ($tipo_agrupa == 1) {
    $sql = "create temp table work as
		 ...
		 0::float8 as vlr12
  	  from orcdotacao
	       inner join orcorgao on orcdotacao.o58_orgao = orcorgao.o40_orgao and orcorgao.o40_anousu = " . $anousu . "
	       $relac
    ...
    if ($nivel == '9') {
        ...
        $sql .= "                inner join empempenho on e60_numemp = c75_numemp and e60_anousu=" . $anousu;
        $sql .= "                inner join orcdotacao on o58_coddot = empempenho.e60_coddot  and o58_anousu = e60_anousu";
        $sql .= "                inner join conplanoorcamento on c60_codcon = orcdotacao.o58_codele and c60_anousu=" . $anousu;
...
for ($mes = 1; $mes < 13; $mes++) {
    db_query("begin");
    $result_ultdia = db_query("select fc_ultimodiames(" . $anousu . ",$mes) as ultimodiames");
    db_fieldsmemory($result_ultdia, 0, true);

    if ($nivel != 9) {
        $result_rec = db_dotacaosaldo(
            8,
            2,
            $opcao,
            true,
        ...
        $dtInicial = $anousu . "-$mes-01";
        $dtFinal = $anousu . "-$mes-$ultimodiames";

        $sSqlDesdobramento = "  select  c60_estrut,";
        $sSqlDesdobramento .= "         c60_descr,";
        $sSqlDesdobramento .= "         orcdotacao.o58_orgao,";
        $sSqlDesdobramento .= "         orcdotacao.o58_unidade,";
        $sSqlDesdobramento .= "         e.o56_elemento as o56_elemento,";
        $sSqlDesdobramento .= "         e.o56_descr,";
        ...
        $sSqlDesdobramento .= "         inner join empempenho on e60_numemp = c75_numemp and e60_anousu=" . $anousu . "";
        $sSqlDesdobramento .= "         inner join orcdotacao on o58_coddot = empempenho.e60_coddot  and o58_anousu = e60_anousu";
        $sSqlDesdobramento .= "         inner join conplanoorcamento on c60_codcon = orcdotacao.o58_codele and c60_anousu=" . $anousu . "";
        ...
        $sSqlDesdobramento .= "         inner join orcelemento e on e.o56_codele=conlancamele.c67_codele ";
        $sSqlDesdobramento .= "                                   and e.o56_anousu = o58_anousu ";
        ...
        $result_rec = db_query($sSqlDesdobramento);
    }
    db_query("rollback");
    $valor = 0;
...
$pdf->init(false);
$pdf->addTitulo("RESUMO DA DESPESA - {$xtipo}");
...
if ($formato == 2) {
    $csvheader = array("Orgao", "Descricao", "Janeiro", "Fevereiro",
        "Marco", "Abril", "Maio", "Junho", "Julho",
        "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro", "Total");

    $timestamp = time();
    $fp = fopen('tmp/despesaorgaoelementounidadecsv'.$timestamp.'.csv', 'w');
    $headers = $csvheader;
    fputcsv($fp, $headers, ";");

    $tvls = array_fill(1, 12, 0);
    $ttvls = 0;
    for ($i = 0; $i < pg_num_rows($result); $i++) {
        $row = db_utils::fieldsMemory($result, $i);
        $row->total = $row->vlr1 + $row->vlr2 + $row->vlr3 + $row->vlr4 + $row->vlr5 + $row->vlr6 +
            $row->vlr7 + $row->vlr8 + $row->vlr9 + $row->vlr10 + $row->vlr11 + $row->vlr12;
        $tvls[1] += $row->vlr1;
        $tvls[2]+= $row->vlr2;
        $tvls[3]+= $row->vlr3;
        $tvls[4]+= $row->vlr4;
        ...
        if ($i == pg_num_rows($result) - 1) {
            for ($j = 1; $j <= 12; $j++) {
                $tvls[$j] =  trim(db_formatar($tvls[$j], 'f'));
            }
        }
        for ($j = 1; $j <= 12; $j++) {
            $rowarray['vlr'.$j] =  trim(db_formatar($rowarray['vlr'.$j], 'f'));
        }
        $ttvls += $row->total;
        $rowarray["total"] = trim(db_formatar($rowarray["total"], 'f'));
        fputcsv($fp, $rowarray, ";");
    }
    $totalfinal = array("linhaembranco" => "", "totalgeral" => "Total Geral") + $tvls;
    $totalfinal['ttvls'] = trim(db_formatar($ttvls, 'f'));
    fputcsv($fp, $totalfinal, ";");
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename=despesaorgaoelementounidadecsv'.$timestamp.'.csv');
    readfile('tmp/despesaorgaoelementounidadecsv'.$timestamp.'.csv');
    fclose($fp);
        ...
        for ($i = 0; $i < pg_num_rows($result); $i++) {
            ...
            if ($nivelele == 8) {
                $fonteRecurso = \App\Domain\Financeiro\Orcamento\Models\FonteRecurso::with('recurso')
                    ...
                    ->where('orctiporec_id', $campo)
                    ->first();
                $complemento = str_pad($fonteRecurso->recurso->o15_complemento, 4, '0', STR_PAD_LEFT);
                $campo = "{$fonteRecurso->gestao} - {$complemento}";
                $descr = "{$fonteRecurso->descricao}";
            }
