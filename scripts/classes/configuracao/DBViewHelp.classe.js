...
DBViewHelp.prototype.__buildHtml = function() {
  ...
  var divTitle = CONTEXT.document.createElement('div')
  divTitle.setAttribute('style', 'display: flex; align-items: center; justify-content: space-between;');
  /**
  ...
  var aLinkiWiki = CONTEXT.document.createElement('a');
  aLinkiWiki.setAttribute('target', '_blank');
  aLinkiWiki.setAttribute('style', 'font-size: 18px;')
  aLinkiWiki.innerHTML = 'Base de Conhecimento!';
  
  _this.aLinkiWiki = aLinkiWiki;

  divTitle.appendChild(oTitleHelp);
  divTitle.appendChild(aLinkiWiki);
   ...
   */
  oDivContainer.appendChild(divTitle);
...
/**
 * Adicionado método para adicionar um link para a wiki https://e-cidade.wiki.br/
 */
DBViewHelp.prototype.setLinkWiki = function(linkWiki) {
  var _this = this;
  if(!linkWiki) {
    _this.aLinkiWiki.remove();
    return false;
  }
  _this.aLinkiWiki.setAttribute('href', linkWiki);
}
...
DBViewHelp.prototype.retrieveHelpData = function() {
  ...
  var oRequisicao = {
    ...
    onComplete: function(oAjax) {
      ...
      _this.setLinkWiki(oRetorno.linkWiki);
