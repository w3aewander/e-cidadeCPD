...
class cl_rechumanohoradisp
{
   // cria variaveis de erro
    ...
    public $ed33_i_codigo = 0;
    public $ed33_rechumanoescola = 0;
    public $ed33_i_diasemana = 0;
    public $ed33_i_periodo = 0;
    public $ed33_ativo = 'f';
    public $ed33_tipohoratrabalho = 0;
    public $ed33_horaatividade = 'f';
    public $ed33_rechumanoativ = 0;
   // cria propriedade com as variaveis do arquivo
    public $campos = "
                 ...
                 ed33_rechumanoativ = int4 = Função
                 ";
    ...
    public function atualizacampos($exclusao = false)
    {
     if($exclusao==false){
       ...
       $this->ed33_rechumanoativ = ($this->ed33_rechumanoativ == ""?@$GLOBALS["HTTP_POST_VARS"]["ed33_rechumanoativ"]:$this->ed33_rechumanoativ);
     }else{
       $this->ed33_i_codigo = ($this->ed33_i_codigo == ""?@$GLOBALS["HTTP_POST_VARS"]["ed33_i_codigo"]:$this->ed33_i_codigo);
     }
   }

    public function incluir($ed33_i_codigo)
    {
      $this->atualizacampos();
     ...
     if($this->ed33_rechumanoativ == null ){
       $this->erro_sql = " Campo Função não informado.";
       $this->erro_campo = "ed33_rechumanoativ";
       $this->erro_banco = "";
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,1010091,1013398,'','".AddSlashes(pg_result($resaco,0,'ed33_rechumanoativ'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   }
    ...
    public function alterar($ed33_i_codigo=null)
    {
     ...
     if(trim($this->ed33_rechumanoativ)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ed33_rechumanoativ"])){
       $sql  .= $virgula." ed33_rechumanoativ = $this->ed33_rechumanoativ ";
       $virgula = ",";
       if(trim($this->ed33_rechumanoativ) == null ){
         $this->erro_sql = " Campo Função não informado.";
         $this->erro_campo = "ed33_rechumanoativ";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["ed33_rechumanoativ"]) || $this->ed33_rechumanoativ != "")
             $resac = db_query("insert into db_acount values($acount,1010091,1013398,'".AddSlashes(pg_result($resaco,$conresaco,'ed33_rechumanoativ'))."','$this->ed33_rechumanoativ',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,1010091,1013398,'','".AddSlashes(pg_result($resaco,$iresaco,'ed33_rechumanoativ'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
   ...
   function sql_query_disponivel_periodo ($ed33_i_codigo = null, $campos="*", $ordem = null, $dbwhere = "") {
    ...
    $sql .= "       inner join rechumano        on rechumano.ed20_i_codigo                = rechumanoescola.ed75_i_rechumano       ";
  	...
  	$sql .= " inner join rechumanoativ    on rechumanoativ.ed22_i_codigo   = ed33_rechumanoativ ";
  ...
  function sql_query_disponibilidade ( $ed33_i_codigo = null, $campos = "*", $ordem = null, $dbwhere = "", $apontaRecHumanoAtiv = false, $apontaAgenda = false) {
    $sql = "select ";
    ...
    $sql .= " from rechumanohoradisp ";
    $sql .= $apontaRecHumanoAtiv ? "inner join rechumanoativ on ed22_i_codigo = ed33_rechumanoativ" : "";
    $sql .= $apontaAgenda ? " join agendaatividade on ed129_rechumanoativ = rechumanoativ.ed22_i_codigo" : "";
    $sql .= "      inner join periodoescola     on periodoescola.ed17_i_codigo        = rechumanohoradisp.ed33_i_periodo";
    $sql .= "      inner join diasemana         on diasemana.ed32_i_codigo            = rechumanohoradisp.ed33_i_diasemana";
    $sql .= "      inner join rechumanoescola   on rechumanoescola.ed75_i_codigo      = rechumanohoradisp.ed33_rechumanoescola";
