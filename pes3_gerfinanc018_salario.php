...
$sSql = <<<SQL
  SELECT DISTINCT
      ...
      CASE WHEN rh143_tipoevento  =  1     THEN 'Provento'
           ...
           ELSE 'Base'
      END              AS tipo,
      CASE WHEN rh143_tipoevento IN (1, 3, 4, 5) THEN rh143_valor
           ELSE  0
      END              AS provento,
    ...
    WHERE rh141_anousu      = {$iAnoFolha}
      ...
      AND rh143_tipoevento NOT IN (3, 4, 5)
...
$sSql = <<<SQL
  SELECT DISTINCT
      ...
      CASE WHEN r14_pd  =  1     THEN 'Provento'
           ...
           ELSE 'Base'
      END              AS tipo,
      CASE WHEN r14_pd IN (1, 3, 4, 5) THEN r14_valor
           ELSE  0
      END              AS provento,
...
<head>
  ...
  <?php
    db_app::load('DBHint.widget.js, prototype.js');
  ?>
</head>
<body>
      ...
      <fieldset id="folha-<?= $mNumero ?>">
        ...
        <table width="100%" id="tabela-calculos" cellspacing="0">
          ...
          <?php foreach ($aRegistroFinanceiro as $oRegistroFinanceiro) { 
              $dados = (object) [
                'matricula'  => $matricula,
                'rubrica'    => $oRegistroFinanceiro->rubrica,
                'anousu'     => $ano,
                'mesusu'     => $mes,
                'instit'     => $iInstituicao,
                'tabela'     => $arquivo
            ...
            ?>
            <tr id="linha-rubrica-<?=$oRegistroFinanceiro->rubrica?>">
              <td>1</td>
              <td>
              ...
              <td><?= $oRegistroFinanceiro->descricao . ($dadosObservacao->observacao ? '<span class="observacao-rubrica"> *</span>' : '') ?></td>
