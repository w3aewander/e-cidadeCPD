...
$oPdf  = new PDF();
$oPdf->addTitulo("Relatório Cadastro de Retenções");
$oPdf->AliasNbPages();
...
for ($iInd = 0; $iInd < $iNumrows; $iInd++) {
    ...
    if (isset($aGrupo[$oDados->e01_sequencial])) {
        if (isset($aGrupo[$oDados->e01_sequencial]->aTipo[$oDados->e32_sequencial])) {
            if (!isset($aGrupo[$oDados->e01_sequencial]
                            ->aTipo[$oDados->e32_sequencial]
                            ->aRetencao[$oDados->e21_sequencial])) {
                $aGrupo[$oDados->e01_sequencial]
                    ->aTipo[$oDados->e32_sequencial]
                    ->aRetencao[$oDados->e21_sequencial] = $oRetencao;
            }
        } else {
       ...
       */
        ...
        foreach ($oTipoCalculo->aRetencao as $oRetencaoDados) {
           ...
           */
            ...
            if ($oPdf->gety() > $oPdf->geth() - 30 || $lTroca != false) {
