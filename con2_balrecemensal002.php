...
$iInstituicao = explode("-", $oGet->db_selinstit);
$sInstituicao = implode(", ", $iInstituicao);

$saida = "pdf";
if (isset($oGet->formato) && $oGet->formato == "2") {
    $saida = "csv";
}
...
if ($saida == "csv") {
    $timestamp = time();
    $fp = fopen('tmp/despesamensalcsv' . $timestamp . '.csv', 'w');

    $csvheader[] = "RECEITA";
    $csvheader[] = "DESCRIÇÃO";
    $csvheader[] = "REC";
    foreach ($aMesesPeriodo as $sPeriodo) {
        $csvheader[] = strtoupper($sPeriodo);
    }

    if (isset($oGet->periodo) && ($oGet->periodo == 2 || $oGet->periodo == 3)) {
        $csvheader[] = "TOTAL PERÍODO";
        $csvheader[] = "TOTAL GERAL";
    }
    ...
    foreach ($oReceitaSaldoMes as $oReceita) {
        ...
        $linha[] = db_formatar($oReceita->o57_fonte, 'receita');
        $linha[] = $oReceita->o57_descr;
        $linha[] = $oReceita->o70_codrec;
        foreach ($aMesesPeriodo as $sPeriodo) {
            $linha[] = trim(db_formatar($oReceita->$sPeriodo, 'f'));
        }
        if (isset($oGet->periodo) && ($oGet->periodo == 2 || $oGet->periodo == 3)) {
            $linha[] = trim(db_formatar($oReceita->nTotalPeriodo, 'f'));
            $linha[] = trim(db_formatar($oReceita->nTotalGeral, 'f'));
        }

        fputcsv($fp, $linha, ";");
    }
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename=despesamensalcsv' . $timestamp . '.csv');
    readfile('tmp/despesamensalcsv' . $timestamp . '.csv');
    fclose($fp);
    exit;
}
