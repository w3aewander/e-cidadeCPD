...
switch ($oParam->exec) {
  ...
  break;
    ...
    case "CarregarItemAbertura2" :

    try {

      $oSolicita =  $_SESSION["oSolicita"];
      $oDaoSolicitacao = db_utils::getDao("solicita");
      $oDaoSolicitacaoItem = db_utils::getDao("solicitem");
      ...
      unset($oPreRegistroPreco);
      unset($aPreRegistroPreco2);
        ...
        pg_fetch_all(
          db_query(
            "select
              ...
              p.idusuarioinc
            from
              plugins.presolicitacao as p
            where
              p.pc10_numero = {$oSolicita->getCodigoAbertura()}
              and p.pc10_estimativa is null
      ...
      if (empty($oPreRegistroPreco)) {
        throw new BusinessException(
          'A Abertura do Registro de Preço ' . $oSolicita->getCodigoAbertura() . ' já está cadastrada no sistema.'
        );
      }

      foreach($oPreRegistroPreco as $i => $k){
        //oDaoSolicitacao
        $oDaoSolicitacao->pc10_data            = date('Y-m-d');
        $oDaoSolicitacao->pc10_resumo          = $i.':'.$oSolicita->getResumo();
        $oDaoSolicitacao->pc10_depto           = $k['coddepto'];
        $oDaoSolicitacao->pc10_instit          = $k['instit'];
        $oDaoSolicitacao->pc10_correto         = 'true';
        $oDaoSolicitacao->pc10_login           = $k['idusuarioinc'];
        $oDaoSolicitacao->pc10_solicitacaotipo = '4';

        $oDaoSolicitacao->incluir(null);

        //aPreRegistroPreco2

        $aPreRegistroPreco2[$i] = pg_fetch_all(
          db_query("select
                    '{$oDaoSolicitacao->pc10_numero}' as pcnumero,
                    idpresolicitacao,
                    pc01_codmater,
                    quantidade,
                    coddepto,
                    ...
                    pc11_numero
                  from
                    plugins.vw_preregistropreco as p
                  where
                    p.pc10_numero = {$oSolicita->getCodigoAbertura()}
                    and p.coddepto = {$k['coddepto']}")
        );
      ...
      //@update
      foreach ($aPreRegistroPreco2 as $y => $v) {
          foreach ($v as $yy => $vv) {

            //oDaoSolicitacaoItem
            $oDaoSolicitacaoItem->pc11_numero = $aPreRegistroPreco2[$y][$yy]['pcnumero'];
            $oDaoSolicitacaoItem->pc11_seq = $yy  + 1;
            $oDaoSolicitacaoItem->pc11_quant = $aPreRegistroPreco2[$y][$yy]['quantidade'];
            $oDaoSolicitacaoItem->pc11_liberado = 'true';
            $oDaoSolicitacaoItem->incluir(null);

            //pc10_estimativa
            pg_query("update
              plugins.presolicitacao
            set
              pc10_estimativa = {$oDaoSolicitacaoItem->pc11_codigo}
            where
              pc01_codmater = {$aPreRegistroPreco2[$y][$yy]['pc01_codmater']}
              and pc10_numero = {$oSolicita->getCodigoAbertura()}
              and datacancelamento is null
              and pc10_compilacao is null");

            //oDaosolicitemPcMater
            $oDaoSolicitemPcMater->incluir(
              $aPreRegistroPreco2[$y][$yy]['pc01_codmater'],
              $oDaoSolicitacaoItem->pc11_codigo
            );

            //oDaoSolicitemVinculo
            $oDaoSolicitemVinculo->pc55_solicitempai = $aPreRegistroPreco2[$y][$yy]['pc11_numero'];
            $oDaoSolicitemVinculo->pc55_solicitemfilho = $oDaoSolicitacaoItem->pc11_codigo;
            $oDaoSolicitemVinculo->incluir(null);

          }

          //oDaoSolicitaVinculo
          $oDaoSolicitaVinculo->pc53_solicitapai = $oSolicita->getCodigoAbertura();
          $oDaoSolicitaVinculo->pc53_solicitafilho = $oDaoSolicitacaoItem->pc11_numero;
          $oDaoSolicitaVinculo->incluir(null);
        }

        //retornar itens para visualizar na table
        $aitens = $oSolicita->getItens();
  ...
  case "salvarItensValor" :
    ...
    try {
      ...
      $result = db_query("select p.* from plugins.vw_preregistropreco p where p.pc01_codmater = {$oItem->getCodigoMaterial()} and p.pc10_numero = {$oSolicita->getCodigoAbertura()} limit 1");

      $pitens = pg_fetch_all($result);
      ...
      if($pitens){
        ...
        foreach ($pitens as $i) {
          ...
          $oItem->setPagamento(utf8_decode(db_stdClass::db_stripTagsJson($oParam->sPgto)));
          $oItem->setPrazos(utf8_decode(db_stdClass::db_stripTagsJson($oParam->sPrazo)));
          $oItem->setQuantidade($i['quantidade']);
          $oItem->save();
          ...
          @pg_query("update
            plugins.presolicitacao
          set
            pc10_estimativa = {$oSolicita->getCodigoSolicitacao()}
          where
            pc01_codmater = {$oItem->getCodigoMaterial()}
            and pc10_numero = {$oSolicita->getCodigoAbertura()}
            and datacancelamento is null
            and pc10_compilacao is null");
