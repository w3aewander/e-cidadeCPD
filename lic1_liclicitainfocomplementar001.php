...
if (empty($licitacao)) {
    db_redireciona("db_erros.php?fechar=true&db_erro=Licitacão não informada.");
}
  ...
  <form name="form1" method="post">
    ...
    <fieldset>
      ...
      <fieldset class="separator">
        <legend>Regime Diferenciado de Contrata&ccedil;&atilde;o</legend>
      ...
      <fieldset class="separator">
        <legend>Dados da Ades&atilde;o &agrave; Ata de Registro de Pre&ccedil;o</legend>
...
<script type="text/javascript">
  ...
  (function(exports) {
    ...
    oAtributos.carregarAtributos(1260, function() {
      ...
      for(var sCampo in aCampos) {
        ...
        if(sCampo == "inversaofases" || sCampo == "orcamentosigiloso") {
      ...
      $("cnpjorgaogerenciador").maxLength = "14";
        ...
        const selectTipoLicitacao = document.getElementById('tipolicitacao');
        const selectLicitacaoCompartilhada = document.getElementById('licitacao_compartilhada');

        selectTipoLicitacao.addEventListener('change', () => {
            if (selectTipoLicitacao.value === 'RPO') {
                selectLicitacaoCompartilhada.readonly = true;
                selectLicitacaoCompartilhada.value = 'N';
            } else {
                selectLicitacaoCompartilhada.readonly = false;
            }
        });
      ...
      $("codigofundamentacao").addEventListener("change", function() {
        ...
        if(this.value == "OUTD" || this.value == "OUTI" || this.value == "OUTC" || this.value == "OUT") {
  ...
  /**
   * Funcao que busca as fundamentacoes permitidas para a modalidade escolhida.
   */
  function atualizarFundamentacao() {

    var aFundamentacao = [];

    for(var i = 0; i < $('codigofundamentacao').length; i++) {
    ...
    var oRequest = new AjaxRequest('lic4_liclicitainfocomplementar.RPC.php', oParametros, function(oRetorno, lErro) {
  ...
  /**
   * Funcao que remove todas as fundamentacoes do select,
   * exceto as que sao passadas por parametro
   */
  function removerFundamentacao(aFundamentacoes) {

    var valorSelecionado = $F('codigofundamentacao');

    for(var i = 0; i < $('codigofundamentacao').length; i++) {
      console.log($('codigofundamentacao').options[i].value);
    ...
    $('codigofundamentacao').value = valorSelecionado;

    /**
     * Desativar os seguintes campos e informar valor vazio
     */
    $('datainiciovigenciacredenciamento').parentElement.parentElement.parentElement.hide();
    $('datafimvigenciacredenciamento').parentElement.parentElement.parentElement.hide();
    $('recebeinscricaoperiodovigencia').parentElement.parentElement.hide();

    $('datainiciovigenciacredenciamento').value = '';
    $('datafimvigenciacredenciamento').value = '';
    $('recebeinscricaoperiodovigencia').value = '';
  }
