...
try {
    ...
    $parametros = JSON::create()->parse(str_replace('\\', '', $post->json));
    ...
    switch ($parametros->exec) {
        ...
        case 'getAlunoComNotaExterna':
            $oTurma = EducacaoSessionManager::carregarTurma($parametros->iTurma);
            $oEtapa = EducacaoSessionManager::carregarEtapa($parametros->iEtapa);
            $oRegencia = RegenciaRepository::getRegenciaByCodigo($parametros->iRegencia);
            ...
            foreach ($oProcedimentoAvaliacao->getElementos() as $oAvaliacao) {
                ...
                if ($oAvaliacao->getOrdemSequencia() == $parametros->iPeriodo) {
            ...
            foreach ($aMatriculas as $oMatricula) {
                ...
                $oAvaliacao = $oMatricula->getDiarioDeClasse()->getDisciplinasPorRegenciaPeriodo($oRegencia,
                    $oPeriodoAvaliacao);

                if (($oAvaliacao->getElementoAvaliacao()->getOrdemSequencia() != $parametros->iPeriodo) ||
                 ...
                 */
                $oDadosAluno->sNomeAluno = $oMatricula->getAluno()->getNome();
                ...
                $oDadosAluno->sFormaAvaliacao = $oProcedimentoAvaliacao->getFormaAvaliacao()->getTipo();
                ...
                $oAvaliacaoOrigem->sFormaAvaliacao = $oAvaliacao->getAproveitamentoOrigem()->getElementoAvaliacao()->getFormaDeAvaliacao()->getTipo();
                $oAvaliacaoOrigem->iFormaAvaliacao = $oAvaliacao->getAproveitamentoOrigem()->getElementoAvaliacao()->getFormaDeAvaliacao()->getCodigo();
                ...
                if ($oAvaliacao->getAproveitamentoOrigem()->getElementoAvaliacao()->getFormaDeAvaliacao()->getTipo() == 'NOTA') {
                    ...
                    $oAvaliacaoOrigem->nMaiorValor = $oAvaliacao->getAproveitamentoOrigem()->getElementoAvaliacao()->getFormaDeAvaliacao()->getMaiorValor();

                    $mNotaOrigem = ArredondamentoNota::formatar($mNotaOrigem, $iAno);
                }

                if ($oAvaliacao->getAproveitamentoOrigem()->getElementoAvaliacao()->getFormaDeAvaliacao()->getTipo() == 'NIVEL') {
                    $oAvaliacaoOrigem->aConceito = $oAvaliacao->getAproveitamentoOrigem()->getElementoAvaliacao()->getFormaDeAvaliacao()->getConceitos();
                }

                $oAvaliacaoOrigem->mNotaOrigem = $mNotaOrigem;
                $oAvaliacaoOrigem->iEscola = $oAvaliacao->getAproveitamentoOrigem()->getEscola()->getCodigo();
                $oAvaliacaoOrigem->sEscola = $oAvaliacao->getAproveitamentoOrigem()->getEscola()->getNome();
                $oAvaliacaoOrigem->sMunicipio = $oAvaliacao->getAproveitamentoOrigem()->getEscola()->getMunicipio();
                $oAvaliacaoOrigem->sEstado = $oAvaliacao->getAproveitamentoOrigem()->getEscola()->getUf();

                $oDadosAluno->oAvaliacaoOrigem = $oAvaliacaoOrigem;
                $aAlunos[] = $oDadosAluno;
            }

            $resposta->aAlunos = $aAlunos;

            break;

        case 'salvarConversao':
            if (isset($parametros->iMatricula) && isset($parametros->iDiarioAvaliacao)) {
                $oMatricula = EducacaoSessionManager::carregarMatricula($parametros->iMatricula);
                $oRegencia = RegenciaRepository::getRegenciaByCodigo($parametros->iRegencia);
                ...
                foreach ($oDiarioAvaliacao->getAvaliacoes() as $oAvaliacaoAproveitamento) {
                    if ($oAvaliacaoAproveitamento->getElementoAvaliacao() instanceof AvaliacaoPeriodica &&
                        $oAvaliacaoAproveitamento->getElementoAvaliacao()->getOrdemSequencia() == $parametros->iPeriodo
                    ) {
                        $oValorAproveitamento = $oAvaliacaoAproveitamento->getValorAproveitamento();
                        $oValorAproveitamento->setAproveitamento($parametros->sAproveitamento);

                        if ($oValorAproveitamento instanceof ValorAproveitamentoNivel) {
                            $oValorAproveitamento->setOrdem($parametros->iOrdem);
                        }
    ...
    $resposta->erro = true;
    $resposta->status = 2;
    $resposta->message = $excecao->getMessage();
}
