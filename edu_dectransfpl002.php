...
if ($diretor != "") {
  ...
  $arr_assinatura = explode("|",utf8_decode($diretor));
...
for ($x=0;$x<$linhas;$x++) {
  ...
  if($sEnsino == "EDUCAÇÃO INFANTIL CRECHE - EIC") /*&& $sEtapa == "MATERNAL III")*/{
  ...
  }elseif($sEnsino == "EDUCAÇÃO INFANTIL PRÉ-ESCOLA - EI" /*&& $sEtapa == "1º PERÍODO"*/){     
    ...
    $pdf->cell(6, 15, "", "L", 0, "R", 0);
    $pdf->setXY(16,$altY);
  }elseif(($sEnsino == "ENSINO FUNDAMENTAL - EF" && ($sEtapa == "1º ANO" || $sEtapa == "2º ANO" || $sEtapa == "3º ANO" || $sEtapa == "4º ANO" || $sEtapa == "5º ANO"))){
    $sSqlDiarioAvaliacao    = $cldiarioavaliacao->sql_query_guia("", "*", "ed41_i_sequencia ASC", "ed60_i_codigo=$codigomatricula");
    $result_diarioavaliacao = $cldiarioavaliacao->sql_record($sSqlDiarioAvaliacao);
    $zzz = pg_fetch_all($result_diarioavaliacao);
    $xed60_i_codigo = $zzz[0]["ed60_i_codigo"];
