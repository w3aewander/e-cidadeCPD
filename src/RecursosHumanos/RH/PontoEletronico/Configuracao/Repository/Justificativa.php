...
class Justificativa {
   ...
   * @throws \DBException
   */
  public function getJustificativaPorTipoAssentamento($iTipoAssentamento, $codigoDepartamento = null) {

    $oDaoJustificativa = new \cl_pontoeletronicojustificativatipoasse();
    $where = "";
    if (!empty($codigoDepartamento)) {
        $where = "AND exists (select 1 from tipoassedb_depart where rh184_db_depart = {$codigoDepartamento} and rh184_tipoasse = h12_codigo) ";
    }
    ...
    $sSqlJustificativa = $oDaoJustificativa->sql_query(
      ...
      null,
      "h12_tipo = 'S' and rh205_tipoasse = {$iTipoAssentamento} " . $where
    );
  ...
  public function getJustificativaPorTipoAssentamentoAfastamento($mTipoAssentamento) {
     ...
     * @throws \DBException
     */
  public function getTipoAssentamentosConfigurados()
  {
      $codigoInstituicao = \InstituicaoRepository::getInstituicaoSessao()->getCodigo();
      $sql= $this->oDao->sql_query_tipoasse_justificativa('rh205_tipoasse', "rh194_instituicao = {$codigoInstituicao}");
