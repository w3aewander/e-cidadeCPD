...
function buscaAcompanhantes($iPedido)
{
    ...
    $sCampos .= ', z01_v_telcel as celular, z01_v_telef  as telefone, z01_v_ident as identidade ';
...
function imprimeModeloAjudaCusto($aPedidos, $oPdf, $oGet)
{
    ...
    foreach ($aPedidos as $oPrestadora) {
        ...
        foreach ($oPrestadora->aPacientes as $oPaciente) {
            ...
            if ($lPrimeiro || $oPdf->getY() >= ($oPdf->getH() - 20)) {
    ...
    if ($iLinhasAjudaDestino + $oPdf->getY() >= ($oPdf->getH() - 20)) {
        $oPdf->addPage();
    }
        ...
        foreach ($oPrestadora->aPacientes as $oPaciente) {
            ...
            if (count($oPaciente->aAcompanhante) > 0) {
                foreach ($oPaciente->aAcompanhante as $oAcompanhante) {
                    ...
                    $aContato = [];
                    if (!empty($oAcompanhante->telefone)) {
                        $aContato[] = $oAcompanhante->telefone;
                    }
                    if (!empty($oAcompanhante->celular)) {
                        $aContato[] = $oAcompanhante->celular;
                    }
...
for ($i = 0; $i < $iLinhas; $i++) {
    ...
    } elseif (!empty($oDadosPedido->valor_ajuda)) {
        $lRetornouRegistro = true;
    }
}
...
$oPdf->addTitulo('', 1);
$dataInicial = $oDataInicial->convertTo(DBDate::DATA_PTBR);
$dataFinal = $oDataFinal->convertTo(DBDate::DATA_PTBR);
$oPdf->addTitulo("Período.: {$dataInicial} a {$dataFinal}", 2);

$oPdf->addTitulo("Destino.: " . ($oGet->destino != '' ? $oGet->sDestino : 'GERAL'), 4);

if ($oGet->tipo == 0) {
    $oPdf->addTitulo('Relatório Diário de Viagens', 1);
    $oPdf->addTitulo('Tipo....: VIAGENS', 3);

    imprimeModeloViagem($aPedidos, $oPdf);
} else {
    $oPdf->addTitulo('Relatório de Saída', 1);
    $titulo = "Tipo....: ";
    $titulo .= $oGet->tipo == 1 ? "SAÍDAS COM TOTAIS" : "SAÍDAS SEM TOTAIS";
    $oPdf->addTitulo($titulo, 3);
