...
class AcordoPosicao
{
    ...
    function __construct($iCodigoPosicao = null)
    {
        if (!empty($iCodigoPosicao)) {
            ...
            $this->setValorAcordo($oDadosPosicao->ac16_valor);
        }
    }
    ...
    /**
     * define o valor do acordo
     * @param number $nValorAcordo
     * @return $this
     */
    public function setValorAcordo($nValorAcordo)
    {
        $this->nValorAcordo = $nValorAcordo;

        return $this;
    }

    /**
     * retorna o valor do acordo
     * @return number
     */
    public function getValorAcordo()
    {
        return $this->nValorAcordo;
    }
    ...
    public function adicionarItemDeLicitacao($iLicitem, $oItemAcordo = null, $lReservarSaldo = true)
    {
        ...
        if (
          $oLicitacao->isChamamentoPublicoComCredenciamento()
          || $oLicitacao->isPNAE()
          || $oLicitacao->isCHP()
        ) {
        ...
        if (
            $oDaoLiclicitem->numrows >= 1 &&
            ($oLicitacao->isPNAE() || $oLicitacao->isChamamentoPublicoComCredenciamento())
            ||  $oDaoLiclicitem->numrows == 1 || (isParaiba() && $oLicitacao->isCHP())
        ) {
             ...
             */
            $oDaoDotacoesItem = new cl_pcdotac();
    ...
    {
        $oDaoLiclicitem = new cl_pcprocitem();
        ...
        if ($oDaoLiclicitem->numrows == 1) {
            ...
            foreach ($aDotacoes as $oDotacaoItem) {
                ...
                if ($oDotacaoItem->o80_valor > 0) {
                    $valorQuantidade = ($oItemLicitacao->pc11_vlrun * $oItemLicitacao->pc11_quant);
                    $calculo = ($oDotacaoItem->pc13_valor / $valorQuantidade);
                    $nDiferenca = ($calculo * $oItem->getValorTotal()) - $oDotacaoItem->o80_valor;
                }
    ...
    public function save()
    {
        ...
        if ($this->getVigenciaInicial() != "" && $this->getVigenciaFinal()) {
            ...
            $oDaoAcordoVigencia->ac18_datainicio = implode("-",
              array_reverse(explode("/", $this->getVigenciaInicial())));
    ...
    static public function calculaPeriodosComerciais($dtDataInicial, $dtDataFinal)
    {
        ...
        for ($iPeriodo = 0; $iPeriodo < $iTotalPeriodos; $iPeriodo++) {
            ...
            $sDataInicial = date("Y-m-d",
              mktime(0, 0, 0, $iMesInicialPeriodo, $iDiaInicialPeriodo, $iAnoInicialPeriodo));
            ...
            $lAnoPeriodoBisexto = (bool)date("L",
              mktime(0, 0, 0, $iMesInicialPeriodo, $iDiaInicialPeriodo, $iAnoInicialPeriodo));;
            ...
            $sDataFinal = date("d-m-Y",
              mktime(0, 0, 0, $iMesInicialPeriodo, $iDiaInicialPeriodo + $iDiasSomar, $iAnoInicialPeriodo));
