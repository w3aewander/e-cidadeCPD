...
class M29926GeraDicionario extends Migration
{
    /**
     * Run the migrations.
     *
...
BEGIN
    ...
    FROM tmp_syscampodep
             LEFT JOIN db_syscampodep ON db_syscampodep.codcam = tmp_syscampodep.codcam
    WHERE db_syscampodep.codcam IS NULL;

    INSERT INTO db_syscampodef
    SELECT tmp_syscampodef.*
    FROM tmp_syscampodef
             LEFT JOIN db_syscampodef ON db_syscampodef.codcam = tmp_syscampodef.codcam
    ...
    FROM tmp_sysforkey
             LEFT JOIN db_sysforkey ON db_sysforkey.codcam = tmp_sysforkey.codcam
    ...
    SELECT tmp_acount.*
    FROM tmp_acount
             LEFT JOIN db_acount ON db_acount.codarq = tmp_acount.codarq
    WHERE db_acount.codarq IS NULL;

    INSERT INTO db_sysclasses
    SELECT tmp_sysclasses.*
    FROM tmp_sysclasses
             LEFT JOIN db_sysclasses ON db_sysclasses.codarq = tmp_sysclasses.codarq
    WHERE db_sysclasses.codarq IS NULL;

    INSERT INTO iptutabelas
    SELECT tmp_iptutabelas.*
    FROM tmp_iptutabelas
             LEFT JOIN iptutabelas ON iptutabelas.j121_sequencial = tmp_iptutabelas.j121_sequencial
    WHERE iptutabelas.j121_sequencial IS NULL;

    INSERT INTO db_processa
    SELECT tmp_processa.*
    FROM tmp_processa
             LEFT JOIN db_processa ON db_processa.codarq = tmp_processa.codarq
    ...
    FROM tmp_iptutabelasconfigcampochave
             LEFT JOIN iptutabelasconfigcampochave
    ...
    FROM tmp_iptutabelasconfigcampocorrecao
             LEFT JOIN iptutabelasconfigcampocorrecao
                       ON iptutabelasconfigcampocorrecao.j123_sequencial =
                          tmp_iptutabelasconfigcampocorrecao.j123_sequencial
    ...
    FROM tmp_iptutabelasconfigvirada
             LEFT JOIN iptutabelasconfigvirada
