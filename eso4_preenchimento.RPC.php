...
try {
    switch ($oParam->exec) {
        case 'buscarAvaliacao':
            ...
            $formularioId = $configuracao->getFormulario($tipo);
        ...
        case 'salvarAvaliacao':
            ...
            $sTipoFormulario = null;

            if ($oParam->lSemVinculo == 'true') {
                $sTipoFormulario = Tipo::TRABALHADOR_SEM_VINCULO;
            }
            ...
            if (!empty($oParam->empregador)) {
                $oAvaliacaoESocial->setCgm(CgmRepository::getByCodigo($oParam->empregador));
            }

            $oAvaliacaoESocial->salvar($iCodigoGrupoPerguntas, $sTipoFormulario);
