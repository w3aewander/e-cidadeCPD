...
try {
  ...
  switch ($oParam->exec) {
    ...
    case 'getEventosDoAcordo':
      ...
      $oRetorno->eventos = array_map(function(AcordoEvento $oEventoAcordo) use ($aTiposPublicacoesDescricao) {
        ...
        $oEvento->descricao_publicacao     = '';
        if ($oEventoAcordo->getVeiculoComunicacao() !== '') {
            $oEvento->descricao_publicacao     = urlencode($aTiposPublicacoesDescricao[$oEventoAcordo->getVeiculoComunicacao()]);
        }
        return $oEvento;
      },$oAcordo->getEventos());
      break;
    ...
    case 'salvar':
      ...
      if (array_key_exists($oParam->iTipoVeiculo, $aTiposPublicacoesObrigatorios)
          && empty($oParam->sDescricaoVeiculo)) {
      ...
      $oRetorno->data = $oEventoAcordo->getData()->getDate(DBDate::DATA_PTBR);;
      $oRetorno->tipo_evento = $oEventoAcordo->getTipoEvento();
      $oRetorno->tipo_veiculo_comunicacao = $oEventoAcordo->getVeiculoComunicacao();
      $oRetorno->descricao_veiculo = urlencode($oEventoAcordo->getDescricaoVeiculo());
      $oRetorno->descricao_publicacao = urlencode($aTiposPublicacoesDescricao[$oEventoAcordo->getVeiculoComunicacao()]);
