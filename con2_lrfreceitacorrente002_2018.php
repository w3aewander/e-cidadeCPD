...
use ECidade\Configuracao\Opcao\Opcao as Opcao;
...
$opcaoRelatorio = Opcao::get('modelo_rreo_anexo3', db_getsession("DB_anousu"));
$versaoIN13 = $opcaoRelatorio == 'in13';
$totalLinhasRelatorio = 23;
if ($versaoIN13) {
    $totalLinhasRelatorio = 32;
}

for ($iLinha = 1; $iLinha <= $totalLinhasRelatorio; $iLinha++) {
...
$rec[21][0] = "    Contrib. para o Plano de Previdência do Servidor";
$rec[22][0] = "    Compensação Financ. entre Regimes Previd.";
$rec[23][0] = "    Deduções da Receitas para Formação do FUNDEB";
$rec[24][0] = "    IRRF sobre Rendimento do Trabalho";
$rec[25][0] = "    Contrib. para Custeio dos Sistemas Previdenciário e Assist.";
$rec[26][0] = "    Compensação Previdenciária ao RPPS";
$rec[27][0] = "    Receitas do RPPS - Remunerações e Outras Receitas";
$rec[28][0] = "    Receitas do Fundo de Assistência Social dos Servidores";
$rec[29][0] = "    Receitas do Fundo de Assistência à Saúde do Servidor";
$rec[30][0] = "    Outras Contribuições Sociais";
$rec[31][0] = "  IV - (+) PERDA PARA O FUNDEB";
$rec[32][0] = " Transferências Advindas de Emendas Parlamentares Individuais (§ 13 do artigo 166 da CF)";
...
$dt2 = ($anousu - 1) . '-12-31';
/**
 * iniciamos o total das linhas
 */
for ($i = 1; $i <= $totalLinhasRelatorio; $i++) {

    foreach (range(1, 13) as $mes) {
        $rec[$i][$mes] = 0;
        $recB[$i][$mes] = 0;
    }
}
...
for ($i = 0; $i < $clreceita_saldo_mes->numrows; $i++) {
    ...
    for ($p = 1; $p <= $totalLinhasRelatorio; $p++) {
        ...
        foreach ($oParametro->contas as $oEstrutural) {
            ...
            if ($oRetornoVerificacao->match) {
                ...
                if ($p <= 20) {
                    ...
                    /**
                     * A última linha nao entra no calculo da deducao, quando a versao for IN13
                     */
                    if ($versaoIN13 && $p >= 31) {
                        continue;
                    }
                    ...
                    if (db_conplano_grupo($anousu - 1, substr($oReceita->o57_fonte, 0, 3) . "%", 9001) == true) {  // 497 e 917
                        ...
                        if (!isset($Trec[1][2])) $Trec[1][2] = ($oReceita->fevereiro); else {
                            $Trec[1][2] += ($oReceita->fevereiro);
                        }
                        if (!isset($Trec[1][3])) $Trec[1][3] = ($oReceita->marco); else {
                            $Trec[1][3] += ($oReceita->marco);
                        }
                        if (!isset($Trec[1][4])) $Trec[1][4] = ($oReceita->abril); else {
                            $Trec[1][4] += ($oReceita->abril);
                        }
                        if (!isset($Trec[1][5])) $Trec[1][5] = ($oReceita->maio); else {
                            $Trec[1][5] += ($oReceita->maio);
                        }
                        if (!isset($Trec[1][6])) $Trec[1][6] = ($oReceita->junho); else {
                            $Trec[1][6] += ($oReceita->junho);
                        }
                        if (!isset($Trec[1][7])) $Trec[1][7] = ($oReceita->julho); else {
                            $Trec[1][7] += ($oReceita->julho);
                        }
                        if (!isset($Trec[1][8])) $Trec[1][8] = ($oReceita->agosto); else {
                            $Trec[1][8] += ($oReceita->agosto);
                        }
                        if (!isset($Trec[1][9])) $Trec[1][9] = ($oReceita->setembro); else {
                            $Trec[1][9] += ($oReceita->setembro);
                        }
                        if (!isset($Trec[1][10])) $Trec[1][10] = ($oReceita->outubro); else {
                            $Trec[1][10] += ($oReceita->outubro);
                        }
                        if (!isset($Trec[1][11])) $Trec[1][11] = ($oReceita->novembro); else {
                            $Trec[1][11] += ($oReceita->novembro);
                        }
                        if (!isset($Trec[1][12])) $Trec[1][12] = ($oReceita->dezembro); else {
                            $Trec[1][12] += ($oReceita->dezembro);
                        }
                    } else {
...
/**
 * Caso a IN13 esteja habilitada, o quadro de deducao muda e passamos
 */
...
for ($x = 7; $x <= 8; $x++) {
    ...
    for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
        ...
        if (isset($rec[$x][$y])) {
            $tot_receita_patrimonial[0][$y] += $rec[$x][$y];
        } else
            $tot_receita_patrimonial[0][$y] += 0;
    }
    // procura valores do exercicio atual
    for ($y = 1; $y <= $iMesFinalAtual; $y++) {

        if (isset($recB[$x][$y])) {
            $tot_receita_patrimonial[0][$y] += $recB[$x][$y];
        } else
            $tot_receita_patrimonial[0][$y] += 0;
    }
    ...
    // listamos a previsão
    if (isset($recB[$x][13])) {
        $tot_receita_patrimonial[0][13] += $recB[$x][13];
    } else {
        $tot_receita_patrimonial[0][13] += 0;
    }
}
...
if (!isset($arqinclude)) {
    ...
    // esse total é sempre calculado para cada linha
    // imprime as linhas/valores do exercicio anterior
    cabecalhoAnexoIII($pdf);

    /**
     * A ultima linha não deve ser impressa nesse bloco, caso a versao seja a IN13
     */
    if ($versaoIN13) {
        $totalLinhasRelatorio = 31;
    }
    for ($x = 1; $x <= $totalLinhasRelatorio; $x++) { // 18 é a qtd de parametros existentes

        if (!$versaoIN13 && $x > 23) {
            continue;
        }
        if ($versaoIN13 && $x >= 21 && $x <= 23) {
            continue;
        }
        ...
        if ($x == 7) {
            ...
            for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
                ...
                if (isset($tot_receita_patrimonial[0][$y])) {

                    $pdf->cell($cl, $alt, db_formatar($tot_receita_patrimonial[0][$y], 'f'), 'R', 0, "R", 0);
                    $total += $tot_receita_patrimonial[0][$y];
                } else {
                    $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
                }
            }
            ...
            // listamos a previsão
            if (isset($tot_receita_patrimonial[0][13])) {
                $pdf->cell($cl, $alt, db_formatar($tot_receita_patrimonial[0][13], 'f'), 0, 0, "R", 0);
            } else {
                $pdf->cell($cl, $alt, db_formatar(0, 'f'), 0, 0, "R", 0);
            }
            $pdf->ln();
        ...
        if ($x == 31) {
            ...
            $pdf->setfont('arial', 'b', 6);
            $pdf->cell(60, $alt, "III - SUBTOTAL (I - II) ", 'RBT', 0, "L", 0);
            $total = 0;
            ...
            for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
                ...
                if (isset($Trec[0][$y])) {
                    ...
                    if (isset($Trec[1][$y])) {
                        ...
                        $pdf->cell($cl, $alt, db_formatar($Trec["0"]["$y"] - abs($Trec["1"]["$y"]), 'f'), 'RBT', 0, "R", 0);
                        $total += $Trec["0"]["$y"] - abs($Trec["1"]["$y"]);
            ...
            for ($y = 1; $y <= $iMesFinalAtual; $y++) {
                ...
                if (isset($TrecB[0][$y])) {
                    ...
                    if (isset($TrecB[1][$y])) {
                        ...
                        $pdf->cell($cl, $alt, db_formatar($TrecB[0][$y] - abs($TrecB[1][$y]), 'f'), 'RTB', 0, "R", 0);
                        $total += $TrecB[0][$y] - abs($TrecB[1][$y]);
                    } else {
            ...
            if (isset($TrecB[0][13])) {
                ...
                if (isset($TrecB[1][13])) {
                    $pdf->cell($cl, $alt, db_formatar($TrecB[0][13] - abs($TrecB[1][13]), 'f'), 'TB', 0, "R", 0);
                } else {
        ...
        for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
            ...
            if (isset($rec[$x][$y])) {
                ...
                $valorImprimir = $rec[$x][$y];
                if ($valorImprimir > 0 && $x == 31) {
                    $valorImprimir = 0;
                }
                $pdf->cell($cl, $alt, db_formatar($valorImprimir, 'f'), 'R', 0, "R", 0);
                $total += $valorImprimir;
            } else {
                $pdf->cell($cl, $alt, db_formatar(0, 'f'), 'R', 0, "R", 0);
            }
        ...
        if (isset($recB[$x][13])) {
            if ($x == $totalLinhasRelatorio && !$versaoIN13) {
                $recB[$x][13] *= -1;
            }

            $valorImprimir = $recB[$x][13];
            if ($valorImprimir > 0 && $x == 31) {
                $valorImprimir = 0;
            }

            $pdf->cell($cl, $alt, db_formatar($valorImprimir, 'f'), 0, 0, "R", 0);
        } else {
            $pdf->cell($cl, $alt, db_formatar(0, 'f'), 0, 0, "R", 0);
        }
        $pdf->ln();
    ...
    $labelFinalRelatorio = "RECEITA CORRENTE LÍQUIDA (III) = (I-II)";
    if ($versaoIN13) {
        $labelFinalRelatorio = " V - RECEITA CORRENTE LÍQUIDA (V = III + IV)";
    }
    $pdf->setfont('arial', 'b', 6);
    $pdf->cell(60, $alt, $labelFinalRelatorio, 'RBT', 0, "L", 0);
    $total = 0;
    ...
    for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
        ...
        if (isset($Trec[0][$y])) {
            ...
            $valor = $Trec[0][$y] - abs($Trec["1"]["$y"]);
            if ($versaoIN13 && $rec[31][$y] < 0) {
                $valor += abs($rec[31][$y]);
            }
            $totalAnterior[$y] = $valor;
            if (isset($Trec[1][$y])) {

                $pdf->cell($cl, $alt, db_formatar($valor, 'f'), 'RBT', 0, "R", 0);
                $total += $valor;
            } else {
    ...
    for ($y = 1; $y <= $iMesFinalAtual; $y++) {
        ...
        if (isset($TrecB[0][$y])) {
            ...
            $valor = $TrecB[0][$y] - abs($TrecB[1][$y]);
            if ($versaoIN13 && $recB[31][$y] < 0) {
                $valor += abs($recB[31][$y]);
            }
            $totalAtual[$y] = $valor;
            if (isset($TrecB[1][$y])) {

                $pdf->cell($cl, $alt, db_formatar($valor, 'f'), 'RTB', 0, "R", 0);
                $total += $valor;
            } else {
    ...
    if (isset($TrecB[0][13])) {
        ...
        $valor = $TrecB[0][13] - abs($TrecB[1][13]);
        if ($versaoIN13 && $rec[31][13] < 0) {
            $valor += abs($recB[31][13]);
        }
        $totalAtual[13] = $valor;
        if (isset($TrecB[1][13])) {
            $pdf->cell($cl, $alt, db_formatar($valor, 'f'), 'TB', 0, "R", 0);
        } else {
            $pdf->cell($cl, $alt, db_formatar($valor, 'f'), 'TB', 0, "R", 0);
        }
    } else {
    ...
    if ($versaoIN13) {
        ...
        cabecalhoAnexoIII($pdf);
        /**
         * primeira linha do quadro novo
         */
        $labelFinalRelatorio = " VI - DEDUÇÃO PREVISTA NO ART. 166 DA CF";
        $pdf->setfont('arial', 'b', 6);
        $pdf->cell(60, $alt, $labelFinalRelatorio, 'RBT', 0, "L", 0);

        $totalDaLinha = 0;
        for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
            $pdf->cell($cl, $alt, db_formatar($rec[32][$y], 'f'), 'RBT', 0, "R", 0);
            $totalDaLinha += $rec[32][$y];
        }
        ...
        $pdf->cell($cl, $alt, db_formatar($totalDaLinha, 'f'), 'TRB', 0, "R", 0);
        $pdf->cell($cl, $alt, db_formatar($rec[32][13], 'f'), 'TLB', 0, "R", 0);
        $pdf->ln();

        /**
         * segunda linha art 166 constituicao
         */
        $labelFinalRelatorio = "  Transferências Advindas de Emendas Parlamentares Individuais (§ 13 do artigo 166 da CF)";
        $pdf->setfont('arial', '', 6);
        $inicioLinha = $pdf->GetY();
        $alturaDaCelula = $pdf->nBlines(60, $labelFinalRelatorio);
        $pdf->MultiCell(60, $alt, $labelFinalRelatorio, 'RBT', 'L', 0);
        $totalDaLinha = 0;
        $posicaoLinha = 70;
        for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
            $pdf->SetXY($posicaoLinha, $inicioLinha);
            $pdf->cell($cl, $alt * $alturaDaCelula, db_formatar($rec[32][$y], 'f'), 'RBT', 0, "R", 0);
            $totalDaLinha += $rec[32][$y];
            $posicaoLinha += $cl;
        }
        // bimestre do exercício corrente
        for ($y = 1; $y <= $iMesFinalAtual; $y++) {
            ...
            $pdf->SetXY($posicaoLinha, $inicioLinha);
            $pdf->cell($cl, $alt * $alturaDaCelula, db_formatar($valor, 'f'), 'RTB', 0, "R", 0);
            $totalDaLinha += $valor;
            $posicaoLinha += $cl;
        }
        $pdf->SetXY($posicaoLinha, $inicioLinha);
        $pdf->cell($cl, $alt * $alturaDaCelula, db_formatar($totalDaLinha, 'f'), 'TRB', 0, "R", 0);
        $pdf->cell($cl, $alt * $alturaDaCelula, db_formatar($rec[32][13], 'f'), 'TLB', 0, "R", 0);
        $pdf->ln();

        /**
         * Total Geral extendido
         */
        $labelFinalRelatorio = "VII - RECEITA CORRENTE LÍQUIDA AJUSTADA - EC 86/2015 (VII = V - VI)";
        $pdf->setfont('arial', 'b', 6);
        $inicioLinha = $pdf->GetY();
        $alturaDaCelula = $pdf->nBlines(60, $labelFinalRelatorio);
        $pdf->MultiCell(60, $alt, $labelFinalRelatorio, 'RBT', 'L', 0);
        ...
        for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
            $pdf->SetXY($posicaoLinha, $inicioLinha);
            $valor =   $totalAnterior[$y] - $rec[32][$y];
            $pdf->cell($cl, $alt * $alturaDaCelula, db_formatar($valor, 'f'), 'RBT', 0, "R", 0);
            $totalDaLinha += $valor;
            $posicaoLinha += $cl;
        ...
        $pdf->SetXY($posicaoLinha, $inicioLinha);
        $pdf->cell($cl, $alt * $alturaDaCelula, db_formatar($totalDaLinha, 'f'), 'TRB', 0, "R", 0);
        $pdf->cell($cl, $alt * $alturaDaCelula, db_formatar($rec[32][13] - $totalAtual[13], 'f'), 'TBL', 0, "R", 0);
        $pdf->ln();
    }
// ----------------------------------------------------------------
...
function cabecalhoAnexoIII(FPDF $pdf)
{
    global $alt, $cl, $bimestre, $bimestreEscrito, $anousu, $iMesFinalAnterior, $iMesInicialAnterior, $mes_dresc,
           $iMesFinalAtual;
    ...
    if ($bimestre != 6 && $bimestreEscrito != "D") {
        $ano = $anousu - 1;
    } else {
        $ano = $anousu;
    }
