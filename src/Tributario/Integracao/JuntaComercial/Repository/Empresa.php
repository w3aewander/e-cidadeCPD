...
class Empresa
{
   ...
   * @throws \Exception
   */
    public static function persist(\Empresa $oEmpresa, ProtocoloModel $oProtocolo)
    {
        ...
        if (!empty($oDaoEmpresa->q02_inscr)) {
            ...
            if ($oDaoEmpresa->numrows == 0) {
                $lTrocaEnderecoMunicipio = false;
                foreach ($oProtocolo->getEventos() as $oEvento) {
                    if ($oEvento->getCodigo() == 211) {
                        $lTrocaEnderecoMunicipio = true;
                    }
                }
                if ($lTrocaEnderecoMunicipio == false && $oDaoEmpresa->numrows == 0) {
                    throw new \Exception("Empresa com inscrição {$oDaoEmpresa->q02_inscr} não cadastrada.");
                }
                $oDaoEmpresa->q02_inscr = null;
            } else {
   ...
   */
    ...
    private static function persistAtividade(\Empresa $oEmpresa)
    {
        ...
        if ($oDaoAtividadePrincipal->erro_status == "0") {
            $sMsg  = "Não foi possível excluir os dados da atividade principal ";
            $sMsg .= " da inscrição {$oEmpresa->getInscricao()}";
        ...
        if ($oDaoAtividade->erro_status == "0") {
            $sMsg = "Não foi possível excluir os dados da atividade da inscrição {$oEmpresa->getInscricao()}";
            throw new \Exception($sMsg);
        }
        ...
        foreach ($oEmpresa->getAtividades() as $oAtividade) {
            ...
            $oDaoAtividade->q07_imprimealvara = 'Sim';
