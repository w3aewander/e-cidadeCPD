...
<form name="form1" method="post" action="" class="form-container">
  ...
  <fieldset style="width:93%">
        ...
        <tr>
          ...
          <td nowrap="nowrap">
              <?php
                db_inputdata(
                    'bi06_dataregistro',
                    $bi06_dataregistro_dia,
                    $bi06_dataregistro_mes,
                    $bi06_dataregistro_ano,
                    true,
                    'text',
                    $db_opcao
                );
    ...
    <fieldset class='separator'>
      ...
      <table>
        ...
        <tr id="linhaNumeroItem" style="display: none;">
          <td>
            <label for="bi06_numeroitem"><?= $Lbi06_numeroitem ?></label>
          </td>

          <td>
            <select id="bi06_numeroitem" name="bi06_numeroitem" style="width: 95px;">
  ...
  <input name="numeroItem" type="hidden" id="numeroItem" value="<?php echo !empty($bi06_numeroitem) ? $bi06_numeroitem : '';?>" />
</form>
<script>
  ...
  function buscaQuantidadeItensColecao()
  {
    $('bi06_numeroitem').length = 0;
    $('bi06_numeroitem').add(new Option('Selecione...', ''));
    $('linhaNumeroItem').setStyle({'display': 'none'});

    if($F('bi06_colecaoacervo') === '') {
      return false;
    }

    new AjaxRequest(
      'bib4_acervo.RPC.php',
      {'exec': 'quantidadeItensColecao', 'colecao': $F('bi06_colecaoacervo')},
      function(retorno, erro) {
        ...
        $('linhaNumeroItem').setStyle({'display': ''});

        for(let contador = 1; contador <= retorno.quantidadeItens; contador++) {
          $('bi06_numeroitem').add(new Option(contador, contador));
        }

        if($F('numeroItem') !== '') {
          $('bi06_numeroitem').value = $F('numeroItem');
        }
      }
    ).execute();
  }
  ...
  (function() {
    ...
    if($F('bi06_seq') !== '' && $F('bi06_colecaoacervo') !== '') {
      $('linhaNumeroItem').setStyle({'display': ''});
      buscaQuantidadeItensColecao();
    }
  })();
