...
class Turma
{
     ...
     */
    private $aAlunosMatriculados = [];

    /**
     * conjunto de alunos vinculados a turma
     * @var array de aluno
     */
    private $aAlunosProgressaoParcial = [];
    ...
    public function __construct($iCodigoTurma = null)
    {
        ...
        if (!empty($iCodigoTurma)) {
            ...
            if (pg_num_rows($rsTurma) > 0) {
                ...
                $this->periodos = PeriodoEscolaRepository::getPeriodoEscolaByTurnoEscola($this->oTurno, $this->oEscola);
            }
        }
    }
    ...
    public function getAlunosMatriculadosAlfa($lOrdenarPorClassificacaoDaTurma = true)
    {
    ...
    public function getAlunosMatriculadosNaTurmaPorSerieAlfa(Etapa $oEtapaOrigem, $lOrdenarPorClassificacaoDaTurma = false)
    {

        $aAlunos = $this->getAlunosMatriculadosAlfa($lOrdenarPorClassificacaoDaTurma);
        $aAlunosRetorno = [];
        foreach ($aAlunos as $oAluno) {
     ...
     * @return Matricula[]
     */
    public function getAlunosMatriculadosNasTurmasPorSerie($iEtapa, $lOrdenarPorClassificacaoDaTurma = true)
    {
        ...
        foreach ($aAlunos as $oAluno) {
            if ($oAluno->getEtapaDeOrigem() instanceof Etapa
                && in_array($oAluno->getEtapaDeOrigem()->getCodigo(), $iEtapa)
            ) {
                $aAlunosRetorno[] = $oAluno;
            }
        }
    ...
    public function getDisciplinas()
    {
        ...
        if (count($this->aDisciplinas) == 0) {
            ...
            $sSqlDisciplina = $oDaoDisciplina->sqlRegencias(null, $sCampos, $sOrder, $sWhere);
     ...
     * @return Regencia[] Retorna todas as regências da turma
     */
    public function getDisciplinasPorEtapas($oEtapa)
    {
        $aDisciplinas = $this->getDisciplinas();
        $aDisciplinasEtapa = [];
        foreach ($aDisciplinas as $oDisciplina) {
            if (in_array(strval($oDisciplina->getEtapa()->getCodigo()), $oEtapa) ) {
                $aDisciplinasEtapa[] = $oDisciplina;
            }
        }
        return $aDisciplinasEtapa;
    ...
    public function getProfessorConselheiro()
    {
        ...
        if (pg_num_rows($rsRegenteConselho) > 0) {
             ...
             * Data: 28/11/2025
             * Demanda: 17982
             * Razão: Algumas turmas possuem registro em regenteconselho com recurso humano sem CGM cadastrado.
             *        DocenteRepository::getDocenteByCodigoRecursosHumano() retornava uma instância com $oCgm = null,
             *        causando erro fatal ao chamar getNome() em diversos relatórios (Ata, Ficha, etc).
             *        A correção garante que apenas Docentes com CGM válido sejam retornados,
             *        evitando o erro e exibindo o campo em branco quando não houver dados consistentes.
             */
            if (!empty($iCodigoRecHumano)) {
                $oDocente = DocenteRepository::getDocenteByCodigoRecursosHumano($iCodigoRecHumano);
                if ($oDocente && $oDocente->getCgm()) {
                    return $oDocente;
                }
            }
        }
