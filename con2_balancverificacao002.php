...
for ($x = 0; $x < pg_num_rows($result); $x++) {
    ...
    if (($pdf->gety() > $pdf->getH() - 32) || $pagina == 1) {
        $pagina = 0;
        $pdf->addpage('L');
        $pdf->setfont('arial', 'b', 6.5);
    ...
    $sql = sprintf(
        'select * from conplanoconta where c63_codcon = %s and c63_reduz = %s and c63_anousu = %s',
        $c61_codcon,
        $c61_reduz,
        $anousu
    );
    $resconta = db_query($sql);
    ...
    $pdf->cell(18, $alt, $recurso, 0, 0, "C", 0);
