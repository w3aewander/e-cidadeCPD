...
$clvistoriaslote     = new cl_fis_vistoriaslote;
$clvistoriaslotevist = new cl_fis_vistoriaslotevist;
...
require_once(modification("forms/db_frm_fis_vistgeral.php"));
...
if (isset($processa) && $processa == 't') {
  ...
  if(!isset($iExercicio) && $iExercicio == ""){
    ...
    echo "<script>(window.CurrentWindow || parent.CurrentWindow).corpo.location.href='fis4_fis_vistgeral001.php'</script>";
    exit;
  }
  ...
  $rsTipovist  = db_query("select y77_diasgeral, y77_mesgeral from fiscalizacao.fis_tipovistorias where y77_codtipo = $tipo");
  ...
  if ($origemori == 1){
    ...
    $where .= " and q02_inscr in (select distinct q02_inscr from w_tfvs4)";
    ...
    $sSql .= "             from fiscalizacao.fis_sanitario                                                                      ";
    $sSql .= "                 inner join fiscalizacao.fis_saniatividade  on y83_codsani = y80_codsani                          ";
    ...
    $sSql .= "                 inner join tipcalc        on q81_codigo  = ativtipo.q80_tipcal                  ";
    $sSql .= "                 left  join fiscalizacao.fis_sanitarioinscr on fis_sanitarioinscr.y18_codsani = fis_sanitario.y80_codsani ";
    $sSql .= "                 $where  ";
    $sSql .= "        ) as x           ";
    $sSql .= "    where  datacad < '".$oDataExercicio->getDate()."' ";
...
//die($sSql);
  ...
  for ($iAno = $iAnoInicial; $iAno <= $iAnoFinal; $iAno++ ) {
    ...
    for ($i=0;$i < $numrows; $i++){
      ...
      if ($origemori != 1){
        ...
        $rsSanitario     = db_query(" select extract(year from y80_data) as anosani from fiscalizacao.fis_sanitario where y80_codsani = $y80_codsani ");
  ...
  if($rel == 't'){
    ...
    echo "<script>
      jan = window.open('fis4_fis_relerrosvistgeral002.php?numlote=".$clvistoriaslote->y06_vistoriaslote."','','width='+(screen.availWidth-5)+',height='+(screen.availHeight-40)+',scrollbars=1,location=0');
    jan.moveTo(0,0);
    </script>";
  }
