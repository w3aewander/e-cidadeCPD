...
class Unpack extends Command
{
    ...
    {
        $this
            ->setName('modification:unpack')
            ->setDescription('Unpackage modification')
            ->setHelp('Unpackage modification');

        $this->addArgument('path', InputArgument::REQUIRED, 'Modification xml path');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $data = $manager->unpack($file, $force);
        echo " - Modificação descompactada: ". $data->getId() ."\n";
