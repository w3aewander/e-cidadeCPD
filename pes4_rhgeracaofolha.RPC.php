...
try {
  ...
  case "geraFolha":
    ...
    case "buscaMatriculas":
      ...
      if (!empty($oDadosFormulario->selecao)) {
          
        $oSelecao = new Selecao($oDadosFormulario->selecao);
        $sWhereServidores .= " and rh02_regist in (select rh02_regist 
                                                     from rhpessoal 
      ...
      if (DBPessoal::utilizaFiltroLotacoesPorUsuario()) {
          ...
          $sWhereServidores .= " and rhpessoalmov.rh02_lota in (".implode(",",$oLotacoesUsuario->aLotacoes).")";
      }

      $sSqlServidores            = $oDaoRhPessoalMov->sql_query_file(null, null, 'rh02_regist', null, $sWhereServidores);
      $sSqlDuploVinculo          = $oDaoRhPessoalMov->sql_servidores_duplo_vinculo_rescindidos($sSqlServidores,DBPessoal::getAnoFolha(),DBPessoal::getMesFolha(),db_getsession('DB_instit'),$oDadosFormulario->opcao_geral);
      ...
      file_put_contents($oRetorno->arquivo, implode(',',$aMatriculasDuploVinculo));
