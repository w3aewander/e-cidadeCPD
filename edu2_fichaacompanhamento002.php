...
$pTurma = new Turma($oGet->iTurma);
$oProfessor =  $pTurma->getProfessorConselheiro() == false ?  "" : $pTurma->getProfessorConselheiro()->getNome();
...
if ($iCodigoReferencia = null || $iCodigoReferencia == "") {
    $iCodigoEscola = $oEscola->getCodigo();
    $sNomeEscola = "{$iCodigoEscola} - {$sNomeEscola}";
} else {
    $sNomeEscola = "{$iCodigoReferencia} - {$sNomeEscola}";
}

$title1 = $oEscola->getNomeEstadoExtenso();
$title2 = $sNomeEscola;
$title3 = $sSecretaria;

$oPdf->addTitulo($title1, 1);
...
for ($iContador = 0; $iContador < $iContadorAlunos; $iContador++) {
    ...
    $oPdf->Cell(20, $iHeigth, "Professor(a): ", 0, 0, "L", 0);
    $oPdf->SetFont('arial', '', 7);
    $oPdf->Cell(20, $iHeigth, $oProfessor, 0, 1, "L", 0);
    $oPdf->SetFont('arial', 'B', 8);
    ...
    $result = db_query($sSqlAlunoNecessidades);
    ...
    $iTamanhoCelulaDescricaoParecerCalculado = $iTamanhoCelulaDescricaoParecerPadrao - ($iNumeroDePeriodos * $iTamanhoCelulaPeriodoPadrao);
    ...
    foreach ($aDisciplinaParecer as $aParecer) {
        ...
        foreach ($aParecer as $oParecer) {
            ...
            if ($oPdf->gety() > $oPdf->getH() - 15) {
                imprimeDisciplina($oPdf, $iYInicial, $iXInicial, $iAlturaRetangulo, $sNomeDisciplina, $iTamanhoCelulaDescricaoParecerCalculado);
                $oPdf->AddPage();
     ...
     * Neste documento estamos utilizando a altura das linhas com o valor 4 mm
     * portanto o quadro terá uma altura de 104 mm 96 mm
     */
    ...
    if ($oPdf->GetY() > ($oPdf->getH() - ($iTotalLinhasObservacao + 15))) {
        $oPdf->AddPage();
    }
    $oPdf->SetFont('arial', 'b', 8);
...
$oPdf->Output('I', 'edu2_fichaacompanhamento002', false);
...
function imprimeDisciplina(
    ...
    $iAlturaTextoDisciplina = $iCalculoDoTextoDaAlturaDaDisciplina < $iYInicial ? $iYInicial : $iCalculoDoTextoDaAlturaDaDisciplina;
...
function setHeader(
    ...
    foreach ($aPeriodos as $oPeriodos) {
        $oPdf->Cell($iTamanhoCelulaPeriodoPadrao, $iHeigth, $oPeriodos->getPeriodoAvaliacao()->getDescricaoAbreviada(), 1, 0, "C", 1);
    }
