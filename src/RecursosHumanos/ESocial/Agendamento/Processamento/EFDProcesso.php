...
class EFDProcesso extends ProcessamentoAbstract implements ProcessamentoInterface
{
    ...
    public function processar()
    {
        ...
        $dadosPreenchimento = $dadosEsocial->getPorTipo(Tipo::EFD_PROCESSOS);

        $formatter = FormatterFactory::get(Tipo::R1070);
        $dadosFormatados = $formatter->formatar($dadosPreenchimento);

        foreach ($dadosFormatados as $dados) {
            $eventoFila = new Evento(Tipo::R1070, $cgm, $dados['referencia'], $dados);
