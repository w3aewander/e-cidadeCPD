...
require_once("libs/db_con"."ecta.php");
...
switch ($oParam->exec) {
  ...
  case 'getRegistros':
    ...
    $sSqlArquivo .= "                               and c63_anousu= ".db_getsession('DB_anousu');
  ...
  case 'processarArquivo' :
    ...
    if (pg_num_rows($rsBanco) > 0) {
       ...
       */
      $sLinhaLote  = $sBanco;
      $sLinhaLote .= substr(str_pad($iCodigoEnvio,4,"0", STR_PAD_LEFT),0,4); //numerodolote;
      $sLinhaLote .= 1; //tipo dalinha
      switch ($sBanco) {
        
        case '001':
          
          $sLinhaLote .= str_pad($iCodigoEnvio, 232, "0", STR_PAD_LEFT);
          break;
        default:
      ...
      $sSqlDadosArquivo .= "       e81_valor - fc_valorretencaomov(e81_codmov, false) as e81_valor,";
...
function getRegistroArquivo ($iArquivo) {
  ...
  $sSqlDadosArquivo  = "select e81_codmov,";
  $sSqlDadosArquivo .= "       e81_valor - fc_valorretencaomov(e81_codmov, false) as e81_valor, ";
  $sSqlDadosArquivo .= "       case when cge.z01_nome is null then cgs.z01_nome else cge.z01_nome end as z01_nome , ";
  $sSqlDadosArquivo .= "       to_char(e87_data,'ddmmYYYY') as dataenvio, ";
  $sSqlDadosArquivo .= "       to_char(e87_data+1,'ddmmYYYY') as datapgto";
  $sSqlDadosArquivo .= "  from empageconfgera ";
  $sSqlDadosArquivo .= "       inner join  empagegera on e90_codgera = e87_codgera";
  ...
  $sSqlDadosArquivo .= "       left  join  cgm cge    on e60_numcgm  = cge.z01_numcgm ";
  $sSqlDadosArquivo .= "	  left join empageslip on e81_codmov = e89_codmov ";
//  $sSqlDadosArquivo .= "	  inner join conplanoreduz on e83_conta = c61_reduz and c61_anousu = ".db_getsession("DB_anousu");
//  $sSqlDadosArquivo .= "	  inner join conplanoconta on c63_codcon = c61_codcon and c63_anousu = c61_anousu ";
  $sSqlDadosArquivo .= "	  left join slip on slip.k17_codigo = e89_codigo";
  $sSqlDadosArquivo .= "	  left join slipnum on slipnum.k17_codigo = slip.k17_codigo";
  $sSqlDadosArquivo .= "	  left join cgm cgs on k17_numcgm = cgs.z01_numcgm";
  $sSqlDadosArquivo .= " where e90_codgera = {$iArquivo}  and empageconfgera.e90_cancelado is false ";
// echo "<br><br> $sSqlDadosArquivo";
  $rsDadosArquivo    = db_query($sSqlDadosArquivo);
