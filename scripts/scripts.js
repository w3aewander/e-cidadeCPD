...
!function (t) {
    ...
    var l = setTimeout, a = "function" == typeof setImmediate && setImmediate || function (t) {
        l(t, 0)
    }, d = function (t) {
...
!function (a) {
    ...
    function s(a, b) {
        b = b || {};
        var c = b.body;
        if ("string" == typeof a) this.url = a; else {
    ...
    if (!a.fetch) {
        ...
        v.redirect = function (a, b) {
            ...
            return new Promise(function (d, e) {
                var f = new s(a, c), g = new XMLHttpRequest;
                g.onload = function () {
                    var a = {status: g.status, statusText: g.statusText, headers: u(g.getAllResponseHeaders() || "")};
...
Object.assign || Object.defineProperty(Object, "assign", {
    ...
    value: function (e) {
        ...
        for (var r = Object(e), t = 1; t < arguments.length; t++) {
            ...
            if (void 0 !== n && null !== n) {
                n = Object(n);
                for (var o = Object.keys(Object(n)), c = 0, i = o.length; c < i; c++) {
                    var a = o[c], b = Object.getOwnPropertyDescriptor(n, a);
                    void 0 !== b && b.enumerable && (r[a] = n[a])
                }
            }
        }
        return r
...
if (CurrentWindow.TopWindow === undefined) {
    CurrentWindow.TopWindow = top;
}
...
if (window.Element && !Element.prototype.closest) {
    Element.prototype.closest = (selector) => {
        const selfDocument = (this.document || this.ownerDocument);
        const matches = selfDocument.querySelectorAll(selector);
...
//#20#//Ano    : Objeto ano para testar a data
    ...
    if (strPartTipo == "_dia") {
        ...
        if (str.match(expr) == null || str > 12 || str == 0 || str == '00') {
            alert("Mes inválido");
            F.elements[nome].select();
            return false;
        } else
...
//#99#//Esta função deve ser utilizada na propriedade onload do objeto body do formulário
    if (qchave != '') {
        if (qjanela == 'cgm') {
            js_OpenJanelaIframe('CurrentWindow.corpo', 'db_janelaCgm', 'prot1_cadgeralmunic005.php?fechar=CurrentWindow.corpo.db_janelaCgm&chavepesquisa=' + qchave, 'Dados Cadastrais');
        }
        if (qjanela == 'iptubase') {
        ...
        if (qjanela == 'empsolicita') {
            js_OpenJanelaIframe('CurrentWindow.corpo', 'db_janelaempsolicita', 'com3_conssolicitacao002.php?cons=item&fechar=CurrentWindow.corpo.db_janelaempsolicitai&pc10_numero=' + qchave, 'Consulta Solicitações', true);
        }
...
function js_visualizarPDF(documentPath, width, height, flagPercentage = false) {
    var janelaDocumento = CurrentWindow.corpo.criaJanela('VisualizePDF', `db_visualizadorPDF.php?documentPath=${documentPath}`, 'Visualizador PDF', true);
    var unit = 'px';
    ...
    if (flagPercentage) {
        unit = '%';
        pxWidth = screen.width * (width / 100);
        pxHeight = screen.height * (height / 100);
    }

    janelaDocumento.setLargura(`${width}${unit}`);
    janelaDocumento.setAltura(`${height}${unit}`);

    var leftSpacing = (screen.width / 2) - (pxWidth / 2);
    var topSpacing = (screen.height / 2) - (pxHeight / 1.25);

    janelaDocumento.setPosX(leftSpacing);
    janelaDocumento.setPosY(topSpacing);

    janelaDocumento.show();
    janelaDocumento.focus();

    return janelaDocumento;
}
...
var windowCalendar = null;

/**
 * Função para mostrar o calendário do sistema
 * @param {String} obj - id do elemento
 * @param {Function} shutdown_function -  função ao ser executada no final da
 *     execução do calendário
 */
function show_calendar(obj, shutdown_function, event = null) {
    const x = PosMouseX - window.scrollX;
    const y = PosMouseY - window.scrollY;
    ...
    iframe.src = `func_calendario.php?nome_objeto_data=${obj}&shutdown_function=${shutdown_function}`;
    ...
    if (windowCalendar !== null) {
        windowCalendar.destroy();
    }

    windowCalendar = new windowAux(
        `iframe_data_${obj}`,
        'Calendário',
        200,
        275
    );
    windowCalendar.setContent(iframe);
    windowCalendar.show(PosMouseY, PosMouseX, false, null, true);
}
...
//#15#//testa_cookie();
    ...
    document.cookie = 'aceita_cookie=sim;path=/;SameSite=Lax;';
    // Checa se conseguiu
    if (document.cookie == '') {
        ...
        // Apaga o cookie.
        document.cookie = 'aceita_cookie=sim;SameSite=Lax; expires=Fri, 13-Apr-1970 00:00:00 GMT';
        return (true);
    }
}
...
function js_ajax_msg(mensagem) {
    ...
    camada.style.background = "#d5d5d596";
    camada.style.position = "absolute";
    camada.style.top = `0px`;
    camada.style.zIndex = 9999;
    camada.style.width = "100%";
    camada.style.height = "100%";
    camada.style.margin = 0;
    camada.style.padding = 0;

    camada.innerHTML = `
             <div style="position:relative; margin:auto;margin-top:50px">
                 <img src="imagens/files/loading.gif" width="50px">
                 <br>
                 <b style="font-size:20px">${mensagem}</b>
             </div>
            `;
...
function array_unique(inputArr) {
     ...
     * +   @bugfixed by: Nate
     * +      @input by: Brett Zamir (http:*brett-zamir.me)    * +   bugfixed by:
 ...
 * @param oElemento Formulario onde se encontram os elementos que serao
 *     desabilitados
 * @param lBloquear Recebe um boolean utilizado como parametro para o readOnly
 *     e disabled
 * @param listaIdsElementosNaoDesabilitar Recebe um array com lista de elementos que
 *     nao devem mudar o seu estado para disabled
 */
function setFormReadOnly(oElemento, lBloquear, listaIdsElementosNaoDesabilitar = []) {
    ...
    for (var iContador = 0; iContador < iTamanho; iContador++) {
        ...
        if (js_search_in_array(aElementosParaDesabilitar, oElemento.elements[iContador].type) 
            && !js_search_in_array(listaIdsElementosNaoDesabilitar, oElemento.elements[iContador].id)) {
...
function require(sArquivo) {
    ...
    const url = isV3 ? CurrentWindow.resolveUrl(sArquivo) : sArquivo;

    const oRequisicao = new XMLHttpRequest();
    oRequisicao.open('GET', url, false);

    const sContentType = "application/x-www-form-urlencoded; charset=ISO-8859-1";
    ...
    const aDadosArquivo = sArquivo.split(".");
    const iUltimoIndice = aDadosArquivo.length - 1;
    const sExtensao = aDadosArquivo[iUltimoIndice];
    ...
    switch (sExtensao) {
        ...
        case "css":
            ...
            oStyle.innerHTML = oRequisicao.responseText;
...
function js_idade(iDia, iMes, iAno) {
     ...
     * Caso o mês de nascimento seja maior que o mês atual, verifica-se a
     * diferença entre o mês de um ano e do outros, calculando o (total de meses
     * no ano subtraindo - mês de nascimento) + mês atual. Em seguida, diminuimos
     * 1 ano. Ex.: Nascimento - 02/02/2012 Atual      - 02/01/2013 Cálculo    -
     * (12 - 2) + 1 Diferença  - 11 meses
     */
    var iMes = 0;
     ...
     * Caso o dia da data de nascimento seja maior que o dia da data atual,
     * faz-se o seguinte cálculo:
     * 1º Busca o número de dias de um mês em determinado ano, chamando a função
     * js_getNumeroDeDiaNoMes
     * 2º (Total de dias de um mês retornado da função - Dia da data de
     * nascimento) + Dia da Data Atual
     * 3º Diminui-se 1 mês
     * 4º Caso ao diminuir, o mês fique menor que zero, fixamos como mês 11 e
     * diminui-se 1 ano Ex.: Nascimento - 30/04/2013 Atual      - 29/04/2013
     * Cálculo    - (30 - 30) + 29 Diferença  - 29 dias iMes       - 04 - 04 = 0
     * (diminuindo 1 mês pelo cálculo do dia, ficaria iMes = -1) iMes       - 11
     * iAno       - iAno - 1
     */
 ...
 * @param {Object} oVariaveis objeto literal com as variaveis que devem ser
 *     substituidas
 * @example DBMensagem.getMensagem('configuracao.mensagem.con4_mensagem001.mensagem_nao_informada');
 *     Aonde: DBPortal. <-area configuracao <- modulo con4_mensagem001<-
 *     Programa mensagem_nao_informada <- mensagem que deve ser exibida
...
/**
 * Checks if the argument variable is empty undefined, null, false, number 0,
 * empty string, string "0", objects without properties and empty arrays are
 * considered empty http://kevin.vanzonneveld.net original by: Philippe Baumann
 * input by: Onno Marsman bugfixed by: Kevin van Zonneveld
 * (http://kevin.vanzonneveld.net) input by: LH improved by: Onno Marsman
 * improved by: Francesco improved by: Marc Jansen input by: Stoyan Kyosev
 * (http://www.svest.org/) improved by: Rafal Kukawski
 * @exemple
 ...
 *
 * @param {string} sEmail
 * @param {boolean} lMostraMensagem Define se deve exibir uma mensagem, padrão
 *     true
 * @returns {boolean}
 */
...
if (CurrentWindow.topo != undefined) {
...
function janela(janP, cFt, Iframe) {
     ...
     * Métodos
     *
     * Adicionado Prototype, para que seja possivel ser feita sobrecarga dos
     * mesmos
     */
    ...
    document.cookie = "modulo=" + janP.id + ';SameSite=Lax';
    netscape = navigator.appName == "Netscape" ? 1 : 0; //@TODO Remove THIS
    if (!(JANS instanceof Array)) {
        JANS = [];
        JANS.isModal = 0;

    } else {
        JANS.forEach(el => {
            el.guardaCorFundoTitulo = el.corFundoTitulo;
            el.setCorFundoTitulo("#A6A6A6");
        })
    }
...
function criaJanela(nomeJan, arquivo, cabecalho, visivel, topo, esquerda, altura, largura) {
    const urlEcidade = CurrentWindow.corpo.Desktop.requestPath;
    var camada = document.createElement("DIV");
    ...
    img3.setAttribute("src", urlEcidade + "skins/img.php?file=Controles/jan_fechar_on.png");
    ...
    img3.onclick = function () {
        js_FecharJan(this, nomeJan);
    };
    this.btnFechar = img3;

    img1.setAttribute("src", urlEcidade + "skins/img.php?file=Controles/jan_mini_on.png");
    img1.setAttribute("title", "Minimizar");
    img1.setAttribute("border", "0");
    img1.setAttribute("id", "minimizar" + nomeJan);

    img1.onclick = function () {
        js_MinimizarJan(this, nomeJan);
    };
    ...
    tab2Coluna1.onmousedown = function (event) {
        js_engage(document.getElementById('Jan' + nomeJan), event);
    };
    tab2Coluna1.onmouseup = function (event) {
        js_release(document.getElementById('Jan' + nomeJan), event);
    };
    tab2Coluna1.onmousemove = function (event) {
        js_dragIt(nomeJan, event);
    };
    tab2Coluna1.onmouseout = function (event) {
        js_release(document.getElementById('Jan' + nomeJan), event);
    };
    ...
    tab2Coluna1.innerHTML = (typeof (cabecalho) == "undefined" || cabecalho == "") ? '&nbsp; Microsist' : ('&nbsp;' + cabecalho);
 ...
 *
 * @exemple document.body.load('teste.php', function(lRetorno){
 *     alert(lRetorno)});
 * @param  {Strind} sUrl      URL a ser carregada
 * @param  {Function} fCallback Função para callback
 * @return - false: erros status de requisicao >= 400, envia false para o
 *     callback
...
function mascaraTelefone(oElemento) {
    ...
    oElemento.onkeypress = function (evt) {
        ...
        /**
         * o evento keypress retorna 0 ao invés do valor ASC para algumas teclas do
         * teclado; exemplo: Tab, DEL, Left, Right, Up, Down ...
         *
         * code == 8 : BackSpace
         */
...
function replaceAll(search, replacement, subject) {
    return subject.split(search).join(replacement);
}


/**
 * Bloqueia os inputs do tipo button em tela
 * @param bloquear bool - true irá bloquear
 * @param naoBloquear Array - array com os IDs que não deve ser bloqueados/liberados
 *
 * EX: bloquearBotoesTela(true, ['campo_id', 'campo_id']);
 */
function bloquearBotoesTela(bloquear = true, naoBloquear = []) {

    var botoesEncontrados = document.querySelectorAll("input[type=button]");
    for (botao of botoesEncontrados) {
        if (js_search_in_array(naoBloquear, botao.id)) {
            continue;
        }
...
/**
 * Verifica os campos que não podem ser nulos na tela
 * @param array
 * @autor Natanael Giacomini (natanael.giacomini@dbseller.com.br)
 *
 * Ex.: if (js_verificaCampoObrigatorio(["z01_numcgm"])) {
            return false;
        }
 */

function js_verificaCampoObrigatorio(aExcecoes = []) {
    var obrigatorio = document.querySelectorAll('[isMandatory="true"]');
    var erro = false;

    obrigatorio.forEach(function (elemento) {
        if (((aExcecoes.length == 0) || !aExcecoes.includes(elemento.name)) && elemento.value == "" && !erro) {
            var campo = elemento.getAttribute("labelValidacao");

            alert("Campo " + campo + " não preenchido.");
...
/**
 * Função para adicionar mascara de moeda em campo text
 * encontrada em: @link https://www.guj.com.br/t/mascara-de-moeda-em-javascript/206522
 */

function js_adicionaMascaraMoeda(objTextBox, SeparadorMilesimo, SeparadorDecimal, e, fnCallBack = "") {
    var sep = 0;
    var key = '';
    var i = j = 0;
    var len = len2 = 0;
...
/**
 * Nova função para adicionar mascara de moeda nos inputs
 * jsFormataMoeda(this, (nValor) => {console.log(nValor);})
 * <input type="text" onkeyup="jsFormataMoeda(this, (nValor) => {console.log(nValor);})">
 * @author Natanael Giacomini (natanael.giacomini@dbseller.com.br)
 */
function jsFormataMoeda(oCampo, fnCallback = null) {
    const iValor = oCampo.value;
    var nNovoValor = 0;

    nNovoValor = iValor.replace(/\D/g, ""); // permite digitar apenas numero
    nNovoValor = nNovoValor.replace(/(\d{1})(\d{14})$/, "$1.$2"); // coloca ponto antes dos ultimos 14 digitos
    nNovoValor = nNovoValor.replace(/(\d{1})(\d{11})$/, "$1.$2"); // coloca ponto antes dos ultimos 11 digitos
    nNovoValor = nNovoValor.replace(/(\d{1})(\d{8})$/, "$1.$2"); // coloca ponto antes dos ultimos 8 digitos
    nNovoValor = nNovoValor.replace(/(\d{1})(\d{5})$/, "$1.$2"); // coloca ponto antes dos ultimos 5 digitos
    nNovoValor = nNovoValor.replace(/(\d{1})(\d{1,2})$/, "$1,$2"); // coloca virgula antes dos ultimos 2 digitos

    oCampo.value = nNovoValor;

    if (fnCallback != null) {
        return fnCallback(nNovoValor, oCampo);
    }
}

function jsRemoveMascaraMoeda(sValor) {
    return Number(sValor.replaceAll(".", "").replace(",", "."));
}
...
var formataValorMonetario = (value) => {
