...
class AlteracaoLicitaCon extends ArquivoLicitaCon
{
    ...
    private function prepararDados()
    {
        ...
        for ($iRowAcordo = 0; $iRowAcordo < $iTotalAcordos; $iRowAcordo++) {
            ...
            for ($iRowEvento = 0; $iRowEvento < $iTotalEventos; $iRowEvento++) {
                ...
                if ($oStdPosicaoAtual->tipo_aditamento == AcordoPosicao::TIPO_RENOVACAO) {
                    $oStdPosicaoAtual->valores->valor_atual = $oStdPosicaoAtual->valores->valor_total;
                    $oStdPosicaoAtual->valores->valor_atual *= -1;
                } elseif ($oStdPosicaoAtual->tipo_aditamento == AcordoPosicao::TIPO_ALTERACAO_DOTACAO) {
                    $oStdPosicaoAtual->valores->valor_atual = $oStdValorPosicaoAnterior->valores->valor_atual - 0;
                } else {
                    $oStdPosicaoAtual->valores->valor_atual = $oStdValorPosicaoAnterior->valores->valor_atual - $oStdPosicaoAtual->valores->valor_total;
                }

                if ($oStdPosicaoAtual->tipo_aditamento == AcordoPosicao::TIPO_ALTERACAO_CESSAO_CONTRATADO ||
                    $oStdPosicaoAtual->tipo_aditamento == AcordoPosicao::TIPO_VIGENCIA) {
                    $oStdPosicaoAtual->valores->valor_total = $oStdValorPosicaoAnterior->valores->valor_total;
                    $oStdPosicaoAtual->valores->quantidade = $oStdValorPosicaoAnterior->valores->quantidade;
                }
                ...
                $oStdPosicaoAtual->dadosContratado = '';
                if ($oStdPosicaoAtual->sigla_tipo_aditamento == "ACC") {
                    $oStdPosicaoAtual->dadosContratado = (object)AlteracaoContratadoRepository::getHistoricoContratado($oAcordoPosicao->getCodigo());
                }
                ...
                $nPercentualAcrescimo = number_format($oValores->percentual_acrescimo, 5, ',','');

                if($oValores->percentual_acrescimo === 0) {

                  $nPercentualAcrescimo = Regra::formatarValor($oValores->percentual_acrescimo);
                }

                $oStdPosicaoAtual->valores->percentual_acrescimo = $oValores->percentual_acrescimo !== null?$nPercentualAcrescimo:$oValores->percentual_acrescimo;
                $oStdPosicaoAtual->valores->valor_reducao        = Regra::formatarValor($oValores->valor_reducao);
                $oStdPosicaoAtual->valores->percentual_reducao   = $oValores->percentual_reducao != null?number_format($oValores->percentual_reducao, 5, ',',''):$oValores->percentual_reducao;
    ...
    private function configurarObjetoLayout(stdClass $oStdDadosAcordo)
    {
        foreach ($oStdDadosAcordo->posicoes as $oStdPosicao) {
            ...
            $dados = $service->buscarDados($oStdDadosAcordo->sequencial_acordo);
            $modalidade = $service->getModalidade($dados->ac63_modalidade)->getSiglaTipoCompraTribunal();

            $oStdImprimir = $this->obterNullObject();
            $oStdImprimir->NR_LICITACAO = $dados->ac63_sequencial == "" ? $oStdDadosAcordo->numero_licitacao : $dados->ac63_numero;
            $oStdImprimir->ANO_LICITACAO = $dados->ac63_sequencial == "" ? $oStdDadosAcordo->ano_licitacao : $dados->ac63_ano;
            $oStdImprimir->CD_TIPO_MODALIDADE = $dados->ac63_sequencial == "" ? $oStdDadosAcordo->sigla_modalidade : $modalidade;
            ...
            $oStdImprimir->TP_DOCUMENTO_ANTERIOR = $oStdPosicao
                ->dadosContratado != "" ? $oStdPosicao->dadosContratado->tipoDocumentoAnterior : '';
            $oStdImprimir->NR_DOCUMENTO_ANTERIOR = $oStdPosicao
                ->dadosContratado != "" ? $oStdPosicao->dadosContratado->numeroDocumentoAnterior : '';
            $oStdImprimir->TP_DOCUMENTO_NOVO = $oStdPosicao
                ->dadosContratado != "" ? $oStdPosicao->dadosContratado->tipoDocumentoNovo : '';
            $oStdImprimir->NR_DOCUMENTO_NOVO = $oStdPosicao
                ->dadosContratado != "" ? $oStdPosicao->dadosContratado->numeroDocumentoNovo : '';
