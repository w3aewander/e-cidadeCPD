...
if (!empty($o40_orgao)) {
    ...
    $rs = db_query($clorcorgao->sql_query_file(db_getsession("DB_anousu"), $o40_orgao));
    $orgao = db_utils::fieldsMemory($rs, 0);
    $filtros[] = "ÓRGÃO: {$orgao->o40_orgao} - $orgao->o40_descr";
}
...
if (!empty($o58_codigo)) {
    ...
    $rs = db_query($clorctiporec->sql_query_file($o58_codigo));
    $recurso = db_utils::fieldsMemory($rs, 0);
    $filtros[] = "RECURSO: {$recurso->o15_recurso} - $recurso->o15_descr | Compl: {$recurso->o15_complemento}";
}
if (!empty($o61_codigo)) {
...
if ($rp == 'somente') {
    $xtipo .= ' and e60_numemp in (select e91_numemp from empresto where e91_anousu = ' . db_getsession("DB_anousu") . ') ';
    $pdf->addTitulo("SOMENTE RP");
}
...
for ($x = 0; $x < $rows; $x++) {
    ...
    if (($pdf->gety() > $pdf->getH() - 40) || $x == 0) {
        ...
        $pdf->Cell(43, $tam, "TIP0", 1, 0, "C", 1);
        $pdf->Cell(17, $tam, "VALOR", 1, 0, "C", 1);
    ...
    $pdf->Cell(43, $tam, $c53_descr, 0, 0, "C", $pre);
    $pdf->Cell(17, $tam, db_formatar($c70_valor, 'f'), 0, 0, "C", $pre);
    ...
    if ($com_mov == 's') {
        ...
        $pdf->multiCell(0, $tam, 'HISTÓRICO DO LANÇAMENTO: ' . $c72_complem, 0, "L", $pre);
        if ($e60_destin != '') {
