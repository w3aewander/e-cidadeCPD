...
class Importacao
{
     ...
     * @return array
     */
    public function getContasNaoExcluidas()
    {
        return $this->estruturaisNaoExcluidos;
    }
    ...
    private function processar()
    {
        ...
        $sSqlSelect = "select c60_codcon from conplano
            ...
            ";//exercicio e estrutural
        $sSqlSelectFilhas = "select c60_estrut, c60_codcon from conplano where c60_anousu = $1
            and substr(c60_estrut, 1, $2) = $3
            ";//nivel e estrutural
    ...
    private function deletaConta(Conta $oConta)
    {
        ...
        $sSqlVerificaLancamento = $oDaoReduzidos->sql_query_razao(
            ...
            "conplanoreduz.*",
            null,
            implode(' and ', $aWhere) . " and c69_anousu >= {$this->oModelo->getExercicio()}"
        );
        ...
        if (pg_num_rows($rsVerificaLancamento) > 0) {
            $this->estruturaisNaoExcluidos[] = $oConta->getEstrutural();
            return false;
        }
        ...
        $sSqlBuscaReduzido = $oDaoReduzidos->sql_query_analitica(
            null,
            null,
            $sCamposReduzidos,
            null,
        ...
        if (!empty($sReduzidos)) {
            $rsDeleteReduzidos = db_query("
                delete from conplanoreduz
                 where c61_reduz in({$sReduzidos})
                   and c61_anousu between {$this->oModelo->getExercicio()} and {$this->iAnoFinal}
            ");
    ...
    ");
        ...
        $rsExcluirAtributos = db_query("
        delete from conplanoatributos
         using conplano
         where conplanoatributos.c120_anousu = conplano.c60_anousu
           and conplanoatributos.c120_conplano = conplano.c60_codcon
           and " . implode(' and ', $aWhere) . "
    ");
...
//                $iCodConta = \db_utils::fieldsMemory($rsConta, 0)->c60_codcon;
    ...
    private function geraContasAnalitica(Conta $oConta, $iCodConta, $iAno = null)
    {
        ...
        $aInstituicoes = $this->instituicoesCadastradas;
