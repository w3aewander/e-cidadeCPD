...
void delay() {
  ...
  fd_set descritores; /* Lista de descritores pro select */
  struct timeval timeout; /* Estrutura que descreve um intervalo de tempo */   
  FD_ZERO(&descritores); /* Inicializa a lista de handles */
  /* Coloca o stdin (teclado) na lista de handles a monitorar */
 // FD_SET(0, &descritores);
  /* Configura o tempo de espera */
  timeout.tv_sec=0; /* Segundos */
  timeout.tv_usec=0.5f; /* Microsegundos */
  /* A funcao select o onde a espera acontece */
...
//  fd_set mestre;
 //pega os parametros no arquivo
  ...
  while(!feof(fp)) {
    ...
    if(!strchr(str,'#')) {
      if(strstr(str,"PORTA_TCPIP"))
        strcpy(portaTcp,strchr(str,'='));
      else if(strstr(str,"PORTA_PARLELA"))
        strcpy(portaParalela,strchr(str,'='));
      else if(strstr(str,"IP_SERVIDOR_PHP"))
        strcpy(ipServidorPHP,strchr(str,'='));      
    }    
  }
  fclose(fp);
  portaTcp[0] = ' ';
  portaParalela[0] = ' ';
  ipServidorPHP[0] = ' ';
  portaTcp[strlen(portaTcp) - 1] = '\0';
  portaParalela[strlen(portaParalela) - 1] = '\0';
  ipServidorPHP[strlen(ipServidorPHP) - 1] = '\0';
  
  printf("E: '%s'\n",portaTcp);
  printf("E: '%s'\n",portaParalela);
  printf("E: '%s'\n",ipServidorPHP);
  PORTA = atoi(portaParalela); //dados
  REGS2 = (PORTA + 1); //status
  REGS3 = (REGS2 + 1); //comandos 
  ...
  while(1) {  // loop principal accept()
    ...
    printf("'%s' == '%s'",(char *)gethostbyname((char *)ipServidorPHP),(char *)gethostbyname((char *)inet_ntoa(outro_end.sin_addr)));
//    if(strcmp((char *)gethostbyname((char *)ipServidorPHP),(char *)gethostbyname((char *)inet_ntoa(outro_end.sin_addr)))) {    
    if(1 > 2) {
      printf("Voce não tem permissao pra acessar esta porta\n");
      close(nova_fd);
    } else {
