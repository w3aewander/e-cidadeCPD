...
?>
			<script>
    	   		if(!confirm("AVISO:\nCGM ja possui uma vistoria lançada para este ano\n\nDados da Vistoria:\nCódigo:<?=$y71_codvist?>\nTipo:<?=$y70_tipovist?>-<?=$y77_descricao?>\nDepartamento:<?=$y70_coddepto?>\n\nDeseja cadastrar uma nova Vistoria?")){
        			(window.CurrentWindow || parent.CurrentWindow).corpo.location = 'fis1_fis_vistorias001.php?cgm=1';
       			}
      		</script>
<?php
...
?>
			<script>
      			if(!confirm("AVISO:\nMatricula ja possui uma vistoria lançada para este ano\n\nDados da Vistoria:\nCódigo:<?=$y71_codvist?>\nTipo:<?=$y70_tipovist?>-<?=$y77_descricao?>\nDepartamento:<?=$y70_coddepto?>\n\nDeseja cadastrar uma nova Vistoria?")){
       				(window.CurrentWindow || parent.CurrentWindow).corpo.location = 'fis1_fis_vistorias001.php?matric=1';
      			}
     		</script>
<?php
...
?><script>
       if(!confirm("AVISO:\nInscrição ja possui uma vistoria lançada para este ano\n\nDados da Vistoria:\nCódigo:<?=$y71_codvist?>\nTipo:<?=$y70_tipovist?>-<?=$y77_descricao?>\nDepartamento:<?=$y70_coddepto?>\n\nDeseja cadastrar uma nova Vistoria?")){
        (window.CurrentWindow || parent.CurrentWindow).corpo.location = 'fis1_fis_vistorias001.php?inscr=1';
       }
      </script>
    <?php
...
?><script>
        if(!confirm("AVISO:\nSanitário ja possui uma vistoria lançada para este ano\n\nDados da Vistoria:\nCódigo:<?=$y71_codvist?>\nTipo:<?=$y70_tipovist?>-<?=$y77_descricao?>\nDepartamento:<?=$y70_coddepto?>\n\nDeseja cadastrar uma nova Vistoria?")){
         (window.CurrentWindow || parent.CurrentWindow).corpo.location = 'fis1_fis_vistorias001.php?sani=1';
        }
       </script>
     <?php
...
?>
<input name="novo" type="button" id="novo" value="Incluir Novo" onclick="parent.location.href='fis1_fis_vistorias005.php?<?=$url?>';" onblur="js_setatabulacao();">
<?php
...
function js_pesquisay70_tipovist(mostra){
  if(mostra==true){
    js_OpenJanelaIframe('','db_iframe_tipovistorias','func_fis_tipovistoriasdepto.php?funcao_js=parent.js_mostratipovistorias1|y77_codtipo|y77_descricao','Pesquisa',true);
  }else{
    js_OpenJanelaIframe('','db_iframe_tipovistorias','func_fis_tipovistoriasdepto.php?pesquisa_chave='+document.form1.y70_tipovist.value+'&funcao_js=parent.js_mostratipovistorias','Pesquisa',false);
  }
}
...
function js_pesquisa(){
  ...
  js_OpenJanelaIframe('','db_iframe_vistorias','func_fis_vistorias.php?funcao_js=parent.js_preenchepesquisa|y70_codvist','Pesquisa',true);
}
...
if ($db_opcao == 2 || $db_opcao == 22) {
	echo " location.href = 'fis1_fis_vistorias002.php?abas=1&chavepesquisa='+chave;";
}
elseif ($db_opcao == 33 || $db_opcao == 3) {
	echo " location.href = 'fis1_fis_vistorias003.php?abas=1&chavepesquisa='+chave;";
}
?>
}
