...
try {
    switch ($oParam->exec) {
        case 'buscarAvaliacao':
            ...
            $avaliacaoEsocial->setLotacao(true);
        ...
        case 'salvarAvaliacao':
            ...
            if (empty($oParam->codLotacao)) {
                throw new BusinessException("Número do Código de Lotação não informado.");
            }

            $aParametros = ['codLotacao' => $oParam->codLotacao];
            ...
            $oAvaliacaoESocial->salvar($iCodigoGrupoPerguntas, "lotacaoTributaria", $aParametros);

            $dao = new cl_rhlotacaotributaria;
            $campos = ['rh268_numcgm', 'rh268_codigolotacao'];
            $where[] = "rh268_numcgm = {$oParam->iCGM}  and rh268_codigolotacao  = '{$oParam->codLotacao}'";
            $sql = $dao->buscaPreenchimento($campos, $where, [], db_getsession("DB_instit"));
            $rsLotacao = db_query($sql);
            if (pg_num_rows($rsLotacao) == 0) {
               $dao->rh268_numcgm        = $oParam->iCGM;
               $dao->rh268_codigolotacao = $oParam->codLotacao;
               $dao->incluir(null);
            }
