...
class cl_arreold {
    ...
    public function sql_query_getReceitasTipo()
    {
        ...
        $sql .="           where arreinstit.k00_instit = ". db_getsession("DB_instit") ." ) as x ";
        $sql .="        inner join termo on v07_parcel = ultimo_parcelamento ";
        $sql .="        inner join arrecad on k00_numpre = v07_numpre ";
        $sql .="  where v07_situacao = 1 ";
        return $sql;
    }

    function alterar_tipo ( $tipo, $where ) {
        $sql  = " update arreold set k00_tipo = $tipo ";
        $sql .= " where $where ";
