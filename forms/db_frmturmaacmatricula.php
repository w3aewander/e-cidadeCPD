  ...
  <table style="width:100%">
    <tr>
      <td valign="top">
        ...
        $chavepri["ed269_aluno"] = isset($ed269_aluno) ? $ed269_aluno : '';
        $chavepri["ed269_i_codigo"] = isset($ed269_i_codigo) ? $ed269_i_codigo : '';
        $chavepri["ed269_i_turmaac"] = isset($ed269_i_turmaac) ? $ed269_i_turmaac : '';
        $chavepri["ed269_d_data"] = isset($ed269_d_data) ? $ed269_d_data : '';
        $chavepri["ed268_c_descr"] =isset($ed268_c_descr) ? $ed268_c_descr : '';
        $chavepri["ed47_v_nome"] = isset($ed47_v_nome) ? $ed47_v_nome : '';
...
function js_validaTurnoTurma(aluno) {
    var oParametros = {};
    oParametros.sExecutar = 'validaTurnoTurma';
    oParametros.codigo_aluno = aluno;
    oParametros.codigo_turma = document.form1.ed269_i_turmaac.value;

    const inputAluno = document.getElementById('ed269_aluno');
    ...
    oRequest.onComplete   = function( oAjax ) {
        ...
        if ( parseInt(oRetorno.iStatus) === 2 ) {
            inputAluno.value = '';
            inputAluno.dispatchEvent(new Event('change'));
            if (oRetorno.status == 3) {
                alert("Não é possível efetuar matrícula, horário incompatível.");
            } else {
                alert(oRetorno.sMensagem);
            }
        }
    };
    js_divCarregando("Validando turno da turma" , 'msgBoxB');
