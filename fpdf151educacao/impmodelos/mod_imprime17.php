...
function retornaProcessoCompra($nosolicitacao){
  $sql = pg_query("SELECT pc10_numero, pc80_codproc from pcprocitem inner join pcproc on pcproc.pc80_codproc = pcprocitem.pc81_codproc inner join solicitem on solicitem.pc11_codigo = pcprocitem.pc81_solicitem inner join solicita on solicita.pc10_numero = solicitem.pc11_numero inner join db_depart on db_depart.coddepto = solicita.pc10_depto left join solicitemunid on solicitemunid.pc17_codigo = solicitem.pc11_codigo left join matunid on matunid.m61_codmatunid = solicitemunid.pc17_unid left join db_usuarios on pcproc.pc80_usuario = db_usuarios.id_usuario left join solicitempcmater on solicitempcmater.pc16_solicitem = solicitem.pc11_codigo left join pcmater on pcmater.pc01_codmater = solicitempcmater.pc16_codmater left join pcsubgrupo on pcsubgrupo.pc04_codsubgrupo = pcmater.pc01_codsubgrupo left join pctipo on pctipo.pc05_codtipo = pcsubgrupo.pc04_codtipo left join solicitemele on solicitemele.pc18_solicitem = solicitem.pc11_codigo left join orcelemento on orcelemento.o56_codele = solicitemele.pc18_codele and orcelemento.o56_anousu = 2022 left join empautitempcprocitem on empautitempcprocitem.e73_pcprocitem = pcprocitem.pc81_codprocitem left join empautitem on empautitem.e55_autori = empautitempcprocitem.e73_autori and empautitem.e55_sequen = empautitempcprocitem.e73_sequen left join empautoriza on empautoriza.e54_autori = empautitem.e55_autori left join cgm on empautoriza.e54_numcgm = cgm.z01_numcgm left join empempaut on empempaut.e61_autori = empautitem.e55_autori left join empempenho on empempenho.e60_numemp = empempaut.e61_numemp left join liclicitem on liclicitem.l21_codpcprocitem = pcprocitem.pc81_codprocitem left join processocompraloteitem on processocompraloteitem.pc69_pcprocitem = pcprocitem.pc81_codprocitem left join processocompralote on processocompralote.pc68_sequencial = processocompraloteitem.pc69_processocompralote where pc10_numero={$nosolicitacao}");
  $resultado = pg_fetch_all($sql);
  return $resultado[0]["pc80_codproc"];
}

$nopc = "";
if($this->Snumero){
  $nopc = retornaProcessoCompra($this->Snumero);  
}
...
//var_dump($this->Snumero); die("Confere 38397");
...
$this->objpdf->text($xcol +115, $xlin +18, ':  '.$this->Stipcom);
...
$this->objpdf->text($xcol +115, $xlin +22, ': '.@$this->processoAdministrativo);
$this->objpdf->text($xcol +165, $xlin +22, ': '.$nopc);
...
for ($i = 0; $i < $this->linhasdasdotac; $i ++) {
  ...
  //string(9) "564649 - " string(79) "VERGALHÃO C.A 60, BARRA RETA DE 12 METROS DE COMPRIMENTO POR 4,2MM DE DIÂMETRO," Confere
  ...
  if(isset ($resum) && $resum != "" && (isset ($scodpcmater) && trim($scodpcmater) != "")) {
    ...
    $mostra = $xlin;
    //++
    $iAlturaPagina = $this->objpdf->h  - 30 * 2;
    while ( $resum != '') {
...
*/
  ...
  //$this->objpdf->text($xcol, $xlin +268, 'Quantidade Total: ');
  $this->objpdf->rect($xcol, $xlin +262, 142, 10, 2, 'DF', '34');
  $this->objpdf->rect($xcol +142, $xlin +262, 30, 10, 2, 'DF', '34');
  $this->objpdf->text($xcol +12, $xlin +268, $qtdeProdutos);
  $this->objpdf->rect($xcol +172, $xlin +262, 30, 10, 2, 'F', '340');
  
  //$this->objpdf->text($xcol +60, $xlin +268, $qtdeProdutos);
  $this->objpdf->text($xcol +120, $xlin +268, 'T O T A L');
  $this->objpdf->text($xcol +180, $xlin +268, db_formatar($xtotal, "f"));
}
