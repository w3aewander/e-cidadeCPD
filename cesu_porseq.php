...
function busca($sequencial){
  $sql = pg_query("SELECT id, e60_numemp, noempenho, nocertificado, e50_codord, CASE WHEN suspensao = 1 THEN 'Sim' ELSE 'Não' END as suspensao, e60_instit, justificativasuspensao, justificativaretiradasuspensao FROM certificacaoconformidade WHERE e60_numemp = '{$sequencial}' AND e50_codord is not null ORDER BY e60_instit, id");
