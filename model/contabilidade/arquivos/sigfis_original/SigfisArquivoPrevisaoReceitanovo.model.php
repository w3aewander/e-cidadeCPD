...
//        if (substr($sEstrut,1,8) == "00000000") continue;
        if ( $iQuantReceita == 0 ) continue;
...
//		 $sEstrut = str_pad(substr($sEstrut, 0,  8),  8, " ", STR_PAD_LEFT);
            $sEstrut = str_pad('9' . substr($sEstrut, 2, 7),  8, " ", STR_PAD_LEFT);
//$this->addLog($sEstrut);
          }else{
            $sEstrut = str_pad(substr($sEstrut, 1,  8),  8, " ", STR_PAD_LEFT);
          }
...
//          echo "receitatce: $oVinculo->receitatce - saldo_inicial: $aReceita->saldo_inicial \n";
    ...
    if (count($aReceitaSoma) > 0) {
      ...
      foreach ($aReceitaSoma as $sFonte => $nValor) {
        ...
        $oDados->Cd_ItemReceita = str_pad($sFonte,  8, " ", STR_PAD_LEFT);
//$this->addLog($sFonte . "-");
        //$oDados->Cd_ItemReceita     = str_pad(substr($sFonte,1,8),  8, ' ', STR_PAD_LEFT);
//        if(substr($sFonte, 0,  1) == '9' ) {
//          $oDados->Cd_ItemReceita = str_pad( '9' . substr($sFonte, 2,  7),  8, " ", STR_PAD_LEFT);
//        }else{
//          $oDados->Cd_ItemReceita = str_pad(substr($sFonte, 1,  8),  8, " ", STR_PAD_LEFT);
//        }
