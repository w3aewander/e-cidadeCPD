...
class AnexoVII
{
   ...
   */
    private function totalizarLinhaSintetica(LinhaAnexoVII &$oLinha, \stdClass $oStdLinha)
    {
        ...
        $oLinha->setValorProcessadoEmExerciciosAnteriores(
            $oLinha->getValorProcessadoEmExerciciosAnteriores() + $oStdLinha->inscricao_ant
        );
        $oLinha->setValorProcessadoNoExercicioAnterior(
            $oLinha->getValorProcessadoNoExercicioAnterior() + $oStdLinha->valor_processado
        );
        ...
        $oLinha->setValorNaoProcessadoEmExerciciosAnteriores(
            $oLinha->getValorNaoProcessadoEmExerciciosAnteriores() + $oStdLinha->valor_nao_processado_ant
        );
        $oLinha->setValorNaoProcessadoNoExercicioAnterior(
            $oLinha->getValorNaoProcessadoNoExercicioAnterior() + $oStdLinha->valor_nao_processado
        );
        ...
        $oLinha->setValorCanceladoNaoProcessado(
            $oLinha->getValorCanceladoNaoProcessado() + $oStdLinha->vlranuliqnaoproc
        );
            ...
            switch ($oStdRestos->tipo_instituicao) {
                case 2:
                    if (empty($this->aPoderLegislativo[$sIndice])) {
                        ...
                        $this->aPoderLegislativo[$sIndice] = $this->agruparValores(
                            $this->aPoderLegislativo[$sIndice],
                            $oStdRestos
                        );
                    }
                    break;
                ...
                case 13:
                    if (empty($this->aPoderJudiciario[$sIndice])) {
                        ...
                        $this->aPoderJudiciario[$sIndice] = $this->agruparValores(
                            $this->aPoderJudiciario[$sIndice],
                            $oStdRestos
                        );
                    }
                    break;
                ...
                case 101:
                    if (empty($this->aMinisterioPublico[$sIndice])) {
                        ...
                        $this->aMinisterioPublico[$sIndice] = $this->agruparValores(
                            $this->aMinisterioPublico[$sIndice],
                            $oStdRestos
                        );
                    }
                    break;
                ...
                default:
                    if (empty($this->aPoderExecutivo[$sIndice])) {
                        ...
                        $this->aPoderExecutivo[$sIndice] = $this->agruparValores(
                            $this->aPoderExecutivo[$sIndice],
                            $oStdRestos
                        );
                    }
            }
   ...
   */
    private function processar()
    {
        ...
        $oLinhaTotal->setValorProcessadoEmExerciciosAnteriores(
            $oLinhaExcetoIntra->getValorProcessadoEmExerciciosAnteriores()
            + $oLinhaIntra->getValorProcessadoEmExerciciosAnteriores()
        );
        $oLinhaTotal->setValorProcessadoNoExercicioAnterior(
            $oLinhaExcetoIntra->getValorProcessadoNoExercicioAnterior()
            + $oLinhaIntra->getValorProcessadoNoExercicioAnterior()
        );
        $oLinhaTotal->setValorPagoProcessado(
            $oLinhaExcetoIntra->getValorPagoProcessado() + $oLinhaIntra->getValorPagoProcessado()
        );
        $oLinhaTotal->setValorCanceladoProcessado(
            $oLinhaExcetoIntra->getValorCanceladoProcessado() + $oLinhaIntra->getValorCanceladoProcessado()
        );
        $oLinhaTotal->setSaldoProcessado(
            $oLinhaExcetoIntra->getSaldoProcessado() + $oLinhaIntra->getSaldoProcessado()
        );
        $oLinhaTotal->setValorNaoProcessadoEmExerciciosAnteriores(
            $oLinhaExcetoIntra->getValorNaoProcessadoEmExerciciosAnteriores()
            + $oLinhaIntra->getValorNaoProcessadoEmExerciciosAnteriores()
        );
        $oLinhaTotal->setValorNaoProcessadoNoExercicioAnterior(
            $oLinhaExcetoIntra->getValorNaoProcessadoNoExercicioAnterior()
            + $oLinhaIntra->getValorNaoProcessadoNoExercicioAnterior()
        );
        $oLinhaTotal->setValorLiquidadoNaoProcessado(
            $oLinhaExcetoIntra->getValorLiquidadoNaoProcessado()
            + $oLinhaIntra->getValorLiquidadoNaoProcessado()
        );
        $oLinhaTotal->setValorPagoNaoProcessado(
            $oLinhaExcetoIntra->getValorPagoNaoProcessado() + $oLinhaIntra->getValorPagoNaoProcessado()
        );
        $oLinhaTotal->setValorCanceladoNaoProcessado(
            $oLinhaExcetoIntra->getValorCanceladoNaoProcessado() + $oLinhaIntra->getValorCanceladoNaoProcessado()
        );
        $oLinhaTotal->setSaldoNaoProcessado(
            $oLinhaExcetoIntra->getSaldoNaoProcessado() + $oLinhaIntra->getSaldoNaoProcessado()
        );
   ...
   */
    private function queryBuilder($sWhere)
    {
        ...
        $sSqlBuscaRP = $oDaoRestos->sql_rp_novo(
            $this->iAno,
            $sInstituicoes,
      ...
      select db21_tipoinstit     as tipo_instituicao
             ...
             ,sum(
              (valor_nao_processado + valor_nao_processado_ant) - vlranuliqnaoproc - vlrpagnproc
             ) as rp_n_proc_saldo
             ,sum(
              ((inscricao_ant + valor_processado) - vlrpag - vlranuliq) +
              ((valor_nao_processado + valor_nao_processado_ant) - vlranuliqnaoproc - vlrpagnproc)
             ) as saldo_total
   ...
   */
    ...
    {
        $valorProcessadoEmExerciciosAnteriores = $oLinhaAnalitica->getValorProcessadoEmExerciciosAnteriores();
        $valorProcessadoNoExercicioAnterior = $oLinhaAnalitica->getValorProcessadoNoExercicioAnterior();
        $nProcessadoInscrito = $valorProcessadoEmExerciciosAnteriores + $valorProcessadoNoExercicioAnterior;
        $valorNaoProcessadoEmExerciciosAnteriores = $oLinhaAnalitica->getValorNaoProcessadoEmExerciciosAnteriores();
        $valorNaoProcessadoNoExercicioAnterior = $oLinhaAnalitica->getValorNaoProcessadoNoExercicioAnterior();
        $nNaoProcessadoInscrito = $valorNaoProcessadoEmExerciciosAnteriores + $valorNaoProcessadoNoExercicioAnterior;

        $oStdLinhaImpressao->nProcessadoInscrito     += $nProcessadoInscrito;
        ...
        $oStdLinhaImpressao->nNaoProcessadoInscrito  += $nNaoProcessadoInscrito;
