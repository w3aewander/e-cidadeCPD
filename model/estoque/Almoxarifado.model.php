...
class Almoxarifado extends DBDepartamento
{
    ...
    public function __construct($iCodigoDepartamento)
    {
        ...
        $oDaoDBAlmox = new cl_db_almox;
        $sSqlBuscaAlmoxarifado = $oDaoDBAlmox->sql_query_file(
            null,
            "m91_codigo",
            null,
            "m91_depto = {$this->iCodigoDepartamento}"
        );
    ...
    public function receberTransferencia($iTransferencia)
    {
        ...
        $sSqlItensTransferencia = $oDaoMatestoqueIni->sql_query_mater(
            $iTransferencia,
            "distinct m70_codigo,
             m70_quant,
             m70_valor,
        ...
        $utilizaIntegracaoBnafar = FarmaciaHelper::utilizaIntegracaoBnafar();
        ...
        for ($i = 0; $i < $iItensTransferencia; $i++) {
            ...
            if ($utilizaIntegracaoBnafar && FarmaciaHelper::isMaterialFarmacia($oDadosItem->m60_codmater)) {
                FarmaciaHelper::validaQuantidade($oDadosItem->m82_quant);
        ...
        $oDaoMatEstoqueItemFabric = new cl_matestoqueitemfabric;
        for ($i = 0; $i < $iItensTransferencia; $i++) {
            $oDadosItem = db_utils::fieldsmemory($rsItensTransferencia, $i);
            $sSqlEstoque = $oDaoMatEstoque->sql_query_file(
                null,
                "m70_codigo,m70_quant,m70_valor",
                "",
                "m70_codmatmater={$oDadosItem->m60_codmater} and m70_coddepto={$this->iCodigoDepartamento}"
            );
            ...
            if ($oDadosItem->m77_sequencial != '') {
                ...
                if ($oDaoMatEstoqueItemLote->erro_status == 0) {
                    $erro_msg = $oDaoMatEstoqueItemLote->erro_msg;
                    throw new Exception($erro_msg);
                }
            }
            ...
            if (!empty($oDadosItem->m76_sequencial)) {
                $oDaoMatEstoqueItemFabric->m78_matestoqueitem = $iCodigoEstoqueItem;
                $oDaoMatEstoqueItemFabric->m78_matfabricante = $oDadosItem->m76_sequencial;
                $oDaoMatEstoqueItemFabric->incluir(null);
                if ($oDaoMatEstoqueItemFabric->erro_status == 0) {
                    throw new Exception($oDaoMatEstoqueItemFabric->erro_msg);
                }
            }
        }
        ...
        /**
         * Caso possua material da farmácia e a integração BNAFAR esteja em uso, faz a inclusão das duas
         * movimentações na tabela do BNAFAR
         */
        if ($hasMaterialFarmacia) {
            EstoqueMovimentacaoBnafarService::salvarTransferencia(
                $ItransferenciaSaida,
                $oDadosTransferencia->m80_coddepto,
                $ItransferenciaEntrada,
                $this->iCodigoDepartamento
            );
        }
     ...
     *
     * @param integer $iTransferencia código da transferencia
     * @return Almoxarifado
     * @throws Exception
     */
    public function cancelarTransferencia($iTransferencia)
    {
        ...
        $iItensTransferencia = $oDaoMatestoqueIni->numrows;
        $oDaoMatestoqueItem = new cl_matestoqueitem;
        $oDaoMatestoqueIiniMei = new cl_matestoqueinimei;
    ...
    private function executarLancamentosContabeis($iCodigoTipoDocumento, $oLancamentoAuxiliar, $dtLancamento = null)
    {
        ...
        if ($iCodigoTipoDocumento != 403) {
            $oDocumentoContabil->setValorVariavel(
                "[codigomovimentacaoestoque]",
                $oLancamentoAuxiliar->getCodigoMovimentacaoEstoque()
            );
        }
