...
if ($oGet->cboAgrupamento == 1) {
...
} else if ($oGet->cboAgrupamento == 6) {

  $sFiltroAgrupamento .= " Estrutural Receita ";
  ...
  switch ($oGet->cboOrdenacao) {
    ...
    case 1: //Receita
      $sFiltroOrdenacao  .= " Estrutural Receita ";
      $sOrdenar          .= " k02_estorc ";
      break;

    case 2: //Valor
      $sFiltroOrdenacao .= " Valor ";
      $sOrdenar   .= " k02_estorc,valor_pago ";
      $sOrdenar   .= $sAscDesc;
      $lOrderValor = true;
      break;
...
$sSql .= "            left  join taborc     on taborc.k02_codigo     = tabrec.k02_codigo            ";
$sSql .= "                                 and taborc.k02_anousu     = " . db_getsession("DB_anousu");
$sSql .= "            left join orcreceita  on taborc.k02_anousu = o70_anousu  ";
$sSql .= "                                 and taborc.k02_codrec = o70_codrec";
$sSql .= "           left join orcfontes    on o57_anousu = o70_anousu   ";
$sSql .= "                                 and o57_codfon = o70_codfon";
...
for ($i = 0; $i < $iNumRows; $i++) {
  ...
  switch ($iTipoRelatorio) {
    ...
    case 6:
      ...
      if (isset($aPagamentos[$oCgmPagamento->estrutural])) {

        $aPagamentos[$oCgmPagamento->estrutural]->valorPago      += $oCgmPagamento->valorPago;
        $aPagamentos[$oCgmPagamento->estrutural]->totalRegistros += 1;
        $aPagamentos[$oCgmPagamento->estrutural]->totalGeral     += $oCgmPagamento->valorPago;
      } else {

        $aPagamentos[$oCgmPagamento->estrutural] = new stdClass;

        $aPagamentos[$oCgmPagamento->estrutural]->valorPago        = $oCgmPagamento->valorPago;
        $aPagamentos[$oCgmPagamento->estrutural]->estrutural       = $oCgmPagamento->estrutural;
        $aPagamentos[$oCgmPagamento->estrutural]->descricaoEstrutural = $oCgmPagamento->descrestrutural;
        $aPagamentos[$oCgmPagamento->estrutural]->totalRegistros   = 1;
        $aPagamentos[$oCgmPagamento->estrutural]->totalGeral       = $oCgmPagamento->valorPago;
      }
...
if ($cboAgrupamento != 2 && $cboAgrupamento != 6) {
  ...
  foreach ($aPagamentos as $iCgm => $oDados) {
    ...
    foreach ($oDados->dados as $iReceita => $oReceita ) {
      ...
      if (isset($aTotais['estrutural'][$oReceita->estrutural])) {
        $aTotais['estrutural'][$oReceita->estrutural]   += $oReceita->valorPago;

      } else {

        $aTotais['estrutural'][$oReceita->estrutural]    = $oReceita->valorPago;
        $aTotais['descricaoestrutural'][$oReceita->estrutural] = $oReceita->descricaoEstrutural;
      }
   ...
   foreach ($aPagamentos as $oReceita) {
      ...
      if (isset($aTotais['codigoreceita'][$oReceita->codigoReceita])) {
        ...
        $aTotais['estrutural'][$oReceita->codigoReceita]    = $oReceita->valorPago;
        $aTotais['descricaoestrutural'][$oReceita->codigoReceita] = $oReceita->descricaoReceita;
      }
    ...
    switch ($iTipoRelatorio){
      ...
      case 5:
        ...
        if ($lNovaPagina) {
          ...
          $pdf->AddPage();
          $lNovaPagina = false;
          cabecalhoEstruturalReceita($pdf, $alt);
          $pdf->ln(3);
        }
          //Imprime os dados do relatorio

          $pdf->setfont('arial','',7);
          $pdf->cell(25, $alt, $oCgm->estrutural             , 0, 0, "R", 0);
          $pdf->cell(145, $alt, $oCgm->descricaoEstrutural         , 0, 0, "L", 0);
          $pdf->cell(22, $alt, db_formatar($oCgm->valorPago,'f'), 0, 1, "R", 0);
        ...
        $pdf->cell(25, $alt, "Registros:"                      , 0, 0, "R", 1);
        $pdf->cell(15, $alt, $oCgm->totalRegistros             , 0, 0, "L", 1);
        $pdf->cell(20, $alt, "Total Estrutural Receita:"                  , 0, 0, "R", 1);
        $pdf->cell(22, $alt, db_formatar($oCgm->totalGeral,"f"), 0, 1, "R", 1);
        $pdf->Ln(3);
        $pdf->SetFont("Arial", "", 8);
...
function cabecalhoEstruturalReceita($pdf, $alt) {

  $pdf->setfont('arial','b',8);
  $pdf->cell(25 , $alt, "Estrutural"     , 1, 0, "C", 1);
  $pdf->cell(145, $alt, "Descrição"  , 1, 0, "C", 1);
  $pdf->cell(22 , $alt, "Valor Pago"         , 1, 1, "C", 1);
