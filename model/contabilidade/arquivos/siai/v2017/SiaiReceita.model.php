...
class SiaiReceita extends SiaiArquivoBase
{
    ...
    public function gerarDados()
    {
        ...
        $oDaoOrcTipoRec    = db_utils::getDao("orctiporec");
            ...
            $sSqlOrcTipoRec = "select distinct 
			                          fonte_tce as o15_codigo, 
			                          o15_descr 
								 from plugins.deparafontetce 
								      inner join orctiporec on o15_codigo = fonte_orcamento
								where exercicio = {$this->iAno} 
								order by fonte_tce";
            $rsOrcTipoRec      = pg_query($sSqlOrcTipoRec);
        ...
        if ($iLinhasOrcTipoRec > 0) {
            ...
            for ($i = 0; $i < $iLinhasOrcTipoRec; $i ++) {
                ...
                $sql_receita = "select substr(coalesce(rec_tce, o57_fonte), 1, 9) as elemento,
									   ...
									   inner join (select distinct fonte_orcamento, fonte_tce, exercicio 
									                 from plugins.deparafontetce 
									   	            where fonte_tce = '{$oDadosOrcTipoRec->o15_codigo}'
									    	  		  and exercicio = {$this->iAno} limit 1) as de_para_fonte on de_para_fonte.exercicio = o70_anousu and orcreceita.o70_codigo = de_para_fonte.fonte_orcamento::integer 
                ...
                for ($iInd = 0; $iInd < $iLinhasOrcReceita; $iInd ++) {
                    ...
                    if ($oDadosOrcReceita->elemento == "413271000") {
                            continue;
                    }
