 ...
 */
class cl_esoacidentetrabalho extends DAOBasica
{
    public function __construct()
    {
        parent::__construct("esocial.esoacidentetrabalho");
    }
    ...
    public function buscaRespostasPorPergunta($pergunta, $preenchimento, $campos = "*", $ordem = null)
    {
        $sql  = <<<SQL
            select {$campos}
            from esoacidentetrabalho
                inner join avaliacaogruporesposta on db107_sequencial = eso36_avaliacaogruporesposta
...
SQL;
    ...
    public function buscaPreenchimento(array $campos, array $where, array $order, $instit = null)
    {
        ...
        $sql = <<<SQL
            select {$campos}
            from esoacidentetrabalho
                join avaliacaogruporesposta on avaliacaogruporesposta.db107_sequencial = esoacidentetrabalho.eso36_avaliacaogruporesposta
                ...
                join cgm on z01_cgccpf = esoacidentetrabalho.eso36_cpf
SQL;
        if($instit) {
        ...
        $where[] = " eso36_empregador = {$cgmInstituicao} ";
    ...
    public function avaliacaoPreenchida(array $campos = array('*'), array $where = array(), $outrosComandos = null)
    {
        ...
        $sql = <<<SQL
            select {$campos}
            from esoacidentetrabalho
                join avaliacaogruporesposta on db107_sequencial = eso36_avaliacaogruporesposta
                ...
                join cgm on eso36_empregador = z01_numcgm
SQL;
        $instituicaoCodigo = db_getsession("DB_instit");
        $cgmInstituicao = \InstituicaoRepository::getInstituicaoByCodigo($instituicaoCodigo)->getCgm()->getCodigo();
        $where[] = " eso36_empregador = {$cgmInstituicao} ";
