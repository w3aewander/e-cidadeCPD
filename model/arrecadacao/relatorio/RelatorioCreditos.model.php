...
class RelatorioCreditos
{
   ...
   */
    private function adicionarCabecalho()
    {
        ...
        $this->oPdf->cell(15, 4, $sOrigem, 1, 0, PDFDocument::ALIGN_CENTER, 1);
        $this->oPdf->cell(85, 4, "Nome", 1, 0, PDFDocument::ALIGN_CENTER, 1);
        $this->oPdf->cell(55, 4, "Tipo de Débito", 1, 0, PDFDocument::ALIGN_CENTER, 1);
        $this->oPdf->cell(15, 4, "Valor", 1, 0, PDFDocument::ALIGN_CENTER, 1);
        $this->oPdf->cell(20, 4, "Valor Disponível", 1, 1, PDFDocument::ALIGN_CENTER, 1);
   ...
   */
    public function emitir()
    {
        ...
        for ($iIndice = 0; $iIndice < $iTotalCreditos; $iIndice++) {
            ...
            $this->oPdf->cell(15, 4, $oCredito->id, 0, 0, PDFDocument::ALIGN_CENTER, $lPreencher);
            $this->oPdf->cell(85, 4, $oCredito->nome, 0, 0, PDFDocument::ALIGN_LEFT, $lPreencher);
            if ($oCredito->origem==null) {
                $this->oPdf->cell(55, 4, "CRÉDITO MANUAL", 0, 0, PDFDocument::ALIGN_CENTER, $lPreencher);
            } else {
                $this->oPdf->cell(55, 4, $oCredito->origem, 0, 0, PDFDocument::ALIGN_CENTER, $lPreencher);
            }
            $this->oPdf->cell(15, 4, db_formatar($oCredito->valor, 'f'), 0, 0, PDFDocument::ALIGN_RIGHT, $lPreencher);
            $this->oPdf->cell(20, 4, db_formatar($oCredito->valordisponivel, 'f'), 0, 1, PDFDocument::ALIGN_RIGHT, $lPreencher);
   ...
   */
    private function getDados()
    {
        ...
        switch ($this->oFiltros->tipo_origem) {
            case "matric":
                $sCampos = "distinct arrematric.k00_matric as id, (select rvnome  from fc_busca_envolvidos(true, {$db21_regracgmiptu}, 'M', arrematric.k00_matric) limit 1) as nome, ";
                $sJoins .= " inner join arrematric on arrematric.k00_numpre = abatimentorecibo.k127_numprerecibo ";
                break;

            case "numcgm":
                $sCampos = "distinct arrenumcgm.k00_numcgm as id, cgm.z01_nome as nome, ";
            ...
            case "inscr":
                $sCampos = "distinct arreinscr.k00_inscr as id, (select rvnome  from fc_busca_envolvidos(true, {$db21_regracgmiptu}, 'I', arreinscr.k00_inscr) limit 1) as nome, ";
        ...
        $sCampos .= " arretipo.k00_descr as origem, k125_valordisponivel as valordisponivel, k125_valor as valor";
        $sSql = "select {$sCampos}
    ...
    ";
        ...
        $sSql .= " and k125_valor > 0";
        $sSql .= " and (k125_datalanc + ((select coalesce(min(case when k155_tempovalidade = '' then null else k155_tempovalidade end::integer), 99999999)";
        $sSql .= "      from abatimentoregracompensacao";
        $sSql .= "          inner join regracompensacao on k155_sequencial = k156_regracompensacao";
        $sSql .= "      where k156_abatimento = abatimento.k125_sequencial)::integer||' days')::interval) >= CURRENT_DATE";
        $sSql .= "          and k125_valordisponivel > 0";
        $sSql .= " group by id, origem, nome, valor, valordisponivel, recibo.k00_numpre,";
