 ...
 */
class LancamentoAuxiliarArrecadacaoReceitaExtraOrcamentaria extends LancamentoAuxiliarBase
    implements ILancamentoAuxiliar
{
     ...
     * @return LancamentoAuxiliarArrecadacaoReceitaExtraOrcamentaria
     * @throws Exception
     */
    public static function getInstance($codigoLancamento)
    {

        $consultaAutenticacao = db_query("
           select k12_valor,
                  k12_estorn,
                  k12_id,
                  k12_data,
             ...
             from conlancamcorrente
                  join corrente on (k12_id, k12_data, k12_autent) = (c86_id, c86_data, c86_autent)
                  join corplacaixa on (k82_id, k82_data, k82_autent) = (k12_id, k12_data, k12_autent)
                  join placaixarec on placaixarec.k81_seqpla = corplacaixa.k82_seqpla
                  join tabplan on tabplan.k02_codigo = placaixarec.k81_receita
                              and tabplan.k02_anousu = extract(year from k12_data)
                  join conlancamcompl on c72_codlan = c86_conlancam
                  join conlancamdoc on c71_codlan = c86_conlancam
            where c86_conlancam = {$codigoLancamento};");
        if (!$consultaAutenticacao) {
            throw new Exception("Ocorreu um erro ao consultar a autenticação.");
        }

        $stdInformacao = db_utils::fieldsMemory($consultaAutenticacao, 0);
        $estorno = $stdInformacao->k12_estorn === 't';

        $oLancamentoAuxiliar = new LancamentoAuxiliarArrecadacaoReceitaExtraOrcamentaria();
        $oLancamentoAuxiliar->setValorTotal(abs($stdInformacao->k12_valor));
        $oLancamentoAuxiliar->setContaCredito($stdInformacao->k02_reduz);
        $oLancamentoAuxiliar->setContaDebito($stdInformacao->k12_conta);
        $oLancamentoAuxiliar->setEstorno($estorno);
        $oLancamentoAuxiliar->setAutenticacao($stdInformacao->k12_id);
        $oLancamentoAuxiliar->setDataAutenticacao($stdInformacao->k12_data);
        $oLancamentoAuxiliar->setAutenticadora($stdInformacao->k12_autent);
