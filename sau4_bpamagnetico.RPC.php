...
try {
  ...
  switch ($oParam->exec) {
      ...
      if ( empty($oParam->iFechamento) ) {
        throw new ParameterException("Competência não foi informada!");
      }
      ...
      /** PLUGIN BPA MAGNETICO GERAR ARQUIVO MODIFICANDO A LINHA ABAIXO */
  	  if ($oParam->sTipo == '02') {
  	    $iTipoLayout = BPAMagnetico::BPA_INDIVIDUAL;
  	  }

  	  $sNomeArquivo            = ECIDADE_PATH . "/tmp/{$oParam->sNomeArquivo}";
