...
class DocumentoEventoLicitacao {
  ...
  public function salvar() {
    ...
    if ($this->getArquivoTemporario()) {
      ...
      $iTamanhoArquivo = filesize($this->getArquivoTemporario());
      ...
      if ( $iTamanhoArquivo > $iTamanhoMaximo ) {
        ...
        throw new FileException("Tamanho do Arquivo Excede o Limite Permitido. Tamanho do arquivo: {$iTamanhoArquivo}. 
      ...
      if($this->getNomeArquivo() != null && $this->getNomeArquivo() != "") {
        ...
        if ($this->iCodigo) { 
          ...
          $sSqlValidaExistencia = "select * 
                               ...
                               where l47_sequencial   <> {$this->iCodigo}
                                 and upper(l47_nomearquivo)  = upper('{$this->getNomeArquivo()}')";
          ...
          $sSqlValidaExistencia = "select * 
                                     from licitacao.liclicitaeventodocumento
                                    where upper(l47_nomearquivo) = upper('{$this->getNomeArquivo()}')";
        }
      }
