...
foreach ($aEtapas as $oEtapaTurma) {
    ...
    if(count($listaEtapa) > 1){
        $tituloEtapa = $oGet->iEtapa;
    }
    if (in_array($oEtapaTurma->getEtapa()->getCodigo(), $arrayEtapa)) {
        $oEtapSelecionada = $oEtapaTurma;
    }
}
$arrayEtapa = array_values($oEtapaLista[$oGet->iTurma]);
$descEtapa = implode(',',$arrayEtapa );
...
$oDadosRelatorio->sEtapa             = $descEtapa;
...
$iNumeroAlunoNaTurma    = count($oTurma->getAlunosMatriculadosNasTurmasPorSerie($oEtapaT));
...
//pdf
    ...
    foreach ($aRegenciasSelecionadas as $oRegencia) {
        ...
        foreach ($oTurma->getAlunosMatriculadosNasTurmasPorSerie($oEtapaT) as $oMatricula) {
            ...
            $oPdf->Cell(14, 4, $oMatricula->getEtapaDeOrigem()->getNome(), 1, 0, 'C');
            $oPdf->SetFont('arial', '', 7);
