...
<table border="0">
          ...
          <td colspan="4">
            <div>
              <fieldset>
                ...
                <?php
                  db_textarea('observacao', 3, 90, "", true, 'text', $db_opcao);
                ?>
            </fieldset>
            </div>
          </td>
  ...
  <tr>
    <td colspan="2" align="center" height="5%">
      <br>
        <input name="acao" type="button"  id="acao" value="Incluir" onClick="js_verificaAcao(this.value);" tabindex="6">
...
function js_getDadosTela(){
  ...
  oDadosRubrica.observacao = $F('observacao');
...
function js_carregaGridRubricas(aRubricas){
  ...
  for ( var iInd=0; iInd < iLinhasRubricas; iInd++ ) {
    ...
    sLinhasRubricas += '  <td style="text-align:right"  class="linhagrid" nowrap>'+js_formatar(aRubricas[iInd].r90_valor.urlDecode(),'f')+'&nbsp;</td>';
      ...
      sLinhasRubricas += '    <input type="button" name="alterar" value="Alterar" onClick="js_telaAlterarExcluir(' + iInd + ',\'Alterar\')">';
      sLinhasRubricas += '  </td>';
      sLinhasRubricas += '  <td class="linhagrid">';
      sLinhasRubricas += '    <input type="button" name="excluir" value="Excluir" onClick="js_telaAlterarExcluir(' + iInd + ',\'Excluir\')">';
      sLinhasRubricas += "    <input type='hidden' id='obj" + aRubricas[iInd].r90_rubric.urlDecode() + "'  value='"+iInd+"'";
      sLinhasRubricas += '  </td>';
    sLinhasRubricas += '</tr>';
...
function js_telaAlterarExcluir(iInd,sAcao){

  var doc          = document.form1;
  var oDadosRubric =  rubricas[iInd];
  var linha = "linha" +rubricas[iInd].r90_rubric.urlDecode();
  ...
  doc.observacao.value     = oDadosRubric.observacao.urlDecode();
...
function js_validaRubrica(){
  ...
  if ( js_verificaCampos() ) {
    ...
    var sQuery  = 'sMethod=validarRubricas';
        sQuery += '&sTipoPonto='+sTipoPonto;
        sQuery += '&oDadosRubrica='+Object.toJSON(oDadosRubrica);
    var oAjax   = new Ajax.Request( sUrl, {
                                            method: 'post',
                                            parameters: sQuery,
...
function js_repassarValores(){
  ...
  if ( js_verificaCampos() ) {
    ...
    var sQuery  = 'sMethod=repassarValores';
        sQuery += '&sTipoPonto='+sTipoPonto;
        sQuery += '&oDadosRubrica='+Object.toJSON(oDadosPonto);
...
function js_incluirRubricasAutomaticas(aListaRubricas,lRepasse){
  ...
  js_divCarregando('Incluindo Rubricas Automáticas...','msgBox');
    ...
    var sQuery  = 'sMethod=incluirRubricasAutomaticas';
        ...
        sQuery += '&dtDataAdm='+document.form1.data_de_admissao.value;
        sQuery += '&aObjListaRubricas='+Object.toJSON(aListaRubricas);
...
function js_calculaDataLimit(){
  ...
  var doc       = document.form1;
    var iQuant = Number(doc.r90_quant.value);

  if ( doc.rh27_presta.value == 't' && doc.rh27_limdat.value == 't' ) {

      var iMesAtu = Number(doc.r90_mesusu.value);
      var iAnoLimit = Number(doc.r90_anousu.value);
...
function js_calculaQuant(sDataLimit){
  ...
  var aDataLimit = sDataLimit.split('/');
    var iAnoLimit = Number(aDataLimit[0]);
    var iMesLimit = Number(aDataLimit[1]);
    var iAnoAtu = Number(doc.r90_anousu.value);
    var iMesAtu = Number(doc.r90_mesusu.value);
  ...
  if ( doc.rh27_presta.value == 't' && doc.rh27_limdat.value == 't' ) {
    ...
    if ( iAnoLimit > iAnoAtu ) {
      ...
      while ( iAnoLimit > (iAnoAtu+1)  ) {
        ...
        var iMesRest = Number(12 - iMesAtu);
...
function js_verificaposicoes(valor,TorF){
  ...
  if(localbarra == -1){
    ...
    mes = valor.substr(5,2);
      anoi = Number(ano);
      mesi = Number(mes);
      anot = Number(document.form1.r90_anousu.value);
      mest = Number(document.form1.r90_mesusu.value);
