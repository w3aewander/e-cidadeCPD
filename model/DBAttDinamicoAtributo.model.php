...
class DBAttDinamicoAtributo
{
    ...
    /**
     * Define se o atributo cadastrado permite edição ou é fixo.
     * @var bool
     */
    private $lFixo = false;
    ...
    public function __construct($iCodigo = null)
    {
        ...
        if (!empty($iCodigo)) {
            ...
            if ($oDaoCadAttDinamicoAtributo->numrows > 0) {
                ...
                $this->setFixo($oAtributo->db109_fixo == 't');
                $this->setFormula($oAtributo->db109_db_formulas);
            }
        }
    }
    ...
    public function getOpcoes()
    {
        ...
        if (is_null($this->aOpcoes)) {
            ...
            if ($this->getCodigo()) {
                ...
                $sSqlOpcoes = $oDaoOpcoes->sql_query_file(null, "*", null,
                  "db18_cadattdinamicoatributos = {$this->getCodigo()}");
    ...
    public function salvar()
    {
        ...
        $oDaoCadAttDinamicoAtributo->db109_fixo = $this->isFixo() ? 'true' : 'false';
    ...
    public function  getNomeFormula()
    {
        ...
        $rsFormula = $oDaoFormula->sql_record($sSql);

        if (!$rsFormula) {
            throw new Exception($oDaoFormula->erro_msg);
        }

        if ($oDaoFormula->numrows > 0) {

            $oFormula =  pg_fetch_object($rsFormula);
