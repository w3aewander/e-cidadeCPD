...
DBViewGeracaoAutorizacao = function (sInstancia, oNode, iTipoOrigemDados) {
    ...
    me.bHiddenPeculiar = false;
    me.listaMetaHistoricos = [];
    ...
    me.oNumeroLicitacao = new DBTextField('oTxtNumeroLicitacao', me.sInstancia + '.oNumeroLicitacao', '', 30);
    me.oAnoLicitacao = new DBTextField('oTxtAnoLicitacao', me.sInstancia + '.oNumeroLicitacao', '', 15);
    ...
    me.oNumeroLicitacao.setMaxLength(20);
    me.oAnoLicitacao.setMaxLength(4);
    ...
    me.oCaractPeculiarCod = new DBTextField('oTxtCaractPeculiarCod', me.sInstancia + '.oCaractPeculiarCod', cp, 5);
    ...
    me.oAnoLicitacao.addEvent("onKeyPress", "return js_mask(event,\"0-9\")");
    ...
    this.show = function () {
        ...
        sContentForm += "          <tr>";
        if(isPB) {
            sContentForm += "            <td nowrap='nowrap'><a href='#' id='ancoraLicitacao' disabled='true'>Licitacao</a></td>";
        } else {
            sContentForm += "            <td nowrap='nowrap'><b>Número da Licitação:</b></td>";
        }
        console.log(me.oNumeroLicitacao.toInnerHtml());
        console.log(me.oAnoLicitacao.toInnerHtml());
        sContentForm += "            <td id='td_numerolicitacao_" + me.sInstancia + "'>"
            + me.oNumeroLicitacao.toInnerHtml() + " / " + me.oAnoLicitacao.toInnerHtml()
            "</td>";
        sContentForm += "          </tr>";
        sContentForm += "          <tr>";
        ...
        sContentForm += "            </td>";
        sContentForm += "            <td id='td_caracpeculiar_" + me.sInstancia + "'>" + me.oCaractPeculiarCod.toInnerHtml() + "";
        ...
        if (me.bHiddenPeculiar) {
            let fieledCaracPF = document.getElementById('ancoraCaractPeculiar');
            let inputCaracPI = document.getElementById('td_caracpeculiar_' + me.sInstancia);
            fieledCaracPF.hide();
            inputCaracPI.hide();
        }
        /**
        ...
        $('btnItens').observe('click', function () {
            ...
            if (!$('oTxtNumeroLicitacao').disabled && $F('oTxtNumeroLicitacao') == '') {
                alert("Informe o número da licitação/ano.");
                return false;
            }

            if (!$('oTxtAnoLicitacao').disabled && $F('oTxtAnoLicitacao') == '') {
                alert("Informe o ano da licitação/ano.");
                return false;
            }
        ...
        this.instituicaoLicitacao = null;
        if (isPB) {
            let numeroLicitacao = document.getElementById('oTxtNumeroLicitacao');
            let anoLicitacao = document.getElementById('oTxtAnoLicitacao');
            numeroLicitacao.lang = 'l20_numero';
            anoLicitacao.lang = 'l20_anousu';
            this.licitacaoLookUp = new DBLookUp(
            ...
            this.licitacaoLookUp.setCallBack('onClick', (resposta) => {
                this.instituicaoLicitacao = resposta[2];
            })
    ...
    this.criarComboComplementos = function (idAutorizacao, dotacao) {

        let stringCombo = '<select style="width:100%" id="complemento_' + idAutorizacao + '">';
        let listaComplementos = me.complementos[dotacao];
    ...
    this.criarComboMetaHistorico = function (idAutorizacao) {
        let stringCombo = '<select style="width:100%" id="metahist_' + idAutorizacao + '">';
        let listaComplementos = me.listaMetaHistoricos;
    ...
    this.getProcessoAdministrativo = function () {
        ...
        oParamCompraEmpenho.iOrigem = me.getOrigem();
    ...
    this.setProcessoAdministrativo = function (oAjaxRetorno) {
        ...
        if (oRetorno.pc90_numeroprocesso != '' && typeof iTipoOrigemDados == "undefined") {
        ...
        if (oRetorno.numeroLicitacao != '') {
            me.oNumeroLicitacao.setValue(oRetorno.numeroLicitacao);
            me.oAnoLicitacao.setValue(oRetorno.anoLicitacao);
            $('oTxtNumeroLicitacao').value = oRetorno.numeroLicitacao;
            $('oTxtAnoLicitacao').value = oRetorno.anoLicitacao;
            $('oTxtNumeroLicitacao').setAttribute('readonly', 'readonly');
            $('oTxtAnoLicitacao').setAttribute('readonly', 'readonly');
            $('oTxtTipoCompra').setAttribute('disabled', 'disabled');
            $('oTxtNumeroLicitacao').className += ' readonly';
            $('oTxtAnoLicitacao').className += ' readonly';
            if (isPB) {
                me.instituicaoLicitacao = oRetorno.instituicaoLicitacao;
                me.licitacaoLookUp.desabilitar();
    ...
    this.getTipoCompraEmpenho = function () {
        ...
        var oAjaxCompraEmpenho = new Ajax.Request(me.sRPC, {
            method: 'post',
            parameters: 'json=' + Object.toJSON(oParamCompraEmpenho),
    ...
    this.configTipoCompraEmpenho = function (oAjaxRetorno) {
        ...
        if (oRetorno.status == 0) {
            ...
            if (oRetorno.l44_obrigalicitacao == "f") {
                $("oTxtNumeroLicitacao").className = 'readonly';
                $("oTxtAnoLicitacao").className = 'readonly';
                $('oTxtNumeroLicitacao').setAttribute('disabled', 'disabled');
                $('oTxtAnoLicitacao').setAttribute('disabled', 'disabled');
                if(isPB){
                    me.licitacaoLookUp.desabilitar();
                    $("oTxtAnoLicitacao").value = "";
    ...
    this.itensAutorizacao = function () {
        ...
        sContentItens += "<div id='divItensEmp' style='width: 90%;'>";
        ...
        me.oDataGridItens.setCellAlign([
            "right", // 1
            "left", // 2
            "right", // 3
            "left", // 4
        ...
        me.oDataGridItens.setCellWidth(['4%', '8%', '4%', '18%', '10%', '12%', '10%', '4%', '3%', '3%', '5%', '5%', '5%', '6%', '0%', '3%']);
        me.oDataGridItens.setCheckbox(0);
        me.oDataGridItens.setHeader([
                "Cod", // 1
                "Ref", // 2
                "Sol", // 3
                "Descrição", // 4
    ...
    this.preencheItens = function (oResponse) {
        ...
        if (oRetorno.status == 1) {
            ...
            me.complementos = oRetorno.complementos;
            me.listaMetaHistoricos = oRetorno.listaMetaHistoricos;
...
f
            ...
            me.aItens.each(function (oItem, id) {
                ...
                aRow[8] = oItem.quantfornecedor;
                aRow[9] = oItem.saldoquantidade;
                aRow[10] = eval(sNomeQuant + " = new DBTextField('" + sNomeQuant + "', '" + sNomeQuant + "', '" + oItem.quanttotaldotacao + "', 3);");
                ...
                if (isPB){
                    oItem.saldovalor = Number(oItem.saldovalor.toString().match(/^\d+(?:.\d{0,2})?/));
                } else {
                ...
                if (!oItem.servico || (oItem.servico && oItem.servicoquantidade == "t")) {
                    nValorTotal = new Number(oItem.quanttotaldotacao) * new Number(oItem.valorunitariofornecedor);
                }
                ...
                aRow[13] = oItem.valorreserva > 0 ? 'Sim' : 'Não';
                aRow[14] = id.toString();
                aRow[15] = "<span id='span_" + id.toString() + "' style='font-weight:bold;color:red;'>&nbsp;&nbsp;!&nbsp;&nbsp;</span>";
                ...
                with(me.oDataGridItens) {
                    ...
                    if (lDisabled) {
                        ...
                        /**
                         * Valida se a dotao do ano  diferente do ano atual. Caso seja a opo para gerar autorizao
                         * do empenho do item  bloqueada. O usurio dever alterar a dotao do item para uma dotao
                         * do ano corrente.
                         */
    ...
    this.calculaValorTotalItem = function (nQuantidade, iItem) {
        ...
        if (isPB){
            me.aItens[iItem].saldovalor = Number(nValorItem.toString().match(/^\d+(?:.\d{0,2})?/));
        }else {
    ...
    this.preencheTipoLicitacao = function (oAjax) {
        ...
        if ($("oTxtTipoCompra").disabled == true) {
            $('oCboTipoLicitacao').setAttribute('disabled', 'disabled');
        }

        me.oTipoLicitacao.clearItens();
        if (oRetorno.aTiposLicitacao != "") {
            if (oRetorno.aTiposLicitacao.length > 1) {
                $('oCboTipoLicitacao').removeAttribute('disabled');
            }
            oRetorno.aTiposLicitacao.each(function (oItem) {
                me.oTipoLicitacao.addItem(oItem.l03_tipo, oItem.l03_tipo + " - " + oItem.l03_descr);
            });
            if (isPB && me.oNumeroLicitacao.getValue() != '') {
                $('oTxtTipoCompra').setAttribute('readonly','readonly');
                $('oTxtTipoCompra').setAttribute('disabled','disabled');
                $('oTxtTipoCompra').className += 'readonly';

                $('oCboTipoLicitacao').setAttribute('readonly','readonly');
                $('oCboTipoLicitacao').setAttribute('disabled','disabled');
                $('oCboTipoLicitacao').className += 'readonly';
            }
        }

        if (oRetorno.obrigaLicitacao == "f") {
            $("oTxtNumeroLicitacao").className = 'readonly';
            $("oTxtAnoLicitacao").className = 'readonly';
            $('oTxtNumeroLicitacao').setAttribute('disabled', 'disabled');
            $('oTxtAnoLicitacao').setAttribute('disabled', 'disabled');
            ...
            $("oTxtNumeroLicitacao").value = '';
            $("oTxtAnoLicitacao").value = '';
        } else if ($('oTxtTipoCompra').disabled !== true) {
            if(isPB) {
                me.licitacaoLookUp.habilitar();
            } else {
                $("oTxtNumeroLicitacao").disabled = false;
                $("oTxtAnoLicitacao").disabled = false;
                $("oTxtNumeroLicitacao").classList.remove('readonly');
                $("oTxtAnoLicitacao").classList.remove('readonly');
            }
    ...
    this.visualizarAutorizacao = function () {
        ...
        if (me.iTipoOrigemDados !== '1' && me.iTipoOrigemDados !== '2') {

            /**
             * Valida se não foi selecionado mais de uma vez o mesmo item.
             */
            var aValidacao = [];
            var aValidacaoDotacao = [];
            var lConflito = false;

            oItensSelecionados.each(function (oItem, iLinha) {

                var oItemLicitacao = me.aItens[oItem.aCells[15].getValue()];
                if (aValidacao.in_array(oItemLicitacao.codigoitemsolicitacao) && aValidacaoDotacao.in_array(oItemLicitacao.codigodotacaoitem)) {
                    lConflito = true;
                    throw $break;
                }
                aValidacao.push(oItemLicitacao.codigoitemsolicitacao);
                aValidacaoDotacao.push(oItemLicitacao.codigodotacaoitem);
            });

            if (lConflito) {
                alert('Em licitações de Chamamento Público, deve-se gerar autorização por fornecedor.');
                return false;
            }
        }
        ...
        me.aAutorizacoesGerar.each(function (oAutorizacao, iAut) {
            ...
            with (oAutorizacao) {
                ...
                aRow[2] = "";
                aRow[3] = "<span title='Complemento'>Complemento:</span>";
                aRow[4] = me.criarComboComplementos(iAut, dotacao);
                aRow[5] = "";
                aRow[6] = js_formatar(valortotal, "f");
                aRow[7] = me.criarComboMetaHistorico(iAut);
    ...
    this.setHiddenPeculiar = function (isPb) {
        me.bHiddenPeculiar = isPb;
    }
    ...
    this.alterarAutorizacao = function () {
        ...
        if (me.oWindowAux == '') {
            me.oWindowAux = new windowAux('window_' + me.sInstancia, 'Autorizao de Empenho', me.getWidth(1), me.getHeight());
    ...
    this.processarAutorizacoes = function () {
        ...
        me.aAutorizacoesGerar.each(function (oDadosAutorizacao, iAut) {
            ...
            if (oDadosAutorizacao.lMarcada) {
                ...
                oAutorizacao.complemento = $F('complemento_' + iAut);
                oAutorizacao.iMetaHistoricos = $F('metahist_' + iAut);

                if (oAutorizacao.iMetaHistoricos == "" && isPB) {
                    lMetasHistoricos =  false;
                }
                ...
                oAutorizacao.iNumeroLicitacao = me.oNumeroLicitacao.getValue() + "/" + me.oAnoLicitacao.getValue();
                if(isPB) {
                    oAutorizacao.iNumeroLicitacao = $('oTxtNumeroLicitacao').value + "/" + $('oTxtAnoLicitacao').value;
                }
...
;
        ...
        if (!lMetasHistoricos) {
            ...
            alert("Selecione o Tipo de Meta / Historico para a Autorização");
            $('btnGerarAutorizacao').disabled = false;
            return false;
        }
    ...
    this.alterarDotacoes = function (aSolicitacoes) {
        ...
        sContent += "    <div id='ctnGridSolicitacoes' style='width: 775px;'></div>";
        sContent += "  </fieldset>";
        sContent += "<div>";
