...
use ECidade\RecursosHumanos\ESocial\Repository\RemuneracaoBeneficioEntePublico as RemuneracaoBEPRepository;
...
/**
 * Class RemuneracaoBeneficioEntePublic
 * referente ao S1207
...
class RemuneracaoBeneficioEntePublico extends ProcessamentoAbstract implements ProcessamentoInterface
{
    ...
    public function processar()
    {
        ...
        $dados->cgms = RemuneracaoBEPRepository::buscarBeneficiarios($competencia, $this->servidores);
        $dados->inscricao_empregador = RemuneracaoBEPRepository::buscarCNPJEmpregador($this->cgm);
        ...
        $dados->rubricaDiferenca = $parametros->getRubricadifsalario();


        $formatter = FormatterFactory::get(Tipo::S1207);
        ...
        if (!empty($naoEnviados)) {
            ...
            file_put_contents("tmp/InconsistenciasS1207.txt", print_r($naoEnviados, true));
        }
        $validaMd5 = true;
        ...
        foreach ($dadosPreenchimentoEmpregador as $indice => $dados) {
            $evento = new Evento(TIPO::S1207, $this->cgm, $dados->referencia, $dados);
