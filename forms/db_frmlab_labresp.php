...
db_input('db12_uf',2,$Idb12_uf,true,'text',3,'')
  ...
  <tr>
    <td nowrap title="<?= $Tla06_permitidoconferencia ?>">
      <b>Permitido Conferência: </b>
    <i class="fas fa-info-circle" style="margin-left: 5px; width: 10px" title="Ao selecionar Permitido Conferência como SIM, o profissional está habilitado a conferir o resultado do exame no momento da digitação."></i>
    </td>
    <td>
        <?php
  ...
  if(mostra==true){
    js_OpenJanelaIframe('','db_iframe_rhcbo','func_rhcbo.php?funcao_js=parent.js_mostrarhcbo1|rh70_sequencial|rh70_descr','Pesquisa',true);
     ...
     if(document.form1.la06_i_cbo.value != ''){
        js_OpenJanelaIframe('','db_iframe_rhcbo','func_rhcbo.php?lCadastroCgm&pesquisa_chave='+document.form1.la06_i_cbo.value+'&funcao_js=parent.js_mostrarhcbo','Pesquisa',false);
     }else{
...
function js_cancela(){
   location.href='lab1_lab_labresp001.php?la06_i_laboratorio=<?=$la06_i_laboratorio?>&la02_c_descr=<?=$la02_c_descr?>';
}
