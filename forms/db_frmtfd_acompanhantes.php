      ...
      <table class="form-container">
        <tr>
          <td nowrap title="<?=$Ttf13_i_cgsund?>" colspan="2">
            ...
            db_input('z01_v_nome2',   50, $Iz01_v_nome,    true, 'text', 1,' onchange="verificaNomePaciente();" ');
            ?>
          </td>
        </tr>
        <tr>
...
// Autocomplete do CGS
let oAutoComplete = new dbAutoComplete($('z01_v_nome2'), 'sau4_pesquisanome.RPC.php?tipo=1');
oAutoComplete.setHeightList(390);
oAutoComplete.show();
oAutoComplete.setCallBackFunction(function (cgsUnd, nomePaciente, resposta) {
    $('tf13_i_cgsund').value = cgsUnd;
    $('z01_v_nome2').value = nomePaciente.urlDecode();
    if(cgsUnd.length > 0){
      js_pesquisatf13_i_cgsund(false);
    }
});
...
function js_pesquisatf13_i_cgsund(mostra) {
  ...
  if(mostra == true) {
    var janela = js_OpenJanelaIframe(
                         '',
                         'db_iframe_cgs_und',
                         'func_cgs_und.php?funcao_js=parent.js_mostracgs1|z01_i_cgsund|z01_v_nome&nome_social=true',
                         'Pesquisa CGS',
                         true,0,0
                       );
    ...
    if(document.form1.tf13_i_cgsund.value != '') {
      js_OpenJanelaIframe(
                           ...
                           'func_cgs_und.php?pesquisa_chave='+document.form1.tf13_i_cgsund.value
                                          +'&funcao_js=parent.js_mostracgs&nome_social=true',
                           'Pesquisa CGS',
                           false
                         );
    } else {
