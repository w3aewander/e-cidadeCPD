...
db_logduplos(date('d-m-Y H:i:s', time()));
...
for ($record_correto=0; $record_correto < pg_num_rows($result_correto); $record_correto++) {
    ...
    db_logduplos("\n\n\n\n\n\n");
    db_logduplos(" processando cgm correto: " . $z10_numcgm . " - codigo: $z10_codigo - $record_correto/" .
                  pg_num_rows($result_correto) . "...");
    ...
    */
    ...
    $sqlsessaodeaprtamento .= "from db_depusu ";
    $sqlsessaodeaprtamento .= "where id_usuario = {$z10_login}";
    ...
    //Desabilita trigger que insere registro na promitente
    ...
    for ($record_errado=0; $record_errado < pg_num_rows($result_errado); $record_errado++) {
        ...
        $sExcecaoTabelas = "736, 737, 511, 1383, 1015, 1010143, 2947, 1382, 2362, 1780, 3012, 3901, 3921, 1010588, ";
        $sExcecaoTabelas .= "2918, 2938, 2939, 3924, 1010267, 1010284, 1010311, 1010407, 1010418, 403";
        $sExcecaoCampos = "5153,5159,216,7872,8213,8195,17180,16650,8205, 20656, 17041, 17042, 1011584";
        ...
        if ($v_cgmcerto > 0) {
            ...
            $sql1 .= "   and db_sysarqcamp.codarq not in ({$sExcecaoTabelas}) ";
            $sql1 .= "   and db_sysarqcamp.codcam not in ({$sExcecaoCampos}) ";
           ...
           */
            ...
            $sSqlCgmCorreto .= "       z01_numcgm as numcgmcorreto_ant, ";
            $sSqlCgmCorreto .= "       z01_nome   as nomecorreto_ant ";
            $sSqlCgmCorreto .= "from cgmcorreto ";
            $sSqlCgmCorreto .= "     join cgm on z01_numcgm = z10_numcgm ";
            $sSqlCgmCorreto .= "     join cgmerrado on z10_codigo = z11_codigo ";
            $sSqlCgmCorreto .= "where z10_numcgm = $v_cgmerrado";
            ...
            if (pg_num_rows($rsCgmCorreto) > 0) {
                for ($y=0; $y < pg_num_rows($rsCgmCorreto); $y++) {
                    ...
                    $sql_corretoant = "update cgmcorreto set z10_numcgm = {$v_cgmcerto} ";
                    $sql_corretoant .= "where z10_codigo = {$codcorreto_ant}";
                                ...
                                switch ($nomearq) {
                                    ...
                                    case "cadescrito":
                                        ...
                                        if ($linhasesc > 0) {
                                            ...
                                            $resultaltescresp = db_query($sqlaltescresp) or die(db_logduplos("sql 18: " . pg_last_error()));
                                        ...
                                        $resultdelcadesc= db_query($sqldelcadesc) or
                                        die(db_logduplos("sql 19: " . pg_last_error()));
                          ...
                          (
                            ...
                            $r52_percadiantamento13
                                    ...
                                    case "cgmdoc":
                                        ...
                                        $rsCgmDocCorreto   = db_query($sSqlCgmDocCorreto) or
                                        die(db_logduplos("sql 619: " . pg_last_error()));
                                    ...
                                    case 'agualeiturista':
                                        ...
                                        $rsAguaLeitura   = db_query($sSqlAguaLeitura) or
                                        die(db_logduplos("SQL pesquisa agualeitura: " . pg_last_error()));
                                    ...
                                    if ($rsAguaLeitura && pg_num_rows($rsAguaLeitura) > 0) {
                                        ...
                                        $rsTabelaBkp    = db_query($sSqlTabelaBkp) or die(db_logduplos("Criação da tabela temporária: " . pg_last_error()));

                                        if ($rsTabelaBkp) {
                                            $sSqlDeletaAguaLeitura = "delete from agualeitura where x21_numcgm = {$v_cgmerrado}";
                                            $rsDeletaAguaLeitura   = db_query($sSqlDeletaAguaLeitura) or die(db_logduplos("Delete registros agualeitura CGM errado: " . pg_last_error()));
                                            ...
                                            if ($rsDeletaAguaLeitura) {
                                                ...
                                                $rsInsereAguaLeitura    = db_query($sSqlInsereAguaLeitura) or die(db_logduplos("Inserção dos registros agualeitura CGM correto: " . pg_last_error()));
                                                ...
                                                if ($rsInsereAguaLeitura) {
                                                    ...
                                                    $rsAtualizaAguaLeiturista    = db_query($sSqlAtualizaAguaLeiturista) or die(db_logduplos("Atualização dos registros agualeiturista CGM correto: " . pg_last_error()));
                                                }
                                            }
                                        }
                        ...
                        if (pg_num_rows($result9) > 0) {
                            ...
                            switch ($nomearq) {
                                 ...
                                 */
                                    ...
                                    $iLegista           = @pg_fetch_result($rsLegistaCorreto, 0, 0);
                                    ...
                                    if (pg_num_rows($rsLegistaCorreto) == 0 && pg_num_rows($rsLegistaErrado) > 0) {
                                        $iCrm     = pg_fetch_result($rsLegistaErrado, 0, 2);
                                        $iLegista = pg_fetch_result(db_query("select nextval('cem_legista_seq')"), 0, 0);
                                    ...
                                    if (pg_num_rows($rsLegistaErrado) > 0) {
                                        $iLegistaErrado = pg_fetch_result($rsLegistaErrado, 0, 0);
                                    ...
                                    $QueryLegista = db_query($sSqlLegistaDel) or
                                    die(db_logduplos("sql 27: $sSqlLegistaDel" . pg_last_error()));
                                ...
                                case "empageformacgm":
                                    ...
                                    $rsEmpageformacgm = db_query($sql_Empageformacgm) or
                                    die(db_logduplos("sql 37: " . pg_last_error()));
                                ...
                                case "issbase":
                                    ...
                                    $rResultIss = db_query($sSqlIss) or
                                    die(db_logduplos("\nsql: $sSqlIss\n" . pg_last_error()));
                                ...
                                case "arrecad":
                                    ...
                                    $rsDadosArrecadInstituicao   = db_query($sSqlDadosArrecadInstituicao) or
                                    die(db_logduplos("\nsql: $sql18\n" . pg_last_error()));
                         ...
                         */
                           ...
                           */
                                ...
                                default:
                                  ...
                                  /**
                                   * Regra aplicada inicialmente para arreold, pois ao atualizar o CGM, os débitos não devem ser corrigidos
                                   */
       ...
       */
        foreach ($aTabelasRegrasEspecificas as $sChave => $sTabela) {
            switch ($sTabela) {
                 ...
                 */
                case 'cgmendereco':
                    $sSqlEndereco = "delete from cgmendereco where z07_numcgm = {$v_cgmerrado}";
                    $rsEndereco   = db_query($sSqlEndereco) or
                    die(db_logduplos("SQL Exclusão vinculo com endereço: " . pg_last_error()));
               ...
               */
                case 'protprocesso':
                    $sSqlVerificaRequer  = "select p58_numcgm ";
                    $sSqlVerificaRequer .= "from protprocesso ";
                    $sSqlVerificaRequer .= "     join cgm on z01_numcgm = p58_numcgm ";
                    $sSqlVerificaRequer .= "where p58_numcgm = {$v_cgmerrado}";
                    $sSqlVerificaRequer .= "  and upper(to_ascii(trim(z01_nome))) = upper(to_ascii(trim(p58_requer)))";
                    $rsVerificaRequer = db_query($sSqlVerificaRequer);

                    $z01_nome = '';
                    if (pg_num_rows($rsVerificaRequer) > 0) {
                        $sSqlCgmCorreto = "select z01_nome from cgm where z01_numcgm = {$v_cgmcerto}";
                        $rsCgmCorreto = db_query($sSqlCgmCorreto);
                        if (pg_num_rows($rsCgmCorreto) > 0) {
                            db_fieldsmemory($rsCgmCorreto, 0);
                        }
                    }

                    $sSqlProtProcesso = "update protprocesso set p58_numcgm = {$v_cgmcerto} ";
                    if ($z01_nome != '') {
                        $z01_nome = addslashes($z01_nome);
                        $sSqlProtProcesso .= ", p58_requer = '{$z01_nome}'";
                    }
                    $sSqlProtProcesso .= "  where p58_numcgm = {$v_cgmerrado}";
                    $rsProtProcesso   = db_query($sSqlProtProcesso) or
                    die(db_logduplos("SQL ajuste ProtProcesso: " . pg_last_error()));
                    $sMsg = "ProtProcesso: Ajustado o registro da tabela protprocesso do CGM ERRADO ({$v_cgmerrado})";
                    db_logduplos($sMsg);

                    break;
                case 'cgmfisico':
                    $sSqlCgmFisico = "delete from cgmfisico where z04_numcgm = {$v_cgmerrado}";
                    $rsCgmFisico   = db_query($sSqlCgmFisico) or
                    die(db_logduplos("SQL Exclusão Cgm Fisico: " . pg_last_error()));
                    db_logduplos("Cgm Fisico: Excluído o registro da tabela cgmfisico do CGM ERRADO ({$v_cgmerrado})");

                    break;
                case 'cgmtipoempresa':
                    $sSqlTipoEmpresa = "delete from cgmtipoempresa where z03_numcgm = {$v_cgmerrado}";
                    $rsTipoEmpresa   = db_query($sSqlTipoEmpresa) or
                    die(db_logduplos("SQL Exclusão Tipo Empresa: " . pg_last_error()));
                ...
                case 'avaliacaogruporespostarhpessoal':
                    ...
                    db_query($sql) or
                    die(db_logduplos("SQL Alteracao avaliacaogruporespostarhpessoal: " . pg_last_error()));
                break;
                case "avaliacaogruporespostaavisoprevio":
                    ...
                    db_query($sql) or
                    die(db_logduplos("SQL Alteracao avaliacaogruporespostaavisoprevio: " . pg_last_error()));
                break;
                case "avaliacaogruporespostaafastamentoesocial":
                    ...
                    db_query($sql) or
                    die(db_logduplos("SQL Alteracao avaliacaogruporespostaafastamentoesocial: " . pg_last_error()));
                break;
                case "avaliacaogruporespostatsveinicial":
                    ...
                    db_query($sql) or
                    die(db_logduplos("SQL Alteracao avaliacaogruporespostatsveinicial: " . pg_last_error()));
                break;
                case "avaliacaogruporespostaesocials1299":
                    ...
                    db_query($sql) or
                    die(db_logduplos("SQL Alteracao avaliacaogruporespostaesocials1299: " . pg_last_error()));
                break;
                case "avaliacaogruporespostatotpgcontingencia":
                    ...
                    db_query($sql) or
                    die(db_logduplos("SQL Alteracao avaliacaogruporespostatotpgcontingencia: " . pg_last_error()));
                break;
            }
        }
