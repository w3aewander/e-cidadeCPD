...
use ECidade\Tributario\Arrecadacao\CGF\Relatorios\TotalDebitos\QuadroParcelamento;
...
if (!empty($numcgm)) {
    ...
    if (!empty($matric)) {
        ...
        $sLabelRefantInscmu = $GLOBALS['RLj40_refant'];
        $oDaoIptuBase        = db_utils::getDao("iptubase");
        $sCamposProprietario = "j34_setor,j34_quadra,j34_lote,tipopri,j39_numero,j39_compl,nomepri,z01_nome,z01_numcgm,z01_cgmpri,j40_refant,pql_localizacao";
        ...
        if ($oDaoIptuBase->numrows > 0) {
            ...
            $sNumeroAnterior = $oDadosProprietario->j40_refant;
        }
        ...
        if (!empty($inscr)) {
            ...
            $sLabelRefantInscmu = $GLOBALS['RLq02_inscmu'];
            ...
            if ($oDaoIssBase->numrows > 0) {
                ...
                $sNumeroAnterior = $oDadosProprietario->q02_inscmu;
            }
...
if(isset($sNumeroAnterior)) {

  $oDadosCgm[0]->labelrefantinscmu = $sLabelRefantInscmu;
  $oDadosCgm[0]->numeroanterior    = $sNumeroAnterior;
}
...
if (!empty($matric)) {
    ...
    $sSqlTipoEnvolvido = $oDaoIptubase->sql_query_envolvidos_matricula($matric, $iCGMEnvolvidoRegra);
    $rsTipoEnvolvido = db_query($sSqlTipoEnvolvido);
    $oTipoEnvolvido = db_utils::getCollectionByRecord($rsTipoEnvolvido)[0];

    $oDadosCgm[0]->z01_nome = $oTipoEnvolvido->nomecomabreviatura;
}
/*
...
foreach ($oDadosDebitos as $aDados) {
    ...
    if ($aDados->k00_numpre != $sHashNumpre) {
        ...
        array_push($aTotalNumpres, $aDados->k00_numpre);
        ...
        if (in_array($oDadosArreTipo->k03_tipo, $aCadTipoBuscaDadosParcelamento)) {
            ...
            $novoNumpre = retornaNumprePai($aDados->k00_numpre);
            ...
            $sSqlTermo = " select *
              ...
              select *
                from (select termo.v07_parcel,
                       ...
                       where termo.v07_numpre = {$numpre}) as x
               where exercicios != ''
              ...
              select *
                     from ( select v07_parcel,
                                   ( select array_to_string(array_accum(distinct case
                                                                                   when divcertter.v01_exerc is not null
                                                                                     then divcertter.v01_exerc
                                                                                   when divtermoreparc.v01_exerc is not null
                                                                                     then divtermoreparc.v01_exerc
                                                                                   else divcertdiv.v01_exerc
                                                                                  end ),',')
                                            from fc_parc_origem_completo({$numpre}) as origem
                                                 ...
                                                 left join certter on certter.v14_certid = inicialcert.v51_certidao
                                                 left join termoreparc on certter.v14_parcel = termoreparc.v08_parcel
                                                 left join termodiv ct on ct.parcel = termoreparc.v08_parcelorigem
                                                 left join divida as divtermoreparc on divtermoreparc.v01_coddiv = ct.coddiv
                                                 left join termodiv tc on tc.parcel = certter.v14_parcel
                                                 left join divida as divcertter on divcertter.v01_coddiv = tc.coddiv ) as exercicios
                              from termo
                             where v07_numpre = {$numpre}) as x
               where exercicios != '' ";
            ...
            if ($iLinhasTermo > 0) {
                ...
                for ($i = 0; $i < $iLinhasTermo; $i++) {
                    ...
                    foreach ($aExerciciosRetornados as $iExercicio) {
                        ...
                        if (array_key_exists($iExercicio, $aExercicios)) {
                            continue;
                        } else {
                            $aExercicios[$iExercicio] = $iExercicio; // reune exercicios parcelamentos
                        }
                    }
                }
                ksort($aExercicios);
                $oDadosTermo->parcel_exerc = implode(",", $aExercicios);
                $aDadosProcessado->termo = $oDadosTermo;
                $aDadosProcessado->complemento_numpre = " - Parcelamento : {$parcelamentoAtual} - Exercício(s): {$oDadosTermo->parcel_exerc}";
        ...
        if (in_array($oDadosArreTipo->k03_tipo, $aCadTipoBuscaDadosInicialForo)) {
            ...
            $sSqlProcessoForo .= " left join processoforoinicial  on processoforoinicial.v71_inicial = inicial.inicial        ";
            $sSqlProcessoForo .= "                               and processoforoinicial.v71_anulado = false                  ";
            $sSqlProcessoForo .= " left join processoforo         on processoforo.v70_sequencial     = processoforoinicial.v71_processoforo ";
        ...
        if (in_array($oDadosArreTipo->k03_tipo, $aCadTipoBuscaDadosInicialForo)
            ...
            if ($aDadosProcessado->complemento_numpre == "") {
                $aDadosProcessado->complemento_numpre = " - Inicial: {$oDadosProcessoForo[0]->inicial}";
                $aDadosProcessado->complemento_numpre .= " - Processo do Foro : {$oDadosProcessoForo[0]->processo_foro}";
                $aDadosProcessado->numero_parcelamento = $aTermo->v07_parcel;
            }
...
$quadroParcelamento = new QuadroParcelamento($oPdf);
...
foreach ($aDadosRelatorio as $aDadosOrigem) {
    ...
    foreach ($aDadosOrigem as $aDadosTipo) {
        ...
        foreach ($aDadosTipo as $aDadosProcessoForo) {
            ...
            $oTotalInicial = new stdClass();
            $oTotalInicial->inicial = null;
            $oTotalInicial->total_historico = 0;
            $oTotalInicial->total_corrigido = 0;
            $oTotalInicial->total_juros = 0;
            $oTotalInicial->total_multa = 0;
            $oTotalInicial->total_desconto = 0;
            $oTotalInicial->total_geral = 0;
            ...
            foreach ($aDadosProcessoForo as $aDados) {
                ...
                foreach ($aDados as $aDadosNumpre) {
                    ...
                    if (count($aDadosNumpre->processo_foro) > 0) {
                        ...
                        $oTotalInicial->total_historico += $aDadosNumpre->total_historico;
                        $oTotalInicial->total_corrigido += $aDadosNumpre->total_corrigido;
                        $oTotalInicial->total_juros += $aDadosNumpre->total_juros;
                        $oTotalInicial->total_multa += $aDadosNumpre->total_multa;
                        $oTotalInicial->total_desconto += $aDadosNumpre->total_desconto;
                        $oTotalInicial->total_geral += $aDadosNumpre->total_geral;

                        $quadroParcelamento->adicionarDados($aDadosNumpre);
                        ...
                        $oTotalInicial->inicial = $aDadosNumpre->processo_foro[0]->inicial;
                    }
                    ...
                    $oPdf->Cell(4, 4, $aDadosNumpre->numpar, 1, 0, "C", 0);
                    $oPdf->Cell(4, 4, $aDadosNumpre->numtot, 1, 0, "C", 0);
                    ...
                    $oPdf->Cell(15, 4, db_formatar($aDadosNumpre->total_corrigido, 'f'), 1, 0, "R", 0);
                    $oPdf->Cell(15, 4, db_formatar($aDadosNumpre->total_juros, 'f'), 1, 0, "R", 0);
                    $oPdf->Cell(15, 4, db_formatar($aDadosNumpre->total_multa, 'f'), 1, 0, "R", 0);
                    $oPdf->Cell(15, 4, db_formatar($aDadosNumpre->total_desconto, 'f'), 1, 0, "R", 0);
                    $oPdf->Cell(15, 4, db_formatar($aDadosNumpre->total_geral, 'f'), 1, 0, "R", 0);
                    $oPdf->Cell(1, 4, "", 0, 1, 0, 0);

                    $lMostraInicial = true;

                    $aProximoNumpre = next($aDados);
                    $aProximoDadosProcessoForo = next($aDadosProcessoForo);

                    if (   (!empty($aProximoNumpre) && (int)$aProximoNumpre->processo_foro[0]->inicial === (int)$aDadosNumpre->processo_foro[0]->inicial)
                        || (!empty($aProximoDadosProcessoForo) && (int)$aProximoDadosProcessoForo[0]->processo_foro[0]->inicial === (int)$aDadosNumpre->processo_foro[0]->inicial)
                    ) {
                        $lMostraInicial = false;
                    } 
                    
                    if (!empty($aDadosNumpre->termo)) {
                        $lTermo = false;
                    }                                    
                ...
                if ($lMostraInicial) {
                    fc_totalInicialForo($oPdf, $oTotalInicial);
                    $oTotalInicial->total_historico = 0;
                    $oTotalInicial->total_corrigido = 0;
                    $oTotalInicial->total_juros = 0;
                    $oTotalInicial->total_multa = 0;
            ...
            if (count($aDadosNumpre->processo_foro) > 0) {
                ...
                if ($oTotalProcesso->processo_foro[0]->processo_foro > 0 && $lTermo) {
...
$quadroParcelamento->montarQuadro();
...
function fc_dadosContribuinte(FPDF $oPdf, $oDadosCgm)
{
    ...
    $oPdf->Text(32, 38, $oDadosCgm[0]->complemento);
    $oPdf->Text(5, 42, "Nome:");
    if(isset($oDadosCgm[0]->numeroanterior)) {
        
   
      $oPdf->Text(115, 42, $oDadosCgm[0]->labelrefantinscmu." : ");
    }
    ...
    $oPdf->Text(18, 38, $oDadosCgm[0]->z01_numcgm);
    $oPdf->Text(23, 42, $oDadosCgm[0]->z01_nome);
    if(isset($oDadosCgm[0]->numeroanterior)) {
        
   
        $oPdf->Text(145, 42, $oDadosCgm[0]->numeroanterior);
    }
...
function fc_totalTipo($oPdf, $oDadosTipo)
{
    ...
    $oPdf->Cell(108, 5, $sTotalTipo, 1, 0, "L", 1);
    ...
    if (in_array($oDadosTipo->tipo, $aCadTipoInicialForo)) {
        ...
        $oPdf->setx(5);
        $oPdf->Cell(183, 5, "TOTAL DAS CUSTAS: ( não isentas e não pagas) ", 1, 0, "L", 1);
...
{
    $strExercicioDiver = "";
    $sSqlExercDiver ="select distinct coalesce(dv05_exerc::varchar,'') as dv05_exerc 
                        from diversos 
                  inner join arrecad  on diversos.dv05_numpre = arrecad.k00_numpre 
                  inner join arretipo on arrecad.k00_tipo     = arretipo.k00_tipo
                       where dv05_numpre = {$oDadosNumpre->numpre} 
                         and arretipo.k03_tipo = 7 /* diversos */
                             limit 1";
    ...
    $y = $oPdf->getY();
    $oPdf->MultiCell(108, 5, "TOTAL DO NUMPRE {$oDadosNumpre->numpre} {$oDadosNumpre->numpre_complemento} {$strExercicioDiver}", 1, "L", 1);
    $y2= $oPdf->getY();

    $oPdf->setXY(113, $y);
    $oPdf->Cell(15, $y2-$y, db_formatar($oDadosNumpre->total_historico, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, $y2-$y, db_formatar($oDadosNumpre->total_corrigido, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, $y2-$y, db_formatar($oDadosNumpre->total_juros, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, $y2-$y, db_formatar($oDadosNumpre->total_multa, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, $y2-$y, db_formatar($oDadosNumpre->total_desconto, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, $y2-$y, db_formatar($oDadosNumpre->total_geral, 'f'), 1, 1, "R", 1);
    $oPdf->setY($y2);
    $oPdf->SetFont('arial', '', 6);
}

function fc_totalInicialForo($oPdf, $oDadosInicialForo)
{
    $oPdf->setx(5);
    $oPdf->SetFont('arial', 'B', 6);

    $sInicial = "INICIAL: {$oDadosInicialForo->inicial}";

    $oPdf->Cell(108, 5, $sInicial, 1, 0, "L", 1);
    $oPdf->Cell(15, 5, db_formatar($oDadosInicialForo->total_historico, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, 5, db_formatar($oDadosInicialForo->total_corrigido, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, 5, db_formatar($oDadosInicialForo->total_juros, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, 5, db_formatar($oDadosInicialForo->total_multa, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, 5, db_formatar($oDadosInicialForo->total_desconto, 'f'), 1, 0, "R", 1);
    $oPdf->Cell(15, 5, db_formatar($oDadosInicialForo->total_geral, 'f'), 1, 1, "R", 1);
...
function fc_totalProcessoForo($oPdf, $oDadosProcessoForo)
{
    ...
    if ($oDadosProcessoForo->custas_total != "") {
        $oPdf->Cell(183, 5, "CUSTAS ({$oDadosProcessoForo->custas_complemento}) :", 1, 0, "L", 1);
...
function fc_totalOrigem($oPdf, $oDadosOrigem)
{
    ...
    $oPdf->Cell(108, 5, "TOTAL DA ORIGEM {$oDadosOrigem->origem}", 1, 0, "L", 1);
...
function fc_totalGeral($oPdf, $oDadosTotal)
{
    ...
    $oPdf->Cell(108, 5, "TOTAL GERAL : ", 1, 0, "L", 1);
...
/*
 * Função que busca se o parcelamento se originou de outros reparcelamentos
 * Recebe o número do parcelamento
 * Se encontra reparcelamentos anteriores vinculados a este, itera novamente a função passando o v08_parcelorigem do registro encontrado
 * Retorna o v07_numpre do parcelamento mais próximo do original da dívida
 */
function retornaParcelOrigem($parcelOrigem) {

    $parcelamento = '';

    // Verifica se o parcelamento foi originado de um reparcelamento
    $sSql           = "select v08_parcelorigem from termoreparc where v08_parcel = $parcelOrigem";
    $oDao           = db_utils::getDao("termoreparc");
    $rsTermoparc    = $oDao->sql_record($sSql);
    $rowsTermoParc  = $oDao->numrows;

    // Se encontrar parcelamento anterior, itera novamente a função buscando se o parcelamento anterior foi originado de reparcelamento
    if ($rowsTermoParc > 0) {
        $aTermo         = db_utils::fieldsMemory($rsTermoparc, 0)->v08_parcelorigem;
        $parcelamento   = retornaParcelOrigem($aTermo);
    }
    // Se não encontra, busca pelo numpre do parcelamento enviado por parâmetro
    else {
        $sSql       = "select v07_numpre from termo where v07_parcel = $parcelOrigem";
        $oDaoTermo  = db_utils::getDao("termo");
        $rsTermo    = $oDaoTermo->sql_record($sSql);
        $rowsTermo  = $oDaoTermo->numrows;

        if ($rowsTermo > 0) {
            $aTermo = db_utils::fieldsMemory($rsTermo, 0)->v07_numpre;
            $parcelamento = $aTermo;
        }
    }
    return $parcelamento;
...
/*
 * Função que busca se existe parcelamento de origem
 * Recebe o numpre do parcelamento
 * Se encontra reparcelamentos anteriores vinculados a este chama a retornaParcelOrigem
 * Retorna o numpre do parcelamento mais próximo do original da dívida
 */
function retornaNumprePai($numpreOrigem) {

    $sSql = "select v08_parcelorigem from termoreparc where v08_parcel = (select v07_parcel from termo where v07_numpre = $numpreOrigem)";

    $oDaoTermoReparc    = db_utils::getDao("termoreparc");
    $rsTermoReparc      = $oDaoTermoReparc->sql_record($sSql);
    $rowsTermoReparc    = $oDaoTermoReparc->numrows;

    if ($rowsTermoReparc > 0) {
        $aTermo = db_utils::fieldsMemory($rsTermoReparc, 0);
        $numpre = retornaParcelOrigem($aTermo->v08_parcelorigem);
        return $numpre;
    } else {
        return $numpreOrigem;
    }
...
function retornaParcelamentoAtual($numpreOrigem){
    $sSql = "SELECT v08_parcel 
               FROM termoreparc 
              INNER JOIN termo 
                 ON v07_parcel = v08_parcel 
              WHERE v07_numpre = {$numpreOrigem}";
    ...
    if ($rowsTermoReparc > 0) {
        $oTermo = db_utils::fieldsMemory($rsTermoReparc, 0);
        
        return $oTermo->v08_parcel;
    } else {
        $sSql = "SELECT v07_parcel 
	 	           FROM termo 
		          WHERE v07_numpre = $numpreOrigem";

        $rsTermo = $oDaoTermoReparc->sql_record($sSql);
        $oTermo  = db_utils::fieldsMemory($rsTermo, 0);

        return $oTermo->v07_parcel;
    }
}
