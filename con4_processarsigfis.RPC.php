...
use ECidade\Financeiro\Contabilidade\Exportacao\Sigfis\SigfisBuilder;
...
switch ($oParam->exec) {
    ...
    /**
     * Processamento do Novo Sigfis
     */
    case 'processarNovoSigfis':
        //echo "<pre>";
        //print_r($oParam);
        //echo "</pre>";
        //die("Confere");
        try {
            $sigfisBuilder = new SigfisBuilder(
                $oParam->aArquivos,
                $oParam->iPeriodo,
                //$oParam->gerarConsolidado,
                //$oParam->utilizaPlanoUniao,
                $oParam->anoExe
            );
            $oRetorno->lista = $sigfisBuilder->buildListaArquivos();
            $oRetorno->arquivosNaoProcessados = $sigfisBuilder->builderErrors;
        } catch (Exception $eErro) {
            $oRetorno->message = urlencode($eErro->getMessage());
            $oRetorno->status  = 0;
        }
