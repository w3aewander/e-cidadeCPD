...
class AnexoVIII
{
    ...
    protected function configuraColunas()
    {
    ...
    private function processar()
    {
        if (empty($this->oRelatorio)) {
            throw new \Exception("Informe o relatório.");
        }

        $this->iAno = $this->oRelatorio->getAno();
        $this->oPeriodo = $this->oRelatorio->getPeriodo();
        $this->sInstituicao = $this->oRelatorio->getInstituicoes();
        $this->configuraColunas();

        $this->aLinhas = $this->oRelatorio->getLinhas();
        $this->oDataFinal = $this->oRelatorio->getDataFinal();
        $this->oDataInicial = $this->oRelatorio->getDataInicialPeriodo();
    }
    ...
    {   
        ...
        for ($i = $this->aPosicoes[$iPosicao]['inicio']; $i <= $this->aPosicoes[$iPosicao]['fim']; $i++) {
            ...
            $fValor2 = db_formatar($this->aLinhas[$i + $this::TOTAL_LINHAS_FINANCEIRO]->valor, "f");
    ...
    private function imprimeValorReceita($iPosicao = self::RECEITA_RESULTANTE_IMPOSTO)
    {
        ...
        for ($i = $this->aPosicoes[$iPosicao]['inicio']; $i <= $this->aPosicoes[$iPosicao]['fim']; $i++) {
            ...
            $fPorcentagem = ($this->aLinhas[$i]->recatebim > 0 && $this->aLinhas[$i]->prevatu > 0) ? db_formatar(($this->aLinhas[$i]->recatebim / $this->aLinhas[$i]->prevatu) * 100, "f") : db_formatar(0, "f");
    ...
    private function imprimeNotaseRodape()
    { 
        switch ($this->getAno()) {
            case 2017:
                $this->imprimeNota();
        ...
        $this->oPdf->cell(0, 15, " ", "", 1, "");
        $classinatura = new \cl_assinatura();
        assinaturas($this->oPdf, $classinatura, 'LRF');
    }

    private function imprimeNota()
    {
        $oRelatorio = new \relatorioContabil($this::CODIGO_RELATORIO, false);
