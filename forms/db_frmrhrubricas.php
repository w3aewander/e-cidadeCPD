...
<form name="form1" method="post" action="<?=$db_action?>" class="container" >
  ...
  <fieldset>
    ...
    <table class="form-container">
      ...
      <tr>
        ...
        <td>
          <?php
          $x = array("1"=>"Vencimento, provento ou pensão","2"=>"Desconto","3"=>"Base","4"=>"Base (Informativa)","5"=>"Base (Informativa Dedutora)");
      ...
      <tr> 
        <td title="<?=$Trh27_rubric?>"> 
          <?php
            db_ancora("Rubrica de Adiantamento do 13o Salario: ", "js_pesquisarrubric(true);", 1);
          ?>
        </td>
        <td> 
          <?php
            db_input('rubrica_codigo', 8, $Irh27_rubric, true, 'text', 1, " onchange='js_pesquisarrubric(false);'");
            db_input('rubrica_descr', 30, $Irh27_descr,  true, 'text', 3, '');
          ?>
        </td>
      </tr>
  ...
  <fieldset id="ctnInformacoes">
    ...
    <table class="form-container">
      ...
      <tr>
        <td title="<?=@$Trh27_calc3?>">Aviso Previo: </td>
        <td>
          <?php
          $x = array("f"=>"NAO","t"=>"SIM");
...
<script>
  ...
  $('rh27_rhfundamentacaolegal').onpaste = function(event) {
    ...
    return setTimeout(function() {
      ...
      if (!lNumeros) {
        alert('Codigo Fundamentacao Legal deve ser preenchido somente com numeros!');
        self.value = '';
      }
    }, 5);
  }
  ...
  function js_validarFormulario(){
    ...
    if ( $('rh27_rubric').value == '' ) {
      alert('Codigo da rubrica nao informado.');
      $('rh27_rubric').focus();
      return false;
    }
    ...
    if ($F('rh27_valorpadrao') != '' && !js_validaSomenteNumeros($F('rh27_valorpadrao'))) {
      alert('Valor padrao deve ser preenchido somente com numeros decimais.');
    ...
    if ($F('rh27_quantidadepadrao') != '' && !js_validaSomenteNumeros($F('rh27_quantidadepadrao'))) {
      alert('Quantidade padrao deve ser preenchido somente com numeros decimais.');
    ...
    if (sValueLimite == 't' && sValuePeriodo == 't') {
      ...
      alert('Nao e possivel usar o parametro \'Data limite\' e \'Periodo de lancamento\' para uma mesma rubrica.');
      return false;
    }

    if ($('pagamentoPrevidenciaComplementar').checked && $F('rh27_previdenciacomplementar') == '') {

      alert('Previdencia Complementar nao informada.');
      return false;
    }
  ...
  function js_validarCodigoRubrica() {
    ...
    /**
     * Valida se o Codigo da Rubrica foi informado no formato correto.
     */
    ...
    if ( !lValidaCodigoRubrica ) {
      ...
      alert('Codigo da rubrica com o formato invalido.');
      $('rh27_rubric').value = '';
      return false;
    }
  ...
  function js_configSelTipo(sPd){
    ...
    var oOptDevolucao   = new Option('Devolucao','d');
  ...
  var oContextCompleteForm1 = new DBContextComplete('teste');
  oContextCompleteForm1.setElementForContext($('rh27_form'));
  oContextCompleteForm1.init();

  var oContextCompleteForm2 = new DBContextComplete('teste1');
  oContextCompleteForm2.setElementForContext($('rh27_form2'));
  oContextCompleteForm2.init();

  var oContextCompleteForm3 = new DBContextComplete('teste2');
  oContextCompleteForm3.setElementForContext($('rh27_form3'));
  oContextCompleteForm3.init();

  oParam = {exec:'getVariaveis'};

  var oAjax = new Ajax.Request('pes4_rubricas.RPC.php',
      {
          method:'post',
          ...
          onComplete: function(oResponse) {
              ...
              oRetorno.oListaVariaveis.bases.each(function(sVariavel) {
                  oContextCompleteForm1.addOption(sVariavel.codigo, sVariavel.codigo +" - "+ sVariavel.descricao);
                  oContextCompleteForm2.addOption(sVariavel.codigo, sVariavel.codigo +" - "+ sVariavel.descricao);
                  oContextCompleteForm3.addOption(sVariavel.codigo, sVariavel.codigo +" - "+ sVariavel.descricao);
              });
          }
      }
  );
    ...
    /**
    * Realiza a busca de rubricas, retornando o codigo e descricao da rubrica escolhida
    */
    function js_pesquisarrubric(lMostra) {
        
        if ( lMostra) {
          js_OpenJanelaIframe('CurrentWindow.corpo.iframe_rhrubricas','db_iframe_rhrubricas','func_rhrubricas.php?funcao_js=parent.js_mostrarubricas1|rh27_rubric|rh27_descr','Pesquisa',true);
        } else {
            
           if ( $F(rubrica_codigo) != '' ) {
               
             quantcaracteres = $F(rubrica_codigo).length;
             
             for ( i=quantcaracteres;i<4;i++ ) {
               $(rubrica_codigo).setValue("0"+$F(rubrica_codigo));
             }
             
             js_OpenJanelaIframe('CurrentWindow.corpo.iframe_rhrubricas','db_iframe_rhrubricas','func_rhrubricas.php?pesquisa_chave='+$F(rubrica_codigo)+'&funcao_js=parent.js_mostrarubricas','Pesquisa',false);
           } else { 
             $(rubrica_descr).setValue(''); 
           }
        }
      }
  
      /**
      * Trata o retorno da funcao js_pesquisarrubric()
      */
      function js_mostrarubricas(sChave, lErro) {
          
        $(rubrica_descr).setValue(sChave);
        if ( lErro ) {
            
          $(rubrica_codigo).setValue('');
          $(rubrica_codigo).focus();
        }
      }
      ...
      */
      function js_mostrarubricas1(sChave1,sChave2){
        ...
        $(rubrica_codigo).setValue(sChave1);
        $(rubrica_descr).setValue(sChave2);
        $(db_iframe_rhrubricas).hide();
      }
  
    function imposibilitaSelecaoRubricaFixa() {
      const rubrica = $F('rh27_rubric');
      const select  = $('rh27_pd');
      const opcoes = select.options;

      rubricaFixa = ['R985', 'R986', 'R987', 'R992', 'R993', 'R950', 'R951', 'R952', 'R953', 'R954', 'R974', 'R975', 'R981', 'R982', 'R983'];

      if (rubricaFixa.includes(rubrica)) {
          opcoes[0].disabled = true;
          opcoes[1].disabled = true;

          opcoes[2].disabled = false;
          opcoes[3].disabled = false;
...
window.addEventListener('load', imposibilitaSelecaoRubricaFixa);
