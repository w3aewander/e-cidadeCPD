...
class UpdateFileTest extends Command
{
    protected function configure()
    {
        $this
            ->setName('modification:update-file-test')
            ->setDescription('Test a update modifications file')
            ->setHelp('Test a update modifications file');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $result = $manager->updateFileTest($file);

        if (!$result) {
            throw new \Exception('Arquivo não pode ser atualizado.');
        }

        $logger->debug("Nenhum erro encontrado.");
