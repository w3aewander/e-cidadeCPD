...
const MENSAGEM_PEDIDOTFD_RPC = "saude.tfd.fd4_pedidotfdRPC.";
...
if ($oParam->exec == 'getCgsCns') {
    ...
    $sSql = $oDaoCgsCartaoSus->sql_query(
        null,
        'cgs_und.z01_i_cgsund, z01_v_nome',
        null,
        ' s115_c_cartaosus = \'' . $oParam->iCns . '\''
    );    
    ...
    $sqlParametros = $oDaoTfdParametros->sql_query();
    $rsParametros = db_query($sqlParametros);
    
    if(pg_num_rows($rsParametros) > 0){
      $parametros = db_utils::fieldsMemory($rsParametros,0);
      $validaMunicipio  = $parametros->tf11_alertamunicipio;    
    }
    
    $oRetorno->residenciaMunicipio = true;
    if($validaMunicipio == 't'){
    
        $municipioPrefeitura = '?@!#';

        $daoDbConfig = new cl_db_config;
        $sqlConfig = $daoDbConfig->sql_query_file(null,"distinct munic as municipio");
        $rsConfig = db_query($sqlConfig);
        if(pg_num_rows($rsConfig) > 0){
          $municipioPrefeitura = db_utils::fieldsMemory($rsConfig,0)->municipio;
        }    
    
        $oRetorno->municipioPaciente = $municipioPrefeitura;

        $daoCgsEndereco = new cl_cgs_undendereco;
        $campos="distinct db72_descricao";
        $where = "sd109_cgs_und = {$oParam->iCgs}";
        $sqlEndereco = $daoCgsEndereco->sql_query_ibge(null,$campos,null,$where);        
        $rsEndereco = db_query($sqlEndereco);

        if(pg_num_rows($rsEndereco) > 0) {
          $municipioPaciente = db_utils::fieldsMemory($rsEndereco,0)->db72_descricao;

          /**
           * Compara os municipios sem espaço e  sem considerar letras maisculas/minusculas
           */

          if(stripos(str_replace(' ', '', $municipioPaciente), 
             str_replace(' ', '', $municipioPrefeitura)) === false
            ){
            $oRetorno->residenciaMunicipio = false;     
            $oRetorno->municipioPaciente = $municipioPaciente; 
          }          
        }    
    }
    ...
    if ($oDaoCgsUnd->numrows > 0) { // se encontrou o cgs
        ...
        if ($codigoIbge === '') {
            ...
            $sql = $dao->sql_query('', 'db125_codigosistema', '', "db72_descricao like '{$oDadosCgsUnd->z01_v_munic}'");
            $rs = $dao->sql_record($sql);
            if ($dao->numrows > 0) {
                $codigoIbge = \db_utils::fieldsMemory($rs, 0)->db125_codigosistema;
            }
        }
        $oRetorno->z01_codigoibge = $codigoIbge;
    } else {
    ...
    $sSql = $oDaoCgsCartaoSus->sql_query(
        null,
        ' s115_c_cartaosus, s115_c_tipo, s115_i_codigo ',
        ' s115_c_tipo asc ',
    ...
    try {
        ...
        if (cpfEmUso($oDaoCgsUnd, $oParam)) {
            throw new DomainException('CPF já existente na base de dados.');
        }
        ...
        $sComplemento = !empty($oParam->z01_v_compl)
            ? db_stdClass::normalizeStringJsonEscapeString($oParam->z01_v_compl)
            : 'null';
        $sCep = !empty($oParam->z01_v_cep) ? $oParam->z01_v_cep : 'null';
        $sCgcCpf = !empty($oParam->z01_v_cgccpf) ? $oParam->z01_v_cgccpf : 'null';
        $sIdentidade = !empty($oParam->z01_v_ident)
            ? db_stdClass::normalizeStringJsonEscapeString($oParam->z01_v_ident)
            : 'null';
        $sPai = !empty($oParam->z01_v_pai)
            ? db_stdClass::normalizeStringJsonEscapeString($oParam->z01_v_pai)
            : 'null';
        $sEmail = !empty($oParam->z01_v_email)
            ? db_stdClass::normalizeStringJsonEscapeString($oParam->z01_v_email)
            : 'null';
        ...
        if ($oDaoCgsUnd->erro_status == '0') {
            throw new Exception($oDaoCgsUnd->erro_msg);
        }

        $dao = new cl_cgs();
        $sql = $dao->sql_query_file($oParam->iCgs);
        $rs = $dao->sql_record($sql);
        if ($dao->numrows == 0) {
            throw new Exception('Erro ao buscar CGS.');
        }
        ...
        $model = new Cgs($oParam->iCgs);
        $model->salvarCgsPadrao($dados);
        $model->salvarCgsCartaoSus($dados);
    ...
    $sSql = $oDaoTfdTipoTratamentoDoc->sql_query_file(
        null,
        '*',
        null,
    ...
    $campos = ' tf01_i_codigo, tf16_d_dataagendamento, tf17_d_datasaida, z01_nome, tf03_i_codigo, tf03_c_descr';
    $campos .= ', tf26_i_codigo, tf26_c_descr, tf01_d_datapedido, rh70_sequencial, rh70_estrutural';
    $campos .= ', rh70_descr, tf16_c_horaagendamento, tf04_c_descr, sd63_c_nome';
    $sWhere = "tf01_i_cgsund = {$oParam->iCgs}";
    $sSql = $oDaoTfdPedidoTfd->sql_query_grid(null, $campos, ' tf01_i_codigo desc ', $sWhere);
    ...
    for ($iCont = 0; $iCont < $oDaoTfdPedidoTfd->numrows; $iCont++) {
        ...
        $oRetorno->oPedidos[$iCont]->tf16_c_horaagendamento = $oDados->tf16_c_horaagendamento;
        $oRetorno->oPedidos[$iCont]->tf04_c_descr = $oDados->tf04_c_descr;
        $oRetorno->oPedidos[$iCont]->sd63_c_nome = $oDados->sd63_c_nome;
    }
    ...
    $sSql = $oDaoTfdAcompanhantes->sql_query(
        null,
        $sCampos,
        ' tf13_i_codigo ',
    ...
    $parametros = \ECidade\Saude\Tfd\Helper\ParametrosHelper::get();
    $oDaoCgsUnd = new cl_cgs_und();
    $sCampos = " tf01_i_codigo, z01_i_cgsund, z01_v_nome, z01_v_ident, ";
    $sCampos .= " z01_v_cgccpf, z01_nome, z01_i_familiamicroarea , tipo ";
    ...
    */
    $sCamposSub = ' select tf19_i_pedidotfd ';
    ...
    if ($parametros->obrigaHoraSaida) {
        $sSubSelect .= "       and tf18_c_horasaida = '{$oParam->sHora}'";
    }
    $sSubSelect .= '       and tf18_i_destino = ' . $oParam->iDestino;
    $sSubSelect .= '       and tf19_i_valido  = 1 ';
    $sSubSelect .= ' and not exists (select 1 from cancelamentoviagem where tf41_veiculodestino = tf18_i_codigo) ';

    /* o Subselect possui todos os pedidos que já foram vinculadas a algum veículo */
    $where = [];
    $where[] = " tf17_d_datasaida = '{$oParam->sData}' ";
    if ($parametros->obrigaHoraSaida) {
        $where[] = " tf17_c_horasaida = '{$oParam->sHora}' ";
    }
    $where[] = "tf17_tiposaida <> 2 ";
    $where[] = "tf25_i_destino = {$oParam->iDestino}";
    $where[] = "tf13_i_anulado = 2";
    $where[] = "tf01_i_situacao = 1";
    $where[] = "tf01_i_codigo not in ({$sCamposSub}{$sSubSelect})";
    $where = implode(' AND ', $where);
    $sSql = $oDaoCgsUnd->sql_query_cgs_beneficiadosajudacusto(
        null,
        $sCampos . ', 2 as vinculado, ' .
        '2 as tf19_i_colo, 2 as tf19_i_fica',
        "tipo,z01_v_nome",
        $where
    );
    ...
    */
    $oRetorno->aListaCgs = [];
    $iNumJaVinc = 0;
    if ($oParam->sHora != '' || $parametros->obrigaHoraSaida) {
        ...
        $sSql = $oDaoCgsUnd->sql_query_cgs_beneficiadosajudacusto(
            null,
            $sCampos . ', 1 as vinculado',
            "tipo,z01_v_nome",
            $sWhereCgsvinculadoVeiculo
        );
        ...
        for ($iCont = 0; $iCont < $iNumJaVinc; $iCont++) {
            $oRetorno->aListaCgs[] = urlencode_all(db_utils::fieldsmemory($rsJaVinc, $iCont));
        }
    }

    for ($iCont = 0; $iCont < $iNumSemVeic; $iCont++) {
        $oRetorno->aListaCgs[] = urlencode_all(db_utils::fieldsMemory($rsSemVeic, $iCont));
    }
    ...
    /**
     * Cria um array com os itens agrupados pelos código
     */
    $agrupadosPedido = [];

    foreach ($oRetorno->aListaCgs as $item) {

        $codigo = $item->tf01_i_codigo;
        if (!isset($agrupadosPedido[$codigo])) {
            $agrupadosPedido[$codigo] = [];
        }
        $agrupadosPedido[$codigo][] = $item;
    }

    /**
     * Processa os dados agrupados para ordenar corretamente
     * a lista dos cgs
     */
    $listaCgsOrdenados = [];
    foreach($agrupadosPedido as $agrupadoPedido){
        foreach($agrupadoPedido as $itemCgsAgrupado){
            $listaCgsOrdenados[] = $itemCgsAgrupado;
        }
    }

    $oRetorno->aListaCgs = $listaCgsOrdenados;
    ...
    if ($iNumJaVinc > 0) {
        ...
        if (isset($oParam->iCodigo) && !empty($oParam->iCodigo)) {
            ...
            $sWhere .= " and tf18_d_datasaida = '{$oParam->sData}'";
            if ($oParam->sHora != '' || $parametros->obrigaHoraSaida) {
                $sWhere .= " and tf18_c_horasaida = '{$oParam->sHora}'";
            }
    ...
    $sSql = $oDaoTfdPassageiroVeiculo->sql_query_file(
        null,
        'count(*) as reservados',
        '',
    ...
    $sSql = $oDaoTfdPassageiroRetorno->sql_query_file(
        null,
        'count(*) as retorno',
        '',
    ...
    $sSql = $oDaoTfdVeiculoDestino->sql_query2(
        null,
        $sCampos,
        'tf18_c_horasaida',
    ...
    $sSql = $oDaoEspecMedico->sql_query_especmedico(
        '',
        $sCampos,
        'rh70_descr',
        $sWhere
    ...
    for ($iI = 0; $iI < $oParam->iNumReg; $iI++) {
        ...
        if ($tf16_i_codigo != '') {
            ...
            if ($oDaotfd_agendasaida->numrows > 0) {
                ...
                db_fim_transacao($oDaotfd_agendamentoprestadora->erro_status == '0');
            }
        } else {
            db_inicio_transacao();
...
} elseif ($oParam->exec == "getPedidoTfd") {
    try {
...
/**
 * @param cl_cgs_und $dao
 * @param $dados
 * @return bool
 */
function cpfEmUso($dao, $dados) {
    if ($dados->z01_v_cgccpf) {
        $campos = ' z01_i_cgsund ';
        $where = " z01_v_cgccpf = '{$dados->z01_v_cgccpf}' ";
        $where .= " and z01_i_cgsund <> '{$dados->iCgs}' ";
