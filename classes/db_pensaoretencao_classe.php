...
class cl_pensaoretencao
{
   // cria variaveis de erro 
    ...
    public $rh77_sequencial = 0; 
    public $rh77_numcgm = 0; 
    public $rh77_regist = 0; 
    public $rh77_anousu = 0; 
    public $rh77_mesusu = 0; 
    public $rh77_retencaotiporec = 0; 
    public $rh77_cgmalimentado = 0; 
   // cria propriedade com as variaveis do arquivo 
    public $campos = "
                 ...
                 rh77_cgmalimentado = int4 = CGM Alimentado 
                 ";
    ...
    public function atualizacampos($exclusao = false)
    {
     if($exclusao==false){
       ...
       $this->rh77_cgmalimentado = ($this->rh77_cgmalimentado == ""?@$GLOBALS["HTTP_POST_VARS"]["rh77_cgmalimentado"]:$this->rh77_cgmalimentado);
     }else{
       $this->rh77_sequencial = ($this->rh77_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["rh77_sequencial"]:$this->rh77_sequencial);
     }
   }

    public function incluir($rh77_sequencial)
    {
      $this->atualizacampos();
     ...
     if($this->rh77_retencaotiporec == null ){ 
       $this->erro_sql = " Campo Retenção não informado.";
     ...
     if($this->rh77_cgmalimentado == null ){ 
       $this->erro_sql = " Campo CGM Alimentado não informado.";
       $this->erro_campo = "rh77_cgmalimentado";
       $this->erro_banco = "";
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
     ...
     $sql = "insert into pensaoretencao(
                ...
                values (
                               ...
                               ,$this->rh77_cgmalimentado 
                      )";
     $result = db_query($sql); 
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "pensaoretencao ($this->rh77_sequencial) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,2539,135485118,'','".AddSlashes(pg_result($resaco,0,'rh77_cgmalimentado'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 

    public function alterar($rh77_sequencial=null)
    {
      $this->atualizacampos();
     ...
     if(trim($this->rh77_cgmalimentado)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh77_cgmalimentado"])){ 
       $sql  .= $virgula." rh77_cgmalimentado = $this->rh77_cgmalimentado ";
       $virgula = ",";
       if(trim($this->rh77_cgmalimentado) == null ){ 
         $this->erro_sql = " Campo CGM Alimentado não informado.";
         $this->erro_campo = "rh77_cgmalimentado";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["rh77_cgmalimentado"]) || $this->rh77_cgmalimentado != "")
             $resac = db_query("insert into db_acount values($acount,2539,135485118,'".AddSlashes(pg_result($resaco,$conresaco,'rh77_cgmalimentado'))."','$this->rh77_cgmalimentado',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     if (!$result) { 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "pensaoretencao não Alterado. Alteração Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->rh77_sequencial;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "pensaoretencao não foi Alterado. Alteração Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->rh77_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
    ...
    public function excluir($rh77_sequencial=null, $dbwhere = null)
    {
     $lSessaoDesativarAccount = db_getsession("DB_desativar_account", false);
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       && ($lSessaoDesativarAccount === false))) {
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,2539,135485118,'','".AddSlashes(pg_result($resaco,$iresaco,'rh77_cgmalimentado'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     ...
     if ($result == false) { 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "pensaoretencao não Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "pensaoretencao não Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$rh77_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
  ...
  function sql_query_retorno ( $rh77_sequencial=null,$campos="*",$ordem=null,$dbwhere="") { 
     ...
     $sql .= "                        and  pensao.r52_cgmalimentado = pensaoretencao.rh77_cgmalimentado";
