...
class itemSolicitacao {
  ...
  function __construct($iItemSolicitacao = null, $iMaterial = null) {
    ...
    if (!empty($iItemSolicitacao)) {
      ...
      if ($oDaoSolicitem->numrows > 0) {
        ...
        $this->sDescricaoMaterial     = urldecode($oItem->pc01_descrmater);
        ...
        $this->sUnidadeDescr          = $oItem->m61_descr;
  ...
  public function getUnidadeDescr()
  {
      return $this->sUnidadeDescr;
  }
    public function getFornecedorItem($codCompilacao, $codMaterial)
    {
        ...
        $sSqlVencedores .= "       pc11_seq as ordem";
        ...
        $sSqlVencedores .= " where pc11_numero    = {$codCompilacao}";
        $sSqlVencedores .= "   and pc24_pontuacao = 1";
        $sSqlVencedores .= "   and pc01_codmater  = {$codMaterial}";
        $sSqlVencedores .= " order by pc23_orcamitem ";
        $rsVencedores    = db_query($sSqlVencedores);

        $itemFornecedores = [];
        while ($state = pg_fetch_object($rsVencedores)) {
            $itemFornecedores[$state->ordem] = $state;
        }
  ...
  public function remover() {
    ...
    if ($this->iCodigoItemSolicitacao != null) {
      ...
      $oDaoSolicitem->reordenarItens($this->getCodigoSolicitacao());
      if ($oDaoSolicitem->erro_status == 0) {
  ...
  public static function getUltimosOrcamentos($iCodigoItem=null, $aUnidades=null, $iFornecedor=null, $dtInicial=null, $dtFinal=null) {
  ...
  public static function calculaMediaPrecoOrcamentos($aMediasPrecos) {
   ...
   * @return array
   */
    public function getReservasSaldoDotacao($iCodigoDotacao, $solicitem = 0)
    {
        $condicoes = [];
        $condicoes[] = "o82_pcdotac = {$iCodigoDotacao}";

        if (!empty($solicitem)) {
            $condicoes[] = "o82_solicitem = {$solicitem}";
        }

        $condicoesSql = implode($condicoes, ' AND ');
