 ...
 *
 * @method static InicialPartilhaCustas getInstance()
 */
class InicialPartilhaCustas extends \BaseClassRepository
{
    ...
    public function delete(InicialPartilhaCustasEntity $oCustas)
    {
        ...
        if (!$lResult) {
            $sMsg = "Erro ao apagar a custas da taxa {$oCustas->getCodigoTaxa()} ";
            $sMsg .= "da partilha {$oCustas->getCodigoInicialPartilha()}";
    ...
    public function getByCustaInicialPartilha($iCodigoCusta, $iCodigoPartilha)
    {
        ...
        if (!$rsResult) {
            $sMsg = "Ocorreu um erro ao buscar as Custas {$iCodigoCusta} da Partilha {$iCodigoPartilha}.";
            throw new DBException($sMsg);
        }
     ...
     * @throws DBException
     */
    public function getDadosRecibo($iNumnov, $inicial = null)
    {
        $oDao = new cl_inicialpartilhacustas();
        $sSql = $oDao->sql_query_recibo_custas($iNumnov, $inicial);
