...
class EtapaRepository {
  ...
  public static function removeAll() {
     ...
     * @throws Exception
     */
    public static function getByEscolaAndCalendario(Escola $escola, Calendario $calendario)
    {
        $sql = "
            SELECT DISTINCT ed11_i_codigo, ed11_c_descr, ed10_ordem, ed11_i_sequencia
            FROM serie
                   JOIN ensino ON ensino.ed10_i_codigo = serie.ed11_i_ensino
                   JOIN serieregimemat ON serieregimemat.ed223_i_serie = serie.ed11_i_codigo
                   JOIN turmaserieregimemat ON turmaserieregimemat.ed220_i_serieregimemat = serieregimemat.ed223_i_codigo
                   JOIN turma ON turma.ed57_i_codigo = turmaserieregimemat.ed220_i_turma
            ...
            WHERE ed18_i_codigo = {$escola->getCodigo()} AND ed52_i_codigo = {$calendario->getCodigo()}
            ORDER BY ed10_ordem, ed11_i_sequencia;
        ";
        ...
        while ($etapa = pg_fetch_object($rs)) {
            $etapas[] = EtapaRepository::getEtapaByCodigo($etapa->ed11_i_codigo);
        }
     ...
     * @throws Exception
     */
    public static function getVagasByEtapa(Etapa $etapa, Escola $escola, Calendario $calendario)
    {
        $sql = "
            SELECT coalesce(sum(ed336_vagas), 0) AS total
            FROM (SELECT DISTINCT ed57_i_codigo, ed336_vagas
                  FROM (SELECT ed57_i_codigo, ed223_i_serie
                        FROM escola
                               JOIN calendarioescola ON ed38_i_escola = ed18_i_codigo
                               JOIN calendario ON ed52_i_codigo = ed38_i_calendario
                               JOIN turma ON ed57_i_calendario = ed38_i_calendario
                        ...
                        WHERE ed57_i_escola = {$escola->getCodigo()}
                      AND ed223_i_serie = {$etapa->getCodigo()}
                      AND ed52_i_codigo = {$calendario->getCodigo()}) AS x
                         JOIN turmaturnoreferente ON ed336_turma = ed57_i_codigo) AS vagas
        ";

        $rs = db_query($sql);

        if (!$rs) {
            throw new Exception("Não foi possível buscar quantas vagas estão disponíveis para a etapa {$etapa->getNome()}.\nContate o suporte.");
        }

        return pg_fetch_object($rs)->total;
    }

    /**
     * @param $baseCurricular
     * @return []
     */
    public function getEtapasBase($codigoBase)
    {
        $daoBasemps = new cl_basemps();
        $campos = "ed34_i_serie, ed11_c_descr";
        $where = " ed34_i_base = {$codigoBase}";
        $sql = $daoBasemps->sql_query( null, $campos, 'ed11_i_sequencia', $where);
        $rs = db_query($sql);
        if (!$rs) {
            throw new Exception("Erro ao buscar Etapas da Base");
        }

        $etapas = [];
        while ($etapa = pg_fetch_assoc($rs)) {
            $array = (object)[
                'codigo' => $etapa['ed34_i_serie'],
                'descricao' => $etapa['ed11_c_descr']
            ];
            if (!in_array($array, $etapas)) {
                $etapas[] = $array;
            }
        }
