...
class EmpenhoFinanceiro
{
    ...
    public function isEmpenhoPassivo()
    {
        ...
        $oDaoEmpEmpAut = new cl_empempaut;
        $sSqlBuscaEmpenho = $oDaoEmpEmpAut->sql_query_empenho_inscricaopassivo($this->getNumero(), "e61_numemp");
    ...
    public function validaDadosDiaria2($seqempenho){
    ...
    {   
        //Se for prestação de contas, marca como suprimento de fundos em /classes/empenho linha 6430
        //return false;
        //var_dump($this->iNumero); die("Confere");
        $xverifica = $this->validaDadosDiaria2($this->iNumero);
        if($xverifica){return false;}
    ...
    public function getAcordo()
    {
        ...
        if (!empty($this->getCodigoContrato())) {

            return new Acordo($this->getCodigoContrato());
        }
    }
    ...
    /**
     * Retorna o codigo da lista de classificação de credor vinculada ao empenho.
     * @return int|null
     * @throws DBException
     */
    ...
    /**
     * Retorna a lista de classificação de credor vinculada ao empenho.
    ...
    /**
     * Verifica se o empenho é de recurso livre.
     *
     * Conforme o que foi conversado com Leandro em 11/07/2023 é considerado
     * recurso livre todos recursos das fontes [860, 861, 862, 869] e todos recursos
     * [500, 501] onde o complemento é 0
     *
     * @return bool
     * @throws Exception
     */
    public function isRecursoLivre()
    {
        $dado = $this->getRecursoEmpenho();
        if (in_array($dado->fonte, [860, 861, 862, 869])) {
            return true;
        }

        if (in_array($dado->fonte, [500, 501]) && $dado->o15_complemento == 0) {
            return true;
        }
        return false;
    }

    public function getRecursoEmpenho()
    {
        $exercicio = db_getsession('DB_anousu');
        $origem = $this->isRP($this->getAno()) ? 10 : 1;
        $where = " o206_origem = {$origem} and o206_numero = {$this->iNumero}";

        $sql = "
         select codigo_siconfi, substr(codigo_siconfi, 2) as fonte,  o15_complemento, o15_codigo
        ...
        $rs = db_query($sql);
        if (!$rs) {
            throw new Exception('Erro ao buscar o recurso do empenho.');
        }
