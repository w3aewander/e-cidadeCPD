...
for ($iCount = 0; $iCount < $iMedicamentosLinhas; $iCount++) {
    ...
    if ($iDispensacao == 1) { // tem que eliminar os registros de pacientes que não possuem saldo
        $infoContinuado = MedicamentoContinuadoService::getInfoContinuado(
            $oMedicamentos->z01_i_cgsund,
            $oMedicamentos->fa01_i_codigo,
            new DateTime($dHoje),
            $oDadosFarParametros->fa02_i_acumularsaldocontinuado == 1,
            $oDadosFarParametros->fa02_i_tipoperiodocontinuado == 1
        );

        if ($infoContinuado->saldo != 0) { // se possui saldo, então está pendente
...
for ($iCount = 0; $iCount < count($aMedicamentos); $iCount++) {
    if (($pdf->gety() > $pdf->h - 30)
        ...
        $texto = "Total de Pacientes: " . $aSomaMedicamento[$aMedicamentos[$iCount]["fa01_i_codigo"]];
        $pdf->cell(50, 5, $texto, "TRB", 1, "R", 1);
