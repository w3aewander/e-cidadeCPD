...
foreach ($aMatricula as $iMatricula) {
  ...
  if ($iContador % 3 == 1) {
    ...
    $iAlturaFinal   = $iAlturaInicial + 65;
    $oPdf->Rect(7, $iAlturaInicial, 97, 65);
    $oPdf->Rect(7, $iAlturaInicial, 97, 17);
    ...
    $iAlturaInicial = $iAlturaInicial + 75;
    $iAlturaFinal   = $iAlturaInicial + 70;
    $oPdf->Rect(7, $iAlturaInicial, 97, 65);
    $oPdf->Rect(7, $iAlturaInicial, 97, 17);

  } if ($iContador % 3 == 0) {

    $iAlturaInicial = $iAlturaInicial + 150;
    $iAlturaFinal   = $iAlturaInicial + 140;
    $oPdf->Rect(7, $iAlturaInicial, 97, 65);
    $oPdf->Rect(7, $iAlturaInicial, 97, 17);
  }
   ...
   */
  $nome = is_null($oMatricula->getAluno()->getNomeSocial()) || empty($oMatricula->getAluno()->getNomeSocial()) ?
      $oMatricula->getAluno()->getNome() : $oMatricula->getAluno()->getNomeSocial();
  $oPdf->Cell(65,   $iHeigth, "Acompanhamento do Rendimento do Aluno /".$oDadosTurma->anocalendario, 0, 0);
  $oPdf->Cell(20,  $iHeigth, "Nº " . $oMatricula->getAluno()->getCodigoAluno(), 0, 1);
  // $oPdf->Cell(20,  $iHeigth, "Ano " . $oDadosTurma->anocalendario, 0, 1);
  $oPdf->Cell(7,   $iHeigth, "{$sNomeEscola}" , 0, 1);
  $oPdf->Cell(130, $iHeigth, "Aluno(a): " . $nome, 0, 1);
  $oPdf->Cell(60,  $iHeigth, "Turma: " . $oDadosTurma->turma, 0, 0);
  ...
  foreach ($oGradeAproveitamento->getPeriodos() as $oPeriodo) {
    ...
    if ($oPeriodo instanceof AvaliacaoPeriodica) {
      ...
      $oPdf->Cell($iTamanhoColunaPeriodo,  8, $oPeriodo->getTipoResultado()->getDescricaoAbreviada(), 1, 1, "C");
    }
  }

  // $iTamanhoDoQuadroDeParecer   = ($oPdf->w - 15) - (45 + ($iTamanhoColunaPeriodo * ($iNumeroDePeriodosAvaliacao)));
  // $oPdf->Cell($iTamanhoDoQuadroDeParecer, 8, "RESULTADO FINAL", 1, 1, "C");
  ...
  foreach ($aTotalFaltas as $aFaltas) {
    ...
    foreach ($aFaltas as $iFalta) {
      $iTotalFaltaPeriodo += intval($iFalta);
    }
...
/** Controla quantos alunos sao impressos por pagina */ 
  if ($iContador % 3 == 0 && $iTotalMatriculas > 3 && ($iContador != $iTotalMatriculas)) {
    $oPdf->AddPage();
  }
  $iContador++;
}
