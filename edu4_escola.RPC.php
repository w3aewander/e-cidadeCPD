...
if ($oParam->exec == 'getDadosUltimaMatriculaAluno') {
    ...
    try {
        if (!isset($oParam->iCartorio)) {
            $oRetorno->iStatus = 0;
            throw new Exception("Cartório não informado");
        }

        //dados Cartorio
        $oDaoCensoCartorio = new cl_censocartorio();
        $rsResultCartorio = $oDaoCensoCartorio->sql_record(
            $oDaoCensoCartorio->sql_query(
                null,
                "*",
                null,
                " ed291_i_serventia = {$oParam->iCartorio} "
            )
        );
        if ($oDaoCensoCartorio->numrows == 0) {
            $oRetorno->iStatus = 3;
            throw new Exception("Não foi possível localizar nenhum cartório com o código: {$oParam->iCartorio}");
        }
        ...
        $sSqlCensoMunic = $oDaoCensoMunic->sql_query(
            ...
            "*",
            "",
            " ed261_i_codigo = {$oDadosCartorio->ed291_i_censomunic} "
        );
        $rsResultCensoMunic = $oDaoCensoMunic->sql_record($sSqlCensoMunic);
        if ($oDaoCensoMunic->numrows == 0) {
            $oRetorno->iStatus = 0;
            throw new Exception("Não foi possível localizar o município do cartório!");
        }
        ...
        if ($oDaoCensoUf->numrows == 0) {
            $oRetorno->iStatus = 0;
            throw new Exception("Não foi possível localizar a UF do cartório!");
        }
        ...
        $oRetorno->sMessage = urlencode($e->getMessage());
    }
    ...
    if (isset($oParam->iAtoLegal)) {
        $oDaoAnexoAtoLegal = new cl_edu_anexoatolegal();
    ...
    if (isset($oParam->iAtoLegal) && isset($oParam->iEscola)) {
        ...
        if ($oDaoAtoLegal->numrows > 0) {
            ...
            for ($iCont1 = 0; $iCont1 < $iLinhasCurso; $iCont1++) {
                $oDaoAtoLegalBase = new cl_atolegal();
                ...
                if ($oDaoAtoLegalBase->numrows > 0) {
                    for ($iCont2 = 0; $iCont2 < $oDaoAtoLegalBase->numrows; $iCont2++) {
                        $oDaoAtoLegalEtapas = new cl_atolegal();
                        ...
                        $sSqlInFim .= "        INNER JOIN serie AS inicio ";
                        $sSqlInFim .= "                ON inicio.ed11_i_codigo = baseserie.ed87_i_serieinicial ";
                        $sSqlInFim .= "        INNER JOIN serie AS fim ";
                        $sSqlInFim .= "                ON fim.ed11_i_codigo = baseserie.ed87_i_seriefinal ";
    ...
    $oDaoCalendario = new cl_serie();
    $sWhereCalendario = "ed38_i_escola = {$iEscola} AND ed52_c_passivo = 'N'";
    $sWhereCalendario .= " and EXISTS( select 1
                                         from regencia
    ...
    if (isset($oParam->escola)) {
        ...
        $sWhereSec  = " ed75_i_escola = $oParam->escola ";
        $sWhereSec .= " AND ed01_i_funcaoadmin = 3 ";
        $sWhereSec .= " AND (ed22_datafim is null OR ed22_datafim >= '".date('Y-m-d')."') ";
    ...
    if (isset($oParam->escola)) {
        $oDaoTipoEnsino = new cl_tipoensino();
        $sSqlTipoEnsino = $oDaoTipoEnsino->sql_query_file(
            "",
            "ed36_i_codigo,ed36_c_descr,ed36_c_abrev",
            "ed36_i_codigo",
            ""
        );
    ...
    if (isset($oParam->tipoensino)) {
        ...
        $sSqlNivelEnsino = $oDaoEnsino->sql_query(
            "",
            "distinct ed10_i_codigo,ed10_c_descr",
            "",
            "ed10_i_tipoensino =" . $oParam->tipoensino
        );
    ...
    try {
        ...
        if ($oParam->tipoaluno == "3") {
            ...
            $sCampos  = " distinct ed47_i_codigo,to_ascii(ed47_v_nome) as ed47_v_nome,ed60_i_codigo";
            $sCampos .= ",escola.ed18_i_codigo as escola";
    ...
    $oDaoAreasConhecimento = new cl_areaconhecimento();
    ...
    if (isset($oParam->iCodigo)) {
        $oDaoCadDisciplinas = new cl_censocaddisciplina();
    ...
    if (isset($oParam->iCurso) && isset($oParam->iEscola)) {
        ...
        $sCamposSerie  = " distinct ed11_i_codigo,ed11_c_descr,ed11_c_abrev";
        $sCamposSerie .= ",ed11_i_ensino,ed29_c_descr,ed10_i_codigo,ed11_i_sequencia";
    ...
    if (isset($oParam->escola)) {
        ...
        if ($iLinhas > 0) {
            ...
            $msg = "Não foi possível localizar Diretores/Secretários Para a Escola Solicitada!";
            $oRetorno->iStatus = 0;
            $oRetorno->sMessage = urlencode($msg);
        }
    } else {
        $oRetorno->iStatus = 0;
...
} elseif ($oParam->exec == 'getTurmas') {
    $oDaoTurma = new cl_serie();
    ...
    if (isset($oParam->lEncerrada) && trim($oParam->lEncerrada) == "true") {
        $sWhereTurma .= " and EXISTS(select 1
                                       from regencia
                                      where ed59_c_encerrada = 'S'
                                        and ed59_i_turma = ed57_i_codigo) ";
    }
...
} elseif ($oParam->exec == 'getDisciplinasTurma') {
    $sWhere = "";
...
} elseif ($oParam->exec == 'getPeriodosAvaliacaoEscola') {
    $sWhere = '';
...
} elseif ($oParam->exec == 'PesquisaCalendarioAnoAgrupado') {
    $oDaoCalendario = new cl_calendario();
    ...
    try {
        ...
        if (!isset($oParam->calendario)) {
            throw new Exception("Ano não informado para a consulta das etapas");
        }
        $where[] = "ed52_i_ano = {$oParam->calendario}";

        if (isset($oParam->escola) && !empty($oParam->escola)) {
            $where[] = "ed57_i_escola = {$oParam->escola}";
        }
        $sqlEtapa = $oDaoTurma->sql_query_relatorio(
            "",
        ...
        $rsEtapa = $oDaoTurma->sql_record($sqlEtapa);
        if ($oDaoTurma->numrows == 0) {
            throw new Exception("Não foram encontradas etapas!");
        }

        $oRetorno->aResult1 = db_utils::getCollectionByRecord($rsEtapa, false, false, true);
    } catch (Exception $e) {
        $oRetorno->iStatus = 0;
...
} elseif ($oParam->exec == 'verificaSituacaoDocumentacao') {
    try {
        ...
        $sSqlAluno = $oDaoAluno->sql_query_file(
            null,
            "ed47_situacaodocumentacao, ed47_v_ident, ed47_c_certidaonum,ed47_v_cnh, ed47_v_cpf, ed47_c_passaporte",
            null,
            "ed47_i_codigo = {$oParam->iAluno}"
        );
...
} elseif ($oParam->exec == 'salvaSituacaoDocumentacao') {
    try {
        db_inicio_transacao();
        ...
        if ($oDaoAluno->numrows > 0) {
            $oDaoAlteracaoAluno = new cl_aluno();
...
} elseif ($oParam->exec == 'getBairrosQuadroVagas') {
...
} elseif ($oParam->exec == 'getCalendariosBairrosQuadroVagas') {
    $cod_bairro = "";
...
} else if ($oParam->exec == 'getCodigoCalendario') {
        $calendarios = "'" . implode("','", $oParam->calendarios) . "'";

        $cCalendario = "SELECT DISTINCT ed52_i_codigo
                                   FROM calendario
                             INNER JOIN turma  ON ed57_i_calendario = ed52_i_codigo
                             INNER JOIN escola ON ed57_i_escola     = ed18_i_codigo
                                  WHERE ed52_i_ano > 2018
                                    AND ed52_c_descr IN ($calendarios)
                               ORDER BY ed52_i_codigo";
        $rsCalendario = db_query($cCalendario);
        $nCalendario  = pg_num_rows($rsCalendario);
    if ($nCalendario > 0) {
        $oRetorno->aResult = db_utils::getCollectionByRecord($rsCalendario, false, false, true);
    } else {
        $oRetorno->iStatus = 0;
        $oRetorno->sMessage = urlencode("Não foi possível encontrar os códigos dos calendários!");
    }
} elseif ($oParam->exec == 'getEtapasBairrosQuadroVagas') {
    $cod_bairro = "";
    $cod_calendario = "";
    for ($abc = 0, $abcMax = count($oParam->bairros); $abc < $abcMax; $abc++) {
        if ($abc > 0) {
    ...
    $bEtapa = "SELECT DISTINCT ed11_i_codigo,
                    ...
                    INNER JOIN escola ON ed18_i_codigo = ed57_i_escola
                         WHERE ed57_i_calendario IN (SELECT ed52_i_codigo
                                                       FROM calendario
                                                      WHERE ed52_c_descr IN('$cod_calendario')
                                                        AND ed52_i_ano > 2018)
                           AND ed18_i_bairro IN($cod_bairro)
                      ORDER BY 2";
...
} elseif ($oParam->exec == 'pesquisaMunicipios') {
    $clcensomunic = new cl_censomunic();

    $sqlMunicipios = $clcensomunic->sql_query_file(
        "",
        "ed261_i_codigo,ed261_c_nome",
        "ed261_c_nome",
        "ed261_i_censouf = {$oParam->uf}"
    );
    $rsResultMunic = $clcensomunic->sql_record($sqlMunicipios);

    $oRetorno->aResult = db_utils::getCollectionByRecord($rsResultMunic, false, false, true);
} elseif ($oParam->exec == 'pesquisaCartorios') {
    if ($oParam->municipio == "") {
        $oRetorno->aResult = [];
        return false;
    }
    ...
    $sqlCartorio = $clcensocartorio->sql_query_file(
        "",
        "ed291_i_codigo,ed291_c_nome",
        "ed291_c_nome",
        "ed291_i_censomunic = {$oParam->municipio}"
    );
    $rsResultCartorio = $clcensocartorio->sql_record($sqlCartorio);

    $oRetorno->aResult = db_utils::getCollectionByRecord($rsResultCartorio, false, false, true);
} elseif ($oParam->exec == 'buscaAlunosNumeroCertidaoMatricula') {
    if ($oParam->numeroCertidao == "") {
        $oRetorno->aResult = [];
        return false;
    }
    ...
    $sWhere = " trim(ed47_certidaomatricula) = trim('{$oParam->numeroCertidao}')";
    if (isset($oParam->codigoAluno) && ! empty($oParam->codigoAluno)) {
        if (isset($oParam->consideraAluno) && $oParam->consideraAluno == "t") {
            $sWhere .= " and ed47_i_codigo = {$oParam->codigoAluno}";
        } else {
            $sWhere .= " and ed47_i_codigo <> {$oParam->codigoAluno}";
        }
    }

    $sSqlAlunos = $claluno->sql_query_file(null, "ed47_i_codigo, ed47_v_nome", "ed47_v_nome", $sWhere);
    $rsAlunos = $claluno->sql_record($sSqlAlunos);

    $oRetorno->aResult = db_utils::getCollectionByRecord($rsAlunos, false, false, true);
} elseif ($oParam->exec == 'buscaAlunosDocumento') {
    try {
        $claluno = new cl_aluno();

        if (!isset($oParam->documentoValidacao) || empty($oParam->documentoValidacao)) {
            throw new Exception('Tipo de Documento não informado');
        }

        if (!isset($oParam->valor) || empty($oParam->valor)) {
            throw new Exception('Sem preenchimento da informação do documento');
        }

        $oRetorno->label = "";
        $campo = "";
        switch ($oParam->documentoValidacao) {
            case "identidade":
            case "ed47_v_ident":
                $campo = "ed47_v_ident";
                $oRetorno->label = "Identidade";
                break;
            case "CNH":
            case "ed47_v_cnh":
                $campo = "ed47_v_cnh";
            ...
            case "cartaoSUS":
            case "ed47_cartaosus":
                $campo = "ed47_cartaosus";
                $oRetorno->label = "Cartao SUS";
                break;
            case "passaporte":
            case "ed47_c_passaporte":
                $campo = "ed47_c_passaporte";
                $oRetorno->label = "Passaporte";
                break;
            default:
                throw new Exception('Tipo de Documento não configurado para validação');
            break;
        }

        $where = "{$campo} = '{$oParam->valor}'";
        if (isset($oParam->codigoAluno) && ! empty($oParam->codigoAluno)) {
            if (isset($oParam->consideraAluno) && $oParam->consideraAluno == "t") {
                $where .= " and ed47_i_codigo = {$oParam->codigoAluno}";
            } else {
                $where .= " and ed47_i_codigo <> {$oParam->codigoAluno}";
            }
        }

        $sqlVerificaDocumento = $claluno->sql_query_file(
            null,
            "ed47_i_codigo, ed47_v_nome",
            "ed47_v_nome",
            "{$where}"
        );
        $rsVerificaDocumento = $claluno->sql_record($sqlVerificaDocumento);

        $oRetorno->aResult = db_utils::getCollectionByRecord($rsVerificaDocumento, false, false, true);
    } catch (Exception $e) {
        $oRetorno->iStatus = 0;
        $oRetorno->sMessage = $e->getMessage();
    }
} elseif ($oParam->exec == "getEscolasBairros") {
    $campos = "j13_codi, j13_descr";
    $cl_bairro = new cl_bairro();
    $sqlBairros = $cl_bairro->sql_query(null, $campos, 'j13_descr', null);
    $rsBairros = $cl_bairro->sql_record($sqlBairros);
    $oRetorno->bairros = db_utils::getCollectionByRecord($rsBairros, false, false, true);
    
    $sqlEscolas = "select mo53_codigo, mo53_nome from plugins.escolas order by mo53_nome";

    $result = db_query($sqlEscolas);
    $oRetorno->escolas = db_utils::getCollectionByRecord($result, false, false, true);
} elseif ($oParam->exec == "processarRelatorioEscolasPorBairroCSV") {
    try {
        $filtros = [];
        $where = "";

        if ($oParam->filtroEscola > 0) {
            $filtros[] = "mo53_codigo = {$oParam->filtroEscola}";
        }

        if ($oParam->filtroBairro > 0) {
            $filtros[] = "j13_codi = {$oParam->filtroBairro}";
        }
        ...
        $sqlEscolasPorBairro = "
        select escolas.mo53_codigo, escolas.mo53_nome escola, bairro.j13_codi, bairro.j13_descr bairro
        from bairro
        ...
        $where
        order by escolas.mo53_nome, bairro.j13_descr";
        
        $result = db_query($sqlEscolasPorBairro);
        $numLinhas = pg_num_rows($result);
    
        $conteudo = "";
        $cabecalho = array("ESCOLA","BAIRRO");
    
        $linha = implode(";", $cabecalho) . "\n";
        $conteudo .= $linha;
    
        for ($x=0; $x < $numLinhas; $x++) {
            db_fieldsmemory($result, $x);
    
            $linha = array(trim($escola), trim($bairro));
            $linha = implode(";", $linha) . "\n";
            $conteudo .= $linha;
        }

        if ($numLinhas > 0) {
            $arquivoCSV = "tmp/relatorio_escolas_por_bairro".time().".csv";
            file_put_contents($arquivoCSV, $conteudo);
            $oRetorno->pathRelatorioCSV = $arquivoCSV;
        } else {
            $oRetorno->iStatus = 0;
            $oRetorno->pathRelatorioCSV = "";
            $oRetorno->sMessage = "Nenhum registro encontrado para os filtros informados.";
        }
    } catch (Exception $e) {
        $oRetorno->iStatus = 0;
        $oRetorno->sMessage = $e->getMessage();
