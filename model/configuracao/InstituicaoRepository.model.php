...
class InstituicaoRepository
{
    ...
    public static function getInstituicoesPorTipo($aTipos)
    {
        ...
        if ($cldb_config->numrows > 0) {
            ...
            for ($iInd = 0; $iInd < $cldb_config->numrows; $iInd++) {
                $aListaInstituicoes[] = self::getInstituicaoByCodigo(db_utils::fieldsMemory($rsInstituicao, $iInd)->codigo);
            }
        }
    ...
    public static function getInstituicoes()
    {
        ...
        $sSqlBuscaInstituicao = $oDaoInstituicao->sql_query_file(null, "codigo", 'codigo');
     ...
     * @return Instituicao[]
     */
    public static function getInstituicaoConsolida($codigo)
    {
        $dao = new cl_db_config();
        $rs = db_query($dao->sql_query_file(null, 'codigo', null, "tribinst = {$codigo}"));

        $instituicoes = [];
        while ($state = pg_fetch_array($rs)) {
            $instituicoes[] = self::getInstituicaoByCodigo($state['codigo']);
        }
