...
<div class="container">
  <form name="form1" method="post" action="">
    <fieldset>
      <legend>Fornecedor da Licitação</legend>
        ...
        <table class="form-container">
          ...
          <tr>
            <td nowrap>
              <label for="pc20_codorc"><?php echo $Lpc21_codorc; ?></label>
            </td>
            <td>
          ...
          <tr>
            <td nowrap>
              <label for="pc21_numcgm"><?php db_ancora($Lpc21_numcgm,"js_pesquisapc21_numcgm(true);",$db_opcao); ?></label>
            </td>
            <td>
              <?php
          ...
          <tr>
            <td nowrap>
              <label for="pc31_liclicitatipoempresa"><?php echo $Lpc31_liclicitatipoempresa; ?></label>
            </td>
            <td>
               <?php
                 $sSqlTipoEmpresas = $oDaoTipoEmpresa->sql_query(null,"*","l32_sequencial");
                 $rsTipoEmpresa    = $oDaoTipoEmpresa->sql_record($sSqlTipoEmpresas);
                 $tiposEmpresa = [0 => "Selecione"];
                 db_utils::makeCollectionFromRecord($rsTipoEmpresa, function ($retorno) use (&$tiposEmpresa)  {
                         $tiposEmpresa[$retorno->l32_sequencial] = $retorno->l32_descricao;
                 });
                 db_select("pc31_liclicitatipoempresa",$tiposEmpresa,true,$db_opcao);
               ?>
            </td>
          </tr>
          <tr>
            <td>
              <label  for="pc31_tipocondicao"><?php echo $Lpc31_tipocondicao; ?></label>
            </td>
            <td>
              <?php
                $aOpcoes = array(
          ...
          <tr>
            ...
            <td>
              ...
              ?>
              <label  for="pc31_horaretira" style="margin-left:32px"><?php echo $Lpc31_horaretira; ?></label>
              <?php
                 if ($db_opcao == 1){
                    $pc31_horaretira=db_hora();
                 }
                 db_input('pc31_horaretira',6,$Ipc31_horaretira,true,'text',$db_opcao);
              ?>
            </td>
          </tr>
          <tr>
            <td nowrap>
              <label  for="pc31_nomeretira"><?php echo $Lpc31_nomeretira; ?></label>
            </td>
            <td nowrap title="<?php echo $Tpc31_nomeretira; ?>">
              <?php db_input('pc31_nomeretira',52,"",true,'text',$db_opcao,""); ?>
            </td>
          </tr>
        </table>
    </fieldset>
    ...
    ?>
    <input name="<?=($db_opcao==1?"incluir":($db_opcao==2||$db_opcao==22?"alterar":"excluir"))?>" type="submit" id="db_opcao" value="<?=($db_opcao==1?"Incluir":($db_opcao==2||$db_opcao==22?"Alterar":"Excluir"))?>" <?=($db_botao==false?"disabled":"")?>  onclick="verificaTipoEmpresa(event)">
    ...
    <div style="margin-top: 5px">
      <?php
        ...
        $cliframe_alterar_excluir->sql     = $clpcorcamforne->sql_query_forneclic(null,"pc21_orcamforne,pc21_codorc,l32_descricao,pc21_numcgm,z01_nome","",$sWhere);
        $cliframe_alterar_excluir->campos  ="pc21_orcamforne,pc21_numcgm,z01_nome, l32_descricao";
...
<script type="text/javascript">
  new DBInputHora($('pc31_horaretira'));
  ...
  function verificaTipoEmpresa(event) {
      liclicitatipoempresa = document.getElementById('pc31_liclicitatipoempresa').value;
      numcgm = document.getElementById('pc21_numcgm').value;
      if (numcgm == "") {
          alert ("Fornecedor não informado, por favor, selecione.");
          event.preventDefault();
          return false;
      }
      if (liclicitatipoempresa == 0) {
          alert("Você deve selecionar o tipo da empresa.");
          event.preventDefault();
          return false;
      }
  }

  document.getElementById("pc31_liclicitatipoempresa").style.width      = '18em';
  document.getElementById("pc31_tipocondicao").style.width      = '18em';
