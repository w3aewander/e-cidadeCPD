...
class ConsultaDados {
  ...
  /**
   * Parte Inicial da Consulta de Dados, m�todo chamado no webservice
   *
  ...
  private function getResultadoSQL( $sMetodo ) {
    ...
    if (!$lEnviouQuery) {
       throw new SoapFault('e-Cidade', 'N�o Foi possivel enviar a Query.'.pg_last_error().'...');
    }
    ...
    while ( $oDados = pg_fetch_object( $rsSQL ) ) {
      ...
      if ( $iMemoriaUtilizada >= $iMemoriaLimite * 0.90 ) { // Deixa 10% da mem�ria para responder
  ...
  private function carregarXMLPesquisa( $sCaminhoXML ) {
    ...
    try {
      ...
      /**
       * Estruturando Vari�veis para criar a String SQL
       */
      ...
      foreach ( $oNodeJuncoes->getElementsByTagName('join') as $oNodeJoin ) {
        ...
        if ( strpos($sCondicaoJoin,'$') ) {
          foreach ($aFiltrosInformados as $key => $variavel) {

            $sCondicaoJoin = str_replace("\${$key}", $variavel, $sCondicaoJoin);
          }
        }
