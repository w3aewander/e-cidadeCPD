    ...
    public function gerarDados()
    {
        ...
        if ($iLinhasDotacaoSaldo > 0) {
            ...
            for ($iInd = 0; $iInd < $iLinhasDotacaoSaldo; $iInd ++) {
                ...
                if ($this->iAno == 2017) {
                    $sSqlDeParaFonte = "SELECT fonte_tce 
					                      FROM plugins.deparafontetce 
					                     WHERE orgao = $oDespesa->o58_orgao
                                           AND unidade = $oDespesa->o58_unidade
                                           AND fonte_orcamento = $oDespesa->o58_codigo
                                           AND exercicio = {$this->iAno}";

                    $fonte_tce = db_utils::fieldsMemory(db_query($sSqlDeParaFonte), 0)->fonte_tce;
                    $oDespesa->o58_codigo = $fonte_tce;
                    if (empty($fonte_tce)) {
                        $sMsgLogErro .= "\n";
                        $sMsgLogErro .= "Não encontrados registros na tabela plugins.deparafontetce: \n";
                        $sMsgLogErro .= "orgao {$oDespesa->o58_orgao} \n";
                        $sMsgLogErro .= "unidade {$oDespesa->o58_unidade}\n";
                                                $sMsgLogErro .= "fonte_orcamento {$oDespesa->o58_codigo}\n";
                                                $sMsgLogErro .= "exercicio = {$this->iAno}\n";
                                                $sMsgLogErro .= "Dotacao: {$oDespesa->o58_coddot}\n";
