...
$oFiltros->iTamanhoTurma = 27;
$oFiltros->iTamanhoDataNasc = 27;
...
foreach ($oFiltros->aQuebras as $iEscola => $aEscola) {
  ...
  //array_push($csv1, $cabecalho);
  //array_push($csv2, $corpanzil);
  //die("Parada de conferência");
...
function corpoRelatorio(PDF $oPdf, $oFiltros, $aEscola) {
        ...
        foreach ($aAlunos as $oMatricula ) {
          ...
          foreach ($oFiltros->aCabecalho as $oCabecalho) {
            switch ($oCabecalho->iTipo) {              
              //Codigo do Aluno               
              case 1:
                $campo = $oMatricula->getAluno()->getCodigoAluno();
                $oPdf->Cell($oCabecalho->iTamanho, 4, $campo, 1, 0, $oCabecalho->sAlinhamento);
                $corpo1 .= $campo . ";";
                break;
              //Nome do aluno
              case 2:
                $campo = $oMatricula->getAluno()->getNome();
                $oPdf->Cell($oCabecalho->iTamanho, 4, $campo, 1, 0, $oCabecalho->sAlinhamento);
                $corpo1 .= $campo . ";";
                break;
              //Nome do curso
              case 3:
                $campo = $oMatricula->getTurma()->getBaseCurricular()->getCurso()->getEnsino()->getNome();
                $oPdf->Cell($oCabecalho->iTamanho, 4, $campo, 1, 0, $oCabecalho->sAlinhamento);
                $corpo1 .= $campo . ";";
                break;
              //Desc. da etapa
              case 4:
                $campo = $sEtapa->sDescricao;
                $oPdf->Cell($oCabecalho->iTamanho, 4, $campo, 1, 0, $oCabecalho->sAlinhamento);
                $corpo1 .= $campo . ";";
                break;
              //Data de matricula
              ...
              case 6:
                ...
                break;
              case 7:
                // Mesmo que esteja incluído ou não, agora não adicionamos o marcador aqui
                $campo = $xturma;
                $oPdf->Cell($oCabecalho->iTamanho, 4, $campo, 1, 0, $oCabecalho->sAlinhamento);
                $corpo1 .= $campo . ";";
                break;
              case 8:
                $campo = $oMatricula->getAluno()->getDataNascimento();
                $oPdf->Cell($oCabecalho->iTamanho, 4, $campo, 1, 0, $oCabecalho->sAlinhamento);
                $corpo1 .= $campo . ";";
                break;
            }
  ...
  $ultimachave = key(array_slice($corpao, -1, 1, true));    
  $corpao2 = array();
  array_push($corpao2, $corpao[$ultimachave]);  
  $corpao3 = explode(";xxxx", $corpao2[0]);
