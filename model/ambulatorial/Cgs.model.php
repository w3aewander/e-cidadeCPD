...
class Cgs
{
    ...
    /**
     * Cgs constructor.
     * @param integer|null $iCgs
     * @throws DBException
     * @throws ParameterException
     */
    public function __construct($iCgs = null)
    {
        if (!empty($iCgs)) {
            ...
            if (pg_num_rows($rsDadosCGS) > 0) {
                ...
                $this->setNomePai($oDadosCGS->z01_v_pai);
                ...
                $this->setDesconheceMae($oDadosCGS->z01_b_descnomemae == 't');
            }
        }
    }
    ...
    /**
     * Retorna o nome do pai do paciente
     * @return string
     */
    ...
    public function salvar($oDadosCgs)
    {
        ...
        if ($oDadosCgs->dados_pessoais->cns == "" && $lCgsExistente) {
            if ($this->getCartaoSus()[0]->sCartaoSus != "") {
                $this->excluirCgsCartaoSus();
            }
        }
        ...
        $cpf = str_replace('-', '', str_replace('.', '', str_replace('_', '', $oDadosCgs->dados_pessoais->cpf)));

        $oDaoCgsUnd->z01_i_cgsund = $this->iCodigo;
        $oDaoCgsUnd->z01_v_cgccpf = (!empty($cpf) ? $cpf : 'null');
         ...
         * CONTATOS
         */
        $email = !empty($oDadosCgs->contato->email) ? $oDadosCgs->contato->email : 'null';
        $telefoneFixo = !empty($oDadosCgs->contato->telefone_fixo) ? $oDadosCgs->contato->telefone_fixo : 'null';
        $celular = !empty($oDadosCgs->contato->telefone_celular) ? $oDadosCgs->contato->telefone_celular : 'null';
        $fax = !empty($oDadosCgs->contato->fax) ? $oDadosCgs->contato->fax : 'null';

        $oDaoCgsUnd->z01_v_email = $email;
        $oDaoCgsUnd->z01_v_telef = $telefoneFixo;
        $oDaoCgsUnd->z01_v_telcel = $celular;
        $oDaoCgsUnd->z01_v_fax = $fax;

        if (isset($oDadosCgs->contato->telefone_celular)) {
            $celular = $oDadosCgs->contato->telefone_celular != '' ? $oDadosCgs->contato->telefone_celular : 'null';
            $oDaoCgsUnd->z01_v_telcel = $celular;
        }

        if (isset($oDadosCgs->contato->fax)) {
        ...
        if (!is_null($oDadosCgs->contato->endereco_principal)) {
            ...
            $numero = is_numeric($oEndereco->getNumeroLocal()) ? $oEndereco->getNumeroLocal() : null;

            $oDaoCgsUnd->z01_v_ender = $oEndereco->getDescricaoRua();
            $oDaoCgsUnd->z01_i_numero = $numero;
        ...
        if (!empty($oDadosCgs->biometria->foto_nova_caminho)) {
            ...
            DBLargeObject::escrita($oDadosCgs->biometria->foto_nova_caminho, $iNovoOid);
         ...
         * OUTROS DADOS
         */
        $ocupacao = $oDadosCgs->outrosDados->ocupacao != '' ? $oDadosCgs->outrosDados->ocupacao : 'null';
        $familia = $oDadosCgs->outrosDados->familia != '' ? $oDadosCgs->outrosDados->familia : 'null';
        ...
        $oDaoCgsUnd->z01_i_codocupacao = $ocupacao;
        ...
        $oDaoCgsUnd->z01_t_obs = db_stdClass::normalizeStringJsonEscapeString($oDadosCgs->outrosDados->observacoes);
        $oDaoCgsUnd->z01_t_obs = db_stdClass::db_stripTagsJsonSemEscape($oDaoCgsUnd->z01_t_obs);
        $oDaoCgsUnd->z01_i_familiamicroarea = $familia;
        ...
        $oDaoCgsUnd->z01_v_ident = (!empty($oDadosCgs->dados_pessoais->rg) ? $oDadosCgs->dados_pessoais->rg : 'null');

        $oDaoCgsUnd->z01_distritosanitario = (!empty($oDadosCgs->dados_pessoais->distritoSanitario) ?
            $oDadosCgs->dados_pessoais->distritoSanitario :
            'null');
        ...
        try {
            ...
            throw new \Exception("Não foi permitido salvar o CGS. {$e->getMessage()}");
        }
        $this->salvarCgsEndereco($oDadosCgs);
        $this->salvarCgsEtnia($oDadosCgs);

        //plugin ESF operation#1 - invocando a função salvarCgsEsf()
        return $oDaoCgsUnd;
    }
    ...
    public function excluir()
    {
        ...
        //plugin ESF operation#2 - verificação para exclusão na tabela plugins.cgs_und_esf
        ...
        $oDaoCgsAuditoria = new cl_cgsauditoria();
        $oDaoCgsAuditoria->excluir("", "z18_cgs = {$this->iCodigo}");

        if ($oDaoCgsAuditoria->erro_status == "0") {
            throw new BusinessException(_M(ARQUIVO_MENSAGEM_CGS . 'erro_excluir_cgs'));
        }
     ...
     * @throws BusinessException
     */
    public function excluirCgsCartaoSus()
    {
        $oDaoCgsCartaoSus = new cl_cgs_cartaosus();
        $oDaoCgsCartaoSus->excluir(null, "s115_i_cgs = {$this->iCodigo} and s115_c_tipo = 'D'");
    ...
    /**
     * @param null $dataValidacao
     * @return int
     */
    public function getIdade($dataValidacao = null)
    {
        $dataAtual = $dataValidacao instanceof DateTime ? $dataValidacao : DateTime::createFromFormat(
            'Y-m-d',
            date('Y-m-d')
        );
        $dataNascimento = DateTime::createFromFormat('Y-m-d', $this->getDataNascimento()->getDate());

        return $dataAtual->diff($dataNascimento)->y;
    }
    //plugin ESF operation#0 - adicionando método salvarCgsEsf()
     ...
     * @throws ParameterException
     */
    public function getIdadeCompleta()
    {
        $dataAtual = new DBDate(date("Y-m-d"));
        $quantidadeDeDias = DBDate::calculaIntervaloEntreDatas($dataAtual, $this->getDataNascimento(), 'd');
        return DBDate::getIdadeCompleta($quantidadeDeDias);
    }
