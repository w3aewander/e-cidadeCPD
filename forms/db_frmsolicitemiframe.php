...
<form name="form1">
    ...
    $result_gerareserva = $clpcparam->sql_record($clpcparam->sql_query_file(db_getsession("DB_instit"), "pc30_gerareserva,pc30_mostrasaldo,pc30_passadepart,pc30_digval,pc30_ultdotac"));
    ...
    <tr>
	  ...
	  <script>
	    ...
	    opcao.setAttribute('value','$pc13_valor_limite');";
                        ...
                        echo"
      jspc13_valor_limite = parent.document.form1.pc11_vlrun.value*parent.document.form1.pc11_quant.value;
      opcao.setAttribute('value',jspc13_valor_limite);";
                    }
                    $isParaiba = isParaiba();
                    echo"
        ...
        if(isPB){
          let [int = 0, decimal = 0] = valinf.toString()
          .split('.')
            if(decimal.length > 2){
                alert('O valor só deve ser preenchido com 2 casas decimais');
                document.form1.pc13_valor.value = '';
                document.form1.pc13_valor.focus();
            }
        }
        valmax = new Number(document.form1.pc13_valorlimite.value);
        pc13_quantsolicitacao = new Number(parent.document.form1.pc11_quant.value);

        console.log('valores aqui ' + valinf.toFixed(2) + ' - ' + valmax.toFixed(2));
	      if(valinf > valmax){
		alert('ATENÇÃO:\\n\\n Informe um valor menor ou igual a R$ '+document.form1.pc13_valorlimite.value+' (Valor restante, não \\ninformado em dotações).\\n\\nAdministrador:');
                document.form1.pc13_valor.value = '';
                document.form1.pc13_valor.focus();
		...
		alert('ATENÇÃO:\\n\\n Informe um valor válido.\\n\\nAdministrador:');
              ...
              console.log(valinf + ' - ' + pc13_quantsolicitacao + '-' + valmax);
              let calculo =  (valinf * pc13_quantsolicitacao)/valmax;
              console.log(calculo);
        document.form1.pc13_quant.value = calculo;
      }
	...
	if(document.form1.pc13_quant.value!=''){
      ...
      ";
            ?>
            <input onclick="return validaValorSaldoPacto();" name="<?=($db_opcao==1?"incluir":($db_opcao==2||$db_opcao==22?"alterar":"excluir"))?>" type="submit" id="db_opcao" value="<?=($db_opcao==1?"Incluir":($db_opcao==2||$db_opcao==22?"Alterar":"Excluir"))?>" <?=($db_botao==false?"disabled":"")?>>
            <?php
    ...
    <tr style="display: none">
        ...
        if (isset($pc13_coddot)) {
            $rsDotacao = $clorcdotacao->sql_record($clorcdotacao->sql_query(db_getsession("DB_anousu"), $pc13_coddot, "o15_tipo, o15_complemento,o200_descricao"));
            if ($clorcdotacao->numrows > 0) {
                echo "<td><b>Contrapartida:</b></td><td>";
                ...
                if ($oDotacao->o15_tipo == 1) {
                    ...
                    $complemento = $oDotacao->o15_complemento. ' - '.$oDotacao->o200_descricao;
                    /*
            ...
            echo "</td>";
            if ($complemento != null) {
                echo "<td><b>Complemento:</b></td><td>";
                db_input('complemento', 20, 0, true, 'text', 3);
                echo "</td>";
            }
        }
        ?>
    ...
    <?php
    if ($pc30_gerareserva=='t' || $pc30_mostrasaldo == 't') {
        ...
        <tr>
            ...
            <td nowrap>
                ...
                ?>
                <?php
                if ($db_opcao!=1 && isset($pc13_coddot) && isset($pc13_codigo) && isset($pc13_sequencial) && $pc30_gerareserva == 't') {
                ...
                } elseif ($db_opcao==1 && $pc30_gerareserva == 't') {
                    if(db_getsession("DB_instit") == 1){
                        if (isset($pc13_coddot)&&trim($pc13_coddot)!="") {
                            $texto = "<strong style='visibility:hidden'>Reservar saldo</strong>";
                        } else {
                            $texto = "";
                        }                    
                        echo "<input type='checkbox' name='nreserva' value='nreserva' checked style='visibility:hidden'>";
                        echo $texto;    
                    }else{
                        if (isset($pc13_coddot)&&trim($pc13_coddot)!="") {
                            $texto = "<strong>Reservar saldo</strong>";
                        } else {
                            $texto = "";
                        }                    
  ...
  <?php }?>
     ...
     */
        ...
        function js_verquant(campo) {
            ...
            console.log("Quantidade restante:" + parent.document.form1.quant_rest.value);
        ...
        } elseif ($desabilitarcampos==true && $errado!="true") {
            ?>
        ...
        <?php
        if (!empty($pc13_planoorcamentariolinhapacto) && FONTE_RECURSO_UNIAO) {
            echo "js_pesquisaLinhaPactos(\$F('planoorcamento'), {$pc13_planoorcamentariolinhapacto});\n";
        }
        ?>

        function validaValorSaldoPacto()
        {
            let isPB = '<?php echo isParaiba() ?>';
            let valorReserva = new Number($F('pc13_valor'));
            let valorDisponivel = new Number($F('atual_menos_reservado'));
            var linhaPacto = $('pc13_planoorcamentariolinhapacto');
            if(isPB && valorReserva > valorDisponivel){
                alert("Reserva orçamentária insuficiente");
            }
            console.log(valorReserva > valorDisponivel);
            ...
            if (new Number($F('pc13_valor')) > saldoFinal) {
                return confirm('o Saldo reservado é maior que o saldo da Linha de pacto selecionado. Deseja continuar');
            }
            return true;
        }
    </script>
    ...
    if ($db_opcao == 3 && FONTE_RECURSO_UNIAO) {
        echo "<script>
            \$('pc13_planoorcamentariolinhapacto').disabled=true;
             \$('pc13_planoorcamentariolinhapacto').className += ' readonly';
      </script>";
    }
