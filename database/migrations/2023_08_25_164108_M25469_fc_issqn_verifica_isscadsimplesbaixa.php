...
class M25469FcIssqnVerificaIsscadsimplesbaixa extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        $sql = <<<SQL
CREATE OR REPLACE FUNCTION public.fc_issqn(integer, date, integer, date, boolean, boolean, integer, character varying, integer, integer, integer DEFAULT 0)
...
-- FIM DA CONFERÃŠNCIA DE EMPRESAS OPTANTE PELO SIMPLES

--
-- Verificando parÃ¢metro Calcula Vistorias para MEI:
--
...
';
    ...
    v_text = 'select distinct
						...
						WHEN (select 1 from isscadsimples left join  isscadsimplesbaixa on q38_sequencial = q39_isscadsimples where q39_isscadsimples is null and isscadsimples.q38_inscr = '||iInscr||' and isscadsimples.q38_dtinicial <= '''||dtOperacao||''') is not null and q83_cadvencdescsimples is not null THEN 
...
';
            ...
            -- Se parÃ¢metro Calcula Vistorias para MEI: estiver NÃƒO então ignora taxa de expediente
        ...
        loop
            IF extract(year from dInicioAtividade)::integer = iAnousu THEN
                IF v_record_cadvenc.q82_parc < extract(month from dInicioAtividade)::integer THEN
                    perform fc_debug('No existia atividade no mï¿½s '||v_record_cadvenc.q82_parc||' do ano de '||iAnousu,lRaise,false,false);
                    perform fc_debug('PULANDO ESTE Mï¿½S',lRaise,false,false);
                    perform fc_debug(' ',lRaise,false,false);
...
raise notice 'k00_numpre = %',v_numpre;
                                ...
                                from ativtipo ativtipo2
                                    ...
                                    left  join tipcalcexe on q83_tipcalc = q81_codigo
                                                        and q83_anousu  = (select extract(year from q02_dtinic)
                                                                            ...
                                                                            where q02_inscr = '||iInscr||')
                            where ativtipo2.q80_ativ = ativtipo.q80_ativ and ativtipo2.q80_tipcal = ativtipo.q80_tipcal )
						WHEN (select 1 from isscadsimples where isscadsimples.q38_inscr = '||iInscr||' and isscadsimples.q38_dtinicial <= '''||dtOperacao||''') is not null and q83_cadvencdescsimples is not null THEN 
                        	q83_cadvencdescsimples
                        else
                            q83_codven
                        end as q83_codven
