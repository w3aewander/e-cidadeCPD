...
class M12278BaixaBancoPgtoParcialJurosFinanciamento extends PostgresMigration
{
    public function up()
    {
        $sql = 
...
begin
  ...
  /**
   * Verifica se o numpre pertence a outra institui??o e insere na tmpnaoprocessar
  ...
  /**
   * Verifica configura??o das receitas de custas.
   */
 ...
 */
  if lAtivaPgtoParcial is true then
    ...
    /* Necess?rio realizar corre??o para efetuar update em arrecad de abatimentos */
    /******************************************************************************/
    ...
    loop
      ...
      if lRaise is true then
        perform fc_debug('  <PgtoParcial>  - lan?a recibo avulso j? pago ',lRaise,false,false);
      end if;
    ...
    loop
      ...
      if r_idret.arrecad is false and r_idret.arrecant is false and r_idret.arreold is false then
        ...
        --    da recibopaga para ajustar o recibo, pressupondo que tenha sido pago ou cancelado
         --    uma parcela do recibo. Este ajuste no recibo ? necessario para que o sistema encontre
        ...
        -- @TODO - corrigir esse comentario, so ? deletado agora se n?o for recibo de custas. Regra 5.5 depende dos registros
        --
    ...
    loop
      ...
      loop
         ...
         *
         * Verifica se deve buscar os dados da arrecant (Caso o d?bito ja tenha sido pago)
         *                                  ou arreold (Caso o d?bito tenha sido importado para divida)
         */
        perform 1
           from arrecant
    ...
    /*******************************************************************************/
    /* Gera recibo avulso para custas de recibos que v?o receber pagamento parcial */
    /*******************************************************************************/
    ...
    loop
      ...
      then
        ...
        /**
         * NORMAL: 
         * Este delete faz com que n?o seja lan?ado desconto para o pagamento de uma cota ?nica com pagamento em atraso.
         * 
         * CREDITO:
         * 1 - PAGAMENTO EM DUPLICIDADE:
         * Using no arrecad existe para que em casos de pagamento em duplicidade indiferente da data de pagamento, n?o remova o desconto.
         * e creditos sejam gerados com o valor total do documento.
         *
         * @TODO - Sistema n?o da suporte a recibos de cota unica com bom pagador pagos atrasados.
         * @TODO - Sistema n?o da suporte a recibos de cota unica e bom pagador pago duas vezes.
         */
        delete from recibopaga
         using arrecad
         where recibopaga.k00_numnov = r_idret.numpre
      ...
      if (
        ...
        /**
         * Recibo com receita de taxa n?o deve receber pagamento parcial.
         * Verifica se recibo tem receita de taxas e n?o processa.
         */
        perform 1
           from disbanco
                inner join recibopaga on recibopaga.k00_numnov = disbanco.k00_numpre
       ...
       */
        ...
        if iTipoAbatimento = 3 then
          ...
          loop
            ...
            --Caso n?o tenha receita de cr?dito configurada para o tipo de d?bito, usamos a receita padr?o dos par?metros
            ...
            if iReceitaCredito is null then
              ...
              codigo_retorno := 8;
              descricao := 'Receita de Cr?dito n?o configurada nos par?metros, na configura??o do tipo de d?bito ou na receita para o numpre: ' || r_idret.numpre;
              return;
            end if;
            ...
            if found and nVlrCalculado = 0 then
              ...
              -- @TODO - testar caso do hist 11403 com numpre que n?o ta no arrecad
    ...
    if r_codret.k00_numpar = 0 and r_codret.quant_arrecad_unica > 0 then
      ...
      if r_codret.arrecad_unica_quant_numpar <> r_codret.arrecad_unica_numtot then
        ...
        if v_contador = 1 then
          ...
          codigo_retorno := 21;
          descricao := 'IDRET ' || r_codret.idret || ' COM MAIS DE UM PAGAMENTO NO MESMO ARQUIVO. CONTATE SUPORTE PARA VERIFICA??ES!';
          return;
        end if;
  ...
  /**
   * Adicionada valida??o para separar recibos e carnes que possuiam suas origens importadas para divida
  ...
  /**
   * Fim da separa??o de recibos e carnes que possuiam suas origens importadas para divida
   */
...
-- arquivo recibo
 ...
 -- Verifica se algum numpre do recibo j? esta pago
-- caso positivo passa para o proximo e deixa inconsistente
      ...
      -- Verifica se o Total Pago ? diferente do que foi Classificado (inserido na Arrepaga)
...
-- GRAVA DISREC DAS RECEITAS PARA A CLASSIFICACAO
            ...
            if lVerificaReceita is false then
              ...
              codigo_retorno := 25;
              descricao := 'Receita: '||q_disrec.k00_receit||' n?o encontrada verifique o cadastro (2).';
              return;
            end if;
...
-- TESTA SE EXISTE NUMPRE E NUMPAR NO ARREIDRET, NAO EXISTINDO INSERE O IDRET DO PAGAMENTO
          ...
          loop
            ...
            loop
              ...
              --
               -- Modifica?ao realizada devido ao erro gerado na tarefa 32607
              -- Motivo do erro:
