...
use ECidade\RecursosHumanos\ESocial\Repository\RemuneracaoRPPS as RemuneracaoRPPSRepository;
...
class RemuneracaoRPPS extends ProcessamentoAbstract implements ProcessamentoInterface
{
    ...
    public function processar()
    {
        ...
        $dados->cgms = RemuneracaoRPPSRepository::buscarTodosCGMCompetencia($competencia, $this->servidores);
        $dados->rubricaDiferenca = RemuneracaoRPPSRepository::buscarRubricaDiferencaReajusteCompetencia($competencia);
        $dados->inscricao_empregador = RemuneracaoRPPSRepository::buscarCNPJEmpregador($this->cgm);
        ...
        $formatter = FormatterFactory::get(Tipo::S1202);
        ...
        foreach ($dadosPreenchimentoEmpregador as $indice => $dados) {
            $evento = new Evento(Tipo::S1202, $this->cgm, $dados->referencia, $dados);
