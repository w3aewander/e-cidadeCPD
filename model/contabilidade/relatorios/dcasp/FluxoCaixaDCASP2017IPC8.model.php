...
class FluxoCaixaDCASP2017IPC8 extends RelatoriosLegaisBase
{

    const CODIGO_RELATORIO = 171;
    const CODIGO_RELATORIO_IPC8 = 189;

    const CAIXA_EQUIVALENTES_CAIXA_INICIAL = 30;
    const CAIXA_EQUIVALENTE_CAIXA_FINAL = 31;
     ...
     * @var array
     */
    protected $aQuadroTransferencias;
...
//        foreach (array(7, 8, 15, 16, 17, 26, 27, 55, 87, 88, 89) as $linha) {
//            $this->calculaLinhaAdministrativa($linha, $oDataInicialAnterior, $oDataFinalAnterior);
//        }
        ...
        sort($linhasCalculo);
        $this->calculaLinhaAdministrativa($linhasCalculo, $oDataInicialAnterior, $oDataFinalAnterior);
    ...
    private function processarQuadros()
    {
        ...
        if ($this->exibirQuadroRelatorio(static::QUADRO_PRINCIPAL)) {
            ...
            for ($iIndice = static::QUADRO_PRINCIPAL_INICIAL; $iIndice <= static::QUADRO_PRINCIPAL_FINAL; $iIndice++) {
                $this->aQuadroPrincipal[] = $this->processarLinha($iIndice, static::QUADRO_PRINCIPAL_FINAL);
            }
        }

        if ($this->exibirQuadroRelatorio(static::QUADRO_RECEITAS)) {

            for (
                $iIndice = static::QUADRO_RECEITAS_DERIVADAS_INICIAL; $iIndice <= static::QUADRO_RECEITAS_DERIVADAS_FINAL;
                $iIndice++
            ) {
                $this->aQuadroReceitas[] = $this->processarLinha($iIndice, static::QUADRO_RECEITAS_DERIVADAS_FINAL);
            }
        }

        if ($this->exibirQuadroRelatorio(static::QUADRO_TRANSFERENCIAS)) {

            for (
                $iIndice = static::QUADRO_TRANSFERENCIAS_INICIAL; $iIndice <= static::QUADRO_TRANSFERENCIAS_FINAL;
                $iIndice++
            ) {
                $this->aQuadroTransferencias[] = $this->processarLinha($iIndice, static::QUADRO_TRANSFERENCIAS_FINAL);
            }
        }

        if ($this->exibirQuadroRelatorio(static::QUADRO_DESEMBOLSOS)) {

            for (
                $iIndice = static::QUADRO_DESEMBOLSOS_PESSOAL_INICIAL; $iIndice <= static::QUADRO_DESEMBOLSOS_PESSOAL_FINAL;
                $iIndice++
            ) {
                $this->aQuadroDesembolsos[] = $this->processarLinha($iIndice, static::QUADRO_DESEMBOLSOS_PESSOAL_FINAL);
            }
        }

        if ($this->exibirQuadroRelatorio(static::QUADRO_DIVIDA)) {

            for ($iIndice = static::QUADRO_DIVIDA_INICIAL; $iIndice <= static::QUADRO_DIVIDA_FINAL; $iIndice++) {
                $this->aQuadroDivida[] = $this->processarLinha($iIndice, static::QUADRO_DIVIDA_FINAL);
            }
        }
    ...
    public function emitir()
    {
        ...
        $this->emitirQuadro("QUADRO DE DESEMBOLSOS DE PESSOAL E DEMAIS DESPESAS POR FUNÇÃO", "",
            $this->aQuadroDesembolsos);
    ...
    /**
     * Calcula linha administrativa aplicando a formula correta.
     *
     * @param array  $linhas
     * @param DBDate $oDataInicialAnterior
     * @param DBDate $oDataFinalAnterior
     */
    private function calculaLinhaAdministrativa($linhas, $oDataInicialAnterior, $oDataFinalAnterior)
    {
