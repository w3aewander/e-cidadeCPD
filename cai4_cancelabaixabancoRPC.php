...
use ECidade\Tributario\Juridico\InicialPartilha\Repository\InicialPartilha;
use ECidade\Tributario\Juridico\InicialPartilha\Repository\InicialPartilhaCustas;
        ...
        try {
            ...
            switch ($oParam->iTipoProcessamento) {
                case 1: // exclui arquivo de retorno
                    ...
                    $oDaoDebContaPedidoHistorico       = new cl_debcontapedidohistorico;
                    $oDaoDebContaPedidoHistorico->excluir(null," d83_codret = {$oParam->codret}");
                    /* Histórico da DebContaPedido */
...
function cancelaAutenticacao($iCodCla) {
    ...
    $daoComplementoLancamento = new cl_conlancamcomplementorecurso();
    ...
    if ($iQtdRegistros > 0) {
        for ($iAutenticacao = 0; $iAutenticacao < $iQtdRegistros; $iAutenticacao++) {
            ...
            foreach ($oLancamentosVinculados as $iIndice => $oLancamento) {
                ...
                $oDaoInfoComplementarValor->excluir(null, "c123_conplanoatributolancamentos in (select c124_sequencial from conplanoatributolancamentos where c124_lancamento = {$oLancamento->c86_conlancam})");
                if ($oDaoInfoComplementarValor->erro_status === '0') {

                    $sMsg  = "Erro ao excluir dados da tabela infocomplamentarvalor gerados pela baixa do arquivo na classificação {$iCodCla}.\n\n";
                    $sMsg .= "Erro: {$oDaoAtributoLancamento->erro_msg} - ".str_replace('"',"\'",pg_last_error());
                    throw new Exception($sMsg);
                }

                $oDaoAtributoLancamento->excluir(null, "c124_lancamento = {$oLancamento->c86_conlancam}");
                if ($oDaoAtributoLancamento->erro_status === "0") {

                    $sMsg  = "Erro ao excluir dados da tabela conplanoatributolancamento gerados pela baixa do arquivo na classificação {$iCodCla}.\n\n";
                    $sMsg .= "Erro: {$oDaoAtributoLancamento->erro_msg} - ".str_replace('"',"\'",pg_last_error());
                    throw new Exception($sMsg);
                }
                ...
                $daoLancamentoDepartamento = new cl_conlancamdepartamento();
                $daoLancamentoDepartamento->excluir(null, "c128_conlancam = {$oLancamento->c86_conlancam}");
                if ($daoLancamentoDepartamento->erro_status === "0") {
                    throw new Exception("Ocorreu um erro ao desvincular o departamento do lançamento contábil");
                }

                $daoLancamentoRecurso = new cl_conlancamrecurso();
                $daoLancamentoRecurso->excluir(null, "c130_conlancam = {$oLancamento->c86_conlancam}");
                if ($daoLancamentoRecurso->erro_status === "0") {
                    throw new Exception("Ocorreu um erro ao desvincular o recurso do lançamento contábil");
                }

                $oDaoConlancamLogAtributos = new cl_conlancamlogatributos;
                $oDaoConlancamLogAtributos->excluir(null, "c134_codlan = {$oLancamento->c86_conlancam}");
                if ($oDaoConlancamLogAtributos->erro_status == "0") {
                    throw new Exception("Ocorreu um erro ao desvincular o LOG do lançamento contábil");
                }

                $daoComplementoLancamento->excluir(null, " o201_codlan = {$oLancamento->c86_conlancam}");
                if ($daoComplementoLancamento->erro_status === "0") {
                    throw new Exception("Ocorreu um erro ao desvincular o complemento do lançamento contábil");
                }
...
function cancelaClassificacao($iCodCla) {
    ...
    $partilhaCustasRepository = InicialPartilhaCustas::getInstance();
    $partilhaRepository       = InicialPartilha::getInstance();
    ...
    if ($iQtdRegistros > 0) {
        ...
        for ($iIdRet = 0; $iIdRet < $iQtdRegistros; $iIdRet++) {
            ...
            if ( $oRegistros->disbanco_numpre != "") {
                ...
                $partilhaCustas = $partilhaCustasRepository->getByNumnov($oRegistros->disbanco_numpre);

                if (!empty($partilhaCustas)) {
                    $partilhaCustas[0]->getCodigoInicialPartilha();
                    $partilha = $partilhaRepository->getByCodigo($partilhaCustas[0]->getCodigoInicialPartilha());
                    $partilha->resetDataPagamento();
                    $partilhaRepository->persist($partilha);
                }
            }
