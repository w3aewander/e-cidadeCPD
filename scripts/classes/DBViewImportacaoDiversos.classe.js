...
var DBViewImportacaoDiversos = function(sInstancia, sNomeComponente) {
  ...
  this.setDataVencimento = function(sDataVencimento) {
    this.sDataVencimento = sDataVencimento;
  };
  ...
  this.renderizarGridDebitos = function() {
    ...
    oInstancia.gridDebitos.setCellAlign(["center", "center", "left", "center", "left", "right", "center"]);
    oInstancia.gridDebitos.setCellWidth(["13%", "8%", "25%", "13%", "20%", "11%", "10%"]);
    oInstancia.gridDebitos.setHeader(['Numpre', 'Numpar', 'Tipo de Débito', 'Código Receita', 'Receita', 'Valor (R$)', 'Auto']);
    oInstancia.gridDebitos.show( oInstancia.oForm.oContainerGrid );
    oInstancia.gridDebitos.setCallbackSingle(
      function (checkBoxOrigem, sRow, oRow) {

        var linhasParaMarcar = [];
        oInstancia.gridDebitos.aRows.each(
          function (row, indice) {

            if ((Number(checkBoxOrigem.value) === Number(row.aCells[1].getValue())) || (oRow.aCells[7].getValue().trim() !== "" &&(Number(oRow.aCells[7].getValue()) === (Number(row.aCells[7].getValue()))))) {
  ...
  this.processarDebitosProcedencias = function() {
    ...
    if (this.importacaoParcial === true && oParam.importaDividaAtiva === true) {
      ...
      oParam.titularProcesso = $F('requerente').replace('&', "");
    ...
    oParam.sDataVencimento = this.sDataVencimento;
    oParam.iOrigemDebito = this.origemDebito;
