...
if (isset($incluir)) {
  ...
  $HTTP_POST_VARS['fm15_cpf'] = preg_replace("/[^0-9]/", "", $HTTP_POST_VARS['fm15_cpf']);

  db_inicio_transacao();
  $oDaoProfissionais->incluir($fm15_codigo);
  db_fim_transacao();
  ...
  if ($oDaoProfissionais->erro_status == '0') {
    ...
    if ($oDaoProfissionais->erro_campo != "") {
      $sPosScripts .= "document.form1.{$oDaoProfissionais->erro_campo}.classList.add('form-error');\n";
      $sPosScripts .= "document.form1.{$oDaoProfissionais->erro_campo}.focus();\n";
    }
  } else {
