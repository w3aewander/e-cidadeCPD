...
class Repository
{
    ...
    public function add(EmissaoGeral $oEmissao)
    {
        ...
        $this->oDao->tr01_emissao_geral_padrao = ($oEmissao->isEmissaoGeralPadrao() == true) ? 't' : 'f';
    ...
    public function getEmissao($iCodigo)
    {
        ...
        $oEmissaoGeral->setEmissaoGeralPadrao((boolean) $oDadosEmissao->tr01_emissao_geral_padrao);
     ...
     * @return array
     */
    public function getEmissoesPorTipo($iTipoEmissao, $emissaoGeralPadrao)
    {
        $where = [];
        $where[] = "tr01_tipoemissao = {$iTipoEmissao}";
        $where[] = "tr01_emissao_geral_padrao = " . ($emissaoGeralPadrao ? "true" : "false");

        $sSqlEmissao = $this->oDao->sql_query_file(
            null,
            "*",
    ...
    {
        $sCampos = "distinct k169_codigo as movimentacao, ocorrenciacobrancaregistrada.*";
