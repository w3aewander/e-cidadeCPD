  ...
  if (pg_numrows($result)>0) {
    ...
    for($y=0;$y<pg_num_rows($result);$y++) {
      ...
      if (trim($periodos)=="") {
        ...
        if( $disciplina == 'TECNOLOGIA E INOVAÇÃO'){
//			echo '<td class="aluno1"></td>';
		}
...
function trocaTurma($iSeqMatricula, $iCodigoTurmaDestino, $lAproveitamento = false, $iMatriculaOrigem = null, $sTurno = null, $lValidaEtapataOrigem = true) {
   ...
   */
  //require_once(modification("model/MatriculaRepositoryVR.model.php"));   
  $iCodigoEtapaOrigem = null;
  if ( !empty($iMatriculaOrigem) ) 
  {
//  	$MatriculaRepositoryVR  = new MatriculaRepositoryVR;
// o erro da mudança de aluno de turma, acontecia na linha abaixo, porque o código da matricula de origem retorna nulo
    $oMatriculaOrigem   = MatriculaRepositoryVR::getMatriculaByFiltros(array("ed60_matricula = {$iMatriculaOrigem}"));
