...
$campoVlr = "(val.pc23_valor)";
if (isParaiba()) {
    $campoVlr = "val.pc23_vlrun";
}
$sSqlItens = $oDaoPcorcamitem->sql_query_pcmaterproc( null,
    ...
    . "(select coalesce(sum($campoVlr), 0) / case when count(val.*) > 0 then count(val.*) else 1 end             \n"
...
for ($iRow = 0; $iRow < pg_num_rows($rsItens); $iRow++) {
    ...
    if ($iLote != $oItem->pc68_sequencial || $iPageNumberItem != $oPdf->PageNo()) {
        ...
        $nValorMedioLote = getValorPrint($nValorMedioLote);
        ...
        if (!isParaiba()){
            ...
            $oPdf->setfont('arial', 'b', 11);
            $oPdf->cell(157, $iLine, "Lote {$oItem->pc68_nome}", 0, 0, 'L', 0);
        }
    ...
    $nValorMedioItem = getValorPrint($oItem->valor_medio_item);
    ...
    if ($iRow == pg_num_rows($rsItens)-1 || db_utils::fieldsMemory($rsItens, $iRow+1)->pc68_sequencial != $iLote) {
        ...
        if ($rsFornecedores && $oDaoPcorcamval->numrows > 0) {
            ...
            for ($iRowFornecedor = 0; $iRowFornecedor < $oDaoPcorcamval->numrows; $iRowFornecedor++) {
                ...
                $nValorCotado = getValorPrint($oFornecedor->valor_cotado);
                ...
                $contFornecedor = $iRowFornecedor + 1;
                $oPdf->cell(157, $iLine, "Cotação $contFornecedor", 1, 0, 'L', ($iRowFornecedor == 0));
            ...
            if(isParaiba()) {
                ...
                $oPdf->cell(157, $iLine, 'Valor Médio Por Lote:', 0, 0, 'R', 0);
                $oPdf->cell(35, $iLine, "R$ {$nValorMedioLote}", 1, 0, 'R', 0);
                $oPdf->ln();
