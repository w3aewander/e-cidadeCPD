...
/**
 * O bloco do código foi acrescentado para uma demanda específica de Capivari do Sul
 * A partir de 2022 para preencher o motivo com referencia a instrucao normativa 1234/2022
 */
...
function buscacnpj($inst){
    $sql = pg_query("SELECT * from db_config inner join cgm on cgm.z01_numcgm = db_config.numcgm inner join db_tipoinstit on db_tipoinstit.db21_codtipo = db_config.db21_tipoinstit where db_config.codigo = {$inst}");
    $resultado = pg_fetch_all($sql);
    return $resultado[0]["z01_cgccpf"];
}

if(db_getsession("DB_instit") == 45 || db_getsession("DB_instit") == 75){
    $this->cgcpref = buscacnpj(db_getsession("DB_instit"));
}


$isCapivari = false;
if ($oPref->municipio == 'CAPIVARI DO SUL' && db_getsession("DB_anousu") >= 2022) {
    $isCapivari = true;
    $this->destino = "O município aderiu a Instrução Normativa 1234/2012 da Receita Federal";
}
...
for ($xxx = 0; $xxx < $this->nvias; $xxx++) {
    ...
    $this->objpdf->text(125, $xlin - 10, 'NOTA DE EMPENHO N' . CHR(176) . ': ');
    $this->objpdf->text(159, $xlin - 10, db_formatar($this->codemp, 's', '0', 6, 'e'));
    $this->objpdf->text(129.5, $xlin - 6, 'DATA DE EMISSÃO : ');
    $this->objpdf->text(159, $xlin - 6, $this->emissao);
    global $uuid;
    $url_autenticidade = env('URL_AUTENTICIDADE_DOCUMENTO');
    if (!empty($uuid) && !empty($url_autenticidade)) {
        $fileQRCode = "tmp/qrcode_{$uuid}.png";
        \PHPQRCode\QRcode::png("{$url_autenticidade}/{$uuid}", $fileQRCode, 'L', 5, 1);
        $this->objpdf->Image($fileQRCode, 188, $xlin - 17.5, 17);
        $this->objpdf->Setfont('Arial', '', 6);
        $this->objpdf->text(166, $xlin + 1.5, $uuid);
        $this->objpdf->Setfont('Arial', 'B', 8);
    }
    ...
    $this->objpdf->text($xcol + 2, $xlin + 43, 'Licitação' . str_repeat(' ', 30) . 'Modalidade: ');
    $this->objpdf->text($xcol + 2, $xlin + 47, "Característica Peculiar:");
    ...
    $codigoRecurso = $this->recurso;
    $origem = \ECidade\Financeiro\Orcamento\Recurso\Origem::getEmpenho($this->numemp, db_getsession('DB_anousu'));
    if ($origem) {
        $codigoRecurso = $origem->o206_recurso;
    }

    $sDescricaoRecurso = descricaoCompletaRecurso($codigoRecurso, db_getsession('DB_anousu'));
     ...
     */
    if (strlen($sDescricaoRecurso) > $iTamanhoLinha) {
    ...
    $sDescricaoRecursoFonte = (FONTE_RECURSO_UNIAO ? $sDescricaoRecurso : $this->recurso . ' - ' . $sDescricaoRecurso);

    $this->objpdf->text($xcol + 17, $xlin + 33, ':  ' . $sDescricaoRecursoFonte);
    ...
    $numeroLicitacao = $this->num_licitacao;
    if (!empty($pdf1->numeroLicitacao) && !empty($pdf1->ano_licitacao)) {
        $numeroLicitacao = "{$pdf1->numeroLicitacao}/{$pdf1->ano_licitacao}";
    }

    $this->objpdf->text($xcol + 17, $xlin + 43, ": {$numeroLicitacao}");
    $this->objpdf->text($xcol + 55, $xlin + 43, $this->descr_licitacao);
    $this->objpdf->text($xcol + 35, $xlin + 47, ":  " . $this->cod_concarpeculiar . " - " . $this->descr_concarpeculiar);
    ...
    if (!isParaiba()) {
        ...
        $this->objpdf->text($xcol + 17, $xlin + 51, ": $nomeEmissor");
    ...
    for ($ii = 0; $ii < $this->linhasdositens; $ii++) {
        ...
        if($this->numemp == 982374){
            $set_altura_row = $this->objpdf->h - 115;
        }else{
            $set_altura_row = $this->objpdf->h - 125;    
        }
        ...
        //$descricaoitemimprime = $this->objpdf->Row_multicell($aCampos, 3, false, 5, 0, true, true, 1, $set_altura_row + 20);
        $retorna_obs = 0;
        ...
        if (($this->objpdf->gety() > $this->objpdf->h - 110 && $pagina == 1) || ($this->objpdf->gety() > $this->objpdf->h - 30 && $pagina != 1) || !empty($descricaoitemimprime)) {
            ...
            if ($pagina == 1) {
                ...
                $this->objpdf->SetFont('Arial', '', 7);
                if ($isCapivari) {
                    $this->objpdf->text($xcol + 2, $xlin + 187, 'OBSERVAÇÃO : ', 0, 1, 'L', 0);
                    $this->objpdf->text($xcol + 22, $xlin + 187, $this->destino, 0, 1, 'L', 0);
                } else {
    ...
    if ($pagina == 1) {
        ...
        if ($possuiFinalidadePagto = strlen($this->finalidade_pagto) > 0) {
            $this->objpdf->text(30, $xlin + 175, 'FINALIDADE PAGAMENTO : ', 0, 1, 'L', 0);
        }
...
//  global $texto_resumo, $qtd_string, $tam_string, $spaco_a_acupar;
        ...
        if ($this->e50_codord != "") {
            ...
            $this->objpdf->text($xcol + 2, $xlin + 170, "ORDEM DE PAGAMENTO: {$ops}", 0, 1, 'L', 0);
        }
        ...
        $this->objpdf->cell(25, 6, str_pad(db_formatar($this->empenhado, 'f'), (  strlen(db_formatar($this->empenhado, 'f')) ), ' ', 0), 0, "R", 'f');

        if (isParaiba()) {
            $sSqlOrdenador = "	select z01_nome as ordenador
			from sagresresponsavelunidadeorcamentaria
			inner join cgm on z01_numcgm = c140_cgm
			where c140_orgao = $this->orgao and c140_unidade = $this->unidade and c140_ativo = 't'";
...
//    db_fieldsmemory($rsOrdenador,0);
        ...
        if (@pg_numrows($resparag) > 0) {
            ...
            //$txt = "$this->objpdf->Setfont('Arial', 'B', 10);";
            //$txt2 = "$this->objpdf->Setfont('Arial', 'B', 6);";
            //str_replace($txt, $txt2, $db02_texto);
            $db02_texto = str_replace("10)", "7)", $db02_texto);
            @eval($db02_texto);
