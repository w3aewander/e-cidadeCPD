...
class M9790CalculoOsorio extends PostgresMigration
{
    public function up()
    {
        $sSql = <<<FUNCAO
create or replace function fc_calculoiptu_osorio_2018(integer,integer,boolean,boolean,boolean,boolean,boolean,integer,integer) returns varchar(100) as
$$
...
begin
   ...
   */
  perform fc_debug('PARAMETROS fc_iptu_calculavvt_osorio_2018 IDBQL: '||iIdbql||' - FRACAO DO LOTE: '||nFracaolote||' DEMO: '||tRetorno||'- ERRO: '||lErro, lRaise);
  select rnvvt, rberro, riCodErro, rtErro
    into nVvt, lErro, iCodErro, tErro
    from fc_iptu_calculavvt_osorio_2018( iMatricula, iIdbql, iAnousu, nFracaolote, nAreaRealLote, lDemonstrativo, lRaise );

  perform fc_debug('RETORNO fc_iptu_calculavvt_osorio_2018 -> VVT: '||nVvt||' - ERRO: '||lErro||' - iCodErro: '||iCodErro||' - tErro: '||tErro, lRaise);
   ...
   */
  perform fc_debug('PARAMETROS fc_iptu_calculavvc_osorio_2018 MATRICULA: '||iMatricula||' - ANOUSU:'||iAnousu||' - DEMO: '||lDemonstrativo, lRaise);

  select rnvvc, rntotarea, rinumconstr, rberro, riCodErro, rtErro
    into nVvc, nTotarea, iNumconstr, lErro, iCodErro, tErro
    from fc_iptu_calculavvc_osorio_2018( iMatricula, iAnousu, lRaise );

  perform fc_debug('RETORNO fc_iptu_calculavvc_osorio_2018 -> VVC: '||nVvc||' - AREA TOTAL: '||nTotarea||' - NUMERO DE CONSTRUÇÕES: '||iNumconstr||' - ERRO: '||lErro, lRaise);
  perform fc_debug('', lRaise);
  ...
  update tmpdadosiptu
     set viptu = nViptu, codvenc = rCfiptu.j18_vencim, aliq = nAliquota;
...
FUNCAO;
    ...
    {
        $this->execute("drop function fc_calculoiptu_osorio_2018(integer,integer,boolean,boolean,boolean,boolean,boolean,integer,integer);");
    }
}
