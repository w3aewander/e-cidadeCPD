...
class M26519DicionarioDados extends Migration
{
    /**
     * Run the migrations.
     *
...
BEGIN
    ...
    INSERT INTO iptutabelasconfigcampocorrecao SELECT * FROM tmp_iptutabelasconfigcampocorrecao;
    INSERT INTO iptutabelasconfigvirada SELECT * FROM tmp_iptutabelasconfigvirada;
...
CREATE OR REPLACE FUNCTION public.fc_gera_dicionario_apartir_tabela(
	text,
	text)
    RETURNS void
    LANGUAGE 'plpgsql'
...
ALTER FUNCTION public.fc_gera_dicionario_apartir_tabela(text, text)
    OWNER TO ecidade;


CREATE OR REPLACE FUNCTION public.fc_remove_dicionario_tabela(
	text,
	text)
    RETURNS void
    LANGUAGE 'plpgsql'
...
BEGIN
	...
	INSERT INTO tmp_iptutabelasdepend
    SELECT iptutabelasdepend.*
    FROM iptutabelasdepend
    WHERE j128_iptutabelasdepend in (select j121_sequencial FROM iptutabelas WHERE j121_codarq = sysarquivo);
...
ALTER FUNCTION public.fc_remove_dicionario_tabela(text, text)
    OWNER TO ecidade;
