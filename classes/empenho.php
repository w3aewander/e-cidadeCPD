...
class empenho
{
    ...
    function empenho()
    {
        ...
        $this->sCamposNota   = "e69_codnota,e69_numero,e69_anousu,e50_codord,e60_numemp, e50_anousu,e69_dtnota,e82_codmov, ";
    ...
    function liquidar($numemp = "", $codele = "", $codnota = "", $valor = "", $historico = "", $sHistoricoOrdem = '')
    {
        ...
        $sCampos = "e60_anousu, e60_emiss, e60_numemp, e60_vlrliq ";
        $res          = $clempempenho->sql_record($clempempenho->sql_query($numemp,$sCampos));
     ...
     */
        ...
        if ($e60_anousu < db_getsession("DB_anousu")) {
            ...
            if (self::ordemDeCompraManual($codnota)) {
                ...
                $lControleLiquidacaoDespesaRP            = $oEmpenhoFinanceiro->empenhoRestosPagarPorDocumento(HistoricoDocumento::CONTROLE_DESPESA_LIQUIDACAO_RP);
                if ($lRegistroEntradaRP || $lControleDespesaRP || $lControleLiquidacaoMaterialAlmoxarifado || (UTILIZA_INCORPORACAO_BEM && $lControleLiquidacaoDespesaRP)) {
                    $codteste = HistoricoDocumento::LIQUIDACAO_RP_ESTOQUES_PATRIMONIO;
                }
            }
        } else {
            $codteste = $this->getDocumentoInclusaoLiquidacao($oEmpenhoFinanceiro, $codnota);
        }
        ...
        if ( ($clempnotaele->e70_vlrliq - $clempnotaele->e70_vlranu) > $clempnotaele->e70_valor) {

            $this->erro_msg    = "Valor Liquidado Maior que o valor da nota";
            $this->erro_status = '0';
            return false;
        }
        ...
        $rsItens       = $clempnotaitem->sql_record($clempnotaitem->sql_query_file(
            null,
            "*",
            null,
            "e72_codnota = {$codnota}"
        ...
        if ($this->erro_status != '0') {
            if ($this->anousu == $oEmpenho->e60_anousu) {
                $documento = $this->getDocumentoInclusaoLiquidacao($oEmpenhoFinanceiro, $codnota);
            } else {
                ...
                if (self::ordemDeCompraManual($codnota)) {
                    ...
                    /**
                     * Em casos em que o empenho foi liquidado e dado entrada na ordem de compra no exercício anterior
                     * Deve ser liquidado com o doc. 39
                     */
                    ...
                    if ($lControleLiquidacaoDespesaRP || $lRegistroEntradaRP || $lControleDespesaRP || $lControleLiquidacaoMaterialAlmoxarifado || (UTILIZA_INCORPORACAO_BEM && ($lControleLiquidacaoDespesaRP || $lControleVerificacaoRP))) {
            ...
            try {
                ...
                if (USE_PCASP && ParametroIntegracaoPatrimonial::possuiIntegracaoContrato($oDataAtual, $oInstituicao)) {
                    ...
                    $sSqlContrato        = $oDaoEmpenhoContrato->sql_query_file(
                        null,
                        "e100_acordo",
                        null,
     ...
     */
        ...
        if (UTILIZA_INCORPORACAO_BEM) {
            $documentoLiquidacao = self::getDocumentoLiquidacao($oEmpenhoFinanceiro);
            if (!empty($documentoLiquidacao)) {
                $evento = EventoContabilRepository::getEventoContabilByCodigo($documentoLiquidacao, db_getsession("DB_anousu"), db_getsession("DB_instit"));
                $codteste = $evento->getEventoInverso()->getCodigoDocumento();
            }
        }
     ...
     */
        ...
        $clempnotaitem = $this->usarDao("empnotaitem", true);
        $rsItens       = $clempnotaitem->sql_record($clempnotaitem->sql_query_ordemCompra(
            null,
            "*",
            null,
            "e72_codnota = {$codnota}"
        ...
        if ($this->anousu == $oEmpenho->e60_anousu) {
            ...
            if (USE_PCASP) {
                ...
                if ($oCodigoGrupoContaOrcamento && self::ordemDeCompraManual($codnota)) {
                    switch ($oCodigoGrupoContaOrcamento->getCodigo()) {
                        case 7:
            ...
            if (self::ordemDeCompraManual($codnota)) {
                ...
                if ($lRegistroEntradaRP || $lControleDespesaRP || $lControleLiquidacaoMaterialAlmoxarifado || (UTILIZA_INCORPORACAO_BEM && $lControleVerificacaoRP)) {
    ...
    private function lancaOP($numemp = "", $codele = "", $codnota = "", $valor = "", $retencoes = "", $historico)
    {
        ...
        WHERE e69_numemp = $e60_numemp
          and e69_codnota = $codnota
        ORDER BY e69_dtnota
        ";
        $rsValida = db_query($sqlIncluido);

        if ( pg_num_rows($rsValida) > 1 ) {

            $this->erro_status = '0';
            $this->erro_msg    = "Nota já foi Liquidada.";
            return false;
        }
    ...
    function estornaOP($numemp = "", $codele = "", $codnota = "", $valor = "", $retencoes = "", $historico)
    {
        ...
        $clpagordemnota = new cl_pagordemnota;
        $res            = $clpagordemnota->sql_record($clpagordemnota->sql_query(
            null,
            null,
            "*",
            null,
        ...
        if ($clpagordemnota->numrows > 0) {
            ...
            if ($rsMov && pg_num_rows($rsMov) > 0) {
                ...
                foreach ($aMovimentos as $oMovimento) {
                    ...
                    } elseif ($oMovimento->e97_codforma == 2 && $oMovimento->e91_cheque == "") {
                        $sMsgErro .= " -- {$oMovimento->e81_codmov}, configurado para emissão de cheque.\n";
                    } elseif ($oMovimento->e97_codforma == 3 && $oMovimento->e90_codgera != "") {
    ...
    function getDados($iEmpenho, $sWhere = null)
    {
        ...
        $sCampos = "e60_numemp, e60_codemp, e60_anousu, e60_coddot, e60_numcgm, e60_resumo, z01_nome, ";
        $sCampos .= "o58_codigo, o15_descr, e60_vlremp, e60_vlrliq, e60_vlrpag, e60_vlranu, e64_codele, e60_codcom ";
    ...
    function getNotas($iEmpenho, $sWhere = '', $lNotaCancelada = true)
    {
        ...
        $sSqlNota .= "       left join empord        on  e82_codord               = e50_codord";
    ...
    function empenho2Json($sWhere = '', $itens = 0, $aItensPendentesPatrimonio = array())
    {
        ...
        $nomeInstituicao = $oEmpenhoFinanceiro->getInstituicao()->getDescricao();

        $cgm = $oEmpenhoFinanceiro->getCgm();
        ...
        $isEmpenhoFolha = $oEmpenhoFinanceiro->isFolhaPagamento();
        $necessitaCodigoAgrupamento = false;
        if (isParaiba()) {
            $oDaoOutrosDados = new cl_empempenhooutrosdados();
            $sWhereDados = "e171_numemp = {$this->numemp}";
            $sSqlDados = $oDaoOutrosDados->sql_query(
                null,
        ...
        if ($this->getDados($this->numemp, $sWhere)) {
            ...
            $strJson["nomeInstituicao"] = $nomeInstituicao;
            $strJson["elemento"] = $elemento;
            $strJson["subelemento"] = $subelemento;
            $strJson["lPrestacaoConta"] = $oEmpenhoFinanceiro->isPrestacaoContas();
            $strJson["isEmpenhoFolha"] = $isEmpenhoFolha;
            $strJson["necessitaCodigoAgrupamento"] = $necessitaCodigoAgrupamento;

            if (isRioDeJaneiro() && substr($desdobramento->o56_elemento, 0, 3) == '331') {
                $strJson["isDespesaPessoal"] = true;
            }

//            $strJson["e171_dados"] = $codigoAgrupamento;
            $strJson["outrosDados"] = $outrosDados;
            $strJson["tipoDeNotas"] = $tiposNotas;
            ...
            if ($this->operacao == 1) {
                ...
                $sWhere               = '';
            } elseif ($this->operacao == 2) {
                $sWhere               = " e69_anousu = {$this->anousu}";
                $strJson["saldo_dis"] = 0;
                $sSQLPgtoOrdem        = "
                    select sum(e53_valor) as e53_valor,
                ...
                if ($rsNotas) {
                    if ($this->iNumRowsNotas > 0) {
                        ...
                        for ($i = 0; $i < $this->iNumRowsNotas; $i++) {
                            ...
                            $objItens = new stdClass();
                            $objItens->elemento = "";
                            $objItens->descricao_unidade_item = "";
                            $objItens->resumo_item = "";

                            if (isset($objNotas->e62_sequencial) && !empty($objNotas->e62_sequencial)) {
                                $Ssql = "
                                  select empempitem.e62_codele as elemento,
                                      matunid.m61_descr  as descricao_unidade_item,
                                      empempitem.e62_descr  as resumo_item
                                  from empempitem
                                      inner join empempaut  on empempaut.e61_numemp  = empempitem.e62_numemp
                                      inner join empautitem on empautitem.e55_autori = empempaut.e61_autori
                                          and empautitem.e55_sequen = empempitem.e62_sequen
                                      left join matunid    on matunid.m61_codmatunid = empautitem.e55_matunid
                            ...
                            $iCodMov = "";
                            if (isset($objNotas->e82_codmov) && !empty($objNotas->e82_codmov)) {
    ...
    function liquidarAjax($iEmpenho, $aNotas, $sHistorico = '')
    {
        ...
        /*
            pegar as notas e verificar uma a uma se ja existe valor liquidado.
        */
        foreach ($aNotas as $iNota) {
            $oNota = new NotaLiquidacao($iNota);
            $nValorNota = db_formatar($oNota->getValorNota(), "f");
            $nValorLiquidado = db_formatar($oNota->getValorLiquidado(), "f");
            if ($nValorLiquidado > $nValorNota) {
                $sMensagem = "Nota {$iNota} no valor de: $nValorNota já possui o valor liquidado de: $nValorLiquidado ";
        ...
        if (!$this->lSqlErro) {
            ...
            for ($i = 0; $i < count($aNotas); $i++) {
                ...
                where e50_numemp = {$iEmpenho}
                  and e71_codnota = {$iCodNota}
                  and e71_anulado is false
              group by  e71_codnota
         having count(e71_codnota) > 1
                ...
                if (pg_num_rows($rs) > 0) {
                    ...
                    $this->lSqlErro = true;
                    $this->sMsgErro = "Erro ao liquidar, pode ocorrer duplicidade.";
    ...
    function gerarOrdemCompra(
        ...
        if (!$this->lSqlErro) {
            ...
            if (!empty($this->oDadosNota->outrosDados)) {
                $objEmpNota->e69_outrosdados = $this->oDadosNota->outrosDados;
            }

            if ($this->oDadosNota && isset($this->oDadosNota->e69_serienota)) {
                $objEmpNota->e69_serienota = $this->oDadosNota->e69_serienota;
            }
        ...
        if (!$this->lSqlErro) {
            ...
            $rsEle = $this->clempelemento->sql_record($this->clempelemento->sql_query($this->numemp,null,"e64_codele"));
     ...
     */
        (float) $nSaldoEmpenho = round(
            $this->dadosEmpenho->e60_vlremp - $this->dadosEmpenho->e60_vlrliq
        ...
        $rsEmpElemento  = $clempelemento->sql_record($clempelemento->sql_query(
            $this->numemp,
            null,
            "e64_vlranu,e64_vlremp,e64_codele,o56_elemento"
        ));
        ...
        if (bccomp($nTotalElemento, $oElemento->e64_vlremp) > 0) { // if $tot > $e64_vlremp
            $this->lSqlErro = true;
            $this->sErroMsg = "Erro[12](Sem saldo no elemento para anular\nNão pode anular o valor digitado para o elemento {$oElemento->o56_elemento} do empenho. Verifique!";
        ...
        $rsParametros   = $clempparametro->sql_record($clempparametro->sql_query_file(
            db_getsession("DB_anousu"),
        ...
        $rsDotacaoSaldo = db_dotacaosaldo(
            8,
            2,
            2,
            "true",
     ...
     */
        ...
        if (isset($oPcParam->pc30_contrandsol) && $oPcParam->pc30_contrandsol == 't') {
            ...
            $rsTransfItens       = $clempautitem->sql_record($clempautitem->sql_query_anuaut(
                null,
                null,
            ...
            if ($clempautitem->numrows > 0) {
                ...
                $rsLocal   = $clsolandam->sql_record($clsolandam->sql_query_andpad(
                    null,
                    "*",
                    null,
                ...
                $rsDestino = $clsolandpadraodepto->sql_record($clsolandpadraodepto->sql_query(
                    null,
                    "*",
                    null,
                                ...
                                if (!$this->lSqlErro) {
                                    ...
                                    if ($this->lSqlErro == false) {
                                        ...
                                        if ($clsolordemtransf->erro_status == 0) {
                                            $this->lSqlErro = true;
                                            $this->sErroMsg = $clsolordemtransf->erro_msg;
                                        }
                                    }
                                }
                            }
        ...
        if (!$this->lSqlErro) {
            ...
            for ($iInd = 0; $iInd < count($aItens); $iInd++) {
                ...
                if (!$this->lSqlErro && $lControlePacto) {
                    ...
                    $sSqlVerificacao = $oDaoEmpempitem->sql_query_item_pacto(
                        null,
                        null,
                        "o88_pactovalor",
                        null,
                    ...
                    if ($oDaoEmpempitem->numrows > 0) {
                        ...
                        try {
                            $this->baixarSaldoPacto(
                                $aItens[$iInd]->e62_sequencial,
     ...
     */
        if (!$this->lSqlErro) {
            ...
            $rsItemSolic     = $clempautitem->sql_record($clempautitem->sql_query_file(
                $this->numemp,
        ...
        /**
         * Caso o empenho seja da folha, devemos excluir a vinculaçao do empenho
         */
        ...
        $sSqlEmpenhoFolha        = $oDaoEmpenhoFolhaEmpenho->sql_query_file(
            null,
            "*",
            null,
       ...
       */
             ...
             */
                        ...
                        if ($oEmpNotaEle->erro_status == 0) {
                            ...
                            $sSqlItensNota = $oEmpNotaItem->sql_query_file(
                                null,
                        ...
                        if ($iTipo == 2) {
                            ...
                            $sSqlDadosOrdem = $oPagOrdemNota->sql_query_valorordem(
                                null,
                                null,
                                "pagordemnota.*,
                                                                      pagordemele.*",
                            ...
                            if ($oPagOrdemNota->numrows > 0) {
                                ...
                                if (round($oPagOrdem->e53_valor, 2) == $nValorPagoMaisAnulado) {
                                    ...
                                    if ($oPagOrdemNota->erro_status == 0) {
                                        $this->lSqlErro = true;
                                        $this->sErroMsg = "Pagordemnota:" . $oPagOrdemNota->erro_msg;
                                        throw new exception($this->sErroMsg);
         ...
         */
                ...
                $oLancam->salvar();
                 ...
                 /**
                 * verifica se tem acordo para estornar
                 * doc 903
                 */
                $oDataImplantacao = new DBDate(date("Y-m-d", db_getsession('DB_datausu')));
                $oInstituicao     = new Instituicao(db_getsession('DB_instit'));
                if (ParametroIntegracaoPatrimonial::possuiIntegracaoContrato($oDataImplantacao, $oInstituicao)) {
                    $oEmpenhoFinanceiro = new EmpenhoFinanceiro($this->dadosEmpenho->e60_numemp);

                    if ($oEmpenhoFinanceiro->getAcordo() != null) {
                        $oEventoContabil           = new EventoContabil(903, db_getsession("DB_anousu"));
                        $aLancamentosCadastrados   = $oEventoContabil->getEventoContabilLancamento();
                        $oLancamentoAuxiliarAcordo = new LancamentoAuxiliarAcordo();
                        $oLancamentoAuxiliarAcordo->setValorTotal($nValorEstornado);
                        $oLancamentoAuxiliarAcordo->setAcordo($oEmpenhoFinanceiro->getAcordo());
                        $oLancamentoAuxiliarAcordo->setEmpenho($oEmpenhoFinanceiro);
                        $oLancamentoAuxiliarAcordo->setHistorico($aLancamentosCadastrados[0]->getHistorico());
                        if ($iCodigoNota != "") {
                            $oLancamentoAuxiliarAcordo->setCodigoNotaLiquidacao($iCodigoNota);
                        }
    ...
    public function getEmpenhosLiberados($oFiltro)
    {
        ...
        if (isset($oFiltro->dtemissini) && isset($oFiltro->dtemissfim)) {
            ...
            if (!empty($dtDataIni) && !empty($dtDataFim)) {
                ...
                $sAnd = " and ";
            } elseif (!empty($dtDataIni)) {
                $sWhere .= "{$sAnd} e60_emiss = '{$dtDataIni}'";
                $sAnd = " and ";
            } elseif (!empty($dtDataFim)) {
        ...
        switch ($oDadosLancamento->tipoevento) {
            ...
            case 20:
                ...
                if (count($aLancamentosEmpenho) > 1 || count($aLancamentosEmpenho) == 0) {
                    throw new Exception("[1] - Lançamento de empenhos invalido.\n");
                }
                $oLancamentoEmpenho = $aLancamentosEmpenho[0];
                if (db_strtotime($dtValidar) < db_strtotime($oLancamentoEmpenho->data)) {
            ...
            case 21:
                 ...
                 */
                $sWhereEmpenho = "c71_coddoc in(3, 23, 33, 203) and c66_codnota = {$oDadosLancamento->codigonotafiscal}";
                $sWhereEmpenho .= " and c70_data <= '{$oDadosLancamento->data}'";
                $aLancamentosEmpenho = $this->getLancamentosContabeis($sWhereEmpenho);
                if (count($aLancamentosEmpenho) > 1 || count($aLancamentosEmpenho) == 0) {
                    throw new Exception("[2] - Lançamento de empenhos invalido.\n");
                }
                $oLancamentoEmpenho = $aLancamentosEmpenho[0];
                if (db_strtotime($dtValidar) < db_strtotime($oLancamentoEmpenho->data)) {

                    //dd($oLancamentoEmpenho, $dtValidar);
    ...
    function excluirLancamento($iCodigoLancamento)
    {
        ...
        switch ($oDadosLancamento->tipoevento) {
            case 10:
                ...
                $sSqlItens      = $oDaoEmpEmpitem->sql_query_left_item_pacto(
                    $this->numemp,
                    null,
                    "e62_sequencial,
                                                                     o105_sequencial,
                ...
                foreach ($aItens as $oItem) {
                    if ($oItem->o105_sequencial != "") {
                        $oDaoPactoValorMovEmpenho = new cl_pactovalormovempempitem;
                    ...
                    if ($oItem->o88_sequencial != "") {
                        $oDaoPactoValorMov = new cl_pactovalormov;
                 ...
                 */
                $oDaoEmpempenhoNL = new cl_empempenhonl;
                ...
                /**
                 * Ligação entre o empenho e uma lista de classificação de credores.
                 */

                $oDaoClassificacaoCredoresEmpenho = new cl_classificacaocredoresempenho;
                $oDaoClassificacaoCredoresEmpenho->excluir(null, "cc31_empempenho = {$this->numemp}");
                if ($oDaoClassificacaoCredoresEmpenho->erro_status == 0) {
                    $sErroMensagem = "Erro ao excluir vinculo de classificação credor \n[ET] - {$oDaoClassificacaoCredoresEmpenho->erro_msg}";
                    throw new Exception($sErroMensagem);
                }
            ...
            case 11: //Estorno
                 ...
                 */
                $oDaoEmpanulado     = new cl_empanulado;
                 ...
                 */
                $oDaoEmpanuladoEle = new cl_empanuladoele;
           ...
           */
                ...
                $oDaoEmpageNotasOrdem       = new cl_empagenotasordem;
                $oDaoEmpageMovForma         = new cl_empagemovforma;
                $oDaoEmpageConf             = new cl_empageconf;
                $oDaoEmpageConfCanc         = new cl_empageconfcanc;
                ...
                $oDaoEmpageConfcheCanc      = new cl_empageconfchecanc;
                ...
                $oDaoempagemovhistorico = new cl_empagemovhistorico;
                ...
                foreach ($aMovimentos as $oMovimento) {
                    ...
                    if ($oDaoEmpageMovForma->erro_status == 0) {
                        $sErroMensagem = "1 - Erro ao excluir forma de pagamento vinculado ao lançamento.\n[ET]-{$oDaoEmpageMovForma->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }
                    ...
                    $oDaoEmpagemovSlips->excluir(null, "k107_empagemov = {$oMovimento->e81_codmov}");
                    if ($oDaoEmpagemovSlips->erro_status == 0) {
                        $sErroMensagem = "Erro ao excluir agendamento de slip vinculado ao lançamento.\n[ET]-{$oDaoEmpagemovSlips->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }
                    $oDaoEmpageMovJustificativa->excluir($oMovimento->e81_codmov);
                    if ($oDaoEmpageMovJustificativa->erro_status == 0) {
                        $sErroMensagem = "Erro ao excluir a justificativa vinculada ao lançamento.\n[ET]-{$oDaoEmpageMovJustificativa->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }
                    $oDaoEmppresta->excluir(null, "e45_codmov = {$oMovimento->e81_codmov}");
                    if ($oDaoEmppresta->erro_status == 0) {
                        $sErroMensagem = "Erro ao excluir a prestação de contas vinculada ao lançamento.\n[ET]-{$oDaoEmppresta->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }


                    $oDaocorempagemov->excluir(null, "k12_codmov = {$oMovimento->e81_codmov}");
                    if ($oDaocorempagemov->erro_status == 0) {
                        $sErroMensagem = "Erro ao excluir o movimento ao lançamento.\n[ET]-{$oDaocorempagemov->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }


                    $oDaoempagemovhistorico->excluir(null, "e141_empagemov = {$oMovimento->e81_codmov}");
                    if ($oDaoempagemovhistorico->erro_status == 0) {
                        $sErroMensagem = "Erro ao excluir forma de pagamento vinculado ao lançamento.\n[ET]-{$oDaoempagemovhistorico->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }

                    $where = sprintf(
                        "(e145_movimento_original = %s or e145_movimento_retencao = %s)",
                        $oMovimento->e81_codmov,
                        $oMovimento->e81_codmov
                    );

                    $daoEmpagemovRetencoes = new cl_empagemovretencoes();
                    $daoEmpagemovRetencoes->excluir(null, $where);
                    if ($daoEmpagemovRetencoes->erro_status == 0) {
                        throw new Exception("Erro ao excluir as retenções. \n{$daoEmpagemovRetencoes->erro_msg}");
                    }
                ...
                foreach ($aRetencoes as $oRetencao) {
                    /* excluimos retencaocorgrupocorrente */

                    $oDaoRetencaoCorGrupoCorr->excluir(null, "e47_retencaoreceita = {$oRetencao->e23_sequencial}");
                    if ($oDaoRetencaoCorGrupoCorr->erro_status == 0) {
                        $sErroMensagem = "[8] Erro ao excluir a ligação da corrente com a retenção, vinculadas ao lançamento.\n[ET]-{$oDaoRetencaoCorGrupoCorr->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }


                    $oDaoslipretencaoreceitas->excluir(null, "k206_retencaoreceitas = {$oRetencao->e23_sequencial}");
                    if ($oDaoslipretencaoreceitas->erro_status == "0") {
                        throw new DBException("Houve um erro ao excluir vinculo do slip com retencao.");
                    }


                    $oDaoretencaoempagemov->excluir(null, "e27_retencaoreceitas = {$oRetencao->e23_sequencial}");
                    if ($oDaoretencaoempagemov->erro_status == "0") {
                        throw new DBException("Houve um erro ao excluir movimento da retencao.");
                    }



                    $oDaoretencaoreceitasadicionais->excluir(null, "e19_retencaoreceitas = {$oRetencao->e23_sequencial}");
                    if ($oDaoretencaoreceitasadicionais->erro_status == 0) {
                        $sErroMensagem = "[8] Erro ao excluir retenções vinculadas ao lançamento.\n[ET]-{$oDaoretencaoreceitasadicionais->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }
                ...
                $oDaoPagOrdemconta    = new cl_pagordemconta;
                $oDaoPagOrdemProcesso = new cl_pagordemprocesso;
                $oDaocairetordem = new cl_cairetordem;
                $oDaoempord = new cl_empord;
                ...
                $oDaoPagOrdemProcesso->excluir(null, "e03_pagordem = {$oNota->e50_codord}");
                if ($oDaoPagOrdemProcesso->erro_status == 0) {
                    $sErroMensagem = "Erro ao excluir o processo da OP vinculada ao lançamento.\n[ET]-{$oDaoPagOrdemProcesso->erro_msg}";
                    throw new Exception($sErroMensagem);
                }


                $oDaocairetordem->excluir(null, "k32_ordpag = {$oNota->e50_codord}");
                if ($oDaocairetordem->erro_status == 0) {
                    $sErroMensagem = "Erro ao excluir cairetordem.\n[ET]-{$oDaocairetordem->erro_msg}";
                    throw new Exception($sErroMensagem);
                }


                $oDaoempord->excluir(null, null, "e82_codord = {$oNota->e50_codord}");
                if ($oDaoempord->erro_status == 0) {
                    $sErroMensagem = "Erro ao excluir cairetordem.\n[ET]-{$oDaoempord->erro_msg}";
                    throw new Exception($sErroMensagem);
                }

                $oDaoissplanitop = new cl_issplanitop;
                $oDaoissplanitop->excluir(null, "q96_pagordem = {$oNota->e50_codord}");
                if ($oDaoissplanitop->erro_status == 0) {
                    $sErroMensagem = "Erro ao excluir cairetordem.\n[ET]-{$oDaoissplanitop->erro_msg}";
                    throw new Exception($sErroMensagem);
                }
                 ...
                 */
                    ...
                    */
                    ...
                    $oDaoEmpnotaEle = new cl_empnotaele;
                    $oDaoEmpnotaEle->excluir($oNota->e69_codnota);

                    if ($oDaoEmpnotaEle->erro_status == 0) {
                           $sErroMensagem = "Erro ao excluir valores liquidados da nota.\n[ET]-{$oDaoEmpnotaEle->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }

                    $oDaoEmpNotaItem      = new cl_empnotaitem;
                    $oDaoEmpNotaItemWhere = "e72_codnota = {$oNota->e69_codnota}";
                    $oDaoEmpNotaItem->excluir(null, $oDaoEmpNotaItemWhere);

                    if ($oDaoEmpNotaItem->erro_status == 0) {
                           $sErroMensagem = "Erro ao excluir a nota.\n[ET]-{$oDaoEmpNotaItem->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }

                    $oDaoEmpnota = new cl_empnota;
                    $oDaoEmpnota->excluir($oNota->e69_codnota);

                    if ($oDaoEmpnota->erro_status == 0) {
                           $sErroMensagem = "Erro ao excluir a nota.\n[ET]-{$oDaoEmpnota->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }
                }
     ...
     */
        ...
        $sSqlOrdens       = $oDaoPagOrdemNota->sql_query_valorordem(
            null,
    ...
    public function excluirLancamentoDesconto($iCodigoDesconto)
    {
        ...
        $oDaoPagOrdemDescontoEmpanulado = db_utils::getDao("pagordemdescontoempanulado");
        $oDaoPagOrdemDescontoEmpanulado->excluir(null, "e06_pagordemdesconto = {$iCodigoDesconto} ");
        if ($oDaoPagOrdemDescontoEmpanulado->erro_status == 0) {
            $sErro = "Houve um erro ao excluir ordem de Desconto Anulado.\n{$oDaoPagOrdemDescontoEmpanulado->erro_msg}";
    ...
    private function manutencaoLancamentosPagamento($oDadosLancamento, $dtLancamento, $iTipo)
    {
                ...
                foreach ($aMovimentos as $oMovimento) {
                    if ($oMovimento->e97_codforma == 2 && $oMovimento->e91_cheque != "") {
                        ...
                        if (db_strtotime($dtLancamento) < db_strtotime($oMovimento->e86_data)) {
                            ...
                            $sMsgErro .= "com o movimento '{$oMovimento->e81_codmov}' com cheque  número ";
                            $sMsgErro .= "'{$oMovimento->e91_cheque}' Emitido com a data de " . db_formatar(
                                $oMovimento->e86_data,
                                "d"
                            ) . ".\n";
        ...
        $sSqlCorrente .= "        corempagemovestorno.k12_sequencial AS sequencialestorno, ";
        $sSqlCorrente .= "        corempagemovestorno.k12_codmov as codigomovimentoestorno, ";
        ...
        $sSqlCorrente .= "       left  join corempagemovestorno on  corrente.k12_id     = corempagemovestorno.k12_id ";
        $sSqlCorrente .= "                                     and  corrente.k12_data   = corempagemovestorno.k12_data ";
        $sSqlCorrente .= "                                     and  corrente.k12_autent = corempagemovestorno.k12_autent ";
...
*/
        ...
        if (pg_num_rows($rsCorrente) > 0) {
          ...
          */
            ...
            if ($iTipo == 1 || $iTipo == 2) {
                ...
                $oDaoCorrente->k12_estorn = $oDadosCorrente->k12_estorn;
            ...
            if ($oDadosCorrente->k12_empen != "") {
                ...
                /**
                 * incluimos a nova autenticacao, case seja alteração de data (iTipo = 1)
                 */
                if ($iTipo == 1 || $iTipo == 2) {
            ...
            if ($oDadosCorrente->sequencial != "") {
                $oDaoCorEmpagemovPagamento                 = new cl_corempagemovpagamento;
                $oDaoCorEmpagemovPagamento->k12_sequencial = $oDadosCorrente->sequencial;
                $oDaoCorEmpagemovPagamento->excluir($oDadosCorrente->sequencial);
                if ($oDaoCorEmpagemovPagamento->erro_status == 0) {
                    $sErroMensagem = "Erro ao excluir informações do cheque na autenticação.\n";
                    $sErroMensagem .= "[ET] - {$oDaoCorEmpagemovPagamento->erro_msg}";
                    throw new Exception($sErroMensagem);
                }
                if ($iTipo == 1 || $iTipo == 2) {
                    $oDaoCorEmpagemovPagamento->k12_id     = $oDadosCorrente->k12_id;
                    $oDaoCorEmpagemovPagamento->k12_data   = $dtLancamento;
                    $oDaoCorEmpagemovPagamento->k12_autent = $iAutenticacao;
                    $oDaoCorEmpagemovPagamento->k12_codmov = $oDadosCorrente->codigomovimento;
                    $oDaoCorEmpagemovPagamento->incluir(null);
                    if ($oDaoCorEmpagemovPagamento->erro_status == 0) {
                        $sErroMensagem = "Erro ao incluir informações da agenda de pagamentos na autenticação.\n";
                        $sErroMensagem .= "[ET] - {$oDaoCorEmpagemovPagamento->erro_msg}";
                        throw new Exception($sErroMensagem);
            ...
            if ($oDadosCorrente->sequencialestorno != "") {
                $oDaoCorEmpagemovEstorno                 = db_utils::getDao("corempagemovestorno");
                $oDaoCorEmpagemovEstorno->k12_sequencial = $oDadosCorrente->sequencialestorno;
                $oDaoCorEmpagemovEstorno->excluir($oDadosCorrente->sequencialestorno);
                if ($oDaoCorEmpagemovEstorno->erro_status == 0) {
                    $sErroMensagem = "Erro ao excluir informações do código do movimento de estono na autenticação.\n";
                    $sErroMensagem .= "[ET] - {$oDaoCorEmpagemovEstorno->erro_msg}";
                    throw new Exception($sErroMensagem);
                }
                if ($iTipo == 1 || $iTipo == 2) {
                    $oDaoCorEmpagemovEstorno->k12_id     = $oDadosCorrente->k12_id;
                    $oDaoCorEmpagemovEstorno->k12_data   = $dtLancamento;
                    $oDaoCorEmpagemovEstorno->k12_autent = $iAutenticacao;
                    $oDaoCorEmpagemovEstorno->k12_codmov = $oDadosCorrente->codigomovimentoestorno;
                    $oDaoCorEmpagemovEstorno->incluir(null);
                    if ($oDaoCorEmpagemovEstorno->erro_status == 0) {
                        $sErroMensagem = "Erro ao incluir informações do código do movimento de estono na autenticação.\n";
                        $sErroMensagem .= "[ET] - {$oDaoCorEmpagemovEstorno->erro_msg}";
                        throw new Exception($sErroMensagem);
                    }
                }
            ...
            if ($oDadosCorrente->k105_sequencial != "") {
                ...
                $oDaoRetencaoCorGrupoCorr = db_utils::getDao("retencaocorgrupocorrente");
                $oDaoRetencaoCorGrupoCorr->excluir(null, "e47_corgrupocorrente = {$oDadosCorrente->k105_sequencial}");
                if ($oDaoRetencaoCorGrupoCorr->erro_status == 0) {
                    $sErroMensagem = "[8] Erro ao excluir a ligação da corrente com a retenção, vinculadas ao lançamento.\n[ET]-{$oDaoRetencaoCorGrupoCorr->erro_msg}";
                    throw new Exception($sErroMensagem);
                }
          ...
          */
            ...
            $oDaoCorNump->excluir($oDadosCorrente->k12_id, $oDadosCorrente->k12_data, $oDadosCorrente->k12_autent);
            if ($oDaoCorNump->erro_status == 0) {
                $sErroMensagem = "Erro ao excluir códigos de arrecadação vinculados ao lançamento contábil da autenticação.\n";
          ...
          * Exclui da corhist
          */
            $oDaoCorHist = db_utils::getDao('corhist');
            $oDaoCorHist->excluir($oDadosCorrente->k12_id, $oDadosCorrente->k12_data, $oDadosCorrente->k12_autent);
            if ($oDaoCorHist->erro_status == 0) {
                $sErroMensagem = "Erro ao excluir informações do histórico vinculados ao lançamento contábil da autenticação.\n";
          ...
          */
            ...
            $oDaoconlancamcorrente->excluir(null, "c86_id = {$oDadosCorrente->k12_id} and c86_data = '{$oDadosCorrente->k12_data}' and c86_autent = {$oDadosCorrente->k12_autent}");

            $oDaocorempagemovpagamento = new cl_corempagemovpagamento;
            $oDaocorempagemovpagamento->excluir(null, "k12_id = {$oDadosCorrente->k12_id} and k12_data = '{$oDadosCorrente->k12_data}' and k12_autent = {$oDadosCorrente->k12_autent}");
            if ($oDaocorempagemovpagamento->erro_status == "0") {
                $sErroMensagem = "Erro ao excluir informações dda autenticação.\n";
                $sErroMensagem .= "[ET] - {$oDaocorempagemovpagamento->erro_msg}";
                throw new Exception($sErroMensagem);
            }
    ...
    /**
     *
     * criada para buscar o controle a partir do ano do empenho,
     * ocorreu de um empenho de 2019 se r pago em 2022
     * funcao buscava somente um ano pra tras do ano do pagamento, no caso
     * buscava somente em 2021
     ...
     * @return bool
     */
    public static function possuiLancamentoDeControleAnoEmpenho($iNumeroEmpenho, $iAno = null, array $aDocumentos)
    {
     ...
     * @throws Exception
     */
    public static function ordemCompraAutomatica($iCodigoNota)
    {
        return self::ordemDeCompraManual($iCodigoNota) === false;
    }

    /**
     * Retorna o código do documento para liquidação ser executada
    ...
    public static function getLiquidacaoPorControleLiquidacao($documento, $estorno = false)
    {
        ...
        return $deParaLiquidacao[$documento];
    }
     ...
     * @throws DBException
     */
    public static function getDocumentoLiquidacao(EmpenhoFinanceiro $empenho)
    {
        $iAno = db_getsession("DB_anousu");
        $numeroEmpenho = $empenho->getNumero();
        $restosAPagar = $empenho->isRestoAPagar($iAno);

        $daoMatOrdemProcessoEntrada = new cl_matordemprocessoentrada();
        $buscaProcessoEntrada = $daoMatOrdemProcessoEntrada->sql_query_empenho("m57_processoentrada", "e60_numemp = {$numeroEmpenho} limit 1");
        $buscaProcessoEntrada = db_query($buscaProcessoEntrada);
        if (!$buscaProcessoEntrada) {
            throw new DBException("Ocorreu um erro na consulta do processo de entrada da nota.");
        }
        if (pg_num_rows($buscaProcessoEntrada) === 1) {
            $tipoEntrada       = db_utils::fieldsMemory($buscaProcessoEntrada, 0)->m57_processoentrada;
            $documentoRegistro = ordemCompra::getDocumentoPorProcessoEntrada($tipoEntrada, $restosAPagar);
            return (int) self::getLiquidacaoPorControleLiquidacao($documentoRegistro);
        }
        return false;
    }
    ...
    public function validaDadosDiaria($seqempenho){
    ...
    $xelemento = substr($resultado[0]["o56_elemento"], 0, 7);
    $xtipocompra = $resultado[0]["pc50_codcom"];  //8 - Suprimento de Fundos

    if($xelemento == "3339014" && $xtipocompra != 8){
        return true;
    }
    /*if($xelemento == "3339039" && $xtipocompra != 8){
        return true;
    }*/
     ...
     * @throws Exception
     */
    public function getDocumentoInclusaoLiquidacao(EmpenhoFinanceiro $oEmpenhoFinanceiro, $codigoNota)
    {

        $anoSessao = db_getsession("DB_anousu");
        $daoBuscaElemento = new cl_orcelemento();
        $buscaEstrutural = $daoBuscaElemento->sql_query_file($oEmpenhoFinanceiro->getDesdobramentoEmpenho(), $oEmpenhoFinanceiro->getAno(), "o56_elemento");
        $rsBuscaEstrutural = db_query($buscaEstrutural);

        if (!$rsBuscaEstrutural) {
            throw new Exception("Ocorreu um erro ao consultar o estrutural do desdobramento do empenho.");
        }
        ...
        //AQUI TINHA QUE VIR 3, ESTAVA VINDO COMO 412
        //var_dump($oEmpenhoFinanceiro->getEmpenho());
        $xseqemp = $oEmpenhoFinanceiro->getNumero();
        $ediaria = $this->validaDadosDiaria($xseqemp);
        //var_dump($ediaria);
        //die("Confere");
        ...
        if($ediaria){
            ...
            if ($oEmpenhoFinanceiro->isPrestacaoContas()) {            
                $documento = HistoricoDocumento::LIQUIDACAO_SUPRIMENTO_FUNDOS;
            }
        }

        /*
        //SE COMENTAR RESOLVE, MAS ATRAPALHA OUTROS
        if ($oEmpenhoFinanceiro->isPrestacaoContas()) {            
            $documento = HistoricoDocumento::LIQUIDACAO_SUPRIMENTO_FUNDOS;
        }
        */

        $oCodigoGrupoContaOrcamento = GrupoContaOrcamento::getGrupoConta($oEmpenhoFinanceiro->getDesdobramentoEmpenho(), $anoSessao);
        if ($oCodigoGrupoContaOrcamento && !$lIsPassivo && !$isPrestacaoContas && self::ordemDeCompraManual($codigoNota)) {
            switch ($oCodigoGrupoContaOrcamento->getCodigo()) {
                case 7:
        ...
        if (UTILIZA_INCORPORACAO_BEM) {
            $documentoLiquidacao = self::getDocumentoLiquidacao($oEmpenhoFinanceiro);
            if (!empty($documentoLiquidacao) && self::ordemDeCompraManual($codigoNota)) {
                $documento = $documentoLiquidacao;
            }
        }
