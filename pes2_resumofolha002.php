...
try {
    ...
    define("TIPO_RELATORIO_RECURSO", "7");
    ...
    switch ($oParametros->iTipoRelatorio) {
        ...
        case TIPO_RELATORIO_RECURSO:
            ...
            $sCampoWhereRecurso = "o15_recurso::int";
            $sCampoEstruturalTipoRelatorio = "o15_recurso";
    ...
    if ($oParametros->iTipoRelatorio <> TIPO_RELATORIO_GERAL) {
        switch ($oParametros->iTipoFiltro) {
            ...
            case TIPO_FILTRO_INTERVALO:
                if ($oParametros->iTipoRelatorio === TIPO_RELATORIO_RECURSO) {
                    $aWhere['tipo_filtro' . $oParametros->iTipoRelatorio] = "{$sCampoWhereRecurso} between '{$oParametros->iIntervaloInicial}'::int and '{$oParametros->iIntervaloFinal}'::int";
                } else {
            ...
            case TIPO_FILTRO_SELECIONADOS:
                if ($oParametros->iTipoRelatorio === TIPO_RELATORIO_RECURSO) {
                    $aWhere['tipo_filtro' . $oParametros->iTipoRelatorio] = "{$sCampoWhereRecurso} in (" . implode("', '", $oParametros->iRegistros) . ")";
                } else {
    ...
    foreach ($aRubricas as $sEstruturalFiltro => $aRubricas) {
        ...
        $oTotais->nValorBaseFGTSMenorAprendiz = 0;
        $oTotais->nValorFGTS = 0;
        $oTotais->nValorBaseIRRF = 0;
        $oTotais->nValorBaseInformativa = 0;
        ...
        foreach ($aRubricasOrdenacao[$sEstruturalFiltro] as $sCodigoRubrica => $sDescricaoRubrica) {
            ...
            foreach ($aEventosFinanceiros as $oEventoFinanceiro) {
                ...
                switch ($oEventoFinanceiro->getNatureza()) {
                    ...
                    case EventoFinanceiroFolha::TIPO_BASE_INFORMATIVA:
                    case EventoFinanceiroFolha::TIPO_BASE_DEDUTORA:

                        if ($oEventoFinanceiro->getNatureza() == EventoFinanceiroFolha::TIPO_BASE_INFORMATIVA) {
                            $oTotais->nValorBaseInformativa += $oEventoFinanceiro->getValor();
                        }
                        ...
                        switch ($oRubrica->getCodigo()) {
                            ...
                            case 'R961'://Valor Base do FGTS Menor Aprendiz
                                $oTotais->nValorBaseFGTSMenorAprendiz += $oEventoFinanceiro->getValor();
                                break;
                            ...
                            case 'R992' :
                                ...
                                switch ($oEventoFinanceiro->getServidor()->getTabelaPrevidencia()) {
                                    ...
                                    case '1' :
                                        $oTotais->nValorBasePrevidencia1 += ($oEventoFinanceiro->getValor() * $oDadosPatronais->aBasePrevidencia1->nValor / 100);
                                        break;

                                    case '2':
                                        $oTotais->nValorBasePrevidencia2 += ($oEventoFinanceiro->getValor() * $oDadosPatronais->aBasePrevidencia2->nValor / 100);
                                        break;

                                    case '3':
                                        $oTotais->nValorBasePrevidencia3 += ($oEventoFinanceiro->getValor() * $oDadosPatronais->aBasePrevidencia3->nValor / 100);
                                        break;

                                    case '4':
                                        $oTotais->nValorBasePrevidencia4 += ($oEventoFinanceiro->getValor() * $oDadosPatronais->aBasePrevidencia4->nValor / 100);
                                        break;
                                }
        ...
        foreach ($aRubricasOrdenacao[$sEstruturalFiltro] as $sCodigoRubrica => $sDescricaoRubrica) {
            ...
            $oTotalEventosRubricas = new stdClass();
            $oTotalEventosRubricas->nValorBase = 0;
            $oTotalEventosRubricas->nQuantidadeBase = 0;
            $oTipoNatureza = '';
                ...
                switch ($oEventoFinanceiro->getNatureza()) {
                    ...
                    case EventoFinanceiroFolha::TIPO_BASE_INFORMATIVA:
                            if (!$criaCabecalho) {
                                ...
                                $oPdf->cell($oTamanhoColunas->quantidade, $iAlt, 'QUANT.', 1, 0, 'C', 1);
                                $oPdf->cell($oTamanhoColunas->descricao, $iAlt, 'DESCRIÇÃO', 1, 0, 'C', 1);
                                $oPdf->cell($oTamanhoColunas->proventos, $iAlt, 'VALOR', 1, 0, 'C', 1);
                                $oPdf->cell($oTamanhoColunas->descontos, $iAlt, 'TIPO BASE', 1, 1, 'C', 1);
                                
                                $criaCabecalho = true;

                            }

                            $oTotalEventosRubricas->nValorBase += $oEventoFinanceiro->getValor();
                            $oTotalEventosRubricas->nQuantidadeBase += $oEventoFinanceiro->getQuantidade();
                            
                            $oTipoNatureza = 'INFORMATIVA';
                        break;
                }
            }

            if($oTotalEventosRubricas->nValorBase > 0) {

                $sValorBase = db_formatar($oTotalEventosRubricas->nValorBase,'f');
                $oPdf->setfont('arial', '', 8);
                $oPdf->cell($oTamanhoColunas->rubrica, $iAlt, $oRubrica->getCodigo(), 0, 0, "R", 0);
                $oPdf->cell($oTamanhoColunas->funcionarios, $iAlt, $iQuantidadeServidores, 0, 0, "R", 0);
                $oPdf->cell($oTamanhoColunas->quantidade, $iAlt, db_formatar("{$oTotalEventosRubricas->nQuantidadeBase}", "f"), 0, 0, "R", 0);
                $oPdf->cell($oTamanhoColunas->descricao, $iAlt, $oRubrica->getDescricao(), 0, 0, "L", 0);
                $oPdf->cell($oTamanhoColunas->proventos, $iAlt, $sValorBase, 0, 0, "R", 0);
                $oPdf->cell($oTamanhoColunas->descontos, $iAlt, $oTipoNatureza, 0, 1, "R", 0);
...
function mostraTotalizador($oPdf, $aRubricas, $oDadosFolha, $oDadosPatronais)
{
    ...
    $oPdf->setX($iEspacoX);
    $oPdf->cell(65, $iAlt, 'BASE INFORMATIVA  ', 0, 0, "L", 0);
    $oPdf->cell(20, $iAlt, db_formatar($oDadosFolha->nValorBaseInformativa, 'f'), 0, 1, "R", 0);
    ...
    $nValorPrevidencia3 = round(($oDadosFolha->nValorBasePrevidencia3 * 100) / $nValorBasePrevidencia3, 2);
    $nValorPrevidencia4 = round(($oDadosFolha->nValorBasePrevidencia4 * 100) / $nValorBasePrevidencia4, 2);
    ...
    $oPdf->ln(3);
    $oPdf->cell(25, $iAlt, "BASE F.G.T.S. Menor Apr.:", 0, 0, "L", 0);
    $oPdf->cell(20, $iAlt, db_formatar($oDadosFolha->nValorBaseFGTSMenorAprendiz, 'f'), 0, 0, "R", 0);

    $oPdf->cell(25, $iAlt, "F.G.T.S. EMPR. Menor Apr.:", 0, 0, "L", 0);
    $oPdf->cell(20, $iAlt, db_formatar($oDadosFolha->nValorBaseFGTSMenorAprendiz * 0.02, 'f'), 0, 1, "R", 0);
