...
$rsParfiscal = $clparfiscal->sql_record($clparfiscal->sql_query_file(db_getsession('DB_instit'),"y32_calculavistoriamei"));
...
$sSqlVistoriasLocalizacao .= "   from fiscalizacao.fis_vistoriaslotevist                                       ";
$sSqlVistoriasLocalizacao .= "        left  join fiscalizacao.fis_vistretornocalc on y04_codmsg  = y05_codmsg  ";
$sSqlVistoriasLocalizacao .= "        inner join fiscalizacao.fis_vistorias       on y05_codvist = y70_codvist ";
$sSqlVistoriasLocalizacao .= "        inner join fiscalizacao.fis_vistinscr       on y71_codvist = y05_codvist ";
...
if ($oParfiscal->y32_calculavistoriamei == "f") {
    $sSqlVistoriasLocalizacao .= " and (SELECT COUNT(isscadsimples.q38_inscr)
                                          FROM isscadsimples
                                          LEFT JOIN isscadsimplesbaixa
                                            ON q39_isscadsimples = q38_sequencial
                                         WHERE q38_inscr = q02_inscr
                                           AND q39_dtbaixa IS NULL
                                           AND q38_categoria = 3) = 0 " ;
}
...
if (empty($iResultadoVistorias)) {
  ...
  $sSqlVistoriasSanitario .= "   from fiscalizacao.fis_vistoriaslotevist                                       ";
  $sSqlVistoriasSanitario .= "        left  join fiscalizacao.fis_vistretornocalc on y04_codmsg  = y05_codmsg  ";
  $sSqlVistoriasSanitario .= "        inner join fiscalizacao.fis_vistorias       on y05_codvist = y70_codvist ";
  $sSqlVistoriasSanitario .= "        inner join fiscalizacao.fis_vistsanitario   on y74_codvist = y70_codvist ";
  $sSqlVistoriasSanitario .= "        inner join fiscalizacao.fis_sanitario       on y74_codsani = y80_codsani ";
  $sSqlVistoriasSanitario .= "        inner join fiscalizacao.fis_sanitarioinscr  on y18_codsani = y80_codsani ";
  ...
  $sSqlVistoriasSanitario .= "     and y70_instit = ".db_getsession('DB_instit')               ;
    if ($oParfiscal->y32_calculavistoriamei == "f") {
        $sSqlVistoriasSanitario .= " and (SELECT COUNT(isscadsimples.q38_inscr)
                                            FROM isscadsimples
                                            LEFT JOIN isscadsimplesbaixa
                                                ON q39_isscadsimples = q38_sequencial
                                            WHERE q38_inscr = q02_inscr
