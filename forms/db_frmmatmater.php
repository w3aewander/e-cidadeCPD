...
function verificaAdmin($idusuario){
  $sql = pg_query("SELECT administrador FROM db_usuarios WHERE id_usuario = {$idusuario}");
  $resultado = pg_fetch_all($sql);
  return $resultado[0]["administrador"];
}
...
<!--
  ...
  <tr>
    ...
    <tr>
        ...
        <td>
            <?php
            $xusu = db_getsession("DB_id_usuario");
            $usuhab = verificaAdmin(db_getsession("DB_id_usuario"));
             
            $xx = array("f"=>"NAO", "t"=>"SIM");
            if($usuhab){
              $m60_servico_disabled = $db_opcao != 1 ? "" : "";
            }else{
