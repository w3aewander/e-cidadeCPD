...
<div class="container">
    <form name=form1 action="" method="POST">
                    ...
                    <fieldset>
                        ...
                        <table>
                            ...
                            <tr>
                                ...
                                <td colspan="1">
                                    <?php db_input('e03_numeroprocesso', 13, '', true, 'text', $db_opcao_inf, null, null, null, null, 30) ?>
                                </td>
                ...
                <td colspan="2px">
                    <fieldset>
                        ...
                        <?php if(isRioDeJaneiro()){ ?>
                            <span>
                              <b style ='color:red'>Campo obrigatório para uso no SIGFIS do TCE-RJ, com no máximo 4000 caracteres.
                              </b>
                            </span>
                            ...
                            <table width="100%">
                                <tr>
                                    <td>
                                        <?php
                                        db_textarea('historico', 4, 110, 0, true, 'text', 1, "onkeyup='js_verificaCaracteresHistorico()'","","");
                                        ?>
                                    </td>
                                </tr>
                                <tr>
                                    <div align = 'right'>
                                        <td>
                                            <span> <b> N° de caracteres :</b></span>
                                            <input type = 'text' size='4' id= 'historico_caracteres' value = '' disabled/>
                                        </td>
                                    </div>
                                </tr>
                            </table> 
        ...
        <input name="confirmar" type="button" id="confirmar" value="Confirmar"
               onclick="return js_liquidar('<?= $metodo ?>')">
        <input name="pesquisar" type="button" id="pesquisar" value="Pesquisar" onclick="js_pesquisa('');">

        <?php db_input("receitas_valores", 30, "", true, 'hidden', 1, 'readonly', '', '', 'text-align:right') ?>
        <?php db_input("valor_liquidar", 30, "", true, 'hidden', 1, 'readonly', '', '', 'text-align:right') ?>
    </form>
</div>
<script>
    ...
    function js_saida(oAjax) {
        ...
        js_verificaCaracteresHistorico();
        ...
        let notas = [];
        for (let nota of obj.data) {
            let encontrou = notas.filter((notaAdicionada) => {
                return notaAdicionada.e69_codnota === nota.e69_codnota
            }).length > 0;
            if (!encontrou) {
                notas.push(nota);
            }
        ...
        if (obj.numnotas > 0) {
            for (var i = 0; i < notas.length; i++) {
                sClassName = 'normal';
                if (notas[i].libera == 'disabled') {
                    sClassName = ' disabled ';
                }

                if (in_array(notas[i].e69_codnota, obj.aItensPendentesPatrimonio) && !UTILIZA_INCORPORACAO_BEM) {
                    sClassName = ' disabled ';
                }

                if (iOperacao == 1) { //liquidacao

                    var nSaldoNota = (js_strToFloat(notas[i].e70_valor) - js_strToFloat(notas[i].e70_vlrliq) -
                        js_strToFloat(notas[i].e70_vlranu) - js_strToFloat(notas[i].e53_vlrpag)).toFixed(2);

                } else if (iOperacao == 2) { //estorno
                    if (js_strToFloat(notas[i].e53_vlrpag) > 0) {
                        var nSaldoNota = 0;
                    } else {
                        var nSaldoNota = (js_strToFloat(notas[i].e70_vlrliq) - js_strToFloat(notas[i].e70_vlranu)).toFixed(2);
                    }
                }

                var nValorLiquidado = js_strToFloat(notas[i].e70_vlrliq).toFixed(2);
                var nValorPago = js_strToFloat(notas[i].e53_vlrpag).toFixed(2);
                if (nValorLiquidado > 0 && nValorLiquidado == nValorPago) {
                    continue;
                }

                /*
                 * se o valor liquidado for maior ou igual o valor da nota
                 * bloqueamos a seleção, pois permitia executar varias vezes a liquidação
                 */
                if ((nValorLiquidado >= js_strToFloat(notas[i].e70_valor)) && !lTelaEstorno) {
                    sClassName = ' disabled ';
                }


                iTotNotas++;
                nValorRetencao = notas[i].vlrretencao;
                numnota = js_decodeUrl(notas[i].e69_numero);
                iCodOrd = notas[i].e50_codord;
                saida += "<tr class='" + sClassName + "' id='trchk" + notas[i].e69_codnota + "' style='height:1em'>";
                saida += "<td class='linhagrid' style='text-align:center'>";
                saida += "<input type='hidden' id='sInfoAgenda" + notas[i].e69_codnota + "' value='" + js_decodeUrl(notas[i].sInfoAgenda) + "'>";
                /* Extensão CotaMensalLiquidacao - pt 4 */
                saida += "<input type='checkbox' " + notas[i].libera + " onclick='js_marcaLinha(this)'";
                saida += " class='chkmarca' name='chk" + notas[i].e69_codnota + "'";
                saida += " id='chk" + notas[i].e69_codnota + "' value='" + notas[i].e69_codnota + "' " + sClassName + "></td>";
                saida += '<td class=\'linhagrid\' style=\'text-align:center\'><b>';
                saida += "<a href='' onclick='js_consultaNota(" + notas[i].e69_codnota + ");return false'>";
                saida += notas[i].e69_codnota + "</a></b></td>";
                saida += "<td class='linhagrid' style='text-align:center' id='numero" + notas[i].e69_codnota + "' >" + numnota + "</td>";
                saida += "<td class='linhagrid' style='text-align:center'>" + notas[i].e69_dtnota + "</td>";
                saida += "<td class='linhagrid' style='text-align:center'>" + notas[i].e70_valor + "</td>";
                saida += "<td class='linhagrid' style='text-align:right;width:10%'>" + notas[i].e70_vlranu + "</td>";
                saida += "<td class='linhagrid' style='text-align:right;width:10%'>" + notas[i].e70_vlrliq + "</td>";
                saida += "<td class='linhagrid' style='text-align:right;width:10%'>" + notas[i].e53_vlrpag + "</td>";
                saida += "<td class='linhagrid' style='text-align:right;width:10%'>";
                if (iOperacao == 1) {
                    saida += "<a href='' id='retencao" + notas[i].e69_codnota + "' ";
                    saida += "   onclick='js_lancarRetencao(" + notas[i].e69_codnota + ",\"" + iCodOrd + "\",\"" + notas[i].e70_valor + "\",\"" + notas[i].e82_codmov + "\");";
                ...
                if (lTelaEstorno) {
                    saida += "<td class='linhagrid competencia" + notas[i].e69_codnota + "' style='text-align:center;width:10%'>" + notas[i].competencia + "</td>";
                }

                saida += "</tr>"
            }

            notas.each(function (oCodigoNota, iLinha) {

                if (in_array(oCodigoNota.e69_codnota, obj.aItensPendentesPatrimonio) && !UTILIZA_INCORPORACAO_BEM) {
         ...
         */
        if (iOperacao == 1 && obj.lPossuiIntegracaoPatrimonial && empty(obj.oGrupoElemento.iGrupo) && !obj.lPrestacaoConta && !UTILIZA_INCORPORACAO_BEM) {
    ...
    function js_liquidar(metodo) {
        ...
        if(isRJ && $F('historico').trim() == '' ){

           alert('É obrigatório informar o histórico do estorno da liquidação!');
           return false;

        } else if (isRJ && $F('historico').length > 4000){
            
            alert('É obrigatório informar no histórico um limite de no máximo 4000 caracteres!');
            return false;
        }

        if (js_strToFloat($F('saldo_disp')) <= 0) {
            alert('Saldo disponível negativo.');
            return false;
        }
        ...
        if (<?php echo $tela_estorno ? "false" : "true"; ?> &&
        !validarCompetencia()
    )
        {
            $('pesquisar').disabled = false;
    ...
    function js_lancarRetencao(iCodNota, iCodOrd, nValor, iCodMov) {
        ...
        js_OpenJanelaIframe('CurrentWindow.corpo', 'db_iframe_retencao',
            ...
            '&iNumCgm=' + iNumCgm +
            "&lSession=" + lSession + "&nValorBase=" + js_strToFloat(nValor) + "&iCodMov=" + iCodMov + "&callback=true",
    ...
    function js_verificaCaracteresHistorico(){
        
        if ($('historico_caracteres')) {
            $('historico_caracteres').value = $('historico').value.length;
        }
    }
