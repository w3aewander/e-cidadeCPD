...
require_once(modification("classes/db_certbaixanumero_classe.php"));
...
if (isset($alterar)) {
  ...
  if (empty($q60_portepadraomei)) {

    $sMsgErro                = "Campo porte padrão para processamento do MEI deve ser informado.";
    $clparissqn->erro_msg    = $sMsgErro;
    $clparissqn->erro_status = "0";
    $clparissqn->erro_campo  = "q60_portepadraomei";
    $sql_erro                = true;
  }
  ...
  if (!$sql_erro) {
    ...
    if ($result == false || $clparissqn->numrows == 0) {
      ...
      if ($q60_parcelasissqn == "") {
          $clparissqn->q60_parcelasissqn = 'NULL';
      } else {
          $clparissqn->q60_parcelasissqn = $q60_parcelasissqn;
      }
      ...
      if (!$sql_erro) {
        ...
        if ($q60_portepadraomei == "") {
          $clparissqn->q60_portepadraomei = 'NULL';
        } else {
          $clparissqn->q60_portepadraomei = $q60_portepadraomei;
        }
  ...
  $db_opcao = 2;
  $result   = $clparissqn->sql_record($clparissqn->sql_query(null,'*'));
  if ($result != false && $clparissqn->numrows > 0) {
  ...
  /**
   * Retorna a descricao do documento template da bic de veiculos
   */
  if (isset($q60_templatebicveiculo) && $q60_templatebicveiculo != "") {

    $sSqlCertidaoBaixaOficial = $cldb_documentotemplate->sql_query_file(null, 'db82_descricao as db82_descricaobicveiculo',
                                                                        null, "db82_sequencial = {$q60_templatebicveiculo}");
    $rsCertidaoBaixaOficial   = $cldb_documentotemplate->sql_record($sSqlCertidaoBaixaOficial);
    if ($rsCertidaoBaixaOficial != false && $cldb_documentotemplate->numrows > 0) {
      db_fieldsmemory($rsCertidaoBaixaOficial, 0);
    }
...
<html>
  ...
  <body class="body-default">
    <?php
	  include(modification("forms/db_frmparissqn.php"));
