...
class ImportacaoRelatorioLegal
{
    ...
    public function __construct($iCodigoRelatorio = null, $sCaminhoArquivo)
    {
        ...
        $sObjectJson = JSON::create()->parse(file_get_contents($sCaminhoArquivo, FILE_TEXT));
    ...
    public function importar($anoUsu = null)
    {
        $anoUsu = $anoUsu ?: db_getsession('DB_anousu');
        ...
        if (!empty($iCodigoRelatorio)) {
            $this->limparPeriodos();
            $this->limparColunas($anoUsu);
            $this->limparFiltroPadrao($anoUsu);
        }
     ...
     * @throws Exception
     */
    public function getAnoConfiguracaoUsuario($anoUsu)
    {
        $oDaoParametro = new cl_orcparamseqfiltroorcamento();
        $sSqlBuscaParametro = $oDaoParametro->sql_query_file(
            null,
            "o133_anousu",
            null,
            "o133_orcparamrel = {$this->getCodigoRelatorio()} and o133_anousu = {$anoUsu}"
        );
    ...
    public function getParamSeqConfiguracaoUsuario($anoUsu)
    {
        $oDaoParametro = new cl_orcparamseqfiltroorcamento();
        $sSqlBuscaParametro = $oDaoParametro->sql_query_file(
            null,
     ...
     * @throws Exception
     */
    private function limparColunas($anoUsu)
    {
        ...
        $orcParamSeq = $this->getParamSeqConfiguracaoUsuario($anoUsu);

        if (!empty($orcParamSeq)) {
            $oDaoColunas = new cl_orcparamseqorcparamseqcoluna();
            $oDaoColunas->excluir(
                null,
                "o116_codparamrel = {$iCodigoRelatorio} and o166_codseq in (" . implode(",", $orcParamSeq) . ")"
            );
     ...
     * @throws Exception
     */
    private function limparFiltroPadrao($anoUsu)
    {
        ...
        $oDaoFiltro = new cl_orcparamseqfiltropadrao();
        $oDaoFiltro->excluir(null, "o132_orcparamrel = {$iCodigoRelatorio} and o132_anousu = {$anoUsu}");
