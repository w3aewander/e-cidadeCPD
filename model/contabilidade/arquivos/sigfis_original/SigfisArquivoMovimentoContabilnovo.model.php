    ...
    if ($clConLanCam->numrows > 0) {
      ...
      for ($i = 0; $i < $clConLanCam->numrows; $i++) {
       ...
       //   if (!isset($aMovimentoContabil[$sIndice])) {
           ...
           $sContaCorrente = "select (case when $iAnoSessao < 2016 then c56_sequencial else c56_contabancaria end) as seq_conta_corrente from contabilidade.conplano left join conplanocontabancaria on conplano.c60_codcon = conplanocontabancaria.c56_codcon and conplano.c60_anousu   = conplanocontabancaria.c56_anousu left join configuracoes.contabancaria on db83_sequencial = c56_contabancaria where c60_anousu = $this->iAnoUso and c60_codcon = $oDadosQuery->conta ";
...
//$this->addLog( $DadosContaCorrente->seq_conta_corrente);
