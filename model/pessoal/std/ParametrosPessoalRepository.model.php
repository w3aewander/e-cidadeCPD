 ...
 * @version $Id: ParametrosPessoalRepository.model.php,v 1.20 2018/06/05 11:41:17 dbiuri Exp $
 */
...
class ParametrosPessoalRepository {
  ...
  private function carregarParametros(ParametrosPessoal $oParametros) {
    ...
    $sCampos       .= "r11_palime,                         ";
    $sCampos       .= "r11_rubricaabonopecuniario, ";
    $sCampos       .= "r11_rubricadifsalario ";
    ...
    if( !empty($oDadoParametros->r11_rubricaabonopecuniario)) {
      $oParametros->setRubricaAbonoPecunia(RubricaRepository::getInstanciaByCodigo($oDadoParametros->r11_rubricaabonopecuniario));
    }

      if(!empty($oDadoParametros->r11_mes13)) {
          $oParametros->setMes13($oDadoParametros->r11_mes13);
      }

      if(!empty($oDadoParametros->r11_mesusu)) {
          $oParametros->setMes($oDadoParametros->r11_mesusu);
      }
      if(!empty($oDadoParametros->r11_anousu)) {
          $oParametros->setAno($oDadoParametros->r11_anousu);
      }

    if(!empty($oDadoParametros->r11_rubricadifsalario)) {
      $oParametros->setRubricadifsalario($oDadoParametros->r11_rubricadifsalario);
    }
    return $oParametros; 
  }
