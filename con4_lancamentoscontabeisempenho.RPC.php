...
switch ($oParam->exec) {
    ...
    case 'alterarLancamento':
        ...
        try {
            ...
            $oRetorno->msgLog = $oEmpenho->getLog();
    ...
    case "getInfoLancamento":
        try {
            ...
            $oRetorno->recurso = recursoLancamento($oParam->iLancamento);
            if (empty($oRetorno->recurso)) {
                $oRetorno->recurso = $complemento->getComplementoPorLancamento(
                    $oParam->iLancamento,
    ...
    case 'consultarLog':
        try {
            $oRetorno->erro = false;
            $oRetorno->logs = lancamentoContabil::getLog($oParam->lancamento);
        } catch (Exception $e) {
            $oRetorno->erro = true;
 ...
 * @throws Exception
 */
function recursoLancamento($codigoLancamento)
{
    $sql = "
            select o15_codigo, o200_sequencial, o15_recurso, o15_descr, o200_descricao
             ...
             where o201_codlan = {$codigoLancamento}
            ";
    $rs = db_query($sql);
    if (!$rs) {
