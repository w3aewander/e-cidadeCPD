...
$certificacao = $_POST["certificado"];
...
if($certificacao == "com"){
  $adendo = "Com certificado.";
} else {
  $adendo = "Sem certificado.";
}

if($ops == "p"){
    $info2 = "Somente OP's pagas. " . $adendo;
} elseif($ops == "n"){
    $info2 = "Somente OP's não pagas. " . $adendo;
} elseif($ops = "gnp"){
  $info2 = "Geral Não Pagos." . $adendo;
}else{
  $info2 = "Todas OP's. "  . $adendo;
} 
...
if($ops == "p") {
    ...
    //$txt_where .= " and c53_coddoc in (3,33,23,39,206,208) ";
    $txt_where .= " and c53_coddoc in (3,33,23,39,204,206,208,210) ";
...
//2026-01-06
if($ops == "n" && db_getsession("DB_instit") != 1){
    $tira = " and o56_elemento not like '3339030%' and o56_elemento not like '3339039%' and o56_elemento not like '3339036%' and o56_elemento not like '344905%' and o56_elemento not like '3339040%' ";
}else{
    $tira = "";
}
...
and o56_elemento not like '3319%'
and o56_elemento not like '3333%'
and o56_elemento not like '3334%'
and o56_elemento not like '3337%'
and o56_elemento not like '3339091%'
and o56_elemento not like '3449091%'
and o56_elemento not like '3469091%'
and o56_elemento not like '3459091%'
and o56_elemento not like '3339067%'
and o56_elemento not like '3469071%'
and o56_elemento not like '3329%'
 ) 
 and(z01_numcgm != '6000'
and z01_numcgm != '6517'
...
and z01_numcgm != '7766')
and ( o56_elemento like '3339030%' 
   or o56_elemento like '3339039%' 
   or (o56_elemento like '3339036%' and o56_elemento not like '333903696%') 
   or o56_elemento like '344905%' 
   or o56_elemento like '3339040%' )
...
//var_dump($sSqlEmpenhos);
//die("Consulta dos Empenhos");

//order by o40_anousu, e50_data, e60_instit, o40_orgao, o15_codigo --ORINGIAL
//order by o40_anousu, e60_instit, o40_orgao, o15_codigo, e50_data
...
//testa($x);
//die("Verifica consulta principal");
  ...
  $html1 = ' <table style="width: 100%; border: 1px solid black; border-collapse: collapse">   
             <thead>
                <tr bgcolor="#EBEBEB">
                    ...
                    <td align="center" style="border: 1px solid black; width: 10; font-weight: bold;  font-size: 8pt;">'.utf8_encode("Nº Nota de Liq.").'</td>
                    <td align="center" style="border: 1px solid black; width: 10; font-weight: bold;  font-size: 8pt;">'.utf8_encode("Data Adimplemento").'</td>
                    <td align="center" style="border: 1px solid black; width: 10; font-weight: bold;  font-size: 8pt;">'.utf8_encode("Data Certificado").'</td>
                    <td align="center" style="border: 1px solid black; width: 9; font-weight: bold;  font-size: 8pt;">'.utf8_encode("Data Pagamento.").'</td> 
                    <td align="center" style="border: 1px solid black; width: 4; font-weight: bold;  font-size: 8pt;">Valor</td> 
                    <td align="center" style="border: 1px solid black; width: 9; font-weight: bold;  font-size: 8pt;">'.utf8_encode("Data Publicação").'</td>
                    <td align="center" style="border: 1px solid black; width: 7; font-weight: bold;  font-size: 8pt;">'.utf8_encode($palavra).'</td>
...
function verificapago2($ordem){
    $sql = pg_query("SELECT * from pagordemele where e53_codord = {$ordem}");
    $resultado = pg_fetch_all($sql);
    if($resultado[0]["e53_vlranu"] == 0 && $resultado[0]["e53_vlrpag"] == 0){
        return false;
    }
    return true;
}
...
function confereCertificacao($ordem){
    //echo "SELECT datageracao FROM certificacaoconformidade WHERE e50_codord = {$ordem}"; echo "<br>";
      $sql = pg_query("SELECT datageracao FROM certificacaoconformidade WHERE e50_codord = {$ordem}");
      $resultado = pg_fetch_all($sql);
      return $resultado[0]["datageracao"];
    }

function confereSuspensao($ordem){
      $sql = pg_query("SELECT suspensao FROM certificacaoconformidade WHERE e50_codord = {$ordem}");
      $resultado = pg_fetch_all($sql);
      return $resultado[0]["suspensao"];
    }
...
for ($iRow = 0; $iRow < pg_num_rows($rsEmpenhos); $iRow++) {
  ...
  if($ops == "t") {
    ...
    $sSqlEmpenhospagos = "
      ...
      and z01_numcgm != '55636'
      and z01_numcgm != '217'
and z01_numcmg != '12143'
and z01_numcmg != '112873'
and z01_numcmg != '103446'
and z01_numcmg != '2686'
and z01_numcmg != '117185'
and z01_numcmg != '367'
and z01_numcmg != '256'
and z01_numcmg != '13428'
and z01_numcmg != '65963'
and z01_numcmg != '11006'
and z01_numcmg != '60626'
and z01_numcmg != '56459'
and z01_numcmg != '77171'
and z01_numcmg != '556'
and z01_numcmg != '118712'
and z01_numcmg != '67387'
and z01_numcmg != '3331'
and z01_numcmg != '60099'
      and z01_numcgm != '7766')
    ...
    {   
      ...
      if($ops == "p"){
        ...
        if(db_getsession("DB_instit") == 45){
            if(!$oEmpenhos->ordem){continue;}
            $veri2 = verificapago2($oEmpenhos->ordem);
        }else{
            if(!$oEmpenhos->ordem){continue;}
    ...
    $datacertificado = confereCertificacao($oEmpenhos->ordem);
    if($datacertificado){
      $sit2 = contaData($datacertificado);
    } else{
    ...
    if(!$verificador){        
      $novoarray[$oEmpenhos->ordem] = array(
            ...
            $oEmpenhos->retencoes, //17
            $oEmpenhos->dataliqnova, //18
...
if($certificacao == "com"){
  foreach ($novoarray as $linha){    
    $verificacert = confereCertificacao($linha[0]);
    //var_dump($verificacert); echo "<br>";
    $verificasus = confereSuspensao($linha[0]);

    if(db_getsession("DB_instit") == 1){
        if( $linha[1] == "1059/2024" ||
            $linha[1] == "1965/2025" ||
            $linha[1] == "375/2024"  ||
            $linha[1] == "1584/2025" ||
            $linha[1] == "1320/2024" 
    ...
    if($verificacert){
      if($ops != "gnp"){
        $linha[18] = $verificacert;
        $linha[21] = $verificasus;
      }
      //$linha[18] = $verificacert;
      array_push($novoarray2, $linha);
    }
  }
}else{
  foreach ($novoarray as $linha){
...
//testa($novoarray2); die("Tem que ter resultado");
if($ops == "gnp"){
  foreach ($novoarray2 as $key => $part) {
       $sort[$key] = strtotime($part[6]);
        //$sort[$key] = $part[19];  
  }
  array_multisort($sort, SORT_ASC, $novoarray2);
  //testa($novoarray2);
  //die("Confere sort");
}
...
if($ops !="gnp"){
    ...
    array_multisort($sort["instituicao"], SORT_ASC, $sort["orgao"], SORT_ASC, $sort["fonte"], SORT_ASC, $novoarray2);
    //testa($novoarray2);
    //die("Confere");
}
...
if($ops =="n"){    
  ...
  $guardafonte = "";
  $totorgao = 0;
  foreach ($novoarray2 as $result) {       
    if($result[19] != $guardaorgao){
      if($guardaorgao != ""){
        //$mpdf->ln();
        //$html1x = '<tr><td colspan="14">TOTAL:'.number_format($totorgao, 2, ",", ".").'</td></tr>';


        $mpdf->WriteHTML('<tr><td colspan="14">TOTAL:'.number_format($totorgao, 2, ",", ".").'</td></tr>');
        $guardafonte = "";
        //$html1x = '<tr><td colspan="14">TOTAL:'.number_format($totorgao2, 2, ",", ".").'</td></tr>';
        //$html1x .= '</table>';
...
//$mpdf->Output();
$mpdf->Output("ordem_cronológica.pdf", "I");
