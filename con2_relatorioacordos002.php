...
$sCampos    .= " acordo.ac16_acordocategoria, liclicita.l20_numero, liclicita.l20_anousu, cflicita.l03_descr";



$sSqlAcordo  = $clacordo->sql_query_completo_posicao(null,$sCampos,$sOrder,$sWhere, true);
...
for ($iInd = 0; $iInd < $clacordo->numrows; $iInd++) {
  ...
  $oDadosAcordo->sTipoInstrumento   = $aTiposInstrumento[$oAcordo->getTipoInstrumento()];
  $oDadosAcordo->sLicitacao         =  $oAcordoCompleto->l20_numero ?
    "{$oAcordoCompleto->l03_descr} - {$oAcordoCompleto->l20_numero}/{$oAcordoCompleto->l20_anousu}" :
    '';
  ...
  if (trim($oPost->listaitens) == 'S') {
    ...
    if (count($oDadoAcordo->aItens) > 0) {
      ...
      foreach ($oDadoAcordo->aItens as $oDadoAcordoItem) {
        ...
        $oPdf->Cell(28 ,$iAlt,trim(db_formatar($oDadoAcordoItem->getValorUnitario,'f', " ", 0, 'e', 4))            ,1,0,'R',$iCorFundo);
  ...
  if (trim($oPost->listaautorizacao) == 'S') {
    ...
    if (count($oDadoAcordo->aAutorizacoes) > 0) {
      ...
      foreach ($oDadoAcordo->aAutorizacoes as $oDadoAcordoAutorizacao) {
        ...
        $oPdf->Cell(56 ,$iAlt,'R$ ' . trim(db_formatar($oDadoAcordoAutorizacao->e54_valor, 'f')),1,0,'R',$iCorFundo);
...
function imprimirCabecalhoAcordos($oPdf, $oDado, $lImprime, $iFonte, $iAlt) {
  ...
  $oPdf->SetFillColor(220);
  $oPdf->SetFont('Arial','B',$iFonte);
  $oPdf->Cell(35 ,$iAlt,'Acordo: ', 0, 0, 'L', 0);
  $oPdf->SetFont('Arial','',$iFonte-1);
  ...
  $oPdf->SetFont('Arial','',$iFonte-1);
  $oPdf->Cell(107 ,$iAlt,$oDado->sTipoInstrumento,0,0,'L',0);

  $oPdf->ln();

  $oPdf->SetFont('Arial','B',$iFonte);
  $oPdf->Cell(35 ,$iAlt,'Processo: ',0,0,'L',0);
  $oPdf->SetFont('Arial','',$iFonte-1);
  $oPdf->Cell(107 ,$iAlt,$oDado->numeroprocesso, 0,0,'L',0);

  $oPdf->SetFont('Arial','B',$iFonte);
  $oPdf->Cell(35 ,$iAlt,'Data da Assinatura: ',0,0,'L',0);
  $oPdf->SetFont('Arial','',$iFonte-1);
  ...
  $oPdf->SetFont('Arial','B',$iFonte);
  $oPdf->Cell(35 ,$iAlt,'Vigência: ',0,0,'L',0);
  $oPdf->SetFont('Arial','',$iFonte-1);
  ...
  $oPdf->SetFont('Arial','B',$iFonte);
  $oPdf->Cell(35,$iAlt,'Número Contrato/Ano: ',0,0,'L',0);
  $oPdf->SetFont('Arial','',$iFonte-1);
  $oPdf->Cell(107 ,$iAlt,$oDado->getNumeroAcordo,0,0,'L',0);
  $oPdf->ln();

  $oPdf->SetFont('Arial','B',$iFonte);
  $oPdf->Cell(35 ,$iAlt,'Categoria: ',0,0,'L',0);
  $oPdf->SetFont('Arial','',$iFonte-1);
