...
class ProfissionalEscolaRepository extends BaseRepository
{
    ...
    //die("Entra aqui?");
    if ($dataSistema === null) {
        $dataUsuTimestamp = db_getsession("DB_datausu");
        $dataSistema = Carbon::createFromTimestamp($dataUsuTimestamp);
    }
    ...
    $turmasQuery = TurmasRegenteView
        ...
        //->where('ano', 2025);
        ->where('ano', $dataSistema->year);
    
    /*if ($dataSistema !== null) {
        $dataFormatada = $dataSistema->format('Y-m-d');
        ...
        $turmasQuery->whereRaw(
            "(ed322_periodoinicial IS NULL OR ? >= ed322_periodoinicial)",
            [$dataFormatada]
        );
        
        $turmasQuery->whereRaw(
            "(ed322_periodofinal IS NULL OR ? <= ed322_periodofinal)",
            [$dataFormatada]
        );
    }*/
