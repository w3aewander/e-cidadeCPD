...
class Operation
{
    ...
    public function execute($rawContent)
    {
        ...
        if ($this->ignore && $this->ignore->type() == 'global' && $this->ignore->match($content)) {
            return $content;
        }
        ...
        for ($iOffset = $offset; $iOffset < $limit; $iOffset++) {
            ...
            // Altera arquivo
            $content = substr_replace($content, $replace, $matches[$iOffset], $searchLength);

            // Corrige posicao das proximas ocorrencias
            // - percorre os matches e adiciona/remove posicoes
    ...
    private function replaceCallback($matches)
    {
        ...
        // ignore
        if ($this->ignore && $this->ignore->type() != 'global' && $this->ignore->match($matches[0])) {
            return $matches[0];
        }
