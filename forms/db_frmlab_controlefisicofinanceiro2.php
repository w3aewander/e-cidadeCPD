    ...
    <table style="border: 0;">
        ...
        if ($iTipoControle == 3 || $iTipoControle == 6 || $iTipoControle == 7) {
            ...
            <tr>
                ...
                <td>
                    <?php
                    db_input('la56_i_subgrupo', 10, $Ila56_i_subgrupo, true, 'hidden');
                    db_input(
                        'sd61_c_subgrupo',
                        2,
                        $Isd61_c_subgrupo,
                    ...
                    db_input('sd61_c_nome', 50, $Isd61_c_nome, true);
                    ?>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <?php
                    db_input('la56_i_formaorganizacao', 10, $Ila56_i_formaorganizacao, true, 'hidden');
                    db_input(
                    ...
                    db_input('sd62_c_nome', 50, $Isd62_c_nome, true);
                    ?>
                </td>
            </tr>
            <?php
        ...
        <tr>
            <td nowrap title="<?= @$Tla56_i_teto ?>" colspan="2">
                ...
                $decimal = isset($la56_i_teto) && $la56_i_teto == 2 ? 2 : 0;
                $la56_n_limite = isset($la56_n_limite) ? number_format($la56_n_limite, $decimal, ',', '.') : '';
                db_input(
                    'la56_n_limite',
                    10,
                    $Ila56_n_limite,
            ...
            <td colspan="2">
                <table>
                    <tr>
                        ...
                        <td>
                            <fieldset style='width: 20%;'>
                                <legend><b>Quando o saldo for insuficiente:</b></legend>
                                <table style="border: 0;">
                                    <tr>
...
<script>
    const inputLimite = $('la56_n_limite');
    inputLimite.addEventListener('focus', () => {
        if ($F('la56_n_limite') == '')  {
            return;
        }
        inputLimite.value = jsRemoveMascaraMoeda($F('la56_n_limite'));
    });
    inputLimite.addEventListener('blur', () => {
        inputLimite.value = formataLimite($F('la56_n_limite'), $F('la56_i_teto') === '1');
    });
    /* GLOBALS */
    ...
    function js_ajax(oParam, jsRetorno, sUrl, lAsync) {
        ...
        js_divCarregando('Carregando dados do Laboratório.', 'loading_message');
    ...
    function js_criaDataGrid() {
        ...
        oDBGrid.setCellWidth(new Array(
            '5%',
            '25%',
            '15%',
            '10%',
        ...
        if (oRetorno.iStatus == 1) {
            ...
            for (var iCont = 0; iCont < oRetorno.aControles.length; iCont++) {
                with (oRetorno.aControles[iCont]) {
                    ...
                    aLinha[++iInd] = formataLimite(la56_n_limite, la56_i_teto == 1);
    ...
    function js_confirmar(sOp, iCod) {
        ...
        if (sOp == 'excluir') {
            ...
            oParam.la56_n_limite = jsRemoveMascaraMoeda($F('la56_n_limite'));
    ...
    function js_pesquisala56_i_laboratorio(mostra) {
        if (mostra == true) {
            js_OpenJanelaIframe(
                ...
                'func_lab_laboratorio.php?funcao_js=parent.js_mostralab_laboratorio1|la02_i_codigo|la02_c_descr',
                'Pesquisa',
                true
            );
        } else {
    ...
    function js_pesquisala56_i_grupo(mostra) {
        if (mostra == true) {
            js_OpenJanelaIframe(
                'CurrentWindow.corpo',
                'db_iframe_sau_grupo',
                'func_sau_grupo.php?funcao_js=parent.js_mostrasau_grupo|sd60_i_codigo|sd60_c_nome|sd60_c_grupo',
                'Pesquisa',
                true
            );
        } else {
    ...
    function listenerLimite(oLimite) {
        const valor = jsRemoveMascaraMoeda(oLimite.value);
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get('iGasto') && parseFloat(valor) < parseInt(urlParams.get('iGasto'), 10)) {
            alert('O limite não pode ser menor que valor já cadastrado!');
            oLimite.value = formataLimite(urlParams.get('iGasto'), $F('la56_i_teto') === '1');
        }
    }

    function formataLimite(valor, isFisico) {
        if (valor === '') {
            return valor;
        }
