...
if(!defined('DB_BIBLIOT')){
  ...
  if (!defined('FPDF_FONTPATH')){
    define('FPDF_FONTPATH','fpdf151educacao/font/');
  }
  require_once modification("fpdf151educacao/fpdf.php");
...
//   $dados = db_query("select nomeinst,ender,munic,uf,telef,email,url,logo from db_config where codigo = ".db_getsession("DB_instit"));
  ...
  //$this->Cell(100,32,"",1);
    ...
    if(db_getsession("DB_itemmenu_acessado") == 2000352 || db_getsession("DB_itemmenu_acessado") == 3992 || db_getsession("DB_itemmenu_acessado") == 2000353 || db_getsession("DB_itemmenu_acessado") == 587073 || db_getsession("DB_itemmenu_acessado") == 2000354 || db_getsession("DB_itemmenu_acessado") == 2000356){
      $this->SetFont('Arial','',20);
      //$this->multicell(0,-20,@$GLOBALS["head1"],0,"C",0);
      $this->multicell(0,10,@$GLOBALS["head1"],0,"J",0);  
      $this->SetFont('Arial','',7);  
      $this->multicell(0,3,@$GLOBALS["head2"],0,"J",0);
      $this->multicell(0,3,@$GLOBALS["head3"],0,"J",0);
      $this->SetFont('Arial','',20);      
      $this->multicell(0,10,@$GLOBALS["head4"],0,"J",0);    
      $this->SetFont('Arial','',7);  
    ...
    $this->multicell(0,3,@$GLOBALS["head9"],0,"J",0);
    */
    //var_dump($_SESSION); die("Confere");
    
    if(db_getsession("DB_itemmenu_acessado") == 9152){
      $this->SetFont('Arial','',30);
      $this->multicell(0,-20,@$GLOBALS["head10"],0,"C",0);
      $this->SetFont('Arial','',7);  
    }
    ...
    //if(db_getsession("DB_itemmenu_acessado") == 3235 && db_getsession("DB_modulo") == 116){}



      //var_dump(db_getsession("DB_itemmenu_acessado")); //3235
      //var_dump(db_getsession("DB_modulo")); //116     
      //die("Confere PDF");
...
//#99#//}
  ...
  global $url;
    if($this->imprime_rodape == true) {
      ...
      //$this->Cell(0,10,$sMenuAcess. "  ". $nome.'   Emissor: '.substr(ucwords(strtolower($emissor)),0,30).'  Exerc: '.db_getsession("DB_anousu"),"T",0,'L');
