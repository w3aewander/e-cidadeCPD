...
//echo $sSqlCredito;die;
      ...
      for($i = 0; $i < pg_num_rows($rsCredito); $i++) {
        ...
        $oDadosCredido->nValor      = db_formatar($oCredito->k00_valor, 'f');
        $oDadosCredido->sTipoDebito = $oCredito->k00_descr;
        $oDadosCredido->iReceita    = $oCredito->k00_receit;
        $oDadosCredido->sReceita    = $oCredito->k02_descr;
        $aCreditos[] = $oDadosCredido;
      }
      $oRetorno->aCreditos = $aCreditos;
      json_encode(DBString::utf8_encode_all($oRetorno));
    break;
