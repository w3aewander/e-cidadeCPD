...
try {
  ...
  switch ($oParam->exec) {
    ...
    case "verificaEventoEmpenho":
      ...
      $sEmpenho           = $oEmpenhoFinanceiro->getCodigo() . "/" . $oEmpenhoFinanceiro->getAno();

      if ($lPrestacao && USE_PCASP) {

        $oPrestacao = $oEmpenhoFinanceiro->getDadosPrestacaoContas();
    	$iCodigoTipoPrestacao = $oPrestacao->e45_tipo;
        $iTotalItensPrestado = count($oPrestacao->iTensPrestacao);
        ...
        /**
         * agora so vai bloquear a rotina se a prestacao de conta ja tenha algum item lancado
         */
        if ($iTotalItensPrestado > 0) {
    ...
    case "getDadosEmpenho":
      ...
      $codigoOrdem = "";
      $empord = new cl_empord();
      $sqlOrdem = $empord->sql_query_file( null, null, "e82_codord", null, "e82_codmov = {$oParam->iCodigoMovimentacao}");
      $rsOrdem = $empord->sql_record($sqlOrdem);
      if ($empord->numrows > 0) {
        $codigoOrdem = db_utils::fieldsMemory($rsOrdem, 0)->e82_codord;
      }
      ...
      $oRetorno->iCodigoOrdem = $codigoOrdem;
