...
switch($oParam->exec) {
  ...
  case "getOrdens" :
   ...
   if ($oParam->params[0]->iRecurso != "") {
    ...
    foreach ($aOrdensAgenda as &$ordem) {
        $ordem->o15_codigo = getFonteReduzidaRecurso($ordem->o15_codigo);
    }
  ...
  case "getSaldos":
    ...
    $oAgenda->setUrlEncode(true);
    if ((int)$_SESSION['DB_itemmenu_acessado'] === 7050) {

      $buscaCodigoContaPagadora = $oDaoEmpAgeTipo->sql_query_file(null, 'e83_codtipo', null, 'e83_conta = '.$oParam->params[0]->iCodTipo);
      ...
      $oParam->params[0]->iCodTipo = $iCodTipo;
    }
    $oRetorno                = new stdClass;
    $oRetorno->status        = 1;
    $oRetorno->oSaldoTes     = $oAgenda->getSaldoConta($oParam->params[0]->iCodTipo, $oParam->params[0]->dtBase, $oParam->params[0]->movimento);
