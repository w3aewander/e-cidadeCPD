...
class itemPacto {
  ...
  public function baixarSaldoSolicitacao($nQuantidade, $nValor, $iItemSolicitacao) {
    ...
    if (empty($this->codigo)) {
      throw new Exception('Informe o código do item do pacto', 1);
    }
    /**
     * Verificamos se nao existe um movimento para o item dessa solicitação
    ...
    if ($oDaoPactoValorMovSolicita->numrows > 0 ) {
      ...
      if ($oDaoPactoValorMov->erro_status == 0) {
        throw new Exception('Erro ao Incluir movimentacão para o pacto.\nOperacao Abortada', 2);
      }     
    } 
    ...
    /**
     * Verificamos se o item do compras já está vinculado ao item do pacto 
     */
  ...
  public function excluirSaldoSolicitacao($iItemSolicitacao) {
    ...
    if ($oDaoPactoValorMovSolicita->numrows > 0 ) {
      ...
      $oDaoPactoValorMov->excluir($oDaoPactoValorMov->o88_sequencial);
      if ($oDaoPactoValorMov->erro_status == 0) {
        throw new Exception('Erro ao cancelar movimentacão para o pacto.\nOperacao Abortada', 2);
      }     
    }
  }
  ...
  public function atualizaSaldoRealizado($nValor, $iConta, $iOrdem = null) {
    ...
    if ($nValor != 0) {
      ...
      if ($oDaoPactoSaldo->erro_status == 0) {
        ...
        $sErroMsg = "Não foi possível atualizar o saldo do item do pacto.";
