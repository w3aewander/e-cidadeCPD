...
try {
    $verificaParametroIntegracao = "SELECT 1 ";
    $verificaParametroIntegracao .= "    FROM parametrointegracaopatrimonial ";
    $verificaParametroIntegracao .= " WHERE c01_instit = " . db_getsession("DB_instit") . " ";
    $verificaParametroIntegracao .= "    AND c01_data IS NOT NULL                         ";
    $verificaParametroIntegracao .= "    AND c01_modulo = " . ParametroIntegracaoPatrimonial::MATERIAL;

    $rsIntegracaoPatrimonial = db_query($verificaParametroIntegracao);
    if (!$rsIntegracaoPatrimonial) {
        throw new Exception("Erro ao buscar Parametros de Integração Patrimonial.");
    }

    $sGrupoConta = "
        SELECT DISTINCT e60_numemp, e62_sequencial
          FROM empempenho
           INNER JOIN empelemento ON e64_numemp = e60_numemp
           INNER JOIN empempitem ON e62_numemp = e64_numemp
           INNER JOIN orcelemento ON o56_codele = e64_codele
                      AND o56_anousu = e60_anousu
           INNER JOIN conplanoorcamentogrupo ON c21_codcon = o56_codele
                      AND c21_anousu = o56_anousu
                      AND c21_instit = e60_instit
           INNER JOIN matordemitem ON m52_numemp = e60_numemp
        WHERE  m52_codordem = {$_GET['m51_codordem']}
    ";
    $rsGrupoConta = db_query($sGrupoConta);

    if (pg_num_rows($rsIntegracaoPatrimonial) > 0 && pg_num_rows($rsGrupoConta) == 0) {
        throw new Exception("Você está a efetuar a entrada da nota de um empenho cuja despesa não está vinculada a nenhum grupo. É necessário que o desdobramento deste empenho esteja vinculado ao Grupo do Plano orçamentário na Contabilidade, pois a Instituição possui Integração Patrimonial. <br/ >
    ...
    if (pg_num_rows($rsGrupoConta) > 0 && pg_num_rows($rsIntegracaoPatrimonial) == 0) {
        throw new Exception("Não foi configurado a Integração do patrimonial <br/> Acesse para configurar: DB:FINANCEIRO > Contabilidade > Procedimentos > Parâmetros > Integração Patrimonial");
    }
...
<form name="form1" method="post">
    <input type="hidden" name="nuinst" id="nuinst" value="<?=$nuinst?>">
    <fieldset style='text-align: left'>
        <Legend><b>Dados da Ordem de Compra</b></legend>
        <table >
            <tr>
                <td title="<?= @$Tm51_codordem ?>">
            ...
            <tr>
                <td title=""><b>Valor da Ordem de Compra:</b></td>
                <td>
                    <?php
            ...
            <?php if (UTILIZA_INCORPORACAO_BEM) { ?>
                <tr>
                    <td><label for="processo_entrada_nota"><b>Processo de Entrada da Nota:</b></label></td>
                    <td>
                        <select id="processo_entrada_nota" style="width: 100%;">
                            <option selected='selected' value="">Selecione</option>
                            <option value="1">Entrada de Serviços Prestados</option>
                            <option value="2">Entrada de Material de Consumo no Almoxarifado</option>
                            <option value="3">Entrada de Bens Permanentes</option>
                            <option value="4">Materiais Incorporáveis a Bens Permanentes</option>
                            <option value="5">Serviços Incorporáveis a Bens Permanentes</option>
                        </select>
                    </td>
                </tr>
            <?php } ?>
                    ...
                    <fieldset>
                        ...
                        <table>
                            <tr>
                                ...
                                <td colspan="7" style='display: <?= $iControlaPit == 1 ? "" : "none" ?>'>
                                    <?php
                            ...
                            <tr id="linhaOutrosCamposNota" style="display: none">
                                <td class="campoNumeroChave" style="display: none">
                                    <label for="numeroChave"  class="bold" >Número da Chave: </label>
                                </td>
                                <td class="campoNumeroChave" colspan="3" style="display: none">
                                    <input type="text" id="numeroChave" name="numeroChave" class="field-size-max"
                                           maxlength="44">
                                </td>

                                <td class="campoNumeroSerie" style="display: none">
                                    <label for="numeroSerie"  class="bold" >Número de Série: </label>
                                </td>
                                <td class="campoNumeroSerie" colspan="3" style="display: none">
                            ...
                            <tr>
                                <td colspan='2'>
                                    <b>Série NF: </b>
                                    <input type="text" maxlength="3" name="e69_serienota" id="e69_serienota" disabled>
                                </td>
                                <td colspan='2'>  
                                    <b>SubSérie NF: </b>
                                    <input type="text" maxlength="3" name="e69_subserienota" id="e69_subserienota" disabled>
                                </td>
                                <script>
                                      var campo = document.getElementById("e69_numero");
                                      campo.addEventListener("focusout",function(){
                                      valor = campo.value;
                                      if(!empty(valor)){
                                        document.getElementById("e69_serienota").disabled = false;
                                        document.getElementById("e69_subserienota").disabled = false;
                                      } else {
                                        document.getElementById("e69_serienota").disabled = true;
                                        document.getElementById("e69_subserienota").disabled = true;
                                      }
                                      },false);
                                </script>
                                <td colspan='3'>  
                                    <b>Nº Processo Licitatório: </b>
                                    <input type="text" name="numprocessolicit" id="numprocessolicit">
                                </td>
                            </tr>
                            <tr>
    ...
    <table width="100%" border='0' cellspacing="0" cellpadding="0">
        <div id="alertaMaterial" class="alert alert-primary text-left" role="alert" hidden>
            não foi possível definir um material de entrada, verifique os dados do lançamento para os itens destacados.
        </div>
        <tr>
            <td width="50%" valign="top">
                <fieldset style='text-align: left'>
                    ...
                    <table border='0' width='100%' cellspacing="0" cellpadding="0"
                           style='border: 2px inset white;table-layout: fixed;' id='gridItens'>
                        <tr>
                            <th class='table_header' width='20'>
                                <input type='checkbox' checked style='display: none' id='mtodos' onclick='js_marca()'>
                                <b>
                                    <a onclick='js_marca()' style='cursor: pointer'>M</a></b>
                            </th>
                            <th class='table_header' align='center'><b>Emp.</b></th>
                            <th class='table_header' width="30%" align='center'><b>Material Compra</b></th>
                        ...
                        <tbody id='dados'
                               style='height: 215px; width: 100%; overflow: scroll; overflow-x: hidden; background-color: white'>
                        </tbody>
                    </table>
                </fieldset>
            </td>
            <td width="50%" valign="top">
                <fieldset style='text-align: left'>
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                db_selectrecord("matmater", $rsInicio, true, 1, "style='width:200pt' disabled='disabled'", "", "", "", "js_atualizarUnidade();");
                                ?>
                        </tr>
                        <tr>
                            <td><b>Lote:</b></td>
                    ...
                    <hr>
                    <button type='button' style='border: 1px solid #999999' title='Salvar informações do item'
                            id='salvar' onClick='js_saveMaterial(false)'><u>S</u>alvar
                    </button>
                    <?php if (db_permissaomenu(db_getsession("DB_anousu"), 480, 3981) == "true") { ?>
                        <button type='button' style='border: 1px solid #999999' title='Cadastradar Novo item de Entrada'
                                id='novoItem' value='' onclick="js_novomatmater()">Novo
                        </button>
                    <?php } ?>
                    <button type='button' style='border: 1px solid #999999' title='Escolher Item de Entrada'
                            id='escolher' onclick='js_escolherMater();'>Escolher
                    </button>
                    <button type='button' style='border: 1px solid #999999' title='Fracionar Entrada do item'
                            id='doFracionar' onclick='js_saveMaterial(true)'><u>F</u>racionar
                    </button>
                    <button type='button' style='border: 1px solid #999999' title='Cancela o fracionamento Escolhido'
                            id='excluirFracionar' onclick='js_cancelarFracionamento();' disabled value=''>Cancelar
                        Fracionamento
                    </button>
                    <br>
                    <button type='button' style='border: 1px solid #999999' title='' id='acertoQtdValor'
                            onclick='js_acertaQtdValor(this.value);' value='acerta' disabled>Acerto Qtd/Valor
                    </button>
                </fieldset>
            </td>
        </tr>
    ...
    <input name="Confirmar" id='confirmar' type="button" value="Confirmar" onclick='js_confirmaEntrada()'>
...
<script type="text/javascript">
    ...
    var grupoContaEmpenho = null;
    const UTILIZA_INCORPORACAO_BEM = '<?php echo UTILIZA_INCORPORACAO_BEM ? 't' : 'f'; ?>' === 't';
    ...
    const linhaOutrosCamposNota = document.getElementById('linhaOutrosCamposNota');

    const camposNumeroChave = document.getElementsByClassName('campoNumeroChave');
    const camposNumeroSerie = document.getElementsByClassName('campoNumeroSerie');
    ...
    const inputNumeroNota = document.getElementById('e69_numero');
    ...
    if (isPB) {
        inputTipoNota.addEventListener('change', () => {
            ...
            linhaOutrosCamposNota.style.display = 'table-row';
            if (tipo.chave != 0) {
                alteraEstilo(camposNumeroChave, 'table-cell');
            }
            if (tipo.serie != 0) {
                alteraEstilo(camposNumeroSerie, 'table-cell');
            }
        });
    }

    const alteraEstilo = (colecao, estilo) => {
    ...
    function js_atualizarUnidade() {

        strJson = '{"method":"getInfoMaterialEntrada","matmater":"' + $F('matmater') + '","m51_codordem":"' + $F('m51_codordem') + '"}';
        url = 'mat4_matordemRPC.php';
        oAjax = new Ajax.Request(
            url, {
                method: 'post',
                parameters: 'json=' + strJson,
                onComplete: js_retornoAtualizarUnidade
            }
        );
    };

    function js_retornoAtualizarUnidade(oAjax) {
        var retorno = JSON.parse(oAjax.responseText);
        ...
        $('quantunid').value = retorno.quantunid;
        $('unidadeentrada').value = retorno.unidade;
    }
    ...
    function js_retornoGetDados(oAjax) {
        ...
        var oJson = JSON.parse(oAjax.responseText);
        if(oJson.eletronico === false){
        ...
        if (oJson.erroGrupo != '') {
            alert(oJson.erroGrupo.urlDecode());
        }

        mapItemGrupo = oJson.mapItemGrupo;
        var result = oJson.result;

        grupoContaEmpenho = oJson.dadosEmpenho.grupo;
        if (oJson.processoEntradaNota != null && UTILIZA_INCORPORACAO_BEM) {

            var inputProcessoEntrada = document.getElementById('processo_entrada_nota');
            inputProcessoEntrada.value = oJson.processoEntradaNota;
            inputProcessoEntrada.disabled = true;
        }
        ...
        if (oJson.itens.length > 0) {
            ...
            for (var iItens = 0; iItens < oJson.itens.length; iItens++) {
                with (oJson.itens[iItens]) {
                    console.log(oJson.itens[iItens]);
                    iCodLinha = m52_codlanc + '_' + iIndiceEntrada;
                    sClassName = " normal";
                    if (m63_codmatmater == '') {
                        sClassName = 'semMatmater';
                    }
                    if (pc01_servico === 't' || m52_valor === "0") {
                        sClassName = "normal";
                    }
                    if (sClassName === "semMatmater") {
                        document.getElementById('alertaMaterial').hidden = false;
                    }
                    sDisabled = "";
                    sReturn = "";

                    if (result !== false) {
                        if ((typeof mapItemGrupo !== 'undefined' && typeof mapItemGrupo[e62_sequencial] === 'undefined') || (new Number(saldovalor).toFixed(3) == 0 || Number(saldoitens).toFixed(2) == 0)) {
                            sClassName = "disabled";
                            sDisabled = "disabled";
                            checked = "";
                    ...
                    if (!fraciona) {
                        ...
                        sRow += " <div class='teste' onmouseover='js_setAjuda(this,true)' onmouseOut='js_setAjuda(null,false)'>";
                        ...
                        sRow += "   <span onmouseover='js_setAjuda(this,true)' onmouseOut='js_setAjuda(null,false)'>" + e62_descr.urlDecode() + "</span>";
                        sRow += " </div></td>";
    ...
    function js_send(obj, iCodLanc, iIndice) {
        ...
        if (obj.checked) {
            ...
            /*
             * atualiza o item na seção, e marca ele como desabilitado
             * somente itens pai(o que não é fracionado);
             */
    ...
    function js_retornoSend(oAjax) {
        ...
        oItemAtivo = JSON.parse(oAjax.responseText);
        ...
        if (oItemAtivo.aMateriaisEstoque.length > 0) {
            ...
            for (var iInd = 0; iInd < oItemAtivo.aMateriaisEstoque.length; iInd++) {
                ...
                oOptions.descr = oItemAtivo.aMateriaisEstoque[iInd].m60_descr; //Propriedade Original da Requisição com URLENCODE
        ...
        if (oJson.aItens.length > 0) {
            ...
            for (var iItens = 0; iItens < oJson.aItens.length; iItens++) {
                with (oJson.aItens[iItens]) {
                    ...
                    if (sClassName === 'semMatmater') {
                        verificaClasse.push(sClassName)
                    }
                    ...
                    if (!fraciona) {
                        ...
                        sRow += " onmouseover='js_setAjuda(this,true)' onmouseOut='js_setAjuda(null,false)'>";
                        sRow += pc01_codmater + "-" + pc01_descrmater.urlDecode() + "</div></td>";
                        sRow += "  <td class='" + sClassTD + "' id='descritem" + iCodLinha + "' nowrap style='text-align:left;overflow:hidden'>";
                        sRow += " <div style='overflow:hidden' onmouseover='js_setAjuda(this,true)' onmouseOut='js_setAjuda(null,false)'>";
            ...
            if (verificaClasse.length === 0 ) {
                document.getElementById('alertaMaterial').hidden = true;
            }
    ...
    function js_mostramatordem(chave, erro) {
        ...
        if (erro == true) {
            ...
            // Recarregar a página inteira após selecionar outra ordem de compra
            window.location = `mat4_entraMaterialLote002.php?m51_codordem=${chave}`;

            // Limpa o campo 'Processo de Entrada da Nota' ao pesquisar ordem de compras quando existe incorporação de bens.
            if (UTILIZA_INCORPORACAO_BEM) {
                $(processo_entrada_nota).value = '';
                $(processo_entrada_nota).disabled = false;
            }
    ...
    function js_novomatmater() {
        ...
        lServico = oItemAtivo.pc01_servico;

        js_OpenJanelaIframe('CurrentWindow.corpo', 'iframe_material',
            'mat1_matmater011.php?m63_codpcmater=' + cod + '&numemp=' + numemp + '&sequen=' + sequen + '&lLotes=1&lServico=' + lServico,
    ...
    function js_confirmaEntrada() {
        ...
        if (UTILIZA_INCORPORACAO_BEM) {
            ...
            var mostrarValidacao = false;
            var labelGrupo = '';
            var inputProcessoEntrada = document.getElementById('processo_entrada_nota');

            if (!inputProcessoEntrada.value) {
                alert("Campo Processo de Entrada da Nota é de preenchimento obrigatório.");
                return;
            }

            grupoContaEmpenho = Number(grupoContaEmpenho);
            if (grupoContaEmpenho === 7 && inputProcessoEntrada.value != 1) {

                mostrarValidacao = true;
                labelGrupo = 'DESPESAS COM serviço';
            }

            if (grupoContaEmpenho === 8 && inputProcessoEntrada.value != 2) {

                mostrarValidacao = true;
                labelGrupo = 'DESPESA COM MATERIAL (ALMOX)';
            }

            if (grupoContaEmpenho === 9 && inputProcessoEntrada.value != 3) {

                mostrarValidacao = true;
                labelGrupo = 'DESPESA EM MATERIAL PERMANENTE';
            }

            var optionSelecionado = inputProcessoEntrada.options[inputProcessoEntrada.value].innerHTML;
            var mensagem = "O desdobramento do empenho constitui " + labelGrupo + " e Você selecionou " + optionSelecionado;
            mensagem += " como Processo de Entrada da Nota.\n\nConfirma a entrada dos itens selecionados no estoque?";
            if (mostrarValidacao && !confirm(mensagem)) {
                return false;
            }

            if (!mostrarValidacao && !confirm('Confirma a entrada dos Itens Selecionados no estoque?')) {
                return false;
            }
        ...
        for (var i = 0; i < aItens.length; i++) {
            ...
            aItensSelecionados.push({
                "iCodLanc": aItens[i].value,
                'iIndiceEntrada': iIndice[1]
            });
        }
        if (iSemPai) {
            var sMsg = "Há Itens que foram fracionados que  nao estáo selecionados.\n";
        ...
        var oParametro = {
            ...
            'e69_numero' : $F('e69_numero'),
            'e69_serienota' : $F('e69_serienota'),
            'e69_subserienota' : $F('e69_subserienota'),
            'numprocessolicit' : $F('numprocessolicit')
        };

        if (isPB) {
            let outrosDados = {}
            oParametro.outrosDadosNota = JSON.stringify({
        ...
        if ($('processo_entrada_nota')) {
            oParametro.processoEntradaNota = $F('processo_entrada_nota');
        }
    ...
    /**
     * Função de retorno da requisição da confirmação da entrada da ordem de compra
     * no estoque
     */
    function js_retorndoConfirmaEntrada(oRetorno) {
        ...
        if (oRetorno.status == 1) {
            ...
            var nuinst = $F('nuinst');
            var e69_numero = $F('e69_numero');
            var e69_dtnota = $F('e69_dtnota');
            var e69_dtrecebe = $F('e69_dtrecebe');
            var e69_dtvencimento = $F('e69_dtvencimento');
            var e70_valor = $F('e70_valor');
            var m51_codordem = $F('m51_codordem');
            var m51_numcgm = $F('m51_numcgm');
            var pa = $F('e04_numeroprocesso');
            var obs =  '.'; //$F('m53_obs');
    
            var query = 'nuinst='+nuinst+'&e69numero='+e69_numero+'&e69dtnota='+e69_dtnota+'&e69dtrecebe='+e69_dtrecebe+'&e69dtvencimento='+e69_dtvencimento+'&e70valor='+e70_valor+'&m51codordem='+m51_codordem+'&m51numcgm='+m51_numcgm+'&pa='+pa+'&obs='+obs;
            //var query = 'nuinst='+nuinst+'&e69numero='+e69_numero+'&e69dtnota='+e69_dtnota+'&e69dtrecebe='+e69_dtrecebe+'&e69dtvencimento='+e69_dtvencimento+'&e70valor='+e70_valor+'&m51codordem='+m51_codordem+'&m51numcgm='+m51_numcgm+'&pa='+pa;


            jan = window.open('rel_entradanota.php?'+query,'','width='+(screen.availWidth-5)+',height='+(screen.availHeight-40)+',scrollbars=1,location=0 ');
            jan.moveTo(0,0);
    ...
    function js_escolherMater() {
        if (oItemAtivo) {
            ...
            var tipoServico = oItemAtivo.pc01_servico;
            const titulo_func = tipoServico == 't' ? 'Escolher Serviço' : 'Escolher Material';
            js_OpenJanelaIframe('CurrentWindow.corpo', 'db_iframe_pcmatmater',
                'func_matmaterentoc.php?codpcmater=' + iCodpcMater +
                '&lServico=' + tipoServico + '&funcao_js=js_mostramatmater|m60_codmater&lLotes=1',
                titulo_func, true);
        }
    }

    function js_setAjuda(elemento, lShow) {
        ...
        if (lShow) {
            ...
            $('ajudaItem').innerHTML = elemento.innerHTML;
    ...
    function js_adicionaCentroCusto() {
        ...
        sUrl += '&iCodigoDepto=<?php echo db_getsession("DB_coddepto") ?>';
        if ($F('matmater')) {
