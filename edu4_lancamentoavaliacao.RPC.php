...
$maiorPermissao = 0;
$temVinculoRH = false;
while ($codRecHumano = pg_fetch_assoc($rs)) {
    $temVinculoRH = true;
    $profissional = new ProfissionalEscola($codRecHumano['codigo']);
...
if ($oUsuarioSistema->isAdministrador()) {
    $maiorPermissao = 1;
}
...
try {
    ...
    switch ($oParam->exec) {
        ...
        case 'getDadosDiario':
            $codus = db_getsession("DB_id_usuario");
            EducacaoSessionManager::limpar();

            $aCalendarios = array();
            $aTurmas = array();

            if ( isset($oParam->lValidarProfessorLogado) && $oParam->lValidarProfessorLogado && $lProfessorLogado && $codus != 1392 ) {
            ...
            } else if (!$lProfessorLogado && $maiorPermissao > 0 || !$temVinculoRH){
                $aTurmas = TurmaRepository::getTurmaByEscola($iEscola);
            }*/
            ...
            foreach ($aTurmas as $oTurma) {
                ...
                /*if($oTurma->getCodigo() != 4198 && $oTurma->getCodigo() != 4201){
                    continue;
                }*/
                //echo $oTurma->getCodigo(); echo "<br>";
                ...
                /**
                 * Validações quando NÃO for para filtrar turmas de Progressão Parcial
                 * Devemos trazer todas turmas que não são turmas de progressão parcial
                 * Obs.: Tipo da turma = 6 (Turma de progressão)
                 */
                ...
                /**
                 * Pesquisamos se já nao existe o calendario no array.
                 */
                ...
                foreach ($oTurma->getEtapas() as $oEtapaTurma) {
                    ...
                    if (!$oParam->lProgressaoParcial && count($oTurma->getAlunosMatriculadosNaTurmaPorSerie($oEtapaTurma)) == 0) {                        
                        $lTemTurma = true;
                        //4201 entra aqui //erro aqui. turma 4201 volta vazio
                    }
                    ...
                    if (!$lTemTurma) {
                        //echo "4"; echo $oTurma->getCodigo() . " - " .$lTemTurma; echo "<br>";
                        //4201 não entra aqui
            ...
            foreach ($aCalendarios as $calendario) {
                ksort($calendario->aEnsinos);
                $calendario->aEnsinos = array_values($calendario->aEnsinos);
            }
            

            $oRetorno->dados = $aCalendarios;
            //die("Antes do retorno");
            break;
        ...
        case 'periodosAvaliacao':
            ...
            if (count($oRetorno->dados) == 0) {
                throw new BusinessException("Não há periodos de avaliacao");
            }
        ...
        case 'buscaDisciplinasTurma':
             ...
             * caso nao existir, lançar exception
             */
        ...
        case 'getParecer':
             ...
             * Data: 2025
             * Correção: Se o parecer estiver vazio ou for uma nota/conceito (não um parecer descritivo),
             * deve retornar string vazia, não uma nota.
             *
             * Validação:
             * - Se vazio ? retorna vazio
             * - Se numérico (ex: "5.0", "7.5") ? retorna vazio (é nota)
             * - Se conceito curto (ex: "A", "B", "R", "PD") ? retorna vazio (é conceito)
             * - Se texto muito curto (até 3 caracteres) ? pode ser conceito, retorna vazio
             * - Caso contrário ? mantém como parecer descritivo
             */
            $sParecer = trim($oParecer->sParecer);
            if (empty($sParecer) ||
                is_numeric($sParecer) ||
                strlen($sParecer) <= 3 ||
                in_array(strtoupper($sParecer), array('A', 'B', 'C', 'D', 'R', 'PD', 'AMP', 'REC'))) {
                $sParecer = '';
            }
        ...
        case 'salvarProgressaoAluno':
            ...
            if (!isset($oParam->aProgressoes)) {
                throw new  ParameterException('Progressões não informadas.');
            }
...
/**
 * Função que verifica se a turma possui ao menos uma das disciplinas setadas pelo usuário
 * @param Turma $oTurma
 * @return boolean
 */
