...
if ($opcao == 'analitico'){
	...
	for($i=0;$i<$numlinhas;$i++){
	    ...
	    if ($descrrua != $j14_nome && $grupo == "r"){
	    	...
	    	$pdf->SetFont('Arial','B',10);
	    	if($totruas != 0){
	    		$pdf->MultiCell(275,5,"Total de ".$totruas." contribuinte".($totruas > 1?"s":"")." para a rua : ".$descrrua,0,1,"R",0);
	    		$totruas = 0;
	    	}

	    	$pdf->Cell(280,2,"",0,1,"C",0);
          $pdf->MultiCell(275,5,$j14_codigo." - ".$j14_nome,"B","L", 1);
	      $pdf->Cell(280,2,"",0,1,"C",0);
	      $descrrua = $j14_nome;
	   ...
	   // SE FOR POR ATIVIDADE
	    	...
	    	$pdf->SetFont('Arial','B',10);
	    	if($totativ != 0){
	    		$pdf->MultiCell(275,5,"Total de ".$totativ." contribuinte".($totativ > 1?"s":"")." para a atividade : ".$descrativ,0,1,"R",0);
	    		$totativ = 0;
	    	}

	    	$pdf->Cell(280,2,"",0,1,"C",0);
          $pdf->MultiCell(275,5,$q03_ativ." - ".$q03_descr,"B","L", 1);
	      $pdf->Cell(280,2,"",0,1,"C",0);
	      $descrativ = $q03_descr;
	     // SE FOR POR CLASSE
	    	...
	    	$pdf->SetFont('Arial','B',10);
	    	if($totclasse != 0){
	    		$pdf->MultiCell(275,5,"Total de ".$totclasse." contribuinte".($totclasse > 1?"s":"")." para a classe : ".$descrclasse,0,1,"R",0);
	    		$totclasse = 0;
	    	}
	    	$pdf->Cell(280,2,"",0,1,"C",0);
          $pdf->MultiCell(275,5,$q12_classe." - ".$q12_descr,"B","L", 1);
	      $pdf->Cell(280,2,"",0,1,"C",0);
	      $descrclasse = $q12_descr;
	    	...
	    	$pdf->SetFont('Arial','B',10);
	    	if($totinscr != 0){
	    		$pdf->MultiCell(275,5,"Total de ".$totinscr." contribuinte".($totinscr > 1?"s":"")." para a inscrição : ".$descrinscr,0,1,"R",0);
	    		$totinscr = 0;
	    	}
	    	$pdf->Cell(280,2,"",0,1,"C",0);
          $pdf->MultiCell(275,5,$q02_inscr." - ".$z01_nome,"B","L", 1);
	      $pdf->Cell(280,2,"",0,1,"C",0);
     ...
     //    $pdf->Cell(35,05,$numCompl,0,1,"L",1);

        $currentY = $pdf->GetY();
	    $currentX = $pdf->GetX();

		$pdf->MultiCell(20,05,$q02_inscr, 0, 'L', 1);
		$currentX += 20;
		$pdf->SetY($pdf->getY() - 5);
		$pdf->SetX($currentX);
	    $pdf->MultiCell(40,05,db_cgccpf($z01_cgccpf), 0,'L', 1);
		$currentX += 40;
	    $pdf->SetY($pdf->getY() - 5);
	    $pdf->SetX($currentX);
        ...
        $pdf->Cell(35,05,db_formatar($q07_datafi,'d'),0,1,"L",1);
      		...
      		$numCompl = $q02_numero."/".$q02_compl;
				   ...
				   /* PARA INSCRICAO */
			// $pdf->MultiCell(20,30,$q12_classe);
			// $pdf->MultiCell(35,30,db_cgccpf($z01_cgccpf));
			// $pdf->MultiCell(50,30,(strlen($q12_descr) > 55?substr($q12_descr,0,55)."...":$q12_descr));
			// $pdf->MultiCell(50,30,(strlen($q03_ativ."-".$q03_descr) > 54?substr($q03_ativ."-".$q03_descr,0,54)."...":$q03_ativ."-".$q03_descr));
			// $pdf->MultiCell(45,30,(strlen($j14_nome) > 44?substr($j14_nome,0,44)."...":$j14_nome));
			// $pdf->MultiCell(25,30,$numCompl);
			// $pdf->MultiCell(25,30,db_formatar($q07_datain,'d'));
			// $pdf->MultiCell(25,30,db_formatar($q07_datafi,'d'));
	...
	if ($grupo == "r"){
    	...
    	$pdf->SetFont('Arial','B',10);
    	if($totruas != 0){
    		$pdf->MultiCell(275,5,"Total de ".$totruas." contribuintes para a rua : ".$descrrua,0,1,"R",0);
    		$totruas = 0;
    	}
  } else if ($grupo == "a"){
    ...
    if($totativ != 0){
    	...
    	$pdf->MultiCell(275,5,"Total de ".$totativ." contribuintes para a atividade : ".$descrativ,0,1,"R",0);
    	$totativ = 0;
    }
  } else if ($grupo == "c"){
    	//q12_classe, q12_descr
    	...
    	$pdf->SetFont('Arial','B',10);
    	if($totclasse != 0){
    		$pdf->MultiCell(275,5,"Total de ".$totclasse." contribuintes para a classe : ".$descrclasse,0,1,"R",0);
    		$totclasse = 0;
    	}
  }
