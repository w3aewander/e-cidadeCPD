...
class cl_lab_resultado
{
    ...
    public $la52_i_codigo = 0;
    public $la52_i_requiitem = 0;
    public $la52_i_usuario = 0;
    public $la52_c_hora = null;
    public $la52_d_data_dia = null;
    public $la52_d_data_mes = null;
    public $la52_d_data_ano = null;
    public $la52_d_data = null;
    public $la52_t_motivo = null;
    public $la52_diagnostico = null;
    ...
    public function incluir($la52_i_codigo)
    {
        $this->atualizacampos();
    ...
    public function alterar($la52_i_codigo = null)
    {
        $this->atualizacampos();
    ...
    public function excluir($la52_i_codigo = null, $dbwhere = null)
    {
    ...
    // funcao do sql
    public function sql_query($la52_i_codigo = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
    ...
    public function sql_query_item($la52_i_codigo = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
            $campos_sql = explode("#", $campos);
    ...
    // funcao do sql
    public function sql_query_file($la52_i_codigo = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select ";
        if ($campos != "*") {
            $campos_sql = explode("#", $campos);
    ...
    public function sql_query_resultado_completo($la52_i_codigo, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $where = " la52_i_codigo = {$la52_i_codigo} ";
        ...
        return <<<SQL
            WITH RECURSIVE atributos_recursivo AS (
                ...
                WHERE la42_i_exame = (SELECT la09_i_exame FROM exame)
                UNION ALL
                ...
                INNER JOIN lab_atributo ON lab_atributo.la25_i_codigo = lab_exameatributoligacao.la26_i_exameatributofilho
                WHERE NOT EXISTS (
                    SELECT 1
                    FROM lab_examedisp
                    INNER JOIN lab_exameatributo ON lab_exameatributo.la42_i_codigo = lab_examedisp.la50_i_exameatributo
                    WHERE lab_examedisp.la50_i_atributo = lab_atributo.la25_i_codigo
                       AND lab_exameatributo.la42_i_exame = (SELECT la09_i_exame FROM exame)
                )
            ), exame AS (
                SELECT la09_i_exame
                FROM lab_setorexame
                INNER JOIN lab_requiitem ON lab_requiitem.la21_i_setorexame = lab_setorexame.la09_i_codigo
                INNER JOIN lab_resultado ON lab_resultado.la52_i_requiitem = lab_requiitem.la21_i_codigo
                {$where}
            ), resultado AS (
                SELECT *
                FROM atributos_recursivo
                LEFT JOIN lab_valorreferencia ON lab_valorreferencia.la27_i_atributo = atributos_recursivo.codigo
                LEFT JOIN lab_resultadoitem ON lab_resultadoitem.la39_i_atributo = atributos_recursivo.codigo
                    AND lab_resultadoitem.la39_i_resultado = {$la52_i_codigo}
                LEFT JOIN lab_resultadonum ON lab_resultadonum.la41_i_result = lab_resultadoitem.la39_i_codigo
                LEFT JOIN lab_resultadoalfa ON lab_resultadoalfa.la40_i_result = lab_resultadoitem.la39_i_codigo
            ) SELECT {$campos} FROM resultado {$ordem};
SQL;
    }
}
