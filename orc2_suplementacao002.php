...
$pdf->init(false);
$pdf->addTitulo("RELATORIO DE SUPLEMENTAÇÔES");
$pdf->addTitulo("MODELO : $txt_modelo");
$pdf->addTitulo("EXERCÍCIO: " . $anousu);
$pdf->addTitulo("PERÍODO: " . db_formatar($data_ini, 'd') . " a " . db_formatar($data_fim, 'd'));
...
for ($tiporel = 0; $tiporel <= 1; $tiporel++) {
    ...
    /**
     * comentado o join com a origemcomplementorecurso pois esse relatorio não necessita olhar LOA e complemento
     * a rotina está em refatoração nesta parte de LOA e Complemento
     */
 ...
 select distinct
  ...
  inner join complementofonterecurso on o15_complemento = o200_sequencial
  /*inner join origemcomplementorecurso on o206_recurso = o15_codigo*/
...
";
    ...
    $sSqlDotacoesPPA .= "                                      and o08_ano      = o56_anousu
  							inner join orctiporec on o08_recurso = o15_codigo
							inner join complementofonterecurso on o15_complemento = o200_sequencial
							/*inner join origemcomplementorecurso on o206_recurso = o15_codigo*/";
...
//      db_redireciona('db_erros.php?db_erro=Sem projetos neste periodo&fechar=true');
    ...
    for ($i = 0; $i < $rows; $i++) {
        ...
        if ($codsup != $o47_codsup) {
            ...
            $ress = db_query(analiseQueryPlanoOrcamento($sSqlReceitas));
            //$codsup_reduzido = 0;
        ...
        $fonte = descricaoCompletaRecurso($o58_codigo, $o47_anousu, 3);

        $pdf->Cell(20, $alt, "$fonte", 0, 0, "C", '0');
        $pdf->Cell(25, $alt, db_formatar($suplementado, 'f'), 0, 0, "R", '0');
        $pdf->Cell(25, $alt, db_formatar($reduzido, 'f'), 0, 1, "R", '0');
        $codsup_suplementado += (float)$suplementado;
        $codsup_reduzido += (float)$reduzido;
        $codsup_receita += (float)$receita;
        $tot_sup += (float)$suplementado;
        $tot_red += (float)$reduzido;
        if ($tiporel == 0 && $o39_usalimite == 't') {
            $nTotalUtilizado += (float)$suplementado;
        }
        ...
        $array_totais[$o48_descr][0] += (float)$suplementado;
        $array_totais[$o48_descr][1] += (float)$reduzido;
    }
...
if ($rows > 0) {
    ...
    $pdf->setX(3);
    $sql = "
        select  o85_codsup,o85_codrec,o85_anousu,o85_valor ,o57_descr
		  from orcsuplemrec
          join orcsuplem on o46_codsup = o85_codsup
		  join orcprojeto on o39_codproj = orcsuplem.o46_codlei
		  join orcsuplemretif on o48_retificado = orcprojeto.o39_codproj
		  join orcreceita on o70_anousu = o85_anousu
		       and o70_codrec = o85_codrec
		       and o70_instit in ($instits)
		  join orcfontes  on o57_anousu = o70_anousu and o57_codfon = o70_codfon
		 where o46_tiposup in ($vTipos) and o85_codsup = $codsup";
    $ress = db_query($sql);
