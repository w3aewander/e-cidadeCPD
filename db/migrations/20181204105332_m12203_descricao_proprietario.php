...
class M12203DescricaoProprietario extends PostgresMigration
{
    public function up() {
        $sql = "
...
SELECT x.z01_numcgm,
   x.j01_matric,
   x.z01_cgccpf,
   case when x.totpropri > 1 then case when x.totpropri = 2 then RTRIM(X.PROPRIETARIO) || ' E OUTRO' else RTRIM(X.PROPRIETARIO) || ' E OUTROS' end else X.PROPRIETARIO end,
   trim(substr(case when x.totpromi > 1 then case when x.totpropri = 2 then RTRIM(X.z01_nome) || ' E OUTRO' else RTRIM(X.z01_nome) || ' E OUTROS' end else X.Z01_NOME end,1,40))::varchar as z01_nome,
   trim(case when x.totpromi > 1 then case when x.totpropri = 2 then RTRIM(X.z01_nomecompleto) || ' E OUTRO' else RTRIM(X.z01_nomecompleto) || ' E OUTROS' end else X.z01_nomecompleto end)::varchar as z01_nomecompleto,
   case when length(trim(x.z01_ender)) = 0 and (length(trim(x.z01_cxpostal)) > 0 and to_number(x.z01_cxpostal,'999999') > 0 ) then 'CAIXA POSTAL: '||x.z01_cxpostal else x.z01_ender::varchar(80) end as z01_ender,
   x.z01_munic,
   x.z01_bairro,
   x.z01_cep,
