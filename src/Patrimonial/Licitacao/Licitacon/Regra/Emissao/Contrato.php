...
class Contrato extends BaseAbstract
{
    ...
    public function getDataVigencia($iCodigoAcordo, $sDataInicio, $sDataFim, $sDependeOrdemInicio, $iDiasPrazo)
    {
        ...
        if ($oDataVigencia->inicio == null) {
            $dataInicial = new \DBDate($sDataInicio);
            $oDataVigencia->inicio = $dataInicial->getDate(DBDate::DATA_PTBR);
        }

        if ($oDataVigencia->fim == null) {
            $dataFinal = new \DBDate($sDataFim);
            $oDataVigencia->fim = $dataFinal->getDate(DBDate::DATA_PTBR);
        }
    ...
    public function getDadosDaLicitacaoDoContrato($iContrato)
    {
        ...
        $sWhere = "ac16_sequencial = {$iContrato}";
        $sSqlAcordo = $oDaoAcordoItem->sql_query_numero_licitacao($sCampos, null, $sWhere);
        $sSqlAcordo = "select * from ({$sSqlAcordo}) as x where numero is not null limit 1";
        ...
        if (empty($oDados->numero)) {
            ...
            $sSqlLicitacao = $oDaoAcordoItem->sql_query_numero_licitacao_processo_compras(
                $sCampos,
                null,
