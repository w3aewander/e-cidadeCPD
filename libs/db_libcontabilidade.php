...
class cl_desdobramento
{
    ...
    inner join orcelemento ele on ele.o56_codele=conlancamele.c67_codele
...
class cl_receita_saldo_mes_complemento
{
    ...
    ";
        ...
        $this->sql .= " GROUP BY complemento_lancamento, complemento_recurso, O70_ANOUSU, O70_CODREC,o70_instit,O71_MES,O57_FONTE,O57_DESCR, O70_VALOR
    ) AS X

    group by complemento_lancamento, complemento_recurso, O70_ANOUSU, O70_CODREC, o70_instit, O71_MES,ARRECADADO,o57_fonte, o57_descr, O70_VALOR, ADICIONAL

    ";

        $this->sql .= ") AS Y
    GROUP BY complemento_lancamento,O70_ANOUSU,O70_CODREC,o70_instit,O57_FONTE,O57_DESCR,O70_VALOR,
    prev_jan,
    prev_fev,
    prev_mar,
    prev_abr,
    ...
    function sql_record($returSql = false)
    {
        global $complemento_lancamento,$o70_anousu, $o70_codrec, $o57_fonte, $o57_descr, $janeiro, $fevereiro, $marco, $abril, $maio, $junho, $julho, $agosto, $setembro, $outubro, $novembro, $dezembro, $o70_valor, $adicional;
        ...
        for ($i = 0; $i < pg_num_rows($result); $i++) {
          ...
          where o70_codrec = {$o70_codrec}");
            ...
            for ($ii = 1; $ii < 11; $ii++) {
                ...
                $sSql = "select o57_descr from work_plano where o57_fonte = '$estrutural'";
                $result_estrut = db_query($sSql);
                if (pg_num_rows($result_estrut) == 0) {
                    ...
                    if (pg_num_rows($result_estrut) <= 0) {
                        echo "Conta não encontrada nas fontes de Receita Comando:<br>"
                            . "select o57_descr from orcfontes where o57_anousu = " . $this->anousu
                            . " and o57_fonte = '$estrutural'";

                           // db_criatabela(db_query("select * from work_plano"));
                           // echo "<br>$sSql<br>";
                    ...
                    $sInsert = "insert into work_plano values( 0,
          " . $this->anousu
                        . ",
          0,
          0,
        ...
        //exit;
        $sql = "
    SELECT complemento_lancamento,O70_ANOUSU,O70_CODREC,o70_instit,O57_FONTE,O57_DESCR,round(O70_VALOR,2) AS o70_valor,
...
class cl_translan extends cl_contranslan
{
     ...
     * @var integer|null
     */
    private $instituicaoLancamento = null;
   ...
   */
    ...
    function db_trans_estorna_empenho($codcom = null, $anousu, $iCodigoDocumento = null, $iNumeroEmpenho = null)
    {
        ...
        if ($iCodigoDocumento == 413) {
            ...
            if (pg_num_rows($rsBuscaLancamentoInclusao) > 0) {
                $oStdContas = db_utils::fieldsMemory($rsBuscaLancamentoInclusao, 0);
                $this->arr_credito[$cont] = !empty($oStdContas->c69_debito) ? $oStdContas->c69_debito : 0;
                $this->arr_debito[$cont] = !empty($oStdContas->c69_credito) ? $oStdContas->c69_credito : 0;
                $this->arr_histori[$cont] = !empty($oStdContas->c69_codhist) ? $oStdContas->c69_codhist : 0;
        ...
        for ($i = 0; $i < $this->numrows; $i++) {
            db_fieldsmemory($this->result, $i);
            $stdDados = db_utils::fieldsMemory($this->result, $i);
            if ($cont == 1 && $stdDados->c46_ordem == 1) {
                continue;
            }
   ...
   */
    ...
    function db_trans_estorna_pagamento_prestacao_contas(
        $codele,
        $reduzido,
        $anousu,
        $iCodigoDocumento,
   ...
   */
    ...
    function db_trans_estorna_liquida($codcom, $codele, $anousu, $iCodigoDocumento = null)
    {
        ...
        for ($i = 0; $i < $this->numrows; $i++) {
            ...
            if (($c47_ref == '' || $c47_ref == 0 || ($c47_ref != 0 && ($c47_ref == $codcom || $c47_compara == 3)))
                && (($c47_compara == 0 || $comparador == $codele) || $c47_compara == 11)) {
                //------------------------------------------------------------------------
                //verificação para naum incluir duas vezes o mesmo seqtranslan
                if (array_key_exists($c46_seqtranslan, $arr_lans)) {
                    continue;
                ...
                /*
                 * Verifica se existe regra de comparação 'Crédito/Elemento'.
                 * Verifica se é o 1º lançamento, pois essa regra se aplica apenas a este.
                 * Se verdadeiro, seta as contas crédito e débito conforme configuração da transação.
                 */
                if($i === 0 && $c47_compara == 11) {

                    $contaOrcamento = ContaOrcamentoRepository::getContaByCodigo($codigoContaOrcamento, $anousu);
                    $estruturalContaOrcamento = $contaOrcamento->getEstrutural();

                    $daoTransacao = new cl_contranslr();
                    $sWhere  = "     c45_coddoc      = {$this->coddoc}";
                    $sWhere .= " and c45_anousu      = {$anousu}";
                    $sWhere .= " and c46_seqtranslan = {$c46_seqtranslan}";
                    $sWhere .= " and c47_instit = " . db_getsession("DB_instit");
                    $sqlTransacao = $daoTransacao->sql_query(null, "*", "c114_elemento desc", $sWhere);
                    $rsTransacao   = $daoTransacao->sql_record($sqlTransacao);

                    $estruturalTransacao = null;
                    for($linhaRegra = 0; $linhaRegra < $daoTransacao->numrows; $linhaRegra++) {

                        $linhaTransacao = db_utils::fieldsMemory($rsTransacao, $linhaRegra);
                        $estruturalTransacaoAtual = $linhaTransacao->c114_elemento;

                        if($estruturalContaOrcamento >= $estruturalTransacaoAtual) {

                            if($estruturalTransacaoAtual > $estruturalTransacao) {
                                $estruturalTransacao = $estruturalTransacaoAtual;
                                $c47_credito = $codele;
                                $c47_debito = $linhaTransacao->c47_debito;
                            }
                        }
                    }
                }
   ...
   */
        ...
        if (USE_PCASP) {
            ...
            if ($oGrupoOrcamento) {
                ...
                if (in_array($oGrupoOrcamento->getCodigo(), array(8, 9))) {
                    ...
                    if (!empenho::possuiLancamentoDeControle($iNumEmp, db_getsession('DB_anousu'),
                        array($aDocumentosEvento['em_liquidacao']))) {
                        /**
                        ...
                        if (empenho::possuiLancamentoDeControle($iNumEmp, db_getsession('DB_anousu'),
                            array($aDocumentosEventoGrupo8['em_liquidacao']))) {
    ...
    function db_trans_pagamentoApropriacaoEstorno($codele, $reduzido, $anousu, $iNumEmp = null, $iCodigoDocumento = null, $iCodOrd, $iLancamento = null)
    {

        $this->result = $this->getRegrasTransacao($iCodigoDocumento, $anousu, db_getsession('DB_anousu'));

        $oEmpenho = new EmpenhoFinanceiro($iNumEmp);

        $aNotas = $oEmpenho->getNotasDeLiquidacao();

        $iContador = 0;

        $aDocsEstorno = array( 6011);


        foreach ($aNotas as $oNotaLiquidacao) {

           $aRetencoes = $oNotaLiquidacao->getRetencoes(null, $iCodigoDocumento, $iLancamento);

           $iOrdemNota = $oNotaLiquidacao->getCodigoOrdemPagamento();

           if ($iOrdemNota != $iCodOrd ) {
           ...
           foreach ($aRetencoes as $oRetencao) {
              ...
              $iTipoCalcRetencao = $oRetencao->e21_retencaotipocalc;
              $iReduzidoRetencao = $oRetencao->k02_reduz;

              // percorre as regras do doc para encontra o compara com o tipocalc da retencao

              for ($i = 0; $i < pg_num_rows($this->result); $i++) {

                $oDadosRegra = db_utils::fieldsMemory($this->result, $i);

                $iCredito = $oDadosRegra->c47_credito;
                $iDebito = $oDadosRegra->c47_debito;
                
                switch($oDadosRegra->c46_ordem){

                    case "1" :
                        
                        if ($oDadosRegra->c47_debito != "0") {
                            

                         if ($iTipoCalcRetencao == $oDadosRegra->c47_ref  ){
                            
                            $this->arr_seqtranslr[$iContador] = $oDadosRegra->c47_seqtranslr;
                            $this->arr_credito[$iContador] = $iCredito;
                            $this->arr_debito[$iContador] = $iDebito;
                            $this->arr_histori[$iContador] = $oDadosRegra->c50_codhist;
                            $iContador++;
                             ...
                             if ($iTipoCalcRetencao == $oDadosRegra->c47_ref) {
                                ...
                                $this->arr_seqtranslr[$iContador] = $oDadosRegra->c47_seqtranslr;
                                $this->arr_credito[$iContador] = $iCredito;
                                $this->arr_debito[$iContador] = $iReduzidoRetencao;
                                $this->arr_histori[$iContador] = $oDadosRegra->c50_codhist;
                                $iContador++;
                            }
        ...
        $this->arr_credito = array_unique( $this->arr_credito );
        $this->arr_debito = array_unique($this->arr_debito);
    }




    function db_trans_pagamentoApropriacao($codele, $reduzido, $anousu, $iNumEmp = null, $iCodigoDocumento = null, $iCodOrd, $iLancamento = null)
    {
        ...
        $aDocsEstorno = array(6003, 6005, 6009, 6011);
        ...
        foreach ($aNotas as $oNotaLiquidacao) {
           ...
           foreach ($aRetencoes as $oRetencao) {
              ...
              for ($i = 0; $i < pg_num_rows($this->result); $i++) {
                ...
                if ( in_array( $iCodigoDocumento,  $aDocsEstorno) ) {

                    $iCredito = $oDadosRegra->c47_debito;
                    $iDebito = $oDadosRegra->c47_credito;
                }
                ...
                switch($oDadosRegra->c46_ordem){
                    ...
                    case "1" :
                     ...
                     //  dump($iTipoCalcRetencao, $oDadosRegra->c47_ref ,$iReduzidoRetencao, $oDadosRegra->c47_credito);
                     //  dump($oDadosRegra->c46_ordem, $oRetencao);
                     //  echo " TipoCalc:  $iTipoCalcRetencao ,  oDadosRegra->c47_ref: {$oDadosRegra->c47_ref} ||
                     //         iReduzidoRetencao:  $iReduzidoRetencao ,
                     //         oDadosRegra->c47_credito: $oDadosRegra->c47_credito <br>";
                         ...
                         if ($iTipoCalcRetencao == $oDadosRegra->c47_ref  ){
                            ...
                            $iContador++;
                             ...
                             // credito = a zero deverá buscar pelo elemento da regra
                             // o reduzido da retencao

                            if ($iTipoCalcRetencao == $oDadosRegra->c47_ref) {

                                $this->arr_seqtranslr[$iContador] = $oDadosRegra->c47_seqtranslr;
                                $this->arr_credito[$iContador] = $iReduzidoRetencao;
                                $this->arr_debito[$iContador] = $iDebito;
                                $this->arr_histori[$iContador] = $oDadosRegra->c50_codhist;
                                $iContador++;
                            }
     ...
     * @throws Exception
     *
     * foi necessario buscar a liquidacao a partir do ano do empenho,
     ...
     */
    public static function getContaLiquidacao(
        $iNumeroEmpenho,
        array $aCodigoDocumento,
        $iAnoSessao = null,
        $lTipoConta = 1,
        $anoEmpenho = null
    ) {
        ...
        $resto = $oEmpenho->isRestoAPagar(db_getsession("DB_anousu"));
        if ($resto && empty($anoEmpenho) ) {
            $anoEmpenho = $oEmpenho->getAnouso();
        }

        if (UTILIZA_INCORPORACAO_BEM) {
            $aCodigoDocumento = array(3,23,33,39,84,202,204,206, 412);
            $iAnoSessao       = null;
        }

        $sCodigoDocumento = implode(",", $aCodigoDocumento);
        //$sCodigoDocumento .= ",204,35, 210";
        $sCodigoDocumento .= ",204,210";
   ...
   */
    ...
    function db_trans_estorna_pagamento($codele, $reduzido, $anousu, $iNumEmp, $iCodigoDocumento = null)
    {
        ...
        if ($oGrupoOrcamento && USE_PCASP) {
            ...
            if (in_array($oGrupoOrcamento->getCodigo(), array(8, 9))) {
                ...
                $coddoc_liq_capital = $aDocumentosEvento['em_liquidacao'];
                if (!empenho::possuiLancamentoDeControle($iNumEmp, db_getsession('DB_anousu'),
                    array($aDocumentosEvento['liquidacao']))) {
                    $coddoc_liq = 3;
                    $coddoc_liq_capital = 3;
                }
            }
        ...
        if (USE_PCASP /*&& (in_array($coddoc_liq, $aDocumentosConferencia) || in_array($coddoc_liq_capital, $aDocumentosConferencia))*/) {
             ...
             */
            $c47_seqtranslr = 0;
            $oEventoContabil = EventoContabilRepository::getEventoContabilByCodigo($coddoc_liq, $iAnoSessao,
                $iInstituicaoSessao);
            $aLancamentos = $oEventoContabil->getEventoContabilLancamento();
            $codhist = $aLancamentos[0]->getHistorico();
            ...
            $sSqlBuscaContaPagamento .= " order by c69_ordem, conlancamval.c69_sequen asc";
    ...
    function db_trans_pagamento_resto($codele, $reduz, $anousu, $numemp, $iCodDoc = 35)
    {
        ...
        for ($i = 0; $i < $this->numrows; $i++) {
            ...
            if ($c46_ordem == 1 && USE_PCASP && $this->coddoc != 35) {
                ...
                if ($this->coddoc == 37 && empenho::possuiLancamentoDeControle($numemp, $iAnoLancamento, array(210, 200, 208, 212, 214, 216))) {

                    $c47_debito = self::getContaLiquidacao($numemp, array(210, 200, 208, 212, 214, 216), $iAnoLancamento);

                } else {


                    if ($this->coddoc == 37 && empenho::possuiLancamentoDeControle($numemp, $iAnoLancamento, array(33))) {
                        ...
                        /**
                         * passar a condicao de maior ou igual o ano do empenho
                         */
                        $oEmpenho = new EmpenhoFinanceiro($numemp);
                        if ($this->coddoc == 37 && empenho::possuiLancamentoDeControleAnoEmpenho($numemp, $oEmpenho->getAno(), array(210, 200, 208, 212, 214, 216))) {

                            $c47_debito = self::getContaLiquidacao($numemp, array(210, 200, 208, 212, 214, 216), null, 1, $oEmpenho->getAno());
    ...
    function db_trans_estorna_pagamento_resto($codele, $reduz, $anousu, $numemp, $iCodDoc = 36)
    {
        ...
        for ($i = 0; $i < $this->numrows; $i++) {
            ...
            if ($c46_ordem == 1 && USE_PCASP && $this->coddoc != 36) {
                 ...
                 */
                if ($this->coddoc == 38 && empenho::possuiLancamentoDeControle($numemp, $iAnoLancamento,
                        array(212, 214))) {
                    $c47_credito = self::getContaLiquidacao($numemp, array(212, 214), $iAnoLancamento);
                } else {
                    if ($this->coddoc == 38 && empenho::possuiLancamentoDeControle($numemp, $iAnoLancamento - 1,
                            array(212, 214))) {
    ...
    function db_trans_arrecada_receita(
        $conta,
        $codcon,
        $anousu,
        $iCodigoDocumento = null,
        $iCodigoReceitaOrcamentaria = null,
        $iLancamento = null
    ) {
        ...
        $this->result = $this->getRegrasTransacao($this->coddoc, db_getsession("DB_anousu"),
        ...
        for ($i = 0; $i < $this->numrows; $i++) {
            ...
            if ($c47_compara == 0) {
                ...
                if ($iCodigoDocumento == 100 && $c46_ordem == 2 && $iLancamento != null) {

                    //para o doc 100 para executar o compara zero, deve ser somente para arrecadacao 4
                    // e nao deducao 9
                    $sSqlDeducao = "
                      ...
                      inner join orcfontes on o70_codfon = o57_codfon
                       ...
                       where c74_codlan = {$iLancamento}
                         and substring(o57_fonte, 1, 1) = '9' ";

                    $rsDeducao = db_query($sSqlDeducao);

                    if (pg_num_rows($rsDeducao) > 0 ) {
                        continue;
                    }
                }
                ...
                $this->arr_seqtranslr[$cont] = $c47_seqtranslr;
                $cont++;
            }  elseif ($c47_compara == 12){
                $sqlReceita = "select o70_concarpeculiar
                                 from orcreceita
                                where o70_codfon = {$iCodigoReceitaOrcamentaria}
                                  and o70_instit = ".db_getsession('DB_instit')."
                                  and o70_anousu = ".db_getsession('DB_anousu');
                $rsReceita  = db_query($sqlReceita);
                $caracteristicaPecurliar = db_utils::fieldsMemory($rsReceita,0)->o70_concarpeculiar;
                if ($c47_ref == $caracteristicaPecurliar) {
                    $this->arr_credito[$cont] = $c47_credito;
                    $this->arr_debito[$cont] = $c47_debito;
                    $this->arr_histori[$cont] = $c46_codhist;
                    $this->arr_seqtranslr[$cont] = $c47_seqtranslr;
            ...
            } elseif ($c47_compara == 15) {

                $this->arr_debito[$cont] = $c47_debito;
                $this->arr_credito[$cont] = $codcon;
                $this->arr_histori[$cont] = $c46_codhist;
                $this->arr_seqtranslr[$cont] = $c47_seqtranslr;
                $cont++;
    ...
    function db_trans_estorno_receita(
        $conta,
        $codcon,
        $anousu,
        $iCodigoDocumento = null,
        ...
        for ($i = 0; $i < $this->numrows; $i++) {
            ...
            if ($oEstorno->c47_compara == 0) {
                ...
                if ($iCodigoDocumento == 101 && $oEstorno->c46_ordem == 2 && $iLancamento != null) {

                    //para o doc 101 para executar o compara zero, deve ser somente para arrecadacao 4
                    // e nao deducao 9
                    $sSqlDeducao = "
                ...
                $this->arr_seqtranslr[$cont] = $oEstorno->c47_seqtranslr;
                $cont++;
            } elseif ($oEstorno->c47_compara == 12){
                ...
                if ($oEstorno->c47_ref == $caracteristicaPecurliar) {
   ...
   */
     ...
     */
        ...
        if ($tipo == 1050) {
            $sDocSuplementacao = "o48_arrecadmaior";
        }
    ...
    where o48_tiposup = $tipo";
        ...
        if ($red == false) {
            ...
            $this->coddoc = $doc_reducao;
        } // reduzido ou arrecadação a maior ( parte que informa receita )
        // ------------ ------------ --------------
        $this->result = $this->getRegrasTransacao($this->coddoc, db_getsession("DB_anousu"));
        $cont = 0;
        for ($i = 0; $i < $this->numrows; $i++) {
    ...
    {
        $sWhere   = array(" c45_coddoc = $iCodigoDocumento ");
        ...
        $novoWhere = implode(' ', $sWhere);
        $this->sql = $this->sql_query_lr(null, "*", 'c46_ordem', $novoWhere);

        /**
         * Verifica se existe transação cadastrada para a instituição da sessão, se não existir pega da prefeitura.
         */
        $verificaCadastroTransacao = db_query($this->sql);
        if (pg_num_rows($verificaCadastroTransacao) === 0) {

            $instituicaoPrefeitura = InstituicaoRepository::getInstituicaoPrefeitura();
            unset($sWhere[1], $sWhere[2]);
            $sWhere[] = " and c45_instit = {$instituicaoPrefeitura->getCodigo()} ";
            $sWhere[] = " and c47_instit = {$instituicaoPrefeitura->getCodigo()} ";
            $novoWhere = implode(' ', $sWhere);
            $this->sql = $this->sql_query_lr(null, "*", 'c46_ordem', $novoWhere, $instituicaoPrefeitura->getCodigo());
        }
     ...
     */
    public static function getContaParaPagamento($iNumEmp, $anousu)
    {

        $coddoc_liq = 3;
        $coddoc_liq_capital = 23;
        $oEmpenhoFinanceiro = new EmpenhoFinanceiro($iNumEmp);
        $iDesdobramentoEmpenho = $oEmpenhoFinanceiro->getDesdobramentoEmpenho();
        if (USE_PCASP) {
            ...
            if ($oGrupoOrcamento) {
                ...
                if (in_array($oGrupoOrcamento->getCodigo(), array(8, 9))) {
                    ...
                    if (!empenho::possuiLancamentoDeControle($iNumEmp, $anousu, array($aDocumentosEvento['em_liquidacao']))) {
                        /**
                         * Verifica se foi executado lançamentos de controle do grupo 8 - no caso de dispensa de tombamento
                         */
                        $aDocumentosEventoGrupo8 = DocumentoContabil::getDocumentosPorGrupo(8);
            ...
            if ($oGrupoOrcamento) {
                ...
                switch($oGrupoOrcamento->getCodigo()){

                  case "7":

                      $coddoc_liq          = "202, 212, 216";
                      $coddoc_liq_capital  = "202, 212, 216";
                      break;

                  case "8":

                      $coddoc_liq          = "204, 212";
                      $coddoc_liq_capital  = "210, 212";
                      break;
        ...
        return self::getContaLiquidacao($iNumEmp, array($coddoc_liq_capital, $coddoc_liq, 84, 3, 23, 33), $anousu);
    }
        ...
        if ($this->mascara == true && $this->input == false) {
            ...
            <tr>
                ...
                <td>
                    ...
                    <input name="mascara_<?= $picture ?>" readonly disabled size='<?= $this->size ?>' type="text"
                           value="<?= $mascara ?>">
    ...
    if ($this->input == false) {
        ...
        <tr>
            ...
            <td>
                ...
                if ($this->botao == true) {
                    ?>
                    <input name='verifica' type="button" value='Verificar'
                           onclick="js_mascara02_<?= $picture ?>(document;.<?= $this
    ...
    for ($i = 0; $i < pg_num_rows($result); $i++) {
      ...
      '$sinal_final')");
        ...
        for ($ii = 1; $ii < 11; $ii++) {
            ...
            if (@pg_num_rows($result_estrut) != true) {
...
select estrut_mae,
  ...
  from (select p.c60_estrut as estrut_mae,
               ...
               p.c60_finali,
               r.c61_instit,
               fc_planosaldonovo_array($anousu, c61_reduz, '$dataini', '$datafim', $encerramento),
    ...
    for ($i = 0; $i < pg_num_rows($result); $i++) {
        ...
        if ($key === false) { // não achou
            ...
            $work_plano[$seq] = array(
                ...
                10 => "$sinal_anterior",
                11 => "$sinal_final",
                12 => "$c60_identificadorfinanceiro",
                13 => "$c60_consistemaconta"
            );
            $seq = $seq + 1;
        } else {
        ...
        for ($ii = 1; $ii < 10; $ii++) {
            ...
            where c60_anousu=" . $anousu . " and c60_estrut = '$estrutural'");
                ...
                if (count($aOrcParametro) > 0) { // Se foram passados parametros...
                    ...
                    $work_plano[$key][6] = dbround_php_52($work_plano[$key][6], 2) + dbround_php_52($tot_anterior, 2);
                    $work_plano[$key][7] = dbround_php_52($work_plano[$key][7],
                            2) + dbround_php_52($tot_anterior_debito, 2);
                    $work_plano[$key][8] = dbround_php_52($work_plano[$key][8],
                            2) + dbround_php_52($tot_anterior_credito, 2);
                    $work_plano[$key][9] = dbround_php_52($work_plano[$key][9], 2) + dbround_php_52($tot_saldo_final,
                            2);
                } else {
                    $work_plano[$key][6] = dbround_php_52($work_plano[$key][6], 2) + dbround_php_52($tot_anterior, 2);
                    $work_plano[$key][7] = dbround_php_52($work_plano[$key][7],
...
function calcula_rcl2($anousu, $dtini, $dtfin, $db_selinstit, $matriz = false, $codrel = 5, $data = 0, $extendida = false, $versao = '')
{
    require_once(modification("libs/db_utils.php"));
    ...
    if ($codrel == 59 || $codrel == 81) {
        for ($iLinha = 1; $iLinha <= 22; $iLinha++) {
            $param[$iLinha] = new linhaRelatorioContabil($codrel, $iLinha, $anousu);
            $param[$iLinha]->parametro = $param[$iLinha]->getParametros($anousu);
        }
    }

    $modeloRelatorio = $opcaoRelatorio = Opcao::get('modelo_rreo_anexo3',db_getsession("DB_anousu"));
    if (!empty($versao)) {
        $modeloRelatorio = $versao;
    }
    $versaoIN13 = $modeloRelatorio == 'in13';
    if ($codrel == 178) {
        $oDaoOrcParamSeq = new cl_orcparamseq();
        $sSqlParamSeq = $oDaoOrcParamSeq->sql_query_file("178", null, "count(*)");
        $rsParamSeq = db_query($sSqlParamSeq);
        $iTotalLinhas = 0;

        if ($rsParamSeq) {
            $iTotalLinhas = db_utils::fieldsMemory($rsParamSeq, 0, true)->count;
        }
        $iTotalLinhas = $modeloRelatorio == 'mdf' ? 23 : $iTotalLinhas;
        /**
         * caso a matriz nao seja emitida extendida, devemos ignorar o ultimo parametro caso modelo in13 e extendida = false
         */
        if ($versaoIN13 && !$extendida) {
            $iTotalLinhas = 31;
        }
        $iContador = 1;
        for ($iLinha = 1; $iLinha <= $iTotalLinhas; $iLinha++) {
            $param[$iContador] = new linhaRelatorioContabil($codrel, $iLinha);
            $param[$iContador]->parametro = $param[$iContador]->getParametros($anousu);
            $iContador++;
        }
    }
    ...
    foreach (range(1, 12) as $mes) {
      $totalPerdaFundeb[$mes] = 0;
    }
    $transferenciaParlamentar = [];
    foreach (range(1, 12) as $mes) {
        $transferenciaParlamentar[$mes] = 0;
    }
    for ($p = 1; $p <= $tot_param; $p++) {
        // 18 para 2007 e 21 para 2008 é a quantidade de parametros ou linhas existentes nos parametros

        /**
         * Verificamos quais linhas devem ser impressas conforme cada modelo.
         */
        if (!$versaoIN13 && $p > 23) {
            continue;
        }
        if ($versaoIN13 && ($p >= 21 && $p <= 23)) {
            continue;
        }
            ...
            foreach ($oParametro->contas as $oEstrutural) {
                ...
                if ($oRetornoVerificacao->match) {
                    ...
                    if ($p <= $ult_param) {
                        ...
                        // SOMA AS DEDUÇÕES
                        if ($p == 31) {
                            $totalPerdaFundeb[1] += $oReceita->janeiro;
                            $totalPerdaFundeb[2] += $oReceita->fevereiro;
                            $totalPerdaFundeb[3] += $oReceita->marco;
                            $totalPerdaFundeb[4] += $oReceita->abril;
                            $totalPerdaFundeb[5] += $oReceita->maio;
                            $totalPerdaFundeb[6] += $oReceita->junho;
                            $totalPerdaFundeb[7] += $oReceita->julho;
                            $totalPerdaFundeb[8] += $oReceita->agosto;
                            $totalPerdaFundeb[9] += $oReceita->setembro;
                            $totalPerdaFundeb[10] += $oReceita->outubro;
                            $totalPerdaFundeb[11] += $oReceita->novembro;
                            $totalPerdaFundeb[12] += $oReceita->dezembro;
                            continue;
                        }
                        if ($p == 32) {
                            $transferenciaParlamentar[1] += $oReceita->janeiro;
                            $transferenciaParlamentar[2] += $oReceita->fevereiro;
                            $transferenciaParlamentar[3] += $oReceita->marco;
                            $transferenciaParlamentar[4] += $oReceita->abril;
                            $transferenciaParlamentar[5] += $oReceita->maio;
                            $transferenciaParlamentar[6] += $oReceita->junho;
                            $transferenciaParlamentar[7] += $oReceita->julho;
                            $transferenciaParlamentar[8] += $oReceita->agosto;
                            $transferenciaParlamentar[9] += $oReceita->setembro;
                            $transferenciaParlamentar[10] += $oReceita->outubro;
                            $transferenciaParlamentar[11] += $oReceita->novembro;
                            $transferenciaParlamentar[12] += $oReceita->dezembro;
                            continue;
                        }
                        if (db_conplano_grupo($anousu, substr($estrutural, 0, 3) . "%", 9001) == true) {
    ...
    /**
     * Ajustamos a regra do valor da linha 31
     */
    foreach ($totalPerdaFundeb as $mes =>  $valor) {
        if ($valor < 0) {
            $total += abs($valor);
        }
    }
    if ($matriz == true) {
        ...
        $rcl_matriz2['janeiro'] = $rcl_matriz[0][1] - ($rcl_matriz[1][1]) + abs($totalPerdaFundeb[1] <= 0 ? $totalPerdaFundeb[1] : 0);
        $rcl_matriz2['fevereiro'] = $rcl_matriz[0][2] - ($rcl_matriz[1][2]) + abs($totalPerdaFundeb[2] <= 0 ? $totalPerdaFundeb[2] : 0);
        $rcl_matriz2['marco'] = $rcl_matriz[0][3] - ($rcl_matriz[1][3]) + abs($totalPerdaFundeb[3] <= 0 ? $totalPerdaFundeb[3] : 0);
        $rcl_matriz2['abril'] = $rcl_matriz[0][4] - ($rcl_matriz[1][4]) + abs($totalPerdaFundeb[4] <= 0 ? $totalPerdaFundeb[4] : 0);
        $rcl_matriz2['maio'] = $rcl_matriz[0][5] - ($rcl_matriz[1][5])+ abs($totalPerdaFundeb[5] <= 0 ? $totalPerdaFundeb[5] : 0);
        $rcl_matriz2['junho'] = $rcl_matriz[0][6] - ($rcl_matriz[1][6])+ abs($totalPerdaFundeb[6] <= 0 ? $totalPerdaFundeb[6] : 0);
        $rcl_matriz2['julho'] = $rcl_matriz[0][7] - ($rcl_matriz[1][7])+ abs($totalPerdaFundeb[7] <= 0 ? $totalPerdaFundeb[7] : 0);
        $rcl_matriz2['agosto'] = $rcl_matriz[0][8] - ($rcl_matriz[1][8])+ abs($totalPerdaFundeb[8] <= 0 ? $totalPerdaFundeb[8] : 0);
        $rcl_matriz2['setembro'] = $rcl_matriz[0][9] - ($rcl_matriz[1][9])+ abs($totalPerdaFundeb[9] <= 0 ? $totalPerdaFundeb[9] : 0);
        $rcl_matriz2['outubro'] = $rcl_matriz[0][10] - ($rcl_matriz[1][10])+ abs($totalPerdaFundeb[10] <= 0 ? $totalPerdaFundeb[10] : 0);
        $rcl_matriz2['novembro'] = $rcl_matriz[0][11] - ($rcl_matriz[1][11])+ abs($totalPerdaFundeb[11] <= 0 ? $totalPerdaFundeb[11] : 0);
        $rcl_matriz2['dezembro'] = $rcl_matriz[0][12] - ($rcl_matriz[1][12])+ abs($totalPerdaFundeb[12] <= 0 ? $totalPerdaFundeb[12] : 0);
...
function duplicaReceitaaCorrenteLiquida($iAnoUsu, $iCodigoRelatorio)
{
     ...
     */
    $oDaoFiltroPadrao = new cl_orcparamseqfiltropadrao;
    ...
    $oDaoFiltroUsuario = new cl_orcparamseqfiltroorcamento;
...
/**
 * Retorna a fonte detalhada completa
 * @param $codigoRecuso
 * @return int|string
 */
function getFonteDetalhadaRecurso($codigoRecurso) {

    $recurso = RecursoRepository::getRecursoPorCodigo($codigoRecurso);
    if (!FONTE_RECURSO_UNIAO) {
        return $recurso->getFonteDeRecurso();
    }
    $recursoRetorno = $recurso->getFonteDeRecurso();
    if (empty($recursoRetorno)) {
...
/**
 * Retorna o grupo e especificação do recurso.
 *
 * Ex: 11256 - Retorna: 256
 *
 * @param $codigoRecurso
 * @return string|integer
 */
function getFonteReduzidaRecurso($codigoRecurso) {
    ...
    if (!FONTE_RECURSO_2020) {
        $recursoRetorno = substr($recursoRetorno, -3);
    }
...
/**
 * Verifica se o reduzido é o da instituição da sessão. Caso não seja, busca o reduzido da instituição.
 *
 ...
 * @throws Exception
 */
function getContaReduzidaNaInstituicao($codigoReduzido, $ano) {

    $contaPlano = ContaPlanoPCASPRepository::getContaPorReduzido($codigoReduzido, $ano);
    if ( (int)$contaPlano->getInstituicao() !== (int)db_getsession('DB_instit') ) {

        $instituicao = db_getsession('DB_instit');
        $contaDestino = ContaPlanoPCASPRepository::getReduzidoPorContaInstituicao($contaPlano->getCodigoConta(), $ano, $instituicao);
