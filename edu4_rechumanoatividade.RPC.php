    ...
    switch ($oParam->exec) {
        ...
        case 'buscaAtividadesProfissional':
            ...
            foreach ($oProfissional->getAtividades() as $oAtividade) {
                ...
                if (!is_null($oAtividade->getDataFim())) {
                    $oDadosAtividade->dataFim = $oAtividade->getDataFim()->getDate(DBDate::DATA_PTBR);
                }
        ...
        case 'excluirAtividade':
            ...
            foreach ($oParam->aAgendas as $oDadosAgendaExcluir) {
                $oAgendaProfissional = AgendaAtividadeProfissionalRepository::getByCodigo(
                    $oDadosAgendaExcluir->iCodigoAgenda
                );
        ...
        case 'salvarAtividade':
            ...
            $dataInicio = DBDate::create($oParam->dataInicio);
            $dataFim =  DBDate::create($sData)->adiantarPeriodo(365*30, 'd');
            if (!empty($oParam->dataFim)) {
                $dataFim = DBDate::create($oParam->dataFim);
            }
            ...
            if (count($aTodasAgendas) > 0) {
                foreach ($oParam->aSalvarAgenda as $oDadosSalvar) {
                    foreach ($aTodasAgendas as $oTodasAgendas) {
                        if (!empty($oDadosSalvar->iCodigoAgenda) &&
                            $oDadosSalvar->iCodigoAgenda == $oTodasAgendas->codigo_agenda) {
                            continue;
                        }
                        if (($oDadosSalvar->iDiaSemana != $oTodasAgendas->codigo_dia) ||
                            ($oTodasAgendas->turno != $oParam->iTurno) ||
                            ($oTodasAgendas->codigo_atividade != $oParam->iAtividade)) {
                            continue;
                        }

                        $dataInicioAgenda = DBDate::create($oTodasAgendas->data_inicio);
                        $dataFimAgenda = DBDate::create($sData)->adiantarPeriodo(365*30, 'd');
                        if ($oTodasAgendas->data_fim) {
                            $dataFimAgenda = DBDate::create($oTodasAgendas->data_fim);
                        }

                        if (DBDate::dataEstaNoIntervalo($dataInicio, $dataInicioAgenda, $dataFimAgenda) ||
                            DBDate::dataEstaNoIntervalo($dataFim, $dataInicioAgenda, $dataFimAgenda) ||
                            DBDate::dataEstaNoIntervalo($dataInicioAgenda, $dataInicio, $dataFim) ||
                            DBDate::dataEstaNoIntervalo($dataFimAgenda, $dataInicio, $dataFim)) {
                            ...
                            if ($iTimeHoraInicioSalvar < $iTimeHoraFimAgenda &&
                                $iTimeHoraFimSalvar > $iTimeHoraInicioAgenda) {
                                if ($dataInicio != $dataFimAgenda) {
                                    if ($dataFim != $dataInicioAgenda) {
                                        $aConflitos[] = mensagemConflito($oTodasAgendas, $oDadosSalvar);
                                    }
                                }
                            }
                        }
...
//            }
            ...
            $oAtividadeProfissional->setTurno($oParam->iTurno);

            $oAtividadeProfissional->setDataInicio(DBDate::create($oParam->dataInicio));
            $oAtividadeProfissional->setDataFim(null);
            if (!empty($oParam->dataFim)) {
                $oAtividadeProfissional->setDataFim(DBDate::create($oParam->dataFim));
            }
...
function buscaTodasAgendasProfissional($iVinculoEscola)
{
    ...
    $sCampos .= " ed129_diasemana as codigo_dia, trim(ed32_c_descr) as dia, ed129_codigo as codigo_agenda, ";
    $sCampos .= " ed22_i_atividade as codigo_atividade, trim(ed01_c_descr) as atividade, ed129_turno as turno, ";
    $sCampos .= " ed22_datainicio as data_inicio, ed22_datafim as data_fim ";
...
function mensagemConflito($oTodasAgendas, $oDadosSalvar)
{
    ...
    $oMsgErro->dInicio = DBDate::converter($oTodasAgendas->data_inicio);
    $oMsgErro->dFim = "(Em aberto)";
    if (!empty($oTodasAgendas->data_fim)) {
        $oMsgErro->dFim = DBDate::converter($oTodasAgendas->data_fim);
    }
    return $oMsgErro;
}
