...
if ( ! empty($recursos_selecionados) ) {
    $db_filtro .= " and o70_codigo in ({$recursos_selecionados}) ";
}

if (EMENTARIO_RECEITA) {
    $result = ReceitaSaldo(11, 1, $opcao, true, $db_filtro, $anousu, $dataini, $datafin);
} else {
...
while ($ln = pg_fetch_array($result)) {
    ...
    $linha = array(
        ...
        $ln["o70_codrec"],
        $ln["o70_codigo"],
        trim(db_formatar($ln["saldo_inicial"], 'f')),
        trim(db_formatar($ln["saldo_prevadic_acum"],'f')),
        trim(db_formatar($ln["saldo_arrecadado"],'f')),
...
$resultado = implode("\n", $todasLinhas);
file_put_contents('tmp/balrec.csv', $resultado);
