...
class db_utils
{
    ...
    {
        if ($mostra) {
            foreach ($aVetor as $chave => $valor) {
                echo $chave . " => " . $valor . " <br>\n";
            }
        }

        return (object)$aVetor;
    }

    /**
     * Trata encoding do array para transformar em JSON.
     *
     * @param array $mixed
     * @return stdClass
     */
    ...
    static function inTransaction($pConexao = null)
    {
        if (is_null($pConexao)) {
            ...
            if($conn !== null){
                ...
                $pConexao = Database::getInstance()->getConnection();
            }
        }
     ...
     *
     * @param $rsRecord - Recordset do Resultado da query
    ...
    public static function makeCollectionFromRecord($rsRecord, Closure $fRetorno)
    {
        ...
        //     $current = \db_utils::fieldsMemory($rsRecord, $i);


        //     if($current->vantdesc == 0) {
        //         $soma += (double) $current->valor;
        //     } 
        //     else if ($current->vantdesc == 1){
        //         $desc += (double) $current->valor;
        //     }
        // }
        
        // dump('Vantagem: ' . $soma);
        // dump('Desconto: ' . $desc);
        // dump('Diferença: ' . $soma - $desc);
        // dd('-------');
    ...
    public static function usarMelhoria($issue)
    {
        try {
            $caminhoPlugin = 'plugins/EntregaContinua';
    ...
    /**
     * Funcao criada para validar se cliente esta usando a regra de parcelamento de custas juridicas
     *
     * @return bool
     */
    public static function ativaParcelamentoCustas()
    {
        $sqlBusca = "select v19_parcelahonorariosmanual
                     from parjuridico
                     where v19_anousu = 2023 and v19_instit = " . db_getsession("DB_instit");
        $rsBusca = db_query($sqlBusca);
        $resultado = db_utils::getCollectionByRecord($rsBusca)[0]->v19_parcelahonorariosmanual  == 'f';
        return $resultado;
    }
}
