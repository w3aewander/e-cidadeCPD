...
for ($iCont = 0; $iCont < $iLinhasRetirada; $iCont++) {
    ...
    for ($iContItens = 0; $iContItens < $iLinhasItens; $iContItens++) {
        ...
        $sql = $dao->sql_query_file('', 'fa01_i_codigo', '', "fa01_i_codmater = {$oDadosItens->codigo}");
        $rs = $dao->sql_record($sql);
        if (!$rs) {
            continue;
        }
        $idMedicamento = db_utils::fieldsMemory($rs, 0)->fa01_i_codigo;
        $infoContinuado = MedicamentoContinuadoService::getInfoContinuado(
            $oDadosRetirada->paciente_cgs,
            $idMedicamento,
            new DateTime($dHoje),
            $lAcumularSaldo,
            $oConfigFarmacia->fa02_i_tipoperiodocontinuado == 1
        );

        if ($infoContinuado->proximaRetirada !== null) {
            $aProxDisp[$oDadosItens->codigo] = $infoContinuado->proximaRetirada->format('d/m/Y');
        }
    }
