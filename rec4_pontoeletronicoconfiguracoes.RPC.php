...
try {
    ...
    switch ($oParametro->exec) {
        ...
        case 'salvarConfiguracoesGerais':
            ...
            $oDaoConfiguracoesGerais->rh200_tipoasse_atraso           = $oParametro->rh200_tipoasse_atraso;
    ...
    case 'buscarTiposAssentamentosConfiguradosNaoPerdeDSR':
      ...
      if (!empty($parametrosAssentamentosNaoPerdeDSR)) {
        $aTipoAssentamentos = $parametrosAssentamentosNaoPerdeDSR->getTiposAssentamento();
      }
    ...
    case 'salvarConfiguracaoAssentamentoNaoPerdeDSR':

      if(!empty($oParametro->iCodigo)) {

        $daoPontoeletronicoassentamentosnaoperdedsr = new cl_pontoeletronicoassentamentosnaoperdedsr();
        ...
        if(!$daoPontoeletronicoassentamentosnaoperdedsr->incluir(null)) {
          throw new DBException($daoPontoeletronicoassentamentosnaoperdedsr->erro_msg);
        }

        $oRetorno->mensagem  = 'Tipo de assentamento configurado com sucesso.';
      }
      break;
      
    case 'excluirConfiguracaoAssentamentoNaoPerdeDSR':
      ...
      if(!empty($oParametro->iCodigo)) {
        ...
        $oRetorno->mensagem  = 'Configuração de tipo de assentamento excluída com sucesso.';
      }
      break;

        case 'buscarAssentamentosConfiguracoesGerais':

            $daoConfiguracoesGerais = new cl_pontoeletronicoconfiguracoesgerais();
            $sqlConfiguracoesGerais = $daoConfiguracoesGerais->sql_query_configuracoes(null, "rh200_instituicao = " . db_getsession('DB_instit'));
            $rsConfiguracoesGerais = db_query($sqlConfiguracoesGerais);

            if(!$rsConfiguracoesGerais) {
                throw new DBException('Erro ao buscar os assentamentos de efetividade.');
            }

            if(pg_num_rows($rsConfiguracoesGerais) == 0) {
                throw new DBException('Nenhum assentamento configurado para a instituição');
            }

            $oRetorno->assentamentos = db_utils::getCollectionByRecord($rsConfiguracoesGerais);
            $oRetorno->tiposConfigurados = new stdClass();
            $oRetorno->tiposConfigurados->faltas = false;
            $oRetorno->tiposConfigurados->adicional = false;
            $oRetorno->tiposConfigurados->extra50 = false;
            $oRetorno->tiposConfigurados->extra75 = false;
            $oRetorno->tiposConfigurados->extra100 = false;
            $oRetorno->tiposConfigurados->atraso = false;

            foreach($oRetorno->assentamentos as $assentamento) {
                if($assentamento->tipo == "rh200_tipoasse_adicionalnoturno") {
                    $oRetorno->tiposConfigurados->adicional = true;
                }

                if($assentamento->tipo == "rh200_tipoasse_falta") {
                    $oRetorno->tiposConfigurados->faltas = true;
                }

                if($assentamento->tipo == "rh200_tipoasse_extra50diurna" || $assentamento->tipo == "rh200_tipoasse_extra50noturna") {
                    $oRetorno->tiposConfigurados->extra50 = true;
                }

                if($assentamento->tipo == "rh200_tipoasse_extra75diurna" || $assentamento->tipo == "rh200_tipoasse_extra75noturna") {
                    $oRetorno->tiposConfigurados->extra75 = true;
                }

                if($assentamento->tipo == "rh200_tipoasse_extra100diurna" || $assentamento->tipo == "rh200_tipoasse_extra100noturna") {
                    $oRetorno->tiposConfigurados->extra100 = true;
                }

                if($assentamento->tipo == "rh200_tipoasse_atraso") {
                    $oRetorno->tiposConfigurados->atraso = true;
                }
            }
