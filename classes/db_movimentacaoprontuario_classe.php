...
class cl_movimentacaoprontuario
{
   // cria variaveis de erro
    ...
    public $sd102_codigo = 0;
    public $sd102_prontuarios = 0;
    public $sd102_db_usuarios = 0;
    public $sd102_setorambulatorial = 0;
    public $sd102_data_dia = null;
    public $sd102_data_mes = null;
    public $sd102_data_ano = null;
    public $sd102_data = null;
    public $sd102_hora = null;
    public $sd102_situacao = 0;
    public $sd102_observacao = null;
    public $sd102_profissionalatendimento = 0;
    public $sd102_profissionalencaminhado = 0;
   // cria propriedade com as variaveis do arquivo
    public $campos = "
                 ...
                 sd102_profissionalatendimento = int4 = Profissional Atendimento
                 sd102_profissionalencaminhado = int4 = Profissional Encaminhado
                 ";
    ...
    public function atualizacampos($exclusao = false)
    {
     if($exclusao==false){
       ...
       $this->sd102_profissionalatendimento = ($this->sd102_profissionalatendimento == ""?@$GLOBALS["HTTP_POST_VARS"]["sd102_profissionalatendimento"]:$this->sd102_profissionalatendimento);
       $this->sd102_profissionalencaminhado = ($this->sd102_profissionalencaminhado == ""?@$GLOBALS["HTTP_POST_VARS"]["sd102_profissionalencaminhado"]:$this->sd102_profissionalencaminhado);
     }else{
       $this->sd102_codigo = ($this->sd102_codigo == ""?@$GLOBALS["HTTP_POST_VARS"]["sd102_codigo"]:$this->sd102_codigo);
     }
   }

    public function incluir($sd102_codigo)
    {
      $this->atualizacampos();
     ...
     if($this->sd102_profissionalatendimento == null ){
       $this->sd102_profissionalatendimento = "null";
     }
     if($this->sd102_profissionalencaminhado == null ){
       $this->sd102_profissionalencaminhado = "null";
     }
     ...
     if(($this->sd102_codigo == null) || ($this->sd102_codigo == "") ){
       $this->erro_sql = " Campo sd102_codigo não declarado.";
     ...
     $sql = "insert into movimentacaoprontuario(
                                      ...
                                      ,sd102_situacao
                                      ,sd102_observacao
                                      ,sd102_profissionalatendimento
                                      ,sd102_profissionalencaminhado
                       )
                values (
                                $this->sd102_codigo
                               ...
                               ,$this->sd102_profissionalatendimento
                               ,$this->sd102_profissionalencaminhado
                      )";
     $result = db_query($sql);
     if($result==false){
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Movimentação da FAA ($this->sd102_codigo) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,3773,1014766,'','".AddSlashes(pg_result($resaco,0,'sd102_profissionalatendimento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,3773,1014765,'','".AddSlashes(pg_result($resaco,0,'sd102_profissionalencaminhado'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   }
    ...
    public function alterar($sd102_codigo=null)
    {
     ...
     if(trim($this->sd102_profissionalatendimento)!="" || isset($GLOBALS["HTTP_POST_VARS"]["sd102_profissionalatendimento"])){
        if(trim($this->sd102_profissionalatendimento)=="" && isset($GLOBALS["HTTP_POST_VARS"]["sd102_profissionalatendimento"])){
           $this->sd102_profissionalatendimento = "0" ;
        }
       $sql  .= $virgula." sd102_profissionalatendimento = $this->sd102_profissionalatendimento ";
       $virgula = ",";
     }
     if(trim($this->sd102_profissionalencaminhado)!="" || isset($GLOBALS["HTTP_POST_VARS"]["sd102_profissionalencaminhado"])){
        if(trim($this->sd102_profissionalencaminhado)=="" && isset($GLOBALS["HTTP_POST_VARS"]["sd102_profissionalencaminhado"])){
           $this->sd102_profissionalencaminhado = "0" ;
        }
       $sql  .= $virgula." sd102_profissionalencaminhado = $this->sd102_profissionalencaminhado ";
       $virgula = ",";
     }
     $sql .= " where ";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["sd102_profissionalatendimento"]) || $this->sd102_profissionalatendimento != "")
             $resac = db_query("insert into db_acount values($acount,3773,1014766,'".AddSlashes(pg_result($resaco,$conresaco,'sd102_profissionalatendimento'))."','$this->sd102_profissionalatendimento',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           if (isset($GLOBALS["HTTP_POST_VARS"]["sd102_profissionalencaminhado"]) || $this->sd102_profissionalencaminhado != "")
             $resac = db_query("insert into db_acount values($acount,3773,1014765,'".AddSlashes(pg_result($resaco,$conresaco,'sd102_profissionalencaminhado'))."','$this->sd102_profissionalencaminhado',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     if (!$result) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Movimentação da FAA não Alterado. Alteração Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->sd102_codigo;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Movimentação da FAA não foi Alterado. Alteração Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->sd102_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
    ...
    public function excluir($sd102_codigo=null, $dbwhere = null)
    {
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,3773,1014766,'','".AddSlashes(pg_result($resaco,$iresaco,'sd102_profissionalatendimento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,3773,1014765,'','".AddSlashes(pg_result($resaco,$iresaco,'sd102_profissionalencaminhado'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     ...
     if ($result == false) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Movimentação da FAA não Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Movimentação da FAA não Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$sd102_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
