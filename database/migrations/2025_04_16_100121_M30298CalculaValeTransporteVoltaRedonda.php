...
class M30298CalculaValeTransporteVoltaRedonda extends Migration
{
    public function up()
 {
    DB::connection()->getPdo()->exec(<<<SQL
      create or replace function pessoal.fc_valetransporte_dias_afasta_vr(integer,integer,integer) returns setof tp_dias_afastamento_valetransporte_vr as
$$
declare
...
from (
  select distinct
         h16_regist,
         date_part('dow', dtconc + gs) as dia_semana,
         dtconc + gs as data_dia
  from (
    select h16_regist,
           case
             when h16_dtconc < to_date(_ano || '-' || _mes || '-01', 'YYYY-MM-DD')
             then to_date(_ano || '-' || _mes || '-01', 'YYYY-MM-DD')
             else h16_dtconc
           end as dtconc,
           case
             when h16_dtterm > to_date(_ano || '-' || _mes || '-' || ndias(_ano, _mes)::char(2), 'YYYY-MM-DD') or h16_dtterm is null
             then to_date(_ano || '-' || _mes || '-' || ndias(_ano, _mes)::char(2), 'YYYY-MM-DD')
             else h16_dtterm
           end as dtterm
    from assenta
         inner join tipoasse on h12_codigo = h16_assent
    where h12_assent in ('001', '004' ,'029', '041', '067', '052', '062', '100', '110', '158')
  ...
  generate_series(0, dtterm - dtconc) as gs
) as dias
...
left join calendf on r62_calend::bigint = rh64_calend
                 and r62_data = data_dia
where date_part('dow', data_dia) between 1 and 5
  and (
    exists (
      select 1
      from assenta a2
      where a2.h16_regist = _registro
        and a2.h16_dtconc <= to_date(_ano || '-' || _mes || '-01', 'YYYY-MM-DD')
        and (a2.h16_dtterm >= to_date(_ano || '-' || _mes || '-' || ndias(_ano, _mes)::char(2), 'YYYY-MM-DD') or a2.h16_dtterm is null)
    )
    or r62_data is null);
...
from
  (select h16_regist,
      ...
      from assenta
           inner join tipoasse on h12_codigo = h16_assent
      where h12_assent in ('001', '004', '029', '041', '067', '052', '062', '100', '110', '158') and h16_regist = _registro
        and (h16_dtconc <= to_date(_ano||'-'||_mes||'-'||ndias(_ano,_mes)::char(2),'YYYY-mm-dd')
        and (h16_dtterm >= to_date(_ano||'-'||_mes||'-01','YYYY-mm-dd') or h16_dtterm is null ))
     ) as x
) as y
...
from (
  ...
  from (
    select h16_regist,
           case
             when h16_dtconc < to_date(ano_prox_ || '-' || mes_prox_ || '-01', 'YYYY-MM-DD')
             then to_date(ano_prox_ || '-' || mes_prox_ || '-01', 'YYYY-MM-DD')
             else h16_dtconc
           end as dtconc,
           case
             when h16_dtterm > to_date(ano_prox_ || '-' || mes_prox_ || '-' || ndias(ano_prox_, mes_prox_)::char(2), 'YYYY-MM-DD') or h16_dtterm is null
    ...
    where h12_assent in ('001', '004' ,'029', '041', '067', '052', '062', '100', '110', '158')
      and h16_regist = _registro
      and h16_dtconc <= to_date(ano_prox_ || '-' || mes_prox_ || '-' || ndias(ano_prox_, mes_prox_)::char(2), 'YYYY-MM-DD')
      and (h16_dtterm >= to_date(ano_prox_ || '-' || mes_prox_ || '-01', 'YYYY-MM-DD') or h16_dtterm is null)
  ) as afastamento,
  generate_series(0, dtterm - dtconc) as gs
) as dias
inner join rhpessoalmov on rh02_anousu = _ano
...
where date_part('dow', data_dia) between 1 and 5
  and (
    exists (
      ...
      from assenta a2
      where a2.h16_regist = _registro
        and a2.h16_dtconc <= to_date(ano_prox_ || '-' || mes_prox_ || '-01', 'YYYY-MM-DD')
        and (a2.h16_dtterm >= to_date(ano_prox_ || '-' || mes_prox_ || '-' || ndias(ano_prox_, mes_prox_)::char(2), 'YYYY-MM-DD') or a2.h16_dtterm is null)
    )
    or r62_data is null
  );
...
dias_validos_ := 30 - (dias_mes_inteiro_ + dias_fer_);
...
raise notice ' 14 r30_per1i_ %', r30_per1i_;
raise notice ' 15 r30_per1f_ %', r30_per1f_;
...
SQL
 ...
 {
    DB::connection()->getPdo()->exec(<<<SQL
      drop function pessoal.fc_valetransporte_dias_afasta_vr(integer,integer,integer);
SQL
     );
