...
abstract class BaseAbstract
{
    ...
    protected function gerarSegmentoP(Registro $oRegistro)
    {
        ...
        $oSegmentoP->valor_titulo             = str_pad(
            number_format((string)$oRegistro->getValor(), 2, '', ''),
            15,
            '0',
            STR_PAD_LEFT
        );
        ...
        $oSegmentoP->data_juros               = (
            is_null($oRegistro->getDataJuros()) ? str_pad('', 8, '0') : $oRegistro->getDataJuros()->getDate("dmY")
        );
        $oSegmentoP->taxa_juros               = str_pad($oRegistro->getTaxaJuros(), 15, '0');
        $oSegmentoP->codigo_desconto          = $oRegistro->getCodigoDesconto();
        $oSegmentoP->data_desconto            = (
            is_null($oRegistro->getDataDesconto()) ? str_pad('', 8, '0') : $oRegistro->getDataDesconto()->getDate("dmY")
        );
        ...
        $oSegmentoP->uso_empresa              = str_pad(
            $oRegistro->getNumeroDocumento(),
            25,
            ' ',
            STR_PAD_RIGHT
        ); //Numnov
    ...
    protected function gerarTrailer()
    {
        ...
        $oTrailer->quantidade_registros = str_pad(
            ((count($this->aRecibosGerados) * 2) + 2 + ($this->oHeader->getLote() * 2)),
            6,
            '0',
            STR_PAD_LEFT
        );
