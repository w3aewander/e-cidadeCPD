...
class pdf1 extends fpdf
{
  ...
  //global $ender;
  //$ender = pg_fetch_result($dados,0,"ender");
        $sql = "select nomeinst,
                 bairro,
                 cgc,
...
//Page footer
        ...
        if ($style1=="N") {
            ...
            while (list($k,$val)=each($this->PileStyle)) {
                ...
                if ($style1=="N") {
                    ...
                    if (preg_replace("B", $style1)) {
                        $style['b']="B";
                    }
                    if (preg_replace("I", $style1)) {
                        $style['i']="I";
                    }
                    if (preg_replace("U", $style1)) {
                        $style['u']="U";
                    }
                }
            }
    ...
    function Parser($text)
    {
        $tab=array();
        // Closing tag
        if (preg_replace("^(</([^>]+)>).*", $text, $regs)) {
            $tab[1]="c";
            $tab[2]=trim($regs[2]);
        }
        // Opening tag
        elseif (preg_replace("^(<([^>]+)>).*", $text, $regs)) {
            $regs[2]=preg_replace("^a", "a ", $regs[2]);
            $tab[1]="o";
            $tab[2]=trim($regs[2]);

            // Presence of attributes
            if (preg_replace("(.+) (.+)='(.+)' *", $regs[2])) {
                $tab1=split(" +", $regs[2]);
                $tab[2]=trim($tab1[0]);
