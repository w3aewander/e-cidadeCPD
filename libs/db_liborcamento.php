...
class cl_permissaodotacao
{
    ...
    {
        global $db20_orgao, $db20_unidade, $db20_funcao, $db20_subfuncao, $db20_programa, $db20_projativ,
               $db20_codele, $db20_codigo;

        $sql ="select 'usuario' as tipo ,p.*
                 from db_permemp p
                      inner join db_usupermemp u on u.db21_codperm = p.db20_codperm
                                                and u.db21_id_usuario = {$id_usuario}
                ...
                /* seleciona os departamentos do usuario */
                       ...
                       inner join db_depusuemp d on d.db22_codperm = p.db20_codperm
                       and d.db22_coddepto = e.coddepto";
        $result = db_query($sql);

        $sqlDotacoes = "select {$this->campos}
                          from orcdotacao
                         where ";
        for ($i = 0; $i < pg_num_rows($result); $i ++) {
            ...
            $tem_and = "";
            if ($db20_orgao > 0) {
                $sqlDotacoes .= " o58_orgao = $db20_orgao ";
                $tem_and = " and ";
            }
            if ($db20_unidade > 0) {
                $sqlDotacoes .= $tem_and." o58_unidade = $db20_unidade ";
                $tem_and = " and ";
            }
            if ($db20_funcao > 0) {
                $sqlDotacoes .= $tem_and." o58_funcao = $db20_funcao ";
                $tem_and = " and ";
            }
            if ($db20_subfuncao > 0) {
                $sqlDotacoes .= $tem_and." o58_subfuncao = $db20_subfuncao ";
                $tem_and = " and ";
            }
            if ($db20_programa > 0) {
                $sqlDotacoes .= $tem_and." o58_programa = $db20_programa ";
                $tem_and = " and ";
            }
            if ($db20_projativ > 0) {
                $sqlDotacoes .= $tem_and." o58_projativ = $db20_projativ ";
                $tem_and = " and ";
            }
            if ($db20_codele > 0) {
                $sqlDotacoes .= $tem_and." o58_codele = $db20_codele ";
                $tem_and = " and ";
            }
            if ($db20_codigo > 0) {
                $sqlDotacoes .= $tem_and." o58_codigo = $db20_codigo ";
            }
        }
        $sqlDotacoes .= " order by o50_estrutdespesa ";

        $this->dotacoes = $sqlDotacoes;
    }

    public function retornarecord()
    {
    ...
    public function mostradotacao($funcao_js = null)
    {
...
class cl_estrutura
{
  // cria variaveis de erro
    ...
    public function estrutura($picture = null)
    {
    ...
    <tr>
      ...
      ?>
        <input title="<?=@$$title?>"
               ...
               <?=($this->db_opcao==22||$this->db_opcao==33||$this->db_opcao==3?"readonly style=\"background-color:#DEB887\" ":"")?>
               <?=$funcao?> >
        <?php
        if ($this->input == false) {
            ?>
    ...
    <script>
      ...
      function js_mascara02_<?=$picture?>(obj){
          obj = document.<?php echo $this->nomeform; ?>.<?php echo $picture; ?>.value;
        while(obj.search(/\./)!='-1'){
          obj=obj.replace(/\./,'');
        }
        <?php
        ...
        document.<?php echo $this->nomeform; ?>.<?php echo $picture; ?>.value = obj;
        <?php
        if ($this->reload == true) {
            ?>
...
function db_selinstit($dbclick = '', $largura = 500, $altura = 120)
{
  //#00#//db_selinstit
  ...
  //#20#//$dbclick    : Funcao java script que será executada no onclick do da selecao dentro
  //#20#//do iframa na func_selinstit.php
  ...
  //#99#//Para os usuário da prefeitura, sempre será listado as instituições.
    ...
    $instits = array(6,7,8,13,14);
    if (pg_num_rows($result) > 0 or (in_array(db_getsession('DB_instit'),$instits) && $isNiteroi) ) {
        echo "<input name='db_selinstit' type='hidden' value='' ><br>";
        echo "<strong>Selecione a(s) Instituição(ões):</strong><br>";
        echo "<iframe name='db_selinstit_iframe'
                      width='".$largura."px'
                      height='".$altura."px'
                      src='func_selinstit.php?funcao=$dbclick'></iframe>";
    } else {
...
function db_receitappa($anoini, $db_where = false, $retsql = false)
{
    ...
    $sqlCriaTabela = "create temporary table work_pl( estrut_mae varchar(15),
                                                      estrut varchar(15),
                                                      ...
                                                      a3 float8,
                                                      a4 float8);
                        create index work_pl_estrut on work_pl(estrut);
                        create index work_pl_estrutmae on work_pl(estrut_mae); ";
    db_query($sqlCriaTabela);

    $sql = "select o57_fonte  as estrut_mae,
                   o57_fonte  as estrut,
              ...
              from (select o57_fonte  ,
                      ...
                      from orcpparec
                           inner join orctiporec on o15_codigo = o70_codigo
                            ...
                            left join orcreceita on o70_codfon = o27_codfon
    ...
    global $descr_rece;
    global $recurso;
    global $complementoRecurso;
    global $filtroLoaComplemento;
    global $descr_recu;
    global $a1;
    global $a2;
    global $a3;
        ...
        for ($ii = 1; $ii < 10; $ii ++) {
            ...
            if ($key === false) { // não achou
              // busca no banco e inclui
                $sql = "select c60_descr as descr_rece,c60_finali,c60_codcon
                          from conplano
                         where c60_anousu = ".db_getsession("DB_anousu")."
                           and c60_estrut = '$estrutural'";
                $res = db_query($sql);
                if ($res == false || pg_num_rows($res) == 0) {
                    $msg = "Está faltando cadastrar esse estrutural na contabilidade. ";
                    $msg .= "Nível : $nivel  Estrutural : $estrutural - ano: " + db_getsession("DB_anousu");
    ...
    for ($i = 0; $i < sizeof($work_planomae); $i ++) {
        ...
        $sql = "insert into work_pl values ('$mae',
                                            '$estrut',
                                            '$descr_rece',
                                            $recurso,
                                            '$descr_recu',
                      ...
                      from (select e91_numemp,
                              ...
                              from (select e91_numemp,
                                      ...
                                      from empresto
                                           ...
                                           left outer join ( select c75_numemp,
                                                                    sum( case
                                                                           when c53_tipo = 11
                                                                             then c70_valor
                                                                           else 0
                                                              ...
                                                              where e60_anousu < {$anousu}
                                                                and c75_data between '{$dt_ini}' and '{$dt_fin}'
...
function db_dotacaosaldo($nivel = 8, $tipo_nivel = 1, $tipo_saldo = 2, $descr = true, $where = '', $anousu = null, $dataini = null, $datafim = null, $primeiro_fim = 8, $segundo_inicio = 0, $retsql = false, $tipo_balanco = 1, $desmembra_segundo_inicio = true, $subelemento = 'nao')
{
  ...
  //   9999999 as o58_coddot,
   ...
   ";
    ...
    (select distinct o58_anousu,
                     ...
                     descricao
                from (select *,
                             fc_dotacaosaldo({$anousu},o58_coddot,{$tipo_saldo},'{$dataini}','{$datafim}')
                        from orcdotacao w
             ...
             /**
               *----------------------------------------------------------------------------------
               * Comentado pois era utilizado pelo balancete da despesa e gera lentidão
               * Importante verificar o campo o206_numero pois ele depende do campo o206_origem
               * para saber com qual tabela ligar
               *----------------------------------------------------------------------------------
               * left join empempenho on e60_coddot = o58_coddot
               *                    and e60_anousu = o58_anousu
               * left join origemcomplementorecurso on o58_codigo = o206_recurso
               *                                  and o206_numero = e60_numemp
               */

              $sql.= " where o58_anousu = {$anousu}
                            {$condicao}
                      order by o58_orgao,
                               o58_unidade,
                               o58_funcao,
  ...
  /////// nivel 8 ///////////////
    ...
    if (8 <= $primeiro_fim || ($desmembra_segundo_inicio == true && $segundo_inicio <= 8 || $desmembra_segundo_inicio == false && $segundo_inicio == 8)) {
        ...
        $nivel8 .= "o58_codigo, o58_localizadorgastos, o58_concarpeculiar ) as i";
    } else {
        $nivel8 = '';
    }
    ...
    if (7 <= $primeiro_fim || ($desmembra_segundo_inicio == true && $segundo_inicio <= 7 || $desmembra_segundo_inicio == false && $segundo_inicio == 7)) {
        ...
        $nivel7 .= " o58_codele, o58_elemento ) as g";
    } else {
        $nivel7 = '';
    }
    ...
    if (3 <= $primeiro_fim || ($desmembra_segundo_inicio == true && $segundo_inicio <= 3 || $desmembra_segundo_inicio == false && $segundo_inicio == 3)) {
        ...
        $nivel3 = "select {$xnivel3}
                     ...
                     from ( select {$xnivel3}
                             ...
                             group by ";
        if ($tipo_nivel != 3) {
            $nivel3 .= " o58_orgao, o58_unidade,";
        }
        $nivel3 .= " o58_funcao ) as c";
    } else {
        $nivel3 = '';
    }
  ...
  // pesquisa as despesas
    ...
    if ($primeiro_fim >= 8) {
        $junta .= "case when o58_coddot = -1 then 0 else o58_coddot end as o58_coddot,
                   case when o58_codigo = -1 then 0 else o58_codigo end as o58_codigo,
                   gestao,
                   descricao,
                   o58_localizadorgastos,
                   o58_concarpeculiar,";
        $xordem .= "o58_codigo,gestao,descricao,o58_coddot,o58_localizadorgastos,o58_concarpeculiar";
    }
    ...
    if ($primeiro_fim < 8) {
        ...
        if ($segundo_inicio <= 1) {
            $junta .= $virg."case
                               when o58_orgao = -1 then 0
                               else o58_orgao
                            end::integer as o58_orgao,
                            o40_descr";
            $xordem .= "o58_orgao,o40_descr";
        ...
        if ($segundo_inicio <= 2 && $tu_para == false) {
            $junta .= $virg."case
                               when o58_unidade = -1 then 0
                               else o58_unidade
                             end::integer as o58_unidade,
                             o41_descr";
        ...
        if ($segundo_inicio <= 3 && $tu_para == false) {
            $junta .= $virg."case
                               when o58_funcao = -1 then 0
                               else o58_funcao
                             end::integer as o58_funcao,
                             o52_descr";
        ...
        if ($segundo_inicio <= 4 && $tu_para == false) {
            $junta .= $virg."case
                               when o58_subfuncao = -1 then 0
                               else o58_subfuncao
                             end::integer as o58_subfuncao,
                             o53_descr";
        ...
        if ($segundo_inicio <= 5 && $tu_para == false) {
            $junta .= $virg."case
                               when o58_programa = -1 then 0
                               else o58_programa
                             end::integer as o58_programa,
                             o54_descr";
        ...
        if ($segundo_inicio <= 6 && $tu_para == false) {
            $junta .= $virg."case
                                when o58_projativ = -1 then 0
                                else o58_projativ
                              end::integer as o58_projativ,
                              o55_descr,
                              o55_finali";
        ...
        if ($segundo_inicio <= 8 && $tu_para == false) {
            $junta .= $virg."case when o58_coddot = -1 then 0 else o58_coddot end::integer as o58_coddot,
                             case when o58_codigo = -1 then 0 else o58_codigo end::integer as o58_codigo,
                             gestao,
                             descricao,
                             o58_localizadorgastos,
                             o58_concarpeculiar";
            $xordem .= $virg."o58_codigo,gestao,descricao,o58_coddot,o58_localizadorgastos, o58_concarpeculiar";
            if ($desmembra_segundo_inicio == false) {
                $tu_para = true;
            }
            $virg = ',';
    ...
    $sql2 = "select {$junta}
                    ...
                    sum(reservado_ate_data)            as reservado_ate_data
               from ( ( {$sql} ) as xx
                      left  outer join orcorgao      o on o40_anousu      = {$anousu}
                                                      and o.o40_orgao     = o58_orgao
                      left  outer join orcunidade    u on o41_anousu      = {$anousu}
                                                      and u.o41_orgao     = o58_orgao
                                                      and u.o41_unidade   = o58_unidade
                      left  outer join orcfuncao     f on f.o52_funcao    = o58_funcao
                      left  outer join orcsubfuncao  s on o53_subfuncao   = o58_subfuncao
                      left  outer join orcprograma   p on o54_anousu      = {$anousu}
                                                      and o54_programa    = o58_programa
                      left  outer join orcprojativ  pa on o55_anousu      = {$anousu}
                      ...
                      left outer join fonterecurso     on fonterecurso.orctiporec_id = o15_codigo
                                                      and exercicio = {$anousu} ) as x
              group by ".$xordem."
              order by ".$xordem;
  ...
  // funcao para gerar work
    ...
    if ($anousu > 2007) {
        ...
        if ($nivel_agrupar == 1 || $nivel_agrupar == 2) {
            $sql .= " case
                        when substr(o57_fonte,1,1)='9'
                          then '4'
                        else substr(o57_fonte,1,1)
                      end::int4 as classe,";
        } elseif ($nivel_agrupar == 0) {
            $sql .= " substr(o57_fonte,1,1)::int4 as classe, ";
        }
        $sql .= "substr(o57_fonte,2,1)::int4  as grupo,
      ...
      ///////// Nivel 3 //////////////
        ...
        $nivel3 .= " subgrupo ) as c";

      ///////// Nivel 2 //////////////

        $nivel2 = " select classe,
                           grupo,
                           0 as subgrupo,
                           0 as elemento,
                           0 as subelemento,
        ...
        $sql2 = " select {$campos}
                    from ( select {$demaiscampos}
                             ...
                             from ( select distinct
                                           ...
                                           x.*
                                      from ($sql) as x
                                           left outer join conplano y on y.c60_estrut = cast(classe as varchar)
                                                                                        ||cast(grupo as varchar)
                                                                                        ||cast(subgrupo as varchar)
                                                                                        ||cast(elemento as varchar)
                                                                                        ||cast(subelemento as varchar)
                                           ...
                                           left join conplanoreduz on c61_codcon = c60_codcon
                                     ...
                                     order by classe,
                                              ...
                                              desdobramento3,
                                              o70_codrec ) as yy
                         order by {$demaiscampos1} o70_codrec desc,
                                  o57_descr,
                                  o70_codigo,
                                  o15_descr,
                                  saldo_inicial,
        ...
        if ($nivel == 11) {
            $sql2 .=" union all
                  ...
                  select {$demaiscampos}
                         ...
                         saldo_prev_anterior
                    from work_receita
                    	 left outer join orcfontes y on y.o57_fonte = cast(classe as varchar)
                                                                      ||cast(grupo as varchar)
                                                                      ||cast(subgrupo as varchar)
                                                                      ||cast(elemento as varchar)
                                                                      ||cast(subelemento as varchar)
      ...
      // aqui inicia o codigo de 2007
        ...
        $sql = "create temp table work_receita as
               select substr(o57_fonte,1,1)::int4  as classe,
...
class cl_permusuario_dotacao
{
    ...
    public function __construct(
        ...
        global $db20_orgao, $db20_unidade, $db20_funcao, $db20_subfuncao, $db20_programa,
        $db20_projativ, $db20_codele, $db20_codigo;
        ...
        $sqle = " SELECT 'USUARIO' AS TIPO ,DB_PERMEMP.*
                    FROM DB_PERMEMP
                         INNER JOIN DB_USUPERMEMP U ON U.DB21_CODPERM = DB20_CODPERM
                                                   AND U.DB21_ID_USUARIO = {$idusuario}
                    LEFT OUTER JOIN DB_DEPARTORG D  ON DB20_ORGAO = D.DB01_ORGAO
                                                   AND D.DB01_ANOUSU = ".db_getsession("DB_anousu")."
       ...
       SELECT 'SETOR' AS TIPO ,
        ...
        if ($result != false && pg_num_rows($result) > 0) {
            ...
            $secretar = " select distinct o40_orgao,o40_descr
                            from orcdotacao
                                 inner join orcorgao on o40_anousu = ".db_getsession("DB_anousu")."
                                                    and o40_orgao = o58_orgao
                                 inner join orcelemento on o56_codele = o58_codele
                                                       and o56_anousu = o58_anousu
                           where ( (  ";
            ...
            $departam = " select distinct coddepto,descrdepto
                            from orcdotacao
                                 inner join orcorgao on o40_anousu = ".db_getsession("DB_anousu")."
                                                    and o40_orgao = o58_orgao
                                 inner join db_departorg on db01_orgao = o40_orgao
                                                        and db01_anousu = o40_anousu
                                 inner join db_depart on db01_coddepto = coddepto
                                                     and (   limite is null or limite >= '{$dataSessao}')
                                 inner join orctiporec on o58_codigo = o15_codigo
                                 inner join orcelemento on o56_codele = o58_codele
                                                       and o56_anousu = o58_anousu
                           where ( ( ";
            ...
            if ($dotacoes != "") {
                ...
                && $aParametroCompras[0]->pc30_dotacaopordepartamento == 't') {
                    $sSqlUnidade = "select *
                                      from db_departorg
                                     where db01_coddepto= {$this->departamento}
...
class cl_selorcdotacao
{
  ...
  /* gets */
    ...
    public function getDadosReceita($alias = true)
    {
        $txt = "1=1";
        if ($alias == true) {
    ...
    public function getDadosLocalizadorGastos()
    {
        return $this->localizadorgastos;
    }

  //00//
  //10// se $elemento = true default, retorna elemento, caso contratio nao retorna,
  //10// nesse caso use o getElemento() para obte-lo
  //10//
    public function getDados($alias = true, $ret_elemento = true)
    {
        $txt = "1=1";
        if ($alias == true) {
            if ($this->orgao != null) {
            ...
            if ($this->elemento != null  && $ret_elemento ==true) {
                if (trim($this->elemento)!="") {
                    ...
                    foreach ($elementos as $elemento) {
                        $filtro[] = "e.o56_elemento ilike '{$elemento}%'";
                    }

                    $filtroElemento = "(" . implode(' or ', $filtro) . ")";
                    $txt .= " and {$filtroElemento} ";
                }
            }
            if ($this->recurso != null) {
            ...
            if ($this->elemento != null  && $ret_elemento ==true) {
                if (trim($this->elemento)!="") {
                    ...
                    $filtro = [];
                    foreach ($elementos as $elemento) {
                        $filtro[] = "o56_elemento ilike '{$elemento}%'";
                    }
  ...
  //10 // nesse caso use o getElemento() para obte-lo
  //10 //
    public function getDadosComplemento($alias = true, $ret_elemento = true)
    {
        $txt = "1=1";
        ...
        if ($alias == true) {
            ...
            if ($this->recurso != null) {
                $txt .= " and o15_codigo in".$this->recurso;
            }

            if ($this->localizadorgastos != null) {
                $txt .= " and o58_localizadorgastos in".$this->localizadorgastos;
            }
        } else {
            if ($this->orgao != null) {
                $txt .= " and o58_orgao in".$this->orgao;
    ...
    public function getParametros($nomes = false)
    {
  ...
  /* sets */
    public function setDados($filtro)
    {
    ...
    public function processa()
    {
        ...
        if ($this->filtra_despesa != "geral") {
            ...
            for ($f = 0; $f < sizeof($qual_filtro); $f ++) {
                ...
                if ($ver[0] == "ele") {
                    $sql_elem = "select o56_elemento
                                   from orcelemento
                                  where o56_anousu = ".db_getsession("DB_anousu")."
                                    and o56_codele = ".$ver[1];
...
/**
 * Realiza o calculo do saldo da linnha de pacto
 * @param $linhaPacto
 * @param null $ateData
 * @return int
 */
function getSaldoPactoLinhaOrcamento($linhaPacto, $ateData = null)
{
    ...
    $where[] = "o161_linhapacto = {$linhaPacto}";
    if (!empty($ateData)) {
    ...
    $sqlSaldoPacto = "select coalesce(sum(o161_saldo), 0) as saldo
                        from linhapactosaldo
                       where {$sWhere}";

    $rsConsultaSaldo = db_query($sqlSaldoPacto);
    if (pg_num_rows($rsConsultaSaldo) > 0) {
        $saldoLinhaPacto = db_utils::fieldsMemory($rsConsultaSaldo, 0)->saldo;
    }
    return $saldoLinhaPacto;
}
