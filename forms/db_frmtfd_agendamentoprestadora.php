...
<form name="form1" method="post" action="">
    <center>
        <table style="width:100%;border:0">
            ...
            <tr>
                ...
                <td nowrap colspan="3">
                    ...
                    db_input('z01_nome', 50, $Iz01_nome, true, 'text', 1, "onchange = 'limparCentral();'");
                    ?>
                </td>
            </tr>
            <tr>
                ...
                <td nowrap colspan="3">
                    ...
                    db_input('z01_nome2', 50, $Iz01_nome, true, 'text', 3, "onchange = 'limparPrestadora();'");
        ...
        <fieldset style='width: 86%;'>
            <legend><b>Dados do Agendamento</b></legend>
            <table>
                <tr>
                ...
                <tr>
                    <td nowrap title="<?= $Ttf16_observacoes ?>">
                        <?= $Ltf16_observacoes ?>
                    </td>
                    <td colspan="4">
                        <?php
                        $js = ' onkeyup="this.value.substring(0,60);"';
                        db_input('tf16_observacoes', 50, $Itf16_observacoes, true, 'text', $db_opcao, $js);
                        ?>
                    </td>
                </tr>
            </table>
...
<script>
    ...
    function js_pesquisatf10_i_prestadora(mostra) {
        ...
        if (document.form1.tf10_i_centralagend.value == '') {
            ...
            alert('Escolha uma central de agendamento primeiro.');
            js_limpaInfoCgm();
            return false;
    ...
    function js_mostratfd_centralagendamento(chave, erro) {
        ...
        document.getElementById("z01_nome2").readOnly = false;
        document.getElementById("z01_nome2").style.backgroundColor = "#FFFFFF";
        if (erro == true) {
            limparCentral();
    ...
    function js_mostratfd_centralagendamento1(chave1, chave2) {
        ...
        document.form1.z01_nome2.disabled = false;
    ...
    function limparCentral(){
        if($('z01_nome').value.length == 0){
            $('tf10_i_centralagend').value = '';
            $('tf10_i_prestadora').value = '';
            $('z01_nome2').value = '';
            document.getElementById("z01_nome2").readOnly = true;
            document.getElementById("z01_nome2").style.backgroundColor = "#DEB887";
        }
    }

    function limparPrestadora(){
        if($('z01_nome2').value.length == 0){
            $('tf10_i_prestadora').value = '';
        } 
    }

    // Autocomplete da Central
    let oAutoComplete = new dbAutoComplete($('z01_nome'), 'tfd4_informacoesgerais.RPC.php?central');
    oAutoComplete.setHeightList(390);
    oAutoComplete.show();
    oAutoComplete.setCallBackFunction(function (codigoCentral, nomeCentral,numCgm) {
        $('tf10_i_centralagend').value = codigoCentral;
        $('z01_nome').value = nomeCentral;   
        document.getElementById("z01_nome2").readOnly = false;
        document.getElementById("z01_nome2").style.backgroundColor = "#FFFFFF";
    });

    // Autocomplete da Prestadora
    let urlPrestadora = 'tfd4_informacoesgerais.RPC.php?prestadora';
    urlPrestadora += '&codigoCentral='+$('tf10_i_centralagend').value;

    let oAutoComplete2 = new dbAutoComplete($('z01_nome2'), urlPrestadora);
    oAutoComplete2.setHeightList(390);
    oAutoComplete2.show();
    oAutoComplete2.setCallBackFunction(function (codigoPrestadora, nomePrestadora) {

        $('tf10_i_prestadora').value = codigoPrestadora;
        $('z01_nome2').value = nomePrestadora.urlDecode();
        
        if(codigoPrestadora.length > 0){
            js_pesquisatf10_i_prestadora(false);
        }
    });      
