...
class M16247MultaDtoper extends PostgresMigration
{
    public function up()
    {
        $sql = <<<STRING

CREATE OR REPLACE FUNCTION public.fc_multa(integer, date, date, date, integer) RETURNS real AS
$$
...
declare
  ...
  lMultaDtoper   boolean default false;

begin
  
  lMultaDtoper := ( case when fc_getsession('DB_multa_dtoper_dtvenc') is null then false else true end );
  
  if data_hoje <= data_venc then
    return 0;
  end if;

  if lMultaDtoper then
     dtoper = data_venc;
  end if;
