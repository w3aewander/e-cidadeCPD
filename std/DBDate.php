...
class DBDate
{
     ...
     * @var array
     */
    private static $aDiasSemanaAbreviado = array(
        self::DOMINGO => 'Dom',
        self::SEGUNDA => 'Seg',
        self::TERCA => 'Ter',
        self::QUARTA => 'Qua',
    ...
    protected function validaData($sData)
    {
        ...
        } elseif (strpos($sData, "T")) {
            list($ano, $mes, $dia) = explode("-", Date('Y-m-d', strtotime($sData)));
        } elseif (strpos($sData, "-")) {
            list($ano, $mes, $dia) = explode("-", $sData);
        } else {
            $sMsgErro = "Data com formato inválido. {$sData} \n";
            $sMsgErro .= "Formatos aceitos: \"Ano-Mês-Dia\" ou \"Dia/Mês/Ano\" \"Ano-Mês-diaT00:00:00.0000Z\"";
            throw new ParameterException($sMsgErro);
        }
    ...
    /**
     * Seta um novo TimeStamp para o objeto
     * @param $iTimeStamp
     */
    ...
    public static function calculaIntervaloEntreDatas(DBDate $oDt1, DBDate $oDt2, $sIntervalo)
    {
        ...
        $nValor = (strtotime($oDt1->getDate(DBDate::DATA_EN)) - strtotime($oDt2->getDate(DBDate::DATA_EN)));
        $nValor = $nValor / $nIntervalo;
        return floor($nValor);
    }
     ...
     */
    public static function getLabelDiaSemana($iDiaSemana)
    {
     ...
     * @return DBDate[]
     */
    public static function getDatasNoIntervalo(
        DBDate $oDtInicio,
        DBDate $oDtFim,
     ...
     * @return array
     */
    public static function getMesesNoIntervalo(DBDate $oDtInicio, DBDate $oDtFim, $lMeExtenso = true)
    {
        if ($oDtInicio->getTimeStamp() > $oDtFim->getTimeStamp()) {
            throw new ParameterException("Data de inicio não pode ser maior que a data final");
        }
        ...
        do {
            ...
            if ($oData->getAno() < $oDtFim->getAno()) {
                ...
                if (($oData->getAno() == $oDtFim->getAno()) && ($oData->getMes() <= $oDtFim->getMes())) {
                    $lContinua = true;
                } else {
                    $lContinua = false;
                }
    ...
    {
        return (
            ($oData1Inicio->getTimeStamp() <= $oData2Fim->getTimeStamp())
            && ($oData1Fim->getTimeStamp() >= $oData2Inicio->getTimeStamp())
        );
    }
     ...
     * @return array|string[]
     */
    public static function getMesesAbreviado()
    {
        return self::$aMesesAbreviatura;
    }

    public static function getDiasSemana($diaSemana)
    {
        if (!empty(self::$aDiasSemana[$diaSemana])) {
            return self::$aDiasSemana[$diaSemana];
        }

        return self::$aDiasSemana;
    }
     ...
     * @return array|mixed
     */
    public static function getDiasSemanaAbreviado($diaSemana)
    {
        if (!empty(self::$aDiasSemanaAbreviado[$diaSemana])) {
            return self::$aDiasSemanaAbreviado[$diaSemana];
        }

        return self::$aDiasSemanaAbreviado;
    }

    public static function format($stringData, $format = DBDate::DATA_PTBR)
    {
        return DBDate::create($stringData)->convertTo($format);
    }

    /**
     * Baseado no objeto do tipo DBDate, gera um novo DBDate com o proximo dia
     * @return DBDate
     */
    public function getProximoDia()
    {
        $proximoDia = DateTime::createFromFormat('Y-m-d', $this->getDate());
        $proximoDia->modify('+1 day');
        return new DBDate($proximoDia->format('Y-m-d'));
    }

    /**
     * Verifica se o dia atual e feriado ou nao
     * Utiliza o calendario cadastrado no modulo RH
     * @return boolean
     */
    public function isFeriado()
    {
        ...
        $rs = \db_query($sql);
        if (!$rs) {
            $mensagem = "Houve um erro ao buscar informações de feriado para a data "
                . $this->getDate(DBDate::DATA_PTBR);
    ...
    /**
     * Baseado no objeto do tipo DBDate, gera um novo DBDate com o proximo dia util (pula finais de samana e feriados)
     * @return DBDate
     */
    public function getProximoDiaUtil()
    {
        $proximoDia = DateTime::createFromFormat('Y-m-d', $this->getDate());
        $proximoDia->modify('+1 day');
        return new DBDate($proximoDia->format('Y-m-d'));
    ...
    /**
     * Baseado no objeto do tipo DBDate, gera um novo DBDate com o dia anterior
     * @return DBDate
     */
    ...
    {
        return new DBDate($data);
    }

    public static function converterDataParaTexto($data)
    {
        ...
        $mes = $partesData[1];
        $ano = $partesData[0];
        // Construindo o texto da data
        $textoData = $dia . ' de ' . self::$aMesesExtenso[(int)$mes] . ' de ' . $ano;
