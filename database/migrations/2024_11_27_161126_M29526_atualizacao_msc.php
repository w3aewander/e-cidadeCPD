...
class M29526AtualizacaoMsc extends Migration
{
    /**
     * Run the migrations.
     *
...
begin
    ...
    for recordContas in (
        ...
        for recordDados in (
            ...
            -- Divida Consolidada: 1 - nï¿½o compoem a DC
            divida_consolidada = case
                 when recordContas.c60_consistemaconta != 9 and strpos(recordContas.informacoescomplementares, 'DC') != 0
                     then 1
                 else null
    ...
    for recordDados in (
        with saldo_inicial as (
            select c151_reduzido  as reduzido,
                   ...
                   null::int as tipo_documento
            from contabilidade.conplanoexeempenho
            join contabilidade.dados_auxiliar_empenho_matriz(c151_empenho, c151_exercicio) as e
                 on e.numemp = c151_empenho
            where c151_reduzido = f_reduz
              and c151_exercicio = f_ano
            ...
            from debitos
            join contabilidade.conlancamemp on c75_codlan = lancamento
            join contabilidade.dados_auxiliar_empenho_matriz(c75_numemp, exercicio) as e
                 on e.numemp = conlancamemp.c75_numemp
            ...
            select debitos.*,
                   ...
                   o53_siconfi as subfuncao,
                   c155_ai as ano_empenho,
                   debitos.exercicio as anousu_rp
            from debitos
            join contabilidade.conlancamajustesaldoconta on c155_conlancam = lancamento
            join orcamento.orcfuncao on o52_funcao = c155_funcao
            join orcamento.orcsubfuncao on o53_subfuncao= c155_subfuncao
            ...
            select creditos.*,
                   ...
                   o53_siconfi as subfuncao,
                   c155_ai as ano_empenho,
                   creditos.exercicio as anousu_rp
            from creditos
            join contabilidade.conlancamajustesaldoconta on c155_conlancam = lancamento
            join orcamento.orcfuncao on o52_funcao = c155_funcao
            join orcamento.orcsubfuncao on o53_subfuncao= c155_subfuncao
...
create or replace function contabilidade.dados_auxiliar_empenho_matriz(f_empenho int, f_ano integer)
returns table (
    numemp integer,
    codemp varchar,
    ano_empenho integer,
...
begin
    ...
    for recordDados in (
        select e60_numemp, e60_codemp, e60_anousu,
               e91_anousu,
               case when e91_anousu is not null then e91_anousu else e60_anousu end anousu,
               o206_recurso as id_recurso,
               planodespesa.conta,
               o52_siconfi as funcao,
               o53_siconfi as subfuncao
        from empenho.empempenho
        join empenho.empelemento on empelemento.e64_numemp = empempenho.e60_numemp
        ...
        where uniao is true
          ...
          and (case when e91_anousu is not null then e91_anousu = f_ano else e60_anousu = f_ano end)
    ) loop
            numemp = recordDados.e60_numemp;
            codemp = recordDados.e60_codemp;
            ano_empenho = recordDados.e60_anousu;
            anousu_rp = recordDados.e91_anousu;
            anousu = recordDados.anousu;
            id_recurso = recordDados.id_recurso;
            conta = recordDados.conta;
            funcao = recordDados.funcao;
            subfuncao = recordDados.subfuncao;
