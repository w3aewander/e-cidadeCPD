...
for ($x=0;$x<$linhas;$x++) {
  ...
  if ($tipo == "TF") {
  	...
  	$campos     .= " (SELECT ed15_c_nome FROM turno INNER JOIN turnoreferente ON ed15_i_codigo = ed231_i_turno INNER JOIN turmaturnoreferente ON ed336_turnoreferente = ed231_i_codigo WHERE ed336_turma = matricula.ed60_i_turma LIMIT 1) as rsturno";
  ...
  $pdf->multicell(192, 8, "Declaração de Transferência", "LR",  "C", 0, 0);
  $pdf->multicell(192, 8,  "",                      "LR", "C", 0, 0);
  $pdf->setfont('arial', '', 9);
...
//  $oParagrafo                         = new libdocumento(5010);
  ...
  $oParagrafo->turma                  = $sTurma;
  $oParagrafo->turno                  = $rsturno;
 ...
 $sObsFixa = "Período Letivo: {$sDataInicio} até {$sDataFim}";
...
// $completar = 240 - $pdf->getY();
 ...
 $pdf->multicell(192, 30, "", "LRB", "C", 0, 0);
}
