<?php
    ...
    $aOperacoesRealizadas = $operacoesrealizadastefRepository->getAllConfirmadasAutorizadoraByNumnov(
        $this->numnov_recibo,
        [
            "*",
            DB::raw("TO_CHAR(k198_dataoperacao, 'DD/MM/YYYY HH24:MI:SS') AS k198_dataoperacao")
        ]
    );
...
for ($i = 0; $i < 2; $i++) {
    ...
    ///Totais
    $this->objpdf->Setfont('Arial', '', 6);
    $this->objpdf->Roundedrect(147, $xlin+103, 32, 9, 2, 'DF', '1234');
    $this->objpdf->Roundedrect(181, $xlin+103, 25, 9, 2, 'DF', '1234');
    $this->objpdf->text(151, $xlin+105, 'Código de Arrecadação');
    $this->objpdf->text(183, $xlin+105, '( = ) Valor Cobrado R$');
    $this->objpdf->setfont('Arial', '', 10);
    $this->objpdf->text(148, $xlin+109, $this->numpre);
    ...
    $this->objpdf->text(62, $xlin+104.5, "Boleto quitado por TEF");

    $this->objpdf->setfont('Arial', '', 6);
    $iAltura = $xlin+108;

    foreach ($aOperacoesRealizadas as $key => $oOperacaoRealizada) {
        if ($key > 0) {
            $iAltura = $iAltura + 4;
        }

        $this->objpdf->text(5, $iAltura, $oOperacaoRealizada->getDataoperacao());
        $this->objpdf->text(27, $iAltura, "AUT:{$oOperacaoRealizada->getCodigoaprovacao()}");

        $this->objpdf->text(39, $iAltura, $oOperacaoRealizada->operacoesTef->getDescricao());
        $this->objpdf->text(65, $iAltura, $oOperacaoRealizada->getParcela());
        $this->objpdf->text(72, $iAltura, formataValorMonetario($oOperacaoRealizada->getValor()));
        $this->objpdf->text(87, $iAltura, $oOperacaoRealizada->getBandeira());
        $this->objpdf->text(110, $iAltura, $oOperacaoRealizada->getCartao());
    }
