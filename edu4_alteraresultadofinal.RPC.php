...
try {
  ...
  switch($oParam->exec) {
    ...
    case 'getAlunosAlteraResultadoFinal':
      ...
      foreach ($oTurma->getAlunosMatriculadosNaTurmaPorSerie($oEtapa) as $oMatricula) {
         ...
         * Data: 17/12/2025
         * Demanda: 18029
         * Razao: Expor ao front-end se o aluno é avaliado por parecer (relatório),
         *        permitindo aplicar regras específicas de forma de aprovação.
         */
        $oAluno->lAvaliadoParecer     = $oMatricula->isAvaliadoPorParecer();
    ...
    case 'excluirAlteracaoResultadoFinal':
      ...
      // IMPORTANTE: NÃO chamar $oDiarioAvaliacaoDisciplina->salvar() aqui!
      // O removerAprovacaoConselho() já chama $oAvaliacaoResultadoFinal->salvar() internamente
      // Se chamar novamente, vai recalcular a média e sobrescrever a nota restaurada
      ...
      /**
       * IMPORTANTE: Limpa a sessão após exclusão para garantir que a próxima
       * requisição busque dados atualizados do banco
       */
    ...
    case 'salvarAlteracaoResultadoFinal':
       ...
       *
       * REGRA DE NEGÓCIO:
       * - Alunos avaliados por relatório NÃO podem ter aprovação pelo conselho
       *   (formas 1 = APROVADO_CONSELHO e 3 = APROVADO_CONFORME_REGIMENTO_ESCOLAR);
       * - Para esses alunos, é permitida apenas RECLASSIFICAÇÃO POR BAIXA FREQUÊNCIA (forma 2).
       */
      /**
       * Autor: Uemerson Santana
       * Data: 17/12/2025
       * Demanda: 18029
       * Razao: Adequar a validação para bloquear apenas aprovações pelo conselho (formas 1 e 3)
       *        mantendo permitida a reclassificação por baixa frequência (forma 2) para alunos
       *        avaliados por relatório.
       */
      if ($oMatricula->isAvaliadoPorParecer()
          && (int)$oParam->iFormaAprovacao !== AprovacaoConselho::RECLASSIFICACAO_BAIXA_FREQUENCIA) {

        $oRetorno->status  = 2;
        $oRetorno->message = urlencode("Alunos avaliados por relatório não podem ter aprovação pelo conselho. Apenas reclassificação por frequência é permitida.");
        break;
      }
      ...
      // Definir período de alteração se informado
      if (!empty($oParam->iPeriodoAlteracao)) {
        $oAvaliacaoResultadoFinal->setPeriodoAlteracao($oParam->iPeriodoAlteracao);
      }
      ...
      /**
       * IMPORTANTE: Limpa a sessão após sucesso para garantir que a próxima
       * requisição busque dados atualizados do banco (com notas e resultado alterados)
       */
      EducacaoSessionManager::limpar();
  ...
  /**
   * IMPORTANTE: Limpa a sessão quando houver erro para garantir que a próxima
   * requisição busque dados frescos do banco, evitando cache de dados incorretos
   */
  EducacaoSessionManager::limpar();
}
