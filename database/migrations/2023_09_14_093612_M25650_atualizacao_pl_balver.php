...
class M25650AtualizacaoPlBalver extends Migration
{
    /**
     * Run the migrations.
     *
...
update db_itensmenu set funcao = 'con2_balancete_verificacao_complemento001.php' where id_item = 228955;
drop function if exists contabilidade.balancete_verificacao_por_recurso;
drop function if exists contabilidade.balancete_verificacao_plano_padrao;
SQL
        );
    }

    private function upEcidade()
    {
        DB::connection()->getPdo()->exec(<<<SQL
drop function if exists contabilidade.balancete_verificacao_por_recurso;
...
SQL
    ...
    private function upPadrao()
    {
        DB::connection()->getPdo()->exec(<<<SQL
drop function if exists contabilidade.balancete_verificacao_plano_padrao;

create or replace function contabilidade.balancete_verificacao_plano_padrao(
    f_ano integer,
    f_uniao boolean,
    f_dataInicio date,
    f_dataFim date,
    ...
    for recordDados in (
      with plano_uniao as (
        select
          ...
          from contabilidade.balancete_verificacao_por_recurso(f_ano, f_dataInicio, f_dataFim, f_encerramento, f_reduzidos) b
          ...
          where pcasp.uniao = f_uniao
            and pcasp.exercicio = f_ano
          group by pcasp.conta, pcasp.nome, pcasp.indicador, b.exercicio, b.classe, b.instituicao, b.id_recurso,
                   b.siconfi, b.gestao, b.subrecurso, b.complemento, b.natureza_informacao
        ...
        classe = recordDados.classe;
        nome = recordDados.nome;
        natureza_informacao = recordDados.natureza_informacao;
        instituicao = recordDados.instituicao;
        id_recurso = recordDados.id_recurso;
        siconfi = recordDados.siconfi;
        gestao = recordDados.gestao;
...
SQL
    ...
    private function atualizaMenu()
    {
        DB::connection()->getPdo()->exec(<<<SQL
update db_itensmenu
set funcao = 'web/financeiro/contabilidade/relatorios/balancetes/verificacao',
    descricao = 'Balancete Verificação por Recurso',
    help = 'Balancete Verificação por Recurso'
where id_item = 228955;
SQL
        );
    }
}
