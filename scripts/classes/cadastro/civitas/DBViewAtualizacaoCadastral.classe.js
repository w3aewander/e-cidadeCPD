...
DBViewAtualizacaoCadastral.prototype.criaWindow = function () {
  ...
  this.oWindow.allowCloseWithEsc(true);
    ...
    var sConteudo  = "<div class='container' style='width:100%; height: 500px'>";
        sConteudo += "  <div class='_header' style='margin-bottom: 20px'>";
      ...
      sConteudo += "  <div id='conteudo' style='width:100%; height: inherit '>";

      sConteudo += "    <div style='position: relative; width: 99%; margin: auto; '> ";    


      sConteudo += "      <div id='a' style='width: 98%; float: left; display: inline-block; '> ";
      sConteudo += "        <div id='conteudoDetalhamento'>";
      sConteudo += "          <iframe style='width:100%; height:100%;' id='iFrameDetalhamento' frameborder='0' ></iframe>";
      sConteudo += "        </div>";
      sConteudo += "      </div>";
      ...
      sConteudo += "      <input type='button' id='btnAtualizar' value='Aprovar' />";
  ...
  this.oWindow.setShutDownFunction( function() {
    ...
    this.fCloseCallback();
    if ( !empty(this.oWindowObservacao) ) {
      this.oWindowObservacao.destroy();
    }
...
DBViewAtualizacaoCadastral.prototype.criaWindowObservacao = function () {

  if ( !empty(this.oWindowObservacao) ) {
    this.oWindowObservacao.show(205, false, true);
    return null;
  }
  this.oWindowObservacao = new windowAux("wndObservacaoRejeicao", "Motivo da Rejeição", 600, 245 );
  this.oWindowObservacao.zIndex = 9999;
  this.oWindowObservacao.allowCloseWithEsc(false);
...
DBViewAtualizacaoCadastral.prototype.atualizarDados = function() {
  ...
  if (this.oMatricula.iNovaMatricula == 1) {
      $('codigoMatricula').value = 'Nova';
  } else {
  ...
  $('iFrameDetalhamento').src = 'cad4_recadastramentoconsultaiptu.php?parametro='+this.oMatricula.iMatricula + '&schema=' +this.oSchema.sSchema ;

  $('btnAtualizar').disabled = false;
  $('btnRejeitar').disabled  = false;
  
  switch(parseInt(this.oMatricula.iSituacao)) {
  ...
  if ( !empty(this.oWindowObservacao) ) {
    this.oWindowObservacao.divContent.querySelector('#motivoRejeicao').value = '';
  }
}

DBViewAtualizacaoCadastral.prototype.existMatricInSetorAndQuadraAndLote = function() {
...
DBViewAtualizacaoCadastral.prototype.rejeitar = function() {
  ...
  var outrasMatriculas = this.existMatricInSetorAndQuadraAndLote();

  if (outrasMatriculas.size() > 0) {
...
DBViewAtualizacaoCadastral.prototype.atualizar = function() {
  ...
  }.bind(this)).setMessage('Aprovando cadastro...').execute();
}
