...
class db_stdClass
{
     ...
     * @return object
     */
    public static function getDadosInstit($iInstit = null)
    {
     ...
     */
    public static function getParametro($sClassParametro, $aKeys = null, $sFields = "*")
    {
     ...
     * @return date
     */
    public static function getIntervaloDiasUteis($dtDataIni = "", $iNroDias = 0)
    {
    ...
    public static function oo2pdf(
        ...
        try {
            $oDocumentoTemplate = new documentoTemplate($tipoDoc, $codDoc, "", $lTemTransacaoAtiva);
        } catch (Exception $eException) {
            $redirectString = "db_erros.php?fechar=true&db_erro=" . $eException->getMessage();
            $hasError = true;
        }
        ...
        if (!$hasError) {
            ...
            if ($lProcessado) {
                try {
                    if ($useLegacyConverter) {
                        DocumentConverter::docToPdf($sCaminhoSalvoSxw, $sNomeRelatorio);
                    } else {
                        convertToPdf($sCaminhoSalvoSxw, ECIDADE_PATH."tmp");
                    }

                    $redirectString = $sNomeRelatorio;
                } catch (Exception $e) {
                    $redirectString = "db_erros.php?fechar=true&db_erro=" . $e->getMessage();
                    $hasError = true;
                }
            } else {
                $redirectString = "db_erros.php?fechar=true&db_erro=Falha ao gerar relatório !!!";
                $hasError = true;
            }
        }

        if ($redirect) {
            db_redireciona($redirectString);
        } else {
            return !$hasError;
        }
    }
     ...
     * @return void
     */
    public static function ex_oo2pdf($sCaminhoSalvoSxw, $sNomeRelatorio)
    {
        try {
            DocumentConverter::docToPdf($sCaminhoSalvoSxw, $sNomeRelatorio);
        } catch (Exception $e) {
     ...
     * @return bool|string
     */
    public static function geraObjetoOid($iOid, $Con, $sTipo = "jpg", $sDir = "tmp/", $sNomeArquivo = null)
    {
     ...
     */
    public static function getCaminhoMenu($mMenu)
    {
     ...
     */
    public static function inCollection($sPropertie, $sValue, $aColection)
    {
     ...
     * @return boolean
     */
    public static function possuiPCASPAtivo()
    {
     ...
     * @return string
     */
    public static function crossUrlDecode($sString)
    {
