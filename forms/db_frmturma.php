...
<form name="form1" method="post" action="">
    <center>
        ...
        <input type="text" id="inputPermissao" value="<?= $lTemPermissao ?>" hidden>
        <input type="text" id="inputExclusao" value="<?= in_array($db_opcao, [1, 2, 22]) ? 0 : 1; ?>" hidden>
        ...
        <table border="0" width="100%">
            ...
            if ($db_opcao == 2 || $db_opcao == 3 && !isset($excluir)) {
                ...
                $ed219_i_codigo = $result_etapa ? pg_fetch_result($result_etapa, 0, 'ed219_i_codigo') : "";
                $ed219_c_nome = $result_etapa ? pg_fetch_result($result_etapa, 0, 'ed219_c_nome') : "";
            ...
            <tr id='trBilingue'>
                <td nowrap title="<?= @$Ted57_is_bilingue ?>">
                    <label for="ed57_is_bilingue"> <?= @$Led57_is_bilingue ?> </label>
                </td>
                <td>
                    <?php
                    $x = array('f' => 'NÃO', 't' => 'SIM', );
                    db_select('ed57_is_bilingue', $x, true, $db_opcao, '');
                    ?>
                </td>
            </tr>

            <?php
            $displayCurso = isset($ed29_censocursoprofiss) ? '' : 'none';
            $visivel = "hidden";
            if ($db_opcao == 2 || $db_opcao == 3 && !isset($excluir)) {
                for ($p = 0; $p < $clturmaserieregimemat->numrows; $p++) {
                    db_fieldsmemory($result_etapa, $p);
            ...
            ?>
            <tr name="cursoprofiss" id="cursoprofiss" style="display: <?=$displayCurso?>">
                <td nowrap title="Curso Profissionalizante">
                    <label for="ed29_censocursoprofiss">
                        <b>Curso Profissionalizante</b>
                    </label>
                </td>
                <td>
                    <?php
                    db_input('ed29_censocursoprofiss', 15, 1, true, 'text', 3,
                        " onchange='js_pesquisaed29_censocursoprofiss(false);'");
...
<script>
    var lTemPermissao = document.getElementById('inputPermissao').value;
    lTemPermissao = lTemPermissao.trim();
    var lTemPermissaoIsTrue = (lTemPermissao === "true");

    const isFromExclusao = document.getElementById('inputExclusao');
    ...
    function js_pesquisaed57_i_base(mostra) {
        ...
        if (mostra == true) {
            ...
            js_OpenJanelaIframe('', 'db_iframe_base', 'func_baseturma.php?funcao_js=parent.js_mostrabase1|ed31_i_codigo|' +
                ...
                'ed218_c_divisao|ed31_c_medfreq|ed29_i_ensino|ed29_c_historico|ed29_censocursoprofiss|ed247_c_descr&lBaseAtiva=' + lBaseAtiva,
            ...
            if (document.form1.ed57_i_base.value != '') {
                ...
                document.form1.ed29_censocursoprofiss.value = '';
    ...
    function js_mostrabase1(chave1, chave2, chave3, chave4, chave5, chave6, chave7, chave8, chave9, chave10, chave11, chave12, chave13) {
        ...
        document.form1.ed29_censocursoprofiss.value = chave12;
        document.form1.ed247_c_descr.value = chave13;
        console.log(chave12)
        if (chave12 != '' ) {
            document.querySelector('#cursoprofiss').style.display = '';
        } else {
            document.querySelector('#cursoprofiss').style.display = 'none';
        }
        document.form1.ed57_i_turno.value = '';
        document.form1.ed15_c_nome.value = '';
    ...
    function js_pesquisaed29_censocursoprofiss(mostra) {
    ...
    function js_mostracensocursoprofiss1(chave1, chave2) {
        ...
        document.form1.ed29_censocursoprofiss.value = chave1;
    ...
    function js_mostraprocedimento1(chave1, chave2) {
        ...
        document.form1['ed220_i_procedimento' + document.form1.linhaproc.value].value = chave1;
        document.form1['ed40_c_descr' + document.form1.linhaproc.value].value = chave2;
    ...
    function js_validacaoInc() {
        ...
        if (document.form1.ver_cursoprofiss.value == "OK" && document.form1.ed29_censocursoprofiss.value == "") {
            ...
            if (tam == undefined) {
                ...
                for (i = 0; i < tam; i++) {
                    ...
                    if (document.form1.etapa[i].checked == true) {
                        ...
                        if (document.form1['ed220_i_procedimento' + i].value == "") {
        ...
        if (document.form1.etapa) {
            ...
            if (tam == undefined) {
                ...
                for (i = 0; i < tam; i++) {
                    ...
                    if (document.form1['ed220_i_procedimento' + i]) {

                        document.form1['ed220_i_procedimento' + i].value = "";
                        document.form1['ed40_c_descr' + i].value = "";
    ...
    function js_insereLinhaVagasTurmaReferencia() {
        ...
        aTurnoReferente = [...new Set(aTurnoReferente)];
        ...
        aTurnoReferente.each(function (sTurno) {
            ...
            var oLabelVagas = new Element("label", {
                "class": "bold",
                "for": sVagas
            }).update("Vagas " + sTurnoLower + ": ");
            var oLabelMatriculados = new Element("label", {
                "class": "bold",
                "for": sMatriculados
            }).update(" Alunos matriculados: ");
            var oLabelDisponivel = new Element("label", {
                "class": "bold",
                "for": sDisponivel
            }).update(" Vagas disponíveis: ");
            var oInputVagas = new Element("input", {
                "type": "text", "name": sVagas, "size": 8, "id": sVagas, "value": iVagasSala,
                "referente": sTurnoLower.removeAcento(), "class": "vagasTurma"
            });

            if (!lTemPermissaoIsTrue || isFromExclusao.value === '1') {
                oInputVagas.classList.add('readonly');
                oInputVagas.disabled = true;
            }

            var oInputMatriculados = new Element("input", {
                "type": "text", "disabled": "disabled", "name": sMatriculados, "size": 8,
                "class": "readonly matriculadosTurno",
                "id": sMatriculados, "value": "0", "referente": sTurnoLower.removeAcento()
            });
            var oInputDisponivel = new Element("input", {
                "type": "text", "disabled": "disabled", "name": sDisponivel, "class": "readonly",
...
*/
        var oInputMatriculados = new Element("input", {
            "type": "text", "disabled": "disabled", "name": sMatriculados, "size": 8, "class": "readonly",
