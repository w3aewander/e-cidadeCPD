...
class SigfisArquivoItemReceita extends SigfisArquivoBase implements iPadArquivoTXTBase {
  ...
  public function gerarDados() {
    ...
    $sSqlOrcFontes = $clOrcFontes->sql_query_previsao(null, $this->iAnoUso, $sCampos, $sOrder, " o70_instit = " . db_getsession("DB_instit"));
    ...
    $sSqlOrcFontes = "select case when substr(o57_fonte,1,1) = '9' then '9' || substr(o57_fonte,3,7) else substr(o57_fonte,2,8) end as o57_fonte, o57_descr, o57_anousu, o57_codfon, reduz from ( $sSqlOrcFontes ) as x group by case when substr(o57_fonte,1,1) = '9' then '9' || substr(o57_fonte,3,7) else substr(o57_fonte,2,8) end, o57_descr, o57_anousu, o57_codfon, reduz order by case when substr(o57_fonte,1,1) = '9' then '9' || substr(o57_fonte,3,7) else substr(o57_fonte,2,8) end";
...
//          exit;
          ...
          foreach ($aVinculo as $a => $b) {
            ...
            if (substr($oDadosQuery->o57_fonte, 0, strlen($a)) == $a) {
              if ($b == 0) {
                $oDados->cd_ItemReceita = str_pad(substr($oDadosQuery->o57_fonte, 1, strlen($a)-1),8, '0', STR_PAD_RIGHT);
              } else {
                if (substr($b,0,1) == '9') {
                  if ($lDebug==1) { echo("ok\n"); }
                  $oDados->cd_ItemReceita = str_pad(substr($b, 0, strlen($b)),8, '0', STR_PAD_RIGHT);
                } else {
                  $oDados->cd_ItemReceita = str_pad(substr($b, 1, strlen($b)-1),8, '0', STR_PAD_RIGHT);
                }
              }
