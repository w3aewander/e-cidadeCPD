...
?>
<?php if ($oParans->it24_carregaconstrucoesbenfeitoriasitbi == "t" AND (!empty($oGet->tipo) and $oGet->tipo == "urbano")) : ?>
    <style>
        .trForm {
            display: none;
        }
...
<form name="form1" method="post" action="">
  ...
  <tr>
    <td colspan="6" align="center">
      <input name="db_opcao" type="submit" id="db_opcao" value="<?=($db_opcao==1?"Incluir":($db_opcao==2||$db_opcao==22?"Alterar":"Excluir"))?>" <?=($db_botao==false?"disabled":"")?> onclick="return js_validaCampos();">
    </td>
  </tr>
  <tr>
    <td align="top" colspan="6">
      ...
      if(empty($it08_guia) or !isset($it08_guia) or trim($it08_guia) == ''){
	    ...
	    if ($oParans->it24_carregaconstrucoesbenfeitoriasitbi == "t") {
            $cliframe_alterar_excluir->escondeOpcoes = true;
        }
...
{
    if (document.getElementById("it08_areatrans").value == 0) {
        alert("A área transmitida não pode ser igual a 0.");
        return false;
    }
}
</script>
