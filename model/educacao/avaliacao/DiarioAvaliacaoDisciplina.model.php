...
class DiarioAvaliacaoDisciplina extends DiarioDisciplina {
  ...
  public function setEncerrado($lEncerrado) {
    ...
    if (!is_bool($lEncerrado)) {
      throw new ParameterException('Par?metro lEncerrado informado deve ser um boolean.');
    }
  ...
  public function adicionarAvaliacao(IElementoAvaliacao $oElementoAvaliacao, $mValorAproveitamento='', $iNumeroFaltas='') {
    ...
    /**
     * Verificamo se a disciplina j? est? lancadada para o periodo.
     * caso j? esteja, apenas alteramos o valor do aproveitamento e o numero de faltas.
     */
      ...
      for ($iDiario = 0; $iDiario < $iTotalLinhas; $iDiario++) {
        ...
        switch ($sTipoAvaliacao) {
          ...
          case 'NOTA' :
            ...
            $oValorAproveitamento          = new ValorAproveitamentoNota($oDadosDiario->valor_nota); // ja passei aqui
   ...
   */
  public function salvar($encerrar=false) {
...
//***********************************************************************************************************************************
  protected function salvarDadosAvaliacao(AvaliacaoAproveitamento $oAvaliacaoAproveitamento) {
     ...
     * Data: 28/10/2025
     * Demanda: 17930
     * Razãoo da mudan?a: N?o inicializar ed72_t_parecer como vazio para preservar pareceres existentes durante atualização
     */
     ...
     * Data: 28/10/2025
     * Demanda: 17930
     * Razãoo da mudan?a: Permitir salvar parecer complementar para alunos avaliados por parecer e preservar pareceres de outros bimestres
     */
    if ($oAvaliacaoAproveitamento->getParecer() != '') {
      // S? sobrescreve parecer se h? um novo parecer vindo de getParecer()
      $oDaoDiarioAvaliacao->ed72_t_parecer = pg_escape_string(("{$oAvaliacaoAproveitamento->getParecer()}"));
    }
    // Se não h? parecer novo, mant?m o parecer existente durante atualização
    // Para inserção, não define ed72_t_parecer (fica vazio) - comportamento padr?o
...
//***********************************************************************************************************************************
  /**
   * Persiste as informações das avalia??es
   * @param AvaliacaoAproveitamento $oAvaliacaoAproveitamento
   * @return bool
   * @throws BusinessException
   * @throws DBException
  ...
  protected function salvarDadosResultado(AvaliacaoAproveitamento $oAvaliacaoAproveitamento) {
	...
	$sql = 'select
			...
			inner join regencia      on ed59_i_disciplina = ed12_i_codigo
			where
			ed59_i_codigo = '.$this->oRegencia->getCodigo();

	$result1 = db_query($sql);
	$disc = db_utils::fieldsMemory($result1,0);
    $GLOBALS["HTTP_POST_VARS"]["disciplina1"] = $disc->ed232_c_descr;
    ...
    $oElementoAvaliacao         = $oAvaliacaoAproveitamento->getElementoAvaliacao();
	if( $oElementoAvaliacao == '')
	{
		$GLOBALS["HTTP_POST_VARS"]["encerrar"] = false;
	}
    ...
    if (!empty($oRecuperacao)) {
      ...
      /**
       * N?mero m?ximo de Reprova??es atingidas
       */
      $lTemDireitoRecuperacao = $iTotalDisciplinasRecuperacao > 0 && $iTotalReprovacoesNoPeriodo <= $iTotalDisciplinasRecuperacao;

      /**
       * Caso o aluno tenha algum aproveitamento na recuperacao, o mesmo nao dever? ficar mais em recupera??o, pois
       * j? concluiu o mesmo.
       */
    ...
    if (!$this->isEncerrado()) {
      ...
      /**
       * Validação necess?ria para tratamento do Resultado Final tratando o tipo de avaliação
       */
      switch ($sFormaAvaliacao) {

        /**
         * NOTA:> Temos que avaliar o valor definido para aproveitamento m?nimo
         */
        case 'NOTA':
          ...
          /**
           * Alterado valida??o para testar com o tipo mais condição
        ...
        /**
         * NIVEL:> Temos que avaliar a ordem das avalia??es
         */
        case 'NIVEL':
    ...
    if ($oAvaliacaoAproveitamento->getElementoAvaliacao()->geraResultadoFinal()) {
       ...
       * Data: 19/12/2025
       * Demanda: 18059
       * Razãoo: Corre??o da lógica de reprovação por frequência.
       *        ANTES: Se disciplina não tinha car?ter reprobat?rio, for?ava aprova??o por frequência.
       *        AGORA: Se disciplina não tem car?ter reprobat?rio, apenas for?amos aprova??o por NOTA/CONCEITO,
       *               mas mantemos a verificação de FREQU?NCIA (< 75% = reprova).
       *        Isso corrige o problema onde alunos do 1? e 2? ano com frequência abaixo de 75%
       *        apareciam aprovados em disciplinas sem car?ter reprobat?rio.
       *
       * Regras:
       * - 1? ano: s? reprova por frequência (não por nota/conceito)
       * - 2? ano: reprova por conceito D e por frequência
       * - 3? ao 5? ano: reprova por nota < 5.0 e por frequência
       * - Anos Finais: reprova por nota < 5.0 e por frequência (com depend?ncia de at? 2 disciplinas)
       * - Frequ?ncia < 75% = reprova (em todos os segmentos, exceto Educa??o Infantil)
       */
      if ( !$lCaracterReprobatorio ) {
        // Sem car?ter reprobat?rio: não reprova por NOTA/CONCEITO
        if (!$this->temAproveitamentoLancado() ) {
          $sResultadoAprovacao  = "A";
        }
        // IMPORTANTE: N?O for?amos mais $sResultadoFrequencia = "A"
        // A verificação de frequência continua ativa (linhas 757-765)
      }
      ...
      /**
       * Validado qual o ?ltimo Resultado que gera Resultado Final e verifica se ele est? sendo utilizado para gerar
       * o Resultado Final
       */
      ...
      if ( $oUltimoResultado->getOrdemSequencia() == $oAvaliacaoAproveitamento->getOrdemSequencia() ) {
        ...
        /**
         * Se procedimento, tem recupera??o, mas o aluno excedeu o limite de disciplinas, o aluno deve ser reprovado.
         */
      ...
      /**
       * Caso haja 2 resultados que geram resultados finais, verifica se o Resultado Final j? foi lan?ado e se o Valor
       * Aproveitamento do segundo resultado est? em branco e retorna, fazendo com que não seja setado os valores vazios
       * para o Diario Final.
       */
      ...
      $oAvaliacaoResultadoFinal->salvar($this->encerrar);
    }
  }

  /**
   * Verifica se a lan?amento de faltas lan?adas por periodo de aula
   * @param PeriodoAvaliacao $oPeriodoAvaliacao periodo de avaliacao que est? sendo verificado
   * @return boolean
   */
  ...
  /**
   * Retorna o total de Faltas do periodo quando existe lan?amento de faltas por periodo de aula
   * @param PeriodoAvaliacao $oPeriodoAvaliacao periodo de avaliacao que est? sendo verificado
   * @return integer;
   */
   ...
   *
   * o C?lculo ? apenas realizado para turma em que o resultado final ? calculado por Nota
   * @param iElementoAvaliacao $oElementoAvaliacao elemento de avaliacao em que nota dever? ser calculada
   * @return float
   */
  public function getNotaParcial(iElementoAvaliacao $oElementoAvaliacao) {
    ...
    if ($oElementoResultadoFinal->getFormaDeAvaliacao()->getTipo() != "NOTA") {
      return '';  // retorna vazio e da erro
    }
    ...
    foreach ($this->getAvaliacoes() as $oAvaliacaoAproveitamento) {
      ...
      if ($iOrdemAvaliacao < $oElementoAvaliacao->getOrdemSequencia()) {
        ...
        /**
         * Quando o per?odo ? uma recupera??o, (tem $oElementoVinculado) ? que permitimos que ele
         * tenha aproveitamento em branco.
         * Nos outros casos, nunca podemos calcular nota parcial com uma nota s?.
         */
  ...
  /**
   * Valida se esta disciplina esta em recupera??o
   * @return boolean
   */
  public function emRecuperacao() {
    ...
    if ( !$rsRecuperacao ) {
        throw new DBException('Falha ao verificar se o aluno est? em recupera??o na disciplina.');
    }
	$media = $this->alunoInicio();
    if (pg_num_rows( $rsRecuperacao ) > 0) {
		if( $media >=5 )
		{
		    return false;
		}else{
			return false; // se a media for inferior a 5 o aluno fica aprovado com dependencia
		}
    }
    return false;
  }

public function alunoInicio()
{
	$sql = "select ed72_i_valornota	from diarioavaliacao where ed72_i_diario = ".$this->iCodigoDiario;
	...
	for($x=0;$x<pg_num_rows($result);$x++)
	{
		...
		if($odados->ed72_i_valornota <> null)
		{
			$notamedia += $odados->ed72_i_valornota;
			$cont++;
		}
	}
    $media =  $notamedia/$cont;
	return $media;
}
  ...
  /**
   * Verifica se o di?rio possui alguma falta lan?ada e se controla reprovação pela frequencia
   * @return boolean
   */
  ...
  /**
   * Retorna os periodos de avaliação do procedimento de avaliação da Reg?ncia
  ...
  /**
   * Retorna o per?odo de avaliação de acordo com  a ordem informada
   *
   * @param  int $iOrdem   Ordem sequencial do per?odo de avaliação
  ...
  /**
   * Retorna a ordem dos períodos que devem ser aplicado o c?lculo da proporcionalidade
   * @return array
   */
  ...
  /**
   * Limpa a order dos períodos com proporcionalidade, quando necess?rio atualizar a ordem dos períodos
   */
  ...
  /**
   * Retorna a regra da avaliacao alternativa, se houver uma avaliação alternativa configurada para o di?rio
  ...
  /**
   * Salva o v?nculo de um diario( DiarioAvaliacaoDisciplina ) com uma avaliação alternativa
  ...
  /**
   * Exclui o v?nculo de um diario com uma avaliação alternativa
   * @throws DBException
   */
   ...
   * --> Escola > Procedimentos > Par?metros Globais > Apresentar nota proporcional: N?O
   *
