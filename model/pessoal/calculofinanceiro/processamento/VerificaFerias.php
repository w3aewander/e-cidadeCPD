...
function verificarFerias($lCalculoFeriasEfetuado)
{
          ...
          // F021 - Numero de dias p/ calc do FGTS no mes
            ...
            ($opcao_geral == PONTO_COMPLEMENTAR && strtolower($cadferia[0]["r30_ponto"]) == "c" && $cadferia[0][$r30_proc] == $subpes)
              ...
              //     lancar o abono inteiro na complementar
                ...
                } elseif (strtolower($cfpess[0]["r11_fersal"]) == "s" && 't' == $cadferia[0]["r30_paga13"]) {
                    $ir_calculado_ferias = false;
                }
              ...
              //echo "<BR> ir_calculado_ferias --> ".($ir_calculado_ferias?'1':'0')."   <---";
                ...
                if (db_selectmax("gerffer_", "select * from gerffer " . bb_condicaosubpes("r31_") . $condicaoaux)) {
                    ...
                    for ($Igerffer = 0; $Igerffer < count($gerffer_); $Igerffer++) {
                        ...
                        } elseif ($gerffer_[$Igerffer]["r31_rubric"] == $cfpess[0]["r11_ferant"]) {
                          // Ferias mes anterior
                        ...
                        } elseif ($gerffer_[$Igerffer]["r31_rubric"] == $cfpess[0]["r11_feabot"]) {
                          // Abono mes anterior
                            if ($opcao_geral == PONTO_SALARIO) {
                                $matriz1 = array();
                                $matriz2 = array();
                        ...
                        } elseif ($gerffer_[$Igerffer]["r31_rubric"] == "R931" ||
                        ...
                        // grava o valor da variavel "valor_ferias_13" na rubrica --> $cfpess[0]["r11_fer13"]
                            ...
                            } elseif ($gerffer_[$Igerffer]["r31_rubric"] == "R932") {
                            ...
                            } elseif ($gerffer_[$Igerffer]["r31_rubric"] == "R940") {
                      ...
                      // Proventos
                        } elseif ($gerffer_[$Igerffer]["r31_pd"] == 1
                        &&
                        substr("#" . $gerffer_[$Igerffer]["r31_rubric"], 1, 1) != "R"
                            ...
                            // vai gerar as 2000 mais um 1/3
                                    ...
                                    if ((strtolower($gerffer_[$Igerffer]["r31_tpp"]) != "d" && $cadferia[0]["r30_paga13"] == 't')
                                    || $cadferia[0]["r30_paga13"] == 'f'
                                        ...
                                        if (strtolower($gerffer_[$Igerffer]["r31_tpp"]) == "a") {
                                            ...
                                            } elseif ($cadferia[0][$r30_proc] < $subpes) {
                                                $OK = true;
                                                $valor_adiantamento_abono_a_descontar  += $gerffer_[$Igerffer]["r31_valor"];
                                            }
                                        }
  ...
  // vai gerar as 2000 mais 1/3
                                    ...
                                    global $transacao;
                                    if ((strtolower($gerffer_[$Igerffer]["r31_tpp"]) == "a" && $cadferia[0]["r30_paga13"] == 't')
                                    || $cadferia[0]["r30_paga13"] == 'f'
                                    ) {
                                          $OK = true;
                                        if (strtolower($gerffer_[$Igerffer]["r31_tpp"]) == "a") {
                                ...
                                } elseif (strtolower($gerffer_[$Igerffer]["r31_tpp"]) == "d") {
                                // adiantamento de ferias
                                    ...
                                    && db_val(substr("#" . $cadferia[0][$r30_proc], 1, 4)) == db_year($cadferia[0][$r30_peri]))
                                      || db_val(substr("#" . $cadferia[0][$r30_proc], 1, 4)) < db_year($cadferia[0][$r30_peri])))
                                        ...
                                        if (isVoltaRedonda() && $DB_instit == 45) {
                                            $baseAbonoFerias = 'B150';

                                            $condicaoaux  = " and r31_regist = " . db_sqlformat($pessoal[$Ipessoal]["r01_regist"]);
                                            $condicaoaux .= " and not exists (
                                                             select 1
                                                             from basesr b
                                                               where b.r09_base = " . db_sqlformat($baseAbonoFerias) . "
                                                               and b.r09_rubric = gerffer.r31_rubric
                                                               and b.r09_instit = gerffer.r31_instit
                                                             )";

                                            if (db_selectmax("gerffer_", "select * from gerffer " . bb_condicaosubpes("r31_") . $condicaoaux)) {
                                                $sql_todas_rubricas = "select r31_rubric, r31_valor from gerffer " . bb_condicaosubpes("r31_") . $condicaoaux;

                                                $result = db_query_params($sql_todas_rubricas, []);
                                                $total_rubricas = 0;

                                                while ($row = pg_fetch_array($result)) {
                                                    if (substr($row['r31_rubric'], 0, 1) != 'R') {
                                                        $total_rubricas += $row['r31_valor'];
                                                    }
                                                }
                                                $gerffer_[$Igerffer]["r31_valor"] = $total_rubricas;
                                                $rub_especial_ferias_abono = $gerffer_[$Igerffer]["r31_valor"] / 3;

                                                $baseAbonoPecunia = 'B250';
                                                $condicaoaux = " and r31_regist = " . db_sqlformat($pessoal[$Ipessoal]["r01_regist"]);
                                                $condicaoaux .= " and exists (
                                                                select 1
                                                                from basesr b
                                                                  where b.r09_base = " . db_sqlformat($baseAbonoPecunia) . "
                                                                  and b.r09_rubric = gerffer.r31_rubric
                                                                  and b.r09_instit = gerffer.r31_instit
                                                                )";

                                                $sSqlAbono = "select sum(r31_valor) as r31_valor from gerffer " . bb_condicaosubpes("r31_") . $condicaoaux;
                                                $rsAbono = db_query($sSqlAbono);

                                                if ($rsAbono != false) {
                                                    $rub_especial_ferias_abono += pg_fetch_object($rsAbono, 0)->r31_valor;
                                                }
                                            }
                                        } else {
                        ...
                        } elseif ($gerffer_[$Igerffer]["r31_pd"] == 2 // Desconto
                        && substr("#" . $gerffer_[$Igerffer]["r31_rubric"], 1, 1) != "R"
                        ) {
                            // sandro
                        ...
                        // Pagar como Férias e recalcula Sim e Pagar 1/3 Não
                                ...
                                $rub_especial_ferias_descontos += $gerffer_[$Igerffer]["r31_valor"];
                                $rub_especial_ferias_adia -= $gerffer_[$Igerffer]["r31_valor"];
                            } elseif (strtolower($gerffer_[$Igerffer]["r31_tpp"]) == "f"
  ...
  // Repassa a rubrica de Pensao Alimentícia de Férias para o salário ou a complementar
...
function gravarRubricasEspeciaisFerias($r30_proc, $r30_peri, $r30_perf, $lCalculoFeriasEfetuado)
{
    ...
    global $anousu, $mesusu, $DB_instit, $valor_adiantamento_abono_a_descontar, $rub_especial_ferias_abono_extra;
            ...
            if (db_substr(db_dtos($cadferia[0][$r30_peri]), 1, 6) > db_strtran($subpes, "/", "") && ('t' ==  $cfpess[0]["r11_recalc"])) {
              // r11_recalc --> Recalcula 1/3 ferias mes gozo
                ...
                if ($rub_especial_ferias_abono > 0) {
                  ...
                  // Abono de ferias:
                    $matriz2[5] = $rub_especial_ferias_abono + $rub_especial_ferias_abono_extra;
                    LogCalculoFolha::write("Inserindo dados na Tabela de Ponto salário");
