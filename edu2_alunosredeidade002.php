...
function retornaTurno($codturma){
  $sql = pg_query("SELECT ed15_c_nome FROM turno INNER JOIN turma ON ed15_i_codigo = ed57_i_turno WHERE ed57_i_codigo = {$codturma}");
  $resultado = pg_fetch_all($sql);
  return trim($resultado[0]["ed15_c_nome"]);
}

function retornaTurma($codturma){
...
$oFiltros->iIdade              = $oParam->iIdade;
$oFiltros->dtDataInicial       = $oParam->dtDataInicial;
$oFiltros->dtDataFinal         = $oParam->dtDataFinal;
...
$oFiltros->iTamanhoEtapa         = 25;
$oFiltros->iTamanhoDataMatricula = 19;
$oFiltros->iTamanhoTurno = 21;
$oFiltros->iTamanhoTurma = 22;
$oFiltros->iTamanhoDataNasc = 19;
...
// if (in_array('xturno', $oFiltros->aCampos)) {
//   $iPosicaoArray = array_search('xturno', $oFiltros->aCampos);  
//   $oFiltros->oTurno                = new stdClass();
//   $oFiltros->oTurno->sDescricao    = 'Turno';
//   $oFiltros->oTurno->iTamanho      = $oFiltros->iTamanhoTurno;
//   $oFiltros->oTurno->sAlinhamento  = $oFiltros->aAlinhamento[$iPosicaoArray];
//   $oFiltros->oTurno->iTipo         = 6;
//   $oFiltros->oNomeAluno->iTamanho         -= $oFiltros->oTurno->iTamanho;
//   $oFiltros->aCabecalho[$iPosicaoArray]    = $oFiltros->oTurno;
// }

// if (in_array('xturma', $oFiltros->aCampos)) {
//   $iPosicaoArray = array_search('xturma', $oFiltros->aCampos);  
//   $oFiltros->oTurma                = new stdClass();
//   $oFiltros->oTurma->sDescricao    = 'Turma';
//   $oFiltros->oTurma->iTamanho      = $oFiltros->iTamanhoTurma;
//   $oFiltros->oTurma->sAlinhamento  = $oFiltros->aAlinhamento[$iPosicaoArray];
//   $oFiltros->oTurma->iTipo         = 7;
//   $oFiltros->oNomeAluno->iTamanho         -= $oFiltros->oTurma->iTamanho;
//   $oFiltros->aCabecalho[$iPosicaoArray]    = $oFiltros->oTurma;
// }

if (in_array('ed47_d_nasc', $oFiltros->aCampos)) {

  $iPosicaoArray = array_search('ed47_d_nasc', $oFiltros->aCampos);
   ...
   * iTipo: Tipo para controle dos dados a serem impressos
   */
  $oFiltros->oDataNasc                = new stdClass();
  $oFiltros->oDataNasc->sDescricao    = 'Dt. Nascimento';
  $oFiltros->oDataNasc->iTamanho      = $oFiltros->iTamanhoDataNasc;
  $oFiltros->oDataNasc->sAlinhamento  = $oFiltros->aAlinhamento[$iPosicaoArray];
  $oFiltros->oDataNasc->iTipo         = 8;
  $oFiltros->oNomeAluno->iTamanho         -= $oFiltros->oDataNasc->iTamanho;
  $oFiltros->aCabecalho[$iPosicaoArray]    = $oFiltros->oDataNasc;
}
...
$sCamposTurma  = "ed57_c_descr, ed18_i_codigo as codigo_escola, ed18_c_nome as nome_escola, ed10_i_codigo as ensino";
$sCamposTurma .= ", ed11_i_codigo as codigo_etapa, ed11_c_descr as descricao_etapa, ed57_i_codigo as codigo_turma";
//$sSqlTurma     = $oDaoTurma->sql_query_turma(null, $sCamposTurma, "ed57_c_descr, ed18_i_codigo", $sWhereEscola);
if($oFiltros->iEnsino == 1){
  $sSqlTurma     = $oDaoTurma->sql_query_turma(null, $sCamposTurma, "nome_escola, ed57_c_descr, ed18_i_codigo", $sWhereEscola);
}else{
  $sSqlTurma     = $oDaoTurma->sql_query_turma(null, $sCamposTurma, "ed57_c_descr, ed18_i_codigo", $sWhereEscola);
}
...
/**
 * NOVO: Array para armazenar o total de alunos por etapa.
 * A chave será o código da etapa e o valor será a contagem.
 */
$oFiltros->aTotalPorEtapa = array();
  ...
  foreach ($aEscola as $iEnsino => $aEnsino) {
    ...
    foreach ($aEnsino as $iTurma => $aTurma) {
      ...
      foreach ($aTurma as $iEtapa => $sEtapa) {
        ...
        $xturno = retornaTurno($iTurma);
        $xturma = retornaTurma($iTurma);
        $totalporturma = 0;
        ...
        foreach ($aAlunos as $oMatricula ) {
          ...
          $dtNasc      = $oMatricula->getAluno()->getDataNascimento();



          if ($dtNasc < $oFiltros->dtDataInicial || $dtNasc > $oFiltros->dtDataFinal){
            continue;
          };
           ...
           */
          //testa($oFiltros->aCabecalho); die("Cabeçalh");
          foreach ($oFiltros->aCabecalho as $oCabecalho) {
            ...
            switch ($oCabecalho->iTipo) {
              ...
              case 6:
                $oPdf->Cell($oCabecalho->iTamanho, 4, $xturno, 1, 0, $oCabecalho->sAlinhamento);
                break;

              case 7:
                $oPdf->Cell($oCabecalho->iTamanho, 4, $xturma, 1, 0, $oCabecalho->sAlinhamento);
                break;
              case 8:
                $dtmat = $oMatricula->getAluno()->getDataNascimento();
                $oPdf->Cell($oCabecalho->iTamanho, 4, $dtmat, 1, 0, $oCabecalho->sAlinhamento);
                break;
            }
          }
          ...
          /**
           * NOVO: Incrementa o contador para a etapa atual.
           * A variável $iEtapa já está disponível pelo loop que chama esta parte do código.
           */
          if (!isset($oFiltros->aTotalPorEtapa[$iEtapa])) {
            $oFiltros->aTotalPorEtapa[$iEtapa] = 0;
          }
          $oFiltros->aTotalPorEtapa[$iEtapa]++;
        }
        $oPdf->SetFont('arial', 'B', $oFiltros->iFonte);
        $oPdf->Cell(192, 4, "Total de Matrículas Turma " . $xturma . ": " . $totalporturma, 1, 1, "R", true);
        $oPdf->SetFont('arial', '', $oFiltros->iFonte);
      }//foreach das turmas
    }
  }
...
/**
 * NOVO: Imprime o totalizador de alunos por etapa no final do relatório.
 * @param PDF $oPdf
 * @param stdClass $oFiltros
 */
function imprimeTotalizadorPorEtapa(PDF $oPdf, $oFiltros) {

  /**
   * Mapeamento dos códigos de etapa para suas descrições, conforme solicitado.
   */
  $aMapaEtapas = array(
    25 => "Berçário",
    26 => "Maternal I",
  ...
  if (empty($oFiltros->aTotalPorEtapa)) {
    return;
  }

  //Define a ordem correta dos códigos.
  $aOrdemCorretaDasEtapas = array(
    25, 26, 27, 28, 23, 24, 29, 30, 31, 32, 33, 1, 2, 3, 4, 5, 9, 6, 7, 8
  );
  
  //Cria um array "molde" com as chaves na ordem certa e valores 0.
  $ordem_com_zeros = array_fill_keys($aOrdemCorretaDasEtapas, 0);

  //Mescla os totais reais sobre o molde. A ordem do primeiro array é mantida.
  $totais_com_ordem = array_replace($ordem_com_zeros, $oFiltros->aTotalPorEtapa);

  //Limpa o array, mantendo apenas as chaves que existem nos totais originais.
  $totais_ordenados_final = array_intersect_key($totais_com_ordem, $oFiltros->aTotalPorEtapa);

  
  $oPdf->Ln();
  $oPdf->SetFont('arial', 'b', $oFiltros->iFonte);
  $oPdf->Cell($oFiltros->iTamanhoMaximoLinha, 4, "Total de Alunos por Etapa:", 1, 1, 'R', 1);

  foreach ($totais_ordenados_final as $iCodigoEtapa => $iTotal) {

    // Busca o nome da etapa no mapa, ou usa o código como padrão
    $sNomeEtapa = isset($aMapaEtapas[$iCodigoEtapa]) ? $aMapaEtapas[$iCodigoEtapa] : "Etapa Cód: {$iCodigoEtapa}";
    $sMensagem  = "{$sNomeEtapa}: {$iTotal}";

    $oPdf->Cell($oFiltros->iTamanhoMaximoLinha, 4, $sMensagem, 1, 1, 'R');
  }
}
...
if ($oFiltros->iEscola == 0) {
  ...
  /**
   * NOVO: Chama a função para imprimir o totalizador por etapa.
   */
  imprimeTotalizadorPorEtapa($oPdf, $oFiltros);
