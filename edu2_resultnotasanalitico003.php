...
$oConfigRelatorio->iColunaNome              = 45*2; // Largura da coluna Aluno // 2018-06-28 Wallace (ATMA) Adicionei 0 * 2
$oConfigRelatorio->iColunaNumero            = 5*2;  // Largura da coluna Nº, S (Situação) e TF (Total de Faltas) // 2018-06-28 Wallace (ATMA) Adicionei 0 * 2
$oConfigRelatorio->iColunaCodigo            = 9*2;  // CódigoAluno // 2018-06-28 Wallace (ATMA) Adicionei 0 * 2
$oConfigRelatorio->iColunaPareceres         = 18; // Coluna Pareceres // 2018-06-28 Wallace (ATMA) Adicionei 0 * 2
...
foreach ($aTurmasSelecionadas as $oTurmaSelecionada) {
  ...
  $oTurmaEtapa->sTipoCalendario = getTipoCalendario($oTurma->getCalendario()->getDescricao());
  ...
  foreach ($oTurma->getProcedimentoDeAvaliacaoDaEtapa($oEtapa)->getElementos() as $oAvaliacaoPeriodicaTurma) {
    ...
    if ($oAvaliacaoPeriodicaTurma->getPeriodoAvaliacao()->getCodigo() != 4) {
      ...
      $oAvaliacaoPeriodica->iPeriodo    = $oAvaliacaoPeriodicaTurma->getPeriodoAvaliacao()->getOrdemPeriodo();
      $aAvalicaoPeriodica[]=$oAvaliacaoPeriodica;
  ...
  foreach ($oTurma->getAlunosMatriculadosNaTurmaPorSerie($oEtapa) as $oMatricula) {
    ...
    $oDadosAluno->sNome               = $oMatricula->getAluno()->getNome();//abreviar($oMatricula->getAluno()->getNome(), 22, true);
    ...
    $aSomaNotas = array();
      ...
      foreach ($aAvalicaoPeriodica as $oAvaliacaoPeriodica ) {
          foreach ($oTurma->getDisciplinasPorEtapa($oEtapa) as $oRegencia) {
              ...
              $oAvaliacao->sNomeDisplina   = $oDisciplinaDiario->getDisciplina()->getNomeDisciplina();
              ...
              if((substr($oTurmaEtapa->sTipoCalendario,0,5) == 'EJA 1' || substr($oTurmaEtapa->sTipoCalendario,0,5) == 'EJA 2') && $oAvaliacaoPeriodica->iPeriodo == 1){
                  ...
                  $aSomaNota1Bim[$stringDisciplina] += $oAvaliacao->lAmparado == true ? 5 : ($oAvaliacao->mAproveitamento->getAproveitamentoReal());

              }else {
                  if (!array_key_exists($stringDisciplina, $aSomaNotas)) {
                      $aSomaNotas[$stringDisciplina] = 0;
                  }
                  $aSomaNotas[$stringDisciplina] += $oAvaliacao->lAmparado == true ? (5 * $oAvaliacaoPeriodica->iPeriodo) : ($oAvaliacao->mAproveitamento->getAproveitamentoReal() * $oAvaliacaoPeriodica->iPeriodo); // 2018-06-26 Wallace (ATMA) Pega a nota do aluno e Multiplica pelo número do bimestre)
              }
          }
      }
      ...
      if(substr($oTurmaEtapa->sTipoCalendario,0,5) == 'EJA 1' || substr($oTurmaEtapa->sTipoCalendario,0,5) == 'EJA 2') {
          ...
          $aSomaNotaDisciplinas = getNotaRestanteRegular($aSomaNotas, $oRegencia->getTurma()->getCalendario()->getAnoExecucao());
      }
      ...
      foreach ($oTurma->getDisciplinasPorEtapa($oEtapa) as $oRegencia) {
          ...
          $oAvaliacao->sNomeDisciplina   = $oDisciplinaDiario->getDisciplina()->getNomeDisciplina();
          $stringDisciplina = strtolower($oAvaliacao->sNomeDisciplina); // 2018-06-26 Wallace (ATMA)
          ...
          $oAvaliacao->mNotaRestante   = $aSomaNotaDisciplinas[$stringDisciplina] <= 0 ? 'APR': $aSomaNotaDisciplinas[$stringDisciplina] ;
...
foreach ($aTurmas as $oTurmaEtapa) {
  ...
  $head1 = "Nota Necessária Para Aprovação";
  ...
  $head7 = "Forma de Avaliação: {$oTurmaEtapa->sNomeFormaAvaliacao}";
  ...
  for ($iPagina = 0; $iPagina < $iPaginas; $iPagina ++) {
    ...
    foreach ($oTurmaEtapa->aAlunos as $oAluno) {
      ...
      $linhaPintada       = $sSituacaoAbreviada != "" ? 1 : 0;
      ...
      if ($oConfigRelatorio->lClassificacaoAlunoTurma) {
        ...
        $oPdf->Cell($oConfigRelatorio->iColunaNumero, $oConfigRelatorio->iAlturaLinha, $oAluno->iClassificacao, 1, 0, 'R',$linhaPintada);
      }
      $oPdf->SetFont("arial", '', 7);
      $oPdf->Cell($iLarguraCelulaNome, $oConfigRelatorio->iAlturaLinha, $oAluno->sNome, 1,0,'',$linhaPintada);

      $oPdf->Cell($oConfigRelatorio->iColunaNumero, $oConfigRelatorio->iAlturaLinha, $sSituacaoAbreviada,   1, 0, 'C',$linhaPintada);
      $oPdf->Cell($oConfigRelatorio->iColunaCodigo, $oConfigRelatorio->iAlturaLinha, $oAluno->iCodigoAluno, 1, 0, 'C',$linhaPintada);


      foreach ($oAluno->aAvaliacao[$iPagina] as $oAvaliacao) {

        $mAvaliacao = $oAvaliacao->mNotaRestante ;
        ...
        if ($oAvaliacao->sTipoAvaliacao == 'NOTA' && $oConfigRelatorio->lCalculaMediaParcial ) {
          ...
          $oPdf->Cell(($iLarguraAvaliacao/2)*2 + 5 , $oConfigRelatorio->iAlturaLinha, $sSituacaoAbreviada != "" ? "" : $oAvaliacao->mNotaRestante, 1, 0, "C", $sSituacaoAbreviada != "" ? 1 : 0);
          $oPdf->SetFont("arial", '', $oConfigRelatorio->iFonteAvaliacao); //Nota parcial não fica em negrito

        } else {
          $oPdf->Cell(($iLarguraAvaliacao/2)*2 + 5, $oConfigRelatorio->iAlturaLinha, $mAvaliacao, 1, 0, "C",$linhaPintada);
        }
        $oPdf->SetFont("arial", '', $oConfigRelatorio->iFonteAvaliacao);
      ...
      $oPdf->Cell($oConfigRelatorio->iColunaNumero, $oConfigRelatorio->iAlturaLinha, "", 0, 1, 'C');
...
function montaQuadroLegendaAssinatura(FPDF $oPdf, $oConfigRelatorio, $lUltimoPeriodo, $oTurmaEtapa) {
  ...
  if (!$lUltimoPeriodo) {
//    $iLarguraQuadro += $oConfigRelatorio->iColunaPareceres; // 2018-06-28 Wallace (ATMA) Retira a coluna pareceres
  }

  $iXInicial = $oPdf->GetX();
  $iYInicial = $oPdf->GetY();
  $oPdf->Rect($oPdf->GetX(), $oPdf->GetY(), $iLarguraQuadro-10, 13);

  if ($oConfigRelatorio->comLegenda) {

    $sLegendasCabeçalho  = "Nº: Número da classificação do aluno;  S: Saída;  ";
    $sLegendasCabeçalho .= "  NNA: Nota necessária para aprovação.";
  ...
  */
  ...
  $oPdf->Line($iXInicial+200, $iYInicial+9, $iXInicial+$iLarguraQuadro-15, $iYInicial+9); // 2018-06-28 Wallace (ATMA) modifica o axis da linha para redimensionar o tamanho
}
...
function adicionaHeader(FPDF $oPdf, $oTurmaEtapa, $oConfigRelatorio, $iPagina) {
  ...
  if ($oConfigRelatorio->lClassificacaoAlunoTurma) {
    ...
    $oPdf->Cell($oConfigRelatorio->iColunaNumero, $oConfigRelatorio->iAlturaLinha, '', "TLR");//wallace
  }
  $oPdf->Cell($iLargura, $oConfigRelatorio->iAlturaLinha, '', 1);

  if (!$oTurmaEtapa->lUltimoPeriodo) {
//    $oPdf->Cell($oConfigRelatorio->iColunaPareceres, $oConfigRelatorio->iAlturaLinha, 'Pareceres', 1, 0, "C"); // 2018-06-28 Wallace (ATMA) Retira o Título coluna pareceres
  }
   ...
   */
//  $oPdf->Cell($oConfigRelatorio->iColunaNumero, $oConfigRelatorio->iAlturaLinha, '', "");
  $oPdf->ln();
  ...
  if (!$oTurmaEtapa->lUltimoPeriodo) {
//    imprimeCelulasParecer($oPdf, $oTurmaEtapa->iLarguraCelulaParecer, $oConfigRelatorio); // 2018-06-28 Wallace (ATMA) Retira o titulo da coluna pareceres
  }
  ...
  foreach ($oTurmaEtapa->aPaginas[$iPagina] as $oRegencia) {
    ...
    if ( $sFormaAvaliacao == 'NOTA' && $oConfigRelatorio->lCalculaMediaParcial ) {
      ...
      $oPdf->Cell(($iLarguraAvaliacao/2)*2 + 5, $oConfigRelatorio->iAlturaLinha, 'NNA', 1, 0, "C");
//      $oPdf->Cell($iLarguraAvaliacao/2, $oConfigRelatorio->iAlturaLinha, 'NP', 1, 0, "C"); // 2018-06-28 Wallace (ATMA) Retira o título da coluna nota parcial
    } else {
      $oPdf->Cell(($iLarguraAvaliacao/2)*2 + 5, $oConfigRelatorio->iAlturaLinha, $sFormaAvaliacao, 1, 0, "C");
    }
//    $oPdf->Cell(5, $oConfigRelatorio->iAlturaLinha, 'Ft.', 1, 0, "C"); // 2018-06-28 Wallace (ATMA) Retira o título da colua
  ...
  for ($i = $iTotalDisciplina; $i < $oConfigRelatorio->iMaximoDisciplinaPagina; $i++) {
    ...
    if ($lPrimeiraLinha) {
      ...
      if ($oTurmaEtapa->sFormaAvaliacao == 'NOTA' && $oConfigRelatorio->lCalculaMediaParcial) {
        ...
        $oPdf->Cell(($iLarguraAvaliacao/2) * 2 +5, $oConfigRelatorio->iAlturaLinha, '', 1);
 ...
 * @param stdClass $oConfigRelatorio
 */
function imprimeCelulasParecer(FPDF $oPdf, $iLarguraCelulaParecer, $oConfigRelatorio,$linhaPintada) {

  $oPdf->Cell($iLarguraCelulaParecer, $oConfigRelatorio->iAlturaLinha, '', 1,'',$linhaPintada);
  $oPdf->Cell($iLarguraCelulaParecer, $oConfigRelatorio->iAlturaLinha, '', 1,'',$linhaPintada);
  $oPdf->Cell($iLarguraCelulaParecer, $oConfigRelatorio->iAlturaLinha, '', 1,'',$linhaPintada);
}
...
//    imprimeCelulasParecer($oPdf, $oTurmaEtapa->iLarguraCelulaParecer, $oConfigRelatorio);
  ...
  $oPdf->Cell($oConfigRelatorio->iColunaNumero, $oConfigRelatorio->iAlturaLinha, '', 0, 1);
...
//        $aMetaNota[$chaveDisciplina] = ArredondamentoNota::arredondar($aMetaNota[$chaveDisciplina],$iAno);
        $aMetaNota[$chaveDisciplina] = round_up($aMetaNota[$chaveDisciplina],1); // ALTERADO PARA SEMPRE ARREDONDAR PARA CIMA A CASA DECIMAL
    }
