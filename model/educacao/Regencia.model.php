...
class Regencia
{
    ...
    public function __construct($iCodigo = null)
    {
        if (!empty($iCodigo)) {
            ...
            if ($iTotalRegencia > 0) {
                ...
                $this->tipoDeBase                   = $oDadosRegencia->ed59_tipobase;
                $this->setUnidadeCurricular($oDadosRegencia->ed59_tipobase);

				$this->areaConhecimento = AreaConhecimentoRegistry::get($oDadosRegencia->ed59_areaconhecimento);
    ...
    public function getDocentes()
    {
     ...
     /**
     * Retorna os docentes que lecionam a Disciplina na turma e estão ativos na data informada.
     * @return Docente[] Colecao de Docentes
     */
    public function getDocentesByData($data)
    {
        if (count($this->aDocentes) == 0 && $this->iCodigo != "") {
            $sWhere = "ed58_i_regencia = {$this->getCodigo()} ";
            $sWhere .= " and ed58_ativo is true";
            //$sWhere .= " and ed58_ativo is true";
            $sWhere .= " and (ed58_datafim >= '$data' OR ed58_datafim is null)";
     ...
     /**
     * Retorna os docentes que lecionam a Disciplina na turma e estão ativos na data atual do sistema.
     * @return Docente[] Colecao de Docentes
     */
    public function getDocentesByDataAtual()
    {
        return $this->getDocentesByData(date('Y-m-d', db_getsession("DB_datausu")));
    }
    ...
    public function adicionarAulasDadasNoPeriodo($iTotalAulas, PeriodoAvaliacao $oPeriodoAvaliacao)
    {
        ...
        $sWhereRegenciaPeriodo .= " AND ed78_i_regencia = {$this->getCodigo()}";
        $oDaoRegenciaPeriodo = new cl_regenciaperiodo();
        $sSqlRegenciaPeriodo = $oDaoRegenciaPeriodo->sql_query_periodo_avaliacao(null, 'ed78_i_codigo', null,
            $sWhereRegenciaPeriodo);
        $rsRegenciaPeriodo = $oDaoRegenciaPeriodo->sql_record($sSqlRegenciaPeriodo);
    ...
    /**
     * @param AreaConhecimento|null $areaConhecimento
     * @return Regencia
     */
    ...
    public function excluirVinculoDiario()
    {
        ...
        if ($rsDiario && pg_num_rows($rsDiario) > 0) {
            ...
            for ($i = 0; $i < $iLinhas; $i++) {
                ...
                $sSqlDiarioAvaliacao = $oDaoDiarioAvaliacao->sql_query_file(null, "ed72_i_codigo", null,
                    " ed72_i_diario = {$iDiario}");
                ...
                $sSqlDiarioFinal = $oDaoDiarioFinal->sql_query_file(null, "ed74_i_codigo", null,
                    "ed74_i_diario = {$iDiario}");
                ...
                if ($oDaoProgressaoParcialAlunoDiarioFinalOrigem->erro_status == 0) {
                    $oMsgErro->sMsgErro = pg_last_error();
                    throw new DBException (_M(MSG_REGENCIA . "erro_excluir_progressaoparcialalunodiariofinalorigem",
                        $oMsgErro));
                }
                ...
                if ($oDaoProgressaoParcialAlunoEncerradoDiario->erro_status == 0) {
                    $oMsgErro->sMsgErro = pg_last_error();
                    throw new DBException (_M(MSG_REGENCIA . "erro_excluir_progressaoparcialalunoencerradodiario",
                        $oMsgErro));
                }
                ...
                $sSqlDiarioResultado = $oDaoDiarioResultado->sql_query_file(null, "ed73_i_codigo", null,
                    " ed73_i_diario = {$iDiario}");
    ...
    /**
     * Valida se a regencia possui vínculo com algum aluno de progressão parcial.
     * @return bool
     * @throws DBException
     */
    protected function existeVinculoComAlunoProgressaoParcial()
    {
        $oDao = new cl_progressaoparcialalunoturmaregencia();
        $sql = $oDao->sql_query_aluno(null, "1", null, " ed115_regencia = {$this->iCodigo}");
        $rs = db_query($sql);
    ...
    {
        if ($this->existeVinculoComAlunoProgressaoParcial()) {
            $msg = "Não é possível excluir a disciplina.\n";
            $msg .= "Existe ao menos um aluno de progressão parcial vínculado a essa regência.\n";
            $msg .= "Para excluir essa regencia é necessário desvincular os alunos da regência.\n";
        ...
        $oDaoDiarioClasseBncc = new cl_diario_classe_bncc();
        ...
        $sSqlDiarioRegenciaAlunoFalta = $oDaoDiarioRegenciaAlunoFalta->sql_query_aluno_falta(null, " ed301_sequencial ",
            null, $sWhereRegenciaHorario);
        ...
        $sSqlRegenciaHorario = $oDaoRegenciaHorario->sql_query_file(null, "ed58_i_codigo", null,
        ...
        //Exclui regencia
        $oDaoDiarioClasseBncc->excluir(null, "ed155_regencia = {$this->iCodigo}");
    ...
    public function salvar()
    {
        ...
        $oDaoRegencia->ed59_areaconhecimento     = "null";
        $oDaoRegencia->ed59_tipobase             = $this->getTipoBase();
        if (!is_null($this->areaConhecimento)) {
            $oDaoRegencia->ed59_areaconhecimento = $this->areaConhecimento->getCodigo();
        }
        $oDaoRegencia->ed59_unidade_curricular = $this->getUnidadeCurricular();
        /**
    ...
    {
        return $this->tipoDeBase;
    }

    public function setTipoBase($idTipo)
    {
        $this->tipoDeBase = $idTipo;
    }
