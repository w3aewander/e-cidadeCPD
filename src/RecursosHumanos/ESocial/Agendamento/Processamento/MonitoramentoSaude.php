...
use ECidade\RecursosHumanos\ESocial\Repository\MonitoramentoSaude as MonitoramentoSaudeRepository;
 ...
 */
class MonitoramentoSaude extends ProcessamentoAbstract implements ProcessamentoInterface
{
    /**
     * @var
     */
    ...
    public function processar()
    {
        ...
        $dados->ano = $this->ano;
        $dados->mes = $this->mes;
        $dados->inscricao_empregador = MonitoramentoSaudeRepository::buscarCNPJEmpregador(
            $oInstituicao->getCgm()->getCodigo()
        );
        $dados->assentamentos = MonitoramentoSaudeRepository::buscarTodosAssentamentosCompetencia(
            $competencia,
            $this->servidores
        );
        $oFormatter = FormatterFactory::get(Tipo::S2220);
        $aDados = $oFormatter->formatar($dados);
