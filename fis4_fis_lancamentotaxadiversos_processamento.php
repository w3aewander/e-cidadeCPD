...
try {
   ...
   */
  $iCodigoSalvo = salvarDados(new cl_fis_lancamentotaxadiversos(),$oPost);
  ...
  db_redireciona('fis4_fis_lancamentotaxadiversos.php' . $sParametros);
} catch (\Exception $e) {

  $aArgumentos             = $_POST;
  $aArgumentos['mensagem'] = urlencode($e->getMessage());
  db_redireciona('fis4_fis_lancamentotaxadiversos.php?' . http_build_query($aArgumentos));
