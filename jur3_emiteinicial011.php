...
if ( !isset($lConsulta) ) {
  ...
  if ($oDaoCertidao->numrows == 0) {
    $sSqlCertidao = $oDaoCertidao->sql_queryCertidao($iCodigoPesquisa, $sTipoPesquisa, $sCampos, true); //se não encontrar tenta com reparcelamento
    $rsCertidao   = $oDaoCertidao->sql_record($sSqlCertidao);
  }
...
if ( $oDaoCertidao->numrows == 1 || isset($lConsulta) ) {
  ...
  if ($oDaoCertidao->numrows == 0) {
    $sSqlCertidao = $oDaoCertidao->sql_queryCertidao($certidao, 'certidao', $sCampos, true); // se não encontrar tenta com reparcelamento
    $rsCertidao   = $oDaoCertidao->sql_record($sSqlCertidao);
  }
...
<fieldset>
  <legend>Dados da Inicial</legend>
    ...
    $repository = new HistoricoDesmembramentoRepository();

      if ($repository->hasHistory($oDadosCertidao->certidao)) {
          $oTabDetalhesCertidao->add("consultaDebitos",
              "Desmembramento",
              "jur3_emiteinicialdebitos.php?sTipoConsulta=desmembramento&certidao=" . $oDadosCertidao->certidao);
      }
