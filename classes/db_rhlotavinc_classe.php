...
class cl_rhlotavinc {
  ...
  function sql_query_servidores($iAnoFolha, $iMesFolha, $iCodigoRecurso, $sCampos, $iInstituicao = null) {
    ...
    function sql_query2($rh25_codlotavinc = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "
        select {$campos}
          from rhlotavinc
              join orctiporec on orctiporec.o15_codigo = rhlotavinc.rh25_recurso
              join fonterecurso on fonterecurso.orctiporec_id = orctiporec.o15_codigo
                   and fonterecurso.exercicio = rhlotavinc.rh25_anousu
              join complementofonterecurso on complementofonterecurso.o200_sequencial = orctiporec.o15_complemento
              join orcprojativ on orcprojativ.o55_anousu = rhlotavinc.rh25_anousu and  orcprojativ.o55_projativ = rhlotavinc.rh25_projativ
              join rhlota on rhlota.r70_codigo = rhlotavinc.rh25_codigo
              join db_estruturavalor on db_estruturavalor.db121_sequencial = orctiporec.o15_db_estruturavalor
              join db_config on db_config.codigo = orcprojativ.o55_instit
              join orcproduto on orcproduto.o22_codproduto = orcprojativ.o55_orcproduto
              join cgm on cgm.z01_numcgm = rhlota.r70_numcgm
              join db_estrutura as a on a.db77_codestrut = rhlota.r70_codestrut
              join concarpeculiar on concarpeculiar.c58_sequencial = rhlota.r70_concarpeculiar
              left join orcfuncao on orcfuncao.o52_funcao = rhlotavinc.rh25_funcao
              left join orcsubfuncao on orcsubfuncao.o53_subfuncao = rhlotavinc.rh25_subfuncao
              left join orcprograma on orcprograma.o54_anousu = rhlotavinc.rh25_anousu and  orcprograma.o54_programa = rhlotavinc.rh25_programa

         ";
        $sql2 = "";
        if (empty($dbwhere)) {
            if (!empty($rh25_codlotavinc)) {
                $sql2 .= " where rhlotavinc.rh25_codlotavinc = {$rh25_codlotavinc} ";
            }
