...
if (1 == $tipo) {
    ...
    for ($x = 0; $x < pg_numrows($result); ++$x) {
        ...
        if ($int_ano != $v01_exerc) {
            if (0 != $int_ano && $int_ano != $v01_exerc) {
                $pdf->SetFont('Arial', 'B', 7);
        ...
        $pdf->Cell(60, 5, substr($v03_descr, 0, 40), 0, 0, 'L', 0);
        if ('c' == $tiporel) {
    ...
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->Cell(85, 5, 'DÍVIDA ATIVA TRIBUTÁRIA:', 'T', 0, 'L', 0);
    ...
    $pdf->Cell(85, 5, 'DÍVIDA ATIVA NÃO TRIBUTÁRIA:', 0, 0, 'L', 0);
    ...
    if (false != $clabre_arquivo->arquivo) {
        ...
        if ('c' == $tiporel) {
            ...
            fputs($clabre_arquivo->arquivo, 'nome;');
        }
