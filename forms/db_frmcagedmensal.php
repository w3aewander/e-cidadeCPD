...
function js_controla_anomes(opcao){
  ...
  anofol = new Number("<?=DBPessoal::getAnoFolha()?>");
  mesfol = new Number("<?=DBPessoal::getMesFolha()?>");
  erro = 0;
  ...
  if(erro > 0){
    if(opcao == "a"){
      document.form1.anousu.value = "<?=DBPessoal::getAnoFolha()?>";
      document.form1.anousu.focus();
    }else{
      document.form1.mesusu.value = "<?=DBPessoal::getMesFolha()?>";
