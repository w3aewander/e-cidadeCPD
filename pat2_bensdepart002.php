...
//  }
  ...
  if ($pdf->gety() > $pdf->h - 30 || $troca != 0 ) {
    ...
    if ($opcao_obs == "S") {
      $pdf->cell(15,$alt,$RLt52_ident,1,0,"C",1);    
      $pdf->cell(75,$alt,"Descrição",1,0,"C",1);
      $pdf->cell(15,$alt,$RLt52_dtaqu,1,0,"C",1);
      if ($flag_valor == true){
        $pdf->cell(15,$alt,"Valor",1,0,"C",1);
      }    
      $pdf->cell(15,$alt,"Processo",1,0,"C",1);    
      $pdf->cell(10,$alt,"Baixado",1,0,"C",1);    
      $pdf->cell(40,$alt,"OBS",1,1,"C",1);
    } else{
...
//  $pdf->cell(15,$alt,$placaIdentificacao,0,1,"L",0);
  ...
  if($opcao_obs == "S"){   
    ...
    $current_y = $pdf->GetY();
    $current_x = $pdf->GetX();
    $pdf->MultiCell(15, 4, $t52_ident, 0, 'C');
    $end_y = $pdf->GetY();

    $current_x = $current_x + 15;
    $pdf->SetXY($current_x, $current_y);
    $pdf->MultiCell(75, 4, $t52_descr, 0, 'L');
    $end_y = ($pdf->GetY() > $end_y)?$pdf->GetY() : $end_y;

    $current_x = $current_x + 75;
    $pdf->SetXY($current_x, $current_y); 
    $pdf->MultiCell(15, 4, db_formatar($t52_dtaqu,"d"), 0, 'C');
    $end_y = ($pdf->GetY() > $end_y)?$pdf->GetY() : $end_y;

    $current_x = $current_x + 15;
    $pdf->SetXY($current_x, $current_y); 
    $pdf->MultiCell(15, 4, db_formatar($t52_valaqu,"f"), 0, 'C');
    $end_y = ($pdf->GetY() > $end_y)?$pdf->GetY() : $end_y;

    $current_x = $current_x + 15;
    $pdf->SetXY($current_x, $current_y);
    //$pdf->MultiCell(20, 4, $num_processo_adm, 0, 'L');
    $pdf->MultiCell(15, 4, ($num_processo_adm == "undefined") ? "" : $num_processo_adm, 0, 'C');
    $end_y = ($pdf->GetY() > $end_y)?$pdf->GetY() : $end_y;

    $current_x = $current_x + 15;
    $pdf->SetXY($current_x, $current_y);
    $pdf->MultiCell(10, 4, $baix, 0, 'C');
    $end_y = ($pdf->GetY() > $end_y)?$pdf->GetY() : $end_y;

    $current_x = $current_x + 10;
    $pdf->SetXY($current_x, $current_y);
    $pdf->MultiCell(40, 4, $t52_obs, 0, 'L');
    $end_y = ($pdf->GetY() > $end_y)?$pdf->GetY() : $end_y;
  ...
  //if obs
    ...
    $pdf->SetXY($current_x, $current_y);
    //$pdf->MultiCell(20, 4, $num_processo_adm, 0, 'L');
    $pdf->MultiCell(20, 4, ($num_processo_adm == "undefined") ? "" : $num_processo_adm, 0, 'L');
    $end_y = ($pdf->GetY() > $end_y)?$pdf->GetY() : $end_y;
