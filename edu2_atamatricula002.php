...
$aDataRelatorio = isset($oGet->dtRelatorio) ? $oGet->dtRelatorio : null;
...
function montaStdClassDadosAluno(Matricula $oMatricula) {
  ...
  $oDadosAluno   = new stdClass();
    ...
    if (empty($oMatricula->getAluno()->getDataNascimento())) {
        $msg = "Foram detectadas inconsistências nas datas de nascimento de um ou mais alunos, ";
        $msg .= "favor acessar a rotina: Relatórios > Turmas > Lista das Turmas > Relatório Editável para análise.";
...
$oDataImpressao = is_null($aDataRelatorio) ? new DBDate(date('Y-m-d')): new DBDate($aDataRelatorio);
