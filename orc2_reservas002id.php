...
function voltaDadosUsuario($codreserva){
	$sql = pg_query("SELECT d.id_usuario, login, datahr from db_acount d inner join db_syscampo c on c.codcam = d.codcam inner join db_usuarios u on u.id_usuario = d.id_usuario where d.codarq = 788 and d.id_acount in ( select id_acount from db_acountkey where ( id_codcam = 5313 and campotext = '{$codreserva}' ) group by id_acount having count(*) = 1 ) and d.codcam = 5313 order by id_acount");
	$resultado = pg_fetch_all($sql);
	return $resultado[0];
}

function voltaDeps($idusuario){
	$inst = db_getsession("DB_instit");
	$hoje = date("Y-m-d");
	$sql = pg_query("SELECT db_depart.coddepto, db_depart.descrdepto from db_depusu inner join db_usuarios on db_usuarios.id_usuario = db_depusu.id_usuario inner join db_depart on db_depart.coddepto = db_depusu.coddepto where db_depart.instit = {$inst} and (limite is null or limite >= '{$hoje}') and db_depusu.id_usuario = {$idusuario} order by db_depusu.db17_ordem");
	$resultado = pg_fetch_all($sql);
	return $resultado;
}
...
/*$sql = "select distinct 
               ...
               orcreserva.*,
				       ...
				       o46_codlei,
				       o38_descr,
				       db_acount.id_usuario, db_usuarios.login 
				       from orcreserva
					     inner join orcdotacao   w        on w.o58_anousu    = o80_anousu
					                                     and w.o58_coddot    = o80_coddot 
					     inner join orcorgao     o        on o.o40_anousu    = w.o58_anousu 
               ...
               left  join solicitemele          on pc11_codigo     = pc18_solicitem
               left  join orcelemento dsol      on dsol.o56_codele = pc18_codele 
               INNER JOIN db_acountkey ON o80_codres = campotext::integer INNER JOIN db_acount ON db_acountkey.id_acount = db_acount.id_acount AND db_acount.codcam = 5313 INNER JOIN db_usuarios ON db_acount.id_usuario = db_usuarios.id_usuario 
 	  		 where o80_anousu = ".db_getsession("DB_anousu")." 
 	  		   and ".$clselorcdotacao->getDados()." 
 	  		   and w.o58_instit in (".$instits.")
 	  		   and o80_dtlanc <= '{$sDataFinal}'
...
$sql .= " order by db_usuarios.login, tiporeserva,{$sCampos} ";
*/
	                ...
	                foreach ($aDadosElemento as $sRecurso => $aDadosRecursos) {
	                  ...
	                  foreach ($aDadosRecursos['aReservas'] as $iCodReserva => $aReserva) {
	                    ...
	                    if ( $forma_impressao == 'a' ) {	                    	
	                    	...
	                    	$dadosusuario = voltaDadosUsuario($iCodReserva);
	                    	//testa($dadosusuario); die("Confere");

	                    	if($dadosusuario){
	                    		$idusuario = $dadosusuario["id_usuario"];

	                    		$datainsere = date("d/m/Y", $dadosusuario["datahr"]);
	                    		$logininsere = $dadosusuario["login"];
	                    		/*$departamentos = "";
	                    		$listadeps = voltaDeps($idusuario);
	                    		foreach ($listadeps as $linha) {
	                    			$departamentos .= $linha["coddepto"] . " - " . $linha["descrdepto"] . "; ";
	                    		}
	                    		$departamentos = rtrim($departamentos, "; ");
	                    		$xxx = "";
	                    		*/

	                    		if($idusuario != $guardausuario){
	                    			
	                    			if($cc != 1){
	                    				$pdf->setfont('arial','b',8);
	                    				$pdf->cell(20,$alt,"",'TBR',0,'C');
	                      				$pdf->cell(20,$alt,"",'TBR',0,'C');
	                      				$guardatotal = db_formatar($guardatotal,'f');
	                      				$pdf->cell(20,$alt,"Total: ",'TBR',0,"R");
	                      				$pdf->multicell(130,$alt,$guardatotal,'TB','L');
	                    				$guardatotal = 0;
	                    				$guardausuario2 = $idusuario;
	                    			}
	                    			...
	                    			$pdf->cell(20,$alt,"",'TBR',0,'C');
	                      			$pdf->cell(20,$alt,"",'TBR',0,'C');
	                      			$pdf->cell(20,$alt,$logininsere,'TBR',0,"R");
	                      			$pdf->multicell(130,$alt, $xxx,'TB','L');

	                    			$guardausuario = $idusuario;
	                    			$guardatotal = 0;
	                    			$cc++;
	                    		}
	                    	}
