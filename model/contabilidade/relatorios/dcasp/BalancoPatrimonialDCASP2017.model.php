...
class BalancoPatrimonialDCASP2017 extends RelatoriosLegaisBase
{
     ...
     * @var boolean
     */
    protected $lExibirExercicioAnterior;

    /**
     * Linhas que não devem serem impressa no pdf
     * @var int[]
     */
    protected $linhasOcultarImpressao = [];
     ...
     */
    protected function adicionarPagina($sNomeQuadro, $sNomeColuna)
    {
     ...
     * @return bool
     */
    protected function verificaTotalizadorFinal($iLinha)
    {
     ...
     */
    protected function escreverLinha(stdClass $oLinha)
    {
        ...
        $larguraDescricao = $this->iLargura * $nPorcemtagemDescricao;
        $this->oPdf->Cell($larguraDescricao, $this->iAltura, $sDescricao, $sBorda, 0, 'L');
        $this->oPdf->Cell(
            $this->iLargura * 0.20,
            $this->iAltura,
     ...
     */
    protected function escreveAssinatura($sNomeQuadro, $sNomeColuna)
    {
     ...
     * @return string
     */
    protected function getDescricaoPeriodo()
    {
     ...
     */
    protected function preparaCabecalhos()
    {
     ...
     * @return bool
     */
    protected function exibirQuadroRelatorio($iCodigo)
    {
     ...
     * @return stdClass
     */
    protected function processarLinha($iLinha, $iLinhaFinalQuadro)
    {
     ...
     */
    protected function processarQuadros()
    {

        if ($this->exibirQuadroRelatorio(static::QUADRO_PRINCIPAL)) {

            for ($i = static::QUADRO_PRINCIPAL_INICIAL; $i <= static::QUADRO_PRINCIPAL_FINAL; $i++) {
                $this->aQuadroPrincipal[] = $this->processarLinha($i, static::QUADRO_PRINCIPAL_FINAL);
            }
        }

        if ($this->exibirQuadroRelatorio(static::QUADRO_ATIVOS_PASSIVOS)) {
            $inicial = $this->linhas['balanco_patrimonial']['inicio']; //static::QUADRO_ATIVOS_PASSIVOS_INICIAL;
            $final = $this->linhas['balanco_patrimonial']['final']; //static::QUADRO_ATIVOS_PASSIVOS_INICIAL;
            for ($i = $inicial; $i <= $final; $i++) {
                $this->aQuadroAtivosPassivos[] = $this->processarLinha($i, $final);
            }
        }

        if ($this->exibirQuadroRelatorio(static::QUADRO_CONTAS_COMPENSACAO)) {

            $inicial = $this->linhas['contas_compensacao']['inicio'];
            $final = $this->linhas['contas_compensacao']['final'];
            for ($i = $inicial; $i <= $final; $i++) {
                $this->aQuadroContasCompensacao[] = $this->processarLinha($i, $final);
            }
        }
    }
     ...
     */
    protected function configurarPdf()
    {
     ...
     * @param string $sNomeColuna Nome da coluna de descrição do quadro.
     */
    protected function escreverCabecalhoQuadro($sNomeColuna)
    {
     ...
     */
    protected function emitirQuadro($sNomeQuadro, $sNomeColuna, $aDadosQuadro)
    {
        /**
        ...
        foreach ($aDadosQuadro as $oLinha) {
            if (in_array($oLinha->ordem, $this->linhasOcultarImpressao)) {
                continue;
            }

            if ($oLinha->ordem == static::QUADRO_PRINCIPAL_INICIO_PASSIVOS) {
        ...
        /*
         * retirado as assinaturas abaixo dos quadros, deverá ser chamada no
         * model do ano antes do showpdf
         */
     ...
     * @param string $sNomeColuna Nome da coluna de descrição do quadro.
     */
    protected function escreverNotaExplicativa($sNomeQuadro, $sNomeColuna)
    {
     ...
     * @throws DBException
     */
    protected function getValorDisponibilidadeFinanceira($iAno)
    {
     ...
     * @return float
     */
    protected function calcularTotal(stdClass $oStdValor)
    {
     ...
     */
    protected function getDadosQuadros()
    {
    ...
    public function emitir()
    {
        ...
        $this->emitirQuadro("QUADRO DE ATIVOS E PASSIVOS FINANCEIROS E PERMANENTES\n(Lei nº 4.320/1964)", "",
            $this->aQuadroAtivosPassivos);
        $this->emitirQuadro("QUADRO DE CONTAS DE COMPENSAÇÃO\n(Lei nº 4.320/1964)", "",
            $this->aQuadroContasCompensacao);
        $this->emitirQuadro("QUADRO DE SUPERÁVIT/DÉFICIT FINANCEIRO\n(Lei nº 4.320/1964)", "FONTES DE RECURSOS",
            $this->aQuadroSuperavitDeficit);
    ...
    public function getQuadroPrincipal()
    {
        return static::QUADRO_PRINCIPAL;
    }

    public function getQuadroAtivosPassivos()
    {
        return static::QUADRO_ATIVOS_PASSIVOS;
    }

    public function getQuadroContasCompensacao()
    {
        return static::QUADRO_CONTAS_COMPENSACAO;
    }

    public function getQuadroSuperavit()
    {
        return static::QUADRO_SUPERAVIT;
    }
}
