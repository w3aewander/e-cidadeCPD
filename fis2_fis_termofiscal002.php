...
require_once modification("classes/db_fis_termovist_classe.php");
require_once modification("dbforms/db_funcoes.php");
db_postmemory($_SERVER);

$clparfiscal     = new cl_fis_parfiscal;
$cltermovist     = new cl_fis_termovist;
//----------------------- Data atual ----------------------------
...
for($cont=0;$cont < $int_linhas;$cont++) {
  ...
  if(isset($reemis) && $reemis == 's' ){
    ...
    if($numrowstipo==0 && $primeiro == 't'){
      $rsTermovist = db_query("select last_value+1 as termovist from fis_fis_termovist_y91_termovist_seq");
      $numrows = pg_num_rows($rsTermovist);
      if ($numrows>0){
        db_fieldsmemory($rsTermovist,0);
      }
      if(!isset($numeroDoTermo) || $numeroDoTermo == ""){
        $numeroDoTermo = $termovist;
      }
  ...
  where db03_tipodoc = 1013 and db03_instit = " . db_getsession("DB_instit")." order by db04_ordem ";
    ...
    if(isset($numeroDoTermo) && $numeroDoTermo != ""){
      $pdf->cell(155,10,mb_strtoupper($titulo)." N° ".$numeroDoTermo,0,1,"C",0);
    } else {
      $pdf->cell(155,10,mb_strtoupper($titulo)." N° ".$y91_termovist,0,1,"C",0);
    }
    ...
    for($i=0;$i<$numrows;$i++){
      ...
      if (isset($db02_descr) && $db02_descr == 'TERMO VISTORIA PARAG 1'){
        ...
        $pdf->setfont('Arial','',10);
        $pdf->multicell(100,5,$q03_ativ.' - '.$q03_descr,0,"J",0);
        $pdf->setx($col);
        $pdf->setfont('Arial','B',10);
        ...
        for( $y=0; $y<$int_linhas_isscalc; $y++ ){
          ...
          if( ( $q01_cadcal == 2 or $q01_cadcal == 3 ) and empty( $str_issqn ) ){ //2 ISS FIXO - 3 ISS VAR
        ...
        //************** TFV... alvara **************
        $sqltfv = "
		select y69_numpre,y71_codvist as visttfv, y70_obs as obs_vist
		from fiscalizacao.fis_vistorias 
		inner join fiscalizacao.fis_vistinscr       on y71_codvist = y70_codvist
		inner join fiscalizacao.fis_vistorianumpre  on y69_codvist = y70_codvist
		where fis_vistinscr.y71_inscr = $q02_inscr 
		  and y70_instit = ".db_getsession('DB_instit')."
		  and not exists (select 1 from fiscalizacao.fis_vistoriasanu where y28_codvist = y70_codvist)		
        ...
        if($linhastfv>0){
          ...
          $sqldebtfv="
        select  
        fis_vistorias.y70_data  as datatfv,
        ...
        from fiscalizacao.fis_vistorias 
        where fis_vistorias.y70_codvist= $visttfv and  y70_instit = ".db_getsession('DB_instit')  ;
		...
		from fiscalizacao.fis_sanitarioinscr 
		inner join fiscalizacao.fis_vistsanitario   on y74_codsani = y18_codsani
		inner join fiscalizacao.fis_vistorianumpre  on y69_codvist = y74_codvist
		inner join fiscalizacao.fis_vistorias       on y74_codvist = y70_codvist
        ...
        if($linhastfs>0){
          ...
          $sqldebtfs="
        select  
        fis_vistorias.y70_data  as datatfs,
        (select k00_numpre from arrecad        where k00_numpre = $y69_numpre limit 1) as arrecad,
        (select k00_numpre from arrecant       where k00_numpre = $y69_numpre limit 1) as arrecant,
        (select k00_numpre from arreold        where k00_numpre = $y69_numpre limit 1) as arreold,
        (select k00_numpre from arrepaga       where k00_numpre = $y69_numpre limit 1) as arrepaga,
        ...
        (select arrepaga.k00_numpre 
          ...
          where divold.k10_numpre = $y69_numpre limit 1 ) as divpaga        
        from fiscalizacao.fis_vistorias 
        where fis_vistorias.y70_codvist= $visttfs and y70_instit = ".db_getsession('DB_instit')." ";
