...
?>
<br />
<form name="form1" method="post" action="" onload="exibePDV()" >
<center>
	  ...
	  <tr>
        ...
        <tr>
            ...
            <td colspan=2>
                ...
                $tipos[0] = "Selecione";
                ksort($tipos).
                db_select("r59_motivoesocial", $tipos, true, $db_opcao, "onChange=exibePDV()");
                ?>
            </td>
        </tr>
		<tr>
		...
		<tr id="trpdv">
            <td nowrap title="<?=@$Tr59_demissaovoluntaria?>">
                <?=@$Lr59_demissaovoluntaria?>
            </td>
            <td colspan=2>
                <?php
                $opcoesDemissaoVoluntaria = ['t' => 'Sim', 'f' => 'Não'];
                db_select("r59_demissaovoluntaria", $opcoesDemissaoVoluntaria, true, $db_opcao);
                ?>
            </td>
        </tr>
	  <tr>
...
function exibePDV() {
	let valoresNaoPermitidos = [10, 11, 12, 13, 28, 29, 30, 34, 36, 37, 40, 43, 44];
	let length = valoresNaoPermitidos.length;
	let exibe = true;
    for(var i = 0; i < length; i++) {
		if (valoresNaoPermitidos[i] == document.form1.r59_motivoesocial.value) {
			exibe = false;
		}
    }
	if (exibe == true) {
		document.getElementById('trpdv').style.display='';
	} else {
		document.getElementById('trpdv').style.display='none';
		document.form1.r59_demissaovoluntaria.value = 'f';
	}
}
</script>
<?php
