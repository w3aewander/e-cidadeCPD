  ...
  if ($sqlerro == false){
    ...
    if ($sqlerro == false) {
      ...
      if($l20_numero > $l25_numero) {
        ...
        $clpccflicitapar->l25_numero=$l20_numero;
    ...
    $sqlveriflicitamod = $clpccflicitapar->sql_query_mod_licita(null,
                                                                "l44_sigla, l25_numero as xx",
                                                                null,
                                                                "l20_instit=$instit 
                                                                 and l25_anousu=$anousu 
                                                                 and l20_codtipocom=$l20_codtipocom 
                                                                 and l20_numero=$l20_numero 
                                                                 and l20_anousu=$anousu");
    $result_verif_licitamod=$clpccflicitapar->sql_record( $sqlveriflicitamod );

    if ($clpccflicitapar->numrows>0){
      db_fieldsmemory($result_verif_licitamod,0,2);
      $erro_msg="Já existe licitação número $l20_numero para modalidade $l44_sigla no exercício de $anousu.";
      $sqlerro = true;
    }
...
/*
    $totalresultado = pg_fetch_all($result_verif_licitaedital);
    $verifica = pg_query("SELECT l20_edital as yy from pccflicitanum inner join liclicita on liclicita.l20_edital = pccflicitanum.l24_numero inner join pccflicitapar on pccflicitapar.l25_codcflicita = liclicita.l20_codtipocom where l20_instit = {$instit} and l25_anousu = {$anousu} and l20_edital = {$edital} and l20_anousu = {$anousu}");
    $resultadoverifica = pg_fetch_all($verifica);
    var_dump($resultadoverifica);
    die("confere 4");
*/
    
    
    if ($clpccflicitanum->numrows>0){
      //var_dump($clpccflicitanum->sql_query_edital(null,"l20_edital as yy",null,"l20_instit=$instit and l25_anousu=$anousu and l20_edital=$edital and l20_anousu=$anousu"));
