...
function verificaIntegral($codigoperiodo){  
  $sql = pg_query("SELECT ed15_c_nome FROM turno INNER JOIN periodoescola ON ed17_i_turno = ed15_i_codigo WHERE ed17_i_codigo = {$codigoperiodo}");  
  $resultado = pg_fetch_all($sql);
  return (trim($resultado[0]["ed15_c_nome"]) == "INTEGRAL") ? true : false;
}
...
if ($oPost->sAction == 'PesquisaPeriodo') {
  ...
  $result     = $clperiodoescola->sql_record($clperiodoescola->sql_query("", $sCampos,"ed15_i_sequencia,ed08_i_sequencia", $sWhere));
  //var_dump($clperiodoescola->sql_query("", $sCampos,"ed15_i_sequencia,ed08_i_sequencia", $sWhere)); die("Confere");
...
if ($oPost->sAction == 'IncluirPeriodo') {
  $integral = verificaIntegral($oPost->periodos);
  ...
  try {
    ...
    for ( $iContador = 0; $iContador < count($aPeriodosInformados); $iContador++ ) {
      ...
      foreach ( $aAgendaProfissionalValida as $oAgendaProfissional ) {
        ...
        //var_dump($oAgendaProfissional->getTurnoReferente()); echo "<br>";
        //var_dump($oPeriodoEscola->getTurno()->getTurnoReferente()); echo "<br>";
        ...
        //var_dump($oPeriodoEscola->getHoraInicio()); echo "<br>";
        //var_dump($oAgendaProfissional->getHoraInicio()); echo "<br>";
        //var_dump($oPeriodoEscola->getHoraFim()); echo "<br>";
        //var_dump($oAgendaProfissional->getHoraFim()); echo "<br>";
        ...
        //if ( ($iHoraInicioPeriodo >= $iHoraInicioAgenda) && ( $iHoraFimPeriodo <= $iHoraFimAgenda) ) {
          if ( (($iHoraInicioPeriodo >= $iHoraInicioAgenda) && ( $iHoraFimPeriodo <= $iHoraFimAgenda)) || $integral) {
          ...
          $sWhereConflito  .= " AND ed15_c_nome != 'INTEGRAL'    ";
          ...
          if ( $iLinhas > 0 && !$integral) {
            ...
            $clrechumanohoradisp->ed33_rechumanoativ =  $oPost->rechumanoativ;
    ...
    for ($x = 0; $x < count($quebra_periodos); $x++) {
      ...
      $result2   = $clrechumanohoradisp->sql_record($clrechumanohoradisp->sql_query_disponibilidade("",
        ...
        $mensagem .= $sep_msg.urlencode($msg_erro);
          ...
          $mensagem .= $sep_msg.urlencode("Erro ao incluir alguns períodos informado.");
            $erro      = true;
          }
