...
/**
 * Class cl_orcparamseqcolunaestruturais
 */
class cl_orcparamseqcolunaestruturais
{
    /**
    ...
    public $campos = "
                 ...
                 o158_estrutural = varchar(255) = Estrutural 
    ...
    /**
     * cl_orcparamseqcolunaestruturais constructor.
     */
    public function __construct()
    {
        $this->rotulo = new rotulo("orcparamseqcolunaestruturais");
    ...
    public function incluir($o158_sequencial)
    {
        ...
        if ($this->o158_estrutural == null) {
            $this->erro_sql = " Campo Estrutural não informado.";
            $this->erro_campo = "o158_estrutural";
            $this->erro_banco = "";
            $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
            $this->erro_msg .= str_replace(
                '"',
        ...
        if ($o158_sequencial == "" || $o158_sequencial == null) {
            $result = db_query("select nextval('orcparamseqcolunaestruturais_o158_sequencial_seq')");
            if ($result == false) {
                $this->erro_banco = str_replace("\n", "", @pg_last_error());
                $this->erro_sql = "Verifique o cadastro da sequencia: orcparamseqcolunaestruturais_o158_sequencial_seq do campo: o158_sequencial";
                $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                $this->erro_msg .= str_replace(
                    '"',
                    "",
        ...
        $sql = "insert into orcparamseqcolunaestruturais(
        ...
        if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
            ...
            if (($resaco != false) || ($this->numrows != 0)) {
                ...
                $resac = db_query("insert into db_acount values($acount,1010423,1010353,'','" . AddSlashes(pg_result(
                        $resaco,
                        0,
                        'o158_orcparamseqcoluna'
                    )) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                $resac = db_query("insert into db_acount values($acount,1010423,1010352,'','" . AddSlashes(pg_result(
                        $resaco,
                        0,
                        'o158_estrutural'
                    )) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                $resac = db_query("insert into db_acount values($acount,1010423,1010351,'','" . AddSlashes(pg_result(
                        $resaco,
                        0,
                        'o158_exclusao'
                    )) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                $resac = db_query("insert into db_acount values($acount,1010423,1010350,'','" . AddSlashes(pg_result(
                        $resaco,
                        0,
                        'o158_sequencial'
                    )) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                $resac = db_query("insert into db_acount values($acount,1010423,1010358,'','" . AddSlashes(pg_result(
                        $resaco,
                        0,
                        'o158_ano'
                    )) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
        ...
        if (!empty($o158_sequencial)) {
            db_query("SELECT setval('orcparamseqcolunaestruturais_o158_sequencial_seq', (SELECT max(o158_sequencial) FROM orcparamseqcolunaestruturais))");
        }
        return true;
    }
    ...
    public function atualizacampos($exclusao = false)
    {
        if ($exclusao == false) {
            ...
            $this->o158_estrutural = ($this->o158_estrutural == "" ? @$GLOBALS["HTTP_POST_VARS"]["o158_estrutural"] : $this->o158_estrutural);
            $this->o158_exclusao = $this->o158_exclusao ?: null;
    ...
    public function sql_record($sql)
    {
        ...
        if ($this->numrows == 0) {
            $this->erro_banco = "";
            $this->erro_sql = "Record Vazio na Tabela:orcparamseqcolunaestruturais";
            $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
            $this->erro_msg .= str_replace(
                '"',
                "",
    ...
    {
        $sql = "select {$campos} ";
        $sql .= "  from orcparamseqcolunaestruturais ";
        $sql2 = "";
        if (empty($dbwhere)) {
            if (!empty($o158_sequencial)) {
                $sql2 .= " where orcparamseqcolunaestruturais.o158_sequencial = $o158_sequencial ";
            }
        } else {
            if (!empty($dbwhere)) {
                $sql2 = " where $dbwhere";
    ...
    {
        $this->atualizacampos();
        $sql = " UPDATE orcparamseqcolunaestruturais SET ";
        $virgula = "";
        ...
        if (trim($this->o158_estrutural) != "" || isset($GLOBALS["HTTP_POST_VARS"]["o158_estrutural"])) {
            $sql .= $virgula . " o158_estrutural = '$this->o158_estrutural' ";
            $virgula = ",";
            if (trim($this->o158_estrutural) == null) {
                $this->erro_sql = " Campo Estrutural não informado.";
                $this->erro_campo = "o158_estrutural";
                $this->erro_banco = "";
                $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                ...
                for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
                    ...
                    if (isset($GLOBALS["HTTP_POST_VARS"]["o158_orcparamseqcoluna"]) || $this->o158_orcparamseqcoluna != "") {
                        $resac = db_query("insert into db_acount values($acount,1010423,1010353,'" . AddSlashes(pg_result(
                                ...
                                $conresaco,
                                'o158_orcparamseqcoluna'
                            )) . "','$this->o158_orcparamseqcoluna'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                    }
                    if (isset($GLOBALS["HTTP_POST_VARS"]["o158_estrutural"]) || $this->o158_estrutural != "") {
                        $resac = db_query("insert into db_acount values($acount,1010423,1010352,'" . AddSlashes(pg_result(
                                $resaco,
                                $conresaco,
                                'o158_estrutural'
                            )) . "','$this->o158_estrutural'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                    }
                    if (isset($GLOBALS["HTTP_POST_VARS"]["o158_exclusao"]) || $this->o158_exclusao != "") {
                        $resac = db_query("insert into db_acount values($acount,1010423,1010351,'" . AddSlashes(pg_result(
                                ...
                                $conresaco,
                                'o158_exclusao'
                            )) . "','$this->o158_exclusao'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                    }
                    if (isset($GLOBALS["HTTP_POST_VARS"]["o158_sequencial"]) || $this->o158_sequencial != "") {
                        $resac = db_query("insert into db_acount values($acount,1010423,1010350,'" . AddSlashes(pg_result(
                                ...
                                $conresaco,
                                'o158_sequencial'
                            )) . "','$this->o158_sequencial'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                    }
                    if (isset($GLOBALS["HTTP_POST_VARS"]["o158_ano"]) || $this->o158_ano != "") {
                        $resac = db_query("insert into db_acount values($acount,1010423,1010358,'" . AddSlashes(pg_result(
                                ...
                                $conresaco,
                                'o158_ano'
                            )) . "','$this->o158_ano'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                    }
                }
            }
        }
    ...
    public function excluir($o158_sequencial = null, $dbwhere = null)
    {
        ...
        $sql = " DELETE FROM orcparamseqcolunaestruturais
                    WHERE ";
        $sql2 = "";
        if (empty($dbwhere)) {
            if (!empty($o158_sequencial)) {
    ...
    public function sql_query($o158_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        $sql = "select {$campos}";
        $sql .= "  from orcparamseqcolunaestruturais ";
        $sql .= "      inner join conplano  on  conplano.c60_anousu = orcparamseqcolunaestruturais.o158_ano";
        $sql .= "      inner join orcparamseqcoluna  on  orcparamseqcoluna.o115_sequencial = orcparamseqcolunaestruturais.o158_orcparamseqcoluna";
