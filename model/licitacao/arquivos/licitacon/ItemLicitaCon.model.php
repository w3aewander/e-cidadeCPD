...
class ItemLicitaCon extends ArquivoLicitaCon
{
    ...
    public function getDados()
    {
        ...
        $sWhere = implode(' and ', LicitacaoLicitaCon::getWhereLicitacao($this->oCabecalho->getInstituicao(),
        ...
        for ($i = 0; $i < $iTotalRegistros; $i++) {
            ...
            $oValoresEstimadoHomologado = $this->oRegra->getValoresEstimadoHomologado(
              $oStdItem->l21_codigo,
              $oStdItem->z01_numcgm,
              $oStdItem->pc23_orcamitem
            );

            $sValorUnitarioEstimado = '0,00';
            if ($oValoresEstimadoHomologado->unitario_estimado) {
              $fValorUnitarioEstimado = floatval(str_replace(',', '.', $oValoresEstimadoHomologado->unitario_estimado));
              $sValorUnitarioEstimado = number_format($fValorUnitarioEstimado, 4, ',', '');
            }
            $oDadosOSE = $this->oRegra->getDadosOSE(
              $oStdItem->l21_codigo,
              $oAtributosDinamicos->getAtributo('tipoobjeto', null)
            );

            $totalHomologado = '0,00';
            ...
            if ($oLicitacao->getFase() == \EventoLicitacao::FASE_ADJUDICACAO_HOMOLOGACAO &&
                ...
                $valorHomologado = $oStdItem->pc23_vlrun ? number_format($oStdItem->pc23_vlrun, 4, ',', '') : '';
            }

            $oDbiEngargos = $this->oRegra->getDBIEncargos(
              $oStdItem->pc23_bdi,
              $oStdItem->pc23_encargossociais,
              $oAtributosDinamicos->getAtributo('tipoobjeto', null)
            );
            ...
            $oDados->NR_ITEM_ORIGINAL = $oStdItem->l21_ordem;
            $oDados->DS_ITEM = $descricaoItem;
            $oDados->QT_ITENS = number_format($oStdItem->pc11_quant, 2, ',', '');
            $oDados->SG_UNIDADE_MEDIDA = $this->oRegra->getSiglaUnidadeMedida(
              $oStdItem->pc17_codigo,
              $oStdItem->m61_codigotribunal
            );
            $oDados->VL_UNITARIO_ESTIMADO = $sValorUnitarioEstimado;
            ...
            $oDados->TP_RESULTADO_ITEM = $this->oRegra->getResultadoItem(
              $oStdItem->z01_numcgm,
              $oStdItem->pc23_orcamitem
            );
            ...
            $oDados->VL_UNITARIO_HOMOLOGADO = $valorHomologado;
            $oDados->VL_TOTAL_HOMOLOGADO = $totalHomologado;
    ...
    {
        if ($oStdItem->l20_licsituacao == SituacaoLicitacao::SITUACAO_EM_ANDAMENTO ||
          $oStdItem->l20_licsituacao == SituacaoLicitacao::SITUACAO_ANULADA ||
          $oStdItem->l20_licsituacao == SituacaoLicitacao::SITUACAO_DESERTA ||
          $oStdItem->l20_licsituacao == SituacaoLicitacao::SITUACAO_FRACASSADA ||
          $oStdItem->l20_licsituacao == SituacaoLicitacao::SITUACAO_REVOGADA ) {
            return false;
        }
