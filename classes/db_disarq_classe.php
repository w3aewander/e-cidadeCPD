...
class cl_disarq {
  ...
  function sql_query_baixa_banco ($codret=null,$campos="*",$ordem=null,$dbwhere=""){
    ...
    public function sqlCodclaByCodretAndInstit($codret, $instit, $campos = "*")
    {
        return "SELECT {$campos}
                  FROM disarq
                 INNER JOIN discla
                    ON discla.codret = disarq.codret
                 INNER JOIN saltes
                    ON saltes.k13_conta = disarq.k00_conta
                 WHERE dtaute IS NULL
                   AND discla.instit = {$instit}
                   AND discla.codret = {$codret}
                 ORDER BY codcla ASC";
    }
}
?>
