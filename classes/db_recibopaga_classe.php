...
class cl_recibopaga {
   ...
   public function alterar ( $oid=null, $dbwhere=null) {
      $this->atualizacampos();
   ...
   // funcao do sql
   public function sql_query ( $oid = null,$campos="recibopaga.oid,*",$ordem=null,$dbwhere=""){
     $sql = "select ";
     if($campos != "*" ){
   ...
   // funcao do sql
   public function sql_query_file ( $oid = null,$campos="*",$ordem=null,$dbwhere="", $groupby = []){
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = explode("#",$campos);
       $virgula = "";
   ...
   public function sql_query_dadosRecibo($iNumpreRecibo) {
  ...
  public function sql_query_descontoConced_cotaUnica($iRegra = 0, $iAno = null, $dDataInicial = null, $dDataFinal = null) {
    ...
    $sSql .= "                   minhist                                                                                 ";
    $sSql .= "UNION ALL
        SELECT iptutaxanump.j151_matric as matricula,
            iptutaxanump.j151_numpre as numpre,
            arrepaga.k00_receit as receita,
            tabrec.k02_descr as descricao,
            iptutaxacalv.j152_valor as vlrcalculado,
            sum(arrepaga.k00_valor) as vlrpago,
            coalesce(
                sum(
                ...
                0
            ) as vlrdescontoabatimento,
            sum(
                case when arrepaga.k00_hist not in (990, 918) then 0 else abs(arrepaga.k00_valor) end
            ) as vlrdesconto,
            (
                select
                    *
                from fc_consultadescontounica(iptutaxanump.j151_numpre::integer)
            ) as qtd_exists,
            (
                select
                    min(k00_hist)
                from
                    arrepaga as x
                where
                    x.k00_numpre = arrepaga.k00_numpre
                    and x.k00_hist in (990, 918)
            ) as minhist
          FROM iptutaxacalv
          ...
          inner join tabrec on tabrec.k02_codigo = iptutaxacalv.j152_receit
            inner join arrepaga on arrepaga.k00_numpre = iptutaxanump.j151_numpre
            and arrepaga.k00_receit = iptutaxacalv.j152_receit
            left join arrecad on arrecad.k00_numpre = iptutaxanump.j151_numpre
            left join arreckey on arreckey.k00_numpre = arrepaga.k00_numpre
            and arreckey.k00_numpar = arrepaga.k00_numpar
            and arreckey.k00_receit = arrepaga.k00_receit
            and arreckey.k00_hist = arrepaga.k00_hist
            left join abatimentoarreckey on abatimentoarreckey.k128_arreckey = arreckey.k00_sequencial
            left join abatimento on abatimento.k125_sequencial = abatimentoarreckey.k128_abatimento
            and abatimento.k125_tipoabatimento = 2
          WHERE j08_anousu = {$iAno}
          and arrepaga.k00_dtpaga between '{$dDataInicial}' and '{$dDataFinal}'
        group by
            matricula,
            numpre,
            vlrcalculado,
  ...
  public function sql_query_cgm($sCampos, $iNumpre) {
   ...
   return $sSqlCGM;
     ...
     * @param  integer $codOrigem
     * @return string  retorna a query montada
     */
    public function  sql_query_cgm_webservice_caixa($regra, $regraCgmIptu, $tipoOrigem, $codOrigem)
    {
        $campos = '*';
        $from = "fc_busca_envolvidos($regra, $regraCgmIptu, '$tipoOrigem', $codOrigem) AS principal ";
        $where = 'true';
        $order = '1';

        switch ($tipoOrigem) {
            case 'I':
                $joinProminente = "LEFT JOIN promitente ON
                    promitente.j41_numcgm = principal.rinumcgm  AND  promitente.j41_matric = $codOrigem";
                if (!$regraCgmIptu) {
                    $joinProminente .= "AND j41_tipopro = 't' ";
                    $order = "j41_tipopro";
                }
                $from .= $joinProminente;
                $from .= "LEFT JOIN proprietario ON
                    z01_numcgm = principal.rinumcgm ";
                $from .= "INNER JOIN issbase ON
                    q02_inscr = {$codOrigem} and q02_numcgm = principal.rinumcgm ";
                break;
            case 'M':
            default:
                $joinProminente = "LEFT JOIN promitente ON
                ...
                $from .= $joinProminente;
                $from .= "LEFT JOIN proprietario ON
                    j01_matric = {$codOrigem} and z01_numcgm = principal.rinumcgm ";
                break;
        }
  ...
  public function sql_query_info_cgm($iNumero)
  {
   ...
   public function sqlTaxaEspecifica($campos = array(), $where = array())
   {
       ...
       $sql = "select {$campos} ";
       $sql .= " from recibopaga ";
       $sql .= "      inner join arrecad  on arrecad.k00_numpre = recibopaga.k00_numpre ";
       $sql .= "                         and arrecad.k00_numpar = recibopaga.k00_numpar ";
       $sql .= "                         and arrecad.k00_receit = recibopaga.k00_receit ";
