...
function verificaEvasao($codaluno){
    //UPDATE progressaoparcialalunomatricula SET ed150_encerrado = 'f' WHERE ed150_sequencial = 1241;
    $sql1 = pg_query("SELECT ed114_sequencial from progressaoparcialaluno left join progressaoparcialalunomatricula on ed114_sequencial = ed150_progressaoparcialaluno left join progressaoparcialalunoresultadofinal on ed150_sequencial = ed121_progressaoparcialalunomatricula where ed114_aluno = {$codaluno} and (ed150_encerrado is false or ed150_sequencial is null)");
    $resultado1 = pg_fetch_all($sql1);
    $seq1 = $resultado1[0]["ed114_sequencial"]; //>>sequencial 1047
    
    $sql2 = pg_query("SELECT ed115_sequencial, ed150_encerrado from progressaoparcialalunoturmaregencia inner join progressaoparcialalunomatricula on progressaoparcialalunomatricula.ed150_sequencial = progressaoparcialalunoturmaregencia.ed115_progressaoparcialalunomatricula where ed150_progressaoparcialaluno = {$seq1} order by ed115_sequencial desc limit 1");
    $resultado2 = pg_fetch_all($sql2);
    $seq2 = $resultado2[0]["ed115_sequencial"]; //1241

    
    $sql3 = pg_query("SELECT * FROM plugins.diarioprogressao WHERE ed993_progressaoparcialalunoturmaregencia = {$seq2}");
    $resultado3 = pg_fetch_all($sql3);
    

    if($resultado3[0]["ed993_evadido"] == "t"){
        return true;
    }
    return false;
...
/*
    ...
    [iCodigoTurma] => 3323
    [iEtapa] => 4
    [iMostrarTrocaTurma] => 1
    [lTrocaTurma] => 
    [exec] => verificaTurmaSemAulasDadas
)
...
switch($oParam->exec) {
    ...
    case 'getAlunosMatriculados':
        ...
        //$aAlunosMatriculadosTurma = array_slice($aAlunosMatriculadosTurma, 0, 1); //Tudo normal nesse aqui
        //$aAlunosMatriculadosTurma = array_slice($aAlunosMatriculadosTurma, 0, 15);
        //testa($aAlunosMatriculadosTurma); die("Confere II");
        ...
        foreach ($aAlunosMatriculadosTurma as $oMatricula) {
            ...
            //Buscamos o resultado final do diario da matricula, e apresentamos o termo correto de acordo com o cadastrado para o ensino, caso exista um resultado final
            
            db_inicio_transacao();

            
             //Caso a matricula esteja com situacao diferente de matriculado (Ex.: Transferido Rede), atribuimos o valor da situacao para ser apresentado no resultado final, para que nao fique em branco
            ...
            if ($oMatricula->getSituacao() != 'MATRICULADO') {
                ...
                $oDadosAluno->sResultadoFinal = $resultadoFinal;
                $anoCalendario = $oMatricula->getTurma()->getCalendario()->getAnoExecucao();
                
                //Buscamos o termo correto para o resultado final

                if (!empty($oDadosAluno->sResultadoFinal)) {
                    if (is_null($areaProcedimento) && $lPermiteAprovacaoParcial && $oDadosAluno->sResultadoFinal == 'A' &&
                        EncerramentoAvaliacao::validaDiarioAlunoEja($oMatricula, $oEtapa) == "P") {
                        $oDadosAluno->sResultadoFinal = 'P';
                    }
                    $aTermosEncerramento = DBEducacaoTermo::getTermoEncerramento(
                        $iCodigoEnsino,
                    ...
                    /*if($oDadosAluno->iCodigoAluno == 115875){
                        $xevadido = verificaEvasao($oDadosAluno->iCodigoAluno);
                        var_dump($xevadido);
                        die("Confere");
                    }*/
                    ...
                    /*if($oDadosAluno->iCodigoAluno == 115875){
                        $xevadido = verificaEvasao($oDadosAluno->iCodigoAluno);
                        var_dump($xevadido); die("Chegurei");
                    }*/
                    //$xevadido = verificaEvasao($oDadosAluno->iCodigoAluno);
                    if($xevadido){
                        $oDadosAluno->sResultadoFinal = urlencode("REPROVADO");
                    }

                    /*if($oDadosAluno->iCodigoAluno == 135256 && $oParam->iCodigoTurma == 2469){
                        $oDadosAluno->sResultadoFinal = urlencode(" {$sLabelAprovado} (Progressão Parcial / Dependência)");
                    }*/
            ...
            //echo $oDadosAluno->sNomeAluno; echo "<br>";

            db_fim_transacao(false);
            $oRetorno->aDadosAlunos[] = $oDadosAluno;
        }
        //echo "Passou aqui"; echo "<br>"; die("Confere");
        break;
    ...
    case 'getAlunosDeProgressao':
        ...
        foreach ($aAlunosDeProgressao as $oAlunoProgressao) {
            ...
            //Buscamos o termo correto para o resultado final
            //if($oDadosAlunoProgressao->iCodigoAluno == 135169){ //425
            /*if($oDadosAlunoProgressao->iCodigoAluno == 120467){ //414
                var_dump($oDadosAlunoProgressao->sResultadoFinal);
                die("MOsra");
            }*/
            ...
            try {
                db_inicio_transacao();
                $oDiarioProgressao = new DiarioProgressaoParcial( $oAlunoProgressao );
                $oDadosAlunoProgressao->lEvadido = $oDiarioProgressao->isEvadido();
                db_fim_transacao();
