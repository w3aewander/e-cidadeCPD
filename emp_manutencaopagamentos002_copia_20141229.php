...
if ( db_getsession("DB_id_usuario") == 44 or db_getsession("DB_id_usuario") == 91 ) {
  ...
  $oParam->dtDataFim    = "";
  $oParam->iNumCgm      = "";
  $oParam->iRecurso     = "100,23,28,40,08";
  $oParam->sDtAut       = "19/09/2014";
  $oParam->iOPauxiliar  = "";
  $oParam->iAutorizadas = "1";
  $oParam->lAtualizadas = false;
  $oParam->lNormais     = true;
  $oParam->lChequeArq   = false;
  $oParam->orderBy      = "cgm.z01_nome";
...
if ($oParam->iRecurso != '') {
  ...
  $sWhere .= " and o15_codigo in ( {$oParam->iRecurso} )";
}
       
$orderBy = $oParam->orderBy;

$oAgenda->setOrdemConsultas($oParam->orderBy);
...
$sCampos  = ",k13_conta,k13_descr, e96_descr,pc63_banco, pc63_agencia,pc63_agencia_dig,pc63_conta,pc63_conta_dig,e150_numeroprocesso,e03_numeroprocesso";
...
foreach ($aOrdensAgenda as $oMovimento) {
  $iRecursoQuebra = $oMovimento->o15_codigo;
  $nTotalQuebraOP = 0;
  $nTotalQuebraAut = 0;
  $nTotalQuebraRetencao = 0;
  $nTotalQuebraValor = 0;
  $iTotalQuebraLinhas = 0;
  break;
}
...
foreach ($aOrdensAgenda as $oMovimento) {
 ...
 if ( $iRecursoQuebra != $oMovimento->o15_codigo and ( db_getsession("DB_id_usuario") == 44 or db_getsession("DB_id_usuario") == 91 ) ) {

   $oPdf->cell(178,5,"Total de Registros: $iTotalQuebraLinhas ", "TBR",0,"R",0);
   $oPdf->cell(20,5,"Totais","TBR",0,"R",0);
   $oPdf->cell(20,5, trim(db_formatar($nTotalQuebraOP,"f")), "TBR", 0, "R");
   $oPdf->cell(20,5, trim(db_formatar($nTotalQuebraAut,"f")), "TBR", 0, "R");
   $oPdf->cell(20,5, trim(db_formatar($nTotalQuebraRetencao,"f")), "TBR", 0, "R");
   $oPdf->cell(20,5, trim(db_formatar($nTotalQuebraValor,"f")),"TBL",1,"R");
   $oPdf->ln();
 ...
 if ($oPdf->Gety() > $oPdf->h - 25 || $lEscreverHeader) {
  ...
  $oPdf->cell(13,5,$oMovimento->e03_numeroprocesso,"TBR",0,"R",0);
  ...
  $nTotalQuebraOP += $oMovimento->e53_valor;
  $nTotalQuebraAut += $oMovimento->e81_valor;
  $nTotalQuebraRetencao += $oMovimento->valorretencao;
  $nTotalQuebraValor += ($oMovimento->e81_valor - $oMovimento->valorretencao);
  $iTotalQuebraLinhas++;
