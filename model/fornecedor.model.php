...
class fornecedor
{
        ...
        if ($this->lBloqueado && $this->iControleFornecedor != 1) {
            ...
            if (!$lLiberacao && $tipoOrigem != 'A') {
                ...
                if ($iRowsLiberacaoGeral > 0) {
                    ...
                    $dtDataAtual = db_getsession('DB_datausu');
                    $dtAtual = mktime(0, 0, 0, date('m', $dtDataAtual), date('d', $dtDataAtual),
                        date('Y', $dtDataAtual));
    ...
    /**
     * REtorna a conta bancaria na contabilidade do fornecedor
     * @param $fornecedor
     * @param $instituicao
     * @return null|int
     */
    public static function getContaContabilNaInstituicao($fornecedor, $instituicao, $ano)
    {

        $sqlConta = "select c61_reduz";
        $sqlConta .= "  from pcfornecon";
        $sqlConta .= "      left join contabancaria  on db83_conta = pc63_conta";
        $sqlConta .= "                              and db83_dvconta = pc63_conta_dig";
        $sqlConta .= "      inner join bancoagencia  on db89_codagencia = pc63_agencia";
        $sqlConta .= "                              and db89_digito = pc63_agencia_dig";
        $sqlConta .= "                              and db89_sequencial = db83_bancoagencia";
        $sqlConta .= "                              and pc63_banco = db89_db_bancos";
        $sqlConta .= "      left join conplanocontabancaria on c56_contabancaria =  db83_sequencial";
        $sqlConta .= "                                     and c56_anousu = {$ano}";
        $sqlConta .= "      left join conplanoreduz on c61_codcon = c56_codcon";
        $sqlConta .= "                             and c61_anousu = c56_anousu";
        $sqlConta .= " where pc63_numcgm = {$fornecedor}";
        $sqlConta .= "   and c61_instit = {$instituicao}";

        $rsContaFornecedor = db_query($sqlConta);
        if (pg_num_rows($rsContaFornecedor) === 0) {
            return null;
        }

        return db_utils::fieldsMemory($rsContaFornecedor, 0)->c61_reduz;
    }
