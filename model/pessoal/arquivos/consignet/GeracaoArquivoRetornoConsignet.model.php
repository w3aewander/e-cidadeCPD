...
class GeracaoArquivoRetornoConsignet {
  ...
  private function getRegistros() {
    ...
    foreach ( $aRegistros as $oRegistro ) {
      ...
      $oCalculoSalario                    = $oRegistro->getServidor()->getCalculoFinanceiro(CalculoFolha::CALCULO_SALARIO);
      $oCalculoComplementar               = $oRegistro->getServidor()->getCalculoFinanceiro(CalculoFolha::CALCULO_COMPLEMENTAR);
      $oCalculoRescisao                   = $oRegistro->getServidor()->getCalculoFinanceiro(CalculoFolha::CALCULO_RESCISAO);

      $aEventosSalario                    = $oCalculoSalario->getEventosFinanceiros(null, $oRegistro->getRubric());
      $aEventosComplementar               = $oCalculoComplementar->getEventosFinanceiros(null, $oRegistro->getRubric());
      $aEventosRescisao                   = $oCalculoRescisao->getEventosFinanceiros(null, $oRegistro->getRubric());
			
      /**
       *  Retornando o valor que estava no ponto
       */
      $nValor  = $this->valorEventoFolha($aEventosSalario);
      $nValor += $this->valorEventoFolha($aEventosComplementar);
      $nValor += $this->valorEventoFolha($aEventosRescisao);
  ...
  private function valorEventoFolha(Array $eventoFolha){

    $eventoExists = count($eventoFolha) > 0;
    return $eventoExists ? $eventoFolha[0]->getValor() : 0;
