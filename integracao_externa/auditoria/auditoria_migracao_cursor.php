...
$sSql  = "SELECT sequencial, observacoes, id_acount_ini, id_acount_fim, datahora_ini, datahora_fim, instit ";
...
if ($iNumrowsverifica>0) {
	...
	for ($i=0; $i<$iNumrowsverifica; $i++) {
		...
		if ($iTentativas <= $iMaxReTentativas) {
			db_log("> limpando logs de auditoria migrados parcialmente", $sArquivoLog, 0, true, true);
			$sSql  = "DELETE ";
			$sSql .= "  FROM db_auditoria ";
			$sSql .= " WHERE datahora_servidor BETWEEN '{$oMigracao->datahora_ini}'::timestamptz AND '{$oMigracao->datahora_fim}'::timestamptz ";
			$sSql .= "   AND instit = ANY('{$oMigracao->instit}'::integer[]) ";
			$sSql .= "   AND sequencial BETWEEN {$oMigracao->id_acount_ini} AND {$oMigracao->id_acount_fim} ";
...
for ($i=0; $i<$iNumrows; $i++) {
	...
	$sSql   = "SELECT (pg_try_advisory_xact_lock({$oMigracao->sequencial}) IS TRUE) as bloqueou ";
	...
	db_log("Retornando dados do cursor...", $sArquivoLog);
	echo "\n";
	$iRegProc = 0;
	for ($w=0; $w<$iCount; $w++) {
		...
		if($iRows > 0) {
			...
			if (!isset($aParticoes[$sParticao])) {
				if ($lDeleteAfter or $lForceCheckPartition) {
					...
					$sSql .= "     'configuracoes', ";
					$sSql .= "     '{$sParticao}', ";
					$sSql .= "     'datahora_servidor >= \'{$oAcount->datahora_ini}\' AND datahora_servidor < \'{$oAcount->datahora_fim}\' AND instit = {$oAcount->instit}' ";
					$sSql .= "   ); ";
					db_query($pConexao, $sSql, $sArquivoLog);
			...
			$aParticoes[$sParticao]->gatilho           = 'f';
