...
class PessoasLicitaCon extends ArquivoLicitaCon {
    ...
    public function getDados() {
        ...
        $sqlAlteracaoContratado = $daoAlteracaoContratado->sql_query_file('', 'ac60_anterior');
        ...
        $sWhereCgm   .= " or z01_numcgm in ({$sqlAlteracaoContratado}) ";
        ...
        for ($i = 0; $i < pg_num_rows($rsCgm); $i++) {
            ...
            $cgm = CgmRepository::getByCodigo($oStdClass->z01_numcgm);
            $sBairro = null;
            if ($cgm->isFisico()) {

                $oStdClass->z01_uf = $cgm->estrangeiro() ? 'EX' : $oStdClass->z01_uf;
                $oStdClass->db125_codigosistema = !$cgm->estrangeiro() ? $oStdClass->db125_codigosistema : '';
                $sBairro = $cgm->estrangeiro() ? "{$cgm->getCidadeEstrangeiro()}/{$cgm->getPaisEstrangeiro()}" : '';
            }
            ...
            $oDados->TP_PESSOA    = LicitanteLicitaCon::getTipoPessoaPorCGM($oStdClass->z01_numcgm);
