...
if (!isset($paga_13)) {
  $result_cfpess = $clcfpess->sql_record($clcfpess->sql_query_file(\DBPessoal::getAnoFolha(), \DBPessoal::getMesFolha(), db_getsession("DB_instit"), "r11_pagarferias as ponto,r11_13ferias as paga_13"));
...
// Se o servidor tiver falta dentro do período aquisitivo, o sistema já traz a falta automaticamente
if (
  !empty($r30_perai_ano) && !empty($r30_perai_mes) && !empty($r30_perai_dia) &&
  !empty($r30_peraf_ano) && !empty($r30_peraf_mes) && !empty($r30_peraf_dia)
) {

  $dataInicio = "{$r30_perai_ano}-{$r30_perai_mes}-{$r30_perai_dia}";
  $dataFim    = "{$r30_peraf_ano}-{$r30_peraf_mes}-{$r30_peraf_dia}";

  // Garantindo formato de data
  $dataInicioPerAquisitivo = date('Y-m-d', strtotime($dataInicio));
  $dataFimPerAquisitivo    = date('Y-m-d', strtotime($dataFim));

  $sql = " SELECT 
        COALESCE(SUM(h16_quant), 0) AS total_faltas
    FROM 
        assenta
    INNER JOIN 
        tipoasse ON h16_assent = h12_codigo
    WHERE 
        h16_regist = $1
        AND h12_gerafaltasperiodoaquisitivo = 't'
        AND h16_dtterm >= $2
        AND h16_dtconc <= $3
  ";

  $params = [$r30_regist, $dataInicioPerAquisitivo, $dataFimPerAquisitivo];
  $result_faltas = db_query_params($sql, $params);

  if (pg_num_rows($result_faltas) > 0) {
    $faltas = db_utils::fieldsMemory($result_faltas, 0);
    $r30_faltas = $faltas->total_faltas;
  } else {
    $r30_faltas = 0;
  }
}

if (!isset($_POST['r30_faltas']) || $_POST['r30_faltas'] === '') {

  $oServidor = new Servidor($r30_regist);

  try {
    $r30_ndias = PeriodoAquisitivoFerias::calculaDiasDireito($oServidor, $r30_faltas);
  } catch (Exception $e) {
    db_redireciona("db_erros.php?fechar=true&db_erro=" . urlencode("Erro ao calcular dias de direito: " . $e->getMessage()));
    exit;
  }

  // Atualiza o campo r30_ndias visualmente no formulário
  echo "<script>
      document.form1.r30_ndias.value = '{$r30_ndias}';
      js_validamtipo(true); 
    </script>";
}
...
<form name="form1" id='form1' method="post" action="pes4_cadferia004.php">
  <center>
    <input type="hidden" value="<?php echo $iPeriodoaquisitivo ?>" id="periodoaquisitivo" name='periodoaquisitivo'>
    <input type="hidden" value="<?php echo $sPeriodosvencidosate ?>" id="periodosvencidosate" name='periodosvencidosate'>
    ...
    <table border="0">
      <tr>
        <td align="center">
          <hr>
          <b><?php echo $r30_regist . " - " . $z01_nome ?></b>
          <hr>
        </td>
      </tr>
      <tr>
            ...
            <table>
              <tr>
                <td style="text-align: right;">
                  <?php echo $Lr30_tipoapuracaomedia ?>
                </td>
                <td>
                  <?php
              ...
              <tr>
                <td nowrap title="<?php echo @$Tr30_perai ?>" align="right">
                  <?php
              ...
              <tr>
                <td nowrap title="<?php echo @$Tr30_faltas ?>" align="right">
                  <?php
                ...
                <td nowrap title="<?php echo @$Tr30_ndias ?>" align="right">
                  <?php
                ...
                <td>
                  <?php
                  if (!isset($r30_ndias) || trim((string)$r30_ndias) === '') {
              ...
              ?>
                <tr>
                  <td nowrap title="Forma de pgto" align="right">
                    <?php
                    db_ancora("<b>Forma de Pagamento:</b>", "", 3);
                    ?>
                  </td>
                  <td colspan="3">
                    <?php
              ...
              ?>
              <tr>
                <td nowrap title="<?php echo @$Tr30_abono ?>" align="right">
                  <?php
                ...
                <td nowrap title="<?php echo @$Tr30_proc1 ?>" align="right">
                  <?php
              ...
              <tr>
                ...
                <td>
                  <?php
                  db_input('nsaldo', 7, $Ir30_ndias, true, 'text', $db_opcao, 'onchange="js_limitaDiasGozo(); js_verificadataini(1);"');
                  ?>
                </td>
        ...
        <td>
          ...
          <tr>
            ...
            <td>
              <?php
              if (!isset($anopagto)) {
                $anopagto = \DBPessoal::getAnoFolha();
              }
              if (!isset($mespagto)) {
                $mespagto = \DBPessoal::getMesFolha();
              }
  ...
  <input name="enviar" type="button" id="db_opcao" value="Processar dados" <?php echo ($dbopcao ? 3 : 1) == 1 ? ($db_opcao == 3 ? 'disabled' : '') : 'disabled' ?> onclick="<?php echo ($db_opcao == 3) ? 'return false;' : 'js_verificadados(false);' ?>">
  <?php if (db_getsession("DB_id_usuario") == 1) { ?>
    <input name="enviar" type="button" id="db_opcao" value="Processar dados com Debug" <?php echo ($dbopcao ? 3 : 1) == 1 ? ($db_opcao == 3 ? 'disabled' : ' ') : 'disabled' ?> onclick="js_verificadados(true);">
  <?php }
...
<script>
  ...
  function js_verificadados(debug) {
    ...
    ?>
      if (document.form1.mtipo.value == "09") {
        ...
        diaci = new Date(<?php echo \DBPessoal::getAnoFolha() ?>, (<?php echo \DBPessoal::getMesFolha() ?> - 1), 1);
        diacf = new Date(<?php echo \DBPessoal::getAnoFolha() ?>, (<?php echo \DBPessoal::getMesFolha() ?> - 1), <?php echo db_dias_mes(\DBPessoal::getAnoFolha(), \DBPessoal::getMesFolha()) ?> + 180);

        if (per2i >= diaci && per2f <= diacf && per2f > per2i) {} else {
    ...
    if (document.form1.anopagto) {
      if (document.form1.anopagto.value < <?php echo \DBPessoal::getAnoFolha() ?>) {
    ...
    if (erro == 0) {
      ...
      if (erro == 0) {
        ...
        if (anomes < <?php echo \DBPessoal::getAnoFolha() . \DBPessoal::getMesFolha() ?>) {
  ...
  function js_faltas(opcao, perai, peraf, antes, nfalt, navos, debug) {
    ...
    qry += '&peraf=' + peraf;
    qry += '&antes=' + antes;
    qry += '&registro=<?php echo @$r30_regist ?>';
    qry += '&nfalt=' + nfalt;
    qry += '&navos=' + navos;
  ...
  function js_verificaaquiini() {
    ...
    if (ano > <?php echo (\DBPessoal::getAnoFolha() + 1) ?>) {
      ...
      diacf = new Date(<?php echo \DBPessoal::getAnoFolha() ?>, (<?php echo \DBPessoal::getMesFolha() ?> - 1), 1);
  ...
  function js_verificadatafim(campo) {
    ...
    if (evaldiai.value != "" && evalmesi.value != "" && evalanoi.value != "") {
      if (evaldiaf.value != "" && evalmesf.value != "" && evalanof.value != "") {
        ...
        qualmess = new Number(<?php echo \DBPessoal::getMesFolha() ?>);
        qualmess -= new Number(1);

        qualdias = new Number(<?php echo db_dias_mes(\DBPessoal::getAnoFolha(), \DBPessoal::getMesFolha()) ?>);
        qualdias += new Number(180);

        diaci = new Date(<?php echo \DBPessoal::getAnoFolha() ?>, qualmess, 1);
        diacf = new Date(<?php echo \DBPessoal::getAnoFolha() ?>, qualmess, qualdias);
  ...
  function js_habilitaperiodo(opcao) {
    if (document.form1.saldo.selectedIndex == 0) {
      ...
      document.form1.nsaldo.value = "<?php echo @$nsaldo ?>";
  ...
  function js_calcFim() {
    ...
    if (evaldiai.value != "" && evalmesi.value != "" && evalanoi.value != "") {
      ...
      //se o ano posterior for bissexto e mes mair que 02 soma mais um dia para fechar o cálculo de ferias
  ...
  function js_calcIni() {
    ...
    if (evaldiaf.value != "" && evalmesf.value != "" && evalanof.value != "") {
      ...
      //se ano atual bissesto e mes maior que 2 diminui um dia para fechar calculo
  ...
  // O sistema já calcula o período de gozo de direito
  function js_limitaDiasGozo() {

    var diasDireito = parseInt(document.form1.r30_ndias.value) || 0;
    var diasGozo = parseInt(document.form1.nsaldo.value) || 0;

    if (diasGozo > diasDireito) {
      alert("Quantidade de dias a gozar não pode ser maior que os dias de direito (" + diasDireito + ") dias.");
      document.form1.nsaldo.value = diasDireito;
    }
  }
