...
class FolhaPagamento13o extends FolhaPagamento {
  ...
  public function fechar() {
   ...
   /**
    * Realiza o fechamento do decimo13, quando utiliza suplementar
    *
    * @param $oCompetencia
    */
  public function fechaDecimo13Anteriores($oCompetencia)
  {
      $filter = "WHERE  rh141_tipofolha = %d AND  rh141_aberto = %s AND  ((rh141_anousu = %s AND rh141_mesusu <= %s) OR (rh141_anousu <= %s ))";
      $where  = sprintf($filter, self::TIPO_FOLHA_13o_SALARIO, 'true', $oCompetencia->getAno(), $oCompetencia->getMes(), $oCompetencia->getAno());
    ...
    /**
     * Realiza o cancelamento fechamento do decimo13, quando utiliza suplementar
     *
     * @param $oCompetencia
     */
  public function cancelarFechamentoDecimo13Anteriores($oCompetencia)
  {
      $filter = "WHERE  rh141_tipofolha = %d AND  rh141_aberto = %s AND  rh141_anousu = %s AND rh141_mesusu = %s";

      $where  = sprintf($filter, self::TIPO_FOLHA_13o_SALARIO, 'false', $oCompetencia->getAno() , $oCompetencia->getMes());
    ...
    if (FolhaPagamentoSalario::hasFolha()) {
      ...
      $oCompetencia = DBPessoal::getCompetenciaFolha();
        ...
        if (!FolhaPagamento13o::hasFolha()) {
          ...
          $oFolha13oSalario->setDescricao("Folha 13 salario - {$oCompetencia->getAno()}/{$oCompetencia->getMes()}");
