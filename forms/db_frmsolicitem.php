...
$tipoSolicitacao       = !empty($tipoSolicitacao) ? $tipoSolicitacao : '';
...
echo "
     <script>
       function js_passaparam(opcao){
				 ...
				 qry += '&tipoSolicitacao=$tipoSolicitacao';
				 qry += opc;
...
<form name="form1" method="post" action="" onsubmit=" return js_validarFormulario(); ">
  <center>
    <table border="0" height="100%">
      ...
      <tr>
        ...
        if(isParaiba()){
            db_input('pc11_vlrun', 8, $Ipc11_vlrun, true, $hidval, $db_opcaovunit,"onkeypress='js_verificaDecimal(event)'");
        }else{
  ...
  <center>
        ...
        <td valign="top" align="center" width="100%">
          ...
          $sql_dot = $clsolicitem->sql_query_dot(null,
                                                 ...
                                                 "group by pc10_numero,
                    							                                   ...
                    							                                   pc11_numero,
                    							                                   pc13_codigo
                    						                              having (round(pc11_quant, 3) > round(sum(pc13_quant), 3) or pc13_codigo is null)
          ...
          $cliframe_alterar_excluir->sql              = $clsolicitem->sql_query_pcmater(null,
                                                                                        ...
                                                                                        "$codigos pc11_numero= "
                                                                                        . @$pc11_numero);
          $cliframe_alterar_excluir->campos           = "pc11_seq,pc11_codigo,pc11_numero,pc13_coddot,pc19_orctiporec,pc01_codmater,pc01_descrmater,pc11_quant,pc11_vlrun";
    ...
    if ($iRegistroPreco) {
        $oRegistroPreco = new compilacaoRegistroPreco($iRegistroPreco);
        $formaCOntrole = $oRegistroPreco->getFormaDeControle();
    }
    $tipoSolicitacao = !empty($tipoSolicitacao) ? $tipoSolicitacao : '';
    if ($tipoSolicitacao == '') {
        $daoTipoSolicitacao = new cl_solicita();
        $sql = $daoTipoSolicitacao->sql_query(null, "pc10_solicitacaotipo", '', "pc11_numero = {$pc11_numero}");
        $rs = $daoTipoSolicitacao->sql_record($sql);

        if ($daoTipoSolicitacao->numrows > 0) {
            $tipoSolicitacao = \db_utils::fieldsMemory($rs, 0)->pc10_solicitacaotipo;
        }
    }
    ?>
  <input id="tipoSolicitacao" name="tipoSolicitacao" type="hidden" value="<?=$tipoSolicitacao;?>" />
  <input id="formaControle" name="formaControle" type="hidden" value="<?=$formaCOntrole;?>" />
  <?php
  $pc11_liberado = 'f';
...
<script>
  ...
  function js_pesquisave01_veiculo(mostra) {
    if (mostra == true) {
      js_OpenJanelaIframe('CurrentWindow.corpo.iframe_solicitem',
                          'db_iframe_veiculos',
                          'func_veiculos.php?funcao_js=parent.CurrentWindow.corpo.iframe_solicitem.js_mostraveiculo1|ve01_codigo|ve01_placa<?=$db_opcao == 1 ? "&opcao_bloq=3&opcao=f" : "&opcao_bloq=1&opcao=i"?>',
                          'Pesquisa',
                          true,
                          '0'
      );
      ...
      js_OpenJanelaIframe('CurrentWindow.corpo.iframe_solicitem',
                          ...
                          + '&funcao_js=parent.CurrentWindow.corpo.iframe_solicitem.js_mostraveiculo<?=$db_opcao == 1 ? "&opcao_bloq=3&opcao=f" : "&opcao_bloq=1&opcao=i"?>',
                          'Pesquisa',
                          false,
                          '0'
      );
  ...
  function js_verificaDecimal(event) {
      let valor = document.form1.pc11_vlrun.value;
      let [int = 0, decimal = 0] = valor.toString()
          .split('.')
      decimal += event.key;
      if(decimal.length > 2){
    ...
    if (mostra == true || iRegistroPrecoFuncao) {
      ...
      if (document.form1.pc16_codmater.value != '') {
        js_OpenJanelaIframe('CurrentWindow.corpo.iframe_solicitem',
                            ...
                            + '&iRegistroPreco=<?=$iRegistroPreco;?>'
                            + '&funcao_js=parent.CurrentWindow.corpo.iframe_solicitem.js_mostrapcmater<?=$db_opcao == 1 ? "&opcao_bloq=3&opcao=f" : "&opcao_bloq=1&opcao=i"?>',
                            'Pesquisa',
                            false,
                            '0'
        );
  ...
  function js_buscaDadosComplementaresMaterial(iCodigoMaterial) {
    ...
    new AjaxRequest("com4_materialsolicitacao.RPC.php", oRequest, function (oResponse, lError) {
        ...
        if (empty(document.form1.pc11_resum.value)) {
  ...
  function js_testaQuant(iOpcao) {
    ...
    var controlaQuantidade = document.getElementById('pc11_servicoquantidade').value;
    var nQuant = new Number(document.getElementById('pc17_quant').value);
    var qtdSolicitada = new Number(document.getElementById('pc11_quant').value);
    if (controlaQuantidade == 'true' && qtdSolicitada <= 1) {
        alert('Serviço controlado por quantidade, quantidade não pode ser igual ou inferior a 1!');
        return false;
    }
    if (nQuant <= 0) {
  ...
  function js_retornogetSaldo(oAjax) {
    ...
    if (oRetorno.status == 1) {
      alert(JSON.stringify(oRetorno.itens));
    }
    return true;
  }
  ...
  function js_mostraservico(chave) {
    ...
    let tipoSolicitacao = document.form1.tipoSolicitacao.value;
    let pc54_formaControle = document.form1.formaControle.value;

    if (tipoSolicitacao == 5 && pc54_formaControle == 2) {
    ...
    if (chave == "t" ) {
      ...
      document.form1.pc11_servicoquantidade.value = 'false';
      js_desabilidaqtd(document.form1.pc17_unid.value);
    }
    <?php
    if ($db_opcao == 3) {
