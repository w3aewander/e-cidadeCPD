...
class M13324BaixaUnica extends PostgresMigration
{
    public function up()
    {
        $sql = <<<SQL_UP
    ...
    loop
      ...
      perform *
         from recibopaga r
              ...
              inner join abatimento ab on ab.k125_sequencial = ak.k128_abatimento
        where k00_numnov    = r_idret.numpre
      ...
      then
        ...
        perform *
           from db_reciboweb
          where k99_numpre_n = r_idret.numpre
            and k99_numpar = 0;

        if found then
            select round(k00_valor * (100-k00_percdes) /100,2)
              into nVlrCalculado
              from arrecad
                   inner join recibounica on arrecad.k00_numpre = recibounica.k00_numpre
                   inner join db_reciboweb on k99_numpre = arrecad.k00_numpre
             where k99_numpre_n = r_idret.numpre;

             perform fc_debug('  <PgtoParcial>  - Tem desconto de Única. Atualizando valor calculado. nVlrCalculado: ' || nVlrCalculado, lRaise, false, false);
        end if;
