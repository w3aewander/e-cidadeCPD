...
for ($i=0, $a=1; $i <= $iTotalRegistros; $i++) {
  ...
  if($i==0 || $this->objpdf->GetY() > 184){
    ...
    $this->objpdf->rect($col+10,$lin+41,135,9);
    $this->objpdf->rect($col,$lin+41,145,9);
    ...
    $this->objpdf->text($col+56,$lin+43,'11 - Nome');
    $this->objpdf->text($col+146,$lin+43,'12 - Cargo');
    $this->objpdf->text($col+12,$lin+52,'13 - Endereço(logradouro, n°, andar, apartamento)');
    $this->objpdf->text($col+123,$lin+52,'14 - Bairro');
    $this->objpdf->text($col+12,$lin+61,'15 - Município');
    $this->objpdf->text($col+92,$lin+61,'16 - UF');
    $this->objpdf->text($col+107,$lin+61,'17 - CEP');
    $this->objpdf->text($col+137,$lin+61,'18 - Carteira de Trabalho(n°,série,UF');
    $this->objpdf->text($col+12,$lin+70,'19 - CPF');
    $this->objpdf->text($col+57,$lin+70,'20 - Data de Nascimento');
    $this->objpdf->text($col+92,$lin+70,'22 - Nome da Mãe');

    $this->objpdf->Setfont('Arial','',8);
    $this->objpdf->text($col+12,$lin+47,$this->pis);
    $this->objpdf->text($col+56,$lin+47,$this->nome);
    $this->objpdf->text($col+146,$lin+47,$this->cargo);
