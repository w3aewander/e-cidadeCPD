...
/**
 * Prove dados para a gera??o do arquivo dos pagamentos que possuiram movimentacao no periodo
...
final class PadArquivoSigapPagamento extends PadArquivoSigap
{
    ...
    public function gerarDados()
    {
        ...
        $sSqlPagamentos .= "       (select case when c53_tipo = 30 then c69_debito else c69_credito end ";
        $sSqlPagamentos .= "                  from contabilidade.conlancamval ";
        $sSqlPagamentos .= "                                   where c69_codlan = c70_codlan";
        $sSqlPagamentos .= "                                   order by c69_ordem";
        $sSqlPagamentos .= "                                   limit 1) as contrapartida, ";
        $sSqlPagamentos .= "       (select case when c53_tipo = 30 then c69_credito else c69_debito end ";
        $sSqlPagamentos .= "          from  contabilidade.conlancamval where c69_codlan = c70_codlan and c69_ordem = 1 limit 1)  as conta_pagadora,";
        $sSqlPagamentos .= "       db_config.codtrib as orgao_unidade, ";
        $sSqlPagamentos .= "       e60_instit, ";
        $sSqlPagamentos .= "       c80_codord ";
        $sSqlPagamentos .= "  from conlancamemp    ";
        $sSqlPagamentos .= "       inner join conlancam      on c70_codlan = c75_codlan ";
        $sSqlPagamentos .= "       left join conlancampag   on c82_codlan = c70_codlan ";
        ...
        $sSqlPagamentos .= "             join conlancamord   on c80_codlan = c70_codlan ";
        ...
        $sSqlPagamentos .= "   and e91_rpcorreto is false ";
        $sSqlPagamentos .= "   and e60_emiss <= '{$this->sDataFinal}' {$sWhere} ";
        ...
        for ($i = 0; $i < $iTotalLinhas; $i++) {
            ...
            if ($iTamanhoCampo != 4) {
                ...
                $sMsg .= "Solicitar para o setor respons?vel pela configura??o do sistema a altera??o da informa??o no Menu: \\n \\n ";
                $sMsg .= "Configura??o -> Cadastros -> Institui??es -> Altera??o. ";
