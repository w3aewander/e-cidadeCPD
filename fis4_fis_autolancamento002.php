...
include(modification("classes/db_fis_levantanotas_classe.php"));
include(modification("classes/db_fis_levvalor_classe.php"));
...
include(modification("classes/db_fis_levusu_classe.php"));
include(modification("classes/db_fis_parfiscal_classe.php"));
...
if ($numrowsinscr > 0) {
    ...
    $rsAtividadePrincipal = $clativprinc->sql_record($clativprinc->sql_query_compl($q02_inscr, "q03_descr"));
    db_fieldsmemory($rsAtividadePrincipal, 0);
    $mostraativ = 't';
}
$sqllevanta = "select case when fis_levinscr.y62_codlev is not null then
                               issbase.q02_inscr
                            else cgm.z01_numcgm
                        end as origem,
                        case when fis_levinscr.y62_codlev is not null then
                               'INSCR.'
                           else 'CGM'
                        end as tipo,
				       cgm.*,
		               ...
		               j14_nome||', '||q02_numero||' - '||q02_compl   as enderinscr,
                       j13_descr     as bairroinscr
			     from fiscalizacao.fis_levanta
				       left join fiscalizacao.fis_levcgm   on fis_levcgm.y93_codlev   = fis_levanta.y60_codlev
				       left join cgm      on cgm.z01_numcgm      = fis_levcgm.y93_numcgm
				       left join fiscalizacao.fis_levinscr on fis_levinscr.y62_codlev = fis_levanta.y60_codlev
				       left join issbase  on issbase.q02_inscr   = fis_levinscr.y62_inscr
				       left join cgm c on c.z01_numcgm = issbase.q02_numcgm
                                       ...
                                       left join issbairro on q13_inscr = issbase.q02_inscr
                                       left join bairro   on j13_codi = q13_bairro
				       left join tabativ  on tabativ.q07_inscr   = fis_levinscr.y62_inscr
...
$pdf->roundedrect(52, 42, 180, 35, 2, '1234');
...
$pdf->setfont('arial', 'b', 12);
$pdf->setx(55);
$pdf->cell(47, $alt + 1, "Bairro : ", 0, 0, "L", 0);
$pdf->setfont('arial', '', 12);
$pdf->cell(110, $alt + 1, ($z01_bairro == '' ? $bairroinscr : $z01_bairro), 0, 1, "L", 0);
